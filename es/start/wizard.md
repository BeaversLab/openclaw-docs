---
summary: "Onboarding de la CLI: configuración guiada para la puerta de enlace, el espacio de trabajo, los canales y las habilidades"
read_when:
  - Ejecutar o configurar el onboarding de la CLI
...
El onboarding de la CLI es la forma **recomendada** de configurar OpenClaw en macOS,
...
<Info>
Primer chat más rápido: abra la interfaz de usuario de Control (no es necesaria la configuración del canal). Ejecute
`openclaw dashboard` y chatee en el navegador. Documentos: [Dashboard](/es/web/dashboard).
</Info>
...
<Note>
`--json` no implica el modo no interactivo. Para scripts, use `--non-interactive`.
</Note>

<Tip>
El onboarding de la CLI incluye un paso de búsqueda web donde puede elegir un proveedor
(Perplexity, Brave, Gemini, Grok o Kimi) y pegar su clave API para que el agente
pueda usar `web_search`. También puede configurar esto más tarde con
`openclaw configure --section web`. Documentos: [Web tools](/es/tools/web).
</Tip>
...
<Tabs>
  <Tab title="QuickStart (predeterminados)">
    - Puerta de enlace local (bucle local)
    - Espacio de trabajo predeterminado (o espacio de trabajo existente)
    - Puerto de puerta de enlace **18789**
    - Autenticación de puerta de enlace **Token** (autogenerado, incluso en bucle local)
    ...
    - Aislamiento de DM predeterminado: el onboarding local escribe `session.dmScope: "per-channel-peer"` cuando no está configurado. Detalles: [CLI Setup Reference](/es/start/wizard-cli-reference#outputs-and-internals)
    - Exposición de Tailscale **Desactivada**
    - Los DM de Telegram + WhatsApp predeterminan en **lista de permitidos** (se le pedirá su número de teléfono)
  </Tab>
...
1. **Modelo/Auth** — elige cualquier proveedor/flujo de autenticación compatible (clave de API, OAuth o token de configuración), incluido el Proveedor personalizado
   (compatible con OpenAI, compatible con Anthropic o detección automática desconocida). Elige un modelo predeterminado.
   Nota de seguridad: si este agente va a ejecutar herramientas o procesar contenido de webhooks/hooks, prefiere el modelo más fuerte de la última generación disponible y mantén la política de herramientas estricta. Los niveles más débiles/antiguos son más fáciles de inyectar mediante instrucciones.
   ...
   En el modo no interactivo `ref`, la variable de entorno del proveedor debe estar configurada; pasar indicadores de clave en línea sin esa variable de entorno falla rápidamente.
...
2. **Espacio de trabajo (Workspace)** — Ubicación de los archivos del agente (por defecto `~/.openclaw/workspace`). Inicializa archivos de arranque.
3. **Puerta de enlace (Gateway)** — Puerto, dirección de enlace, modo de autenticación, exposición a Tailscale.
   En el modo de token interactivo, elige el almacenamiento de token en texto plano predeterminado u opta por SecretRef.
...
5. **Demonio (Daemon)** — Instala un LaunchAgent (macOS) o una unidad de usuario systemd (Linux/WSL2).
   Si la autenticación por token requiere un token y `gateway.auth.token` está gestionado por SecretRef, la instalación del demonio lo valida pero no persiste el token resuelto en los metadatos del entorno del servicio de supervisor.
   Si la autenticación por token requiere un token y la SecretRef del token configurado no está resuelta, la instalación del demonio se bloquea con una guía accionable.
...
La CLI `--reset` tiene como valor predeterminado la configuración, las credenciales y las sesiones; use `--reset-scope full` para incluir el espacio de trabajo.
Si la configuración no es válida o contiene claves heredadas, la incorporación le pide que ejecute `openclaw doctor` primero.
</Note>
...
- Agregue `bindings` para enrutar mensajes entrantes (la incorporación puede hacerlo).
