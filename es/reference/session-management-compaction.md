---
summary: "Análisis profundo: almacén de sesiones + transcripciones, ciclo de vida e internos de (auto)compacción"
read_when:
  - Necesitas depurar ids de sesión, JSONL de transcripción o campos de sessions.
  - Estás cambiando el comportamiento de autocompacción o añadiendo mantenimiento de “pre-compacción”
...
- **Compactación** (manual + autocompacción) y dónde enganchar el trabajo de precompacción
- **Mantenimiento silencioso** (p. ej., escrituras en memoria que no deberían producir salida visible para el usuario)

Si primero quieres una visión general de alto nivel, comienza con:
...
- En modo remoto, los archivos de sesión están en el host remoto; “verificar tus archivos locales de Mac” no reflejará lo que el Gateway está usando.
...
2. **Transcripción (`<sessionId>.jsonl`)**
   ...
   - Se utiliza para reconstruir el contexto del modelo para futuros turnos

---

## Ubicaciones en disco

Por agente, en el host de Gateway:
...
- `rotateBytes`: rotar `sessions.json` cuando supere el tamaño (predeterminado `10mb`)
...
2. Si aún está por encima del objetivo, desalojar las entradas de sesión más antiguas y sus archivos de transcripción.
3. Continuar hasta que el uso esté en o por debajo de `highWaterBytes`.

En `mode: "warn"`, OpenClaw informa de posibles desalojos pero no modifica el almacén/archivos.
...
- `cron.sessionRetention` (predeterminado `24h`) poda las sesiones de ejecución de Cron aisladas antiguas del almacén de sesiones (`false` desactiva).
...
- **Expiración por inactividad** (`session.reset.idleMinutes` o legado `session.idleMinutes`) crea un nuevo `sessionId` cuando llega un mensaje después de la ventana de inactividad. Cuando se configuran diario + inactividad, gana el que expire primero.
- **Protección de bifurcación del hilo principal** (`session.parentForkMaxTokens`, predeterminado `100000`) omite la bifurcación de la transcripción principal cuando la sesión principal ya es demasiado grande; el nuevo hilo comienza desde cero. Establezca `0` para desactivar.
...
- `sessionFile`: anulación opcional de la ruta de transcripción explícita
...
El almacenamiento es seguro de editar, pero la Gateway es la autoridad: puede reescribir o rehidratar las entradas a medida que se ejecutan las sesiones.

---

## Estructura de la transcripción (`*.jsonl`)

Las transcripciones son gestionadas por `SessionManager` de `@mariozechner/pi-coding-agent`.
...
- `contextTokens` en el almacén es un valor estimado/reportado en tiempo de ejecución; no lo trates como una garantía estricta.
...
- Si ya es mayor, OpenClaw lo deja intacto.

Por qué: dejar suficiente margen para el "mantenimiento" de varios turnos (como escrituras de memoria) antes de que la compactación sea inevitable.
...
A partir de `2026.1.10`, OpenClaw también suprime el **streaming de borrador/escritura** cuando un fragmento parcial comienza con `NO_REPLY`, para que las operaciones silenciosas no filtren resultados parciales a mitad de un turno.

---

## "Volcado de memoria" previo a la compactación (implementado)

Objetivo: antes de que ocurra la auto-compactación, ejecutar un turno agente silencioso que escriba el estado
...
- `systemPrompt` (prompt del sistema adicional añadido para el turno de volcado)

Notas:

- El prompt del sistema por defecto incluye una sugerencia `NO_REPLY` para suprimir la entrega.
- La limpieza se ejecuta una vez por ciclo de compactación (rastreado en `sessions.json`).
- La limpieza se ejecuta solo para sesiones de Pi incrustadas (los backends de CLI la omiten).
- La limpieza se omite cuando el espacio de trabajo de la sesión es de solo lectura (`workspaceAccess: "ro"` o `"none"`).
- Consulte [Memoria](/es/concepts/memory) para ver el diseño de archivos del espacio de trabajo y los patrones de escritura.

Pi también expone un gancho `session_before_compact` en la API de extensión, pero la lógica de limpieza de OpenClaw vive en el lado del Gateway hoy.
...
- ¿Fugas de turnos silenciosos? Confirme que la respuesta comienza con `NO_REPLY` (token exacto) y que está en una versión que incluye la corrección de supresión de transmisión.
