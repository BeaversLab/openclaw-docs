...
Ejecutar un Gateway OpenClaw persistente en el nivel **Always Free** ARM de Oracle Cloud.

El nivel gratuito de Oracle puede ser una gran opción para OpenClaw (especialmente si ya tienes una cuenta OCI), pero tiene sus desventajas:
...
- Cuenta de Oracle Cloud ([registrarse](https://www.oracle.com/cloud/free/)) — consulta la [guía de registro de la comunidad](https://gist.github.com/rssnyder/51e3cfedd730e7dd5f4a816143b25dbd) si tienes problemas
...
3. Configura:
   ...
   - **Forma (Shape):** `VM.Standard.A1.Flex` (Ampere ARM)
   - **OCPUs:** 2 (o hasta 4)
...
**Consejo:** Si la creación de la instancia falla con "Out of capacity", intenta con un dominio de disponibilidad diferente o inténtalo más tarde. La capacidad del nivel gratuito es limitada.
...
Cuando se le pregunte "¿Cómo quieres hacer eclosionar tu bot?", seleccione **"Hacer esto más tarde"**.
...
## 6) Configurar puerta de enlace (loopback + autenticación por token) y habilitar Tailscale Serve
...
## 8) Restringir la seguridad de la VCN

Ahora que todo está funcionando, bloquee la VCN para bloquear todo el tráfico excepto Tailscale. La Red Virtual en la Nube (VCN) de OCI actúa como un cortafuegos en el borde de la red: el tráfico se bloquea antes de llegar a su instancia.
...
Esto bloquea SSH en el puerto 22, HTTP, HTTPS y todo lo demás en el borde de la red. De ahora en adelante, solo puede conectarse a través de Tailscale.
...
Con la VCN bloqueada (solo abierto el UDP 41641) y la puerta de enlace vinculada al loopback, obtiene una fuerte defensa en profundidad: el tráfico público se bloquea en el borde de la red y el acceso de administración se realiza a través de su tailnet.

Esta configuración a menudo elimina la _necesidad_ de reglas de cortafuegos adicionales basadas en el host puramente para detener la fuerza bruta SSH en toda Internet, pero aún debe mantener el sistema actualizado, ejecutar `openclaw security audit` y verificar que no esté escuchando accidentalmente en interfaces públicas.

### Ya protegido

| Paso tradicional   | ¿Necesario?     | Por qué                                                                          |
| ------------------ | ----------- | ---------------------------------------------------------------------------- |
| Cortafuegos UFW       | No          | La VCN bloquea antes de que el tráfico llegue a la instancia                                   |
| fail2ban           | No          | Sin fuerza bruta si el puerto 22 está bloqueado en la VCN                                     |
| Endurecimiento de sshd     | No          | El SSH de Tailscale no usa sshd                                               |
| Deshabilitar el acceso de root | No          | Tailscale usa la identidad de Tailscale, no los usuarios del sistema                          |
| Autenticación solo con clave SSH  | No          | Tailscale se autentica a través de su tailnet                                     |
| Endurecimiento de IPv6     | Usualmente no | Depende de la configuración de su VCN/subred; verifique lo que realmente está asignado/expuesto |
...
- **Monitorear Tailscale:** Revisar dispositivos en [consola de administración de Tailscale](https://login.tailscale.com/admin)
...
Las instancias ARM de nivel gratuito son populares. Intente:

- Dominio de disponibilidad diferente
- Reintentar durante horas no pico (temprano en la mañana)
- Use el filtro "Always Free" al seleccionar la forma
...
Es posible que algunas herramientas no tengan compilaciones ARM. Verifique:

```bash
uname -m  # Should show aarch64
```

La mayoría de los paquetes npm funcionan bien. Para los binarios, busque versiones `linux-arm64` o `aarch64`.
...
- [Guía de DigitalOcean](/es/platforms/digitalocean) — si desea pago + registro más fácil
