...
read_when:
  ...
  - Quieres orientación de configuración de Venice AI
title: "Venice AI"
---

# Venice AI (Venice highlight)

**Venice** es nuestra configuración destacada de Venice para una inferencia priorizada por la privacidad con acceso anonimizado opcional a modelos propietarios.

Venice AI proporciona inferencia de IA centrada en la privacidad con soporte para modelos sin censura y acceso a modelos propietarios importantes a través de su proxy anonimizado. Toda la inferencia es privada de manera predeterminada; no se entrena con tus datos, no hay registro.
...
Venice ofrece dos niveles de privacidad; comprender esto es clave para elegir tu modelo:

| Modo            | Descripción                                                                                                                                      | Modelos                                                       |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------- |
| **Privado**     | Completamente privado. Los mensajes/respuestas **nunca se almacenan ni registran**. Efímero.                                                     | Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored, etc. |
| **Anonimizado** | Accesible a través de Venice con los metadatos eliminados. El proveedor subyacente (OpenAI, Anthropic, Google, xAI) ve solicitudes anonimizadas. | Claude, GPT, Gemini, Grok                                     |
...
- **Llamada de funciones (Function calling)**: ✅ Compatible con modelos seleccionados (consulta las capacidades del modelo)
...
- **Modelo predeterminado**: `venice/kimi-k2-5` para un razonamiento privado fuerte más visión.
- **Opción de alta capacidad**: `venice/claude-opus-4-6` para la ruta Venice anonimizada más fuerte.
- **Privacidad**: Elija modelos "privados" para una inferencia totalmente privada.
- **Capacidad**: Elija modelos "anonimizados" para acceder a Claude, GPT, Gemini a través del proxy de Venice.
...
2. Seleccionar **Modelo/autenticación**
3. Elegir **Venice AI**
...
| **Mejor calidad general**         | `claude-opus-4-6`                | Opción Venice anonimizada más fuerte                         |
...
| `openai-gpt-53-codex`           | GPT-5.3 Codex (a través de Venice)     | 400k     | Razonamiento, visión, codificación |
| `openai-gpt-52`                 | GPT-5.2 (a través de Venice)           | 256k     | Razonamiento                       |
| `openai-gpt-52-codex`           | GPT-5.2 Codex (a través de Venice)     | 256k     | Razonamiento, visión, codificación |
...
| `grok-code-fast-1`              | Grok Code Fast 1 (vía Venice)          | 256k     | Razonamiento, codificación         |

## Descubrimiento de modelos

OpenClaw descubre automáticamente los modelos de la API de Venice cuando se establece `VENICE_API_KEY`. Si la API es inalcanzable, recurre a un catálogo estático.
...
| **Llamada a funciones** | ✅ La mayoría de los modelos (verifique `supportsFunctionCalling` en la API) |
...
## Comparación: Venice frente a la API directa
...
| **Características** | La mayoría de las características admitidas | Características completas |
