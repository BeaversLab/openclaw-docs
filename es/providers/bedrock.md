...
Si se detectan credenciales de AWS, OpenClaw puede descubrir automáticamente los
...
- `defaultContextWindow` (predeterminado: `32000`) y `defaultMaxTokens` (predeterminado:
  `4096`) se usan para los modelos descubiertos (anúlelos si conoce los límites de su modelo).
...
AWS utilizará automáticamente el servicio de metadatos de instancia (IMDS) para la
autenticación. Sin embargo, la detección de credenciales de OpenClaw actualmente solo
verifica las variables de entorno, no las credenciales de IMDS.
...
- Si utiliza perfiles, establezca `AWS_PROFILE` en el host de la puerta de enlace (gateway host).
...
- La compatibilidad con el razonamiento depende del modelo; consulte la tarjeta del modelo de Bedrock para
...
- Si prefiere un flujo de clave administrada, también puede colocar un proxy compatible con OpenAI
  delante de Bedrock y configurarlo como un proveedor de OpenAI.
