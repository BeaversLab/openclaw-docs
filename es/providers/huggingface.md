...
Cuando configuras una clave de API de Hugging Face (a través de la incorporación, `HUGGINGFACE_HUB_TOKEN`, o `HF_TOKEN`), OpenClaw usa este GET para descubrir los modelos de completado de chat disponibles. Durante la **incorporación interactiva**, después de ingresar tu token, verás un menú desplegable de **Modelo Hugging Face predeterminado** que se completa con esa lista (o el catálogo integrado si falla la solicitud). En tiempo de ejecución (por ejemplo, al iniciar Gateway), cuando hay una clave presente, OpenClaw vuelve a llamar a **GET** `https://router.huggingface.co/v1/models` para actualizar el catálogo. La lista se combina con un catálogo integrado (para metadatos como la ventana de contexto y el costo). Si falla la solicitud o no se establece ninguna clave, solo se usa el catálogo integrado.
