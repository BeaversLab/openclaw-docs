...
<Warning>
  **Usuarios de Ollama remoto**: No utilicen la URL compatible con OpenAI `/v1`
  (`http://host:11434/v1`) con OpenClaw. Esto interrumpe la llamada a herramientas y los modelos
  podrían generar el JSON de la herramienta en formato de texto sin formato. Utilicen en su lugar la
  URL de la API nativa de Ollama: `baseUrl: "http://host:11434"` (sin `/v1`).
</Warning>
...
<Warning>
  No añadas `/v1` a la URL. La ruta `/v1` usa el modo compatible con OpenAI, donde la llamada a
...
<Warning>
  **La llamada a herramientas no es confiable en modo compatible con OpenAI.** Use este modo solo si
  necesita el formato OpenAI para un proxy y no depende del comportamiento nativo de llamada a
  herramientas.
</Warning>
