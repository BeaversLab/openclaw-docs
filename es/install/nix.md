...
>
> El repositorio nix-openclaw es la fuente de verdad para la instalación de Nix. Esta página es solo un resumen rápido.

## Lo que obtienes

- Gateway + aplicación macOS + herramientas (whisper, spotify, cámaras) — todo fijado (pinned)
...
En macOS, la aplicación GUI no hereda automáticamente las variables de entorno del shell. También
puedes habilitar el modo Nix a través de defaults:
...
OpenClaw lee la configuración JSON5 desde `OPENCLAW_CONFIG_PATH` y almacena datos mutables en `OPENCLAW_STATE_DIR`.
...
Al ejecutarse bajo Nix, establece estos explícitamente en ubicaciones administradas por Nix para que el estado y la configuración en tiempo de ejecución
se mantengan fuera del almacenamiento inmutable.
...
y las construcciones de Nix (que no dependen de un toolchain de Xcode completo).
