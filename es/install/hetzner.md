...
## 3) Clone el repositorio de OpenClaw
...
## 4) Cree directorios persistentes en el host
...
`--allow-unconfigured` es solo por conveniencia de inicio, no es un reemplazo para una configuración de puerta de enlace adecuada. Aún establezca la autenticación (`gateway.auth.token` o contraseña) y use configuraciones de enlace seguro para su implementación.

---

## 7) Pasos compartidos del tiempo de ejecución de Docker VM

Use la guía compartida de tiempo de ejecución para el flujo común de host Docker:

- [Incorpore los binarios necesarios en la imagen](/es/install/docker-vm-runtime#bake-required-binaries-into-the-image)
- [Construya e inicie](/es/install/docker-vm-runtime#build-and-launch)
...
Después de los pasos compartidos de construcción e inicio, cree un túnel desde su laptop:
...
- Aprovisionamiento automatizado a través de cloud-init
- Scripts de implementación (arranque, implementación, copia de seguridad/restauración)
- Endurecimiento de seguridad (firewall, UFW, acceso solo SSH)
- Configuración de túnel SSH para acceso a la puerta de enlace
...
Este enfoque complementa la configuración de Docker anterior con implementaciones reproducibles, infraestructura con control de versiones y recuperación automatizada de desastres.
