...
<Note>
  Los valores predeterminados de Docker aquí asumen modos de enlace (`lan`/`loopback`), no alias de
  host. Use valores de modo de enlace en `gateway.bind` (por ejemplo `lan` o `loopback`), no alias
  de host como `0.0.0.0` o `localhost`.
</Note>
