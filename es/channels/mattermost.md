...
Estado: compatible mediante un plugin (token de bot + eventos de WebSocket). Se admiten canales, grupos y MDs.
Mattermost es una plataforma de mensajería para equipos que puede ser autoalojada; consulte el sitio oficial en
...
- Para configuraciones multicuenta, `commands` se puede establecer en el nivel superior o en
...
- Requisito de accesibilidad: el endpoint de devolución de llamada debe ser accesible desde el servidor de Mattermost.
  - No configure `callbackUrl` como `localhost` a menos que Mattermost se ejecute en el mismo host/espacio de nombres de red que OpenClaw.
  - No configure `callbackUrl` en su URL base de Mattermost a menos que esa URL actúe como proxy inverso de `/api/channels/mattermost/command` hacia OpenClaw.
...
Use `channels.mattermost.replyToMode` para controlar si las respuestas de canal y grupo se mantienen en el
...
- Predeterminado: `channels.mattermost.dmPolicy = "pairing"` (los remitentes desconocidos obtienen un código de emparejamiento).
- Aprobar a través de:
...
- Predeterminado: `channels.mattermost.groupPolicy = "allowlist"` (limitado por mención).
...
- Canales abiertos: `channels.mattermost.groupPolicy="open"` (limitado por mención).
- Nota de ejecución: si `channels.mattermost` falta completamente, la ejecución vuelve a `groupPolicy="allowlist"` para las comprobaciones de grupo (incluso si `channels.defaults.groupPolicy` está establecido).
...
- `callback_data` (obligatorio): valor enviado al hacer clic (se utiliza como el ID de acción).
- `style` (opcional): `"default"`, `"primary"` o `"danger"`.

Cuando un usuario hace clic en un botón:

1. Todos los botones se reemplazan por una línea de confirmación (por ejemplo, "✓ **Sí** seleccionado por @usuario").
...
- Los ID de acción que contienen guiones o guiones bajos se sanitizan automáticamente (limitación de enrutamiento de Mattermost).

Configuración:

- `channels.mattermost.capabilities`: array de cadenas de capacidades. Agregue `"inlineButtons"` para habilitar la descripción de la herramienta de botones en el mensaje del sistema del agente.
- `channels.mattermost.interactions.callbackBaseUrl`: URL base externa opcional para las devoluciones de llamada de los botones (por ejemplo, `https://gateway.example.com`). Úselo cuando Mattermost no pueda alcanzar la puerta de enlace en su host de enlace directamente.
- En configuraciones de múltiples cuentas, también puede establecer el mismo campo bajo `channels.mattermost.accounts.<id>.interactions.callbackBaseUrl`.
...
5. `context.action_id` debe coincidir con el `id` del botón para que el mensaje de confirmación muestre el
   nombre del botón (p. ej., "Aprobar") en lugar de una ID sin procesar.
...
4. Firme: `HMAC-SHA256(key=secret, data=serializedContext)`
...
- El `json.dumps` de Python agrega espacios por defecto (`{"key": "val"}`). Use
  `separators=(",", ":")` para coincidir con la salida compacta de JavaScript (`{"key":"val"}`).
- Firme siempre **todos** los campos de contexto (menos `_token`). La puerta de enlace elimina `_token` y luego
...
El complemento de Mattermost incluye un adaptador de directorio que resuelve los nombres de canal y usuario a través de la API de Mattermost. Esto habilita los objetivos `#channel-name` y `@username` en entregas `openclaw message send` y cron/webhook.
...
- Sin respuestas en los canales: asegúrese de que el bot esté en el canal y lo mencione (oncall), use un prefijo de activación (onchar), o establezca `chatmode: "onmessage"`.
...
- Los botones aparecen como cuadros blancos: el agente puede estar enviando datos de botones malformados. Verifique que cada botón tenga los campos `text` y `callback_data`.
...
- Los botones devuelven 404 al hacer clic: el `id` del botón probablemente contiene guiones o guiones bajos. El enrutador de acciones de Mattermost falla con IDs no alfanuméricos. Use solo `[a-zA-Z0-9]`.
- La puerta de enlace registra `invalid _token`: discrepancia de HMAC. Verifique que firme todos los campos de contexto (no un subconjunto), use claves ordenadas y use JSON compacto (sin espacios). Vea la sección HMAC anterior.
- La puerta de enlace registra `missing _token in context`: el campo `_token` no está en el contexto del botón. Asegúrese de que se incluya al crear el payload de integración.
