...
```
  ...
  <Step title="Iniciar puerta de enlace y aprobar el primer MD">
...
<Note>
El orden de resolución de tokens es consciente de la cuenta. En la práctica, los valores de configuración tienen prioridad sobre la alternativa de entorno, y `TELEGRAM_BOT_TOKEN` solo se aplica a la cuenta predeterminada.
</Note>
...
<Tabs>
  <Tab title="Política de MD">
    ...
    Onboarding acepta entrada `@username` y la resuelve a IDs numéricos.
...
```
  ...
  <Tab title="Política de grupos y listas de permitidos">
    ...
    Nota de ejecución: si `channels.telegram` falta completamente, la ejecución utiliza por defecto `groupPolicy="allowlist"` de cierre seguro a menos que `channels.defaults.groupPolicy` se establezca explícitamente.
...
- Los mensajes de DM pueden llevar `message_thread_id`; OpenClaw los enruta con claves de sesión conscientes de hilos y preserva el ID de hilo para las respuestas.
...
<AccordionGroup>
  ...
  <Accordion title="Formato y alternativa HTML">
    El texto saliente utiliza el formato `parse_mode: "HTML"` de Telegram.
...
```
    ...
    Las pulsaciones en las devoluciones de llamada se pasan al agente como texto:
    `callback_data: <value>`
  ...
  <Accordion title="Etiquetas de hebra de respuesta">
    Telegram admite etiquetas explícitas de hebra de respuesta en la salida generada:
    ...
    Nota: `off` deshabilita la hebra de respuesta implícita. Las etiquetas explícitas `[[reply_to_*]]` todavía se respetan.
  ...
  <Accordion title="Temas del foro y comportamiento de los hilos">
    ...
    - las respuestas y la actividad de escritura apuntan al hilo del tema
    ...
    **Generación de ACP vinculada al hilo desde el chat**:
    ...
    - los chats privados con `message_thread_id` mantienen el enrutamiento MD pero utilizan claves de sesión/destinos de respuesta conscientes del hilo.
...
```
  ...
  <Accordion title="Notificaciones de reacción">
    ...
    - `own` significa reacciones de los usuarios solo a mensajes enviados por el bot (mejor esfuerzo a través de la caché de mensajes enviados).
    ...
    - Telegram no proporciona IDs de hilo en las actualizaciones de reacción.
      - los grupos que no son foros se dirigen a la sesión de chat de grupo
...
```
  ...
  <Accordion title="Sondeo prolongado vs webhook">
    Por defecto: sondeo prolongado.
...
```
  ...
  <Accordion title="Aprobaciones de ejecución en Telegram">
    Telegram admite aprobaciones de ejecución en los mensajes directos (DM) de los aprobadores y, opcionalmente, puede publicar solicitudes de aprobación en el chat o tema de origen.
    ...
    Los aprobadores deben ser IDs numéricos de usuario de Telegram. Cuando `enabled` es false o `approvers` está vacío, Telegram no actúa como cliente de aprobación de ejecución. Las solicitudes de aprobación recurren a otras rutas de aprobación configuradas o a la política de reserva de aprobación de ejecución.

    Reglas de entrega:

    - `target: "dm"` envía solicitudes de aprobación solo a los mensajes directos de los aprobadores configurados
    ...
    La entrega en el canal muestra el texto del comando en el chat, así que habilite `channel` o `both` solo en grupos/temas de confianza. Cuando la solicitud llega a un tema de foro, OpenClaw preserva el tema tanto para la solicitud de aprobación como para el seguimiento posterior a la aprobación.
...
<AccordionGroup>
  ...
  <Accordion title="Los comandos funcionan parcialmente o no funcionan">
    ...
    - autorice su identidad de remitente (emparejamiento y/o ID numérico `allowFrom`)
  ...
  <Accordion title="Sondeo o inestabilidad de red">
    ...
    - Algunos hosts resuelven `api.telegram.org` a IPv6 primero; una salida IPv6 rota puede causar fallas intermitentes de la API de Telegram.
...
- `channels.telegram.allowFrom`: lista de permitidos de DM (IDs numéricos de usuario de Telegram). `allowlist` requiere al menos un ID de remitente. `open` requiere `"*"`. `openclaw doctor --fix` puede resolver entradas de `@username` heredadas a IDs y puede recuperar entradas de lista de permitidos desde archivos de almacenamiento de emparejamiento en flujos de migración de lista de permitidos.
...
- `channels.telegram.groupAllowFrom`: lista de permitidos de remitentes de grupo (ID de usuario de Telegram numéricos). `openclaw doctor --fix` puede resolver entradas `@username` heredadas a ID. Las entradas no numéricas se ignoran en el momento de la autenticación. La autenticación de grupo no utiliza el respaldo del almacén de emparejamiento de MD (`2026.2.25+`).
...
- `channels.telegram.groups`: valores predeterminados por grupo + lista de permitidos (use `"*"` para valores predeterminados globales).
  ...
  - `channels.telegram.groups.<id>.topics.<threadId>.*`: anulaciones por tema (campos de grupo + campos exclusivos de tema `agentId`).
...
- `bindings[]` de nivel superior con `type: "acp"` e id de tema canónico `chatId:topic:topicId` en `match.peer.id`: campos de vinculación de tema ACP persistentes (ver [ACP Agents](/es/tools/acp-agents#channel-specific-settings)).
...
- `channels.telegram.execApprovals.approvers`: IDs de usuario de Telegram permitidos para aprobar o denegar solicitudes de exec. Requerido cuando las aprobaciones de exec están habilitadas.
...
- `channels.telegram.retry`: política de reintento para los asistentes de envío de Telegram (CLI/herramientas/acciones) ante errores de API salientes recuperables (attempts, minDelayMs, maxDelayMs, jitter).
- `channels.telegram.network.autoSelectFamily`: anular Node autoSelectFamily (true=activar, false=desactivar). Predeterminado activado en Node 22+, con WSL2 predeterminado desactivado.
- `channels.telegram.network.dnsResultOrder`: anular el orden de resultados de DNS (`ipv4first` o `verbatim`). Predeterminado `ipv4first` en Node 22+.
...
- `channels.telegram.actions.reactions`: controlar las reacciones de la herramienta de Telegram.
- `channels.telegram.actions.sendMessage`: controlar los envíos de mensajes de la herramienta de Telegram.
- `channels.telegram.actions.deleteMessage`: controlar las eliminaciones de mensajes de la herramienta de Telegram.
...
- inicio de sesión/autenticación: `enabled`, `botToken`, `tokenFile`, `accounts.*` (`tokenFile` debe apuntar a un archivo normal; se rechazan los enlaces simbólicos)
