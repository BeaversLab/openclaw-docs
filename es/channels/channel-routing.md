...
OpenClaw enruta las respuestas **de vuelta al canal desde donde se originó un mensaje**. El
...
- Cuenta predeterminada opcional del canal: `channels.<channel>.defaultAccount` elige
  ...
  - En configuraciones de múltiples cuentas, establezca un valor predeterminado explícito (`defaultAccount` o `accounts.default`) cuando se configuran dos o más cuentas. Sin esto, el enrutamiento de respaldo puede elegir la primera ID de cuenta normalizada.
...
- Los temas del foro de Telegram incrustan `:topic:<topicId>` en la clave del grupo.
...
Para evitar que el `lastRoute` de la sesión sea sobrescrito por MDs que no son del propietario,
OpenClaw infiere un propietario fijado a partir de `allowFrom` cuando se cumple todo lo siguiente:
...
En ese caso de discordancia, OpenClaw aún registra los metadatos de la sesión entrante, pero
...
8. **Agente predeterminado** (`agents.list[].default`, si no, la primera entrada de la lista, recurso a `main`).
...
Puede anular la ruta del almacén mediante el uso de plantillas de `session.store` y `{agentId}`.
