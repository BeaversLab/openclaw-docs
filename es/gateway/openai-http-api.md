...
OpenClaw trata el campo `model` de OpenAI como un **destino de agente**, no como una identificación de modelo de proveedor sin procesar.
...
- `x-openclaw-message-channel: <channel>` establece el contexto de canal de entrada sintético para prompts y políticas con conocimiento de canal.
...
De manera predeterminada, el endpoint es **sin estado por solicitud** (se genera una nueva clave de sesión en cada llamada).
...
- Los clientes más nativos de agentes cada vez prefieren `/v1/responses`.
...
<AccordionGroup>
  ...
  <Accordion title="¿`/v1/models` lista agentes o subagentes?">
    Enumera los destinos de agentes de nivel superior, no los modelos del proveedor de backend ni los subagentes.
  ...
  <Accordion title="¿Cómo encajan las incrustaciones en este contrato?">
    ...
    Sin ese encabezado, la solicitud se transfiere a la configuración de incrustación normal del agente seleccionado.
...
- Las anulaciones del proveedor/modelo del backend pertenecen a `x-openclaw-model`, no al campo `model` de OpenAI.
- `/v1/embeddings` soporta `input` como una cadena o una matriz de cadenas.
