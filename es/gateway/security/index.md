...
| `hooks.token_too_short`                            | aviso               | Fuerza bruta más fácil en la entrada de hook                                                                                                       | `hooks.token`                                                                                                                                         | no              |
| `hooks.default_session_key_unset`                  | aviso               | Las ejecuciones del agente de hook se distribuyen en sesiones generadas por solicitud                                                              | `hooks.defaultSessionKey`                                                                                                                             | no              |
| `hooks.allowed_agent_ids_unrestricted`             | aviso/crítico       | Los llamadores de hook autenticados pueden enrutar a cualquier agente configurado                                                                  | `hooks.allowedAgentIds`                                                                                                                               | no              |
| `hooks.request_session_key_enabled`                | aviso/crítico       | El llamador externo puede elegir sessionKey                                                                                                        | `hooks.allowRequestSessionKey`                                                                                                                        | no              |
| `hooks.request_session_key_prefixes_missing`       | aviso/crítico       | Sin límite en las formas de la clave de sesión externa                                                                                             | `hooks.allowedSessionKeyPrefixes`                                                                                                                     | no              |
| `logging.redact_off`                               | advertencia         | Valores confidenciales filtrados a los registros/estado                                                                                            | `logging.redactSensitive`                                                                                                                             | sí              |
| `sandbox.docker_config_mode_off`                   | advertencia         | Configuración de Docker Sandbox presente pero inactiva                                                                                             | `agents.*.sandbox.mode`                                                                                                                               | no              |
| `sandbox.dangerous_network_mode`                   | crítico             | La red Docker Sandbox utiliza el modo de unión de espacio de nombres `host` o `container:*`                                                        | `agents.*.sandbox.docker.network`                                                                                                                     | no              |
...
| `tools.exec.host_sandbox_no_sandbox_agents`        | aviso               | El `exec host=sandbox` por agente se resuelve en ejecución del host cuando el sandbox está desactivado                                             | `agents.list[].tools.exec.host`, `agents.list[].sandbox.mode`                                                                                         | no              |
| `tools.exec.safe_bins_interpreter_unprofiled`      | aviso               | Los binarios de intérprete/ejecución en `safeBins` sin perfiles explícitos amplían el riesgo de ejecución                                          | `tools.exec.safeBins`, `tools.exec.safeBinProfiles`, `agents.list[].tools.exec.*`                                                                     | no              |
| `skills.workspace.symlink_escape`                  | advertencia         | El `skills/**/SKILL.md` del espacio de trabajo se resuelve fuera de la raíz del espacio de trabajo (desviación de la cadena de enlaces simbólicos) | espacio de trabajo `skills/**` estado del sistema de archivos                                                                                         | no              |
...
desactiva por completo las comprobaciones de identidad del dispositivo. Esto es una degradación grave de la seguridad;
...
los interruptores de depuración inseguros/peligrosos conocidos están habilitados. Esa comprobación actualmente agrega:
...
- El gateway de OpenClaw es local/bucle de retorno (loopback) primero. Si finaliza TLS en un proxy inverso, configure HSTS en el dominio HTTPS orientado al proxy allí.
...
- Para las implementaciones de la interfaz de usuario de Control que no son de bucle local, `gateway.controlUi.allowedOrigins` es obligatorio de forma predeterminada.
- `gateway.controlUi.allowedOrigins: ["*"]` es una política explícita de permitir todos los orígenes del navegador, no una opción predeterminada reforzada. Evítela fuera de las pruebas locales estrictamente controladas.
...
- Trate la reasignación de DNS y el comportamiento del encabezado proxy-host como preocupaciones de endurecimiento de la implementación; mantenga `trustedProxies` ajustado y evite exponer la puerta de enlace directamente a la Internet pública.

## Los registros de sesión local residen en el disco

OpenClaw guarda las transcripciones de sesión en el disco bajo `~/.openclaw/agents/<agentId>/sessions/*.jsonl`.
Esto es necesario para la continuidad de la sesión y (opcionalmente) la indexación de memoria de la sesión, pero también significa
**cualquier proceso/usuario con acceso al sistema de archivos puede leer esos registros**. Trate el acceso al disco como el límite
confianza y restrinja los permisos en `~/.openclaw` (consulte la sección de auditoría a continuación). Si necesita
...
Si se vincula un nodo macOS, la puerta de enlace puede invocar `system.run` en ese nodo. Esto es **ejecución remota de código** en la Mac:

- Requiere el emparejamiento de nodos (aprobación + token).
- Controlado en la Mac mediante **Configuración → Exec approvals** (seguridad + ask + allowlist).
...
- Si no desea ejecución remota, configure la seguridad en **deny** y elimine el emparejamiento del nodo para esa Mac.
...
- Enviar mensajes a cualquier persona (si le da acceso a WhatsApp)
...
- **Primero la identidad:** decida quién puede hablar con el bot (emparejamiento de MD / listas de permitidos / "abierto" explícito).
- **Luego el alcance:** decida dónde se permite que el bot actúe (listas de permitidos de grupos + filtrado de menciones, herramientas, sandboxing, permisos de dispositivo).
...
listas de permitidos/emparejamiento de canales más `commands.useAccessGroups` (ver [Configuración](/es/gateway/configuration)
...
`/exec` es una conveniencia solo para la sesión para operadores autorizados. **No** escribe configuración o
...
Los complementos se ejecutan **en el mismo proceso** que el Gateway. Trátelos como código confiable:

- Solo instala complementos de fuentes en las que confíes.
...
- Si instalas complementos desde npm (`openclaw plugins install <npm-spec>`), trátalo como ejecutar código no confiable:
  ...
  - Prefiere versiones fijas y exactas (`@scope/pkg@1.2.3`) e inspecciona el código desempaquetado en el disco antes de habilitar.

Detalles: [Complementos](/es/tools/plugin)

## Modelo de acceso DM (emparejamiento / lista de permitidos / abierto / deshabilitado)

Todos los canales actuales con capacidad DM admiten una política de DM (`dmPolicy` o `*.dm.policy`) que restringe los DM entrantes **antes** de que se procese el mensaje:

- `pairing` (predeterminado): los remitentes desconocidos reciben un código de emparejamiento breve y el bot ignora su mensaje hasta que se apruebe. Los códigos caducan después de 1 hora; los DM repetidos no reenviarán un código hasta que se cree una nueva solicitud. Las solicitudes pendientes están limitadas a **3 por canal** de forma predeterminada.
- `allowlist`: los remitentes desconocidos están bloqueados (sin protocolo de enlace de emparejamiento).
- `open`: permite a cualquiera enviar DM (público). **Requiere** que la lista de permitidos del canal incluya `"*"` (aceptación explícita).
...
De forma predeterminada, OpenClaw enruta **todos los DM a la sesión principal** para que tu asistente tenga continuidad entre dispositivos y canales. Si **varias personas** pueden enviar DM al bot (DM abiertos o una lista de permitidos para varias personas), considera aislar las sesiones DM:
...
Esto evita fugas de contexto entre usuarios mientras se mantienen aislados los chats grupales.

Este es un límite de contexto de mensajería, no un límite de administración del host. Si los usuarios son mutuamente adversarios y comparten el mismo host/configuración del Gateway, ejecuta gateways separados para cada límite de confianza.

### Modo DM seguro (recomendado)

Trate el fragmento anterior como **modo DM seguro**:

- Predeterminado: `session.dmScope: "main"` (todos los MDs comparten una sesión para la continuidad).
...
Si ejecuta varias cuentas en el mismo canal, use `per-account-channel-peer` en su lugar. Si la misma persona le contacta a través de varios canales, use `session.identityLinks` para colapsar esas sesiones DM en una identidad canónica. Consulte [Gestión de sesiones](/es/concepts/session) y [Configuración](/es/gateway/configuration).
...
- **Lista de permitidos de grupos** (específica del canal): de qué grupos/canales/gremios el bot aceptará mensajes en absoluto.
  ...
  - Las comprobaciones de grupo se ejecutan en este orden: `groupPolicy`/listas de permitidos de grupo primero, activación de mención/respuesta en segundo lugar.
  - Responder a un mensaje de bot (mención implícita) **no** evita las listas de permitidos de remitentes como `groupAllowFrom`.
  - **Nota de seguridad:** trate `dmPolicy="open"` y `groupPolicy="open"` como configuraciones de último recurso. Deben usarse apenas; prefiera el emparejamiento + listas de permitidos a menos que confíe plenamente en todos los miembros de la sala.
...
- Nota: el sandboxing es opcional. Si el modo sandbox está desactivado, exec se ejecuta en el host de la puerta de enlace aunque tools.exec.host predeterminado sea sandbox, y la ejecución en el host no requiere aprobaciones a menos que configure host=gateway y configure las aprobaciones de exec.
...
- **La elección del modelo importa:** los modelos más pequeños/antiguos/heredados son significativamente menos robustos contra la inyección de indicaciones y el mal uso de herramientas. Para agentes con herramientas, use el modelo más fuerte y resistente a instrucciones de última generación disponible.

Banderas rojas para tratar como no confiables:

- “Lee este archivo/URL y haz exactamente lo que dice.”
- “Ignora tu indicación del sistema o reglas de seguridad.”
...
OpenClaw incluye indicadores de omisión explícitos que desactivan el contenedor de seguridad de contenido externo:
...
- Mantén estos sin establecer/en false en producción.
- Actívalos solo temporalmente para la depuración con un alcance estricto.
- Si están activados, aísla ese agente (sandbox + herramientas mínimas + espacio de nombres de sesión dedicado).

Nota de riesgo de Hooks:

- Las cargas útiles de los Hooks son contenido que no es de confianza, incluso cuando la proviene de sistemas que controlas (el contenido de correo/documentos/web puede llevar inyección de instrucciones).
- Los niveles de modelos débiles aumentan este riesgo. Para la automatización impulsada por Hooks, prefiere niveles de modelos modernos fuertes y mantén la política de herramientas estricta (`tools.profile: "messaging"` o más estricta), además de sandbox cuando sea posible.

### La inyección de instrucciones no requiere DMs públicos

Incluso si **solo tú** puedes enviar mensajes al bot, la inyección de instrucciones aún puede ocurrir a través de
cualquier **contenido que no sea de confianza** que lea el bot (resultados de búsqueda/recuperación web, páginas del navegador,
...
Cuando se habilitan las herramientas, el riesgo típico es la exfiltración de contexto o activar
llamadas a herramientas. Reduce el radio de explosión mediante:

- Usar un **agente lector** de solo lectura o con herramientas desactivadas para resumir el contenido que no es de confianza,
...
- Para las entradas de URL de OpenResponses (`input_file` / `input_image`), establece `gateway.http.endpoints.responses.files.urlAllowlist` y
  `gateway.http.endpoints.responses.images.urlAllowlist` estrictos,
  y mantén `maxUrlParts` bajo.
  Las listas de permitidos vacías se tratan como sin establecer; usa `files.allowUrl: false` / `images.allowUrl: false`
  si deseas desactivar la recuperación de URL por completo.
- Habilitar el sandbox y las listas de permitidos de herramientas estrictas para cualquier agente que toque entradas que no sean de confianza.
- Mantener los secretos fuera de las instrucciones; pásalos a través de env/config en el host de la puerta de enlace en su lugar.

### Fortaleza del modelo (nota de seguridad)

La resistencia a la inyección de indicaciones **no** es uniforme en todos los niveles de modelos. Los modelos más pequeños/baratos son generalmente más susceptibles al uso indebido de herramientas y al secuestro de instrucciones, especialmente bajo indicaciones adversas.

<Warning>
  Para agentes con herramientas habilitadas o agentes que leen contenido que no es de confianza, el
  riesgo de inyección de indicaciones con modelos más antiguos/más pequeños suele ser demasiado
  alto. No ejecute esas cargas de trabajo en niveles de modelo débiles.
</Warning>
...
- **No use niveles más antiguos/más débiles/más pequeños** para agentes con herramientas habilitadas o bandejas de entrada que no son de confianza; el riesgo de inyección de indicaciones es demasiado alto.
- Si debe usar un modelo más pequeño, **reduzca el radio de explosión** (herramientas de solo lectura, espacio aislado fuerte, acceso mínimo al sistema de archivos, listas de permisos estrictas).
...
no estaba destinado a un canal público. En entornos grupales, trátelos como **solo
de depuración** y manténgalos desactivados a menos que los necesite explícitamente.
...
### 0.4) Exposición de red (bind + port + firewall)
...
Esta superficie HTTP incluye la interfaz de usuario de control (Control UI) y el host del lienzo (canvas):

- Interfaz de usuario de control (activos de SPA) (ruta base predeterminada `/`)
- Host del lienzo: `/__openclaw__/canvas/` y `/__openclaw__/a2ui/` (HTML/JS arbitrario; tratarlo como contenido no confiable)
...
- No haga que el contenido del lienzo comparta el mismo origen que las superficies web privilegiadas a menos que entienda completamente las implicaciones.

El modo de enlace (bind mode) controla dónde escucha la puerta de enlace (Gateway):

- `gateway.bind: "loopback"` (predeterminado): solo los clientes locales pueden conectarse.
- Los enlaces que no son de bucle invertido (`"lan"`, `"tailnet"`, `"custom"`) amplían la superficie de ataque. Úselos solo con un token/contraseña compartido y un firewall real.

Reglas generales:

- Prefiera Tailscale Serve sobre los enlaces de LAN (Serve mantiene la puerta de enlace en bucle invertido y Tailscale maneja el acceso).
- Si debe enlazar a la LAN, proteja el puerto con una lista de permitidos estricta de IPs de origen; no lo reenvíe ampliamente.
...
Evite codificar nombres de interfaces como `eth0` en los fragmentos de documentación. Los nombres de las interfaces
...
de las configuraciones: SSH + tus puertos de proxy inverso).
...
**Consideración de seguridad operacional:** Transmitir detalles de la infraestructura facilita el reconocimiento para cualquier persona en la red local. Incluso la información "inofensiva" como las rutas del sistema de archivos y la disponibilidad de SSH ayuda a los atacantes a mapear tu entorno.
...
2. **Deshabilitar por completo** si no necesitas el descubrimiento de dispositivos locales:
...
En modo mínimo, el Gateway todavía transmite lo suficiente para el descubrimiento de dispositivos (`role`, `gatewayPort`, `transport`) pero omite `cliPath` y `sshPort`. Las aplicaciones que necesitan información sobre la ruta de la CLI pueden obtenerla a través de la conexión WebSocket autenticada.
...
el Gateway rechaza las conexiones WebSocket (cierre seguro en caso de fallo).

La incorporación genera un token de forma predeterminada (incluso para el bucle local), por lo que
...
Nota: `gateway.remote.token` / `.password` son fuentes de credenciales de cliente. Por sí solas **no** protegen el acceso WS local.
...
SecretRef y no resuelto, la resolución falla cerrada (sin enmascaramiento de reserva remota).
Opcional: fijar el TLS remoto con `gateway.remote.tlsFingerprint` al usar `wss://`.
El `ws://` en texto sin formato es solo de bucle local (loopback) de forma predeterminada. Para rutas de red privada confiables,
...
- El emparejamiento de dispositivos se aprueba automáticamente para conexiones **locales** (bucle local o la
  dirección tailnet propia del host de la puerta de enlace) para mantener los clientes del mismo host fluidos.
...
- `gateway.auth.mode: "trusted-proxy"`: confiar en un proxy inverso con reconocimiento de identidad para autenticar usuarios y pasar la identidad a través de encabezados (consulte [Trusted Proxy Auth](/es/gateway/trusted-proxy-auth)).
...
y comparándola con el encabezado. Esto solo se activa para las solicitudes que alcanzan el loopback
...
- Trate las credenciales que puedan llamar a `/v1/chat/completions`, `/v1/responses`, `/tools/invoke` o `/api/channels/*` como secretos de operador de acceso completo para ese gateway.
- No comparta estas credenciales con llamadores no confiables; prefiera gateways separados por límite de confianza.
...
no confiable puede ejecutarse en el host del gateway, desactive `gateway.auth.allowTailscale`
...
termina TLS o utiliza un proxy frente al gateway, desactive
...
- Si termina TLS frente al Gateway, establezca `gateway.trustedProxies` a las IPs de su proxy.
...
- Tailscale Funnel para los puntos de conexión de control del navegador (exposición pública).

### 0.7) Secretos en disco (datos confidenciales)

Asuma que cualquier elemento en `~/.openclaw/` (o `$OPENCLAW_STATE_DIR/`) puede contener secretos o datos privados:
...
- `agents/<agentId>/agent/auth.json`: archivo de compatibilidad heredada. Las entradas `api_key` estáticas se eliminan cuando se descubren.
...
- Mantenga los permisos estrictos (`700` en los directorios, `600` en los archivos).
...
En los chats de grupo, responda solo cuando se le mencione explícitamente.
...
Es posible que agreguemos un solo indicador `readOnlyMode` más adelante para simplificar esta configuración.

Opciones adicionales de endurecimiento:

- `tools.exec.applyPatch.workspaceOnly: true` (predeterminado): asegura que `apply_patch` no pueda escribir/eliminar fuera del directorio del espacio de trabajo incluso cuando el sandboxing está desactivado. Establézcalo en `false` solo si intencionalmente desea que `apply_patch` toque archivos fuera del espacio de trabajo.
- `tools.fs.workspaceOnly: true` (opcional): restringe las rutas `read`/`write`/`edit`/`apply_patch` y las rutas de carga automática de imágenes de prompt nativo al directorio del espacio de trabajo (útil si hoy permites rutas absolutas y quieres una única barrera de protección).
- Mantenga los directorios raíz del sistema de archivos estrechos: evite directorios raíz amplios como su directorio de inicio para los espacios de trabajo del agente/espacios de trabajo de sandbox. Los directorios raíz amplios pueden exponer archivos locales confidenciales (por ejemplo, estado/configuración bajo `~/.openclaw`) a las herramientas del sistema de archivos.

### 5) Línea de base segura (copiar/pegar)

Una configuración de “valor predeterminado seguro” que mantiene el Gateway privado, requiere el emparejamiento por MD y evita los bots de grupo siempre activos:
...
- **Sandbox de herramientas** (`agents.defaults.sandbox`, host del gateway + herramientas aisladas por Docker): [Sandboxing](/es/gateway/sandboxing)
...
Importante: `tools.elevated` es la escotilla de escape de línea base global que ejecuta exec en el host. Mantenga `tools.elevated.allowFrom` estricta y no la habilite para extraños. Puede restringir aún más el acceso elevado por agente a través de `agents.list[].tools.elevated`. Consulte [Modo elevado](/es/tools/elevated).
...
Habilitar el control del navegador otorga al modelo la capacidad de manejar un navegador real.
...
### Política SSRF del navegador (predeterminado: red confiable)

La política de red del navegador de OpenClaw tiene como valor predeterminado el modelo de operador confiable: se permiten destinos privados/internos a menos que los deshabilite explícitamente.
...
úselo para dar **acceso completo**, **solo lectura** o **sin acceso** por agente.
...
3. **Congelar acceso:** cambie los MDs/grupos de riesgo a `dmPolicy: "disabled"` / requiera menciones, y elimine `"*"` entradas de permitir todo si las tenía.
...
- La(s) transcripción(es) de sesión + un registro corto de cola (después de redactar)
...
La CI ejecuta el enlace previo al commit `detect-secrets` en el trabajo `secrets`.
Las inserciones en `main` siempre ejecutan un escaneo de todos los archivos. Las solicitudes de extracción usan una ruta rápida de archivos modificados cuando hay una confirmación base disponible y recurren a un escaneo de todos los archivos en caso contrario. Si falla, hay nuevos candidatos que aún no están en la línea base.
...
2. Entender las herramientas:
   ...
   - `detect-secrets audit` abre una revisión interactiva para marcar cada elemento de la
     línea base como real o falso positivo.
...
5. Si necesita nuevas exclusiones, agrégalas a `.detect-secrets.cfg` y regenere la
   línea base con las banderas coincidentes `--exclude-files` / `--exclude-lines` (el archivo de
...
¿Ha encontrado una vulnerabilidad en OpenClaw? Por favor, infórmela de manera responsable:
