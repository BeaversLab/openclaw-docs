...
- `channel` opcional (cadena): El canal de mensajería para la entrega. Canales principales: `last`, `whatsapp`, `telegram`, `discord`, `slack`, `signal`, `imessage`, `irc`, `googlechat`, `line`. Canales de extensión (plugins): `msteams`, `mattermost` y otros. Por defecto es `last`.
- `to` opcional (cadena): El identificador del destinatario para el canal (p. ej., número de teléfono para WhatsApp/Signal, ID de chat para Telegram, ID de canal para Discord/Slack/Mattermost (plugin), ID de conversación para Microsoft Teams). Por defecto es el último destinatario en la sesión principal.
- `model` opcional (cadena): Sobrescritura del modelo (p. ej., `anthropic/claude-sonnet-4-6` o un alias). Debe estar en la lista de modelos permitidos si está restringido.
- `thinking` opcional (cadena): Sobrescritura del nivel de pensamiento (p. ej., `low`, `medium`, `high`).
...
Las sobrescrituras de carga útil `/hooks/agent` `sessionKey` están desactivadas por defecto.

- Recomendado: establezca una `hooks.defaultSessionKey` fija y mantenga las sobrescrituras de solicitud desactivadas.
...
Los nombres de hook personalizados se resuelven mediante `hooks.mappings` (ver configuración). Un mapeo puede
...
- `hooks.presets: ["gmail"]` activa el mapeo de Gmail integrado.
...
- `hooks.transformsDir` + `transform.module` carga un módulo JS/TS para lógica personalizada.
  ...
  - `transform.module` debe resolverse dentro del directorio de transformaciones efectivo (se rechazan las rutas de cruce/escapatoria).
- Use `match.source` para mantener un endpoint de ingesta genérico (enrutamiento basado en payload).
- Las transformaciones TS requieren un cargador TS (ej. `bun` o `tsx`) o `.js` precompilado en tiempo de ejecución.
- Establezca `deliver: true` + `channel`/`to` en las asignaciones para enrutar las respuestas a una superficie de chat
...
- `hooks.allowedAgentIds` restringe el enrutamiento explícito de `agentId`. Omítalo (o incluya `*`) para permitir cualquier agente. Establezca `[]` para denegar el enrutamiento explícito de `agentId`.
- `hooks.defaultSessionKey` establece la sesión predeterminada para las ejecuciones del agente de enlace cuando no se proporciona una clave explícita.
...
- `413` en caso de cargas útiles excesivamente grandes
...
Añada `model` a la carga útil del agente (o mapeo) para anular el modelo de esa ejecución:
...
- Prefiera un agente de enlace dedicado con `tools.profile` estrictos y sandbox para que la entrada del enlace tenga un radio de explosión más reducido.
...
- Si utiliza el enrutamiento multiagente, establezca `hooks.allowedAgentIds` para limitar la selección explícita de `agentId`.
- Mantenga `hooks.allowRequestSessionKey=false` a menos que requiera sesiones seleccionadas por quien realiza la llamada.
- Si habilita la `sessionKey` de la solicitud, restrinja `hooks.allowedSessionKeyPrefixes` (por ejemplo, `["hook:"]`).
