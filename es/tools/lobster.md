...
OpenClaw lanza la CLI local `lobster` en **modo de herramienta** y analiza un sobre JSON desde stdout.
Si la canalización se detiene para aprobación, la herramienta devuelve un `resumeToken` para que puedas continuar más tarde.
...
Para los flujos de trabajo que necesitan un **paso de LLM estructurado**, active la herramienta de complemento opcional
`llm-task` y llámela desde Lobster. Esto mantiene el flujo de trabajo
determinista mientras aún le permite clasificar/resumir/borrar con un modelo.
...
Lobster es una herramienta de complemento **opcional** (no activada de forma predeterminada).
...
Nota: las listas de permisos son optativas para los complementos opcionales. Si su lista de permisos solo nombra
herramientas de complemento (como `lobster`), OpenClaw mantiene las herramientas principales activadas. Para restringir las herramientas
principales, incluya también las herramientas principales o los grupos que desee en la lista de permisos.
...
Si está presente `requiresApproval`, inspeccione el indicador y decida:

- `approve: true` → reanudar y continuar los efectos secundarios
- `approve: false` → cancelar y finalizar el flujo de trabajo

Use `approve --preview-from-stdin --limit N` para adjuntar una vista previa JSON a las solicitudes de aprobación sin adhesivos jq/heredoc personalizados. Los tokens de reanudación ahora son compactos: Lobster almacena el estado de reanudación del flujo de trabajo en su directorio de estado y devuelve una pequeña clave de token.
...
- **Consciente del espacio aislado (sandbox)** — deshabilitado cuando el contexto de la herramienta está aislado.
