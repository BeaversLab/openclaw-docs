...
read_when:
  - Ajustar el análisis o los valores predeterminados de las directivas de pensamiento, modo rápido o detallado
...
4. Alternativa: `adaptive` para los modelos Anthropic Claude 4.6, `low` para otros modelos con capacidad de razonamiento, `off` en caso contrario.

## Establecer un valor predeterminado de sesión

- Envíe un mensaje que sea **solo** la directiva (se permiten espacios en blanco), p. ej. `/think:medium` o `/t high`.
- Esto se mantiene para la sesión actual (por remitente de forma predeterminada); se borra con `/think:off` o al restablecer la inactividad de la sesión.
- Se envía una respuesta de confirmación (`Thinking level set to high.` / `Thinking disabled.`). Si el nivel no es válido (ej. `/thinking big`), el comando se rechaza con una sugerencia y el estado de la sesión permanece sin cambios.
...
- Niveles: `on|off`.
- Un mensaje de solo directiva alterna una anulación del modo rápido de la sesión y responde `Fast mode enabled.` / `Fast mode disabled.`.
...
- OpenClaw resuelve el modo rápido en este orden:
  1. Solo en línea/solo directiva `/fast on|off`
  2. Invalidación de sesión
  3. Configuración por modelo: `agents.defaults.models["<provider>/<model>"].params.fastMode`
  4. Alternativa (fallback): `off`
- Para `openai/*`, el modo rápido aplica el perfil rápido de OpenAI: `service_tier=priority` cuando es compatible, además de bajo esfuerzo de razonamiento y baja verbosidad de texto.
- Para `openai-codex/*`, el modo rápido aplica el mismo perfil de baja latencia en las respuestas de Codex. OpenClaw mantiene un único interruptor compartido `/fast` en ambas rutas de autenticación.
- Para solicitudes directas con clave de API `anthropic/*`, el modo rápido se asigna a los niveles de servicio de Anthropic: `/fast on` establece `service_tier=auto`, `/fast off` establece `service_tier=standard_only`.
...
- Un mensaje de solo directiva alterna el modo detallado (verbose) de la sesión y responde `Verbose logging enabled.` / `Verbose logging disabled.`; los niveles no válidos devuelven una sugerencia sin cambiar el estado.
- `/verbose off` almacena una anulación explícita de la sesión; límpiela a través de la UI de Sesiones eligiendo `inherit`.
...
- Cuando verbose es `full`, las salidas de las herramientas también se reenvían después de la finalización (burbuja separada, truncada a una longitud segura). Si alternas `/verbose on|full|off` mientras una ejecución está en curso, las siguientes burbujas de herramientas respetan la nueva configuración.
...
- La entrega de latidos se limita únicamente a la carga útil final. Para también enviar el mensaje `Reasoning:` por separado (cuando esté disponible), establezca `agents.defaults.heartbeat.includeReasoning: true` o `agents.list[].heartbeat.includeReasoning: true` por agente.
