---
summary: "Plan: Añadir el endpoint /v1/responses de OpenResponses y deprecar las finalizaciones de chat de forma limpia"
read_when:
  - Diseñar o implementar el soporte de gateway `/v1/responses`
  - Planificar la migración desde la compatibilidad con Chat Completions
owner: "openclaw"
status: "draft"
...
Open Responses es un estándar de inferencia abierto basado en la API de Responses de OpenAI. Está diseñado
...
- Cambiar el comportamiento existente `/v1/chat/completions` durante la primera fase.

## Resumen de la Investigación

Fuentes: OpenAPI de OpenResponses, sitio de especificaciones de OpenResponses y la entrada de blog de Hugging Face.
...
- `ItemParam` es una unión discriminada de:
  ...
  - `reasoning`
  - `item_reference`
- Las respuestas exitosas devuelven un `ResponseResource` con `object: "response"`, `status` y
  `output`.
...
- Mantener `gateway.http.endpoints.chatCompletions.enabled` independiente; permitir que ambos puntos finales se
...
- Mantener límites estrictos de módulos: sin tipos de esquemas compartidos entre responses y chat completions.
...
- Paso futuro opcional: mapear las solicitudes de Chat Completions al controlador Responses para una ruta
  de eliminación más sencilla.
...
- Devolver `usage` con valores en cero hasta que se conecte el conteo de tokens.
...
- Implementar esquemas Zod para el subconjunto compatible de:
  ...
  - `ResponseResource`
  - Formas de eventos de streaming utilizados por la puerta de enlace
...
- Añadir cobertura e2e para `/v1/responses`:
  ...
  - Ordenación de eventos de streaming y `[DONE]`
  - Enrutamiento de sesión con cabeceras y `user`
- Mantener `src/gateway/openai-http.test.ts` sin cambios.
- Manual: curl a `/v1/responses` con `stream: true` y verificar la ordenación de eventos y el terminal
  `[DONE]`.
...
- Actualizar `/gateway/openai-http-api` con una nota de legado y un puntero a `/v1/responses`.
