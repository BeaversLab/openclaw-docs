---
summary: "Talk mode: conversaciones de voz continua con ElevenLabs TTS"
read_when:
...
El modo Talk es un bucle de conversación de voz continua:
...
- Transiciones de fase **Listening → Thinking → Speaking** (Escuchando → Pensando → Hablando).
...
- `modelId`: por defecto es `eleven_v3` si no se establece
...
- Pestaña Configuración: grupo **Modo Talk** (id de voz + interruptor de interrupción)
- Superposición:
...
- El TTS usa la API de streaming de ElevenLabs con `ELEVENLABS_API_KEY` y reproducción incremental en macOS/iOS/Android para una menor latencia.
...
- Android admite formatos de salida `pcm_16000`, `pcm_22050`, `pcm_24000` y `pcm_44100` para streaming de AudioTrack de baja latencia.
