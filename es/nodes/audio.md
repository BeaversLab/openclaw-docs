...
- **Comprensión de medios (audio)**: Si la comprensión de audio está habilitada (o se detecta automáticamente), OpenClaw:
  ...
  2. Enforces `maxBytes` antes de enviar a cada entrada de modelo.
...
- **Análisis de comandos**: Cuando la transcripción tiene éxito, `CommandBody`/`RawBody` se establecen en la transcripción para que los comandos de barra sigan funcionando.
...
- El límite de tamaño predeterminado es 20MB (`tools.media.audio.maxBytes`). Los archivos de audio que excedan este tamaño se omiten para ese modelo y se intenta con la siguiente entrada.
- Los archivos de audio pequeños/vacíos de menos de 1024 bytes se omiten antes de la transcripción del proveedor/CLI.
- El valor predeterminado de `maxChars` para audio está **sin establecer** (transcripción completa). Establezca `tools.media.audio.maxChars` o el `maxChars` por entrada para recortar la salida.
- El valor predeterminado de auto de OpenAI es `gpt-4o-mini-transcribe`; establezca `model: "gpt-4o-transcribe"` para mayor precisión.
...
Cuando `requireMention: true` está establecido para un chat de grupo, OpenClaw ahora transcribe el audio **antes** de verificar las menciones. Esto permite que las notas de voz se procesen incluso cuando contienen menciones.
...
2. La transcripción se verifica en busca de patrones de mención (por ejemplo, `@BotName`, disparadores de emoji).
...
- Establezca `channels.telegram.groups.<chatId>.disableAudioPreflight: true` para omitir las verificaciones de mención de transcripción previas para ese grupo.
...
- El valor predeterminado es `false` (previa activada cuando las condiciones de restricción por mención coinciden).
...
- Las reglas de alcance utilizan el primer elemento coincidente. `chatType` se normaliza a `direct`, `group` o `room`.
...
- Mantén los tiempos de espera razonables (`timeoutSeconds`, 60 s por defecto) para evitar bloquear la cola de respuestas.
