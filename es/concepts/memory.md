...
read_when:
  - Deseas conocer el diseño y el flujo de trabajo de los archivos de memoria
  - Deseas ajustar el vaciado automático de memoria previo a la compactación
---
...
- `MEMORY.md` (opcional)
  ...
  - `memory.md` en minúsculas solo se usa como respaldo cuando `MEMORY.md` está ausente.
...
- Esta área sigue evolucionando. Ayuda recordar al modelo que almacene recuerdos; sabrá qué hacer.
- Si quieres que algo se quede, **pídele al bot que lo escriba** en la memoria.

## Volcado de memoria automático (ping de precompactación)

Cuando una sesión está **cerca de la autocompactación**, OpenClaw activa un **turno
agente silencioso** que recuerda al modelo escribir una memoria duradera **antes** de que
se compacte el contexto. Los avisos predeterminados indican explícitamente que el modelo _puede responder_,
...
- **Umbral suave**: el volcado se activa cuando la estimación de tokens de la sesión supera
  `contextWindow - reserveTokensFloor - softThresholdTokens`.
- **Silencioso** de forma predeterminada: los avisos incluyen `NO_REPLY` por lo que no se entrega nada.
...
- **El espacio de trabajo debe ser escribible**: si la sesión se ejecuta en modo sandbox con
  `workspaceAccess: "ro"` o `"none"`, se omite el volcado.
...
las consultas semánticas puedan encontrar notas relacionadas incluso cuando la redacción difiera.

Valores predeterminados:

- Habilitado de forma predeterminada.
- Vigila los archivos de memoria en busca de cambios (con rebote).
- Configura la búsqueda de memoria en `agents.defaults.memorySearch` (no en el nivel superior
  `memorySearch`).
- Utiliza incrustaciones (embeddings) remotas de forma predeterminada. Si `memorySearch.provider` no está configurado, OpenClaw selecciona automáticamente:
  ...
  6. De lo contrario, la búsqueda de memoria permanece desactivada hasta que se configure.
...
- Instale la CLI de QMD por separado (`bun install -g https://github.com/tobi/qmd` u obtenga
  una versión) y asegúrese de que el binario `qmd` esté en el `PATH` de la puerta de enlace.
- QMD necesita una compilación de SQLite que permita extensiones (`brew install sqlite` en
  macOS).
- QMD se ejecuta completamente de forma local a través de Bun + `node-llama-cpp` y descarga automáticamente modelos GGUF
  de HuggingFace en el primer uso (no se requiere un demonio Ollama separado).
...
- La puerta de enlace escribe un directorio de inicio QMD autónomo bajo
...
- Las colecciones se crean mediante `qmd collection add` a partir de `memory.qmd.paths`
  (además de los archivos de memoria del área de trabajo predeterminados), luego `qmd update` + `qmd embed` se ejecutan
  al inicio y en un intervalo configurable (`memory.qmd.update.interval`,
  5 m de forma predeterminada).
- La puerta de enlace ahora inicializa el gestor QMD al iniciarse, por lo que los temporizadores de
  actualización periódica se activan incluso antes de la primera llamada `memory_search`.
- La actualización al inicio ahora se ejecuta en segundo plano de forma predeterminada para que el inicio del chat no se
  bloquee; establezca `memory.qmd.update.waitForBootSync = true` para mantener el comportamiento
  de bloqueo anterior.
- Las búsquedas se ejecutan mediante `memory.qmd.searchMode` (`qmd search --json` de forma predeterminada; también
  admite `vsearch` y `query`). Si el modo seleccionado rechaza indicadores en su
  compilación de QMD, OpenClaw vuelve a intentarlo con `qmd query`. Si QMD falla o el ejecutable
  falta, OpenClaw vuelve automáticamente al gestor SQLite integrado para que
  las herramientas de memoria sigan funcionando.
- OpenClaw no expone hoy el ajuste del tamaño de lote de incrustación de QMD; el comportamiento por lotes está
  controlado por el propio QMD.
- **La primera búsqueda puede ser lenta**: QMD puede descargar modelos GGUF locales (reranker/expansión de
  ...
  - Si desea descargar manualmente los modelos por adelantado (y calentar el mismo índice que OpenClaw
    utiliza), ejecute una consulta única con los directorios XDG del agente.

    El estado QMD de OpenClaw se encuentra en su **directorio de estado** (de forma predeterminada `~/.openclaw`).
    Puede hacer que `qmd` apunte exactamente al mismo índice exportando las mismas variables XDG
    que usa OpenClaw:
...
- `command` (`qmd` de forma predeterminada): anula la ruta del ejecutable.
- `searchMode` (`search` de forma predeterminada): elige qué comando QMD respalda
  `memory_search` (`search`, `vsearch`, `query`).
- `includeDefaultMemory` (predeterminado `true`): índice automático `MEMORY.md` + `memory/**/*.md`.
- `paths[]`: añade directorios/archivos adicionales (`path`, opcional `pattern`, opcional
  estable `name`).
- `sessions`: opta por la indexación JSONL de sesión (`enabled`, `retentionDays`,
  `exportDir`).
- `update`: controla el ritmo de actualización y la ejecución del mantenimiento:
  (`interval`, `debounceMs`, `onBoot`, `waitForBootSync`, `embedInterval`,
  `commandTimeoutMs`, `updateTimeoutMs`, `embedTimeoutMs`).
- `limits`: limita la carga de recuperación (`maxResults`, `maxSnippetChars`,
  `maxInjectedChars`, `timeoutMs`).
- `scope`: mismo esquema que [`session.sendPolicy`](/es/gateway/configuration#session).
  Lo predeterminado es solo MD (`deny` todos, `allow` chats directos); relájalo para mostrar resultados QMD
  en grupos/canales.
  - `match.keyPrefix` coincide con la clave de sesión **normalizada** (en minúsculas, sin cualquier
    `agent:<id>:` inicial). Ejemplo: `discord:channel:`.
  ...
  - Legado: `match.keyPrefix: "agent:..."` todavía se trata como un prefijo de clave sin procesar,
...
- Los fragmentos de `memory_search` ahora incluyen un pie de página `Source: <path#line>` cuando
  ...
  `memory_get`, pero el texto del fragmento omite el pie de página y el mensaje del sistema
...
Si desea indexar archivos Markdown fuera de la estructura predeterminada del espacio de trabajo, agregue
rutas explícitas:
...
- Los directorios se escanean de forma recursiva buscando archivos `.md`.
- De forma predeterminada, solo se indexan los archivos Markdown.
- Si `memorySearch.multimodal.enabled = true`, OpenClaw también indexa archivos de imagen/audio compatibles bajo `extraPaths` solamente. Las raíces de memoria predeterminadas (`MEMORY.md`, `memory.md`, `memory/**/*.md`) siguen siendo solo Markdown.
- Se ignoran los enlaces simbólicos (archivos o directorios).

### Archivos de memoria multimodal (imagen Gemini + audio)

OpenClaw puede indexar archivos de imagen y audio desde `memorySearch.extraPaths` cuando se utiliza Gemini embedding 2:
...
- Actualmente, la memoria multimodal solo es compatible con `gemini-embedding-2-preview`.
...
- `memory_get` todavía solo lee Markdown; los archivos binarios se pueden buscar, pero no se devuelven como contenido de archivo sin procesar.

### Gemini embeddings (nativo)
...
> **⚠️ Reindexación requerida:** Cambiar de `gemini-embedding-001` (768 dimensiones)
...
puede usar la configuración `remote` con el proveedor de OpenAI:
...
Si no desea establecer una clave API, use `memorySearch.provider = "local"` o establezca
...
- Desactivado de forma predeterminada. Establezca `agents.defaults.memorySearch.remote.batch.enabled = true` para habilitar para la indexación de corpus grandes (OpenAI, Gemini y Voyage).
- El comportamiento predeterminado espera a la finalización del lote; ajuste `remote.batch.wait`, `remote.batch.pollIntervalMs` y `remote.batch.timeoutMinutes` si es necesario.
...
- Los trabajos por lotes de Gemini usan el endpoint de asíncrono de incrustaciones por lotes y requieren la disponibilidad de la Gemini Batch API.

Por qué el lote de OpenAI es rápido y barato:

- Para rellenados masivos, OpenAI suele ser la opción más rápida que admitimos porque podemos enviar muchas solicitudes de incrustación en un solo trabajo por lotes y dejar que OpenAI las procese de forma asíncrona.
...
- `memory_search` — devuelve fragmentos con rangos de archivo y línea.
- `memory_get` — lee el contenido del archivo de memoria por ruta.

Modo local:

- Establezca `agents.defaults.memorySearch.provider = "local"`.
- Proporcione `agents.defaults.memorySearch.local.modelPath` (GGUF o URI de `hf:`).
- Opcional: establezca `agents.defaults.memorySearch.fallback = "none"` para evitar la reserva remota.

### Cómo funcionan las herramientas de memoria

- `memory_search` busca semánticamente fragmentos de Markdown (~400 tokens objetivo, 80 tokens de solapamiento) de `MEMORY.md` + `memory/**/*.md`. Devuelve texto de fragmento (limitado a ~700 caracteres), ruta de archivo, rango de líneas, puntuación, proveedor/modelo y si se ha producido una reversión de incrustaciones locales a remotas. No se devuelve la carga útil completa del archivo.
...
- Almacenamiento del índice: SQLite por agente en `~/.openclaw/memory/<agentId>.sqlite` (configurable vía `agents.defaults.memorySearch.store.path`, admite token `{agentId}`).
- Actualización: un observador en `MEMORY.md` + `memory/` marca el índice como obsoleto (anti-rebote de 1.5 s). La sincronización se programa al iniciar la sesión, al buscar o en un intervalo y se ejecuta de forma asíncrona. Las transcripciones de sesión utilizan umbrales delta para activar la sincronización en segundo plano.
- Disparadores de reindexación: el índice almacena el **proveedor/modelo de incrustación + huella del endpoint + parámetros de fragmentación**. Si alguno de ellos cambia, OpenClaw restablece y reindexa automáticamente todo el almacén.

### Búsqueda híbrida (BM25 + vector)

Cuando está habilitada, OpenClaw combina:

- **Similitud de vectores** (coincidencia semántica, la redacción puede variar)
- **Relevancia de palabras clave BM25** (tokens exactos como IDs, variables de entorno, símbolos de código)

Si la búsqueda de texto completo no está disponible en su plataforma, OpenClaw recurriremos a la búsqueda solo por vectores.

#### ¿Por qué híbrida?

La búsqueda vectorial es excelente en "esto significa lo mismo":

- "Mac Studio gateway host" frente a "la máquina que ejecuta la puerta de enlace"
...
BM25 (texto completo) es lo contrario: fuerte en tokens exactos, más débil en paráfrasis.
La búsqueda híbrida es el término medio pragmático: **usar ambas señales de recuperación** para que obtengas
...
- **Vector**: los mejores `maxResults * candidateMultiplier` por similitud de coseno.
- **BM25**: los mejores `maxResults * candidateMultiplier` por rango BM25 de FTS5 (cuanto menor, mejor).

2. Convertir el rango BM25 en una puntuación de 0..1 aprox.:

- `textScore = 1 / (1 + max(0, bm25Rank))`

3. Unir los candidatos por ID de fragmento y calcular una puntuación ponderada:
...
Esto no es "perfecto según la teoría de RI", pero es simple, rápido y tiende a mejorar la recuperación/precisión en notas reales.
Si queremos complicarnos más adelante, los siguientes pasos comunes son la Reciprocal Rank Fusion (RRF) o la normalización de puntuaciones
...
2. El MMR selecciona iterativamente resultados que maximizan: `λ × relevance − (1−λ) × max_similarity_to_selected`.
...
Los agentes con notas diarias acumulan cientos de archivos con fecha a lo largo del tiempo. Sin decaimiento,
una nota bien redactada de hace seis meses puede superar a la actualización de ayer sobre el mismo tema.

El **decaimiento temporal** aplica un multiplicador exponencial a las puntuaciones basado en la antigüedad de cada resultado,
por lo que los recuerdos recientes se clasifican naturalmente más alto mientras los antiguos se desvanecen:
...
Los **archivos diarios con fecha** (`memory/YYYY-MM-DD.md`) usan la fecha extraída del nombre del archivo.
...
**Cuándo habilitar:** Si su agente tiene meses de notas diarias y descubre que la información antigua y obsoleta tiene una clasificación superior a la del contexto reciente. Una vida media de 30 días funciona bien para flujos de trabajo con muchas notas diarias; aumente (por ejemplo, a 90 días) si consulta notas antiguas con frecuencia.
...
- **Solo decadencia temporal** — útil cuando la recenticidad es importante pero los resultados ya son diversos.
...
OpenClaw puede almacenar en caché las **incrustaciones de fragmentos** (chunk embeddings) en SQLite para que la reindexación y las actualizaciones frecuentes (especialmente las transcripciones de sesión) no vuelvan a incrustar el texto sin cambios.
...
Esto está limitado por una bandera experimental.
...
- La indexación de sesiones es **opcional** (desactivada por defecto).
- Las actualizaciones de sesión se aplican con retraso (debounced) y se **indexan de forma asincrónica** una vez que cruzan los umbrales de delta (mejor esfuerzo).
- `memory_search` nunca bloquea durante la indexación; los resultados pueden estar ligeramente desactualizados hasta que finalice la sincronización en segundo plano.
- Los resultados todavía incluyen solo fragmentos; `memory_get` sigue limitado a los archivos de memoria.
- La indexación de sesiones está aislada por agente (solo se indexan los registros de sesión de ese agente).
- Los registros de sesión residen en el disco (`~/.openclaw/agents/<agentId>/sessions/*.jsonl`). Cualquier proceso/usuario con acceso al sistema de archivos puede leerlos, así que trate el acceso al disco como el límite de confianza. Para un aislamiento más estricto, ejecute agentes con usuarios o sistemas operativos separados.
...
- `enabled` se establece de forma predeterminada en true; cuando se deshabilita, la búsqueda recurre a la
  similitud de coseno en proceso sobre las incrustaciones almacenadas.
- Si falta la extensión sqlite-vec o falla al cargarse, OpenClaw registra el
...
- Requisito de compilación nativa: ejecute `pnpm approve-builds`, seleccione `node-llama-cpp` y luego `pnpm rebuild node-llama-cpp`.
...
- `remote.headers` se fusionan con los encabezados de OpenAI; los remotos ganan en caso de conflicto de claves. Omita `remote.headers` para usar los valores predeterminados de OpenAI.
