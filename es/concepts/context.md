...
## Skills: lo que se inyecta frente a lo que se carga bajo demanda
...
Por defecto, OpenClaw utiliza el motor de contexto `legacy` integrado para el ensamblaje y
la compactación. Si instala un complemento que proporcione `kind: "context-engine"` y
lo selecciona con `plugins.slots.contextEngine`, OpenClaw delega el ensamblaje del contexto,
...
- `System prompt (run)` = capturado de la última ejecución integrada (con capacidad de herramientas) y persistido en el almacén de la sesión.
