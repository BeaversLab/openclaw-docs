---
summary: "Herramientas de sesión de agente para listar sesiones, obtener historial y enviar mensajes entre sesiones"
read_when:
  - Agregar o modificar herramientas de sesión
...
- El cubo de chat directo principal es siempre la clave literal `"main"` (resuelta a la clave principal del agente actual).
...
`global` y `unknown` son valores reservados y nunca se listan. Si `session.scope = "global"`, lo convertimos en un alias de `main` para todas las herramientas, de modo que las llamadas nunca vean `global`.
...
- `limit?: number` filas máximas (predeterminado: predeterminado del servidor, límite p. ej. 200)
...
- Los resultados de las herramientas se filtran en la salida de la lista; use `sessions_history` para mensajes de herramientas.
...
- `lastChannel`, `lastTo`
- `deliveryContext` (normalizado `{ channel, to, accountId }` cuando esté disponible)
...
- `limit?: number` máx. de mensajes (el servidor lo limita)
...
- Cuando se proporciona un `sessionId`, OpenClaw lo resuelve a la clave de sesión correspondiente (error de ids faltantes).
...
- `sessionKey` (obligatorio; acepta la clave de sesión o `sessionId` de `sessions_list`)
- `message` (obligatorio)
- `timeoutSeconds?: number` (por defecto >0; 0 = fuego y olvido)
...
- Si se agota el tiempo de espera: `{ runId, status: "timeout", error }`. La ejecución continúa; llame a `sessions_history` más tarde.
- Si falla la ejecución: `{ runId, status: "error", error }`.
- Las ejecuciones de entrega de anuncios se producen después de que se completa la ejecución principal y son de mejor esfuerzo; `status: "ok"` no garantiza que se haya entregado el anuncio.
- Las esperas se realizan a través de la pasarela `agent.wait` (lado del servidor) para que las reconexiones no interrumpan la espera.
- El contexto del mensaje de agente a agente se inyecta para la ejecución principal.
- Los mensajes entre sesiones se conservan con `message.provenance.kind = "inter_session"` para que los lectores de transcripciones puedan distinguir las instrucciones del agente enrutadas de la entrada externa del usuario.
- Después de que se completa la ejecución principal, OpenClaw ejecuta un **bucle de respuesta (reply-back loop)**:
  ...
  - El número máximo de turnos es `session.agentToAgent.maxPingPongTurns` (0–5, por defecto 5).
...
- Para los grupos, `channel` es el canal registrado en la entrada de sesión.
- Para los chats directos, `channel` se asigna desde `lastChannel`.
...
- `sendPolicy: "allow" | "deny"` (sin configurar = heredar configuración)
- Configurable mediante `sessions.patch` o solo propietario `/send on|off|inherit` (mensaje independiente).

Puntos de aplicación:

- `chat.send` / `agent` (pasarela)
...
- `agentId?` (opcional; genera en otro id. de agente si está permitido)
- `model?` (opcional; anula el modelo de subagente; valores no válidos causan error)
- `thinking?` (opcional; anula el nivel de pensamiento para la ejecución del subagente)
- `runTimeoutSeconds?` (por defecto es `agents.defaults.subagents.runTimeoutSeconds` cuando se establece, de lo contrario `0`; cuando se establece, aborta la ejecución del subagente después de N segundos)
- `thread?` (por defecto falso; solicita enrutamiento ligado a hilo para esta generación cuando es compatible con el canal/complemento)
- `mode?` (`run|session`; por defecto es `run`, pero por defecto es `session` cuando `thread=true`; `mode="session"` requiere `thread=true`)
- `cleanup?` (`delete|keep`, por defecto `keep`)
- `sandbox?` (`inherit|require`, por defecto `inherit`; `require` rechaza la generación a menos que el tiempo de ejecución del hijo objetivo esté en sandbox)
...
- `attachAs?` (opcional; sugerencia `{ mountPath? }` reservada para futuras implementaciones de montaje)

Lista de permitidos:

- `agents.list[].subagents.allowAgents`: lista de ids de agentes permitidos a través de `agentId` (`["*"]` para permitir cualquiera). Predeterminado: solo el agente solicitante.
...
- Los sub-agentes tienen como valor predeterminado el conjunto de herramientas completo **menos las herramientas de sesión** (configurable mediante `tools.subagents.tools`).
- A los sub-agentes no se les permite llamar a `sessions_spawn` (sin generación de sub-agente → sub-agente).
...
- Tras la finalización, OpenClaw ejecuta un **paso de anuncio** del subagente y publica el resultado en el canal de chat del solicitante.
  - Si la respuesta final del asistente está vacía, se incluye el último `toolResult` del historial del sub-agente como `Result`.
...
- `tools.sessions.visibility` tiene como valor predeterminado `tree` (sesión actual + sesiones de subagentes generadas).
- Para las sesiones en sandbox, `agents.defaults.sandbox.sessionToolsVisibility` puede limitar estrictamente la visibilidad.
...
- Cuando una sesión está en sandbox y `sessionToolsVisibility="spawned"`, OpenClaw limita la visibilidad a `tree` incluso si estableces `tools.sessions.visibility="all"`.
