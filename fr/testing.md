---
summary: "Testing kit: unit/e2e/live suites, Docker runners, et ce que chaque test couvre"
read_when:
  - Exécution des tests localement ou en CI
  - Ajout de régressions pour les bugs de modèle/fournisseur
  - Débogage du comportement de la passerelle + de l'agent
title: "Testing"
---
...
Conseil : lorsque vous avez besoin d'un seul cas échouant, préférez restreindre les tests live via les variables d'environnement de liste d'autorisation décrites ci-dessous.

## Test suites (ce qui s'exécute où)

Pensez aux suites comme à un "réalisme croissant" (et instabilité/coût croissants) :

### Unit / integration (défaut)
...
- Portée :
  ...
  - Tests d'intégration en cours de processus (auth de passerelle, routage, outilage, analyse, config)
...
- Portée :
  ...
  - Surfaces WebSocket/HTTP, appairage de nœuds et réseaux plus lourds
- Attentes :
...
- Portée :
  - « Est-ce que ce fournisseur/modèle fonctionne vraiment _aujourd'hui_ avec de vraies identifiants ? »
  - Détecter les changements de format du fournisseur, les singularités des appels d'outils, les problèmes d'authentification et le comportement des limites de taux
- Attentes :
  ...
  - Les exécutions en direct utiliseront le source de `~/.profile` pour récupérer les clés API manquantes
...
- « Test de fumée Gateway » nous indique si le pipeline complet Gateway+agent fonctionne pour ce modèle (sessions, historique, outils, stratégie de bac à sable, etc.).

### Couche 1 : Achèvement direct du modèle (sans Gateway)
...
- Détails des sondes (afin que vous puissiez expliquer rapidement les échecs) :
  ...
  - Sonde `exec+read` : le test demande à l'agent d'`exec`-écrire un nonce dans un fichier temporaire, puis de `read`.
...
- Objectif : vérifier que le setup-token CLI Claude Code (ou un profil setup-token collé) peut compléter une invite Anthropic.
- Activer :
  - `pnpm test:live` (ou `OPENCLAW_LIVE_TEST=1` si vous appelez Vitest directement)
  - `OPENCLAW_LIVE_SETUP_TOKEN=1`
- Sources de jetons (choisir une) :
...
## Live : test de fumée du backend CLI (CLI Claude Code ou autres CLI locaux)
...
- `OPENCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` pour garder la configuration MCP du CLI Claude Code activée (désactivée par défaut avec un fichier vide temporaire).
...
- API Gemini vs CLI Gemini :
  ...
  - CLI : OpenClaw exécute un binaire local `gemini` ; il possède sa propre authentification et peut se comporter différemment (streaming/support des outils/décalage de version).
...
Il s'agit de la série « modèles courants » que nous nous attendons à voir fonctionner :

- OpenAI (non-Codex) : `openai/gpt-5.2` (optionnel : `openai/gpt-5.1`)
- Codex OpenAI : `openai-codex/gpt-5.2` (optionnel : `openai-codex/gpt-5.2-codex`)
...
### Base : appel d'outil (Read + Exec optionnel)
...
Couverture supplémentaire optionnelle (nice to have) :

- xAI : `xai/grok-4` (ou dernière version disponible)
- Mistral : `mistral/`… (choisissez un model compatible « tools » que vous avez activé)
- Cerebras : `cerebras/`… (si vous y avez accès)
- LM Studio : `lmstudio/`… (local ; l'appel d'outil dépend du mode API)

### Vision : envoi d'image (pièce jointe → message multimodal)

Incluez au moins un model capable de gérer les images dans `OPENCLAW_LIVE_GATEWAY_MODELS` (variantes Claude/Gemini/OpenAI compatibles vision, etc.) pour tester la sonde d'image.

### Agrégateurs / passerelles alternatives

Si vous avez des clés activées, nous prenons également en charge les tests via :

- OpenRouter : `openrouter/...` (centaines de models ; utilisez `openclaw models scan` pour trouver les candidats compatibles outil+image)
...
- Si un test live indique « pas d'identifiants », débuggez de la même manière que vous débuggeriez `openclaw models list` / la sélection de modèle.

- Magasin de profils : `~/.openclaw/credentials/` (préféré ; signification de « clés de profil » dans les tests)
- Config : `~/.openclaw/openclaw.json` (ou `OPENCLAW_CONFIG_PATH`)

Si vous souhaitez vous fier aux clés d'environnement (par ex. exportées dans votre `~/.profile`), exécutez les tests locaux après `source ~/.profile`, ou utilisez les runners Docker ci-dessous (ils peuvent monter `~/.profile` dans le conteneur).
...
- Plugins (chargement d'extension personnalisé + smoke registry) : `pnpm test:docker:plugins` (script : `scripts/e2e/plugins-docker.sh`)
...
Exécutez les vérifications de la documentation après les modifications : `pnpm docs:list`.

## Régression hors ligne (sûr pour CI)

Ce sont des régressions de « pipeline réel » sans providers réels :

- Appel d'outil Gateway (mock OpenAI, vrai gateway + boucle d'agent) : `src/gateway/gateway.tool-calling.mock-openai.test.ts`
- Assistant Gateway (WS `wizard.start`/`wizard.next`, écriture config + auth appliquée) : `src/gateway/gateway.wizard.e2e.test.ts`
...
- Appel d'outil simulé via le vrai gateway + boucle d'agent (`src/gateway/gateway.tool-calling.mock-openai.test.ts`).
- Flux de bout en bout de l'assistant qui valident le câblage de session et les effets de configuration (`src/gateway/gateway.wizard.e2e.test.ts`).

Ce qui manque encore pour les Skills (voir [Skills](/fr/tools/skills)) :

- **Prise de décision :** lorsque des Skills sont listés dans le prompt, l'agent choisit-il le bon Skill (ou évite-t-il ceux non pertinents) ?
- **Conformité :** l'agent lit-il `SKILL.md` avant utilisation et suit-il les étapes/arguments requis ?
- **Contrats de workflow :** scénarios à plusieurs tours qui assertent l'ordre des outils, la continuité de l'historique de session et les limites du bac à sable.

Les évaluations futures doivent d'abord rester déterministes :

- Un exécuteur de scénario utilisant des providers simulés pour vérifier les appels d'outils + l'ordre, les lectures de fichiers de Skills et le câblage de session.
...
- Ajoutez une régression sûre pour CI si possible (provider simulé/bouchon, ou capturez la transformation exacte de la forme de la requête)
- S'il est intrinsèquement uniquement en direct (limites de débit, politiques d'auth), gardez le test en direct étroit et en opt-in via env vars
- Privilégiez le ciblage de la plus petite couche qui détecte le bug :
  ...
  - bug de pipeline session/historique/outils du gateway → test de fumée en direct gateway ou test simulé gateway sûr pour CI
