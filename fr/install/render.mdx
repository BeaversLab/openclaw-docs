...
Les Render Blueprints sont des fichiers YAML qui définissent votre infrastructure. Le `render.yaml` de ce
dépôt configure tout ce dont OpenClaw a besoin pour fonctionner :
...
Le Blueprint est par défaut réglé sur `starter`. Pour utiliser l'offre gratuite, modifiez `plan: free` dans le
...
Cela arrive sur l'offre gratuite (pas de disque persistant). Passez à un plan payant, ou
