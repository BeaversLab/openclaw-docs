...
<Steps>
  ...
  <Step title="S'assurer de la présence de Git">Installe Git s'il est manquant.</Step>
  <Step title="Installer OpenClaw">
    - `npm` méthode (par défaut) : installation globale npm - `git` méthode : cloner/mettre à jour
    le dépôt, installer les dépendances avec pnpm, construire, puis installer le wrapper à
    `~/.local/bin/openclaw`
  </Step>
  <Step title="Tâches post-installation">
    ...
    (au mieux) - Tente l'onboarding lorsque approprié (TTY disponible, onboarding non désactivé, et
    les vérifications bootstrap/config réussissent) - Défauts `SHARP_IGNORE_GLOBAL_LIBVIPS=1`
  </Step>
</Steps>

### Détection de l'extraction de source

S'il est exécuté dans une extraction OpenClaw (`package.json` + `pnpm-workspace.yaml`), le script propose :

- utiliser l'extraction (`git`), ou
- utiliser l'installation globale (`npm`)

Si aucune TTY n'est disponible et qu'aucune méthode d'installation n'est définie, il utilise par défaut `npm` et avertit.
...
<Info>
  Conçu pour les environnements où vous souhaitez tout dans un préfixe local (par défaut
...
<Steps>
  <Step title="Installer le runtime Node local">
    Télécharge une archive Node prise en charge épinglée (actuellement par défaut `22.22.0`) vers `<prefix>/tools/node-v<version>` et vérifie le SHA-256.
  </Step>
...
<Steps>
  <Step title="Assurer l'environnement PowerShell + Windows">Nécessite PowerShell 5+.</Step>
  ...
  <Step title="Installer OpenClaw">
    - `npm` méthode (par défaut) : installation OpenClaw globale en utilisant `-Tag` sélectionné -
    `git` méthode : clone/mise à jour du dépôt, installation/build avec pnpm, et installation du
...
<Accordion title="Pourquoi npm rencontre-t-il EACCES sur Linux ?">
  Certaines configurations npm pointent le préfixe global Linux vers des chemins détenus par root.
  `install.sh` peut changer le préfixe vers `~/.npm-global` et ajouter les exportations PATH aux
  fichiers rc du shell (lorsque ces fichiers existent).
</Accordion>
