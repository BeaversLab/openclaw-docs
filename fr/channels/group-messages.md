...
- WhatsApp envoie toujours des mentions canoniques via `mentionedJids` lorsque quelqu'un appuie sur le contact, le recours au numéro est donc rarement nécessaire, mais constitue un filet de sécurité utile.
...
Seul le numéro du propriétaire ( issu de `channels.whatsapp.allowFrom`, ou l'E.164 du bot lui-même s'il n'est pas défini) peut modifier cela. Envoyez `/status` comme un message autonome dans le groupe pour voir le mode d'activation actuel.
...
- Test manuel :
  - Envoyez un ping `@openclaw` dans le groupe et confirmez que la réponse mentionne le nom de l'expéditeur.
