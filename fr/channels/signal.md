...
Prise en charge multi-compte : utilisez `channels.signal.accounts` avec une configuration par compte et un `name` optionnel. Voir [`gateway/configuration`](/fr/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) pour le modèle partagé.
...
4. Configurez OpenClaw, redémarrez la passerelle, vérifiez le channel :
