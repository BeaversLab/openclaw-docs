---
summary: "Statut du support Matrix, capacités et configuration"
read_when:
...
Matrix est un protocole de messagerie ouvert et décentralisé. OpenClaw se connecte en tant qu'**utilisateur** Matrix sur n'importe quel serveur d'accueil (homeserver), vous devez donc disposer d'un compte Matrix pour le bot. Une fois connecté, vous pouvez envoyer un DM au bot directement ou l'inviter dans des salons (Matrix "groups"). Beeper est également une option client valide, mais cela nécessite que l'E2EE soit activé.

Statut : pris en charge via le plugin (@vector-im/matrix-bot-sdk). Messages directs, salons, fils de discussion, médias, réactions, sondages (envoi + démarrage de sondage en tant que texte), localisation et E2EE (avec support crypto).
...
3. Obtenir un jeton d'accès pour le compte du bot :
   - Utiliser l'API de connexion Matrix avec `curl` sur votre serveur personnel :
...
- Si le module de chiffrement ne peut pas être chargé, l'E2EE est désactivé et les salles chiffrées ne seront pas déchiffrées ;
  OpenClaw enregistre un avertissement.
- Si vous voyez des erreurs de module de chiffrement manquant (par exemple, `@matrix-org/matrix-sdk-crypto-nodejs-*`),
...
vérifié à nouveau pour les salles chiffrées.

**Vérification de l'appareil :**
Lorsque l'E2EE est activé, le bot demandera une vérification à vos autres sessions au démarrage.
Ouvrez Element (ou un autre client) et approuvez la demande de vérification pour établir la confiance.
Une fois vérifié, le bot peut déchiffrer les messages dans les salles chiffrées.
...
Chaque compte fonctionne en tant qu'utilisateur Matrix distinct sur n'importe quel serveur d'accueil. La configuration par compte hérite des paramètres `channels.matrix` de niveau supérieur et peut remplacer n'importe quelle option (stratégie de DM, groupes, chiffrement, etc.).
...
- `channels.matrix.dm.allowFrom` accepte les ID utilisateur complets Matrix (exemple : `@user:server`). L'assistant résout les noms d'affichage en ID utilisateur lorsque la recherche dans l'annuaire trouve une seule correspondance exacte.
- N'utilisez pas de noms d'affichage ou de parties locales nues (exemple : `"Alice"` ou `"alice"`). Ils sont ambigus et sont ignorés pour la correspondance de liste autorisée. Utilisez des ID `@user:server` complets.
...
- Note d'exécution : si `channels.matrix` est complètement manquant, l'exécution revient à `groupPolicy="allowlist"` pour les vérifications de salon (même si `channels.defaults.groupPolicy` est défini).
- Liste blanche des salons avec `channels.matrix.groups` (identifiants ou alias de salon ; les noms sont résolus en identifiants lorsque la recherche de répertoire trouve une seule correspondance exacte) :
...
- `groups."*"` peut définir les valeurs par défaut pour le filtrage par mention dans les salons.
- `groupAllowFrom` limite les expéditeurs pouvant déclencher le bot dans les salons (identifiants utilisateur Matrix complets).
- Les listes blanches `users` par salon peuvent restreindre davantage les expéditeurs dans un salon spécifique (utilisez les identifiants utilisateur Matrix complets).
...
- Les invitations sont acceptées automatiquement par défaut ; contrôlez cela avec `channels.matrix.autoJoin` et `channels.matrix.autoJoinAllowlist`.
- Pour n'autoriser **aucun salon**, définissez `channels.matrix.groupPolicy: "disabled"` (ou conservez une liste blanche vide).
...
- `channels.matrix.replyToMode` contrôle les métadonnées de réponse lorsque l'on ne répond pas dans un fil :
...
- Connecté mais messages de salle ignorés : salle bloquée par `groupPolicy` ou liste d'autorisation de salle.
...
- `channels.matrix.dm.policy` : `pairing | allowlist | open | disabled` (par défaut : jumelage).
- `channels.matrix.dm.allowFrom` : liste d'autorisation DM (ID d'utilisateur complets Matrix). `open` nécessite `"*"`. L'assistant résout les noms en ID lorsque cela est possible.
- `channels.matrix.groupPolicy` : `allowlist | open | disabled` (par défaut : liste d'autorisation).
- `channels.matrix.groupAllowFrom` : expéditeurs autorisés pour les messages de groupe (ID d'utilisateur complets Matrix).
- `channels.matrix.allowlistOnly` : forcer les règles de liste d'autorisation pour les DMs + salles.
- `channels.matrix.groups` : liste d'autorisation de groupe + carte des paramètres par salle.
- `channels.matrix.rooms` : liste d'autorisation/Configuration de groupe héritée.
- `channels.matrix.replyToMode` : mode de réponse pour les fils/discussions.
...
- `channels.matrix.autoJoinAllowlist` : IDs de salles/alias autorisés pour la jointure automatique.
