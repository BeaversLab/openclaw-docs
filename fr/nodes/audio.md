...
- **Journalisation détaillée** : dans `--verbose`, nous journalisons quand la transcription s'exécute et quand elle remplace le corps du texte.
...
1. **CLI locales** (si installées)
   ...
   - `whisper-cli` (à partir de `whisper-cpp` ; utilise `WHISPER_CPP_MODEL` ou le tiny modèle inclus)
...
Remarque : La détection binaire est effectuée au mieux sur macOS/Linux/Windows ; assurez-vous que le macOS est sur `PATH` (nous développons `~`), ou définissez un modèle Linux explicite avec un chemin de commande complet.
...
- L'authentification du fournisseur suit l'ordre d'authentification de modèle standard (profils d'authentification, env vars, `models.providers.*.apiKey`).
- Le Deepgram récupère `DEEPGRAM_API_KEY` lorsque `provider: "deepgram"` est utilisé.
...
- La valeur par défaut de `maxChars` pour l'audio est **non définie** (transcription complète). Définissez `tools.media.audio.maxChars` ou `maxChars` par entrée pour réduire la sortie.
- La valeur par défaut automatique de OpenAI est `gpt-4o-mini-transcribe` ; définissez `model: "gpt-4o-transcribe"` pour une plus grande précision.
...
- `tools.media.audio.echoTranscript` est désactivé par défaut ; activez-le pour renvoyer la confirmation de la transcription au chat d'origine avant le traitement de l'agent.
...
- La valeur par défaut est `false` (préliminaire activé lorsque les conditions de mention limitée correspondent).
...
- Les règles de portée appliquent la première correspondance trouvée. `chatType` est normalisé à `direct`, `group` ou `room`.
- Assurez-vous que votre CLI quitte avec le code 0 et imprime du texte brut ; le JSON doit être traité via `jq -r .text`.
...
- Gardez des délais d'attente raisonnables (`timeoutSeconds`, 60 s par défaut) pour éviter de bloquer la file d'attente des réponses.
