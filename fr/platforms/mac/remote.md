...
Ce flux permet à l'application macOS d'agir comme une télécommande complète pour une passerelle OpenClaw s'exécutant sur un autre hôte (bureau/serveur). C'est la fonctionnalité **Remote over SSH** (exécution à distance) de l'application. Toutes les fonctionnalités — les vérifications de santé, le transfert Voice Wake et Web Chat — réutilisent la même configuration SSH distante depuis _Settings → General_.
