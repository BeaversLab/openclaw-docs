...
- **Microsoft** (provider principal ou de secours ; l'implémentation groupée actuelle utilise `node-edge-tts`, par défaut lorsqu'il n'y a pas de clés API)
...
TTS neuronal en ligne de Microsoft Edge via la bibliothèque `node-edge-tts`. C'est un service hébergé (non
...
toutes les options ne sont pas prises en charge par le service. La configuration héritée et l'entrée de directive
utilisant `edge` fonctionnent toujours et sont normalisées en `microsoft`.

Comme cette voie est un service Web public sans SLA ni quota publiés,
considérez-la comme étant au mieux effort. Si vous avez besoin de limites garanties et d'un support, utilisez OpenAI
ou ElevenLabs.
...
Non. La synthèse vocale automatique (Auto‑TTS) est **désactivée** par défaut. Activez-la dans la configuration avec
...
lorsqu'aucune clé API OpenAI ou ElevenLabs API n'est disponible.

## Config

La configuration TTS se trouve sous `messages.tts` dans `openclaw.json`.
Le schéma complet se trouve dans [Gateway configuration](/fr/gateway/configuration).
...
- `auto` : mode TTS automatique (`off`, `always`, `inbound`, `tagged`).
  ...
  - `tagged` n'envoie de l'audio que si la réponse inclut des balises `[[tts]]`.
- `enabled` : bascule obsolète (doctor migre cela vers `auto`).
- `mode` : `"final"` (par défaut) ou `"all"` (inclut les réponses tool/bloc).
- `provider` : id du provider vocal tel que `"elevenlabs"`, `"microsoft"`, ou `"openai"` (le repli est automatique).
...
- `summaryModel` : model peu coûteux optionnel pour le résumé automatique ; par défaut `agents.defaults.model.primary`.
...
- `microsoft.rate` / `microsoft.pitch` / `microsoft.volume` : chaînes de pourcentage (par exemple `+10%`, `-5%`).
...
- `microsoft.timeoutMs` : substitution du délai de requête (ms).
...
- `provider` (id de fournisseur de parole enregistré, par exemple `openai`, `elevenlabs` ou `microsoft` ; nécessite `allowProvider: true`)
...
Liste d'autorisation optionnelle (permet le changement de fournisseur tout en gardant les autres paramètres configurables) :
...
- **Telegram** : Note vocale Opus (`opus_48000_64` d'ElevenLabs, `opus` de OpenAI).
  - 48 kHz / 64 kbps est un bon compromis pour les notes vocales et est requis pour la bulle ronde.
...
- **Microsoft** : utilise `microsoft.outputFormat` (`audio-24khz-48kbitrate-mono-mp3` par défaut).
  ...
  - Telegram `sendVoice` accepte OGG/MP3/M4A ; utilisez OpenAI/ElevenLabs si vous avez besoin
    de notes vocales Opus garanties.
  - Si le format de sortie Microsoft configuré échoue, OpenClaw réessaie avec MP3.

Les formats OpenAI/ElevenLabs sont fixes ; Telegram attend de l'Opus pour l'UX des notes vocales.
...
- `/tts audio` génère une réponse audio ponctuelle (n'active pas la synthèse vocale).
