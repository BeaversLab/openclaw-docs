...
- `--remote-url <url>` : URL de la passerelle distante.
- `--remote-token <token>` : jeton de la passerelle distante.

L'assistant s'exécute automatiquement lorsque des indicateurs d'assistant sont présents (`--non-interactive`, `--mode`, `--remote-url`, `--remote-token`).

### `onboard`

Assistant interactif pour configurer la passerelle, l'espace de travail et les compétences.
...
- `--secret-input-mode <plaintext|ref>` (par défaut `plaintext` ; utiliser `ref` pour stocker les références d'environnement par défaut du provider au lieu des clés en texte clair)
...
- `--gateway-token <token>`
- `--gateway-token-ref-env <name>` (non-interactif ; stocker `gateway.auth.token` en tant que SecretRef d'env var ; nécessite que cette env var soit définie ; ne peut pas être combiné avec `--gateway-token`)
...
Contrôles de santé + corrections rapides (config + passerelle + services hérités).
...
- Conseil : `channels status` affiche des avertissements avec des corrections suggérées lorsqu'il peut détecter des erreurs de configuration courantes (puis vous redirige vers `openclaw doctor`).
- `channels logs` : afficher les journaux de canal récents à partir du fichier journal de la passerelle.
- `channels add` : configuration de type assistant si aucun indicateur n'est passé ; les indicateurs basculent en mode non interactif.
  - Lors de l'ajout d'un compte non par défaut à un canal utilisant toujours une configuration de niveau supérieur à compte unique, OpenClaw déplace les valeurs limitées au compte dans `channels.<channel>.accounts.default` avant d'écrire le nouveau compte.
  - Le `channels add` non interactif ne crée/met pas à jour automatiquement les liaisons ; les liaisons limitées au canal continuent de correspondre au compte par défaut.
...
- `--account <id>` : identifiant du compte channel (par défaut `default`)
...
- `--no-usage` : ignorer les instantanés d'utilisation/quota du provider de modèle (uniquement avec OAuth/API).
...
Gérer les entrées de jumelage d'appareils passerelle et les jetons d'appareil par rôle.
...
- `webhooks gmail run` (remplacements lors de l'exécution pour les mêmes indicateurs)
...
Les spécifications de liaison utilisent `channel[:accountId]`. Lorsque `accountId` est omis, OpenClaw peut résoudre la portée du compte via les hooks de plug-in/les valeurs par défaut du channel ; sinon, il s'agit d'une liaison de channel sans portée de compte explicite.
...
- `--usage` (afficher l'utilisation/le quota du provider de modèle)
- `--timeout <ms>`
- `--verbose`
...
- La vue d'ensemble inclut l'état du Gateway et du service hôte du nœud lorsque disponibles.
...
- `openclaw status --usage` (imprime la répartition complète du provider)
...
- Providers : Anthropic, GitHub Copilot, OpenAI Codex OAuth, plus Gemini CLI/Antigravity lorsque ces plugins de provider sont activés.
...
Désinstaller le service de passerelle + les données locales (CLI reste installé).
...
- `--allow-unconfigured`
- `--dev`
- `--reset` (réinitialiser la configuration de développement + les identifiants + les sessions + l'espace de travail)
...
Gérer le service de Gateway (launchd/systemd/schtasks).
...
Remarque sur la politique : il s'agit d'une compatibilité technique. Anthropic a bloqué par le passé certaines utilisations d'abonnement en dehors de Claude Code ; vérifiez les conditions actuelles d'Anthropic avant de vous fier au setup-token en production.
...
`node` exécute un **hôte de nœud sans interface (headless)** ou le gère en tant que service d'arrière-plan. Voir
