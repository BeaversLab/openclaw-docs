---
summary: "Utilisez OpenAI via les clés API ou l'abonnement Codex dans OpenClaw"
read_when:
  - Vous souhaitez utiliser les modèles OpenAI dans OpenClaw
  - Vous souhaitez utiliser l'authentification par abonnement Codex au lieu des clés API
title: "OpenAI"
---

# OpenAI

OpenAI fournit des API de développeur pour les modèles GPT. Codex prend en charge la **connexion ChatGPT** pour l'accès par abonnement
ou la connexion par **clé API** pour l'accès à l'utilisation. Codex cloud nécessite une connexion ChatGPT.
...
La documentation actuelle des modèles OpenAI de l'API répertorie `gpt-5.4` et `gpt-5.4-pro` pour l'utilisation directe de l'OpenAI API. OpenClaw transfère les deux via le chemin Réponses `openai/*`.
OpenClaw supprime intentionnellement la ligne obsolète `openai/gpt-5.3-codex-spark`,
car les appels directs à l'OpenAI API la rejettent dans le trafic en direct.

OpenClaw n'expose **pas** `openai/gpt-5.3-codex-spark` sur le chemin direct de l'OpenAI API.
`pi-ai` inclut toujours une ligne intégrée pour ce modèle, mais les requêtes actuelles à l'OpenAI API
la rejettent actuellement. Spark est traité comme exclusif à Codex dans OpenClaw.

## Option B : abonnement OpenAI Code (Codex)

**Idéal pour :** utiliser l'accès par abonnement ChatGPT/Codex au lieu d'une clé API.
Codex cloud nécessite une connexion ChatGPT, tandis que la CLI Codex prend en charge la connexion ChatGPT ou par clé API.
...
La documentation actuelle de Codex de l'OpenAI répertorie `gpt-5.4` comme modèle Codex actuel. OpenClaw
le mappe à `openai-codex/gpt-5.4` pour l'utilisation de l'OAuth ChatGPT/Codex.
...
par clé API pour `openai/gpt-5.3-codex-spark`.
...
séparé de GPT-5.4 `/fast`, et sa disponibilité dépend du compte Codex /
ChatGPT connecté.

### Transport par défaut

OpenClaw utilise `pi-ai` pour le streaming de model. Pour `openai/*` et
...
OpenClaw, définissez `agents.defaults.models["openai/<model>"].params.serviceTier` pour
transmettre ce champ lors des requêtes directes aux réponses `openai/*`.
...
- `service_tier = "priority"` pour les appels directs aux réponses `openai/*` vers `api.openai.com`
...
Les substitutions de session l'emportent sur la configuration. Effacer la substitution de session dans l'interface utilisateur des Sessions
