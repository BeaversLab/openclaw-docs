...
read_when: "Vous voulez plusieurs agents isolés (espaces de travail + auth) dans un processus de gateway unique."
status: active
---

# Routage multi-agent

Objectif : plusieurs agents _isolés_ (espace de travail distinct + `agentDir` + sessions), plus plusieurs comptes de canal (p. ex. deux WhatsApp) dans un Gateway en cours d'exécution. Le trafic entrant est acheminé vers un agent via des liaisons.
...
bac à sable strict. Les chemins relatifs sont résolus à l'intérieur de l'espace de travail, mais les chemins absolus peuvent
...
Créez un compte par agent sur vos channels préférés :
...
Voir les guides de channel : [Discord](/fr/channels/discord), [Telegram](/fr/channels/telegram), [WhatsApp](/fr/channels/whatsapp).
  ...
  <Step title="Ajouter les agents, comptes et bindings">

Ajoutez les agents sous `agents.list`, les comptes de channel sous `channels.<channel>.accounts`, et connectez-les avec `bindings` (exemples ci-dessous).
...
- **Auth + sessions séparées** (pas de croisement sauf si activé explicitement).
...
## Un numéro WhatsApp, plusieurs personnes (division des DM)

Vous pouvez router différents **WhatsApp DMs** vers différents agents tout en restant sur **un WhatsApp compte**. Faites correspondre l'expéditeur E.164 (comme `+15551234567`) avec `peer.kind: "direct"`. Les réponses proviennent toujours du même numéro WhatsApp (pas d'identité d'expéditeur par agent).
...
- Le contrôle d'accès DM est **global par WhatsApp compte** (appairage/liste blanche), et non par agent.
...
8. retour à l'agent par défaut (`agents.list[].default`, sinon première entrée de la liste, défaut : `main`)
...
- Utilisez `accountId: "*"` pour un retour de secours à l'échelle du canal sur tous les comptes.
- Si vous ajoutez ultérieurement la même liaison pour le même agent avec un identifiant de compte explicite, OpenClaw met à niveau la liaison existante de canal uniquement à la portée du compte au lieu de la dupliquer.
...
each connexion. Chaque `accountId` peut être routé vers un agent différent, de sorte qu'un seul serveur peut héberger
plusieurs numéros de téléphone sans mélanger les sessions.

Si vous souhaitez un compte par défaut pour l'ensemble du channel lorsque `accountId` est omis, définissez
`channels.<channel>.defaultAccount` (facultatif). Lorsqu'il n'est pas défini, OpenClaw revient
...
Diviser par channel : acheminez WhatsApp vers un agent rapide du quotidien et Telegram vers un agent Opus.
...
- Pour acheminer un DM/groupe unique vers Opus tout en gardant le reste sur chat, ajoutez une liaison `match.peer` pour ce pair ; les correspondances de pairs l'emportent toujours sur les règles à l'échelle du channel.
...
Liez un agent famille dédié à un seul groupe WhatsApp, avec filtrage par mention
et une stratégie d'outil plus stricte :
...
- Les listes d'autorisation/refus d'outils sont des **tools**, et non des compétences. Si une compétence a besoin d'exécuter un
...
- **Contrôle des ressources** : sandbox des agents spécifiques tout en gardant les autres sur l'hôte
