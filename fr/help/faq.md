...
- `openclaw status` : capture instantanée rapide de la santé de la passerelle/de l'agent + configuration de base.
...
Le dépôt recommande de lancer à partir du code source et d'utiliser l'assistant d'onboarding :
...
- **Tailscale Serve** (recommandé) : gardez la liaison bouclage (bind loopback), lancez `openclaw gateway --tailscale serve`, ouvrez `https://<magicdns>/`. Si `gateway.auth.allowTailscale` est `true`, les en-têtes d'identité satisfont l'authentification de l'interface de contrôle/WebSocket (pas de jeton, suppose un hôte de passerelle de confiance) ; les API HTTP nécessitent toujours un jeton/mot de passe.
...
Node **>= 22** est requis. `pnpm` est recommandé. Bun est **pas recommandé** pour le Gateway.
...
**Important :** si vous ne faites que committer/pousser votre espace de travail vers GitHub, vous sauvegardez
...
- Ajoutez ce répertoire à votre variable d'environnement PATH utilisateur (pas de suffixe `\bin` nécessaire sur Windows ; sur la plupart des systèmes, c'est `%AppData%\npm`).
...
### Que fait réellement l'assistant de configuration

`openclaw onboard` est la méthode de configuration recommandée. En **mode local**, il vous guide à travers :
...
OpenClaw prend en charge **OpenAI Code (Codex)** via OAuth (connexion ChatGPT). L'assistant peut exécuter le flux OAuth et définira le modèle par défaut sur `openai-codex/gpt-5.4` si nécessaire. Voir [Fournisseurs de modèles](/fr/concepts/model-providers) et [Assistant](/fr/start/wizard).

### Prenez-vous en charge l'authentification par abonnement OpenAI Codex OAuth

Oui. OpenClaw prend entièrement en charge **l'authentification par abonnement OpenAI Code (Codex)**.
OAuth autorise explicitement l'utilisation de l'authentification par abonnement OpenAI dans des outils/flux de travail externes
tels que OAuth. L'assistant de configuration peut exécuter le flux d'authentification OpenClaw pour vous.

Voir [OAuth](/fr/concepts/oauth), [Fournisseurs de modèles](/fr/concepts/model-providers) et [Assistant](/fr/start/wizard).

### Comment configurer l'authentification CLI du OAuth Gemini

Le CLI Gemini utilise un **flux d'authentification de plugin**, et non un identifiant client ou un secret dans `openclaw.json`.
...
Oui. Le **Mac mini peut exécuter la Gateway**, et votre MacBook Pro peut se connecter en tant que **nœud** (appareil compagnon). Les nœuds n'exécutent pas la Gateway - ils fournissent des capacités supplémentaires comme l'écran/caméra/canvas et `system.run` sur cet appareil.
...
L'assistant de configuration accepte une entrée `@username` et la résout en un ID numérique, mais l'autorisation OpenClaw utilise uniquement des ID numériques.
...
Si vous exécutez OpenClaw via systemd, assurez-vous que le PATH du service inclut `/home/linuxbrew/.linuxbrew/bin` (ou votre préfixe brew) afin que les outils installés via `brew` soient résolus dans les shells non de connexion.
Les versions récentes ajoutent également au début les répertoires bin courants des utilisateurs sur les services systemd Linux (par exemple `~/.local/bin`, `~/.npm-global/bin`, `~/.local/share/pnpm`, `~/.bun/bin`) et honorent `PNPM_HOME`, `NPM_CONFIG_PREFIX`, `BUN_INSTALL`, `VOLTA_HOME`, `ASDF_DATA_DIR`, `NVM_DIR` et `FNM_DIR` lorsqu'ils sont définis.
...
Utilisez des substitutions gérées au lieu de modifier la copie du dépôt. Placez vos modifications dans `~/.openclaw/skills/<name>/SKILL.md` (ou ajoutez un dossier via `skills.load.extraDirs` dans `~/.openclaw/openclaw.json`). La priorité est `<workspace>/skills` > `~/.openclaw/skills` > bundled, donc les substitutions gérées l'emportent sans toucher à git. Seules les modifications dignes d'intérêt pour l'amont devraient résider dans le dépôt et être envoyées sous forme de PR.

### Puis-je charger des compétences depuis un dossier personnalisé ?

Oui. Ajoutez des répertoires supplémentaires via `skills.load.extraDirs` dans `~/.openclaw/openclaw.json` (la priorité la plus basse). La priorité par défaut reste : `<workspace>/skills` → `~/.openclaw/skills` → bundled → `skills.load.extraDirs`. `clawhub` installe dans `./skills` par défaut, ce que OpenClaw considère comme `<workspace>/skills`.

### Comment puis-je utiliser différents modèles pour différentes tâches

Aujourd'hui, les modèles pris en charge sont :

- **Tâches planifiées (Cron jobs)** : les tâches isolées peuvent définir une remplacement `model` par tâche.
...
Pas directement. Les compétences macOS sont limitées par `metadata.openclaw.os` ainsi que par les binaires requis, et elles n'apparaissent dans l'invite système que lorsqu'elles sont éligibles sur l'**hôte macOS**. Sur Gateway, les compétences exclusives à `darwin` (telles que `apple-notes`, `apple-reminders`, `things-mac`) ne se chargeront pas à moins que vous ne contourniez la restriction.
...
Vous devez toujours cliquer sur le bouton de l'extension sur l'onglet que vous souhaitez contrôler (elle ne s'attache pas automatiquement).
...
Demandez au bot d'**écrire le fait dans la mémoire**. Les notes à long terme vont dans `MEMORY.md`,
...
Seulement si vous utilisez les **embeddings OpenAI**. Codex OAuth couvre la conversation/les complétions et
...
Si vous ne définissez pas explicitement un fournisseur, OpenClaw en sélectionne un automatiquement lorsqu'il
...
### Comment faire fonctionner une Gateway centrale avec des workers spécialisés sur plusieurs appareils
...
Non. Un Gateway peut héberger plusieurs agents, chacun avec son propre espace de travail, des valeurs par défaut de modèle, et un routage. C'est la configuration normale et elle est beaucoup moins chère et plus simple que de faire tourner un VPS par agent.

Utilisez des VPS distincts uniquement lorsque vous avez besoin d'un isolement strict (limites de sécurité) ou de configurations très différentes que vous ne souhaitez pas partager. Sinon, gardez un seul Gateway et utilisez plusieurs agents ou sous-agents.
...
### Y a-t-il un moyen de constituer une équipe d'instances OpenClaw avec un PDG et de nombreux agents ?
...
moins efficace que l'utilisation d'un seul bot avec des sessions distinctes. Le modèle type que
...
Relancez ensuite l'intégration (onboarding) :
...
Les heartbeats s'exécutent toutes les **30 m** par défaut. Réglez-les ou désactivez-les :
...
L'accès via navigateur est puissant mais pas « faire tout ce qu'un humain peut faire » — anti-bot, CAPTCHAs et MFA peuvent
toujours bloquer l'automatisation. Pour le contrôle le plus fiable du navigateur, utilisez le relais de l'extension Chrome
sur la machine qui exécute le navigateur (et gardez le Gateway n'importe où).
...
**Par défaut recommandé :** utilisez le modèle le plus puissant de la dernière génération disponible dans votre stack de fournisseurs.
...
Oui. Ollama est la méthode la plus simple pour les modèles locaux.
...
- Ces déploiements peuvent différer et peuvent évoluer dans le temps ; il n'y a pas de recommandation fixe de fournisseur.
...
n'a été trouvé), le modèle ne peut donc pas être résolu. Un correctif pour cette détection est
prévu dans la version **2026.1.12** (non publié au moment de la rédaction).
...
### Puis-je utiliser MiniMax par défaut et OpenAI pour des tâches complexes

Oui. Utilisez **MiniMax par défaut** et changez de modèle **par session** si nécessaire.
Les basculements (fallbacks) sont pour les **erreurs**, pas pour les « tâches difficiles », utilisez donc `/model` ou un agent distinct.
...
### Comment définir/remplacer les raccourcis et alias de modèles
...
- **Confirmer l'emplacement des profils d'authentification** (nouveaux chemins par rapport aux chemins hérités)
...
Un profil d'authentification est un enregistrement d'identification nommé (OAuth ou clé API) lié à un fournisseur. Les profils résident dans :

```
~/.openclaw/agents/<agentId>/agent/auth-profiles.json
```

### Quels sont les ID de profil types
...
### OAuth contre clé API : quelle est la différence

OpenClaw prend en charge les deux :

- Le OAuth exploite souvent l'accès par abonnement (le cas échéant).
- Les clés **API** utilisent la facturation à la demande (pay-per-token).

L'assistant prend en charge explicitement le jeton de configuration Anthropic et OpenAI OAuth Codex, et peut stocker les clés API pour vous.
...
Généralement non - une Gateway peut exécuter plusieurs canaux de messagerie et agents. Utilisez plusieurs passerelles uniquement lorsque vous avez besoin de redondance (ex : bot de secours) ou d'un isolement strict.
...
Journaux du service/superviseur (lorsque la passerelle fonctionne via launchd/systemd) :
...
- Blocage des réponses par l'appariement/la liste d'autorisation des canaux (vérifiez la configuration des canaux + les journaux).
- Le WebChat/Le tableau de bord est ouvert sans le bon jeton.
...
Ensuite, consultez les journaux (tail logs) :
...
- Le channel cible prend en charge les médias sortants et n'est pas bloqué par les listes autorisées.
...
### Mon bot doit-il avoir son propre compte e-mail GitHub ou numéro de téléphone ?
...
### Puis-je lui donner une autonomie sur mes messages texte et est-ce sans danger ?

Nous **ne recommandons pas** une autonomie totale sur vos messages personnels. Le modèle le plus sûr est :

- Garder les DMs en **mode d'appairage** ou dans une liste d'autorisation restreinte.
...
Si vous souhaitez faire des expériences, faites-le sur un compte dédié et gardez-le isolé. Voir
...
OpenClaw bloque la messagerie inter-fournisseurs par défaut. Si un appel d'outil est lié à Telegram, il ne sera pas envoyé à Discord sauf si vous l'autorisez explicitement.
...
## Répondre exactement à la question de la capture d'écran/journal de chat
