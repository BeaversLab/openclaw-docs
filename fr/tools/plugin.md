...
<Accordion title="Plugins de mémoire">
  ...
  `memory-lancedb` — mémoire à long terme installée à la demande avec rappel/capture automatique
  (définir `plugins.slots.memory = "memory-lancedb"`)
</Accordion>

<Accordion title="Fournisseurs de reconnaissance vocale (activés par défaut)">
...
<Accordion title="États des plugins : désactivé vs manquant vs invalide">
  - **Désactivé** : le plugin existe mais les règles d'activation l'ont éteint. La configuration est
  conservée. - **Manquant** : la configuration fait référence à un identifiant de plugin que la
  découverte n'a pas trouvé. - **Invalide** : le plugin existe mais sa configuration ne correspond
  pas au schéma déclaré.
</Accordion>
...
<Step title="Extensions globales">
  `~/.openclaw/extensions/*.ts` et `~/.openclaw/extensions/*/index.ts`.
</Step>
