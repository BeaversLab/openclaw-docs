---
summary: "ClawHub guide : registre public de compétences + flux de travail CLI"
read_when :
  - Présentation de ClawHub aux nouveaux utilisateurs
  - Installation, recherche ou publication de compétences
  - Explication des indicateurs ClawHub et du comportement de synchronisation CLI
title : "ClawHub"
---

# ClawHub

ClawHub est le **registre public de compétences pour OpenClaw**. C'est un service gratuit : toutes les compétences sont publiques, ouvertes et visibles par tous pour le partage et la réutilisation. Une compétence n'est qu'un dossier contenant un fichier `SKILL.md` (ainsi que des fichiers texte d'accompagnement). Vous pouvez parcourir les compétences dans l'application web ou utiliser la CLI pour rechercher, installer, mettre à jour et publier des compétences.
...
## À qui cela s'adresse (convient aux débutants)
...
Pour plus de détails sur la manière dont les compétences sont chargées, partagées et contrôlées, voir
...
- Des métadonnées telles que les balises, le résumé et les prérequis d'installation.

ClawHub utilise les métadonnées pour alimenter la découverte et exposer en toute sécurité les capacités des compétences.
...
- **Gestion des versions** avec semver, journaux des modifications et balises (y compris `latest`).
- **Téléchargements** sous forme de zip par version.
- **Étoiles et commentaires** pour les commentaires de la communauté.
...
- Les modérateurs peuvent afficher les compétences masquées, les révéler, les supprimer ou bannir les utilisateurs.
- L'abus de la fonctionnalité de signalement peut entraîner l'interdiction du compte.

Intéressé par devenir modérateur ? Demandez sur le OpenClaw Discord et contactez un modérateur ou un mainteneur.
...
- `--all` : Tout télécharger sans demander de confirmation.
...
### Versionnage et étiquettes

- Chaque publication crée une nouvelle version **semver** `SkillVersion`.
- Les étiquettes (comme `latest`) pointent vers une version ; déplacer les étiquettes vous permet de revenir en arrière.
...
Les mises à jour comparent le contenu de la compétence locale aux versions du registre à l'aide d'un hachage de contenu. Si les fichiers locaux ne correspondent à aucune version publiée, la CLI demande avant d'écraser (ou nécessite `--force` lors des exécutions non interactives).
...
- Les jetons d'authentification sont stockés dans le fichier de configuration ClawHub CLI (remplacer via `CLAWHUB_CONFIG_PATH`).

### Télémétrie (comptes d'installation)

Lorsque vous exécutez `clawhub sync` alors que vous êtes connecté, la CLI envoie un instantané minimal pour calculer les comptes d'installation. Vous pouvez désactiver ceci entièrement :
