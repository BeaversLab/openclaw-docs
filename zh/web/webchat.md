---
summary: "Loopback WebChat 静态主机和用于聊天 UI 的 Gateway(网关) WS 用法"
read_when:
  - 调试或配置 WebChat 访问
title: "WebChat"
---
...
- 该 UI 连接到 Gateway(网关) WebSocket 并使用 `chat.history`、`chat.send` 和 `chat.inject`。
- `chat.history` 为稳定性设限：Gateway(网关) 可能会截断长文本字段，省略繁重的元数据，并用 `[chat.history omitted: message too large]` 替换过大的条目。
- `chat.inject` 将助手备注直接追加到记录并将其广播到 UI（不运行 agent）。
...
- 控制 UI `/agents` 工具面板通过 `tools.catalog` 获取运行时目录，并将每个
  工具标记为 `core` 或 `plugin:<id>`（外加 `optional` 表示可选插件工具）。
- 如果 `tools.catalog` 不可用，该面板将回退到内置的静态列表。
- 该面板编辑配置文件和覆盖配置，但有效的运行时访问仍然遵循策略
  优先级（`allow`/`deny`，每个 agent 以及提供商/渠道覆盖）。
...
- `gateway.auth.mode: "trusted-proxy"`：浏览器客户端的反向代理身份验证（参见 [Trusted Proxy Auth](/zh/gateway/trusted-proxy-auth)）。
