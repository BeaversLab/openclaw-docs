---
summary: "Gateway 仪表板（控制 UI）的访问和身份验证"
read_when:
...
Gateway 仪表板是默认在 `/` 提供的基于浏览器的控制 UI
...
- 有关使用和 UI 功能，请参阅[控制 UI](/en/web/control-ui)。
- 有关 Serve/Funnel 自动化，请参阅[Tailscale](/en/gateway/tailscale)。
- 有关绑定模式和注意事项，请参阅 [Web 表面](/en/web)。

身份验证通过 `connect.params.auth` （令牌或密码）在 WebSocket 握手时强制执行。请参阅 [Gateway configuration](/en/gateway/configuration) 中的 `gateway.auth`。
...
- **非本地主机**：使用 Tailscale Serve （如果 `gateway.auth.allowTailscale: true`，则控制 UI/WebSocket 无需令牌，假定受信任的网关主机；HTTP API 仍需令牌/密码）、带令牌的 tailnet 绑定或 SSH 隧道。请参阅 [Web surfaces](/en/web)。
...
- 有关令牌偏差修复步骤，请遵循 [Token drift recovery checklist](/en/cli/devices#token-drift-recovery-checklist)。
- 从网关主机检索或提供令牌：
