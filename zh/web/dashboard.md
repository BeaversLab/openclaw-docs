...
- [Control UI](/en/web/control-ui) 了解用法和 UI 功能。
- 用于 Serve/Funnel 自动化的 Tailscale(/en/gateway/tailscale)。
- [Web surfaces](/en/web) 了解绑定模式和安全注意事项。

身份验证在 WebSocket 握手期间通过 `connect.params.auth`（令牌或密码）强制执行。请参阅 [Gateway(网关) 配置](/en/gateway/configuration) 中的 `gateway.auth`。
...
- **非 localhost**：使用 Tailscale Serve（如果 `gateway.auth.allowTailscale: true`，则 Control UI/WebSocket 无需令牌，假设是受信任的 gateway 主机；HTTP API 仍需要令牌/密码）、带有令牌的 tailnet 绑定或 SSH 隧道。请参阅 [Web surfaces](/en/web)。
...
- 有关令牌漂移修复步骤，请遵循 [Token drift recovery checklist](/en/cli/devices#token-drift-recovery-checklist)。
- 从网关主机检索或提供令牌：
