...
身份验证通过 `connect.params.auth` （令牌或密码）在 WebSocket 握手时强制执行。请参阅 [Gateway 网关 configuration](/zh/en/gateway/configuration) 中的 `gateway.auth`。
...
- **非本地主机**：使用 Tailscale Serve （如果 `gateway.auth.allowTailscale: true`，则控制 UI/WebSocket 无需令牌，假定受信任的网关主机；HTTP API 仍需令牌/密码）、带令牌的 tailnet 绑定或 SSH 隧道。请参阅 [Web surfaces](/zh/en/web)。
