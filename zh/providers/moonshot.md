...
- 如果 Moonshot 为模型发布了不同的上下文限制，请调整
  `contextWindow`。
- 对国际端点使用 `https://api.moonshot.ai/v1`，对中国端点使用 `https://api.moonshot.cn/v1`。
...
- 任何非 off 思维级别 -> `thinking.type=enabled`

当启用 Moonshot 思维时，`tool_choice` 必须是 `auto` 或 `none`。为了兼容性，OpenClaw 会将不兼容的 `tool_choice` 值规范化为 `auto`。
