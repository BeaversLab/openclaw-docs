...
- 如果 Moonshot 为模型发布了不同的上下文限制，请相应调整
  相应地 `contextWindow`。
- 对于国际端点使用 `https://api.moonshot.ai/v1`，对于中国端点使用 `https://api.moonshot.cn/v1`。
...
当启用 Moonshot 思考时，`tool_choice` 必须是 `auto` 或 `none`。OpenClaw 会将不兼容的 `tool_choice` 值标准化为 `auto` 以确保兼容性。
