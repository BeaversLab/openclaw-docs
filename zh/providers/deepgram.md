...
Deepgram 是一个语音转文本 API。在 OpenClaw 中，它通过 `tools.media.audio` 用于**传入音频/语音笔记
转录**。

启用后，OpenClaw 会将音频文件上传到 Deepgram 并将转录
注入回复管道（`{{Transcript}}` + `[Audio]` 块）。这**不是流式传输**；
它使用预录制的转录端点。
...
- 身份验证遵循标准提供商身份验证顺序；`DEEPGRAM_API_KEY` 是最简单的方法。
