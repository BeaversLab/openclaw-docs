...
OpenAI 当前的 API 模型文档列出了用于直接
使用 OpenAI API 的 `gpt-5.4` 和 `gpt-5.4-pro`。OpenClaw 通过 `openai/*` 响应路径转发这两者。
OpenClaw 故意抑制了过时的 `openai/gpt-5.3-codex-spark` 行，
因为直接调用 OpenAI API 在实时流量中会拒绝它。

OpenClaw **不** 在直接 OpenAI
API 路径上暴露 `openai/gpt-5.3-codex-spark`。 `pi-ai` 仍然为该模型提供了一个内置行，但实时的 OpenAI API
...
将其映射到 `openai-codex/gpt-5.4` 以便用于 ChatGPT/Codex OAuth。
...
当 `pi-ai`
发现它时，OpenClaw 也会保留 `openai-codex/gpt-5.3-codex-spark`。将其视为依赖于授权和实验性的：Codex Spark 与
GPT-5.4 `/fast` 分开，可用性取决于登录的 Codex /
ChatGPT 账户。

### 传输默认值

OpenClaw 使用 `pi-ai` 进行模型流式传输。对于 `openai/*` 和
`openai-codex/*`，默认传输方式是 `"auto"` (优先使用 WebSocket，然后是 SSE
回退)。
...
对于 `openai/*` (Responses API)，当使用 WebSocket 传输时，OpenClaw 默认也会启用
...
OpenAI 文档将预热描述为可选。OpenClaw 默认为 `openai/*` 启用它，以在使用 WebSocket 传输时减少首轮延迟。
...
- 当负载未指定推理模式时，`reasoning.effort = "low"`
- 当负载未指定详细程度时，`text.verbosity = "low"`
- 对于直接 `openai/*` Responses 对 `api.openai.com` 的调用，`service_tier = "priority"`
...
对于直接 OpenAI Responses 模型（`openai/*` 在 `api.openai.com` 上使用 `api: "openai-responses"` 配合 `baseUrl`），OpenClaw 现在自动启用 OpenAI 服务端压缩负载提示：
...
当您想要在兼容的 Responses 模型上强制注入 `context_management` 时（例如 Azure OpenAI Responses），请使用此选项：
...
直接的 OpenAI Responses 模型仍会强制 `store: true`，除非 compat 设置了
`supportsStore: false`。
