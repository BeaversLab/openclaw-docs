...
在找聊天频道文档（WhatsApp/Telegram/Discord/Slack/Mattermost（插件）等）？参见 [通道](/zh/channels)。

## 亮点：Venice（Venice AI）

Venice 是我们推荐的 Venice AI 配置，主打隐私优先推理，并可选用 Opus 处理难题。
...
- [Venice（Venice AI，隐私优先）](/zh/providers/venice)
...
完整的 provider 目录（xAI、Groq、Mistral 等）与高级配置，参见 [模型 providers](/zh/concepts/model-providers)。
