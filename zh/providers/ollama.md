...
read_when:
  - 您希望通过 Ollama 使用云模型或本地模型运行 OpenClaw
  - 您需要 Ollama 的设置和配置指南
title: "Ollama"
---
...
Ollama 是一个本地 LLM 运行时，可让您轻松在机器上运行开源模型。OpenClaw 集成了 Ollama 的原生 API (`/api/chat`)，支持流式传输和工具调用，并且当您选择使用 `OLLAMA_API_KEY`（或身份验证配置文件）且未定义明确的 `models.providers.ollama` 条目时，它可以自动发现本地 Ollama 模型。

<Warning>
  **远程 Ollama 用户**：请勿将 `/v1` OpenAI 兼容 URL (`http://host:11434/v1`) 与 OpenClaw
  配合使用。这会破坏工具调用，且模型可能会将原始工具 JSON 作为纯文本输出。请改用原生 Ollama API
  URL：`baseUrl: "http://host:11434"`（不带 `/v1`）。
</Warning>
...
1. 询问可以访问您的实例的 Ollama 基本 URL（默认为 `http://127.0.0.1:11434`）。
2. 让您选择 **云 + 本地**（云模型和本地模型）或 **本地**（仅限本地模型）。
3. 如果您选择 **云 + 本地** 且尚未登录 ollama.com，将打开浏览器登录流程。
4. 发现可用模型并建议默认值。
...
- 将 `maxTokens` 设置为 Ollama 使用的默认 OpenClaw 最大 token 上限
- 将所有成本设置为 `0`
...
如果您显式设置 `models.providers.ollama`，则将跳过自动发现，您必须手动定义模型（见下文）。
...
<Warning>
  不要在 URL 中添加 `/v1`。`/v1` 路径使用 OpenAI
  兼容模式，其中工具调用不可靠。请使用不带路径后缀的基本 Ollama URL。
</Warning>
...
云模型允许您在本地模型之外运行云托管模型（例如 `kimi-k2.5:cloud`、`minimax-m2.5:cloud`、`glm-5:cloud`）。
...
OpenClaw 默认将名称为 `deepseek-r1`、`reasoning` 或 `think` 的模型视为具备推理能力：
...
Ollama 是免费的并且在本地运行，因此所有模型成本均设置为 $0。
...
<Warning>
  **在 OpenAI 兼容模式下，工具调用不可靠。** 仅当您需要代理使用 OpenAI
  格式且不依赖原生工具调用行为时，才使用此模式。
</Warning>
...
此模式可能无法同时支持流式传输 + 工具调用。您可能需要在模型配置中使用 `params: { streaming: false }` 禁用流式传输。

当 `api: "openai-completions"` 与 Ollama 一起使用时，OpenClaw 默认会注入 `options.num_ctx`，以防止 Ollama 静默回退到 4096 的上下文窗口。如果您的代理/上游拒绝未知的 `options` 字段，请禁用此行为：
...
请确保 Ollama 正在运行并且您已设置 `OLLAMA_API_KEY`（或身份验证配置文件），并且您**没有**定义显式的 `models.providers.ollama` 条目：
...
- [模型提供商](/zh/concepts/model-providers) - 所有提供商概览
