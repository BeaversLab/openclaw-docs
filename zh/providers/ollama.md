...
<Warning>
**远程 Ollama 用户**：请勿将 `/v1` OpenAI 兼容 URL (`http://host:11434/v1`) 与 OpenClaw 配合使用。这会破坏工具调用，且模型可能会将原始工具 JSON 作为纯文本输出。请改用原生 Ollama API URL：`baseUrl: "http://host:11434"`（不带 `/v1`）。
</Warning>
...
<Warning>
不要在 URL 中添加 `/v1`。`/v1` 路径使用 OpenAI 兼容模式，其中工具调用不可靠。请使用不带路径后缀的基本 Ollama URL。
</Warning>
