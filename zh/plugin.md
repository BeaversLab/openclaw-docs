...
请参阅 [语音通话](/en/plugins/voice-call) 以获取具体的插件示例。
...
OpenClaw 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不执行插件代码**；它使用插件清单和 JSON Schema 代替。请参阅 [插件清单](/en/plugins/manifest)。
...
工具创作指南：[插件代理工具](/zh-CN/plugins/agent-tools)。
...
有关设置和使用，请参阅 [Voice Call](/en/plugins/voice-call) 和 `extensions/voice-call/README.md`。
