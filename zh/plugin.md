...
- 截至 2026.1.15，Microsoft Teams 仅支持插件形式；如果你使用 Teams，请安装 `@openclaw/msteams`。
...
- Copilot Proxy（提供商身份验证）— 本地 VS Code Copilot Proxy 桥接器；与内置的 `github-copilot` 设备登录不同（已捆绑，默认禁用）
...
必须通过 `plugins.entries.<id>.enabled` 或 `openclaw plugins enable <id>` 显式启用捆绑插件。已安装的插件默认启用，但也可以通过相同方式禁用。

每个插件必须在其根目录中包含一个 `openclaw.plugin.json` 文件。如果路径指向文件，则插件根目录为该文件所在的目录，并且必须包含清单。
...
每个条目成为一个插件。如果集合列出了多个扩展，插件 ID 将变为 `name/<fileBase>`。

如果您的插件导入了 npm 依赖项，请在该目录中安装它们，以便 `node_modules` 可用（`npm install` / `pnpm install`）。

### 通道目录元数据

通道插件可以通过 `openclaw.channel` 宣传入门元数据，并通过 `openclaw.install` 提供安装提示。这使核心目录不包含数据。
...
如果插件导出 `id`，OpenClaw 会使用它，但在与配置的 ID 不匹配时会发出警告。
...
- 未知的 `channels.<id>` 键属于**错误**，除非插件清单声明
  该通道 ID。
- 插件配置使用嵌入在
...
如果多个插件声明 `kind: "memory"`，则仅加载选定的那个。其他插件
将被禁用并输出诊断信息。
...
OpenClaw 会根据发现的插件在运行时增强 `uiHints`：

- 为 `plugins.entries.<id>` / `.enabled` / `.config` 添加针对每个插件的标签
- 合并可选的插件提供的配置字段提示于：
  `plugins.entries.<id>.config.<field>`

如果您希望插件配置字段显示良好的标签/占位符（并将机密信息标记为敏感），请在插件清单中与 JSON Schema 一起提供 `uiHints`。
...
通过 `api.registerProvider(...)` 注册提供者。每个提供者公开一个或多个身份验证方法（OAuth、API 密钥、设备代码等）。这些方法支持：
...
- 当您需要添加默认模型或提供者配置时，返回 `configPatch`。
...
- `meta.preferOver` 列出在两者都已配置时跳过自动启用的频道 ID。
- `meta.detailLabel` 和 `meta.systemImage` 让 UI 能够显示更丰富的频道标签/图标。

### 编写一个新的消息频道（分步教程）

当您想要一个新的**聊天界面**（即“消息频道”）而不是模型提供程序时，请使用此选项。
...
- `meta.preferOver` 允许插件替换另一个频道（自动启用优先选择它）。
- `meta.detailLabel` 和 `meta.systemImage` 被 UI 用于详细信息文本/图标。
...
- `args`：命令之后传递的参数（如果 `acceptsArgs: true`）
- `commandBody`：完整的命令文本
...
- `acceptsArgs`：命令是否接受参数（默认：false）。如果为 false 且提供了参数，则命令将不会匹配，消息将传递给其他处理程序
...
插件可以在代码仓库中附带一个技能 (`skills/<name>/SKILL.md`)。
...
- 配置键稳定性：对于 `plugins.entries.*`，作用域包（scoped packages）会被规范化为 **非作用域**（unscoped）的 ID。
