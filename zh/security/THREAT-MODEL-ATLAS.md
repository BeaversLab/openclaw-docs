...
此威胁模型基于 [MITRE ATLAS](https://atlas.mitre.org/) 构建，这是用于记录 AI/ML 系统对抗性威胁的行业标准框架。ATLAS 由 [MITRE](https://www.mitre.org/) 与 AI 安全社区合作维护。
...
此威胁模型记录了针对 OpenClaw AI 代理平台和 ClawHub 技能市场的对抗性威胁，使用了专为 AI/ML 系统设计的 MITRE ATLAS 框架。

### 1.2 范围

| 组件                | 包含 | 备注                                          |
| ------------------- | ---- | --------------------------------------------- |
| OpenClaw 代理运行时 | Yes  | 核心代理执行、工具调用、会话                  |
| Gateway(网关)       | Yes  | 身份验证、路由、渠道集成                      |
| 渠道集成            | Yes  | WhatsApp、Telegram、Discord、Signal、Slack 等 |
| ClawHub 市场        | Yes  | 技能发布、审核、分发                          |
| MCP 服务器          | 是   | 外部工具提供商                                |
| 用户设备            | 部分 | 移动应用、桌面客户端                          |

### 1.3 范围之外

对于此威胁模型，没有明确排除在范围之外的内容。
...
| ---------------- | ---------------------------------------- |
| **ATLAS ID**     | AML.T0040 - AI 模型推理 API 访问         |
| **描述**         | 攻击者在渠道中欺骗允许的发送者身份       |
| **攻击向量**     | 取决于渠道 - 电话号码欺骗、用户名冒充    |
| **受影响组件**   | 针对每个渠道的 AllowFrom 验证            |
| **当前缓解措施** | 特定于渠道的身份验证                     |
| **剩余风险**     | 中等 - 部分渠道易受欺骗攻击              |
...
| **描述**         | 攻击者在获取的内容中嵌入恶意指令         |
| **攻击向量**     | 恶意 URL、投毒的电子邮件、受损的 webhook |
| **受影响的组件** | web_fetch，电子邮件摄取，外部数据源      |
| **当前缓解措施** | 使用 XML 标记和安全通知进行内容包装      |
...
| 属性             | 值                                 |
| ---------------- | ---------------------------------- |
| **ATLAS ID**     | AML.T0043 - 编造对抗性数据         |
| **描述**         | 攻击者精心设计命令以绕过审批白名单 |
| **攻击向量**     | 命令混淆、别名利用、路径操纵       |
...
| 属性             | 值                                                |
| ---------------- | ------------------------------------------------- |
| **ATLAS ID**     | AML.T0010.001 - 供应链受损：AI 软件               |
| **描述**         | 攻击者将恶意技能发布到 ClawHub                    |
| **攻击向量**     | 创建账户，发布包含隐藏恶意代码的技能              |
| **受影响的组件** | ClawHub，技能加载，代理执行                       |
| **当前缓解措施** | GitHub 账户时长验证，基于模式的审核标记           |
| **剩余风险**     | 严重 - 无沙箱隔离，审查有限                       |
| **建议**         | VirusTotal 集成（进行中），技能沙箱隔离，社区审核 |
...
| 属性             | 值                                     |
| ---------------- | -------------------------------------- |
| **ATLAS ID**     | AML.T0010.002 - 供应链受损：数据       |
| **描述**         | 攻击者修改代理配置以维持访问权限       |
| **攻击向量**     | 配置文件修改，设置注入                 |
| **受影响的组件** | 代理配置，工具策略                     |
| **当前缓解措施** | 文件权限                               |
...
| 属性             | 值                                                      |
| ---------------- | ------------------------------------------------------- |
| **ATLAS ID**     | AML.T0043 - 构建对抗性数据                              |
| **描述**         | 攻击者精心设计技能内容以规避审核模式                    |
...
| 控制            | 实施                          | 有效性                                    |
| --------------- | ----------------------------- | ----------------------------------------- |
| GitHub 账号期限 | `requireGitHubAccountAge()`   | 中等 - 提高了新攻击者的门槛               |
| 路径清理        | `sanitizePath()`              | 高 - 防止路径遍历                         |
| 文件类型验证    | `isTextFile()`                | 中等 - 仅限文本文件，但仍可能包含恶意内容 |
| 大小限制        | 总计 50MB 包                  | 高 - 防止资源耗尽                         |
| 必需的 SKILL.md | 强制自述文件                  | 安全价值低 - 仅提供信息                   |
| 模式审核        | moderation.ts 中的 FLAG_RULES | 低 - 易于绕过                             |
