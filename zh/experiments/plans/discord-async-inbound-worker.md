---
summary: “状态和下一步计划：使用 Discord 特定的入站工作器将 Discord 网关侦听器与长时间运行的代理轮次解耦”
owner: "openclaw"
status: "in_progress"
...
这是针对排队的 Discord 运行在 `channels.discord.eventQueue.listenerTimeout` 处超时，而代理运行本身仍在取得进展这一问题的长期修复方案。
...
- 命令语义（`stop`、`new`、`reset`、未来的会话控制）尚未完全适配工作器原生模式
- 工作器可观测性和操作员状态仍然很少
- 仍然没有重启持久性
...
- `src/discord/monitor/message-handler.process.ts` 执行内联的媒体加载、路由、分发、输入指示、草稿流传输以及最终回复传递。
...
- 剩余工作：
  ...
  - 将实时运行时依赖项移动到 worker 拥有的服务中，而不是每个作业的 payload 中
  - 停止通过将实时 listener 引用拼接到作业中来重建进程上下文
...
- 已完成：
  ...
  - listener 将作业入队，而不是直接等待 `processDiscordMessage(...)`
  - worker 在进程内执行作业，仅限内存中
...
- 如果需要，暂时将 `processDiscordMessage(...)` 保留为外观，然后将其拆分。
...
1. 将实时运行时依赖从 `DiscordInboundJob` 中移出
...
在该部分落地后，下一个后续工作应该是针对 `stop`、`new` 和 `reset` 的命令状态清理。
...
- 风险：回复投递丢失线程或回复上下文
  缓解措施：在 `DiscordInboundJob` 中使投递身份成为一等公民

- 风险：重试或队列重启期间重复发送
  缓解措施：第一阶段仅保留在内存中，或在持久化之前添加显式的投递幂等性

- 风险：在迁移过程中，`message-handler.process.ts` 变得更难以推断
...
5. `stop` 针对的是 Worker 拥有的运行，而不是旧的监听器拥有的调用栈。
...
1. 使 `DiscordInboundJob` 仅包含纯数据，并将实时运行时引用移至 Worker 上
