...
- 类型前缀：`W` (world，世界)，`B` (experience/biographical，经历/传记)，`O` (opinion，观点)，`S` (observation/summary，观察/总结；通常由生成)
- 实体：`@Peter`，`@warelay` 等（slug 映射到 `bank/entities/*.md`）
- 观点置信度：`O(c=0.0..1.0)` 可选

如果您不希望作者为此操心：反思任务可以从日志的其余部分推断这些要点，但拥有一个明确的 `## Retain` 部分是最简单的“质量杠杆”。
...
反射是一个定时任务（每日或心跳 `ultrathink`），它：

- 根据近期事实（实体摘要）更新 `bank/entities/*.md`
- 根据强化/矛盾更新 `bank/opinions.md` 的置信度
- 可选地提议对 `memory.md`（“类似核心”的持久事实）进行编辑
