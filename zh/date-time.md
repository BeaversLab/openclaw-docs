---
summary: "信封、提示词、工具和连接器中的日期和时间处理"
read_when:
...
OpenClaw 默认对**传输时间戳采用主机本地时间**，并且仅在**系统提示词中采用用户时区**。
...
- `envelopeElapsed: "off"` 移除经过时间的后缀（`+2m` 样式）。
...
当代理需要当前时间时，请使用 `session_status` 工具；状态卡包含一行时间戳。
...
- `timeFormat` 控制提示词中的 **12小时制/24小时制显示**。`auto` 遵循操作系统首选项。

## 时间格式检测（自动）

当 `timeFormat: "auto"` 时，OpenClaw 会检查操作系统首选项（macOS/Windows）
并回退到区域设置格式。检测到的值将在**每个进程中缓存**，
以避免重复的系统调用。
...
- [系统提示](/zh/en/concepts/system-prompt)
- [时区](/zh/en/concepts/timezone)
- [消息](/zh/en/concepts/messages)
