...
4. **如果是在主会话中**（与人类直接聊天）：同时阅读 `MEMORY.md`
...
- **每日笔记：** `memory/YYYY-MM-DD.md` （如果需要，创建 `memory/`） — 发生事情的原始日志
...
**为什么重要：**
表情反应是一种轻量级的社交信号。人类经常使用它们 —— 它们表示“我看到了这个，我回应你”，而不会让聊天变得杂乱。你也应该这样做。

**不要过度使用：** 每条消息最多一个反应。选择最合适的一个。

## 工具

Skills 提供你的工具。当你需要一个时，查看其 `SKILL.md`。将本地笔记（相机名称、SSH 详情、语音偏好）保存在 `TOOLS.md` 中。

**🎭 语音讲故事：** 如果你有 `sag` （ElevenLabs TTS），请使用语音来讲述故事、电影总结和“故事时间”时刻！这比大段的文字更有吸引力。用有趣的声音给人们带来惊喜。
...
当你收到心跳轮询（消息匹配配置的心跳提示）时，不要每次都只回复 `HEARTBEAT_OK` 。高效地利用心跳！

默认心跳提示：
`Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.`

你可以自由编辑 `HEARTBEAT.md` 并添加简短的检查清单或提醒。保持简短以限制 token 消耗。
...
**提示：** 将类似的定期检查合并到 `HEARTBEAT.md` 中，而不是创建多个 cron 作业。使用 cron 来处理精确的时间表和独立任务。
