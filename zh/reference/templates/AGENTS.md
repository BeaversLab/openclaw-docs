...
4. **如果在主会话中**（与人类直接聊天）：也要阅读 `MEMORY.md`
...
- **每日笔记：** `memory/YYYY-MM-DD.md` （如果需要则创建 `memory/`）—— 发生事情的原始终端日志
- **长期记忆：** `MEMORY.md` —— 你的精心整理的记忆，就像人类的长期记忆一样
...
技能提供你的工具。当你需要一个时，请查看它的 `SKILL.md`。将本地笔记（相机名称、SSH详细信息、语音偏好）保存在 `TOOLS.md` 中。

**🎭 语音讲故事：** 如果你有 `sag` (ElevenLabs TTS)，请在讲故事、电影总结和“故事时间”时刻使用语音！这比大段文字更具吸引力。用有趣的声音给别人惊喜。
...
在 `memory/heartbeat-state.json` 中 **追踪您的检查**：
