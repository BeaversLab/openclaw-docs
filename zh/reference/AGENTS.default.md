...
- 读取 `SOUL.md`、`USER.md`、`memory.md` 以及 `memory/` 中的今天和昨天记录。
- 在回复之前执行此操作。
...
- 每日日志：`memory/YYYY-MM-DD.md`（如需要则创建 `memory/`）。
...
- 工具驻留在技能中；需要时请遵循每个技能的 `SKILL.md`。
- 在 `TOOLS.md`（技能笔记）中保留特定于环境的笔记。
...
- 直接聊天默认折叠到代理的 `main` 会话中；群组保持为 `agent:<agentId>:<channel>:group:<id>`（房间/频道：`agent:<agentId>:<channel>:channel:<id>`）隔离；心跳保持后台任务存活。
...
- **discord** — Discord 操作：反应、表情贴纸、投票。使用 `user:<id>` 或 `channel:<id>` 目标（纯数字 ID 有歧义）。
...
- 对于浏览器驱动的验证，请使用 `openclaw browser`（标签页/状态/截图），并配合 OpenClaw 管理的 Chrome 配置文件。
- 对于 DOM 检查，请使用 `openclaw browser eval|query|dom|snapshot`（当需要机器输出时，使用 `--json`/`--out`）。
- 对于交互，请使用 `openclaw browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（点击/输入需要快照引用；使用 `evaluate` 来定位 CSS 选择器）。
