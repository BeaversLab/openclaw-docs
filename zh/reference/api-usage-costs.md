...
有关详细信息和示例，请参阅 [Token 使用与成本](/en/reference/token-use)。
...
有关价格配置，请参阅 [模型](/en/providers/models)；有关显示，请参阅 [Token 使用与成本](/en/reference/token-use)。
...
参见[媒体理解](/en/nodes/media-understanding)。
...
参见[会话管理 + 压缩](/en/reference/session-management-compaction)。

### 8) 模型扫描/探测

`openclaw models scan` 可以探测 OpenRouter 模型，并在启用探测时使用 `OPENROUTER_API_KEY`。

参见[模型 CLI](/en/cli/models)。
