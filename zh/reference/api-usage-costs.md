---
summary: "审计哪些内容可能会花费资金、使用哪些密钥以及如何查看使用情况"
read_when:
  - 您想了解哪些功能可能会调用付费 API
  - 您需要审计密钥、成本和使用情况可见性
...
可能会产生提供商使用量或付费 OpenClaw 调用的 API 功能。

## 成本显示位置（聊天 + CLI）

**每会话成本快照**

- `/status` 显示当前会话模型、上下文使用量和上次响应的令牌数。
- 如果模型使用 **API 密钥认证**，`/status` 还会显示上次回复的 **预估成本**。

**每条消息成本页脚**

- `/usage full` 会在每条回复后附加一个使用情况页脚，包括 **预估成本**（仅限 API 密钥）。
- `/usage tokens` 仅显示令牌数；OAuth 流程会隐藏美元成本。

**CLI 使用情况窗口（提供商配额）**

- `openclaw status --usage` 和 `openclaw channels list` 显示提供商的 **使用情况窗口**
  （配额快照，而非每条消息的成本）。

有关详细信息和示例，请参阅 [令牌使用与成本](/zh/reference/token-use)。

## 如何发现密钥

OpenClaw 可以从以下位置获取凭据：

- **认证配置文件**（按代理，存储在 `auth-profiles.json` 中）。
...
- **Skills**（`skills.entries.<name>.apiKey`）可能会将密钥导出到技能进程环境。
...
有关价格配置，请参阅 [模型](/zh/providers/models)；有关显示方式，请参阅 [令牌使用与成本](/zh/reference/token-use)。

### 2) 媒体理解（音频/图像/视频）

入站媒体可以在回复运行之前进行总结/转录。这会使用模型/提供商 API。

- 音频：OpenAI / Groq / Deepgram（当密钥存在时现已**自动启用**）。
...
- `memorySearch.provider = "ollama"` → Ollama 嵌入（本地/自托管；通常没有托管的 API 费用）
- 如果本地嵌入失败，可选回退到远程提供商
...
`web_search` 使用 API 密钥，并且可能会根据您的提供商产生使用费用：
...
**Brave 搜索免费额度**：每个 Brave 计划都包含每月续充的 5 美元免费额度。搜索计划每 1,000 次请求收费 5 美元，因此该额度涵盖每月 1,000 次免费请求。在 Brave 仪表板中设置您的使用限制，以避免意外产生费用。
...
如果未配置 Firecrawl，该工具将回退到直接获取 + 可读性解析（无付费 API）。

请参阅 [Web 工具](/zh/tools/web)。

### 6) 提供商使用快照（状态/健康状况）

某些状态命令会调用 **提供商使用端点** 以显示配额窗口或身份验证健康状况。
这些通常是低流量调用，但仍会访问提供商 API：
