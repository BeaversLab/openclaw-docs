...
OpenClaw 是围绕单个 Gateway(网关) 进程设计的，该进程拥有会话状态。
...
规范规则记录在 [/concepts/会话](/en/concepts/session) 中。
...
- 会话密钥错误？首先从 [/concepts/会话](/en/concepts/session) 开始，并确认 `/status` 中的 `sessionKey`。
