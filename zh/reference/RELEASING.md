...
- 在各处使用相同的版本字符串，但在不使用 Git 标签的地方去掉前缀 `v`：
...
- Stable 和 beta 是 npm 的 dist-tags，不是单独的发布线：
  - `latest` = 稳定版
  - `beta` = 预发布版/测试版
- Dev 是 `main` 的移动头部，不是普通的 git 标记发布。
- 发布工作流会强制执行当前的稳定版/beta 标签格式，并拒绝 CalVer 日期距离发布日期超过 2 个 UTC 日历日的版本。

历史记录说明：

- 仓库历史中存在较旧的标签，例如 `v2026.1.11-1`、`v2026.2.6-3` 和 `v2.0.0-beta2`。
- 将这些视为旧版标签模式。新发布应使用 `vYYYY.M.D` 作为稳定版，使用 `vYYYY.M.D-beta.N` 作为 beta 版。
...
- [ ] 确认包元数据（名称、描述、仓库、关键词、许可证）并且 `bin` 映射指向 `openclaw` 的 [`openclaw.mjs`](https://github.com/openclaw/openclaw/blob/main/openclaw.mjs)。
...
- [ ] 可选：构建后运行 `npm pack --pack-destination /tmp`；检查压缩包内容并将其保留以备 GitHub 发布使用（请**勿**提交）。

3. **变更日志与文档**

- [ ] 使用面向用户的亮点更新 `CHANGELOG.md`（如果文件缺失则创建）；条目按版本严格降序排列。
...
- [ ] `OPENCLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`（Docker 安装冒烟测试，快速路径；发布前必须执行）
  - 如果紧邻的上一个 npm 版本已知存在问题，请为预安装步骤设置 `OPENCLAW_INSTALL_SMOKE_PREVIOUS=<last-good-version>` 或 `OPENCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1`。
- [ ] （可选）完整安装程序冒烟测试（增加非 root + CLI 覆盖）：`pnpm test:install:smoke`
- [ ] （可选）安装程序 E2E（Docker，运行 `curl -fsSL https://openclaw.ai/install.sh | bash`，进行接入，然后运行真实工具调用）：
...
- [ ] 生成 Sparkle appcast（HTML 说明通过 [`scripts/make_appcast.sh`](https://github.com/openclaw/openclaw/blob/main/scripts/make_appcast.sh)）并更新 `appcast.xml`。
- [ ] 准备好应用 zip（以及可选的 dSYM zip）以附加到 GitHub 版本中。
- [ ] 请遵循 [macOS release](/en/platforms/mac/release) 中的确切命令和所需的环境变量。
  - `APP_BUILD` 必须是数字且单调递增的（不允许 `-beta`），以便 Sparkle 正确比较版本。
  - 如果正在进行公证，请使用从 App Store Connect API 环境变量创建的 `openclaw-notary` 钥匙串配置文件（请参阅 [macOS release](/en/platforms/mac/release)）。
...
- [ ] 推送匹配的 git 标签以触发 `.github/workflows/openclaw-npm-release.yml`。
  ...
  - 如果标签不匹配 `package.json`、不在 `main` 上，或者其 CalVer 日期距离发布日期超过 2 个 UTC 日历日，工作流将拒绝该标签。
...
- **npm pack/publish 挂起或生成巨大的 tarball**：`dist/OpenClaw.app`（以及发布 zip 包）中的 macOS 应用程序包被包含到了包中。修复方法是通过 `package.json` `files` 将发布内容列入白名单（包括 dist 子目录、docs、skills；排除应用程序包）。使用 `npm pack --dry-run` 确认 `dist/OpenClaw.app` 未被列出。
- **npm auth web loop for dist-tags**：使用传统身份验证以获取 OTP 提示：
  - `NPM_CONFIG_AUTH_TYPE=legacy npm dist-tag add openclaw@X.Y.Z latest`
- **`npx` 验证失败并提示 `ECOMPROMISED: Lock compromised`**：使用清空的缓存重试：
...
- [ ] 为 `vX.Y.Z` 创建/刷新 GitHub 发布版本，使用 **标题 `openclaw X.Y.Z`**（不仅仅是标签）；正文应包含该版本的**完整**更新日志部分（亮点 + 变更 + 修复），内联（无裸链接），并且**不得在正文中重复标题**。
- [ ] 附加工件：`npm pack` tarball（可选）、`OpenClaw-X.Y.Z.zip` 和 `OpenClaw-X.Y.Z.dSYM.zip`（如果已生成）。
- [ ] 提交更新后的 `appcast.xml` 并将其推送（Sparkle 从 main 获取）。
- [ ] 在干净的临时目录（无 `package.json`）中，运行 `npx -y openclaw@X.Y.Z send --help` 以确认安装/CLI 入口点正常工作。
- [ ] 公告/分享发布说明。

## 插件发布范围 (npm)

我们仅发布 `@openclaw/*` 范围下的**现有 npm 插件**。未在 npm 上的捆绑插件保持**仅磁盘树**（仍包含在
`extensions/**` 中）。
...
发布说明还必须指出**默认未开启**的**新的可选捆绑插件**（例如：`tlon`）。
