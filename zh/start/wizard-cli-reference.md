---
summary: "CLI 入门流程、身份验证/模型设置、输出和内部机制的完整参考"
read_when:
...
有关简短指南，请参阅 [入门向导 (CLI)](/zh/en/start/wizard)。
...
<Steps>
  ...
  <Step title="工作区">
    ...
    - 为首次运行的启动仪式所需的工作区文件进行初始化。
    - 工作区布局：[Agent 工作区](/zh/en/concepts/agent-workspace)。
  </Step>
  <Step title="网关">
    - 提示输入端口、绑定、身份验证模式和 Tailscale 暴露设置。
    - 建议：即使对于本地回环，也保持令牌身份验证已启用，以便本地 WS 客户端必须进行身份验证。
    - 在令牌模式下，交互式入职向导提供：
      - **生成/存储明文令牌**（默认）
      - **使用 SecretRef**（可选加入）
    - 在密码模式下，交互式入职向导也支持明文或 SecretRef 存储。
  ...
  <Step title="频道">
    - [WhatsApp](/zh/en/channels/whatsapp)：可选的二维码登录
    ...
    - [iMessage](/zh/en/channels/imessage)：旧版 `imsg` CLI 路径 + 数据库访问
    - 私信（DM）安全性：默认为配对。第一条私信发送代码；通过 `openclaw pairing approve <channel> <code>` 批准或使用允许列表。
  </Step>
  <Step title="守护进程安装">
    - macOS：LaunchAgent
  ...
  <Step title="健康检查">
    ...
    - `openclaw status --deep` 将网关健康探测添加到状态输出中。
  </Step>
  <Step title="技能">
    - 读取可用技能并检查要求。
...
如果缺少控制 UI 资源，向导将尝试构建它们；后备方案是 `pnpm ui:build`（自动安装 UI 依赖）。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Anthropic OAuth (Claude Code CLI)">
    - macOS：检查钥匙串 项 "Claude Code-credentials"
  ...
  <Accordion title="Vercel AI 网关">
    提示输入 `AI_GATEWAY_API_KEY`。
    更多详情：[Vercel AI 网关](/zh/en/providers/vercel-ai-gateway)。
  </Accordion>
  <Accordion title="Cloudflare AI 网关">
    提示输入账户 ID、网关 ID 和 `CLOUDFLARE_AI_GATEWAY_API_KEY`。
    更多详情：[Cloudflare AI 网关](/zh/en/providers/cloudflare-ai-gateway)。
  </Accordion>
...
- `--secret-input-mode ref` 启用引用模式，而不是纯文本密钥存储。
  ...
  - 配置的提供商引用（`file` 或 `exec`），需提供提供商别名和 ID
...
- 在非交互模式下，`--secret-input-mode ref` 仅支持基于环境变量。
  - 在入职过程环境中设置提供商环境变量。
  - 内联密钥标志（例如 `--openai-api-key`）要求必须设置该环境变量；否则入职过程会快速失败。
  - 对于自定义提供商，非交互式 `ref` 模式会将 `models.providers.<id>.apiKey` 存储为 `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`。
  - 在该自定义提供商情况下，`--custom-api-key` 要求必须设置 `CUSTOM_API_KEY`；否则入职过程会快速失败。
...
<Note>
无头和服务器提示：在带有浏览器的机器上完成 OAuth，然后将
...
- `tools.profile`（未设置时，本地入职默认为 `"coding"`；保留现有的显式值）
- `gateway.*`（模式、绑定、认证、Tailscale）
- `session.dmScope`（未设置时，本地入职将其默认为 `per-channel-peer`；保留现有的显式值）
...
WhatsApp 凭据位于 `~/.openclaw/credentials/whatsapp/<accountId>/` 下方。
会话存储在 `~/.openclaw/agents/<agentId>/sessions/` 下方。
