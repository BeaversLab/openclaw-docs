...
<Steps>
  ...
  <Step title="Gateway(网关)">
    ...
    - 建议：即使对于回环地址，也保持令牌认证已启用，以便本地 WS 客户端必须进行身份验证。
    - 在令牌模式下，交互式新手引导提供：
      - **生成/存储明文令牌**（默认）
      - **使用 SecretRef**（可选加入）
    - 在密码模式下，交互式新手引导也支持明文或 SecretRef 存储。
    - 非交互式令牌 SecretRef 路径：`--gateway-token-ref-env <ENV_VAR>`。
      - 需要在新手引导流程环境中提供一个非空的环境变量。
...
<Note>
  - If gateway is loopback-only, use SSH tunneling or a tailnet. - 设备发现提示： - macOS: Bonjour
...
<AccordionGroup>
  ...
  <Accordion title="自定义提供商">
    ...
    - **立即粘贴 API 密钥**（纯文本）
    - **使用密钥引用**（环境变量引用或已配置的提供商引用，附带预检验证）
...
- `--secret-input-mode ref` 启用引用模式，而不是纯文本密钥存储。
  ...
  - 已配置的提供商引用（`file` 或 `exec`），附带提供商别名 + ID
...
- 在非交互模式下，`--secret-input-mode ref` 仅由环境变量支持。
  ...
  - 内联键标志（例如 `--openai-api-key`）要求必须设置该环境变量；否则 新手引导 会快速失败。
  - 对于自定义提供商，非交互式 `ref` 模式会将 `models.providers.<id>.apiKey` 存储为 `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`。
  - 在该自定义提供商的情况下，`--custom-api-key` 要求必须设置 `CUSTOM_API_KEY`；否则 新手引导 会快速失败。
- Gateway(网关) 认证凭据在交互式 新手引导 中支持明文和 SecretRef 选项：
...
- `tools.profile`（如果未设置，本地 新手引导 默认为 `"coding"`；保留现有的显式值）
- `gateway.*`（mode, bind, auth, tailscale）
- `session.dmScope`（如果未设置，本地 新手引导 将其默认为 `per-channel-peer`；保留现有的显式值）
...
<Note>
  某些渠道以插件形式提供。在引导过程中选择时，向导会提示在渠道配置之前 安装插件（npm 或本地路径）。
</Note>
