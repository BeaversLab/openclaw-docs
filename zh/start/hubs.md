---
summary: "Hubs that link to every OpenClaw doc"
read_when:
  - You want a complete map of the documentation
title: "Docs Hubs"
---

# 文档中心

<Note>如果您是 OpenClaw 的新手，请从[入门指南](/en/start/getting-started)开始。</Note>

使用这些中心来探索每一个页面，包括左侧导航中未显示的深度剖析和参考文档。

## 从这里开始

- [索引](/en/)
- [入门指南](/en/start/getting-started)
- [快速开始](/en/start/quickstart)
- [新手引导](/en/start/onboarding)
- [向导](/en/start/wizard)
- [设置](/en/start/setup)
- [Dashboard (本地 Gateway(网关))](http://127.0.0.1:18789/)
- [帮助](/en/help)
- [文档目录](/en/start/docs-directory)
- [配置](/en/gateway/configuration)
- [配置示例](/en/gateway/configuration-examples)
- [OpenClaw 助手](/en/start/openclaw)
- [展示](/en/start/showcase)
- [背景故事](/en/start/lore)

## 安装 + 更新

- [Docker](/en/install/docker)
- [Nix](/en/install/nix)
- [更新 / 回滚](/en/install/updating)
- [Bun 工作流（实验性）](/en/install/bun)

## 核心概念

- [架构](/en/concepts/architecture)
- [功能](/en/concepts/features)
- [网络枢纽](/en/network)
- [代理运行时](/en/concepts/agent)
- [代理工作区](/en/concepts/agent-workspace)
- [记忆](/en/concepts/memory)
- [代理循环](/en/concepts/agent-loop)
- [流式传输 + 分块](/en/concepts/streaming)
- [多代理路由](/en/concepts/multi-agent)
- [压缩](/en/concepts/compaction)
- [会话](/en/concepts/session)
- [会话修剪](/en/concepts/session-pruning)
- [会话工具](/en/concepts/session-tool)
- [队列](/en/concepts/queue)
- [斜杠命令](/en/tools/slash-commands)
- [RPC 适配器](/en/reference/rpc)
- [TypeBox 模式](/en/concepts/typebox)
- [时区处理](/en/concepts/timezone)
- [在线状态](/en/concepts/presence)
- [设备发现 + 传输协议](/en/gateway/discovery)
- [Bonjour](/en/gateway/bonjour)
- [Channel routing](/en/channels/channel-routing)
- [Groups](/en/channels/groups)
- [Group messages](/en/channels/group-messages)
- [Model failover](/en/concepts/model-failover)
- [OAuth](/en/concepts/oauth)

## Providers + ingress

- [Chat channels hub](/en/channels)
- [Model providers hub](/en/providers/models)
- [WhatsApp](/en/channels/whatsapp)
- [Telegram](/en/channels/telegram)
- [Slack](/en/channels/slack)
- [Discord](/en/channels/discord)
- [Mattermost](/en/channels/mattermost) (plugin)
- [Signal](/en/channels/signal)
- [BlueBubbles (iMessage)](/en/channels/bluebubbles)
- [iMessage (legacy)](/en/channels/imessage)
- [Location parsing](/en/channels/location)
- [WebChat](/en/web/webchat)
- [Webhooks](/en/automation/webhook)
- [Gmail Pub/Sub](/en/automation/gmail-pubsub)

## Gateway 网关 + operations

- [Gateway 网关 runbook](/en/gateway)
- [网络模型](/en/gateway/network-model)
- [Gateway 网关 pairing](/en/gateway/pairing)
- [Gateway 网关 lock](/en/gateway/gateway-lock)
- [Background process](/en/gateway/background-process)
- [Health](/en/gateway/health)
- [Heartbeat](/en/gateway/heartbeat)
- [Doctor](/en/gateway/doctor)
- [Logging](/en/gateway/logging)
- [沙箱隔离](/en/gateway/sandboxing)
- [Dashboard](/en/web/dashboard)
- [Control UI](/en/web/control-ui)
- [Remote access](/en/gateway/remote)
- [Remote gateway README](/en/gateway/remote-gateway-readme)
- [Tailscale](/en/gateway/tailscale)
- [Security](/en/gateway/security)
- [Troubleshooting](/en/gateway/troubleshooting)

## Tools + automation

- [Tools surface](/en/tools)
- [OpenProse](/en/prose)
- [CLI reference](/en/cli)
- [Exec 工具](/en/tools/exec)
- [PDF 工具](/en/tools/pdf)
- [Elevated mode](/en/tools/elevated)
- [Cron jobs](/en/automation/cron-jobs)
- [Cron vs Heartbeat](/en/automation/cron-vs-heartbeat)
- [Thinking + verbose](/en/tools/thinking)
- [Models](/en/concepts/models)
- [Sub-agents](/en/tools/subagents)
- [Agent 发送 CLI](/en/tools/agent-send)
- [Terminal UI](/en/web/tui)
- [Browser control](/en/tools/browser)
- [浏览器（Linux 故障排除）](/en/tools/browser-linux-troubleshooting)
- [Polls](/en/automation/poll)

## Nodes, media, voice

- [Nodes overview](/en/nodes)
- [Camera](/en/nodes/camera)
- [Images](/en/nodes/images)
- [Audio](/en/nodes/audio)
- [Location command](/en/nodes/location-command)
- [Voice wake](/en/nodes/voicewake)
- [Talk mode](/en/nodes/talk)

## 平台

- [Platforms overview](/en/platforms)
- [macOS](/en/platforms/macos)
- [iOS](/en/platforms/ios)
- [Android](/en/platforms/android)
- [Windows (WSL2)](/en/platforms/windows)
- [Linux](/en/platforms/linux)
- [Web surfaces](/en/web)

## macOS 伴侣应用（高级）

- [macOS 开发环境设置](/en/platforms/mac/dev-setup)
- [macOS 菜单栏](/en/platforms/mac/menu-bar)
- [macOS 语音唤醒](/en/platforms/mac/voicewake)
- [macOS 语音覆盖层](/en/platforms/mac/voice-overlay)
- [macOS WebChat](/en/platforms/mac/webchat)
- [macOS Canvas](/en/platforms/mac/canvas)
- [macOS 子进程](/en/platforms/mac/child-process)
- [macOS 运行状况](/en/platforms/mac/health)
- [macOS 图标](/en/platforms/mac/icon)
- [macOS 日志记录](/en/platforms/mac/logging)
- [macOS 权限](/en/platforms/mac/permissions)
- [macOS 远程控制](/en/platforms/mac/remote)
- [macOS 签名](/en/platforms/mac/signing)
- [macOS 版本发布](/en/platforms/mac/release)
- [macOS 网关 (launchd)](/en/platforms/mac/bundled-gateway)
- [macOS XPC](/en/platforms/mac/xpc)
- [macOS 技能](/en/platforms/mac/skills)
- [macOS Peekaboo](/en/platforms/mac/peekaboo)

## 工作区 + 模板

- [Skills](/en/tools/skills)
- [ClawHub](/en/tools/clawhub)
- [Skills 配置](/en/tools/skills-config)
- [默认 AGENTS](/en/reference/AGENTS.default)
- [模板：AGENTS](/en/reference/templates/AGENTS)
- [模板：BOOTSTRAP](/en/reference/templates/BOOTSTRAP)
- [模板：HEARTBEAT](/en/reference/templates/HEARTBEAT)
- [模板：IDENTITY](/en/reference/templates/IDENTITY)
- [模板：SOUL](/en/reference/templates/SOUL)
- [模板：TOOLS](/en/reference/templates/TOOLS)
- [模板：USER](/en/reference/templates/USER)

## 实验（探索性）

- [新手引导配置协议](/en/experiments/onboarding-config-protocol)
- [研究：内存](/en/experiments/research/memory)
- [模型配置探索](/en/experiments/proposals/model-config)

## 项目

- [致谢](/en/reference/credits)

## 测试 + 发布

- [测试](/en/reference/test)
- [发布检查清单](/en/reference/RELEASING)
- [设备型号](/en/reference/device-models)

import zh from "/components/footer/zh.mdx";

<zh />
