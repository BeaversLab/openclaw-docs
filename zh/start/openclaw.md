---
summary: "使用安全注意事项将 OpenClaw 作为个人助手运行的端到端指南"
read_when:
...
OpenClaw 是 **Pi** 智能体的 WhatsApp + Telegram + Discord + iMessage 网关。插件增加了 Mattermost 支持。本指南是“个人助手”设置：一个专用的 WhatsApp 号码，表现为您始终在线的智能体。
...
- 心跳现在默认为每 30 分钟一次。通过设置 `agents.defaults.heartbeat.every: "0m"` 禁用它，直到您信任该设置为止。
...
入职完成后，我们会自动打开仪表板并打印一个干净的（非令牌化）链接。如果它提示进行身份验证，请将 `gateway.auth.token` 中的令牌粘贴到 Control UI 设置中。要稍后重新打开：`openclaw dashboard`。

## 给智能体一个工作区 (AGENTS)

OpenClaw 从其工作区目录读取操作指令和“记忆”。

默认情况下，OpenClaw 使用 `~/.openclaw/workspace` 作为代理工作区，并会在设置/首次代理运行时自动创建它（以及初始 `AGENTS.md`、`SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、`HEARTBEAT.md`）。仅当工作区是全新的时才会创建 `BOOTSTRAP.md`（删除它后不应再出现）。`MEMORY.md` 是可选的（不会自动创建）；如果存在，则会在正常会话中加载。子代理会话仅注入 `AGENTS.md` 和 `TOOLS.md`。

提示：将此文件夹视为 OpenClaw 的“记忆”并将其设为 git 仓库（最好是私有的），以便备份您的 `AGENTS.md` 和记忆文件。如果安装了 git，全新的工作区将自动初始化。
...
- 如果代理回复 `HEARTBEAT_OK`（可以选择带有短填充；参见 `agents.defaults.heartbeat.ackMaxChars`），OpenClaw 将阻止该心跳的出站传递。
- 默认情况下，允许向 DM 风格的 `user:<id>` 目标进行心跳传递。设置 `agents.defaults.heartbeat.directPolicy: "block"` 以阻止直接目标传递，同时保持心跳运行处于活动状态。
...
来自代理的出站附件：在单独的一行中包含 `MEDIA:<path-or-url>`（不要有空格）。示例：
...
- 网关操作：[Gateway runbook](/zh/en/gateway)
