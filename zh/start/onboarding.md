...
如需了解入门路径的总体概述，请参阅[入门概述](/en/start/onboarding-overview)。
...
- 共享/多用户设置需要锁定（分离信任边界，保持最低限度的工具访问，并遵循[安全](/en/gateway/security)指南）。
...
<Step title="入门聊天（专用会话）">
  设置完成后，该应用会打开一个专用的入门聊天会话，以便代理可以自我介绍并指导后续步骤。这会将首次运行的指导与您的正常对话区分开来。有关首次运行代理期间网关主机上发生的情况，请参阅 [引导启动](/en/start/bootstrapping)。
</Step>
</Steps>
