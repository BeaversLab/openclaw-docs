...
- Footer: connection state + agent + 会话 + 模型 + think/verbose/reasoning + token counts + deliver.
...
- 如果您期望在聊天渠道中收到消息，请启用传递（`/deliver on` 或 `--deliver`）。
