...
Edge TTS 通过 `node-edge-tts` 库使用 Microsoft Edge 的在线神经网络 TTS 服务。它是一项托管服务（非本地），使用 Microsoft 的端点，并且不需要 API 密钥。`node-edge-tts` 公开了语音配置选项和输出格式，但并非所有选项都受 Edge 服务支持。citeturn2search0
...
Edge TTS **不**需要 API 密钥。如果未找到 API 密钥，OpenClaw 默认使用
Edge TTS（除非通过 `messages.tts.edge.enabled=false` 禁用）。

如果配置了多个提供商，则首先使用选定的提供商，其他的作为备用选项。
...
否。自动 TTS 默认为**关闭**状态。可以在配置中通过
`messages.tts.auto` 启用，或通过 `/tts always`（别名：`/tts on`）按会话启用。
...
完整架构请参阅 [Gateway configuration](/zh/en/gateway/configuration)。
...
- `elevenlabs.languageCode`：双字母 ISO 639-1 代码 (例如 `en`, `de`)
- `elevenlabs.seed`：整数 `0..4294967295` (尽力确定的确定性)
...
- **Telegram**：Opus 语音消息 (`opus_48000_64` 来自 ElevenLabs，`opus` 来自 OpenAI)。
  - 48kHz / 64kbps 是语音消息的良好折衷方案，并且是圆形气泡所必需的。
...
- **Edge TTS**：使用 `edge.outputFormat` (默认 `audio-24khz-48kbitrate-mono-mp3`)。
  - `node-edge-tts` 接受 `outputFormat`，但并非所有格式在 Edge 服务中都可用。
...
OpenAI/ElevenLabs 格式是固定的；Telegram 语音消息体验需要 Opus。
...
- 跳过非常短的回复（< 10 个字符）。
- 当使用 `agents.defaults.model.primary`（或 `summaryModel`）启用时，会对长回复进行总结。
- 将生成的音频附加到回复中。

如果回复超过 `maxLength` 且总结功能已关闭（或没有总结模型的 API 密钥），则跳过音频并发送普通文本回复。
...
只有一个命令：`/tts`。有关启用的详细信息，请参阅 [斜杠命令](/zh/en/tools/slash-commands)。

Discord 注意：`/tts` 是 Discord 的内置命令，因此 OpenClaw 在此处将 `/voice` 注册为原生命令。文本 `/tts ...` 仍然有效。
...
- `off|always|inbound|tagged` 是单次会话开关（`/tts on` 是 `/tts always` 的别名）。
