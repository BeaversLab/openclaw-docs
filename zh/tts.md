...
完整架构在 [网关配置](/en/gateway/configuration) 中。
...
- `timeoutMs`：请求超时（毫秒）。
- `prefsPath`：覆盖本地首选项 JSON 路径（provider/limit/summary）。
...
- `openai.baseUrl`：覆盖 OpenAI TTS 端点。
  ...
  - 非默认值被视为兼容 OpenAI 的 TTS 端点，因此接受自定义模型和语音名称。
...
默认情况下，模型**可以**针对单个回复发出 TTS 指令。
当 `messages.tts.auto` 为 `tagged` 时，必须存在这些指令才能触发音频。

启用后，模型可以发出 `[[tts:...]]` 指令来覆盖单次回复的语音，以及一个可选的 `[[tts:text]]...[[/tts:text]]` 块，用于提供仅应出现在音频中的表达性标签（如笑声、歌唱提示等）。
...
- **Edge TTS**：使用 `edge.outputFormat`（默认 `audio-24khz-48kbitrate-mono-mp3`）。
  - `node-edge-tts` 接受 `outputFormat`，但并非所有格式都能从
    Edge 服务获得。 citeturn2search0
  - 输出格式值遵循 Microsoft 语音输出格式（包括 Ogg/WebM Opus）。 citeturn1search0
  - Telegram `sendVoice` 接受 OGG/MP3/M4A；如果您需要
    保证的 Opus 语音笔记，请使用 OpenAI/ElevenLabs。 citeturn1search1
...
如果回复超过 `maxLength` 且摘要功能已关闭（或者摘要模型没有 API 密钥），
将跳过音频并发送正常的文本回复。
...
只有一个命令：`/tts`。
有关启用详情，请参阅 [斜杠命令](/en/tools/slash-commands)。

Discord 注意：`/tts` 是 Discord 的内置命令，因此 OpenClaw 在那里注册
