...
- 有用的覆盖选项：
  - `OPENCLAW_E2E_WORKERS=<n>` 用于强制工作进程数量（上限为 16）。
...
- API 密钥轮换（特定于提供商）：使用逗号/分号格式设置 `*_API_KEYS` 或 `*_API_KEY_1`、`*_API_KEY_2`（例如 `OPENAI_API_KEYS`、`ANTHROPIC_API_KEYS`、`GEMINI_API_KEYS`），或通过 `OPENCLAW_LIVE_*_KEY` 进行每次运行的覆盖；测试在遇到速率限制响应时会重试。

## 我应该运行哪个测试套件？

请使用此决策表：

- 编辑逻辑/测试：运行 `pnpm test`（如果改动较大，还请运行 `pnpm test:coverage`）
- 涉及网关网络 / WS 协议 / 配对：添加 `pnpm test:e2e`
- 调试“我的机器人已停止”/ 特定于提供商的故障 / 工具调用：运行 narrowed `pnpm test:live`
...
- 范围：
  ...
  - 针对所选 Android 节点的逐条命令网关 `node.invoke` 验证。
- 必需的预设置：
  - Android 应用已连接并与网关配对。
  - 应用保持在前台。
  - 已为你期望通过的功能授予权限/捕获同意。
...
- 设置 `OPENCLAW_LIVE_MODELS=modern`（或 `all`，modern 的别名）以实际运行此测试套件；否则它会跳过，以保持 `pnpm test:live` 专注于网关冒烟测试
...
- Probe details (so you can explain failures quickly):
  ...
  - `exec+read` 探测：测试要求代理将 nonce `exec` 写入临时文件，然后 `read` 回来。
...
- 启用：
  - `pnpm test:live` (或者如果是直接调用 Vitest 则使用 `OPENCLAW_LIVE_TEST=1`)
...
- 覆盖（可选）：
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` 发送真实的图像附件（路径被注入到提示词中）。
...
- `OPENCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` 保持 Claude Code CLI MCP 配置启用（默认情况下使用临时空文件禁用 MCP 配置）。
...
在 `OPENCLAW_LIVE_GATEWAY_MODELS` 中至少包含一个支持图像的模型 (Claude/Gemini/OpenAI 支持视觉的变体等)，以测试图像探测器。

### 聚合器 / 备用网关

如果你已启用密钥，我们还支持通过以下方式进行测试：

- OpenRouter： `openrouter/...` （数百个模型；使用 `openclaw models scan` 查找支持工具和图像功能的候选）
...
提示：不要试图在文档中硬编码“所有模型”。权威列表是 `discoverModels(...)` 在你的机器上返回的内容加上任何可用的密钥。
...
- 如果实时测试显示“no creds”（无凭证），请按照调试 `openclaw models list` / 模型选择的方式进行调试。

- 配置存储： `~/.openclaw/credentials/` （首选；即测试中“profile keys”的含义）
- 配置： `~/.openclaw/openclaw.json` （或 `OPENCLAW_CONFIG_PATH` ）

如果你想依赖环境密钥（例如在你的 `~/.profile` 中导出的），请在 `source ~/.profile` 之后运行本地测试，或者使用下面的 Docker 运行器（它们可以将 `~/.profile` 挂载到容器中）。
...
这些运行 `pnpm test:live` 在仓库 Docker 镜像内部，挂载你的本地配置目录和工作区（如果挂载了则获取 `~/.profile`）：
...
- `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` 以确保凭据来自配置文件存储（而不是环境变量）
...
- Gateway(网关) 工具调用 (模拟 OpenAI, 真实网关 + 代理循环): `src/gateway/gateway.test.ts` (案例: "runs a mock OpenAI 工具 call end-to-end via gateway agent loop")
- Gateway(网关) 向导 (WS `wizard.start`/`wizard.next`, 写入配置 + 强制认证): `src/gateway/gateway.test.ts` (案例: "runs wizard over ws and writes auth token config")
