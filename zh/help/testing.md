...
- 池说明：
  ...
  - 使用 `OPENCLAW_TEST_VM_FORKS=0` 手动覆盖（强制 `forks`）或 `OPENCLAW_TEST_VM_FORKS=1` （强制 `vmForks`）。
...
- 预期：
  - 出于设计原因，在 CI 中不稳定（真实网络、真实的提供商策略、配额、服务中断）
  - 需要付费 / 使用速率限制
  - 优先运行缩小的子集，而不是“全部”
  - Live 运行将从 `~/.profile` 获取配置以拾取缺失的 API 密钥
- API 密钥轮换（特定于提供商）：使用逗号/分号格式设置 `*_API_KEYS` 或 `*_API_KEY_1`、`*_API_KEY_2`（例如 `OPENAI_API_KEYS`、`ANTHROPIC_API_KEYS`、`GEMINI_API_KEYS`），或者通过 `OPENCLAW_LIVE_*_KEY` 进行每个 live 的覆盖；测试会在遇到速率限制响应时重试。

## 我应该运行哪个套件？

请使用此决策表：

- 编辑逻辑/测试：运行 `pnpm test`（如果您修改了很多内容，还需运行 `pnpm test:coverage`）
...
- 范围：
  - 预先条件/手动设置（该套件不安装/运行/配对应用程序）。
  - 对所选 Android 节点进行逐条命令的网关 `node.invoke` 验证。
- 必需的预先设置：
  - Android 应用已连接并与网关配对。
  - 应用保持在前台。
  - 已为您期望通过的权限授予捕获权限/同意。
...
- 如何选择模型：
  ...
  - 或者 `OPENCLAW_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` （逗号分隔的允许列表）
- 如何选择提供商：
...
- Goal:
  ...
  - 迭代带密钥的模型并断言：
    ...
    - 真实的工具调用有效（读取探测）
    - 可选的额外工具探测（执行+读取探测）
    - OpenAI 回归路径（仅工具调用 → 后续跟进）保持有效
- Probe details（以便快速解释失败原因）：
  - `read` 探测：测试在工作区中写入一个 nonce 文件，并要求代理 `read` 它并将 nonce 回传。
...
- `OPENCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` 保持 Claude Code CLI MCP 配置启用（默认情况下会使用临时空文件禁用 MCP 配置）。
...
- `google-gemini-cli/...` 使用你机器上的本地 Gemini CLI（独立的认证和工具怪癖）。
...
提示：不要尝试在文档中硬编码“所有模型”。权威列表是 `discoverModels(...)` 在你的机器上返回的内容加上任何可用的密钥。
...
- 如果实时测试提示“no creds”（无凭据），请像调试 `openclaw models list` / 模型选择那样进行调试。

- 配置文件存储：`~/.openclaw/credentials/`（首选；即测试中“profile keys”的含义）
- 配置：`~/.openclaw/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）

如果你希望依赖环境变量密钥（例如在你的 `~/.profile` 中导出的），请在 `source ~/.profile` 之后运行本地测试，或使用下面的 Docker 运行器（它们可以将 `~/.profile` 挂载到容器中）。
...
- SecretRef 遍历防护：
  ...
  - 如果你在 `src/secrets/target-registry-data.ts` 中添加了新的 `includeInPlan` SecretRef 目标系列，请更新该测试中的 `classifyTargetClass`。该测试故意在未分类的目标 ids 上失败，以便无法静默跳过新类。
