...
- API 密钥轮换（特定于提供商）：设置 `*_API_KEYS` 使用逗号/分号格式或 `*_API_KEY_1`, `*_API_KEY_2`（例如 `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`）或通过 `OPENCLAW_LIVE_*_KEY` 进行每次实时覆盖；测试会在速率限制响应时重试。

## 我应该运行哪个测试套件？

请使用此决策表：

- 编辑逻辑/测试：运行 `pnpm test`（如果你改动较大，还要运行 `pnpm test:coverage`）
- 涉及网关网络 / WS 协议 / 配对：添加 `pnpm test:e2e`
- 调试“我的机器人宕机了”/ 特定于提供商的故障 / 工具调用：运行缩小范围的 `pnpm test:live`

## Live: Android 节点功能扫描

- 测试：`src/gateway/android-node.capabilities.live.test.ts`
- 脚本：`pnpm android:test:integration`
- 目标：调用连接的 Android 节点当前公布的**每个命令**，并断言命令契约行为。
- 范围：
  - 预设条件/手动设置（该套件不安装/运行/配对应用）。
...
- 完整的 Android 设置详细信息：[Android App](/en/platforms/android)
...
- “Gateway 网关 smoke” 告诉我们完整的 gateway+agent 流管道适用于该模型（会话、历史记录、工具、沙箱策略等）。
...
- Goal:
  ...
  - 对每个模型运行一次小规模的补全（并在需要时运行针对性的回归测试）
- How to enable:
  - `pnpm test:live` (or `OPENCLAW_LIVE_TEST=1` if invoking Vitest directly)
- 设置 `OPENCLAW_LIVE_MODELS=modern`（或 `all`，modern 的别名）以实际运行此套件；否则它会跳过，以保持 `pnpm test:live` 专注于 gateway smoke
- How to select models:
  - `OPENCLAW_LIVE_MODELS=modern` 运行现代化白名单 (Opus/Sonnet/Haiku 4.5, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.5, Grok 4)
  - `OPENCLAW_LIVE_MODELS=all` 是现代化白名单的别名
...
- Why this exists:
  - 将“提供商 API 已损坏 / 密钥无效”与“gateway agent 流管道已损坏”区分开来
  - 包含小型、隔离的回归测试（例如：OpenAI Responses/Codex Responses 推理回放 + 工具调用流）
...
- Probe details (so you can explain failures quickly):
  - `read` 探针：测试在工作区中写入一个 nonce 文件，并要求代理 `read` 读取该文件并将 nonce 回显回来。
...
- 如何选择模型：
  - 默认值：现代允许列表 (Opus/Sonnet/Haiku 4.5, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.5, Grok 4)
...
## Live：Anthropic 设置令牌冒烟测试

- 测试：`src/agents/anthropic.setup-token.live.test.ts`
- 目标：验证 Claude Code CLI setup-token（或粘贴的 setup-token profile）能否完成 Anthropic 提示词。
- 启用：
  - `pnpm test:live` （如果直接调用 Vitest，则使用 `OPENCLAW_LIVE_TEST=1`）
...
- Gemini API 对比 Gemini CLI：
  - API：OpenClaw 通过 HTTP 调用 Google 托管的 Gemini API (API 密钥 / 配置文件身份验证)；这是大多数用户所说的“Gemini”的意思。
  - CLI：OpenClaw 调用本地 `gemini` 二进制文件；它有自己的身份验证，并且行为可能不同（流式传输/工具支持/版本偏差）。

## Live：模型矩阵（我们覆盖的内容）

没有固定的“CI 模型列表”（live 是可选参与的），但这些是建议在拥有密钥的开发机器上定期覆盖的**推荐**模型。
...
Live 测试以与 CLI 相同的方式发现凭证。实际含义：

- 如果 CLI 可以工作，Live 测试应该能找到相同的密钥。
- 如果实时测试提示“no creds”，请按照调试 `openclaw models list` / 模型选择的方式调试。

- 配置文件存储：`~/.openclaw/credentials/`（首选；即测试中“profile keys”的含义）
- 配置：`~/.openclaw/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）

如果你想依赖环境变量密钥 (例如在 `~/.profile` 中导出的)，请在 `source ~/.profile` 之后运行本地测试，或者使用下面的 Docker 运行器 (它们可以将 `~/.profile` 挂载到容器中)。
...
这些在仓库 Docker 镜像内运行 `pnpm test:live`，并挂载您的本地配置目录和工作区（如果挂载了 `~/.profile`，还会获取它）：

- 直接模型：`pnpm test:docker:live-models`（脚本：`scripts/test-live-models-docker.sh`）
- Gateway(网关) + 开发代理：`pnpm test:docker:live-gateway` (脚本：`scripts/test-live-gateway-models-docker.sh`)
- 新手引导向导（TTY，完整脚手架）：`pnpm test:docker:onboard`（脚本：`scripts/e2e/onboard-docker.sh`）
- Gateway(网关) 网络连接（两个容器，WS 认证 + 健康检查）： `pnpm test:docker:gateway-network` (脚本: `scripts/e2e/gateway-network-docker.sh`)
- 插件（自定义扩展加载 + 注册冒烟测试）：`pnpm test:docker:plugins`（脚本：`scripts/e2e/plugins-docker.sh`）

实时模型 Docker 运行器还以只读方式绑定挂载当前的检出目录，并将其暂存到容器内的临时工作目录中。这既保持了运行时镜像的精简，又能针对您的确切本地源码/配置运行 Vitest。
...
- Gateway 网关工具调用（模拟 OpenAI，真实 Gateway 网关 + agent 循环）：`src/gateway/gateway.test.ts`（用例：“runs a mock OpenAI 工具 call end-to-end via gateway agent loop”）
- Gateway(网关) 网关向导 (WS `wizard.start`/`wizard.next`, 强制执行写入配置 + 身份验证): `src/gateway/gateway.test.ts` (case: "runs 向导 over ws and writes auth token config")
...
Skills 方面仍然缺少的内容（请参阅 [Skills](/en/tools/skills)）：

- **决策制定：** 当提示中列出了 Skills 时，agent 是否会选择正确的 skill（或避开不相关的 skill）？
- **合规性：** agent 在使用之前是否读取 `SKILL.md` 并遵循所需的步骤/参数？
- **工作流合约：** 断言工具顺序、会话历史继承以及沙箱边界的多轮场景。

未来的评估应首先保持确定性：

- 使用模拟提供商来断言工具调用及顺序、技能文件读取和会话连线的场景运行器。
- 一小套专注于技能的场景（使用 vs 避免、闸控、提示注入）。
- 只有在 CI 安全的套件就绪后，才进行可选的实时评估（选择性加入、环境门控）。

## 添加回归（指导）

当您修复在实时测试中发现的提供商/模型问题时：

- 如果可能，请添加 CI 安全的回归（模拟/存根提供商，或捕获确切的请求形状转换）
- 如果它本质上是仅实时的（速率限制、身份验证策略），请保持实时测试狭窄，并通过环境变量选择性加入
- 优先定位捕获该 Bug 的最小层：
...
- SecretRef 遍历防护栏：
  ...
  - 如果您在 `src/secrets/target-registry-data.ts` 中添加新的 `includeInPlan` SecretRef 目标系列，请更新该测试中的 `classifyTargetClass`。该测试故意在未分类的目标 ID 上失败，以便无法静默跳过新类。
