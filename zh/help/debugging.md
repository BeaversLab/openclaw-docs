...
`package.json` 和 `tsdown.config.ts` 上发生更改时重启。扩展元数据更改会
重启网关而不会强制 `tsdown` 重新构建；源文件和配置更改仍然
会先重新构建 `dist`。

在 `gateway:watch` 之后添加任何网关 CLI 标志，它们将在每次
重启时被传递。

## Dev 配置文件 + 开发网关 (--dev)

使用 dev 配置文件来隔离状态并启动一个安全、可一次性设置的调试
环境。有 **两个** `--dev` 标志：
...
注意：`--dev` 是一个 **全局** profile 标志，且会被某些运行器“吃掉”。
如果需要明确指定，请使用环境变量形式：
