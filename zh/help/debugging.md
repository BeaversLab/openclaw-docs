...
`package.json` 和 `tsdown.config.ts` 发生变化时，监视器会重新启动。
扩展元数据的更改会重新启动
网关而无需强制 `tsdown` 重建；源文件和配置的更改仍
会先重建 `dist`。
...
使用开发配置文件来隔离状态，并启动一个安全、可
一次性设置的调试环境。有**两个** `--dev` 标志：

- **全局 `--dev` (profile)：** 在 `~/.openclaw-dev` 下隔离状态，
  并将网关端口默认为 `19001`（派生端口随之偏移）。
...
2. **开发引导**（`gateway --dev`）
   - 如果缺少则写入最小配置（`gateway.mode=local`，绑定回环）。
...
注意：`--dev` 是一个**全局**配置标志，某些运行器会将其“吃掉”。
如果需要明确指定，请使用环境变量形式：
...
这是查看推理是否以纯文本增量形式到达（或作为单独的思考块）的最佳方式。
