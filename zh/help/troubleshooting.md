---
summary: "OpenClaw 的基于症状的初步故障排除中心"
read_when:
  - OpenClaw 无法正常工作，且您需要最快的修复途径
  - 在深入研究详细的操作手册之前，您希望进行分流流程
title: "常规故障排除"
---

# 故障排除

如果您只有 2 分钟时间，请将此页面作为分流的入口。
...
- `openclaw status` → 显示已配置的频道且没有明显的身份验证错误。
- `openclaw status --all` → 完整的报告存在且可分享。
- `openclaw gateway probe` → 预期的网关目标可访问 (`Reachable: yes`)。`RPC: limited - missing scope: operator.read` 表示降级的诊断信息，而非连接失败。
- `openclaw gateway status` → `Runtime: running` 和 `RPC probe: ok`。
- `openclaw doctor` → 没有阻碍性的配置/服务错误。
...
如果安装失败并出现 `package.json missing openclaw.extensions`，则该插件包
...
<AccordionGroup>
  <Accordion title="No replies">
    ...
    - `drop guild message (mention required` → 提及拦截在 Discord 中阻止了消息。
    - `pairing request` → 发送者未获批准，正在等待私信配对批准。
    - `blocked` / `allowlist` 在渠道日志中 → 发送者、房间或组被过滤。
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `gateway connect failed:` → UI 定位到错误的 URL/端口或无法访问的网关。
  ...
  <Accordion title="Gateway(网关) 无法启动或服务已安装但未运行">
    ...
    - `refusing to bind gateway ... without auth` → 非环回绑定且未提供令牌/密码。
