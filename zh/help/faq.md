...
针对实际环境设置（本地开发、VPS、多代理、OAuth/API 密钥、模型故障转移）的快速解答以及更深入的故障排除。有关运行时诊断，请参阅 [Troubleshooting](/en/gateway/troubleshooting)。有关完整配置参考，请参阅 [Configuration](/en/gateway/configuration)。
...
- [快速入门和首次运行设置]
  ...
  - [如何在本地与远程服务器上对仪表板（令牌）进行身份验证？](#how-do-i-authenticate-the-dashboard-token-on-localhost-vs-remote)
  - [我需要什么运行时？](#what-runtime-do-i-need)
  - [它能在树莓派上运行吗？](#does-it-run-on-raspberry-pi)
  - [在树莓派上安装有什么技巧吗？](#any-tips-for-raspberry-pi-installs)
  - [卡在“wake up my friend” / 入职无法完成。现在怎么办？](#it-is-stuck-on-wake-up-my-friend-onboarding-will-not-hatch-what-now)
  - [我可以将我的设置迁移到新机器而无需重新进行入职吗？](#can-i-migrate-my-setup-to-a-new-machine-mac-mini-without-redoing-onboarding)
  ...
  - [我如何试用最新的版本？](#how-do-i-try-the-latest-bits)
  ...
  - [Windows 可执行文件输出显示乱码中文文本，我该怎么办](#windows-exec-output-shows-garbled-chinese-text-what-should-i-do)
  ...
  - [入职向导实际上做了什么？](#what-does-the-onboarding-wizard-actually-do)
  ...
  - [Anthropic “setup-token” 身份验证是如何工作的？](#how-does-anthropic-setuptoken-auth-work)
  ...
  - [为什么我从 Anthropic 收到 `HTTP 429: rate_limit_error`？](#why-am-i-seeing-http-429-ratelimiterror-from-anthropic)
  ...
  - [我可以运行一个“快速聊天”代理和一个“用于编码的 Opus”代理吗？](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Homebrew 在 Linux 上有效吗？](#does-homebrew-work-on-linux)
  - [可黑客的安装和 npm 安装有什么区别？](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [我以后可以在 npm 和 git 安装之间切换吗？](#can-i-switch-between-npm-and-git-installs-later)
  - [我应该在我的笔记本电脑还是 VPS 上运行网关？](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
...
- [配置基础](#config-basics)
  ...
  - [为什么我现在在本地主机 上需要一个令牌？](#why-do-i-need-a-token-on-localhost-now)
  - [更改配置后我需要重启吗？](#do-i-have-to-restart-after-changing-config)
  ...
  - [如何在跨设备的专用 Worker 上运行中央 Gateway？](#how-do-i-run-a-central-gateway-with-specialized-workers-across-devices)
  - [OpenClaw 浏览器可以无头模式 运行吗？](#can-the-openclaw-browser-run-headless)
...
- [网关：端口、“已在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [控制 UI 显示“unauthorized”（或持续重连）。现在该怎么办？](#the-control-ui-says-unauthorized-or-keeps-reconnecting-what-now)
...
- [日志记录和调试](#logging-and-debugging)
  ...
  - [“已从网关断开连接：无原因” - 现在该怎么办？](#disconnected-from-gateway-no-reason-what-now)
  - [Telegram setMyCommands 失败。我应该检查什么？](#telegram-setmycommands-fails-what-should-i-check)
  - [TUI 不显示任何输出。我应该检查什么？](#tui-shows-no-output-what-should-i-check)
  - [如何完全停止然后启动 Gateway？](#how-do-i-completely-stop-then-start-the-gateway)
  - [ELI5：`openclaw gateway restart` vs `openclaw gateway`](#eli5-openclaw-gateway-restart-vs-openclaw-gateway)
  - [当发生故障时，获取更多详细信息的最快方法是什么？](#whats-the-fastest-way-to-get-more-details-when-something-fails)
...
6. **运行诊断程序（修复）**
   ...
   修复/迁移配置/状态并运行健康检查。请参阅 [Doctor](/en/gateway/doctor)。
...
7. **网关快照**
   ...
   向运行中的网关请求完整快照（仅限 WS）。请参阅 [健康检查](/en/gateway/health)。
...
有关绑定模式和身份验证详情，请参阅 [Dashboard](/en/web/dashboard) 和 [Web surfaces](/en/web)。
...
提示：一台小型 Pi/VPS 即可托管网关，您可以在笔记本电脑/手机上配对 **节点**，以用于本地屏幕/摄像头/画布或命令执行。请参阅 [节点](/en/nodes)。
...
如果网关是远程的，请确保隧道/Tailscale 连接正常，并且 UI 指向了正确的网关。请参阅 [远程访问](/en/gateway/remote)。
...
相关：[迁移](/en/install/migrating)，[数据在磁盘上的位置](/en/help/faq#where-does-openclaw-store-its-data)，
[Agent 工作区](/en/concepts/agent-workspace)，[诊断工具](/en/gateway/doctor)，
...
某些 Comcast/Xfinity 连接通过 Xfinity 高级安全功能错误地阻止了 `docs.openclaw.ai`。请禁用该功能或将 `docs.openclaw.ai` 加入允许列表，然后重试。更多详情：[故障排除](/en/help/troubleshooting#docsopenclawai-shows-an-ssl-error-comcastxfinity)。
...
如果挂起，请使用 [Installer stuck](/en/help/faq#installer-stuck-how-do-i-get-more-feedback)
以及 [Im stuck](/en/help/faq#im-stuck--whats-the-fastest-way-to-get-unstuck) 中的快速调试循环。
...
- 完整指南：[Getting Started](/en/start/getting-started)。
...
Hub: [Platforms](/en/platforms)。远程访问: [Gateway remote](/en/gateway/remote)。
Nodes: [Nodes](/en/nodes), [Nodes CLI](/en/cli/nodes)。
...
`claude setup-token` 通过 Claude Code CLI 生成一个 **令牌字符串**（在网络控制台中不可用）。您可以在 **任何机器** 上运行它。在向导中选择 **Anthropic 令牌 (粘贴 setup-token)** 或使用 `openclaw models auth paste-token --provider anthropic` 粘贴它。该令牌作为 **anthropic** 提供商的身份验证配置文件存储，并像 API 密钥一样使用（不自动刷新）。更多详情：[OAuth](/en/concepts/oauth)。
...
复制它打印出的令牌，然后在向导中选择 **Anthropic 令牌 (粘贴 setup-token)**。如果您想在网关主机上运行它，请使用 `openclaw models auth setup-token --provider anthropic`。如果您在其他地方运行了 `claude setup-token`，请使用 `openclaw models auth paste-token --provider anthropic` 将其粘贴到网关主机上。请参阅 [Anthropic](/en/providers/anthropic)。

### 您是否支持 Claude 订阅身份验证（Claude Pro 或 Max）

是的 - 通过 **setup-token**。OpenClaw 不再复用 Claude Code CLI OAuth 令牌；请使用 setup-token 或 Anthropic API 密钥。可以在任何地方生成令牌，然后将其粘贴到网关主机上。请参阅 [Anthropic](/en/providers/anthropic) 和 [OAuth](/en/concepts/oauth)。
...
请参阅 [Models](/en/cli/models)、[OAuth](/en/concepts/oauth) 和
...
是的 - 通过 pi-ai 的 **Amazon Bedrock (Converse)** 提供程序并使用**手动配置**。您必须在网关主机上提供 AWS 凭证/区域，并在模型配置中添加 Bedrock 提供程序条目。请参阅 [Amazon Bedrock](/en/providers/bedrock) 和 [Model providers](/en/providers/models)。如果您更倾向于托管密钥流程，在 Bedrock 前面部署一个兼容 OpenAI 的代理仍然是一个有效的选择。

### Codex 认证如何工作

OpenClaw 通过 OAuth (ChatGPT 登录) 支持 **OpenAI Code (Codex)**。向导可以运行 OAuth 流程，并将在适当时将默认模型设置为 `openai-codex/gpt-5.4`。请参阅 [Model providers](/en/concepts/model-providers) 和 [Wizard](/en/start/wizard)。
...
这会将 OAuth 令牌存储在网关主机的身份验证配置文件中。详情：[模型提供商](/en/concepts/model-providers)。

### 本地模型是否适合用于非正式聊天？

通常不行。OpenClaw 需要大上下文 + 强安全性；小卡会截断并泄露。如果你必须使用，请在本地运行你能运行的 **最大的** MiniMax M2.5 版本（LM Studio），并参阅 [/gateway/local-models](/en/gateway/local-models)。更小/量化的模型会增加提示词注入风险 - 请参阅 [Security](/en/gateway/security)。
...
你只需要一台 Mac **用于仅限 macOS 的工具**。对于 iMessage，请使用 [BlueBubbles](/en/channels/bluebubbles)（推荐）- BlueBubbles 服务器可在任何 Mac 上运行，而 Gateway 可在 Linux 或其他地方运行。如果你想要其他仅限 macOS 的工具，请在 Mac 上运行 Gateway 或配对一个 macOS 节点。

文档：[BlueBubbles](/en/channels/bluebubbles)、[Nodes](/en/nodes)、[Mac 远程模式](/en/platforms/mac/remote)。

### 我是否需要 Mac mini 才能支持 iMessage

您需要一台**已登录到信息的 macOS 设备**。它**不一定**要是 Mac mini——任何 Mac 都可以。**使用 [BlueBubbles](/en/channels/bluebubbles)**（推荐）来实现 iMessage 功能——BlueBubbles 服务器运行在 macOS 上，而 Gateway 可以运行在 Linux 或其他地方。
...
可以。**Mac mini 可以运行 Gateway**，而您的 MacBook Pro 可以作为**节点**（companion device，配套设备）连接。节点不运行 Gateway —— 它们提供额外的功能，例如该设备上的屏幕/相机/画布和 `system.run`。
...
- 私信（DM）您的机器人，然后运行 `openclaw logs --follow` 并读取 `from.id`。

官方 Bot API：

- 私信（DM）您的机器人，然后调用 `https://api.telegram.org/bot<bot_token>/getUpdates` 并读取 `message.from.id`。

第三方（隐私性较低）：

- 私信 `@userinfobot` 或 `@getidsbot`。

参见 [/channels/telegram](/en/channels/telegram#access-control-dms--groups)。

### 多个人可以共用一个 WhatsApp 号码搭配不同的 OpenClaw 实例吗

可以，通过**多智能体路由**。将每个发送者的 WhatsApp **私信**（DM）（peer `kind: "direct"`，发送者 E.164 号码如 `+15551234567`）绑定到不同的 `agentId`，这样每个人都可以获得自己的工作区和会话存储。回复仍然来自**同一个 WhatsApp 账户**，且私信访问控制（`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`）是针对每个 WhatsApp 账户的全局设置。请参阅[多智能体路由](/en/concepts/multi-agent)和 [WhatsApp](/en/channels/whatsapp)。

### 我可以同时运行一个快速聊天代理和一个用于编程的 Opus 代理吗

是的。使用多代理路由：为每个代理指定其默认模型，然后将入站路由（提供商账户或特定对等端）绑定到每个代理。配置示例位于 [Multi-Agent Routing](/en/concepts/multi-agent)。另请参阅 [Models](/en/concepts/models) 和 [Configuration](/en/gateway/configuration)。
...
备份提示：请参阅 [Backup strategy](/en/help/faq#whats-the-recommended-backup-strategy)。
...
**OpenClaw 专属说明：** 在 VPS 上使用 WhatsApp/Telegram/Slack/Mattermost（插件）/Discord 均可正常运行。唯一的真正权衡是 **无头浏览器（headless browser）** 与可见窗口之间的选择。请参阅 [浏览器](/en/tools/browser)。
...
如果您想要两全其美，请将 Gateway 保留在专用主机上，并将您的笔记本电脑作为本地屏幕/摄像头/exec工具的 **node** 进行配对。请参阅 [Nodes](/en/nodes)。
有关安全指南，请阅读 [Security](/en/gateway/security)。
...
文档：[Gateway](/en/gateway)、[Channels](/en/channels)、[Multi-agent](/en/concepts/multi-agent)、
...
文档：[Cron jobs](/en/automation/cron-jobs)、[Cron vs Heartbeat](/en/automation/cron-vs-heartbeat)、[Heartbeat](/en/gateway/heartbeat)。

### 我可以在 Linux 上运行仅限 Apple macOS 的技能吗？

不能直接运行。macOS 技能受 `metadata.openclaw.os` 以及必需的二进制文件限制，并且这些技能只有在 **Gateway 主机** 上符合条件时才会出现在系统提示中。在 Linux 上，除非您覆盖限制，否则 `darwin` 专属技能（如 `apple-notes`、`apple-reminders`、`things-mac`）将不会加载。
...
在 Linux 上运行 Gateway，配对一个 macOS 节点（菜单栏应用），并在 Mac 上将 **节点运行命令** 设置为“始终询问”或“始终允许”。当节点上存在必需的二进制文件时，OpenClaw 可以将 macOS 专属技能视为符合条件。代理通过 `nodes` 工具运行这些技能。如果您选择“始终询问”，在提示中批准“始终允许”会将该命令添加到允许列表中。
...
ClawHub 安装到当前目录下的 `./skills` 中（或者回退到您配置的 OpenClaw 工作区）；OpenClaw 在下一次会话中将其视为 `<workspace>/skills`。为了在代理之间共享技能，请将它们放在 `~/.openclaw/skills/<name>/SKILL.md` 中。某些技能需要通过 Homebrew 安装二进制文件；在 Linux 上这意味着 Linuxbrew（请参阅上面的 Homebrew Linux FAQ 条目）。参见 [技能](/en/tools/skills) 和 [ClawHub](/en/tools/clawhub)。
...
完整指南（包括远程网关 + 安全说明）：[Chrome 扩展程序](/en/tools/chrome-extension)
...
是的。请参阅 [沙箱隔离](/en/gateway/sandboxing)。有关 Docker 特定设置（Docker 中的完整网关或沙箱镜像），请参阅 [Docker](/en/install/docker)。
...
使用 `agents.defaults.sandbox.mode: "non-main"`，使群组/频道会话（非主键）在 Docker 中运行，而主 DM 会话保留在主机上。然后通过 `tools.sandbox.tools` 限制沙盒会话中可用的工具。

设置演练 + 示例配置：[群组：个人私信 + 公共群组](/en/channels/groups#pattern-personal-dms-public-groups-single-agent)

关键配置参考：[网关配置](/en/gateway/configuration#agentsdefaultssandbox)

### 如何将主机文件夹挂载到沙箱中

将 `agents.defaults.sandbox.docker.binds` 设置为 `["host:path:mode"]`（例如 `"/home/user/src:/src:ro"`）。全局绑定和每个代理的绑定会合并；当 `scope: "shared"` 时，每个代理的绑定会被忽略。对任何敏感内容使用 `:ro`，并记住绑定会绕过沙盒文件系统隔离墙。有关示例和安全说明，请参阅 [Sandboxing](/en/gateway/sandboxing#custom-bind-mounts) 和 [Sandbox vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check)。
...
OpenClaw 还会运行 **静默预压缩内存刷新**，以提醒模型在自动压缩之前写入持久化笔记。这仅在工作区可写时运行（只读沙箱会跳过此步骤）。请参阅 [Memory](/en/concepts/memory)。
...
文档：[内存](/en/concepts/memory)，[Agent 工作区](/en/concepts/agent-workspace)。
...
如果您宁愿保持本地化，请设置 `memorySearch.provider = "local"`（以及可选的 `memorySearch.fallback = "none"`）。如果您想要 Gemini 嵌入，请设置 `memorySearch.provider = "gemini"` 并提供 `GEMINI_API_KEY`（或 `memorySearch.remote.apiKey`）。我们支持 **OpenAI、Gemini、Voyage、Mistral、Ollama 或本地** 嵌入模型 - 有关设置详细信息，请参阅 [内存](/en/concepts/memory)。
...
[`agents.defaults.sandbox`](/en/gateway/sandboxing) 或每代理沙箱设置。如果您
...
会话状态由 **网关主机** 拥有。如果您处于远程模式，您关注的会话存储位于远程计算机上，而不是您的本地笔记本电脑。请参阅[会话管理](/en/concepts/session)。
...
OpenClaw 默认强制执行令牌身份验证，包括回环。如果未配置令牌，网关启动时会自动生成一个并将其保存到 `gateway.auth.token`，因此 **本地 WS 客户端必须进行身份验证**。这会阻止其他本地进程调用网关。
...
文档：[节点](/en/nodes)、[远程访问](/en/gateway/remote)、[多代理路由](/en/concepts/multi-agent)、[子代理](/en/tools/subagents)、[TUI](/en/web/tui)。
...
默认为 `false`（有界面）。无头模式在某些网站上更有可能触发反机器人检查。请参阅 [浏览器](/en/tools/browser)。
...
简短回答：**将您的电脑配对为节点**。Gateway 在其他地方运行，但它可以通过 Gateway WebSocket 调用您本地机器上的 `node.*` 工具（屏幕、摄像头、系统）。
...
安全提醒：配对 macOS 节点允许在该机器上进行 `system.run`。仅配对您信任的设备，并查看 [安全性](/en/gateway/security)。

文档：[节点](/en/nodes)、[网关协议](/en/gateway/protocol)、[macOS 远程模式](/en/platforms/mac/remote)、[安全性](/en/gateway/security)。
...
文档：[Tailscale](/en/gateway/tailscale)、[远程访问](/en/gateway/remote)、[频道](/en/channels)。
...
**CLI 桥接（通用）：** 运行一个脚本，使用 `openclaw agent --message ... --deliver` 调用另一个 Gateway，目标是另一个机器人监听的聊天。如果一个机器人在远程 VPS 上，请通过 SSH/Tailscale 将您的 CLI 指向该远程 Gateway（请参阅 [远程访问](/en/gateway/remote)）。
...
文档：[节点](/en/nodes)、[节点 CLI](/en/cli/nodes)、[Chrome 扩展](/en/tools/chrome-extension)。
...
文档：[节点](/en/nodes)、[节点 CLI](/en/cli/nodes)、[多网关](/en/gateway/multiple-gateways)。

### 节点是否运行网关服务

不。除非你有意运行隔离的配置文件（参见 [Multiple gateways](/en/gateway/multiple-gateways)），否则每台主机应只运行**一个网关**。节点是连接到网关的外设（iOS/Android 节点，或菜单栏应用程序中的 macOS “节点模式”）。对于无头节点主机和 CLI 控制，请参阅 [Node host CLI](/en/cli/node)。

`gateway`、`discovery` 和 `canvasHost` 的更改需要完全重启。

### 是否有 API RPC 方式来应用配置

是的。`config.apply` 会验证并写入完整配置，并在操作过程中重启网关。
...
这将使网关绑定到回环地址并通过 Tailscale 暴露 HTTPS。请参阅 [Tailscale](/en/gateway/tailscale)。
...
文档：[Gateway 协议](/en/gateway/protocol)，[发现机制](/en/gateway/discovery)，[macOS 远程模式](/en/platforms/mac/remote)。
...
关于完整的优先级和来源，请参阅 [/environment](/en/help/environment)。
...
请参阅 [/concepts/model-providers](/en/concepts/model-providers) 和 [/environment](/en/help/environment)。
...
- 如果向导检测到现有配置，还会提供 **重置** 选项。请参阅 [向导](/en/start/wizard)。
...
按代理覆盖使用 `agents.list[].heartbeat`。文档：[心跳](/en/gateway/heartbeat)。
...
默认情况下，直接聊天会折叠到主会话中。群组/频道拥有自己的会话密钥，而 Telegram 主题 / Discord 线程则是独立的会话。请参阅 [群组](/en/channels/groups) 和 [群组消息](/en/channels/group-messages)。
...
文档：[多智能体路由](/en/concepts/multi-agent)、[Slack](/en/channels/slack)、
[浏览器](/en/tools/browser)、[Chrome 扩展程序](/en/tools/chrome-extension)、[节点](/en/nodes)。
...
MiniMax M2.5 有自己的文档：[MiniMax](/en/providers/minimax) 和
[Local models](/en/gateway/local-models)。

经验法则：在高风险工作中使用你能负担得起的**最佳模型**，而在日常聊天或摘要中使用更便宜
的模型。你可以为每个智能体路由模型，并使用子智能体来
并行处理长任务（每个子智能体都会消耗令牌）。请参阅 [Models](/en/concepts/models) 和
[Sub-agents](/en/tools/subagents)。

严重警告：较弱或过度量化的模型更容易受到提示词注入和不安全行为的影响。请参阅 [安全](/en/gateway/security)。
...
文档：[Models](/en/concepts/models), [Configure](/en/cli/configure), [Config](/en/cli/config), [Doctor](/en/gateway/doctor).
...
请参阅 [MiniMax](/en/providers/minimax) 和 [Models](/en/concepts/models)。
...
- **确认您的环境变量已被 Gateway 加载**
  - 如果您在 shell 中设置了 `ANTHROPIC_API_KEY`，但通过 systemd/launchd 运行 Gateway，它可能无法继承该变量。请将其放入 `~/.openclaw/.env` 中或启用 `env.shellEnv`。
- **确保你正在编辑正确的代理**
...
- 仍然卡住了？运行 `openclaw status --all` 并按照 [故障排除](/en/gateway/troubleshooting) 操作。有关身份验证详细信息，请参阅 [仪表板](/en/web/dashboard)。
...
完整指南：[多网关](/en/gateway/multiple-gateways)。

### 无效的握手代码 1008 是什么意思

网关是一个 **WebSocket 服务器**，它期望收到的第一条消息
...
有关更多信息，请参阅 [故障排除](/en/gateway/troubleshooting#log-locations)。
...
如果您手动运行网关，`openclaw gateway --force` 可以回收端口。请参阅 [网关](/en/gateway)。
...
文档：[Windows (WSL2)](/en/platforms/windows)，[Gateway service runbook](/en/gateway)。
...
文档：[频道](/en/channels)、[故障排除](/en/gateway/troubleshooting)、[远程访问](/en/gateway/remote)。
...
文档：[控制台](/en/web/dashboard)，[远程访问](/en/gateway/remote)，[故障排除](/en/gateway/troubleshooting)。
...
使用 `--verbose` 启动网关以获取更多控制台详细信息。然后检查日志文件中的频道身份验证、模型路由和 RPC 错误。

## 媒体和附件

### 我的技能生成了图像或 PDF，但没有发送任何内容

来自代理的出站附件必须包含一行 `MEDIA:<path-or-url>`（独占一行）。请参阅 [OpenClaw 助手设置](/en/start/openclaw) 和 [代理发送](/en/tools/agent-send)。
...
如果您想进行实验，请在专用账户上进行，并保持其隔离。请参阅[安全](/en/gateway/security)。

### 我可以在个人助理任务中使用更便宜的模型吗

可以，**如果**该代理仅用于聊天且输入是可信的。较小的层级更容易受到指令劫持，因此对于启用了工具的代理或在读取不可信内容时，应避免使用它们。如果您必须使用较小的模型，请锁定工具并在沙盒中运行。请参阅[安全](/en/gateway/security)。
...
斜杠命令概览：请参阅 [Slash commands](/en/tools/slash-commands)。
