...
针对实际设置（本地开发、VPS、多代理、OAuth/API 密钥、模型故障转移）的快速解答以及更深入的故障排除。有关运行时诊断，请参阅 [故障排除](/en/gateway/troubleshooting)。有关完整的配置参考，请参阅 [配置](/en/gateway/configuration)。

## 目录

- [快速入门和首次运行设置]
  - [我卡住了，最快解决问题的方法是什么？](#im-stuck-whats-the-fastest-way-to-get-unstuck)
  ...
  - [关于 Raspberry Pi 安装有什么提示吗？](#any-tips-for-raspberry-pi-installs)
  - [它卡在“wake up my friend”/ 新手引导无法完成。现在怎么办？](#it-is-stuck-on-wake-up-my-friend-onboarding-will-not-hatch-what-now)
  - [我可以将我的设置迁移到新机器（Mac mini）而无需重新进行新手引导吗？](#can-i-migrate-my-setup-to-a-new-machine-mac-mini-without-redoing-onboarding)
  - [我在哪里可以看到最新版本的新功能？](#where-do-i-see-what-is-new-in-the-latest-version)
  ...
  - [我可以让 OpenClaw 自行更新吗？](#can-i-ask-openclaw-to-update-itself)
  - [新手引导向导到底做了什么？](#what-does-the-onboarding-wizard-actually-do)
  - [运行这个需要 Claude 或 OpenAI 的订阅吗？](#do-i-need-a-claude-or-openai-subscription-to-run-this)
  ...
  - [我该如何设置 Gemini CLI OAuth](#how-do-i-set-up-gemini-cli-oauth)
  - [本地模型适合日常聊天吗？](#is-a-local-model-ok-for-casual-chats)
  - [如何让托管模型的流量保持在特定区域？](#how-do-i-keep-hosted-model-traffic-in-a-specific-region)
  - [我必须购买 Mac Mini 才能安装它吗？](#do-i-have-to-buy-a-mac-mini-to-install-this)
  - [支持 iMessage 需要一台 Mac mini 吗？](#do-i-need-a-mac-mini-for-imessage-support)
  ...
  - [我可以同时运行一个“快速聊天”代理和一个“Opus 编程”代理吗？](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Homebrew 可以在 Linux 上运行吗？](#does-homebrew-work-on-linux)
  - [可破解版 (git) 安装和 npm 安装有什么区别？](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  ...
  - [VPS 的最低要求和建议的操作系统是什么？](#what-are-the-minimum-vps-requirements-and-recommended-os)
...
- [什么是 OpenClaw？](#what-is-openclaw)
  ...
  - [OpenClaw 排名前五的日常用例是什么？](#what-are-the-top-five-everyday-use-cases-for-openclaw)
  - [OpenClaw 能否协助 SaaS 进行潜在客户开发、外联广告和博客撰写？](#can-openclaw-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
  - [在 Web 开发方面，与 Claude Code 相比有哪些优势？](#what-are-the-advantages-vs-claude-code-for-web-development)
- [Skills 和自动化](#skills-and-automation)
  - [如何在不弄脏代码仓库的情况下自定义 Skills？](#how-do-i-customize-skills-without-keeping-the-repo-dirty)
  ...
  - [OpenClaw 可以按计划或持续在后台运行任务吗？](#can-openclaw-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [文件内容的存储位置](#where-things-live-on-disk)
  - [与 OpenClaw 一起使用的所有数据都会在本地保存吗？](#is-all-data-used-with-openclaw-saved-locally)
...
- [配置基础](#config-basics)
  ...
  - [我该如何禁用搞笑的 CLI 标语？](#how-do-i-disable-funny-cli-taglines)
  - [我该如何启用网络搜索（和网络抓取）？](#how-do-i-enable-web-search-and-web-fetch)
  - [config.apply 清除了我的配置。我该如何恢复并避免这种情况？](#configapply-wiped-my-config-how-do-i-recover-and-avoid-this)
  - [我该如何运行一个中央 %%PH:GLOSSARY:1167:31d1c5c%% 并在各个设备上配备专用工作节点？](#how-do-i-run-a-central-gateway-with-specialized-workers-across-devices)
  - [OpenClaw 浏览器可以无头模式运行吗？](#can-the-openclaw-browser-run-headless)
...
- [远程网关和节点](#remote-gateways-and-nodes)
  ...
  - [Tailscale 已连接但我没有收到回复。现在该怎么办？](#tailscale-is-connected-but-i-get-no-replies-what-now)
  ...
  - [与从 VPS 使用 SSH 相比，在个人笔记本电脑上使用节点有什么好处吗？](#is-there-a-benefit-to-using-a-node-on-my-personal-laptop-instead-of-ssh-from-a-vps)
  - [节点会运行网关服务吗？](#do-nodes-run-a-gateway-service)
  ...
  - [我应该安装在第二台笔记本电脑上还是直接添加一个节点？](#should-i-install-on-a-second-laptop-or-just-add-a-node)
- [环境变量 和 .env 加载](#env-vars-and-env-loading)
  - [OpenClaw 如何加载环境变量？](#how-does-openclaw-load-environment-variables)
  - [“我通过服务启动了 Gateway(网关)，我的环境变量消失了。”现在怎么办？](#i-started-the-gateway-via-the-service-and-my-env-vars-disappeared-what-now)
...
- [会话和多重聊天](#sessions-and-multiple-chats)
  ...
  - [如果我从不错过 `/new`，会话会自动重置吗？](#do-sessions-reset-automatically-if-i-never-send-new)
  - [有没有办法将一组 OpenClaw 实例组成一个 CEO 和多个代理的团队？](#is-there-a-way-to-make-a-team-of-openclaw-instances-one-ceo-and-many-agents)
  - [为什么上下文会在任务中途被截断？如何防止这种情况？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 OpenClaw 但保留已安装状态？](#how-do-i-completely-reset-openclaw-but-keep-it-installed)
  ...
  - [我是否需要向 WhatsApp 群组添加一个“bot account”？](#do-i-need-to-add-a-bot-account-to-a-whatsapp-group)
  ...
  - [我可以同时运行多个机器人或聊天 (Slack)，该如何设置？](#can-i-run-multiple-bots-or-chats-at-the-same-time-slack-and-how-should-i-set-that-up)
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [我可以在日常任务中使用 GPT 5.2，在编程中使用 Codex 5.3 吗](#can-i-use-gpt-52-for-daily-tasks-and-codex-53-for-coding)
  ...
  - [我可以将 MiniMax 作为默认模型，并在复杂任务中使用 OpenAI 吗？](#can-i-use-minimax-as-my-default-and-openai-for-complex-tasks)
...
- [Auth profiles：它们是什么以及如何管理它们](#auth-profiles-what-they-are-and-how-to-manage-them)
  ...
  - [典型的 profile ID 是什么？](#what-are-typical-profile-ids)
  - [我可以控制首先尝试哪个 auth profile 吗？](#can-i-control-which-auth-profile-is-tried-first)
...
- [Gateway(网关)：端口、“正在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [为什么 `openclaw gateway status` 显示 `Runtime: running` 但 `RPC probe: failed` 却不是？](#why-does-openclaw-gateway-status-say-runtime-running-but-rpc-probe-failed)
  ...
  - [“另一个 gateway 实例正在监听”是什么意思？](#what-does-another-gateway-instance-is-already-listening-mean)
  - [如何在远程模式下运行 OpenClaw（客户端连接到其他地方的 Gateway(网关)）？](#how-do-i-run-openclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere)
  - [控制 UI 显示“unauthorized”（或一直重新连接）。该怎么办？](#the-control-ui-says-unauthorized-or-keeps-reconnecting-what-now)
...
- [日志记录和调试](#logging-and-debugging)
  ...
  - [Gateway(网关)已启动但从未收到回复。我应该检查什么？](#the-gateway-is-up-but-replies-never-arrive-what-should-i-check)
...
- [媒体和附件](#media-and-attachments)
  - [我的技能生成了图像/PDF，但没有任何内容被发送](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [安全与访问控制](#security-and-access-control)
  ...
  - [我的机器人应该拥有自己的电子邮件 GitHub 账户还是电话号码](#should-my-bot-have-its-own-email-github-account-or-phone-number)
  - [我可以让它自主控制我的短信吗？这样安全吗](#can-i-give-it-autonomy-over-my-text-messages-and-is-that-safe)
...
7. **Gateway(网关) 快照**
   ...
   向运行中的网关请求完整快照（仅限 WS）。请参阅 [Health](/en/gateway/health)。
...
快速调试循环：[出现问题时的前 60 秒](#first-60-seconds-if-somethings-broken)。
...
提示：一台小型 Pi/VPS 可以托管 Gateway(网关)，您可以将**节点**与笔记本电脑/手机配对，以进行本地屏幕/摄像头/画布或命令执行。请参阅 [节点](/en/nodes)。
...
如果 Gateway(网关)是远程的，请确保隧道/Tailscale 连接已建立，并且 UI 指向正确的 Gateway(网关)。请参阅 [远程访问](/en/gateway/remote)。
...
相关内容：[迁移](/en/install/migrating)、[文件存储位置](/en/help/faq#where-does-openclaw-store-its-data)、
...
某些 Comcast/Xfinity 连接通过 Xfinity 高级安全功能错误地阻止了 `docs.openclaw.ai`。请禁用它或将 `docs.openclaw.ai` 加入白名单，然后重试。更多详细信息：[故障排除](/en/help/troubleshooting#docsopenclawai-shows-an-ssl-error-comcastxfinity)。
...
更多详细信息：[开发通道](/en/install/development-channels) 和 [安装程序标志](/en/install/installer)。
...
如果挂起，请使用 [安装程序卡住](/en/help/faq#installer-stuck-how-do-i-get-more-feedback)
以及 [我卡住了](/en/help/faq#im-stuck--whats-the-fastest-way-to-get-unstuck) 中的快速调试循环。
...
文档：[更新](/en/cli/update)、[开发通道](/en/install/development-channels)、
[安装](/en/install)。
...
如果您希望获得最流畅的 Windows 安装体验，请使用 **WSL2** 而不是原生 Windows。
...
`claude setup-token` 通过 Claude Code CLI 生成 **令牌字符串**（Web 控制台中不提供此功能）。您可以在 **任何机器** 上运行它。在向导中选择 **Anthropic 令牌（粘贴 setup-token）** 或使用 `openclaw models auth paste-token --provider anthropic` 粘贴它。该令牌作为 **anthropic** 提供商的身份验证配置文件存储，其用法类似于 API 密钥（不会自动刷新）。更多详情：[OAuth](/en/concepts/oauth)。
...
复制它打印的令牌，然后在向导中选择 **Anthropic 令牌（粘贴 setup-token）**。如果要在网关主机上运行它，请使用 `openclaw models auth setup-token --provider anthropic`。如果在其他地方运行了 `claude setup-token`，请使用 `openclaw models auth paste-token --provider anthropic` 将其粘贴到网关主机上。参见 [Anthropic](/en/providers/anthropic)。

### 是否支持 Claude 订阅身份验证（Claude Pro 或 Max）

是的 - 通过 **setup-token**。OpenClaw 不再重用 Claude Code CLI OAuth 令牌；请使用 setup-token 或 Anthropic API 密钥。可以在任何地方生成令牌，并将其粘贴到网关主机上。参见 [Anthropic](/en/providers/anthropic) 和 [OAuth](/en/concepts/oauth)。
...
提示：设置一个 **回退模型**，以便在提供商受到速率限制时 OpenClaw 可以继续回复。
参见 [Models](/en/cli/models)、[OAuth](/en/concepts/oauth) 和
...
是的 - 通过 pi-ai 的 **Amazon Bedrock (Converse)** 提供商进行 **手动配置**。您必须在网关主机上提供 AWS 凭证/区域，并在模型配置中添加 Bedrock 提供商条目。参见 [Amazon Bedrock](/en/providers/bedrock) 和 [Model providers](/en/providers/models)。如果您更喜欢托管密钥流程，在 Bedrock 前面使用 OpenAI 兼容代理仍然是一个有效的选项。

### Codex 身份验证是如何工作的

OpenClaw 通过 OAuth（ChatGPT 登录）支持 **OpenAI Code (Codex)**。向导可以运行 OAuth 流程，并在适当时将默认模型设置为 `openai-codex/gpt-5.4`。参见 [Model providers](/en/concepts/model-providers) 和 [Wizard](/en/start/wizard)。
...
参见 [OAuth](/en/concepts/oauth)、[Model providers](/en/concepts/model-providers) 和 [Wizard](/en/start/wizard)。
...
这会将 OAuth 令牌存储在网关主机的身份验证配置文件中。详情：[Model providers](/en/concepts/model-providers)。

### 本地模型适合用于随意聊天吗

通常不行。OpenClaw 需要大上下文 + 强安全性；小显卡会导致截断和泄漏。如果必须这样做，请在本地运行您能运行的**最大** MiniMax M2.5 版本 (LM Studio) 并参阅 [/gateway/local-models](/en/gateway/local-models)。较小/量化的模型会增加提示注入风险 - 请参阅 [Security](/en/gateway/security)。
...
您只需要一台 Mac **用于仅限 macOS 的工具**。对于 iMessage，请使用 [BlueBubbles](/en/channels/bluebubbles)（推荐）- BlueBubbles 服务器可以在任何 Mac 上运行，而 Gateway(网关) 可以在 Linux 或其他地方运行。如果您需要其他仅限 macOS 的工具，请在 Mac 上运行 Gateway(网关) 或配对 macOS 节点。

文档：[BlueBubbles](/en/channels/bluebubbles)、[Nodes](/en/nodes)、[Mac remote mode](/en/platforms/mac/remote)。

### 我是否需要 Mac mini 来支持 iMessage

您需要**有一台 macOS 设备**登录到 Messages。它**不**一定非得是 Mac mini -
任何 Mac 都可以。**使用 [BlueBubbles](/en/channels/bluebubbles)**（推荐）用于 iMessage - BlueBubbles 服务器在 macOS 上运行，而 Gateway(网关) 可以在 Linux 或其他地方运行。
...
是的，通过**多代理路由**。将每个发送者的 WhatsApp **私信 (私信)**（对等 `kind: "direct"`，发送者 E.164 类似 `+15551234567`）绑定到不同的 `agentId`，这样每个人都可以获得自己的工作区和会话存储。回复仍然来自**同一个 WhatsApp 账户**，并且私信访问控制（`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`）是针对每个 WhatsApp 账户全局的。请参阅 [Multi-Agent Routing](/en/concepts/multi-agent) 和 [WhatsApp](/en/channels/whatsapp)。

### 我可以同时运行一个快速聊天代理和一个用于编码的 Opus 代理吗

是的。使用多代理路由：为每个代理分配其自己的默认模型，然后将入站路由（提供商帐户或特定对等端）绑定到每个代理。配置示例位于[多代理路由](/en/concepts/multi-agent)。另请参阅[模型](/en/concepts/models)和[配置](/en/gateway/configuration)。
...
**OpenClaw 特别说明：** WhatsApp/Telegram/Slack/Mattermost（插件）/Discord 均可在 VPS 上正常运行。唯一真正的权衡是**无头浏览器**与可见窗口之间的区别。请参阅[浏览器](/en/tools/browser)。
...
如果您想两全其美，请将 Gateway(网关)保留在专用主机上，并将您的笔记本电脑作为**节点**配对，用于本地屏幕/摄像头/执行工具。请参阅[节点](/en/nodes)。
有关安全指南，请阅读[安全](/en/gateway/security)。
...
如果您在 VM 中运行 macOS，请参阅 [macOS VM](/en/install/macos-vm)。
...
文档：[Gateway(网关)](/en/gateway)，[频道](/en/channels)，[多代理](/en/concepts/multi-agent)，
...
请参阅[定时任务](/en/automation/cron-jobs)，[多代理路由](/en/concepts/multi-agent)和[斜杠命令](/en/tools/slash-commands)。
...
在存在 macOS 二进制文件的地方运行 Gateway，然后通过 [remote mode](#how-do-i-run-openclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere) 或 Tailscale 从 Linux 连接。Skills 正常加载，因为 Gateway 主机是 macOS。
...
ClawHub 安装到当前目录下的 `./skills` 中（或者回退到您配置的 OpenClaw 工作区）；OpenClaw 在下一次会话中将其视为 `<workspace>/skills`。对于代理之间共享的 skills，请将它们放在 `~/.openclaw/skills/<name>/SKILL.md` 中。某些 skills 需要通过 Homebrew 安装的二进制文件；在 Linux 上这意味着 Linuxbrew（请参阅上面的 Homebrew Linux 常见问题条目）。参见 [Skills](/en/tools/skills) 和 [ClawHub](/en/tools/clawhub)。
...
完整指南（包括远程 Gateway + 安全说明）：[Chrome extension](/en/tools/chrome-extension)
...
关键配置参考：[Gateway configuration](/en/gateway/configuration#agentsdefaultssandbox)

### 如何将主机文件夹绑定到沙箱中

将 `agents.defaults.sandbox.docker.binds` 设置为 `["host:path:mode"]`（例如，`"/home/user/src:/src:ro"`）。全局 + 每个代理的绑定会合并；当 `scope: "shared"` 时，每个代理的绑定会被忽略。对于任何敏感内容请使用 `:ro`，并记住绑定会绕过沙箱文件系统墙。有关示例和安全说明，请参阅 [沙箱隔离](/en/gateway/sandboxing#custom-bind-mounts) 和 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check)。
...
OpenClaw 还会运行 **静默预压缩内存刷新**，以提醒模型在自动压缩之前写入持久化笔记。这仅在工作区可写时运行（只读沙箱会跳过它）。请参阅 [Memory](/en/concepts/memory)。
...
如果您希望保持本地化，请设置 `memorySearch.provider = "local"`（以及可选的 `memorySearch.fallback = "none"`）。如果您需要 Gemini 嵌入，请设置 `memorySearch.provider = "gemini"` 并提供 `GEMINI_API_KEY`（或 `memorySearch.remote.apiKey`）。我们支持 **OpenAI、Gemini、Voyage、Mistral、Ollama 或本地** 嵌入模型 - 有关设置详细信息，请参阅 [Memory](/en/concepts/memory)。
...
- **工作区（每个代理）**：`AGENTS.md`、`SOUL.md`、`IDENTITY.md`、`USER.md`、
  `MEMORY.md`（或者在 `MEMORY.md` 不存在时的旧版回退 `memory.md`）、
...
**请勿**提交 `~/.openclaw` 下的任何内容（凭据、会话、令牌或加密的机密负载）。
如果您需要完全恢复，请分别备份工作空间和状态目录
（请参阅上面的迁移问题）。

文档：[Agent 工作空间](/en/concepts/agent-workspace)。
...
是的。工作空间是**默认 cwd** 和内存锚点，而非严格的沙箱。
相对路径在工作空间内解析，但绝对路径可以访问其他
主机位置，除非启用了沙箱隔离。如果您需要隔离，请使用
[`agents.defaults.sandbox`](/en/gateway/sandboxing) 或按代理设置的沙箱设置。如果您
希望仓库成为默认工作目录，请将该代理的
`workspace` 指向仓库根目录。OpenClaw 仓库仅包含源代码；请将
工作空间分开，除非您有意让代理在其中工作。
...
如果文件丢失，它将使用相对安全的默认值（包括默认工作空间 `~/.openclaw/workspace`）。
...
- 仅在 `gateway.auth.*` 未设置时，本地调用路径才能将 `gateway.remote.*` 作为回退选项。
- 如果 `gateway.auth.token` / `gateway.auth.password` 是通过 SecretRef 显式配置且未解析，则解析将失败关闭（无远程回退屏蔽）。
- 控制 UI 通过 `connect.params.auth.token` 进行身份验证（存储在应用程序/UI 设置中）。请避免将令牌放在 URL 中。

### 为什么现在在 localhost 上也需要令牌

OpenClaw 默认强制执行令牌身份验证，包括回环。如果未配置令牌，Gateway 启动时会自动生成一个并将其保存到 `gateway.auth.token`，因此**本地 WS 客户端必须进行身份验证**。这会阻止其他本地进程调用 Gateway(网关)。
...
- **节点（设备）：** Mac/iOS/Android 作为外设连接并公开本地工具（`system.run`、`canvas`、`camera`）。
...
默认为 `false` (有头模式)。无头模式在某些网站上更容易触发反机器人检测。请参阅[浏览器](/en/tools/browser)。
...
简短回答：**将您的电脑作为节点配对**。Gateway(网关) 在其他地方运行，但它可以通过 Gateway(网关) WebSocket 调用您本地计算机上的 `node.*` 工具（屏幕、摄像头、系统）。
...
安全提醒：配对 macOS 节点允许该机器上的 `system.run` 操作。仅
...
- 频道健康状态：`openclaw channels status`
...
- **更多设备工具。** 除了 `system.run` 之外，节点还暴露了 `canvas`、`camera` 和 `screen`。
- **本地浏览器自动化。** 将 Gateway(网关) 保留在 VPS 上，但在本地运行 Chrome，并通过 Chrome 扩展程序 + 笔记本电脑上的节点主机来中继控制。

SSH 适用于临时 Shell 访问，但对于持续的代理工作流和设备自动化，节点更简单。

文档：[节点](/en/nodes)、[节点 CLI](/en/cli/nodes)、[Chrome 扩展程序](/en/tools/chrome-extension)。
...
不。除非您有意运行隔离的配置文件（请参阅[多个网关](/en/gateway/multiple-gateways)），否则每个主机应仅运行 **一个网关**。节点是连接到网关的外设（iOS/Android 节点，或菜单栏应用程序中的 macOS “节点模式”）。对于无头节点主机和 CLI 控制，请参阅[节点主机 CLI](/en/cli/node)。

更改 `gateway`、`discovery` 和 `canvasHost` 需要完全重启。

### 是否有 API RPC 方式来应用配置

是的。`config.apply` 会在操作过程中验证并写入完整配置，并重启网关。
...
- 对于小更改，请使用 `openclaw config set`。
- 对于交互式编辑，请使用 `openclaw configure`。

文档：[配置](/en/cli/config)、[Configure](/en/cli/configure)、[Doctor](/en/gateway/doctor)。
...
这将保持网关绑定到环回地址，并通过 Tailscale 暴露 HTTPS。请参阅[Tailscale](/en/gateway/tailscale)。
...
文档：[网关协议](/en/gateway/protocol)、[设备发现](/en/gateway/discovery)、[macOS 远程模式](/en/platforms/mac/remote)。
...
这会运行您的登录 shell 并仅导入缺失的预期键名（绝不覆盖）。等效的环境变量：
...
并**不**意味着您的环境变量缺失——它只是意味着 OpenClaw 不会自动加载
您的登录 shell。
...
将 `/new` 或 `/reset` 作为独立消息发送。请参阅[会话管理](/en/concepts/session)。

### 如果我不发送新消息，会话会自动重置吗？

是的。会话在 `session.idleMinutes`（默认 **60**）后过期。**下一条**
...
- 让 bot 总结当前状态并将其写入文件。
- 在长时间任务前使用 `/compact`，并在切换主题时使用 `/new`。
...
- 如果新手引导向导检测到现有配置，它也会提供 **Reset**（重置）选项。请参阅[向导](/en/start/wizard)。
...
这是一个提供商验证错误：模型发出的 `tool_use` 块缺少必需的
`input`。这通常意味着会话历史已过期或损坏（通常发生在长对话
或工具/架构更改之后）。
...
不需要。OpenClaw 在**您自己的账户**上运行，因此如果您在群组中，OpenClaw 就可以看到它。
...
- 您配置了 `channels.whatsapp.groups` 但未配置 `"*"`，且该群组未被列入允许列表。

请参阅 [群组]（/en/channels/groups）和 [群组消息]（/en/channels/group-messages）。

### 群组/线程是否与私信共享上下文

默认情况下，直接聊天会折叠到主会话。群组/频道拥有自己的会话密钥，而 Telegram 话题 / Discord 线程是独立的会话。参见 [群组](/en/channels/groups) 和 [群组消息](/en/channels/group-messages)。
...
- 为每个代理保持一个 **活动** 工作区 (`agents.defaults.workspace`)。
...
模型引用为 `provider/model`（例如：`anthropic/claude-opus-4-6`）。如果省略提供商，OpenClaw 目前会假定 `anthropic` 作为临时弃用回退方案 - 但您仍应 **显式** 设置 `provider/model`。
...
经验法则：对高风险工作使用您能负担的 **最佳模型**，而对常规聊天或摘要使用较便宜的
模型。您可以为每个代理路由模型，并使用子代理来
并行处理长任务（每个子代理消耗令牌）。参见 [模型](/en/concepts/models) 和
[子代理](/en/tools/subagents)。

强烈警告：较弱/过度量化的模型更容易受到提示
注入和不安全行为的影响。参见 [安全性](/en/gateway/security)。

更多背景：[模型](/en/concepts/models)。
...
3. 如果您同时也需要 Ollama Cloud，请运行 `ollama signin`
4. 运行 `openclaw onboard` 并选择 `Ollama`
5. 选择 `Local` 或 `Cloud + Local`

注意：

- `Cloud + Local` 提供给您 Ollama Cloud 模型以及您的本地 Ollama 模型
...
- **快速切换（每次会话）：** `/model gpt-5.2` 用于日常任务，`/model openai-codex/gpt-5.4` 用于结合 Codex OAuth 进行编程。
- **默认 + 切换：** 将 `agents.defaults.model.primary` 设置为 `openai/gpt-5.2`，然后在编程时切换到 `openai-codex/gpt-5.4`（反之亦然）。
...
如果设置了 `agents.defaults.models`，它将成为 `/model` 和任何会话覆盖的 **允许列表**。选择一个不在该列表中的模型将返回：

```
Model "provider/model" is not allowed. Use /model to list available models.
```

该错误会在正常回复**之前**返回。修复方法：将该模型添加到 `agents.defaults.models` 中，移除允许列表，或从 `/model list` 中选择一个模型。
...
1. 升级到 **2026.1.12**（或从源码 `main` 运行），然后重启网关。
2. 确保已配置 MiniMax（向导或 JSON），或者 env/auth 配置文件中存在 MiniMax API 密钥，以便注入提供商。
3. 使用确切的模型 ID（区分大小写）：`minimax/MiniMax-M2.5` 或 `minimax/MiniMax-M2.5-highspeed`。
4. 运行：
...
是的。使用 **MiniMax 作为默认**，并根据需要**按会话**切换模型。回退机制是针对**错误**的，而不是“困难任务”，因此请使用 `/model` 或单独的代理。
...
**请勿**在代理之间重复使用 `agentDir`；这会导致身份验证/会话冲突。
...
如果您的模型配置包含 Google Gemini 作为故障转移（或者您切换到了 Gemini 简写），OpenClaw 将在模型故障转移期间尝试使用它。如果您尚未配置 Google 凭据，您将看到 `No API key found for provider "google"`。

修复方法：提供 Google 身份验证，或者在 `agents.defaults.model.fallbacks` / 别名中删除/避免使用 Google 模型，以免故障转移路由到那里。
...
修复：OpenClaw 现在会为 Google Antigravity Claude 剥离未签名的思考块。如果仍然出现，请启动一个**新会话** 或为该代理设置 `/thinking off`。
...
是的。配置支持配置文件的可选元数据以及每个提供商的排序（`auth.order.<provider>`）。这**不**存储机密；它将 ID 映射到提供商/模式并设置轮换顺序。

OpenClaw 如果配置文件处于短期的 **cooldown**（速率限制/超时/身份验证失败）或更长的 **disabled** 状态（计费/积分不足），可能会暂时跳过它。要检查此情况，请运行 `openclaw models status --json` 并检查 `auth.unusableProfiles`。调整：`auth.cooldowns.billingBackoffHours*`。
...
因为“运行中”是**主管** 的看法（launchd/systemd/schtasks）。RPC 探测是 CLI 实际连接到网关 WebSocket 并调用 `status`。

使用 `openclaw gateway status` 并相信这些行：

- `Probe target:`（探测实际使用的 URL）
- `Listening:`（端口上实际绑定的内容）
- `Last gateway error:`（进程存活但端口未侦听时的常见根本原因）
...
从您希望服务使用的同一个 `--profile` / 环境运行它。

### 另一个 gateway 实例已在监听是什么意思

OpenClaw 通过在启动时立即绑定 WebSocket 监听器（默认 `ws://127.0.0.1:18789`）来强制执行运行时锁定。如果绑定失败并显示 `EADDRINUSE`，它会抛出 `GatewayLockError`，表明另一个实例已在监听。
...
您的网关运行时启用了身份验证（`gateway.auth.*`），但 UI 未发送匹配的令牌/密码。

事实（来自代码）：

- 控制 UI 会将令牌保存在 `sessionStorage` 中，用于当前浏览器标签页会话和选定的网关 URL，因此同标签页刷新可以继续工作，而无需恢复长期存在的 localStorage 令牌持久性。
...
- 最快的方法：`openclaw dashboard`（打印 + 复制仪表板 URL，尝试打开；如果是无头模式则显示 SSH 提示）。
- 如果您还没有令牌：`openclaw doctor --generate-gateway-token`。
- 如果是远程，请先进行隧道传输：`ssh -N -L 18789:127.0.0.1:18789 user@host` 然后打开 `http://127.0.0.1:18789/`。
- 在网关主机上设置 `gateway.auth.token`（或 `OPENCLAW_GATEWAY_TOKEN`）。
- 在控制 UI 设置中，粘贴相同的令牌。
- 如果在一次重试后仍然不匹配，请轮换/重新批准配对的设备令牌：
  - `openclaw devices list`
  - `openclaw devices rotate --device <id> --role operator`
- 仍然卡住了？运行 `openclaw status --all` 并按照 [故障排除](/en/gateway/troubleshooting) 操作。有关身份验证的详细信息，请参阅 [仪表板](/en/web/dashboard)。

### 我设置了 gatewaybind tailnet 但无法绑定 没有任何监听

`tailnet` 绑定从您的网络接口中选择一个 Tailscale IP（100.64.0.0/10）。如果机器未连接到 Tailscale（或接口已关闭），则没有任何内容可供绑定。
...
注意：`tailnet` 是显式的。`auto` 优先使用环回地址；当您仅希望绑定 tailnet 时，请使用 `gateway.bind: "tailnet"`。
...
更多信息请参阅 [故障排除](/en/gateway/troubleshooting#log-locations)。
...
如果您手动运行 Gateway(网关)，`openclaw gateway --force` 可以收回端口。请参阅 [Gateway(网关)](/en/gateway)。
...
文档：[Windows (WSL2)](/en/platforms/windows)，[Gateway(网关) service runbook](/en/gateway)。
...
- 模型身份验证未在 **gateway host** 上加载（请检查 `models status`）。
- 渠道配对/白名单阻止回复（检查渠道配置 + 日志）。
- WebChat/Dashboard 在没有正确令牌的情况下打开。

如果您在远程，请确认 tunnel/Tailscale 连接已建立，并且 Gateway(网关) WebSocket 是可访问的。

文档：[Channels](/en/channels)，[Troubleshooting](/en/gateway/troubleshooting)，[Remote access](/en/gateway/remote)。
...
文档：[Dashboard](/en/web/dashboard)，[Remote access](/en/gateway/remote)，[Troubleshooting](/en/gateway/troubleshooting)。
...
- `BOT_COMMANDS_TOO_MUCH`：Telegram 菜单有太多条目。OpenClaw 已经修剪到 Telegram 的限制并以更少的命令重试，但某些菜单条目仍然需要被丢弃。减少插件/技能/自定义命令，或者如果您不需要该菜单，请禁用 `channels.telegram.commands.native`。
- `TypeError: fetch failed`，`Network request for 'setMyCommands' failed!` 或类似的网络错误：如果您在 VPS 上或在代理后面，请确认允许出站 HTTPS 并且 `api.telegram.org` 的 DNS 正常工作。

如果 Gateway(网关) 是远程的，请确保您正在 Gateway(网关) 主机上查看日志。

文档：[Telegram](/en/channels/telegram)，[Channel 故障排除](/en/channels/troubleshooting)。
...
在 TUI 中，使用 `/status` 查看当前状态。如果您期望在聊天渠道中获得回复，请确保传递已启用（`/deliver on`）。
...
- `openclaw gateway`：在此终端会话的**前台**运行 Gateway(网关)。

如果您安装了该服务，请使用 gateway 命令。当您想要一次性前台运行时，请使用 `openclaw gateway`。

### 当出现故障时，获取更多详细信息的最快方法是什么

使用 `--verbose` 启动 Gateway(网关) 以获取更多控制台详细信息。然后检查日志文件中的渠道身份验证、模型路由和 RPC 错误。

## 媒体和附件

### 我的技能生成了图片或 PDF，但未发送任何内容

从代理发出的出站附件必须包含 `MEDIA:<path-or-url>` 行（单独一行）。请参阅 [OpenClaw 助手设置](/en/start/openclaw) 和 [代理发送](/en/tools/agent-send)。
...
- 公开开启私信需要明确选择加入（`dmPolicy: "open"` 和允许列表 `"*"`）。
...
如果您想进行实验，请在专用帐户上进行并保持隔离。请参阅
[安全性](/en/gateway/security)。

### 我能否为个人助理任务使用更便宜的模型

可以，**前提是** 代理仅限聊天且输入受信任。较小的层级更容易
受到指令劫持，因此对于启用工具的代理或在读取不受信任内容时
应避免使用它们。如果必须使用较小的模型，请锁定
工具并在沙箱中运行。请参阅 [安全性](/en/gateway/security)。

### 我在 Telegram 中运行了 start 但没有获得配对代码

配对代码**仅**在未知发送者向机器人发送消息并
启用 `dmPolicy: "pairing"` 时发送。单独使用 `/start` 不会生成代码。
...
如果您想立即访问，请将您的发送者 ID 加入允许列表或为该帐户设置 `dmPolicy: "open"`
。
...
如果仍然很嘈杂，请在控制 UI 中检查会话设置并将 verbose（详细模式）设置为 **inherit**（继承）。同时确认您没有在配置中使用设置了 `verboseDefault` 为 `on` 的机器人配置文件。

文档：[Thinking and verbose（思考与详细模式）](/en/tools/thinking)、[Security（安全）](/en/gateway/security#reasoning--verbose-output-in-groups)。
...
斜杠命令概览：参见 [Slash commands（斜杠命令）](/en/tools/slash-commands)。
...
编辑配置后重启 Gateway。如果您只想对单个代理执行此操作，请在 `agents.list[].tools.message` 下进行设置。
...
仍然卡住了？在 [Discord](https://discord.com/invite/clawd) 中提问或发起一个 [GitHub 讨论](https://github.com/openclaw/openclaw/discussions)。
