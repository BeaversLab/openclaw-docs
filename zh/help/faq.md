...
针对真实环境设置（本地开发、VPS、多代理、OAuth/API 密钥、模型故障转移）的快速解答以及更深入的故障排除。有关运行时诊断，请参阅[故障排除](/en/gateway/troubleshooting)。有关完整的配置参考，请参阅[配置](/en/gateway/configuration)。

## 目录

- [快速入门和首次运行设置]
  - [卡住了，最快的解决问题方法是什么？](#im-stuck-whats-the-fastest-way-to-get-unstuck)
  ...
  - [关于 Raspberry Pi 安装有什么建议吗？](#any-tips-for-raspberry-pi-installs)
  - [它卡在“wake up my friend”上 / 新手引导无法启动。现在怎么办？](#it-is-stuck-on-wake-up-my-friend-onboarding-will-not-hatch-what-now)
  ...
  - [Windows 可执行输出显示乱码中文，我该怎么办](#windows-exec-output-shows-garbled-chinese-text-what-should-i-do)
  ...
  - [新手引导向导到底是做什么的？](#what-does-the-onboarding-wizard-actually-do)
  ...
  - [Telegram：`allowFrom` 里面填什么？](#telegram-what-goes-in-allowfrom)
  - [多个人可以在不同的 OpenClaw 实例中使用一个 WhatsApp 号码吗？](#can-multiple-people-use-one-whatsapp-number-with-different-openclaw-instances)
  - [我可以运行一个“快速聊天”代理和一个“Opus 用于编码”代理吗？](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Homebrew 在 Linux 上能运行吗？](#does-homebrew-work-on-linux)
  - [可黑客修改的 (git) 安装和 npm 安装有什么区别？](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
...
- [OpenClaw 是什么？](#what-is-openclaw)
  ...
  - [OpenClaw 能否帮助 SaaS 进行潜在客户开发、外联广告和博客运营](#can-openclaw-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
  - [与 Claude Code 相比，它在 Web 开发方面有哪些优势？](#what-are-the-advantages-vs-claude-code-for-web-development)
- [Skills 和自动化](#skills-and-automation)
  ...
  - [机器人在执行繁重任务时冻结。我该如何卸载该任务？](#the-bot-freezes-while-doing-heavy-work-how-do-i-offload-that)
  ...
  - [OpenClaw 可以按计划运行任务还是在后台连续运行？](#can-openclaw-run-tasks-on-a-schedule-or-continuously-in-the-background)
  - [我可以从 macOS 运行仅适用于 Apple Linux 的 Skills 吗？](#can-i-run-apple-macos-only-skills-from-linux)
...
- [配置基础](#config-basics)
  ...
  - [为什么我现在在 localhost 上需要令牌？](#why-do-i-need-a-token-on-localhost-now)
  ...
  - [如何使用跨设备的专用工作程序运行中央 Gateway(网关)？](#how-do-i-run-a-central-gateway-with-specialized-workers-across-devices)
...
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  ...
  - [“我通过服务启动了 Gateway，我的环境变量消失了。”现在该怎么办？](#i-started-the-gateway-via-the-service-and-my-env-vars-disappeared-what-now)
...
- [会话和多个聊天](#sessions-and-multiple-chats)
  ...
  - [有没有办法让一组 OpenClaw 实例组成一个 CEO 和多个代理的团队](#is-there-a-way-to-make-a-team-of-openclaw-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断？如何防止这种情况？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 OpenClaw 但保留其安装状态？](#how-do-i-completely-reset-openclaw-but-keep-it-installed)
  ...
  - [群组/主题是否与私信共享上下文？](#do-groupsthreads-share-context-with-dms)
...
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [为什么我会看到“未知模型：minimax/MiniMax-M2.5”？](#why-do-i-see-unknown-model-minimaxminimaxm25)
  - [我可以将 MiniMax 作为默认值，并将 OpenAI 用于复杂任务吗？](#can-i-use-minimax-as-my-default-and-openai-for-complex-tasks)
...
- [身份验证配置文件：它们是什么以及如何管理它们](#auth-profiles-what-they-are-and-how-to-manage-them)
  ...
  - [我可以控制先尝试哪个身份验证配置文件吗？](#can-i-control-which-auth-profile-is-tried-first)
...
- [Gateway：端口、“正在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  - [Gateway 使用哪个端口？](#what-port-does-the-gateway-use)
  ...
  - [控制界面显示“未授权”（或不断重新连接）。现在该怎么办？](#the-control-ui-says-unauthorized-or-keeps-reconnecting-what-now)
  - [我设置了 `gateway.bind: "tailnet"` 但它无法绑定 / 没有任何监听](#i-set-gatewaybind-tailnet-but-it-cant-bind-nothing-listens)
  - [我可以在同一台主机上运行多个 Gateway 吗？](#can-i-run-multiple-gateways-on-the-same-host)
...
- [安全性与访问控制](#security-and-access-control)
  ...
  - [提示词注入仅仅是公共机器人需要担心的问题吗？](#is-prompt-injection-only-a-concern-for-public-bots)
  - [我的机器人应该拥有自己的电子邮箱 GitHub 账号还是电话号码](#should-my-bot-have-its-own-email-github-account-or-phone-number)
...
- [聊天命令、中止任务和“它停不下来”的情况](#chat-commands-aborting-tasks-and-it-wont-stop)
  ...
  - [为什么感觉机器人会“忽略”快速连续发送的消息？](#why-does-it-feel-like-the-bot-ignores-rapidfire-messages)
...
4. **深度探测**
   ...
   运行网关健康检查 + 提供商探测（需要可访问的网关）。参见 [健康检查](/en/gateway/health)。
...
6. **运行医生（修复）**
   ...
   修复/迁移配置/状态 + 运行健康检查。参见 [诊断程序](/en/gateway/doctor)。
...
7. **Gateway(网关) 快照**
   ...
   请求正在运行的网关提供完整快照（仅限 WS）。参见 [健康检查](/en/gateway/health)。
...
快速调试循环：[如果出现问题，前 60 秒该怎么做](#first-60-seconds-if-somethings-broken)。
...
有关绑定模式和身份验证详细信息，请参阅 [Dashboard](/en/web/dashboard) 和 [Web 界面](/en/web)。
...
提示：一台小型 Pi/VPS 可以托管 Gateway(网关)，您可以在笔记本电脑/手机上配对 **节点** 以进行本地屏幕/摄像头/画布或命令执行。请参阅 [节点](/en/nodes)。
...
相关：[迁移](/en/install/migrating)、[磁盘上的文件位置](/en/help/faq#where-does-openclaw-store-its-data)、
...
某些 Comcast/Xfinity 连接通过 Xfinity 高级安全设置错误地阻止了 `docs.openclaw.ai`。请禁用它或将 `docs.openclaw.ai` 加入允许列表，然后重试。更多详情：[故障排除](/en/help/troubleshooting#docsopenclawai-shows-an-ssl-error-comcastxfinity)。
请在此处报告以帮助我们解除阻止：[https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status)。
...
更多详情：[开发频道](/en/install/development-channels) 和 [安装程序标志](/en/install/installer)。
...
如果安装挂起，请使用 [安装程序卡住](/en/help/faq#installer-stuck-how-do-i-get-more-feedback)
...
文档：[更新](/en/cli/update)、[开发频道](/en/install/development-channels)、
[安装](/en/install)。
...
如果您希望获得最流畅的Windows设置，请使用 **WSL2** 而不是原生Windows。
...
`claude setup-token` 通过 Claude Code CLI 生成一个**令牌字符串**（Web 控制台中不可用）。您可以在**任何机器**上运行它。在向导中选择 **Anthropic 令牌（粘贴 setup-token）** 或使用 `openclaw models auth paste-token --provider anthropic` 粘贴它。该令牌作为 **anthropic** 提供商的身份验证配置文件存储，并像 API 密钥一样使用（无自动刷新）。更多详情：[OAuth](/en/concepts/oauth)。
...
复制它打印的令牌，然后在向导中选择 **Anthropic 令牌（粘贴 setup-token）**。如果要在网关主机上运行它，请使用 `openclaw models auth setup-token --provider anthropic`。如果 `claude setup-token` 在其他地方运行，请使用 `openclaw models auth paste-token --provider anthropic` 将其粘贴到网关主机上。参见 [Anthropic](/en/providers/anthropic)。

### 是否支持 Claude 订阅身份验证（Claude Pro 或 Max）

是的 - 通过 **setup-token**。OpenClaw 不再重用 Claude Code CLI OAuth 令牌；请使用 setup-token 或 Anthropic API 密钥。可以在任何地方生成令牌并将其粘贴到网关主机上。参见 [Anthropic](/en/providers/anthropic) 和 [OAuth](/en/concepts/oauth)。
...
是的 - 通过 pi-ai 的 **Amazon Bedrock (Converse)** 提供商，使用 **手动配置**。您必须在网关主机上提供 AWS 凭证/区域，并在模型配置中添加 Bedrock 提供商条目。参见 [Amazon Bedrock](/en/providers/bedrock) 和 [模型提供商](/en/providers/models)。如果您更喜欢托管密钥流程，在 Bedrock 前面放置一个兼容 OpenAI 的代理仍然是一个有效的选项。

### Codex 身份验证是如何工作的

OpenClaw 通过 OAuth（ChatGPT 登录）支持 **OpenAI Code (Codex)**。向导可以运行 OAuth 流程，并在适当时将默认模型设置为 `openai-codex/gpt-5.4`。参见 [模型提供商](/en/concepts/model-providers) 和 [向导](/en/start/wizard)。
...
参见 [OAuth](/en/concepts/oauth)、[模型提供商](/en/concepts/model-providers) 和 [向导](/en/start/wizard)。
...
通常不行。OpenClaw 需要大上下文 + 强安全性；小显卡会截断并泄露。如果必须这样做，请在本地运行你能运行的 **最大** MiniMax M2.5 版本（LM Studio），并参阅 [/gateway/local-models](/en/gateway/local-models)。较小/量化的模型会增加提示词注入的风险 - 请参阅 [安全](/en/gateway/security)。
...
你只需要一台 Mac **用于仅限 macOS 的工具**。对于 iMessage，请使用 [BlueBubbles](/en/channels/bluebubbles)（推荐）- BlueBubbles 服务器可以在任何 Mac 上运行，而 Gateway(网关) 可以在 Linux 或其他地方运行。如果你想要其他仅限 macOS 的工具，请在 Mac 上运行 Gateway(网关) 或配对一个 macOS 节点。

文档：[BlueBubbles](/en/channels/bluebubbles)、[节点](/en/nodes)、[Mac 远程模式](/en/platforms/mac/remote)。

### 我是否需要 Mac mini 来支持 iMessage

你需要 **某台 macOS 设备** 登录到信息。它 **不必** 是 Mac mini - 任何 Mac 都可以。**对于 iMessage 请使用 [BlueBubbles](/en/channels/bluebubbles)**（推荐）- BlueBubbles 服务器在 macOS 上运行，而 Gateway(网关) 可以在 Linux 或其他地方运行。
...
是的，通过 **多智能体路由**。将每个发送者的 WhatsApp **私信**（对端 `kind: "direct"`，发送者 E.164 格式如 `+15551234567`）绑定到不同的 `agentId`，这样每个人都能获得自己的工作区和会话存储。回复仍然来自 **同一个 WhatsApp 账户**，并且私信访问控制（`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`）是每个 WhatsApp 账户的全局设置。请参阅 [多智能体路由](/en/concepts/multi-agent) 和 [WhatsApp](/en/channels/whatsapp)。

### 我可以同时运行一个快速聊天代理和一个用于编码的 Opus 代理吗

可以。使用多代理路由：为每个代理分配其默认模型，然后将入站路由（提供商账户或特定对等点）绑定到每个代理。示例配置位于[多代理路由](/en/concepts/multi-agent)中。另请参阅[模型](/en/concepts/models)和[配置](/en/gateway/configuration)。
...
**OpenClaw 特别说明：** WhatsApp/Telegram/Slack/Mattermost (插件)/Discord 均可在 VPS 上正常运行。唯一的真正取舍在于**无头浏览器**与可见窗口之间。请参阅[浏览器](/en/tools/browser)。
...
如果您想两全其美，请将 Gateway 保留在专用主机上，并将您的笔记本电脑配对为用于本地屏幕/摄像头/执行工具的**节点**。请参阅[节点](/en/nodes)。
有关安全指南，请阅读[安全性](/en/gateway/security)。
...
如果您使用的是 Windows，**WSL2 是最简单的 VM 风格设置**，并且具有最佳的工具兼容性。请参阅 [Windows](/en/platforms/windows)，[VPS 托管](/en/vps)。
...
请参阅 [Cron 任务](/en/automation/cron-jobs)，[多代理路由](/en/concepts/multi-agent) 和 [斜杠命令](/en/tools/slash-commands)。
...
在存在 macOS 二进制文件的地方运行 Gateway，然后从 Linux 通过 [remote mode](#how-do-i-run-openclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere) 或通过 Tailscale 连接。skills 正常加载，因为 Gateway 主机是 macOS。
...
ClawHub 安装到当前目录下的 `./skills` 中（或者回退到您配置的 OpenClaw 工作区）；OpenClaw 在下一次会话中将其视为 `<workspace>/skills`。为了在代理之间共享 skills，请将它们放在 `~/.openclaw/skills/<name>/SKILL.md` 中。某些 skills 需要通过 Homebrew 安装的二进制文件；在 Linux 上这意味着 Linuxbrew（请参阅上面的 Homebrew Linux 常见问题条目）。请参阅 [Skills](/en/tools/skills) 和 [ClawHub](/en/tools/clawhub)。
...
完整指南（包括 remote Gateway + 安全说明）：[Chrome extension](/en/tools/chrome-extension)
...
是的。请参阅 [沙箱隔离](/en/gateway/sandboxing)。关于 Docker 特定的设置（Docker 中的完整网关或沙箱镜像），请参阅 [Docker](/en/install/docker)。
...
将 `agents.defaults.sandbox.docker.binds` 设置为 `["host:path:mode"]`（例如 `"/home/user/src:/src:ro"`）。全局 + 每个代理的绑定会合并；当 `scope: "shared"` 时，每个代理的绑定会被忽略。对于任何敏感内容请使用 `:ro`，并请记住绑定会绕过沙箱文件系统隔离。有关示例和安全说明，请参阅 [沙箱隔离](/en/gateway/sandboxing#custom-bind-mounts) 和 [沙箱 vs 工具策略 vs 提权](/en/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check)。
...
可写时运行（只读沙箱会跳过此步骤）。请参阅 [内存](/en/concepts/memory)。
...
如果您希望保持本地使用，请设置 `memorySearch.provider = "local"`（并可选设置
...
相关：[代理工作区](/en/concepts/agent-workspace)，[内存](/en/concepts/memory)。
...
- **工作区（每个代理）**：`AGENTS.md`、`SOUL.md`、`IDENTITY.md`、`USER.md`、
  `MEMORY.md`（当 `MEMORY.md` 不存在时，为旧版后备 `memory.md`）、
  `memory/YYYY-MM-DD.md`、可选 `HEARTBEAT.md`。
...
**请勿**提交 `~/.openclaw` 下的任何内容（凭据、会话、令牌或加密的秘密载荷）。
如果您需要完全还原，请分别备份工作区目录和状态目录
（请参阅上面的迁移问题）。

文档：[Agent 工作区](/en/concepts/agent-workspace)。
...
是。工作区是**默认 cwd**（当前工作目录）和记忆锚点，而不是硬性沙箱。
相对路径在工作区内解析，但绝对路径可以访问其他
主机位置，除非启用了沙箱隔离。如果您需要隔离，请使用
[`agents.defaults.sandbox`](/en/gateway/sandboxing) 或针对每个代理的沙箱设置。如果您
希望某个存储库成为默认工作目录，请将该代理的
`workspace` 指向存储库根目录。OpenClaw 存储库只是源代码；请将
工作区分开，除非您有意让代理在其中工作。
...
会话状态归**网关主机**所有。如果您处于远程模式，您关心的会话存储位于远程计算机上，而不是您的本地笔记本电脑。请参阅[会话管理](/en/concepts/session)。
...
- 如果 `gateway.auth.token` / `gateway.auth.password` 是通过 SecretRef 显式配置的且未解析，解析将失败关闭（没有远程回退屏蔽）。
- 控制 UI 通过 `connect.params.auth.token` 进行身份验证（存储在应用/UI 设置中）。避免将令牌放在 URL 中。

### 为什么现在在 localhost 上也需要令牌

OpenClaw 默认强制执行令牌认证，包括环回。如果未配置令牌，网关启动时会自动生成一个并保存到 `gateway.auth.token`，因此**本地 WS 客户端必须进行认证**。这会阻止其他本地进程调用 Gateway(网关)。

如果您**真的**想要开放环回，请在配置中明确设置 `gateway.auth.mode: "none"`。Doctor 可以随时为您生成令牌：`openclaw doctor --generate-gateway-token`。
...
默认值为 `false` (有界面)。无界面模式在某些网站上更容易触发反机器人检查。参见 [浏览器](/en/tools/browser)。
...
有关完整配置示例，请参阅 [浏览器](/en/tools/browser#use-brave-or-another-chromium-based-browser)。
...
简短回答：**将您的电脑配对为节点**。Gateway(网关) 运行在其他地方，但它可以通过 Gateway(网关) WebSocket 在您的本地计算机上调用 `node.*` 工具（屏幕、摄像头、系统）。
...
安全提醒：配对 macOS 节点允许在该机器上执行 `system.run`。仅配对您信任的设备，并查看 [安全](/en/gateway/security)。
...
**CLI 桥接（通用）：** 运行一个脚本，使用 `openclaw agent --message ... --deliver` 调用另一个 Gateway(网关)，目标是另一个机器人监听的聊天。如果一个机器人在远程 VPS 上，请通过 SSH/Tailscale 将您的 CLI 指向该远程 Gateway(网关)（参见 [远程访问](/en/gateway/remote)）。
...
不。除非您有意运行隔离的配置文件（请参阅 [多网关](/en/gateway/multiple-gateways)），否则每个主机应仅运行 **一个网关**。节点是连接到网关的外设（iOS/Android 节点，或菜单栏应用程序中的 macOS “节点模式”）。对于无头节点主机和 CLI 控制，请参阅 [节点主机 CLI](/en/cli/node)。
...
- 使用 `openclaw config set` 进行小幅更改。
- 使用 `openclaw configure` 进行交互式编辑。

文档：[配置](/en/cli/config)、[配置](/en/cli/configure)、[诊断](/en/gateway/doctor)。
...
这将使网关绑定到环回接口，并通过 Tailscale 暴露 HTTPS。请参阅 [Tailscale](/en/gateway/tailscale)。
...
- 如果新手引导向导检测到现有配置，也会提供 **重置** 选项。请参阅 [向导](/en/start/wizard)。
...
不需要。OpenClaw 在 **您自己的账户** 上运行，所以如果您在群组中，OpenClaw 就可以看到它。
...
- 您配置了 `channels.whatsapp.groups` 但没有 `"*"`，并且该群组不在允许列表中。

请参阅 [群组](/en/channels/groups) 和 [群组消息](/en/channels/group-messages)。

### 群组/线程是否与私信共享上下文

直接聊天默认折叠到主会话。群组/频道拥有自己的会话密钥，Telegram 主题 / Discord 线程是独立的会话。请参阅[群组](/en/channels/groups)和[群组消息](/en/channels/group-messages)。

### 我可以创建多少个工作区和代理

没有硬性限制。几十个（甚至几百个）都可以，但请注意：

- **磁盘增长：** 会话 + 副本位于 `~/.openclaw/agents/<agentId>/sessions/` 下。
- **Token 成本：** 更多代理意味着更多的并发模型使用。
- **运维开销：** 每个代理的身份验证配置文件、工作区和渠道路由。

提示：

- 为每个代理（`agents.defaults.workspace`）保留一个**活跃**工作区。
...
模型被引用为 `provider/model`（例如：`anthropic/claude-opus-4-6`）。如果您省略提供商，OpenClaw 目前假设 `anthropic` 作为临时弃用回退方案 - 但您仍应**明确**设置 `provider/model`。
...
- 云模型（如 `kimi-k2.5:cloud`）不需要本地拉取
- 要手动切换，请使用 `openclaw models list` 和 `openclaw models set ollama/<model>`
...
如果要返回默认值，请从 `/model` 中选择它（或发送 `/model <default provider/model>`）。
...
- **快速切换（每次会话）：** `/model gpt-5.2` 用于日常任务，`/model openai-codex/gpt-5.4` 用于使用 Codex OAuth 编程。
...
该错误是 **代替** 正常回复返回的。修复方法：将模型添加到 `agents.defaults.models`，移除允许列表，或从 `/model list` 中选择一个模型。
...
可以。使用 **MiniMax 作为默认**，并在需要时 **每次会话** 切换模型。
回退是针对 **错误** 的，而不是“困难任务”，因此请使用 `/model` 或单独的代理。
...
可以。OpenClaw 提供了一些默认的简写（仅当模型存在于 `agents.defaults.models` 中时才应用）：
...
- **确认您的环境变量已由 Gateway 加载**
  - 如果您在 shell 中设置了 `ANTHROPIC_API_KEY`，但通过 systemd/launchd 运行 Gateway(网关)，它可能无法继承该设置。将其放入 `~/.openclaw/.env` 或启用 `env.shellEnv`。
- **确保您正在编辑正确的代理**
...
如果您的模型配置包含 Google Gemini 作为回退（或者您切换到了 Gemini 简写），OpenClaw 将在模型回退期间尝试使用它。如果您未配置 Google 凭据，您将看到 `No API key found for provider "google"`。

修复方法：提供 Google 身份验证，或者在 `agents.defaults.model.fallbacks` / 别名中移除/避免使用 Google 模型，以使回退不会路由到那里。
...
修复：OpenClaw 现在会为 Google Antigravity Claude 剥离未签名的思考块。如果它仍然出现，请启动一个 **new 会话** 或为该代理设置 `/thinking off`。
...
OpenClaw 可能会暂时跳过某个配置文件，如果它处于短暂的 **cooldown**（速率限制/超时/身份验证失败）或更长的 **disabled** 状态（计费/积分不足）。要检查此情况，请运行 `openclaw models status --json` 并检查 `auth.unusableProfiles`。调整：`auth.cooldowns.billingBackoffHours*`。
...
`gateway.port` 控制 WebSocket + HTTP 的单一多路复用端口（控制 UI、挂钩等）。
...
因为“正在运行”是 **supervisor's** 的视图（launchd/systemd/schtasks）。RPC 探测是 CLI 实际连接到网关 WebSocket 并调用 `status`。
...
OpenClaw 通过在启动时立即绑定 WebSocket 监听器（默认 `ws://127.0.0.1:18789`）来强制执行运行时锁。如果绑定失败并显示 `EADDRINUSE`，它将抛出 `GatewayLockError`，表明另一个实例已经在监听。
...
- 控制 UI 会将令牌保存在 `sessionStorage` 中，用于当前浏览器标签页会话和选定的网关 URL，因此无需恢复长期的 localStorage 令牌持久性，同一标签页的刷新即可继续工作。
...
- 如果是远程连接，请先进行隧道连接：`ssh -N -L 18789:127.0.0.1:18789 user@host`，然后打开 `http://127.0.0.1:18789/`。
...
`tailnet` 绑定从您的网络接口中选择一个 Tailscale IP (100.64.0.0/10)。如果机器不在 Tailscale 上（或接口已关闭），则没有任何内容可以绑定。
...
注意：`tailnet` 是显式的。`auto` 优先使用环回地址；当您仅限 tailnet 绑定时，请使用 `gateway.bind: "tailnet"`。
...
`connect` 帧。如果收到其他任何内容，它将以 **代码 1008**（策略违规）关闭连接。
...
- macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认值：`~/.openclaw/logs/...`；配置文件使用 `~/.openclaw-<profile>/logs/...`）
- Linux：`journalctl --user -u openclaw-gateway[-<profile>].service -n 200 --no-pager`
- Windows：`schtasks /Query /TN "OpenClaw Gateway (<profile>)" /V /FO LIST`

更多信息请参阅 [Troubleshooting](/en/gateway/troubleshooting#log-locations)。
...
- 未在 **gateway host** 上加载模型身份验证（检查 `models status`）。
...
- `BOT_COMMANDS_TOO_MUCH`：Telegram 菜单条目过多。OpenClaw 已经修剪至 Telegram 限制并使用较少的命令重试，但某些菜单条目仍需丢弃。减少插件/技能/自定义命令，如果不需要该菜单，请禁用 `channels.telegram.commands.native`。
- `TypeError: fetch failed`，`Network request for 'setMyCommands' failed!` 或类似网络错误：如果您使用 VPS 或位于代理之后，请确认允许出站 HTTPS 并且 `api.telegram.org` 的 DNS 正常工作。
...
如果您安装了该服务，请使用网关命令。当您需要一次性前台运行时，使用 `openclaw gateway`。

### 当出现故障时，获取更多详细信息的最快方法是什么

使用 `--verbose` 启动 Gateway 以获取更多控制台详细信息。然后检查日志文件中的渠道认证、模型路由和 RPC 错误。

## 媒体和附件

### 我的技能生成了图片或 PDF，但未发送任何内容

从代理发出的出站附件必须包含一个 `MEDIA:<path-or-url>` 行（独占一行）。请参阅 [OpenClaw assistant setup](/en/start/openclaw) 和 [Agent send](/en/tools/agent-send)。
...
- 支持私信的渠道上的默认行为是**配对**：
  ...
  - 待处理请求限制为每个渠道 **3 个**；如果未收到代码，请检查 `openclaw pairing list --channel <channel> [--account <id>]`。
...
- 对于启用了工具的代理，保持 `web_search` / `web_fetch` / `browser` 关闭
- 沙箱隔离和严格的工具允许列表

详细信息：[Security](/en/gateway/security)。
...
文档：[Security](/en/gateway/security)，[Pairing](/en/channels/pairing)。
...
如果您想进行实验，请在专用帐户上进行并将其隔离。请参阅
[Security](/en/gateway/security)。

### 我能否为个人助理任务使用更便宜的模型

是的，**如果**代理仅用于聊天且输入是受信任的，则可以。较小的层级更容易
受到指令劫持，因此请避免将其用于启用了工具的代理
或在读取不受信任的内容时。如果您必须使用较小的模型，请锁定
工具并在沙箱中运行。请参阅 [Security](/en/gateway/security)。

### 我在 Telegram 中运行了 start 但没有获得配对代码

仅当未知发件人向代理发送消息并
启用了 `dmPolicy: "pairing"` 时，才会发送配对代码。仅 `/start` 本身不会生成代码。
...
如果您想立即获得访问权限，请将您的发件人 ID 加入允许列表或为该帐户设置 `dmPolicy: "open"`
。
...
向导电话号码提示：它用于设置您的 **allowlist/owner**，以便允许您自己的私信。它不用于自动发送。如果您在个人 WhatsApp 号码上运行，请使用该号码并启用 `channels.whatsapp.selfChatMode`。
...
如果仍然很吵，请检查控制 UI 中的会话设置并将 verbose（详细）设置为 **inherit**（继承）。此外，请确认您未在配置中使用设置了 `verboseDefault` 为 `on` 的机器人配置文件。
...
大多数命令必须作为以 `/` 开头的**独立**消息发送，但对于列入允许列表的发件人，一些快捷方式（如 `/status`）也可以内联使用。
...
编辑配置后重启 Gateway(网关)。如果您只想针对单个代理进行设置，请改为在 `agents.list[].tools.message` 下设置它。

### 为什么感觉机器人忽略了连珠炮式的消息

队列模式控制新消息与正在运行的任务如何交互。使用 `/queue` 更改模式：
