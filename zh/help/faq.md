...
针对实际环境设置（本地开发、VPS、多智能体、OAuth/API 密钥、模型故障转移）的快速解答和更深入的故障排除。有关运行时诊断，请参阅 [故障排除](/zh/gateway/troubleshooting)。有关完整的配置参考，请参阅 [配置](/zh/gateway/configuration)。

## 目录

- [快速入门和首次运行设置]
  - [我卡住了，最快的解决方法是什么？](#im-stuck-whats-the-fastest-way-to-get-unstuck)
  ...
  - [Raspberry Pi 安装有什么提示吗？](#any-tips-for-raspberry-pi-installs)
  - [它卡在“wake up my friend” / 新手引导无法完成。现在该怎么办？](#it-is-stuck-on-wake-up-my-friend-onboarding-will-not-hatch-what-now)
  - [我可以将我的设置迁移到新机器（Mac mini）而无需重新进行新手引导吗？](#can-i-migrate-my-setup-to-a-new-machine-mac-mini-without-redoing-onboarding)
  ...
  - [如何安装 Beta 版本，Beta 版和开发版有什么区别？](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [我如何尝试最新的版本？](#how-do-i-try-the-latest-bits)
  - [安装和新手引导通常需要多长时间？](#how-long-does-install-and-onboarding-usually-take)
  - [安装程序卡住了？如何获取更多反馈信息？](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows 安装提示找不到 git 或无法识别 openclaw](#windows-install-says-git-not-found-or-openclaw-not-recognized)
  - [Windows 可执行文件输出显示乱码中文字符，我该怎么办](#windows-exec-output-shows-garbled-chinese-text-what-should-i-do)
  - [文档没有回答我的问题 - 如何获得更好的答案？](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [我如何在 OpenClaw 上安装 Linux？](#how-do-i-install-openclaw-on-linux)
  - [我如何在 VPS 上安装 OpenClaw？](#how-do-i-install-openclaw-on-a-vps)
  - [云服务/VPS 安装指南在哪里？](#where-are-the-cloudvps-install-guides)
  - [我可以让 OpenClaw 自我更新吗？](#can-i-ask-openclaw-to-update-itself)
  - [新手引导实际上是做什么的？](#what-does-onboarding-actually-do)
  ...
  - [为什么我会收到来自 Anthropic 的 `HTTP 429: rate_limit_error`？](#why-am-i-seeing-http-429-ratelimiterror-from-anthropic)
  ...
  - [我如何设置 Gemini CLI OAuth](#how-do-i-set-up-gemini-cli-oauth)
  - [本地模型适合日常聊天吗？](#is-a-local-model-ok-for-casual-chats)
  - [我如何将托管模型的流量保持在特定区域？](#how-do-i-keep-hosted-model-traffic-in-a-specific-region)
  ...
  - [如果我购买 Mac mini 来运行 OpenClaw，我可以将它连接到我的 MacBook Pro 吗？](#if-i-buy-a-mac-mini-to-run-openclaw-can-i-connect-it-to-my-macbook-pro)
  - [我可以使用 Bun 吗？](#can-i-use-bun)
  - [Telegram：在 `allowFrom` 中填什么？](#telegram-what-goes-in-allowfrom)
  ...
  - [可黑客修改（git）安装与 npm 安装有什么区别？](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  ...
  - [我可以在虚拟机中运行 OpenClaw 吗，有什么要求](#can-i-run-openclaw-in-a-vm-and-what-are-the-requirements)
...
- [Skills 和自动化](#skills-and-automation)
  - [如何在不弄脏仓库的情况下自定义 Skills？](#how-do-i-customize-skills-without-keeping-the-repo-dirty)
  - [我可以从自定义文件夹加载 Skills 吗？](#can-i-load-skills-from-a-custom-folder)
  - [如何针对不同的任务使用不同的模型？](#how-can-i-use-different-models-for-different-tasks)
  - [机器人在执行繁重任务时冻结。如何卸载该任务？](#the-bot-freezes-while-doing-heavy-work-how-do-i-offload-that)
  - [Cron 或提醒无法触发。我应该检查什么？](#cron-or-reminders-do-not-fire-what-should-i-check)
  ...
  - [我可以从 Linux 运行仅限 Apple macOS 的 Skills 吗？](#can-i-run-apple-macos-only-skills-from-linux)
  - [你们有 Notion 或 HeyGen 集成吗？](#do-you-have-a-notion-or-heygen-integration)
  - [如何将我现有的已登录 Chrome 与 OpenClaw 结合使用？](#how-do-i-use-my-existing-signed-in-chrome-with-openclaw)
...
- [配置基础](#config-basics)
  ...
  - [如何跨设备运行具有专门工作程序的中央 Gateway(网关)？](#how-do-i-run-a-central-gateway-with-specialized-workers-across-devices)
...
- [远程网关和节点](#remote-gateways-and-nodes)
  ...
  - [如果 Gateway(网关) 托管在远程，我的 Agent 如何访问我的计算机？](#how-can-my-agent-access-my-computer-if-the-gateway-is-hosted-remotely)
  - [Tailscale 已连接，但我没有收到回复。现在该怎么办？](#tailscale-is-connected-but-i-get-no-replies-what-now)
  - [两个 OpenClaw 实例可以互相通信吗（本地 + VPS）？](#can-two-openclaw-instances-talk-to-each-other-local-vps)
  - [多个代理是否需要独立的 VPS](#do-i-need-separate-vpses-for-multiple-agents)
  - [与从 VPS 使用 SSH 相比，在我的个人笔记本电脑上使用节点有什么好处吗？](#is-there-a-benefit-to-using-a-node-on-my-personal-laptop-instead-of-ssh-from-a-vps)
  - [节点运行网关服务吗？](#do-nodes-run-a-gateway-service)
  - [有没有应用配置的 API / RPC 方式？](#is-there-an-api-rpc-way-to-apply-config)
  ...
  - [我应该在第二台笔记本电脑上安装，还是直接添加一个节点？](#should-i-install-on-a-second-laptop-or-just-add-a-node)
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  - [OpenClaw 如何加载环境变量？](#how-does-openclaw-load-environment-variables)
  - [“我通过服务启动了 Gateway(网关)，我的环境变量消失了。”现在该怎么办？](#i-started-the-gateway-via-the-service-and-my-env-vars-disappeared-what-now)
  - [我设置了 `COPILOT_GITHUB_TOKEN`，但模型状态显示“Shell env: off.”。为什么？](#i-set-copilotgithubtoken-but-models-status-shows-shell-env-off-why)
- [会话和多重聊天](#sessions-and-multiple-chats)
  - [如何开始一个新的对话？](#how-do-i-start-a-fresh-conversation)
  - [如果我从发送 `/new`，会话会自动重置吗？](#do-sessions-reset-automatically-if-i-never-send-new)
  - [有没有办法组建一个 OpenClaw 实例团队，设一个 CEO 和多个代理](#is-there-a-way-to-make-a-team-of-openclaw-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断？如何防止这种情况？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 OpenClaw 但保留其安装？](#how-do-i-completely-reset-openclaw-but-keep-it-installed)
  - [我收到“上下文过大”错误 - 如何重置或压缩？](#im-getting-context-too-large-errors-how-do-i-reset-or-compact)
  - [为什么我会看到“LLM request rejected: messages.content.tool_use.input field required”？](#why-am-i-seeing-llm-request-rejected-messagescontenttool_useinput-field-required)
  ...
  - [群组/线索是否与私信共享上下文？](#do-groupsthreads-share-context-with-dms)
...
- [身份验证配置文件：它们是什么以及如何管理它们](#auth-profiles-what-they-are-and-how-to-manage-them)
  ...
  - [我可以控制优先尝试哪个认证配置文件吗？](#can-i-control-which-auth-profile-is-tried-first)
  - [OAuth 与 API 密钥：有什么区别？](#oauth-vs-api-key-whats-the-difference)
- [Gateway(网关)：端口、“正在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  - [Gateway(网关) 使用哪个端口？](#what-port-does-the-gateway-use)
  - [为什么 `openclaw gateway status` 显示 `Runtime: running` 却是 `RPC probe: failed`？](#why-does-openclaw-gateway-status-say-runtime-running-but-rpc-probe-failed)
  ...
  - [“另一个网关实例正在监听”是什么意思？](#what-does-another-gateway-instance-is-already-listening-mean)
  ...
  - [我设置了 `gateway.bind: "tailnet"` 但无法绑定 / 没有任何监听](#i-set-gatewaybind-tailnet-but-it-cant-bind-nothing-listens)
...
- [日志记录和调试](#logging-and-debugging)
  ...
  - [“从网关断开连接：无原因” - 现在怎么办？](#disconnected-from-gateway-no-reason-what-now)
  ...
  - [当出现故障时，获取更多详情的最快方法是什么？](#whats-the-fastest-way-to-get-more-details-when-something-fails)
...
- [安全与访问控制](#security-and-access-control)
  ...
  - [提示词注入是否只是公共机器人需要关注的问题？](#is-prompt-injection-only-a-concern-for-public-bots)
  - [我的机器人应该有自己的邮箱 GitHub 账户或电话号码吗？](#should-my-bot-have-its-own-email-github-account-or-phone-number)
  - [我可以让它自主处理我的短信，这安全吗？](#can-i-give-it-autonomy-over-my-text-messages-and-is-that-safe)
  - [我可以使用更便宜的模型来处理个人助理任务吗？](#can-i-use-cheaper-models-for-personal-assistant-tasks)
  - [我在 Telegram 中运行了 `/start` 但没有收到配对代码](#i-ran-start-in-telegram-but-didnt-get-a-pairing-code)
  - [WhatsApp：它会向我的联系人发送消息吗？配对如何工作？](#whatsapp-will-it-message-my-contacts-how-does-pairing-work)
- [聊天命令、中止任务和“它停不下来”](#chat-commands-aborting-tasks-and-it-wont-stop)
  ...
  - [如何从 Discord 发送 Telegram 消息？（“跨上下文消息被拒绝”）](#how-do-i-send-a-discord-message-from-telegram-crosscontext-messaging-denied)
  - [为什么感觉机器人会“忽略”连续发送的消息？](#why-does-it-feel-like-the-bot-ignores-rapidfire-messages)
...
7. **Gateway(网关) 快照**
   ...
   请求正在运行的网关提供完整快照（仅限 WS）。请参阅 [健康检查](/zh/gateway/health)。
...
这会**从 git 检出**安装 OpenClaw，因此代理可以读取代码和文档并
推理您正在运行的确切版本。您以后随时可以切换回稳定版本，
只需在不使用 `--install-method git` 的情况下重新运行安装程序。
...
- **Tailscale Serve**（推荐）：保持绑定回环，运行 `openclaw gateway --tailscale serve`，打开 `https://<magicdns>/`。如果 `gateway.auth.allowTailscale` 是 `true`，身份头满足 Control UI/WebSocket 认证（无需令牌，假设受信任的网关主机）；HTTP API 仍然需要令牌/密码。
- **Tailnet bind**：运行 `openclaw gateway --bind tailnet --token "<token>"`，打开 `http://<tailscale-ip>:18789/`，在仪表板设置中粘贴令牌。
- **SSH 隧道**：`ssh -N -L 18789:127.0.0.1:18789 user@host` 然后打开 `http://127.0.0.1:18789/` 并将令牌粘贴到 Control UI 设置中。

有关绑定模式和身份验证详细信息，请参阅 [Dashboard](/zh/web/dashboard) 和 [Web surfaces](/zh/web)。

### 我需要什么运行时

需要 Node **>= 22**。建议使用 `pnpm`。对于 Gateway(网关)，**不推荐**使用 Bun。
...
提示：小型 Pi/VPS 可以托管 Gateway(网关)，您可以将笔记本电脑/手机上的 **节点** 与其配对，用于本地屏幕/摄像头/画布或命令执行。请参阅 [Nodes](/zh/nodes)。
...
如果 Gateway(网关) 是远程的，请确保隧道/Tailscale 连接已启动，并且 UI 指向正确的 Gateway(网关)。请参阅 [Remote access](/zh/gateway/remote)。
...
相关内容：[Migrating](/zh/install/migrating)、[Where things live on disk](/zh/help/faq#where-does-openclaw-store-its-data)、
[Agent workspace](/zh/concepts/agent-workspace)、[Doctor](/zh/gateway/doctor)、
[Remote mode](/zh/gateway/remote)。
...
某些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.openclaw.ai`。请禁用它或将 `docs.openclaw.ai` 加入白名单，然后重试。更多详情：[Troubleshooting](/zh/help/troubleshooting#docsopenclawai-shows-an-ssl-error-comcastxfinity)。
请通过此处报告来帮助我们解除拦截：[https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status)。

如果您仍无法访问该站点，文档已在 GitHub 上镜像：
...
我们将构建版本发布到 **beta**，进行测试，一旦构建版本稳定，我们就会将该版本 **升级为 `latest`**。这就是为什么 beta 和 stable 可能指向 **同一个版本**。
...
**Dev** 是 `main` (git) 的最新头部；发布时，它使用 npm dist-tag `dev`。
...
以及 [我卡住了](/zh/help/faq#im-stuck--whats-the-fastest-way-to-get-unstuck) 中的快速调试循环。
...
- `system.run`/`exec` 输出将中文呈现为乱码（mojibake）
- 同一条命令在其他终端配置文件中显示正常
...
`claude setup-token` 通过 Claude Code CLI 生成一个**令牌字符串**（该功能在 Web 控制台中不可用）。您可以在**任何机器**上运行它。在新手引导中选择 **Anthropic token (paste setup-token)**，或使用 `openclaw models auth paste-token --provider anthropic` 粘贴它。该令牌作为 **anthropic** 提供商的身份验证配置文件存储，并像 API 密钥一样使用（无自动刷新）。更多详情：[OAuth](/zh/concepts/oauth)。
...
复制它打印出的令牌，然后在新手引导中选择 **Anthropic token (paste setup-token)**。如果您想在 Gateway(网关) 主机上运行它，请使用 `openclaw models auth setup-token --provider anthropic`。如果您在其他地方运行了 `claude setup-token`，请使用 `openclaw models auth paste-token --provider anthropic` 将其粘贴到 Gateway(网关) 主机上。参见 [Anthropic](/zh/providers/anthropic)。

### 是否支持 Claude 订阅身份验证（Claude Pro 或 Max）

是的 - 通过 **setup-token**。OpenClaw 不再复用 Claude Code CLI OAuth 令牌；请使用 setup-token 或 Anthropic API 密钥。在任何地方生成令牌并将其粘贴到 Gateway(网关) 主机上。参见 [Anthropic](/zh/providers/anthropic) 和 [OAuth](/zh/concepts/oauth)。
...
Anthropic 的 100 万上下文测试版 (`context1m: true`)。这仅在您的
凭据符合长上下文计费条件（API 密钥计费或启用了额外使用的订阅）时才有效。
...
是的 - 通过 pi-ai 的 **Amazon Bedrock (Converse)** 提供商并使用**手动配置**。您必须在网关主机上提供 AWS 凭据/区域，并在模型配置中添加 Bedrock 提供商条目。请参阅 [Amazon Bedrock](/zh/providers/bedrock) 和 [模型提供商](/zh/providers/models)。如果您更喜欢托管密钥流程，在 Bedrock 前面使用 OpenAI 兼容的代理仍然是一个有效的选项。

### Codex 身份验证是如何工作的

OpenClaw 通过 OAuth（ChatGPT 登录）支持 **OpenAI Code (Codex)**。新手引导可以运行 OAuth 流程，并在适当时将默认模型设置为 `openai-codex/gpt-5.4`。请参阅[模型提供商](/zh/concepts/model-providers) 和 [新手引导 (CLI)](/zh/start/wizard)。

### 你们支持 OpenAI 订阅身份验证 Codex OAuth 吗

是的。OpenClaw 完全支持 **OpenAI Code (Codex) 订阅 OAuth**。
OpenAI 明确允许在外部工具/工作流程中使用订阅 OAuth，
例如 OpenClaw。新手引导可以为您运行 OAuth 流程。
...
这会将 OAuth 令牌存储在网关主机的身份配置文件中。详情：[模型提供商](/zh/concepts/model-providers)。

### 本地模型适合用于随意聊天吗

通常不需要。OpenClaw 需要大上下文 + 强安全性；小显卡会导致截断和泄露。如果必须，请在本地（LM Studio）运行你能运行的**最大** MiniMax M2.5 版本，并参阅 [/gateway/local-models](/zh/gateway/local-models)。更小/量化的模型会增加提示词注入风险 - 请参阅 [Security](/zh/gateway/security)。

### 如何将托管模型流量保留在特定区域

选择区域固定的端点。OpenRouter 为 MiniMax、Kimi 和 GLM 提供了美国托管的选项；选择美国托管的变体以将数据保留在区域内。您仍然可以通过使用 `models.mode: "merge"` 将 Anthropic/OpenAI 与这些并列列出，以便在遵守您选择的区域性提供商的同时保持回退可用。
...
只有在使用 **仅限 macOS 的工具** 时才需要 Mac。对于 iMessage，请使用 [BlueBubbles](/zh/channels/bluebubbles)（推荐）- BlueBubbles 服务器可在任何 Mac 上运行，而 Gateway 可以在 Linux 或其他地方运行。如果您想要其他仅限 macOS 的工具，请在 Mac 上运行 Gateway 或配对 macOS 节点。

文档：[BlueBubbles](/zh/channels/bluebubbles)、[Nodes](/zh/nodes)、[Mac remote mode](/zh/platforms/mac/remote)。

### 我是否需要 Mac mini 来支持 iMessage

您需要**一台 macOS 设备**登录到 Messages。它**不必**是 Mac mini——
任何 Mac 都可以。对于 iMessage，**请使用 [BlueBubbles](/zh/channels/bluebubbles)**（推荐）- BlueBubbles 服务器在 macOS 上运行，而 Gateway 可以在 Linux 或其他地方运行。
...
是的。**Mac mini 可以运行 Gateway**，您的 MacBook Pro 可以作为
**节点**（配套设备）连接。节点不运行 Gateway - 它们提供额外的
...
是的，通过**多智能体路由**。将每个发送者的 **WhatsApp** **私信**（peer `kind: "direct"`，发送者 E.164 如 `+15551234567`）绑定到不同的 `agentId`，这样每个人都可以获得自己的工作区和会话存储。回复仍然来自**同一个 WhatsApp 账户**，且私信访问控制（`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`）是每个 WhatsApp 账户的全局设置。请参阅 [多智能体路由](/zh/concepts/multi-agent) 和 [WhatsApp](/zh/channels/whatsapp)。

### 我可以同时运行一个快速聊天代理和一个用于编码的 Opus 代理吗

是的。使用多智能体路由：为每个智能体指定其默认模型，然后将入站路由（提供商账户或特定对等节点）绑定到每个智能体。示例配置位于 [多智能体路由](/zh/concepts/multi-agent)。另请参阅 [模型](/zh/concepts/models) 和 [配置](/zh/gateway/configuration)。
...
如果你通过 systemd 运行 OpenClaw，请确保服务 PATH 包含 `/home/linuxbrew/.linuxbrew/bin`（或你的 brew 前缀），以便通过 `brew` 安装的工具在非登录 shell 中也能解析。
最近的版本还会在 Linux systemd 服务中预先添加常见的用户 bin 目录（例如 `~/.local/bin`、`~/.npm-global/bin`、`~/.local/share/pnpm`、`~/.bun/bin`），并在设置时遵守 `PNPM_HOME`、`NPM_CONFIG_PREFIX`、`BUN_INSTALL`、`VOLTA_HOME`、`ASDF_DATA_DIR`、`NVM_DIR` 和 `FNM_DIR`。
...
是的。安装其他版本，然后运行 Doctor，以便 gateway 服务指向新的入口点。
...
**OpenClaw 特别说明：** WhatsApp/Telegram/Slack/Mattermost (插件)/Discord 都可以在 VPS 上正常运行。唯一的真正权衡是**无头浏览器** 与可见窗口的区别。请参阅[浏览器](/zh/tools/browser)。
...
如果您想两全其美，请将 Gateway(网关) 保留在专用主机上，并将您的笔记本电脑作为**节点** 配对，以便使用本地屏幕/摄像头/exec 工具。请参阅[节点](/zh/nodes)。
有关安全指导，请阅读[安全](/zh/gateway/security)。
...
如果您使用的是 Windows，**WSL2 是最简单的 VM 风格设置**，并且具有最佳的工具
兼容性。请参阅 [Windows](/zh/platforms/windows)、[VPS 托管](/zh/vps)。
...
文档：[Gateway(网关)](/zh/gateway)、[频道](/zh/channels)、[多代理](/zh/concepts/multi-agent)、
...
使用托管覆盖功能，而不是编辑仓库副本。将您的更改放在 `~/.openclaw/skills/<name>/SKILL.md` 中（或通过 `~/.openclaw/openclaw.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级为 `<workspace>/skills` > `~/.openclaw/skills` > 捆绑包，因此托管覆盖会在不触及 git 的情况下获胜。只有值得合并到上游的编辑才应存放在仓库中并作为 PR 提交。

### 我可以从自定义文件夹加载 Skills 吗

是的。可以通过 `~/.openclaw/openclaw.json` 中的 `skills.load.extraDirs` 添加额外的目录（优先级最低）。默认优先级保持不变：`<workspace>/skills` → `~/.openclaw/skills` → bundled → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，OpenClaw 将其视为 `<workspace>/skills`。

### 如何为不同的任务使用不同的模型

目前支持的模式有：

- **Cron jobs（定时任务）**：隔离的作业可以为每个作业设置 `model` 覆盖。
- **子代理**：将任务路由到具有不同默认模型的独立代理。
- **按需切换**：使用 `/model` 随时切换当前会话的模型。

请参阅 [Cron jobs（定时任务）](/zh/automation/cron-jobs)、[Multi-Agent Routing（多智能体路由）](/zh/concepts/multi-agent) 和 [Slash commands（斜杠命令）](/zh/tools/slash-commands)。
...
让您的机器人“为此任务生成一个子智能体”或使用 `/subagents`。
在聊天中使用 `/status` 查看 Gateway(网关) 当前正在做什么（以及它是否繁忙）。

Token 提示：长任务和子智能体都会消耗 token。如果担心成本，可以通过 `agents.defaults.subagents.model` 为子智能体设置更便宜的模型。

文档：[Sub-agents（子智能体）](/zh/tools/subagents)。
...
- 使用 `thread: true` 通过 `sessions_spawn` 生成（并可选择 `mode: "session"` 用于持续跟进）。
...
文档：[Sub-agents（子智能体）](/zh/tools/subagents)、[Discord](/zh/channels/discord)、[Configuration Reference（配置参考）](/zh/gateway/configuration-reference)、[Slash commands（斜杠命令）](/zh/tools/slash-commands)。
...
文档：[Cron 作业](/zh/automation/cron-jobs)、[Cron 与 Heartbeat](/zh/automation/cron-vs-heartbeat)。
...
不能直接运行。macOS Skills 受 `metadata.openclaw.os` 和必需二进制文件的限制，并且只有当它们在 **Gateway(网关) 主机** 上符合条件时，才会出现在系统提示中。在 Linux 上，除非您覆盖限制，否则仅限 `darwin` 的 Skills（如 `apple-notes`、`apple-reminders`、`things-mac`）将不会加载。

您有三种支持的模式：

**选项 A - 在 Mac 上运行 Gateway（最简单）。**
在存在 macOS 二进制文件的地方运行 Gateway，然后从 Linux 通过 [远程模式](#how-do-i-run-openclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere) 或通过 Tailscale 进行连接。由于 Gateway 主机是 macOS，Skills 会正常加载。

**选项 B - 使用 macOS 节点（无 SSH）。**
在 Linux 上运行 Gateway，配对一个 macOS 节点（菜单栏应用），并在 Mac 上将 **节点运行命令** 设置为“始终询问”或“始终允许”。当节点上存在必需的二进制文件时，OpenClaw 可以将仅限 macOS 的 Skills 视为符合条件。代理通过 `nodes` 工具运行这些 Skills。如果您选择“始终询问”，在提示中批准“始终允许”会将该命令添加到允许列表中。
...
ClawHub 会安装到当前目录下的 `./skills` 中（或者回退到你配置的 OpenClaw 工作区）；OpenClaw 在下次会话时将其视为 `<workspace>/skills`。若要在代理之间共享 Skills，请将它们放在 `~/.openclaw/skills/<name>/SKILL.md` 中。某些 Skills 需要通过 Homebrew 安装二进制文件；在 Linux 上这意味着 Linuxbrew（请参阅上方的 Homebrew Linux 常见问题条目）。请参阅 [Skills](/zh/tools/skills) 和 [ClawHub](/zh/tools/clawhub)。

### 如何使用我现有的已登录 Chrome 配合 OpenClaw
...
此路径是主机本地的。如果 Gateway 运行在其他地方，请在浏览器机器上运行节点主机，或者改用远程 CDP。
...
包括系统包、Homebrew 或捆绑的浏览器。要进行更完整的设置：
...
**我可以让私信保持私密，但使用一个代理将群组设为公共沙箱隔离吗**

可以 - 如果你的私人流量是 **私信** 而公共流量是 **群组**。

使用 `agents.defaults.sandbox.mode: "non-main"`，以便群组/渠道会话（非主键）在 Docker 中运行，而主私信会话保留在主机上。然后通过 `tools.sandbox.tools` 限制沙箱隔离会话中可用的工具。

设置演练 + 示例配置：[Groups: personal 私信 + public groups](/zh/channels/groups#pattern-personal-dms-public-groups-single-agent)

关键配置参考：[Gateway(网关) configuration](/zh/gateway/configuration#agentsdefaultssandbox)

### 如何将主机文件夹绑定到沙箱中

将 `agents.defaults.sandbox.docker.binds` 设置为 `["host:path:mode"]`（例如，`"/home/user/src:/src:ro"`）。全局 + 每个代理的绑定会合并；当 `scope: "shared"` 时，每个代理的绑定将被忽略。对于任何敏感内容，请使用 `:ro`，并请记住绑定会绕过沙箱文件系统隔离墙。有关示例和安全说明，请参阅 [沙箱隔离](/zh/gateway/sandboxing#custom-bind-mounts) 和 [沙箱 vs Tool Policy vs Elevated](/zh/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check)。
...
OpenClaw 还会运行 **静默的预压缩内存刷新**，以提醒模型
在自动压缩之前写入持久笔记。这仅在工作区
可写时运行（只读沙箱会跳过此步骤）。请参阅 [Memory](/zh/concepts/memory)。

### 内存总是忘记事情，我如何让它记住

要求机器人 **将事实写入内存**。长期笔记应放在 `MEMORY.md` 中，
短期上下文应放在 `memory/YYYY-MM-DD.md` 中。

这仍是我们正在改进的领域。提醒模型存储记忆会有所帮助；
它知道该怎么做。如果它一直忘记，请验证 Gateway(网关) 在每次运行时
是否使用相同的工作区。
...
如果您未明确设置提供商，当OpenClaw可以解析 API 密钥（身份验证配置文件、`models.providers.*.apiKey` 或环境变量）时，它将自动选择提供商。如果解析到 OpenAI 密钥，它优先选择 OpenAI，否则如果解析到 Gemini 密钥则选择 Gemini，然后是 Voyage，接着是 Mistral。如果没有可用的远程密钥，内存搜索将保持禁用状态，直到您对其进行配置。如果您配置并存在本地模型路径，OpenClaw 会优先选择 `local`。当您明确设置 `memorySearch.provider = "ollama"` 时，支持 Ollama。

如果您希望保持本地运行，请设置 `memorySearch.provider = "local"`（并可选设置 `memorySearch.fallback = "none"`）。如果您想要 Gemini 嵌入，请设置 `memorySearch.provider = "gemini"` 并提供 `GEMINI_API_KEY`（或 `memorySearch.remote.apiKey`）。我们支持 **OpenAI、Gemini、Voyage、Mistral、Ollama 或本地**嵌入模型 - 有关设置详细信息，请参阅 [内存](/zh/concepts/memory)。

### 内存是否会永久保存有哪些限制

内存文件存储在磁盘上，直到您将其删除为止。限制在于您的存储空间，而不是模型。**会话上下文**仍然受到模型上下文窗口的限制，因此长对话可能会压缩或截断。这就是存在内存搜索的原因——它仅将相关部分拉回上下文中。
...
- **默认本地：** 会话、内存文件、配置和工作区位于 Gateway(网关) 主机上（`~/.openclaw` + 您的工作区目录）。
- **必要远程：** 您发送给模型提供商（Anthropic/OpenAI 等）的消息会发送到它们的 API，聊天平台（WhatsApp/Telegram/Slack 等）会将消息数据存储在它们的服务器上。
- **您控制足迹：** 使用本地模型可以将提示保留在您的机器上，但渠道流量仍然会经过渠道的服务器。
...
| `$OPENCLAW_STATE_DIR/secrets.json`                              | 可选的基于文件的秘密负载，用于 `file` SecretRef 提供程序      |
...
- **工作区（每个代理）**：`AGENTS.md`、`SOUL.md`、`IDENTITY.md`、`USER.md`，
  `MEMORY.md`（或者当 `MEMORY.md` 不存在时的旧版后备 `memory.md`），
  `memory/YYYY-MM-DD.md`，可选的 `HEARTBEAT.md`。
- **状态目录（`~/.openclaw`）**：配置、凭据、身份配置文件、会话、日志
...
如果机器人在重启后“忘记”了之前的对话，请确认 Gateway(网关) 在每次启动时使用的是
同一个工作区（并且请记住：远程模式使用的是 **Gateway 主机的**工作区，而不是您的本地笔记本电脑）。

提示：如果您希望持久化某种行为或偏好，请要求机器人将其**写入
...
将您的 **agent workspace** 放入一个**私有** git 仓库中，并将其备份到某个私有
位置（例如 GitHub 私有仓库）。这会捕获内存以及 AGENTS/SOUL/USER
文件，并允许您稍后恢复助手的“大脑”。

**切勿**提交 `~/.openclaw` 下的任何内容（凭据、会话、令牌或加密的
密钥负载）。如果您需要完全恢复，请分别备份工作区
和状态目录（请参阅上面的迁移问题）。
...
会话状态归 **网关主机** 所有。如果您处于远程模式，您关心的会话存储位于远程机器上，而不是您的本地笔记本电脑。请参阅 [Session management](/zh/concepts/session)。
...
- 如果通过 SecretRef 显式配置了 `gateway.auth.token` / `gateway.auth.password` 但未解析，解析将失败并关闭（无远程后备屏蔽）。
- 控制 UI 通过 `connect.params.auth.token` 进行身份验证（存储在 app/UI 设置中）。避免在 URL 中放入令牌。

### 为什么我现在在 localhost 上也需要令牌

OpenClaw 默认强制执行令牌身份验证，包括环回。如果未配置令牌，网关启动时会自动生成一个并将其保存到 `gateway.auth.token`，因此**本地 WS 客户端必须进行身份验证**。这会阻止其他本地进程调用 Gateway(网关)。

如果您**确实**想要开放环回，请在配置中显式设置 `gateway.auth.mode: "none"`。Doctor 可以随时为您生成令牌：`openclaw doctor --generate-gateway-token`。
...
`web_fetch` 无需 API 密钥即可工作。`web_search` 需要为您选择的提供商（Brave、Gemini、Grok、Kimi 或 Perplexity）提供密钥。
...
- 如果您使用允许列表（allowlists），请添加 `web_search`/`web_fetch` 或 `group:web`。
- `web_fetch` 默认已启用（除非被明确禁用）。
- 守护进程从 `~/.openclaw/.env`（或服务环境）读取环境变量。

文档：[Web 工具](/zh/tools/web)。

### 如何运行一个中央 Gateway(网关) 并在不同设备上部署专用 Worker
...
- **代理：** 用于特殊角色的独立大脑/工作空间（例如“Hetzner 运维”、“个人数据”）。
...
默认是 `false`（有头模式）。无头模式在某些网站上更有可能触发反机器人检查。请参阅 [浏览器](/zh/tools/browser)。

Headless 模式使用 **相同的 Chromium 引擎**，适用于大多数自动化操作（表单、点击、抓取、登录）。主要区别在于：

- 没有可见的浏览器窗口（如果需要视觉效果，请使用屏幕截图）。
- 某些站点在 Headless 模式下对自动化限制更严格（验证码、反机器人）。
  例如，X/Twitter 经常阻止 Headless 会话。
...
并且只有在需要节点 Gateway(网关) 时，才会通过 **Gateway(网关) WebSocket** 调用节点：
...
简短回答：**将您的计算机作为节点配对**。Gateway(网关) 在其他地方运行，但它可以
通过 Gateway(网关) WebSocket 在您的本地机器上调用 `node.*` 工具（屏幕、摄像头、系统）。
...
可以。虽然没有内置的“机器人对机器人”桥接，但您可以通过几种可靠的方式进行连接：
...
目标指向另一个机器人监听的聊天。如果一个机器人位于远程 VPS 上，请通过 SSH/CLI 将您的 Gateway(网关) 指向该远程 Tailscale
（参见 [远程访问](/zh/gateway/remote)）。

示例模式（从可以连接到目标 Gateway(网关) 的机器运行）：

```bash
openclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```

提示：添加一个防护措施，防止两个机器人无限循环（仅提及、渠道
允许列表，或“不回复机器人消息”规则）。
...
和路由。这是常规设置，比为每个代理运行一个 VPS 更便宜、更简单。
...
是的 - 节点是从远程 Gateway(网关) 连接到笔记本电脑的首选方式，并且
它们解锁的功能不仅仅是 Shell 访问。Gateway(网关) 运行在 macOS/Linux（Windows 通过 WSL2）上，并且
轻量级（小型 VPS 或 Raspberry Pi 级别的盒子即可；4 GB RAM 就足够了），因此常见的
设置是常开主机加上作为节点的笔记本电脑。

- **不需要入站 SSH。** 节点向外连接到 Gateway(网关) WebSocket 并使用设备配对。
...
如果您只需要第二台笔记本电脑上的 **本地工具**（屏幕/摄像头/exec），请将其添加为 **节点**。这样可以保持单个 Gateway(网关) 并避免重复配置。本地节点工具目前仅支持 macOS，但我们计划将其扩展到其他操作系统。

仅当您需要 **硬隔离** 或两个完全独立的机器人时，才应安装第二个 Gateway(网关)。

文档：[节点](/zh/nodes)、[节点 CLI](/zh/cli/nodes)、[多个网关](/zh/gateway/multiple-gateways)。

### 节点是否运行网关服务

不。除非您有意运行隔离的配置文件（请参阅 [多个网关](/zh/gateway/multiple-gateways)），否则每个主机应仅运行 **一个网关**。节点是连接到网关的外设（iOS/Android 节点，或菜单栏应用中的 macOS “节点模式”）。对于无头节点主机和 CLI 控制，请参阅 [节点主机 CLI](/zh/cli/node)。
...
- 从备份（git 或复制的 `~/.openclaw/openclaw.json`）恢复。
- 如果您没有备份，请重新运行 `openclaw doctor` 并重新配置通道/模型。
- 如果这是意外发生的，请提交错误报告并包含您最后已知的配置或任何备份。
...
如果您想要在不使用 SSH 的情况下使用控制 UI，请在 VPS 上使用 Tailscale Serve：

```bash
openclaw gateway --tailscale serve
```

这将使网关绑定到回环地址，并通过 Tailscale 暴露 HTTPS。参见 [Tailscale](/zh/gateway/tailscale)。
...
2. **在远程模式下使用 macOS 应用程序**（SSH 目标可以是 tailnet 主机名）。
   应用程序将隧道传输 Gateway(网关) 端口并作为节点连接。
3. **在网关上批准节点**：
...
如果 Gateway(网关) 作为服务运行，它将不会继承您的 Shell
环境。通过执行以下操作之一进行修复：
...
Copilot 令牌从 `COPILOT_GITHUB_TOKEN`（以及 `GH_TOKEN` / `GITHUB_TOKEN`）读取。
...
是的。会话会在 `session.idleMinutes` 之后过期（默认为 **60**）。**下一条**
...
- 让机器人总结当前状态并将其写入文件。
- 在执行长任务前使用 `/compact`，在切换主题时使用 `/new`。
- 将重要上下文保留在工作区中，并要求机器人读回它。
- 对于长时间或并行工作，使用子代理，以便主聊天保持较小规模。
...
这是提供商验证错误：模型发出了 `tool_use` 块，但缺少必需的
...
默认情况下，心跳每 **30 分钟** 运行一次。您可以调整或禁用它们：
...
如果文件缺失，心跳仍会运行，模型将决定要执行的操作。

每个代理的覆盖设置使用 `agents.list[].heartbeat`。文档：[心跳](/zh/gateway/heartbeat)。

### 我是否需要将机器人账号添加到 WhatsApp 群组

不需要。OpenClaw 在**您自己的账号**上运行，所以如果您在群组中，OpenClaw 就能看到它。
...
直接聊天默认折叠到主会话。群组/渠道拥有自己的会话密钥，而 Telegram 主题/Discord 线程是独立的会话。请参阅[群组](/zh/channels/groups)和[群组消息](/zh/channels/group-messages)。

### 我可以创建多少个工作区和代理

没有硬性限制。几十个（甚至几百个）都可以，但请注意：

- **磁盘增长：**会话和抄录位于 `~/.openclaw/agents/<agentId>/sessions/` 下。
- **Token 成本：**代理越多意味着并发模型使用量越多。
...
可以。使用**多代理路由**来运行多个隔离的代理，并通过渠道/账户/对等端路由传入的消息。Slack 支持作为渠道，并且可以绑定到特定的代理。

浏览器访问功能强大，但并非“可以做人类能做的任何事”——反机器人、验证码和多因素身份验证仍然可以阻止自动化。为了获得最可靠的浏览器控制，请在主机上使用本地 Chrome MCP，或者在实际运行浏览器的机器上使用 CDP。
...
模型被引用为 `provider/model`（例如：`anthropic/claude-opus-4-6`）。如果您省略提供商，OpenClaw 目前会假定 `anthropic` 作为临时弃用回退选项 - 但您仍然应该 **明确地** 设置 `provider/model`。

### 您推荐什么模型

**推荐默认设置：** 使用您提供商堆栈中可用的最强的最新一代模型。
**对于启用工具或不可信输入的代理：** 优先考虑模型强度而非成本。
**对于日常/低风险聊天：** 使用更便宜的回退模型并按代理角色进行路由。

MiniMax M2.5 有自己的文档：[MiniMax](/zh/providers/minimax) 和
[Local models](/zh/gateway/local-models)。

经验法则：对于高风险工作，请使用您 **能负担得起的最佳模型**；对于日常聊天或摘要，请使用
更便宜的模型。您可以为每个代理路由模型，并使用子代理来
并行化长任务（每个子代理消耗令牌）。请参阅 [Models](/zh/concepts/models) 和
[Sub-agents](/zh/tools/subagents)。

强烈警告：较弱/过度量化的模型更容易受到提示注入和不安全行为的影响。请参阅 [Security](/zh/gateway/security)。

更多背景信息：[Models](/zh/concepts/models)。

### 我可以使用自托管模型 llamacpp vLLM Ollama 吗

是的。Ollama 是本地模型最简单的途径。
...
- 对于手动切换，请使用 `openclaw models list` 和 `openclaw models set ollama/<model>`

安全提示：较小或严重量化的模型更容易受到提示注入攻击。对于可以使用工具的任何机器人，我们强烈建议使用**大型模型**。如果您仍想使用小型模型，请启用沙箱隔离并实施严格的工具允许列表。
...
- 对于对安全敏感或启用了工具的代理，请使用可用的最强最新一代模型。
...
`/model`（以及 `/model list`）显示一个紧凑的、带编号的选择器。通过数字选择：

```
/model 3
```

您还可以强制提供商使用特定的身份验证配置文件（每次会话）：
...
提示：`/model status` 显示哪个 agent 处于活动状态，正在使用哪个 `auth-profiles.json` 文件，以及接下来将尝试哪个 auth 配置文件。
...
使用 `/model status` 确认哪个 auth 配置文件处于活动状态。

### 我能否将 GPT 5.2 用于日常任务，将 Codex 5.3 用于编码

可以。将一个设置为默认，并根据需要进行切换：

- **快速切换（每次会话）：** `/model gpt-5.2` 用于日常任务，`/model openai-codex/gpt-5.4` 使用 Codex OAuth 进行编码。
...
### 我可以将 MiniMax 设为默认，并在复杂任务中使用 OpenAI 吗

可以。**将 MiniMax 作为默认模型**，并在需要时**按会话切换模型**。
...
是的。OpenClaw 附带了一些默认简写（仅当该模型存在于 `agents.defaults.models` 中时才应用）：
...
然后 `/model sonnet`（或在受支持时输入 `/<alias>`）将解析为该模型 ID。
...
如果您引用了提供商/模型，但缺少所需的提供商密钥，将会收到运行时身份验证错误（例如 `No API key found for provider "zai"`）。

**添加新代理后未找到提供商的 API 密钥**

这通常意味着**新代理**拥有一个空的身份验证存储。身份验证是针对每个代理的，并存储在：
...
这意味着系统尝试使用身份验证配置 ID `anthropic:default`，但在预期的身份验证存储中找不到其凭据。

### 找不到配置文件 anthropicdefault 的凭据时的修复检查清单

- **确认身份验证配置的位置**（新路径与旧路径）
  - 当前路径：`~/.openclaw/agents/<agentId>/agent/auth-profiles.json`
  - 旧路径：`~/.openclaw/agent/*`（由 `openclaw doctor` 迁移）
- **确认您的环境变量已由 Gateway(网关) 加载**
  - 如果您在 Shell 中设置了 `ANTHROPIC_API_KEY`，但通过 systemd/launchd 运行 Gateway(网关)，它可能无法继承该变量。将其放入 `~/.openclaw/.env` 或启用 `env.shellEnv`。
- **确保您正在编辑正确的代理**
  - 多代理设置意味着可能存在多个 `auth-profiles.json` 文件。
- **健全性检查模型/身份验证状态**
  - 使用 `openclaw models status` 查看已配置的模型以及提供商是否已通过身份验证。

**找不到配置文件 anthropic 的凭据时的修复检查清单**

这意味着运行被固定到了 Anthropic 身份验证配置，但 Gateway(网关)在其身份验证存储中找不到它。
...
- **如果您想改用 API 密钥**
  - 在 **网关主机** 上，将 `ANTHROPIC_API_KEY` 放入 `~/.openclaw/.env`。
  - 清除任何强制使用缺失配置文件的固定顺序：
...
- **确认您正在网关主机上运行命令**
  - 在远程模式下，认证配置文件位于网关计算机上，而不是您的笔记本电脑上。

### 为什么它还尝试了 Google Gemini 并失败了

如果您的模型配置包含 Google Gemini 作为后备（或者您切换到了 Gemini 简写），OpenClaw 会在模型回退期间尝试它。如果您尚未配置 Google 凭据，您将看到 `No API key found for provider "google"`。

修复：提供 Google 认证，或者在 `agents.defaults.model.fallbacks` / 别名中移除/避免使用 Google 模型，以免回退路由到那里。

**LLM 请求被拒绝，提示认为需要签名 google antigravity**

原因：会话历史包含 **没有签名的思考块**（通常来自
中止/部分流）。Google Antigravity 要求思考块必须具有签名。

修复：OpenClaw 现在会为 Google Antigravity Claude 剥离无符号的思考块。如果仍然出现，请启动 **新会话** 或为该代理设置 `/thinking off`。
...
是的。配置支持配置文件的可选元数据和每个提供商的顺序（`auth.order.<provider>`）。这**不**存储机密；它将 ID 映射到提供商/模式并设置轮换顺序。

如果配置文件处于短期的 **冷却** （速率限制/超时/认证失败）或较长的 **禁用** 状态（计费/积分不足），OpenClaw 可能会暂时跳过该配置文件。要检查此情况，请运行 `openclaw models status --json` 并检查 `auth.unusableProfiles`。调整：`auth.cooldowns.billingBackoffHours*`。
...
### 为什么 openclaw gateway 状态显示 Runtime 正在运行但 RPC 探测失败

因为“正在运行”是 **supervisor 的** 视图（launchd/systemd/schtasks）。RPC 探测是指 CLI 实际连接到 gateway WebSocket 并调用 `status`。
...
OpenClaw 通过在启动时立即绑定 WebSocket 监听器来强制执行运行时锁（默认 `ws://127.0.0.1:18789`）。如果绑定失败并出现 `EADDRINUSE`，它将抛出 `GatewayLockError`，指示另一个实例已在监听。
...
### 控制界面显示未授权或不断重新连接 现在该怎么办

您的网关在启用了身份验证（`gateway.auth.*`）的情况下运行，但 UI 没有发送匹配的令牌/密码。

事实（来自代码）：

- 控制界面将令牌保存在 `sessionStorage` 中，用于当前浏览器选项卡会话和选定的网关 URL，因此同一选项卡的刷新可以继续工作，而无需恢复长期的 localStorage 令牌持久性。
...
- 最快的方法：`openclaw dashboard`（打印并复制控制面板 URL，尝试打开；如果是无头模式则显示 SSH 提示）。
- 如果您还没有令牌：`openclaw doctor --generate-gateway-token`。
- 如果是远程模式，请先建立隧道：`ssh -N -L 18789:127.0.0.1:18789 user@host` 然后打开 `http://127.0.0.1:18789/`。
- 在网关主机上设置 `gateway.auth.token`（或 `OPENCLAW_GATEWAY_TOKEN`）。
- 在控制界面设置中，粘贴相同的令牌。
- 如果在一次重试后仍然不匹配，请轮换/重新批准配对的设备令牌：
  - `openclaw devices list`
  - `openclaw devices rotate --device <id> --role operator`
- 仍然卡住了？运行 `openclaw status --all` 并按照 [故障排除](/zh/gateway/troubleshooting) 操作。有关身份验证的详细信息，请参阅 [控制面板](/zh/web/dashboard)。

### 我设置了 gatewaybind tailnet 但它无法绑定 没有任何监听

`tailnet` 绑定会从您的网络接口中选择一个 Tailscale IP (100.64.0.0/10)。如果机器不在 Tailscale 上（或者接口已关闭），则没有任何东西可以绑定。
...
注意：`tailnet` 是显式的。`auto` 首选环回；当您仅需要 tailnet 绑定时，请使用 `gateway.bind: "tailnet"`。

### 我可以在同一主机上运行多个 Gateway(网关) 吗

通常不需要——一个 Gateway(网关) 可以运行多个消息通道和代理。仅在您需要冗余（例如：救援机器人）或硬隔离时，才使用多个 Gateway(网关)。
...
是一个 `connect` 帧。如果它收到其他任何内容，它将关闭连接
并返回 **代码 1008**（策略违规）。
...
更多信息请参见 [Troubleshooting](/zh/gateway/troubleshooting#log-locations)。
...
### 我在 Windows 上关闭了终端，如何重新启动 OpenClaw
...
如果您处于远程环境，请确认 tunnel/Tailscale 连接已启动，并且
Gateway(网关) WebSocket 可访问。

文档：[渠道](/zh/channels)、[Troubleshooting](/zh/gateway/troubleshooting)、[Remote access](/zh/gateway/remote)。

### 与 gateway 断开连接，没有原因，现在怎么办
...
4. 如果是远程连接，tunnel/Tailscale 链接是否已启动？
...
文档：[Dashboard](/zh/web/dashboard)、[Remote access](/zh/gateway/remote)、[故障排除](/zh/gateway/troubleshooting)。

### Telegram setMyCommands 失败了，我该检查什么
...
- `BOT_COMMANDS_TOO_MUCH`：Telegram 菜单项太多。OpenClaw 已经会自动修剪以符合 Telegram 的限制并尝试减少命令重试，但有些菜单项仍需删除。请减少插件/技能/自定义命令，或者如果不需要菜单，请禁用 `channels.telegram.commands.native`。
- `TypeError: fetch failed`、`Network request for 'setMyCommands' failed!` 或类似网络错误：如果您在 VPS 上或代理后面，请确认允许出站 HTTPS 且 `api.telegram.org` 的 DNS 解析正常。
...
在 TUI 中，使用 `/status` 查看当前状态。如果您期望在聊天渠道中收到回复，请确保投递已启用 (`/deliver on`)。

文档：[TUI](/zh/web/tui)、[Slash commands](/zh/tools/slash-commands)。

### 如何完全停止然后重新启动 Gateway(网关)
...
- `openclaw gateway`：在该终端会话中以前台模式运行网关。

如果您安装了服务，请使用 gateway 命令。当您需要进行单次、前台运行时，请使用 `openclaw gateway`。

### 当发生故障时，获取更多详细信息的最快方法是什么
...
- 支持私信的渠道上的默认行为是**配对**：
  ...
  - 待处理的请求限制为**每个渠道 3 个**；如果代码未到达，请检查 `openclaw pairing list --channel <channel> [--account <id>]`。
...
不。提示注射是关于**不受信任的内容**，而不仅仅是谁可以给机器人发私信。
如果你的助手读取外部内容（网络搜索/获取、浏览器页面、电子邮件、
文档、附件、粘贴的日志），这些内容可能包含试图
劫持模型的指令。即使**你是唯一的发送者**，也可能发生这种情况。

最大的风险是启用了工具时：模型可能被诱骗
泄露上下文或代表你调用工具。通过以下方式减小爆炸半径：
...
### 我的机器人应该拥有自己的电子邮件 GitHub 账户还是电话号码

是的，对于大多数设置来说都是如此。使用单独的帐户和电话号码将机器人隔离
可以在出现问题时减小受影响范围。这也使得轮换凭据或撤销访问权限变得更容易，
而不会影响您的个人帐户。

从小处着手。仅授予您实际需要的工具和帐户的访问权限，并在需要时稍后扩展。

文档：[安全](/zh/gateway/security)，[配对](/zh/channels/pairing)。

### 我可以让它在短信方面拥有自主权，这安全吗

我们**不**建议让其在您的个人消息上拥有完全的自主权。最安全的模式是：

- 将私信保持在**配对模式**或严格的允许列表中。
- 如果您希望它代表您发送消息，请使用**单独的号码或帐户**。
- 让它起草草稿，然后**在发送前批准**。

如果您想进行试验，请在专用帐户上进行并将其隔离。请参阅
[安全](/zh/gateway/security)。

### 我可以为个人助理任务使用更便宜的模型吗

是的，**如果**代理仅用于聊天且输入是可信的。较小的等级更容易
受到指令劫持，因此请避免为启用工具的代理或在读取不受信任的内容时使用它们。
如果您必须使用较小的模型，请锁定工具并在沙箱中运行。请参阅[安全](/zh/gateway/security)。

### 我在 Telegram 中运行了 start 但没有收到配对代码

仅当未知发送者向机器人发送消息且
`dmPolicy: "pairing"` 已启用时，才会发送配对代码。仅 `/start` 本身不会生成代码。
...
不会。默认的 WhatsApp 私信策略是**配对**。未知发送者只会收到配对代码，并且他们的消息**不会被处理**。OpenClaw 仅回复其收到的聊天或您触发的显式发送。
...
只有为该**会话**启用了 **verbose** 或 **reasoning** 时，大多数内部或工具消息才会显示。
...
如果仍然很嘈杂，请在控制 UI 中检查会话设置并将 verbose 设置为 **inherit**。还要确认您没有在配置中使用设置了 `verboseDefault` 为 `on` 的 bot 配置文件。
...
大多数命令必须作为以 `/` 开头的**独立**消息发送，但少数快捷方式（例如 `/status`）也可以针对允许列表中的发送者内联使用。
...
**A:** 在 OpenClaw 中，凭据和模型选择是分开的。设置 `ANTHROPIC_API_KEY`（或在身份验证配置文件中存储 Anthropic API 密钥）可以启用身份验证，但实际的默认模型是您在 `agents.defaults.model.primary` 中配置的任何内容（例如，`anthropic/claude-sonnet-4-5` 或 `anthropic/claude-opus-4-6`）。如果您看到 `No credentials found for profile "anthropic:default"`，这意味着 Gateway 无法在正在运行的代理的预期 `auth-profiles.json` 中找到 Anthropic 凭据。
