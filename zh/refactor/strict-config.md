...
其他所有内容必须硬失败并提示：“配置无效。请运行 `openclaw doctor --fix`。”
...
- `src/config/zod-schema.ts`：移除根传递；各处严格对象。
- `src/config/zod-schema.providers.ts`：确保严格的频道 schema。
- `src/config/validation.ts`：未知的键失败；不应用旧版迁移。
- `src/config/io.ts`：移除旧版自动迁移；始终运行 doctor 干运行。
- `src/config/legacy*.ts`：将使用移至仅限 doctor。
- `src/plugins/*`：添加 schema 注册表 + 闸门。
- `src/cli` 中的 CLI 命令闸门。
