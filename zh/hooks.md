...
- **Hooks** (本页)：当代理事件触发时在网关内部运行，例如 `/new`、`/reset`、`/stop` 或生命周期事件。
- **Webhooks**：允许其他系统触发 OpenClaw 中工作的外部 HTTP webhooks。请参阅 [Webhook Hooks](/en/automation/webhook) 或使用 `openclaw webhooks` 获取 Gmail 辅助命令。
...
- **💾 session-memory**：当您发出 `/new` 时，将会话上下文保存到您的工作区（默认为 `~/.openclaw/workspace/memory/`）
...
- **`agent:bootstrap`**：在注入工作区启动文件之前（hooks 可能会修改 `context.bootstrapFiles`）
...
- **`tool_result_persist`**：在将工具结果写入会话记录之前转换它们。必须是同步的；返回更新后的工具结果负载或 `undefined` 以保持原样。参见 [Agent Loop](/en/concepts/agent-loop)。
...
- **工作区 hooks** (`<workspace>/hooks/`)：针对每个代理，优先级最高
...
- `2026-01-16-1430.md`（如果 slug 生成失败时的后备时间戳）
