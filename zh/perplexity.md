...
如果您使用 `OPENROUTER_API_KEY`、`tools.web.search.perplexity.apiKey` 中的 `sk-or-...` 密钥，或者设置了 `tools.web.search.perplexity.baseUrl` / `model`，提供商将切换到 chat-completions 路径，并返回带有引用的 AI 综合答案，而不是结构化的 Search API 结果。
