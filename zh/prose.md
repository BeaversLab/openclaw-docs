...
OpenProse 是一种便携式的、以 Markdown 为优先的工作流格式，用于编排 AI 会话。在 OpenClaw 中，它作为插件提供，安装了一个 OpenProse 技能包和一个 `/prose` 斜杠命令。程序存放在 `.prose` 文件中，可以生成具有显式控制流的多个子代理。
...
直接 URL 按原样获取。这使用 `web_fetch` 工具（对于 POST 则使用 `exec`）。
