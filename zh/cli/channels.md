...
- 探测是特定于提供商的：Discord intents + 可选渠道权限；Slack bot + user scopes；Telegram bot flags + webhook；Signal daemon 版本；MS Teams app token + Graph roles/scopes（在已知处标注）。没有探测的渠道会报告 `Probe: unavailable`。
