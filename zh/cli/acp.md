...
`openclaw acp` 是一个由网关支持的 ACP 网桥，而非完整的 ACP 原生编辑器
...
| `listSessions`, 斜杠命令                                        | 已实现 | 会话列表适用于 Gateway 会话状态；命令通过 `available_commands_update` 进行通告。                                                                                                                                       |
| `loadSession`                                                         | 部分     | 将 ACP 会话重新绑定到 Gateway 会话密钥并重放存储的用户/助手文本历史记录。工具/系统历史记录尚未重建。                                                                                                   |
| 提示内容 (`text`, 嵌入式 `resource`, 图像)                  | 部分     | 文本/资源被扁平化到聊天输入中；图像成为 Gateway 附件。                                                                                                                                                                 |
| 会话模式                                                         | 部分     | 支持 `session/set_mode`，桥接暴露了初始支持的 Gateway 会话控制，包括思维级别、工具详细程度、推理、使用详情和提升的操作。更广泛的 ACP 原生模式/配置界面仍不在范围内。 |
| 会话信息和使用更新                                        | 部分     | 桥接从缓存的 Gateway 会话快照发出 `session_info_update` 和尽力而为的 `usage_update` 通知。使用情况是近似的，仅当 Gateway token 总数被标记为最新时才发送。                                        |
| 工具流式传输                                                        | 部分     | 当 Gateway 工具参数/结果暴露它们时，`tool_call` / `tool_call_update` 事件包括原始 I/O、文本内容和尽力而为的文件位置。嵌入式终端和更丰富的 diff 原生输出仍未暴露。                        |
...
每个 ACP 会话映射到单个 Gateway 会话密钥。一个代理可以拥有许多
会话；除非您覆盖
...
针对特定的 Gateway 和会话密钥，请覆盖 `~/.acpx/config.json` 中的 `openclaw` 代理命令：
...
默认情况下，ACP 会话将获得带有 `acp:` 前缀的隔离 Gateway 会话密钥。
要重用已知会话，请传递会话密钥或标签：
...
在 [/concepts/session](/en/concepts/session) 了解更多关于会话密钥的信息。
