...
- [`daemon`](/zh/cli/daemon) （网关服务命令的旧式别名）
- [`clawbot`](/zh/cli/clawbot) （旧式别名命名空间）
...
大多数插件更改都需要重启网关。请参阅 [/plugin](/zh/tools/plugin)。
...
- `openclaw memory index` — 重新建立记忆文件索引。
...
- `--secret-input-mode <plaintext|ref>` (默认为 `plaintext`；使用 `ref` 存储提供商默认环境变量引用，而不是纯文本密钥)
...
- `--custom-base-url <url>` (非交互式；与 `--auth-choice custom-api-key` 或 `--auth-choice ollama` 结合使用)
- `--custom-model-id <id>` (非交互式；与 `--auth-choice custom-api-key` 或 `--auth-choice ollama` 结合使用)
- `--custom-api-key <key>` (非交互式；可选；与 `--auth-choice custom-api-key` 结合使用；省略时回退到 `CUSTOM_API_KEY`)
...
- `--gateway-token <token>`
- `--gateway-token-ref-env <name>` (非交互式；将 `gateway.auth.token` 作为环境变量 SecretRef 存储；要求必须设置该环境变量；不能与 `--gateway-token` 结合使用)
...
- `--skip-health`
- `--skip-ui`
- `--node-manager <npm|pnpm|bun>` (推荐使用 pnpm；不建议在 Gateway(网关) 运行时使用 bun)
- `--json`
...
非交互式配置帮助器（get/set/unset/file/validate）。在没有子命令的情况下运行 `openclaw config` 会启动向导。
...
- `config validate`：在不启动网关的情况下，根据架构验证当前配置。
...
管理聊天渠道帐户（WhatsApp/Telegram/Discord/Google Chat/Slack/Mattermost（插件）/Signal/iMessage/MS Teams）。
...
- 提示：当 `channels status` 检测到常见错误配置时，会打印带有建议修复的警告（然后指向 `openclaw doctor`）。
- `channels logs`：显示来自网关日志文件的最近渠道日志。
- `channels add`：未传递标志时使用向导式设置；标志切换到非交互模式。
  - 向仍使用单帐户顶级配置的渠道添加非默认帐户时，OpenClaw 会在写入新帐户之前将帐户范围的值移至 `channels.<channel>.accounts.default`。
  - 非交互式 `channels add` 不会自动创建/升级绑定；仅限渠道的绑定将继续匹配默认账户。
- `channels remove`：默认禁用；传递 `--delete` 以在没有提示的情况下删除配置条目。
...
通过 Gateway(网关)（或嵌入的 `--local`）运行一次 Agent 轮次。
...
添加一个新的隔离 Agent。除非传递了标志（或 `--non-interactive`），否则将运行引导向导；在非交互模式下，`--workspace` 是必填项。
...
绑定规范使用 `channel[:accountId]`。当省略 `accountId` 时，OpenClaw 可以通过渠道默认值/插件挂钩解析账号范围；否则，这是没有明确账号范围的渠道绑定。
...
运行连接 IDE 与 Gateway(网关) 的 ACP 桥接服务。
...
- 概述包括 Gateway(网关) 和节点主机服务状态（如果可用）。

### 使用量跟踪

当 OAuth/API 凭据可用时，OpenClaw 可以显示提供商使用量/配额。

展示位置：

- `/status` (如果可用，添加简短的提供商使用量行)
...
- 数据直接来自提供商的使用量端点（非估算）。
- 提供商：Anthropic、GitHub Copilot、OpenAI Codex OAuth，以及通过捆绑的 `google` 插件提供的 Gemini CLI，以及已配置的 Antigravity。
- 如果不存在匹配的凭据，则隐藏使用量信息。
- 详情：请参阅 [Usage tracking](/zh/concepts/usage-tracking)。

### `health`

从运行的 Gateway(网关) 获取健康状态。
...
- `gateway status` 默认使用服务的已解析端口/配置探测 Gateway(网关) RPC（可使用 `--url/--token/--password` 覆盖）。
- `gateway status` 支持用于脚本编写的 `--no-probe`、`--deep`、`--require-rpc` 和 `--json`。
- 当 `gateway status` 可以检测到遗留或额外的 Gateway 服务时，也会显示这些服务（`--deep` 会添加系统级扫描）。以 Profile 命名的 OpenClaw 服务被视为一等服务，不会被标记为“额外”。
...
- `gateway install|uninstall|start|stop|restart` 支持用于脚本编写的 `--json`（默认输出保持适合人类阅读）。
...
Gateway(网关) CLI 辅助工具（对于 RPC 子命令，请使用 `--url`、`--token`、`--password`、`--timeout`、`--expect-final`）。
...
提示：直接调用 `config.set`/`config.apply`/`config.patch` 时，如果配置已存在，请传递
...
政策说明：这是技术兼容性。Anthropic 过去曾阻止在 Claude Code 之外使用某些订阅；在生产环境中依赖 setup-token 之前，请核实当前的 Anthropic 条款。
...
- `cron status [--json]`
- `cron list [--all] [--json]`（默认为表格输出；使用 `--json` 获取原始格式）
- `cron add`（别名：`create`；需要 `--name` 以及 `--at`、`--every`、`--cron` 中的恰好一个，并且需要 `--system-event` 或 `--message` 中的恰好一个有效载荷）
- `cron edit <id>`（修补字段）
...
- `node` 从环境/配置解析网关认证（无 `--token`/`--password` 标志）：`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`，然后是 `gateway.auth.*`。在本地模式下，节点主机有意忽略 `gateway.remote.*`；在 `gateway.mode=remote` 中，`gateway.remote.*` 按远程优先级规则参与。
- 遗留的 `CLAWDBOT_GATEWAY_*` 环境变量在解析节点主机认证时会被有意忽略。
...
浏览器控制 CLI（专用的 Chrome/Brave/Edge/Chromium）。请参阅 [`openclaw browser`](/zh/cli/browser) 和 [Browser 工具](/zh/tools/browser)。
