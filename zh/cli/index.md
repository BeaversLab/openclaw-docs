...
添加一个新的隔离代理。除非传递了标志（或 `--non-interactive`），否则运行引导向导；在非交互模式下需要 `--workspace`。
...
绑定规范使用 `channel[:accountId]`。当省略 `accountId` 时，OpenClaw 可以通过渠道默认值/插件挂钩解析帐户范围；否则，它是没有明确帐户范围的渠道绑定。
...
- 概述在可用时包括 Gateway(网关) + 节点主机服务状态。

### 使用情况跟踪

当可用 OAuth/API 凭据时，OpenClaw 可以显示提供商使用量/配额。
...
- 数据直接来自提供商使用情况端点（无估算）。
- 提供商：Anthropic、GitHub Copilot、OpenAI Codex OAuth，以及通过捆绑的 `google` 插件和 Antigravity（在已配置的情况下）提供的 Gemini CLI。
- 如果不存在匹配的凭据，则使用情况将被隐藏。
- 详细信息：请参阅 [使用情况跟踪](/zh/concepts/usage-tracking)。

### `health`

从正在运行的 Gateway(网关) 获取运行状况。
...
- `gateway status` 默认使用服务已解析的端口/配置来探测 Gateway(网关) RPC（可用 `--url/--token/--password` 覆盖）。
- `gateway status` 支持 `--no-probe`、`--deep`、`--require-rpc` 和 `--json` 以便编写脚本。
- `gateway status` 还会在检测到时显示旧版或额外的 OpenClaw 服务（`--deep` 添加系统级扫描）。以配置文件命名的 OpenClaw 服务被视为一等服务，不会被标记为“额外”。
- `gateway status` 会打印 CLI 使用的配置路径与服务可能使用的配置（服务环境变量）之间的对比，以及已解析的探测目标 URL。
- 如果当前命令路径中的网关认证 SecretRefs 未解析，则 `gateway status --json` 仅在探测连接性/认证失败时才报告 `rpc.authWarning`（探测成功时将取消显示警告）。
...
通过 Gateway(网关) 追踪 RPC 文件日志。

注意事项：

- TTY 会话呈现彩色的结构化视图；非 TTY 则回退到纯文本。
- `--json` 发出逐行分隔的 JSON（每行一个日志事件）。
...
策略说明：这属于技术兼容性。Anthropic 过去曾在 Claude Code 之外阻止某些
订阅的使用；在生产环境中依赖 setup-token 之前，请核实当前的 Anthropic
条款。
...
- `cron add`（别名：`create`；需要 `--name` 以及 `--at` | `--every` | `--cron` 中的 exactly one，以及 `--system-event` | `--message` 中的 exactly one 负载）
- `cron edit <id>`（修补字段）
...
- `node` 从环境/配置解析 Gateway(网关) 认证（无 `--token`/`--password` 标志）：先 `OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`，然后 `gateway.auth.*`。在本地模式下，节点主机有意忽略 `gateway.remote.*`；在 `gateway.mode=remote` 中，`gateway.remote.*` 根据远程优先级规则参与其中。
- 用于节点主机认证解析时，有意忽略旧的 `CLAWDBOT_GATEWAY_*` 环境变量。

## 节点

`nodes` 与 Gateway(网关) 通信并以配对的节点为目标。请参阅 [/nodes](/zh/nodes)。
