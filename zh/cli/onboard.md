...
- macOS 新手引导：[新手引导 (macOS App)](/zh/start/onboarding)
...
对于纯文本私有网络 `ws://` 目标（仅限受信任网络），请在新手引导进程环境中设置
...
`--custom-base-url` 默认为 `http://127.0.0.1:11434`。`--custom-model-id` 是可选的；如果省略，新手引导将使用 Ollama 建议的默认值。云模型 ID（如 `kimi-k2.5:cloud`）也可以在此处使用。
...
使用 `--secret-input-mode ref` 时，新手引导将写入由环境变量支持的引用，而不是纯文本键值。
对于由身份验证配置文件支持的提供商，这将写入 `keyRef` 条目；对于自定义提供商，这将 `models.providers.<id>.apiKey` 写入为环境变量引用（例如 `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`）。

非交互式 `ref` 模式约定：

- 在新手引导进程环境中设置提供商的环境变量（例如 `OPENAI_API_KEY`）。
- 除非同时也设置了该环境变量，否则不要传递内联键标志（例如 `--openai-api-key`）。
...
- `--gateway-token-ref-env` 要求在新手引导进程环境中设置非空的环境变量。
- 使用 `--install-daemon` 时，当令牌身份验证需要令牌时，由 SecretRef 管理的网关令牌将被验证，但不会作为解析后的明文持久化在 supervisor 服务环境元数据中。
- 使用 `--install-daemon` 时，如果令牌模式需要令牌但配置的令牌 SecretRef 未解析，新手引导将失败关闭并给出修复指导。
- 使用 `--install-daemon` 时，如果配置了 `gateway.auth.token` 和 `gateway.auth.password` 但未设置 `gateway.auth.mode`，新手引导将阻止安装，直到明确设置模式。
...
- 除非传递 `--skip-health`，否则新手引导将等待本地网关可达后才会成功退出。
- `--install-daemon` 首先启动托管网关安装路径。如果没有它，您必须已经有一个本地网关在运行，例如 `openclaw gateway run`。
- 如果您只想在自动化中写入 config/workspace/bootstrap，请使用 `--skip-health`。
