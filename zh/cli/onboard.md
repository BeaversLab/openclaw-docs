---
summary: "`openclaw onboard`（交互式入职向导）的 CLI 参考"
read_when:
...
对于纯文本私有网络 `ws://` 目标（仅限受信任网络），请在入职流程环境中设置
...
在非交互模式下，`--custom-api-key` 是可选的。如果省略，入职将检查 `CUSTOM_API_KEY`。
...
使用 `--secret-input-mode ref`，入职会写入环境支持的引用，而不是纯文本键值。
对于由 auth-profile 支持的提供者，这将写入 `keyRef` 条目；对于自定义提供者，这将 `models.providers.<id>.apiKey` 作为环境引用写入（例如 `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`）。

非交互式 `ref` 模式约定：

- 在入职流程环境中设置提供者环境变量（例如 `OPENAI_API_KEY`）。
...
- `--gateway-token-ref-env` 要求入职流程环境中存在非空的环境变量。
- 使用 `--install-daemon` 时，当令牌身份验证需要令牌时，由 SecretRef 管理的网关令牌将经过验证，但不会作为已解析的纯文本持久化到主管服务环境元数据中。
- 使用 `--install-daemon` 时，如果令牌模式需要令牌且配置的令牌 SecretRef 未解析，入职将以失败关闭并提供修复指导。
- 使用 `--install-daemon` 时，如果同时配置了 `gateway.auth.token` 和 `gateway.auth.password` 并且未设置 `gateway.auth.mode`，则入职流程会阻止安装，直到明确设置模式。
