...
- 多代理路由：[多代理路由](/zh/en/concepts/multi-agent)
- 代理工作区：[代理工作区](/zh/en/concepts/agent-workspace)
...
如果省略 `accountId` (`--bind <channel>`)，OpenClaw 将在可用时从通道默认值和插件设置钩子中解析它。

### 绑定范围行为

- 没有 `accountId` 的绑定仅匹配通道默认账户。
- `accountId: "*"` 是通道范围的回退（所有账户），比显式账户绑定的优先级低。
- 如果同一代理已经有一个没有 `accountId` 的匹配通道绑定，并且您稍后使用显式或解析的 `accountId` 进行绑定，OpenClaw 将就地升级现有绑定而不是添加重复项。
