...
- `reload`：网关 RPC（`secrets.reload`），仅当完全成功时才重新解析引用并交换运行时快照（不写入配置）。
- `audit`：只读扫描 configuration/auth/generated-模型 存储和遗留残留物，查找明文、未解析引用和优先级偏移（除非设置了 `--allow-exec`，否则跳过 exec 引用）。
- `configure`：提供商设置、目标映射和预检的交互式计划程序（需要 TTY）。
- `apply`：执行保存的计划（`--dry-run` 仅用于验证；试运行默认跳过 exec 检查，写入模式拒绝包含 exec 的计划，除非设置了 `--allow-exec`），然后清理目标明文残留物。
...
如果您的计划包含 `exec` SecretRefs/提供商，请在试运行和写入应用命令上传递 `--allow-exec`。

CI/门控的退出代码说明：

- `audit --check` 在发现结果时返回 `1`。
- 未解析的引用返回 `2`。

相关：

- Secrets 指南：[Secrets Management](/zh/gateway/secrets)
- 凭证范围：[SecretRef Credential Surface](/zh/reference/secretref-credential-surface)
- 安全指南：[Security](/zh/gateway/security)

## 重新加载运行时快照

重新解析 secret 引用并以原子方式交换运行时快照。
...
- 如果解析失败，网关将保留最后已知良好的快照并返回错误（不会部分激活）。
...
- 首先进行提供商设置（`secrets.providers` 别名的 `add/edit/remove`）。
...
- `--skip-provider-setup`：跳过提供商设置，并将凭据映射到现有提供商。
...
- 它在应用之前执行预检解析。
- 如果预检/应用包含 exec 引用，请在这两个步骤中都保持 `--allow-exec` 设置。
...
- 仅在受信任的包管理器路径需要时设置 `allowSymlinkCommand: true`，并将其与 `trustedDirs` 配对（例如 `["/opt/homebrew"]`）。
...
- 使用 `--allow-exec` 以选择加入任一模式下的 exec 提供商检查/执行。
...
- `openclaw.json` (SecretRef targets + 提供商 upserts/deletes)
- `auth-profiles.json` (提供商-target scrubbing)
