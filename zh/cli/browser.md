...
- 浏览器工具 + API：[浏览器工具](/en/tools/browser)
- Chrome 扩展中继：[Chrome 扩展](/en/tools/chrome-extension)
...
- `chrome-relay`：通过 Chrome 扩展中继控制您现有的 Chrome 标签页。
...
如果 Gateway(网关) 运行在与浏览器不同的机器上，请在装有 Chrome/Brave/Edge/Chromium 的机器上运行 **node host**。Gateway(网关) 将把浏览器操作代理到该节点（不需要单独的浏览器控制服务器）。
