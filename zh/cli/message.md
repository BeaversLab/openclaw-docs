...
- `send`
  ...
  - 仅限 Telegram：`--buttons`（需要 `channels.telegram.capabilities.inlineButtons` 允许此操作）
  - 仅限 Telegram：`--force-document`（将图片和 GIF 作为文档发送，以避免 Telegram 压缩）
  - 仅限 Telegram：`--thread-id`（论坛主题 ID）
  - 仅限 Slack：`--thread-id`（话题时间戳；`--reply-to` 使用相同的字段）
...
- `react`
  ...
  - 注意：`--remove` 需要 `--emoji`（在支持的情况下，省略 `--emoji` 以清除自己的反应；请参阅 /tools/reactions）
