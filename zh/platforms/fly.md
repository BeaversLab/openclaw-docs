...
**目标：** 在 [OpenClaw](https://fly.io) 机器上运行 Gateway(网关) Fly.io，具有持久存储、自动 HTTPS 和 Discord/渠道访问权限。
...
**安全提示：** 默认配置会公开一个公共 URL。对于没有公共 IP 的加固部署，请参阅[私有部署](#private-deployment-hardened)或使用 `fly.private.toml`。
...
| `--allow-unconfigured`         | 在没有配置文件的情况下启动（稍后你会创建一个）                          |
...
- 像对待密码一样对待这些令牌。
- **首选环境变量而非配置文件** 来处理所有 API 密钥和令牌。这可以防止密钥进入 `openclaw.json` 而被意外泄露或记录。
...
粘贴您的 Gateway 令牌（来自 `OPENCLAW_GATEWAY_TOKEN` 的那个）以进行身份验证。
...
Gateway 正绑定到 `127.0.0.1` 而不是 `0.0.0.0`。
...
**注意：** 512MB 太小了。1GB 可能可以工作，但在负载下或使用详细日志记录时可能会 OOM。**建议使用 2GB。**

### Gateway 锁定问题

Gateway 拒绝启动并出现 "already running" 错误。

当容器重启但 PID 锁定文件仍保留在卷上时，会发生这种情况。
...
如果使用 `--allow-unconfigured`，Gateway 会创建一个最小配置。重启时应读取位于 `/data/openclaw.json` 的自定义配置。
...
**修复：** 确保 `OPENCLAW_STATE_DIR=/data` 在 `fly.toml` 中已设置并重新部署。
...
默认情况下，Fly 会分配公共 IP，使您的网关可以通过 `https://your-app.fly.dev` 访问。这很方便，但也意味着您的部署会被互联网扫描器（Shodan、Censys 等）发现。

要获得**没有公开暴露**的加固部署，请使用私有模板。
...
如果您需要没有公开暴露的 webhook 回调（Twilio、Telnyx 等）：
...
ngrok 隧道在容器内运行，并提供一个公共 webhook URL，而无需暴露 Fly 应用程序本身。将 `webhookSecurity.allowedHosts` 设置为公共隧道主机名，以便接受转发的主机头。
