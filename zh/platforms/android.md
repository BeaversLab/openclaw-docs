...
- 安装：[入门指南](/zh/en/start/getting-started) + [配对](/zh/en/channels/pairing)。
- Gateway 网关：[手册](/zh/en/gateway) + [配置](/zh/en/gateway/configuration)。
- 协议：[Gateway 网关 协议](/zh/en/gateway/protocol)（节点 + 控制平面）。

## 系统控制

系统控制 (launchd/systemd) 位于 Gateway 网关 主机上。参见 [Gateway 网关](/zh/en/gateway)。
...
更多调试说明：[Bonjour](/zh/en/gateway/bonjour)。
...
详细信息和 CoreDNS 配置示例：[Bonjour](/zh/en/gateway/bonjour)。
...
配对详细信息：[配对](/zh/en/channels/pairing)。
...
#### Canvas Gateway 网关 Canvas主机（推荐用于 Web 内容）

如果您希望节点显示代理可以在磁盘上编辑的真实 HTML/CSS/JS，请将节点指向 Gateway 网关 画布主机。

注意：节点从 Gateway 网关 HTTP 服务器加载画布（与 `gateway.port` 端口相同，默认 `18789`）。
...
有关参数和 CLI 帮助程序，请参阅[相机节点](/zh/en/nodes/camera)。
