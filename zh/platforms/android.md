---
summary: "Android 应用（节点）：连接手册 + 连接/聊天/语音/画布命令界面"
read_when:
...
- 网关：[手册](/zh/en/gateway) + [配置](/zh/en/gateway/configuration)。
  - 协议：[网关协议](/zh/en/gateway/protocol)（节点 + 控制平面）。

## 系统控制

系统控制 (launchd/systemd) 位于网关主机上。参见 [网关](/zh/en/gateway)。
...
Android 聊天选项卡支持会话选择（默认 `main`，以及其他现有会话）：
...
该服务器会将实时重载客户端注入 HTML，并在文件更改时重新加载。
