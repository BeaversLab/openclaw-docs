...
在 DigitalOcean 上运行持久化的 OpenClaw 网关，费用为 **$6/月**（预留价格则为 $4/月）。

如果您希望选择 $0/月的选项，并且不介意 ARM 架构和特定于提供商的设置，请参阅 [Oracle Cloud 指南](/en/platforms/oracle)。
...
- Hetzner：性价比高（参见 [Hetzner 指南](/en/install/hetzner)）
- Oracle Cloud：可以免费 ($0/月)，但较为挑剔且仅限 ARM（参见 [Oracle 指南](/en/platforms/oracle)）
...
- Serve 保持网关仅限环回访问，并通过 Tailscale 身份标头验证控制界面/WebSocket 流量（无令牌认证假定网关主机受信任；HTTP API 仍需令牌/密码）。
...
有关完整设置指南，请参阅 [Oracle Cloud](/en/platforms/oracle)。有关注册提示和故障排除注册过程，请参阅此 [社区指南](https://gist.github.com/rssnyder/51e3cfedd730e7dd5f4a816143b25dbd)。
...
- [Docker 安装](/en/install/docker) — 容器化部署
- [Tailscale](/en/gateway/tailscale) — 安全的远程访问
