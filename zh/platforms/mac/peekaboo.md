...
代理。这允许 `peekaboo` CLI 驱动 UI 自动化，同时复用
macOS 应用程序的 TCC 权限。
...
使用 `peekaboo bridge status --verbose` 查看哪个主机处于活动状态以及正在使用
哪个套接字路径。您可以通过以下方式覆盖：
...
- 桥接验证 **调用者代码签名**；强制执行 TeamID 的
  允许列表（Peekaboo 主机 TeamID + OpenClaw 应用 TeamID）。
- 请求在大约 10 秒后超时。
...
- 如果 `peekaboo` 报告“桥接客户端未授权”，请确保客户端
  已正确签名，或仅以 **debug** 模式使用 `PEEKABOO_ALLOW_UNSIGNED_SOCKET_CLIENTS=1` 运行主机。
- 如果未找到主机，请打开其中一个主机应用程序（Peekaboo.app 或 OpenClaw.app）
  并确认已授予权限。
