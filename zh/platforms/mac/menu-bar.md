...
- 菜单中的“节点”块仅列出**设备**（通过 `node.list` 配对的节点），而不列出客户端/在线条目。
- 当提供商使用情况快照可用时，“使用情况”部分会显示在 Context 下方。

## 状态模型

- 会话：事件带有 `runId`（每次运行）和负载中的 `sessionKey` 到达。“主”会话是键 `main`；如果不存在，我们会回退到最近更新的会话。
...
- `workingOther`：带图标的徽章，静音色调，无疾跑动画。
...
- 解析字段：
  - `stream: "job"` 搭配 `data.state` 用于开始/停止。
  - `stream: "tool"` 搭配 `data.phase`、`name`、可选的 `meta`/`args`。
- 标签：
  - `exec`：`args.command` 的第一行。
  - `read`/`write`：缩短的路径。
  - `edit`：路径加上从 `meta`/diff counts 推断出的变更类型。
  - 回退：工具名称。
