...
此应用程序现在包含 Sparkle 自动更新功能。Release 版本必须使用 Developer ID 签名，压缩为 zip 包，并附带签名的 appcast 条目进行发布。

## 先决条件

- 已安装 Developer ID Application 证书（例如：`Developer ID Application: <Developer Name> (<TEAMID>)`）。
- 在环境变量中设置 Sparkle 私钥路径为 `SPARKLE_PRIVATE_KEY_FILE`（指向您的 Sparkle ed25519 私钥的路径；公钥已内置在 Info.plist 中）。如果缺失，请检查 `~/.profile`。
- 用于 `xcrun notarytool` 的公证凭据（钥匙串配置文件或 API 密钥），如果您需要通过 Gatekeeper 安全验证的 DMG/zip 分发。
  - 我们使用一个名为 `openclaw-notary` 的钥匙串配置文件，它是根据您 Shell 配置文件中的 App Store Connect API 密钥环境变量创建的：
...
- `APP_BUILD` 映射到 `CFBundleVersion`/`sparkle:version`；请保持其为数字且单调递增（不要有 `-beta`），否则 Sparkle 会将其视为相同版本。
- 如果省略 `APP_BUILD`，`scripts/package-mac-app.sh` 会根据 `APP_VERSION` 推导出一个 Sparkle 兼容的默认值（`YYYYMMDDNN`：stable 默认为 `90`，prereleases 使用派生自后缀的通道），并使用该值与 git 提交计数中的较大者。
- 当发布工程需要特定的单调递增值时，您仍然可以显式覆盖 `APP_BUILD`。
- 对于 `BUILD_CONFIG=release`，`scripts/package-mac-app.sh` 现在默认自动为通用（`arm64 x86_64`）。您仍然可以使用 `BUILD_ARCHS=arm64` 或 `BUILD_ARCHS=x86_64` 进行覆盖。对于本地/开发构建（`BUILD_CONFIG=debug`），它默认为当前架构（`$(uname -m)`）。
...
发布时，将更新后的 `appcast.xml` 与发布资产（zip + dSYM）一起提交。

## 发布与验证

- 将 `OpenClaw-2026.3.13.zip`（和 `OpenClaw-2026.3.13.dSYM.zip`）上传到标签 `v2026.3.13` 的 GitHub 发布中。
- 确保原始 appcast URL 与生成的 feed 匹配：`https://raw.githubusercontent.com/openclaw/openclaw/main/appcast.xml`。
- 完整性检查：
  - `curl -I https://raw.githubusercontent.com/openclaw/openclaw/main/appcast.xml` 返回 200。
  - `curl -I <enclosure url>` 在资产上传后返回 200。
  - 在之前的公共版本上，从“关于”选项卡运行“检查更新…”，并验证 Sparkle 是否干净地安装了新版本。

完成的定义：已签名的应用程序 + appcast 已发布，更新流程从旧安装版本正常工作，并且发布资产已附加到 GitHub 发布。
