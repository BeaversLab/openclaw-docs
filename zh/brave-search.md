...
3. 将密钥存储在配置中，或者在 Gateway 环境中设置 `BRAVE_API_KEY`。
...
- 每个 Brave 计划都包含 **$5/月的免费额度**（自动续期）。Search 计划每 1,000 次请求费用为 $5，因此额度涵盖了每月 1,000 次查询。请在 Brave 仪表板中设置您的使用限制以避免意外收费。有关当前计划，请参阅 [Brave API portal](https://brave.com/search/api/)。
- 搜索计划包括 LLM 上下文端点和 AI 推理权限。存储结果以训练或微调模型需要具有明确存储权限的计划。请参阅 Brave [服务条款](https://api-dashboard.search.brave.com/terms-of-service)。
