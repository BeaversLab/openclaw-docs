...
Gateway WS 协议是 OpenClaw 的 **单一控制平面 + 节点传输**。
所有客户端（CLI、Web UI、macOS 应用、iOS/Android 节点、无头节点）都通过 WebSocket 连接，并在握手时声明其 **role（角色）** + **scope（范围）**。
...
方法范围只是第一道关卡。通过 `chat.send` 达到的某些斜杠命令会在此基础上应用更严格的命令级检查。例如，持久的 `/config set` 和 `/config unset` 写入需要 `operator.admin`。
...
- 状态条目包括 `deviceId`、`roles` 和 `scopes`，以便 UI 即使设备同时作为 **operator** 和 **node** 连接，也能为每个设备显示单行。

### 节点辅助方法

- 节点可以调用 `skills.bins` 来获取当前的可执行技能列表，以便进行自动允许检查。
...
- 配对后，Gateway(网关) 会颁发一个范围为连接角色 + 作用域的 **设备令牌**。它在 `hello-ok.auth.deviceToken` 中返回，应由客户端保存以供将来连接使用。
- 设备令牌可以通过 `device.token.rotate` 和
  `device.token.revoke` 进行轮换/撤销（需要 `operator.pairing` scope）。
...
- 所有 WS 客户端必须在 `connect` 期间包含 `device` 身份（操作员 + 节点）。
  ...
  - `gateway.controlUi.dangerouslyDisableDeviceAuth=true`（应急模式，严重的安全降级）。
...
- 首选签名负载为 `v3`，除 device/client/role/scopes/token/nonce 字段外，
  它还绑定了 `platform` 和 `deviceFamily`。
- 出于兼容性原因，仍接受旧版 `v2` 签名，但在重新连接时，
  配对设备元数据固定仍然控制命令策略。
...
此协议公开了**完整的网关 API**（状态、频道、模型、聊天、
代理、会话、节点、审批等）。确切范围由 `src/gateway/protocol/schema.ts` 中的 TypeBox 模式定义。
