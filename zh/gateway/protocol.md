...
- Presence 条目包括 `deviceId`、`roles` 和 `scopes`，以便 UI 可以每个设备显示一行
...
- 操作员可以调用 `tools.catalog` (`operator.read`) 来获取运行时工具目录
  代理。响应包括分组工具和来源元数据：
...
- 节点应包含一个稳定的设备标识（`device.id`），该标识派生自
  密钥对指纹。
- 网关针对每个设备 + 角色颁发令牌。
...
- 所有 WS 客户端必须在 `connect` 期间包含 `device` 身份信息（操作员 + 节点）。
  控制 UI 仅在以下模式下可以省略它：
...
对于仍使用预质询签名行为的旧版客户端，`connect` 现在返回
...
- 首选的签名载荷是 `v3`，它绑定了 `platform` 和 `deviceFamily`
  以及设备/客户端/角色/范围/令牌/随机数字段。
- 出于兼容性考虑，旧版 `v2` 签名仍然被接受，但已配对设备
  元数据固定仍然控制重新连接时的命令策略。
...
此协议暴露了**完整的网关 API**（状态、频道、模型、聊天、
代理、会话、节点、审批等）。具体的表面由 `src/gateway/protocol/schema.ts` 中的 TypeBox 架构定义。
