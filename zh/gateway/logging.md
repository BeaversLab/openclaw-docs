...
控制 UI 日志选项卡通过网关（`logs.tail`）跟踪此文件。
CLI 也可以执行相同操作：
...
- `--verbose` 仅影响 **控制台详细程度**（和 WS 日志样式）；它 **不**
  提高文件日志级别。
- 要在文件日志中捕获仅详细模式的信息，请将 `logging.level` 设置为 `debug` 或
  `trace`。
...
详细的工具摘要（例如 `🛠️ Exec: ...`）可以在敏感令牌到达控制台流之前将其屏蔽。这 **仅适用于工具**，不会更改文件日志。
...
- `--ws-log auto`（默认）：优化了正常模式；详细模式使用紧凑输出
- `--ws-log compact`：详细时使用紧凑输出（配对的请求/响应）
- `--ws-log full`：详细时使用每帧完整输出
