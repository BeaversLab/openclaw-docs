...
`~/.openclaw/openclaw.json` 中提供的每个字段。有关面向任务的概述，请参阅 [配置](/zh/gateway/configuration)。
...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目为论坛主题配置持久的 ACP 绑定（在 `match.peer.id` 中使用规范的 `chatId:topic:topicId`）。字段语义在 [ACP Agents](/zh/tools/acp-agents#channel-specific-settings) 中共享。
...
- 带有 `type: "acp"` 的顶层 `bindings[]` 条目可为渠道和线程配置持久的 ACP 绑定（在 `match.peer.id` 中使用渠道/线程 ID）。字段语义在 [ACP Agents](/zh/tools/acp-agents#channel-specific-settings) 中共享。
...
- 完整的 BlueBubbles 渠道配置在 [BlueBubbles](/zh/channels/bluebubbles) 中有详细记录。
...
- 完整的 Teams 配置（凭据、webhook、私信/群组策略、每个团队/每个渠道的覆盖）在 [Microsoft Teams](/zh/channels/msteams) 中有详细记录。
...
许多扩展渠道配置为 `channels.<id>`，并在其专门的渠道页面中进行了记录（例如飞书、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
...
- Per-agent: 设置 `agents.list[].heartbeat`。当任何代理定义了 `heartbeat` 时，**只有那些代理** 会运行心跳。
...
- `mode`: `default` 或 `safeguard`（针对长历史记录的分块摘要）。请参阅 [Compaction](/zh/concepts/compaction)。
- `identifierPolicy`: `strict`（默认值）、`off` 或 `custom`。`strict` 会在压缩摘要期间内置不透明标识符保留指导的前缀。
- `identifierInstructions`: 可选的自定义标识符保留文本，在 `identifierPolicy=custom` 时使用。
- `postCompactionSections`: 可选的 AGENTS.md H2/H3 部分名称，用于在压缩后重新注入。默认为 `["Session Startup", "Red Lines"]`；设置为 `[]` 可禁用重新注入。当未设置或显式设置为该默认对时，较旧的 `Every Session`/`Safety` 标题也会作为传统备选方案被接受。
- `model`: 可选的 `provider/model-id` 覆盖设置，仅用于压缩摘要。当主会话应保留一个模型，但压缩摘要应在另一个模型上运行时使用此选项；未设置时，压缩使用会话的主模型。
...
- 渠道覆盖：`channels.<channel>.blockStreamingCoalesce`（以及按帐户区分的变体）。Signal/Slack/Discord/Google Chat 默认为 `minChars: 1500`。
...
为嵌入式代理提供可选的 **Docker 沙箱隔离**。有关完整指南，请参阅[沙箱隔离](/zh/gateway/sandboxing)。
...
- `none`：位于 `~/.openclaw/sandboxes` 下的逐范围沙箱工作区
...
**`docker.binds`** 挂载其他主机目录；全局和逐代理绑定会被合并。
...
- `network` 默认为 `openclaw-sandbox-browser`（专用桥接网络）。仅在您明确想要全局桥接连接时才设置为 `bridge`。
...
- 启动默认值在 `scripts/sandbox-browser-entrypoint.sh` 中定义，并针对容器主机进行了调整：
  ...
  - `--renderer-process-limit=2` 可以通过
    ...
    默认进程限制。
  - 当 `noSandbox` 启用时，加上 `--no-sandbox` 和 `--disable-setuid-sandbox`。
...
- `params`: 合并到 `agents.defaults.models` 中选定模型条目的按代理流参数。将其用于代理特定的覆盖，如 `cacheRetention`、`temperature` 或 `maxTokens`，而无需复制整个模型目录。
- `runtime`: 可选的按代理运行时描述符。当代理应默认为 ACP harness 会话时，使用带有 `runtime.acp` 默认值 (`agent`、`backend`、`mode`、`cwd`) 的 `type: "acp"`。
- `identity.avatar`: 工作区相对路径、`http(s)` URL 或 `data:` URI。
- `identity` 派生默认值: `ackReaction` 来自 `emoji`，`mentionPatterns` 来自 `name`/`emoji`。
- `subagents.allowAgents`: 用于 `sessions_spawn` 的代理 ID 允许列表 (`["*"]` = 任意; 默认值: 仅限相同代理)。
- 沙箱继承保护: 如果请求者会话已沙箱隔离，则 `sessions_spawn` 会拒绝将以非沙箱方式运行的目标。

---

## 多代理路由

在一个 Gateway(网关) 内部运行多个隔离的代理。请参阅 [Multi-Agent](/zh/concepts/multi-agent)。
...
- `type` (可选): 用于普通路由的 `route` (缺少类型时默认为 route)，用于持久化 ACP 会话绑定的 `acp`。
- `match.channel` (必需)
...
对于 `type: "acp"` 条目，OpenClaw 通过精确的对话标识（`match.channel` + account + `match.peer.id`）进行解析，并且不使用上述的路由绑定层级顺序。
...
- **`dmScope`**: 如何对私信进行分组。
  - `main`: 所有私信共享主会话。
  - `per-peer`: 在所有渠道中按发送者 ID 隔离。
  - `per-channel-peer`: 按渠道 + 发送者隔离（推荐用于多用户收件箱）。
  - `per-account-channel-peer`: 按账户 + 渠道 + 发送者隔离（推荐用于多账户）。
- **`identityLinks`**: 将规范 ID 映射到提供商前缀的对等节点，以便跨渠道共享会话。
- **`reset`**: 主要重置策略。`daily` 在 `atHour` 本地时间重置；`idle` 在 `idleMinutes` 之后重置。如果两者都已配置，则以先过期者为准。
...
- **`sendPolicy`**: 按 `channel`、`chatType`（`direct|group|channel`，旧版别名为 `dm`）、`keyPrefix` 或 `rawKeyPrefix` 匹配。优先匹配拒绝项。
- **`maintenance`**: 会话存储清理 + 保留控制。
  ...
  - `maxDiskBytes`: 可选的会话目录磁盘预算。在 `warn` 模式下，它会记录警告；在 `enforce` 模式下，它会先删除最旧的人工制品/会话。
...
- **`threadBindings`**: 线程绑定会话功能的全局默认值。
  ...
  - `maxAgeHours`: 默认的硬性最大时间（小时）（`0` 禁用；提供商可以覆盖）
...
解析顺序（最具体的优先）：account → 渠道 → global。`""` 会禁用并停止级联。`"auto"` 派生自 `[{identity.name}]`。
...
- 默认为当前活跃代理的 `identity.emoji`，否则为 `"👀"`。设置 `""` 以禁用。
...
- 对于自动摘要，`summaryModel` 会覆盖 `agents.defaults.model.primary`。
- `modelOverrides` 默认启用；`modelOverrides.allowProvider` 默认为 `false`（需手动启用）。
- API 密钥会回退到 API keys fall back to `ELEVENLABS_API_KEY`/`XI_API_KEY` 和 `OPENAI_API_KEY`。
- `openai.baseUrl` 会覆盖 OpenAI TTS 端点。解析顺序依次为配置，然后是 `OPENAI_TTS_BASE_URL`，然后是 `https://api.openai.com/v1`。
- 当 `openai.baseUrl` 指向非 OpenAI 端点时，OpenClaw 会将其视为 OpenAI 兼容的 TTS 服务器，并放宽对模型/声音的验证。
...
设置可以在 `tools.loopDetection` 中全局定义，并在 `agents.list[].tools.loopDetection` 处针对每个代理进行覆盖。
...
- `detectors.genericRepeat`：在重复使用相同工具/相同参数的调用时发出警告。
...
控制哪些会话可以被会话工具 (`sessions_list`, `sessions_history`, `sessions_send`) 定向。
...
- `self`: 仅当前会话密钥。
- `tree`：当前会话 + 由当前会话（子代理）生成的会话。
- `agent`：属于当前代理 ID 的任何会话（如果您在同一代理 ID 下运行按发送方区分的会话，则可能包含其他用户）。
- `all`：任何会话。跨代理定位仍然需要 `tools.agentToAgent`。
- 沙箱限制：当当前会话被沙箱隔离并且 `agents.defaults.sandbox.sessionToolsVisibility="spawned"` 时，即使 `tools.sessions.visibility="all"`，可见性也会被强制为 `tree`。
...
- 文件会在子工作区中的 `.openclaw/attachments/<uuid>/` 处以 `.manifest.json` 实现。
- 附件内容会从转录持久化中自动编辑掉。
- Base64 输入会通过严格的字母/填充检查和预解码大小保护进行验证。
- 目录的文件权限是 `0700`，文件的权限是 `0600`。
...
OpenClaw 使用 pi-coding-agent 模型目录。可以通过配置中的 `models.providers` 或 `~/.openclaw/agents/<agentId>/agent/models.json` 添加自定义提供商。
...
- 匹配提供商 ID 的合并优先级：
  ...
  - SecretRef 管理的提供商 `apiKey` 值从源标记刷新（环境引用使用 `ENV_VAR_NAME`，文件/exec 引用使用 `secretref-managed`），而不是持久化解析后的密钥。
  - SecretRef 管理的提供商标头值从源标记刷新（环境引用使用 `secretref-env:ENV_VAR_NAME`，文件/exec 引用使用 `secretref-managed`）。
  ...
  - 标记持久化以源为准：标记是从活动源配置快照（解析前）写入的，而不是从解析后的运行时密钥值写入的。
...
- `models.providers.*.authHeader`：在需要时，强制凭据通过 `Authorization` 请求头传输。
- `models.providers.*.baseUrl`: 上游 API 基础 URL。
- `models.providers.*.headers`：用于代理/租户路由的额外静态请求头。
- `models.providers.*.models`：显式的提供商模型目录条目。
- `models.providers.*.models.*.compat.supportsDeveloperRole`: 可选的兼容性提示。对于具有非空非原生 `baseUrl`（主机不是 `api.openai.com`）的 `api: "openai-completions"`，OpenClaw 会在运行时将其强制设为 `false`。为空/省略的 `baseUrl` 将保持默认的 OpenAI 行为。
...
设置 `OPENCODE_API_KEY` (或 `OPENCODE_ZEN_API_KEY`)。为 Zen 目录使用 `opencode/...` 引用，或为 Go 目录使用 `opencode-go/...` 引用。快捷方式：`openclaw onboard --auth-choice opencode-zen` 或 `openclaw onboard --auth-choice opencode-go`。
...
参见 [Local Models](/zh/gateway/local-models)。TL;DR：在性能强劲的硬件上通过 LM Studio Responses API 运行 MiniMax M2.5；保留托管的模型合并以用于回退。
...
- `entries.<skillKey>.enabled: false` 会禁用一个 Skill，即使它已被捆绑/安装。
- `entries.<skillKey>.apiKey`：用于声明主要环境变量的 Skills 的便捷字段（纯文本字符串或 SecretRef 对象）。
...
- `plugins.entries.<id>.hooks.allowPromptInjection`：当为 `false` 时，核心会拦截 `before_prompt_build` 并忽略来自旧版 `before_agent_start` 的提示词变更字段，同时保留旧版的 `modelOverride` 和 `providerOverride`。
- `plugins.entries.<id>.config`：插件定义的配置对象（由插件架构验证）。
- `plugins.slots.memory`：选择活动的内存插件 ID，或设为 `"none"` 以禁用内存插件。
- `plugins.slots.contextEngine`：选择活动的上下文引擎插件 ID；默认为 `"legacy"`，除非你安装并选择了其他引擎。
...
- 自动检测顺序：如果是基于 Chromium，则为默认浏览器 → Chrome → Brave → Edge → Chromium → Chrome Canary。
- 控制服务：仅限环回（端口源自 `gateway.port`，默认为 `18791`）。
- `extraArgs` 将额外的启动标志附加到本地 Chromium 启动中（例如
...
- **Docker note**：默认的 `loopback` 绑定在容器内的 `127.0.0.1` 上监听。使用 Docker bridge 网络时（`-p 18789:18789`），流量到达 `eth0`，因此无法访问网关。请使用 `--network host`，或设置 `bind: "lan"`（或带有 `customBindHost: "0.0.0.0"` 的 `bind: "custom"`）以在所有接口上监听。
- **Auth**：默认情况下需要。非 local loopback 绑定需要共享令牌/密码。新手引导 向导 默认生成令牌。
...
- `gateway.auth.mode: "none"`：显式的无身份验证模式。仅用于受信任的 local loopback 设置；新手引导 提示有意不提供此选项。
- `gateway.auth.mode: "trusted-proxy"`：将身份验证委托给感知身份的反向代理，并信任来自 `gateway.trustedProxies` 的身份标头（请参阅 [Trusted Proxy Auth](/zh/gateway/trusted-proxy-auth)）。
- `gateway.auth.allowTailscale`：当 `true` 时，Tailscale Serve 身份标头可以满足 Control UI/WebSocket 身份验证（通过 `tailscale whois` 验证）；HTTP API 端点仍需要令牌/密码身份验证。此无令牌流程假设网关主机是受信任的。当 `tailscale.mode = "serve"` 时，默认为 `true`。
- `gateway.auth.rateLimit`：可选的身份验证失败限制器。适用于每个客户端 IP 和每个身份验证范围（共享密钥和设备令牌独立跟踪）。阻止的尝试返回 `429` + `Retry-After`。
  - `gateway.auth.rateLimit.exemptLoopback` 默认为 `true`；当您有意希望本地主机流量也受到速率限制时（用于测试设置或严格的代理部署），请设置 `false`。
- 浏览器源 WS 身份验证尝试始终会受到限制，并且禁用 local loopback 豁免（针对基于浏览器的本地主机暴力破解的纵深防御）。
- `tailscale.mode`：`serve`（仅 tailnet，local loopback 绑定）或 `funnel`（公开，需要身份验证）。
- `controlUi.allowedOrigins`：Gateway 网关 WebSocket 连接的显式浏览器源允许列表。当预期浏览器客户端来自非 local loopback 源时，这是必需的。
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`：危险模式，为有意依赖 Host-header 源策略的部署启用 Host-header 源回退。
- `remote.transport`：`ssh`（默认）或 `direct`（ws/wss）。对于 `direct`，`remote.url` 必须是 `ws://` 或 `wss://`。
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`：客户端紧急覆盖，允许纯文本 `ws://` 到受信任的专用网络 IP；纯文本的默认设置仍然是仅限 local loopback。
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`：仅用于开发的 local loopback HTTP 中继 URL 的紧急逃生舱。生产中继 URL 应保持 HTTPS。
- 本地网关调用路径仅在 `gateway.auth.*` 未设置时才能将 `gateway.remote.*` 作为回退。
- 如果 `gateway.auth.token` / `gateway.auth.password` 是通过 SecretRef 显式配置且未解析，则解析失败关闭（没有远程回退掩盖）。
...
- 可选响应加固标头：
  - `gateway.http.securityHeaders.strictTransportSecurity`（仅为您控制的 HTTPS 源设置；参见[受信任的代理身份验证](/zh/gateway/trusted-proxy-auth#tls-termination-and-hsts)）

### 多实例隔离

在一个主机上运行多个网关，使用唯一端口和状态目录：
...
- Gateway(网关) 网关在配置后会在启动时自动启动 `gog gmail watch serve`。设置 `OPENCLAW_SKIP_GMAIL_WATCHER=1` 以禁用。
- 不要在 Gateway(网关) 网关旁边运行单独的 `gog gmail watch serve`。
...
- 在 Gateway(网关) 网关端口下通过 HTTP 提供代理可编辑的 HTML/CSS/JS 和 A2UI：
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- 仅限本地：保留 `gateway.bind: "loopback"`（默认）。
- 非环回绑定：canvas 路由需要 Gateway(网关) 网关身份验证（令牌/密码/受信任代理），与其他 Gateway(网关) 网关 HTTP 表面一样。
- 节点 WebViews 通常不发送身份验证标头；节点配对并连接后，Gateway(网关) 网关会通告节点范围的 Canvas/A2UI 访问功能 URL。
- 功能 URL 绑定到活动的节点 WS 会话并很快过期。不使用基于 IP 的回退。
- 将实时重新加载客户端注入到提供的 HTML 中。
- 当为空时自动创建初始 `index.html`。
...
在 `~/.openclaw/dns/` 下写入单播 DNS-SD 区域。对于跨网络设备发现，请搭配 DNS 服务器（推荐 CoreDNS）+ Tailscale 分离 DNS 使用。
...
- `exec` 提供商需要绝对 `command` 路径并在 stdin/stdout 上使用协议载荷。
- 默认情况下，符号链接命令路径会被拒绝。设置 `allowSymlinkCommand: true` 以在验证解析后的目标路径时允许符号链接路径。
- 如果配置了 `trustedDirs`，信任目录检查将应用于解析后的目标路径。
- `exec` 子环境默认是最小的；请通过 `passEnv` 显式传递所需的变量。
- Secret 引用在激活时解析为内存中的快照，然后请求路径仅读取该快照。
- 激活期间应用活动面过滤：已启用表面上未解析的引用会导致启动/重新加载失败，而未启用的表面将被跳过并输出诊断信息。
...
- Secret 运行时行为和 `audit/configure/apply` 工具：[Secret 管理](/zh/gateway/secrets)。
...
- 来自 `identity.emoji` 的 `messages.ackReaction`（回退至 👀）
- 来自 `identity.name`/`identity.emoji` 的 `mentionPatterns`
...
当前版本不再包含 TCP 桥接。节点通过 Gateway 网关 WebSocket 连接。`bridge.*` 键不再是配置架构的一部分（必须移除否则验证失败；`openclaw doctor --fix` 可以去除未知键）。
...
- `sessionRetention`：在从 `sessions.json` 中修剪之前保留已完成的隔离 cron 运行会话的时间。同时控制已归档的已删除 cron 脚本的清理。默认值：`24h`；设置为 `false` 以禁用。
- `runLog.maxBytes`：修剪前每个运行日志文件（`cron/runs/<jobId>.jsonl`）的最大大小。默认值：`2_000_000` 字节。
- `runLog.keepLines`：触发运行日志修剪时保留的最新行数。默认值：`2000`。
- `webhookToken`：用于 cron webhook POST 传递（`delivery.mode = "webhook"`）的 bearer token，如果省略则不发送认证头。
...
- 单个文件：替换包含它的对象。
- 文件数组：按顺序深度合并（后者的覆盖前者的）。
- 兄弟键：在包含之后合并（覆盖包含的值）。
- 嵌套包含：最多 10 层深。
- 路径：相对于包含文件解析，但必须保持在顶级配置目录（`dirname` 的 `openclaw.json`）内。仅当解析结果仍在该边界内时，才允许使用绝对/`../` 形式。
- 错误：针对缺失文件、解析错误和循环包含的明确消息。

---

_相关内容：[Configuration](/zh/gateway/configuration) · [Configuration Examples](/zh/gateway/configuration-examples) · [Doctor](/zh/gateway/doctor)_
