...
- 具有 `type: "acp"` 的顶级 `bindings[]` 条目为渠道和线程配置持久的 ACP 绑定（在 `match.peer.id` 中使用渠道/线程 ID）。字段语义在 [ACP 代理](/en/tools/acp-agents#channel-specific-settings) 中共享。
...
- 完整的 Teams 配置（凭据、webhook、私信/群组策略、每个团队/每个渠道的覆盖）记录在 [Microsoft Teams](/en/channels/msteams) 中。
...
群组消息默认为 **需要提及**（元数据提及或安全的正则模式）。适用于 WhatsApp、Telegram、Discord、Google Chat 和 iMessage 群组聊天。
...
- `postCompactionSections`：可选的 AGENTS.md H2/H3 部分名称，用于在压缩后重新注入。默认为 `["Session Startup", "Red Lines"]`；设置为 `[]` 可禁用重新注入。如果未设置或显式设置为该默认值对，则较旧的 `Every Session`/`Safety` 标题也将作为传统后备方案被接受。
- `model`：可选的 `provider/model-id` 覆盖设置，仅用于压缩摘要。当主会话应保留一个模型但压缩摘要应在另一个模型上运行时，请使用此选项；如果未设置，压缩将使用会话的主模型。
- `memoryFlush`：在自动压缩之前进行的静默智能体回合，用于存储持久记忆。当工作区为只读时跳过。

### `agents.defaults.contextPruning`

在发送给 LLM 之前，从内存上下文中修剪 **旧的工具结果**。**不** 会修改磁盘上的会话历史记录。
...
- 修剪首先对过大的工具结果进行软修剪，然后如果需要，对较旧的工具结果进行硬清除。
...
- 频道覆盖：`channels.<channel>.blockStreamingCoalesce` （以及每个帐户的变体）。Signal/Slack/Discord/Google Chat 默认为 `minChars: 1500`。
...
**`setupCommand`** 在容器创建后运行一次（通过 `sh -lc`）。需要网络出站权限、可写根目录、root 用户。

**容器默认为 `network: "none"`** —— 如果代理需要出站访问，请设置为 `"bridge"`（或自定义桥接网络）。
`"host"` 已被阻止。`"container:<id>"` 默认被阻止，除非您显式设置
...
**沙箱隔离浏览器** （`sandbox.browser.enabled`）：容器中的 Chromium + CDP。noVNC URL 被注入到系统提示词中。不需要 `browser.enabled` 中的 `openclaw.json`。
...
- 启动默认值在 `scripts/sandbox-browser-entrypoint.sh` 中定义，并针对容器主机进行了调整：
  ...
  - `--disable-3d-apis`、`--disable-software-rasterizer` 和 `--disable-gpu`
    默认已启用，如果 WebGL/3D 使用需要，可以使用
    `OPENCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` 禁用。
  - `OPENCLAW_BROWSER_DISABLE_EXTENSIONS=0` 如果您的工作流依赖于扩展，
    则会重新启用它们。
...
- `id`：稳定的代理 ID（必需）。
- `default`：当设置了多个时，第一个生效（会记录警告）。如果未设置任何项，则默认为列表中的第一个条目。
- `model`：字符串形式仅覆盖 `primary`；对象形式 `{ primary, fallbacks }` 覆盖两者（`[]` 禁用全局回退）。仅覆盖 `primary` 的定时任务仍然会继承默认回退，除非您设置 `fallbacks: []`。
- `params`：合并到 `agents.defaults.models` 中所选模型条目的每个代理流参数。使用它来进行特定于代理的覆盖，如 `cacheRetention`、`temperature` 或 `maxTokens`，而无需复制整个模型目录。
- `runtime`：可选的每个代理运行时描述符。当代理默认为 ACP harness 会话时，请使用带有 `runtime.acp` 默认值（`agent`、`backend`、`mode`、`cwd`）的 `type: "acp"`。
...
- 沙箱继承保护：如果请求者会话已启用沙箱隔离，`sessions_spawn` 将拒绝未启用沙箱隔离运行的目标。
...
- `type` (可选)：用于正常路由的 `route` (缺少类型时默认为 route)，用于持久化 ACP 会话绑定的 `acp`。
- `match.channel` (必需)
...
2. `match.guildId`
3. `match.teamId`
4. `match.accountId` (精确，无对端/服务器/团队)
...
- **`reset`**: 主要重置策略。`daily` 在 `atHour` 本地时间重置；`idle` 在 `idleMinutes` 后重置。如果两者都配置了，则以先到期的为准。
...
- **`sendPolicy`**: 按 `channel`、`chatType`（`direct|group|channel`，带有旧 `dm` 别名）、`keyPrefix` 或 `rawKeyPrefix` 匹配。优先拒绝生效。
- **`maintenance`**: 会话存储清理 + 保留控制。
  ...
  - `maxDiskBytes`: 可选的会话目录磁盘预算。在 `warn` 模式下它会记录警告；在 `enforce` 模式下它会首先删除最旧的工件/会话。
...
- **`threadBindings`**: 线程绑定会话功能的全局默认值。
  ...
  - `maxAgeHours`: 默认硬最大时长（小时）（`0` 禁用；提供商可以覆盖）
...
解析顺序（最具体优先）：账户 → 渠道 → 全局。`""` 禁用并停止级联。`"auto"` 派生 `[{identity.name}]`。
...
将同一发送者快速发送的仅文本消息合并为单个代理轮次。媒体/附件会立即刷新。控制命令会绕过防抖。
...
- 当 `openai.baseUrl` 指向非 OpenAI 端点时，OpenClaw 会将其视为与 OpenAI 兼容的 TTS 服务器，并放宽模型/语音验证。
...
- `ELEVENLABS_API_KEY` 回退仅在未配置交谈 API 密钥时适用。
- `voiceAliases` 允许交谈指令使用友好名称。
- `silenceTimeoutMs` 控制交谈模式在用户停止说话后发送转录文本之前等待的时间。未设置则保持平台默认的暂停窗口 (`700 ms on macOS and Android, 900 ms on iOS`)。
...
- 每个代理的覆盖 (`agents.list[].tools.elevated`) 只能进一步限制访问。
...
可以在 `tools.loopDetection` 中全局定义设置，并在 `agents.list[].tools.loopDetection` 中针对每个代理进行覆盖。
...
- `detectors.genericRepeat`：在重复调用相同工具/参数时发出警告。
- `detectors.knownPollNoProgress`：针对已知轮询工具（`process.poll`、`command_status` 等）发出警告或进行阻断。
- `detectors.pingPong`：针对交替无进展对模式发出警告或进行阻断。
...
- `self`: 仅当前会话密钥。
- `tree`: 当前会话 + 由当前会话衍生的会话 (子代理)。
- `agent`：属于当前代理 ID 的任何会话（如果您在同一代理 ID 下运行每个发送方的会话，则可能包括其他用户）。
- `all`：任何会话。跨代理定位仍然需要 `tools.agentToAgent`。
- 沙箱钳制：当当前会话处于沙箱隔离状态并且 `agents.defaults.sandbox.sessionToolsVisibility="spawned"` 时，即使设置了 `tools.sessions.visibility="all"`，可见性也会被强制为 `tree`。
...
- 文件通过 `.manifest.json` 具体化到子工作空间的 `.openclaw/attachments/<uuid>/` 位置。
- 附件内容会从会话记录持久化中自动编辑掉。
...
- `runTimeoutSeconds`：当工具调用省略 `runTimeoutSeconds` 时，`sessions_spawn` 的默认超时时间（秒）。`0` 表示没有超时。
...
- 匹配提供商 ID 的合并优先级：
  ...
  - SecretRef 管理的提供商 `apiKey` 值从源标记（环境引用为 `ENV_VAR_NAME`，文件/执行引用为 `secretref-managed`）刷新，而不是持久化已解析的密钥。
...
- `models.providers.*.authHeader`: 在需要时强制在 `Authorization` 头中传输凭证。
- `models.providers.*.baseUrl`: 上游 API 基础 URL。
- `models.providers.*.headers`: 用于代理/租户路由的额外静态头。
- `models.providers.*.models`: 显式的提供商模型目录条目。
- `models.providers.*.models.*.compat.supportsDeveloperRole`: 可选的兼容性提示。对于具有非空非原生 `baseUrl`（主机不是 `api.openai.com`）的 `api: "openai-completions"`，OpenClaw 会在运行时将其强制设为 `false`。为空或省略的 `baseUrl` 将保留默认的 OpenAI 行为。
...
参见 [Local Models](/en/gateway/local-models)。TL;DR：在性能强劲的硬件上通过 LM Studio Responses MiniMax 运行 API M2.5；保持托管模型合并以进行回退。
...
- `entries.<skillKey>.enabled: false` 即使已捆绑/安装，也会禁用 Skill。
- `entries.<skillKey>.apiKey`：针对声明了主要环境变量的 Skills 的便捷配置（纯文本字符串或 SecretRef 对象）。
...
- `plugins.entries.<id>.hooks.allowPromptInjection`：当 `false` 时，核心会阻止 `before_prompt_build` 并忽略来自旧式 `before_agent_start` 的提示修改字段，同时保留旧式的 `modelOverride` 和 `providerOverride`。适用于原生插件挂钩和受支持的包提供的挂钩目录。
- `plugins.entries.<id>.config`：插件定义的配置对象（当可用时由原生 OpenClaw 插件架构验证）。
- 已启用的 Claude 包插件还可以从 `settings.json` 贡献嵌入的 Pi 默认值；OpenClaw 会将这些作为经过清理的代理设置应用，而不是作为原始的 OpenClaw 配置补丁。
...
- `relayBindHost` 更改 Chrome 扩展中继的侦听位置。保持未设置为仅限环回访问；仅当中继必须跨越命名空间边界（例如 WSL2）且主机网络已受信任时，才设置显式的非环回绑定地址（例如 `0.0.0.0`）。
...
- `mode`: `local` (运行 Gateway) 或 `remote` (连接到远程 Gateway)。除非是 `local`，否则 Gateway 拒绝启动。
...
- **Docker note**: 默认的 `loopback` 绑定监听容器内部的 `127.0.0.1`。使用 Docker 桥接网络 (`-p 18789:18789`) 时，流量到达 `eth0`，因此无法访问 Gateway。请使用 `--network host`，或设置 `bind: "lan"` (或在 `customBindHost: "0.0.0.0"` 时设置 `bind: "custom"`) 以监听所有接口。
- **Auth**: 默认需要。非环回绑定需要共享的令牌/密码。新手引导向导默认生成令牌。
- 如果同时配置了 `gateway.auth.token` 和 `gateway.auth.password` (包括 SecretRefs)，则必须将 `gateway.auth.mode` 显式设置为 `token` 或 `password`。如果两者都已配置但未设置模式，则启动和服务安装/修复流程将失败。
- `gateway.auth.mode: "none"`: 显式的无身份验证模式。仅用于受信任的本地环回设置；新手引导提示有意不提供此模式。
- `gateway.auth.mode: "trusted-proxy"`: 将身份验证委托给支持身份感知的反向代理，并信任来自 `gateway.trustedProxies` 的身份标头 (参见 [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth))。
- `gateway.auth.allowTailscale`: 当为 `true` 时，Tailscale Serve 身份标头可以满足控制 UI/WebSocket 身份验证 (通过 `tailscale whois` 验证)；HTTP API 端点仍需要令牌/密码身份验证。此无令牌流程假定 Gateway 主机受信任。当 `tailscale.mode = "serve"` 时默认为 `true`。
- `gateway.auth.rateLimit`: 可选的身份验证失败限制器。应用于每个客户端 IP 和每个身份验证范围 (共享密钥和设备令牌独立跟踪)。被阻止的尝试返回 `429` + `Retry-After`。
  - `gateway.auth.rateLimit.exemptLoopback` 默认为 `true`；如果您有意对本地主机流量也进行速率限制 (例如用于测试设置或严格的代理部署)，请设置 `false`。
- 基于浏览器源的 WS 身份验证尝试始终受到限制，并禁用环回豁免 (针对基于浏览器的本地主机暴力破解的纵深防御)。
- `tailscale.mode`: `serve` (仅限 tailnet，环回绑定) 或 `funnel` (公开，需要身份验证)。
- `controlUi.allowedOrigins`: Gateway WebSocket 连接的显式浏览器源白名单。当预期有来自非环回源的浏览器客户端时，这是必需的。
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: 危险模式，为有意依赖 Host 头源策略的部署启用 Host 头源回退。
- `remote.transport`: `ssh` (默认) 或 `direct` (ws/wss)。对于 `direct`，`remote.url` 必须是 `ws://` 或 `wss://`。
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: 客户端端打破玻璃覆盖，允许纯文本 `ws://` 连接到受信任的专用网络 IP；对于纯文本，默认保持仅为环回。
- `gateway.remote.token` / `.password` 是远程客户端凭据字段。它们本身不配置 Gateway 身份验证。
- `gateway.push.apns.relay.baseUrl`: 官方/TestFlight iOS 版本在将基于中继的注册发布到 Gateway 后，使用的外部 APNs 中继的基本 HTTPS URL。此 URL 必须与编译到 iOS 版本中的中继 URL 匹配。
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: 仅用于开发的环回 HTTP 中继 URL 逃生舱。生产环境中继 URL 应保持为 HTTPS。
- `gateway.channelHealthCheckMinutes`: 渠道健康监视间隔 (分钟)。设置 `0` 可全局禁用健康监视重启。默认值：`5`。
- `gateway.channelStaleEventThresholdMinutes`: 陈旧套接字阈值 (分钟)。请保持此值大于或等于 `gateway.channelHealthCheckMinutes`。默认值：`30`。
- `gateway.channelMaxRestartsPerHour`: 每个频道/账户在滚动一小时内的最大健康监视重启次数。默认值：`10`。
- `channels.<provider>.healthMonitor.enabled`: 每个频道的健康监视重启退出选项，同时保持全局监视器处于启用状态。
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: 多账户频道的每账户覆盖。设置后，它优先于频道级别的覆盖。
- 本地 Gateway 调用路径仅当 `gateway.auth.*` 未设置时，才可以将 `gateway.remote.*` 用作回退。
- 如果 `gateway.auth.token` / `gateway.auth.password` 通过 SecretRef 显式配置但未解析，则解析失败关闭 (无远程回退掩盖)。
- `trustedProxies`: 终止 TLS 的反向代理 IP。仅列出您控制的代理。
- `allowRealIpFallback`: 当为 `true` 时，如果缺少 `X-Forwarded-For`，Gateway 接受 `X-Real-IP`。默认为 `false` 以实现失效关闭行为。
...
- 可选的响应硬化请求头：
  - `gateway.http.securityHeaders.strictTransportSecurity` （仅为您控制的 HTTPS 源设置；参见 [受信任的代理身份验证](/en/gateway/trusted-proxy-auth#tls-termination-and-hsts)）
...
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - 仅当 `hooks.allowRequestSessionKey=true` 时（默认：`false`），才接受来自请求负载的 `sessionKey`。
...
- `transform` 可以指向返回 Hook 操作的 JS/TS 模块。
  - `transform.module` 必须是相对路径且必须保持在 `hooks.transformsDir` 内部（绝对路径和路径遍历将被拒绝）。
- `agentId` 路由到特定的 agent；未知 ID 将回退到默认值。
- `allowedAgentIds`：限制显式路由（`*` 或省略 = 允许所有，`[]` = 拒绝所有）。
- `defaultSessionKey`：用于没有显式 `sessionKey` 的 Hook agent 运行的可选固定会话键。
...
- `model` 覆盖此次 Hook 运行的 LLM（如果设置了模型目录，则必须允许）。
...
- 在 Gateway(网关) 端口下通过 HTTP 提供可编辑的代理 HTML/CSS/JS 和 A2UI：
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- 仅限本地：保持 `gateway.bind: "loopback"`（默认）。
- 非环回绑定：canvas 路由需要 Gateway(网关) 认证（令牌/密码/受信任代理），与其他 Gateway(网关) HTTP 表面相同。
- Node WebView 通常不发送认证头；在节点配对并连接后，Gateway(网关) 会广播用于 canvas/A2UI 访问的节点范围功能 URL。
...
- 标准矩阵：[SecretRef 凭据表面](/en/reference/secretref-credential-surface)
...
- `exec` 提供商需要绝对 `command` 路径，并使用 stdin/stdout 上的协议负载。
...
- 静态运行时凭据来自内存解析快照；发现时会清除遗留的静态 `auth.json` 条目。
...
当前构建不再包含 TCP 网桥。节点通过 Gateway(网关) WebSocket 连接。`bridge.*` 键不再是配置模式的一部分（除非移除，否则验证失败；`openclaw doctor --fix` 可以去除未知键）。
...
- `sessionRetention`：在从 `sessions.json` 中修剪之前，保留已完成的隔离 cron 运行会话的时间。还控制已删除的 cron 归档脚本的清理。默认值：`24h`；设置为 `false` 以禁用。
...
| `{{Transcript}}`   | 音频转录                                     |
| `{{Prompt}}`       | 针对 CLI 条目的已解析媒体提示                |
| `{{MaxChars}}`     | 针对 CLI 条目的已解析最大输出字符数          |
...
- 同级键：在包含后合并（覆盖包含的值）。
- 嵌套包含：最多 10 层深度。
- 路径：相对于包含的文件解析，但必须保持在顶层配置目录内（`dirname` 的 `openclaw.json`）。仅当绝对/`../` 形式仍解析到该边界内时才允许。
- 错误：针对缺失文件、解析错误和循环包含的清晰消息。

---

_相关：[配置](/en/gateway/configuration) · [配置示例](/en/gateway/configuration-examples) · [诊断](/en/gateway/doctor)_
