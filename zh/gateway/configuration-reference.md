...
`~/.openclaw/openclaw.json` 中可用的每个字段。如需面向任务的概述，请参阅 [配置](/zh/en/gateway/configuration)。
...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目为论坛主题配置持久的 ACP 绑定（在 `match.peer.id` 中使用规范 `chatId:topic:topicId`）。字段语义在 [ACP Agents](/zh/en/tools/acp-agents#渠道-specific-settings) 中共享。
...
- 顶级 `bindings[]` 条目中带有 `type: "acp"` 的项可配置频道和线程的持久化 ACP 绑定（在 `match.peer.id` 中使用频道/线程 ID）。字段语义在 [ACP Agents](/zh/en/tools/acp-agents#渠道-specific-settings) 中共享。
...
- 完整的 Teams 配置（凭据、webhook、私信/群组策略、每个团队/每个频道的覆盖）记录在 [Microsoft Teams](/zh/en/channels/msteams) 中。
...
有关行为详情，请参阅[会话修剪](/zh/en/concepts/会话-pruning)。
...
嵌入式代理的可选 **Docker 沙箱**。有关完整指南，请参阅[沙箱](/zh/en/gateway/沙箱隔离)。
...
在一个 Gateway 网关 内运行多个隔离的代理。请参阅 [多代理](/zh/en/concepts/multi-agent)。
...
- `mode`：`local`（运行 Gateway 网关）或 `remote`（连接到远程 Gateway 网关）。除非设置了 `local`，否则 Gateway 网关 拒绝启动。
...
- `gateway.auth.mode: "trusted-proxy"`：将认证委托给具有身份感知的反向代理，并信任来自 `gateway.trustedProxies` 的身份标头（请参阅 [Trusted Proxy Auth](/zh/en/gateway/trusted-proxy-auth)）。
...
- `controlUi.allowedOrigins`：用于 Gateway 网关 WebSocket 连接的显式浏览器源允许列表。当预期来自非回环源的浏览器客户端时，此项是必需的。
...
- 可选的响应硬化标头：
  - `gateway.http.securityHeaders.strictTransportSecurity` （仅为您控制的 HTTPS 源设置；请参阅 [Trusted Proxy Auth](/zh/en/gateway/trusted-proxy-auth#tls-termination-and-hsts)）
...
- 不要在 Gateway 网关 之外运行单独的 `gog gmail watch serve`。
...
- 节点 WebViews 通常不发送身份验证头；在节点配对并连接后，Gateway 网关 会通告节点范围的能力 URL 以用于 canvas/A2UI 访问。
...
当前构建不再包含 TCP 网桥。节点通过 Gateway 网关 WebSocket 连接。`bridge.*` 键不再是配置架构的一部分（移除前验证会失败；`openclaw doctor --fix` 可以去除未知键）。
