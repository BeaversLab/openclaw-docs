...
`~/.openclaw/openclaw.json` 中提供的所有字段。如需面向任务的概述，请参阅 [配置](/zh/gateway/configuration)。
...
如果完全缺少提供商块（缺少 `channels.<provider>`），运行时组策略将回退到 `allowlist` （默认拒绝），并显示启动警告。
</Note>
...
- 如果存在，出站命令默认使用账号 `default`；否则使用第一个配置的账号 id（排序后）。
- 可选的 `channels.whatsapp.defaultAccount` 会在其与已配置的账号 id 匹配时覆盖该回退默认账号选择。
- 旧版单账号 Baileys 认证目录会通过 `openclaw doctor` 迁移到 `whatsapp/default` 中。
...
- Bot token：`channels.telegram.botToken` 或 `channels.telegram.tokenFile`（仅限常规文件；拒绝符号链接），其中 `TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
- 可选的 `channels.telegram.defaultAccount` 会在其与已配置的账号 id 匹配时覆盖默认账号选择。
- 在多账号设置中（2 个或更多账号 id），请设置显式的默认值（`channels.telegram.defaultAccount` 或 `channels.telegram.accounts.default`）以避免回退路由；如果此项缺失或无效，`openclaw doctor` 会发出警告。
- `configWrites: false` 会阻止 Telegram 发起的配置写入（超级群组 ID 迁移、`/config set|unset`）。
- 具有 `type: "acp"` 的顶级 `bindings[]` 条目为论坛主题配置持久的 ACP 绑定（在 `match.peer.id` 中使用规范 `chatId:topic:topicId`）。字段语义在 [ACP Agents](/zh/tools/acp-agents#channel-specific-settings) 中共享。
...
- Token：`channels.discord.token`，其中 `DISCORD_BOT_TOKEN` 作为默认账号的回退。
- 提供了显式 Discord `token` 的直接出站调用将使用该 token 进行调用；账号重试/策略设置仍来自活动运行时快照中选定的账号。
- 可选的 `channels.discord.defaultAccount` 在匹配到配置的账户 ID 时会覆盖默认的账户选择。
- 使用 `user:<id>` (私信) 或 `channel:<id>` (服务器频道) 作为投递目标；不接受纯数字 ID。
- 服务器标识符 (slugs) 为小写，空格替换为 `-`；频道键使用 slugged 名称（无 `#`）。建议优先使用服务器 ID。
- 默认情况下忽略由机器人发送的消息。`allowBots: true` 可启用它们；使用 `allowBots: "mentions"` 可仅接收提及该机器人的机器人消息（自身的消息仍会被过滤）。
- `channels.discord.guilds.<id>.ignoreOtherMentions`（以及频道覆盖设置）会丢弃提及其他用户或角色但未提及该机器人的消息（不包括 @everyone/@here）。
- `maxLinesPerMessage`（默认为 17）会分割高大的消息，即使字符数未超过 2000。
- `channels.discord.threadBindings` 控制 Discord 线程绑定路由：
  ...
  - `maxAgeHours`：针对硬性最大存活时间（小时）的 Discord 覆盖设置 (`0` 表示禁用)
  - `spawnSubagentSessions`：`sessions_spawn({ thread: true })` 自动线程创建/绑定的选择性启用开关
- 具有 `type: "acp"` 的顶级 `bindings[]` 条目可为频道和线程配置持久的 ACP 绑定（在 `match.peer.id` 中使用频道/线程 ID）。字段语义在 [ACP Agents](/zh/tools/acp-agents#channel-specific-settings) 中共享。
...
- **Socket 模式**需要同时设置 `botToken` 和 `appToken`（对于默认账号环境变量回退，需要 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。
- **HTTP 模式**需要 `botToken` 加上 `signingSecret`（在根目录或每个账号）。
- `configWrites: false` 阻止 Slack 发起的配置写入。
- 可选的 `channels.slack.defaultAccount` 在匹配配置的账号 ID 时会覆盖默认账号选择。
- `channels.slack.streaming` 是规范的流模式键。旧的 `streamMode` 和布尔 `streaming` 值会自动迁移。
- 使用 `user:<id>` (私信) 或 `channel:<id>` 作为交付目标。

**表情符号回复通知模式：** `off`、`own`（默认）、`all`、`allowlist`（来自 `reactionAllowlist`）。

**线程会话隔离：** `thread.historyScope` 为每个线程单独设置（默认）或在渠道间共享。`thread.inheritParent` 会将父渠道的记录复制到新线程中。

- `typingReaction` 会在回复运行时向传入的 Slack 消息添加一个临时表情符号，完成后将其移除。使用 Slack 表情符号代码，例如 `"hourglass_flowing_sand"`。
...
- 可选的 `channels.signal.defaultAccount` 会在匹配已配置的帐户 ID 时覆盖默认的帐户选择。
...
OpenClaw 会生成 `imsg rpc`（通过 stdio 传输的 JSON-RPC）。无需守护进程或端口。
...
- `cliPath` 可以指向 SSH 封装程序；设置 `remoteHost`（`host` 或 `user@host`）以进行 SCP 附件获取。
...
- 完整的 Teams 配置（凭据、webhook、私信/群组策略、每个团队/每个渠道的覆盖）记录在 [Microsoft Teams](/zh/channels/msteams) 中。
...
- 当 `channels.irc.defaultAccount` 与已配置的账户 ID 匹配时，可选的 `channels.irc.defaultAccount` 会覆盖默认的账户选择。
- 完整的 IRC 渠道配置（主机/端口/TLS/渠道/白名单/提及门控）记录在 [IRC](/zh/channels/irc) 中。
...
- 如果您仍处于单账户顶层渠道配置中，通过 `openclaw channels add`（或渠道新手引导）添加非默认账户，OpenClaw 会先将账户作用域的顶层单账户值移动到 `channels.<channel>.accounts.default` 中，以确保原账户继续工作。
- 现有的仅渠道绑定（无 `accountId`）保持匹配默认账户；账户作用域的绑定仍然是可选的。
- 当存在命名账户但缺少 `default` 时，`openclaw doctor --fix` 还会通过将账户作用域的顶层单账户值移动到 `accounts.default` 来修复混合形状。

### 其他扩展渠道

许多扩展渠道配置为 `channels.<id>` 并在其专门的渠道页面中记录（例如飞书、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
...
- 每渠道覆盖：`channels.discord.commands.native`（布尔值或 `"auto"`）。`false` 清除先前注册的命令。
...
- `config: true` 启用 `/config`（读取/写入 `openclaw.json`）。对于网关 `chat.send` 客户端，持久 `/config set|unset` 写入也需要 `operator.admin`；只读 `/config show` 仍对普通写入范围的操作员客户端可用。
- `channels.<provider>.configWrites` 对每个渠道的配置变更进行限制（默认：true）。
...
- `pdfModel`：接受字符串（`"provider/model"`）或对象（`{ primary, fallbacks }`）。
  - 由 `pdf` 工具用于模型路由。
  - 如果省略，PDF 工具将回退到 `imageModel`，然后回退到尽力而为的提供商默认值。
- `pdfMaxBytesMb`：调用时未传递 `maxBytesMb` 时，`pdf` 工具的默认 PDF 大小限制。
- `pdfMaxPages`：`pdf` 工具中提取回退模式考虑的默认最大页数。
- `model.primary`：格式 `provider/model`（例如 `anthropic/claude-opus-4-6`）。如果省略提供商，OpenClaw 假定使用 `anthropic`（已弃用）。
...
- 改变这些字段的配置编写器（例如 `/models set`、`/models set-image` 和回退添加/删除命令）保存规范对象形式，并尽可能保留现有的回退列表。
...
- `directPolicy`：直连/私信投递策略。`allow`（默认）允许直接目标投递。`block` 抑制直接目标投递并发出 `reason=dm-blocked`。
- `lightContext`：为 true 时，心跳运行使用轻量级引导上下文，并且仅保留工作区引导文件中的 `HEARTBEAT.md`。
- `isolatedSession`：为 true 时，每次心跳都在没有先前对话历史的新会话中运行。与 cron `sessionTarget: "isolated"` 的隔离模式相同。将每次心跳的 token 成本从约 10 万减少到约 2-5 千个 token。
...
- `postCompactionSections`：可选的 AGENTS.md H2/H3 节名称，用于在压缩后重新注入。默认为 `["Session Startup", "Red Lines"]`；设置为 `[]` 以禁用重新注入。当未设置或显式设置为该默认对时，较旧的 `Every Session`/`Safety` 标题也会作为旧版后备被接受。
- `model`：仅用于压缩摘要的可选 `provider/model-id` 覆盖。当主会话应保持一个模型但压缩摘要应在另一个模型上运行时，请使用此选项；当未设置时，压缩使用会话的主模型。
...
- 频道覆盖设置：`channels.<channel>.blockStreamingCoalesce`（以及每个账户的变体）。Signal/Slack/Discord/Google Chat 默认为 `minChars: 1500`。
...
当选择 `backend: "openshell"` 时，特定于运行时的设置将移至
...
- `workspaceRoot`：用于每个作用区工作空间的绝对远程根目录
- `identityFile` / `certificateFile` / `knownHostsFile`：传递给 OpenSSH 的现有本地文件
- `identityData` / `certificateData` / `knownHostsData`：OpenClaw 在运行时具体化到临时文件的内联内容或 SecretRef
...
- `none`：`~/.openclaw/sandboxes` 下的每个作用区沙箱工作空间
...
- `mirror`：执行前从本地播种到远程，执行后同步回本地；本地工作空间保持权威
- `remote`：创建沙箱时播种远程一次，然后保持远程工作空间为权威
...
`"host"` 被阻止。除非您明确设置
`sandbox.docker.dangerouslyAllowContainerNamespaceJoin: true`（紧急破窗），否则 `"container:<id>"` 默认被阻止。

**入站附件** 被暂存到活动工作空间中的 `media/inbound/*`。

**`docker.binds`** 挂载其他主机目录；全局和每个代理的绑定将被合并。

**沙箱隔离浏览器**（`sandbox.browser.enabled`）：容器中的 Chromium + CDP。noVNC URL 注入到系统提示中。不需要 `openclaw.json` 中的 `browser.enabled`。
noVNC 观察者访问默认使用 VNC 认证，OpenClaw 发出一个短期令牌 URL（而不是在共享 URL 中暴露密码）。

- `allowHostControl: false`（默认）阻止沙箱隔离会话定位主机浏览器。
- `network` 默认为 `openclaw-sandbox-browser`（专用桥接网络）。仅在您明确需要全局桥接连接时才设置为 `bridge`。
- `cdpSourceRange` 可选择将容器边缘的 CDP 入站限制为 CIDR 范围（例如 `172.21.0.1/32`）。
- `sandbox.browser.binds` 仅将其他主机目录挂载到沙箱浏览器容器中。当设置时（包括 `[]`），它会替换浏览器容器的 `docker.binds`。
- 启动默认值在 `scripts/sandbox-browser-entrypoint.sh` 中定义，并针对容器主机进行了调整：
  ...
  - `--disable-3d-apis`、`--disable-software-rasterizer` 和 `--disable-gpu`
    默认情况下已启用，如果 WebGL/3D 使用需要，可以使用
...
Browser 沙箱隔离和 `sandbox.docker.binds` 目前仅支持 Docker。
...
- `id`：稳定的 agent id（必需）。
- `default`：当设置多个时，第一个生效（记录警告）。如果未设置，列表中的第一个条目为默认值。
- `model`：字符串形式仅覆盖 `primary`；对象形式 `{ primary, fallbacks }` 覆盖两者（`[]` 禁用全局回退）。仅覆盖 `primary` 的定时任务仍会继承默认回退，除非您设置 `fallbacks: []`。
- `params`：合并到 `agents.defaults.models` 中所选模型条目的每个 Agent 流参数。将其用于特定 Agent 的覆盖，如 `cacheRetention`、`temperature` 或 `maxTokens`，而无需复制整个模型目录。
- `runtime`：可选的每个 Agent 运行时描述符。当 Agent 应默认为 ACP harness 会话时，请使用带有 `runtime.acp` 默认值（`agent`、`backend`、`mode`、`cwd`）的 `type: "acp"`。
- `identity.avatar`：相对于工作区的路径、`http(s)` URL 或 `data:` URI。
- `identity` 推导默认值：`ackReaction` 来自 `emoji`，`mentionPatterns` 来自 `name`/`emoji`。
- `subagents.allowAgents`：用于 `sessions_spawn` 的 agent id 允许列表（`["*"]` = 任意；默认值：仅限同一 agent）。
...
在一个 Gateway(网关) 内运行多个隔离的 Agent。请参阅 [Multi-Agent](/zh/concepts/multi-agent)。
...
- `type` （可选）：用于正常路由的 `route` （缺少类型时默认为路由），用于持久化 ACP 对话绑定的 `acp`。
- `match.channel` （必需）
...
对于 `type: "acp"` 条目，OpenClaw 按精确对话身份（`match.channel` + 账户 + `match.peer.id`）解析，并且不使用上述路由绑定层级顺序。
...
有关优先级的详细信息，请参阅 [多智能体沙箱与工具](/zh/tools/multi-agent-sandbox-tools)。
...
- **`dmScope`**: 私信的分组方式。
  - `main`: 所有私信共享主会话。
  - `per-peer`: 在各渠道之间按发送者 ID 隔离。
  - `per-channel-peer`: 按渠道 + 发送者隔离（推荐用于多用户收件箱）。
  - `per-account-channel-peer`: 按账户 + 渠道 + 发送者隔离（推荐用于多账户）。
- **`identityLinks`**: 将规范 ID 映射到带提供商前缀的对等端，以实现跨渠道会话共享。
- **`reset`**: 主要重置策略。`daily` 在 `atHour` 本地时间重置；`idle` 在 `idleMinutes` 后重置。如果同时配置了两者，则以先过期的为准。
- **`resetByType`**: 按类型的覆盖项（`direct`、`group`、`thread`）。旧版 `dm` 被接受为 `direct` 的别名。
...
- **`sendPolicy`**: 按 `channel`、`chatType`（`direct|group|channel`，附带旧版 `dm` 别名）、`keyPrefix` 或 `rawKeyPrefix` 匹配。优先应用拒绝规则。
- **`maintenance`**: 会话存储清理 + 保留控制。
  ...
  - `maxDiskBytes`: 可选的会话目录磁盘预算。在 `warn` 模式下记录警告；在 `enforce` 模式下先删除最旧的人工制品/会话。
...
- **`threadBindings`**: 线程绑定会话功能的全局默认值。
  ...
  - `idleHours`: 默认非活动自动取消聚焦时间（以小时为单位，`0` 表示禁用；提供商可以覆盖）
  - `maxAgeHours`: 默认硬性最大期限（以小时为单位，`0` 表示禁用；提供商可以覆盖）
...
解析顺序（最具体优先）：account → 渠道 → global。`""` 会禁用并停止级联。`"auto"` 派生自 `[{identity.name}]`。
...
- `openai.baseUrl` 会覆盖 OpenAI TTS 端点。解析顺序依次为配置，然后是 `OPENAI_TTS_BASE_URL`，最后是 `https://api.openai.com/v1`。
- 当 `openai.baseUrl` 指向非 OpenAI 端点时，OpenClaw 会将其视为 OpenAI 兼容的 TTS 服务器，并放宽模型/语音的验证。
...
- `ELEVENLABS_API_KEY` 回退仅在未配置对话 API 密钥时应用。
- `voiceAliases` 允许对话指令使用友好名称。
- `silenceTimeoutMs` 控制对话模式在用户停止说话后发送转录文本之前等待的时间。未设置则保持平台默认的暂停窗口 (`700 ms on macOS and Android, 900 ms on iOS`)。
...
本地新手引导默认将新的本地配置设置为 `tools.profile: "coding"`（如果未设置）（现有的显式配置文件将被保留）。
...
全局工具允许/拒绝策略（拒绝优先）。不区分大小写，支持 `*` 通配符。即使关闭了 Docker 沙箱也会应用。
...
- `warningThreshold`：重复无进度模式警告的阈值。
- `criticalThreshold`：用于阻止严重循环的更高重复阈值。
- `globalCircuitBreakerThreshold`：任何无进度运行的硬性停止阈值。
...
- `detectors.pingPong`：对交替的无进度对模式发出警告/进行阻止。
...
- `agent`: 属于当前代理 ID 的任何会话（如果您在同一代理 ID 下运行每个发送方的会话，则可以包括其他用户）。
...
- 文件将在 `.openclaw/attachments/<uuid>/` 处以 `.manifest.json` 的形式具象化到子工作区中。
- 附件内容会自动从脚本持久化中编辑掉。
...
- 匹配提供商 ID 的合并优先级：
  ...
  - 非空的代理 `apiKey` 值仅在该提供商在当前配置/身份验证配置文件上下文中未由 SecretRef 管理时优先。
  - 由 SecretRef 管理的提供商 `apiKey` 值会从源标记（环境引用为 `ENV_VAR_NAME`，文件/exec 引用为 `secretref-managed`）刷新，而不是持久化已解析的密钥。
  - 由 SecretRef 管理的提供商标头值会从源标记（环境引用为 `secretref-env:ENV_VAR_NAME`，文件/exec 引用为 `secretref-managed`）刷新。
  - 空的或缺失的代理 `apiKey`/`baseUrl` 会回退到配置中的 `models.providers`。
  - 匹配的 `contextWindow`/`maxTokens` 使用显式配置和隐式目录值中较高的那个值。
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`：可选的兼容性提示。对于具有非空非本机 `baseUrl`（主机不是 `api.openai.com`）的 `api: "openai-completions"`，OpenClaw 会在运行时强制将其设置为 `false`。为空或省略的 `baseUrl` 将保持默认的 OpenAI 行为。
...
请参阅 [Local Models](/zh/gateway/local-models)。TL;DR：在强力硬件上通过 LM Studio Responses MiniMax 运行 API M2.5；保持托管模型合并以用于回退。
...
- `entries.<skillKey>.enabled: false` 会禁用一个 Skill，即使它是捆绑/已安装的。
...
- `plugins.entries.<id>.hooks.allowPromptInjection`：当 `false` 时，核心会阻止 `before_prompt_build` 并忽略来自旧版 `before_agent_start` 的提示更改字段，同时保留旧版 `modelOverride` 和 `providerOverride`。适用于原生插件钩子和支持的捆绑包提供的钩子目录。
- `plugins.entries.<id>.config`：插件定义的配置对象（当可用时，由原生 OpenClaw 插件架构验证）。
- 已启用的 Claude 捆绑包插件也可以从 `settings.json` 贡献嵌入式 Pi 默认值；OpenClaw 会将这些值应用为经过清理的代理设置，而不是原始的 OpenClaw 配置补丁。
...
- 设置 `ssrfPolicy.dangerouslyAllowPrivateNetwork: false` 以实施严格的仅公开浏览器导航。
- 在严格模式下，远程 CDP 配置文件端点（`profiles.*.cdpUrl`）在可达性/发现检查期间同样受到私有网络阻止。
...
- `mode`：`local`（运行 gateway）或 `remote`（连接到远程 gateway）。除非 `local`，否则 Gateway(网关) 将拒绝启动。
...
- **Docker 注意**：默认的 `loopback` 绑定监听容器内部的 `127.0.0.1`。使用 Docker 网桥网络（`-p 18789:18789`）时，流量到达 `eth0`，因此无法访问 gateway。请使用 `--network host`，或设置 `bind: "lan"`（或结合 `customBindHost: "0.0.0.0"` 使用 `bind: "custom"`）以监听所有接口。
- **Auth**：默认需要。非回环绑定需要共享令牌/密码。新手引导向导默认生成令牌。
- 如果配置了 `gateway.auth.token` 和 `gateway.auth.password`（包括 SecretRefs），请显式将 `gateway.auth.mode` 设置为 `token` 或 `password`。当两者都已配置但未设置模式时，启动和服务安装/修复流程将失败。
- `gateway.auth.mode: "none"`：显式的无认证模式。仅用于受信任的本地回环设置；新手引导提示故意不提供此选项。
- `gateway.auth.mode: "trusted-proxy"`：将认证委托给感知身份的反向代理，并信任来自 `gateway.trustedProxies` 的身份标头（参见[可信代理认证](/zh/gateway/trusted-proxy-auth)）。
- `gateway.auth.allowTailscale`：当 `true` 时，Tailscale Serve 身份标头可以满足控制 UI/WebSocket 认证（通过 `tailscale whois` 验证）；HTTP API 端点仍需要令牌/密码认证。此无令牌流程假设 gateway 主机是受信任的。当 `tailscale.mode = "serve"` 时，默认为 `true`。
- `gateway.auth.rateLimit`：可选的认证失败限制器。应用于每个客户端 IP 和每个认证范围（共享密钥和设备令牌独立跟踪）。受阻尝试返回 `429` + `Retry-After`。
  - `gateway.auth.rateLimit.exemptLoopback` 默认为 `true`；如果您有意对本地主机流量也进行速率限制（用于测试设置或严格的代理部署），请设置 `false`。
- 浏览器源 WS 认证尝试始终受到限制，并禁用回环豁免（防止基于浏览器的本地主机暴力破解的纵深防御）。
- `tailscale.mode`：`serve`（仅 tailnet，回环绑定）或 `funnel`（公开，需要认证）。
- `controlUi.allowedOrigins`：Gateway(网关) WebSocket 连接的显式浏览器源允许列表。当预期来自非回环源的浏览器客户端时需要。
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`：一种危险模式，对于有意依赖 Host 标头源策略的部署，启用 Host 标头源回退。
- `remote.transport`：`ssh`（默认）或 `direct`（ws/wss）。对于 `direct`，`remote.url` 必须是 `ws://` 或 `wss://`。
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`：客户端紧急覆盖，允许对受信任的专用网络 IP 使用纯文本 `ws://`；纯文本的默认设置保持仅回环。
- `gateway.remote.token` / `.password` 是远程客户端凭据字段。它们本身不配置 gateway 认证。
- `gateway.push.apns.relay.baseUrl`：官方/TestFlight iOS 版本向 gateway 发布基于中继的注册后，使用的外部 APNs 中继的基本 HTTPS URL。此 URL 必须与编译到 iOS 版本中的中继 URL 匹配。
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`：仅用于回环 HTTP 中继 URL 的开发应急手段。生产中继 URL 应保持 HTTPS。
...
- `channels.<provider>.healthMonitor.enabled`：每个渠道退出健康监控重启，同时保持全局监控启用。
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`：多账户渠道的每个账户覆盖。设置后，它的优先级高于渠道级别覆盖。
...
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - 仅当 `hooks.allowRequestSessionKey=true` 时（默认：`false`），才会接受来自请求负载的 `sessionKey`。
...
- `transform` 可以指向返回 hook 操作的 JS/TS 模块。
  - `transform.module` 必须是相对路径，并且必须位于 `hooks.transformsDir` 内部（绝对路径和遍历路径将被拒绝）。
...
- 非环回绑定：画布路由需要 Gateway(网关) 身份验证（令牌/密码/受信任代理），与其他 Gateway(网关) HTTP 表面相同。
- 节点 WebView 通常不发送身份验证标头；节点配对并连接后，Gateway(网关) 会通告用于画布/A2UI 访问的节点作用域功能 URL。
...
在 `~/.openclaw/dns/` 下写入单播 DNS-SD 区域。对于跨网络发现，请搭配 DNS 服务器（推荐 CoreDNS）+ Tailscale 分割 DNS 使用。
...
- `exec` 提供商 需要一个绝对的 `command` 路径，并在 stdin/stdout 上使用协议负载。
...
- 激活期间应用活动表面过滤：启用的表面上未解析的引用会导致启动/重新加载失败，而非活动表面则被跳过并生成诊断信息。
...
- 静态运行时凭证来自内存中解析的快照；发现旧的静态 `auth.json` 条目时会被清除。
...
- 设置 `logging.file` 以指定稳定路径。
- 当 `--verbose` 时，`consoleLevel` 会递增到 `debug`。
...
当前构建版本不再包含 TCP 网桥。节点通过 Gateway(网关) WebSocket 连接。`bridge.*` 键不再属于配置架构的一部分（验证会失败，直到将其删除；`openclaw doctor --fix` 可以去除未知键）。

<Accordion title="传统桥接配置（历史参考）">
...
- `sessionRetention`：在从 `sessions.json` 中清理之前，保留已完成的独立 cron 运行会话的时间。还控制已归档的已删除 cron 副本的清理。默认值：`24h`；设置为 `false` 以禁用。
- `runLog.maxBytes`：清理前每个运行日志文件的最大大小 (`cron/runs/<jobId>.jsonl`)。默认值：`2_000_000` 字节。
- `runLog.keepLines`：触发运行日志清理时保留的最新行数。默认值：`2000`。
- `webhookToken`：用于 cron webhook POST 传递 (`delivery.mode = "webhook"`) 的持有者令牌，如果省略，则不发送身份验证标头。
...
- 单个文件：替换包含对象。
- 文件数组：按顺序深度合并（后一个覆盖前一个）。
- 兄弟键：在包含之后合并（覆盖包含的值）。
- 嵌套包含：最多支持 10 层深度。
- 路径：相对于包含文件解析，但必须保持在顶层配置目录（`dirname` of `openclaw.json`）内。仅当它们仍然解析在该边界内时，才允许使用绝对/`../` 形式。
