...
有关完整优先级和来源，请参阅 [/environment](/en/environment)。
...
另请参阅：[/concepts/oauth](/en/concepts/oauth)
...
有关优先级和其他示例，请参阅[多代理沙箱和工具](/en/multi-agent-sandbox-tools)。
...
- `/reasoning stream` 将推理流式传输到草稿中，然后发送最终答案。
  重试策略默认值和行为记录在[重试策略](/en/concepts/retry) 中。
...
有关行为详情，请参阅[/concepts/session-pruning](/en/concepts/session-pruning)。

#### `agents.defaults.compaction`（预留空间 + 内存刷新）

`agents.defaults.compaction.mode` 选择压缩摘要策略。默认为 `default`；设置 `safeguard` 以启用分块摘要来处理非常长的历史。请参阅[/concepts/compaction](/en/concepts/compaction)。
...
- 工具策略：仅允许 `exec`、`process`、`read`、`write`、`edit`、`apply_patch`、`sessions_list`、`sessions_history`、`sessions_send`、`sessions_spawn`、`session_status`（拒绝获胜）
  ...
  - 沙箱策略中支持工具组简写：`group:runtime`、`group:fs`、`group:sessions`、`group:memory`（参见[沙箱 vs 工具策略 vs 提升](/en/gateway/sandbox-vs-tool-policy-vs-elevated#tool-groups-shorthands)）
...
提供商概览 + 示例：[/concepts/model-providers](/en/concepts/model-providers)。
...
参见 [/gateway/local-models](/en/gateway/local-models) 了解当前的本地指南。简而言之：在强大的硬件上通过 LM Studio Responses API 运行 MiniMax M2.1；保持托管模型合并以进行回退。
...
参见 [Gateway手册](/en/gateway) 了解派生端口映射（Gateway/浏览器/canvas）。
参见 [多Gateway](/en/gateway/multiple-gateways) 了解浏览器/CDP 端口隔离详情。
...
Cron 是Gateway拥有的调度程序，用于唤醒和计划作业。有关功能概述和 CLI 示例，请参见 [Cron 作业](/en/automation/cron-jobs)。
...
_下一篇：[代理运行时](/en/concepts/agent)_ 🦞
