...
查看[完整参考](/en/gateway/configuration-reference)以了解所有可用字段。

<Tip>
  **刚接触配置？** 从 `openclaw onboard` 开始进行交互式设置，或查看
  [配置示例](/en/gateway/configuration-examples) 指南以获取完整的复制粘贴
  配置。
</Tip>
...
<Tabs>
  ...
  <Tab title="Control UI">
    ...
    会根据配置架构渲染一个表单，并提供 **Raw JSON** 编辑器作为应急手段。
  </Tab>
  <Tab title="Direct edit">
    直接编辑 `~/.openclaw/openclaw.json`。Gateway(网关) 会监视该文件并自动应用更改
    （参见 [热重载](#config-hot-reload)）。
  </Tab>
</Tabs>
...
<Warning>
  OpenClaw 仅接受完全匹配架构的配置。未知的键、格式错误的类型
  或无效的值会导致 Gateway(网关) **拒绝启动**。唯一的一级例外是
  `$schema` （字符串），以便编辑器可以附加 JSON Schema 元数据。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="控制谁可以向机器人发送消息">
    ...
    请参阅 [完整参考资料](/en/gateway/configuration-reference#dm-and-group-access) 了解按渠道的详细信息。
  ...
  <Accordion title="设置群聊提及门控">
    ...
    - 有关按渠道覆盖和自聊模式，请参阅 [完整参考](/en/gateway/configuration-reference#group-chat-mention-gating)。
  ...
  <Accordion title="配置会话和重置">
    ...
    - 参阅 [会话管理](/en/concepts/session) 了解作用域、身份链接和发送策略。
  ...
  <Accordion title="启用沙箱隔离">
    ...
    请参阅 [沙箱隔离](/en/gateway/sandboxing) 以获取完整指南，并参阅 [完整参考](/en/gateway/configuration-reference#sandbox) 以了解所有选项。
  ...
  <Accordion title="设置心跳（定期检查）">
    ...
    - 参见 [Heartbeat](/en/gateway/heartbeat) 获取完整指南。
  ...
  <Accordion title="配置定时任务">
    ...
    - 参见 [Cron jobs](/en/automation/cron-jobs) 获取功能概述和 CLI 示例。
  ...
  <Accordion title="配置多代理路由">
    ...
    有关绑定规则和每个代理访问配置文件，请参阅 [Multi-Agent](/en/concepts/multi-agent) 和 [完整参考](/en/gateway/configuration-reference#multi-agent-routing)。
...
<Note>
  `gateway.reload` 和 `gateway.remote` 是例外情况——更改它们**不会**触发
  重启。
</Note>
...
<Note>
  控制平面写入 RPC （`config.apply`、`config.patch`、`update.run`）的速率限制为每个 `deviceId+clientIp` 每 60 秒 **3
  次请求**。受限时，RPC 将返回 `UNAVAILABLE`
  并带有 `retryAfterMs`。
</Note>
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    当一个重启请求已经处于待处理或正在执行时，后续的重启请求会被合并，且在重启周期之间应用 30 秒的冷却时间。
...
SecretRef 的详细信息（包括用于 `env`/`file`/`exec` 的 `secrets.providers`）位于 [Secrets Management](/en/gateway/secrets) 中。
