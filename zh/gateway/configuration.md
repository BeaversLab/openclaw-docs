...
<Tip>
**初次接触配置？** 从 `openclaw onboard` 开始进行交互式设置，或者查看 [配置示例](/zh/en/gateway/configuration-examples) 指南以获取完整的可复制粘贴配置。
</Tip>
...
<Tabs> <Tab title="交互式向导"> ```bash
    openclaw onboard       # full setup wizard
    openclaw configure     # config wizard
    ``` </Tab> <Tab title="CLI (单行命令)"> ```bash
    openclaw config get agents.defaults.workspace
    openclaw config set agents.defaults.heartbeat.every "2h"
    openclaw config unset tools.web.search.apiKey
    ``` </Tab> <Tab title="控制 UI"> 打开 [http://127.0.0.1:18789](http://127.0.0.1:18789) 并使用 **Config** 选项卡。 控制 UI 根据配置架构渲染表单，并提供 **Raw JSON** 编辑器作为备用选项。 </Tab> <Tab title="直接编辑"> 直接编辑 `~/.openclaw/openclaw.json`。Gateway 网关 会监视该文件并自动应用更改matically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
OpenClaw 仅接受完全匹配架构的配置。未知键、格式错误的类型或无效值会导致 Gateway 网关**拒绝启动**。唯一的根级例外是 `$schema`（字符串），以便编辑器可以附加 JSON 架构元数据。
</Warning>
...
<AccordionGroup>
  <Accordion title="设置渠道（WhatsApp、Telegram、Discord 等）">
    每个渠道在 `channels.<provider>` 下都有自己的配置部分。有关设置步骤，请参阅专用渠道页面：
  ...
  <Accordion title="选择并配置模型">
    ...
    - `agents.defaults.imageMaxDimensionPx` 控制转录/工具图像的下采样（默认 `1200`）；较低的值通常会在大量截图的运行中减少视觉令牌的使用。
    - 有关在聊天中切换模型，请参阅 [Models CLI](/zh/en/concepts/models)；有关认证轮换和后备行为，请参阅 [Model Failover](/zh/en/concepts/模型-failover)。
    - 对于自定义/自托管提供商，请参阅参考中的 [Custom providers](/zh/en/gateway/configuration-reference#custom-providers-and-base-urls)。
  ...
  <Accordion title="控制谁可以向机器人发送消息">
    ...
    - `"pairing"` (默认): 未知发送者将获得一次性配对代码以供批准
    - `"allowlist"`: 仅允许 `allowFrom` (或配对的允许存储) 中的发送者
  ...
  <Accordion title="设置群聊提及控制">
    群消息默认**需要提及**。为每个代理配置模式：
    ...
    - 有关每频道覆盖和自聊模式，请参阅[完整参考](/zh/en/gateway/configuration-reference#group-chat-mention-gating)。
  ...
  <Accordion title="配置会话和重置">
    ...
    - 有关作用域、身份链接和发送策略，请参阅[会话管理](/zh/en/concepts/会话)。
    - 有关所有字段，请参阅[完整参考](/zh/en/gateway/configuration-reference#会话)。
  ...
  <Accordion title="启用沙箱">
    ...
    有关完整指南，请参阅[沙箱](/zh/en/gateway/沙箱隔离)；有关所有选项，请参阅[完整参考](/zh/en/gateway/configuration-reference#sandbox)。
  ...
  <Accordion title="为官方 iOS 版本启用基于中继的推送">
    ...
    - 保持本地/手动 iOS 版本使用直接 APNs。基于中继的发送仅适用于通过中继注册的官方分发版本。
  ...
  <Accordion title="设置心跳（定期检查）">
    ...
    - `directPolicy`: `allow` (默认) 或用于 私信 风格心跳目标的 `block`
    - 完整指南请参阅 [Heartbeat](/zh/en/gateway/heartbeat)。
  ...
  <Accordion title="配置定时任务">
    ...
    - `sessionRetention`: 从 `sessions.json` 中清理已完成的隔离运行会话 (默认为 `24h`; 设置为 `false` 以禁用)。
    - `runLog`: 根据大小和保留行数清理 `cron/runs/<jobId>.jsonl`。
    - 功能概述和 CLI 示例请参阅 [Cron jobs](/zh/en/automation/cron-jobs)。

  </Accordion>

<Accordion title="设置 Webhook (hooks)"> 在 Gateway 网关 上启用 HTTP webhook 端点：
    ...
    - 对于由 hook 驱动的代理，建议使用强大的现代模型层级和严格的工具策略 (例如，仅限消息传递并在可能的情况下使用沙箱)。
...
Gateway 网关 监视 `~/.openclaw/openclaw.json` 并自动应用更改 — 大多数设置无需手动重启。
...
大多数字段在不中断服务的情况下热应用。在 `hybrid` 模式下，需要重启的更改会自动处理。
...
<Note>
控制平面写入 RPC（`config.apply`、`config.patch`、`update.run`）的速率限制为每个 `deviceId+clientIp` **每 60 秒 3 次请求**。受限时，RPC 返回 `UNAVAILABLE` 并带有 `retryAfterMs`。
</Note>

<AccordionGroup> <Accordion title="config.apply (full replace)"> 验证并写入完整配置，然后一步重启 Gateway 网关。
    ...
    - `baseHash` (可选) — 来自 `config.get` 的配置哈希（配置存在时必须）
  ...
  <Accordion title="config.patch (partial update)">
    ...
    - `baseHash` (必须) — 来自 `config.get` 的配置哈希
    - `sessionKey`、`note`、`restartDelayMs` — 与 `config.apply` 相同

    重启行为与 `config.apply` 匹配：合并待处理的重启，并且重启周期之间有 30 秒的冷却时间。
...
SecretRef 详细信息（包括用于 `env`/`file`/`exec` 的 `secrets.providers`）位于 [Secrets Management](/zh/en/gateway/secrets) 中。
支持的凭证路径列于 [SecretRef Credential Surface](/zh/en/reference/secretref-credential-surface)。
</Accordion>
