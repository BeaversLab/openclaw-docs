...
请参阅[完整参考](/zh/gateway/configuration-reference)以查看所有可用字段。

<Tip>
**配置新手？** 从 `openclaw onboard` 开始进行交互式设置，或者查看 [配置示例](/zh/gateway/configuration-examples) 指南以获取完整的复制粘贴配置。
</Tip>
...
<Tabs>
  ...
  <Tab title="控制 UI">
    ...
    控制 UI 会根据配置架构呈现一个表单，并提供 **Raw JSON** 编辑器作为备用手段。
  </Tab>
  <Tab title="直接编辑">
    直接编辑 `~/.openclaw/openclaw.json`。Gateway(网关) 会监视文件并自动应用更改（参见 [热重载](#config-hot-reload)）。
  </Tab>
</Tabs>
...
<Warning>
OpenClaw 仅接受完全匹配架构的配置。未知的键、格式错误的类型或无效的值会导致 Gateway(网关) **拒绝启动**。唯一的一级例外是 `$schema`（字符串），以便编辑器可以附加 JSON Schema 元数据。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="选择并配置模型">
    ...
    - `agents.defaults.models` 定义了模型目录，并充当 `/model` 的允许列表。
    - 模型引用使用 `provider/model` 格式（例如 `anthropic/claude-opus-4-6`）。
    - `agents.defaults.imageMaxDimensionPx` 控制脚本/工具图片的缩小比例（默认为 `1200`）；较低的值通常会在包含大量截图的运行中减少视觉令牌的使用量。
    - 请参阅 [模型 CLI](/zh/concepts/models) 以在聊天中切换模型，并参阅 [模型故障转移](/zh/concepts/model-failover) 以了解身份验证轮换和故障转移行为。
  ...
  <Accordion title="控制谁可以向机器人发送消息">
    私信访问权限是通过 `dmPolicy` 针对每个渠道进行控制的：

    - `"pairing"`（默认）：未知发送者会收到一次性配对代码以进行审批
  ...
  <Accordion title="设置群聊提及门控">
    ...
    - 有关每个渠道的覆盖项和自聊天模式，请参阅 [完整参考](/zh/gateway/configuration-reference#group-chat-mention-gating)。
  ...
  <Accordion title="调整网关渠道健康监控">
    ...
    - 使用 `channels.<provider>.healthMonitor.enabled` 或 `channels.<provider>.accounts.<id>.healthMonitor.enabled` 来禁用单个渠道或账户的自动重启，而不禁用全局监控器。
  ...
  <Accordion title="Configure sessions and resets">
    ...
    - 请参阅[会话管理](/zh/concepts/session)以了解作用域、身份链接和发送策略。
    - 请参阅[完整参考](/zh/gateway/configuration-reference#session)以了解所有字段。
  ...
  <Accordion title="Enable 沙箱隔离">
    ...
    请参阅[沙箱隔离](/zh/gateway/sandboxing)了解完整指南，并参阅[完整参考](/zh/gateway/configuration-reference#sandbox)了解所有选项。
  ...
  <Accordion title="为官方 iOS 构建启用基于中继的推送">
    ...
    - 将本地/手动 iOS 构建保留在直接 APNs 上。基于中继的发送仅适用于通过中继注册的正式分发包。
    ...
    - `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true` 仍然是仅限回环的开发逃生舱；不要在配置中持久化 HTTP 中继 URL。
  ...
  <Accordion title="配置定时任务">
    ...
    - `sessionRetention`：从 `sessions.json` 中清除已完成的隔离运行会话（默认 `24h`；设置为 `false` 以禁用）。
  ...
  <Accordion title="设置 Webhooks (hooks)">
    ...
    - 保持不安全内容绕过标志处于禁用状态（`hooks.gmail.allowUnsafeExternalContent`、`hooks.mappings[].allowUnsafeExternalContent`），除非进行范围受限的调试。
    - 对于由 hook 驱动的代理，建议使用强大的现代模型层级和严格的工具策略（例如，仅限消息传递并在可能的情况下进行沙箱隔离）。

    有关所有映射选项和 Gmail 集成，请参阅 [full reference](/zh/gateway/configuration-reference#hooks)。
  ...
  <Accordion title="配置多代理路由">
    ...
    参阅 [Multi-Agent](/zh/concepts/multi-agent) 和 [完整参考](/zh/gateway/configuration-reference#multi-agent-routing) 了解绑定规则和每个代理的访问配置文件。
  ...
  <Accordion title="将配置拆分为多个文件 ($include)">
    ...
    - **嵌套包含**：最多支持 10 层深度
    - **相对路径**：相对于包含文件解析
    - **错误处理**：针对文件丢失、解析错误和循环包含提供清晰的错误提示
...
| **`hot`**              | 仅热应用安全更改。当需要重启时记录警告 — 由您进行处理。 |
| **`restart`**          | 在任何配置更改时重启 Gateway(网关)，无论更改是否安全。                                 |
...
大多数字段都可以无停机地热应用。在 `hybrid` 模式下，需要重启的更改会自动处理。
...
<Note>
控制平面写入 RPC (`config.apply`, `config.patch`, `update.run`) 对每个 `deviceId+clientIp` 限速为 **每 60 秒 3 个请求**。受限时，RPC 返回 `UNAVAILABLE` 并带有 `retryAfterMs`。
</Note>
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    当一个重启请求已在处理/进行中时，重启请求会被合并，并且在重启周期之间应用 30 秒的冷却时间。
...
SecretRef 的详细信息（包括 `env`/`file`/`exec` 的 `secrets.providers`）请参阅 [Secrets Management](/zh/gateway/secrets)。
支持的凭据路径列于 [SecretRef Credential Surface](/zh/reference/secretref-credential-surface)。
</Accordion>
...
有关逐字段的完整参考，请参阅 **[Configuration Reference](/zh/gateway/configuration-reference)**。
