...
<Tip>
  **刚接触配置？** 从 `openclaw onboard` 开始进行交互式设置，或查看
  [配置示例](/zh/gateway/configuration-examples) 指南以获取完整的复制粘贴 配置。
</Tip>
...
<Tabs>
  ...
  <Tab title="Direct edit">
    直接编辑 `~/.openclaw/openclaw.json`。Gateway(网关) 会监视该文件并自动应用更改 （参见
...
<Warning>
  OpenClaw 仅接受完全匹配架构的配置。未知的键、格式错误的类型 或无效的值会导致 Gateway(网关)
  **拒绝启动**。唯一的一级例外是 `$schema` （字符串），以便编辑器可以附加 JSON Schema 元数据。
</Warning>
...
<Note>`gateway.reload` 和 `gateway.remote` 是例外情况——更改它们**不会**触发 重启。</Note>

## 配置 RPC（编程更新）

<Note>
  控制平面写入 RPC （`config.apply`、`config.patch`、`update.run`）的速率限制为每个
  `deviceId+clientIp` 每 60 秒 **3 次请求**。受限时，RPC 将返回 `UNAVAILABLE` 并带有
  `retryAfterMs`。
</Note>
...
SecretRef 的详细信息（包括用于 `env`/`file`/`exec` 的 `secrets.providers`）位于 [Secrets Management](/zh/gateway/secrets) 中。
