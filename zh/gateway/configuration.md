...
<Tip>
**初次接触配置？** 从 `openclaw onboard` 开始进行交互式设置，或者查看 [配置示例](/en/gateway/configuration-examples) 指南以获取完整的可复制粘贴配置。
</Tip>
...
<Tabs>
  ...
  <Tab title="控制 UI">
    ...
    控制 UI 根据配置架构渲染表单，并提供 **Raw JSON** 编辑器作为备用选项。
  </Tab>
  <Tab title="直接编辑">
    直接编辑 `~/.openclaw/openclaw.json`。网关会监视该文件并自动应用更改matically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
OpenClaw 仅接受完全匹配架构的配置。未知键、格式错误的类型或无效值会导致网关**拒绝启动**。唯一的根级例外是 `$schema`（字符串），以便编辑器可以附加 JSON 架构元数据。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="选择并配置模型">
    ...
    - 有关在聊天中切换模型，请参阅 [Models CLI](/en/concepts/models)；有关认证轮换和后备行为，请参阅 [Model Failover](/en/concepts/model-failover)。
  ...
  <Accordion title="控制谁可以向机器人发送消息">
    ...
    有关每个频道的详细信息，请参阅[完整参考](/en/gateway/configuration-reference#dm-and-group-access)。
  ...
  <Accordion title="设置群聊提及控制">
    ...
    - 有关每频道覆盖和自聊模式，请参阅[完整参考](/en/gateway/configuration-reference#group-chat-mention-gating)。
  ...
  <Accordion title="配置会话和重置">
    ...
    - 有关作用域、身份链接和发送策略，请参阅[会话管理](/en/concepts/session)。
    - 有关所有字段，请参阅[完整参考](/en/gateway/configuration-reference#session)。
  ...
  <Accordion title="启用沙箱">
    ...
    有关完整指南，请参阅[沙箱](/en/gateway/sandboxing)；有关所有选项，请参阅[完整参考](/en/gateway/configuration-reference#sandbox)。
  ...
  <Accordion title="设置心跳（定期检查）">
    ...
    - `directPolicy`: `allow` (默认) 或用于 DM 风格心跳目标的 `block`
    - 完整指南请参阅 [Heartbeat](/en/gateway/heartbeat)。
  ...
  <Accordion title="配置定时任务">
    ...
    - 功能概述和 CLI 示例请参阅 [Cron jobs](/en/automation/cron-jobs)。
  ...
  <Accordion title="配置多代理路由">
    ...
    有关绑定规则和每代理访问配置文件，请参阅[多代理](/en/concepts/multi-agent)和[完整参考](/en/gateway/configuration-reference#multi-agent-routing)。
...
SecretRef 详细信息（包括用于 `env`/`file`/`exec` 的 `secrets.providers`）位于 [Secrets Management](/en/gateway/secrets) 中。
