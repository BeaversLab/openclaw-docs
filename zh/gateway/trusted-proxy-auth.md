...
read_when:
  - 在身份感知代理后运行 OpenClaw
  - 在 OpenClaw 前端设置 Pomerium、Caddy 或 nginx 并使用 OAuth
  - 修复反向代理设置中的 WebSocket 1008 未授权错误
  - 决定设置 HSTS 和其他 HTTP 加固标头的位置
---

# 受信任的代理身份验证

> ⚠️ **安全敏感功能。** 此模式将身份验证完全委托给您的反向代理。配置错误可能会将 Gateway 暴露给未授权的访问。启用前请仔细阅读此页面。
...
trusted-proxy 检查时，控制 UI WebSocket 会话可以在没有设备
配对身份的情况下连接。

影响：

- 在此模式下，配对不再是控制 UI 访问的主要关口。
- 您的反向代理身份验证策略和 `allowUsers` 成为有效的访问控制。
- 将 Gateway(网关) 入站流量仅锁定到受信任的代理 IP（`gateway.trustedProxies` + 防火墙）。
...
如果 `gateway.bind` 为 `loopback`，请在 `gateway.trustedProxies` 中包含环回代理地址（`127.0.0.1`、`::1` 或等效的环回 CIDR）。
...
| `gateway.auth.trustedProxy.userHeader`      | 是   | 包含经过身份验证的用户身份的 Header 名称               |
| `gateway.auth.trustedProxy.requiredHeaders` | 否   | 请求受信任时必须存在的额外 Header                      |
| `gateway.auth.trustedProxy.allowUsers`      | 否   | 用户身份允许列表。空表示允许所有经过身份验证的用户。   |
...
- 适合面向互联网的部署。
- 将证书和 HTTP 加固策略保持在同一位置。
- OpenClaw 可以保留在代理后方的环回 HTTP 上。
...
- 只有在信心充足后，才增加到长期有效的值（例如 `max-age=31536000`）。
- 仅当每个子域都准备好 HTTPS 时，才添加 `includeSubDomains`。
- 仅当您有意为整个域集满足预加载要求时，才使用预加载。
...
- [ ] **代理是唯一路径**：Gateway 端口已通过防火墙隔离，仅允许您的代理访问
...
`openclaw security audit` 将标记受信任代理身份验证，并给出一个**严重**级别的发现。这是有意为之——它是提醒您将安全委托给了代理设置。
...
如果您正在从令牌身份验证迁移到 trusted-proxy：

1. 配置您的代理以对用户进行身份验证并传递标头
2. 独立测试代理设置（使用带有标头的 curl）
3. 使用 trusted-proxy 身份验证更新 OpenClaw 配置
4. 重启 Gateway(网关)
...
- [安全](/zh/gateway/security) — 完整的安全指南
- [配置](/zh/gateway/configuration) — 配置参考
- [远程访问](/zh/gateway/remote) — 其他远程访问模式
- [Tailscale](/zh/gateway/tailscale) — 仅用于 tailnet 访问的更简单替代方案
