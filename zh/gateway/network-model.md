...
- 建议每个主机运行一个 Gateway 网关。它是唯一允许拥有 WhatsApp Web 会话的进程。对于救援机器人或严格隔离，请使用隔离的配置文件和端口运行多个网关。请参阅[多个网关](/en/gateway/multiple-gateways)。
...
- Canvas Canvas主机由 Gateway 网关 HTTP 服务器在与 Gateway 网关 **相同的端口**（默认为 `18789`）上提供服务：
  - `/__openclaw__/canvas/`
  - `/__openclaw__/a2ui/`
    当配置了 `gateway.auth` 且 Gateway(网关) 绑定到 loopback 之外时，这些路由由 Gateway(网关) 认证保护。节点客户端使用绑定到其活动 WS 会话的节点范围功能 URL。请参阅 [Gateway(网关) 配置](/en/gateway/configuration) (`canvasHost`, `gateway`)。
