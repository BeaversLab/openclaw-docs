...
- 建议每个主机运行一个 Gateway 网关。它是唯一允许拥有 WhatsApp Web 会话的进程。对于救援机器人或严格隔离，请运行多个具有独立配置文件和端口的 Gateway 网关。请参阅[多 Gateway 网关](/zh/en/gateway/multiple-gateways)。
...
- Canvas Canvas主机由 Gateway 网关 HTTP 服务器在与 Gateway 网关 **相同的端口**（默认为 `18789`）上提供服务：
 - `/__openclaw__/canvas/` - `/__openclaw__/a2ui/` 当配置了 `gateway.auth` 且 Gateway 网关 绑定到回环地址之外时，这些路由受 Gateway 网关 身份验证保护。节点客户端使用与其活动 WS 会话关联的节点范围的 capability URL。请参阅 [Gateway 网关 configuration](/zh/en/gateway/configuration) (`canvasHost`, `gateway`)。
