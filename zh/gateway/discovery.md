...
- **旧版 TCP bridge（已弃用/移除）**：旧 node 传输（见 [Bridge protocol](/zh/gateway/bridge-protocol)）；不再用于发现。

协议详情：
- [Gateway protocol](/zh/gateway/protocol)
- [Bridge protocol（旧）](/zh/gateway/bridge-protocol)
...
参见 [Remote access](/zh/gateway/remote)。
