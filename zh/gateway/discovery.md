---
summary: "节点发现和传输方式（Bonjour、Tailscale、SSH）用于查找网关"
read_when:
...
2. **节点配对**：iOS/Android（及未来的节点）查找网关并安全配对。

设计目标是将所有网络发现/广播保留在**节点网关**（`openclaw gateway`）中，并让客户端（mac 应用、iOS）作为使用者。

## 术语

- **网关**：拥有状态（会话、配对、节点注册表）并运行频道的单一长期运行的网关进程。大多数设置每个主机使用一个；可以进行隔离的多网关设置。
- **网关 WS（控制平面）**：默认位于 `127.0.0.1:18789` 上的 WebSocket 端点；可以通过 `gateway.bind` 绑定到局域网/尾网。
- **直接 WS 传输**：面向局域网/尾网的网关 WS 端点（无 SSH）。
...
如果网关可以检测到它在 Tailscale 下运行，它会将 `tailnetDns` 作为可选提示发布给客户端（包括广域网信标）。

### 3) 手动/SSH 目标

当没有直接路由（或直接路由被禁用）时，客户端可以通过转发环回网关端口通过 SSH 连接。
...
2. 否则，如果 Bonjour 在局域网上找到网关，提供一键”使用此网关”选择并将其保存为直接端点。
...
- 配对请求在网关中创建/批准/拒绝（参见[网关配对](/zh/gateway/pairing)）。
- 网关执行：
  - 认证（令牌/密钥对）
...
- **macOS 应用**：帮助你选择网关，显示配对提示，并仅将 SSH 作为备用方案。
- **iOS/Android 节点**：浏览 Bonjour 作为便利，并连接到配对的网关 WS。
