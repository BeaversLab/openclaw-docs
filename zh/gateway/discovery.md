...
- **网关 WS（控制平面）**：默认情况下 `127.0.0.1:18789` 上的 WebSocket 端点；可以通过 `gateway.bind` 绑定到局域网/tailnet。
- **直接 WS 传输**：面向局域网/tailnet 的网关 WS 端点（无 SSH）。
- **SSH 传输（备用）**：通过 SSH 转发 `127.0.0.1:18789` 进行远程控制。
- **传统 TCP 桥接（已弃用/已移除）**：较旧的节点传输（参见 [桥接协议](/en/gateway/bridge-protocol)）；不再通告以供发现。

协议详情：

- [网关协议](/en/gateway/protocol)
- [桥接协议（传统）](/en/gateway/bridge-protocol)
...
- 配对请求在网关中创建/批准/拒绝（参见 [网关配对](/en/gateway/pairing)）。
- 网关强制执行：
  - 认证（令牌 / 密钥对）
