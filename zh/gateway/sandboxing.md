...
Gateway 始终位于主机上；启用后，工具执行将在隔离的沙盒中运行。
...
- 任何明确允许在主机上运行的工具（例如 `tools.elevated`）。
  - **提升权限的执行在主机上运行并绕过沙盒。**
  - 如果关闭了沙盒，`tools.elevated` 不会改变执行方式（已经在主机上）。参见[提升模式](/en/tools/elevated)。
...
- 有关绑定挂载如何与工具策略和提升权限执行交互的信息，请参阅 [Sandbox vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)。
...
`OPENCLAW_DOCKER_SOCKET` 覆盖套接字位置。完整设置和环境变量参考：[Docker](/en/install/docker#enable-agent-sandbox-for-docker-gateway-opt-in)。
...
`exec`，请使用工具策略拒绝（参见 [Sandbox vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)）。

调试：

- 使用 `openclaw sandbox explain` 检查有效的沙箱模式、工具策略和修复配置键。
- 有关“为什么这被阻止？”的心理模型，请参阅 [Sandbox vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)。
  保持锁定状态。
...
有关优先级，请参阅 [多代理沙盒与工具](/en/tools/multi-agent-sandbox-tools)。
...
- [多代理沙箱与工具](/en/tools/multi-agent-sandbox-tools)
