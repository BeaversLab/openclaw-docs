...
- 任何显式允许在主机上运行的工具（例如 `tools.elevated`）。
  ...
  - 如果关闭沙箱隔离，`tools.elevated` 不会改变执行方式（已在主机上）。请参阅 [提升模式](/en/tools/elevated)。
...
- 有关绑定如何与工具策略和提升执行交互的信息，请参阅 [沙箱与工具策略与提升模式](/en/gateway/sandbox-vs-tool-policy-vs-elevated)。
...
参考：[Docker](/en/install/docker#enable-agent-sandbox-for-docker-gateway-opt-in)。
...
`exec`，请使用工具策略拒绝（参见 [沙箱与工具策略与提权](/en/gateway/sandbox-vs-tool-policy-vs-elevated)）。

调试：

- 使用 `openclaw sandbox explain` 来检查有效的沙箱模式、工具策略和修复配置键。
- 请参阅 [沙箱与工具策略与提权](/en/gateway/sandbox-vs-tool-policy-vs-elevated) 以了解“为什么这被阻止？”的思维模型。
  保持锁定状态。
...
请参阅 [多代理沙箱与工具](/en/tools/multi-agent-sandbox-tools) 了解优先级。
...
- [多智能体沙箱与工具](/en/tools/multi-agent-sandbox-tools)
