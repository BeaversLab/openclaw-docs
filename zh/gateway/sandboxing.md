...
- 任何明确允许在主机上运行的工具（例如 `tools.elevated`）。
  ...
  - 如果沙箱隔离关闭，`tools.elevated` 不会改变执行方式（已在主机上）。请参阅[提升模式](/zh/tools/elevated)。
...
- 在创建或重新创建后首次使用时，OpenClaw 会从本地工作区一次性填充该远程工作区。
- 此后，`exec`、`read`、`write`、`edit`、`apply_patch`、提示媒体读取和入站媒体暂存将直接通过 SSH 在远程工作区上运行。
- OpenClaw 不会自动将远程更改同步回本地工作区。

认证材料：

- `identityFile`、`certificateFile`、`knownHostsFile`：使用现有的本地文件，并通过 OpenSSH 配置传递它们。
- `identityData`、`certificateData`、`knownHostsData`：使用内联字符串或 SecretRefs。OpenClaw 通过正常的 secrets 运行时快照解析它们，使用 `0600` 将它们写入临时文件，并在 SSH 会话结束时将其删除。
- 如果为同一项目同时设置了 `*File` 和 `*Data`，则 `*Data` 在该 SSH 会话中优先。

这是一个 **远程权威（remote-canonical）** 模型。远程 SSH 工作区在初始种子后将成为真正的沙箱状态。

重要后果：

- 在种子步骤之后，在 OpenClaw 之外进行的主机本地编辑在重新创建沙箱之前远程不可见。
- `openclaw sandbox recreate` 删除每个作用域的远程根目录，并在下次使用时从本地重新进行种子处理。
- SSH 后端不支持浏览器沙箱隔离。
...
- `mirror` (默认)：本地工作区保持权威性。OpenClaw 在执行前将本地文件同步到 OpenShell，并在执行后将远程工作区同步回来。
- `remote`：创建沙箱后，OpenShell 工作区即为权威。OpenClaw 会从本地工作区一次性初始化远程工作区，随后文件工具和执行操作直接在远程沙箱上运行，而不会将更改同步回来。

OpenShell 复用与通用 SSH 后端相同的核心 SSH 传输和远程文件系统桥接。
该插件添加了 OpenShell 特定的生命周期 (`sandbox create/get/delete`, `sandbox ssh-config`) 以及可选的 `mirror` 模式。

远程传输详细信息：

- OpenClaw 向 OpenShell 请求特定于沙箱的 SSH 配置，方式是通过 `openshell sandbox ssh-config <name>`。
- Core 将该 SSH 配置写入临时文件，打开 SSH 会话，并复用由 `backend: "ssh"` 使用的同一远程文件系统桥接。
- 在 `mirror` 模式下，仅生命周期有所不同：在执行前从本地同步到远程，然后在执行后同步回本地。

当前 OpenShell 的限制：

- 暂不支持沙箱浏览器
- OpenShell 后端不支持 `sandbox.docker.binds`
- Docker 特定的运行时开关在 `sandbox.docker.*` 下仍然仅适用于 Docker 后端

## OpenShell 工作区模式

OpenShell 有两种工作区模型。这是实际中最重要的一部分。
...
- 在 `exec` 之前，OpenClaw 将本地工作区同步到 OpenShell 沙箱中。
- 在 `exec` 之后，OpenClaw 将远程工作区同步回本地工作区。
- 文件工具仍然通过沙箱桥运行，但在两次操作之间，本地工作区仍然是事实的来源。

在以下情况使用此模式：

- 您在 OpenClaw 之外的本地编辑文件，并希望这些更改自动显示在沙箱中
- 您希望 OpenShell 沙箱的行为尽可能像 Docker 后端
- 您希望主机工作区在每次执行后反映沙箱的写入操作
...
当您希望 **OpenShell 工作区成为权威来源** 时，请使用 `plugins.entries.openshell.config.mode: "remote"`。

行为：

- 当首次创建沙箱时，OpenClaw 会从本地工作区一次性向远程工作区播种。
- 此后，`exec`、`read`、`write`、`edit` 和 `apply_patch` 将直接对远程 OpenShell 工作区进行操作。
- 执行后，OpenClaw **不会**将远程更改同步回本地工作区。
- 提示时的媒体读取仍然有效，因为文件和媒体工具通过沙箱桥进行读取，而不是假定本地主机路径。
- 传输方式是通过 SSH 进入由 `openshell sandbox ssh-config` 返回的 OpenShell 沙箱。

重要后果：

- 如果在种子步骤之后，你在 OpenClaw 之外的宿主机上编辑文件，远程沙箱将**不会**自动看到这些更改。
- 如果重新创建沙箱，远程工作区将从本地工作区再次进行种子同步。
...
- 你希望降低每轮对话的同步开销
- 你不希望对宿主机的本地编辑悄悄覆盖远程沙箱状态

如果你将沙箱视为临时执行环境，请选择 `mirror`。
如果你将沙箱视为真正的工作区，请选择 `remote`。
...
- `"ro"`：以只读方式在 `/agent` 挂载 agent 工作区（禁用 `write`/`edit`/`apply_patch`）。
- `"rw"`：以读写方式在 `/workspace` 挂载 agent 工作区。

使用 OpenShell 后端时：

- `mirror` 模式在执行轮次之间仍使用本地工作区作为标准来源
- `remote` 模式在初始种子之后使用远程 OpenShell 工作区作为标准来源
- `workspaceAccess: "ro"` 和 `"none"` 仍然以相同方式限制写入行为

入站媒体会被复制到活动的沙盒工作区（`media/inbound/*`）。
Skills 注意：`read` 工具是沙盒根目录下的。使用 `workspaceAccess: "none"` 时，
OpenClaw 会将符合条件的 Skills 镜像到沙盒工作区（`.../skills`）以便
...
全局和每个代理的绑定会被**合并**（而非替换）。在 `scope: "shared"` 下，将忽略每个代理的绑定。
...
- 如果您只需要对工作区进行读取访问，请将其与 `workspaceAccess: "ro"` 结合使用；绑定模式保持独立。
- 请参阅 [沙箱与工具策略与提权执行](/zh/gateway/sandbox-vs-tool-policy-vs-elevated) 以了解绑定如何与工具策略和提权执行交互。
...
注意：默认镜像**不**包含 Node。如果技能需要 Node（或其他运行时），可以制作自定义镜像或通过 `sandbox.docker.setupCommand` 安装（需要网络出口 + 可写根目录 + root 用户）。

如果您想要一个功能更全面的沙箱镜像，其中包含常用工具（例如 `curl`、`jq`、`nodejs`、`python3`、`git`），请构建：
...
随附的沙箱浏览器镜像还针对容器化工作负载应用了保守的 Chromium 启动默认设置。
当前的容器默认设置包括：
...
- `--no-sandbox` 和 `--disable-setuid-sandbox`，当启用 `noSandbox` 时。
- 这三个图形加固标志（`--disable-3d-apis`，
  `--disable-software-rasterizer`，`--disable-gpu`）是可选的，并且当容器缺乏 GPU 支持时非常有用。如果您的
...
`OPENCLAW_DOCKER_SOCKET` 覆盖 socket 位置。完整的设置和环境变量
...
- `docker.network: "container:<id>"` 需要 `dangerouslyAllowContainerNamespaceJoin: true` 并且仅限紧急情况使用。
...
工具允许/拒绝策略仍会在沙箱规则之前应用。如果某个工具在全局或针对特定代理被拒绝，启用沙箱隔离也不会恢复它。

`tools.elevated` 是一个显式的逃生舱，用于在主机上运行 `exec`。
`/exec` 指令仅适用于经过授权的发送者，并且在每个会话中持久存在；若要彻底禁用
...
- 请参阅 [沙箱 vs Tool Policy vs Elevated](/zh/gateway/sandbox-vs-tool-policy-vs-elevated) 以了解“为什么会被阻止？”的心智模型。
  请保持锁定状态。
...
- [多代理沙箱和工具](/zh/tools/multi-agent-sandbox-tools)
