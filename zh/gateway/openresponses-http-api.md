---
summary: "从网关暴露兼容 OpenResponses 的 /v1/responses HTTP 端点"
read_when:
...
OpenClaw 的网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
在底层，请求作为正常的网关代理运行执行（与 `openclaw agent` 代码路径相同），因此路由/权限/配置与您的网关相匹配。
...
如果请求包含 OpenResponses `user` 字符串，网关将从中派生一个稳定的会话密钥，以便重复调用可以共享代理会话。
...
PDF.js 构建需要浏览器 worker/DOM 全局变量，因此不用于网关。
