...
- **Hooks**（本页）：在代理事件触发时（例如 `/new`、`/reset`、`/stop` 或生命周期事件）在 Gateway 内部运行。
- **Webhooks**：外部 HTTP webhooks，允许其他系统在 OpenClaw 中触发工作。请参阅 [Webhook Hooks](/en/automation/webhook) 或使用 `openclaw webhooks` 获取 Gmail 辅助命令。

Hooks 也可以捆绑在插件中；请参阅 [Plugins](/en/tools/plugin#plugin-hooks)。
...
- **💾 session-memory**：当您发出 `/new` 时，将会话上下文保存到您的工作区（默认 `~/.openclaw/workspace/memory/`）
...
- **`homepage`**：文档 URL
- **`requires`**：可选要求
  - **`bins`**: PATH 中的必需二进制文件（例如，`["git", "node"]`）
...
内部钩子有效负载将这些作为 `type: "session"` 与 `action: "compact:before"` / `action: "compact:after"` 一起发出；监听器使用上述组合键进行订阅。
...
- **`message:received`**：当从任何渠道接收到入站消息时触发。在媒体理解之前的处理早期触发。内容可能包含原始占位符，例如尚未处理的媒体附件的 `<media:audio>`。
- **`message:transcribed`**：当消息已完全处理时触发，包括音频转录和链接理解。此时，`transcript` 包含音频消息的完整转录文本。当您需要访问转录后的音频内容时，请使用此钩子。
- **`message:preprocessed`**：在所有媒体和链接理解完成后对每条消息触发，在代理看到消息之前，允许钩子访问完全丰富的正文（转录、图像描述、链接摘要）。
...
- **`tool_result_persist`**：在工具结果写入会话转录之前对其进行转换。必须是同步的；返回更新后的工具结果有效载荷或 `undefined` 以保持原样。参见 [Agent Loop](/en/concepts/agent-loop)。
...
- 仅加载可识别的引导基本名称。
- 子代理允许列表会被保留（仅 `AGENTS.md` 和 `TOOLS.md`）。
...
- [Webhook 钩子](/en/automation/webhook)
