...
- **Hooks**（本页）：当代理事件触发时，例如 `/new`、`/reset`、`/stop` 或生命周期事件，在网关内部运行。
- **Webhooks**：外部 HTTP webhooks，允许其他系统在 OpenClaw 中触发工作。请参阅 [Webhook Hooks](/zh/en/automation/webhook) 或使用 `openclaw webhooks` 获取 Gmail 辅助命令。

Hooks 也可以捆绑在插件中；请参阅 [Plugins](/zh/en/tools/plugin#plugin-hooks)。
...
- **🚀 boot-md**：在网关启动时运行 `BOOT.md`（需要启用内部挂钩）
...
在入职过程（`openclaw onboard`）中，系统将提示您启用推荐的挂钩。向导会自动发现符合条件的挂钩并供您选择。

## 挂钩发现

挂钩会从三个目录中自动发现（按优先级顺序）：

1. **工作区挂钩**：`<workspace>/hooks/`（每个代理单独设置，优先级最高）
2. **受管挂钩**：`~/.openclaw/hooks/`（用户安装，在工作区之间共享）
3. **捆绑挂钩**：`<openclaw>/dist/hooks/bundled/`（随 OpenClaw 附带）
...
挂钩包是标准的 npm 包，通过 `package.json` 中的 `openclaw.hooks` 导出一个或多个挂钩。
使用以下命令安装：
...
裸规范和 `@latest` 保持在稳定轨道上。如果 npm 将其中任何一个解析为预发布版本，OpenClaw 会停止并要求您通过预发布标签（如 `@beta`/`@rc`）或确切的预发布版本显式选择加入。
...
每个条目指向一个包含 `HOOK.md` 和 `handler.ts`（或 `index.ts`）的挂钩目录。
挂钩包可以附带依赖项；它们将安装在 `~/.openclaw/hooks/<id>` 下。
每个 `openclaw.hooks` 条目在解析符号链接后必须保持在包目录内；转义的条目将被拒绝。

安全说明：`openclaw hooks install` 使用 `npm install --ignore-scripts` 安装依赖项
（无生命周期脚本）。保持挂钩包依赖树为“纯 JS/TS”，并避免依赖 `postinstall` 构建的包。

## 挂钩结构

### HOOK.md 格式

`HOOK.md` 文件包含 YAML 前置元数据以及 Markdown 文档：
...
`handler.ts` 文件导出了一个 `HookHandler` 函数：
...
- **`session:compact:after`**：在压缩完成并包含摘要元数据之后

内部 hook 负载将这些作为 `type: "session"` 发出，并带有 `action: "compact:before"` / `action: "compact:after"`；监听器使用上述组合键进行订阅。
特定的处理程序注册使用字面键格式 `${type}:${action}`。对于这些事件，请注册 `session:compact:before` 和 `session:compact:after`。

### 代理事件

- **`agent:bootstrap`**：在注入工作区引导文件之前（钩子可能会修改 `context.bootstrapFiles`）
...
- **`message:received`**：当从任何通道接收到入站消息时。在处理过程的早期触发，在媒体理解之前。内容可能包含像 `<media:audio>` 这样的原始占位符，用于尚未处理的媒体附件。
- **`message:transcribed`**：当消息已完全处理时，包括音频转录和链接理解。此时，`transcript` 包含音频消息的完整转录文本。当您需要访问转录的音频内容时，请使用此钩子。
- **`message:preprocessed`**：在所有媒体 + 链接理解完成后为每条消息触发，在代理看到之前，让钩子能够访问完全丰富的内容正文（转录、图像描述、链接摘要）。
...
- **工作区 Hook** (`<workspace>/hooks/`)：针对每个 Agent，优先级最高
...
- `2026-01-16-1430.md`（如果标识生成失败，则回退到时间戳）
...
- 仅加载可识别的引导基本名称。
- 子代理允许列表已保留（仅限 `AGENTS.md` 和 `TOOLS.md`）。
...
- [捆绑钩子自述文件](https://github.com/openclaw/openclaw/tree/main/src/hooks/bundled)
- [Webhook 钩子](/zh/en/automation/webhook)
