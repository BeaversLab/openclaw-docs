...
| ------------------------------------ | ------------------- | ---------------------------------------- |
| 每 30 分钟检查收件箱                  | Heartbeat           | 与其他检查合并，具有上下文感知能力        |
| 在上午 9 点整发送每日报告             | Cron (isolated)     | 需要精确的时间                            |
| 监控日历中的即将发生的事件            | Heartbeat           | 自然适合周期性感知                        |
| 运行每周深度分析                      | Cron (isolated)     | 独立任务，可以使用不同的模型               |
| 20 分钟后提醒我                       | Cron (main, `--at`) | 具有精确时间的一次性任务                   |
| 后台项目健康检查                      | Heartbeat           | 搭载现有周期运行                          |
...
- **智能抑制**：如果无需关注，代理回复 `HEARTBEAT_OK` 且不会发送任何消息。
...
有关完整配置，请参阅 [Heartbeat](/en/gateway/heartbeat)。
...
- **每项任务的控制**：使用 `--stagger <duration>` 覆盖交错设置，或使用 `--exact` 强制精确计时。
...
有关完整的 CLI 参考，请参阅 [Cron 作业](/en/automation/cron-jobs)。
...
- 如果工具返回 `needs_approval`，你将以 `resumeToken` 和 `approve` 标志继续。
...
当你想要以下情况时，使用带有 `--system-event` 的 `--session main`：
...
- 如果你只想要内部处理，请在 heartbeat 上使用 `target: "none"`。
- 对于例行任务，使用带有更便宜模型的独立 Cron。

## 相关

- [Heartbeat](/en/gateway/heartbeat) - 完整的心跳配置
- [Cron jobs](/en/automation/cron-jobs) - 完整的 Cron CLI 和 API 参考
- [System](/en/cli/system) - 系统事件 + 心跳控制
