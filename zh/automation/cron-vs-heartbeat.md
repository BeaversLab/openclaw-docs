...
- **模型覆盖**：每个任务可选更省或更强模型。
- **投递控制**：隔离作业默认使用 `announce`（发布摘要）；可按需选择 `none`。
- **立即投递**：Announce 模式会直接发布，无需等待 heartbeat。
...
它会在纽约时间早上 7:00 准时运行，使用 Opus 以提高质量，并发布摘要到 WhatsApp。
...
- **Heartbeat/Cron** 决定*何时*触发一次运行。
- **Lobster** 定义运行时*具体步骤*是什么。
