---
summary: "对 cron 和 heartbeat 的调度及交付进行故障排除"
read_when:
  - Cron 未运行
  - Cron 已运行但未传递消息
  - Heartbeat 似乎静默或被跳过
...
- 渠道认证错误（`unauthorized`, `missing_scope`, `Forbidden`）→ 交付被渠道凭据/权限阻止。
...
- Heartbeat 已启用且间隔非零。
- 上一次 heartbeat 结果为 `ran`（或已理解跳过原因）。

常见特征：

- `heartbeat skipped` 且带有 `reason=quiet-hours` → 在 `activeHours` 之外。
- `requests-in-flight` → 主通道繁忙；heartbeat 已推迟。
- `empty-heartbeat-file` → 跳过了间隔 heartbeat，因为 `HEARTBEAT.md` 没有可操作的内容且没有排队的带标签 cron 事件。
- `alerts-disabled` → 可见性设置会阻止出站心跳消息。
...
- `Config path not found: agents.defaults.userTimezone` 表示该键未设置；心跳将回退到主机时区（如果设置了 `activeHours.timezone`，则使用该值）。
- 不带 `--tz` 的 Cron 使用网关主机时区。
- 心跳 `activeHours` 使用配置的时区解析方式（`user`、`local` 或显式 IANA 时区）。
- 不带时区的 ISO 时间戳在 cron `at` 计划中被视为 UTC 时间。

常见特征：

- 主机时区更改后，作业在错误的挂钟时间运行。
- 在您的白天期间心跳总是被跳过，因为 `activeHours.timezone` 不正确。
