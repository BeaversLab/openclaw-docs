...
> **Cron 对比 Heartbeat？** 有关何时使用各自的指导，请参阅 [Cron vs Heartbeat](/en/automation/cron-vs-heartbeat)。
...
当您需要正常的心跳提示和主会话上下文时，这是最佳选择。参见 [Heartbeat](/en/gateway/heartbeat)。
...
- 在下一次成功运行后，退避会自动重置。
- 一次性 (`at`) 作业会对瞬时错误（速率限制、过载、网络、server_error）进行最多 3 次带退避的重试；永久错误则会立即禁用。请参阅[重试策略](/en/automation/cron-jobs#retry-policy)。
