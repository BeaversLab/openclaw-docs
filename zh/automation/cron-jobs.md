...
> **Cron 对比 Heartbeat？** 有关何时使用各自的指导，请参阅 [Cron vs Heartbeat](/zh/automation/cron-vs-heartbeat)。
...
当您需要正常的心跳提示和主会话上下文时，这是最佳选择。参见 [Heartbeat](/zh/gateway/heartbeat)。
...
Telegram 通过 `message_thread_id` 支持论坛主题。对于 cron 交付，您可以将
主题/主题线程编码到 `to` 字段中：
...
带有前缀的目标如 `telegram:...` / `telegram:group:...` 也是可接受的：

- `telegram:group:-1001234567890:topic:123`

## 工具调用的 JSON 架构

直接调用 Gateway `cron.*` 工具时（代理工具调用或 RPC）请使用这些格式。
CLI 标志接受人类可读的时长，如 `20m`，但工具调用应对 `schedule.at` 使用 ISO 8601 字符串，
并对 `schedule.everyMs` 使用毫秒。
...
- 自定义会话（`session:xxx`）在多次运行之间维持持久上下文。
...
- `sessions.json` 中的隔离 cron 运行会话由 `cron.sessionRetention` 修剪（默认为 `24h`；设置 `false` 可禁用）。
...
配置 `cron.retry` 以覆盖这些默认值（参见 [配置](/zh/automation/cron-jobs#configuration)）。
...
- 首选：为每个作业设置 `delivery.mode: "webhook"` 并附带 `delivery.to: "https://..."`。
- Webhook URL 必须是有效的 `http://` 或 `https://` URL。
- 发布时，负载为 cron 完成事件 JSON。
- 如果设置了 `cron.webhookToken`，则 auth header 为 `Authorization: Bearer <cron.webhookToken>`。
- 如果未设置 `cron.webhookToken`，则不会发送 `Authorization` header。
- 已弃用的回退机制：存储的带有 `notify: true` 的旧版任务在存在时仍会使用 `cron.webhook`。
...
- 清理器（reaper）会移除早于 `cron.sessionRetention` 的过期运行会话条目。
...
- 许多写入同一个 `cron/runs/<jobId>.jsonl` 的嘈杂周期性任务

建议做法：

- 在您的调试/审计需求允许的范围内，尽可能保持 `cron.sessionRetention` 短暂
...
`cron.run` 现在在手动运行排队后即确认，而不是在作业完成后确认。成功的排队响应看起来像 `{ ok: true, enqueued: true, runId }`。如果作业已在运行或 `--due` 发现没有到期的任务，响应保持为 `{ ok: true, ran: false, reason }`。使用 `openclaw cron runs --id <jobId>` 或 `cron.runs` 网关方法来检查最终完成的条目。
...
- 在下一次成功运行后，退避会自动重置。
- 一次性 (`at`) 作业会对瞬时错误（速率限制、过载、网络、server_error）进行最多 3 次带退避的重试；永久错误则会立即禁用。请参阅[重试策略](/zh/automation/cron-jobs#retry-policy)。

### Telegram 投递到了错误的地方

- 对于论坛话题，请使用 `-100…:topic:<id>`，这样既明确又无歧义。
- 如果您在日志或存储的“最后路由”目标中看到 `telegram:...` 前缀，那是正常的；
  cron 投递接受它们，并且仍能正确解析话题 ID。

### 子代理公告投递重试

- 当子代理运行完成时，网关会将结果公告给请求者会话。
- 如果公告流程返回 `false`（例如请求者会话正忙），网关会通过 `announceRetryCount` 最多重试 3 次。
- 为了防止陈旧条目无限循环，超过 `endedAt` 5 分钟的公告将被强制过期。
