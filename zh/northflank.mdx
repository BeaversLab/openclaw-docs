...
这是最简单的“无需服务器终端”的路径：Northflank 为您运行 Gateway(网关)，
并且您可以通过 `/setup` 网页向导配置所有内容。

## 如何开始

1. 点击 [Deploy OpenClaw](https://northflank.com/stacks/deploy-openclaw) 打开模板。
2. 如果您还没有 [account on Northflank](https://app.northflank.com/signup)，请创建一个。
...
- 通过 Northflank 卷 (`/data`) 提供的持久化存储，以便配置/凭据/工作空间在重新部署后得以保留
...
3. **Bot** → **Add Bot**
4. 在 Bot → Privileged Gateway Intents 下启用 **MESSAGE CONTENT INTENT**（必填，否则机器人将在启动时崩溃）
