...
Render 蓝图是用于定义基础设施的 YAML 文件。本仓库中的 `render.yaml` 配置了运行 OpenClaw 所需的一切：
...
该蓝图默认为 `starter`。要使用免费套餐，请在你的 fork 仓库的 `render.yaml` 中修改 `plan: free`（但请注意：无持久化磁盘意味着每次部署时配置会重置）。
...
这种情况发生在免费套餐上（无持久化磁盘）。升级到付费套餐，或者定期通过 `/setup/export` 导出你的配置。
