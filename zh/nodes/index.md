...
- 无法识别的节点 `platform` / `deviceFamily` 元数据使用保守的默认允许列表，其中不包括 `system.run` 和 `system.which`。如果您针对未知平台特意需要这些命令，请通过 `gateway.nodes.allowCommands` 明确添加它们。
- `system.run` 支持 `--cwd`、`--env KEY=VAL`、`--command-timeout` 和 `--needs-screen-recording`。
- 对于 shell 封装器（`bash|sh|zsh ... -c/-lc`），请求作用域的 `--env` 值被缩减为明确的允许列表（`TERM`、`LANG`、`LC_*`、`COLORTERM`、`NO_COLOR`、`FORCE_COLOR`）。
- 在允许列表模式下，对于始终允许的决策，已知的分发封装器（`env`、`nice`、`nohup`、`stdbuf`、`timeout`）将保留内部可执行文件路径，而不是封装器路径。如果解包不安全，则不会自动保留任何允许列表条目。
- 在处于允许列表模式的 Windows 节点主机上，通过 `cmd.exe /c` 运行的 shell-wrapper 需要批准（仅凭允许列表条目不会自动允许包装器形式）。
- `system.notify` 支持 `--priority <passive|active|timeSensitive>` 和 `--delivery <system|overlay|auto>`。
- 节点主机将忽略 `PATH` 覆盖，并去除危险的启动/Shell 密钥（`DYLD_*`、`LD_*`、`NODE_OPTIONS`、`PYTHON*`、`PERL*`、`RUBYOPT`、`SHELLOPTS`、`PS4`）。如果您需要额外的 PATH 条目，请配置节点主机服务环境（或将工具安装在标准位置），而不是通过 `--env` 传递 `PATH`。
- 在 macOS 节点模式下，`system.run` 受 macOS 应用中的 exec 批准控制（设置 → Exec 批准）。
...
WebSocket 并公开 `system.run` / `system.which`。这在 Linux/Windows
上或在服务器上运行最小节点时非常有用。
...
- macOS 菜单栏应用程序作为节点连接到 Gateway(网关) WS 服务器（因此 `openclaw nodes …` 可针对此 Mac 运行）。
- 在远程模式下，应用会为 Gateway(网关) 端口打开一个 SSH 隧道并连接到 `localhost`。
