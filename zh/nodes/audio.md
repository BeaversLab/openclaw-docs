...
- **详细日志记录**：在 `--verbose` 中，我们记录转录运行的时间以及它替换正文的时间。

## 自动检测（默认）

如果您**不配置 模型**且 `tools.media.audio.enabled` **未**设置为 `false`，
OpenClaw 将按此顺序自动检测并在第一个有效的选项处停止：
...
注意：二进制检测在 macOS/Linux/Windows 上是尽力而为的；确保 CLI 位于 `PATH` 上（我们展开 `~`），或者使用完整命令路径设置显式 CLI 模型。
...
- 提供商身份验证遵循标准 模型 身份验证顺序（身份验证配置文件、环境变量、`models.providers.*.apiKey`）。
- Deepgram 会在使用 `provider: "deepgram"` 时拾取 `DEEPGRAM_API_KEY`。
...
- 默认大小上限为 20MB (`tools.media.audio.maxBytes`)。超过该大小的音频将被跳过，并尝试下一个条目。
- 小于 1024 字节的微小/空音频文件会在提供商/CLI 转录之前被跳过。
- 音频的默认 `maxChars` 为 **未设置**（完整转录）。设置 `tools.media.audio.maxChars` 或每个条目的 `maxChars` 以裁剪输出。
- OpenAI 自动检测的默认值为 `gpt-4o-mini-transcribe`；设置 `model: "gpt-4o-transcribe"` 以获得更高的准确率。
- 使用 `tools.media.audio.attachments` 处理多条语音笔记 (`mode: "all"` + `maxAttachments`)。
- 转录文本以 `{{Transcript}}` 的形式提供给模板使用。
- `tools.media.audio.echoTranscript` 默认处于关闭状态；启用它可在代理处理之前将转录确认发回原始聊天。
...
当为群聊设置 `requireMention: true` 时，OpenClaw 现在会在检查提及**之前**转录音频。这使得即使语音笔记包含提及也能被处理。
...
- 设置 `channels.telegram.groups.<chatId>.topics.<threadId>.disableAudioPreflight` 以覆盖按主题的设置（`true` 表示跳过，`false` 表示强制启用）。
- 默认值为 `false`（当符合提及限制条件时启用预检）。

**示例：** 用户在设置了 `requireMention: true` 的 Telegram 群组中发送一条语音笔记说“嘿 @Claude，天气怎么样？”。该语音笔记被转录，检测到提及，然后代理回复。

## 注意事项

- 作用域规则采用首次匹配优先原则。`chatType` 会被规范化为 `direct`、`group` 或 `room`。
- 确保您的 CLI 退出码为 0 并打印纯文本；JSON 需要通过 `jq -r .text` 进行处理。
- 对于 `parakeet-mlx`，如果您传递 `--output-dir`，OpenClaw 会在 `--output-format` 为 `txt`（或省略）时读取 `<output-dir>/<media-basename>.txt`；非 `txt` 输出格式则回退到 stdout 解析。
- 保持超时设置合理（`timeoutSeconds`，默认 60s），以避免阻塞回复队列。
