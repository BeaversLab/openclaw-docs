---
summary: "节点定位命令、权限模式以及 Android 前台行为"
read_when:
...
可选。macOS 节点通过权限映射报告 `location`；iOS/Android 可以省略它。
...
- `LOCATION_BACKGROUND_UNAVAILABLE`: 应用处于后台，但仅允许使用期间。
- `LOCATION_TIMEOUT`: 未能及时获取定位。
- `LOCATION_UNAVAILABLE`: 系统故障 / 无提供程序。
