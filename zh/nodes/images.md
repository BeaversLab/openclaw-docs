---
summary: "针对发送、网关和代理回复的图像与媒体处理规则"
read_when:
...
- 当存在媒体时，网页发送者使用与 `openclaw message send` 相同的管道来解析本地路径或 URL。
- 如果提供了多个媒体条目，将按顺序发送。
...
- 媒体理解（如果通过 `tools.media.*` 或共享的 `tools.media.models` 配置）在模板化之前运行，并可以将 `[Image]`、`[Audio]` 和 `[Video]` 块插入 `Body` 中。
