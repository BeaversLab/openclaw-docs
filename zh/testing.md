...
- How to select models:
  ...
  - 或 `OPENCLAW_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-5,..."` (comma allowlist)
...
没有固定的 “CI 模型列表” (实时测试是可选的)，但以下是建议在开发机器上使用密钥定期覆盖的**推荐**模型。
...
在 `OPENCLAW_LIVE_GATEWAY_MODELS` 中至少包含一个支持图像的模型（Claude/Gemini/OpenAI 支持视觉的变体等），以测试图像探测。
