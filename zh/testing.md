...
- 预期：
  - 设计上在 CI 中不稳定（真实的网络、真实的提供商策略、配额、故障）
  - 需要花钱 / 消耗速率限制
  - 优先运行缩小的子集而不是“全部”
  - Live 运行将源 `~/.profile` 以获取缺失的 API 密钥
...
- 编辑逻辑/测试：运行 `pnpm test`（如果改动很大，则运行 `pnpm test:coverage`）
...
- 如何选择模型：
  ...
  - 或 `OPENCLAW_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-5,..."`（逗号分隔的白名单）
- 如何选择提供商：
...
- 覆盖（可选）：
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` 以通过 CLI 参数传递图像文件路径，而不是通过提示词注入。
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"`（或 `"list"`）以在设置了 `IMAGE_ARG` 时控制图像参数的传递方式。
  - `OPENCLAW_LIVE_CLI_BACKEND_RESUME_PROBE=1` 以发送第二轮对话并验证恢复流程。
- `OPENCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` 用于保持 Claude Code CLI MCP 配置启用（默认通过一个临时空文件禁用 MCP 配置）。
...
- `google-gemini-cli/...` 使用您机器上的本地 Gemini CLI（独立的身份验证和工具怪癖）。
- Gemini API 与 Gemini CLI：
  - API：OpenClaw 通过 HTTP 调用 Google 托管的 Gemini API (API 密钥 / 配置文件身份验证)；这是大多数用户所说的“Gemini”的含义。
  - CLI：OpenClaw 调用本地的 `gemini` 二进制文件；它有自己的身份验证，并且行为可能有所不同（流式传输/工具支持/版本差异）。
...
提示：不要试图在文档中硬编码“所有模型”。权威列表是你的机器上 `discoverModels(...)` 返回的内容加上任何可用的密钥。
...
- 如果实时测试显示“no creds”（无凭据），请按照调试 `openclaw models list` / 模型选择的方式进行调试。
...
这些在仓库 Docker 镜像中运行 `pnpm test:live`，挂载您的本地配置目录和工作区（并在挂载时导入 `~/.profile`）：
...
- 网络网络（两个容器，WS 认证 + 健康）：`pnpm test:docker:gateway-network`（脚本：`scripts/e2e/gateway-network-docker.sh`）
...
- 通过真实的网关 + 代理循环模拟工具调用 (`src/gateway/gateway.tool-calling.mock-openai.test.ts`)。
...
- **合规性 (Compliance)：** 代理是否会在使用前阅读 `SKILL.md` 并遵循所需的步骤/参数？
