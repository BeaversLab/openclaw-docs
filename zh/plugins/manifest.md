...
有关兼容的捆绑包布局，请参阅[插件包](/zh/plugins/bundles)。
...
有关完整的 `plugins.*` 模式，请参阅[配置参考](/zh/configuration)。
...
- `providerAuthEnvVars` 是用于身份验证探测、环境标记
  验证以及类似的提供商身份验证表面的低成本元数据路径，这些操作不应仅为了检查环境名称而启动插件
  运行时。
- `providerAuthChoices` 是用于身份验证选择器、
  `--auth-choice` 解析、首选提供商映射以及在提供商运行时加载之前进行的简单新手引导
  CLI 标志注册的低成本元数据路径。对于需要提供商代码的运行时向导
  元数据，请参阅
  [提供商运行时钩子](/zh/plugins/architecture#provider-runtime-hooks)。
