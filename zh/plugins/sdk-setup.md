...
<Tip>
  **在寻找演练教程？** 操作指南介绍了上下文中的打包内容：[渠道
  插件](/zh/plugins/sdk-渠道-plugins#step-1-package-and-manifest) 和 [提供商
  插件](/zh/plugins/sdk-提供商-plugins#step-1-package-and-manifest)。
</Tip>
...
<Warning>
  仅当您的 `setupEntry` 在网关开始监听之前注册了网关所需的所有内容（渠道注册、HTTP
  路由、网关方法）时，才启用延迟加载。如果完整入口拥有必需的启动能力，请保留默认行为。
</Warning>
...
<Info>
  对于来自 npm 的安装，`openclaw plugins install` 运行 `npm install --ignore-scripts`（没有
  生命周期脚本）。保持插件依赖树为纯 JS/TS，并避免需要 `postinstall` 构建的包。
</Info>
