...
- 上下文注入：**仅 pending** 的群消息（默认 50 条）且 _未触发运行_ 的，会被放在 `[Chat messages since your last reply - for context]` 前缀块中，触发消息则在 `[Current message - respond to this]` 下。已进入会话的消息不会重复注入。
