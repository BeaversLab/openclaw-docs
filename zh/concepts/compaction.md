...
在压缩之前，OpenClaw 可以运行一个 **静默内存刷新** 轮次，将持久化笔记存储到磁盘。有关详细信息和配置，请参阅 [Memory](/zh/concepts/memory)。
...
有关剪枝的详细信息，请参阅 [/concepts/会话-pruning](/zh/concepts/session-pruning)。
...
压缩行为由当前活动的
[context engine](/zh/concepts/context-engine) 拥有。传统引擎使用上述内置的
摘要功能。插件引擎（通过
