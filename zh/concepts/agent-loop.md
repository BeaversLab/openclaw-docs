...
- 消息传递通道可以选择输入到此通道系统的队列模式。请参阅 [Command Queue](/en/concepts/queue)。
...
请参阅 [挂钩](/en/automation/hooks) 了解设置和示例。
...
有关 API API 和注册详细信息，请参阅 [插件](/en/tools/plugin#plugin-hooks)。
...
- `NO_REPLY` 被视为静默令牌，并会从传出负载中过滤掉。
- 重复的消息传递工具会从最终负载列表中移除。
- 如果不存在可渲染的负载且工具出错，则会发出回退工具错误回复
  （除非消息传递工具已发送用户可见的回复）。
...
- 有关压缩管道，请参阅 [压缩](/en/concepts/compaction)。
...
- `agent.wait` 默认值：30秒（仅等待时间）。`timeoutMs` 参数覆盖。
