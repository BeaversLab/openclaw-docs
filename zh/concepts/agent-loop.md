...
2. `agentCommand` 运行代理：
   ...
   - 如果嵌入式循环未发出，则发出 **lifecycle end/error**
...
- 获取会话写入锁；在流式传输之前打开并准备 `SessionManager`。

## 提示词组装 + 系统提示词

- 系统提示词由 OpenClaw 的基础提示词、技能提示词、启动上下文和每次运行的覆盖项构建而成。
- 强制执行特定于模型的限制和压缩保留令牌（tokens）。
- 请参阅 [系统提示词](/en/concepts/system-prompt) 以了解模型看到的内容。
...
- **`agent:bootstrap`**: 在系统提示完成之前构建引导文件时运行。
  使用此挂钩来添加/删除引导上下文文件。
- **Command hooks**: `/new`, `/reset`, `/stop`, 以及其他命令事件（参见 Hooks 文档）。

请参阅 [挂钩](/en/automation/hooks) 以了解设置和示例。

### 插件挂钩（代理 + 网关生命周期）

这些在代理循环或网关管道内运行：

- **`before_model_resolve`**: 在会话前运行（无 `messages`），以便在模型解析之前确定性地覆盖提供程序/模型。
- **`before_prompt_build`**: 在会话加载后（带 `messages`）运行，以便在提交提示之前注入 `prependContext`、`systemPrompt`、`prependSystemContext` 或 `appendSystemContext`。使用 `prependContext` 处理每轮动态文本，并使用 system-context 字段处理应位于系统提示空间中的稳定指导内容。
- **`before_agent_start`**: 传统兼容性挂钩，可能在任一阶段运行；优先使用上述显式挂钩。
...
有关钩子 API 和注册详细信息，请参阅 [插件](/en/tools/plugin#plugin-hooks)。
...
- 有关分块和块回复行为，请参阅 [流式传输](/en/concepts/streaming)。
...
- 有关压缩管道，请参阅 [Compaction](/en/concepts/compaction)。

## 事件流（目前）

- `lifecycle`: 由 `subscribeEmbeddedPiSession` 发出（并作为后备由 `agentCommand` 发出）
...
- 代理运行时： `agents.defaults.timeoutSeconds` 默认 600秒；在 `runEmbeddedPiAgent` 中止计时器中强制执行。
