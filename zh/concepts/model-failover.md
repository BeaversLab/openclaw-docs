---
summary: "OpenClaw 如何轮换身份配置文件并在模型之间回退"
read_when:
...
- 默认：当没有可用的电子邮件时使用 `provider:default`。
- 带电子邮件的 OAuth：`provider:<email>`（例如 `google-antigravity:user@gmail.com`）。

配置文件存在于 `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` 中的 `profiles` 下。
...
- **次键**：`usageStats.lastUsed`（每种类型中，最早创建的优先）。
...
- 通过 `/model …` 使用每次会话的覆盖（当您的 UI/聊天界面支持时），并带有配置文件覆盖。

## 冷却期

当配置文件由于身份验证/速率限制错误（或看起来像速率限制的超时）而失败时，OpenClaw 会将其标记为冷却并移动到下一个配置文件。
格式/无效请求错误（例如 Cloud Code Assist 工具调用 ID
验证失败）被视为值得进行故障转移的情况，并使用相同的冷却期。
兼容 OpenAI 的停止原因错误，例如 `Unhandled stop reason: error`、
`stop reason: error` 和 `reason: error` 被归类为超时/故障转移
信号。
...
如果某个提供商的所有配置文件均失败，OpenClaw 将移动到
`agents.defaults.model.fallbacks` 中的下一个模型。这适用于认证失败、速率限制以及
已耗尽配置文件轮次的超时（其他错误不会触发回退）。

当运行以模型覆盖（hooks 或 CLI）开始时，在尝试任何配置的回退后，回退仍会在
`agents.defaults.model.primary` 结束。

## 相关配置

参见 [Gateway configuration](/zh/en/gateway/configuration) 了解：
