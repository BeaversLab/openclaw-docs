...
这些文件位于工作区（`agents.defaults.workspace`，默认 `~/.openclaw/workspace`）。完整布局参见 [Agent 工作区](/zh/concepts/agent-workspace)。
...
完整压缩生命周期参见
[会话 management + compaction](/zh/reference/session-management-compaction)。
