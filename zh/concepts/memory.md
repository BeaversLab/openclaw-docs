...
- `memory_search` — 基于索引片段的语义回忆。
- `memory_get` — 针对特定 Markdown 文件/行范围的目标读取。

`memory_get` 现在在文件不存在时会**优雅降级**（例如，
在首次写入之前的今日每日日志）。内置管理器和 QMD
...
代理轮次**，提醒模型在上下文被压缩**之前**写入持久化记忆。默认提示明确指出模型_可以回复_，
...
- 默认启用。
- 监视内存文件变更（防抖）。
- 在 `agents.defaults.memorySearch` 下配置内存搜索（而非顶层
  `memorySearch`)。
...
- `memorySearch.provider = "ollama"` 也支持本地/自托管
  Ollama 嵌入 (`/api/embeddings`)，但它不会被自动选择。

远程嵌入**需要**嵌入提供商的 API 密钥。OpenClaw
从身份验证配置文件、`models.providers.*.apiKey` 或环境
变量中解析密钥。Codex OAuth 仅涵盖聊天/补全，并且**不**满足
...
[QMD](https://github.com/tobi/qmd)：一个优先考虑本地的搜索伴侣，结合了
BM25 + 向量 + 重排序。Markdown 仍然是真实数据源；OpenClaw 调用
QMD 进行检索。关键点：
...
- QMD 需要一个允许扩展的 SQLite 构建 (`brew install sqlite` 在
  macOS)。
- QMD 通过 Bun + `node-llama-cpp` 完全本地运行并自动下载 GGUF
...
- 网关现在在启动时初始化 QMD 管理器，因此定期更新
  计时器甚至在第一次 `memory_search` 调用之前就已启动。
- 启动刷新现在默认在后台运行，以便聊天启动不会
  被阻塞；设置 `memory.qmd.update.waitForBootSync = true` 以保持之前的
  阻塞行为。
- 搜索通过 `memory.qmd.searchMode` 运行 (默认 `qmd search --json`；也
  支持 `vsearch` 和 `query`)。如果所选模式拒绝您的
  QMD 构建上的标志，OpenClaw 将使用 `qmd query` 重试。如果 QMD 失败或二进制文件
  缺失，OpenClaw 会自动回退到内置 SQLite 管理器，以便
  内存工具继续工作。
- OpenClaw 目前未公开 QMD 嵌入批次大小的调优；批处理行为是
  由 QMD 本身控制。
- **首次搜索可能会很慢**：QMD 可能会下载本地 GGUF 模型（重排序器/查询
  expansion) 在第一次 `qmd query` 运行时。
  - OpenClaw 在运行 QMD 时会自动设置 `XDG_CONFIG_HOME`/`XDG_CACHE_HOME`。
  - 如果您想手动预先下载模型（并预热 OpenClaw 使用的同一索引），请使用代理的 XDG 目录运行一次性查询。
...
- `searchMode` (默认 `search`)：选择支持哪个 QMD 命令
...
- `scope`：与 [`session.sendPolicy`](/en/gateway/configuration#session) 的架构相同。
  ...
  - 遗留设置：`match.keyPrefix: "agent:..."` 仍被视为原始密钥前缀，但为清晰起见，首选使用 `rawKeyPrefix`。
- 当 `scope` 拒绝搜索时，OpenClaw 会记录一条包含派生的
  `channel`/`chatType` 的警告，以便更容易调试空结果。
- 源自工作区外部的片段显示为
  `memory_search` 结果中的 `qmd/<collection>/<relative-path>` 前缀；`memory_get`
  能够识别此前缀并从配置的 QMD 集合根目录读取。
- 当 `memory.qmd.sessions.enabled = true` 时，OpenClaw 会将清理后的会话
  记录（用户/助手对话）导出到 `~/.openclaw/agents/<id>/qmd/sessions/` 下的专用 QMD 集合，
  以便 `memory_search` 能够在不触及内置 SQLite 索引的情况下回顾最近的对话。
- `memory_search` 摘录现在包含一个 `Source: <path#line>` 页脚，当
  `memory.citations` 为 `auto`/`on` 时；设置 `memory.citations = "off"` 以将
  路径元数据保留在内部（代理仍会接收 `memory_get` 的路径，但摘录文本会省略页脚，并且系统提示
  会警告代理不要引用它）。
...
- 如果 `memorySearch.multimodal.enabled = true`，OpenClaw 还将仅索引 `extraPaths` 下的支持的图像/音频文件。默认内存根目录（`MEMORY.md`, `memory.md`, `memory/**/*.md`）保持仅限 Markdown。
- 符号链接会被忽略（文件或目录）。
...
- `gemini-embedding-2-preview` 也受支持：8192 token 限制和可配置维度（768 / 1536 / 3072，默认为 3072）。
...
- 当 `memorySearch.provider = "openai"` 或 `"gemini"` 时适用批处理模式，并使用相应的 API 密钥。
...
- `memory_search` 语义搜索来自 `MEMORY.md` + `memory/**/*.md` 的 Markdown 块（目标约 400 个 token，重叠 80 个 token）。它返回片段文本（上限约 700 个字符）、文件路径、行范围、分数、提供商/模型，以及我们是否从本地 → 远程嵌入回退。不返回完整的文件负载。
...
- 索引存储：位于 `~/.openclaw/memory/<agentId>.sqlite` 的每个代理 SQLite 数据库（通过 `agents.defaults.memorySearch.store.path` 可配置，支持 `{agentId}` token）。
- 新鲜度：对 `MEMORY.md` + `memory/` 的监视器会将索引标记为脏（防抖 1.5 秒）。同步在会话开始时、搜索时或按时间间隔调度，并异步运行。会话记录使用增量阈值来触发后台同步。
...
- **向量**：按余弦相似度排名的前 `maxResults * candidateMultiplier` 条结果。
- **BM25**：按 FTS5 BM25 排名（越低越好）的前 `maxResults * candidateMultiplier` 条结果。
...
- `vectorWeight` + `textWeight` 在配置解析时被归一化为 1.0，因此权重表现为百分比。
...
**何时启用：**如果您注意到 `memory_search` 返回冗余或近乎重复的片段，特别是对于经常在不同日期重复相似信息的每日笔记。
...
- `memory_search` 永远不会阻塞索引；在后台同步完成之前，结果可能会略有滞后。
- 结果仍然仅包含片段；`memory_get` 仍仅限于记忆文件。
- 会话索引是按代理隔离的（仅索引该代理的会话日志）。
- 会话日志存储在磁盘上（`~/.openclaw/agents/<agentId>/sessions/*.jsonl`）。任何具有文件系统访问权限的进程/用户都可以读取它们，因此请将磁盘访问视为信任边界。为了更严格的隔离，请在单独的操作系统用户或主机下运行代理。
...
SQLite 虚拟表（`vec0`）中，并在数据库中
执行向量距离查询。这可以保持搜索速度快，而无需将每个嵌入加载到 JS 中。
...
- `enabled` 默认为 true；禁用时，搜索回退到进程内
  基于存储嵌入的余弦相似度。
- 如果 sqlite-vec 扩展缺失或加载失败，OpenClaw 会记录
错误并继续使用 JS 回退方案（无向量表）。
- `extensionPath` 覆盖捆绑的 sqlite-vec 路径（适用于自定义构建
  或非标准安装位置）。

### 本地嵌入自动下载

- 默认本地嵌入模型：`hf:ggml-org/embeddinggemma-300m-qat-q8_0-GGUF/embeddinggemma-300m-qat-Q8_0.gguf`（约 0.6 GB）。
- 当 `memorySearch.provider = "local"` 时，`node-llama-cpp` 解析 `modelPath`；如果缺少 GGUF 文件，它会 **自动下载** 到缓存（如果设置了 `local.modelCacheDir` 则下载到该路径），然后加载。下载支持断点续传。
- 原生构建要求：运行 `pnpm approve-builds`，选择 `node-llama-cpp`，然后 `pnpm rebuild node-llama-cpp`。
- 回退方案：如果本地设置失败且 `memorySearch.fallback = "openai"`，我们将自动切换到远程嵌入（除非被覆盖，否则使用 `openai/text-embedding-3-small`）并记录原因。
...
- `remote.headers` 与 OpenAI 标头合并；键冲突时远程端优先。省略 `remote.headers` 以使用 OpenAI 默认值。
