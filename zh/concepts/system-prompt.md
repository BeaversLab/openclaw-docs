...
- `minimal`：用于子代理；省略 **技能**、**记忆召回**、**OpenClaw
  ...
  **Messaging**、**Silent Replies** 和 **Heartbeats**。Tooling、**Safety**、
  Workspace、沙箱、Current Date & Time（如果已知）、Runtime 和注入的
  context 保持可用。
- `none`：仅返回基础身份行。
...
要检查每个注入文件的贡献程度（原始大小 vs 注入大小、截断以及工具架构开销），请使用 `/context list` 或 `/context detail`。请参阅 [上下文](/zh/en/concepts/context)。
