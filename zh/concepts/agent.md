...
如果启用了 `agents.defaults.sandbox`，非主会话可以使用 `agents.defaults.sandbox.workspaceRoot` 下的基于会话的工作区覆盖此设置（参见 [Gateway 网关 configuration](/en/gateway/configuration)）。
...
Skills 可以通过配置/环境变量进行控制（参见 [Gateway 网关 configuration](/en/gateway/configuration) 中的 `skills`）。
...
当队列模式为 `followup` 或 `collect` 时，入站消息将被保留，直到当前轮次结束，然后新的代理轮次将使用排队中的负载启动。有关模式 + 防抖/上限行为，请参见 [Queue](/en/concepts/queue)。
