...
如果启用了 `agents.defaults.sandbox`，非主会话可以通过 `agents.defaults.sandbox.workspaceRoot` 下的每个会话工作区覆盖此设置（参见 [Gateway 网关 configuration](/zh/en/gateway/configuration)）。
...
技能可以通过配置/环境进行限制（参见 [Gateway 网关 configuration](/zh/en/gateway/configuration) 中的 `skills`）。
...
当前轮次结束，然后一个新的代理轮次将使用排队的负载开始。请参阅
[Queue](/zh/en/concepts/queue) 以了解模式 + 防抖/上限行为。

块流式传输在完成后立即发送已完成的助手块；它
...
800–1200 个字符；优先选择段落分隔符，然后是换行符；最后是句子）。
