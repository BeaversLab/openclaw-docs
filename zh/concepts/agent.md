...
如果启用了 `agents.defaults.sandbox`，非主会话可以通过 `agents.defaults.sandbox.workspaceRoot` 下的每个会话工作区覆盖此设置（参见 [Gateway configuration](/en/gateway/configuration)）。
...
技能可以通过配置/环境进行限制（参见 [Gateway configuration](/en/gateway/configuration) 中的 `skills`）。
...
[Queue](/en/concepts/queue) 以了解模式 + 防抖/上限行为。
