...
- 连接器派生自 `blockStreamingChunk.breakPreference`
  (`paragraph` → `\n\n`, `newline` → `\n`, `sentence` → 空格)。
- 可以通过 `*.blockStreamingCoalesce` 获取频道覆盖设置（包括每个账户的配置）。
- 对于 Signal/Slack/Discord，除非被覆盖，否则默认合并 `minChars` 会提升至 1500。
...
- **流式传输块：** `blockStreamingDefault: "on"` + `blockStreamingBreak: "text_end"`（边进行边发送）。非 Telegram 频道还需要 `*.blockStreaming: true`。
- **在末尾流式传输所有内容：** `blockStreamingBreak: "message_end"`（刷新一次，如果很长可能会有多个块）。
- **无块流式传输：** `blockStreamingDefault: "off"`（仅最终回复）。

**频道注意：** 除非将 `*.blockStreaming` 显式设置为 `true`，否则块流式传输处于**关闭**状态。频道可以在没有块回复的情况下流式传输实时预览（`channels.<channel>.streaming`）。

配置位置提醒：`blockStreaming*` 的默认值位于 `agents.defaults` 下，而不是根配置中。
...
- 在私信和群组/主题中，使用 `sendMessage` + `editMessageText` 进行预览更新。
