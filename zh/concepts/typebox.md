...
TypeBox 是一个优先支持 TypeScript 的模式库。我们使用它来定义 **Gateway
WebSocket protocol**（握手、请求/响应、服务器事件）。这些模式驱动
**runtime validation**、**JSON Schema export** 以及 macOS 应用的 **Swift codegen**。单一事实来源；其他所有内容均由此生成。

如果您需要更高层的协议上下文，请从
[Gateway architecture](/zh/concepts/architecture) 开始。
...
第一个帧**必须**是一个 `connect` 请求。之后，客户端可以调用方法（例如
`health`、`send`、`chat.send`）并订阅事件（例如
`presence`、`tick`、`agent`）。
...
- **服务器端**：每个传入帧都使用 AJV 进行验证。握手仅接受参数匹配 `ConnectParams` 的 `connect` 请求。
- **客户端**：JS 客户端在使用事件和响应帧之前会对其进行验证。
- **方法层**：Gateway 在 `hello-ok` 中通告支持的 `methods` 和 `events`。
...
在 `src/gateway/server-methods.ts` 中注册（已合并 `systemHandlers`），然后在 `src/gateway/server.ts` 中将 `"system.echo"` 添加到 `METHODS`。
...
- 大多数对象使用 `additionalProperties: false` 来表示严格的负载。
- `NonEmptyString` 是 ID 和方法/事件名称的默认选择。
- 顶层 `GatewayFrame` 在 `type` 上使用 **discriminator（鉴别器）**。
- 具有副作用的方法通常需要在参数中包含 `idempotencyKey`
  （例如：`send`、`poll`、`agent`、`chat.send`）。
- `agent` 接受可选的 `internalEvents` 用于运行时生成的编排上下文
  （例如子代理/定时任务完成交接）；请将此视为内部 API 表面。
