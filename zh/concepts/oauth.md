...
上述所有内容也遵循 `$OPENCLAW_STATE_DIR`（状态目录覆盖）。完整参考：[/gateway/configuration](/zh/en/gateway/configuration#auth-storage-oauth--api-keys)
...
<Warning>
Anthropic setup-token 支持属于技术兼容性，而非政策保证。
Anthropic 过去曾阻止在 Claude Code 之外的某些订阅使用。
请自行决定是否使用订阅身份验证，并核实 Anthropic 的当前条款。
</Warning>
...
向导路径为 `openclaw onboard` → 身份验证选择 `setup-token` (Anthropic)。
...
如果你希望“个人”和“工作”环境互不干扰，请使用独立的代理（独立的会话 + 凭据 + 工作区）：
