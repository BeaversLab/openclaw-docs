...
这发生在生成正常回复 **之前**，因此消息可能看起来像是“没有响应”。解决方法是：
...
- `/model`（以及 `/model list`）是一个紧凑的编号选择器（模型系列 + 可用的提供商）。
...
显示已解析的主模型、后备模型、图像模型以及已配置提供商的认证概览。它还会显示在认证存储中找到的配置文件的 OAuth 过期状态（默认在 24 小时内发出警告）。`--plain` 仅打印已解析的主模型。
OAuth 状态始终显示（并包含在 `--json` 输出中）。如果已配置的提供商没有凭据，`models status` 会打印一个 **缺少认证 (Missing auth)** 部分。
JSON 包括 `auth.oauth`（警告窗口 + 配置文件）和 `auth.providers`（每个提供商的有效认证）。
请使用 `--check` 进行自动化（当缺失/过期时退出 `1`，即将过期时退出 `2`）。

认证方式取决于提供商/账户。对于常驻网关主机，API 密钥通常是最可预测的；同时也支持订阅令牌流程。
...
在 TTY 中运行时，您可以交互式地选择后备模型。在非交互模式下，传递 `--yes` 以接受默认值。

## 模型注册表 (`models.json`)

`models.providers` 中的自定义提供商会被写入代理目录（默认为 `~/.openclaw/agents/<agentId>/agent/models.json`）下的 `models.json` 中。除非将 `models.mode` 设置为 `replace`，否则默认会合并此文件。

匹配提供商 ID 的合并模式优先级：

- 代理 `models.json` 中已存在的非空 `baseUrl` 优先。
- 代理 `models.json` 中的非空 `apiKey` 仅在该提供商在当前配置/身份验证配置文件上下文中未由 SecretRef 管理时优先。
- SecretRef 管理的提供商 `apiKey` 值将从源标记（环境引用为 `ENV_VAR_NAME`，文件/exec 引用为 `secretref-managed`）刷新，而不是持久化解析出的密钥。
- SecretRef 管理的提供商标头值将从源标记（环境引用为 `secretref-env:ENV_VAR_NAME`，文件/exec 引用为 `secretref-managed`）刷新。
- 如果代理 `apiKey`/`baseUrl` 为空或缺失，则回退到配置 `models.providers`。
- 其他提供商字段会从配置和标准化的目录数据中刷新。
