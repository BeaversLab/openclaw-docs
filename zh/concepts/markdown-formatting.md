---
summary: "用于出站频道的 Markdown 格式化管道"
read_when:
...
`markdown.tables` 控制每个频道（以及每个账户）的转换。

- `code`：将表格渲染为代码块（大多数频道的默认设置）。
- `bullets`：将每一行转换为项目符号（Signal + WhatsApp 的默认选项）。
...
- **Slack：** `[label](url)` -> `<url|label>`；裸 URL 保持裸露。解析期间
  禁用自动链接以避免重复链接。
...
剧透标记（`||spoiler||`）仅针对 Signal 进行解析，在 Signal 中它们映射到
SPOILER 样式范围。其他渠道将其视为纯文本。

## 如何添加或更新渠道格式化程序

1. **解析一次：** 使用共享的 `markdownToIR(...)` 辅助函数，配合适合渠道的
   选项（自动链接、标题样式、引用块前缀）。
