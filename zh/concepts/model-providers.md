---
summary: "包含示例配置和 CLI 流程的模型提供商概述"
read_when:
...
- Direct API-key 模型支持共享的 `/fast` 切换开关和 `params.fastMode`；OpenClaw 将其映射到 Anthropic `service_tier`（`auto` vs `standard_only`）
...
- 提供商：`google`
- 身份验证：`GEMINI_API_KEY`
- 可选轮换：`GEMINI_API_KEYS`、`GEMINI_API_KEY_1`、`GEMINI_API_KEY_2`、`GOOGLE_API_KEY` 备用和 `OPENCLAW_LIVE_GEMINI_KEY`（单次覆盖）
...
- Gemini CLI OAuth 作为捆绑插件提供 (`google-gemini-cli-auth`，默认禁用)。
  ...
  - 注意：您**不**需要将客户端 ID 或密钥粘贴到 `openclaw.json` 中。CLI 登录流程会将令牌存储在网关主机的身份验证配置文件中。
...
- Hugging Face 推理：`huggingface` (`HUGGINGFACE_HUB_TOKEN` 或 `HF_TOKEN`) — OpenAI 兼容路由器；示例模型：`huggingface/deepseek-ai/DeepSeek-R1`；CLI：`openclaw onboard --auth-choice huggingface-api-key`。请参阅 [Hugging Face (Inference)](/zh/providers/huggingface)。
...
当您选择加入 `OLLAMA_API_KEY` 时，会在 `http://127.0.0.1:11434` 本地检测到 Ollama，并且内置提供商插件会直接将 Ollama 添加到
`openclaw onboard` 和模型选择器中。有关入门、云/本地模式和自定义配置，请参阅 [/providers/ollama](/zh/en/providers/ollama)。
...
- 如果 `baseUrl` 为空/被省略，OpenClaw 将保留默认的 OpenAI 行为（其解析为 `api.openai.com`）。
- 出于安全考虑，显式的 `compat.supportsDeveloperRole: true` 仍会在非原生 `openai-completions` 端点上被覆盖。
