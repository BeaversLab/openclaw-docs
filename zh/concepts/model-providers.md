...
- 对于 Google 提供商，`GOOGLE_API_KEY` 也作为备用包含在内。
- 密钥选择顺序保留优先级并去重数值。
- 仅在遇到速率限制响应时（例如 `429`、`rate_limit`、`quota`、`resource exhausted`），才会使用下一个密钥重试请求。
- 非速率限制失败会立即失败；不会尝试密钥轮换。
- 当所有候选密钥都失败时，将返回最后一次尝试的最终错误。

## 内置提供商（pi-ai 目录）

OpenClaw 附带 pi-ai 目录。这些提供商**不**需要
...
- 通过 `agents.defaults.models["openai/<model>"].params.transport` 为每个模型覆盖传输方式（`"sse"`、`"websocket"` 或 `"auto"`）
...
- 直接的 API 密钥模型支持共享的 `/fast` 切换开关和 `params.fastMode`；OpenClaw 将其映射到 Anthropic `service_tier`（`auto` 与 `standard_only`）
...
- 提供商：`google`
- 认证：`GEMINI_API_KEY`
- 可选轮换：`GEMINI_API_KEYS`、`GEMINI_API_KEY_1`、`GEMINI_API_KEY_2`、`GOOGLE_API_KEY` 后备，以及 `OPENCLAW_LIVE_GEMINI_KEY`（单项覆盖）
...
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  ...
  - 注意：请**勿**将客户端 ID 或密钥粘贴到 `openclaw.json` 中。CLI 登录流程将令牌存储在网关主机上的身份配置文件中。
...
## 通过 `models.providers` 的提供商（custom/base URL）
...
Qwen 提供通过设备代码流程访问 Qwen Coder + Vision 的 OAuth。
启用捆绑插件，然后登录：
...
有关设置详情和说明，请参见 [/providers/qwen](/en/providers/qwen)。
...
当您选择加入 `OLLAMA_API_KEY` 时，会在 `http://127.0.0.1:11434` 本地检测到 Ollama，并且捆绑的提供商插件会直接将 Ollama 添加到 `openclaw onboard` 和模型选择器中。请参阅 [/providers/ollama](/en/providers/ollama) 了解新手引导、云/本地模式和自定义配置。
...
- 对于非原生端点上的 `api: "openai-completions"`（任何主机不是 `api.openai.com` 的非空 `baseUrl`），OpenClaw 会强制执行 `compat.supportsDeveloperRole: false`，以避免因不支持的 `developer` 角色而导致的提供商 400 错误。
