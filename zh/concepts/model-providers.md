...
read_when:
  - 您需要逐个提供商的模型设置参考
  - 您想要模型提供商的示例配置或 CLI 新手引导命令
title: "模型提供商"
---

# 模型提供商

本页面涵盖 **LLM/模型提供商**（而非 WhatsApp/Telegram 等聊天频道）。
...
- 提供商插件可以通过 `registerProvider({ catalog })` 注入模型目录；
  OpenClaw 在写入 `models.json` 之前，会将该输出合并到 `models.providers` 中。
- 提供商清单可以声明 `providerAuthEnvVars`，这样通用的基于环境变量的
  身份验证探测就不需要加载插件运行时。剩余的核心环境变量映射现在仅用于非插件/核心提供商
  以及少数通用优先级的情况，例如 Anthropic API 密钥优先的新手引导。
- 提供商插件还可以通过以下方式拥有提供商运行时行为：
  ...
  `fetchUsageSnapshot`。
- 注意：提供商运行时 `capabilities` 是共享的运行器元数据（提供商系列、transcript/工具特性、传输/缓存提示）。这与描述插件注册内容的[公共能力模型](/zh/tools/plugin#public-capability-model)（文本推理、语音等）不同。

## 插件拥有的提供商行为

提供商插件现在可以拥有大部分特定于提供商的逻辑，而 OpenClaw 则保留通用推理循环。

典型分工：

- `auth[].run` / `auth[].runNonInteractive`：提供商拥有 `openclaw onboard`、`openclaw models auth` 和无头设置的新手引导/登录流程
- `wizard.setup` / `wizard.modelPicker`：提供商拥有身份验证选择标签、旧别名、新手引导允许列表提示，以及新手引导/模型选择器中的设置条目
...
- `capabilities`：提供商发布 transcript/工具/提供商系列的特性
...
- `refreshOAuth`：当共享的 `pi-ai` 刷新器不足时，提供商负责 OAuth 刷新
- `buildAuthDoctorHint`：当 OAuth 刷新失败时，提供商附加修复指导
- `isCacheTtlEligible`：提供商决定哪些上游模型 ID 支持 prompt-cache TTL
- `buildMissingAuthMessage`：提供商将通用的身份验证存储错误替换为特定于提供商的恢复提示
- `suppressBuiltInModel`：提供商隐藏过时的上游行，并且可以返回供应商拥有的错误以用于直接解析失败
...
- `isModernModelRef`：提供商拥有实时/冒烟测试的首选模型匹配
- `prepareRuntimeAuth`：提供商将配置的凭据转换为短期运行时令牌
- `resolveUsageAuth`：提供商解析 `/usage` 的使用量/配额凭据以及相关的状态/报告界面
- `fetchUsageSnapshot`：提供商拥有使用量端点的获取/解析，而核心仍然拥有摘要外壳和格式化

当前捆绑的示例：

- `anthropic`：Claude 4.6 前向兼容回退、身份验证修复提示、使用量端点获取以及缓存 TTL/提供商系列元数据
- `openrouter`：透传模型 ID、请求包装器、提供商功能提示以及缓存 TTL 策略
- `github-copilot`：新手引导/设备登录、前向兼容模型回退、Claude 思维记录提示、运行时令牌交换以及使用量端点获取
- `openai`：GPT-5.4 前向兼容回退、直接 OpenAI 传输标准化、Codex 感知的缺失身份验证提示、Spark 抑制、合成 OpenAI/Codex 目录行、思维/实时模型策略以及提供商系列元数据
- `google` 和 `google-gemini-cli`：Gemini 3.1 前向兼容回退和现代模型匹配；Gemini CLI OAuth 还拥有使用量界面的身份验证配置文件令牌格式化、使用量令牌解析和配额端点获取
- `moonshot`：共享传输、插件拥有的思维负载标准化
- `kilocode`：共享传输、插件拥有的请求头、推理负载标准化、Gemini 副本提示以及缓存 TTL 策略
- `zai`：GLM-5 向前兼容回退、`tool_stream` 默认值、缓存 TTL 策略、二元思维/实时模型策略以及使用身份验证 + 配额获取
- `mistral`、`opencode` 和 `opencode-go`：插件拥有的功能元数据
- `byteplus`、`cloudflare-ai-gateway`、`huggingface`、`kimi-coding`、
  `modelstudio`、`nvidia`、`qianfan`、`synthetic`、`together`、`venice`、
  `vercel-ai-gateway` 和 `volcengine`：仅限插件拥有的目录
- `qwen-portal`：插件拥有的目录、OAuth 登录以及 OAuth 刷新
- `minimax` 和 `xiaomi`：插件拥有的目录以及使用身份验证/快照逻辑

捆绑的 `openai` 插件现在拥有两个提供商 ID：`openai` 和
`openai-codex`。

这涵盖了仍然适合 OpenClaw 常规传输的提供商。需要完全自定义请求执行器的提供商是一个独立的、更深层次的扩展表面。
...
- 密钥选择顺序保留优先级并对值进行去重。
- 请求仅在速率限制响应时使用下一个密钥重试（例如 `429`、`rate_limit`、`quota`、`resource exhausted`）。
- 非速率限制故障会立即失败；不会尝试密钥轮换。
- 当所有候选密钥都失败时，返回最后一次尝试的最终错误。

## 内置提供商 (pi-ai catalog)

OpenClaw 随附 pi‑ai catalog。这些提供商**不**需要 `models.providers` 配置；只需设置身份验证并选择一个模型。
...
- `openai/gpt-5.3-codex-spark` 在 OpenClaw 中被有意抑制，因为在线 OpenAI API 会拒绝它；Spark 被视为仅限 Codex
...
- 可选轮换: `ANTHROPIC_API_KEYS`、`ANTHROPIC_API_KEY_1`、`ANTHROPIC_API_KEY_2`，以及 `OPENCLAW_LIVE_ANTHROPIC_KEY`（单项覆盖）
- 示例模型: `anthropic/claude-opus-4-6`
- CLI: `openclaw onboard --auth-choice token`（粘贴 setup-token）或 `openclaw models auth paste-token --provider anthropic`
- 直接 API 密钥的模型支持共享的 `/fast` 切换开关和 `params.fastMode`；OpenClaw 将其映射到 Anthropic `service_tier`（`auto` vs `standard_only`）
- 政策说明：对 setup-token 的支持属于技术兼容性；在过去，Anthropic 曾阻止某些在 Claude Code 之外的订阅使用。请核实当前的 Anthropic 条款，并根据您的风险承受能力做出决定。
- 建议：与订阅 setup-token 身份验证相比，Anthropic API 密钥身份验证是更安全且推荐的路径。
...
- 与直接 `openai/*` 共享相同的 `/fast` 切换开关和 `params.fastMode` 配置
- 当 Codex OAuth 目录公开 `openai-codex/gpt-5.3-codex-spark` 时，它仍然可用；取决于授权
- 政策说明：对于像 OpenAI 这样的外部工具/工作流，明确支持 OAuth Codex OpenClaw。
...
- 提供商：`google`
- 身份验证：`GEMINI_API_KEY`
- 可选轮换：`GEMINI_API_KEYS`、`GEMINI_API_KEY_1`、`GEMINI_API_KEY_2`、`GOOGLE_API_KEY` 后备以及 `OPENCLAW_LIVE_GEMINI_KEY`（单项覆盖）
...
- 注意：CLI 中的 Gemini OAuth OpenClaw 属于非官方集成。部分用户报告在使用第三方客户端后 Google 账号受到了限制。请查看 Google 条款，如果选择继续，请使用非关键账号。
- Gemini CLI OAuth 作为捆绑的 `google` 插件的一部分提供。
  - 启用：`openclaw plugins enable google`
  - 登录：`openclaw models auth login --provider google-gemini-cli --set-default`
  - 注意：您**切勿**将客户端 ID 或密钥粘贴到 `openclaw.json` 中。CLI 登录流程会将令牌存储在 Gateway 主机的身份验证配置文件中。
...
仅当您想要覆盖默认基础 URL、请求头或模型列表时，才使用显式的 `models.providers.<id>` 条目。
...
Qwen 通过设备代码流提供 OAuth Coder + Vision 的 Qwen 访问权限。
...
Volcano Engine (火山引擎) 提供对中国境内的 Doubao 和其他模型的访问权限。
...
当您使用 `OLLAMA_API_KEY` 选择加入时，系统会在 `http://127.0.0.1:11434` 本地检测到 Ollama，并且捆绑的提供商插件会将 Ollama 直接添加到 `openclaw onboard` 和模型选择器中。有关新手引导、云/本地模式和自定义配置，请参阅 [/providers/ollama](/zh/providers/ollama)。
...
若要在本地选择加入自动发现（如果您的服务器不强制执行身份验证，则任何值均可）：
...
- 对于非原生端点上的 `api: "openai-completions"`（任何主机不为 `api.openai.com` 的非空 `baseUrl`），OpenClaw 会强制使用 `compat.supportsDeveloperRole: false`，以避免因不支持的 `developer` 角色而导致提供商出现 400 错误。
- 如果 `baseUrl` 为空/被省略，OpenClaw 将保持默认的 OpenAI 行为（解析为 `api.openai.com`）。
...
另请参阅：[/gateway/configuration](/zh/gateway/configuration) 获取完整的配置示例。
