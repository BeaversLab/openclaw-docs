...
- `web_search` — 使用 Brave Search API、Gemini（结合 Google Search grounding）、Grok、Kimi 或 Perplexity Search API 搜索 Web。
...
| 提供商                  | 结果形状                       | 提供商特定过滤器                    | 注释                                                                          | API 密钥                                     |
| ------------------------- | ---------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------- |
| **Brave Search API**      | 带有摘要的结构化结果   | `country`, `language`, `ui_lang`, time       | 支持 Brave `llm-context` 模式                                              | `BRAVE_API_KEY`                             |
| **Gemini**                | AI 综合答案 + 引用 | —                                            | 使用 Google Search grounding                                                   | `GEMINI_API_KEY`                            |
| **Grok**                  | AI 综合答案 + 引用 | —                                            | 使用 xAI 网络响应                                                | `XAI_API_KEY`                               |
| **Kimi**                  | AI 综合答案 + 引用 | —                                            | 使用 Moonshot 网络搜索                                                       | `KIMI_API_KEY` / `MOONSHOT_API_KEY`         |
| **Perplexity Search API** | 带有摘要的结构化结果   | `country`, `language`, time, `domain_filter` | 支持内容提取控制；OpenRouter 使用 Sonar 兼容路径 | `PERPLEXITY_API_KEY` / `OPENROUTER_API_KEY` |

### 自动检测

上表按字母顺序排列。如果未显式设置任何 `provider`，运行时自动检测将按以下顺序检查提供商：
...
每个 Brave 计划都包含 **每月 $5 的免费额度**（可续期）。Search 计划每 1,000 次请求收费 $5，因此免费额度覆盖每月 1,000 次查询。请在 Brave 仪表板中设置您的使用限制以避免意外收费。有关当前计划和定价，请参阅 [Brave API portal](https://brave.com/search/api/)。
...
为了实现传统的 Sonar/OpenRouter 兼容性，请改为设置 `OPENROUTER_API_KEY`，或者使用 `sk-or-...` 密钥配置 `tools.web.search.perplexity.apiKey`。设置 `tools.web.search.perplexity.baseUrl` 或 `model` 也会使 Perplexity 重新加入 chat-completions 兼容路径。
...
对于网关安装，请将这些放入 `~/.openclaw/.env` （或您的服务环境）中。请参阅 [环境变量](/zh/en/help/faq#how-does-openclaw-load-environment-variables)。
...
`llm-context` 返回提取的页面块用于上下文增强，而不是标准的 Brave 摘要。
在此模式下，`country` 和 `language` / `search_lang` 仍然有效，但 `ui_lang`，
`freshness`， `date_after` 和 `date_before` 将被拒绝。
...
Gemini 模型支持内置的 [Google 搜索增强](https://ai.google.dev/gemini-api/docs/grounding)，
...
如果您设置 `tools.web.search.perplexity.baseUrl` / `model`、使用 `OPENROUTER_API_KEY` 或配置 `sk-or-...` 密钥，Search API 专用过滤器将返回显式错误。
...
- 仅当 Firecrawl 处于活动状态时（`tools.web.fetch.enabled !== false` 和 `tools.web.fetch.firecrawl.enabled !== false`），才会解析 Firecrawl SecretRefs。
...
- `maxChars` 限制为 `tools.web.fetch.maxCharsCap`。
- `web_fetch` 在解析之前将下载的响应正文大小限制为 `tools.web.fetch.maxResponseBytes`；超大的响应将被截断并包含警告。
