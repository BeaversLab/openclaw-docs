...
- 如果未提供 PDF 输入，该工具将报错。
- `pages` 被解析为从 1 开始的页码，进行去重、排序，并限制在配置的最大页数内。
...
- 其他 URI 方案（例如 `ftp://`）将被拒绝并返回 `unsupported_pdf_reference`。
...
原生模式用于提供程序 `anthropic` 和 `google`。
该工具将原始 PDF 字节直接发送到提供程序 API。
...
有关完整字段详细信息，请参阅 [配置参考](/zh/en/gateway/configuration-reference)。
...
- `native`：原生提供程序模式为 `true`，回退模式为 `false`
