...
- 其他提供商的提取回退模式（首先提取文本，然后在需要时提取页面图像）。
- 支持单（`pdf`）或多（`pdfs`）输入，每次调用最多 10 个 PDF。

## 可用性

该工具仅在 OpenClaw 可以为 agent 解析支持 PDF 的模型配置时注册：

1. `agents.defaults.pdfModel`
2. 回退到 `agents.defaults.imageModel`
3. 根据可用的身份验证回退到尽力而为的提供商默认值

如果无法解析可用的模型，则不会暴露 `pdf` 工具。
...
- `pdf` 和 `pdfs` 在加载之前会合并并去重。
- 如果未提供 PDF 输入，工具将报错。
- `pages` 被解析为从 1 开始的页码，会去重、排序并限制为配置的最大页数。
...
- 在沙箱模式下，远程 `http(s)` URL 将被拒绝。
- 启用仅工作区文件策略时，将拒绝允许根目录之外的本地文件路径。
...
- 原生模式与 `pages`：抛出清晰的 `pages is not supported with native PDF providers` 错误
