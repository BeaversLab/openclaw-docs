---
summary: "用于向 OpenClaw 添加新共享功能的指南"
read_when:
  - 添加新的核心功能和插件注册界面
  - 确定代码是属于核心、供应商插件还是功能插件
  - 为渠道或工具连接新的运行时辅助程序
title: "功能指南"
---
...
当 OpenClaw 需要一个新的领域（例如图像生成、视频生成或某些未来的供应商支持的功能领域）时，请使用本指南。
...
这意味着你不应首先将供应商直接连接到渠道或工具中。首先从定义功能开始。
...
1. 可能有多个供应商可以实现它
2. 渠道、工具或功能插件应该使用它，而无需关心
   具体的供应商
3. 核心需要拥有回退、策略、配置或交付行为

如果该工作仅涉及供应商且尚不存在共享契约，请停止并首先
定义契约。
...
4. `openai` 和 `google` 插件注册了供应商支持的实现
