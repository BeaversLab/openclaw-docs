...
`OPENCLAW_HOME_VOLUME` 或绑定挂载持久化。参见 [Docker](/zh/install/docker)。
...
- `snapshot`：
  ...
  - 配置默认值（仅工具/CLI）：设置 `browser.snapshotDefaults.mode: "efficient"` 以在调用方未传递模式时使用高效快照（参见 [Gateway configuration](/zh/gateway/configuration#browser-openclaw-managed-browser)）。
...
[Browser troubleshooting](/zh/tools/browser-linux-troubleshooting)。
