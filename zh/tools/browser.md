...
- `chrome`：扩展程序中继到您的**系统浏览器**（需要将 OpenClaw
  扩展程序附加到标签页）。
- `existing-session`：用于正在运行的 Chrome
  配置文件的官方 Chrome MCP 附加流程。
...
- 浏览器控制服务绑定到从 `gateway.port` 派生的端口的回环地址
  （默认值：`18791`，即网关端口 + 2）。中继使用下一个端口（`18792`）。
- 如果您覆盖 Gateway(网关) 端口（`gateway.port` 或 `OPENCLAW_GATEWAY_PORT`），
  派生的浏览器端口将发生偏移以保持在同一个“系列”中。
- 如果未设置，`cdpUrl` 默认为中继端口。
- `remoteCdpTimeoutMs` 适用于远程（非环回）CDP 可达性检查。
- `remoteCdpHandshakeTimeoutMs` 适用于远程 CDP WebSocket 可达性检查。
- 浏览器导航/打开选项卡在导航前会进行 SSRF 防护检查，并在导航后在最终的 `http(s)` URL 上进行尽力而为的复查。
- `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` 默认为 `true`（受信任网络模型）。将其设置为 `false` 以仅进行严格的公共浏览。
- `browser.ssrfPolicy.allowPrivateNetwork` 作为遗留别名仍受支持以保持兼容性。
- `attachOnly: true` 意味着“永不启动本地浏览器；仅当其已在运行时才进行连接”。
- `color` + 每个配置文件的 `color` 会为浏览器 UI 着色，以便您可以看到哪个配置文件处于活动状态。
- 默认配置文件是 `openclaw`（OpenClaw 托管的独立浏览器）。使用 `defaultProfile: "chrome"` 选择加入 Chrome 扩展程序中继。
...
OpenClaw 在调用 `/json/*` 端点和连接到 CDP WebSocket 时会保留认证信息。对于令牌，请优先使用环境变量或机密管理器，而不是将其提交到配置文件中。
...
[Browserless](https://browserless.io) 是一项托管的 Chromium 服务，通过 HTTPS 暴露
...
- **WebSocket 端点** (`ws://` / `wss://`) — OpenClaw 直接连接，跳过 `/json/version`。将此用于 [Browserbase](https://www.browserbase.com) 等服务或任何为您提供 WebSocket URL 的提供商。

### Browserbase

[Browserbase](https://www.browserbase.com) 是一个云平台，用于运行内置 CAPTCHA 求解、隐身模式和住宅代理的无头浏览器。
...
- 免费层级允许每月一个并发会话和一小时浏览器使用时间。
  请参阅 [pricing](https://www.browserbase.com/pricing) 了解付费计划的限制。
- 请参阅 [Browserbase docs](https://docs.browserbase.com) 获取完整的 API
  参考、SDK 指南和集成示例。
...
- 请将 Gateway(网关) 和任何节点主机保持在私有网络（Tailscale）上；避免暴露在公网。
- 请将远程 CDP URL/令牌视为机密信息；优先使用环境变量或机密管理器。
...
- `chrome` 配置文件是为 Chrome 扩展程序中继内置的（默认指向 `http://127.0.0.1:18792`）。
- Existing-会话 配置文件是可选加入的；使用 `--driver existing-session` 创建它们。
...
完整指南：[Chrome 扩展程序](/zh/tools/chrome-extension)

流程：

- Gateway(网关) 在本地运行（同一台机器）或节点主机在浏览器机器上运行。
- 本地 **中继服务器** 侦听回环 `cdpUrl`（默认：`http://127.0.0.1:18792`）。
- 您点击选项卡上的 **OpenClaw Browser Relay** 扩展图标进行连接（它不会自动连接）。
- 代理通过选择正确的配置文件，使用正常的 `browser` 工具来控制该选项卡。

如果 Gateway(网关) 在其他地方运行，请在浏览器机器上运行节点主机，以便 Gateway(网关) 可以代理浏览器操作。

### 沙箱隔离会话

如果代理会话被沙箱隔离，`browser` 工具可能会默认为 `target="sandbox"`（沙箱浏览器）。
Chrome 扩展中继接管需要主机浏览器控制权，因此要么：
...
- 固定该扩展程序，然后在您想要控制的标签页上点击它（徽章显示 `ON`）。
...
- 此模式的大多数操作（截图/快照/操作）依赖于 Playwright-on-CDP。
- 再次点击扩展程序图标即可分离。

## 通过 MCP 连接 Chrome 现有会话

OpenClaw 也可以通过官方的 Chrome DevTools MCP 服务器附加到正在运行的 Chrome 配置文件。这将重用该 Chrome 配置文件中已打开的标签页和登录状态。
...
- Chrome 显示了附加同意提示，且您已接受
- Gateway(网关) 或节点主机可以生成 `npx chrome-devtools-mcp@latest --autoConnect`

注意事项：

- 此路径的风险高于隔离的 `openclaw` 配置文件，因为它可以在您登录的浏览器会话中执行操作。
- OpenClaw 不会为此驱动程序启动 Chrome；它仅附加到现有会话。
- OpenClaw 在此处使用官方的 Chrome DevTools MCP `--autoConnect` 流程，而非
  传统的默认配置文件远程调试端口工作流程。
- 现有会话截图支持从快照进行页面捕获和 `--ref` 元素捕获，但不支持 CSS `--element` 选择器。
- 现有会话 `wait --url` 与其他浏览器驱动一样，支持精确匹配、子字符串匹配和 glob 模式。尚不支持 `wait --load networkidle`。
- 某些功能仍然需要扩展中继或托管浏览器路径，例如
  PDF 导出和下载拦截。
- 默认情况下，将中继设置为仅限环回。如果中继必须可从不同的网络命名空间访问（例如 Gateway(网关) 中的 WSL2，Windows 上的 Chrome），请将 `browser.relayBindHost` 设置为显式绑定地址，例如 `0.0.0.0`，同时保持周围网络私有和经过身份验证。
...
Playwright 包（而不是 `playwright-core`）并重启网关，或者重新安装
支持浏览器的 OpenClaw。

#### Docker Playwright 安装

如果你的 Gateway(网关) 运行在 Docker 中，请避免使用 `npx playwright`（npm 覆盖冲突）。
请改用捆绑的 CLI：
...
- 对于高级操作（点击/输入/截图/PDF），它在 CDP 之上使用 **Playwright**。
- 当缺少 Playwright 时，仅支持非 Playwright 操作。

这种设计使 Agent 保持在一个稳定、确定的接口上，同时允许您交换本地/远程浏览器和配置文件。
...
- 下载和跟踪输出路径被限制在 OpenClaw 临时根目录：
  - traces：`/tmp/openclaw`（备用：`${os.tmpdir()}/openclaw`）
  - downloads：`/tmp/openclaw/downloads`（备用：`${os.tmpdir()}/openclaw/downloads`）
...
- `snapshot`：
  ...
  - 配置默认值（仅限工具/CLI）：设置 `browser.snapshotDefaults.mode: "efficient"` 以在调用方未传递模式时使用高效快照（参见 [CLI 配置](/zh/gateway/configuration#browser-openclaw-managed-browser)）。
  - 角色快照选项（`--interactive`、`--compact`、`--depth`、`--selector`）强制使用基于角色的快照，并包含如 `ref=e12` 的引用。
  - `--frame "<iframe selector>"` 将角色快照的范围限定在一个 iframe 内（与如 `e12` 的角色引用配对使用）。
  - `--interactive` 输出一个扁平的、易于选择的交互元素列表（最适合驱动操作）。
  - `--labels` 添加一个仅视口截图，并覆盖 ref 标签（打印 `MEDIA:<path>`）。
...
- **角色快照（角色引用如 `e12`）**：`openclaw browser snapshot --interactive`（或 `--compact`、`--depth`、`--selector`、`--frame`）
  ...
  - 在内部，该引用是通过 `getByRole(...)` 解析的（对于重复项，再加上 `nth()`）。
  - 添加 `--labels` 以包含带有叠加 `e12` 标签的视口截图。

引用行为：

- 引用在导航过程中**不稳定**；如果操作失败，请重新运行 `snapshot` 并使用新的引用。
- 如果角色快照是通过 `--frame` 拍摄的，则角色引用将限定在该 iframe 内，直到拍摄下一个角色快照。
...
3. 如果仍然失败：`openclaw browser highlight <ref>` 查看 Playwright 正在以什么为目标
4. 如果页面行为异常：
   - `openclaw browser errors --clear`
   - `openclaw browser requests --filter api --clear`
5. 进行深度调试：记录跟踪：
...
- `browser act kind=evaluate` / `openclaw browser evaluate` 和 `wait --fn`
  在页面上下文中执行任意 JavaScript。提示注入可能会操纵这一点。
...
对于 WSL2 Gateway(网关) + Windows Chrome 分宿主设置，请参阅
...
- `browser` 接受：
  ...
  - `target` (`sandbox` | `host` | `node`) 选择浏览器的运行位置。
