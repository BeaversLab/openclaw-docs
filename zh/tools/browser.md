...
- 内置的 `user` 配置文件会附加到您真实登录的 Chrome 会话；
...
- 当现有的登录会话很重要并且用户
  在计算机前以点击/批准任何附加提示时，首选 `profile="user"`。
- 仅当用户明确需要 Chrome 扩展程序/工具栏按钮附加流程时，才使用 `profile="chrome-relay"`。
- 当您需要特定的浏览器模式时，`profile` 是显式覆盖项。
...
- `attachOnly: true` 意味着“从不启动本地浏览器；仅当它已经在运行时才附加。”
- `color` + 每个配置文件的 `color` 会对浏览器 UI 进行着色，以便您可以查看哪个配置文件处于活动状态。
- 默认配置文件是 `openclaw`（由 OpenClaw 管理的独立浏览器）。使用 `defaultProfile: "user"` 以选择加入已登录用户的浏览器，或使用 `defaultProfile: "chrome-relay"` 进行扩展中继。
- 自动检测顺序：如果是基于 Chromium 的，则为系统默认浏览器；否则为 Chrome → Brave → Edge → Chromium → Chrome Canary。
...
- **远程控制（节点主机）：** 在拥有浏览器的机器上运行节点主机；Gateway(网关) 将浏览器操作代理到它。
...
OpenClaw 会在调用 `/json/*` 端点和连接到 CDP WebSocket 时保留身份验证。对于令牌，请优先使用环境变量或密钥管理器，而不是将其提交到配置文件中。

## 节点浏览器代理（零配置默认值）

如果您在装有浏览器的机器上运行 **节点主机**，OpenClaw 可以将浏览器工具调用自动路由到该节点，而无需任何额外的浏览器配置。这是远程网关的默认路径。
...
[Browserless](https://browserless.io) 是一项托管 Chromium 服务，通过 HTTPS 公开 CDP 端点。您可以将 OpenClaw 浏览器配置文件指向 Browserless 区域端点，并使用您的 API 密钥进行身份验证。
...
某些托管浏览器服务公开 **直接 WebSocket** 端点，而不是基于标准的 HTTP CDP 发现 (`/json/version`)。OpenClaw 支持这两种方式：

- **HTTP(S) 端点**（例如 Browserless）— OpenClaw 调用 `/json/version` 来发现 WebSocket 调试器 URL，然后进行连接。
- **WebSocket 端点** (`ws://` / `wss://`) — OpenClaw 直接连接，跳过 `/json/version`。将其用于 [Browserbase](https://www.browserbase.com) 等服务或任何向您提供 WebSocket URL 的提供商。

### Browserbase

[Browserbase](https://www.browserbase.com) 是一个运行无头浏览器的云平台，具有内置的 CAPTCHA 求解、隐身模式和住宅代理。
...
- Browserbase 会在 WebSocket 连接时自动创建浏览器会话，因此无需手动执行创建会话的步骤。
- 免费版允许一个并发会话和每月一个浏览器小时。请参阅 [pricing](https://www.browserbase.com/pricing) 了解付费计划的限制。
- 请参阅 [Browserbase docs](https://docs.browserbase.com) 以获取完整的 API 参考、SDK 指南和集成示例。
...
- 将 Gateway(网关) 和任何节点主机保持在私有网络 (Tailscale) 上；避免公开暴露。
...
- 现有会话配置文件是可选的；请使用 `--driver existing-session` 创建它们。
...
- 本地 **relay server** 在 loopback `cdpUrl` 上监听（默认：`http://127.0.0.1:18792`）。
- 点击标签页上的 **OpenClaw Browser Relay** 扩展图标以进行连接（它不会自动连接）。
- 代理通过选择正确的配置文件，使用普通的 `browser` 工具控制该标签页。

如果 Gateway(网关) 运行在其他地方，请在浏览器机器上运行节点主机，以便 Gateway(网关) 可以代理浏览器操作。

### 沙箱隔离会话

如果代理会话是沙箱隔离的，`browser` 工具可能会默认使用 `target="sandbox"` (sandbox browser)。
Chrome extension relay 接管需要主机浏览器控制，因此：
...
- 固定该扩展，然后在您想要控制的标签页上点击它（徽章显示 `ON`）。
...
- 再次点击扩展图标以分离连接。
- 代理使用：对于已登录的站点，首选使用 `profile="user"`。仅当您特别想要扩展流程时才使用 `profile="chrome-relay"`。
  用户必须在场以点击扩展并连接标签页。

## 通过 MCP 使用 Chrome 现有会话

OpenClaw 还可以通过官方 Chrome DevTools MCP 服务器附加到正在运行的 Chrome 配置文件。这将重用该 Chrome 配置文件中已打开的标签页和登录状态。
...
可选：如果您需要不同的名称或颜色，请创建您自己的自定义现有会话配置文件。
...
- Chrome 已显示并且您接受了附加同意提示

Agent 用法：

- 当您需要用户的已登录浏览器状态时，请使用 `profile="user"`。
- 如果您使用自定义现有会话配置文件，请传递该明确的配置文件名称。
- 除非用户明确需要扩展程序 / 附加标签页流程，否则首选 `profile="user"` 而非 `profile="chrome-relay"`。
...
- OpenClaw 在此处使用官方 Chrome DevTools MCP `--autoConnect` 流程，而不是旧版的默认配置文件远程调试端口工作流。
- 现有会话屏幕截图支持页面捕获和来自快照的 `--ref` 元素捕获，但不支持 CSS `--element` 选择器。
- 现有会话 `wait --url` 支持与其他浏览器驱动程序类似的精确、子字符串和 glob 模式。尚不支持 `wait --load networkidle`。
- 某些功能仍需要扩展程序中继或托管浏览器路径，例如 PDF 导出和下载拦截。
- 默认情况下，将中继仅限于回环。如果中继必须能够从不同的网络命名空间（例如 Gateway(网关) 中的 WSL2，Windows 上的 Chrome）访问，请将 `browser.relayBindHost` 设置为显式绑定地址（例如 `0.0.0.0`），同时保持周围网络的私密性和身份验证。
...
您可以使用 `browser.executablePath` 覆盖设置。
...
Playwright 包（而非 `playwright-core`）并重启 Gateway，或者重新安装
支持浏览器的 OpenClaw。
...
`OPENCLAW_HOME_VOLUME` 或绑定挂载进行持久化。参见 [Docker](/en/install/docker)。
...
- `snapshot`：
  ...
  - 配置默认值（仅限工具/CLI）：设置 `browser.snapshotDefaults.mode: "efficient"` 以在调用方未传递模式时使用高效快照（参见 [Gateway(网关) configuration](/en/gateway/configuration#browser-openclaw-managed-browser)）。
  - 角色快照选项（`--interactive`、`--compact`、`--depth`、`--selector`）强制生成带有类似 `ref=e12` 引用的基于角色的快照。
  - `--frame "<iframe selector>"` 将角色快照范围限定在 iframe（与类似 `e12` 的角色引用配对）。
...
- **角色快照（类似 `e12` 的角色 refs）**：`openclaw browser snapshot --interactive`（或 `--compact`，`--depth`，`--selector`，`--frame`）
  - 输出：包含 `[ref=e12]`（以及可选 `[nth=1]`）的基于角色的列表/树。
...
- 如果使用 `--frame` 获取了角色快照，角色 ref 将被限定在该 iframe 内，直到下一次角色快照。
...
JSON 中的角色快照包含 `refs` 以及一个小的 `stats` 块（行/字符/引用/交互），以便工具可以推断有效负载的大小和密度。

## 状态和环境调节

这些对于“让网站像 X 一样行为”的工作流很有用：

- Cookies：`cookies`、`cookies set`、`cookies clear`
- Storage：`storage local|session get|set|clear`
- Offline：`set offline on|off`
- Headers：`set headers --headers-json '{"X-Debug":"1"}'`（传统的 `set headers --json '{"X-Debug":"1"}'` 仍然受支持）
...
- `browser act kind=evaluate` / `openclaw browser evaluate` 和 `wait --fn`
  ...
  这种行为。如果不需要，请使用 `browser.evaluateEnabled=false` 禁用它。
- 有关登录和反机器人说明（X/Twitter 等），请参阅 [Browser login + X/Twitter posting](/en/tools/browser-login)。
- 请确保 Gateway(网关)/节点主机保持私密（仅限回环或 tailnet 访问）。
