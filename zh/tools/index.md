...
请参阅 [Plugins](/en/tools/plugin) 了解安装和配置，参阅 [Skills](/en/tools/skills) 了解
...
- [LLM Task](/en/tools/llm-task)：用于结构化工作流输出的仅限 JSON 的 LLM 步骤（可选架构验证）。
- [Diffs](/en/tools/diffs)：用于文本前后对比或统一补丁的只读差异查看器和 PNG 或 PDF 文件渲染器。
...
- 网关/节点 批准和允许列表：[Exec approvals](/en/tools/exec-approvals)。
...
- 响应会被缓存（默认 15 分钟）。
- 请参阅 [Web tools](/en/tools/web) 了解设置方法。
...
- 请参阅 [Web tools](/en/tools/web) 了解设置方法。
- 请参阅 [Firecrawl](/en/tools/firecrawl) 了解可选的反机器人回退方案。
...
- `send` 通过网关路由 WhatsApp；其他通道直接连接。
- `poll` 对 WhatsApp 和 MS Teams 使用网关；Discord 轮询直接连接。
...
- `sessions_spawn` 支持 `runtime: "subagent" | "acp"`（默认为 `subagent`）。有关 ACP 运行时行为，请参阅 [ACP 代理](/en/tools/acp-agents)。
...
- 故障排除指南：
  ...
  - WSL2 网关 + Windows 远程 Chrome CDP：[WSL2 + Windows + 远程 Chrome CDP 故障排除](/en/tools/browser-wsl2-windows-remote-cdp-troubleshooting)
