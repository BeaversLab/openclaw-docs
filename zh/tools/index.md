...
插件可以在核心集之外注册**其他工具**（和 CLI 命令）。请参阅 [插件](/en/tools/plugin) 了解安装和配置，参阅 [技能](/en/tools/skills) 了解工具使用指南是如何注入到提示词中的。某些插件会随工具一起附带自己的技能（例如语音通话插件）。

可选插件工具：

- [Lobster](/en/tools/lobster)：具有可恢复审批的类型化工作流运行时（需要网关主机上安装 Lobster CLI）。
- [LLM Task](/en/tools/llm-task)：用于结构化工作流输出的仅 JSON LLM 步骤（可选架构验证）。
- [Diffs](/en/tools/diffs)：用于前后文本或统一补丁的只读差异查看器和 PNG 或 PDF 文件渲染器。
...
- 请参阅 [Web 工具](/en/tools/web) 了解设置。
- 请参阅 [Firecrawl](/en/tools/firecrawl) 了解可选的反机器人回退。
...
- 当现有的登录/cookie 很重要且用户在场以点击/批准任何附加提示时，使用 `profile="user"` 来连接真实的本地主机浏览器。
- 仅将 `profile="chrome-relay"` 用于 Chrome 扩展程序/工具栏按钮附加流程。
- `profile="user"` 和 `profile="chrome-relay"` 仅限主机；请勿将它们与沙盒/节点目标结合使用。
...
- `snapshot` 也支持角色快照选项（`interactive`、`compact`、`depth`、`selector`），它们返回类似 `e12` 的引用。
- `act` 需要来自 `snapshot` 的 `ref`（来自 AI 快照的数字 `12`，或来自角色快照的 `e12`）；对于罕见的 CSS 选择器需求，请使用 `evaluate`。
- 默认情况下避免 `act` → `wait`；仅在特殊情况下使用（没有可靠的 UI 状态可等待）。
- `upload` 可以选择性地传递 `ref` 以在准备就绪后自动点击。
...
- 仅当配置了 `agents.defaults.imageModel`（主备或后备），或者可以从默认模型 + 配置的身份验证推断出隐式图像模型时才可用（尽力而为的配对）。
- 直接使用图像模型（独立于主聊天模型）。
...
有关完整的行为、限制、配置和示例，请参阅 [PDF 工具](/en/tools/pdf)。
...
- `poll` 针对 Gateway(网关) 和 MS Teams 使用 WhatsApp；Discord 投票直接连接。
- 当消息工具调用绑定到活跃的聊天会话时，发送操作将限制在该会话的目标，以避免跨上下文泄露。

### `cron`

管理 Gateway(网关) 的 cron 作业和唤醒。
...
- `add` 需要一个完整的 cron 作业对象（与 `cron.add` RPC 的架构相同）。
- `update` 使用 `{ jobId, patch }`（`id` 为兼容性接受）。

### `gateway`

重启或将更新应用于正在运行的 Gateway(网关) 进程（就地）。
...
- `config.schema.lookup` 需要特定的配置路径，例如 `gateway.auth` 或 `agents.list.*.heartbeat`。
- 在访问 `plugins.entries.<id>` 时，路径可能包含用斜杠分隔的插件 ID，例如 `plugins.entries.pack/one.config`。
- 使用 `delayMs`（默认为 2000）以避免中断正在进行的回复。
- `config.schema` 仍然可供内部控制 UI 流程使用，且不通过代理 `gateway` 工具公开。
...
- `main` 是规范的直接聊天键；global/unknown 是隐藏的。
- `messageLimit > 0` 获取每个会话的最后 N 条消息（工具消息已过滤）。
- 会话定位由 `tools.sessions.visibility` 控制（默认为 `tree`：当前会话 + 生成的子代理会话）。如果您为多个用户运行共享代理，请考虑设置 `tools.sessions.visibility: "self"` 以防止跨会话浏览。
- 当 `timeoutSeconds > 0` 时，`sessions_send` 会等待最终完成。
- 交付/公告在完成后进行，且尽力而为；`status: "ok"` 确认代理运行已完成，而不是公告已交付。
- `sessions_spawn` 支持 `runtime: "subagent" | "acp"`（默认为 `subagent`）。有关 ACP 运行时行为，请参阅 [ACP Agents](/en/tools/acp-agents)。
- 对于 ACP 运行时，`streamTo: "parent"` 将初始运行的进度摘要作为系统事件（而不是直接子级交付）路由回请求者会话。
- `sessions_spawn` 启动子代理运行，并将公告回复发布回请求者聊天。
  - 支持一次性模式（`mode: "run"`）和持久线程绑定模式（带有 `thread: true` 的 `mode: "session"`）。
  ...
  - 回复格式包括 `Status`、`Result` 和紧凑的统计信息。
  - `Result` 是助手完成文本；如果缺失，则使用最新的 `toolResult` 作为后备。
- 手动完成模式首先直接发送生成，并在临时故障时进行队列后备和重试（`status: "ok"` 表示运行完成，而非公告已送达）。
- `sessions_spawn` 仅支持子代理运行时的内联文件附件（ACP 会拒绝它们）。每个附件具有 `name`、`content` 和可选的 `encoding`（`utf8` 或 `base64`）以及 `mimeType`。文件被具象化到 `.openclaw/attachments/<uuid>/` 的子工作区中，并附带 `.manifest.json` 元数据文件。该工具返回包含 `count`、`totalBytes`、每个文件的 `sha256` 和 `relDir` 的回执。附件内容会从文字记录持久化中自动编辑。
...
- `sessions_send` 运行回复式乒乓操作（回复 `REPLY_SKIP` 以停止；最大回合数通过 `session.agentToAgent.maxPingPongTurns` 设置，0–5）。
- 在乒乓操作之后，目标代理运行一个**通告步骤（announce step）**；回复 `ANNOUNCE_SKIP` 以抑制该通告。
...
注意：当设置了 `gatewayUrl` 时，必须显式包含 `gatewayToken`。工具不会继承配置或环境凭据用于覆盖设置，缺少显式凭据将被视为错误。
...
- 故障排除指南：
  ...
  - WSL2 Gateway + Windows 远程 Chrome CDP：[WSL2 + Windows + 远程 Chrome CDP 故障排除](/en/tools/browser-wsl2-windows-remote-cdp-troubleshooting)
...
这意味着智能体既能看到“存在哪些工具”，也能看到“如何调用它们”。如果工具未出现在系统提示或架构中，模型将无法调用它。
