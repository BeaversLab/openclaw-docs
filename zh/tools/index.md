...
插件可以在核心集之外注册**其他工具**（以及 CLI 命令）。请参阅[插件](/zh/tools/plugin)了解安装和配置，参阅[Skills](/zh/tools/skills)了解如何将工具使用指南注入到提示词中。某些插件会随工具一起提供自己的技能（例如语音通话插件）。

可选插件工具：

- [Lobster](/zh/tools/lobster)：具有可恢复审批功能的类型化工作流运行时（需要在网关主机上安装 Lobster CLI）。
- [LLM Task](/zh/tools/llm-task)：仅限 LLM 的步骤，用于结构化工作流输出（可选架构验证）。
- [Diffs](/zh/tools/diffs)：用于显示之前/之后文本或统一补丁的只读差异查看器和 PNG 或 PDF 文件渲染器。
...
For full behavior, limits, config, and examples, see [PDF 工具](/zh/tools/pdf).
...
- `sessions_spawn` 支持 `runtime: "subagent" | "acp"` (`subagent` 默认)。有关 ACP 运行时行为，请参阅 [ACP Agents](/zh/tools/acp-agents)。
