...
插件可以注册核心集之外的**其他工具**（以及 CLI 命令）。请参阅 [插件](/zh/tools/plugin) 了解安装和配置，以及 [Skills](/zh/tools/skills) 了解工具使用指导如何注入到提示词中。一些插件随工具附带自己的 skills（例如，语音通话插件）。

可选插件工具：

- [Lobster](/zh/tools/lobster)：具有可恢复审批功能的类型化工作流运行时（需要在网关主机上安装 Lobster CLI）。
- [LLM Task](/zh/tools/llm-task)：仅限 JSON 的 LLM 步骤，用于结构化工作流输出（可选架构验证）。
...
- 网关/节点审批和允许列表：[Exec approvals](/zh/tools/exec-approvals)。
...
- 响应会被缓存（默认 15 分钟）。
- 请参阅 [Web tools](/zh/tools/web) 了解设置。
...
- 请参阅 [Web tools](/zh/tools/web) 了解设置。
- 请参阅 [Firecrawl](/zh/tools/firecrawl) 了解可选的反机器人回退方案。
...
- `profile="user"` 仅限主机；不要将其与沙盒/节点目标组合使用。
...
- 如果连接了具有浏览器功能的节点，工具可能会自动路由到该节点（除非您固定 `target`）。
- 安装 Playwright 后，`snapshot` 默认为 `ai`；使用 `aria` 获取辅助功能树。
- `snapshot` 还支持角色快照选项（`interactive`、`compact`、`depth`、`selector`），它们返回类似 `e12` 的引用。
- `act` 需要 `snapshot` 中的 `ref`（来自 AI 快照的数字 `12`，或来自角色快照的 `e12`）；请使用 `evaluate` 来满足罕见的 CSS 选择器需求。
- 默认情况下避免使用 `act` → `wait`；仅在特殊情况下（没有可靠的 UI 状态可供等待）使用。
...
- A2UI 仅限 v0.8（没有 `createSurface`）；CLI 会拒绝带有行错误的 v0.9 JSONL。
...
- 仅在配置了 `agents.defaults.imageModel`（主要或备用）时可用，或者当可以从默认模型 + 配置的身份验证推断出隐式图像模型时（尽力配对）。
- 直接使用图像模型（独立于主聊天模型）。
...
- `send` 通过 WhatsApp 路由 Gateway(网关)；其他通道直接发送。
- `poll` 对 Gateway(网关) 和 MS Teams 使用 WhatsApp；Discord 轮询直接发送。
...
- `add` 期望一个完整的 cron 任务对象（与 `cron.add` RPC 的模式相同）。
- `update` 使用 `{ jobId, patch }`（为兼容性接受 `id`）。

### `gateway`

重新启动或将更新应用于正在运行的 Gateway(网关) 进程（就地）。
...
- 在寻址 `plugins.entries.<id>` 时，路径可能包含以斜杠分隔的插件 ID，例如 `plugins.entries.pack/one.config`。
- 使用 `delayMs`（默认为 2000）以避免中断正在进行的回复。
- `config.schema` 对内部控制 UI 流仍然可用，且不通过代理 `gateway` 工具暴露。
- `restart` 默认处于启用状态；设置 `commands.restart: false` 以将其禁用。

### `sessions_list` / `sessions_history` / `sessions_send` / `sessions_spawn` / `session_status`

列出会话，检查历史记录，或发送到另一个会话。
...
- `main` 是规范直接对话键；全局/未知项被隐藏。
- `messageLimit > 0` 获取每个会话的最后 N 条消息（过滤了工具消息）。
- 会话定位由 `tools.sessions.visibility` 控制（默认 `tree`：当前会话 + 生成的子代理会话）。如果您运行供多个用户使用的共享代理，请考虑设置 `tools.sessions.visibility: "self"` 以防止跨会话浏览。
- 当 `timeoutSeconds > 0` 时，`sessions_send` 等待最终完成。
- 交付/通知在完成后发生且为尽力而为；`status: "ok"` 确认代理运行已结束，而非通知已交付。
- `sessions_spawn` 支持 `runtime: "subagent" | "acp"`（默认 `subagent`）。关于 ACP 运行时行为，请参阅 [ACP Agents](/zh/tools/acp-agents)。
- 对于 ACP 运行时，`streamTo: "parent"` 将初始运行进度摘要作为系统事件而非直接子交付路由回请求者会话。
...
- 手动完成模式首先直接生成发送，队列作为后备并在暂时性故障时重试（`status: "ok"` 意味着运行完成，而非公告已送达）。
- `sessions_spawn` 仅支持子代理运行时的内联文件附件（ACP 会拒绝它们）。每个附件具有 `name`、`content` 和可选的 `encoding`（`utf8` 或 `base64`）以及 `mimeType`。文件在 `.openclaw/attachments/<uuid>/` 处被具体化到子工作区中，并带有一个 `.manifest.json` 元数据文件。该工具返回包含 `count`、`totalBytes`、每个文件的 `sha256` 和 `relDir` 的回执。附件内容会从脚本持久化中自动编辑。
...
- ACP `streamTo: "parent"` 响应可能包含 `streamLogPath`（会话作用域 `*.acp-stream.jsonl`）用于追踪进度历史。
- `sessions_send` 运行一个回复式乒乓（回复 `REPLY_SKIP` 以停止；通过 `session.agentToAgent.maxPingPongTurns` 设置最大轮次，0–5）。
- 在乒乓之后，目标代理运行一个 **announce step**（公告步骤）；回复 `ANNOUNCE_SKIP` 以抑制公告。
- 沙箱限制：当当前会话处于沙箱隔离状态且为 `agents.defaults.sandbox.sessionToolsVisibility: "spawned"` 时，OpenClaw 会将 `tools.sessions.visibility` 限制为 `tree`。
...
由 Gateway(网关) 支持的工具（`canvas`，`nodes`，`cron`）：

- `gatewayUrl`（默认为 `ws://127.0.0.1:18789`）
- `gatewayToken`（如果启用了身份验证）
- `timeoutMs`

注意：当设置了 `gatewayUrl` 时，必须显式包含 `gatewayToken`。工具不会为覆盖项继承配置或环境凭据，缺少显式凭据将导致错误。
...
- 避免直接使用 `system.run`；仅在获得明确用户同意时才使用 `nodes` → `run`。
- 尊重用户对摄像头/屏幕采集的同意。
...
1. **系统提示文本**：人类可读的列表 + 指导。
2. **工具架构 (Tool schema)**：发送到模型 API 的结构化函数定义。

这意味着智能体能同时看到“存在哪些工具”以及“如何调用它们”。如果工具未出现在系统提示或架构中，模型将无法调用它。
