...
- 如果连接了支持浏览器的节点，该工具可能会自动路由到该节点（除非你固定了 `target`）。
- 当安装了 Playwright 时，`snapshot` 默认为 `ai`；使用 `aria` 获取辅助功能树。
- `snapshot` 还支持角色快照选项 (`interactive`, `compact`, `depth`, `selector`)，它们返回类似 `e12` 的引用。
- `act` 需要 `ref` 来自 `snapshot`（来自 AI 快照的数字 `12`，或来自角色快照的 `e12`）；对于罕见的 CSS 选择器需求，请使用 `evaluate`。
- 默认情况下避免 `act` → `wait`；仅在特殊情况下使用它（没有可靠的 UI 状态可等待）。
- `upload` 可以选择传递 `ref` 以在准备好后自动点击。
- `upload` 也支持 `inputRef` (aria ref) 或 `element` (CSS selector) 来直接设置 `<input type="file">`。

### `canvas`

Canvas 驱动节点Canvas（present, eval, snapshot, A2UI）。
...
- A2UI 仅支持 v0.8（没有 `createSurface`）；CLI 会拒绝带有行错误的 v0.9 JSONL。
...
- 仅当配置了 `agents.defaults.imageModel` (主要或回退) 时可用，或者当可以从默认模型 + 配置的身份验证中推断出隐式图像模型时 (尽力配对)。
- 直接使用图像模型（独立于主聊天模型）。
...
- `send` 通过 Gateway 网关 路由 WhatsApp；其他通道直接连接。
- `poll` 对 WhatsApp 和 MS Teams 使用 Gateway 网关；Discord 轮询直接连接。
...
- `config.schema.lookup` 需要一个特定的配置路径，例如 `gateway.auth` 或 `agents.list.*.heartbeat`。
- 在定位 `plugins.entries.<id>` 时，路径可能包含以斜杠分隔的插件 ID，例如 `plugins.entries.pack/one.config`。
- 使用 `delayMs`（默认为 2000）以避免中断正在进行的回复。
- `config.schema` 仍然可供内部 Control UI 流程使用，并且不通过代理 `gateway` 工具暴露。
...
- 交付/公告在完成后发生，并采用尽力而为的方式；`status: "ok"` 确认代理运行已完成，而非公告已送达。
- `sessions_spawn` 支持 `runtime: "subagent" | "acp"`（默认为 `subagent`）。有关 ACP 运行时行为，请参阅 [ACP 代理](/zh/en/tools/acp-agents)。
- 对于 ACP 运行时，`streamTo: "parent"` 将初始运行的进度摘要作为系统事件路由回请求者会话，而不是通过直接的子级交付。
- `sessions_spawn` 启动子代理运行，并将公告回复发布回请求者聊天。
  - 支持单次运行模式（`mode: "run"`）和持久线程绑定模式（带有 `thread: true` 的 `mode: "session"`）。
  - 如果省略 `thread: true` 和 `mode`，模式默认为 `session`。
  - `mode: "session"` 需要 `thread: true`。
  - 如果省略 `runTimeoutSeconds`，OpenClaw 会在设置时使用 `agents.defaults.subagents.runTimeoutSeconds`；否则超时默认为 `0`（无超时）。
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.openclaw/attachments/<uuid>/` with a `.manifest.json` metadata file. The 工具 returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- ACP `streamTo: "parent"` responses may include `streamLogPath` (会话-scoped `*.acp-stream.jsonl`) for tailing progress history.
- `sessions_send` runs a reply‑back ping‑pong (reply `REPLY_SKIP` to stop; max turns via `session.agentToAgent.maxPingPongTurns`, 0–5).
- After the ping‑pong, the target agent runs an **announce step**; reply `ANNOUNCE_SKIP` to suppress the announcement.
- 沙箱 clamp: when the current 会话 is 沙箱隔离 and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OpenClaw clamps `tools.sessions.visibility` to `tree`.
...
- 结果受限于各代理的允许列表 (`agents.list[].subagents.allowAgents`)。
...
注意：当设置了 `gatewayUrl` 时，必须显式包含 `gatewayToken`。工具不会继承配置或环境凭证作为覆盖值，缺少显式凭证会导致错误。
...
- 故障排除指南：
 - Linux 启动/CDP 问题：[浏览器故障排除 (Linux)](/zh/en/tools/browser-linux-故障排除) - WSL2 Gateway 网关 + Windows 远程 Chrome CDP：[WSL2 + Windows + 远程 Chrome CDP 故障排除](/zh/en/tools/browser-wsl2-windows-remote-cdp-故障排除)
