...
请参阅 [Plugins](/en/tools/plugin) 了解安装和配置，并参阅 [Skills](/en/tools/skills) 了解如何将
工具使用指导注入到提示中。一些插件随工具附带自己的 Skills
（例如，语音通话插件）。

可选插件工具：

- [Lobster](/en/tools/lobster)：具有可恢复审批的类型化工作流运行时（需要在网关主机上安装 Lobster CLI）。
- [LLM Task](/en/tools/llm-task)：用于结构化工作流输出的纯 JSON LLM 步骤（可选架构验证）。
- [Diffs](/en/tools/diffs)：用于之前/之后文本或统一补丁的只读差异查看器和 PNG 或 PDF 文件渲染器。
...
- 网关/节点审批和允许列表：[执行审批](/en/tools/exec-approvals)。
...
- 有关设置，请参阅 [Web 工具](/en/tools/web)。
- 有关可选的反机器人回退，请参阅 [Firecrawl](/en/tools/firecrawl)。
...
- `snapshot` 还支持角色快照选项（`interactive`、`compact`、`depth`、`selector`），这些选项返回类似 `e12` 的引用。
- `act` 需要来自 `snapshot` 的 `ref`（来自 AI 快照的数字 `12`，或来自角色快照的 `e12`）；对于罕见的 CSS 选择器需求，请使用 `evaluate`。
- 默认情况下避免 `act` → `wait`；仅在特殊情况下使用它（没有可靠的 UI 状态可供等待）。
- `upload` 可以选择传递一个 `ref`，以便在布防后自动点击。
...
- 底层使用网关 `node.invoke`。
- 如果未提供 `node`，该工具将选择默认值（单个连接节点或本地 mac 节点）。
- A2UI 仅支持 v0.8（没有 `createSurface`）；如果 v0.9 JSONL 出现行错误，CLI 会拒绝它。
...
- 仅当配置了 `agents.defaults.imageModel`（主要或后备）时，或者当可以从默认模型 + 配置的身份验证推断出隐式图像模型时（尽力而为的配对），才可用。
- 直接使用图像模型（独立于主聊天模型）。
...
有关完整行为、限制、配置和示例，请参阅 [PDF 工具](/en/tools/pdf)。
...
- `send` 通过 Gateway(网关) 路由 WhatsApp；其他渠道直接连接。
- `poll` 针对 WhatsApp 和 MS Teams 使用 Gateway(网关)；Discord 轮询直接进行。
- 当消息工具调用绑定到活动的聊天会话时，发送操作将受限于该会话的目标，以避免跨上下文泄露。

### `cron`

管理 Gateway(网关) 的定时任务和唤醒。
...
- `add` 期望一个完整的 cron 任务对象（与 `cron.add` RPC 具有相同的架构）。
- `update` 使用 `{ jobId, patch }`（出于兼容性接受 `id`）。

### `gateway`

重启或就地应用更新到正在运行的 Gateway(网关) 进程。

核心操作：

- `restart`（授权并发送 `SIGUSR1` 以进行进程内重启；`openclaw gateway` 就地重启）
...
- `config.schema.lookup` 期望一个定向的配置路径，例如 `gateway.auth` 或 `agents.list.*.heartbeat`。
- 在处理 `plugins.entries.<id>` 时，路径可能包含以斜杠分隔的插件 ID，例如 `plugins.entries.pack/one.config`。
- 使用 `delayMs`（默认为 2000）以避免中断正在进行的回复。
- `config.schema` 仍然可用于内部控制 UI 流，并且不通过代理 `gateway` 工具暴露。
...
- Delivery/announce 发生在完成后，是尽力而为的；`status: "ok"` 确认的是代理运行已结束，而非 announce 已送达。
- `sessions_spawn` 支持 `runtime: "subagent" | "acp"`（默认为 `subagent`）。有关 ACP 运行时行为，请参阅 [ACP Agents](/en/tools/acp-agents)。
- 对于 ACP 运行时，`streamTo: "parent"` 将初始运行进度摘要作为系统事件路由回请求者会话，而不是直接通过子级传送。
- `sessions_spawn` 启动子代理运行，并将 announce 回复发回至请求者聊天。
  - 支持单次模式（`mode: "run"`）和持久线程绑定模式（`mode: "session"` 搭配 `thread: true`）。
  ...
  - Discord 线程绑定流取决于 `session.threadBindings.*` 和 `channels.discord.threadBindings.*`。
  - 回复格式包括 `Status`、`Result` 和紧凑统计信息。
  - `Result` 是助手完成文本；如果缺失，则使用最新的 `toolResult` 作为后备。
- 手动完成模式首先直接发送 spawn，并在暂时性失败时通过队列回退和重试（`status: "ok"` 表示运行已完成，而非 announce 已送达）。
- `sessions_spawn` 仅支持子代理运行时的内联文件附件（ACP 会拒绝它们）。每个附件都有 `name`、`content` 和可选的 `encoding`（`utf8` 或 `base64`）以及 `mimeType`。文件被具象化到 `.openclaw/attachments/<uuid>/` 中的子工作区，并附带 `.manifest.json` 元数据文件。该工具返回一个包含 `count`、`totalBytes`、每个文件的 `sha256` 和 `relDir` 的回执。附件内容会从对话记录持久化中自动编辑掉。
  - 通过 `tools.sessions_spawn.attachments` 配置限制（`enabled`、`maxTotalBytes`、`maxFiles`、`maxFileBytes`、`retainOnSessionKeep`）。
  - `attachAs.mountPath` 是为未来的挂载实现保留的提示。
- `sessions_spawn` 是非阻塞的，并立即返回 `status: "accepted"`。
- ACP `streamTo: "parent"` 响应可能包含 `streamLogPath`（会话范围的 `*.acp-stream.jsonl`）用于跟踪进度历史。
- `sessions_send` 运行回传乒乓（reply `REPLY_SKIP` 以停止；通过 `session.agentToAgent.maxPingPongTurns` 设置最大轮数，0–5）。
- 在乒乓之后，目标代理运行一个 **announce step**；回复 `ANNOUNCE_SKIP` 以抑制公告。
...
- 结果限制于每个代理的允许列表（`agents.list[].subagents.allowAgents`）。
- 当配置了 `["*"]` 时，该工具包括所有配置的代理并标记 `allowAny: true`。

## 参数（通用）

Gateway(网关) 网关支持的工具（`canvas`、`nodes`、`cron`）：

- `gatewayUrl`（默认为 `ws://127.0.0.1:18789`）
- `gatewayToken`（如果启用了身份验证）
- `timeoutMs`

注意：当设置了 `gatewayUrl` 时，必须显式包含 `gatewayToken`。工具不会继承用于覆盖的配置或环境凭据，缺少显式凭据将导致错误。
...
- 避免直接使用 `system.run`；仅在获得用户明确同意的情况下使用 `nodes` → `run`。
- 尊重用户对相机/屏幕录制的同意。
- 在调用媒体命令之前，请使用 `status/describe` 来确保已获得权限。
...
这意味着代理既能看到“存在哪些工具”，也能看到“如何调用它们”。如果某个工具未出现在系统提示词或模式中，模型将无法调用它。
