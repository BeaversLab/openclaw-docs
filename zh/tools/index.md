...
请参阅 [Plugins](/zh/tools/plugin) 了解安装和配置，并参阅 [Skills](/zh/tools/skills) 了解如何将
...
- [LLM Task](/zh/tools/llm-task)：用于结构化工作流输出的纯 JSON LLM 步骤（可选架构验证）。
- [Diffs](/zh/tools/diffs)：用于之前/之后文本或统一补丁的只读差异查看器和 PNG 或 PDF 文件渲染器。
...
- 有关设置，请参阅 [Web 工具](/zh/tools/web)。
- 有关可选的反机器人回退，请参阅 [Firecrawl](/zh/tools/firecrawl)。
...
- `send` 通过 WhatsApp 路由 Gateway(网关)；其他渠道直接连接。
...
- `sessions_spawn` 支持 `runtime: "subagent" | "acp"`（默认为 `subagent`）。有关 ACP 运行时行为，请参阅 [ACP Agents](/zh/tools/acp-agents)。
