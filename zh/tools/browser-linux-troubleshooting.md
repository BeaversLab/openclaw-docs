---
summary: "修复在 Brave 上使用 OpenClaw 浏览器控制时的 Chrome/Linux/Edge/Chromium CDP 启动问题"
read_when: "在 Linux 上浏览器控制失败，特别是使用 snap Chromium 时"
title: "浏览器故障排除"
---
...
在 Ubuntu（以及许多 Linux 发行版）上，默认安装的 Chromium 是一个 **snap package**。Snap 的 AppArmor 沙箱隔离会干扰 OpenClaw 生成和监控浏览器进程的方式。
...
安装官方的 Google Chrome `.deb` 包，它不受 snap 的沙箱隔离限制：
...
| `browser.executablePath` | 基于 Chromium 的浏览器二进制文件路径 (Chrome/Brave/Edge/Chromium) | 自动检测（当默认浏览器基于 Chromium 时优先选择） |
...
2. **使用 Chrome MCP：** 确保本地 Chrome 正在运行并且至少打开了一个标签页，然后使用 `--browser-profile user` 重试。

注意：

- `user` 仅限主机使用。对于 Linux 服务器、容器或远程主机，建议使用 CDP 配置文件。
