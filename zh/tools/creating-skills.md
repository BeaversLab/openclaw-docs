...
在该目录中创建一个 `SKILL.md` 文件。该文件使用 YAML frontmatter 作为元数据，并使用 Markdown 编写指令。
...
让你的代理“刷新技能”或重启网关。OpenClaw 将发现新目录并为 `SKILL.md` 建立索引。

## 最佳实践

- **保持简洁**：指示模型做_什么_，而不是如何做一个 AI。
- **安全第一**：如果你的技能使用了 `bash`，请确保提示词不允许来自不受信任用户输入的任意命令注入。
