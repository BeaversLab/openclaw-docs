...
3. 重启网关 (Gateway)，然后在 `plugins.entries.<id>.config` 下进行配置。

有关具体插件示例，请参阅 [Voice Call](/en/plugins/voice-call)。
...
OpenClaw 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema 代替。请参阅 [插件清单](/en/plugins/manifest)。
...
插件与 Gateway **同进程运行**，因此请将它们视为受信任的代码。工具编写指南：[插件 Agent 工具](/en/plugins/agent-tools)。
...
有关设置和用法，请参阅 [语音通话](/en/plugins/voice-call) 和 `extensions/voice-call/README.md`。
