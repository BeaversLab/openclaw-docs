---
summary: "通过 signal-cli (JSON-Signal + SSE) 提供 RPC 支持、设置路径和号码模型"
read_when:
  - 设置 Signal 支持
  - 调试 Signal 发送/接收
title: "Signal"
...
- 您的服务器上已安装 OpenClaw（以下 Linux 流程在 Ubuntu 24 上测试）。
- 运行 Gateway(网关) 的主机上可用 `signal-cli`。
...
3. 选择一种设置路径：
   ...
   - **路径 B（SMS 注册）：** 注册一个专用号码，需验证码 + SMS 验证。
4. 配置 OpenClaw 并重启 OpenClaw。
...
| `allowFrom` | 被允许发送私信的电话号码或 `uuid:<id>` 值                |
...
- Signal 连接到一个 **Signal 设备**（`signal-cli` 账户）。
- 如果您在**您的个人 Signal 账户**上运行机器人，它将忽略您自己的消息（循环保护）。
- 对于“我给机器人发短信，它回复”的情况，请使用**单独的机器人号码**。
...
多账户支持：使用 `channels.signal.accounts` 配置每个账户的配置和可选的 `name`。有关共享模式，请参阅 [`gateway/configuration`](/zh/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)。

## 设置路径 B：注册专用机器人号码（SMS、Linux）

当您想要一个专用的机器人号码而不是链接现有的 Signal 应用程序账户时，请使用此选项。
...
保持 `signal-cli` 更新；上游指出，随着 Signal 服务器 API 的更改，旧版本可能会损坏。
...
重要：使用 `signal-cli` 注册电话号码账户可能会取消该号码的主 Signal 应用程序会话的验证。首选专用的机器人号码，或者如果您需要保留现有的电话应用程序设置，请使用二维码链接模式。
...
- 仅 UUID 的发送者（来自 `sourceUuid`）将作为 `uuid:<id>` 存储在 `channels.signal.allowFrom` 中。

群组：

- `channels.signal.groupPolicy = open | allowlist | disabled`。
- 当设置了 `allowlist` 时，`channels.signal.groupAllowFrom` 控制谁可以在群组中触发操作。
- `channels.signal.groups["<group-id>" | "*"]` 可以使用 `requireMention`、`tools` 和 `toolsBySender` 覆盖群组行为。
- 在多账户设置中，使用 `channels.signal.accounts.<id>.groups` 进行每个账户的覆盖设置。
...
- **正在输入指示器**：OpenClaw 通过 `signal-cli sendTyping` 发送正在输入的信号，并在运行回复时进行刷新。
...
- `channels.signal.reactionLevel`：`off | ack | minimal | extensive`。
  - `off`/`ack` 禁用代理表情回应（消息工具 `react` 将会报错）。
  - `minimal`/`extensive` 启用代理表情回应并设置指导级别。
...
- `channels.signal.dmHistoryLimit`：私信（私信）历史记录限制（以用户轮次为单位）。按用户覆盖设置：`channels.signal.dms["<phone_or_uuid>"].historyLimit`。
- `channels.signal.textChunkLimit`：出站块大小（字符）。
- `channels.signal.chunkMode`：`length`（默认）或 `newline`，用于在按长度分块之前按空行（段落边界）拆分。
