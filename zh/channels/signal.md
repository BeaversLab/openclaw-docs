...
多账号支持：结合每个账号的配置及可选的 `name` 使用 `channels.signal.accounts`。有关共享模式，请参阅 [`gateway/configuration`](/en/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)。
