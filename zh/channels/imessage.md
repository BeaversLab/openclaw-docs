...
<Tip>
权限是按进程上下文授予的。如果网关以无头模式（LaunchAgent/SSH）运行，请在同一上下文中运行一次交互式命令以触发提示：
...
<Tabs>
  ...
  <Tab title="群组策略 + 提及">
    ...
    运行时说明：如果 `channels.imessage` 完全缺失，运行时将回退到 `groupPolicy="allowlist"` 并记录警告（即使 `channels.defaults.groupPolicy` 已设置）。
  ...
  <Tab title="会话和确定性回复">
    ...
    如果该 `chat_id` 在 `channels.imessage.groups` 下进行了显式配置，OpenClaw 会将其视为群组流量（群组筛选 + 群组会话隔离）。
...
iMessage 默认允许渠道发起的配置写入（用于当 `commands.config: true` 为 `/config set|unset` 时）。
...
```
  ...
  <Accordion title="Remote attachments fail">
    ...
    - 网关主机上的 `~/.ssh/known_hosts` 中是否存在主机密钥
    - 运行 Messages 的 Mac 上的远程路径可读性

  </Accordion>

  <Accordion title="macOS 权限提示被忽略">
...
```
    ...
    确认运行 OpenClaw/`imsg` 的进程上下文已授予“完全磁盘访问权限”+“自动化”权限。
