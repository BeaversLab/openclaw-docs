...
状态：支持文本和私信附件；渠道/群组文件发送需要 `sharePointSiteId` + Graph 权限（参见 [在群组聊天中发送文件](#sending-files-in-group-chats)）。投票通过 Adaptive Cards 发送。
...
如果您在安装过程中选择 Teams 并且检测到 git 检出，
...
注意：群组聊天默认被阻止（`channels.msteams.groupPolicy: "allowlist"`）。要允许群组回复，请设置 `channels.msteams.groupAllowFrom`（或使用 `groupPolicy: "open"` 允许任何成员，@提及限制）。
...
- 默认值：`channels.msteams.dmPolicy = "pairing"`。在批准之前，未知发送者将被忽略。
- `channels.msteams.allowFrom` 应使用稳定的 AAD 对象 ID。
- UPN/显示名称是可变的；默认情况下禁用直接匹配，仅在使用 `channels.msteams.dangerouslyAllowNameMatching: true` 时启用。
...
- 设置 `groupPolicy: "open"` 以允许任何成员（默认仍受 @提及限制）。
- 要允许**无渠道**，请设置 `channels.msteams.groupPolicy: "disabled"`。
...
- 当设置 `groupPolicy="allowlist"` 且存在团队允许列表时，仅接受列出的团队/渠道（@提及限制）。
- 配置向导接受 `Team/Channel` 条目并为您存储它们。
- 在启动时，OpenClaw 会将团队/渠道和用户允许列表的名称解析为 ID（当 Graph 权限允许时），并记录映射关系；未解析的团队/渠道名称将保留为输入的原始格式，但除非启用了 `channels.msteams.dangerouslyAllowNameMatching: true`，否则在路由时默认会被忽略。
...
5. 在 `~/.openclaw/openclaw.json`（或 环境变量）中配置 `msteams` 并启动网关。
...
1. 转到 [Create Azure Bot](https://portal.azure.com/#create/Microsoft.AzureBot)
2. 填写 ** Basics ** 选项卡：
...
5. 转到 **Overview** → 复制 **Directory (tenant) ID** → 这是您的 `tenantId`
...
2. 将 **消息传递终结点** 设置为你的 Webhook URL：
   ...
   - 本地开发：使用隧道（请参阅下方的 [Local Development](#local-development-tunneling)）
...
您可以不手动创建 ZIP 清单，而是使用 [Teams Developer Portal](https://dev.teams.microsoft.com/apps)：
...
6. **运行网关**
   - 安装插件并存在包含凭据的 `msteams` 配置时，Teams 渠道会自动启动。

## 历史记录上下文

- `channels.msteams.historyLimit` 控制将多少条最近的渠道/群组消息封装到提示词中。
- 回退到 `messages.groupChat.historyLimit`。设置 `0` 以禁用（默认为 50）。
- 私信历史记录可以使用 `channels.msteams.dmHistoryLimit`（用户轮次）进行限制。按用户覆盖：`channels.msteams.dms["<user_id>"].historyLimit`。
...
- `ChannelMessage.Read.Group` (Application) - 在不使用 @提及 的情况下接收所有渠道消息
...
- `ChatMessage.Read.Chat` (Application) - 在不使用 @提及 的情况下接收所有群组聊天消息
...
**结论：** RSC 用于实时监听；Graph API 用于历史访问。若要赶上离线时错过的消息，您需要带有 `ChannelMessage.Read.All` 的 Graph API（需要管理员同意）。
...
**用户提及的额外权限：** 对于对话中的用户，用户 @提及 开箱即用。但是，如果您想要动态搜索并提及**不在当前对话中**的用户，请添加 `User.Read.All` (Application) 权限并授予管理员同意。
...
- `channels.msteams.chunkMode`：`length`（默认）或 `newline` 以在按长度分块之前按空行（段落边界）拆分。
...
- `channels.msteams.teams.<teamId>.tools`：当缺少渠道覆盖设置时使用的默认每个团队工具策略覆盖设置（`allow`/`deny`/`alsoAllow`）。
- `channels.msteams.teams.<teamId>.toolsBySender`：默认每个团队每个发送者的工具策略覆盖设置（支持 `"*"` 通配符）。
...
- `channels.msteams.sharePointSiteId`：用于群组聊天/渠道中文件上传的 SharePoint 网站 ID（请参阅[在群组聊天中发送文件](#sending-files-in-group-chats)）。
...
- 在线程式风格的渠道中使用 `thread` → 回复会显示得很尴尬地嵌套在一起
- 在帖子式风格的渠道中使用 `top-level` → 回复会显示为单独的顶级帖子，而不是在主题内
...
默认情况下，OpenClaw 仅从 Microsoft/Teams 主机名下载媒体文件。可以使用 `channels.msteams.mediaAllowHosts` 覆盖此设置（使用 `["*"]` 允许任何主机）。
授权标头仅附加到 `channels.msteams.mediaAuthAllowHosts` 中的主机（默认为 Graph + Bot Framework 主机）。请保持此列表严格（避免使用多租户后缀）。
...
机器人没有个人 OneDrive 驱动器（`/me/drive` Graph API 端点不适用于应用程序身份）。要在群组聊天/渠道中发送文件，机器人会上传到 **SharePoint 站点** 并创建共享链接。
...
- 网关会在 `~/.openclaw/msteams-polls.json` 中记录投票。
- 网关必须保持在线才能记录投票。
...
`card` 参数接受 Adaptive Card JSON 对象。当提供了 `card` 时，消息文本是可选的。
...
有关卡片架构和示例，请参阅 [Adaptive Cards 文档](https://adaptivecards.io/)。有关目标格式的详细信息，请参阅下面的 [Target formats](#target-formats)。
...
注意：如果没有 `user:` 前缀，名称默认解析为组/团队。在按显示名称定向人员时，请始终使用 `user:`。
...
- **频道内无响应：** 默认情况下需要提及；请设置 `channels.msteams.requireMention=false` 或针对每个团队/频道进行配置。
...
- **"Icon file cannot be empty"：** 清单引用了大小为 0 字节的图标文件。请创建有效的 PNG 图标（`outline.png` 为 32x32，`color.png` 为 192x192）。
...
- [Teams bot file handling](https://learn.microsoft.com/en-us/microsoftteams/platform/bots/how-to/bots-filesv4)（频道/组需要 Graph）
