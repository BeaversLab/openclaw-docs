...
状态：支持文本 + 私聊附件；频道/群组文件发送需要 `sharePointSiteId` + Graph 权限（请参阅[在群组聊天中发送文件](#sending-files-in-group-chats)）。投票通过自适应卡片发送。
...
- 启动时，OpenClaw 会将团队/频道和用户允许列表名称解析为 ID（当 Graph 权限允许时）
  并记录映射；除非启用了 `channels.msteams.dangerouslyAllowNameMatching: true`，否则未解析的团队/频道名称将保持原样，但在默认情况下会被路由忽略。
...
2. 填写 ** Basics**（基本信息）选项卡：
   ...
   | **Pricing tier**   | **Free**（免费），用于开发/测试                                 |
   | **Type of App**    | **Single Tenant**（单租户）（推荐 - 见下文说明）         |
   | **Creation type**  | **Create new Microsoft App ID**（新建 Microsoft App ID）                          |
...
2. 复制 **Microsoft App ID** → 这就是您的 `appId`
3. 点击 **Manage Password** → 进入 App Registration
4. 在 **Certificates & secrets**（证书和机密）下 → **New client secret**（新建客户端机密）→ 复制 **Value**（值）→ 这就是您的 `appPassword`
5. 转到 **Overview**（概述）→ 复制 **Directory (tenant) ID**（目录（租户）ID）→ 这就是您的 `tenantId`
...
您可以使用 [Teams 开发人员门户](https://dev.teams.microsoft.com/apps)，而不是手动创建 ZIP 清单文件：
...
1. **安装 Microsoft Teams 插件**
   ...
   - 从本地签出：`openclaw plugins install ./extensions/msteams`
...
3. **Teams 应用清单**
   ...
   - 将所有三个文件一起打包：`manifest.json`、`outline.png`、`color.png`。
...
6. **运行网关**
   - 当插件已安装并且存在包含凭据的 `msteams` 配置时，Teams 频道会自动启动。

## 历史记录上下文

- `channels.msteams.historyLimit` 控制有多少条最近的频道/群组消息被包装到提示中。
- 回退到 `messages.groupChat.historyLimit`。设置 `0` 以禁用（默认为 50）。
- 可以使用 `channels.msteams.dmHistoryLimit`（用户轮次）限制 私信 历史记录。每个用户的覆盖设置：`channels.msteams.dms["<user_id>"].historyLimit`。
...
- `ChannelMessage.Read.Group` (Application) - 接收所有不包含 @提及 的频道消息
...
- `ChatMessage.Read.Chat` (Application) - 接收所有不包含 @提及 的群聊消息
...
- 如果您需要频道流量，`authorization.permissions.resourceSpecific` 必须包含频道读/写权限。
...
- `channels.msteams.webhook.port` (默认为 `3978`)
- `channels.msteams.webhook.path` (默认为 `/api/messages`)
- `channels.msteams.dmPolicy`：`pairing | allowlist | open | disabled` (默认：配对)
- `channels.msteams.allowFrom`: 私信 允许列表（建议使用 AAD 对象 ID）。如果可以使用 Graph 访问，向导会在设置期间将名称解析为 ID。
- `channels.msteams.dangerouslyAllowNameMatching`: 紧急情况（break-glass）开关，用于重新启用可变的 UPN/显示名称匹配以及直接的团队/频道名称路由。
- `channels.msteams.textChunkLimit`: 出站文本块大小。
- `channels.msteams.chunkMode`: `length`（默认）或 `newline`，用于在按长度分块之前按空行（段落边界）分割。
...
- `channels.msteams.requireMention`: 在频道/群组中需要 @提及（默认为 true）。
...
- `channels.msteams.teams.<teamId>.tools`: 缺少频道覆盖设置时使用的默认每个团队工具策略覆盖（`allow`/`deny`/`alsoAllow`）。
- `channels.msteams.teams.<teamId>.toolsBySender`: 默认每个团队每个发送者的工具策略覆盖（支持 `"*"` 通配符）。
...
- `channels.msteams.teams.<teamId>.channels.<conversationId>.tools`: 每个频道的工具策略覆盖（`allow`/`deny`/`alsoAllow`）。
- `channels.msteams.teams.<teamId>.channels.<conversationId>.toolsBySender`: 每个频道每个发送者的工具策略覆盖（支持 `"*"` 通配符）。
- `toolsBySender` 键应使用显式前缀：
  `id:`, `e164:`, `username:`, `name:` （旧的无前缀键仍然仅映射到 `id:`）。
- `channels.msteams.sharePointSiteId`：用于群聊/频道中文件上传的 SharePoint 站点 ID（请参阅[在群聊中发送文件](#sending-files-in-group-chats)）。

## 路由和会话

- 会话键遵循标准代理格式（参见 [/concepts/会话](/zh/en/concepts/会话)）：
...
| ------------------------ | --------------------------------------------------------- | ------------------------ |
| **Posts**（经典）      | 消息以卡片形式出现，下方有线程回复 | `thread`（默认）       |
| **Threads**（类似 Slack） | 消息线性流动，更像 Slack                   | `top-level`              |

**问题：** Teams API 不会公开频道使用的 UI 样式。如果您使用了错误的 `replyStyle`：

- 在 Threads 样式的频道中使用 `thread` → 回复会显示为尴尬的嵌套形式
- 在 Posts 样式的频道中使用 `top-level` → 回复显示为单独的顶层帖子，而不是在线程内

**解决方案：** 根据频道的设置方式，逐个频道配置 `replyStyle`：
...
如果没有 Graph 权限，包含图片的频道消息将仅以文本形式接收（机器人无法访问图片内容）。
默认情况下，OpenClaw 仅从 Microsoft/Teams 主机名下载媒体。通过 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任何主机）。
...
| 上下文 | 文件发送方式 | 所需设置 |
| ------------------------ | -------------------------------------------- | ----------------------------------------------- |
| **私聊 (私信)** | FileConsentCard → 用户接受 → 机器人上传 | 开箱即用 |
| **群聊/频道** | 上传到 SharePoint → 分享链接 | 需要 `sharePointSiteId` + Graph 权限 |
| **图片（任何上下文）** | Base64 编码内联 | 开箱即用 |

### 为什么群聊需要 SharePoint

机器人没有个人 OneDrive 驱动器（`/me/drive` Graph API 端点不适用于应用程序身份）。为了在群聊/频道中发送文件，机器人会上传到 **SharePoint 站点** 并创建共享链接。
...
`card` 参数接受一个自适应卡 JSON 对象。当提供 `card` 时，消息文本是可选的。
...
注意：如果没有 `user:` 前缀，名称默认解析为群组/团队。当通过显示名称定位人员时，请始终使用 `user:`。
...
Teams URL 中的 `groupId` 查询参数**不是**配置中使用的团队 ID。请改为从 URL 路径中提取 ID：
...
- **频道中无响应：** 默认情况下需要提及；请设置 `channels.msteams.requireMention=false` 或为每个团队/频道进行配置。
...
- **"图标文件不能为空"：** 清单引用了大小为 0 字节的图标文件。请创建有效的 PNG 图标（`outline.png` 为 32x32，`color.png` 为 192x192）。
- **“webApplicationInfo.Id already in use” (webApplicationInfo.Id 已被使用)：** 该应用仍安装在其他团队/聊天中。先找到并卸载它，或者等待 5-10 分钟以完成传播。
- **上传时提示“出现问题”：** 请改为通过 [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) 上传，打开浏览器开发者工具 (F12) → Network 标签页，并检查响应正文以获取实际错误。
