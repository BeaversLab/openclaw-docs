...
- 可以使用 `channels.msteams.dmHistoryLimit`（用户轮次）限制 DM 历史记录。每个用户的覆盖设置：`channels.msteams.dms["<user_id>"].historyLimit`。
...
- `channels.msteams.allowFrom`: DM 允许列表（建议使用 AAD 对象 ID）。如果可以使用 Graph 访问，向导会在设置期间将名称解析为 ID。
...
- 会话键遵循标准代理格式（参见 [/concepts/session](/en/concepts/session)）：
