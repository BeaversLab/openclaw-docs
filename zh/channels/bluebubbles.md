...
状态：内置插件，通过 HTTP 连接 BlueBubbles macOS 服务器。**推荐用于 iMessage 集成**，相比旧的 imsg 渠道，API 更丰富、配置更简单。
...
- Reactions 以 system event 形式呈现（类似 Slack/Telegram），agent 可在回复前“引用”它们。
...
```json5
{
  channels: {
    bluebubbles: {
      blockStreaming: true  // 启用分块流式（默认行为）
    }
  }
}
```
...
- `channels.bluebubbles.blockStreaming`：启用分块流式（默认：`true`）。
