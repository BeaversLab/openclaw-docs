...
- [Synology Chat](/zh/channels/synology-chat) — 通过传出和传入 Webhook 连接的 Synology NAS Chat（插件，需单独安装）。
...
- [Voice Call](/zh/plugins/voice-call) — 通过 Plivo 或 Twilio 提供的电话服务（插件，需单独安装）。
- [WebChat](/zh/web/webchat) — 基于 WebSocket 的 Gateway(网关) WebChat 界面。
- [WhatsApp](/zh/channels/whatsapp) — 最受欢迎；使用 Baileys 并需要二维码配对。
- [Zalo](/zh/channels/zalo) — Zalo Bot API；越南流行的通讯应用（插件，需单独安装）。
- [Zalo Personal](/zh/channels/zalouser) — 通过二维码登录的 Zalo 个人账户（插件，需单独安装）。

## 注意

- 渠道可以同时运行；配置多个渠道后，OpenClaw 将根据聊天进行路由。
- 最快的设置通常是 **Telegram**（简单的 Bot 令牌）。WhatsApp 需要二维码配对，
  并且在磁盘上存储更多状态。
- 群组行为因渠道而异；请参阅 [Groups](/zh/channels/groups)。
- 出于安全考虑，会强制执行私信配对和允许列表；请参阅 [Security](/zh/gateway/security)。
- 故障排除：[Channel 故障排除](/zh/channels/troubleshooting)。
- 模型提供商另有文档说明；请参阅 [Model Providers](/zh/providers/models)。
