...
状态：通过 WhatsApp Web (Baileys) 实现生产就绪。Gateway 网关 拥有链接的会话。

<CardGroup cols={3}> <Card title="配对" icon="link" href="/zh/en/channels/pairing"> 对于未知发送者，默认的私信策略是配对。 </Card> <Card title="频道故障排除" icon="wrench" href="/zh/en/channels/troubleshooting"> 跨频道诊断和修复手册。 </Card> <Card title="Gateway 网关 配置" icon="settings" href="/zh/en/gateway/configuration"> 完整的频道配置模式和示例。 </Card>
</CardGroup>
...
- Gateway 网关 owns the WhatsApp socket and reconnect loop.
- Outbound sends require an active WhatsApp listener for the target account.
- 状态和广播聊天会被忽略 (`@status`, `@broadcast`)。
- 直接聊天使用 私信 会话规则 (`session.dmScope`; 默认 `main` 将 私信 折叠到代理主会话中)。
