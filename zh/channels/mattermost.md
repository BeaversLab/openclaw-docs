...
状态：通过插件支持（Bot 令牌 + WebSocket 事件）。支持频道、群组和私信。
Mattermost 是一个可自行托管团队消息平台；有关产品详情和下载，请访问
...
原生斜杠命令是可选启用的。启用后，OpenClaw 会通过
Mattermost API 注册 `oc_*` 个斜杠命令，并在网关 HTTP 服务器上接收回调 POST 请求。
...
- 对于多账户设置，`commands` 可以设置在顶层或
  `channels.mattermost.accounts.<id>.commands` 下（账户值会覆盖顶层的字段）。
...
- Mattermost 出站白名单要求：
  ...
  - 使用主机/域名条目，而不是完整的 URL。
    - 好的示例：`gateway.tailnet-name.ts.net`
    - 坏的示例：`https://gateway.tailnet-name.ts.net`
...
- `onchar` 仍会响应显式的 @提及。
- `channels.mattermost.requireMention` 对于旧版配置仍然有效，但首选 `chatmode`。

## 会话串与会话

使用 `channels.mattermost.replyToMode` 来控制频道和群组的回复是留在主频道中，还是在触发帖子下启动一个会话串。

- `off` (默认)：仅当入站帖子本身已在会话串中时，才在会话串中回复。
- `first`：对于顶级频道/群组帖子，在该帖子下启动一个会话串，并将
  对话路由到限定范围的会话会话中。
- `all`：对于目前的 Mattermost，其行为与 `first` 相同。
...
- `first` 和 `all` 目前是等效的，因为一旦 Mattermost 拥有了会话串根节点，
...
- 运行时说明：如果 `channels.mattermost` 完全缺失，运行时会回退到 `groupPolicy="allowlist"` 进行群组检查 (即使设置了 `channels.defaults.groupPolicy`)。
...
原始的不透明 ID (如 `64ifufp...`) 在 Mattermost 中是**有歧义的** (用户 ID 与频道 ID)。

OpenClaw 以**用户优先**的方式解析它们：

- 如果该 ID 作为用户存在 (`GET /api/v4/users/<id>` 成功)，OpenClaw 会通过 `/api/v4/channels/direct` 解析直接频道来发送 **DM**。
- 否则，该 ID 将被视为**频道 ID**。
...
- `channels.mattermost.capabilities`：功能字符串数组。添加 `"inlineButtons"` 以
  在代理系统提示中启用按钮工具描述。
- `channels.mattermost.interactions.callbackBaseUrl`：按钮的可选外部基本 URL
  回调（例如 `https://gateway.example.com`）。当 Mattermost 无法
  直接通过其绑定主机访问网关时使用。
- 在多账户设置中，您也可以在
  `channels.mattermost.accounts.<id>.interactions.callbackBaseUrl` 下设置相同的字段。
- 如果省略 `interactions.callbackBaseUrl`，OpenClaw 将从
  `gateway.customBindHost` + `gateway.port` 推导回调 URL，然后回退到 `http://localhost:<port>`。
- 可达性规则：按钮回调 URL 必须能从 Mattermost 服务器访问。
  `localhost` 仅在 Mattermost 和 OpenClaw 运行在同一主机/网络命名空间中时才有效。
- 如果您的回调目标是私有/tailnet/内部的，请将其主机/域名添加到 Mattermost
  `ServiceSettings.AllowedUntrustedInternalConnections`。

### 直接 API 集成（外部脚本）

外部脚本和 Webhooks 可以通过 Mattermost REST API 直接发布按钮，
而不通过代理的 `message` 工具。尽可能使用扩展中的 `buildButtonAttachments()`；如果发布原始 JSON，请遵循以下规则：
...
1. 附件应放在 `props.attachments` 中，而不是顶级 `attachments` 中（将被静默忽略）。
2. 每个操作都需要 `type: "button"` — 没有它，点击将被静默吞掉。
3. 每个操作都需要一个 `id` 字段 — Mattermost 会忽略没有 ID 的操作。
4. 操作 `id` 必须**仅包含字母数字**（`[a-zA-Z0-9]`）。连字符和下划线会破坏
...
2. 构建上下文对象，其中包含除 `_token` 以外的所有字段。
3. 使用**排序后的键**和**无空格**进行序列化（网关使用 `JSON.stringify`
   配合排序后的键，这会产生紧凑的输出）。
...
- 始终对**所有**上下文字段进行签名（不包括 `_token`）。网关会移除 `_token` 然后
  对剩余的所有内容进行签名。对子集进行签名会导致静默验证失败。
- 使用 `sort_keys=True` —— 网关在签名前会对键进行排序，而 Mattermost 可能会
  在存储负载时重新排序上下文字段。
...
Mattermost 插件包含一个目录适配器，可通过 Mattermost API 解析频道和用户名。
这使得在 `openclaw message send` 和 cron/webhook 投递中可以使用 `#channel-name` 和 `@username` 目标。
...
- 按钮已渲染但点击无反应：请验证 Mattermost 服务器配置中的 `AllowedUntrustedInternalConnections` 包含 `127.0.0.1 localhost`，并且 ServiceSettings 中的 `EnablePostActionIntegration` 为 `true`。
- 点击按钮返回 404：按钮的 `id` 可能包含连字符或下划线。Mattermost 的操作路由器在遇到非字母数字 ID 时会中断。请仅使用 `[a-zA-Z0-9]`。
- 网关日志记录 `invalid _token`：HMAC 不匹配。请检查您是否对所有上下文字段（而非子集）进行了签名，使用了排序后的键，并使用了紧凑的 JSON（无空格）。请参阅上面的 HMAC 部分。
