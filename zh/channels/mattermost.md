...
如果您在设置期间选择了 Mattermost 并且检测到了 git 检出，
...
1. 安装 Mattermost 插件。
2. 创建一个 Mattermost 机器人帐户并复制 **机器人令牌**。
...
原生斜杠命令是可选启用的。启用后，OpenClaw 通过
Mattermost API 注册 `oc_*` 斜杠命令，
...
- `native: "auto"` 在 Mattermost 上默认禁用。设置 `native: true` 以启用。
- 如果省略 `callbackUrl`，OpenClaw 将根据网关主机/端口 + `callbackPath` 推导出一个。
- 对于多帐户设置，`commands` 可以设置在顶层或
  `channels.mattermost.accounts.<id>.commands` 下（帐户值将覆盖顶层字段）。
- 命令回调将使用每个命令的令牌进行验证，并在令牌检查失败时拒绝访问。
- 可达性要求：回调端点必须可从 Mattermost 服务器访问。
  - 除非 Mattermost 与 OpenClaw 运行在同一主机/网络命名空间中，否则不要将 `callbackUrl` 设置为 `localhost`。
  - 除非您的 Mattermost 基础 URL 将 `/api/channels/mattermost/command` 反向代理到 OpenClaw，否则不要将 `callbackUrl` 设置为您的 Mattermost 基础 URL。
...
- `onchar` 仍会响应显式的 @ 提及。
- 旧版配置会遵循 `channels.mattermost.requireMention`，但建议使用 `chatmode`。

## 主题与会话

使用 `channels.mattermost.replyToMode` 来控制渠道和群组回复是留在主渠道中，还是在触发帖子下开启主题。

- `off`（默认）：仅在入站帖子已存在于主题中时，才在主题中回复。
- `first`：对于顶层渠道/群组帖子，在该帖子下开启主题并将对话
  路由到限定范围的会话中。
- `all`：对于当前的 Mattermost，其行为与 `first` 相同。
- 私信会忽略此设置并保持非主题模式。
...
- 限定范围的会话使用触发帖子 ID 作为主题根。
- `first` 和 `all` 目前是等效的，因为一旦 Mattermost 有了主题根，
  后续的块和媒体都会继续在同一主题中。

## 访问控制（私信）

- 默认值：`channels.mattermost.dmPolicy = "pairing"`（未知发送者将收到配对码）。
- 通过以下方式批准：
...
- 运行时说明：如果完全缺少 `channels.mattermost`，运行时将回退到 `groupPolicy="allowlist"` 进行群组检查（即使设置了 `channels.defaults.groupPolicy`）。
...
- 用于渠道的 `channel:<id>`
- 用于私信的 `user:<id>`
- 用于私信的 `@username`（通过 Mattermost API 解析）

原始的不透明 ID（如 `64ifufp...`）在 Mattermost 中是**有歧义的**（用户 ID 与渠道 ID）。

OpenClaw 按**用户优先**原则解析它们：
...
当 OpenClaw 发送到 Mattermost 私信目标并且需要先解析直接渠道时，默认会重试临时的直接渠道创建失败。
...
- 重试适用于速率限制、5xx 响应以及网络或超时错误等瞬时故障。
...
- `emoji` 接受类似 `thumbsup` 或 `:+1:` 的名称（冒号是可选的）。
- 设置 `remove=true`（布尔值）以移除表情回应。
- 表情回应添加/移除事件作为系统事件转发到路由代理的会话。
...
1. 所有按钮都会被一行确认文本替换（例如，“✓ **Yes** selected by @user”）。
2. 代理会收到选择作为入站消息并做出响应。

注意事项：

- 按钮回调使用 HMAC-SHA256 验证（自动进行，无需配置）。
- Mattermost 会从其 API 响应中剥离回调数据（安全功能），因此点击时
所有按钮都会被移除 —— 无法进行部分移除。
...
- 可达性规则：按钮回调 URL 必须可以从 Mattermost 服务器访问。`localhost` 仅在 Mattermost 和 OpenClaw 运行在同一主机/网络命名空间时才有效。
- 如果您的回调目标是私有/tailnet/内部网络，请将其主机/域名添加到 Mattermost `ServiceSettings.AllowedUntrustedInternalConnections` 中。

### 直接 API 集成（外部脚本）

外部脚本和 webhooks 可以直接通过 Mattermost REST API 发送按钮，而不必通过代理的 `message` 工具。尽可能使用扩展中的 `buildButtonAttachments()`；如果发布原始 JSON，请遵循以下规则：
...
1. 附件位于 `props.attachments` 中，而非顶层的 `attachments`（会被静默忽略）。
2. 每个操作都需要 `type: "button"` — 没有它，点击将被静默吞没。
3. 每个操作都需要一个 `id` 字段 — Mattermost 会忽略没有 ID 的操作。
4. 操作 `id` 必须**仅包含字母数字**（`[a-zA-Z0-9]`）。连字符和下划线会破坏 Mattermost 的服务器端操作路由（返回 404）。在使用前请将它们去除。
...
2. 构建包含除 `_token` 之外所有字段的上下文对象。
3. 使用**排序的键**和**无空格**进行序列化（网关使用 `JSON.stringify` 配合排序的键，这会产生紧凑的输出）。
...
- 始终对**所有**上下文字段进行签名（不包括 `_token`）。网关会去除 `_token`，然后
  对剩余的所有内容进行签名。仅对子集进行签名会导致静默验证失败。
- 使用 `sort_keys=True` —— 网关在签名前会对键进行排序，而 Mattermost 在存储负载时可能会
  重新排序上下文字段。
- 从 bot 令牌派生密钥（确定性的），而不是随机字节。密钥
  在创建按钮的进程和进行验证的网关之间必须保持一致。

## 目录适配器

Mattermost 插件包含一个目录适配器，用于通过 Mattermost API 解析渠道和用户名。
这启用了 `openclaw message send` 和 cron/webhook 投递中的 `#channel-name` 和 `@username` 目标。

无需配置 —— 适配器使用账户配置中的 bot 令牌。

## 多账户

Mattermost 支持在 `channels.mattermost.accounts` 下拥有多个账户：
...
- 渠道内无回复：确保 bot 在渠道中并被提及（oncall），使用触发前缀（onchar），或设置 `chatmode: "onmessage"`。
- 身份验证错误：检查 bot 令牌、基础 URL 以及账户是否已启用。
- 多账户问题：环境变量仅适用于 `default` 账户。
- 按钮显示为白框：代理可能发送了格式错误的按钮数据。检查每个按钮是否同时具有 `text` 和 `callback_data` 字段。
- 按钮已渲染但点击无反应：请验证 `AllowedUntrustedInternalConnections` 在 Mattermost 服务器配置中包含 `127.0.0.1 localhost`，并且 `EnablePostActionIntegration` 在 ServiceSettings 中为 `true`。
- 点击按钮时返回 404：按钮的 `id` 可能包含连字符或下划线。Mattermost 的操作路由器会因非字母数字 ID 而中断。请仅使用 `[a-zA-Z0-9]`。
- Gateway(网关) 日志记录 `invalid _token`：HMAC 不匹配。请检查您是否对所有上下文字段（而非子集）进行了签名，使用了排序键，并使用了紧凑型 JSON（无空格）。请参阅上面的 HMAC 部分。
- Gateway(网关) 日志记录 `missing _token in context`：`_token` 字段不在按钮的上下文中。确保在构建集成负载时包含该字段。
- 确认信息显示原始 ID 而非按钮名称：`context.action_id` 与按钮的 `id` 不匹配。将两者设置为相同的清理值。
