...
- 私信与代理的主会话共享。
- 支持通过策略控制（`groupPolicy` + `groupAllowFrom`）进行群组操作，并且默认为失败时关闭的允许列表行为。
...
多账户支持：使用 `channels.zalo.accounts` 配置每个账户的令牌，并可选地配置 `name`。
...
- 媒体下载/上传受到 `channels.zalo.mediaMaxMb` 限制（默认为 5）。
...
- 默认：`channels.zalo.dmPolicy = "pairing"`。未知发送者会收到配对码；消息在被批准前将被忽略（代码在 1 小时后过期）。
- 通过以下方式批准：
...
- Webhook 模式：设置 `channels.zalo.webhookUrl` 和 `channels.zalo.webhookSecret`。
  ...
  - 重复事件（`event_name + message_id`）在短的重放窗口内将被忽略。
...
- `channels.zalo.tokenFile`：从常规文件路径读取令牌。符号链接将被拒绝。
