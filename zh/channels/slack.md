  ...
  <Tab title="Socket 模式（默认）">
    <Steps>
      <Step title="创建 Slack 应用和令牌">
        ...
        - 启用 **Socket 模式**
        - 使用 `connections:write` 创建 **应用令牌**（`xapp-...`）
        - 安装应用并复制 **机器人令牌**（`xoxb-...`）
      </Step>
  ...
  <Tab title="HTTP Events API 模式">
    <Steps>
      <Step title="配置 Slack 应用的 HTTP 设置">
        ...
        - 将 Event Subscriptions（事件订阅）、Interactivity（交互性）和 Slash command Request URL（斜杠命令请求 URL）设置为相同的 Webhook 路径（默认为 `/slack/events`）
...
```
      ...
      <Step title="为多账号 HTTP 使用唯一的 Webhook 路径">
        ...
        为每个账号分配一个独特的 `webhookPath`，以免注册冲突。
      </Step>
    </Steps>
...
<Tip>
对于操作/目录读取，配置后可以优先使用用户令牌。对于写入操作，机器人令牌保持优先；仅当 `userTokenReadOnly: false` 且机器人令牌不可用时，才允许使用用户令牌进行写入。
</Tip>
...
<Tabs>
  ...
  <Tab title="Channel policy">
    ...
    - 频道允许列表条目和私信允许列表条目在启动时根据令牌访问权限进行解析
    - 未解析的频道名称条目将按配置保留，但在默认情况下会被路由忽略
    - 入站授权和频道路由默认以 ID 为准；直接的用户名/别名匹配需要 `channels.slack.dangerouslyAllowNameMatching: true`
  ...
  <Tab title="Mentions and channel users">
    ...
    每个频道的控制 (`channels.slack.channels.<id>`; 名称仅通过启动解析或 `dangerouslyAllowNameMatching`)：
...
- 当启用本机命令时，在 Slack 中注册匹配的斜杠命令 (`/<command>` 名称)，有一个例外：
...
并且仍然针对目标会话会话路由命令执行 (`CommandTargetSessionKey`)。
...
- `channels.slack.thread.initialHistoryLimit` 控制在新线程会话开始时获取多少现有的线程消息（默认为 `20`；设置 `0` 可禁用）。
...
注意：`replyToMode="off"` 会禁用 Slack 中 **所有** 的回复线程，包括显式的 `[[reply_to_*]]` 标签。这与 Telegram 不同，在 Telegram 的 `"off"` 模式下显式标签仍然有效。这种差异反映了平台的线程模型：Slack 线程会向频道隐藏消息，而 Telegram 回复则在主聊天流程中保持可见。
...
- 块操作和模态交互会发出结构化的 `Slack interaction: ...` 系统事件，其中包含丰富的负载字段：
  ...
  - 模态 `view_submission` 和 `view_closed` 事件，其中包含路由的频道元数据和表单输入
...
`typingReaction` 在 OpenClaw 处理回复时向传入的 Slack 消息添加临时反应，然后在运行完成时将其移除。当 Slack 原生助手输入功能不可用时，这是一个有用的后备方案，特别是在私信中。
...
```
  ...
  <Accordion title="可选的用户令牌范围（读取操作）">
    如果您配置了 `channels.slack.userToken`，典型的读取范围有：
...
```
  ...
  <Accordion title="Socket 模式未连接">
    在 Slack 应用设置中验证 bot + 应用令牌以及 Socket 模式是否已启用。
  </Accordion>
  ...
  <Accordion title="HTTP mode not receiving events">
    ...
    - Slack Request URLs (Events + Interactivity + Slash Commands)（Slack 请求 URL：事件 + 交互 + 斜杠命令）
  ...
  <Accordion title="Native/slash commands not firing">
    ...
    - 本地命令模式 (`channels.slack.commands.native: true`)，并已在 Slack 中注册匹配的斜杠命令
...
当 `streaming` 为 `partial` 时（默认：`true`），`channels.slack.nativeStreaming` 控制 Slack 的本地流式 API (`chat.startStream` / `chat.appendStream` / `chat.stopStream`)。
...
2. 确保应用具有 `assistant:write` scope（作用域）。
3. 该消息必须有一条可用的回复线程。线程选择仍遵循 `replyToMode`。
