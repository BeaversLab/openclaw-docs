...
<Tip>
  对于操作/目录读取，配置后可以优先使用用户令牌。对于写入，机器人令牌仍然是首选；只有当
  `userTokenReadOnly: false` 且机器人令牌不可用时，才允许使用用户令牌进行写入。
</Tip>
...
<Accordion title="Outbound text and files">
  ...
  `channels.slack.mediaMaxMb` when configured; otherwise 渠道 sends use MIME-kind defaults from
  media pipeline
</Accordion>
