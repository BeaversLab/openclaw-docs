...
一般**不需要** **Presence Intent**。设置 bot 自身的 presence（`setPresence` action）使用 gateway OP3，不需要此 intent；仅当你要接收其他服务器成员的 presence 更新时才需要。
...
- **Discord 中的执行批准**：Discord 支持 **按钮 UI** 在 DM 中进行执行批准（允许一次/始终允许/拒绝）。`/approve <id> ...` 仅用于转发的批准，无法解决 Discord 的按钮提示。如果看到 `❌ Failed to submit approval: Error: unknown approval id` 或 UI 从未显示，请检查：
  - 配置中设置了 `channels.discord.execApprovals.enabled: true`。
  - 你的 Discord 用户 ID 列在 `channels.discord.execApprovals.approvers` 中（UI 仅发送给批准者）。
  - 使用 DM 提示中的按钮（**Allow once**、**Always allow**、**Deny**）。
...
- `actions`：动作级工具开关；省略则允许所有（设 `false` 禁用）。
  ...
  - `presence`（bot status/activity，默认 `false`）
...
启用 PK 查找以便代理消息解析为底层系统 + 成员。
启用后，OpenClaw 使用成员身份进行 allowlist 匹配和标签，
并将发送者标记为 `Member (PK:System)` 以避免意外 ping Discord。
...
- 同时匹配成员的显示名称（按名称/slug）。
- 查找使用**原始** Discord 消息 ID（代理前消息），因此
  PK API 仅在其 30 分钟窗口内解析。
- 如果 PK 查找失败（例如，没有 token 的私有系统），代理消息
  被视为 bot 消息并被丢弃，除非设置 `channels.discord.allowBots=true`。
...
| presence       | disabled | Bot 状态/活动 (setPresence)        |
