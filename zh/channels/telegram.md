...
更多帮助：[Channel troubleshooting](/zh/channels/troubleshooting)。
...
- 配对是 Telegram 私聊的默认 token 交换机制。详情见 [Pairing](/zh/start/pairing)
...
更多： [Streaming + chunking](/zh/concepts/streaming)。

## 重试策略
Telegram API 出站调用在临时网络/429 错误时会指数退避并带抖动重试。通过 `channels.telegram.retry` 配置。参见 [Retry policy](/zh/concepts/retry)。
