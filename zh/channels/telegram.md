...
<Steps>
  <Step title="Create the bot token in BotFather">
    打开 Telegram 并与 **@BotFather** 对话（确认用户名准确为 `@BotFather`）。
...
```
    ...
    Telegram **不**使用 `openclaw channels login telegram`；请在配置/环境中设置令牌，然后启动网关。
...
<Note>
令牌解析顺序是区分账户的。实际上，配置值优先于环境变量回退，且 `TELEGRAM_BOT_TOKEN` 仅适用于默认账户。
</Note>
...
<Tabs>
  <Tab title="DM policy">
    ...
    - `pairing` （默认）
    - `allowlist` （至少需要在 `allowFrom` 中有一个发送者 ID）
    - `open` （需要 `allowFrom` 包含 `"*"`）
    - `disabled`

    `channels.telegram.allowFrom` 接受数字 Telegram 用户 ID。`telegram:` / `tg:` 前缀会被接受并标准化。
    `dmPolicy: "allowlist"` 如果 `allowFrom` 为空，则会阻止所有直接消息，并且会被配置验证拒绝。
    入门向导接受 `@username` 输入并将其解析为数字 ID。
    如果您已升级且配置中包含 `@username` 允许列表条目，请运行 `openclaw doctor --fix` 来解析它们（尽力而为；需要 Telegram 机器人令牌）。
    如果您之前依赖于 pairing-store 允许列表文件，`openclaw doctor --fix` 可以在允许列表流程中将条目恢复到 `channels.telegram.allowFrom` 中（例如，当 `dmPolicy: "allowlist"` 尚无显式 ID 时）。

    对于单一所有者的机器人，首选 `dmPolicy: "allowlist"` 并配置明确的数字 `allowFrom` ID，以便在配置中保持访问策略的持久性（而不是依赖于先前的配对批准）。
...
```
  ...
  <Tab title="Group policy and allowlists">
    ...
    1. **允许哪些群组** (`channels.telegram.groups`)
       ...
       - `groups` 已配置：作为允许列表（明确的 ID 或 `"*"`）

    2. **允许哪些发送者在群组中发送消息** (`channels.telegram.groupPolicy`)
       - `open`
       - `allowlist`（默认）
       - `disabled`
    ...
    非数字条目在发送者授权时将被忽略。
    安全边界（`2026.2.25+`）：群组发送者授权**不**继承私信 配对存储中的批准。
    配对仅限私信。对于群组，请设置 `groupAllowFrom` 或针对每个群组/每个话题的 `allowFrom`。
    运行时说明：如果完全缺少 `channels.telegram`，除非显式设置了 `channels.defaults.groupPolicy`，否则运行时默认为故障关闭 `groupPolicy="allowlist"`。
...
```
    ...
    <Warning>
      ...
      - 将负数的 Telegram 群组或超级群组聊天 ID（如 `-1001234567890`）放入 `channels.telegram.groups` 中。
      - 当您想限制已允许群组内哪些人可以触发机器人时，将 Telegram 用户 ID（如 `8734062810`）放入 `groupAllowFrom` 中。
      - 仅当您希望已允许群组的任何成员都能与机器人交谈时，才使用 `groupAllowFrom: ["*"]`。
    </Warning>
...
- 私信消息可以携带 `message_thread_id`；OpenClaw 使用具有线程感知能力的会话键路由它们，并保留话题 ID 以便回复。
- 长轮询使用 grammY 运行器进行每个聊天/每个线程的排序。运行器接收器的整体并发性使用 `agents.defaults.maxConcurrent`。
...
```
    ...
    - `setMyCommands failed` 配合 `BOT_COMMANDS_TOO_MUCH` 表示 Telegram 菜单在裁剪后仍然溢出；请减少插件/技能/自定义命令或禁用 `channels.telegram.commands.native`。
    - `setMyCommands failed` 配合网络/抓取错误通常表示指向 `api.telegram.org` 的出站 DNS/HTTPS 被阻止。
...
```
  ...
  <Accordion title="面向代理和自动化的 Telegram 消息操作">
    ...
    Channel 消息操作提供了符合人体工学的别名 (`send`, `react`, `delete`, `edit`, `sticker`, `sticker-search`, `topic-create`)。
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    `agentId` 仅限主题，不从群组默认设置继承。

    **按主题代理路由**：每个主题可以通过在主题配置中设置 `agentId` 来路由到不同的代理。这为每个主题提供了其独立的工作区、记忆和会话。示例：
    ...
    每个主题随后都有自己的会话密钥：`agent:zu:telegram:group:-1001234567890:topic:3`
...
```
  ...
  <Accordion title="反应通知">
    ...
    - `own` 意味着仅限用户对 Bot 发送消息的反应（通过已发送消息缓存尽力而为）。
...
```
  ...
  <Accordion title="Telegram 中的执行审批">
    Telegram 支持在审批者的私信中进行执行审批，并且可以选择在原始聊天或主题中发布审批提示。
    ...
    审批者必须是数字形式的 Telegram 用户 ID。当 `enabled` 为 false 或 `approvers` 为空时，Telegram 不充当执行审批客户端。审批请求将回退到其他配置的审批路由或执行审批回退策略。

    传递规则：

    - `target: "dm"` 仅将审批提示发送给配置的审批者私信
    - `target: "channel"` 将提示发送回发起的 Telegram 聊天/话题
    - `target: "both"` 发送给审批者私信和发起的聊天/话题

    只有配置的审批者可以批准或拒绝。非审批者无法使用 `/approve`，也无法使用 Telegram 审批按钮。

    频道投递会在聊天中显示命令文本，因此请仅在受信任的群组/话题中启用 `channel` 或 `both`。当提示进入论坛话题时，OpenClaw 会保留话题，用于审批提示和审批后的后续跟进。
...
<AccordionGroup>
  ...
  <Accordion title="Bot 完全看不到群组消息">
    ...
    - 验证 bot 在群组中的成员身份
    - 检查日志：`openclaw logs --follow` 以了解跳过的原因
  ...
  <Accordion title="命令部分工作或完全不工作">
    ...
    - `setMyCommands failed` 并带有网络/fetch 错误通常表示到 `api.telegram.org` 的 DNS/HTTPS 可达性问题
...
- `channels.telegram.accounts.<account>.execApprovals`：针对 Telegram 执行审批路由和审批人授权的逐账号覆盖。
- `channels.telegram.capabilities.inlineButtons`：`off | dm | group | all | allowlist`（默认：allowlist）。
- `channels.telegram.accounts.<account>.capabilities.inlineButtons`：逐账号覆盖。
...
- `channels.telegram.chunkMode`：`length`（默认）或 `newline`，用于在按长度分块之前在空行（段落边界）处分割。
...
- `channels.telegram.actions.reactions`：控制 Telegram 工具回应（reactions）。
