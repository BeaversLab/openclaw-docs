...
- Telegram 论坛主题会在群组 ID 后添加 `:topic:<threadId>`，以便每个主题拥有自己的会话。
...
原因：在单代理模式下，私信通常落在 **main** 会话键 (`agent:main:main`) 中，而群组始终使用 **non-main** 会话键 (`agent:main:<channel>:group:<id>`)。如果您通过 `mode: "non-main"` 启用沙盒，这些群组会话将在 Docker 中运行，而您的主私信会话则保持在宿主机上。
...
> 如果您需要真正独立的工作区/角色（“个人”和“公共”绝不能混合），请使用第二个代理 + 绑定。请参阅 [多代理路由](/en/concepts/multi-agent)。
...
想要“群组只能看到文件夹 X”而不是“无主机访问权限”？保留 `workspaceAccess: "none"` 并且仅将列入白名单的路径挂载到沙箱中：
...
- 调试工具被阻止的原因：[沙盒 vs 工具策略 vs 提升权限](/en/gateway/sandbox-vs-tool-policy-vs-elevated)
- 绑定挂载详细信息：[沙盒化](/en/gateway/sandboxing#custom-bind-mounts)

## 显示标签

- 如果可用，UI 标签使用 `displayName`，格式为 `<channel>:<token>`。
- `#room` 是为房间/频道保留的；群聊使用 `g-<slug>`（小写，空格 -> `-`，保留 `#@+._-`）。
...
- DM 配对批准（`*-allowFrom` 存储条目）仅适用于 DM 访问；群组发送者授权保持对群组白名单的显式指定。
- Discord：白名单使用 `channels.discord.guilds.<id>.channels`。
- Slack：白名单使用 `channels.slack.channels`。
- Matrix：白名单使用 `channels.matrix.groups`（房间 ID、别名或名称）。使用 `channels.matrix.groupAllowFrom` 来限制发送者；每个房间的 `users` 白名单也受支持。
...
- 运行时安全性：当提供程序块完全缺失（`channels.<provider>` 缺失）时，群组策略将回退到故障关闭模式（通常是 `allowlist`），而不是继承 `channels.defaults.groupPolicy`。
...
除非每个群组单独覆盖，否则群组消息需要提及。默认值按子系统位于 `*.groups."*"` 下。
...
- 仅当可以进行提及检测（配置了原生提及或 `mentionPatterns`）时，才会强制执行提及控制。
- Discord 的默认值位于 `channels.discord.guilds."*"` 中（可按公会/频道覆盖）。
- 群组历史上下文在所有频道中统一包装，并且是 **仅待处理（pending-only）** 的（因提及控制而被跳过的消息）；使用 `messages.groupChat.historyLimit` 作为全局默认值，使用 `channels.<channel>.historyLimit`（或 `channels.<channel>.accounts.*.historyLimit`）进行覆盖。设置 `0` 以禁用。
...
当配置了 `channels.whatsapp.groups`、`channels.telegram.groups` 或 `channels.imessage.groups` 时，这些键将作为群组允许列表。使用 `"*"` 允许所有群组，同时仍然设置默认提及行为。
...
所有者由 `channels.whatsapp.allowFrom` 确定（如果未设置，则由机器人自身的 E.164 确定）。将该命令作为独立消息发送。其他表面目前忽略 `/activation`。
...
Agent 系统提示在新群组会话的第一轮中包含群组简介。它提醒模型像人类一样回复，避免使用 Markdown 表格，并避免输入字面的 `\n` 序列。
...
请参阅 [群组消息](/en/channels/group-messages) 了解仅限 WhatsApp 的行为（历史记录注入、提及处理详情）。
