---
summary: "通过 NIP-04 加密消息的 Nostr 直连消息频道"
read_when:
...
- 入门向导 (`openclaw onboard`) 和 `openclaw channels add` 中列出了可选的频道插件。
...
个人资料数据作为 NIP-01 `kind:0` 事件发布。您可以在控制 UI（Channels -> Nostr -> Profile）中管理它，也可以直接在配置中设置。
...
| NIP-17 | 计划中   | 礼物包裹式私信 (Gift-wrapped DMs)                      |
| NIP-44 | 计划中   | 版本化加密                  |
