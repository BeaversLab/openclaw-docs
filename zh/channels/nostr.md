...
- The 新手引导 向导 (`openclaw onboard`) and `openclaw channels add` list optional 渠道 plugins.
...
| `dmPolicy`   | string   | `pairing`                                   | 私信 access policy                    |
| `allowFrom`  | string[] | `[]`                                        | Allowed sender pubkeys              |
| `enabled`    | boolean  | `true`                                      | Enable/disable 渠道              |
...
- **allowlist**: only pubkeys in `allowFrom` can 私信.
- **open**: public inbound 私信 (requires `allowFrom: ["*"]`).
- **disabled**：忽略入站私信。
