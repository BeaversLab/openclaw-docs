...
4. 将下载的 JSON 文件保存在您的网关主机上（例如 `~/.openclaw/googlechat-service-account.json`）。
5. 在 [Google Cloud Console Chat Configuration](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat) 中创建一个 Google Chat 应用：
   ...
   - 在 **Triggers（触发器）** 下，选择 **Use a common HTTP endpoint URL for all triggers（对所有触发器使用通用的 HTTP 端点 URL）** 并将其设置为您的网关的公共 URL 后加上 `/googlechat`。
...
Google Chat webhook 需要公共 HTTPS 端点。为了安全起见，**仅将 `/googlechat` 路径暴露**给互联网。将 OpenClaw 仪表板和其他敏感端点保留在您的私有网络上。

### Option A: Tailscale Funnel (Recommended)

使用 Tailscale Serve 作为私有仪表板，使用 Funnel 作为公共 webhook 路径。这保持 `/` 私有，同时仅暴露 `/googlechat`。
...
使用此配置，任何对 `your-domain.com/` 的请求都将被忽略或返回 404，而 `your-domain.com/googlechat` 则被安全地路由到 OpenClaw。
...
1. Google Chat 向网关发送 Webhook POST 请求。每个请求都包含一个 `Authorization: Bearer <token>` 请求头。
   - 当存在该请求头时，OpenClaw 会在读取/解析完整的 Webhook 内容之前验证 Bearer 认证。
   - 在正文中携带 `authorizationEventObject.systemIdToken` 的 Google Workspace 插件请求通过更严格的预认证正文预算得到支持。
...
- 私信： `users/<userId>` （推荐）。
- 原始邮箱 `name@example.com` 是可变的，仅当 `channels.googlechat.dangerouslyAllowNameMatching: true` 时用于直接的允许列表匹配。
- 已弃用： `users/<email>` 被视为用户 ID，而不是邮箱允许列表。
- 空间： `spaces/<spaceId>`。
...
- `serviceAccountRef` 也支持（环境变量/文件 SecretRef），包括 `channels.googlechat.accounts.<id>.serviceAccountRef` 下的每个账号的引用。
- 如果未设置 `webhookPath`，默认的 webhook 路径为 `/googlechat`。
- `dangerouslyAllowNameMatching` 重新为允许列表启用了可变电子邮件主体匹配（紧急兼容模式）。
...
1. **未配置频道**：您的配置中缺少 `channels.googlechat` 部分。使用以下命令验证：
...
- 如果提及 gating 阻止回复，请将 `botUser` 设置为应用的用户资源名称并验证 `requireMention`。
