...
- 可选的渠道默认账户：`channels.<channel>.defaultAccount` 选择
  ...
  - 在多账户设置中，当配置了两个或更多账户时，请设置一个显式的默认值（`defaultAccount` 或 `accounts.default`）。否则，回退路由可能会选择第一个标准化的账户 ID。
...
在这种不匹配的情况下，OpenClaw 仍会记录入站会话元数据，但
会跳过更新主会话 `lastRoute`。
...
参见：[广播组](/en/channels/broadcast-groups)。
...
Gateway 和 ACP 会话发现还会扫描默认 `agents/` 根目录和模板化 `session.store` 根目录下的基于磁盘的代理存储。发现的存储必须保留在该解析的代理根目录内，并使用常规的 `sessions.json` 文件。符号链接和根目录外的路径将被忽略。
