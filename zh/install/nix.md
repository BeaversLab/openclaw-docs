...
通过 Nix 运行 OpenClaw 的推荐方式是使用 **[nix-openclaw](https://github.com/openclaw/nix-openclaw)** —— 一个功能齐全的 Home Manager 模块。
...
>
> nix-openclaw 仓库是 Nix 安装的事实来源。本页仅作简要概述。

## 您将获得

- Gateway 网关 + macOS 应用 + 工具 —— 全部固定版本
- 重启后仍能存活的 Launchd 服务
- 具有声明式配置的插件系统
...
- UI 界面会显示一个只读的 Nix 模式横幅

## 打包说明 (macOS)

macOS 打包流程期望在以下位置拥有一个稳定的 Info.plist 模板：
...
（Bundle ID、版本/构建号、Git SHA、Sparkle 密钥）。这可以使 plist 对于 SwiftPM
