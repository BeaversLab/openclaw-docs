...
<AccordionGroup>
  ...
  <Accordion title="npm / pnpm" icon="package">
    如果您自己管理 Node，我们推荐使用 Node 24。为了兼容性，OpenClaw 仍然支持 Node 22 LTS，即 `22.16+`：
    ...
    <Tabs>
      <Tab title="npm">
        ...
        <Accordion title="sharp 构建错误？">
          如果您全局安装了 libvips（在 macOS 上通过 Homebrew 安装时很常见）并且 `sharp` 失败，请强制使用预构建的二进制文件：
      ...
      <Tab title="pnpm">
        ...
        <Note>
        pnpm 需要明确批准带有构建脚本的包。在首次安装显示“Ignored build scripts”警告后，运行 `pnpm approve-builds -g` 并选择列出的包。
        </Note>
      </Tab>
    </Tabs>
  ...
  <Accordion title="From source" icon="github">
    ...
    <Steps>
      <Step title="Clone and build">
        克隆 [OpenClaw 代码库](https://github.com/openclaw/openclaw) 并构建：
