...
- **沙盒说明**：代理沙盒也使用 Docker，但**不**要求整个网关在 Docker 中运行。请参阅 [沙盒](/en/gateway/sandboxing)。
...
<Note>
此处的 Docker 默认设置假设为绑定模式（`lan`/`loopback`），而非主机别名。请在 `gateway.bind` 中使用绑定模式值
（例如 `lan` 或 `loopback`），而不要使用主机别名，如
`0.0.0.0` 或 `localhost`。
</Note>
...
- `OPENCLAW_SANDBOX` — 选择启用 Docker 网关沙箱引导。只有明确的真值才能启用它：`1`、`true`、`yes`、`on`
- `OPENCLAW_INSTALL_DOCKER_CLI` — 本地镜像构建的构建参数透传（`1` 在镜像中安装 Docker CLI）。当为本地构建 `OPENCLAW_SANDBOX=1` 时，`docker-setup.sh` 会自动设置此项。
...
- 如果沙盒设置无法完成，脚本将重置
  `agents.defaults.sandbox.mode` 到 `off` 以避免在重新运行时
  出现过时/损坏的沙箱配置。
- 如果缺少 `Dockerfile.sandbox`，脚本会打印警告并继续；
  如有必要，使用 `scripts/sandbox-setup.sh`
  构建 `openclaw-sandbox:bookworm-slim`。
...
如果您的自动化未导出 Claude 会话变量，现在保持它们未设置状态，默认情况下在 `docker-compose.yml` 中解析为
空值，以避免重复出现“未设置变量”警告。
...
将此视为共享信任边界：回环绑定并不在这两个容器之间提供隔离。如果您需要更强的隔离，请从单独的容器/主机网络路径运行命令，而不是使用捆绑的 `openclaw-cli` 服务。

为了降低 CLI 进程被攻陷后的影响，Compose 配置丢弃了 `NET_RAW`/`NET_ADMIN` 并在 `openclaw-cli` 上启用了 `no-new-privileges`。
...
使用镜像名称 `ghcr.io/openclaw/openclaw`（而不是名称相似的 Docker Hub 镜像）。
...
参考：[OCI 镜像注释](https://github.com/opencontainers/image-spec/blob/main/annotations.md)

发布上下文：此仓库的标记历史已经在 `v2026.2.22` 和更早的 2026 标签中使用了 Bookworm（例如 `v2026.2.21`、`v2026.2.9`）。

默认情况下，设置脚本从源代码构建镜像。要改为拉取预构建的镜像，请在运行脚本前设置 `OPENCLAW_IMAGE`：
...
脚本检测到 `OPENCLAW_IMAGE` 不是默认的 `openclaw:local`，因此运行 `docker pull` 而不是 `docker build`。其他所有内容（onboarding、网关启动、令牌生成）的工作方式相同。

`docker-setup.sh` 仍然从存储库根目录运行，因为它使用本地的 `docker-compose.yml` 和辅助文件。`OPENCLAW_IMAGE` 跳过了本地镜像构建时间；它不会替换 compose/setup 工作流。

### Shell 助手（可选）

为了更方便日常管理 Docker，请安装 `ClawDock`：
...
注意：请从仓库根目录运行 `docker compose ...`。如果您启用了 `OPENCLAW_EXTRA_MOUNTS` 或 `OPENCLAW_HOME_VOLUME`，设置脚本会写入 `docker-compose.extra.yml`；在其他地方运行 Compose 时请包含它：
...
更多信息：[Dashboard](/en/web/dashboard)、[Devices](/en/cli/devices)。

### 额外挂载（可选）

如果您想将额外的主机目录挂载到容器中，请在运行 `docker-setup.sh` 之前设置 `OPENCLAW_EXTRA_MOUNTS`。这接受一个逗号分隔的 Docker 绑定挂载列表，并通过生成 `docker-compose.extra.yml` 将其应用于 `openclaw-gateway` 和 `openclaw-cli`。
...
- 每个条目必须是 `source:target[:options]`，且没有空格、制表符或换行符。
...
如果您希望 `/home/node` 在容器重新创建后持久保存，请通过 `OPENCLAW_HOME_VOLUME` 设置一个命名卷。这会创建一个 Docker 卷并将其挂载到 `/home/node`，同时保留标准的 config/workspace 绑定挂载。请在此处使用命名卷（而不是绑定路径）；对于绑定挂载，请使用 `OPENCLAW_EXTRA_MOUNTS`。
...
- 如果您更改了 `OPENCLAW_HOME_VOLUME`，请重新运行 `docker-setup.sh` 以重新生成
  额外的 compose 文件。
- 命名卷会一直存在，直到使用 `docker volume rm <name>` 将其删除。

### 安装额外的 apt 软件包（可选）

如果您需要在镜像内部安装系统软件包（例如构建工具或媒体库），请在运行 `docker-setup.sh` 之前设置 `OPENCLAW_DOCKER_APT_PACKAGES`。这会在镜像构建期间安装这些软件包，因此即使删除了容器，它们也会保留。
...
拥有自己的 `package.json` 的扩展（例如 `diagnostics-otel`、`matrix`、`msteams`）会在首次加载时安装其 npm 依赖项。要将这些依赖项烘焙到镜像中，请在运行 `docker-setup.sh` 之前设置 `OPENCLAW_EXTENSIONS`：
...
- 只有带有 `package.json` 的扩展才会受到影响；没有该文件的轻量级插件将被忽略。
- 如果您更改了 `OPENCLAW_EXTENSIONS`，请重新运行 `docker-setup.sh` 以重新构建
  该镜像。

### 高级用户 / 功能齐全的容器（可选）

默认的 Docker 镜像是 **安全优先** 的，并以非 root `node` 用户身份运行。这减小了攻击面，但意味着：
...
`OPENCLAW_DOCKER_APT_PACKAGES` 而不是在运行时使用 `--with-deps` 来重新构建镜像。

4. **持久化 Playwright 浏览器下载**：

- 在以下位置设置 `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright`
  `docker-compose.yml`。
...
`/home/node/.openclaw` 上看到权限错误，请确保您的主机绑定挂载归 uid 1000 所有。
...
为了加快重新构建的速度，请对您的 Dockerfile 进行排序，以便缓存依赖层。
这可以避免在锁定文件（lockfiles）更改时重新运行 `pnpm install`：
...
如果您在向导中选择 OpenAI Codex OAuth，它会打开一个浏览器 URL 并尝试
在 `http://127.0.0.1:1455/auth/callback` 上捕获回调。在 Docker 或
无头（headless）设置中，该回调可能会显示浏览器错误。复制您跳转到的完整重定向
URL 并将其粘贴回向导以完成身份验证。
...
`/readyz` 在启动宽限期间保持就绪状态，只有在所需
的托管通道在宽限期后仍未连接或稍后断开连接时，才会变为 `503`。

Docker 镜像包含一个内置的 `HEALTHCHECK`，它会在
后台对 `/healthz` 进行 ping 检查。简而言之：Docker 会持续检查 OpenClaw 是否
仍然响应。如果检查持续失败，Docker 会将容器标记为 `unhealthy`，
并且编排系统（Docker Compose 重启策略、Swarm、Kubernetes
等）可以自动重启或替换它。
...
`docker-setup.sh` 默认为 `OPENCLAW_GATEWAY_BIND=lan`，以便主机对 `http://127.0.0.1:18789` 的访问
能与 Docker 端口发布配合使用。

- `lan`（默认）：主机浏览器 + 主机 CLI 可以访问发布的网关端口。
- `loopback`：只有容器网络命名空间内的进程可以访问
  直接访问网关；主机发布的端口访问可能会失败。

设置脚本还会在引导后将 `gateway.mode=local` 固定为本地回环，以便 Docker CLI
命令默认以本地回环为目标。
...
如果您在 Docker CLI 命令中看到 `Gateway target: ws://172.x.x.x:18789` 或重复的 `pairing required`
错误，请运行：
...
- Dockerfile CMD 使用 `--allow-unconfigured`；使用 `gateway.mode` 而非 `local` 的挂载配置仍将启动。覆盖 CMD 以强制执行保护。
- 网关容器是会话的可信来源（`~/.openclaw/agents/<agentId>/sessions/`）。

### 存储模型

- **持久化主机数据：** Docker Compose 将 `OPENCLAW_CONFIG_DIR` 绑定挂载到 `/home/node/.openclaw` 并将 `OPENCLAW_WORKSPACE_DIR` 绑定挂载到 `/home/node/.openclaw/workspace`，因此这些路径在容器替换后仍然保留。
- **临时沙箱 tmpfs：** 当启用 `agents.defaults.sandbox` 时，沙箱容器会对 `/tmp`、`/var/tmp` 和 `/run` 使用 `tmpfs`。这些挂载与顶层的 Compose 栈是分开的，并会随沙箱容器一起消失。
- **磁盘增长热点：** 请注意 `media/`、`agents/<agentId>/sessions/sessions.json`、transcript JSONL 文件、`cron/runs/*.jsonl` 以及 `/tmp/openclaw/`（或您配置的 `logging.file`）下的滚动文件日志。如果您也在 Docker 外部运行 macOS 应用，其服务日志也是分开的：`~/.openclaw/logs/gateway.log`、`~/.openclaw/logs/gateway.err.log` 和 `/tmp/openclaw/openclaw-gateway.log`。

## Agent Sandbox（主机网关 + Docker 工具）

深入探讨：[沙箱机制](/en/gateway/sandboxing)

### 其作用

当启用 `agents.defaults.sandbox` 时，**非主会话**会在 Docker 容器内运行工具。网关保留在您的主机上，但工具执行是隔离的：

- scope：默认为 `"agent"`（每个代理一个容器 + 工作区）
- scope：`"session"` 用于每会话隔离
- 每个 scope 的工作区文件夹挂载于 `/workspace`
- 可选的代理工作区访问（`agents.defaults.sandbox.workspaceAccess`）
- 允许/拒绝工具策略（拒绝优先）
- 入站媒体会被复制到活动的沙箱工作区（`media/inbound/*`）中，以便工具读取（如果 `workspaceAccess: "rw"`，这会进入代理工作区）

警告：`scope: "shared"` 会禁用跨会话隔离。所有会话共享一个容器和一个工作区。

### Per-agent sandbox profiles (multi-agent)

如果您使用多代理路由，每个代理都可以覆盖沙箱 + 工具设置：`agents.list[].sandbox` 和 `agents.list[].tools`（加上 `agents.list[].tools.sandbox.tools`）。这允许您在一个网关中运行混合访问级别：

- 完全访问（个人代理）
- 只读工具 + 只读工作区（家庭/工作代理）
- 无文件系统/Shell 工具（公开代理）

有关示例、优先级和故障排除，请参阅[Multi-Agent Sandbox & Tools](/en/tools/multi-agent-sandbox-tools)。
...
- 代理工作区访问：`workspaceAccess: "none"`（默认）使用 `~/.openclaw/sandboxes`
  - `"ro"` 将沙盒工作区保留在 `/workspace`，并以只读方式在 `/agent` 挂载代理工作区（禁用 `write`/`edit`/`apply_patch`）
  - `"rw"` 以读/写方式在 `/workspace` 挂载代理工作区
- 自动清理：空闲 > 24h 或 年龄 > 7d
- 网络：默认为 `none`（如果需要出站流量，请显式选择加入）
  - `host` 被阻止。
...
- `user` 必须是 root 才能执行 `apt-get`（省略 `user` 或设置 `user: "0:0"`）。
  当 `setupCommand`（或 docker 配置）发生变化时，除非容器是**最近使用过的**（约 5 分钟内），否则
  OpenClaw 会自动重新创建容器。热容器会记录一条包含确切 `openclaw sandbox recreate ...` 命令的警告。
...
`dangerouslyAllowContainerNamespaceJoin`（仅限紧急情况使用）。
...
这会使用
`Dockerfile.sandbox-browser` 构建 `openclaw-sandbox-browser:bookworm-slim`。容器运行启用了 CDP 的 Chromium 以及
可选的 noVNC 观察器（通过 Xvfb 有头模式）。
...
- 针对共享/容器工作负载，浏览器容器启动默认值较为保守，包括：
  ...
  - `--no-zygote`
  - `--disable-extensions`
  - 如果设置了 `agents.defaults.sandbox.browser.noSandbox`，也会追加
  ...
  - 扩展行为由 `--disable-extensions` 控制，并且可以通过
    `OPENCLAW_BROWSER_DISABLE_EXTENSIONS=0` 禁用（启用扩展），
    用于依赖扩展的页面或重度使用扩展的工作流。
  - `--renderer-process-limit=2` 也可以使用
    `OPENCLAW_BROWSER_RENDERER_PROCESS_LIMIT` 进行配置；当需要调整浏览器并发时，设置 `0` 以让 Chromium 选择其
    默认进程限制。
...
- `deny` 优先于 `allow`。
- 如果 `allow` 为空：所有工具（拒绝列表中的除外）均可用。
- 如果 `allow` 非空：则仅 `allow` 中的工具可用（减去被拒绝的）。
...
- 在沙箱中允许 `browser` 会**破坏隔离性**（浏览器在宿主机上运行）。
...
- 沙箱中出现权限错误：将 `docker.user` 设置为与您匹配的 UID:GID
  挂载的工作区所有权（或对工作区文件夹执行 chown）。
- 找不到自定义工具：OpenClaw 使用 `sh -lc`（登录 shell）运行命令，这会
  导入 `/etc/profile` 并可能重置 PATH。设置 `docker.env.PATH` 以在前面添加您的
