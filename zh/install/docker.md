...
- **沙箱注意事项**：代理沙箱隔离也使用 Docker，但它**不**要求完整的 Gateway 网关在 Docker 中运行。请参阅[沙箱隔离](/zh/gateway/sandboxing)。
...
- 足够的磁盘空间用于镜像 + 日志
- 如果在 VPS/公共主机上运行，请查看
  [针对网络暴露的安全加固](/zh/gateway/security#04-network-exposure-bind--port--firewall)，
...
有关示例、
优先级和故障排除，请参阅 [多代理沙箱与工具](/zh/tools/multi-agent-sandbox-tools)。
