...
- **沙盒说明**：代理沙盒也使用 Docker，但**不**要求整个网关在 Docker 中运行。请参阅 [沙盒](/zh/en/gateway/沙箱隔离)。
...
脚本检测到 `OPENCLAW_IMAGE` 不是默认的 `openclaw:local`，因此运行 `docker pull` 而不是 `docker build`。其他所有内容（新手引导、网关启动、令牌生成）的工作方式相同。
...
- Gateway 网关 绑定默认为 `lan` 以供容器使用（`OPENCLAW_GATEWAY_BIND`）。
...
有关示例、优先级和故障排除，请参阅[Multi-Agent 沙箱 & Tools](/zh/en/tools/multi-agent-sandbox-tools)。
