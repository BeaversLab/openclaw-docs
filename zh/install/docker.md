...
- **沙箱隔离注意事项**：代理沙箱隔离也使用 Docker，但这**并**不要求完整的网关在 Docker 中运行。请参阅 [沙箱隔离](/zh/gateway/sandboxing)。
...
- 足够的磁盘空间用于镜像 + 日志
- 如果在 VPS/公共主机上运行，请查看
  [网络暴露的安全加固](/zh/gateway/security#04-network-exposure-bind--port--firewall)，
...
深入探索：[沙箱隔离](/zh/gateway/sandboxing)
...
请参阅 [Multi-Agent 沙箱 & Tools](/zh/tools/multi-agent-sandbox-tools) 了解示例、
优先级和故障排除。
...
- Docker 和其他无头/容器浏览器流程保持在原始 CDP 上。Chrome MCP `existing-session` 用于主机本地的 Chrome，而非容器接管。
- 相比无头模式，有头模式 (Xvfb) 可减少机器人拦截。
...
- 可选的 `agents.defaults.sandbox.browser.cdpSourceRange` 通过 CIDR 限制容器边缘的 CDP 入站流量（例如 `172.21.0.1/32`）。
- 默认情况下，noVNC 观察者访问受密码保护；OpenClaw 提供一个短期观察者令牌 URL，该 URL 提供本地引导页面并将密码保留在 URL 片段中（而不是 URL 查询中）。
- 浏览器容器启动默认值对于共享/容器工作负载是保守的，包括：
  ...
  - 上述三个图形加固标志是可选的。如果你的工作负载需要
    WebGL/3D，请设置 `OPENCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` 以便在不使用
...
请记住：如果你对工具使用允许列表，请添加 `browser`（并从拒绝列表中
移除它），否则该工具仍将被阻止。
...
- 如果 `allow` 非空：则仅 `allow` 中的工具可用（减去拒绝列表）。
...
- 容器未运行：它将按需为每个会话自动创建。
- 沙箱中的权限错误：将 `docker.user` 设置为与您
  挂载的工作区所有权匹配的 UID:GID（或 chown 工作区文件夹）。
- 找不到自定义工具：OpenClaw 使用 `sh -lc`（登录 shell）运行命令，
  该命令会导入 `/etc/profile` 并可能重置 PATH。设置 `docker.env.PATH` 以在前面添加您的
  自定义工具路径（例如 `/custom/bin:/usr/local/share/npm-global/bin`），或者
  在 Dockerfile 的 `/etc/profile.d/` 下添加脚本。
