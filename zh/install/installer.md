...
OpenClaw 提供了三个安装程序脚本，由 `openclaw.ai` 提供。
...
<Note>
  如果安装成功但在新终端中找不到 `openclaw`，请参阅 [Node.js 故障排除](/en/install/node#故障排除)。
</Note>
...
<Steps>
  <Step title="检测操作系统">
    支持 macOS 和 Linux（包括 WSL）。如果检测到 macOS，若缺少 Homebrew 则会安装它。
  </Step>
  <Step title="默认确保 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（在 macOS 上使用 Homebrew，在 Linux apt/dnf/yum 上使用
    NodeSource 设置脚本）。OpenClaw 仍然支持 Node 22 LTS，目前为 `22.16+`，以确保 兼容性。
  </Step>
  ...
  <Step title="安装后任务">
    ...
    在适当时尝试新手引导（TTY 可用，未禁用新手引导，并且 bootstrap/config 检查通过） - 默认
...
如果不存在 TTY 且未设置安装方法，则默认为 `npm` 并发出警告。
...
<Info>
  专为您希望将所有内容置于本地前缀（默认 `~/.openclaw`）下且无系统 Node 依赖的环境而设计。
</Info>
...
<Steps>
  ...
  <Step title="在 prefix 下安装 OpenClaw">
...
<Steps>
  ...
  <Step title="默认确保 Node.js 24">
    如果缺失，会尝试通过 winget、然后 Chocolatey、再然后 Scoop 进行安装。Node 22 LTS，目前为
    `22.16+`，为了兼容性仍然受支持。
  </Step>
  <Step title="安装 OpenClaw">
...
<Note>如果使用 `-InstallMethod git` 且缺少 Git，脚本将退出并打印 Git for Windows 的链接。</Note>
...
<Accordion title="为什么 npm 在 Linux 上遇到 EACCES？">
  某些 Linux 设置将 npm 全局前缀指向 root 所拥有的路径。`install.sh` 可以将前缀切换到
  `~/.npm-global` 并将 PATH 导出附加到 shell rc 文件（当这些文件存在时）。
</Accordion>
