...
OpenClaw 提供三个安装程序脚本，由 `openclaw.ai` 托管。

| 脚本                               | 平台                 | 作用                                                                            |
| ---------------------------------- | -------------------- | ------------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL  | 如需要则安装 Node，通过 OpenClaw（默认）或 git 安装 npm，并且可以运行新手引导。 |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | 将 Node + OpenClaw 安装到本地前缀（`~/.openclaw`）中。无需 root 权限。          |
| [`install.ps1`](#installps1)       | Windows (PowerShell) | 如需要则安装 Node，通过 OpenClaw（默认）或 git 安装 npm，并且可以运行新手引导。 |
...
<Note>
  如果安装成功但在新终端中找不到 `openclaw`，请参阅 [Node.js
  故障排除](/zh/install/node#troubleshooting)。
</Note>
...
<Steps>
  <Step title="检测操作系统">
    支持 macOS 和 Linux（包括 WSL）。如果检测到 macOS，若缺少 Homebrew 则安装它。
  </Step>
  <Step title="默认确保 Node.js 24">
    检查 Node 版本，如需要则安装 Node 24（macOS 上使用 Homebrew，Linux 上使用 NodeSource 设置脚本
    apt/dnf/yum）。OpenClaw 目前仍支持 `22.16+` 以保证兼容性。
  </Step>
  <Step title="确保 Git">如果缺少 Git 则安装。</Step>
  ...
  <Step title="安装后任务">
    ...
    在适当的情况下尝试新手引导（TTY 可用、未禁用新手引导，且 bootstrap/config 检查通过） - 默认为
...
| `--no-git-update`                     | 对于现有的检出，跳过 `git pull`                      |
| `--no-prompt`                         | 禁用提示                                             |
| `--no-onboard`                        | 跳过新手引导                                         |
| `--onboard`                           | 启用新手引导                                         |
| `--dry-run`                           | 在不应用更改的情况下打印操作                         |
...
<Info>
  专为以下环境设计：您希望将所有内容置于本地前缀（默认 `~/.openclaw`）下，且无系统 Node 依赖。
</Info>
...
<Steps>
  <Step title="安装本地 Node 运行时">
    下载固定的受支持 Node 压缩包（当前默认 `22.22.0`）到 `<prefix>/tools/node-v<version>` 并验证 SHA-256。
  </Step>
  <Step title="确保 Git">
    如果缺少 Git，尝试通过 Linux 上的 apt/dnf/yum 或 macOS 上的 Homebrew 进行安装。
  </Step>
...
<Steps>
  ...
  <Step title="默认确保 Node.js 24">
    如果缺失，尝试通过 winget 安装，然后是 Chocolatey，接着是 Scoop。Node 22 LTS，目前是
    `22.16+`，仍受支持以保证兼容性。
  </Step>
  <Step title="安装 OpenClaw">
    - `npm` 方法（默认）：使用选定的 `-Tag` 进行全局 npm 安装 - `git` 方法：克隆/更新仓库，使用 pnpm
    安装/构建，并在 `%USERPROFILE%\.local\bin\openclaw.cmd` 安装包装器
  </Step>
  <Step title="安装后任务">
    在可能的情况下，将所需的 bin 目录添加到用户 PATH，然后在升级和 git 安装时运行 `openclaw doctor
...
<Accordion title="为什么 npm 在 Linux 上遇到 EACCES？">
  一些 Linux 设置将 npm 全局前缀指向 root 所拥有的路径。`install.sh` 可以将前缀切换到
  `~/.npm-global` 并将 PATH 导出添加到 shell rc 文件（当这些文件存在时）。
</Accordion>
...
<Accordion title="Windows：“npm error spawn git / ENOENT”">
  为 Windows 安装 Git，重新打开 PowerShell，重新运行安装程序。
</Accordion>
