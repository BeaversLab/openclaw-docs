...
<Note>
如果安装成功但在新终端中找不到 `openclaw`，请参阅 [Node.js 故障排除](/zh/install/node#troubleshooting)。
</Note>
...
<Steps>
  ...
  <Step title="默认确保 Node.js 24">
    检查 Node 版本，如需要则安装 Node 24（macOS 上使用 Homebrew，Linux 上使用 NodeSource 设置脚本 apt/dnf/yum）。OpenClaw 目前仍支持 `22.16+` 以保证兼容性。
  </Step>
  <Step title="确保 Git">
    如果缺少 Git 则安装。
  </Step>
  ...
  <Step title="安装后任务">
    ...
    - 在适当的情况下尝试新手引导（TTY 可用、未禁用新手引导，且 bootstrap/config 检查通过）
...
<Steps>
  ...
  <Step title="默认确保 Node.js 24">
    如果缺失，尝试通过 winget 安装，然后是 Chocolatey，接着是 Scoop。Node 22 LTS，目前是 `22.16+`，仍受支持以保证兼容性。
  </Step>
  <Step title="安装 OpenClaw">
  ...
  <Step title="安装后任务">
    在可能的情况下，将所需的 bin 目录添加到用户 PATH，然后在升级和 git 安装时运行 `openclaw doctor --non-interactive`（尽力而为）。
  </Step>
</Steps>
...
<Note>
如果使用了 `-InstallMethod git` 但缺少 Git，脚本将退出并打印 Git for Windows 链接。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="为什么 npm 在 Linux 上遇到 EACCES？">
    一些 Linux 设置将 npm 全局前缀指向 root 所拥有的路径。`install.sh` 可以将前缀切换到 `~/.npm-global` 并将 PATH 导出添加到 shell rc 文件（当这些文件存在时）。
  </Accordion>
