...
<Note>
  如果安装成功但在新的终端中找不到 `openclaw`，请参阅 [Node.js
...
<Steps>
  ...
  <Step title="确保默认使用 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（在 macOS 上使用 Homebrew，在 Linux 上使用 NodeSource
    设置脚本 针对 apt/dnf/yum）。OpenClaw 仍支持 Node 22 LTS，目前为 `22.16+`，以确保 兼容性。
  </Step>
  <Step title="确保 Git">如果缺少 Git，则安装 Git。</Step>
  <Step title="安装 OpenClaw">
    - `npm` 方法（默认）：全局 npm 安装 - `git` 方法：克隆/更新仓库，使用 pnpm 安装依赖，
    然后构建，接着在 `~/.local/bin/openclaw` 安装包装器
  </Step>
  <Step title="安装后任务">
    - 在升级和 git 安装时运行 `openclaw doctor --non-interactive`（尽力而为） - 在适当时尝试
    新手引导（TTY 可用、未禁用新手引导，并且 bootstrap/config 检查 通过） - 默认
...
<Steps>
  <Step title="安装本地 Node 运行时">
    下载固定的受支持 Node 压缩包（目前默认为 `22.22.0`）到 `<prefix>/tools/node-v<version>` 并验证 SHA-256。
  </Step>
  <Step title="确保 Git">
    如果缺少 Git，则尝试通过 Linux 上的 apt/dnf/yum 或 macOS 上的 Homebrew 进行安装。
  </Step>
  <Step title="在前缀下安装 OpenClaw">
    使用 npm 和 `--prefix <prefix>` 进行安装，然后将包装器写入 `<prefix>/bin/openclaw`。
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="默认确保 Node.js 24">
    如果缺失，会尝试通过 winget、Chocolatey 然后 Scoop 进行安装。为了兼容性，目前仍支持 Node 22
    LTS（ `22.16+`）。
  </Step>
  <Step title="安装 OpenClaw">
    - `npm` 方法（默认）：使用选定的 `-Tag` 进行全局 npm 安装 - `git` 方法：克隆/更新 仓库，使用
    pnpm 安装/构建，并在 `%USERPROFILE%\.local\bin\openclaw.cmd` 安装包装器
  </Step>
  <Step title="安装后任务">
...
<Accordion title="为什么 npm 在 Linux 上会遇到 EACCES？">
  某些 Linux 设置将 npm 全局前缀指向 root 所有的路径。`install.sh` 可以将前缀切换到 `~/.npm-global`
  并将 PATH 导出添加到 shell rc 文件（当这些文件存在时）。
</Accordion>
