...
如果打印出 `v24.x.x` 或更高版本，说明您使用的是推荐的默认版本。如果打印出 `v22.16.x` 或更高版本，说明您使用的是受支持的 Node 22 LTS 版本，但我们仍然建议您在方便时升级到 Node 24。如果未安装 Node 或版本过旧，请从下方选择一种安装方法。
...
```
  ...
  <Warning>
  请确保你的版本管理器在你的 shell 启动文件（`~/.zshrc` 或 `~/.bashrc`）中已初始化。如果没有，新的终端会话中可能无法找到 `openclaw`，因为 PATH 不会包含 Node 的 bin 目录。
  </Warning>
</Accordion>
