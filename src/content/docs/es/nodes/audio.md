...
4. **Autenticación del proveedor**
   - Las entradas `models.providers.*` configuradas que admiten audio se intentan primero
   - Orden de reserva incluido: OpenAI → Groq → Deepgram → Google → Mistral
...
Nota: La detección binaria es de mejor esfuerzo en macOS/Linux/Windows; asegúrese de que la CLI esté en `PATH` (expandimos `~`), o establezca un modelo de CLI explícito con una ruta de comando completa.
...
- OpenAI auto predeterminado es `gpt-4o-mini-transcribe`; establezca `model: "gpt-4o-transcribe"` para mayor precisión.
...
- `tools.media.audio.echoTranscript` está desactivado por defecto; actívelo para enviar la confirmación de la transcripción de vuelta al chat de origen antes del procesamiento del agente.
- `tools.media.audio.echoFormat` personaliza el texto de eco (marcador de posición: `{transcript}`).
- El stdout de CLI está limitado (5MB); mantenga la salida de CLI concisa.
...
Si no se establecen variables de entorno de proxy, se usa una salida directa. Si la configuración del proxy está mal formada, OpenClaw registra una advertencia y recurre a una recuperación directa.

## Detección de menciones en grupos

Cuando `requireMention: true` se establece para un chat de grupo, OpenClaw ahora transcribe el audio **antes** de verificar las menciones. Esto permite que se procesen las notas de voz incluso cuando contienen menciones.
...
2. La transcripción se verifica para detectar patrones de mención (por ejemplo, `@BotName`, disparadores de emoji).
3. Si se encuentra una mención, el mensaje continúa por la canalización de respuesta completa.
4. La transcripción se usa para la detección de menciones para que las notas de voz puedan pasar el filtro de menciones.

**Comportamiento de respaldo:**

- Si la transcripción falla durante el preflight (tiempo de espera, error de API, etc.), el mensaje se procesa basándose en la detección de menciones solo de texto.
- Esto asegura que los mensajes mixtos (texto + audio) nunca se descarten incorrectamente.

**Opt-out por grupo/tema de Telegram:**

- Establezca `channels.telegram.groups.<chatId>.disableAudioPreflight: true` para omitir las verificaciones de mención de transcripción de preflight para ese grupo.
- Establezca `channels.telegram.groups.<chatId>.topics.<threadId>.disableAudioPreflight` para anular por tema (`true` para omitir, `false` para forzar la activación).
- El valor predeterminado es `false` (preflight habilitado cuando coinciden las condiciones con filtrado por menciones).

**Ejemplo:** Un usuario envía una nota de voz diciendo "Hola @Claude, ¿qué tal el clima?" en un grupo de Telegram con `requireMention: true`. La nota de voz se transcribe, la mención se detecta y el agente responde.

## Advertencias

- Las reglas de ámbito usan el sistema de primera coincidencia gana. `chatType` se normaliza a `direct`, `group` o `room`.
- Asegúrese de que su CLI salga con 0 e imprima texto plano; el JSON necesita ser procesado a través de `jq -r .text`.
- Para `parakeet-mlx`, si pasa `--output-dir`, OpenClaw lee `<output-dir>/<media-basename>.txt` cuando `--output-format` es `txt` (u se omite); los formatos de salida que no son `txt` vuelven al análisis de stdout.
- Mantenga los tiempos de espera razonables (`timeoutSeconds`, por defecto 60s) para evitar bloquear la cola de respuestas.
