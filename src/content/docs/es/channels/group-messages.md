...
- Sesiones por grupo: las claves de sesión tienen el aspecto de `agent:<agentId>:whatsapp:group:<jid>`, por lo que comandos como `/verbose on`, `/trace on` o `/think high` (enviados como mensajes independientes) están limitados a ese grupo; el estado del MD personal no se ve afectado. Los latidos se omiten en los hilos de grupo.
- Inyección de contexto: los mensajes de grupo **solo pendientes** (50 por defecto) que _no_ activaron una ejecución se prefijan bajo `[Chat messages since your last reply - for context]`, con la línea desencadenante bajo `[Current message - respond to this]`. Los mensajes que ya están en la sesión no se vuelven a inyectar.
...
- WhatsApp todavía envía menciones canónicas a través de `mentionedJids` cuando alguien toca el contacto, por lo que el respaldo del número rara vez es necesario pero es una red de seguridad útil.
...
4. Las directivas a nivel de sesión (`/verbose on`, `/trace on`, `/think high`, `/new` o `/reset`, `/compact`) se aplican solo a la sesión de ese grupo; envíelas como mensajes independientes para que se registren. Su sesión personal de DM sigue siendo independiente.
...
- Los indicadores de escritura en grupos siguen `agents.defaults.typingMode` (predeterminado: `message` cuando no se mencionan).
