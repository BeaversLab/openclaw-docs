...
<Info>Si no ha creado ningún complemento de OpenClaw antes, lea [Getting Started](/es/plugins/building-plugins) primero para conocer la estructura básica del paquete y la configuración del manifiesto.</Info>
...
adaptador declara qué capacidades duraderas de envío final soporta realmente el
transporte nativo y dirige los envíos de texto/medios a las mismas funciones de
transporte que el adaptador heredado `outbound`. Solo declare una capacidad cuando una
prueba de contrato demuestre el efecto secundario nativo y el recibo devuelto.
Para ver el contrato completo de la API, ejemplos, matriz de capacidades,
reglas de recibo, finalización de vista previa en vivo, política de acuse de
recibo, pruebas y tabla de migración, consulte [Channel outbound API](/es/plugins/sdk-channel-outbound).
Si el adaptador `outbound` existente ya tiene los métodos de envío y los
metadatos de capacidad correctos, use `createChannelMessageAdapterFromOutbound(...)` para
derivar el adaptador `message` en lugar de escribir manualmente otro puente.
Los envíos del adaptador deben devolver valores `MessageReceipt`. Cuando el código de
compatibilidad aún necesita identificadores heredados, derívelos con `listMessageReceiptPlatformIds(...)`
o `resolveMessageReceiptPrimaryId(...)` en lugar de mantener campos `messageIds` paralelos
en el nuevo código del ciclo de vida.
...
`quietFinalization`. Los canales que finalicen una vista previa de borrador in situ
también deben declarar `message.live.finalizer.capabilities`, como `finalEdit`,
...
el progreso, la edición, la reserva/retención, la limpieza y el comportamiento de
recibo no puedan desviarse silenciosamente.
Los receptores entrantes que difieren los reconocimientos de la plataforma deben
declarar `message.receive.defaultAckPolicy` y `supportedAckPolicies` en lugar de ocultar el
tiempo de reconocimiento en el estado local del monitor. Cubra cada política
declarada con `verifyChannelMessageReceiveAckPolicyAdapterProofs(...)`.
...
`openclaw/plugin-sdk/channel-ingress-runtime` de las rutas de recepción en tiempo de ejecución.
La subruta mantiene la búsqueda de la plataforma y los efectos secundarios en el complemento, mientras
comparte la resolución del estado de la lista blanca, las decisiones de ruta/remitente/comando/evento/activación,
los diagnósticos redactados y el mapeo de admisión de turnos. Mantenga la normalización
de la identidad del complemento en el descriptor que pasa al resolvedor; no
serialice valores de coincidencia brutos del estado resuelto o de la decisión. Vea
[Channel ingress API](/es/plugins/sdk-channel-ingress) para el diseño de la API,
los límites de propiedad y las expectativas de prueba.
...
- Use `createNativeApprovalChannelRouteGates` de `openclaw/plugin-sdk/approval-native-runtime` cuando un canal admite tanto la entrega nativa de origen de sesión como objetivos de reenvío de aprobación explícitos. El asistente centraliza la selección de configuración de aprobación, el manejo de `mode`, los filtros de agente/sesión, el enlace de cuenta, la coincidencia de sesión-objetivo y la coincidencia de lista de objetivos, mientras que las personas que llaman siguen siendo propietarias del ID del canal, el modo de reenvío predeterminado, la búsqueda de cuenta, la verificación de transporte habilitado, la normalización del objetivo y la resolución del objetivo de origen del turno. No lo use para crear valores predeterminados de política de canal propiedad del núcleo; pase explícitamente el modo predeterminado documentado del canal.
- `createChannelNativeOriginTargetResolver` usa el comparador de rutas de canal compartido de forma predeterminada para objetivos `{ to, accountId, threadId }`. Pase `targetsMatch` solo cuando un canal tiene reglas de equivalencia específicas del proveedor, como la coincidencia de prefijo de marca de tiempo de Slack.
- Pase `normalizeTargetForMatch` a `createChannelNativeOriginTargetResolver` cuando el canal necesita canonicalizar los ID del proveedor antes de que se ejecute el comparador de rutas predeterminado o una devolución de llamada `targetsMatch` personalizada, conservando el objetivo original para la entrega. Use `normalizeTarget` solo cuando el objetivo de entrega resuelto en sí deba ser canonicalizado.
...
- `interactions` - ganchos opcionales de bind/unbind/clear-action para botones nativos o reacciones, además de un gancho opcional `cancelDelivered`. Implemente `cancelDelivered` cuando `deliverPending` registre estado en proceso o persistente (como un almacén de objetivos de reacción) para que el estado pueda liberarse si una parada del controlador cancela la entrega antes de que se ejecute `bindPending` o cuando `bindPending` no devuelve ningún manejador
- `observe` - ganchos opcionales de diagnóstico de entrega
- Si el canal necesita objetos propiedad del runtime como un cliente, token, aplicación Bolt o receptor de webhooks, regístrelos a través de `openclaw/plugin-sdk/channel-runtime-context`. El registro genérico de contexto de runtime permite al núcleo arrancar controladores impulsados por capacidades desde el estado de inicio del canal sin agregar pegamento específico de aprobación.
...
- El núcleo ahora también posee los avisos de rerute de aprobación. Los complementos del canal no deben enviar sus propios mensajes de seguimiento de "la aprobación fue a DMs / otro canal" desde `createChannelNativeApprovalRuntime`; en su lugar, exponga el origen exacto + el enrutamiento de DM del aprobador a través de los ayudantes de capacidad de aprobación compartidos y deje que el núcleo agregue las entregas reales antes de publicar cualquier aviso de vuelta al chat iniciador.
- Conservar el tipo de id de aprobación entregado de extremo a extremo. Los clientes nativos no deben
  adivinar ni reescribir el enrutamiento de aprobación exec vs plugin desde el estado local del canal.
- Diferentes tipos de aprobación pueden exponer intencionalmente diferentes superficies nativas.
  Ejemplos agrupados actuales:
  - Slack mantiene disponible el enrutamiento de aprobación nativo tanto para ids exec como plugin.
  - Matrix mantiene el mismo enrutamiento nativo de DM/canal y la UX de reacción para aprobaciones
    exec y de plugin, al tiempo que sigue permitiendo que la autenticación difiera por tipo de aprobación.
...
Para los puntos de entrada de canal activos (hot channel), prefiera las subrutas de tiempo de ejecución más específicas cuando solo necesite una parte de esa familia:
...
debe devolver los metadatos del canal, el adaptador de configuración seguro para la configuración, el adaptador de estado
y los metadatos del objetivo secreto del canal necesarios para esos resúmenes. No
inicie clientes, escuchas o tiempos de ejecución de transporte desde el punto de entrada de configuración.
...
el canal. Los archivos como `channel-plugin-api.ts` deben exportar el objeto
del complemento del canal sin importar asistentes de configuración, clientes de transporte, escuchas de socket,
iniciadores de subprocesos o módulos de inicio de servicio. Coloque esas partes
del tiempo de ejecución en módulos cargados desde `registerFull(...)`, definidores de tiempo de ejecución o adaptadores
de capacidades diferidas.
...
Si su canal solo quiere anunciar "instale primero este complemento" en las superficies
de configuración, prefiera `createOptionalChannelSetupSurface(...)`. El adaptador/asistente
generado falla de forma cerrada en las escrituras y finalización de la configuración, y reutilizan
el mismo mensaje de instalación requerida en la validación, finalización y copia del enlace de documentación.

Para otras rutas activas del canal, prefiera los asistentes estrechos sobre las superficies heredadas
más amplias:
...
- `openclaw/plugin-sdk/inbound-envelope` y
  `openclaw/plugin-sdk/channel-inbound` para la ruta/sobre entrante y
  el cableado de registro y despacho
- `openclaw/plugin-sdk/channel-targets` para los asistentes de análisis de objetivos
...
- `buildThreadAwareOutboundSessionRoute(...)` de
  `openclaw/plugin-sdk/channel-core` cuando una ruta saliente debe conservar un `replyToId`/`threadId` explícito o recuperar la sesión `:thread:` actual
  después de que la clave de sesión base siga coincidiendo. Los complementos del proveedor pueden anular
  la precedencia, el comportamiento del sufijo y la normalización del ID de hilo cuando su plataforma
...
- `openclaw/plugin-sdk/telegram-command-config` para la normalización de comandos personalizados de Telegram,
  la validación de duplicados/conflictos y un contrato de configuración de comandos estable de respaldo
...
Use `openclaw/plugin-sdk/channel-mention-gating` para decisiones sobre políticas de menciones.
Use `openclaw/plugin-sdk/channel-inbound` solo cuando necesite el contenedor de ayudantes entrantes más amplio.
...
Adecuado para el ayudante compartido:
...
1. Calcule los datos locales de menciones.
2. Pase esos datos a `resolveInboundMentionDecision({ facts, policy })`.
3. Use `decision.effectiveWasMentioned`, `decision.shouldBypassMention` y `decision.shouldSkip` en su puerta de enlace entrante.
...
`openclaw/plugin-sdk/channel-mention-gating` para evitar cargar asistentes de tiempo de ejecución entrantes no relacionados.

Usa `resolveInboundMentionDecision({ facts, policy })` para la puerta de enlace de menciones.
...
<Steps>
  ...
  <Step title="Paquete y manifiesto">
    ...
    consulta [Configuración y ajustes del complemento](/es/plugins/sdk-setup#openclaw-channel):
    ...
    configuraciones propias del complemento que no son la configuración de la cuenta del canal. `channelConfigs`
    valida `channels.acme-chat` y es la fuente de ruta fría utilizada por el esquema de configuración,
    configuración y superficies de interfaz de usuario antes de que se cargue el tiempo de ejecución del complemento.
  ...
  <Step title="Construir el objeto del complemento de canal">
    ...
    Para canales que aceptan tanto claves DM canónicas de nivel superior como claves anidadas heredadas, use los auxiliares de `plugin-sdk/channel-config-helpers`: `resolveChannelDmAccess`, `resolveChannelDmPolicy`, `resolveChannelDmAllowFrom` y `normalizeChannelDmPolicy` mantienen los valores locales de la cuenta por delante de los valores raíz heredados. Empareje el mismo solucionador con la reparación de doctor a través de `normalizeLegacyDmAliases` para que el tiempo de ejecución y la migración lean el mismo contrato.
    ...
    <Accordion title="Lo que createChatChannelPlugin hace por usted">
      ...
      cuando se resolvió un objetivo de respuesta nativo, por lo que los auxiliares de carga útil pueden conservar
      etiquetas de respuesta explícitas sin consumir un slot de respuesta implícito de un solo uso.
    </Accordion>
  ...
  <Step title="Añadir una entrada de configuración">
    ...
    o sin configurar. Evita introducir código de tiempo de ejecución pesado durante los flujos de configuración.
    Vea [Configuración y ajustes](/es/plugins/sdk-setup#setup-entry) para obtener más detalles.
    ...
    definidor de tiempo de ejecución explícito en el momento de la configuración.
  ...
  <Step title="Manejar mensajes entrantes">
    ...
    <Note>
      ...
      de su propia canalización de entrada. Mire los complementos de canal empaquetados
...
<a id="step-6-test"></a>
<Step title="Prueba">
Escribe pruebas co-localizadas en `src/channel.test.ts`:
    ...
    Para ver asistentes de pruebas compartidos, consulta [Pruebas](/es/plugins/sdk-testing).
...
<CardGroup cols={2}>
  ...
  <Card title="Integración de herramientas de mensaje" icon="puzzle" href="/es/plugins/architecture#channel-plugins-and-the-shared-message-tool">
  ...
  <Card title="API de entrada del canal" icon="bolt" href="/es/plugins/sdk-channel-inbound">
    Ciclo de vida de eventos de entrada compartidos: ingest, resolve, record, dispatch, finalize
  </Card>
</CardGroup>

<Note>Algunos puntos de conexión de asistentes agrupados todavía existen para el mantenimiento y la compatibilidad de los complementos agrupados. No son el patrón recomendado para los nuevos complementos de canal; prefiere las subrutas genéricas channel/setup/reply/runtime de la superficie común del SDK a menos que estés manteniendo directamente esa familia de complementos agrupados.</Note>
