...
- **Nota sobre el aislamiento**: el backend de aislamiento predeterminado utiliza Docker cuando el aislamiento está activado, pero el aislamiento está desactivado de forma predeterminada y **no** requiere que toda la puerta de enlace se ejecute en Docker. También están disponibles los backends de aislamiento SSH y OpenShell. Consulte [Aislamiento](/es/gateway/sandboxing).
...
- Si se ejecuta en un VPS/host público, revise
  [Endurecimiento de seguridad para la exposición de red](/es/gateway/security),
...
<Steps>
  ...
  <Step title="Configurar canales (opcional)">
    Utilice el contenedor de CLI para añadir canales de mensajería:
...
oculta los instaladores de dependencias de habilidades que solo funcionan con brew cuando se ejecuta en un
contenedor de Linux sin `brew`; esas dependencias deben ser proporcionadas por una imagen personalizada
o instaladas manualmente. Para las dependencias disponibles desde los paquetes de Debian, utilice
`OPENCLAW_IMAGE_APT_PACKAGES` durante la compilación de la imagen. El nombre
`OPENCLAW_DOCKER_APT_PACKAGES` heredado todavía se acepta.

Los mantenedores pueden probar el código fuente del complemento empaquetado contra una imagen empaquetada montando
un directorio de código fuente del complemento sobre su ruta de código fuente empaquetada, por ejemplo
`OPENCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
Ese directorio de código fuente montado anula el paquete compilado `/app/dist/extensions/synology-chat` correspondiente para el mismo id de complemento.
...
las instalaciones empaquetadas de Docker antes de habilitar la exportación. Las imágenes personalizadas construidas desde el código fuente todavía
pueden incluir la fuente del complemento local con
...
Las métricas de Prometheus utilizan el puerto de Gateway ya publicado. Instale
...
no su máquina host. Use `host.docker.internal` para proveedores de IA que
...
La configuración de Docker incluida utiliza esas URLs de host como valores predeterminados para la incorporación de LM Studio y Ollama, y `docker-compose.yml` asigna `host.docker.internal` a la puerta de enlace del host de Docker para el Docker Engine de Linux. Docker Desktop ya proporciona el mismo nombre de host en macOS y Windows.
...
Si utiliza su propio archivo Compose o el comando `docker run`, agregue la misma asignación de host usted mismo, por ejemplo
`--add-host=host.docker.internal:host-gateway`.

### Bonjour / mDNS

La red de puente de Docker generalmente no reenvía de forma confiable la multidifusión Bonjour/mDNS
(`224.0.0.251:5353`). Por lo tanto, la configuración de Compose incluida establece de forma predeterminada
`OPENCLAW_DISABLE_BONJOUR=1` para que la Gateway no entre en un bucle de fallos o se reinicie repetidamente
el anuncio cuando el puente deja caer el tráfico de multidifusión.

Utilice la URL publicada de la Gateway, Tailscale o DNS-SD de área amplia para los hosts de Docker.
...
rutas sobrevivan al reemplazo del contenedor. Cuando cualquier variable no está establecida, el archivo `docker-compose.yml` incluido se retrotrae bajo `${HOME}`, o `/tmp` cuando `HOME` mismo también
carece. Esto evita que `docker compose up` emita una especificación de volumen de origen vacía en entornos básicos.

Ese directorio de configuración montado es donde OpenClaw guarda:

- `openclaw.json` para la configuración de comportamiento
- `agents/<agentId>/agent/auth-profiles.json` para la autenticación de OAuth/clave API de proveedor almacenada
- `.env` para secretos de tiempo de ejecución respaldados por variables de entorno, como `OPENCLAW_GATEWAY_TOKEN`

El directorio de claves secretas del perfil de autenticación almacena la clave de cifrado local utilizada para
el material de tokens de perfiles de autenticación respaldados por OAuth. Guárdelo con el estado de su host de Docker,
...
**Puntos calientes de crecimiento del disco:** vigile `media/`, los archivos JSONL de sesión,
...
Para facilitar la gestión diaria de Docker, instale `ClawDock`:
...
Si instaló ClawDock desde la ruta raw antigua `scripts/shell-helpers/clawdock-helpers.sh`, vuelva a ejecutar el comando de instalación anterior para que su archivo auxiliar local rastree la nueva ubicación.

Luego use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Ejecute
`clawdock-help` para todos los comandos.
Consulte [ClawDock](/es/install/clawdock) para obtener la guía completa del auxiliar.
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    a `off`. Los turnos en modo de código de Codex todavía están restringidos a Codex
...
<Accordion title="Shared-network security note">
  ...
  <Accordion title="Docker Desktop DNS failures in openclaw-cli">
    Algunas configuraciones de Docker Desktop fallan en las búsquedas de DNS desde el sidecar de
    `openclaw-cli` de red compartida después de que se elimina `NET_RAW`, lo que se muestra como
    `EAI_AGAIN` durante comandos respaldados por npm como `openclaw plugins install`.
    Mantenga el archivo compose endurecido predeterminado para la operación normal de la puerta de enlace. La
    sustitución local a continuación relaja la postura de seguridad del contenedor CLI al
    restaurar las capacidades predeterminadas de Docker, así que úselo solo para el comando CLI único
    que necesita acceso al registro de paquetes, no como su invocación predeterminada de Compose:
  ...
  <Accordion title="Permissions and EACCES">
    ...
    La misma discrepancia puede aparecer como una advertencia de complemento como
    ...
    uid 1000 predeterminado y corregir la propiedad del montaje de enlace. Solo use chown en
    `/path/to/openclaw-config/npm` para `root:root` si ejecuta
    OpenClaw como root a largo plazo intencionalmente.
  ...
  <Accordion title="Opciones de contenedor para usuarios avanzados">
    ...
    3. **Incluir Chromium de Playwright**: `export OPENCLAW_INSTALL_BROWSER=1`
    4. **O instalar los navegadores de Playwright en un volumen persistente**:
       ```bash
...
<Accordion title="OpenAI Codex OAuth (Docker sin cabeza)">Si elige OpenAI Codex OAuth en el asistente, este abre una URL del navegador. En configuraciones de Docker o sin cabeza, copie la URL de redireccionamiento completa a la que llegue y péguela de nuevo en el asistente para finalizar la autenticación.</Accordion>
  ...
  <Accordion title="Metadatos de la imagen base">
    La imagen de tiempo de ejecución principal de Docker utiliza `node:24-bookworm-slim` e incluye `tini` como proceso de inicio de entrada (PID 1) para garantizar que los procesos zombies se recojan y que las señales se manejen correctamente en contenedores de larga duración. Publica anotaciones de imagen base OCI, incluyendo `org.opencontainers.image.base.name`,
    `org.opencontainers.image.source` y otras. El resumen base de Node se
    actualiza a través de los PR de la imagen base de Docker de Dependabot; las compilaciones de lanzamiento no ejecutan
...
que incluyen la creación de binarios, la persistencia y las actualizaciones.
...
Docker aislados, mientras que la propia puerta de enlace se mantiene en el host. Esto le proporciona un muro sólido
alrededor de las sesiones de agentes no confiables o multiinquilino sin contenedorizar toda la
puerta de enlace.
...
- [OpenShell](/es/gateway/openshell) -- acceso a shell interactivo para contenedores del sandbox
...
Para instalaciones de npm sin un checkout del código fuente, consulte [Sandboxing § Images and setup](/es/gateway/sandboxing#images-and-setup) para comandos `docker build` en línea.

## Solución de problemas

<AccordionGroup>
  <Accordion title="Falta la imagen o el contenedor del sandbox no se inicia">
    Cree la imagen del sandbox con
    [`scripts/sandbox-setup.sh`](https://github.com/openclaw/openclaw/blob/main/scripts/sandbox-setup.sh)
    (checkout del código fuente) o el comando `docker build` en línea de [Sandboxing § Images and setup](/es/gateway/sandboxing#images-and-setup) (npm install),
...
<Accordion title="Errores de permisos en el sandbox">Establezca `docker.user` en un UID:GID que coincida con la propiedad de su espacio de trabajo montado, o cambie el propietario de la carpeta del espacio de trabajo con chown.</Accordion>

<Accordion title="Herramientas personalizadas no encontradas en el sandbox">OpenClaw ejecuta comandos con `sh -lc` (shell de inicio de sesión), lo que obtiene recursos de `/etc/profile` y puede restablecer PATH. Establezca `docker.env.PATH` para anteponer sus rutas de herramientas personalizadas, o agregue un script bajo `/etc/profile.d/` en su Dockerfile.</Accordion>
...
- [Updating](/es/install/updating) — manteniendo OpenClaw actualizado
