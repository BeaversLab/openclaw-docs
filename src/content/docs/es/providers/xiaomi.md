...
- `xiaomi-token-plan` para claves del Plan de Tokens (`tp-...`) con preajustes de endpoint regionales

El mismo complemento también registra el proveedor de voz (TTS) `xiaomi`.

| Propiedad                             | Valor                                                                                                                                              |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| IDs de proveedor                      | `xiaomi` (pago por uso), `xiaomi-token-plan` (Plan de Tokens)                                                                                      |
...
| URL base                              | Pago por uso: `https://api.xiaomimimo.com/v1`; preajustes del Plan de Tokens: `token-plan-{cn,sgp,ams}...`                                         |
...
<Steps>
  <Step title="Get the right key">
    Cree una clave de pago por uso en la [consola de Xiaomi MiMo](https://platform.xiaomimimo.com/#/console/api-keys) o abra la página de su suscripción al Plan de Tokens y copie la URL base compatible con OpenAI regional más la clave `tp-...` correspondiente.
  </Step>
...
<Tip>La referencia de modelo predeterminada es `xiaomi/mimo-v2-flash`. El proveedor se inyecta automáticamente cuando se establece `XIAOMI_API_KEY` o existe un perfil de autenticación.</Tip>
...
`messages.tts`. Llama al contrato TTS de completado de chat de Xiaomi con el texto como
un mensaje `assistant` y una guía de estilo opcional como un mensaje `user`.
...
`Mia`, `Chloe`, `Milo` y `Dean`. Los modelos de voz preestablecida usan `audio.voice`, por lo que
...
a partir de un mensaje de estilo en lenguaje natural en lugar de una ID de voz preestablecida. Configure
`style` con la descripción de voz deseada; OpenClaw la envía como el mensaje `user`,
envía el texto hablado como el mensaje `assistant` y omite
...
Para objetivos de notas de voz como Feishu y Telegram, OpenClaw transcodifica la salida de Xiaomi a Opus de 48 kHz con `ffmpeg` antes de la entrega.
...
Los precios provienen del manifiesto incluido (los modelos de Token Plan incluyen precios de lectura de caché por niveles), por lo que el ejemplo de configuración omite `cost`.

<AccordionGroup>
  <Accordion title="Comportamiento de autoinyección">
    El proveedor `xiaomi` se inyecta automáticamente cuando `XIAOMI_API_KEY` está configurado en su entorno o existe un perfil de autenticación. `xiaomi-token-plan` necesita una URL base regional, por lo que la ruta admitida es la opción de incorporación del Token Plan incluido o un bloque de configuración `models.providers.xiaomi-token-plan` explícito.
  </Accordion>
  ...
  <Accordion title="Detalles del modelo">
    ...
    - **mimo-v2-pro** — soporta razonamiento con una ventana de contexto de 1M tokens para cargas de trabajo de documentos largos.
  ...
  <Accordion title="Solución de problemas">
    - Si los modelos no aparecen, confirme que la variable de entorno de la clave relevante o el perfil de autenticación estén presentes y sean válidos.
    - Para el Plan de Tokens, confirme que la región de incorporación elegida coincida con la URL base de la página de suscripción y que la clave comience con `tp-`.
    - Cuando el Gateway se ejecuta como un demonio, asegúrese de que la clave esté disponible para ese proceso (por ejemplo, en `~/.openclaw/.env` o mediante `env.shellEnv`).
