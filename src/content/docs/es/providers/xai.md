...
`XAI_API_KEY` también puede alimentar `web_search` con respaldo de Grok, `x_search` de primera clase,
y `code_execution` remoto.
Si almacena una clave de xAI bajo `plugins.entries.xai.config.webSearch.apiKey`,
el proveedor de modelo xAI incluido ahora reutiliza esa clave como alternativa.
...
El complemento también resuelve de manera anticipada los ids `grok-4*` y `grok-code-fast*` más nuevos cuando
...
- `grok-4.20-multi-agent-experimental-beta-0304` no es compatible con la ruta del proveedor xAI normal porque requiere una superficie de API upstream diferente que el transporte OpenClaw xAI estándar.
...
- `code_execution` es la ejecución remota del sandbox xAI, no [`exec`](/en/tools/exec) local.
