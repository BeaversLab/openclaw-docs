...
- `channel` opcional (cadena): El canal de mensajería para la entrega. Use `last` o cualquier canal o id de plugin configurado, por ejemplo `discord`, `matrix`, `telegram` o `whatsapp`. El valor predeterminado es `last`.
- `to` opcional (cadena): El identificador del destinatario para el canal (por ejemplo, número de teléfono para WhatsApp/Signal, ID de chat para Telegram, ID de canal para Discord/Slack/Mattermost (plugin), ID de conversación para Microsoft Teams). El valor predeterminado es el último destinatario en la sesión principal.
- `model` opcional (cadena): Anulación del modelo (por ejemplo, `anthropic/claude-sonnet-4-6` o un alias). Debe estar en la lista de modelos permitidos si está restringido.
- `thinking` opcional (cadena): Anulación del nivel de pensamiento (por ejemplo, `low`, `medium`, `high`).
...
Las anulaciones de carga útil `sessionKey` de `/hooks/agent` están deshabilitadas de forma predeterminada.

- Recomendado: establezca una `hooks.defaultSessionKey` fija y mantenga las anulaciones de solicitud desactivadas.
...
Los nombres de enlace personalizados se resuelven a través de `hooks.mappings` (ver configuración). Un mapeo puede
...
- `hooks.transformsDir` + `transform.module` carga un módulo JS/TS para lógica personalizada.
  - `hooks.transformsDir` (si está configurado) debe permanecer dentro de la raíz de transformaciones en su directorio de configuración de OpenClaw (típicamente `~/.openclaw/hooks/transforms`).
...
- Use `match.source` para mantener un punto final de ingestión genérico (enrutamiento basado en payload).
- Las transformaciones TS requieren un cargador TS (ej. `bun` o `tsx`) o `.js` precompilado en tiempo de ejecución.
- Configure `deliver: true` + `channel`/`to` en las asignaciones para enrutar las respuestas a una superficie de chat
  (`channel` por defecto es `last` y se respalda a WhatsApp).
- `agentId` enruta el enlace a un agente específico; los IDs desconocidos vuelven al agente predeterminado.
...
- `hooks.allowRequestSessionKey` controla si los payloads `/hooks/agent` pueden establecer `sessionKey` (predeterminado: `false`).
...
- `429` después de fallos de autenticación repetidos del mismo cliente (verificar `Retry-After`)
- `400` en caso de payload no válido
- `413` en payloads sobredimensionados
...
Añada `model` al payload del agente (o mapeo) para anular el modelo para esa ejecución:
...
- Prefiera un agente de enlace dedicado con `tools.profile` estrictas y sandboxing para que el ingreso del enlace tenga un radio de explosión más estrecho.
...
- Los payloads de enlace se tratan como no confiables y se envuelven con límites de seguridad de forma predeterminada.
  Si debe deshabilitar esto para un enlace específico, establezca `allowUnsafeExternalContent: true`
