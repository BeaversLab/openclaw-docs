...
- El agente se ejecuta de forma autónoma dentro de los límites definidos
- El trabajo rutinario se realiza a tiempo sin necesidad de instrucciones
- Usted solo interviene para excepciones y aprobaciones
- El agente aprovecha el tiempo de inactividad de manera productiva

## Cómo funcionan

Las órdenes permanentes se definen en los archivos de su [espacio de trabajo del agente](/es/concepts/agent-workspace). El enfoque recomendado es incluirlas directamente en `AGENTS.md` (que se inyecta automáticamente en cada sesión) para que el agente siempre las tenga en contexto. Para configuraciones más grandes, también puede colocarlas en un archivo dedicado como `standing-orders.md` y referenciarlo desde `AGENTS.md`.
...
Las órdenes permanentes definen **qué** está autorizado a hacer el agente. Los [trabajos cron](/es/automation/cron-jobs) definen **cuándo** ocurre. Trabajan juntos:
...
## Patrón ejecutar-verificar-reportar
...
Para agentes que gestionan múltiples asuntos, organiza las órdenes permanentes como programas separados con límites claros:
