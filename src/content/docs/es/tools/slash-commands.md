...
<AccordionGroup>
  <Accordion title="Sesiones y ejecuciones">
    ...
    - La interfaz de usuario de control intercepta `/new` escrito para crear y cambiar a una nueva sesión del panel, excepto cuando `session.dmScope: "main"` está configurado y el padre actual es la sesión principal del agente; en ese caso, `/new` restablece la sesión principal en su lugar. `/reset` escrito todavía ejecuta el restablecimiento en su lugar del Gateway.
    - `/reset soft [message]` mantiene la transcripción actual, descarta los identificadores de sesión de backend de CLI reutilizados y vuelve a ejecutar la carga de inicio/system-prompt en su lugar.
    ...
    - `/export-trajectory [path]` solicita aprobación de ejecución y luego exporta un [paquete de trayectoria](/es/tools/trajectory) JSONL para la sesión actual. Úselo cuando necesite la línea de tiempo de indicaciones, herramientas y transcripciones para una sesión de OpenClaw. En chats grupales, la solicitud de aprobación y el resultado de exportación van al propietario de forma privada. Alias: `/trajectory`.
  ...
  <Accordion title="Modelo y controles de ejecución">
    ...
    - `/models [provider] [page] [limit=<n>|size=<n>|all]` lista los proveedores configurados/disponibles para autenticación o los modelos de un proveedor; agregue `all` para navegar por el catálogo completo de ese proveedor. Las entradas `provider/*` en `agents.defaults.models` hacen que `/model` y `/models` muestren solo los modelos descubiertos para esos proveedores.
    - `/queue <mode>` gestiona el comportamiento de la cola (`steer`, `queue` heredado, `followup`, `collect`, `steer-backlog`, `interrupt`) además de opciones como `debounce:0.5s cap:25 drop:summarize`; `/queue default` o `/queue reset` borra la anulación de la sesión. Consulte [Cola de comandos](/es/concepts/queue) y [Cola de dirección](/es/concepts/queue-steering).
    - `/steer <message>` inyecta orientación en la ejecución activa para la sesión actual, independientemente del modo `/queue`. No inicia una nueva ejecución cuando la sesión está inactiva. Alias: `/tell`. Consulte [Dirección](/es/tools/steer).
  ...
  <Accordion title="Descubrimiento y estado">
    ...
    - `/diagnostics [note]` es el flujo de informe de soporte solo para propietarios para errores de Gateway y ejecuciones de arnés de Codex. Pide aprobación de ejecución explícita cada vez antes de ejecutar `openclaw gateway diagnostics export --json`; no apruebes el diagnóstico con una regla de permitir todo. Después de la aprobación, envía un informe pegable con la ruta del paquete local, el resumen del manifiesto, las notas de privacidad y los identificadores de sesión relevantes. En chats grupales, la solicitud de aprobación y el informe van al propietario de forma privada. Cuando la sesión activa usa el arnés de OpenAI Codex, la misma aprobación también envía comentarios relevantes de Codex a los servidores de OpenAI y la respuesta completada lista los identificadores de sesión de OpenClaw, los identificadores de hilo de Codex y los comandos `codex resume <thread-id>`. Consulte [Exportación de diagnósticos](/es/gateway/diagnostics).
  ...
  <Accordion title="Habilidades, listas de permitidos, aprobaciones">
    ...
    - `/approve <id> <decision>` resuelve los prompts de aprobación de ejecución.
    - `/btw <question>` hace una pregunta secundaria sin cambiar el contexto de la sesión futura. Alias: `/side`. Véase [BTW](/es/tools/btw).
  ...
  <Accordion title="Subagentes y ACP">
    ...
    - `/acp spawn|cancel|steer|close|sessions|status|set-mode|set|cwd|permissions|timeout|model|reset-options|doctor|install|help` gestiona las sesiones y las opciones de tiempo de ejecución de ACP.
    ...
    - `/subagents steer <id|#> <message>` envía instrucciones a un subagente en ejecución. Véase [Steer](/es/tools/steer).
  ...
  <Accordion title="Voz, TTS, control de canal">
    ...
    - `/bash <command>` ejecuta un comando de shell del host. Solo texto. Alias: `! <command>`. Requiere `commands.bash: true` más las listas de permitidos `tools.elevated`.
...
Los comandos de acoplamiento cambian la ruta de respuesta de la sesión actual a otro canal vinculado. Consulte [Channel docking](/es/concepts/channel-docking) para obtener instrucciones de configuración, ejemplos y solución de problemas.
...
- `/dreaming [on|off|status|help]` alterna la "soñación" de la memoria. Consulte [Dreaming](/es/concepts/dreaming).
- `/pair [qr|status|pending|approve|cleanup|notify]` gestiona el flujo de vinculación/configuración del dispositivo. Consulte [Pairing](/es/channels/pairing).
...
- `/card ...` envía preajustes de rich card de LINE. Consulte [LINE](/es/channels/line).
...
<AccordionGroup>
  <Accordion title="Notas sobre argumentos y analizadores">
    ...
    - `/plugins install <spec>` acepta las mismas especificaciones de complemento que `openclaw plugins install`: ruta local/archivo, paquete npm, `git:<repo>` o `clawhub:<pkg>`, y luego solicita un reinicio de Gateway porque cambiaron los módulos de origen del complemento.
    - `/plugins enable|disable` actualiza la configuración del complemento y activa la recarga del complemento de Gateway para los nuevos turnos del agente.
  ...
  <Accordion title="Comportamiento específico del canal">
    ...
    - Comandos de vinculación de hilos de Discord (`/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`) requieren que las vinculaciones de hilos efectivas estén habilitadas (`session.threadBindings.enabled` y/o `channels.discord.threadBindings.enabled`).
  ...
  <Accordion title="Verbose / trace / fast / reasoning safety">
    ...
    - `/trace` es más limitado que `/verbose`: solo revela las líneas de seguimiento/depuración propiedad del complemento y mantiene desactivado el chatter normal detallado de las herramientas.
    - `/fast on|off` persiste una anulación de sesión. Utilice la opción de interfaz de usuario de Sesiones `inherit` para borrarla y volver a los valores predeterminados de configuración.
    - `/fast` es específico del proveedor: OpenAI/OpenAI Codex lo mapean a `service_tier=priority` en los puntos de conexión Responses nativos, mientras que las solicitudes directas públicas de Anthropic, incluido el tráfico autenticado con OAuth enviado a `api.anthropic.com`, lo mapean a `service_tier=auto` o `standard_only`. Consulte [OpenAI](/es/providers/openai) y [Anthropic](/es/providers/anthropic).
    - Los resúmenes de fallos de herramientas aún se muestran cuando es relevante, pero el texto detallado del fallo solo se incluye cuando `/verbose` está `on` o `full`.
    - `/reasoning`, `/verbose` y `/trace` son arriesgados en entornos grupales: pueden revelar el razonamiento interno, el resultado de las herramientas o el diagnóstico de complementos que no tenía la intención de exponer. Es preferible dejarlos desactivados, especialmente en chats grupales.
  ...
  <Accordion title="Model switching">
    ...
    - En la TUI local, `/crestodian [request]` regresa de la TUI normal del agente a Crestodian. Esto es separado del modo de rescate del canal de mensajes y no otorga autoridad de configuración remota.

  </Accordion>
  <Accordion title="Ruta rápida y atajos en línea">
    - **Ruta rápida:** los mensajes que solo contienen comandos de remitentes en la lista de permitidos se manejan inmediatamente (sin cola ni modelo).
    - **Bloqueo de mención de grupo:** los mensajes que solo contienen comandos de remitentes en la lista de permitidos omiten los requisitos de mención.
  ...
  <Accordion title="Comandos de habilidad y argumentos nativos">
    - **Comandos de habilidad:** las habilidades `user-invocable` se exponen como comandos de barra. Los nombres se sanitizan a `a-z0-9_` (máx. 32 caracteres); las colisiones obtienen sufijos numéricos (p. ej., `_2`).
    ...
    - **Argumentos de comandos nativos:** Discord usa autocompletado para opciones dinámicas (y menús de botones cuando omite argumentos requeridos). Telegram y Slack muestran un menú de botones cuando un comando admite elecciones y omite el argumento. Las elecciones dinámicas se resuelven contra el modelo de sesión de destino, por lo que las opciones específicas del modelo, como los niveles `/think`, siguen la anulación `/model` de esa sesión.
...
`/tools` responde a una pregunta en tiempo de ejecución, no a una pregunta de configuración: **lo que este agente puede usar justo ahora en esta conversación**.

- El `/tools` predeterminado es compacto y está optimizado para una lectura rápida.
...
- **El uso/cuota del proveedor** (ejemplo: "Claude 80% restante") aparece en `/status` para el proveedor del modelo actual cuando el seguimiento de uso está habilitado. OpenClaw normaliza las ventanas del proveedor a `% left`; para MiniMax, los campos de porcentaje solo restantes se invierten antes de mostrarse, y las respuestas de `model_remains` prefieren la entrada del modelo de chat más una etiqueta de plan etiquetada por modelo.
- **Las líneas de token/caché** en `/status` pueden recurrir a la última entrada de uso de la transcripción cuando la instantánea de la sesión en vivo es escasa. Los valores en vivo distintos de cero existentes todavía tienen prioridad, y el recurso alternativo de la transcripción también puede recuperar la etiqueta del modelo de ejecución activo más un total orientado al prompt mayor cuando los totales almacenados faltan o son menores.
- **Ejecución vs tiempo de ejecución:** `/status` informa `Execution` para la ruta de acceso efectiva del sandbox y `Runtime` para quien está ejecutando realmente la sesión: `OpenClaw Pi Default`, `OpenAI Codex`, un backend CLI o un backend ACP.
- Los **tokens/costo por respuesta** se controlan mediante `/usage off|tokens|full` (agregado a las respuestas normales).
...
- `/model` y `/model list` muestran un selector numerado compacto (familia de modelos + proveedores disponibles).
...
- `/model status` muestra la vista detallada, incluido el punto de conexión del proveedor configurado (`baseUrl`) y el modo de API (`api`) cuando esté disponible.

## Invalidaciones de depuración

`/debug` le permite establecer anulaciones de configuración **solo en tiempo de ejecución** (memoria, no disco). Solo para el propietario. Deshabilitado de forma predeterminada; habilítelo con `commands.debug: true`.
...
`/trace` le permite alternar **líneas de rastreo/depuración de complementos con ámbito de sesión** sin activar el modo detallado completo.
...
- `/trace` no reemplaza a `/debug`; `/debug` aún administra las anulaciones de configuración solo en tiempo de ejecución.
- `/trace` no reemplaza a `/verbose`; la salida normal detallada de herramientas/estado aún pertenece a `/verbose`.

## Actualizaciones de configuración

`/config` escribe en su configuración en disco (`openclaw.json`). Solo para el propietario. Deshabilitado de forma predeterminada; habilítelo con `commands.config: true`.
...
`/mcp` escribe definiciones de servidores MCP gestionados por OpenClaw bajo `mcp.servers`. Solo para el propietario. Deshabilitado de forma predeterminada; habilítelo con `commands.mcp: true`.
...
- Los cambios de habilitar y deshabilitar recargan en caliente las superficies de tiempo de ejecución del complemento Gateway para nuevos turnos del agente; la instalación solicita un reinicio de Gateway porque cambiaron los módulos de origen del complemento.
...
Consulta [Preguntas laterales BTW](/es/tools/btw) para obtener el comportamiento completo y los detalles de la experiencia del usuario (UX) del cliente.
