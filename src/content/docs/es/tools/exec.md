...
- `tools.exec.safeBins`: binarios seguros solo de stdin que pueden ejecutarse sin entradas explícitas en la lista blanca. Para obtener detalles sobre el comportamiento, consulte [Safe bins](/es/tools/exec-approvals-advanced#safe-bins-stdin-only).
...
- `host=node`: solo se envían al nodo las anulaciones de entorno no bloqueadas que pase. Las anulaciones de `env.PATH` son
  rechazadas para la ejecución en el host e ignoradas por los hosts de nodos. Si necesita entradas PATH adicionales en un nodo,
  configure el entorno del servicio del host del nodo (systemd/launchd) o instale herramientas en ubicaciones estándar.

Enlace de nodo por agente (use el índice de la lista de agentes en la configuración):
...
Interfaz de usuario de control: la pestaña Nodos incluye un pequeño panel "Enlace de nodo exec" para la misma configuración.
...
Actualiza **solo el estado de la sesión** y no escribe la configuración. Para deshabilitar exec de forma permanente, deniegalo a través de la política
de herramientas (`tools.deny: ["exec"]` o por agente). Las aprobaciones del host siguen aplicándose a menos que configures explícitamente
`security=full` y `ask=off`.

## Aprobaciones de Exec (aplicación complementaria / host de nodo)

Los agentes en espacio aislado pueden requerir aprobación por solicitud antes de que `exec` se ejecute en la puerta de enlace o el host del nodo.
Consulta [Aprobaciones de Exec](/es/tools/exec-approvals) para obtener información sobre la política, la lista de permitidos y el flujo de la interfaz de usuario.
...
interfaz de usuario nativa y solo incluir un comando manual `/approve` cuando el resultado de la herramienta
indique explícitamente que las aprobaciones del chat no están disponibles o que la aprobación manual es la
única opción.

## Lista de permitidos + bins seguros

La aplicación manual de la lista de permitidos coincide con los patrones glob de ruta binaria resuelta y los patrones glob de nombre de comando simple.
...
se rechazan en el modo de lista de permitidos a menos que cada segmento de nivel superior cumpla con la
...
las entradas manuales de lista de permitidos (allowlist) de rutas. Para una confianza estricta y explícita, mantén `autoAllowSkills` deshabilitado.

Usa los dos controles para diferentes trabajos:

- `tools.exec.safeBins`: filtros de flujo pequeños, solo de stdin.
- `tools.exec.safeBinTrustedDirs`: directorios adicionales de confianza explícita para rutas ejecutables de safe-bin.
- `tools.exec.safeBinProfiles`: política de argv explícita para safe bins personalizadas.
- allowlist: confianza explícita para rutas ejecutables.

No trates `safeBins` como una lista de permitidos (allowlist) genérica y no agregues binarios de intérprete/runtime (por ejemplo `python3`, `node`, `ruby`, `bash`). Si necesitas esos, usa entradas explícitas en la lista de permitidos y mantén los avisos de aprobación habilitados.
`openclaw security audit` advierte cuando a las entradas de intérprete/runtime `safeBins` les faltan perfiles explícitos, y `openclaw doctor --fix` puede andamiar las entradas personalizadas `safeBinProfiles` faltantes.
...
Si permites explícitamente intérpretes, habilita `tools.exec.strictInlineEval` para que los formularios de evaluación de código en línea aún requieran una aprobación nueva.

Para detalles completos de la política y ejemplos, consulta [Exec approvals](/es/tools/exec-approvals-advanced#safe-bins-stdin-only) y [Safe bins versus allowlist](/es/tools/exec-approvals-advanced#safe-bins-versus-allowlist).
...
El sondeo es para el estado bajo demanda, no para bucles de espera. Si el despertar automático al completar
...
Está habilitada por defecto para los modelos OpenAI y OpenAI Codex. Usa la configuración solo
...
- `tools.exec.applyPatch.enabled` tiene como valor predeterminado `true`; establézcalo en `false` para desactivar la herramienta para modelos de OpenAI.
- `tools.exec.applyPatch.workspaceOnly` tiene como valor predeterminado `true` (contenido en el área de trabajo). Establézcalo en `false` solo si intencionalmente desea que `apply_patch` escriba/elimine fuera del directorio del área de trabajo.

## Relacionado

- [Aprobaciones Exec](/es/tools/exec-approvals) — puertas de aprobación para comandos de shell
- [Aislamiento (Sandboxing)](/es/gateway/sandboxing) — ejecución de comandos en entornos aislados
