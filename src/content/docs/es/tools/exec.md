...
- El exec en el host sin aprobación es el predeterminado para gateway + node. Si desea el comportamiento de aprobaciones/lista de permitidos, ajuste tanto `tools.exec.*` como el host `~/.openclaw/exec-approvals.json`; consulte [Exec approvals](/es/tools/exec-approvals#yolo-mode-no-approval).
...
- `tools.exec.strictInlineEval` (predeterminado: false): cuando es true, los formularios de evaluación del intérprete en línea, como `python -c`, `node -e`, `ruby -e`, `perl -e`, `php -r`, `lua -e` y `osascript -e`, requieren un revisor o una aprobación explícita. En `mode=auto`, el revisor automático nativo puede permitir un comando único de claramente bajo riesgo; si el revisor lo solicita, la solicitud se envía a un humano. `allow-always` aún puede persistir invocaciones benignas de intérprete/secuencia de comandos, pero los formularios de evaluación en línea no se convierten en reglas de permiso duraderas.
...
- `tools.exec.pathPrepend`: lista de directorios que se anteponen a `PATH` para ejecuciones exec (solo puerta de enlace + sandbox).
...
- `tools.exec.safeBinProfiles`: política argv personalizada opcional por contenedor seguro (`minPositional`, `maxPositional`, `allowedValueFlags`, `deniedFlags`).
...
- `host=gateway`: combina su `PATH` de shell de inicio de sesión en el entorno de ejecución. Se rechazan las anulaciones de `env.PATH` para la ejecución del host. El propio demonio aún se ejecuta con un `PATH` mínimo:
  - macOS: `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin`, `/bin`
  - Linux: `/usr/local/bin`, `/usr/bin`, `/bin`
    - Para evitar que la configuración del shell del usuario (como `~/.zshenv` o `/etc/zshenv`) anule las rutas de prioridad durante el inicio, las entradas `tools.exec.pathPrepend` se anteponen de forma segura al `PATH` final dentro del comando del shell justo antes de la ejecución.
...
- `host=node`: solo se envían al nodo las anulaciones de entorno no bloqueadas que pases. Las anulaciones de `env.PATH` se
  rechazan para la ejecución en el host y son ignoradas por los hosts nodales. Si necesitas entradas PATH adicionales en un nodo,
...
`/exec` solo se respeta para **remitentes autorizados** (listas de permisos/emparejamiento de canales más `commands.useAccessGroups`).
Actualiza **solo el estado de la sesión** y no escribe configuración. Los remitentes externos autorizados de canales pueden
establecer estos valores predeterminados de sesión. Los clientes internos de pasarela/webchat necesitan `operator.admin` para conservarlos.
Para deshabilitar ejecución de forma permanente, denieguela mediante la política de herramientas (`tools.deny: ["exec"]` o por agente). Las aprobaciones del host
...
Los agentes en espacio aislado pueden requerir aprobación por solicitud antes de que `exec` se ejecute en la pasarela o el host de nodo.
Consulte [Aprobaciones de ejecución](/es/tools/exec-approvals) para conocer la política, la lista de permisos y el flujo de la interfaz de usuario.
...
la Pasarela emite eventos de sistema de progreso y finalización de comandos solo para ejecuciones aprobadas
...
En canales con tarjetas/botones de aprobación nativos, el agente debe confiar en esa
interfaz de usuario nativa primero y solo incluir un comando manual `/approve` cuando el resultado
...
La aplicación manual de la lista de permitidos coincide con las rutas de acceso binarias resueltas y los comodines de nombres de comandos simples. Los nombres simples coinciden solo con comandos invocados a través de PATH, por lo que `rg` puede coincidir con `/opt/homebrew/bin/rg` cuando el comando es `rg`, pero no con `./rg` o `/tmp/rg`. Cuando `security=allowlist`, los comandos de shell se permiten automáticamente solo si cada segmento de la tubería está en la lista de permitidos o es un binario seguro. El encadenamiento (`;`, `&&`, `||`) y las redirecciones se rechazan en el modo de lista de permitidos a menos que cada segmento de nivel superior satisfaga la lista de permitidos (incluyendo los bins seguros). Las redirecciones siguen sin ser compatibles. La confianza duradera de `allow-always` no elude esa regla: un comando encadenado todavía requiere que cada segmento de nivel superior coincida.

`autoAllowSkills` es una ruta de acceso separada de conveniencia en las aprobaciones de ejecución. No es lo mismo que las entradas manuales de la lista de permitidos de rutas. Para una confianza explícita estricta, mantenga `autoAllowSkills` deshabilitado.
...
`openclaw security audit` advierte cuando faltan perfiles explícitos para las entradas de intérprete/tiempo de ejecución `safeBins`, y `openclaw doctor --fix` puede andamiar entradas `safeBinProfiles` personalizadas faltantes.
`openclaw security audit` y `openclaw doctor` también advierten cuando añade explícitamente bins de comportamiento amplio como `jq` de nuevo a `safeBins`.
Si permite explícitamente intérpretes, habilite `tools.exec.strictInlineEval` para que los formularios de evaluación de código en línea sigan requiriendo el revisor o una aprobación explícita.
...
- `tools.exec.applyPatch.enabled` predeterminado es `true`; establézcalo en `false` para deshabilitar la herramienta para modelos de OpenAI.
...
- [Background Process](/es/gateway/background-process) — herramienta de ejecución y procesos de larga duración
