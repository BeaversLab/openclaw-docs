...
4. **Sondas profundas**
   ...
   Ejecuta una sonda de salud de la puerta de enlace en vivo, incluidas las sondas de canal cuando se admiten
...
6. **Ejecutar el doctor (reparaciones)**
   ...
   Repara/migra el estado/la configuración + ejecuta comprobaciones de salud. Consulte [Doctor](/es/gateway/doctor).
...
Las preguntas y respuestas de la primera ejecución — instalación, incorporación, rutas de autenticación, suscripciones, fallos iniciales —
se encuentran en las [Preguntas frecuentes de primera ejecución](/es/help/faq-first-run).
...
<AccordionGroup>
  ...
  <Accordion title="Propuesta de valor">
    ...
    sesiones con estado, memoria y herramientas, sin entregar el control de tus flujos de trabajo a un SaaS hospedado.
    ...
    - **Canales reales, no un entorno de prueba web:** WhatsApp/Telegram/Slack/Discord/Signal/iMessage/etc.,
      más voz móvil y Canvas en las plataformas compatibles.
    ...
    - **Código abierto y modificable:** inspecciona, extiende y autohospeda sin bloqueo de proveedor.
  ...
  <Accordion title="¿Puede OpenClaw ayudar con la generación de leads, el alcance, los anuncios y los blogs para un SaaS?">
    ...
    Para **campañas de alcance o anuncios**, mantén a una persona en el ciclo. Evita el spam, respeta las leyes locales y
    las políticas de las plataformas, y revisa todo antes de enviarlo. El patrón más seguro es dejar
...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo funcionan las sesiones de subagentes vinculadas a hilos en Discord?">
    Use enlaces de hilos. Puede vincular un hilo de Discord a un subagente o destino de sesión para que los mensajes de seguimiento en ese hilo se mantengan en esa sesión vinculada.
    ...
    - Auto-vinculación al generar: establezca `channels.discord.threadBindings.spawnSubagentSessions: true`.
  ...
  <Accordion title="Un subagente finalizó, pero la actualización de finalización se fue al lugar equivocado o nunca se publicó. ¿Qué debería comprobar?">
    ...
    - Si no existe ni una ruta enlazada ni una ruta almacenada utilizable, la entrega directa puede fallar y el resultado recurre a la entrega de sesión en cola en lugar de publicarse inmediatamente en el chat.
    - Los objetivos inválidos o obsoletos aún pueden forzar la recuperación de la cola o el fallo final de la entrega.
    ...
    - Si el hijo agotó el tiempo de espera después de solo llamadas a herramientas, el anuncio puede colapsar eso en un breve resumen de progreso parcial en lugar de reproducir la salida sin procesar de la herramienta.
  ...
  <Accordion title="Cron o recordatorios no se ejecutan. ¿Qué debería comprobar?">
    ...
    - Compruebe que el Gateway está ejecutándose 24/7 (sin suspensión/reinicios).
...
también persiste en eso antes de reintentar.
    ...
    - La anulación del modelo del gancho de Gmail gana primero cuando es aplicable.
    ...
    cron aborta en lugar de bucle infinito.
  ...
  <Accordion title="¿Cómo instalo habilidades en Linux?">
    ...
    La escritura nativa de `openclaw skills install` se realiza en el directorio del espacio de trabajo activo `skills/`. Instale la CLI `clawhub` por separado solo si desea publicar o sincronizar sus propias habilidades. Para instalaciones compartidas entre agentes, coloque la habilidad bajo `~/.openclaw/skills` y use `agents.defaults.skills` o `agents.list[].skills` si desea limitar qué agentes pueden verla.

  </Accordion>

  <Accordion title="¿Puede OpenClaw ejecutar tareas en una programación o continuamente en segundo plano?">
  ...
  <Accordion title="¿Puedo ejecutar habilidades exclusivas de Apple macOS desde Linux?">
    ...
    **Opción B: usar un nodo macOS (sin SSH).**
    Ejecuta el Gateway en Linux, empareja un nodo macOS (aplicación de la barra de menús) y establece **Node Run Commands** en "Always Ask" o "Always Allow" en la Mac. OpenClaw puede considerar las habilidades exclusivas de macOS como elegibles cuando los binarios requeridos existen en el nodo. El agente ejecuta esas habilidades a través de la herramienta `nodes`. Si eliges "Always Ask", aprobar "Always Allow" en el mensaje añade ese comando a la lista de permitidos.

    **Opción C: intermediar binarios de macOS a través de SSH (avanzado).**
    Mantén el Gateway en Linux, pero haz que los binarios de la CLI requeridos resuelvan a envoltorios SSH que se ejecutan en una Mac. Luego anula la habilidad para permitir Linux de modo que siga siendo elegible.
  ...
  <Accordion title="¿Tienes una integración con Notion o HeyGen?">
    ...
    - **Habilidad / complemento personalizado:** lo mejor para un acceso API fiable (tanto Notion como HeyGen tienen APIs).
    - **Automatización del navegador:** funciona sin código pero es más lenta y frágil.

    Si desea mantener el contexto por cliente (flujos de trabajo de agencias), un patrón simple es:

    - Una página de Notion por cliente (contexto + preferencias + trabajo activo).
    - Pregunte al agente que obtenga esa página al comienzo de una sesión.
    ...
    Las instalaciones nativas aterrizan en el directorio del espacio de trabajo activo `skills/`. Para habilidades compartidas entre agentes, colóquelas en `~/.openclaw/skills/<name>/SKILL.md`. Si solo algunos agentes deben ver una instalación compartida, configure `agents.defaults.skills` o `agents.list[].skills`. Algunas habilidades esperan binarios instalados a través de Homebrew; en Linux eso significa Linuxbrew (ver la entrada de FAQ de Homebrew Linux anterior). Ver [Habilidades](/es/tools/skills), [Configuración de habilidades](/es/tools/skills-config) y [ClawHub](/es/tools/clawhub).
  ...
  <Accordion title="¿Cómo uso mi Chrome existente con sesión iniciada con OpenClaw?">
    ...
    Esta ruta puede usar el navegador host local o un nodo de navegador conectado. Si la puerta de enlace se ejecuta en otro lugar, ejecute un host de nodo en la máquina del navegador o use CDP remoto en su lugar.

    Límites actuales en `existing-session` / `user`:

    - las acciones son impulsadas por referencias, no por selectores CSS
    - las cargas requieren `ref` / `inputRef` y actualmente admiten un archivo a la vez
    - `responsebody`, exportación PDF, intercepción de descargas y acciones por lotes aún necesitan un navegador administrado o un perfil CDP sin procesar
...
<AccordionGroup>
  <Accordion title="¿Existe un documento dedicado al aislamiento (sandboxing)?">
    Sí. Consulte [Aislamiento (Sandboxing)](/es/gateway/sandboxing). Para la configuración específica de Docker (puerta de enlace completa en Docker o imágenes de aislamiento), consulte [Docker](/es/install/docker).
  </Accordion>
  ...
  <Accordion title="Docker se siente limitado: ¿cómo habilito todas las funciones?">
    ...
    - Incluya las dependencias del sistema en la imagen con `OPENCLAW_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="La memoria sigue olvidando cosas. ¿Cómo hago que se quede?">
    ...
    Esta es todavía un área que estamos mejorando. Ayuda recordar al modelo que almacene recuerdos;
  ...
  <Accordion title="¿La memoria persiste para siempre? ¿Cuáles son los límites?">
    ...
    contexto del modelo, por lo que las conversaciones largas pueden compactarse o truncarse. Por eso existe la
    búsqueda en memoria: recupera solo las partes relevantes al contexto.
  ...
  <Accordion title="¿La búsqueda de memoria semántica requiere una clave API de OpenAI?">
    Solo si usas **embeddings de OpenAI**. El OAuth de Codex cubre chat/completiones y
    **no** otorga acceso a embeddings, por lo que **iniciar sesión con Codex (OAuth o el
    ...
    todavía necesitan una clave API real (`OPENAI_API_KEY` o `models.providers.openai.apiKey`).
...
<AccordionGroup>
  ...
  <Accordion title="¿Dónde almacena OpenClaw sus datos?">
    Todo reside en `$OPENCLAW_STATE_DIR` (por defecto: `~/.openclaw`):
    ...
    Su **espacio de trabajo** (AGENTS.md, archivos de memoria, habilidades, etc.) es independiente y se configura a través de `agents.defaults.workspace` (por defecto: `~/.openclaw/workspace`).
  ...
  <Accordion title="¿Dónde deben vivir los archivos AGENTS.md / SOUL.md / USER.md / MEMORY.md?">
    ...
    Si el bot "olvida" después de un reinicio, confirme que la Gateway está usando el mismo
    espacio de trabajo en cada lanzamiento (y recuerde: el modo remoto usa el espacio de trabajo del **host de la gateway**,
...
<Accordion title="¿Cómo desinstalo completamente OpenClaw?">Consulte la guía dedicada: [Desinstalación](/es/install/uninstall).</Accordion>
  ...
  <Accordion title="¿Pueden los agentes trabajar fuera del espacio de trabajo?">
    Sí. El espacio de trabajo es el **directorio de trabajo por defecto (cwd)** y el ancla de memoria, no un sandbox estricto.
    Las rutas relativas se resuelven dentro del espacio de trabajo, pero las rutas absolutas pueden acceder a otras
    ubicaciones del host a menos que se habilite el modo sandbox. Si necesitas aislamiento, utiliza
    [`agents.defaults.sandbox`](/es/gateway/sandboxing) o la configuración de sandbox por agente. Si
    quieres que un repositorio sea el directorio de trabajo por defecto, apunta el
    ...
    Ejemplo (repositorio como cwd por defecto):
  ...
  <Accordion title="Modo remoto: ¿dónde está el almacenamiento de la sesión?">
    El estado de la sesión es propiedad del **host de la puerta de enlace (gateway)**. Si estás en modo remoto, el almacenamiento de la sesión que te importa está en la máquina remota, no en tu portátil local. Consulta [Gestión de sesiones](/es/concepts/session).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="¿Qué formato tiene la configuración? ¿Dónde está?">
    OpenClaw lee una configuración opcional de **JSON5** desde `$OPENCLAW_CONFIG_PATH` (por defecto: `~/.openclaw/openclaw.json`):

    ```
    $OPENCLAW_CONFIG_PATH
    ```

    Si falta el archivo, utiliza valores predeterminados relativamente seguros (incluyendo un espacio de trabajo por defecto de `~/.openclaw/workspace`).

  </Accordion>

  <Accordion title='Establecí gateway.bind: "lan" (o "tailnet") y ahora nada escucha / la interfaz de usuario dice no autorizado'>
    Los enlaces de no retorno **requieren una ruta de autenticación de puerta de enlace válida**. En la práctica, esto significa:

    - autenticación de secreto compartido: token o contraseña
    - `gateway.auth.mode: "trusted-proxy"` detrás de un proxy inverso con conocimiento de identidad y de no retorno correctamente configurado
    ...
    - Las configuraciones de interfaz de usuario de control con secreto compartido se autentican mediante `connect.params.auth.token` o `connect.params.auth.password` (almacenados en la configuración de la aplicación/interfaz). Los modos portadores de identidad como Tailscale Serve o `trusted-proxy` utilizan encabezados de solicitud en su lugar. Evite poner secretos compartidos en URL.
    - Con `gateway.auth.mode: "trusted-proxy"`, los proxies inversos de retorno del mismo host todavía **no** satisfacen la autenticación de proxy confiable. El proxy confiable debe ser una fuente de no retorno configurada.

  </Accordion>

  <Accordion title="¿Por qué ahora necesito un token en localhost?">
    OpenClaw aplica la autenticación de puerta de enlace por defecto, incluyendo el bucle local (loopback). En la ruta predeterminada normal, esto significa autenticación por token: si no se configura una ruta de autenticación explícita, el inicio de la puerta de enlace resuelve al modo token y genera uno automáticamente, guardándolo en `gateway.auth.token`, por lo que **los clientes WS locales deben autenticarse**. Esto bloquea que otros procesos locales llamen a la puerta de enlace.

    Si prefieres una ruta de autenticación diferente, puedes elegir explícitamente el modo de contraseña (o, para proxies inversos con reconocimiento de identidad que no sean de bucle local, `trusted-proxy`). Si **realmente** quieres un bucle local abierto, establece `gateway.auth.mode: "none"` explícitamente en tu configuración. El Doctor puede generar un token para ti en cualquier momento: `openclaw doctor --generate-gateway-token`.
  ...
  <Accordion title="¿Cómo habilito la búsqueda web (y la recuperación web)?">
    ...
    - SearXNG es gratuito/autohospedado; configura `SEARXNG_BASE_URL` o `plugins.entries.searxng.config.webSearch.baseUrl`.
    ...
    **Recomendado:** ejecuta `openclaw configure --section web` y elige un proveedor.
    ...
    Las rutas de proveedor `tools.web.search.*` heredadas aún se cargan temporalmente por compatibilidad, pero no se deben usar en configuraciones nuevas.
    La configuración de respaldo de recuperación web de Firecrawl se encuentra en `plugins.entries.firecrawl.config.webFetch.*`.
  ...
  <Accordion title="config.apply borró mi configuración. ¿Cómo me recupero y evito esto?">
    ...
    - Las escrituras propiedad de OpenClaw que sean inválidas o destructivas se rechazan y guardan como `openclaw.json.rejected.*`.
    - Si una edición directa rompe el inicio o la recarga en caliente (hot reload), el Gateway restaura la última configuración conocida como buena y guarda el archivo rechazado como `openclaw.json.clobbered.*`.
    - El agente principal recibe una advertencia de arranque después de la recuperación para no volver a escribir ciegamente la mala configuración.

    Recuperación:

    - Verifica `openclaw logs --follow` para ver `Config auto-restored from last-known-good`, `Config write rejected:` o `config reload restored last-known-good config`.
    - Inspecciona el `openclaw.json.clobbered.*` o `openclaw.json.rejected.*` más reciente junto a la configuración activa.
    - Mantén la configuración activa restaurada si funciona, luego copia solo las claves deseadas con `openclaw config set` o `config.patch`.
    - Ejecuta `openclaw config validate` y `openclaw doctor`.
    - Si no tienes una última configuración conocida como buena o una carga rechazada, restaura desde una copia de seguridad, o vuelve a ejecutar `openclaw doctor` y reconfigura los canales/modelos.
    ...
    - Si estás utilizando la herramienta `gateway` solo para propietarios desde una ejecución de agente, aún rechazará las escrituras en `tools.exec.ask` / `tools.exec.security` (incluyendo alias `tools.bash.*` heredados que se normalizan a las mismas rutas de ejecución protegidas).
  ...
  <Accordion title="¿Puede el navegador de OpenClaw ejecutarse en modo headless?">
    ...
    El modo headless utiliza el **mismo motor de Chromium** y funciona para la mayor parte de la automatización (formularios, clics, scraping, inicios de sesión). Las principales diferencias son:
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale está conectado pero no recibo respuestas. ¿Qué hago?">
    ...
    - La pasarela está en ejecución: `openclaw gateway status`
    - Estado de la pasarela: `openclaw status`
  ...
  <Accordion title="¿Pueden dos instancias de OpenClaw comunicarse entre sí (local + VPS)?">
    Sí. No hay un puente "de bot a bot" integrado, pero puede configurarlo de algunas
    formas confiables:
    ...
    **Puente CLI (genérico):** ejecute un script que llame a la otra Pasarela con
    `openclaw agent --message ... --deliver`, apuntando a un chat donde el otro bot
    escuche. Si un bot está en un VPS remoto, apunte su CLI a esa Pasarela remota
    a través de SSH/Tailscale (consulte [Acceso remoto](/es/gateway/remote)).

    Patrón de ejemplo (ejecutar desde una máquina que pueda alcanzar la Pasarela de destino):

    ```bash
    openclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```

    Consejo: añada una salvaguarda para que los dos bots no entren en un bucle infinito (solo menciones,
    listas de permitidos de canales o una regla de "no responder a mensajes de bots").
  ...
  <Accordion title="¿Hay algún beneficio en usar un nodo en mi laptop personal en lugar de SSH desde un VPS?">
    ...
    más que el acceso a shell. El Gateway se ejecuta en macOS/Linux (Windows vía WSL2) y es
    ligero (un pequeño VPS o una caja tipo Raspberry Pi está bien; 4 GB de RAM son suficientes), por lo que una configuración
    ...
    - **Controles de ejecución más seguros.** `system.run` está restringido por listas de permitidos/aprobaciones de nodo en esa laptop.
    - **Más herramientas de dispositivo.** Los nodos exponen `canvas`, `camera` y `screen` además de `system.run`.
    - **Automatización del navegador local.** Mantenga el Gateway en un VPS, pero ejecute Chrome localmente a través de un host de nodo en la laptop, o conéctese al Chrome local en el host vía Chrome MCP.
  ...
  <Accordion title="¿Los nodos ejecutan un servicio de puerta de enlace?">
    No. Solo se debe ejecutar **una puerta de enlace** por host, a menos que ejecute intencionalmente perfiles aislados (consulte [Múltiples puertas de enlace](/es/gateway/multiple-gateways)). Los nodos son periféricos que se conectan
  ...
  <Accordion title="¿Existe una forma de API/RPC para aplicar la configuración?">
    ...
    - `config.apply`: validar y reemplazar la configuración completa; recarga en caliente cuando es posible y se reinicia cuando es necesario
  ...
  <Accordion title="¿Cómo configuro Tailscale en un VPS y me conecto desde mi Mac?">
    ...
    Esto mantiene el enlace (gateway) vinculado al bucle local (loopback) y expone HTTPS a través de Tailscale. Consulta [Tailscale](/es/gateway/tailscale).
  ...
  <Accordion title="¿Debo instalar en un segundo portátil o simplemente agregar un nodo?">
    ...
    **nodo**. Esto mantiene un solo Gateway y evita configuración duplicada. Las herramientas de nodo local
...
<AccordionGroup>
  ...
  <Accordion title="Inicié el Gateway mediante el servicio y mis variables de entorno desaparecieron. ¿Qué ahora?">
    ...
    2. Habilite la importación de shell (comodidad opt-in):
...
<AccordionGroup>
  ...
  <Accordion title="¿Existe alguna forma de crear un equipo de instancias de OpenClaw (un CEO y muchos agentes)?">
    ...
    Dicho esto, esto es mejor visto como un **experimento divertido**. Es pesado en tokens y a menudo
    menos eficiente que usar un solo bot con sesiones separadas. El modelo típico que
  ...
  <Accordion title='Recibo errores de "contexto demasiado grande" - ¿cómo restablezco o compacto?'>
    Use uno de estos:
    ...
    - Active o ajuste el **poda de sesión** (`agents.defaults.contextPruning`) para recortar el resultado de herramientas antiguo.
  ...
  <Accordion title="¿Por qué recibo mensajes de latido cada 30 minutos?">
    ...
    Si `HEARTBEAT.md` existe pero está efectivamente vacío (solo líneas en blanco y encabezados de markdown como `# Heading`), OpenClaw omite la ejecución del latido para guardar llamadas a la API. Si falta el archivo, el latido aún se ejecuta y el modelo decide qué hacer.
  ...
  <Accordion title='¿Necesito añadir una "cuenta de bot" a un grupo de WhatsApp?'>
    ...
    De forma predeterminada, las respuestas de grupo están bloqueadas hasta que permitas los remitentes (`groupPolicy: "allowlist"`).
...
<Accordion title="¿Los grupos/hilos comparten el contexto con los MDs?">Los chats directos se colapsan en la sesión principal de forma predeterminada. Los grupos/canales tienen sus propias claves de sesión, y los temas de Telegram / los hilos de Discord son sesiones separadas. Consulta [Grupos](/es/channels/groups) y [Mensajes de grupo](/es/channels/group-messages).</Accordion>
  ...
  <Accordion title="¿Cuántos espacios de trabajo y agentes puedo crear?">
    ...
    - Elimine sesiones antiguas (elimine archivos JSONL o entradas de almacenamiento) si el disco crece.
  ...
  <Accordion title="¿Puedo ejecutar varios bots o chats al mismo tiempo (Slack) y cómo debo configurarlo?">
    ...
    seguir bloqueando la automatización. Para el control del navegador más confiable, use Chrome MCP local en el host,
...
residen en las [Preguntas frecuentes sobre modelos](/es/help/faq-models).

## Gateway: puertos, "ya se está ejecutando" y modo remoto

<AccordionGroup>
  <Accordion title="¿Qué puerto usa el Gateway?">
    `gateway.port` controla el puerto multiplexado único para WebSocket + HTTP (Interfaz de usuario de control, ganchos, etc.).
  ...
  <Accordion title='¿Por qué el estado del gateway de openclaw dice "Runtime: running" pero "Connectivity probe: failed"?'>
  ...
  <Accordion title='¿Por qué el estado del gateway de openclaw muestra "Config (cli)" y "Config (service)" diferentes?'>
    ...
    Ejecute eso desde el mismo `--profile` / entorno en el que desea que el servicio opere.

  </Accordion>

  <Accordion title='¿Qué significa "another gateway instance is already listening"?'>
    OpenClaw impone un bloqueo en tiempo de ejecución vinculando el escucha de WebSocket inmediatamente al inicio (por defecto `ws://127.0.0.1:18789`). Si la vinculación falla con `EADDRINUSE`, lanza `GatewayLockError` indicando que otra instancia ya está escuchando.
  ...
  <Accordion title="¿Cómo ejecuto OpenClaw en modo remoto (el cliente se conecta a un Gateway en otro lugar)?">
    ...
    - La aplicación de macOS observa el archivo de configuración y cambia los modos en tiempo real cuando estos valores cambian.
  ...
  <Accordion title='La Interfaz de Control dice "no autorizado" (o sigue reconectando). ¿Qué hacer?'>
    La ruta de autenticación de tu puerta de enlace y el método de autenticación de la interfaz no coinciden.

    Datos (del código):

    - La Interfaz de Control mantiene el token en `sessionStorage` para la sesión actual de la pestaña del navegador y la URL de la puerta de enlace seleccionada, por lo que las recargas en la misma pestaña siguen funcionando sin restaurar la persistencia del token a largo plazo en localStorage.
    ...
    - Ese reintento con token en caché ahora reutiliza los alcances aprobados en caché almacenados con el token de dispositivo. Los llamadores explícitos de `deviceToken` / `scopes` aún mantienen su conjunto de alcances solicitados en lugar de heredar los alcances en caché.
    ...
    - El más rápido: `openclaw dashboard` (imprime + copia la URL del panel, intenta abrirlo; muestra una sugerencia SSH si no tiene interfaz gráfica).
    ...
    - Modo de proxy de confianza: asegúrate de que estás accediendo a través del proxy con reconocimiento de identidad que no sea de bucle de retorno configurado, no a través de un proxy de bucle de retorno del mismo host o de la URL de la puerta de enlace sin procesar.
  ...
  <Accordion title="Establezco gateway.bind tailnet pero no puede enlazar y nada escucha">
  ...
  <Accordion title='¿Qué significa "handshake no válido" / código 1008?'>
    ...
    3. Si la autenticación está activa, incluya el token/contraseña en la trama `connect`.
...
<AccordionGroup>
  ...
  <Accordion title="Cerré mi terminal en Windows: ¿cómo reinicio OpenClaw?">
    Existen **dos modos de instalación en Windows**:
    ...
    **2) Windows nativo (no recomendado):** la Gateway se ejecuta directamente en Windows.
    ...
    Docs: [Windows (WSL2)](/es/platforms/windows), [Manual de servicio del Gateway](/es/gateway).

  </Accordion>

  <Accordion title="La Gateway está activa pero las respuestas nunca llegan. ¿Qué debo revisar?">
    ...
    - La autenticación del modelo no se cargó en el **host de la gateway** (verifique `models status`).
    - El emparejamiento de canales/lista de permitidos bloquea las respuestas (verifique la configuración del canal + registros).
  ...
  <Accordion title='"Desconectado de la gateway: sin razón" - ¿y ahora qué?'>
  ...
  <Accordion title="El setMyCommands de Telegram falla. ¿Qué debería comprobar?">
  ...
  <Accordion title="ELI5: openclaw gateway restart vs openclaw gateway">
    ...
    Si instalaste el servicio, usa los comandos de la puerta de enlace. Usa `openclaw gateway` cuando
    quieras una ejecución única en primer plano.

  </Accordion>

  <Accordion title="Fastest way to get more details when something fails">
    Inicia la puerta de enlace con `--verbose` para obtener más detalles en la consola. Luego inspecciona el archivo de registro para ver errores de autenticación de canal, enrutamiento de modelos y RPC.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    ...
    - `tools.fs.workspaceOnly=false` permite que `MEDIA:` envíe archivos locales del host que el agente ya puede leer, pero solo para medios más tipos de documentos seguros (imágenes, audio, video, PDF y documentos de Office). Los archivos de texto sin formato y los similares a secretos todavía están bloqueados.
...
<AccordionGroup>
  ...
  <Accordion title="¿Es la inyección de prompts solo una preocupación para los bots públicos?">
    ...
    exfiltrar contexto o llamar a herramientas en su nombre. Reduzca el radio de impacto mediante:
    ...
    - tratar también el texto de archivos/documentos decodificados como no confiable: OpenResponses
      `input_file` y la extracción de archivos adjuntos de medios ambos envuelven el texto extraído en
  ...
  <Accordion title="¿Mi bot debería tener su propio correo electrónico, cuenta de GitHub o número de teléfono?">
    ...
    Empiece pequeño. Conceda acceso solo a las herramientas y cuentas que realmente necesite y amplíelo
  ...
  <Accordion title="¿Puedo darle autonomía sobre mis mensajes de texto y es seguro?">
    **No** recomendamos la plena autonomía sobre sus mensajes personales. El patrón más seguro es:
...
<Accordion title="¿Puedo usar modelos más baratos para tareas de asistente personal?">
  Sí, **siempre que** el agente sea solo de chat y la entrada sea confiable. Los niveles inferiores son más susceptibles al secuestro de instrucciones, por lo que debe evitarlos para agentes con herramientas o al leer contenido no confiable. Si debe usar un modelo más pequeño, restrinja las herramientas y ejecútelo dentro de un entorno limitado. Vea [Seguridad](/es/gateway/security).
</Accordion>
  ...
  <Accordion title="WhatsApp: ¿le enviará mensajes a mis contactos? ¿Cómo funciona el emparejamiento?">
    ...
    El aviso de número de teléfono del asistente: se usa para configurar su **allowlist/owner** para que se permitan sus propios MD. No se usa para el envío automático. Si usa su número personal de WhatsApp, use ese número y habilite `channels.whatsapp.selfChatMode`.
...
<AccordionGroup>
  <Accordion title="¿Cómo evito que los mensajes internos del sistema aparezcan en el chat?">
    ...
    Si sigue siendo ruidoso, verifique la configuración de la sesión en la Interfaz de Control y configure verbose
    ...
    Documentación: [Pensamiento y detallado](/es/tools/thinking), [Seguridad](/es/gateway/security#reasoning-verbose-output-in-groups).
  ...
  <Accordion title="¿Cómo detengo/cancelo una tarea en ejecución?">
    ...
    Resumen de comandos de barra: consulte [Comandos de barra](/es/tools/slash-commands).
  ...
  <Accordion title='¿Por qué da la impresión de que el bot "ignora" los mensajes rápidos?'>
    ...
    - `steer-backlog` - guíe ahora, luego procese el retraso
