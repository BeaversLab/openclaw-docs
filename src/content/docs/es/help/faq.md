...
4. **Sondas profundas**
   ...
   Ejecuta una prueba de salud activa de la pasarela, incluyendo pruebas de canal cuando es compatible
   (requiere una pasarela accesible). Consulte [Salud](/es/gateway/health).
...
5. **Ver el último registro**
   ...
   Los registros de archivos son separados de los registros del servicio; consulte [Registro](/es/logging) y [Solución de problemas](/es/gateway/troubleshooting).
...
7. **Instantánea de la puerta de enlace**
   ...
   Solicita a la pasarela en ejecución una instantánea completa (solo WS). Consulte [Salud](/es/gateway/health).
...
<AccordionGroup>
  ...
  <Accordion title="Propuesta de valor">
    ...
    - **Sus dispositivos, sus datos:** ejecute la Pasarela donde quiera (Mac, Linux, VPS) y mantenga el
    ...
    - **Código abierto y modificable:** inspeccione, extienda y aloje por su cuenta sin bloqueo de proveedor.

    Documentación: [Pasarela](/es/gateway), [Canales](/es/channels), [Multiagente](/es/concepts/multi-agent),
  ...
  <Accordion title="¿Puede OpenClaw ayudar con la generación de clientes, el contacto, la publicidad y los blogs para un SaaS?">
    Sí para **investigación, calificación y redacción**. Puede escanear sitios, crear listas,
    resumir prospectos y escribir borradores de mensajes de contacto o publicidad.

    Para **campañas de contacto o publicidad**, mantenga a un humano en el bucle. Evite el spam, cumpla con las leyes locales y
...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo puedo usar diferentes modelos para diferentes tareas?"
    ...
    Consulte [Cron jobs](/es/automation/cron-jobs), [Enrutamiento Multiagente](/es/concepts/multi-agent) y [Comandos de barra](/es/tools/slash-commands).
  ...
  <Accordion title="El bot se congela mientras realiza trabajos pesados. ¿Cómo puedo descargar eso?">
    ...
    modelo más económico para los sub-agentes a través de `agents.defaults.subagents.model`.
  ...
  <Accordion title="¿Cómo funcionan las sesiones de subagente vinculadas a hilos en Discord?">
    Utilice vinculaciones de hilos (thread bindings). Puede vincular un hilo de Discord a un subagente o un objetivo de sesión para que los mensajes de seguimiento en ese hilo permanezcan en esa sesión vinculada.
    ...
    - Auto-vincular al generar: `channels.discord.threadBindings.spawnSessions` tiene como valor predeterminado `true`; establézcalo en `false` para desactivar la generación de sesiones vinculadas a hilos.
  ...
  <Accordion title="El programador (cron) o los recordatorios no se ejecutan. ¿Qué debería verificar?">
  ...
  <Accordion title="Cron fired, but nothing was sent to the channel. Why?">
    ...
    - Un objetivo de anuncio (announce target) ausente o no válido (`channel` / `to`) significa que el ejecutor omitió la entrega saliente.
  ...
  <Accordion title="Why did an isolated cron run switch models or retry once?">
    ...
    El cron aislado puede persistir un traspaso de modelo en tiempo de ejecución y reintentar cuando la ejecución activa lanza `LiveSessionModelSwitchError`. El reintento mantiene el proveedor/modelo cambiado, y si el cambio llevaba una nueva anulación de perfil de autenticación, el cron también la persiste antes de reintentar.

    Reglas de selección relacionadas:

    - La anulación de modelo del enlace de Gmail (Gmail hook model override) gana primero cuando es aplicable.
    ...
    El bucle de reintento está limitado. Después del intento inicial más 2 reintentos de cambio, el cron aborta en lugar de hacer un bucle infinito.
  ...
  <Accordion title="¿Cómo instalo habilidades en Linux?">
    ...
    de forma predeterminada. Agregue `--global` para instalar en el directorio de habilidades
    administradas compartidas para todos los agentes locales. Instale la CLI `clawhub` separada
  ...
  <Accordion title="¿Puedo ejecutar habilidades exclusivas de Apple macOS desde Linux?">
    ...
    **Opción B: usar un nodo macOS (sin SSH).**
    Ejecuta el Gateway en Linux, empareja un nodo macOS (aplicación de la barra de menús) y configura **Node Run Commands** (Ejecutar comandos del nodo) en "Always Ask" (Preguntar siempre) o "Always Allow" (Permitir siempre) en la Mac. OpenClaw puede tratar las habilidades exclusivas de macOS como elegibles cuando los binarios requeridos existen en el nodo. El agente ejecuta esas habilidades a través de la herramienta `nodes`. Si eliges "Preguntar siempre", aprobar "Permitir siempre" en el mensaje añade ese comando a la lista de permitidos.

    **Opción C: usar proxies de binarios macOS a través de SSH (avanzado).**
    Mantén el Gateway en Linux, pero haz que los binarios de CLI requeridos se resuelvan a contenedores SSH que se ejecutan en una Mac. Luego anula la habilidad para permitir Linux de modo que siga siendo elegible.

    1. Crea un contenedor SSH para el binario (ejemplo: `memo` para Notas de Apple):
    ...
    2. Pon el contenedor en `PATH` en el host Linux (por ejemplo `~/bin/memo`).
  ...
  <Accordion title="¿Tienes una integración con Notion o HeyGen?">
    ...
    - **Habilidad / complemento personalizado:** es la mejor opción para un acceso confiable a la API (tanto Notion como HeyGen tienen APIs).
    ...
    - Pídale al agente que obtenga esa página al inicio de una sesión.
...
<AccordionGroup>
  <Accordion title="¿Hay algún documento dedicado al aislamiento (sandboxing)?">
    Sí. Consulte [Aislamiento](/es/gateway/sandboxing). Para una configuración específica de Docker (puerta de enlace completa en Docker o imágenes de aislamiento), consulte [Docker](/es/install/docker).
  </Accordion>
  ...
  <Accordion title="Docker parece limitado: ¿cómo habilito todas las funciones?">
    ...
    - Incorpora las dependencias del sistema a la imagen con `OPENCLAW_IMAGE_APT_PACKAGES`.
  ...
  <Accordion title="¿Puedo mantener los MDs personales pero hacer que los grupos sean públicos/sandboxed con un agente?">
    ...
    Usa `agents.defaults.sandbox.mode: "non-main"` para que las sesiones de grupo/canal (claves no principales) se ejecuten en el backend de sandbox configurado, mientras que la sesión principal de MDs permanece en el host. Docker es el backend predeterminado si no eliges uno. Luego, restringe qué herramientas están disponibles en las sesiones de sandbox mediante `tools.sandbox.tools`.
  ...
  <Accordion title="¿Cómo funciona la memoria?">
    ...
    OpenClaw también ejecuta un **flush de memoria de precompacción silencioso** para recordar al modelo
    que escriba notas duraderas antes de la autocompacción. Esto solo se ejecuta cuando el espacio de trabajo
  ...
  <Accordion title="La memoria sigue olvidando cosas. ¿Cómo hago que se guarde?">
    ...
    Esto es todavía un área que estamos mejorando. Ayuda recordarle al modelo que almacene memorias;
  ...
  <Accordion title="¿La memoria persiste para siempre? ¿Cuáles son los límites?">
    ...
    existe la búsqueda de memoria: extrae solo las partes relevantes al contexto.
  ...
  <Accordion title="¿La búsqueda de memoria semántica requiere una clave de API de OpenAI?">
    ...
    **no** otorga acceso a embeddings, por lo que **iniciar sesión con Codex (OAuth o el
    ...
    Si no hay disponible una clave de API de OpenAI, la búsqueda de memoria semántica permanece no disponible
    ...
    `memorySearch.remote.apiKey`). Admitimos modelos de embedding **OpenAI, compatibles con OpenAI, Gemini,
    Voyage, Mistral, Bedrock, Ollama, LM Studio, GitHub Copilot, DeepInfra o locales**
    - consulta [Memory](/es/concepts/memory) para obtener detalles de configuración.
...
<AccordionGroup>
  <Accordion title="¿Todos los datos utilizados con OpenClaw se guardan localmente?">
    ...
    - **Local de forma predeterminada:** las sesiones, archivos de memoria, configuración y el espacio de trabajo residen en el host de la Gateway
  ...
  <Accordion title="¿Dónde deben estar AGENTS.md / SOUL.md / USER.md / MEMORY.md?">
    ...
    - **Espacio de trabajo (por agente)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md`, `memory/YYYY-MM-DD.md`, opcional `HEARTBEAT.md`.
      La raíz en minúsculas `memory.md` es solo para entrada de reparación heredada; `openclaw doctor --fix`
      puede fusionarla en `MEMORY.md` cuando existan ambos archivos.
    - **Dir. de estado (`~/.openclaw`)**: configuración, estado del canal/proveedor, perfiles de autenticación, sesiones, registros,
    ...
    Si el bot "olvida" después de un reinicio, confirma que la Gateway está usando el mismo
    espacio de trabajo en cada inicio (y recuerda: el modo remoto usa el espacio de trabajo del **host de la gateway**,
    no de tu computadora portátil local).
...
<Accordion title="¿Cómo desinstalo OpenClaw completamente?">Consulta la guía dedicada: [Desinstalación](/es/install/uninstall).</Accordion>
  ...
  <Accordion title="Modo remoto: ¿dónde está el almacenamiento de sesiones?">
    El estado de la sesión es propiedad del **host de la puerta de enlace**. Si estás en modo remoto, el almacenamiento de sesiones que te interesa está en la máquina remota, no en tu portátil local. Consulta [Gestión de sesiones](/es/concepts/session).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo habilito la búsqueda web (y la recuperación web)?">
    ...
    - Grok puede reutilizar xAI OAuth de la autenticación del modelo, o usar como alternativa `XAI_API_KEY` / config del plugin web-search.
    - Ollama Web Search no requiere clave, pero usa su host Ollama configurado y requiere `ollama signin`.
    ...
    **Recomendado:** ejecute `openclaw configure --section web` y elija un proveedor.
    ...
    Las rutas de proveedor `tools.web.search.*` heredadas todavía se cargan temporalmente por compatibilidad, pero no se deben usar para nuevas configuraciones.
    La configuración alternativa de web-fetch de Firecrawl reside bajo `plugins.entries.firecrawl.config.webFetch.*`.
    ...
    - Si se omite `tools.web.fetch.provider`, OpenClaw detecta automáticamente el primer proveedor de recuperación (fetch) alternativo listo de las credenciales disponibles. Hoy el proveedor incluido es Firecrawl.
  ...
  <Accordion title="config.apply borró mi configuración. ¿Cómo me recupero y evito esto?">
    ...
    - Si una edición directa interrumpe el inicio o la recarga en caliente, Gateway falla de forma segura (fails closed) u omite la recarga; no reescribe `openclaw.json`.
    - `openclaw doctor --fix` se encarga de las reparaciones y puede restaurar el último estado conocido bueno (last-known-good) mientras guarda el archivo rechazado como `openclaw.json.clobbered.*`.
    ...
    - Inspeccione el `openclaw.json.clobbered.*` o el `openclaw.json.rejected.*` más reciente junto a la configuración activa.
    ...
    - Si está utilizando la herramienta `gateway` orientada al agente desde una ejecución de agente, todavía rechazará las escrituras en `tools.exec.ask` / `tools.exec.security` (incluidos los alias `tools.bash.*` heredados que se normalizan a las mismas rutas de ejecución protegidas).
  ...
  <Accordion title="¿Cómo ejecuto un Gateway central con trabajadores especializados en varios dispositivos?">
    ...
    - **Sub-agentes:** generan trabajo en segundo plano desde un agente principal cuando quieres paralelismo.
  ...
  <Accordion title="¿Cómo uso Brave para el control del navegador?">
    ...
    Vea los ejemplos completos de configuración en [Browser](/es/tools/browser#use-brave-or-another-chromium-based-browser).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo puede mi agente acceder a mi ordenador si el Gateway está alojado de forma remota?">
    ...
    Recordatorio de seguridad: el emparejamiento de un nodo macOS permite `system.run` en esa máquina. Solo
  ...
  <Accordion title="¿Pueden dos instancias de OpenClaw comunicarse entre sí (local + VPS)?">
    ...
    **Puente CLI (genérico):** ejecuta un script que llame al otro Gateway con
    ...
    Consejo: añade una medida de seguridad para que los dos bots no entren en un bucle infinito (solo menciones, listas blancas de canales o una regla de "no responder a mensajes de bots").
  ...
  <Accordion title="¿Hay algún beneficio en usar un nodo en mi computadora portátil personal en lugar de SSH desde un VPS?">
    ...
    - **Controles de ejecución más seguros.** `system.run` está limitado por listas de aprobación/aprobaciones de nodos en esa computadora portátil.
    - **Más herramientas de dispositivo.** Los nodos exponen `canvas`, `camera` y `screen` además de `system.run`.
    - **Automatización del navegador local.** Mantenga la puerta de enlace en un VPS, pero ejecute Chrome localmente a través de un host de nodo en la computadora portátil, o conéctese al Chrome local en el host a través de Chrome MCP.
  ...
  <Accordion title="¿Los nodos ejecutan un servicio de puerta de enlace?">
    ...
    Se requiere un reinicio completo para `gateway`, `discovery` y los cambios de superficie del complemento alojado.
  ...
  <Accordion title="¿Cómo configuro Tailscale en un VPS y me conecto desde mi Mac?">
    ...
    Si deseas la interfaz de usuario de control (Control UI) sin SSH, usa Tailscale Serve en el VPS:
  ...
  <Accordion title="¿Cómo conecto un nodo Mac a un Gateway remoto (Tailscale Serve)?">
    Serve expone la **Interfaz de usuario de control del Gateway + WS**. Los nodos se conectan a través del mismo endpoint WS del Gateway.
  ...
  <Accordion title="¿Debo instalar en un segundo portátil o simplemente agregar un nodo?">
    ...
    **nodo**. Esto mantiene un único Gateway y evita configuraciones duplicadas. Las herramientas de nodo locales
...
<AccordionGroup>
  <Accordion title="¿Cómo carga OpenClaw las variables de entorno?">
    ...
    Las variables de credenciales del proveedor son una excepción para el espacio de trabajo `.env`: claves como
    `GEMINI_API_KEY`, `XAI_API_KEY` o `MISTRAL_API_KEY` se ignoran del espacio de trabajo
    `.env` y deben residir en el entorno del proceso, `~/.openclaw/.env` o en la configuración `env`.
    ...
    Consulte [/environment](/es/help/environment) para obtener precedencia y fuentes completas.
  ...
  <Accordion title='Establecí COPILOT_GITHUB_TOKEN, pero el estado de los modelos muestra "Shell env: off." ¿Por qué?'>
    ...
    **no** significa que falten sus variables de entorno; solo significa que OpenClaw no cargará
    ...
    Si Gateway se ejecuta como servicio (launchd/systemd), no heredará su entorno
    ...
    3. O agréguelo a su bloque de configuración `env` (se aplica solo si falta).

    Luego reinicie el gateway y verifique nuevamente:
...
<AccordionGroup>
  ...
  <Accordion title='Estoy recibiendo errores de "context too large" - ¿cómo restablezco o compacto?'>
    ...
    Docs: [Compactación](/es/concepts/compaction), [Recorte de sesión](/es/concepts/session-pruning), [Gestión de sesiones](/es/concepts/session).
  ...
  <Accordion title="¿Por qué estoy recibiendo mensajes de latido cada 30 minutos?">
    Los latidos se ejecutan cada **30m** por defecto (**1h** al usar autenticación OAuth). Ajuste o desactívelos:
    ...
    de markdown como `# Heading`), OpenClaw omite la ejecución del latido para guardar llamadas a la API.
  ...
  <Accordion title="¿Cómo obtengo el JID de un grupo de WhatsApp?">
    Opción 1 (la más rápida): siga los registros y envíe un mensaje de prueba en el grupo:
    ...
    Opción 2 (si ya está configurado/en la lista permitida): liste los grupos desde la configuración:
  ...
  <Accordion title="¿Por qué OpenClaw no responde en un grupo?">
    ...
    - El filtrado de menciones está activado (predeterminado). Debes mencionar (@mention) al bot (o coincidir con `mentionPatterns`).
...
<Accordion title="¿Los grupos/hilos comparten el contexto con los MD?">Los chats directos se colapsan en la sesión principal de forma predeterminada. Los grupos/canales tienen sus propias claves de sesión, y los temas de Telegram / hilos de Discord son sesiones separadas. Consulta [Grupos](/es/channels/groups) y [Mensajes de grupo](/es/channels/group-messages).</Accordion>
  ...
  <Accordion title="¿Puedo ejecutar varios bots o chats al mismo tiempo (Slack) y cómo debo configurarlo?">
    Sí. Utiliza el **enrutamiento multiagente** para ejecutar varios agentes aislados y enrutar los mensajes entrantes por
    canal/cuenta/par. Slack es compatible como canal y puede vincularse a agentes específicos.

    El acceso del navegador es potente pero no "hacer cualquier cosa que un humano pueda"; el antibots, los CAPTCHAs y la MFA pueden
    seguir bloqueando la automatización. Para el control del navegador más fiable, utilice Chrome MCP local en el host,
...
Las preguntas y respuestas sobre modelos —valores predeterminados, selección, alias, cambios, conmutación por error, perfiles de autenticación—
...
<AccordionGroup>
  ...
  <Accordion title='La Interfaz de Control dice "unauthorized" (o sigue reconectando). ¿Qué hacer ahora?'>
    La ruta de autenticación de su puerta de enlace y el método de autenticación de la interfaz de usuario no coinciden.

    Datos (del código):

    - La Interfaz de Control mantiene el token en `sessionStorage` para la sesión de la pestaña actual del navegador y la URL de la puerta de enlace seleccionada, por lo que las recargas en la misma pestaña siguen funcionando sin restaurar la persistencia del token de localStorage a largo plazo.
    ...
    - Ese reintento con token en caché ahora reutiliza los alcances aprobados en caché almacenados con el token del dispositivo. Los que llaman con `deviceToken` explícita / `scopes` explícita aún mantienen su conjunto de alcances solicitado en lugar de heredar alcances en caché.
    - Fuera de esa ruta de reintento, la precedencia de autenticación de conexión es primero token/contraseña compartido explícito, luego `deviceToken` explícito, luego token de dispositivo almacenado, luego token de inicio (bootstrap).
    - El inicio (bootstrap) integrado del código de configuración es solo para nodos. Después de la aprobación, devuelve un token de dispositivo de nodo con `scopes: []` y no devuelve un token de operador entregado.

    Solución:

    - El más rápido: `openclaw dashboard` (imprime + copia la URL del panel, intenta abrirla; muestra una sugerencia de SSH si no hay interfaz gráfica).
    - Si aún no tiene un token: `openclaw doctor --generate-gateway-token`.
    - Si es remoto, haga un túnel primero: `ssh -N -L 18789:127.0.0.1:18789 user@host` y luego abra `http://127.0.0.1:18789/`.
    - Modo de secreto compartido: configure `gateway.auth.token` / `OPENCLAW_GATEWAY_TOKEN` o `gateway.auth.password` / `OPENCLAW_GATEWAY_PASSWORD`, luego pegue el secreto correspondiente en la configuración de la Interfaz de Control.
    - Modo Tailscale Serve: asegúrese de que `gateway.auth.allowTailscale` esté habilitado y de que está abriendo la URL de Serve, no una URL de loopback/tailnet sin procesar que omita los encabezados de identidad de Tailscale.
    - Modo de proxy de confianza: asegúrese de que está accediendo a través del proxy con conocimiento de identidad configurado, no a través de una URL de puerta de enlace sin procesar. Los proxies de loopback del mismo host también necesitan `gateway.auth.trustedProxy.allowLoopback = true`.
    ...
    - Si esa llamada de rotación dice que se denegó, verifique dos cosas:
    ...
    - ¿Todavía atascado? Ejecute `openclaw status --all` y siga [Solución de problemas](/es/gateway/troubleshooting). Consulte [Panel](/es/web/dashboard) para obtener detalles de autenticación.
  ...
  <Accordion title="¿Puedo ejecutar múltiples Gateways en el mismo host?">
    Generalmente no: un Gateway puede ejecutar múltiples canales de mensajería y agentes. Utilice múltiples Gateways solo cuando necesite redundancia (ej: bot de rescate) o aislamiento estricto.
  ...
  <Accordion title='¿Qué significa "invalid handshake" / código 1008?'>
    ...
    3. Si la autenticación está activada, incluya el token/contraseña en el frame `connect`.
...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo inicio/detengo/reinicio el servicio de puerta de enlace?">
    Use los asistentes de la puerta de enlace:
    ...
    Si ejecuta la puerta de enlace manualmente, `openclaw gateway --force` puede reclamar el puerto. Consulte [Puerta de enlace](/es/gateway).
  ...
  <Accordion title="El Gateway está activo pero las respuestas nunca llegan. ¿Qué debo verificar?">
    Comienza con un chequeo rápido de estado:
    ...
    - Autenticación del modelo no cargada en el **gateway host** (verifica `models status`).
    - Emparejamiento de canales/lista blanca bloqueando respuestas (verifica la configuración del canal y los registros).
    - WebChat/Dashboard está abierto sin el token correcto.

    Si estás de forma remota, confirma que la conexión del túnel/Tailscale esté activa y que el
  ...
  <Accordion title='"Desconectado del gateway: sin razón" - ¿qué hacer ahora?'>
    Esto generalmente significa que la IU perdió la conexión WebSocket. Verifica:
...
<AccordionGroup>
  ...
  <Accordion title="¿La inyección de indicaciones (prompt injection) solo es una preocupación para los bots públicos?">
    No. La inyección de indicaciones se trata del **contenido no confiable**, no solo de quién puede enviar mensajes privados al bot.
    ...
    - usar un agente "lector" de solo lectura o sin herramientas para resumir el contenido no confiable
    - mantener `web_search` / `web_fetch` / `browser` desactivado para los agentes con herramientas habilitadas
  ...
  <Accordion title="¿OpenClaw es menos seguro porque usa TypeScript/Node en lugar de Rust/WASM?">
    El lenguaje y el tiempo de ejecución importan, pero no son el principal riesgo para un agente
    personal. Los riesgos prácticos de OpenClaw son la exposición de la puerta de enlace (gateway), quién puede enviar mensajes al
    bot, la inyección de indicaciones, el alcance de las herramientas, el manejo de credenciales, el acceso al navegador, el acceso
    de ejecución y la confianza en habilidades de terceros o complementos.

    Rust y WASM pueden proporcionar un aislamiento más sólido para algunas clases de código, pero
    no resuelven la inyección de indicaciones, listas de permitidos deficientes, la exposición pública de la puerta de enlace,
    herramientas demasiado amplias o un perfil de navegador que ya haya iniciado sesión en cuentas
    sensibles. Trátelos como los controles principales:

    - mantener la puerta de enlace privada o autenticada
    - usar emparejamiento y listas de permitidos para mensajes privados y grupos
    - denegar o poner en sandbox herramientas arriesgadas para entradas no confiables
    - instalar solo complementos y habilidades de confianza
    - ejecutar `openclaw security audit --deep` después de cambios en la configuración

    Detalles: [Seguridad](/es/gateway/security), [Sandboxing](/es/gateway/sandboxing).

  </Accordion>

  <Accordion title="Vi informes sobre instancias de OpenClaw expuestas. ¿Qué debería comprobar?">
    ...
    - Gateway vinculado a `loopback`, o expuesto solo a través de acceso privado autenticado como una tailnet, túnel SSH, autenticación por token/contraseña, o un proxy de confianza configurado correctamente
    - MDs en modo `pairing` o `allowlist`
    - grupos en lista de permitidos y restringidos por mención a menos que cada miembro sea de confianza
    - herramientas de alto riesgo (`exec`, `browser`, `gateway`, `cron`) denegadas o con un alcance muy limitado para los agentes que leen contenido no confiable
    - sandboxing habilitado donde la ejecución de herramientas necesita un radio de explosión menor

    Los vínculos públicos sin autenticación, MDs/grupos abiertos con herramientas y el control del navegador expuesto son los hallazgos que se deben corregir primero. Detalles:
    [Security audit checklist](/es/gateway/security#security-audit-checklist).

  </Accordion>

  <Accordion title="¿Es seguro instalar habilidades de ClawHub y complementos de terceros?">
    Trata las habilidades y complementos de terceros como código que decides confiar.
    ...
    - preferir autores de confianza y versiones fijas
    - leer la habilidad o el complemento antes de habilitarlo
    - mantener las listas de permitidos para complementos y habilidades limitadas
    - ejecutar flujos de trabajo con entrada no confiable en un sandbox con herramientas mínimas
    - evitar dar al código de terceros acceso amplio al sistema de archivos, ejecución, navegador o secretos
  ...
  <Accordion title="¿Mi bot debería tener su propio correo electrónico, cuenta de GitHub o número de teléfono?">
    ...
    Comience de forma pequeña. Otorgue acceso solo a las herramientas y cuentas que realmente necesite, y amplíelo
  ...
  <Accordion title="¿Puedo darle autonomía sobre mis mensajes de texto y es seguro?">
    **No** recomendamos una autonomía total sobre sus mensajes personales. El patrón más seguro es:
...
<Accordion title="¿Puedo usar modelos más económicos para tareas de asistente personal?">
  Sí, **siempre que** el agente sea solo de chat y la entrada sea confiable. Los niveles inferiores son más susceptibles al secuestro de instrucciones, por lo que debe evitarlos para agentes con herramientas o al leer contenido no confiable. Si debe usar un modelo más pequeño, bloquee las herramientas y ejecútelas dentro de un sandbox. Vea [Seguridad](/es/gateway/security).
</Accordion>
  ...
  <Accordion title="WhatsApp: ¿enviará mensajes a mis contactos? ¿Cómo funciona el emparejamiento?">
    ...
    El prompt del número de teléfono del asistente: se usa para configurar su **lista de permitidos/propietario** para que se permitan sus propios MD. No se usa para el envío automático. Si ejecuta en su número personal de WhatsApp, use ese número y habilite `channels.whatsapp.selfChatMode`.
...
<AccordionGroup>
  <Accordion title="¿Cómo evito que los mensajes internos del sistema aparezcan en el chat?">
    ...
    Solucionarlo en el chat donde lo ve:
    ...
    Si todavía es ruidoso, verifique la configuración de la sesión en la Interfaz de Control (Control UI) y configure verbose
  ...
  <Accordion title="¿Cómo detengo/cancelo una tarea en ejecución?">
    ...
    La mayoría de los comandos deben enviarse como un mensaje **independiente** que comienza con `/`, pero algunos accesos directos (como `/status`) también funcionan en línea para remitentes en la lista de permitidos.
  ...
  <Accordion title='¿Por qué parece que el bot "ignora" los mensajes rápidos?'>
    Las indicaciones durante la ejecución se dirigen a la ejecución activa de forma predeterminada. Use `/queue` para elegir el comportamiento de la ejecución activa:
...
¿Todavía atascado? Pregunte en [Discord](https://discord.com/invite/clawd) o abra una [discusión en GitHub](https://github.com/openclaw/openclaw/discussions).

## Relacionado

- [Preguntas frecuentes de la primera ejecución](/es/help/faq-first-run) — instalación, incorporación, autenticación, suscripciones, fallos tempranos
