...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo añado modelos de otros proveedores como OpenRouter o Z.AI?">
    ...
    **No se encontró ninguna clave de API para el proveedor después de añadir un nuevo agente**
    ...
    - O copia solo perfiles estáticos portátiles `api_key` / `token` del almacén de autenticación del agente principal al almacén del nuevo agente.
...
<AccordionGroup>
  <Accordion title="¿Cómo funciona el conmutación por error?">
    ...
    errores de razón de detención como `Unhandled stop reason:
  ...
  <Accordion title='¿Qué significa "No credentials found for profile anthropic:default"?'>
    ...
    **Lista de comprobación de corrección para "No credentials found for profile anthropic"**
    ...
    - **Usar Claude CLI**
      - Ejecute `openclaw models auth login --provider anthropic --method cli --set-default` en el host de la gateway.
    - **Si desea usar una clave de API en su lugar**
      - Ponga `ANTHROPIC_API_KEY` en `~/.openclaw/.env` en el **host de la gateway**.
    ...
    - **Confirmar que está ejecutando comandos en el host de la gateway**
...
<AccordionGroup>
  ...
  <Accordion title="¿Puedo controlar qué perfil de autenticación se prueba primero?">
    Sí. La configuración admite metadatos opcionales para los perfiles y un orden por proveedor (`auth.order.<provider>`). Esto **no** almacena secretos; asigna identificadores a proveedor/modo y establece el orden de rotación.
    ...
    mientras que las ventanas de facturación/deshabilitado todavía bloquean todo el perfil.
