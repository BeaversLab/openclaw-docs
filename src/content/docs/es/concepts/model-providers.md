...
- Las reglas de tiempo de ejecución de respaldo (fallback), las sondas de enfriamiento y la persistencia de anulación de sesión están
...
- Los complementos del proveedor también pueden poseer el comportamiento de tiempo de ejecución del proveedor a través de
...
- El proveedor incluido `codex` está emparejado con el arnés del agente Codex incluido.
  Use `codex/gpt-*` cuando desee inicio de sesión de Codex, descubrimiento de modelos,
  reanudación nativa de hilos y ejecución en el servidor de aplicaciones. Las referencias simples `openai/gpt-*` continúan
...
- `auth[].run` / `auth[].runNonInteractive`: el proveedor posee los flujos
  de incorporación/inicio de sesión para `openclaw onboard`, `openclaw models auth` y la configuración sin cabeza (headless).
...
- `normalizeModelId`: el proveedor normaliza los IDs de modelos heredados/de vista previa antes
...
- `normalizeTransport`: el proveedor normaliza la familia de transporte `api` / `baseUrl`
  ...
  luego otros complementos de proveedor con capacidad de enlace hasta que uno realmente cambie
  el transporte.
- `normalizeConfig`: el proveedor normaliza la configuración `models.providers.<id>` antes
  de que el tiempo de ejecución la use; OpenClaw verifica primero el proveedor coincidente, luego otros
  complementos de proveedor con capacidad de enlace hasta que uno realmente cambie la configuración. Si ningún
  enlace de proveedor reescribe la configuración, los asistentes de la familia Google incluidos aún
  normalizan las entradas de proveedores Google compatibles.
- `applyNativeStreamingUsageCompat`: el proveedor aplica reescrituras de compatibilidad de uso de transmisión nativa impulsadas por el punto final para proveedores de configuración.
- `resolveConfigApiKey`: el proveedor resuelve la autenticación de marcador de entorno para proveedores de configuración
  ...
  solucionador de marcador de entorno AWS incorporado aquí, aunque la autenticación en tiempo de ejecución de Bedrock usa
  la cadena predeterminada del AWS SDK.
- `resolveSyntheticAuth`: el proveedor puede exponer disponibilidad de autenticación local/autoalojada u otra basada en configuración sin persistir secretos en texto plano
- `shouldDeferSyntheticProfileAuth`: el proveedor puede marcar los marcadores de posición de perfiles sintéticos almacenados con menor precedencia que la autenticación basada en env/ configuración
...
- `contributeResolvedModelCompat`: el proveedor aporta indicadores de compatibilidad para sus modelos de proveedor incluso cuando llegan a través de otro transporte compatible
- `capabilities`: el proveedor publica peculiaridades de transcripción/herramientas/familia de proveedor
- `normalizeToolSchemas`: el proveedor limpia los esquemas de herramientas antes de que el ejecutor integrado los vea
- `inspectToolSchemas`: el proveedor expone advertencias de esquema específicas del transporte después de la normalización
...
- `wrapStreamFn`: el proveedor aplica contenedores de compatibilidad de encabezados/cuerpo/modelo de solicitud
...
- `createEmbeddingProvider`: el proveedor es propietario del comportamiento de incrustación de memoria cuando corresponde al complemento del proveedor en lugar del concentrador de incrustación central
...
- `buildAuthDoctorHint`: el proveedor añade guía de reparación cuando falla la actualización de OAuth
...
- `classifyFailoverReason`: el proveedor asigna errores específicos del proveedor de transporte/API sin procesar a motivos de conmutación por error, como límite de velocidad o sobrecarga
- `isCacheTtlEligible`: el proveedor decide qué identificadores de modelo upstream admiten TTL de caché de indicaciones (prompt-cache)
...
- `augmentModelCatalog`: el proveedor agrega filas sintéticas/finales del catálogo después del descubrimiento y la combinación de configuraciones
...
- `supportsAdaptiveThinking`: el proveedor opta por los modelos seleccionados en `adaptive`
- `supportsMaxThinking`: el proveedor opta por los modelos seleccionados en `max`
- `resolveDefaultThinkingLevel`: el proveedor posee la política predeterminada de `/think` para una familia de modelos
...
- `isModernModelRef`: el proveedor posee la coincidencia de modelos preferidos en vivo/prueba de humo (live/smoke)
...
- `fetchUsageSnapshot`: el proveedor posee la obtención y el análisis del endpoint de uso, mientras que el núcleo sigue siendo el propietario del resumen y el formato
...
- `anthropic`: reserva de compatibilidad futura de Claude 4.6, sugerencias de reparación de autenticación, obtención del punto de conexión de uso, metadatos de caché-TTL/familia de proveedores y valores predeterminados de configuración global con conocimiento de autenticación
- `amazon-bedrock`: coincidencia de desbordamiento de contexto propiedad del proveedor y clasificación de motivos de conmutación por error para errores de limitación/no preparación específicos de Bedrock, además de la familia de retransmisión compartida `anthropic-by-model` para protecciones de política de retransmisión solo para Claude en el tráfico de Anthropic
- `anthropic-vertex`: protecciones de política de retransmisión solo para Claude en el tráfico de mensajes de Anthropic
- `openrouter`: identificadores de modelo de paso a través, contenedores de solicitudes, sugerencias de capacidad del proveedor, saneamiento de firma de pensamiento de Gemini en el tráfico de proxy de Gemini, inyección de razonamiento proxy a través de la familia de transmisión `openrouter-thinking`, reenvío de metadatos de enrutamiento y política de caché-TTL
- `github-copilot`: incorporación/inicio de sesión de dispositivo, reserva de compatibilidad futura del modelo, sugerencias de transcripción de pensamiento de Claude, intercambio de tokens en tiempo de ejecución y obtención del punto de conexión de uso
- `openai`: reserva de compatibilidad futura de GPT-5.4, normalización de transporte directo de OpenAI, sugerencias de autenticación faltante con conocimiento de Codex, supresión de Spark, filas sintéticas del catálogo de OpenAI/Codex, política de modelos en vivo/pensamiento, normalización de alias de tokens de uso (familias `input` / `output` y `prompt` / `completion`), la familia de transmisión compartida `openai-responses-defaults` para contenedores nativos de OpenAI/Codex, metadatos de familia de proveedores, registro de proveedor de generación de imágenes empaquetado para `gpt-image-1` y registro de proveedor de generación de video empaquetado para `sora-2`
- `google` y `google-gemini-cli`: retrocompatibilidad futura de Gemini 3.1, validación de repetición nativa de Gemini, saneamiento de repetición de arranque, modo de salida de razonamiento etiquetado, coincidencia de modelos modernos, registro de proveedor de generación de imágenes incluido para modelos de vista previa de imágenes de Gemini, y registro de proveedor de generación de video incluido para modelos Veo; Gemini CLI OAuth también se encarga del formato de token de perfil de autenticación, el análisis de token de uso y la obtención del endpoint de cuota para superficies de uso
...
- `zai`: retrocompatibilidad futura de GLM-5, valores predeterminados de `tool_stream`, política de TTL de caché, política de pensamiento binario/modelo en vivo y obtención de autenticación de uso + cuota; los IDs `glm-5*` desconocidos se sintetizan a partir de la plantilla incluida `glm-4.7`
- `xai`: normalización de transporte nativo de Responses, reescrituras de alias de `/fast` para variantes rápidas de Grok, `tool_stream` predeterminado, limpieza de esquema de herramientas específico de xAI / carga útil de razonamiento y registro de proveedor de generación de video incluido para `grok-imagine-video`
- `mistral`: metadatos de capacidad propiedad del complemento
- `opencode` y `opencode-go`: metadatos de capacidad propiedad del complemento además de saneamiento de firma de pensamiento proxy-Gemini
...
- `byteplus`: catálogos propiedad del complemento además de registro de proveedor de generación de video incluido para modelos de texto a video/imagen a video de Seedance
- `fal`: registro de proveedor de generación de video incluido para registro de proveedor de generación de imágenes de terceros alojado para modelos de imagen FLUX además de registro de proveedor de generación de video incluido para modelos de video de terceros alojados
- `cloudflare-ai-gateway`, `huggingface`, `kimi`, `nvidia`, `qianfan`,
  `stepfun`, `synthetic`, `venice`, `vercel-ai-gateway` y `volcengine`:
  solo catálogos propiedad del complemento
- `qwen`: catálogos propiedad del complemento para modelos de texto más registros de proveedores de comprensión de medios y generación de videos compartidos para sus superficies multimodales; la generación de video Qwen utiliza los endpoints de video estándar de DashScope con modelos Wan incluidos como `wan2.6-t2v` y `wan2.7-r2v`
- `runway`: registro de proveedor de generación de videos propiedad del complemento para modelos nativos basados en tareas de Runway como `gen4.5`
- `minimax`: catálogos propiedad del complemento, registro de proveedor de generación de videos incluido para modelos de video Hailuo, registro de proveedor de generación de imágenes incluido para `image-01`, selección de política de repetición híbrida Anthropic/OpenAI y lógica de autenticación/instantánea de uso
- `together`: catálogos propiedad del complemento más registro de proveedor de generación de videos incluido para modelos de video Wan
...
- El calentamiento (warm-up) de WebSocket de OpenAI Responses está habilitado de forma predeterminada mediante `params.openaiWsWarmup` (`true`/`false`)
...
- Los encabezados de atribución ocultos de OpenClaw (`originator`, `version`,
  `User-Agent`) solo se aplican al tráfico nativo de OpenAI hacia `api.openai.com`, no a
  los proxies compatibles con OpenAI genéricos
- Las rutas nativas de OpenAI también mantienen las Responses `store`, sugerencias de caché de prompt y
  configuración de payload de compatibilidad de razonamiento de OpenAI; las rutas de proxy no lo hacen
...
- Rotación opcional: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, además de `OPENCLAW_LIVE_ANTHROPIC_KEY` (única anulación)
- Modelo de ejemplo: `anthropic/claude-opus-4-6`
- CLI: `openclaw onboard --auth-choice apiKey`
- Las solicitudes públicas directas a Anthropic admiten el botón compartido `/fast` y `params.fastMode`, incluido el tráfico autenticado con clave de API y OAuth enviado a `api.anthropic.com`; OpenClaw lo mapea a Anthropic `service_tier` (`auto` vs `standard_only`)
...
- El token de configuración de Anthropic sigue disponible como una ruta de token compatible con OpenClaw, pero ahora OpenClaw prefiere el reuso de la CLI de Claude y `claude -p` cuando están disponibles.
...
- [Qwen Cloud](/es/providers/qwen): superficie del proveedor Qwen Cloud más mapeo de puntos finales de Alibaba DashScope y Coding Plan
- [MiniMax](/es/providers/minimax): acceso OAuth o clave de API de MiniMax Coding Plan
- [Modelos GLM](/es/providers/glm): Z.AI Coding Plan o puntos finales de API generales
...
- Proveedor: `google`
- Auth: `GEMINI_API_KEY`
- Rotación opcional: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, reserva `GOOGLE_API_KEY` y `OPENCLAW_LIVE_GEMINI_KEY` (anulación única)
...
- Las ejecuciones directas de Gemini también aceptan `agents.defaults.models["google/<model>"].params.cachedContent`
  (o el heredado `cached_content`) para reenviar un identificador `cachedContents/...` nativo del proveedor; los aciertos en la caché de Gemini aparecen como `cacheRead` de OpenClaw
...
- Precaución: OAuth de Gemini CLI en OpenClaw es una integración no oficial. Algunos usuarios han reportado restricciones en la cuenta de Google después de usar clientes de terceros. Revise los términos de Google y use una cuenta no crítica si decide continuar.
- OAuth de Gemini CLI se incluye como parte del complemento `google` incluido.
  - Primero instale Gemini CLI:
  ...
  - Si las solicitudes fallan después del inicio de sesión, configure `GOOGLE_CLOUD_PROJECT` o `GOOGLE_CLOUD_PROJECT_ID` en el host de puerta de enlace.
...
- El enrutamiento preciso ascendente detrás de `kilocode/kilo/auto` es propiedad de Kilo Gateway,
...
- OpenRouter permanece en la ruta compatible con OpenAI de estilo proxy, por lo que el modelado de solicitud
  nativo solo de OpenAI (`serviceTier`, Respuestas `store`,
  sugerencias de caché de avisos, cargas de compatibilidad de razonamiento de OpenAI) no se reenvía
- Las referencias de OpenRouter respaldadas por Gemini mantienen solo la saneamiento de firmas de pensamiento
  proxy-Gemini; la validación de repetición nativa de Gemini y las reescrituras de arranque permanecen desactivadas
- Kilo Gateway: `kilocode` (`KILOCODE_API_KEY`)
- Modelo de ejemplo: `kilocode/kilo/auto`
- Las referencias de Kilo respaldadas por Gemini mantienen la misma ruta de saneamiento de firmas de pensamiento
  proxy-Gemini; `kilocode/kilo/auto` y otras sugerencias no admitidas para el razonamiento proxy
...
- La configuración de incorporación/clave de API de MiniMax escribe definiciones de modelos M2.7 explícitas con
  `input: ["text", "image"]`; el catálogo de proveedores incluido mantiene las referencias de chat
  solo en texto hasta que se materializa la configuración de ese proveedor
...
La incorporación se establece por defecto en la superficie de codificación, pero el catálogo general `volcengine/*`
...
La incorporación se establece por defecto en la superficie de codificación, pero el catálogo general `byteplus/*`
se registra al mismo tiempo.

En los selectores de modelo de incorporación/configuración, la elección de autenticación de BytePlus prefiere las filas
...
En la ruta de transmisión compatible con Anthropic de MiniMax, OpenClaw desactiva el pensamiento de forma predeterminada
a menos que lo configure explícitamente, y `/fast on` reescribe
...
OpenClaw utiliza el `/api/v1/models` nativo de LM Studio y `/api/v1/models/load`
para el descubrimiento y la carga automática, con `/v1/chat/completions` para la inferencia de forma predeterminada.
Consulte [/providers/lmstudio](/es/providers/lmstudio) para la configuración y la solución de problemas.
...
para el onboarding, el modo en la nube/local y la configuración personalizada.
...
SGLang se distribuye como un complemento de proveedor incluido para servidores
...
- Para proveedores personalizados, `reasoning`, `input`, `cost`, `contextWindow` y `maxTokens` son opcionales.
  Cuando se omiten, OpenClaw utiliza por defecto:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- Las rutas compatibles con OpenAI de tipo proxy también omiten el modelado de solicitudes nativas solo de OpenAI:
  sin `service_tier`, sin Respuestas `store`, sin sugerencias de caché de indicaciones, sin
  modelado de carga de compatibilidad de razonamiento de OpenAI y sin encabezados de atribución
  ocultos de OpenClaw.
...
Consulte también: [/gateway/configuration](/es/gateway/configuration) para ejemplos de configuración completa.
