...
- Las reglas de tiempo de ejecución de respaldo, sondas de enfriamiento y persistencia de anulación de sesión están
...
- Los manifiestos de proveedores pueden declarar `providerAuthEnvVars` y
  `providerAuthAliases` para que las sondas de autenticación genéricas basadas en entorno y las variantes de proveedores
  no necesiten cargar el tiempo de ejecución del complemento. El mapa principal restante de variables de entorno es ahora
  solo para proveedores principales/no complementarios y algunos casos de precedencia genérica, como
  la incorporación prioritaria de clave de API de Anthropic.
- Los complementos de proveedores también pueden poseer el comportamiento de tiempo de ejecución del proveedor a través de
  ...
  `onModelSelected`.
- Nota: el tiempo de ejecución del proveedor `capabilities` son metadatos compartidos del ejecutor (familia del proveedor, peculiaridades de la transcripción/herramientas, sugerencias de transporte/caché). No es lo mismo que el [modelo de capacidad pública](/en/plugins/architecture#public-capability-model) que describe lo que un complemento registra (inferencia de texto, voz, etc.).
- El proveedor incluido `codex` está emparejado con el arnés del agente Codex incluido. Use `codex/gpt-*` cuando desee inicio de sesión propiedad de Codex, descubrimiento de modelos, reanudación nativa de hilos y ejecución en el servidor de aplicaciones. Las referencias simples `openai/gpt-*` continúan usando el proveedor OpenAI y el transporte normal del proveedor OpenClaw. Las implementaciones solo de Codex pueden deshabilitar la reserva automática a PI con `agents.defaults.embeddedHarness.fallback: "none"`; consulte [Codex Harness](/en/plugins/codex-harness).
...
- `auth[].run` / `auth[].runNonInteractive`: el proveedor posee los flujos de incorporación/inicio de sesión para `openclaw onboard`, `openclaw models auth` y la configuración sin cabeza
...
- `normalizeModelId`: el proveedor normaliza los identificadores de modelos heredados/de vista previa antes de la búsqueda o canonización
- `normalizeTransport`: el proveedor normaliza la familia de transporte `api` / `baseUrl` antes del ensamblaje del modelo genérico; OpenClaw verifica primero el proveedor coincidente y luego otros complementos de proveedor con capacidad de enlace hasta que uno realmente cambie el transporte
- `normalizeConfig`: el proveedor normaliza la configuración `models.providers.<id>` antes de que el tiempo de ejecución la use; OpenClaw verifica primero el proveedor coincidente y luego otros complementos de proveedor con capacidad de enlace hasta que uno realmente cambie la configuración. Si ningún enlace de proveedor reescribe la configuración, los asistentes de la familia Google incluidos aún normalizan las entradas de proveedores de Google compatibles.
...
- `resolveConfigApiKey`: el proveedor resuelve la autenticación de marcador de entorno para proveedores de configuración
  ...
  resolutor de marcador de entorno de AWS integrado aquí, aunque la autenticación en tiempo de ejecución de Bedrock utiliza
  la cadena predeterminada del AWS SDK.
- `resolveSyntheticAuth`: el proveedor puede exponer disponibilidad de autenticación local/autoalojada u otra
...
- `resolveDynamicModel`: el proveedor acepta IDs de modelo no presentes en el catálogo
  estático local aún
...
- `contributeResolvedModelCompat`: el proveedor contribuye con banderas de compatibilidad para sus
...
- `createStreamFn`: el proveedor reemplaza la ruta de transmisión normal con un
...
- `resolveTransportTurnState`: el proveedor proporciona encabezados o metadatos de transporte nativos
  por turno
- `resolveWebSocketSessionPolicy`: el proveedor proporciona encabezados de sesión de WebSocket nativos
  o política de enfriamiento de sesión
- `createEmbeddingProvider`: el proveedor posee el comportamiento de incrustación de memoria cuando pertenece
  al complemento del proveedor en lugar del concentrador de incrustación principal
...
- `refreshOAuth`: el proveedor es propietario de la actualización de OAuth cuando los `pi-ai` compartidos no son suficientes
...
- `classifyFailoverReason`: el proveedor asigna errores brutos de transporte/API específicos del proveedor a motivos de conmutación por error, como límite de velocidad o sobrecarga
- `isCacheTtlEligible`: el proveedor decide qué identificadores de modelo ascendentes soportan el TTL de caché de solicitud (prompt-cache)
- `buildMissingAuthMessage`: el proveedor reemplaza el error genérico de almacenamiento de autenticación con una sugerencia de recuperación específica del proveedor
...
- `isBinaryThinking`: el proveedor es propietario de la experiencia de usuario de pensamiento (thinking) binario encendido/apagado
...
- `isModernModelRef`: el proveedor es propietario de la coincidencia de modelo preferido en vivo/de humo (live/smoke)
...
- `fetchUsageSnapshot`: el proveedor es propietario de la obtucción/análisis del punto final de uso, mientras que el núcleo sigue siendo propietario del shell de resumen y el formato
...
- `anthropic`: respaldo de compatibilidad futura de Claude 4.6, sugerencias de reparación de autenticación, obtención del punto de conexión de uso, metadatos de TTL de caché/familia de proveedor y valores predeterminados de configuración global con conocimiento de autenticación
- `amazon-bedrock`: coincidencia de desbordamiento de contexto propiedad del proveedor y clasificación de motivos de conmutación por error para errores de estrangulamiento/no preparación específicos de Bedrock, además de la familia `anthropic-by-model` de repetición compartida para protecciones de política de repetición solo para Claude en el tráfico de Anthropic
- `anthropic-vertex`: protecciones de política de repetición solo para Claude en el tráfico de mensajes de Anthropic
- `openrouter`: identificadores de modelo de paso a través, contenedores de solicitudes, sugerencias de capacidad del proveedor, saneamiento de firma de pensamiento de Gemini en el tráfico proxy de Gemini, inyección de razonamiento proxy a través de la familia `openrouter-thinking` de flujo, reenvío de metadatos de enrutamiento y política de TTL de caché
- `github-copilot`: incorporación/inicio de sesión de dispositivo, respaldo de compatibilidad futura del modelo, sugerencias de transcripción de pensamiento de Claude, intercambio de tokens en tiempo de ejecución y obtención del punto de conexión de uso
- `openai`: respaldo de compatibilidad futura de GPT-5.4, normalización de transporte directo de OpenAI, sugerencias de autenticación faltante con conocimiento de Codex, supresión de Spark, filas de catálogo sintético de OpenAI/Codex, política de modelo en vivo/pensamiento, normalización de alias de token de uso (familias `input` / `output` y `prompt` / `completion`), la familia `openai-responses-defaults` de flujo compartida para contenedores nativos de OpenAI/Codex, metadatos de familia de proveedor, registro de proveedor de generación de imágenes incluido para `gpt-image-1` y registro de proveedor de generación de video incluido para `sora-2`
- `google` y `google-gemini-cli`: reserva de compatibilidad futura de Gemini 3.1, validación de repetición nativa de Gemini, saneamiento de repetición de arranque, modo de salida de razonamiento etiquetado, coincidencia de modelos modernos, registro de proveedor de generación de imágenes incluido para modelos de vista previa de imágenes de Gemini y registro de proveedor de generación de video incluido para modelos de Veo; Gemini CLI OAuth también posee el formato del token del perfil de autenticación, el análisis del token de uso y la obtención del punto final de cuota para superficies de uso
- `moonshot`: transporte compartido, normalización de carga útil de pensamiento propiedad del complemento
- `kilocode`: transporte compartido, encabezados de solicitud propiedad del complemento, normalización de carga útil de razonamiento, saneamiento de firma de pensamiento proxy-Gemini y política de caché-TTL
- `zai`: reserva de compatibilidad futura de GLM-5, valores predeterminados de `tool_stream`, política de caché-TTL, política de pensamiento binario/modelo en vivo y autenticación de uso + obtención de cuota; los ids de `glm-5*` desconocidos se sintetizan a partir de la plantilla incluida `glm-4.7`
- `xai`: normalización de transporte nativo de Responses, reescrituras de alias `/fast` para variantes rápidas de Grok, `tool_stream` predeterminado, limpieza de esquema de herramienta específico de xAI / carga útil de razonamiento y registro de proveedor de generación de video incluido para `grok-imagine-video`
...
- `byteplus`: catálogos propiedad del complemento más registro de proveedor de generación de video incluido para modelos de Seedance de texto a video/imagen a video
- `fal`: registro de proveedor de generación de video incluido para registro de proveedor de generación de imágenes de terceros alojado para modelos de imágenes FLUX más registro de proveedor de generación de video incluido para modelos de video de terceros alojados
- `cloudflare-ai-gateway`, `huggingface`, `kimi`, `nvidia`, `qianfan`,
  `stepfun`, `synthetic`, `venice`, `vercel-ai-gateway`, y `volcengine`:
  solo catálogos propiedad del complemento
- `qwen`: catálogos propiedad del complemento para modelos de texto más registros de proveedores compartidos para comprensión de medios y generación de video para sus superficies multimodales; la generación de video Qwen utiliza los puntos finales de video estándar de DashScope con modelos Wan incluidos como `wan2.6-t2v` y `wan2.7-r2v`
- `runway`: registro de proveedor de generación de video propiedad del complemento para modelos nativos basados en tareas de Runway como `gen4.5`
- `minimax`: catálogos propiedad del complemento, registro de proveedor de generación de video incluido para modelos de video Hailuo, registro de proveedor de generación de imágenes incluido para `image-01`, selección híbrida de política de repetición Anthropic/OpenAI y lógica de autenticación/instantánea de uso
...
- `/fast` y `params.fastMode` asignan solicitudes directas de `openai/*` Responses a `service_tier=priority` en `api.openai.com`
...
- Las rutas nativas de OpenAI también conservan las Responses `store`, las sugerencias de caché de prompts y
  la conformación de carga útil compatible con el razonamiento de OpenAI; las rutas de proxy no
- `openai/gpt-5.3-codex-spark` se suprime intencionalmente en OpenClaw porque la API en vivo de OpenAI la rechaza; Spark se trata como solo para Codex
...
- Nota de Anthropic: El personal de Anthropic nos informó que el uso de la CLI de Claude estilo OpenClaw vuelve a estar permitido, por lo que OpenClaw trata la reutilización de la CLI de Claude y el uso de `claude -p` como autorizados para esta integración, a menos que Anthropic publique una nueva política.
- El token de configuración de Anthropic permanece disponible como una ruta de token compatible con OpenClaw, pero OpenClaw ahora prefiere la reutilización de la CLI de Claude y `claude -p` cuando están disponibles.
...
- Comparte el mismo botón `/fast` y la configuración `params.fastMode` que `openai/*` directo; OpenClaw asigna eso a `service_tier=priority`
...
- [Qwen Cloud](/en/providers/qwen): superficie del proveedor Qwen Cloud más asignación de extremos de Alibaba DashScope y Coding Plan
- [MiniMax](/en/providers/minimax): acceso OAuth o clave de API de MiniMax Coding Plan
- [Modelos GLM](/en/providers/glm): extremos de Z.AI Coding Plan o API generales
...
- Las ejecuciones directas de Gemini también aceptan `agents.defaults.models["google/<model>"].params.cachedContent`
  (o el heredado `cached_content`) para reenviar un identificador `cachedContents/...` nativo del proveedor; los aciertos de caché de Gemini aparecen como `cacheRead` de OpenClaw
...
- OAuth de la CLI de Gemini se incluye como parte del complemento `google` incluido.
  ...
  - Nota: **no** pega un ID de cliente ni un secreto en `openclaw.json`. El flujo de inicio de sesión de la CLI almacena
  ...
  - Las respuestas JSON de la CLI de Gemini se analizan desde `response`; el uso vuelve a `stats`, con `stats.cached` normalizado a `cacheRead` de OpenClaw.
...
- El catálogo de reserva estático incluye `kilocode/kilo/auto`; el descubrimiento en vivo
  `https://api.kilo.ai/api/gateway/models` puede expandir aún más el catálogo
...
- Los marcadores `cache_control` de Anthropic específicos de OpenRouter también están limitados a
  rutas de OpenRouter verificadas, no a URL de proxy arbitrarias
- OpenRouter permanece en la ruta compatible con OpenAI de estilo proxy, por lo que el modelado de solicitudes
  nativo exclusivo de OpenAI (`serviceTier`, Respuestas `store`,
  sugerencias de caché de indicaciones, payloads de compatibilidad de razonamiento de OpenAI) no se reenvía
...
- Modelo de ejemplo: `kilocode/kilo/auto`
- Las referencias de Kilo respaldadas por Gemini mantienen la misma ruta de saneamiento de firmas de pensamiento
  de proxy-Gemini; `kilocode/kilo/auto` y otras sugerencias no admitidas para razonamiento por proxy
...
- La configuración de incorporación/clave API de MiniMax escribe definiciones explícitas de modelos M2.7 con
...
Muchos de los complementos de proveedor integrados a continuación ya publican un catálogo predeterminado.
...
Moonshot se envía como un complemento de proveedor integrado. Use el proveedor integrado de
manera predeterminada y agregue una entrada explícita de `models.providers.moonshot` solo cuando
...
La incorporación (onboarding) por defecto va a la superficie de codificación, pero el catálogo general `volcengine/*`
se registra al mismo tiempo.

En la incorporación/configuración de selectores de modelo, la elección de autenticación de Volcengine prefiere ambas
...
La incorporación (onboarding) por defecto va a la superficie de codificación, pero el catálogo general `byteplus/*`
se registra al mismo tiempo.

En los selectores de modelo de incorporación/configuración, la elección de autenticación de BytePlus prefiere ambas filas
`byteplus/*` y `byteplus-plan/*`. Si esos modelos aún no se han cargado,
OpenClaw recurre al catálogo sin filtrar en lugar de mostrar un selector
...
En la ruta de transmisión compatible con Anthropic de MiniMax, OpenClaw desactiva el pensamiento de forma
predeterminada a menos que lo configure explícitamente, y `/fast on` reescribe
...
Ollama se envía como un complemento de proveedor incluido y utiliza la API nativa de Ollama:
...
Ollama se detecta localmente en `http://127.0.0.1:11434` cuando activas esta opción con
...
vLLM se envía como un complemento de proveedor incluido para servidores
...
Para optar por el autodescubrimiento localmente (cualquier valor funciona si su servidor no impone autenticación):
...
SGLang se envía como un complemento de proveedor incluido para servidores
...
Para optar por el autodescubrimiento localmente (cualquier valor funciona si su servidor no
impone autenticación):
...
Consulte también: [/gateway/configuration](/en/gateway/configuration) para ver ejemplos de configuración completos.

## Relacionado

- [Models](/en/concepts/models) — configuración y alias de modelos
- [Model Failover](/en/concepts/model-failover) — cadenas de respaldo y comportamiento de reintentos
- [Configuration Reference](/en/gateway/configuration-reference#agent-defaults) — claves de configuración de modelos
- [Providers](/en/providers) — guías de configuración por proveedor
