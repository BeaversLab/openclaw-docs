...
Esta es una herramienta exclusiva para mantenedores. Las versiones empaquetadas de OpenClaw omiten intencionalmente `qa-lab`, por lo que `openclaw qa` solo está disponible desde una verificación del código fuente. Las verificaciones del código fuente cargan el ejecutor incluido directamente; no se necesita ningún paso de instalación del complemento.

Para obtener un contexto más amplio del marco de QA, consulte [Resumen de QA](/es/concepts/qa-e2e-automation).
...
| `--scenario <id>`     | —                                             | Ejecute solo este escenario. Repetible. Consulte [Escenarios](#scenarios).                                                                             |
...
| `fast`                 | Subconjunto de puerta de lanzamiento que ejercita el contrato de transporte en vivo: canary, filtrado de menciones, bloqueo de lista blanca, forma de respuesta, reanudación de reinicio, seguimiento de hilos, aislamiento de hilos, observación de reacciones. |
| `transport`            | Escenarios de hilos, MD, sala, unión automática, mención/lista blanca a nivel de transporte.                                                                                                                                                                     |
| `media`                | Cobertura de archivos adjuntos de imagen, audio, video, PDF, EPUB.                                                                                                                                                                                               |
| `e2ee-smoke`           | Cobertura E2EE mínima: respuesta cifrada básica, seguimiento de hilos, éxito del arranque.                                                                                                                                                                       |
...
- filtro de menciones y listas permitidas — `matrix-mention-*`, `matrix-allowlist-*`, `matrix-multi-actor-ordering`, `matrix-inbound-edit-*`, `matrix-mxid-prefixed-command-block`, `matrix-observer-allowlist-override`
- E2EE — `matrix-e2ee-*` (respuesta básica, seguimiento de hilo, arranque inicial, ciclo de vida de la clave de recuperación, variantes de pérdida de estado, comportamiento de respaldo del servidor, higiene de dispositivos, verificación SAS / QR / MD, reinicio, redacción de artefactos)
- CLI de E2EE — `matrix-e2ee-cli-*` (configuración de cifrado, configuración idempotente, fallo de arranque inicial, ciclo de vida de la clave de recuperación, multicuenta, viaje de ida y vuelta de respuesta de puerta de enlace, autoverificación)

Pase `--scenario <id>` (repetible) para ejecutar un conjunto seleccionado a mano; combinar con `--profile all` para ignorar el filtrado de perfiles.
...
| `OPENCLAW_QA_MATRIX_TUWUNEL_IMAGE`      | `ghcr.io/matrix-construct/tuwunel:v1.5.1` | Anular la imagen del servidor doméstico al validar contra una versión diferente de Tuwunel.                                                                                                                                                                      |
...
| `OPENCLAW_QA_MATRIX_CAPTURE_CONTENT`    | redactado                                 | `1` mantiene el cuerpo del mensaje y `formatted_body` en `matrix-qa-observed-events.json`. Por defecto se redacta para mantener los artefactos de CI seguros.                                                                                                    |
| `OPENCLAW_QA_MATRIX_DISABLE_FORCE_EXIT` | desactivado                               | `1` omite la `process.exit` determinista después de escribir el artefacto. El valor por defecto fuerza la salida porque los manejadores de cifrado nativos de matrix-js-sdk pueden mantener el bucle de eventos activo después de la finalización del artefacto. |
| `OPENCLAW_RUN_NODE_OUTPUT_LOG`          | sin establecer                            | Cuando es establecido por un lanzador externo (ej. `scripts/run-node.mjs`), Matrix QA reutiliza esa ruta de registro en lugar de iniciar su propio tee.                                                                                                          |

## Artefactos de salida

Escrito en `--output-dir`:

- `matrix-qa-report.md` — Informe de protocolo Markdown (qué pasó, falló, se omitió y por qué).
- `matrix-qa-summary.json` — Resumen estructurado adecuado para el análisis de CI y tableros.
- `matrix-qa-observed-events.json` — Eventos de Matrix observados desde los clientes conductor y observador. Los cuerpos están redactados a menos que `OPENCLAW_QA_MATRIX_CAPTURE_CONTENT=1`.
- `matrix-qa-output.log` — Stdout/stderr combinado de la ejecución. Si `OPENCLAW_RUN_NODE_OUTPUT_LOG` está establecido, se reutiliza el registro del lanzador externo en su lugar.

El directorio de salida por defecto es `<repo>/.artifacts/qa-e2e/matrix-<timestamp>` para que las ejecuciones sucesivas no se sobrescriban entre sí.

## Consejos de triaje

- **La ejecución se bloquea cerca del final:** los manejadores de cifrado nativo de `matrix-js-sdk` pueden sobrevivir al arnés. El valor predeterminado fuerza una `process.exit` limpia después de escribir el artefacto; si ha desactivado `OPENCLAW_QA_MATRIX_DISABLE_FORCE_EXIT=1`, espere que el proceso persista.
- **Error de limpieza:** busque el comando de recuperación impreso (una invocación de `docker compose ... down --remove-orphans`) y ejecútelo manualmente para liberar el puerto del homeserver.
- **Ventanas de aserción negativa intermitentes en CI:** reduzca `OPENCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS` (predeterminado 8 s) cuando la CI sea rápida; aumentelo en ejecutores compartidos lentos.
- **Necesita cuerpos redactados para un informe de error:** ejecute de nuevo con `OPENCLAW_QA_MATRIX_CAPTURE_CONTENT=1` y adjunte `matrix-qa-observed-events.json`. Trate el artefacto resultante como sensible.
- **Versión diferente de Tuwunel:** apunte `OPENCLAW_QA_MATRIX_TUWUNEL_IMAGE` a la versión bajo prueba. El carril solo incluye la imagen predeterminada fijada.

## Contrato de transporte en vivo

Matrix es uno de los tres carriles de transporte en vivo (Matrix, Telegram, Discord) que comparten una sola lista de verificación de contratos definida en [QA overview → Live transport coverage](/es/concepts/qa-e2e-automation#live-transport-coverage). `qa-channel` sigue siendo la suite sintética amplia y intencionalmente no forma parte de esa matriz.
...
- [Testing](/es/help/testing) — ejecución de pruebas y adición de cobertura de QA
- [Matrix](/es/channels/matrix) — el complemento de canal bajo prueba
