...
Las mutaciones de la transcripción usan un bloqueo de escritura de sesión en el archivo de transcripción. La adquisición del bloqueo espera hasta `session.writeLock.acquireTimeoutMs` antes de mostrar un error de sesión ocupada; el valor predeterminado es `60000` ms. Aumente esto solo cuando el trabajo legítimo de preparación, limpieza, compactación o duplicación de transcripción compite más tiempo en máquinas lentas. La detección de bloqueos obsoletos y las advertencias de retención máxima siguen siendo políticas separadas.
...
En `mode: "warn"`, OpenClaw informa los desalojos potenciales pero no muta el almacenamiento/archivos.
...
- `cron.sessionRetention` (por defecto `24h`) poda las sesiones antiguas de ejecución de cron aisladas del almacenamiento de sesión (`false` desactiva).
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` podan los archivos `~/.openclaw/cron/runs/<jobId>.jsonl` (valores predeterminados: `2_000_000` bytes y `2000` líneas).

Cuando cron fuerza la creación de una nueva sesión de ejecución aislada, sanea la entrada de sesión `cron:<jobId>` anterior antes de escribir la nueva fila. Mantiene preferencias seguras como la configuración de thinking/fast/verbose, etiquetas y anulaciones explícitas de modelo/autenticación seleccionadas por el usuario. Elimina el contexto de conversación ambiental como el enrutamiento de canal/grupo, la política de envío o cola, la elevación, el origen y el enlace de tiempo de ejecución de ACP para que una ejecución aislada nueva no pueda heredar la entrega obsoleta o la autoridad de tiempo de ejecución de una ejecución anterior.
...
- **Expiración por inactividad** (`session.reset.idleMinutes` o `session.idleMinutes` heredado) crea un nuevo `sessionId` cuando llega un mensaje después de la ventana de inactividad. Cuando tanto diario como inactividad están configurados, gana el que expire primero.
...
- **Política de bifurcación principal** usa la rama activa del PI al crear un hilo o bifurcación de subagente. Si esa rama es demasiado grande, OpenClaw inicia el hijo con contexto aislado en lugar de fallar o heredar un historial inutilizable. La política de tamaño es automática; la configuración heredada `session.parentForkMaxTokens` es eliminada por `openclaw doctor --fix`.
...
- `sessionId`: id de transcripción actual (el nombre de archivo se deriva de este a menos que `sessionFile` esté establecido)
- `sessionStartedAt`: marca de tiempo de inicio para el `sessionId` actual; la vigencia
  del restablecimiento diario usa esto. Las filas heredadas pueden derivarlo del encabezado de sesión JSONL.
- `lastInteractionAt`: marca de tiempo de la última interacción real de usuario/canal; la vigencia
  del restablecimiento por inactividad usa esto para que los eventos de latido, cron y exec no mantengan las sesiones
  vivas. Las filas heredadas sin este campo recurren a la hora de inicio de sesión recuperada
  para la vigencia por inactividad.
- `updatedAt`: marca de tiempo de la última mutación de la fila del almacén, utilizada para el listado, la poda y la contabilidad. No es la autoridad para la frescura del restablecimiento diario/inactivo.
...
- `memoryFlushCompactionCount`: recuento de compactación cuando se ejecutó la última descarga
...
- `branch_summary`: resumen persistente al navegar una rama de árbol

OpenClaw intencionalmente _no_ "corrige" las transcripciones; el Gateway usa `SessionManager` para leerlas/escribirlas.
...
La compactación resume la conversación antigua en una entrada `compaction` persistente en la transcripción y mantiene los mensajes recientes intactos.
...
coste de reapertura local, no un archivo en bruto: OpenClaw todavía ejecuta la compactación semántica normal,
...
añade un guardián de bucle de herramientas opcional. Después de que se añade un resultado de herramienta y antes de
la siguiente llamada al modelo, OpenClaw estima la presión del prompt utilizando la misma lógica de presupuesto
prevuelo utilizada al inicio del turno. Si el contexto ya no cabe, el guardián no
compacta dentro del enlace `transformContext` de Pi. Genera una señal
estructurada de preverificación de mitad de turno, detiene el envío del prompt actual y permite que
el bucle de ejecución externo utilice la ruta de recuperación existente: truncar los resultados de herramientas excesivamente grandes
cuando eso sea suficiente, o activar el modo de compactación configurado y reintentar. La
opción está deshabilitada por defecto y funciona tanto con `default` como con `safeguard`
modos de compactación, incluida la compactación de safeguard respaldada por el proveedor.
Esto es independiente de `maxActiveTranscriptBytes`: el guardián de tamaño en bytes se ejecuta
antes de que se abra un turno, mientras que la preverificación de mitad de turno se ejecuta más tarde en el bucle de herramientas de Pi integrado
después de que se hayan añadido los nuevos resultados de herramientas.
...
- La `/compact` manual respeta un `agents.defaults.compaction.keepRecentTokens` explícito
  y mantiene el punto de corte de la cola reciente de Pi. Sin un presupuesto de mantenimiento explícito,
  la compactación manual sigue siendo un punto de control duro y el contexto reconstruido comienza desde
  el nuevo resumen.
- Establezca `agents.defaults.compaction.midTurnPrecheck.enabled: true` para ejecutar la
  preverificación opcional del bucle de herramientas después de los nuevos resultados de herramientas y antes de la siguiente llamada
  al modelo. Esto es solo un disparador; la generación de resumen todavía usa la ruta de
  compactación configurada. Es independiente de `maxActiveTranscriptBytes`, que es un
  guardia de tamaño de bytes de la transcripción activa al inicio del turno.
- Establezca `agents.defaults.compaction.maxActiveTranscriptBytes` en un valor de bytes o
  ...
  activa se vuelve grande. Este guardia solo está activo cuando
...
- Cuando `agents.defaults.compaction.truncateAfterCompaction` está habilitado,
  ...
  de la compactación. La transcripción completa antigua permanece archivada y vinculada desde el
...
Los complementos pueden registrar un proveedor de compactación a través de `registerCompactionProvider()` en la API de complementos. Cuando `agents.defaults.compaction.provider` se establece en un id de proveedor registrado, la extensión de salvaguarda delega la resumen a ese proveedor en lugar de usar la canalización `summarizeInStages` integrada.

- `provider`: id de un complemento de proveedor de compactación registrado. Déjelo sin establecer para el resumen predeterminado de LLM.
...
- El modo de mecanismo de seguridad habilita las auditorías de calidad de resumen de forma predeterminada; establezca
