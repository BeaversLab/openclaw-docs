...
Para una recuperación delimitada, pase `rerun_group` al paraguas. `all` es la ejecución real de versión candidata, `ci` ejecuta solo el hijo de CI normal, `plugin-prerelease` ejecuta solo el hijo del complemento solo de versión, `release-checks` ejecuta cada casilla de versión, y los grupos de versión más estrechos son `install-smoke`, `cross-os`, `live-e2e`, `package`, `qa`, `qa-parity`, `qa-live` y `npm-telegram`. Las repeticiones enfocadas de `npm-telegram` requieren `release_package_spec` o `npm_telegram_package_spec`; las ejecuciones completas/totales con `release_profile=full` utilizan el artefacto del paquete release-checks. Las repeticiones enfocadas entre sistemas operativos pueden agregar `cross_os_suite_filter=windows/packaged-upgrade` u otro filtro de sistema operativo/suite. Los fallos de release-check de QA son consultivos; un fallo solo de QA no bloquea la validación de la versión.

### Vitest

La casilla Vitest es el flujo de trabajo secundario manual `CI`. El CI manual omite intencionalmente el alcance de los cambios y fuerza el gráfico de pruebas normal para el candidato de versión: fragmentos de Linux Node, fragmentos de complementos empaquetados, contratos de canal, compatibilidad con Node 22, `check`, `check-additional`, pruebas de humo de compilación, comprobaciones de documentación, habilidades de Python, Windows, macOS, Android e internacionalización de la Interfaz de Usuario de Control.
