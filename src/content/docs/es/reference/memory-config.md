...
<CardGroup cols={2}>
  ...
  <Card title="Motor integrado" href="/es/concepts/memory-builtin">
    Backend de SQLite predeterminado.
  </Card>
  ...
  <Card title="Memoria activa" href="/es/concepts/active-memory">
    Subagente de memoria para sesiones interactivas.
  </Card>
</CardGroup>

Todos los ajustes de búsqueda de memoria se encuentran bajo `agents.defaults.memorySearch` en `openclaw.json` a menos que se indique lo contrario.

<Note>
Si está buscando el interruptor de función de **memoria activa** y la configuración del subagente, eso se encuentra bajo `plugins.entries.active-memory` en lugar de `memorySearch`.

La memoria activa utiliza un modelo de dos niveles:

1. el complemento debe estar habilitado y apuntar al ID de agente actual
2. la solicitud debe ser una sesión de chat persistente interactiva elegible

Consulte [Memoria activa](/es/concepts/active-memory) para el modelo de activación, la configuración propiedad del complemento, la persistencia de la transcripción y el patrón de implementación segura.
...
<AccordionGroup>
  ...
  <Accordion title="Tipos de entrada compatibles con OpenAI">
    Los endpoints de integración compatibles con OpenAI pueden optar por campos de solicitud `input_type` específicos del proveedor. Esto es útil para modelos de integración asimétricos que requieren diferentes etiquetas para las integraciones de consultas y documentos.

    | Clave                 | Tipo     | Predeterminado | Descripción                                             |
    | ------------------- | -------- | ------- | ------------------------------------------------------- |
    | `inputType`         | `string` | no establecido   | `input_type` compartida para las integraciones de consulta y documentos   |
    | `queryInputType`    | `string` | no establecido   | `input_type` en el momento de la consulta; anula `inputType`          |
    ...
    Cambiar estos valores afecta la identidad de la caché de integración para la indexación por lotes del proveedor y debe ir seguido de una reindexación de la memoria cuando el modelo ascendente trata las etiquetas de manera diferente.
  ...
  <Accordion title="Bedrock">
    ...
    **Modelos compatibles** (con detección de familia y dimensiones predeterminadas):
    ...
    | ID del modelo                               | Proveedor  | Dimensiones pred. | Dimensiones config.  |
    | ------------------------------------------ | ---------- | ----------------- | -------------------- |
    ...
    **Autenticación:** La autenticación de Bedrock utiliza el orden estándar de resolución de credenciales del AWS SDK:
    ...
    La región se resuelve a partir de `AWS_REGION`, `AWS_DEFAULT_REGION`, el `baseUrl` del proveedor `amazon-bedrock`, o por defecto es `us-east-1`.
  ...
  <Accordion title="Local (GGUF + node-llama-cpp)">
    ...
    | `local.contextSize`   | `number \| "auto"` | `4096`                 | Tamaño de la ventana de contexto para el contexto de incrustación. 4096 cubre fragmentos típicos (128–512 tokens) limitando la VRAM no ponderada. Reduzca a 1024–2048 en hosts con restricciones. `"auto"` usa el máximo entrenado del modelo — no recomendado para modelos de 8B+ (Qwen3-Embedding-8B: 40 960 tokens → ~32 GB VRAM vs ~8.8 GB en 4096). |
    ...
    Si `provider` es `auto`, `local` se selecciona solo cuando `local.modelPath` apunta a un archivo local existente. Las referencias a modelos `hf:` y HTTP(S) aún se pueden usar explícitamente con `provider: "local"`, pero no hacen que `auto` seleccione local antes de que el modelo esté disponible en el disco.
...
Sin establecer, se usa el valor predeterminado del proveedor: 600 segundos para proveedores locales/autoalojados como `local`, `ollama` y `lmstudio`, y 120 segundos para proveedores alojados. Aumente esto cuando los lotes de incrustación locales limitados por la CPU funcionen correctamente pero sean lentos.
...
<Tabs>
  <Tab title="MMR (diversidad)">
    | Clave           | Tipo      | Predeterminado | Descripción                          |
    | ------------- | --------- | ------- | ------------------------------------ |
    | `mmr.enabled` | `boolean` | `false` | Habilitar reordenamiento MMR                |
...
Para la búsqueda de transcripciones entre agentes con alcance de agente, use `agents.list[].memorySearch.qmd.extraCollections` en lugar de `memory.qmd.paths`. Esas colecciones adicionales siguen la misma forma `{ path, name, pattern? }`, pero se fusionan por agente y pueden conservar nombres compartidos explícitos cuando la ruta apunta fuera del espacio de trabajo actual. Si la misma ruta resuelta aparece tanto en `memory.qmd.paths` como en `memorySearch.qmd.extraCollections`, QMD conserva la primera entrada y omite el duplicado.
...
| Clave              | Tipo      | Predeterminado | Descripción                                        |
| ------------------ | --------- | -------------- | -------------------------------------------------- |
| `cache.enabled`    | `boolean` | `false`        | Cachear las incrustaciones de fragmentos en SQLite |
...
Esto es independiente de `sync.embeddingBatchTimeoutSeconds`, que controla las llamadas de incrustación en línea utilizadas por proveedores locales/autohospedados y proveedores hospedados cuando las API por lotes del proveedor no están activas.
...
<Warning>La indexación de sesión es opcional y se ejecuta de forma asincrónica. Los resultados pueden estar ligeramente obsoletos. Los registros de sesión residen en el disco, por lo que se debe tratar el acceso al sistema de archivos como el límite de confianza.</Warning>
...
`searchMode: "search"` es solo léxico/BM25. OpenClaw no ejecuta sondeos de preparación de vectores semánticos ni mantenimiento de incrustaciones de QMD para ese modo, incluso durante `memory status --deep`; `vsearch` y `query` continúan requiriendo la preparación de vectores de QMD y las incrustaciones.

OpenClaw prefiere las formas de colección de QMD actuales y las formas de consulta de MCP, pero mantiene las versiones anteriores de QMD funcionando al intentar indicadores de patrón de colección compatibles y nombres de herramientas de MCP más antiguos cuando es necesario. Cuando QMD anuncia soporte para múltiples filtros de colección, las colecciones de la misma fuente se buscan con un solo proceso de QMD; las compilaciones anteriores de QMD mantienen la ruta de compatibilidad por colección. Misma fuente significa que las colecciones de memoria duradera se agrupan juntas, mientras que las colecciones de transcripciones de sesión permanecen como un grupo separado para que la diversificación de fuentes siga teniendo ambas entradas.

<Note>Las anulaciones del modelo QMD permanecen en el lado de QMD, no en la configuración de OpenClaw. Si necesita anular los modelos de QMD globalmente, establezca variables de entorno como `QMD_EMBED_MODEL`, `QMD_RERANK_MODEL` y `QMD_GENERATE_MODEL` en el entorno de ejecución de la puerta de enlace.</Note>
...
<AccordionGroup>
  <Accordion title="Programa de actualización">
    ...
    | `update.debounceMs`       | `number`  | `15000` | Debounce de cambios de archivo                 |
  ...
  <Accordion title="Alcance">
    ...
    El valor predeterminado incluido permite sesiones directas y de canal, mientras que sigue denegando grupos.
...
Soñar se ejecuta como un barrido programado y utiliza fases internas ligeras/profundas/REM como un detalle de implementación.
...
| Clave       | Tipo      | Predeterminado        | Descripción                                                      |
| ----------- | --------- | --------------------- | ---------------------------------------------------------------- |
| `enabled`   | `boolean` | `false`               | Habilitar o deshabilitar la función de soñar completamente       |
...
<Note>
  - El sueño escribe el estado de la máquina en `memory/.dreams/`. - El sueño escribe una salida narrativa legible por humanos en `DREAMS.md` (o en el `dreams.md` existente). - `dreaming.model` utiliza el control de confianza del subagente de plugin existente; configure `plugins.entries.memory-core.subagent.allowModelOverride: true` antes de habilitarlo. - La política y los umbrales de la fase
  ligera/profunda/REM son comportamiento interno, no configuración para el usuario.
</Note>
