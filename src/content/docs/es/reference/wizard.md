...
<Note>Si no se detecta ninguna GUI, el onboarding imprime instrucciones de reenvío de puerto SSH para la interfaz de usuario de control (Control UI) en lugar de abrir un navegador. Si faltan los recursos de la interfaz de usuario de control, el onboarding intenta compilarlos; la alternativa es `pnpm ui:build` (autoinstala dependencias de la UI).</Note>
