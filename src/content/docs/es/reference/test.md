...
- `pnpm check:changed`: ejecuta el gate de comprobación de cambios inteligente para el diff contra `origin/main`. Ejecuta los comandos typecheck, lint y guard para los carriles arquitectónicos afectados, pero no ejecuta las pruebas de Vitest. Use `pnpm test:changed` o `pnpm test <target>` explícito para la prueba de validación.
- Árboles de trabajo de Codex y checkouts vinculados/esparcidos: evita `pnpm test*`, `pnpm check*` y `pnpm crabbox:run` locales directos a menos que hayas verificado que pnpm no reconciliará las dependencias. Para una prueba diminuta de archivos explícitos usa `node scripts/run-vitest.mjs <path-or-filter>`; para puertas modificadas o una prueba amplia usa `node scripts/crabbox-wrapper.mjs run --provider blacksmith-testbox ... --shell -- "pnpm check:changed"` para que pnpm se ejecute dentro de Testbox.
