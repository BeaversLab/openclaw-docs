...
- Para clientes móviles nativos que se conectan directamente a una puerta de enlace remota, se prefiere [WebChat](/es/web/webchat) o el [Protocolo de puerta de enlace](/es/gateway/protocol) e implementar el flujo de inicio de dispositivo emparejado/token de dispositivo para que el dispositivo no necesite un token/contraseña HTTP compartido.
- Cree un complemento de canal en su lugar cuando esté integrando una red de mensajería externa con sus propios usuarios, salas, entrega de webhooks o transporte de salida. Consulte [Creación de complementos](/es/plugins/building-plugins).
...
- `messages[*].tool_call_id` para vincular los resultados de la herramienta a una llamada de herramienta anterior
- `max_completion_tokens`: número; límite por llamada para el total de tokens de finalización (tokens de razonamiento incluidos). Nombre de campo actual de OpenAI Chat Completions; se prefiere cuando se envían tanto `max_completion_tokens` como `max_tokens`.
...
- `temperature`: número; temperatura de muestreo con mejor esfuerzo reenviada al proveedor ascendente a través del canal de parámetros de flujo del agente.
- `top_p`: número; muestreo de núcleo con mejor esfuerzo reenviado al proveedor ascendente a través del canal de parámetros de flujo del agente.
- `frequency_penalty`: número; penalización de frecuencia con mejor esfuerzo reenviada al proveedor ascendente a través del canal de parámetros de flujo del agente. Rango validado: -2.0 a 2.0. Devuelve `400 invalid_request_error` para valores fuera de rango.
- `presence_penalty`: número; penalización de presencia con mejor esfuerzo reenviada al proveedor ascendente a través del canal de parámetros de flujo del agente. Rango validado: -2.0 a 2.0. Devuelve `400 invalid_request_error` para valores fuera de rango.
- `seed`: número (entero); semilla con el mejor esfuerzo enviada al proveedor de origen a través del canal de parámetros de flujo del agente. Devuelve `400 invalid_request_error` para valores no enteros.
- `stop`: cadena o matriz de hasta 4 cadenas; secuencias de parada con el mejor esfuerzo enviadas al proveedor de origen a través del canal de parámetros de flujo del agente. Devuelve `400 invalid_request_error` si hay más de 4 secuencias o entradas que no son cadenas o están vacías.

Cuando se establece cualquier campo de límite de tokens, el valor se reenvía al proveedor de origen a través del canal de parámetros de flujo del agente. El nombre real del campo de cable enviado al proveedor de origen lo elige el transporte del proveedor: `max_completion_tokens` para los puntos de conexión de la familia OpenAI, y `max_tokens` para los proveedores que solo aceptan el nombre heredado (como Mistral y Chutes). Los campos de muestreo (`temperature`, `top_p`, `frequency_penalty`, `presence_penalty`, `seed`) siguen el mismo canal de parámetros de flujo; el backend de respuestas de Codex basado en ChatGPT los elimina en el servidor ya que utiliza un muestreo fijo. `stop` también utiliza el canal de parámetros de flujo y se asigna al campo de parada del transporte (`stop` para los backends de Chat Completions, `stop_sequences` para Anthropic); la API de OpenAI Responses no tiene un parámetro de parada, por lo que `stop` no se aplica en los modelos respaldados por Responses.

### Variantes no compatibles

El punto de conexión devuelve `400 invalid_request_error` para variantes de herramientas no compatibles, incluyendo:
...
- valores `tool_choice.function.name` que no coinciden con `tools` proporcionado

Para `tool_choice: "required"` y `tool_choice` con función fijada, el endpoint reduce el conjunto de herramientas de función del cliente expuesto, instruye al tiempo de ejecución para que llame a una herramienta del cliente antes de responder y devuelve un error si la respuesta del agente no incluye una llamada estructurada coincidente a la herramienta del cliente. Este contrato se aplica a la lista `tools` HTTP proporcionada por el llamador, no a cada herramienta interna del agente de OpenClaw.
...
Reutilice el mismo valor `user` en llamadas posteriores para esa conversación para continuar la misma sesión del agente.
