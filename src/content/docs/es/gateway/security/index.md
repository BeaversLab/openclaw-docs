...
- Los niveles de ámbito del operador y las comprobaciones en el momento de aprobación se resumen en
...
se activan interruptores de depuración inseguros/peligrosos conocidos. Mantenga estos sin establecer en
producción. Cada bandera activada se informa como su propio hallazgo. Si se configuran
supresiones de auditoría, `security.audit.suppressions.active` permanece en la
...
- los proxies inversos de bucle invertido del mismo host pueden satisfacer `gateway.auth.mode: "trusted-proxy"` solo cuando `gateway.auth.trustedProxy.allowLoopback = true`; de lo contrario, use autenticación de token/contraseña
...
Los encabezados de proxy de confianza no hacen que el emparejamiento de dispositivos del nodo sea confiable automáticamente.
`gateway.nodes.pairing.autoApproveCidrs` es una política de operador separada, deshabilitada de forma predeterminada.
Incluso cuando está habilitada, las rutas de encabezados de proxy de confianza de origen de bucle local (loopback)
se excluyen de la aprobación automática del nodo porque los autores de llamadas locales pueden falsificar esos
encabezados, incluso cuando la autenticación de proxy de confianza de bucle local está explícitamente habilitada.
...
- Si la puerta de enlace misma termina HTTPS, puede establecer `gateway.http.securityHeaders.strictTransportSecurity` para emitir el encabezado HSTS desde las respuestas de OpenClaw.
...
- Para las implementaciones de la Interfaz de usuario de Control (Control UI) que no sean de bucle local, se requiere `gateway.controlUi.allowedOrigins` de forma predeterminada.
- `gateway.controlUi.allowedOrigins: ["*"]` es una política explícita de permitir todos los orígenes del navegador, no una configuración predeterminada endurecida. Evítela fuera de las pruebas locales estrechamente controladas.
- Los fallos de autenticación de origen del navegador en bucle local aún están limitados por velocidad incluso cuando la
  exención general de bucle local está habilitada, pero la clave de bloqueo se limita por cada
  valor normalizado de `Origin` en lugar de un cubo de localhost compartido.
...
- Trate el comportamiento de reenlace de DNS y el encabezado de host de proxy como problemas de endurecimiento de la implementación; mantenga `trustedProxies` ajustado y evite exponer la puerta de enlace directamente a la Internet pública.

## Los registros de sesión local residen en el disco

OpenClaw almacena las transcripciones de la sesión en el disco bajo `~/.openclaw/agents/<agentId>/sessions/*.jsonl`.
...
- La puerta de enlace aplica una política global de comandos de nodo aproximada a través de `gateway.nodes.allowCommands` / `denyCommands`.
...
- Para `host=node`, las ejecuciones respaldadas por aprobaciones también almacenan un `systemRunPlan` preparado canónico; los reenvíos aprobados posteriormente reutilizan ese plan almacenado, y la validación de la puerta de enlace rechaza las ediciones de la persona que llama al contexto de comando/cwd/sesión después de que se haya creado la solicitud de aprobación.
...
Los comandos de barra y directivas solo se respetan para **remitentes autorizados**. La autorización se deriva de listas de permitidos/emparejamiento de canales más `commands.useAccessGroups` (consulte [Configuration](/es/gateway/configuration)
...
de falla cerrada (fail-closed) de forma predeterminada: solo un conjunto limitado de rutas de indicador, modelo y filtrado de menciones
...
`commands.restart=false` solo bloquea las acciones de reinicio. No deshabilita las acciones de configuración/actualización `gateway`.
...
- Prefiera listas de permitidos (allowlists) `plugins.allow` explícitas.
...
- Si instala o actualiza complementos (`openclaw plugins install <package>`, `openclaw plugins update <id>`), trátelo como ejecutar código no confiable:
  ...
  - OpenClaw ejecuta un análisis integrado de código peligroso antes de la instalación/actualización. Los hallazgos `critical` bloquean de forma predeterminada.
  - Las instalaciones de complementos npm y git ejecutan la convergencia de dependencias del administrador de paquetes solo durante el flujo explícito de instalación/actualización. Las rutas locales y los archivos se tratan como paquetes de complementos autocontenidos; OpenClaw los copia o hace referencia a ellos sin ejecutar `npm install`.
  - Prefiera versiones fijas y exactas (`@scope/pkg@1.2.3`) e inspeccione el código descomprimido en el disco antes de habilitar.
  - `--dangerously-force-unsafe-install` es solo de emergencia (break-glass) para falsos positivos del análisis integrado en los flujos de instalación/actualización de complementos. No evita los bloqueos de política del enlace (hook) `before_install` del complemento ni evita los fallos del análisis.
  - Las instalaciones de dependencias de habilidades respaldadas por la pasarela siguen la misma división peligroso/sospechoso: los hallazgos `critical` integrados bloquean a menos que la persona que llama establezca explícitamente `dangerouslyForceUnsafeInstall`, mientras que los hallazgos sospechosos solo advierten. `openclaw skills install` sigue siendo el flujo separado de descarga/instalación de habilidades de ClawHub.

Detalles: [Plugins](/es/tools/plugin)

## Modelo de acceso DM: emparejamiento, lista de permitidos, abierto, deshabilitado

Todos los canales actuales compatibles con DM admiten una política de DM (`dmPolicy` o `*.dm.policy`) que bloquea los mensajes entrantes **antes** de que se procese el mensaje:

- `pairing` (predeterminado): los remitentes desconocidos reciben un código de emparejamiento corto y el bot ignora su mensaje hasta que se aprueba. Los códigos caducan después de 1 hora; los DM repetidos no reenviarán un código hasta que se cree una nueva solicitud. Las solicitudes pendientes se limitan a **3 por canal** de forma predeterminada.
- `allowlist`: los remitentes desconocidos están bloqueados (sin protocolo de enlace de emparejamiento).
- `open`: permitir que cualquier persona envíe un DM (público). **Requiere** que la lista de permitidos del canal incluya `"*"` (aceptación explícita).
- `disabled`: ignorar los DM entrantes por completo.
...
Detalles + archivos en el disco: [Emparejamiento](/es/channels/pairing)
...
- Modo de DM seguro: `session.dmScope: "per-channel-peer"` (cada par de canal+remitente obtiene un contexto de DM aislado).
...
- **Lista de permitidos de grupo** (específica del canal): de qué grupos/canales/gremios el bot aceptará mensajes en absoluto.
  ...
  - Las comprobaciones de grupo se ejecutan en este orden: listas de permitidas `groupPolicy`/grupo primero, activación por mención/respuesta en segundo lugar.
...
- Si permite los intérpretes en la lista de permitidos (`python`, `node`, `ruby`, `perl`, `php`, `lua`, `osascript`), habilite `tools.exec.strictInlineEval` para que los formularios de evaluación en línea aún necesiten aprobación explícita.
- El análisis de aprobación de shell también rechaza las formas de expansión de parámetros POSIX (`$VAR`, `$?`, `$$`, `$1`, `$@`, `${…}`) dentro de **heredocs sin comillas**, por lo que un cuerpo de heredoc en la lista blanca no puede colar una expansión de shell más allá de la revisión de la lista blanca como texto plano. Ponga comillas al terminador del heredoc (por ejemplo, `<<'EOF'`) para optar por la semántica de cuerpo literal; se rechazan los heredocs sin comillas que habrían expandido variables.
...
- Los backends compatibles con OpenAI que actúan de intermediario para modelos autoalojados a veces conservan tokens especiales que aparecen en el texto del usuario, en lugar de enmascararlos. De lo contrario, un atacante que pueda escribir en contenido externo entrante (una página obtenida, un cuerpo de correo electrónico, una salida de herramienta de contenidos de archivo) podría inyectar un límite de rol sintético `assistant` o `system` y escapar de las barreras de protección del contenido envuelto.
...
- Las respuestas del modelo saliente ya tienen un desinfectante separado que elimina el andamiaje interno de tiempo de ejecución filtrado, como `<tool_call>`, `<function_calls>`, `<system-reminder>`, `<previous_response>` y otros similares, de las respuestas visibles para el usuario en el límite de entrega del canal final. El desinfectante de contenido externo es la contraparte entrante.

Esto no reemplaza el endurecimiento restante en esta página: `dmPolicy`, listas blancas, aprobaciones de ejecución, sandboxing y `contextVisibility` todavía realizan el trabajo principal. Cierra una omisión específica de la capa de tokenizador contra pilas autoalojadas que reenvían texto de usuario con tokens especiales intactos.
...
- Los niveles de modelos débiles aumentan este riesgo. Para la automatización impulsada por hooks, prefiera niveles de modelos modernos y fuertes y mantenga la política de herramientas estricta (`tools.profile: "messaging"` o más estricta), además de sandboxing cuando sea posible.
...
- Mantener `web_search` / `web_fetch` / `browser` desactivado para los agentes con herramientas habilitadas, a menos que sea necesario.
- Para las entradas de URL de OpenResponses (`input_file` / `input_image`), establezca `gateway.http.endpoints.responses.files.urlAllowlist` y `gateway.http.endpoints.responses.images.urlAllowlist` estrictas, y mantenga `maxUrlParts` bajo. Las listas de permitidos vacías se tratan como no definidas; use `files.allowUrl: false` / `images.allowUrl: false` si desea deshabilitar completamente la obtención de URL.
- Para las entradas de archivo de OpenResponses, el texto `input_file` decodificado aún se inyecta como **contenido externo no confiable**. No confíe en que el texto del archivo sea confiable solo porque el Gateway lo decodificó localmente. El bloque inyectado todavía lleva marcadores de límite `<<<EXTERNAL_UNTRUSTED_CONTENT ...>>>` explícitos más metadatos `Source: External`, aunque esta ruta omite el banner más largo `SECURITY NOTICE:`.
...
`/reasoning`, `/verbose` y `/trace` pueden exponer el razonamiento interno, la salida de herramientas o el diagnóstico de complementos que no estaban destinados a un canal público. En entornos grupales, trátelos como **solo para depuración** y manténgalos desactivados a menos que los necesite explícitamente.

Guía:

- Mantenga `/reasoning`, `/verbose` y `/trace` desactivados en las salas públicas.
...
- Anfitrión de Canvas: `/__openclaw__/canvas/` y `/__openclaw__/a2ui/` (HTML/JS arbitrario; tratar como contenido que no es de confianza)
...
- Los enlaces que no son de bucle de retorno (`"lan"`, `"tailnet"`, `"custom"`) amplían la superficie de ataque. Úselos solo con autenticación de puerta de enlace (token/contraseña compartido o un proxy de confianza configurado correctamente) y un firewall real.
...
varían según las imágenes de VPS (`ens3`, `enp*`, etc.) y las discordancias pueden saltar
...
Cuando el complemento `bonjour` incluido está habilitado, la Gateway transmite su presencia a través de mDNS (`_openclaw-gw._tcp` en el puerto 5353) para el descubrimiento de dispositivos locales. En modo completo, esto incluye registros TXT que pueden exponer detalles operativos:
...
Cuando Bonjour está habilitado en modo mínimo, la Gateway transmite lo suficiente para el descubrimiento de dispositivos (`role`, `gatewayPort`, `transport`) pero omite `cliPath` y `sshPort`. Las aplicaciones que necesitan información de la ruta de la CLI pueden obtenerla a través de la conexión WebSocket autenticada en su lugar.
...
<Note>
  `gateway.remote.token` y `gateway.remote.password` son fuentes de credenciales de cliente. Por sí solas, **no** protegen el acceso local al WS. Las rutas de llamada locales pueden usar `gateway.remote.*` como alternativa solo cuando `gateway.auth.*` no está establecido. Si `gateway.auth.token` o `gateway.auth.password` están configurados explícitamente a través de SecretRef y no se resuelven, la
  resolución falla cerrada (sin enmascaramiento de alternativa remota).
</Note>
Opcional: ancle el TLS remoto con `gateway.remote.tlsFingerprint` cuando use `wss://`. Se acepta `ws://` en texto plano para loopback, literales de IP privada, `.local` y URL de gateway Tailnet `*.ts.net`. Para otros nombres DNS privados de confianza, establezca `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` en el proceso del cliente como medida de emergencia. Esto es intencionalmente solo del entorno del
proceso, no una clave de configuración `openclaw.json`. El emparejamiento móvil y las rutas de gateway manuales o escaneadas de Android son más estrictas: se acepta texto claro para loopback, pero las LAN privadas, de enlace local, `.local` y los nombres de host sin punto deben usar TLS, a menos que opte explícitamente por la ruta de texto claro de red privada de confianza.
...
- La evidencia de encabezados reenviados en una solicitud de loopback descalifica la
  localidad de loopback. La autoaprobación de actualización de metadatos tiene un alcance limitado. Vea
...
- `gateway.auth.mode: "password"`: autenticación por contraseña (prefiera establecer a través de env: `OPENCLAW_GATEWAY_PASSWORD`).
- `gateway.auth.mode: "trusted-proxy"`: confíe en un proxy inverso con reconocimiento de identidad para autenticar usuarios y pasar la identidad a través de encabezados (consulte [Trusted Proxy Auth](/es/gateway/trusted-proxy-auth)).
...
Para esta ruta de verificación de identidad asincrónica, los intentos fallidos para el mismo `{scope, ip}`
...
autenticación HTTP configurado en el gateway.
...
- La semántica de ámbito por solicitud en HTTP solo se aplica cuando la solicitud proviene de un modo con identidad, como la autenticación de proxy de confianza, o desde un ingreso privado explícitamente sin autenticación.
...
- `/tools/invoke` sigue la misma regla de secreto compartido: la autenticación de portador con token/contraseña también se trata como acceso de operador completo allí, mientras que los modos con identidad todavía respetan los alcances declarados.
...
termina TLS o usa un proxy frente a la puerta de enlace, desactive
...
- Si termina TLS frente a la Gateway, establezca `gateway.trustedProxies` a las IPs de su proxy.
...
Si su Gateway está remoto pero el navegador se ejecuta en otra máquina, ejecute un **node host** en la máquina del navegador y deje que el Gateway actúe como proxy para las acciones del navegador (consulte [Browser tool](/es/tools/browser)). Trate el emparejamiento de nodos como un acceso de administrador.
...
- `openclaw.json`: la configuración puede incluir tokens (gateway, gateway remoto), configuración del proveedor y listas de permitidos (allowlists).
- `credentials/**`: credenciales del canal (ejemplo: credenciales de WhatsApp), listas de permitidos para emparejamiento (pairing allowlists), importaciones heredadas de OAuth.
- `agents/<agentId>/agent/auth-profiles.json`: claves de API, perfiles de token, tokens de OAuth y `keyRef`/`tokenRef` opcionales.
- `agents/<agentId>/agent/codex-home/**`: cuenta del servidor de aplicaciones (app-server) de Codex por agente, configuración, habilidades (skills), complementos (plugins), estado del subproceso nativo y diagnósticos.
...
- `agents/<agentId>/agent/auth.json`: archivo de compatibilidad heredada. Las entradas estáticas de `api_key` se eliminan (scrubbed) cuando se descubren.
...
- paquetes de complementos (plugins) incluidos: complementos instalados (más sus `node_modules/`).
- `sandboxes/**`: espacios de trabajo de la caja de arena (sandbox) de herramientas; pueden acumular copias de los archivos que lee/escribe dentro de la caja de arena.
...
- Cualquier clave que comience con `OPENCLAW_*` está bloqueada para los archivos `.env` de espacios de trabajo que no son de confianza.
- La configuración del endpoint del canal para Matrix, Mattermost, IRC y Synology Chat también está bloqueada en las anulaciones del espacio de trabajo `.env`, por lo que los espacios de trabajo clonados no pueden redirigir el tráfico del conector empaquetado a través de la configuración del endpoint local. Las claves de entorno del endpoint (como `MATRIX_HOMESERVER`, `MATTERMOST_URL`, `IRC_HOST`, `SYNOLOGY_CHAT_INCOMING_URL`) deben provenir del entorno de proceso de la puerta de enlace o `env.shellEnv`, no de un `.env` cargado por el espacio de trabajo.
- El bloqueo es de fallo cerrado: una nueva variable de control de tiempo de ejecución añadida en una versión futura no se puede heredar de un `.env` verificado o proporcionado por un atacante; la clave se ignora y la puerta de enlace mantiene su propio valor.
- Las variables de entorno de procesos/SO de confianza (el propio shell de la puerta de enlace, la unidad launchd/systemd, el paquete de la aplicación) siguen aplicándose; esto solo restringe la carga de archivos `.env`.

Por qué: los archivos `.env` del espacio de trabajo frecuentemente residen junto al código del agente, se confirman por accidente o son escritos por herramientas. Bloquear todo el prefijo `OPENCLAW_*` significa que agregar un nuevo indicador `OPENCLAW_*` más tarde nunca puede retroceder a una herencia silenciosa desde el estado del espacio de trabajo.
...
Detalles: [Registro de日志](/es/gateway/logging)
...
- `tools.fs.workspaceOnly: true` (opcional): restringe las rutas `read`/`write`/`edit`/`apply_patch` y las rutas de carga automática de imágenes de indicaciones nativas al directorio del espacio de trabajo (útil si permite rutas absolutas hoy y desea una sola barrera de protección).
...
Línea base integrada para turnos de agente impulsados por chat: los remitentes que no son propietarios no pueden utilizar las herramientas `cron` o `gateway`.
...
- **Ejecute el Gateway completo en Docker** (límite del contenedor): [Docker](/es/install/docker)
- **Sandbox de herramientas** (`agents.defaults.sandbox`, gateway host + herramientas aisladas en sandbox; Docker es el backend predeterminado): [Sandboxing](/es/gateway/sandboxing)
...
- Los `sandbox.docker.binds` adicionales se validan frente a rutas de origen normalizadas y canónicas. Los trucos de enlaces simbólicos principales y los alias de inicio canónicos aún fallan de forma segura si se resuelven en raíces bloqueadas como `/etc`, `/var/run` o directorios de credenciales bajo el directorio principal del sistema operativo.

<Warning>
  `tools.elevated` es la escotilla de escape de la línea base global que ejecuta exec fuera del sandbox. El host efectivo es `gateway` de forma predeterminada, o `node` cuando el destino exec está configurado en `node`. Mantenga `tools.elevated.allowFrom` restringido y no lo habilite para desconocidos. Puede restringir aún más el modo elevado por agente mediante `agents.list[].tools.elevated`.
...
- Mantenga `agents.defaults.subagents.allowAgents` y cualquier anulación `agents.list[].subagents.allowAgents` por agente restringida a agentes de destino conocidos como seguros.
...
- Modo de participación: configure `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true` para permitir destinos privados/internos/de uso especial.
...
3. **Congelar acceso:** cambie los MDs/grupos de riesgo a `dmPolicy: "disabled"` / exija menciones y elimine las entradas de permitir todo `"*"` si las tenía.
...
3. Rote las credenciales del proveedor/API (credenciales de WhatsApp, tokens de Slack/Discord, claves de modelo/API en `auth-profiles.json` y valores de carga útil de secretos cifrados cuando se usan).
...
4. Vuelva a ejecutar `openclaw security audit --deep` y confirme que los hallazgos críticos se han resuelto.
...
CI ejecuta el enlace `detect-private-key` de confirmación previa sobre el repositorio. Si falla,
elimine o rote el material de clave confirmado, luego reproduzca localmente:
