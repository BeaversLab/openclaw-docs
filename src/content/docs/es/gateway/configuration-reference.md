...
- Las entradas de `bindings[]` de nivel superior con `type: "acp"` configuran enlaces ACP persistentes para temas del foro (use el `chatId:topic:topicId` canónico en `match.peer.id`). La semántica de los campos se comparte en [Agentes ACP](/en/tools/acp-agents#channel-specific-settings).
...
- Las entradas `bindings[]` de nivel superior con `type: "acp"` pueden vincular conversaciones de BlueBubbles a sesiones ACP persistentes. Use un identificador o una cadena de destino de BlueBubbles (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica de campo compartida: [ACP Agents](/en/tools/acp-agents#channel-specific-settings).
...
Consulta el índice completo de canales: [Canales](/en/channels).
...
Los modelos Z.AI habilitan `tool_stream` por defecto para la transmisión de llamadas a herramientas. Establezca `agents.defaults.models["zai/<model>"].params.tool_stream` en `false` para desactivarlo.
...
Sandboxing opcional para el agente incrustado. Consulte [Sandboxing](/en/gateway/sandboxing) para la guía completa.
...
En el modo `remote`, las ediciones locales del host realizadas fuera de OpenClaw no se sincronizan automáticamente en el entorno limitado después del paso de siembra.
El transporte es SSH hacia el entorno limitado de OpenShell, pero el complemento posee el ciclo de vida del entorno limitado y la sincronización opcional del espejo.
...
**Los contenedores usan por defecto `network: "none"`** — configure en `"bridge"` (o una red puente personalizada) si el agente necesita acceso saliente.
...
Ejecuta múltiples agentes aislados dentro de una sola Gateway. Consulta [Multi-Agent](/en/concepts/multi-agent).
...
La configuración se puede definir globalmente en `tools.loopDetection` y anular por agente en `agents.list[].tools.loopDetection`.
...
El catálogo de modelos ahora tiene como predeterminado solo M2.7.

</Accordion>

<Accordion title="Modelos locales (LM Studio)">

Consulte [Local Models](/en/gateway/local-models). Resumen: ejecute un modelo local grande a través de la API de Responses de LM Studio en hardware potente; mantenga los modelos alojados fusionados para la conmutación por error.
...
- `gateway.auth.mode: "trusted-proxy"`: delegar la autenticación a un proxy inverso con reconocimiento de identidad y confiar en los encabezados de identidad de `gateway.trustedProxies` (ver [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth)).
...
- Endurecimiento de la entrada de URL de Responses:
  ...
  - `gateway.http.endpoints.responses.images.urlAllowlist`
    Las listas de permitidos (allowlists) vacías se tratan como no definidas; use `gateway.http.endpoints.responses.files.allowUrl=false`
...
Consulte [Cron Jobs](/en/automation/cron-jobs). Las ejecuciones cron aisladas se rastrean como [background tasks](/en/automation/tasks).
