...
- `openclaw config schema` imprime el esquema JSON en vivo que se utiliza para la validación y la interfaz de usuario de control, con los metadatos del complemento/canal incluidos combinados cuando están disponibles
...
- [Comandos de barra diagonal](/es/tools/slash-commands) para el catálogo de comandos integrados actuales + incluidos
...
Si falta un bloque de proveedor por completo (`channels.<provider>` ausente), la política de grupos en tiempo de vuelve a `allowlist` (fail-closed) con una advertencia de inicio.
</Note>

### Sobrescrituras del modelo de canal

Use `channels.modelByChannel` para fijar IDs de canales específicos a un modelo. Los valores aceptan `provider/model` o alias de modelo configurados. La asignación de canales se aplica cuando una sesión aún no tiene una anulación de modelo (por ejemplo, establecida a través de `/model`).
...
- `channels.defaults.contextVisibility`: modo de visibilidad de contexto complementario predeterminado para todos los canales. Valores: `all` (predeterminado, incluye todo el contexto citado/hilo/historial), `allowlist` (solo incluye contexto de remitentes en lista permitida), `allowlist_quote` (igual que la lista permitida pero mantiene el contexto explícito de cita/respuesta). Anulación por canal: `channels.<channel>.contextVisibility`.
...
- El directorio de autenticación heredado de cuenta única Baileys es migrado por `openclaw doctor` a `whatsapp/default`.
...
- En configuraciones de varias cuentas (2+ IDs de cuenta), establezca un predeterminado explícito (`channels.telegram.defaultAccount` o `channels.telegram.accounts.default`) para evitar el enrutamiento de respaldo; `openclaw doctor` advierte cuando esto falta o no es válido.
...
- Las llamadas salientes directas que proporcionan un `token` de Discord explícito usan ese token para la llamada; la configuración de reintento/política de la cuenta proviene de la cuenta seleccionada en la instantánea de tiempo de ejecución activa.
...
- Use `user:<id>` (MD) o `channel:<id>` (canal de gremio) como destinos de entrega; se rechazan los IDs numéricos simples.
...
- `channels.discord.threadBindings` controla el enrutamiento vinculado a hilos de Discord:
  ...
  - `idleHours`: Anulación de Discord para el desenfoque automático por inactividad en horas (`0` deshabilita)
...
- `channels.discord.autoPresence` asigna la disponibilidad en tiempo de ejecución a la presencia del bot (saludable => en línea, degradado => ausente, agotado => no molestar) y permite anulaciones opcionales del texto de estado.
...
- `channels.discord.execApprovals`: Entrega de aprobación de ejecución nativa de Discord y autorización de aprobadores.
  ...
  - `target`: dónde enviar las solicitudes de aprobación. `"dm"` (predeterminado) envía a los MD de los aprobadores, `"channel"` envía al canal de origen, `"both"` envía a ambos. Cuando el objetivo incluye `"channel"`, los botones solo pueden ser utilizados por los aprobadores resueltos.
  - `cleanupAfterResolve`: cuando `true`, elimina los MD de aprobación después de la aprobación, denegación o tiempo de espera.
...
- Use `user:<id>` (MD) o `channel:<id>` para objetivos de entrega.
...
- La transmisión nativa de Slack junto con el estado del hilo "está escribiendo..." estilo asistente de Slack requieren un objetivo de hilo de respuesta. Los MD de nivel superior se mantienen fuera del hilo de forma predeterminada, por lo que usan `typingReaction` o entrega normal en lugar de la vista previa estilo hilo.
- `typingReaction` añade una reacción temporal al mensaje entrante de Slack mientras se ejecuta una respuesta, y luego la elimina al finalizar. Use un código corto de emoji de Slack como `"hourglass_flowing_sand"`.
...
- Las devoluciones de llamada de barra nativas se autentican con los tokens por comando devueltos por Mattermost durante el registro del comando de barra. Si el registro falla o no hay comandos activados, OpenClaw rechaza las devoluciones de llamada con `Unauthorized: invalid command token.`
...
BlueBubbles es la ruta de iMessage recomendada (con soporte de complementos, configurado bajo `channels.bluebubbles`).
...
- Las entradas de nivel superior `bindings[]` con `type: "acp"` pueden vincular conversaciones de BlueBubbles a sesiones ACP persistentes. Use un identificador o cadena de destino de BlueBubbles (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica de campo compartida: [Agentes ACP](/es/tools/acp-agents#channel-specific-settings).
...
- Prefiera objetivos `chat_id:<id>`. Use `imsg chats --limit 20` para enumerar chats.
- `cliPath` puede apuntar a un contenedor SSH; configure `remoteHost` (`host` o `user@host`) para la recuperación de adjuntos SCP.
...
- `channels.matrix.network.dangerouslyAllowPrivateNetwork` permite servidores domésticos privados/internos. `proxy` y esta aceptación opcional de red son controles independientes.
- `channels.matrix.defaultAccount` selecciona la cuenta preferida en configuraciones de múltiples cuentas.
- `channels.matrix.autoJoin` tiene como valor predeterminado `off`, por lo que las salas invitadas y las invitaciones nuevas de estilo DM se ignoran hasta que establezca `autoJoin: "allowlist"` con `autoJoinAllowlist` o `autoJoin: "always"`.
- `channels.matrix.execApprovals`: entrega de aprobación de ejecución nativa de Matrix y autorización de aprobadores.
  ...
  - `agentFilter`: lista de permitidos (allowlist) opcional de ID de agente. Omita para reenviar aprobaciones para todos los agentes.
  - `sessionFilter`: patrones de clave de sesión opcionales (subcadena o regex).
...
Microsoft Teams está respaldado por un complemento y se configura bajo `channels.msteams`.
...
IRC está respaldado por un complemento y se configura bajo `channels.irc`.
...
- `channels.irc.defaultAccount` opcional anula la selección de cuenta predeterminada cuando coincide con una ID de cuenta configurada.
...
Ejecutar varias cuentas por canal (cada una con su propia `accountId`):
...
- Si agrega una cuenta no predeterminada a través de `openclaw channels add` (o incorporación de canal) mientras todavía está en una configuración de canal de nivel superior de cuenta única, OpenClaw promueve primero los valores de cuenta única de nivel superior con ámbito de cuenta en el mapa de cuentas del canal para que la cuenta original siga funcionando. La mayoría de los canales las mueven a `channels.<channel>.accounts.default`; Matrix puede preservar un objetivo con nombre/predeterminado coincidente existente en su lugar.
...
- `openclaw doctor --fix` también repara formas mixtas moviendo los valores de nivel superior de cuenta única con ámbito de cuenta a la cuenta promovida elegida para ese canal. La mayoría de los canales usan `accounts.default`; Matrix puede preservar en su lugar un objetivo predeterminado/nombrado existente que coincida.
...
- **Patrones de texto**: Patrones de expresiones regulares seguros en `agents.list[].groupChat.mentionPatterns`. Los patrones no válidos y la repetición anidada insegura se ignoran.
...
Incluya su propio número en `allowFrom` para habilitar el modo de autoservicio (ignora las @menciones nativas, solo responde a patrones de texto):
...
- Este bloque configura las superficies de comandos. Para el catálogo de comandos integrados + empaquetados actual, consulte [Slash Commands](/es/tools/slash-commands).
- Esta página es una **referencia de clave de configuración**, no el catálogo completo de comandos. Los comandos propiedad del canal/complemento, como QQ Bot `/bot-ping` `/bot-help` `/bot-logs`, LINE `/card`, device-pair `/pair`, memory `/dreaming`, phone-control `/phone` y Talk `/voice` están documentados en sus páginas de canal/complemento además de [Slash Commands](/es/tools/slash-commands).
...
- `config: true` habilita `/config` (lee/escribe `openclaw.json`). Para clientes del gateway `chat.send`, las escrituras persistentes `/config set|unset` también requieren `operator.admin`; solo lectura `/config show` permanece disponible para los clientes de operador con alcance de escritura normales.
...
- `channels.<provider>.configWrites` bloquea las mutaciones de configuración por canal (predeterminado: true).
- Para canales multicuenta, `channels.<provider>.accounts.<id>.configWrites` también bloquea las escrituras que apuntan a esa cuenta (por ejemplo `/allowlist --config --account <id>` o `/config set channels.<provider>.accounts.<id>...`).
- `restart: false` deshabilita `/restart` y las acciones de herramientas de reinicio del gateway. Predeterminado: `true`.
- `ownerAllowFrom` es la lista de permitidos explícita del propietario para comandos/herramientas solo para el propietario. Es separada de `allowFrom`.
...
- `allowFrom` es por proveedor. Cuando se establece, es la **única** fuente de autorización (las listas de permitidas del canal/emparejamiento y `useAccessGroups` se ignoran).
...
- Mapa de documentación de comandos:
  - catálogo integrado + empaquetado: [Slash Commands](/es/tools/slash-commands)
  ...
  - recuerdo de memoria: [Dreaming](/es/concepts/dreaming)
...
- Omita `agents.defaults.skills` para habilidades sin restricciones de manera predeterminada.
...
Controla cuándo se inyectan los archivos de arranque del espacio de trabajo en el indicador del sistema (system prompt). Predeterminado: `"always"`.

- `"continuation-skip"`: los turnos de continuación segura (después de una respuesta completada del asistente) omiten la reinyección del arranque del espacio de trabajo, reduciendo el tamaño del indicador. Las ejecuciones de latido (heartbeat) y los reintentos posteriores a la compactación todavía reconstruyen el contexto.
...
Máximo de caracteres por archivo de arranque del espacio de trabajo antes de la truncación. Predeterminado: `12000`.
...
- `"off"`: nunca inyectar texto de advertencia en el indicador del sistema.
- `"once"`: inyectar advertencia una vez por firma de truncación única (recomendado).
- `"always"`: inyectar advertencia en cada ejecución cuando existe truncación.
...
- `agents.defaults.startupContext.*`:
  one-shot `/new` y `/reset` preludio de inicio, incluyendo reciente diario
  archivos `memory/*.md`.
- `skills.limits.*`:
  la lista compacta de habilidades inyectada en el prompt del sistema.
- `agents.defaults.contextLimits.*`:
  fragmentos del tiempo de ejecución delimitados y bloques inyectados propiedad del tiempo de ejecución.
- `memory.qmd.limits.*`:
...
- `memoryGetMaxChars`: límite de fragmento `memory_get` predeterminado antes de que se agreguen
  metadatos de truncamiento y avisos de continuación.
- `memoryGetDefaultLines`: ventana de línea `memory_get` predeterminada cuando `lines`
  se omite.
...
- `postCompactionMaxChars`: límite de fragmento de AGENTS.md utilizado durante la inyección de actualización
  posterior a la compactación.

#### `agents.list[].contextLimits`

Anulación por agente para los controles `contextLimits` compartidos. Los campos omitidos heredan
...
- `imageModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  - Lo utiliza la ruta de herramientas `image` como su configuración de modelo de visión.
...
- `imageGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Valores típicos: `google/gemini-3.1-flash-image-preview` para la generación de imágenes nativa de Gemini, `fal/fal-ai/flux/dev` para fal, o `openai/gpt-image-2` para OpenAI Images.
  - Si selecciona un proveedor/modelo directamente, configure también la clave API/auth del proveedor correspondiente (por ejemplo, `GEMINI_API_KEY` o `GOOGLE_API_KEY` para `google/*`, `OPENAI_API_KEY` para `openai/*`, `FAL_KEY` para `fal/*`).
...
- `pdfModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Si se omite, la herramienta de PDF recurre a `imageModel` y luego al modelo de sesión/predeterminado resuelto.
...
- `model.primary`: formato `provider/model` (p. ej. `openai/gpt-5.4`). Si omite el proveedor, OpenClaw intenta primero un alias, luego una coincidencia única de proveedor configurado para ese ID de modelo exacto, y solo después recurre al proveedor predeterminado configurado (comportamiento de compatibilidad obsoleto, por lo que se prefiere un `provider/model` explícito). Si ese proveedor ya no expone el modelo predeterminado configurado, OpenClaw recurre al primer proveedor/modelo configurado en lugar de mostrar un predeterminado obsoleto de proveedor eliminado.
- `models`: el catálogo de modelos configurado y la lista de permitidos para `/model`. Cada entrada puede incluir `alias` (acceso directo) y `params` (específico del proveedor, por ejemplo `temperature`, `maxTokens`, `cacheRetention`, `context1m`).
  - Ediciones seguras: use `openclaw config set agents.defaults.models '<json>' --strict-json --merge` para agregar entradas. `config set` rechaza los reemplazos que eliminarían las entradas existentes de la lista de permitidos, a menos que pase `--replace`.
...
- Precedencia de fusión de `params` (config): `agents.defaults.params` (base global) se anula por `agents.defaults.models["provider/model"].params` (por modelo), luego `agents.list[].params` (ID de agente coincidente) anula por clave. Consulte [Prompt Caching](/es/reference/prompt-caching) para obtener detalles.
- `embeddedHarness`: política de tiempo de ejecución predeterminada de bajo nivel del agente integrado. Use `runtime: "auto"` para permitir que los arneses de complementos registrados reclamen los modelos compatibles, `runtime: "pi"` para forzar el arnés PI integrado, o un ID de arnés registrado como `runtime: "codex"`. Establezca `fallback: "none"` para deshabilitar la reserva automática de PI.
- Los escritores de configuración que mutan estos campos (por ejemplo `/models set`, `/models set-image` y los comandos de agregar/eliminar de respaldo) guardan la forma canónica del objeto y preservan las listas de respaldo existentes cuando es posible.
- `maxConcurrent`: máx. ejecuciones paralelas del agente a través de sesiones (cada sesión todavía serializada). Predeterminado: 4.
...
Úselo cuando un complemento de confianza proporcione un arnés nativo, como el arnés
Codex app-server incluido.
...
- `fallback`: `"pi"` o `"none"`. `"pi"` mantiene el arnés PI integrado como la reserva de compatibilidad cuando no se selecciona ningún arnés de complemento. `"none"` hace que la selección de arnés de complemento faltante o no compatible falle en lugar de usar PI silenciosamente. Los fallos del arnés de complemento seleccionado siempre se muestran directamente.
- Opciones del entorno: `OPENCLAW_AGENT_RUNTIME=<id|auto|pi>` anula `runtime`; `OPENCLAW_AGENT_HARNESS_FALLBACK=none` desactiva la reserva de PI para ese proceso.
- Para despliegues solo de Codex, configure `model: "codex/gpt-5.4"`, `embeddedHarness.runtime: "codex"` y `embeddedHarness.fallback: "none"`.
- Esto solo controla el arnés de chat integrado. La generación de medios, visión, PDF, música, video y TTS todavía usan sus configuraciones de proveedor/modelo.

**Atajos de alias integrados** (solo aplican cuando el modelo está en `agents.defaults.models`):
...
- Traspaso de imágenes compatible cuando `imageArg` acepta rutas de archivo.

### `agents.defaults.systemPromptOverride`

Reemplace todo el prompt del sistema ensamblado por OpenClaw con una cadena fija. Establézcalo en el nivel predeterminado (`agents.defaults.systemPromptOverride`) o por agente (`agents.list[].systemPromptOverride`). Los valores por agente tienen prioridad; se ignora un valor vacío o que solo contenga espacios en blanco. Útil para experimentos controlados de prompts.
...
- `"off"` desactiva solo la capa amigable; el contrato de comportamiento etiquetado GPT-5 permanece activado.
...
- `includeSystemPromptSection`: cuando es false, omite la sección Heartbeat del mensaje del sistema y omite la inyección de `HEARTBEAT.md` en el contexto de arranque. Predeterminado: `true`.
...
- `provider`: id de un plugin de proveedor de compactación registrado. Cuando se establece, se invoca el `summarize()` del proveedor en lugar del resumen LLM integrado. Vuelve al integrado en caso de fallo. Establecer un proveedor fuerza `mode: "safeguard"`. Consulte [Compaction](/es/concepts/compaction).
...
- `notifyUser`: cuando es `true`, envía breves avisos al usuario cuando comienza la compactación y cuando se completa (por ejemplo, "Compactando contexto..." y "Compactación completada"). Desactivado de forma predeterminada para mantener la compactación en silencio.
...
Poda **resultados de herramientas antiguos** del contexto en memoria antes de enviarlos al LLM. **No** modifica el historial de sesiones en disco.
...
<Accordion title="comportamiento del modo de caché-ttl">
...
- La poda realiza un recorte suave primero en los resultados de herramientas sobredimensionados y luego un borrado duro de los resultados de herramientas más antiguos si es necesario.
...
<Accordion title="Detalles del entorno de pruebas">
...
- Los valores `*Data` respaldados por SecretRef se resuelven desde la instantánea activa del tiempo de ejecución de secretos antes de que inicie la sesión del entorno de pruebas
...
- no admite contenedores del navegador del entorno de pruebas

**Acceso al espacio de trabajo:**

- `none`: espacio de trabajo del entorno de pruebas por ámbito bajo `~/.openclaw/sandboxes`
- `ro`: espacio de trabajo del entorno de pruebas en `/workspace`, espacio de trabajo del agente montado como solo lectura en `/agent`
- `rw`: espacio de trabajo del agente montado como lectura/escritura en `/workspace`

**Ámbito:**

- `session`: contenedor por sesión + espacio de trabajo
...
- `remote`: sembrar el remoto una vez cuando se crea el entorno de pruebas, luego mantener el espacio de trabajo remoto canónico

En el modo `remote`, las ediciones locales al host realizadas fuera de OpenClaw no se sincronizan en el entorno de pruebas automáticamente después del paso de siembra.
El transporte es SSH hacia el entorno de pruebas de OpenShell, pero el complemento es propietario del ciclo de vida del entorno de pruebas y la sincronización opcional del espejo.
...
**Los contenedores tienen como valor predeterminado `network: "none"`** — configúrelo en `"bridge"` (o una red puente personalizada) si el agente necesita acceso saliente.
...
**Los archivos adjuntos entrantes** se ponen en staged en `media/inbound/*` en el espacio de trabajo activo.

**`docker.binds`** monta directorios de host adicionales; los enlaces globales y por agente se combinan.

**Navegador en entorno de pruebas** (`sandbox.browser.enabled`): Chromium + CDP en un contenedor. URL noVNC inyectada en el mensaje del sistema. No requiere `browser.enabled` en `openclaw.json`.
...
- `allowHostControl: false` (predeterminado) bloquea las sesiones en entorno de pruebas para que no apunten al navegador del host.
...
- `sandbox.browser.binds` monta directorios de host adicionales solo en el contenedor del navegador del entorno de pruebas. Cuando se configura (incluyendo `[]`), reemplaza `docker.binds` para el contenedor del navegador.
...
- `model`: el formato de cadena solo anula `primary`; el formato de objeto `{ primary, fallbacks }` anula ambos (`[]` desactiva los respaldos globales). Los trabajos de Cron que solo anulan `primary` aún heredan los respaldos predeterminados a menos que establezcas `fallbacks: []`.
- `params`: parámetros de flujo por agente combinados sobre la entrada del modelo seleccionado en `agents.defaults.models`. Úsalo para anulaciones específicas del agente como `cacheRetention`, `temperature` o `maxTokens` sin duplicar el catálogo completo de modelos.
- `skills`: lista de permitidos de habilidades opcional por agente. Si se omite, el agente hereda `agents.defaults.skills` cuando está establecido; una lista explícita reemplaza los valores predeterminados en lugar de combinarlos, y `[]` significa sin habilidades.
- `thinkingDefault`: nivel de pensamiento predeterminado opcional por agente (`off | minimal | low | medium | high | xhigh | adaptive | max`). Anula `agents.defaults.thinkingDefault` para este agente cuando no se establece ninguna anulación por mensaje o por sesión.
...
- `embeddedHarness`: anulación de política de arnés de bajo nivel opcional por agente. Usa `{ runtime: "codex", fallback: "none" }` para hacer que un agente sea solo Codex mientras otros agentes mantienen el respaldo PI predeterminado.
- `runtime`: descriptor de tiempo de ejecución opcional por agente. Use `type: "acp"` con `runtime.acp` predeterminados (`agent`, `backend`, `mode`, `cwd`) cuando el agente deba usar por defecto sesiones de harness ACP.
...
- `subagents.allowAgents`: lista de permitidos (allowlist) de ids de agente para `sessions_spawn` (`["*"]` = cualquiera; predeterminado: solo el mismo agente).
...
- `subagents.requireAgentId`: cuando es true, bloquea llamadas a `sessions_spawn` que omitan `agentId` (fuerza la selección explícita de perfil; predeterminado: false).

---

## Enrutamiento multiagente

Ejecute múltiples agentes aislados dentro de una sola Gateway. Vea [Multi-Agent](/es/concepts/multi-agent).
...
- `type` (opcional): `route` para enrutamiento normal (el tipo faltante predetermina a route), `acp` para vinculaciones de conversación ACP persistentes.
...
5. `match.accountId: "*"` (toda la canal)
...
- **`parentForkMaxTokens`**: `totalTokens` máxima de la sesión padre permitida al crear una sesión de hilo bifurcada (predeterminado `100000`).
  - Si la `totalTokens` padre supera este valor, OpenClaw inicia una sesión de hilo nueva en lugar de heredar el historial de transcripciones del padre.
...
- **`agentToAgent.maxPingPongTurns`**: máximo de turnos de respuesta entre agentes durante los intercambios de agente a agente (entero, rango: `0`–`5`). `0` desactiva la encadenación ping-pong.
...
- `removeAckAfterReply`: elimina el acuse de recibo después de la respuesta en Slack, Discord y Telegram.
- `messages.statusReactions.enabled`: activa las reacciones de estado del ciclo de vida en Slack, Discord y Telegram.
  En Slack y Discord, sin configurar mantiene las reacciones de estado activadas cuando las reacciones de acuse de recibo están activas.
  En Telegram, establézcalo explícitamente en `true` para activar las reacciones de estado del ciclo de vida.
...
- `providers.*.voiceAliases` permite que las directivas de Hablar usen nombres amigables.
...
La incorporación local predetermina las nuevas configuraciones locales a `tools.profile: "coding"` cuando no está configurado (los perfiles explícitos existentes se conservan).
...
Controla el acceso de ejecución elevado fuera del entorno seguro (sandbox):
...
- El `exec` elevado omite el aislamiento y utiliza la ruta de escape configurada (`gateway` de forma predeterminada, o `node` cuando el objetivo de ejecución es `node`).
...
- `detectors.knownPollNoProgress`: advertir/bloquear herramientas de encuesta conocidas (`process.poll`, `command_status`, etc.).
...
- `asyncCompletion.directSend`: cuando es `true`, las tareas de `music_generate`
  y `video_generate` asíncronas completadas intentan primero la entrega directa a través del canal. Predeterminado: `false`
...
- Fijación de zona de pruebas: cuando la sesión actual está en zona de pruebas y `agents.defaults.sandbox.sessionToolsVisibility="spawned"`, la visibilidad se fuerza a `tree` incluso si `tools.sessions.visibility="all"`.
...
- Las entradas Base64 se validan con comprobaciones estrictas de alfabeto/relleno y un guardia de tamaño pre-decodificación.
...
Marcadores de herramientas integradas experimentales. Desactivado por defecto a menos que se aplique una regla de activación automática estricta de GPT-5.
...
- Predeterminado: `false` a menos que `agents.defaults.embeddedPi.executionContract` (o una invalidación por agente) esté establecido en `"strict-agentic"` para una ejecución de la familia GPT-5 de OpenAI u OpenAI Codex. Establezca `true` para forzar la herramienta fuera de ese ámbito, o `false` para mantenerla desactivada incluso para ejecuciones estrictamente agénticas de GPT-5.
- Cuando está activado, el prompt del sistema también añade directrices de uso para que el modelo solo lo utilice para trabajos sustanciales y mantenga como máximo un paso `in_progress`.
...
- `model`: modelo predeterminado para sub-agentes generados. Si se omite, los sub-agentes heredan el modelo del que realiza la llamada.
...
- Política de herramienta por sub-agente: `tools.subagents.tools.allow` / `tools.subagents.tools.deny`.
...
- Invalide la raíz de configuración del agente con `OPENCLAW_AGENT_DIR` (o `PI_CODING_AGENT_DIR`, un alias de variable de entorno heredado).
- Precedencia de fusión para IDs de proveedor coincidentes:
  - Los valores no vacíos `models.json` `baseUrl` del agente ganan.
  - Los valores no vacíos `apiKey` del agente ganan solo cuando ese proveedor no está gestionado por SecretRef en el contexto de configuración/perfil de autenticación actual.
  - Los valores `apiKey` del proveedor gestionados por SecretRef se actualizan desde los marcadores de origen (`ENV_VAR_NAME` para referencias de entorno, `secretref-managed` para referencias de archivo/exec) en lugar de persistir secretos resueltos.
  ...
  - El agente `apiKey`/`baseUrl` vacío o ausente recurre a `models.providers` en la configuración.
  ...
  - El modelo coincidente `contextTokens` conserva un límite de ejecución explícito cuando está presente; úselo para limitar el contexto efectivo sin cambiar los metadatos nativos del modelo.
  - Use `models.mode: "replace"` cuando desee que la configuración reescriba completamente `models.json`.
  - La persistencia de los marcadores tiene autoridad de origen: los marcadores se escriben desde la instantánea de configuración de origen activa (pre-resolución), no desde los valores secretos de tiempo de ejecución resueltos.

### Detalles del campo de proveedor

- `models.mode`: comportamiento del catálogo de proveedores (`merge` o `replace`).
- `models.providers`: mapa de proveedor personalizado clave por ID de proveedor.
...
- `models.providers.*.request`: anulaciones de transporte para las solicitudes HTTP del proveedor de modelos.
  ...
  - `request.allowPrivateNetwork`: cuando es `true`, permite HTTPS hacia `baseUrl` cuando el DNS resuelve a rangos privados, CGNAT o similares, mediante el guardia de obtención HTTP del proveedor (opt-in del operador para puntos finales compatibles con OpenAI autohospedados de confianza). WebSocket usa el mismo `request` para encabezados/TLS pero no esa puerta de obtención SSRF. Predeterminado `false`.
...
- `models.providers.*.models.*.contextTokens`: límite opcional del contexto en tiempo de ejecución. Úselo cuando desee un presupuesto de contexto efectivo menor que el `contextWindow` nativo del modelo.
- `models.providers.*.models.*.compat.supportsDeveloperRole`: sugerencia opcional de compatibilidad. Para `api: "openai-completions"` con un `baseUrl` no nativo no vacío (host distinto de `api.openai.com`), OpenClaw lo fuerza a `false` en tiempo de ejecución. El `baseUrl` vacío u omitido mantiene el comportamiento predeterminado de OpenAI.
- `models.providers.*.models.*.compat.requiresStringContent`: sugerencia opcional de compatibilidad para endpoints de chat compatibles con OpenAI que solo aceptan cadenas. Cuando `true`, OpenClaw convierte los arrays de `messages[].content` de texto plano en cadenas simples antes de enviar la solicitud.
...
- `plugins.entries.amazon-bedrock.config.discovery.providerFilter`: filtro opcional de id. de proveedor para la detección dirigida.
...
- `plugins.entries.amazon-bedrock.config.discovery.defaultMaxTokens`: máximo de tokens de salida alternativo para los modelos detectados.
...
Configure `ZAI_API_KEY`. `z.ai/*` y `z-ai/*` son alias aceptados. Acceso directo: `openclaw onboard --auth-choice zai-api-key`.
...
Los endpoints nativos de Moonshot anuncian compatibilidad de uso de streaming en el transporte compartido `openai-completions`, y las claves de OpenClaw priorizan las capacidades del endpoint en lugar de solo el ID del proveedor integrado.
...
La URL base debe omitir `/v1` (el cliente de Anthropic lo agrega). Acceso directo: `openclaw onboard --auth-choice synthetic-api-key`.
...
Configure `MINIMAX_API_KEY`. Accesos directos:
`openclaw onboard --auth-choice minimax-global-api` o
`openclaw onboard --auth-choice minimax-cn-api`.
El catálogo de modelos predeterminado es solo M2.7.
En la ruta de streaming compatible con Anthropic, OpenClaw deshabilita el pensamiento de MiniMax
de forma predeterminada a menos que configure `thinking` explícitamente. `/fast on` o
...
Consulte [Modelos locales](/es/gateway/local-models). TL;DR: ejecute un modelo local grande a través de la API de respuestas de LM Studio en hardware potente; mantenga los modelos alojados combinados como respaldo.
...
- `load.extraDirs`: raíces de habilidades compartidas adicionales (menor prioridad).
...
- `allow`: lista de permitidos opcional (solo se cargan los complementos enumerados). `deny` tiene prioridad.
- `plugins.entries.<id>.apiKey`: campo de conveniencia de clave de API a nivel de complemento (cuando el complemento lo admite).
- `plugins.entries.<id>.env`: mapa de variables de entorno con alcance de complemento.
- `plugins.entries.<id>.hooks.allowPromptInjection`: cuando `false`, el núcleo bloquea `before_prompt_build` e ignora los campos de mutación de solicitud del `before_agent_start` heredado, mientras conserva los `modelOverride` y `providerOverride` heredados. Se aplica a los enlaces nativos de complementos y a los directorios de enlaces proporcionados por paquetes compatibles.
- `plugins.entries.<id>.subagent.allowModelOverride`: confiar explícitamente en este complemento para solicitar `provider` por ejecución y anulaciones de `model` para ejecuciones de subagentes en segundo plano.
...
- `plugins.entries.<id>.config`: objeto de configuración definido por el complemento (validado por el esquema nativo de complementos de OpenClaw cuando esté disponible).
- `plugins.entries.firecrawl.config.webFetch`: configuración del proveedor de recuperación web de Firecrawl.
  ...
  - `timeoutSeconds`: tiempo de espera de solicitud de raspado en segundos (predeterminado: `60`).
...
- `plugins.entries.memory-core.config.dreaming`: configuración de "dreaming" de memoria. Consulte [Dreaming](/es/concepts/dreaming) para ver las fases y umbrales.
...
- En modo estricto, los endpoints de perfil CDP remotos (`profiles.*.cdpUrl`) están sujetos al mismo bloqueo de red privada durante las comprobaciones de alcance/descubrimiento.
...
- `profiles.*.cdpUrl` acepta `http://`, `https://`, `ws://` y `wss://`.
  Use HTTP(S) cuando quieras que OpenClaw descubra `/json/version`; usa WS(S)
  cuando tu proveedor te dé una URL directa de WebSocket de DevTools.
- Los perfiles `existing-session` usan Chrome MCP en lugar de CDP y pueden conectarse al
  host seleccionado o a través de un nodo de navegador conectado.
...
- Los perfiles `existing-session` mantienen los límites actuales de la ruta Chrome MCP:
  acciones impulsadas por instantáneas/referencias en lugar de orientación por selectores CSS, ganchos de subida
...
- `seamColor`: color de acento para el cromo de la UI de la aplicación nativa (tinte de burbuja del modo Talk, etc.).
- `assistant`: anulación de identidad de la UI de control. Se retrocede a la identidad del agente activo.
...
- `mode`: `local` (ejecutar gateway) o `remote` (conectar a gateway remoto). El gateway se niega a iniciar a menos que `local`.
...
- `gateway.auth.mode: "none"`: modo sin autenticación explícito. Úselo solo para configuraciones de bucle local de confianza; esto intencionalmente no se ofrece en los mensajes de incorporación.
...
- `gateway.auth.allowTailscale`: cuando `true`, los encabezados de identidad de Tailscale Serve pueden satisfacer la autenticación de Control UI/WebSocket (verificado a través de `tailscale whois`). Los endpoints de la API HTTP **no** usan esa autenticación de encabezado de Tailscale; siguen el modo de autenticación HTTP normal del gateway. Este flujo sin token asume que el host del gateway es confiable. Predeterminado en `true` cuando `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: limitador opcional de autenticación fallida. Se aplica por IP de cliente y por ámbito de autenticación (secrets compartidos y tokens de dispositivo se rastrean de forma independiente). Los intentos bloqueados devuelven `429` + `Retry-After`.
  ...
  - `gateway.auth.rateLimit.exemptLoopback` predeterminado en `true`; configure `false` cuando desee intencionalmente que el tráfico de localhost también tenga límite de velocidad (para configuraciones de prueba o implementaciones de proxy estrictas).
...
- En el bucle local, esos bloqueos de origen del navegador se aíslan por valor normalizado de `Origin`, por lo que fallas repetidas desde un origen de localhost no bloquean automáticamente un origen diferente.
- `tailscale.mode`: `serve` (solo tailnet, enlace de bucle local) o `funnel` (público, requiere autenticación).
- `controlUi.allowedOrigins`: lista de permitidos explícita de origen del navegador para conexiones WebSocket del Gateway. Requerido cuando se esperan clientes de navegador desde orígenes que no son de bucle local.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: anulación de emergencia del lado del cliente que permite `ws://` en texto plano a IPs de red privada de confianza; el valor predeterminado sigue siendo solo de bucle local para texto plano.
...
- `gateway.push.apns.relay.timeoutMs`: tiempo de espera de envío de gateway a relé en milisegundos. Predeterminado: `10000`.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: escape hatch solo para desarrollo para URLs de relé HTTP de bucle local. Las URL de relé de producción deben mantenerse en HTTPS.
- `gateway.channelHealthCheckMinutes`: intervalo de monitoreo de salud del canal en minutos. Configure `0` para desactivar los reinicios del monitoreo de salud globalmente. Predeterminado: `5`.
...
- `gateway.channelMaxRestartsPerHour`: máximo de reinicios de monitoreo de salud por canal/cuenta en una hora móvil. Predeterminado: `10`.
- `channels.<provider>.healthMonitor.enabled`: exclusión voluntaria por canal para reinicios de monitoreo de salud manteniendo el monitor global habilitado.
...
- `trustedProxies`: IPs de proxy inverso que terminan TLS o inyectan encabezados de cliente reenviado. Liste solo los proxies que controla. Las entradas de bucle local siguen siendo válidas para configuraciones de proxy/detección local en el mismo host (por ejemplo, Tailscale Serve o un proxy inverso local), pero **no** hacen que las solicitudes de bucle local sean elegibles para `gateway.auth.mode: "trusted-proxy"`.
...
- `mode`: controla cómo se aplican las ediciones de configuración en tiempo de ejecución.
  ...
  - `"restart"`: reiniciar siempre el proceso del gateway ante un cambio de configuración.
  - `"hot"`: aplicar cambios dentro del proceso sin reiniciar.
...
- `deferralTimeoutMs`: tiempo máximo en ms a esperar para las operaciones en vuelo antes de forzar un reinicio (predeterminado: `300000` = 5 minutos).
...
- Si una asignación (mapping) o preajuste (preset) usa un `sessionKey` con plantilla, establezca `hooks.allowedSessionKeyPrefixes` y `hooks.allowRequestSessionKey=true`. Las claves de asignación estáticas no requieren esa aceptación explícita.
...
- `POST /hooks/<name>` → resuelto a través de `hooks.mappings`
  - Los valores de asignación `sessionKey` renderizados con plantilla se tratan como suministrados externamente y también requieren `hooks.allowRequestSessionKey=true`.
...
- Si necesita `hooks.allowRequestSessionKey: false`, anule el valor predefinido con un `sessionKey` estático en lugar del valor predeterminado con plantilla.
...
- Las URLs de capacidades están vinculadas a la sesión WS del nodo activo y caducan rápidamente. No se usa una alternativa basada en IP.
...
- Si `trustedDirs` está configurado, la verificación de trusted-dir se aplica a la ruta de destino resuelta.
...
- `billingBackoffHours`: retroceso base en horas cuando un perfil falla debido a verdaderos
  errores de facturación/crédito insuficiente (predeterminado: `5`). El texto de facturación explícito aún
  puede terminar aquí incluso en respuestas `401`/`403`, pero los comparadores de texto
  específicos del proveedor permanecen limitados al proveedor que los posee (por ejemplo, OpenRouter
  `Key limit exceeded`). Los reutilizables HTTP `402` de ventana de uso o
  mensajes de límite de gasto de organización/espacio de trabajo permanecen en su lugar en la ruta `rate_limit`.
...
- `authPermanentBackoffMinutes`: retroceso base en minutos para fallos de alta confianza `auth_permanent` (predeterminado: `10`).
...
- `overloadedProfileRotations`: máximo de rotaciones de perfil de autenticación del mismo proveedor para errores de sobrecarga antes de cambiar al alternativo del modelo (predeterminado: `1`). Las formas de ocupación del proveedor, como `ModelNotReadyException`, se incluyen aquí.
- `overloadedBackoffMs`: retraso fijo antes de reintentar una rotación de proveedor/perfil sobrecargado (predeterminado: `0`).
- `rateLimitedProfileRotations`: máximo de rotaciones de perfil de autenticación del mismo proveedor para errores de límite de velocidad antes de cambiar al alternativo del modelo (predeterminado: `1`). Ese cubo de límite de velocidad incluye texto con forma de proveedor como `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` y `resource exhausted`.
...
- `maxFileBytes`: tamaño máximo del archivo de registro en bytes antes de que se supriman las escrituras (entero positivo; predeterminado: `524288000` = 500 MB). Use rotación de registros externa para implementaciones de producción.
...
- `flags`: matriz de cadenas de marcas que habilitan la salida de registros dirigida (admite comodines como `"telegram.*"` o `"*"`).
...
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`: controla qué se incluye en la salida de la traza de caché (todos predeterminados: `true`).
...
- `auto.betaCheckIntervalHours`: frecuencia de las comprobaciones del canal beta en horas (predeterminado: `1`; máx: `24`).
...
- `enabled`: puerta de enlace (gate) global de la función ACP (predeterminado: `false`).
- `dispatch.enabled`: puerta independiente para el despacho de turnos de sesión ACP (predeterminado: `true`). Establezca `false` para mantener los comandos ACP disponibles mientras se bloquea la ejecución.
...
- `defaultAgent`: id del agente de destino ACP de respaldo cuando los generados no especifican un objetivo explícito.
...
- `stream.deliveryMode`: `"live"` transmite de forma incremental; `"final_only"` almacena en el búfer hasta los eventos terminales del turno.
...
- `runtime.ttlMinutes`: TTL de inactividad en minutos para los trabajadores de sesión ACP antes de ser elegibles para la limpieza.
...
Consulte los campos de identidad `agents.list` en [Valores predeterminados del agente](#agent-defaults).

---

## Puente (legado, eliminado)
...
Solo se aplica a trabajos cron one-shot. Los trabajos recurrentes usan un manejo de fallos separado.
...
- Cuando no se establece ningún destino de fallo global ni por trabajo, los trabajos que ya se entregan a través de `announce` vuelven a ese objetivo de anuncio principal en caso de fallo.
...
- Matriz de archivos: combinados en profundidad en orden (los posteriores sobrescriben a los anteriores).
...
- Rutas: resueltas en relación con el archivo que incluye, pero deben permanecer dentro del directorio de configuración de nivel superior (`dirname` de `openclaw.json`). Las formas absolutas/`../` solo se permiten cuando todavía se resuelven dentro de ese límite.
...
- Las inclusiones raíz, las matrices de inclusión y las inclusiones con anulaciones de hermanas son de solo lectura para las escrituras propiedad de OpenClaw; esas escrituras fallan cerradas en lugar de aplanar la configuración.
