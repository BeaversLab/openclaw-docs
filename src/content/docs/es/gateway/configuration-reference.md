...
Ruta de búsqueda del agente: use la acción de herramienta `gateway` `config.schema.lookup` para obtener documentación y restricciones exactas a nivel de campo antes de editar. Use [Configuration](/es/gateway/configuration) para obtener orientación orientada a tareas y esta página para el mapa de campos más amplio, valores predeterminados y enlaces a referencias de subsistemas.
...
Claves de configuración por canal movidas a una página dedicada; consulte [Configuration - channels](/es/gateway/config-channels) para `channels.*`, incluyendo Slack, Discord, Telegram, WhatsApp, Matrix, iMessage y otros canales incluidos (autenticación, control de acceso, multicuenta, filtrado de menciones).

## Valores predeterminados del agente, multiagente, sesiones y mensajes

Movido a una página dedicada; consulte [Configuration - agents](/es/gateway/config-agents) para:
...
Política de herramientas, interruptores experimentales, configuración de herramientas respaldadas por proveedores y configuración de proveedores personalizados / URL base se movieron a una página dedicada; consulte [Configuration - tools and custom providers](/es/gateway/config-tools).

## Modelos

Las definiciones de proveedores, listas de permisos de modelos y configuración de proveedores personalizados se encuentran en [Configuration - tools and custom providers](/es/gateway/config-tools#custom-providers-and-base-urls). La raíz `models` también posee el comportamiento global del catálogo de modelos.
...
- `models.providers.*.localService`: administrador de procesos opcionales bajo demanda para servidores de modelos locales. OpenClaw sondea el punto final de salud configurado, inicia el `command` absoluto cuando es necesario, espera la preparación y luego envía la solicitud del modelo. Consulte [Local model services](/es/gateway/local-model-services).
...
- Los cambios bajo `mcp.*` se aplican en caliente desechando los tiempos de ejecución MCP de sesión en caché.
  El siguiente descubrimiento/uso de herramientas los recrea con la nueva configuración, por lo que las entradas `mcp.servers` eliminadas
  se recolectan inmediatamente en lugar de esperar el TTL de inactividad.
...
- `load.allowSymlinkTargets`: raíces de destino reales de confianza a las que los enlaces simbólicos de habilidades pueden
...
- `install.nodeManager`: preferencia del instalador de nodos para las `metadata.openclaw.install`
  especificaciones (`npm` | `pnpm` | `yarn` | `bun`).
- `install.allowUploadedArchives`: permite a los clientes de confianza de `operator.admin` Gateway
  instalar archivos zip privados preparados a través de `skills.upload.*`
  (predeterminado: false). Esto solo habilita la ruta del archivo subido; las instalaciones
...
- El descubrimiento acepta plugins nativos de OpenClaw además de paquetes de Codex compatibles y paquetes de Claude, incluyendo paquetes de diseño predeterminado de Claude sin manifiesto.
...
- `bundledDiscovery`: por defecto es `"allowlist"` para nuevas configuraciones, por lo que un `plugins.allow` no vacío
  también controla los plugins de proveedor empaquetados, incluidos los proveedores
  de tiempo de ejecución de búsqueda web. Doctor escribe `"compat"` para configuraciones
  heredadas migradas de listas blancas para preservar el comportamiento del proveedor
  empaquetado existente hasta que usted lo acepte.
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: cuando está `false`, el núcleo bloquea `before_prompt_build` e ignora los campos que modifican el prompt de `before_agent_start` heredados,
  mientras preserva `modelOverride` y `providerOverride` heredados. Se aplica a los hooks de plugins nativos y a los directorios de hooks proporcionados por paquetes compatibles.
...
- `plugins.entries.<id>.llm.allowedModels`: lista de permitidos opcional de objetivos `provider/model` canónicos para anulaciones de finalización LLM de complementos de confianza. Use `"*"` solo cuando intencionalmente desee permitir cualquier modelo.
...
El complemento empaquetado `codex` es propietario de la configuración nativa del arnés del servidor de aplicaciones Codex bajo
`plugins.entries.codex.config`. Consulte la
[Referencia del arnés Codex](/es/plugins/codex-harness-reference) para conocer la superficie completa de configuración
y [Arnés Codex](/es/plugins/codex-harness) para conocer el modelo en tiempo de ejecución.

`codexPlugins` se aplica solo a las sesiones que seleccionan el arnés nativo de Codex.
No habilita los complementos de Codex para Pi, ejecuciones normales del proveedor de OpenAI, enlaces de conversación de ACP
...
- `plugins.entries.codex.config.codexPlugins.enabled`: habilita la compatibilidad nativa de complementos/aplicaciones de Codex
  para el arnés de Codex. Predeterminado: `false`.
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  política de acciones destructivas predeterminada para las elicitaciones de aplicaciones de complementos migrados.
  Predeterminado: `true`.
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.allow_destructive_actions`:
  anulación de acciones destructivas por complemento. Cuando se omite, se utiliza el valor
  `allow_destructive_actions` global.

`codexPlugins.enabled` es la directiva global de habilitación. Las entradas explícitas de complementos
escritas por la migración son el conjunto duradero de instalación y elegibilidad de reparación.
`plugins["*"]` no es compatible, no hay un interruptor `install` y los valores `marketplacePath` locales
...
- `plugins.entries.firecrawl.config.webFetch`: configuración del proveedor de obtención web de Firecrawl.
  ...
  - `baseUrl`: URL base de la API de Firecrawl (predeterminado: `https://api.firecrawl.dev`; las anulaciones autohospedadas deben apuntar a puntos de conexión privados/internos).
...
- `plugins.slots.contextEngine`: selecciona el id del plugin del motor de contexto activo; por defecto es `"legacy"` a menos que instales y selecciones otro motor.
...
`commitments` controla la memoria de seguimiento inferida: OpenClaw puede detectar los registros de los turnos de conversación y entregarlos a través de ejecuciones de latido.

- `commitments.enabled`: habilita la extracción, almacenamiento y entrega por latido oculta del LLM para compromisos de seguimiento inferidos. Predeterminado: `false`.
...
- `tabCleanup` reclama las pestañas rastreadas del agente principal después del tiempo de inactividad o cuando una
...
- `remoteCdpTimeoutMs` y `remoteCdpHandshakeTimeoutMs` se aplican a la accesibilidad CDP remota y
  `attachOnly` más las solicitudes de apertura de pestañas. Los perfiles de bucle de retorno
  administrados mantienen los valores predeterminados de CDP locales.
- Si un servicio CDP administrado externamente es accesible a través del bucle de retorno, establezca el `attachOnly: true` de ese
  perfil; de lo contrario, OpenClaw trata el puerto de bucle de retorno como un
  perfil de navegador administrado local y puede informar errores de propiedad del puerto local.
...
- Los perfiles administrados locales pueden establecer `executablePath` para anular el `browser.executablePath` global
  para ese perfil. Úselo para ejecutar un perfil en
...
- Los perfiles administrados locales usan `browser.localLaunchTimeoutMs` para el descubrimiento HTTP de CDP de Chrome
  ...
  la disponibilidad del websocket CDP posterior al lanzamiento. Auméntelos en hosts más lentos donde Chrome
  se inicie correctamente pero las comprobaciones de disponibilidad compitan con el inicio. Ambos valores deben ser
...
- `assistant`: anulación de identidad de la interfaz de usuario de control. Por defecto, la identidad del agente activo.
...
- `mode`: `local` (ejecutar gateway) o `remote` (conectarse a gateway remoto). Gateway se niega a iniciarse a menos que `local`.
...
- `gateway.auth.mode: "none"`: modo explícito sin autenticación. Úselo solo para configuraciones de loopback local confiables; intencionalmente no se ofrece en los indicadores de incorporación.
- `gateway.auth.mode: "trusted-proxy"`: delegue la autenticación del navegador/usuario a un proxy inverso con conocimiento de identidad y confíe en los encabezados de identidad de `gateway.trustedProxies` (consulte [Trusted Proxy Auth](/es/gateway/trusted-proxy-auth)). Este modo espera una fuente de proxy **que no sea de loopback** de forma predeterminada; los proxies inversos de loopback del mismo host requieren `gateway.auth.trustedProxy.allowLoopback = true` explícito. Los llamadores internos del mismo host pueden usar `gateway.auth.password` como respaldo local directo; `gateway.auth.token` sigue siendo mutuamente exclusivo con el modo de proxy confiable.
- `gateway.auth.allowTailscale`: cuando `true`, los encabezados de identidad de Tailscale Serve pueden satisfacer la autenticación de Control UI/WebSocket (verificado a través de `tailscale whois`). Los puntos finales de la API HTTP **no** usan esa autenticación de encabezado de Tailscale; en su lugar, siguen el modo de autenticación HTTP normal del gateway. Este flujo sin token asume que el host del gateway es confiable. El valor predeterminado es `true` cuando `tailscale.mode = "serve"`.
...
- `tailscale.preserveFunnel`: cuando `true` y `tailscale.mode = "serve"`, OpenClaw verifica `tailscale funnel status` antes de volver a aplicar Serve al inicio y lo omite si una ruta de Funnel configurada externamente ya cubre el puerto del gateway. Predeterminado `false`.
- `controlUi.allowedOrigins`: lista blanca explícita de origen del navegador para conexiones WebSocket de Gateway. Requerido cuando se esperan clientes de navegador de orígenes que no son de loopback.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: salida de emergencia solo para desarrollo para URLs de relé HTTP de loopback. Las URLs de relé de producción deben permanecer en HTTPS.
- `gateway.handshakeTimeoutMs`: tiempo de espera de protocolo de enlace de WebSocket de Gateway previo a la autenticación en milisegundos. Predeterminado: `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` tiene prioridad cuando se establece. Aumente esto en hosts con carga o de baja potencia donde los clientes locales pueden conectarse mientras el calentamiento del inicio aún se está estabilizando.
...
- `channels.<provider>.healthMonitor.enabled`: exclusión voluntaria por canal para los reinicios del monitor de salud mientras mantiene el monitor global habilitado.
...
- `gateway.nodes.pairing.autoApproveCidrs`: lista blanca de CIDR/IP opcional para aprobar automáticamente el emparejamiento de dispositivos de nodo por primera vez sin ámbitos solicitados. Está deshabilitado cuando no se establece. Esto no aprueba automáticamente el emparejamiento de operador/navegador/Control UI/WebChat, y no aprueba automáticamente actualizaciones de rol, ámbito, metadatos o clave pública.
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`: configuración global de permitir/denegar para comandos de nodo declarados después del emparejamiento y la evaluación de la lista blanca de la plataforma. Use `allowCommands` para optar por comandos de nodo peligrosos como `camera.snap`, `camera.clip` y `screen.record`; `denyCommands` elimina un comando incluso si una permitida predeterminada de la plataforma o explícita lo incluiría de otro modo. Después de que un nodo cambia su lista de comandos declarados, rechace y vuelva a aprobar ese emparejamiento de dispositivos para que el gateway almacene la instantánea de comandos actualizada.
...
- Chat Completions: deshabilitado de forma predeterminada. Habilite con `gateway.http.endpoints.chatCompletions.enabled: true`.
...
- `enabled`: habilita la terminación TLS en el escucha del gateway (HTTPS/WSS) (predeterminado: `false`).
...
- `deferralTimeoutMs`: tiempo máximo opcional en ms para esperar a que las operaciones en curso terminen antes de forzar un reinicio o una recarga en caliente del canal. Omítalo para usar la espera limitada predeterminada (`300000`); establece `0` para esperar indefinidamente y registrar advertencias periódicas de operaciones pendientes.
...
- Si una asignación (mapping) o preajuste (preset) utiliza un `sessionKey` con plantilla, establece `hooks.allowedSessionKeyPrefixes` y `hooks.allowRequestSessionKey=true`. Las claves de asignación estáticas no requieren esa activación.
...
- `transform` puede apuntar a un módulo JS/TS que devuelva una acción de hook.
  ...
  - Mantenga `hooks.transformsDir` bajo `~/.openclaw/hooks/transforms`; se rechazan los directorios de habilidades del área de trabajo. Si `openclaw doctor` indica que esta ruta no es válida, mueva el módulo de transformación al directorio de transformaciones de hooks o elimine `hooks.transformsDir`.
...
- `allowRequestSessionKey`: permite a los llamadores de `/hooks/agent` y a las claves de sesión de mapeo controladas por plantillas establecer `sessionKey` (predeterminado: `false`).
- `allowedSessionKeyPrefixes`: lista de permitidos de prefijo opcional para valores de `sessionKey` explícitos (solicitud + mapeo), ej. `["hook:"]`. Se vuelve obligatorio cuando cualquier mapeo o ajuste preestablecido usa una `sessionKey` con plantilla.
...
- Gateway inicia automáticamente `gog gmail watch serve` al arrancar cuando está configurado. Configure `OPENCLAW_SKIP_GMAIL_WATCHER=1` para desactivar.
...
- Enlaces que no son de bucle invertido: las rutas de canvas requieren autenticación de Gateway (token/contraseña/proxy confiable), al igual que otras superficies HTTP de Gateway.
- Los WebViews de Nodo generalmente no envían encabezados de autenticación; después de que un nodo se ha emparejado y conectado, Gateway anuncia URL de capacidad con ámbito de nodo para el acceso a canvas/A2UI.
- Las URL de capacidad están vinculadas a la sesión WS del nodo activo y caducan rápidamente. No se utiliza la reserva basada en IP.
...
- `full`: incluir `cliPath` + `sshPort`; la publicidad de multidifusión LAN aún requiere que el complemento `bonjour` incluido esté habilitado.
- `off`: suprimir la publicidad de multidifusión LAN sin cambiar la habilitación del complemento.
...
- Los destinos `secrets apply` son rutas de credencial `openclaw.json` compatibles.
...
- Las rutas de los proveedores de archivos y exec fallan cerradas cuando la verificación de ACL de Windows no está disponible. Establezca `allowInsecurePath: true` solo para rutas confiables que no se pueden verificar.
...
- Si `trustedDirs` está configurado, la verificación del directorio confiable se aplica a la ruta de destino resuelta.
...
- Comportamiento en tiempo de ejecución de secretos y herramientas `audit/configure/apply`: [Secrets Management](/es/gateway/secrets).
...
- `billingBackoffHours`: tiempo de espera base en horas cuando un perfil falla debido a errores
  reales de facturación/saldo insuficiente (predeterminado: `5`). El texto de facturación explícito aún
  puede llegar aquí incluso en respuestas `401`/`403`, pero los comparadores de texto
  específicos del proveedor se limitan al proveedor que los posee (por ejemplo, OpenRouter
  `Key limit exceeded`). Los mensajes reintentables de `402` HTTP de ventana de uso
...
- `overloadedProfileRotations`: máximo de rotaciones de perfiles de autenticación del mismo proveedor para errores de sobrecarga antes de cambiar a la alternativa del modelo (predeterminado: `1`). Las formas de ocupación del proveedor como `ModelNotReadyException` se manejan aquí.
- `overloadedBackoffMs`: retraso fijo antes de reintentar una rotación de proveedor/perfil sobrecargado (predeterminado: `0`).
- `rateLimitedProfileRotations`: rotaciones máximas del perfil de autenticación del mismo proveedor para errores de límite de velocidad antes de cambiar al modelo de respaldo (predeterminado: `1`). Ese cubo de límite de velocidad incluye texto con forma de proveedor como `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` y `resource exhausted`.
...
- `redactSensitive` / `redactPatterns`: enmascaramiento de mejor esfuerzo para la salida de la consola, registros de archivos, registros OTLP y texto de transcripción de sesión persistente. `redactSensitive: "off"` solo deshabilita esta política general de registro/transcripción; las superficies de seguridad de interfaz de usuario/herramienta/diagnóstico siguen redactando secretos antes de su emisión.
...
- `stuckSessionWarnMs`: umbral de edad sin progreso en ms para clasificar sesiones de procesamiento de larga duración como `session.long_running`, `session.stalled` o `session.stuck`. El progreso de respuesta, herramienta, estado, bloque y ACP restablece el temporizador; los diagnósticos repetidos `session.stuck` se reducen mientras no cambian.
- `stuckSessionAbortMs`: umbral de edad sin progreso en ms antes de que el trabajo activo detenido elegible pueda ser drenado por aborto para la recuperación. Cuando no está establecido, OpenClaw usa la ventana extendida más segura de ejecución integrada de al menos 10 minutos y 5x `stuckSessionWarnMs`.
...
- `otel.captureContent`: captura opcional de contenido sin procesar para los atributos de los spans de OTEL. El valor predeterminado es desactivado. El booleano `true` captura el contenido de mensajes/herramientas que no sean del sistema; el formato de objeto le permite habilitar `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` y `systemPrompt` explícitamente.
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: interruptor de entorno para los atributos del proveedor de spans GenAI experimentales más recientes. De manera predeterminada, los spans mantienen el atributo heredado `gen_ai.system` para la compatibilidad; las métricas de GenAI utilizan atributos semánticos limitados.
- `OPENCLAW_OTEL_PRELOADED=1`: interruptor de entorno para hosts que ya hayan registrado un SDK global de OpenTelemetry. OpenClaw luego omite el inicio/apagado del SDK propiedad del complemento mientras mantiene los oyentes de diagnóstico activos.
...
- `cacheTrace.filePath`: ruta de salida para el JSONL de seguimiento de caché (predeterminado: `$OPENCLAW_STATE_DIR/logs/cache-trace.jsonl`).
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`: controla qué se incluye en la salida de seguimiento de caché (todos predeterminados: `true`).
...
- `auto.stableJitterHours`: ventana de propagación de despliegue adicional del canal estable en horas (predeterminado: `12`; máx: `168`).
...
- `enabled`: puerta de características global de ACP (predeterminado: `true`; establezca `false` para ocultar el envío y la creación de ACP).
...
- `stream.coalesceIdleMs`: ventana de vaciado de inactividad en ms para el texto transmitido (streamed).
...
- `stream.deliveryMode`: `"live"` transmite de forma incremental; `"final_only"` almacena en búfer hasta los eventos de terminación del turno.
...
- `cli.banner.taglineMode` controla el estilo del lema del banner:
  - `"random"` (predeterminado): lemas graciosos/de temporada rotativos.
  - `"default"`: lema neutral fijo (`All your chats, one OpenClaw.`).
  - `"off"`: sin texto de lema (el título/versión del banner aún se muestra).
- Para ocultar todo el banner (no solo los lemas), establezca la variable de entorno `OPENCLAW_HIDE_BANNER=1`.
...
- `maxAttempts`: reintentos máximos para trabajos únicos en errores transitorios (predeterminado: `3`; rango: `0`-`10`).
...
- `enabled`: habilitar las alertas de fallo para trabajos cron (predeterminado: `false`).
...
- `includeSkipped`: contar las ejecuciones omitidas consecutivas hacia el umbral de alerta (predeterminado: `false`). Las ejecuciones omitidas se rastrean por separado y no afectan el retroceso por error de ejecución.
...
- Cuando no se establece ni el destino de fallo global ni el por trabajo, los trabajos que ya se entregan a través de `announce` recurren a ese objetivo de anuncio principal en caso de fallo.
...
## Inclusión de configuración (`$include`)
...
- Matriz de archivos: fusionados profundamente en orden (los posteriores sobrescriben a los anteriores).
- Claves adyacentes: se fusionan después de las inclusiones (sobrescriben los valores incluidos).
- Inclusiones anidadas: hasta 10 niveles de profundidad.
- Rutas: resueltas en relación con el archivo que las incluye, pero deben permanecer dentro del directorio de configuración de nivel superior (`dirname` de `openclaw.json`). Solo se permiten formularios absolutos/`../` cuando aún se resuelven dentro de ese límite.
...
- Las inclusiones raíz, las matrices de inclusión y las inclusiones con anulaciones de nivel hermano son de solo lectura para las escrituras propiedad de OpenClaw; esas escrituras fallan (cerradas) en lugar de aplanar la configuración.
