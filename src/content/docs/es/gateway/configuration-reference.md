...
summary: "Referencia de configuración de puerta de enlace para las claves principales de OpenClaw, valores predeterminados y enlaces a referencias de subsistemas dedicados"
read_when:
...
Referencia de configuración principal para `~/.openclaw/openclaw.json`. Para obtener una descripción general orientada a tareas, consulte [Configuración](/en/gateway/configuration).
...
- `openclaw config schema` imprime el esquema JSON en vivo utilizado para la validación y la interfaz de usuario de Control, con los metadatos integrados/de complementos/de canales fusionados cuando están disponibles
...
- [Referencia de configuración de memoria](/en/reference/memory-config) para `agents.defaults.memorySearch.*`, `memory.qmd.*`, `memory.citations` y configuración de soñar (dreaming) bajo `plugins.entries.memory-core.config.dreaming`
- [Comandos de barra](/en/tools/slash-commands) para el catálogo de comandos integrados + empaquetados actual
...
| `allowlist`                | Solo remitentes en `allowFrom` (o almacenamiento de allow vinculado)                                      |
...
Si falta por completo un bloque de proveedor (`channels.<provider>` ausente), la política de grupos en tiempo de ejecución vuelve a `allowlist` (fail-closed) con una advertencia de inicio.
</Note>

### Sobrescrituras del modelo de canal

Use `channels.modelByChannel` para fijar ID de canal específicos a un modelo. Los valores aceptan `provider/model` o alias de modelo configurados. La asignación de canales se aplica cuando una sesión aún no tiene una sobrescritura de modelo (por ejemplo, establecida mediante `/model`).
...
Use `channels.defaults` para compartir la política de grupo y el comportamiento de latido entre proveedores:
...
- `channels.defaults.groupPolicy`: política de grupo de respaldo cuando no se establece un `groupPolicy` a nivel de proveedor.
- `channels.defaults.contextVisibility`: modo de visibilidad de contexto adicional predeterminado para todos los canales. Valores: `all` (predeterminado, incluir todo el contexto citado/hilo/historial), `allowlist` (incluir solo contexto de remitentes en lista de permitidos), `allowlist_quote` (igual que la lista de permitidos pero mantener el contexto de cita/respuesta explícito). Sobrescritura por canal: `channels.<channel>.contextVisibility`.
...
- Opcional `channels.whatsapp.defaultAccount` anula esa selección de cuenta predeterminada de reserva cuando coincide con un id de cuenta configurado.
...
- Token de bot: `channels.telegram.botToken` o `channels.telegram.tokenFile` (solo archivo regular; los enlaces simbólicos son rechazados), con `TELEGRAM_BOT_TOKEN` como alternativa para la cuenta predeterminada.
- Opcional `channels.telegram.defaultAccount` anula la selección de cuenta predeterminada cuando coincide con un id de cuenta configurado.
- En configuraciones multicuenta (2+ ids de cuenta), establezca un valor predeterminado explícito (`channels.telegram.defaultAccount` o `channels.telegram.accounts.default`) para evitar el enrutamiento de reserva; `openclaw doctor` advierte cuando esto falta o no es válido.
...
- Entradas `bindings[]` de nivel superior con `type: "acp"` configuran enlaces ACP persistentes para temas del foro (use `chatId:topic:topicId` canónico en `match.peer.id`). La semántica de los campos se comparte en [Agentes ACP](/en/tools/acp-agents#channel-specific-settings).
- Las vistas previas de stream de Telegram usan `sendMessage` + `editMessageText` (funciona en chats directos y grupales).
...
- Use `user:<id>` (MD) o `channel:<id>` (canal de guild) para los destinos de entrega; los IDs numéricos simples se rechazan.
- Los slugs de guild están en minúsculas con los espacios reemplazados por `-`; las claves de canal usan el nombre con slug (sin `#`). Se prefieren los IDs de guild.
- Los mensajes escritos por bots se ignoran de forma predeterminada. `allowBots: true` los habilita; use `allowBots: "mentions"` para aceptar solo mensajes de bots que mencionen al bot (los propios mensajes aún se filtran).
...
- `channels.discord.threadBindings` controla el enrutamiento vinculado a hilos de Discord:
  ...
  - `idleHours`: Anulación de Discord para el auto-enfoque por inactividad en horas (`0` deshabilita)
  - `maxAgeHours`: Anulación de Discord para la antigüedad máxima hard en horas (`0` deshabilita)
...
- Las entradas `bindings[]` de nivel superior con `type: "acp"` configuran enlaces ACP persistentes para canales e hilos (use el ID de canal/hilo en `match.peer.id`). La semántica de los campos se comparte en [Agentes ACP](/en/tools/acp-agents#channel-specific-settings).
...
- `channels.discord.voice` habilita las conversaciones de canales de voz de Discord y las anulaciones opcionales de unión automática + TTS.
...
- `channels.discord.autoPresence` mapea la disponibilidad en tiempo de ejecución a la presencia del bot (saludable => en línea, degradado => inactivo, agotado => no molestar) y permite anulaciones opcionales de texto de estado.
...
- `channels.discord.execApprovals`: entrega y autorización de aprobación de ejecución nativa de Discord.
  ...
  - `approvers`: IDs de usuario de Discord permitidos para aprobar solicitudes de ejecución. Recurre a `commands.ownerAllowFrom` cuando se omite.
  ...
  - `target`: dónde enviar los mensajes de aprobación. `"dm"` (por defecto) envía a los MD de los aprobadores, `"channel"` envía al canal de origen, `"both"` envía a ambos. Cuando el objetivo incluye `"channel"`, los botones solo son utilizables por los aprobadores resueltos.
...
- `channels.googlechat.dangerouslyAllowNameMatching` vuelve a habilitar la coincidencia de entidades principales de correo electrónico mutable (modo de compatibilidad de emergencia).
...
- La transmisión nativa de Slack, además del estado de hilo "est escribiendo..." estilo asistente de Slack, requieren un objetivo de hilo de respuesta. Los MD de nivel superior se mantienen fuera del hilo de forma predeterminada, por lo que utilizan `typingReaction` o entrega normal en lugar de la vista previa estilo hilo.
- `typingReaction` añade una reacción temporal al mensaje entrante de Slack mientras se ejecuta una respuesta y luego la elimina al completarse. Utilice un código corto de emoji de Slack como `"hourglass_flowing_sand"`.
- `channels.slack.execApprovals`: entrega de aprobación de ejecución nativa de Slack y autorización del aprobador. Mismo esquema que Discord: `enabled` (`true`/`false`/`"auto"`), `approvers` (IDs de usuario de Slack), `agentFilter`, `sessionFilter` y `target` (`"dm"`, `"channel"` o `"both"`).
...
- `channels.mattermost.groups.<channelId>.requireMention`: anulación de filtrado de menciones por canal (`"*"` para el valor predeterminado).
- El `channels.mattermost.defaultAccount` opcional anula la selección de cuenta predeterminada cuando coincide con una id de cuenta configurada.
...
- El `channels.signal.defaultAccount` opcional anula la selección de cuenta predeterminada cuando coincide con una id de cuenta configurada.

### BlueBubbles

BlueBubbles es la ruta recomendada para iMessage (con respaldo de complemento, configurado en `channels.bluebubbles`).
...
- El `channels.bluebubbles.defaultAccount` opcional anula la selección de cuenta predeterminada cuando coincide con una id de cuenta configurada.
- Las entradas de `bindings[]` de nivel superior con `type: "acp"` pueden vincular conversaciones de BlueBubbles a sesiones ACP persistentes. Utilice un identificador o una cadena de destino de BlueBubbles (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica de campo compartida: [ACP Agents](/en/tools/acp-agents#channel-specific-settings).
- La configuración completa del canal BlueBubbles está documentada en [BlueBubbles](/en/channels/bluebubbles).

### iMessage

OpenClaw genera `imsg rpc` (JSON-RPC sobre stdio). No se requiere ningún demonio ni puerto.
...
- Prefiera los objetivos `chat_id:<id>`. Use `imsg chats --limit 20` para listar chats.
- `cliPath` puede apuntar a un contenedor de SSH; configure `remoteHost` (`host` o `user@host`) para la obtención de archivos adjuntos mediante SCP.
...
- SCP utiliza una verificación estricta de la clave de host, por lo que debe asegurarse de que la clave de host del relé ya exista en `~/.ssh/known_hosts`.
- `channels.imessage.configWrites`: permitir o denegar escrituras de configuración iniciadas por iMessage.
- Las entradas de `bindings[]` de nivel superior con `type: "acp"` pueden vincular conversaciones de iMessage a sesiones ACP persistentes. Utilice un identificador normalizado o un objetivo de chat explícito (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica de campo compartida: [ACP Agents](/en/tools/acp-agents#channel-specific-settings).
...
- `channels.matrix.autoJoin` por defecto es `off`, por lo que las salas invitadas y las invitaciones nuevas de estilo DM se ignoran hasta que establezcas `autoJoin: "allowlist"` con `autoJoinAllowlist` o `autoJoin: "always"`.
- `channels.matrix.execApprovals`: entrega de aprobación de ejecución nativa de Matrix y autorización del aprobador.
  - `enabled`: `true`, `false` o `"auto"` (por defecto). En modo automático, las aprobaciones de ejecución se activan cuando se pueden resolver los aprobadores desde `approvers` o `commands.ownerAllowFrom`.
  - `approvers`: ID de usuario de Matrix (por ejemplo, `@owner:example.org`) con permiso para aprobar solicitudes de ejecución.
  - `agentFilter`: lista de permitidos de ID de agente opcional. Omitir para reenviar aprobaciones para todos los agentes.
  - `sessionFilter`: patrones de clave de sesión opcionales (subcadena o regex).
  - `target`: dónde enviar los avisos de aprobación. `"dm"` (por defecto), `"channel"` (sala de origen) o `"both"`.
  - Anulaciones por cuenta: `channels.matrix.accounts.<id>.execApprovals`.
- `channels.matrix.dm.sessionScope` controla cómo se agrupan los MD de Matrix en sesiones: `per-user` (por defecto) comparte por par enrutado, mientras que `per-room` aísla cada sala de MD.
...
- La configuración completa de Matrix, las reglas de destino y los ejemplos de configuración están documentados en [Matrix](/en/channels/matrix).
...
- Si agregas una cuenta no predeterminada a través de `openclaw channels add` (o incorporación del canal) mientras sigues en una configuración de canal de nivel superior de cuenta única, OpenClaw promueve primero los valores de nivel superior de cuenta única con ámbito de cuenta al mapa de cuentas del canal para que la cuenta original siga funcionando. La mayoría de los canales los mueven a `channels.<channel>.accounts.default`; Matrix puede preservar un objetivo con nombre/predeterminado coincidente existente en su lugar.
...
- `openclaw doctor --fix` también repara formas mixtas moviendo los valores de nivel superior de cuenta única con ámbito de cuenta a la cuenta promovida elegida para ese canal. La mayoría de los canales usan `accounts.default`; Matrix puede preservar un objetivo con nombre/predeterminado coincidente existente en su lugar.
...
Incluya su propio número en `allowFrom` para habilitar el modo de autollamada (ignora las menciones nativas @, solo responde a patrones de texto):
...
- Este bloque configura las superficies de comandos. Para el catálogo de comandos integrados y empaquetados actual, consulte [Comandos de barra](/en/tools/slash-commands).
- Esta página es una **referencia de claves de configuración**, no el catálogo completo de comandos. Los comandos propiedad del canal/complemento, como QQ Bot `/bot-ping` `/bot-help` `/bot-logs`, LINE `/card`, emparejamiento de dispositivos `/pair`, memoria `/dreaming`, control de teléfono `/phone` y Talk `/voice` están documentados en sus páginas de canal/complemento y además en [Comandos de barra](/en/tools/slash-commands).
...
- `nativeSkills: "auto"` activa los comandos de habilidades nativas para Discord/Telegram, deja Slack desactivado.
- Anular por canal: `channels.discord.commands.native` (bool o `"auto"`). `false` borra los comandos registrados anteriormente.
- Anule el registro de habilidades nativas por canal con `channels.<provider>.commands.nativeSkills`.
- `channels.telegram.customCommands` agrega entradas de menú adicionales para el bot de Telegram.
...
- `config: true` habilita `/config` (lee/escribe `openclaw.json`). Para clientes de puerta de enlace `chat.send`, las escrituras persistentes de `/config set|unset` también requieren `operator.admin`; la `/config show` de solo lectura permanece disponible para los clientes operadores normales con ámbito de escritura.
...
- `ownerAllowFrom` es la lista de permitidos explícita del propietario para comandos/herramientas solo para propietarios. Es independiente de `allowFrom`.
- `ownerDisplay: "hash"` aplica hash a los IDs de propietario en el mensaje del sistema. Configure `ownerDisplaySecret` para controlar el hash.
- `allowFrom` es por proveedor. Cuando se configura, es la **única** fuente de autorización (se ignoran las listas de permitidos de canal/emparejamiento y `useAccessGroups`).
- `useAccessGroups: false` permite que los comandos omitan las políticas de grupo de acceso cuando `allowFrom` no está configurado.
- Mapa de documentos de comandos:
  - catálogo integrado + empaquetado: [Comandos de barra](/en/tools/slash-commands)
  - superficies de comandos específicas del canal: [Canales](/en/channels)
  - comandos de QQ Bot: [QQ Bot](/en/channels/qqbot)
  - comandos de emparejamiento: [Emparejamiento](/en/channels/pairing)
  - comando de tarjeta LINE: [LINE](/en/channels/line)
  - soñar con memoria: [Soñar](/en/concepts/dreaming)
...
Lista blanca de habilidades (skill) predeterminada opcional para agentes que no establecen
`agents.list[].skills`.
...
- `"continuation-skip"`: los turnos de continuación segura (después de una respuesta completada del asistente) omiten la reinyección del arranque del espacio de trabajo, reduciendo el tamaño del prompt. Las ejecuciones de latido y los reintentos posteriores a la compactación aún reconstruyen el contexto.
...
Máximo de caracteres por archivo de arranque del espacio de trabajo antes de truncar. Predeterminado: `20000`.
...
- `model`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - El formato de objeto establece el principal más modelos de conmutación por error ordenados.
...
- `imageGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Si selecciona un proveedor/modelo directamente, configure también la clave de API/autenticación del proveedor coincidente (por ejemplo `GEMINI_API_KEY` o `GOOGLE_API_KEY` para `google/*`, `OPENAI_API_KEY` para `openai/*`, `FAL_KEY` para `fal/*`).
...
- `musicGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  - Lo usa la capacidad de generación de música compartida y la herramienta integrada `music_generate`.
  - Valores típicos: `google/lyria-3-clip-preview`, `google/lyria-3-pro-preview` o `minimax/music-2.5+`.
  - Si se omite, `music_generate` todavía puede inferir un proveedor predeterminado con autenticación. Primero intenta el proveedor predeterminado actual, luego los proveedores de generación de música registrados restantes en orden de ID de proveedor.
...
- `videoGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  - Lo usa la capacidad de generación de video compartida y la herramienta integrada `video_generate`.
  - Valores típicos: `qwen/wan2.6-t2v`, `qwen/wan2.6-i2v`, `qwen/wan2.6-r2v`, `qwen/wan2.6-r2v-flash` o `qwen/wan2.7-r2v`.
  - Si se omite, `video_generate` todavía puede inferir un proveedor predeterminado con autenticación. Primero intenta el proveedor predeterminado actual, luego los proveedores de generación de video registrados restantes en orden de ID de proveedor.
  - Si selecciona un proveedor/modelo directamente, configure también la autenticación/clave de API del proveedor correspondiente.
  - El proveedor de generación de video Qwen incluido actualmente admite hasta 1 video de salida, 1 imagen de entrada, 4 videos de entrada, 10 segundos de duración, y las opciones a nivel de proveedor `size`, `aspectRatio`, `resolution`, `audio` y `watermark`.
...
- `pdfMaxPages`: número máximo de páginas considerado por el modo de respaldo de extracción en la herramienta `pdf`.
- `verboseDefault`: nivel de detalle (verbose) predeterminado para los agentes. Valores: `"off"`, `"on"`, `"full"`. Predeterminado: `"off"`.
- `elevatedDefault`: nivel de salida elevada predeterminado para los agentes. Valores: `"off"`, `"on"`, `"ask"`, `"full"`. Predeterminado: `"on"`.
- `model.primary`: formato `provider/model` (ej. `openai/gpt-5.4`). Si omite el proveedor, OpenClaw intenta primero un alias, luego una coincidencia única de proveedor configurado para ese ID de modelo exacto, y solo luego recurre al proveedor predeterminado configurado (comportamiento de compatibilidad obsoleto, por lo que se prefiere `provider/model` explícito). Si ese proveedor ya no expone el modelo predeterminado configurado, OpenClaw recurre al primer proveedor/modelo configurado en lugar de mostrar un predeterminado obsoleto de un proveedor eliminado.
...
- `params`: parámetros de proveedor predeterminados globales aplicados a todos los modelos. Establézcalo en `agents.defaults.params` (ej. `{ cacheRetention: "long" }`).
- `params` precedencia de fusión (config): `agents.defaults.params` (base global) es anulado por `agents.defaults.models["provider/model"].params` (por modelo), luego `agents.list[].params` (id de agente coincidente) anula por clave. Consulte [Prompt Caching](/en/reference/prompt-caching) para obtener detalles.
...
- `maxConcurrent`: máximo de ejecuciones de agente paralelas entre sesiones (cada sesión aún serializada). Predeterminado: 4.
...
Backends de CLI opcionales para ejecuciones de respaldo de solo texto (sin llamadas a herramientas). Útil como respaldo cuando fallan los proveedores de API.
...
- Transferencia de imágenes admitida cuando `imageArg` acepta rutas de archivo.
...
- `every`: cadena de duración (ms/s/m/h). Predeterminado: `30m` (autenticación con clave de API) o `1h` (autenticación OAuth). Establézcalo en `0m` para desactivar.
...
- `isolatedSession`: cuando es verdadero, cada latido se ejecuta en una sesión nueva sin historial de conversación previo. El mismo patrón de aislamiento que el cron `sessionTarget: "isolated"`. Reduce el costo de tokens por latido de ~100K a ~2-5K tokens.
...
- `provider`: ID de un complemento de proveedor de compactación registrado. Cuando se establece, se invoca el `summarize()` del proveedor en lugar del resumen LLM integrado. Se vuelve al integrado en caso de error. Establecer un proveedor fuerza `mode: "safeguard"`. Consulte [Compaction](/en/concepts/compaction).
...
- `postCompactionSections`: nombres opcionales de secciones H2/H3 de AGENTS.md para volver a inyectar después de la compactación. Por defecto es `["Session Startup", "Red Lines"]`; establezca `[]` para deshabilitar la reinyección. Cuando no está configurado o se establece explícitamente en ese par predeterminado, los encabezados antiguos `Every Session`/`Safety` también se aceptan como reserva heredada.
- `model`: anulación opcional de `provider/model-id` solo para el resumen de compactación. Use esto cuando la sesión principal debe mantener un modelo pero los resúmenes de compactación deben ejecutarse en otro; cuando no está configurado, la compactación usa el modelo principal de la sesión.
- `notifyUser`: cuando `true`, envía un breve aviso al usuario cuando comienza la compactación (por ejemplo, "Compactando contexto..."). Deshabilitado por defecto para mantener la compactación silenciosa.
...
- `ttl` controla la frecuencia con la que se puede ejecutar la poda nuevamente (después del último toque al caché).
- La poda recorta primero de forma suave los resultados de herramientas excesivamente grandes y luego borra de forma estricta los resultados de herramientas más antiguos si es necesario.

**Recorte suave** mantiene el principio + el final e inserta `...` en el medio.

**Borrado estricto** reemplaza el resultado completo de la herramienta con el marcador de posición.
...
Consulte [Poda de sesiones](/en/concepts/session-pruning) para obtener detalles sobre el comportamiento.
...
Sandboxing opcional para el agente integrado. Consulte [Sandboxing](/en/gateway/sandboxing) para obtener la guía completa.
...
- Los valores `*Data` respaldados por SecretRef se resuelven desde la instantánea activa de secretos del tiempo de ejecución antes de que inicie la sesión del sandbox
...
En el modo `remote`, las ediciones locales en el host realizadas fuera de OpenClaw no se sincronizan en el sandbox automáticamente después del paso de inicialización.
El transporte es SSH hacia el sandbox de OpenShell, pero el complemento es el propietario del ciclo de vida del sandbox y la sincronización opcional del espejo.
...
**Los contenedores tienen como valor predeterminado `network: "none"`** — configúrelo como `"bridge"` (o una red puente personalizada) si el agente necesita acceso de salida.
...
El acceso del observador noVNC utiliza la autenticación VNC de forma predeterminada y OpenClaw emite una URL de token de corta duración (en lugar de exponer la contraseña en la URL compartida).
...
- `model`: la forma de cadena solo anula `primary`; la forma de objeto `{ primary, fallbacks }` anula ambos (`[]` desactiva los respaldos globales). Los trabajos cron que solo anulan `primary` aún heredan los respaldos predeterminados a menos que establezcas `fallbacks: []`.
- `params`: parámetros de flujo por agente combinados sobre la entrada del modelo seleccionado en `agents.defaults.models`. Úsalo para anulaciones específicas del agente como `cacheRetention`, `temperature` o `maxTokens` sin duplicar todo el catálogo de modelos.
- `skills`: lista de permitidos (allowlist) de habilidades opcional por agente. Si se omite, el agente hereda `agents.defaults.skills` cuando está establecido; una lista explícita reemplaza los valores predeterminados en lugar de combinarse, y `[]` significa sin habilidades.
...
- `runtime`: descriptor de tiempo de ejecución opcional por agente. Usa `type: "acp"` con valores predeterminados de `runtime.acp` (`agent`, `backend`, `mode`, `cwd`) cuando el agente debe usar por defecto sesiones del arnés ACP.
...
- `subagents.requireAgentId`: cuando es verdadero, bloquea las llamadas a `sessions_spawn` que omitan `agentId` (fuerza la selección explícita de perfil; valor predeterminado: falso).
...
Para las entradas `type: "acp"`, OpenClaw resuelve por la identidad exacta de la conversación (`match.channel` + cuenta + `match.peer.id`) y no utiliza el orden de nivel de enlace de ruta anterior.
...
- **`scope`**: estrategia de agrupación de sesiones base para contextos de chat de grupo.
...
- **`reset`**: política de restablecimiento primaria. `daily` restablece a la hora local `atHour`; `idle` restablece después de `idleMinutes`. Cuando ambos están configurados, gana el que expire primero.
...
- **`maintenance`**: controles de limpieza + retención del almacén de sesiones.
  ...
  - `resetArchiveRetention`: retención para archivos de transcripciones `*.reset.<timestamp>`. El valor predeterminado es `pruneAfter`; establecer `false` para desactivar.
...
- **`threadBindings`**: valores globales predeterminados para características de sesión vinculadas a hilos.
  ...
  - `idleHours`: autoenfocado predeterminado por inactividad en horas (`0` desactiva; los proveedores pueden anular)
...
- Por defecto, `identity.emoji` del agente activo; de lo contrario, `"👀"`. Establezca `""` para deshabilitar.
...
- `messages.statusReactions.enabled`: habilita las reacciones de estado del ciclo de vida en Slack, Discord y Telegram.
  En Slack y Discord, al no establecerlo se mantienen habilitadas las reacciones de estado cuando las reacciones de confirmación están activas.
...
Agrupa mensajes rápidos de solo texto del mismo remitente en un solo turno del agente. Los medios/archivos se envían inmediatamente. Los comandos de control omiten el anti-rebote.
...
- `openai.baseUrl` anula el punto de conexión de TTS de OpenAI. El orden de resolución es la configuración, luego `OPENAI_TTS_BASE_URL`, luego `https://api.openai.com/v1`.
- Cuando `openai.baseUrl` apunta a un punto de conexión que no es de OpenAI, OpenClaw lo trata como un servidor TTS compatible con OpenAI y relaja la validación del modelo/voz.
...
- `talk.provider` debe coincidir con una clave en `talk.providers` cuando se hayan configurado varios proveedores de Hablar.
...
- La alternativa de `ELEVENLABS_API_KEY` se aplica solo cuando no se ha configurado ninguna clave de API de Hablar.
...
El incorporación local establece de forma predeterminada las nuevas configuraciones locales en `tools.profile: "coding"` cuando no están configuradas (se conservan los perfiles explícitos existentes).
...
- El `exec` elevado omite el aislamiento y utiliza la ruta de escape configurada (`gateway` por defecto, o `node` cuando el objetivo de ejecución es `node`).
...
Las comprobaciones de seguridad del bucle de herramientas están **desactivadas por defecto**. Establezca `enabled: true` para activar la detección.
...
- `historySize`: historial máximo de llamadas a herramientas retenidas para el análisis de bucles.
...
- `detectors.pingPong`: advierte/bloquea patrones de pares alternantes sin progreso.
...
Configura la comprensión de los medios entrantes (imagen/audio/vídeo):
...
- `asyncCompletion.directSend`: cuando `true`, las tareas `music_generate`
  y `video_generate` completadas de forma asíncrona intentan primero la entrega directa por canal. Valor predeterminado: `false`
  (ruta de entrega de modelo/despertar de sesión de solicitud heredada).
...
- `all`: cualquier sesión. El destino entre agentes aún requiere `tools.agentToAgent`.
...
- Los archivos adjuntos solo son compatibles con `runtime: "subagent"`. El runtime de ACP los rechaza.
...
Marcadores de herramientas integradas experimentales. Desactivadas por defecto a menos que se aplique una regla de activación automática específica del runtime.
...
- Por defecto: `false` para proveedores que no son de OpenAI. OpenAI y OpenAI Codex la activan automáticamente cuando no está configurada; establezca `false` para desactivar esa activación automática.
- Cuando está activada, el prompt del sistema también añade orientaciones de uso para que el modelo solo lo utilice para tareas sustanciales y mantenga como máximo un paso `in_progress`.
...
- `model`: modelo predeterminado para sub-agentes generados. Si se omite, los sub-agentes heredan el modelo del que los invoca.
- `allowAgents`: lista de permitidos (allowlist) predeterminada de IDs de agentes de destino para `sessions_spawn` cuando el agente solicitante no establece su propio `subagents.allowAgents` (`["*"]` = cualquiera; predeterminado: solo el mismo agente).
...
- Precedencia de combinación para IDs de proveedores coincidentes:
  ...
  - El agente `apiKey`/`baseUrl` vacío o ausente vuelve a `models.providers` en la configuración.
  ...
  - La persistencia de los marcadores es autoritativa por fuente: los marcadores se escriben desde la instantánea de configuración de la fuente activa (pre-resolución), no desde los valores de secreto resueltos en tiempo de ejecución.
...
- `models.providers.*.authHeader`: fuerza el transporte de la credencial en el encabezado `Authorization` cuando sea necesario.
...
- `models.providers.*.request`: anulaciones de transporte para solicitudes HTTP del modelo-proveedor.
...
- `models.providers.*.models.*.contextTokens`: límite de contexto en tiempo de ejecución opcional. Úselo cuando desee un presupuesto de contexto efectivo menor que el `contextWindow` nativo del modelo.
...
- `models.providers.*.models.*.compat.requiresStringContent`: sugerencia de compatibilidad opcional para endpoints de chat compatibles con OpenAI solo de cadenas. Cuando `true`, OpenClaw convierte los arreglos `messages[].content` de texto puro en cadenas simples antes de enviar la solicitud.
- `plugins.entries.amazon-bedrock.config.discovery`: raíz de configuración de autodescubrimiento de Bedrock.
- `plugins.entries.amazon-bedrock.config.discovery.enabled`: activar o desactivar el descubrimiento implícito.
...
Los endpoints nativos de Moonshot anuncian compatibilidad de uso de transmisión (streaming) en el transporte compartido
`openai-completions`, y OpenClaw ahora basa esto en las capacidades del endpoint
...
El catálogo de modelos ahora predetermina solo a M2.7.
...
Consulte [Modelos locales](/en/gateway/local-models). TL;DR: ejecute un modelo local grande a través de la API de respuestas de LM Studio en hardware potente; mantenga los modelos alojados fusionados para reserva.
...
- `load.extraDirs`: raíces adicionales de habilidades compartidas (precedencia más baja).
...
- Discovery acepta complementos nativos de OpenClaw además de paquetes Codex compatibles y paquetes Claude, incluidos los paquetes de diseño predeterminado de Claude sin manifiesto.
- **Los cambios de configuración requieren reiniciar el gateway.**
- `allow`: lista blanca opcional (solo se cargan los complementos listados). `deny` gana.
...
- `plugins.entries.<id>.hooks.allowPromptInjection`: cuando `false`, el núcleo bloquea `before_prompt_build` e ignora los campos de modificación de prompts del `before_agent_start` heredado, mientras preserva el `modelOverride` y el `providerOverride` heredados. Se aplica a los ganchos de complementos nativos y a los directorios de ganchos proporcionados por paquetes compatibles.
- `plugins.entries.<id>.subagent.allowModelOverride`: confía explícitamente en este complemento para solicitar `provider` y anulaciones de `model` por ejecución para ejecuciones de subagentes en segundo plano.
- `plugins.entries.<id>.subagent.allowedModels`: lista blanca opcional de objetivos `provider/model` canónicos para anulaciones de subagentes de confianza. Use `"*"` solo cuando desee intencionalmente permitir cualquier modelo.
...
- `plugins.entries.firecrawl.config.webFetch`: configuración del proveedor de recuperación web de Firecrawl.
  - `apiKey`: clave API de Firecrawl (acepta SecretRef). Recurre a la variable de entorno `plugins.entries.firecrawl.config.webSearch.apiKey`, `tools.web.fetch.firecrawl.apiKey` heredada o `FIRECRAWL_API_KEY`.
...
- `plugins.entries.memory-core.config.dreaming`: configuración de soñar con la memoria (experimental). Consulte [Dreaming](/en/concepts/dreaming) para conocer las fases y los umbrales.
...
- La configuración completa de la memoria se encuentra en [Referencia de configuración de memoria](/en/reference/memory-config):
  - `agents.defaults.memorySearch.*`
  - `memory.backend`
  - `memory.citations`
  - `memory.qmd.*`
  - `plugins.entries.memory-core.config.dreaming`
- Los complementos de bundles de Claude habilitados también pueden contribuir con valores predeterminados de Pi integrados desde `settings.json`; OpenClaw los aplica como configuraciones de agente saneadas, no como parches de configuración sin procesar de OpenClaw.
...
- Los perfiles `existing-session` mantienen los límites de ruta actuales de Chrome MCP:
  acciones impulsadas por instantáneas/referencias en lugar de orientación por selector CSS, enlaces de carga
...
- `seamColor`: color de acento para el cromo de la IU de la aplicación nativa (tinte de la burbuja del Modo Charla, etc.).
- `assistant`: anulación de la identidad de la IU de control. Se recurre a la identidad del agente activo.
...
- **Autenticación**: requerida de forma predeterminada. Los enlaces no de bucle invertido requieren autenticación de gateway. En la práctica, eso significa un token/contraseña compartido o un proxy inverso con reconocimiento de identidad con `gateway.auth.mode: "trusted-proxy"`. El asistente de incorporación genera un token de forma predeterminada.
- Si tanto `gateway.auth.token` como `gateway.auth.password` están configurados (incluyendo SecretRefs), configure `gateway.auth.mode` explícitamente en `token` o `password`. El flujo de inicio y la instalación/reparación del servicio fallan cuando ambos están configurados y el modo no está establecido.
- `gateway.auth.mode: "none"`: modo explícito sin autenticación. Úselo solo para configuraciones de bucle invertido local de confianza; esto intencionalmente no se ofrece en los mensajes de incorporación.
- `gateway.auth.mode: "trusted-proxy"`: delegar la autenticación a un proxy inverso con reconocimiento de identidad y confiar en los encabezados de identidad de `gateway.trustedProxies` (ver [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth)). Este modo espera una fuente de proxy **no de bucle invertido**; los proxies inversos de bucle invertido en el mismo host no satisfacen la autenticación de proxy de confianza.
- `gateway.auth.allowTailscale`: cuando `true`, los encabezados de identidad de Tailscale Serve pueden satisfacer la autenticación de la interfaz de usuario de control/WebSocket (verificado a través de `tailscale whois`). Los puntos finales de la API HTTP **no** usan esa autenticación de encabezado de Tailscale; en su lugar, siguen el modo de autenticación HTTP normal del gateway. Este flujo sin token asume que el host del gateway es confiable. El valor predeterminado es `true` cuando `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: limitador opcional de autenticación fallida. Se aplica por IP de cliente y por ámbito de autenticación (el secreto compartido y el token de dispositivo se rastrean de forma independiente). Los intentos bloqueados devuelven `429` + `Retry-After`.
  - En la ruta asíncrona de la interfaz de usuario de control de Tailscale Serve, los intentos fallidos para el mismo `{scope, clientIp}` se serializan antes de la escritura de falla. Por lo tanto, los malos intentos simultáneos del mismo cliente pueden activar el limitador en la segunda solicitud en lugar de competir como simples discrepancias.
  - `gateway.auth.rateLimit.exemptLoopback` es `true` de forma predeterminada; configure `false` cuando intencionalmente también desee limitar la tasa de tráfico de localhost (para configuraciones de prueba o implementaciones de proxy estrictas).
- Los intentos de autenticación WS de origen del navegador siempre están limitados con la exención de bucle invertido deshabilitada (defensa en profundidad contra la fuerza bruta de localhost basada en el navegador).
- En el bucle invertido, esos bloqueos de origen del navegador están aislados por valor normalizado de `Origin`, por lo que los fallos repetidos desde un origen localhost no bloquean automáticamente un origen diferente.
- `tailscale.mode`: `serve` (solo tailnet, enlace de bucle invertido) o `funnel` (público, requiere autenticación).
- `controlUi.allowedOrigins`: lista de permitidos explícita de origen del navegador para conexiones WebSocket de Gateway. Requerido cuando se esperan clientes de navegador de orígenes que no son de bucle invertido.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: modo peligroso que habilita la alternativa de origen del encabezado Host para implementaciones que dependen intencionalmente de la política de origen del encabezado Host.
- `remote.transport`: `ssh` (predeterminado) o `direct` (ws/wss). Para `direct`, `remote.url` debe ser `ws://` o `wss://`.
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: anulación de emergencia del lado del cliente que permite `ws://` en texto plano a IPs de red privada de confianza; el valor predeterminado permanece solo de bucle invertido para texto plano.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: salida de emergencia solo para desarrollo para URL de relé HTTP de bucle invertido. Las URL de relé de producción deben permanecer en HTTPS.
...
- `channels.<provider>.healthMonitor.enabled`: exclusión voluntaria por canal para reinicios del monitor de salud mientras se mantiene el monitor global habilitado.
...
- Si `gateway.auth.token` / `gateway.auth.password` se configura explícitamente a través de SecretRef y no se resuelve, la resolución falla de forma cerrada (sin máscara de alternativa remota).
- `trustedProxies`: IPs de proxy inverso que terminan TLS o inyectan encabezados de cliente reenviados. Solo enumere los proxies que controle. Las entradas de bucle invertido siguen siendo válidas para configuraciones de proxy/detección local en el mismo host (por ejemplo, Tailscale Serve o un proxy inverso local), pero **no** hacen que las solicitudes de bucle invertido sean elegibles para `gateway.auth.mode: "trusted-proxy"`.
- `allowRealIpFallback`: cuando `true`, el gateway acepta `X-Real-IP` si falta `X-Forwarded-For`. Predeterminado `false` para el comportamiento de falla cerrada.
- `gateway.tools.deny`: nombres de herramientas adicionales bloqueados para `POST /tools/invoke` de HTTP (extiende la lista de denegación predeterminada).
...
- `autoGenerate`: genera automáticamente un par de certificados/clave local autofirmado cuando no se configuran archivos explícitos; solo para uso local/desarrollo.
...
- `mode`: controla cómo se aplican las ediciones de configuración en tiempo de ejecución.
  ...
  - `"hybrid"` (predeterminado): intenta la recarga en caliente primero; vuelve al reinicio si es necesario.
...
- Enlaces que no son de loopback: las rutas de canvas requieren autenticación de Gateway (token/contraseña/proxy de confianza), igual que otras superficies HTTP del Gateway.
- Los Node WebViews generalmente no envían encabezados de autenticación; después de que un nodo se empareja y conecta, el Gateway anuncia URLs de capacidades con ámbito de nodo para el acceso a canvas/A2UI.
- Las URLs de capacidad están vinculadas a la sesión WS del nodo activo y expiran rápidamente. No se utiliza una reserva basada en IP.
...
Haga referencia a las variables de entorno en cualquier cadena de configuración con `${VAR_NAME}`:
...
- los ids `source: "exec"` no deben contener segmentos de ruta delimitados por barras `.` o `..` (por ejemplo `a/../b` es rechazado)

### Superficie de credenciales compatible

- Matriz canónica: [SecretRef Credential Surface](/en/reference/secretref-credential-surface)
...
- De forma predeterminada, se rechazan las rutas de comandos de enlaces simbólicos. Establezca `allowSymlinkCommand: true` para permitir rutas de enlaces simbólicos mientras valida la ruta de destino resuelta.
...
- Las referencias secretas se resuelven en el momento de la activación en una instantánea en memoria, luego las rutas de solicitud leen solo la instantánea.
...
- Las credenciales estáticas de tiempo de ejecución provienen de instantáneas resueltas en memoria; las entradas `auth.json` estáticas heredadas se eliminan cuando se descubren.
- Importaciones de OAuth heredadas de `~/.openclaw/credentials/oauth.json`.
- Consulte [OAuth](/en/concepts/oauth).
- Comportamiento en tiempo de ejecución de Secrets y herramientas `audit/configure/apply`: [Secrets Management](/en/gateway/secrets).
...
- `billingBackoffHours`: tiempo de espera de retroceso base en horas cuando un perfil falla debido a errores reales de facturación/crédito insuficiente (predeterminado: `5`). El texto de facturación explícito aún puede aparecer aquí incluso en respuestas `401`/`403`, pero los comparadores de texto específicos del proveedor permanecen limitados al proveedor que los posee (por ejemplo, OpenRouter `Key limit exceeded`). Los mensajes reintentables de HTTP `402` sobre ventana de uso o límites de gasto de organización/espacio de trabajo permanecen en su lugar en la ruta `rate_limit`.
...
- `overloadedProfileRotations`: máximo de rotaciones del perfil de autenticación del mismo proveedor para errores de sobrecarga antes de cambiar al alternativo del modelo (predeterminado: `1`). Las formas de proveedor ocupado como `ModelNotReadyException`\*\* entran aquí.
- `overloadedBackoffMs`: retraso fijo antes de reintentar una rotación de proveedor/perfil sobrecargado (predeterminado: `0`).
- `rateLimitedProfileRotations`: máximo de rotaciones del perfil de autenticación del mismo proveedor para errores de límite de tasa antes de cambiar al alternativo del modelo (predeterminado: `1`). Ese depósito de límite de tasa incluye texto con forma de proveedor como `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded`, y `resource exhausted`.
...
- `stuckSessionWarnMs`: umbral de edad en ms para emitir advertencias de sesión bloqueada mientras una sesión permanece en estado de procesamiento.
- `otel.enabled`: habilita la canalización de exportación de OpenTelemetry (predeterminado: `false`).
- `otel.endpoint`: URL del recolector para la exportación OTel.
...
- `cacheTrace.filePath`: ruta de salida para la traza de caché JSONL (por defecto: `$OPENCLAW_STATE_DIR/logs/cache-trace.jsonl`).
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`: controlar qué se incluye en la salida de la traza de caché (todos por defecto: `true`).
...
- `auto.stableDelayHours`: retraso mínimo en horas antes de la aplicación automática del canal estable (por defecto: `6`; máx: `168`).
...
- `enabled`: puerta de características global de ACP (por defecto: `false`).
- `dispatch.enabled`: puerta independiente para el despacho de turnos de sesión de ACP (por defecto: `true`). Establezca `false` para mantener los comandos ACP disponibles mientras se bloquea la ejecución.
...
- `defaultAgent`: id del agente de destino ACP de reserva cuando los spawns no especifican un objetivo explícito.
- `allowedAgents`: lista blanca de ids de agente permitidos para sesiones de tiempo de ejecución de ACP; vacío significa que no hay restricción adicional.
...
- `stream.maxSessionUpdateChars`: máximo de caracteres para las líneas de estado/actualización de ACP proyectadas.
...
- `runtime.installCommand`: comando de instalación opcional para ejecutar al inicializar un entorno de tiempo de ejecución de ACP.
...
Consulte los campos de identidad de `agents.list` en [Predeterminados del agente](#agent-defaults).
...
Se aplica solo a los trabajos de cron de un solo disparo. Los trabajos recurrentes utilizan un manejo de fallas separado.
...
- `enabled`: habilitar alertas de fallos para trabajos cron (predeterminado: `false`).
- `after`: fallos consecutivos antes de que se active una alerta (entero positivo, min: `1`).
...
- Cuando no se establece ni el destino de fallos global ni el de cada trabajo, los trabajos que ya se entregan a través de `announce` ahora recurren a ese destino de anuncio principal en caso de fallo.
...
| `{{RawBody}}`      | Cuerpo sin formato (sin contenedores de historial/remitente) |
...
- Rutas: se resuelven en relación con el archivo que incluye, pero deben permanecer dentro del directorio de configuración de nivel superior (`dirname` de `openclaw.json`). Solo se permiten las formas absolutas/`../` cuando aún se resuelven dentro de ese límite.
