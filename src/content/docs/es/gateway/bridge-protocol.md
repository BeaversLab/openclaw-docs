...
<Warning>El puente TCP ha sido **eliminado**. Las compilaciones actuales de OpenClaw no incluyen el escucha del puente y las claves de configuración `bridge.*` ya no están en el esquema. Esta página se mantiene solo como referencia histórica. Utilice el [Protocolo de puerta de enlace](/es/gateway/protocol) para todos los clientes de nodos/operadores.</Warning>
...
- `req` / `res`: RPC de puerta de enlace con ámbito (chat, sesiones, configuración, salud, voicewake, skills.bins)
- `event`: señales de nodo (transcripción de voz, solicitud de agente, suscripción de chat, ciclo de vida de ejecución)
...
La aplicación heredada de listas de permitidos vivía en `src/gateway/server-bridge.ts` (eliminado).
...
El puente era **v1 implícito** (sin negociación min/max). Esta sección es
