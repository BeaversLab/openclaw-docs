...
- **La recherche vectorielle** trouve des notes ayant une signification similaire ("hôte de passerelle" correspond à
  "la machine exécutant OpenClaw").
- **La recherche de mots-clés BM25** trouve des correspondances exactes (ID, chaînes d'erreur, clés
  de configuration).
...
<Tip>Activez la décroissance temporelle si votre agent a des mois de notes quotidiennes et que des informations obsolètes dépassent constamment le contexte récent.</Tip>
...
<Tip>Activez MMR si `memory_search` continue à renvoyer des extraits presque identiques provenant de différentes notes quotidiennes.</Tip>
...
Vous pouvez éventuellement indexer les transcriptions de session pour que `memory_search` puisse rappeler
les conversations précédentes. Cela est optionnel via
`memorySearch.experimental.sessionMemory`. Voir la
[référence de configuration](/en/reference/memory-config) pour plus de détails.
