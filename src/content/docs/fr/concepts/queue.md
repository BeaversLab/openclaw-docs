...
- Des voies supplémentaires peuvent exister (par ex. `cron`, `subagent`) afin que les tâches d'arrière-plan puissent s'exécuter en parallèle sans bloquer les réponses entrantes. Ces exécutions détachées sont suivies en tant que [tâches d'arrière-plan](/fr/automation/tasks).
