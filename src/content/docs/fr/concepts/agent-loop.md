...
- Voir [System prompt](/en/concepts/system-prompt) pour ce que le modèle voit.
...
- **`tool_result_persist`** : transformer de manière synchrone les résultats des outils avant qu'ils ne soient écrits dans la transcription de session.
...
Règles de décision des hooks pour les gardes de sortie/outil :

- `before_tool_call` : `{ block: true }` est terminal et arrête les gestionnaires de priorité inférieure.
- `before_tool_call` : `{ block: false }` est une no-op et ne supprime pas un blocage précédent.
- `before_install` : `{ block: true }` est terminal et arrête les gestionnaires de priorité inférieure.
- `before_install` : `{ block: false }` est une no-op et ne supprime pas un blocage précédent.
- `message_sending` : `{ cancel: true }` est terminal et arrête les gestionnaires de priorité inférieure.
- `message_sending` : `{ cancel: false }` est une no-op et ne supprime pas une annulation précédente.

Voir [Plugin hooks](/en/plugins/architecture#provider-runtime-hooks) pour les détails de l'API des hooks et de l'enregistrement.
...
- Le Reasoning streaming peut être émis comme un flux séparé ou sous forme de réponses de bloc.
- Voir [Streaming](/en/concepts/streaming) pour le comportement de découpage et de réponse de bloc.

## Exécution d'outil + outils de messagerie
...
- Si aucune charge utile pouvant être rendue ne reste et qu'un outil a échoué, une réponse d'erreur d'outil de secours est émise
...
- En cas de nouvelle tentative, les tampons en mémoire et les résumés d'outils sont réinitialisés pour éviter les doublons.
- Voir [Compactage](/en/concepts/compaction) pour le pipeline de compactage.
...
- Les deltas de l'assistant sont mis en mémoire tampon dans les messages de chat `delta`.
- Un `final` de chat est émis lors de la **fin/erreur du cycle de vie**.

## Délais d'expiration

- `agent.wait` par défaut : 30 s (uniquement l'attente). Le paramètre `timeoutMs` prime.
...
- [Outils](/en/tools) — outils d'agent disponibles
- [Crochets (Hooks)](/en/automation/hooks) — scripts pilotés par les événements déclenchés par les événements du cycle de vie de l'agent
- [Compactage](/en/concepts/compaction) — comment les longues conversations sont résumées
