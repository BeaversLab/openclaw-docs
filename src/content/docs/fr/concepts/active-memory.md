...
| `config.thinking`           | `"off" \| "minimal" \| "low" \| "medium" \| "high" \| "xhigh" \| "adaptive" \| "max"`                | Remplacement avancé de la réflexion (thinking) pour le sous-agent de mémoire bloquante ; `off` par défaut pour la vitesse                  |
