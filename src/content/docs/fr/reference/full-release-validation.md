...
| Vitest et CI normale     | **Tâche :** `Run normal full CI`<br />**Workflow enfant :** `CI`<br />**Prouve :** graphe CI complet manuel contre la cible, incluant les voies Node Linux, les shards de plugin regroupés, les contrats de channel, la compatibilité Node 22, `check`, `check-additional`, build smoke, vérifications docs, compétences Python, Windows, macOS, i18n de l'interface de contrôle et Android via l'umbrella.<br />**Réexécution :** `rerun_group=ci`.                                                                                                  |
...
Les canaux de vérification de version QA sont consultatifs. Un échec uniquement QA est signalé comme un avertissement
