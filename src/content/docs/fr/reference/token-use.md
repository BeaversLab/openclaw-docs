...
cependant toujours la priorité sur les valeurs de secours de la transcription, et les totaux de
cranscription orientés prompt peuvent l'emporter lorsque les totaux stockés sont manquants ou plus petits.
L'authentification d'utilisation pour les fenêtres de quota de fournisseur provient de crochets
spécifiques au fournisseur lorsqu'ils sont disponibles ; sinon OpenClaw revient à faire correspondre les identifiants
OAuth/clé API provenant des profils d'authentification, de l'environnement ou de la configuration.
Les entrées de transcription de l'assistant conservent la même forme d'utilisation normalisée, y compris
`usage.cost` lorsque le modèle actif dispose d'une tarification configurée et que le fournisseur
renvoie des métadonnées d'utilisation. Cela offre à `/usage cost` et à l'état de session
...
n'affichent jamais de coûts en dollars.
...
Configurez-le dans [la configuration Gateway](/fr/gateway/configuration) et consultez les
...
Dans les configurations multi-agent, vous pouvez conserver une configuration de modèle partagée et régler le comportement du cache
par agent avec `agents.list[].params.cacheRetention`.

Pour un guide détaillé de tous les paramètres, voir [Prompt Caching](/fr/reference/prompt-caching).

Pour la tarification de l'Anthropic API, les lectures de cache sont nettement moins chères que les tokens
d'entrée, tandis que les écritures de cache sont facturées avec un multiplicateur plus élevé. Consultez la tarification du prompt
caching de Anthropic pour les derniers tarifs et multiplicateurs TTL :
...
remplacer uniquement `cacheRetention` et hériter des autres valeurs par défaut du modèle sans changement.
