...
- Les fournisseurs de complétion compatibles avec OpenAI ne reçoivent `prompt_cache_key` que lorsque leur configuration de model définit explicitement `compat.supportsPromptCacheKey: true` ; avec ce même opt-in, un `cacheRetention: "long"` explicite transmet également `prompt_cache_retention: "24h"`, et `cacheRetention: "none"` supprime les deux champs.
...
- OpenAI n'expose pas de compteur de token d'écriture de cache séparé, donc `cacheWrite` reste `0` sur les chemins OpenAI même lorsque le fournisseur réchauffe un cache.
- OpenAI renvoie des en-têtes de traçage et de limite de taux utiles tels que `x-request-id`, `openai-processing-ms`, et `x-ratelimit-*`, mais la comptabilité des succès de cache doit provenir du payload d'utilisation, pas des en-têtes.
- En pratique, OpenAI se comporte souvent comme un cache de préfixe initial plutôt qu'une réutilisation complète de l'historique mobile style Anthropic. Les tours de texte avec un long préfixe stable peuvent atteindre un plateau de tokens mis en cache près de `4864` dans les sondages en direct actuels, tandis que les transcriptions lourdes en outils ou style MCP atteignent souvent un plateau près de `4608` tokens mis en cache même lors de répétitions exactes.
...
- `cacheRetention: "long"` correspond au véritable TTL de cache de prompt d'une heure sur les points de terminaison Vertex AI.
- La rétention de cache par défaut pour `anthropic-vertex`Anthropic correspond aux valeurs par défaut d'Anthropic direct.
...
- Les références de modèles Anthropic Claude (Anthropic`amazon-bedrock/*anthropic.claude*`) prennent en charge le transfert explicite de `cacheRetention`.
- Les modèles Bedrock non-Anthropic sont forcés à Anthropic`cacheRetention: "none"` lors de l'exécution.

### Modèles OpenRouter

Pour les références de modèles `openrouter/anthropic/*`OpenClawAnthropic, OpenClaw injecte les `cache_control`OpenRouter Anthropic
sur les blocs de prompts système/développeur pour améliorer la réutilisation du cache de prompt
uniquement lorsque la requête cible toujours une route OpenRouter vérifiée
(`openrouter` sur son point de terminaison par défaut, ou n'importe quel fournisseur/URL de base qui résout
vers `openrouter.ai`).

Pour les références de modèles `openrouter/deepseek/*`, `openrouter/moonshot*/*` et `openrouter/zai/*`,
`contextPruning.mode: "cache-ttl"`OpenRouterOpenClawAnthropic est autorisé car OpenRouter
gère la mise en cache du prompt côté fournisseur automatiquement. OpenClaw n'injecte pas
les marqueurs `cache_control` Anthropic dans ces requêtes.

La construction du cache DeepSeek est au mieux effort et peut prendre quelques secondes. Une
suite immédiate peut toujours afficher `cached_tokens: 0` ; vérifiez avec une requête
de même préfixe répétée après un court délai et utilisez `usage.prompt_tokens_details.cached_tokens`
comme signal de succès du cache.
...
- Le transport Gemini direct (`api: "google-generative-ai"`) signale les succès de cache
  via le `cachedContentTokenCount`OpenClaw en amont ; OpenClaw mappe cela vers `cacheRead`.
- Lorsque `cacheRetention` est défini sur un modèle Gemini direct, OpenClaw crée,
  ...
  un identifiant de contenu mis en cache.
...
- La sortie JSON de la CLI Gemini peut également afficher les accès au cache via `stats.cached` ;
  OpenClaw mappe cela vers `cacheRead`.
- Si la CLI omet une valeur `stats.input` directe, OpenClaw dérive les jetons d'entrée
...
- Les fichiers stables de contexte de projet d'espace de travail sont ordonnés avant `HEARTBEAT.md` afin que
  le turnover du heartbeat ne casse pas le préfixe stable.
...
du contenu volatil sous la limite (ou sa stabilisation) résout souvent
le problème.
...
- Les catalogues d'outils MCP de bundle sont triés de manière déterministe avant l'enregistrement
  des outils, afin que les changements d'ordre `listTools()` ne fassent pas tourner le bloc d'outils et
...
Pour les diagnostics destinés aux utilisateurs normaux, `/status` et autres résumés d'utilisation peuvent utiliser
...
- Pour OpenAI, attendez-vous à `cacheRead` lors des accès au cache et à ce que `cacheWrite` reste `0` ; OpenAI ne publie pas de champ de jeton d'écriture de cache séparé.
...
- Requêtes Bedrock Nova/Mistral avec paramètres de cache : runtime forcé attendu à `none`.

Documentation connexe :

- [Anthropic](/fr/providers/anthropic)
- [Utilisation et coûts des jetons](/fr/reference/token-use)
- [Nettoyage de la session](/fr/concepts/session-pruning)
