...
- En-têtes et identifiants de demande de l'OpenAI API : [https://developers.openai.com/api/reference/overview](https://developers.openai.com/api/reference/overview)
- Identifiants de demande et erreurs Anthropic : [https://platform.claude.com/docs/en/api/errors](https://platform.claude.com/docs/en/api/errors)
...
Consultez [Session Pruning](/fr/concepts/session-pruning) pour le comportement complet.
...
- Les fournisseurs de complétions compatibles avec OpenAI ne reçoivent `prompt_cache_key` que lorsque leur configuration de modèle définit explicitement `compat.supportsPromptCacheKey: true` ; `cacheRetention: "none"` le supprime toujours.
- Les réponses OpenAI exposent les jetons de prompts mis en cache via `usage.prompt_tokens_details.cached_tokens` (ou `input_tokens_details.cached_tokens` sur les événements de l'API Responses). OpenClaw mappe cela vers `cacheRead`.
- OpenAI n'expose pas de compteur de jetons d'écriture de cache distinct, donc `cacheWrite` reste `0` sur les chemins OpenAI même lorsque le fournisseur réchauffe un cache.
...
- En pratique, OpenAI se comporte souvent comme un cache de préfixe initial plutôt qu'une réutilisation de l'historique complet mobile à la manière de Anthropic. Les tours de texte longs avec un préfixe stable peuvent atteindre un plateau de `4864` jetons mis en cache lors des sondages en direct actuels, tandis que les transcriptions lourdes en outils ou de style MCP atteignent souvent un plateau près de `4608` jetons mis en cache, même lors de répétitions exactes.

### Anthropic Vertex

- Les modèles Anthropic sur Vertex AI (`anthropic-vertex/*`) prennent en charge `cacheRetention` de la même manière que Anthropic direct.
- `cacheRetention: "long"` correspond au temps de vie réel (TTL) de 1 heure du cache de prompt sur les points de terminaison Vertex AI.
- La rétention du cache par défaut pour `anthropic-vertex` correspond aux valeurs par défaut de Anthropic direct.
...
- Les références de modèles Claude de Anthropic (`amazon-bedrock/*anthropic.claude*`) prennent en charge le passage explicite de `cacheRetention`.
- Les modèles Bedrock non Anthropic sont forcés à `cacheRetention: "none"` lors de l'exécution.

### Modèles OpenRouter

Pour les références de modèle `openrouter/anthropic/*`, OpenClaw injecte les `cache_control` de Anthropic sur les blocs de prompt système/développeur pour améliorer la réutilisation du cache de prompt uniquement lorsque la demande cible toujours une route OpenRouter vérifiée (`openrouter` sur son point de terminaison par défaut, ou toute URL de base/provider qui résout à `openrouter.ai`).

Pour les références de modèle `openrouter/deepseek/*`, `openrouter/moonshot*/*` et `openrouter/zai/*`, `contextPruning.mode: "cache-ttl"` est autorisé car OpenRouter gère automatiquement la mise en cache du prompt côté provider. OpenClaw n'injecte pas les marqueurs `cache_control` de Anthropic dans ces demandes.

La construction du cache DeepSeek est effectuée au mieux et peut prendre quelques secondes. Une suite immédiate peut toujours afficher `cached_tokens: 0` ; vérifiez avec une demande répétée de même préfixe après un court délai et utilisez `usage.prompt_tokens_details.cached_tokens` comme signal de succès du cache.
...
- Le transport direct Gemini (`api: "google-generative-ai"`) signale les succès de cache
  via le `cachedContentTokenCount` en amont ; OpenClaw mappe cela vers `cacheRead`.
- Lorsque `cacheRetention` est défini sur un modèle Gemini direct, OpenClaw crée,
  ...
  lors des exécutions sur Google AI Studio. Cela signifie que vous n'avez plus besoin de pré-créer
  manuellement un identifiant de contenu mis en cache.
- Vous pouvez toujours transmettre un identifiant de contenu mis en cache Gemini existant
  en tant que `params.cachedContent` (ou l'ancien `params.cached_content`) sur le modèle
  configuré.
- Ceci est distinct de la mise en cache des préfixes d'invite Anthropic/OpenAI. Pour Gemini,
  OpenClaw gère une ressource `cachedContents` native du fournisseur plutôt que
...
- La sortie JSON CLI Gemini peut également afficher les succès de cache via `stats.cached`
  ; OpenClaw mappe cela vers `cacheRead`.
...
volatile** séparés par une limite interne de préfixe de cache. Le contenu au-dessus de
la limite (définitions d'outils, métadonnées de compétences, fichiers de l'espace de travail et autre
contexte relativement statique) est ordonné de manière à rester identique octet pour octet d'un tour à l'autre.
...
- Les fichiers de contexte de projet d'espace de travail stables sont ordonnés avant `HEARTBEAT.md` afin
  que l'activité du heartbeat ne brise pas le préfixe stable.
...
vérifiez si le changement se situe au-dessus ou en dessous de la limite du cache. Le déplacement
de contenu volatil en dessous de la limite (ou sa stabilisation) résout souvent
le problème.
...
- Les catalogues d'outils MCP de bundle sont triés de manière déterministe avant l'enregistrement de l'outil,
  afin que les changements d'ordre `listTools()` ne fassent pas tourner le bloc d'outils et
  ne brisent pas les préfixes de cache d'invite.
...
- Les assertions en direct actuelles utilisent des vérifications de planchers conservatrices dérivées du comportement en direct observé sur `gpt-5.4-mini` :
  - préfixe stable : `cacheRead >= 4608`, taux de réussite `>= 0.90`
  - transcript tool : `cacheRead >= 4096`, taux de réussite `>= 0.85`
  - transcript image : `cacheRead >= 3840`, taux de réussite `>= 0.82`
  - transcript style MCP : `cacheRead >= 4096`, taux de réussite `>= 0.85`

Une vérification combinée en direct fraîche le 2026-04-04 a donné :

- préfixe stable : `cacheRead=4864`, taux de réussite `0.966`
- transcript tool : `cacheRead=4608`, taux de réussite `0.896`
- transcript image : `cacheRead=4864`, taux de réussite `0.954`
- transcript style MCP : `cacheRead=4608`, taux de réussite `0.891`

Le temps de mur horloge local récent pour la porte combinée était d'environ `88s`.
...
- `OPENCLAW_CACHE_TRACE_MESSAGES=0|1` bascule la capture de la charge utile complète des messages.
...
- Les événements de trace du cache sont au format JSONL et incluent des instantanés intermédiaires tels que `session:loaded`, `prompt:before`, `stream:context` et `session:after`.
...
- Pour Anthropic, attendez-vous à la fois à `cacheRead` et `cacheWrite` lorsque le cache est actif.
- Pour OpenAI, attendez-vous à `cacheRead` lors des succès de cache et à ce que `cacheWrite` reste `0` ; OpenAI ne publie pas de champ distinct pour les jetons d'écriture de cache.
...
- `cacheWrite` élevé sur la plupart des tours : vérifiez la présence d'entrées de prompt système volatiles et assurez-vous que le modèle/le fournisseur prend en charge vos paramètres de cache.
- Élevé `cacheWrite` sur Anthropic : signifie souvent que le point d'arrêt du cache tombe sur un contenu qui change à chaque requête.
- Faible OpenAI `cacheRead` : vérifiez que le préfixe stable est au début, que le préfixe répété fait au moins 1024 jetons et que le même `prompt_cache_key` est réutilisé pour les tours qui devraient partager un cache.
