...
<AccordionGroup>
  ...
  <Accordion title="Garde-fou de génération de médias simultanée">
    Tant qu'une tâche de génération de médias soutenue par une session est toujours active, l'outil agit également comme un garde-fou : les appels répétés `image_generate`, `music_generate` ou `video_generate` dans cette même session renvoient l'état de la tâche active au lieu de démarrer une deuxième génération simultanée. Utilisez `action: "status"` lorsque vous souhaitez une recherche explicite de progression/état du côté de l'agent.
  </Accordion>
