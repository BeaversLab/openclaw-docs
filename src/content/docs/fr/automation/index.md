...
Le Heartbeat est un tour de session principal périodique (par défaut toutes les 30 minutes). Il regroupe plusieurs vérifications (boîte de réception, calendrier, notifications) en un seul tour d'agent avec le contexte complet de la session. Les tours de Heartbeat ne créent pas d'enregistrements de tâches et n'étendent pas la fraîcheur de la réinitialisation quotidienne/inactive de la session. Utilisez `HEARTBEAT.md` pour une petite liste de contrôle, ou un bloc `tasks:` lorsque vous souhaitez des vérifications périodiques "due-only" à l'intérieur du heartbeat lui-même. Les fichiers heartbeat vides sont ignorés en tant que `empty-heartbeat-file` ; le mode de tâche "due-only" est ignoré en tant que `no-tasks-due`. Les heartbeats sont différés tant que le travail cron est actif ou en file d'attente, et `heartbeat.skipWhenBusy` peut également les différer lorsque les sous-agents ou les volets imbriqués sont occupés.
