---
summary: "CLIRéférence CLI pour `openclaw agents` (liste/ajout/suppression/liens/délier/définir identité)"
read_when:
...
Si vous souhaitez également des compétences visibles différentes par agent, configurez `agents.defaults.skills` et `agents.list[].skills` dans `openclaw.json`. Voir [Configuration des compétences](/fr/tools/skills-config) et [Référence de configuration](/fr/gateway/config-agents#agents-defaults-skills).
...
Si vous omettez `accountId` (`--bind <channel>`OpenClaw), OpenClaw le résout à partir des hooks de configuration du plugin, de la liaison de compte forcée, ou du nombre de comptes configurés pour le channel.

Si vous omettez `--agent` pour `bind` ou `unbind`OpenClaw, OpenClaw cible l'agent par défaut actuel.

### `--bind` format

| Format                       | Signification                                                                                                                         |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `--bind <channel>:*`         | Correspond à tous les comptes sur le channel.                                                                                         |
| `--bind <channel>:<account>` | Correspond à un compte.                                                                                                               |
| `--bind <channel>`           | Correspond uniquement au compte par défaut, sauf si la CLI peut résoudre en toute sécurité une portée de compte spécifique au plugin. |

### Comportement de la portée de liaison

- Une liaison stockée sans `accountId` correspond uniquement au compte par défaut du channel.
...
- Si le même agent possède déjà une liaison de channel correspondante sans `accountId`, et que vous liez ultérieurement avec un `accountId`OpenClaw explicite ou résolu, OpenClaw met à niveau cette liaison existante sur place au lieu d'ajouter un doublon.
...
Après la mise à niveau, le routage pour cette liaison est délimité à `telegram:alerts`. Si vous souhaitez également un routage par défaut du compte, ajoutez-le explicitement (par exemple `--bind telegram:default`).
...
- Le passage de n'importe quel indicateur d'ajout explicite bascule la commande vers le chemin non interactif.
...
- En mode interactif, l'amorçage de l'authentification copie uniquement les profils statiques portables
  (`api_key` et `token` statiques par défaut). Les profils de jeton d'actualisation OAuth restent
  disponibles uniquement par héritage en lecture directe à partir du véritable magasin d'agents `main`.
  Si l'agent par défaut configuré n'est pas `main`, connectez-vous séparément pour les profils
...
- Lorsque le Gateway est accessible, la suppression est envoyée via le Gateway afin que le nettoyage de la configuration et du magasin de sessions partage le même writer que le trafic d'exécution. Si le Gateway ne peut pas être atteint, la CLI revient au chemin local hors ligne.
...
- Lorsque aucun champ d'identité explicite n'est fourni, la commande lit les données d'identité à partir de `IDENTITY.md`.
