...
Écrivez un fichier zip de diagnostic local conçu pour être joint aux rapports de bogues. Pour le model de confidentialité et le contenu du bundle, consultez [Diagnostics Export](/fr/gateway/diagnostics).
...
<AccordionGroup>
  <Accordion title="Sémantique de l'état">
    - `gateway status` reste disponible pour le diagnostic, même lorsque la configuration locale de la CLI est manquante ou invalide.
    ...
    - Les sondes de diagnostic sont non mutantes pour l'authentification initiale de l'appareil : elles réutilisent un jeton d'appareil existant en cache lorsqu'il en existe un, mais elles ne créent pas une nouvelle identité d'appareil CLI ni un enregistrement d'appareil en lecture seule juste pour vérifier l'état.
  ...
  <Accordion title="LinuxVérifications de dérive d'authentification systemd sous Linux"Linux>
    ...
    - Les vérifications de dérive résolvent les SecretRefs `gateway.auth.token` en utilisant l'environnement d'exécution fusionné (d'abord l'environnement de commande du service, puis l'environnement de processus de secours).
...
<AccordionGroup>
  ...
  <Accordion title="Sortie JSON">
    ...
    - `degraded` : au moins une cible a accepté une connexion mais n'a pas terminé les diagnostics détaillés de RPC.
    ...
    - `discovery.timeoutMs` et `discovery.count` : le budget de détection réel / le nombre de résultats utilisés pour ce passage de sonde.

    Par cible (`targets[].connect`) :

    - `ok` : accessibilité après connexion + classification dégradée.
    - `rpcOk` : succès détaillé du RPC.
    - `scopeLimited` : échec du détail RPC en raison d'une portée d'opérateur manquante.
...
Le mode "Distant via SSH" de l'application macOS utilise un transfert de port local pour que la passerelle distante (qui peut être liée uniquement à la boucle locale) devienne accessible à `ws://127.0.0.1:<port>`.
...
<ParamField path="--ssh-auto" type="boolean">
  Choisir le premier hôte de passerelle découvert comme cible SSH à partir du point de terminaison de découverte résolu (`local.` plus le domaine étendu configuré, le cas échéant). Les indications TXT-only sont ignorées.
</ParamField>
...
responsable de l'exécution finale (exec) de `openclaw` ou de Node avec ces arguments.
...
un fichier exécutable, écrit le wrapper dans le fichier `ProgramArguments` du service et persiste
...
<AccordionGroup>
  ...
  <Accordion title="Comportement du cycle de vie">
    ...
    - Sur macOS, `gateway stop` utilise `launchctl bootout` par défaut, ce qui supprime le LaunchAgent de la session de démarrage actuelle sans rendre la désactivation persistante — la récupération automatique KeepAlive reste active pour les futurs plantages et `gateway start` réactive proprement sans `launchctl enable` manuel. Passez `--disable` pour supprimer de manière persistante KeepAlive et RunAtLoad afin que la passerelle ne redémarre pas jusqu'au prochain `gateway start` explicite ; à utiliser lorsqu'un arrêt manuel doit survivre aux redémarrages ou aux redémarrages système.
    - `gateway restart --safe` demande au Gateway en cours d'exécution de préparer le travail actif OpenClaw et de différer le redémarrage jusqu'à la livraison des réponses, aux exécutions intégrées et aux exécutions de tâches. `--safe` ne peut pas être combiné avec `--force` ou `--wait`.
    - `gateway restart --wait 30s` remplace le budget de vidange de redémarrage configuré pour ce redémarrage. Les nombres seuls sont des millisecondes ; des unités telles que `s`, `m` et `h` sont acceptées. `--wait 0` attend indéfiniment.
    - `gateway restart --safe --skip-deferral` exécute le redémarrage sécurisé conscient OpenClaw mais contourne la porte de délai, de sorte que le Gateway émet le redémarrage immédiatement même lorsque des bloqueurs sont signalés. Soupape de sécurité pour l'opérateur en cas de différés d'exécution de tâches bloquées ; nécessite `--safe`.
    - `gateway restart --force` ignore la vidange du travail actif et redémarre immédiatement. À utiliser lorsqu'un opérateur a déjà inspecté les bloqueurs de tâches répertoriés et souhaite récupérer la passerne immédiatement.
    - Les commandes de cycle de vie acceptent `--json` pour les scripts.

  </Accordion>
  <Accordion title="Auth et SecretRefs lors de l'installation">
    - Lorsque l'auth par jeton nécessite un jeton et que `gateway.auth.token` est géré par SecretRef, `gateway install` valide que le SecretRef peut être résolu mais ne persiste pas le jeton résolu dans les métadonnées de l'environnement de service.
    - Si l'auth par jeton nécessite un jeton et que le SecretRef du jeton configuré est non résolu, l'installation échoue de manière sécurisée au lieu de persister du texte en clair de repli.
    - Pour l'auth par mot de passe sur `gateway run`, préférez `OPENCLAW_GATEWAY_PASSWORD`, `--password-file`, ou un `gateway.auth.password` soutenu par SecretRef plutôt qu'un `--password` en ligne.
    - En mode d'auth inféré, `OPENCLAW_GATEWAY_PASSWORD` shell-only ne relâche pas les exigences de jeton d'installation ; utilisez une configuration durable (`gateway.auth.password` ou config `env`) lors de l'installation d'un service géré.
    - Si à la fois `gateway.auth.token` et `gateway.auth.password` sont configurés et que `gateway.auth.mode` est non défini, l'installation est bloquée jusqu'à ce que le mode soit défini explicitement.
...
- DNS-SD unidiffusion (Bonjour étendu) : choisissez un domaine (exemple : `openclaw.internal.`) et configurez le DNS fractionné + un serveur DNS ; voir [Bonjour](/fr/gateway/bonjour).
...
- `wsUrl` dans la sortie JSON est dérivé du point de terminaison de service résolu, et non des indications TXT uniquement telles que `lanHost` ou `tailnetDns`.
- Sur le mDNS `local.`, `sshPort` et `cliPath` sont diffusés uniquement lorsque `discovery.mdns.mode` est `full`. Le DNS-SD étendu écrit toujours `cliPath` ; `sshPort` reste également facultatif.
