...
Avec `--secret-input-mode ref`, l'intégration écrit des références basées sur l'environnement au lieu des valeurs de clé en clair.
...
<Note>`--auth-choice zai-api-key` détecte automatiquement le meilleur point de terminaison Z.AI pour votre clé (préfère l'API générale avec `zai/glm-5.1`). Si vous souhaitez spécifiquement les points de terminaison du GLM Coding Plan, choisissez `zai-coding-global` ou `zai-coding-cn`.</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Suites de recherche Web">
    ...
    - **Grok** peut offrir une configuration `x_search` facultative avec le même `XAI_API_KEY` et un choix de modèle `x_search`MoonshotAPI.
  ...
  <Accordion title="Autres comportements"CLI>
    ...
    - Premier chat le plus rapide : `openclaw dashboard`OpenAIAnthropic (Interface utilisateur de contrôle, aucune configuration de canal).
    ...
    - Si un état Hermes est détecté, l'intégration propose un flux de migration. Utilisez [Migrate](/fr/cli/migrate) pour les plans à blanc, le mode de sur écriture, les rapports et les mappages exacts.
