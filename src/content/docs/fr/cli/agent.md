...
- `--channel <channel>` : channel de livraison ; omettre pour utiliser le channel de la session principale
...
- Lorsque cette commande déclenche la régénération `models.json`, les identifiants de provider gérés par SecretRef sont conservés sous forme de marqueurs non secrets (par exemple noms de env var, `secretref-env:ENV_VAR_NAME` ou `secretref-managed`), et non en tant que texte en clair secret résolu.
