...
- **Sandboxing note** : le backend sandbox par défaut utilise Docker lorsque le sandboxing est activé, mais le sandboxing est désactivé par défaut et ne nécessite **pas** que l'intégralité de la gateway s'exécute dans Docker. Les backends de sandbox SSH et OpenShell sont également disponibles. Voir [Sandboxing](/fr/gateway/sandboxing).
...
- Si vous utilisez un VPS/hôte public, consultez
  [Durcissement de la sécurité pour l'exposition au réseau](/fr/gateway/security),
...
<Steps>
  <Step title="Build the image">
    ...
    Cela construit l'image de la gateway localement. Pour utiliser une image préconstruite à la place :
...
La route est protégée par l'authentification Gateway. N'exposez pas de port public `/metrics` séparé ni de chemin de proxy inverse non authentifié. Voir
...
Pour les pièges et le dépannage, voir [Bonjour discovery](/fr/gateway/bonjour).
...
**Points chauds de croissance du disque :** surveillez `media/`, les fichiers JSONL de session, la base de données d'état SQLite partagée, les racines des packages de plugins installés et les fichiers journaux tournants sous `/tmp/openclaw/`.
...
Si vous avez installé ClawDock à partir de l'ancien chemin brut `scripts/shell-helpers/clawdock-helpers.sh`, relancez la commande d'installation ci-dessus afin que votre fichier d'aide local suive le nouvel emplacement.

Utilisez ensuite `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Exécutez `clawdock-help` pour toutes les commandes.
Voir [ClawDock](/fr/install/clawdock) pour le guide complet de l'aide.
...
<AccordionGroup>
  <Accordion title="Activer le bac à sable de l'agent pour la passerelle Docker">
    ...
    Le script monte `docker.sock` uniquement après que les prérequis du bac à sable sont remplis. Si la configuration du bac à sable ne peut pas être terminée, le script réinitialise `agents.defaults.sandbox.mode` à `off`. Les tours de mode code de Codex sont toujours limités au `workspace-write` de Codex tandis que le bac à sable OpenClaw est actif ; ne montez pas le socket Docker de l'hôte dans les conteneurs du bac à sable de l'agent.
...
<Accordion title="Note de sécurité du réseau partagé">`openclaw-cli` utilise `network_mode: "service:openclaw-gateway"` afin que les commandes CLI puissent atteindre la passerelle via `127.0.0.1`. Considérez cela comme une frontière de confiance partagée. La configuration compose supprime `NET_RAW`/`NET_ADMIN` et active `no-new-privileges` sur `openclaw-gateway` et `openclaw-cli`.</Accordion>
  ...
  <Accordion title="Échecs DNS de Docker Desktop dans openclaw-cli">
    Certains paramétrages Docker Desktop échouent aux résolutions DNS depuis le sidecar
    `openclaw-cli` sur le réseau partagé une fois `NET_RAW` supprimé, ce qui se manifeste par
    `EAI_AGAIN` lors des commandes utilisant npm, comme `openclaw plugins install`.
    ...
    restaurant les capacités par défaut de Docker, à n'utiliser que pour une commande CLI
    ponctuelle nécessitant un accès au registre de paquets, et non comme invocation Compose
    par défaut :
    ...
    Si vous avez déjà créé un conteneur `openclaw-cli` à exécution prolongée, recréez-le
  ...
  <Accordion title="Autorisations et EACCES">
    ...
    Le même conflit peut apparaître sous la forme d'un avertissement de plugin tel que
    `blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root)`
    suivi de `plugin present but blocked`. Cela signifie que l'uid du processus et le
    propriétaire du répertoire du plugin monté ne sont pas les mêmes. Préférez l'exécution du conteneur avec l'uid
    par défaut 1000 et la correction de la propriété du montage de liaison. N'utilisez chown
  ...
  <Accordion title="Reconstructions plus rapides">
    Organisez votre Dockerfile afin que les couches de dépendances soient mises en cache. Cela évite de ré-exécuter
  ...
  <Accordion title="Options de conteneur pour utilisateurs avancés">
    ...
    5. **Ou installer les navigateurs Playwright dans un volume persistant** :
       ...
       ```
    6. **Conserver les téléchargements du navigateur** : utilisez `OPENCLAW_HOME_VOLUME` ou
...
<Accordion title="OpenAIOAuthDockerOpenAI Codex OAuth (headless Docker)" OpenAIOAuthDocker>
  ...
  <Accordion title="Métadonnées de l'image de base">
    ...
    actualisé via les PR d'image de base Dependabot Docker ; les builds de release n'exécutent
    pas de couche de mise à niveau de distribution. Voir
...
[Runtime de VM Docker](/fr/install/docker-vm-runtime) pour les étapes de déploiement sur VM partagée,
y compris la création de binaire, la persistance et les mises à jour.
...
La portée du Sandbox peut être par agent (par défaut), par session ou partagée. Chaque portée dispose de son propre espace de travail monté sur `/workspace`. Vous pouvez également configurer des stratégies d'autorisation/refus des outils, l'isolement du réseau, les limites de ressources et les conteneurs de navigateur.
...
<AccordionGroup>
  <Accordion title="Image manquante ou conteneur sandbox ne démarrant pas">
    ...
    (checkout des sources) ou la commande en ligne `docker build` issue de [Sandboxing § Images et configuration](/fr/gateway/sandboxing#images-and-setupnpm) (npm install),
...
<Accordion title="Outils personnalisés introuvables dans le sandbox" OpenClaw>
  OpenClaw exécute les commandes avec `sh -lc` (shell de connexion), qui sourcé `/etc/profile` et peut réinitialiser le PATH. Définissez `docker.env.PATH` pour préfixer vos chemins d'outils personnalisés, ou ajoutez un script sous `/etc/profile.d/` dans votre Dockerfile.
</Accordion>
...
<Accordion title="OOM-killed lors de la construction de l'image (exit 137)">La VM a besoin d'au moins 2 Go de RAM. Utilisez une classe de machine plus grande et réessayez.</Accordion>
  ...
  <Accordion title="Non autorisé ou appairage requis dans l'interface de contrôle">
    Récupérez un lien fraîchement généré vers le tableau de bord et approuvez l'appareil du navigateur :
...
- [Mise à jour](/fr/install/updatingOpenClaw) — garder OpenClaw à jour
