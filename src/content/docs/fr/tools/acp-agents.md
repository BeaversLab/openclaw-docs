...
plugin d'application serveur natif Codex possède les contrôles `/codex ...` et le runtime
intégré par défaut `openai/gpt-*` pour les tours d'agent ; ACP possède
...
directement aux conversations de channel OpenClaw existantes, utilisez
...
Les outils de plugin OpenClaw et les outils intégrés OpenClaw ne sont **pas** exposés aux
harnais ACP par défaut. Activez les ponts MCP explicites dans
[Configuration des agents ACP](/fr/tools/acp-agents-setup) uniquement lorsque le harnais
...
<AccordionGroup>
  ...
  <Accordion title="Règles de routage Codex natif">
    ...
    - « Liez ce channel Discord à Codex. »
    - « Attachez cette conversation au fil Codex `<id>`. »
    - « Affichez les fils Codex, puis liez celui-ci. »

    La liaison de conversation Codex native est le chemin de contrôle de conversation par défaut.
    ...
    `after_tool_call`, et acheminer les événements Codex `PermissionRequest` via
    les approbations OpenClaw. Les hooks Codex `Stop` sont relayés à
    ...
    délibérément conservateur : il ne mute pas les arguments des outils natifs Codex
    et ne réécrit pas les enregistrements de fils Codex. Utilisez ACP explicitement
    uniquement lorsque vous souhaitez le modèle d'exécution/de session ACP. La limite de support Codex
  ...
  <Accordion title="Aide-memoire pour la sélection du modèle / fournisseur / runtime">
    - références de modèle Codex héritées - itinéraire de modèle d'abonnement/OAuth Codex hérité réparé par le docteur.
  ...
  <Accordion title="Déclencheurs de routage en langage naturel vers l'ACP">
    ...
    - « Utilisez le CLI pour cette tâche dans un fil, puis continuez les suites dans ce même fil. »
    ...
    achemine les suites vers cette session jusqu'à la fermeture ou l'expiration. Codex ne suit
    ce chemin que lorsque ACP/acpx est explicite ou que le plugin Codex natif
    n'est pas disponible pour l'opération demandée.

    Pour `sessions_spawn`, `runtime: "acp"` n'est annoncé que lorsque l'ACP
    est activé, que le demandeur n'est pas  et qu'un backend d'exécution
    ACP est chargé. `acp.dispatch.enabled=false` met en pause la distribution automatique
    du fil ACP mais ne masque ni ne bloque les appels explicites
    ...
    d'agent de configuration OpenClaw normal depuis `agents_list` à moins que cette entrée ne soit
...
Utilisez ACP lorsque vous souhaitez un runtime de harnais externe. Utilisez le **serveur d'application Codex natif**
pour la liaison/le contrôle de conversation Codex lorsque le plugin `codex`
est activé. Utilisez les **sous-agents** lorsque vous souhaitez des exécutions déléguées natives OpenClaw.
...
Les backends CLI sont des runtimes de repli locaux texte-only distincts - voir [CLI Backends](/fr/gateway/cli-backends).

Pour les opérateurs, la règle pratique est la suivante :

- **Vous voulez `/acp spawn`, des sessions liables, des contrôles d'exécution ou un travail persistant du harness ?** Utilisez ACP.
...
- **Fil de discussion/sujet enfant** - une surface de messagerie supplémentaire optionnelle créée uniquement par `--thread ...`.
- **Espace de travail d'exécution** - l'emplacement du système de fichiers (`cwd`, extraction du repo, espace de travail backend) où le harness s'exécute. Indépendant de la surface de chat.
...
<AccordionGroup>
  <Accordion title="Règles de liaison et exclusivité">
    ...
    - `--bind here` fonctionne uniquement sur les canaux qui annoncent une liaison à la conversation en cours ; OpenClaw renvoie un message clair de non-support dans le cas contraire. Les liaisons persistent après les redémarrages de la passerelle.
    - Sur Discord, `spawnSessions` limite la création de fils de discussion enfants pour `--thread auto|here`, et non pour `--bind here`.
    - Si vous effectuez un spawn vers un autre agent ACP sans `--cwd`, OpenClaw hérite par défaut de l'espace de travail de l'**agent cible**. Les chemins hérités manquants (`ENOENT`/`ENOTDIR`) reviennent par défaut à celui du backend ; les autres erreurs d'accès (p. ex. `EACCES`) apparaissent comme des erreurs de spawn.
  ...
  <Accordion title="Sessions liées aux fils de discussion">
    Lorsque les liaisons de fils de discussion (thread bindings) sont activées pour un adaptateur de canal :
    ...
    Indicateurs de fonctionnalités requis pour les sessions ACP liées aux fils :
    ...
    - Créations de sessions de fils d'adaptateur de canal activées (par défaut : `true`) :
...
- **Canal/DM Slack :** `match.channel="slack"` + `match.peer.id="<channelId|channel:<channelId>|#<channelId>|userId|user:<userId>|slack:<userId>|<@userId>>"`. Privilégiez les identifiants stables Slack ; les liaisons de canal correspondent également aux réponses dans les fils de ce canal.
- **Sujet de forum Telegram :** `match.channel="telegram"` + `match.peer.id="<chatId>:topic:<topicId>"`
- **DM/groupe iMessage :** `match.channel="imessage"` + `match.peer.id="<handle|chat_id:*|chat_guid:*|chat_identifier:*>"`. Privilégiez `chat_id:*` pour des liaisons de groupe stables.
...
3. Paramètres globaux ACP par défaut (p. ex. `acp.backend`)
...
- Pour les lancements ACP inter-agents sans `cwd`OpenClaw explicite, OpenClaw hérite de l'espace de travail de l'agent cible depuis la configuration de l'agent.
...
<ParamField path="thread" type="boolean" default="false">
  Demander le flux de liaison de thread là où c'est pris en charge.
</ParamField>
<ParamField path="mode" type='"run" | "session"' default="run">
  `"run"` est unique (one-shot) ; `"session"` est persistant. Si `thread: true` et
...
<ParamField path="cwd" type="string">
  ...
  En cas d'omission, le spawn ACP hérite de l'espace de travail de l'agent cible
...
<ParamField path="streamTo" type='"parent"'>
  ...
  (`<sessionId>.acp-stream.jsonl`) que vous pouvez suivre pour obtenir l'historique complet de relais.
</ParamField>
<ParamField path="runTimeoutSeconds" type="number">
  Abandonne le tour enfant ACP après N secondes. `0` maintient le tour sur le
  chemin sans délai d'expiration de la passerelle (gateway). La même valeur est appliquée à l'exécution Gateway
  et au runtime ACP afin que les harnais bloqués ou épuisés en quota ne
  n'occupent pas indéfiniment la voie de l'agent parent.
</ParamField>
<ParamField path="model" type="string">
  Remplacement explicite de modèle pour la session enfant ACP. Les spawns Codex ACP
  normalisent les références OpenAI telles que `openai/gpt-5.4` vers la configuration de démarrage
  Codex ACP avant `session/new` ; les formes avec barre oblique telles que `openai/gpt-5.4/high`
  définissent également l'effort de raisonnement Codex ACP.
...
<ParamField path="thinking" type="string">
  ...
  omette le remplacement au démarrage de l'effort de raisonnement.
</ParamField>
...
<Tabs>
  ...
  <Tab title="--thread auto|here|off">
    ...
    | `here` | Nécessite un fil actif actuel ; échoue si l'on n'est pas dans un fil.                                                  |
    | `off`  | Aucune liaison. La session démarre non liée.                                                                 |

    Notes :

    - Sur les surfaces de liaison non filées, le comportement par défaut est effectivement `off`.
...
<AccordionGroup>
  <Accordion title="Sessions ACP interactives">
    ...
    Les messages de suivi dans la conversation liée sont acheminés directement vers la session ACP, et la sortie de l'ACP est renvoyée vers ce même canal/fil/sujet.

    Ce que OpenClaw envoie au harnais :

    - Les suivis liés normaux sont envoyés sous forme de texte d'invite, avec des pièces jointes uniquement si le harnais/backend les prend en charge.
    - Les commandes de gestion `/acp` et les commandes locales du Gateway sont interceptées avant l'acheminement ACP.
    ...
    - Les entrées de transcription ACP utilisent le texte de déclencheur visible par l'utilisateur ou l'invite de complétion simple. Les métadonnées d'événement internes restent structurées dans OpenClaw dans la mesure du possible et ne sont pas traitées comme du contenu de chat rédigé par l'utilisateur.
  ...
  <Accordion title="Sessions ACP ponctuelles détenues par le parent">
    ...
    - L'achèvement est signalé via le chemin d'annonce de fin de tâche. OpenClaw convertit les métadonnées d'achèvement internes en une invite ACP simple avant de l'envoyer à un harnais externe, les harnais ne voient donc pas les marqueurs de contexte d'exécution propres à OpenClaw.
    - Le parent réécrit le résultat de l'enfant avec une voix d'assistant normale lorsqu'une réponse visible par l'utilisateur est utile.

    Ne **pas** traiter ce chemin comme une conversation de pair à pair entre le parent et l'enfant. L'enfant a déjà un canal d'achèvement pour revenir vers le parent.

  </Accordion>
  <Accordion title="sessions_send et livraison A2A">
    `sessions_send` peut cibler une autre session après la génération. Pour les sessions homologues normales, OpenClaw utilise un chemin de suivi agent-à-agent (A2A) après avoir injecté le message :

    - Attendre la réponse de la session cible.
    - Facultativement, laisser le demandeur et la cible échanger un nombre limité de tours de suivi.
    - Demander à la cible de produire un message d'annonce.
    - Livrer cette annonce au canal ou au fil visible.

    Ce chemin A2A est un repli pour les envois homologues où l'expéditeur a besoin d'un suivi visible. Il reste activé lorsqu'une session non liée peut voir et envoyer un message à une cible ACP, par exemple sous des paramètres `tools.sessions.visibility` larges.

    OpenClaw ignore le suivi A2A uniquement lorsque le demandeur est le parent de son propre enfant ACP ponctuel détenu par le parent. Dans ce cas, exécuter A2A par-dessus l'achèvement de la tâche peut réveiller le parent avec le résultat de l'enfant, transférer la réponse du parent vers l'enfant et créer une boucle d'écho parent/enfant. Le résultat `sessions_send` signale `delivery.status="skipped"` pour ce cas d'enfant détenu car le chemin d'achèvement est déjà responsable du résultat.
  ...
  <Accordion title="Reprendre une session existante">
    ...
    - Continuer une session de codage que vous avez commencée de manière interactive dans le CLI, désormais sans tête via votre agent.
    - Reprendre un travail qui a été interrompu par un redémarrage de la passerelle ou un délai d'inactivité.
    ...
    - `resumeSessionId` est un identifiant de reprise ACP/harness local à l'hôte, et non une clé de session de OpenClaw channel; OpenClaw vérifie toujours la stratégie de génération ACP et la stratégie de l'agent cible avant l'expédition, tandis que le backend ACP ou le harness possède l'autorisation pour charger cet identifiant amont.
  ...
  <Accordion title="Test de fumée post-déploiement">
    Après le déploiement d'une passerelle, effectuez une vérification de bout en bout en direct plutôt que de faire confiance aux tests unitaires :
    ...
    Gardez la porte sur `mode: "run"` et ignorez `streamTo: "parent"` -
...
- OpenClaw applique toujours les portes de fonctionnalité ACP, les agents autorisés, la propriété de la session, les liaisons de channel et la politique de livraison du Gateway.
...
- Si la session du demandeur est isolée (sandboxed), les spawns ACP sont bloqués pour `sessions_spawn({ runtime: "acp" })` et `/acp spawn`.
- `sessions_spawn` avec `runtime: "acp"` ne prend pas en charge `sandbox: "require"`.

## Résolution de la cible de session

La plupart des actions `/acp` acceptent une session cible facultative (`session-key`,
...
| ---------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | clé de configuration d'exécution `model`    | Pour Codex ACP, OpenClaw normalise OpenClaw`openai/<model>` vers l'id du modèle de l'adaptateur et mappe les suffixes de raisonnement avec barre oblique tels que `openai/gpt-5.4/high` vers `reasoning_effort`.                             |
...
| `/acp set <key> <value>`     | générique                                   | `key=cwd` utilise le chemin de substitution cwd.                                                                                                                                                                                             |
| `/acp reset-options`         | efface toutes les substitutions d'exécution | -                                                                                                                                                                                                                                            |

## acpx harness, plugin setup, and permissions

Pour la configuration du harnais acpx (alias Claude Code / Codex / Gemini CLI), les ponts MCP plugin-tools et OpenClaw-tools, et les modes de permission ACP, voir
...
| ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ACP runtime backend is not configured`                                      | Plugin backend manquant, désactivé ou bloqué par `plugins.allow`.                                                                                  | Installez et activez le plugin backend, incluez `acpx` dans `plugins.allow` lorsque cette liste blanche est définie, puis exécutez `/acp doctor`.                                                                                      |
...
| `/acp doctor` signale que le backend n'est pas prêt juste après le démarrage | Le plugin backend est manquant, désactivé, bloqué par une stratégie d'autorisation/refus, ou son exécutable configuré n'est pas disponible.        | Installez/activez le plugin backend, réexécutez `/acp doctor` et inspectez l'erreur d'installation ou de stratégie du backend s'il reste non fonctionnel.                                                                              |
| Commande Harness introuvable                                                 | L'adaptateur CLI n'est pas installé, le plugin externe est manquant ou la récupération initiale de CLI`npx` a échoué pour un adaptateur non-Codex. | Exécutez `/acp doctor`Gateway, installez/préchauffez l'adaptateur sur l'hôte de la passerelle, ou configurez explicitement la commande de l'agent acpx.                                                                                |
...
| `--bind here requires running /acp spawn inside an active ... conversation`  | `--bind here` utilisé sans une conversation liable active.                                                                                         | Déplacez-vous vers le chat/la channel cible et réessayez, ou utilisez un spawn non lié.                                                                                                                                                |
| `Conversation bindings are unavailable for <channel>.`                       | L'adaptateur manque de la capacité de liaison ACP de conversation actuelle.                                                                        | Utilisez `/acp spawn ... --thread ...` si pris en charge, configurez `bindings[]` de premier niveau, ou passez à un canal pris en charge.                                                                                              |
| `--thread here requires running /acp spawn inside an active ... thread`      | `--thread here` utilisé hors du contexte d'un fil de discussion.                                                                                   | Déplacez-vous vers le fil de discussion cible ou utilisez `--thread auto`/`off`.                                                                                                                                                       |
...
| `Thread bindings are unavailable for <channel>.`                             | L'adaptateur manque de la capacité de liaison de fil de discussion.                                                                                | Utilisez `--thread off` ou passez à un adaptateur/canal pris en charge.                                                                                                                                                                |
| `Sandboxed sessions cannot spawn ACP sessions ...`                           | L'exécution ACP est côté hôte ; la session du demandeur est sandboxed.                                                                             | Utilisez `runtime="subagent"` depuis des sessions sandboxées, ou lancez un spawn ACP depuis une session non-sandboxée.                                                                                                                 |
| `sessions_spawn sandbox="require" is unsupported for runtime="acp" ...`      | `sandbox="require"` demandé pour le runtime ACP.                                                                                                   | Utilisez `runtime="subagent"` pour le sandboxing requis, ou utilisez ACP avec `sandbox="inherit"` depuis une session non-sandboxée.                                                                                                    |
| `Cannot apply --model ... did not advertise model support`                   | Le harnais cible n'expose pas de changement de model ACP générique.                                                                                | Utilisez un harnais qui annonce la prise en charge ACP `models`/`session/set_model`, utilisez les références de modèle Codex ACP, ou configurez le modèle directement dans le harnais s'il possède son propre indicateur de démarrage. |
...
| `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`     | `permissionMode` bloque les écritures/exécutions dans une session ACP non interactive.                                                             | Définissez `plugins.entries.acpx.config.permissionMode` sur `approve-all` et redémarrez la passerelle. Voir [Configuration des permissions](/fr/tools/acp-agents-setup#permission-configuration).                                      |
| La session ACP échoue rapidement avec peu de sortie                          | Les invites de permission sont bloquées par `permissionMode`/`nonInteractivePermissions`.                                                          | Vérifiez les journaux de la passerelle (gateway) pour `AcpRuntimeError`. Pour des autorisations complètes, définissez `permissionMode=approve-all` ; pour une dégradation progressive, définissez `nonInteractivePermissions=deny`.    |
...
<Note>
  `Command blocked by PreToolUse hook: Native hook relay unavailable` appartient au relais de hook (hook relay) natif Codex, et non à ACP/acpx. Dans un chat Codex lié, démarrez une nouvelle session avec `/new` ou `/reset` ; si cela fonctionne une fois, puis que l'erreur revient à l'appel d'outil natif suivant, redémarrez le serveur d'application Codex ou la passerelle OpenClaw Gateway au lieu de
...
- [Envoi d'agent (Agent send)](/fr/tools/agent-send)
- [Backends CLI](/fr/gateway/cli-backends)
- [Harnais Codex (Codex harness)](/fr/plugins/codex-harness)
...
- [Sous-agents (Sub-agents)](/fr/tools/subagents)
