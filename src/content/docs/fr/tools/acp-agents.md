...
Si vous demandez à OpenClaw en langage clair de « lancer ceci dans Codex » ou de « démarrer Claude Code dans un fil de discussion », OpenClaw doit acheminer cette demande vers le runtime ACP (et non le runtime natif des sous-agents). Chaque création de session ACP est suivie en tant que [tâche d'arrière-plan](/fr/automation/tasks).
...
- Les nouvelles installations incluent désormais le plugin d'exécution `acpx` groupé activé par défaut.
...
- `/acp spawn codex` : OpenClaw devrait être prêt à amorcer `acpx`, mais l'adaptateur ACP Codex peut encore avoir besoin d'une récupération de première exécution.
...
2. Résolvez le harnais cible demandé (`agentId`, par exemple `codex`).
...
- `--bind here` garde la même surface de chat. Sur Discord, le channel actuel reste le channel actuel.
...
- `--bind here` ne crée pas par lui-même un fil de discussion enfant Discord ou un sujet Telegram.
...
- Si vous lancez vers un agent ACP différent et ne passez pas `--cwd`, OpenClaw hérite par défaut de l'espace de travail **de l'agent cible**, et non de celui du demandeur.
- Si ce chemin d'espace de travail hérité est manquant (`ENOENT`/`ENOTDIR`), OpenClaw revient au répertoire de travail par défaut du backend au lieu de réutiliser silencieusement la mauvaise arborescence.
- Si l'espace de travail hérité existe mais ne peut pas être consulté (par exemple `EACCES`), spawn renvoie la véritable erreur d'accès au lieu d'abandonner `cwd`.
...
- `/acp spawn codex --bind here` : conserver cette discussion, créer ou attacher une session ACP Codex, et y acheminer les futurs messages
...
- Les canaux de discussion/messagerie qui annoncent la prise en charge de la liaison de conversation en cours peuvent utiliser `--bind here` via le chemin de liaison de conversation partagé.
...
- Sur Discord, `--bind here` lie le canal ou le fil actuel en place. `spawnAcpSessions` n'est requis que lorsque OpenClaw a besoin de créer un fil enfant pour `--thread auto|here`.
...
- `resume` et les questions de « nouvelle session » sont des questions de session ACP, et non des questions de canal. Vous pouvez réutiliser ou remplacer l'état d'exécution sans modifier la surface de discussion actuelle.
...
Pour les workflows non éphémères, configurez les liaisons ACP persistantes dans les entrées de premier niveau `bindings[]`.
...
- `bindings[].match` identifie la conversation cible :
  ...
  - Conversation de groupe/MP BlueBubbles : BlueBubbles DM/group chat: `match.channel="bluebubbles"` + `match.peer.id="<handle|chat_id:*|chat_guid:*|chat_identifier:*>"`
    Privilégiez `chat_id:*` ou `chat_identifier:*` pour les liaisons de groupe stables.
  - Conversation de groupe/MP iMessage : iMessage DM/group chat: `match.channel="imessage"` + `match.peer.id="<handle|chat_id:*|chat_guid:*|chat_identifier:*>"`
...
- `agentId` (optionnel) : id du harnais cible ACP. Revient à `acp.defaultAgent` si défini.
- `thread` (optionnel, par défaut `false`) : demande le flux de liaison de thread lorsque pris en charge.
...
- `label` (optionnel) : label orienté opérateur utilisé dans le texte de session/bannière.
...
- `streamTo` (optionnel) : `"parent"` diffuse les résumés de progression de l'exécution ACP initiale vers la session demandeur sous forme d'événements système.
  - Lorsqu'elles sont disponibles, les réponses acceptées incluent `streamLogPath` pointant vers un journal JSONL délimité à la session (`<sessionId>.acp-stream.jsonl`) que vous pouvez suivre pour l'historique de relais complet.
...
- Gardez ce test de fumée sur `mode: "run"` sauf si vous testez intentionnellement
  les sessions ACP persistantes liées aux fils de discussion.
...
- Traitez les tests `mode: "session"` liés aux fils de discussion comme une deuxième passe d'intégration plus riche à partir d'un fil Discord réel ou d'un sujet Telegram.
...
Utilisez `runtime: "subagent"` lorsque vous avez besoin d'une exécution imposée par un bac à sable.
...
`/acp sessions` lit le stockage pour la session liée actuelle ou la session du demandeur. Les commandes qui acceptent les jetons `session-key`, `session-id` ou `session-label` résolvent les cibles via la découverte de session de passerelle, y compris les racines `session.store` personnalisées par agent.
...
- `/acp set <key> <value>` est le chemin générique.
  - Cas spécial : `key=cwd` utilise le chemin de remplacement du cwd.
...
Les nouvelles installations incluent le plugin d'exécution groupé `acpx` activé par défaut, donc ACP
...
Si vous avez désactivé `acpx`, refusé l'accès via `plugins.allow` / `plugins.deny`, ou souhaitez
...
Par défaut, le plugin de backend acpx groupé (`acpx`) utilise le binaire épinglé local au plugin :

1. La commande correspond par défaut au `node_modules/.bin/acpx` local au plugin à l'intérieur du package de plugin ACPX.
...
Lorsque vous installez OpenClaw globalement avec `npm install -g openclaw`, les dépendances d'exécution d'acpx (binaires spécifiques à la plateforme) sont installées automatiquement via un hook postinstall. Si l'installation automatique échoue, la passerelle démarre quand même normalement et signale la dépendance manquante via `openclaw acp doctor`.
...
Les `mcpServers` personnalisés fonctionnent toujours comme avant. Le pont plugin-tools intégré est une commodité d'adhésion supplémentaire, et non un remplacement de la configuration générique du serveur MCP.

### Configuration du délai d'expiration d'exécution

Le plugin `acpx` inclus par défaut définit un délai d'expiration de 120 secondes pour le runtime intégré. Cela donne aux harnais plus lents tels que Gemini CLI suffisamment de temps pour achever le démarrage et l'initialisation de l'ACP. Modifiez cette valeur si votre hôte nécessite une limite de runtime différente :
...
Le plugin `acpx` inclus sonde un agent harnais tout en décidant si le backend du runtime intégré est prêt. Il est par défaut sur `codex`. Si votre déploiement utilise un agent ACP différent par défaut, définissez l'agent de sonde avec le même identifiant :
...
Ces autorisations de harnais ACPX sont distinctes des approbations d'exécution de OpenClaw et distinctes des indicateurs de contournement des fournisseurs de backend CLI tels que le CLI de Claude `--permission-mode bypassPermissions`. Le `approve-all` ACPX est le commutateur de rupture de verre (break-glass) au niveau du harnais pour les sessions ACP.

### `permissionMode`

Contrôle les opérations que l'agent harnais peut effectuer sans invite.

| Valeur          | Comportement                                                                                                      |
| --------------- | ----------------------------------------------------------------------------------------------------------------- |
| `approve-all`   | Approuver automatiquement toutes les écritures de fichiers et les commandes shell.                                |
| `approve-reads` | Approuver automatiquement les lectures uniquement ; les écritures et les exécutions nécessitent une confirmation. |
| `deny-all`      | Refuser toutes les demandes d'autorisation.                                                                       |
...
| `ACP dispatch is disabled by policy (acp.dispatch.enabled=false)`           | Répartition depuis les messages de discussion normaux désactivée.                          | Définissez `acp.dispatch.enabled=true`.                                                                                                                                                                                 |
...
| `--bind here requires running /acp spawn inside an active ... conversation` | `--bind here` utilisé sans conversation active pouvant être liée.                          | Déplacez-vous vers la discussion/le channel cible et réessayez, ou utilisez un génération non liée.                                                                                                                     |
| `Conversation bindings are unavailable for <channel>.`                      | L'adaptateur manque de capacité de liaison ACP pour la conversation actuelle.              | Utilisez `/acp spawn ... --thread ...` lorsque pris en charge, configurez `bindings[]` de premier niveau, ou déplacez-vous vers un channel pris en charge.                                                              |
...
| `Only <user-id> can rebind this channel/conversation/thread.`               | Un autre utilisateur possède la cible de liaison active.                                   | Reliez en tant que propriétaire ou utilisez une autre conversation ou un autre fil de discussion.                                                                                                                       |
| `Thread bindings are unavailable for <channel>.`                            | L'adaptateur ne prend pas en charge la liaison de fil de discussion.                       | Utilisez `--thread off` ou passez à un adaptateur/channel pris en charge.                                                                                                                                               |
| `Sandboxed sessions cannot spawn ACP sessions ...`                          | Le runtime ACP est côté hôte ; la session du demandeur est sandboxed.                      | Utilisez `runtime="subagent"` à partir de sessions sandboxed, ou lancez un spawn ACP à partir d'une session non sandboxed.                                                                                              |
...
| La session ACP bloque indéfiniment après avoir terminé le travail           | Le processus du harnais est terminé mais la session ACP n'a pas signalé la fin.            | Surveillez avec `ps aux \| grep acpx` ; tuez manuellement les processus obsolètes.                                                                                                                                      |
