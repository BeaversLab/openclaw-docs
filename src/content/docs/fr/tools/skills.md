...
la surface de l' qu'enseignent ces compétences.
...
Utilisez-le pour des corrections telles que _"la prochaine fois, vérifiez l'attribution du GIF"_ ou
pour des workflows difficiles à obtenir tels que les listes de contrôle QA médias. Commencez par l'approbation
en attente ; n'utilisez les écritures automatiques que dans les espaces de travail de confiance après avoir
revu ses propositions. Guide complet : [Skill Workshop plugin](/fr/plugins/skill-workshop).

## ClawHub (installation et synchronisation)

[ClawHub](https://clawhub.ai) est le registre public de compétences pour OpenClaw.
Utilisez les commandes natives `openclaw skills` pour la découverte/l'installation/la mise à jour, ou le
CLI `clawhub` distinct pour les workflows de publication/synchronisation. Guide complet :
[ClawHub](/fr/clawhub).
...
Native `openclaw skills install` s'installe dans le répertoire de l'espace de travail actif `skills/`. La CLI `clawhub`CLI distincte s'installe également dans `./skills` sous votre répertoire de travail actuel (ou revient à l'espace de travail OpenClaw configuré). OpenClaw le détecte en tant que `<workspace>/skills` lors de la prochaine session. Les racines de compétences configurées prennent également en charge un niveau de regroupement, tel que `skills/<group>/<skill>/SKILL.md`, afin que les compétences tierces associées puissent être conservées dans un dossier partagé sans analyse récursive large.

Les clients Gateway qui ont besoin d'une livraison privée, autre que ClawHub, peuvent préparer une archive de compétences zip avec `skills.upload.begin`, `skills.upload.chunk` et `skills.upload.commit`, puis installer le téléchargement validé avec `skills.install({ source: "upload", uploadId, slug, force?, sha256? })`. Il s'agit d'un chemin de téléchargement administratif explicite pour les clients de confiance, et non du flux d'installation normal `openclaw skills install <slug>` ou ClawHub. Il est désactivé par défaut et ne fonctionne que lorsque `skills.install.allowUploadedArchives: true` est défini dans `openclaw.json`. Le mode téléchargement installe toujours dans le répertoire `skills/<slug>` de l'espace de travail de l'agent par défaut ; le nom du dossier interne de l'archive est ignoré pour la cible d'installation finale.

Les pages de compétences ClawHub exposent le dernier état de l'analyse de sécurité avant l'installation, avec des pages de détail de l'analyseur pour VirusTotal, ClawScan et l'analyse statique. `openclaw skills install <slug>` reste le seul chemin d'installation ; les éditeurs corrigent les faux positifs via le tableau de bord ClawHub ou `clawhub skill rescan <slug>`.

## Sécurité

<Warning>Traitez les compétences tierces comme du **code non fiable**. Lisez-les avant de les activer. Privilégiez les exécutions en mode pour les entrées non fiables et les outils risqués. Voir [Sandboxing](/fr/gateway/sandboxing) pour les contrôles côté agent.</Warning>

- La découverte de compétences dans l'espace de travail, l'agent de projet et le répertoire supplémentaire n'accepte que les racines de compétences dont le chemin réel résolu reste à l'intérieur de la racine configurée, sauf si `skills.load.allowSymlinkTargets` approuve explicitement une racine cible. Les compétences groupées restent toujours contenues. Les racines gérées `~/.openclaw/skills` et personnelles `~/.agents/skills` peuvent contenir des dossiers de compétences liés par des liens symboliques installés par ClawHub ou un autre gestionnaire de compétences local, mais chaque chemin réel `SKILL.md` doit toujours rester à l'intérieur de son répertoire de compétences résolu.
- Les installations d'archives privées Gateway sont désactivées par défaut. Lorsqu'elles sont explicitement activées, elles nécessitent un téléversement zip validé contenant `SKILL.md` et réutilisent les mêmes protections d'extraction d'archive, de traversée de chemin, de lien symbolique, de forçage et de retour en arrière que les installations de compétences ClawHub. Elles sont régies par `skills.install.allowUploadedArchives` ; les installations normales ClawHub ne nécessitent pas ce paramètre.
- Les installations de dépendances de compétences soutenues par Gateway (`skills.install`, l'intégration et l'interface utilisateur des paramètres de compétences) exécutent l'analyse de code dangereux intégrée avant d'exécuter les métadonnées de l'installateur. Les résultats `critical` bloquent par défaut sauf si l'appelant définit explicitement la substitution dangereuse ; les résultats suspects n'avertissent que.
- `openclaw skills install <slug>` est différent : il télécharge un dossier de compétences ClawHub dans l'espace de travail et n'utilise pas le chemin des métadonnées de l'installateur mentionné ci-dessus.
...
OpenClaw suit la spécification AgentSkills pour la disposition/l'intention. L'analyseur utilisé par l'agent intégré prend en charge uniquement les clés de frontmatter **sur une seule ligne** ; `metadata` doit être un **objet JSON sur une seule ligne**. Utilisez `{baseDir}` dans les instructions pour référencer le chemin du dossier de compétences.

### Clés de frontmatter optionnelles

<ParamField path="homepage" type="string">
  URL affichée sous la forme « Site Web » dans l'interface Skills de macOS. Également pris en charge via `metadata.openclaw.homepage`.
</ParamField>
<ParamField path="user-invocable" type="boolean" default="true">
  Lorsque `true`, la skill est exposée en tant que commande slash utilisateur.
</ParamField>
<ParamField path="disable-model-invocation" type="boolean" default="false">
  Lorsque `true`, OpenClaw garde les instructions de la skill en dehors du prompt normal de l'agent. La skill est toujours installée et peut toujours être exécutée explicitement en tant que commande slash lorsque `user-invocable` est également `true`.
</ParamField>
<ParamField path="command-dispatch" type='"tool"'>
  Lorsqu'il est défini sur `tool`, la commande slash contourne le modèle et distribue directement vers un outil.
</ParamField>
...
OpenClaw filtre les skills au moment du chargement en utilisant `metadata` (JSON sur une seule ligne) :
...
<ParamField path="always" type="boolean">
  Lorsque `true`, toujours inclure le skill (ignorer les autres portes).
</ParamField>
<ParamField path="emoji" type="string">
  Emoji optionnel utilisé par l'interface utilisateur Skills de macOS.
</ParamField>
<ParamField path="homepage" type="string">
  URL optionnel affiché comme « Site Web » dans l'interface utilisateur Skills de macOS.
</ParamField>
<ParamField path="os" type='"darwin" | "linux" | "win32"' >
  Liste optionnelle de plateformes. Si défini, le skill n'est éligible que sur ces systèmes d'exploitation.
</ParamField>
...
<ParamField path="requires.config" type="string[]">
  Liste des chemins `openclaw.json` qui doivent être vérifiables (truthy).
</ParamField>
<ParamField path="primaryEnv" type="string">
  Nom de la variable d'environnement associée à `skills.entries.<name>.apiKey`.
</ParamField>
<ParamField path="install" type="object[]">
  Spécifications d'installation optionnelles utilisées par l'interface utilisateur Skills de macOS (brew/node/go/uv/download).
</ParamField>
...
<Note>Les blocs `metadata.clawdbot` hérités sont toujours acceptés lorsque `metadata.openclaw` est absent, afin que les skills installés plus anciens conservent leurs portes de dépendance et leurs indices d'installation. Les nouveaux skills et ceux mis à jour doivent utiliser `metadata.openclaw`.</Note>

### Notes sur le sandboxing

- `requires.bins` est vérifié sur l'**hôte** au moment du chargement du skill.
- Si un agent est sandboxed, le binaire doit également exister **à l'intérieur du conteneur**. Installez-le via `agents.defaults.sandbox.docker.setupCommand` (ou une image personnalisée). `setupCommand` s'exécute une seule fois après la création du conteneur. Les installations de packages nécessitent également un accès réseau sortant, un système de fichiers racine accessible en écriture et un utilisateur root dans le bac à sable.
- Exemple : la compétence `summarize` (`skills/summarize/SKILL.md`) nécessite le `summarize` CLI dans le conteneur bac à sable pour s'exécuter.
...
<AccordionGroup>
  <Accordion title="Règles de sélection de l'installateur">
    ...
    - Les installations Node honorent `skills.install.nodeManager` dans `openclaw.json` (par défaut : npm ; options : npm/pnpm/yarn/bun). Cela n'affecte que les installations de compétences ; le runtime du Gateway doit toujours être Node - Bun n'est pas recommandé pour WhatsApp/Telegram.
    - La sélection de l'installateur prise en charge par Gateway est basée sur les préférences : lorsque les spécifications d'installation mélangent les types, OpenClaw préfère Homebrew quand `skills.install.preferBrew` est activé et que `brew` existe, puis `uv`, puis le gestionnaire de nœud configuré, puis autres solutions de repli comme `go` ou `download`.
...
<ParamField path="enabled" type="boolean">
  `false` désactive la compétence même si elle est intégrée ou installée. La compétence intégrée `coding-agent` est optionnelle : définissez `skills.entries.coding-agent.enabled: true` avant de l'exposer aux agents, puis assurez-vous que l'un des `claude`, `codex`, `opencode`, ou `pi` est installé et authentifié pour son propre CLI.
</ParamField>
...
<ParamField path="config" type="object">
  Sac facultatif pour les champs personnalisés par compétence. Les clés personnalisées doivent figurer ici.
</ParamField>
<ParamField path="allowBundled" type="string[]">
  Liste d'autorisation facultative pour les compétences **intégrées** uniquement. Si défini, seules les compétences intégrées de la liste sont éligibles (les compétences gérées/espace de travail ne sont pas affectées).
</ParamField>
...
Pour le backend `claude-cli` intégré, OpenClaw matérialise également le même
...
OpenClaw conserve toujours la priorité, les listes d'autorisation par agent, le filtrage et
l'injection de clé env/API `skills.entries.*`. Les autres backends CLI utilisent uniquement
...
Utilisez `allowSymlinkTargets` pour les configurations d'espace de travail, d'agent de projet ou de répertoire supplémentaire
intentionnels où une racine de compétences contient un lien symbolique, par exemple
`<workspace>/skills/manager -> ~/Projects/manager/skills`. Les
`~/.openclaw/skills` gérés et les `~/.agents/skills` personnels peuvent suivre les liens symboliques
de répertoire de compétences provenant de gestionnaires de compétences locaux par défaut, mais la liste cible est toujours
mise en correspondance après la résolution du realpath et doit rester étroite lorsqu'elle est configurée.
...
Cela repose sur le nœud signalant son support de commande et sur une sonde de binaire
via `system.which` ou `system.run`. Les nœuds hors ligne ne rendent **pas**
les compétences exclusivement à distance visibles. Si un nœud connecté cesse de répondre aux sondes
de binaire, OpenClaw efface ses correspondances de binaire mises en cache afin que les agents ne voient plus
les compétences qui ne peuvent pas actuellement s'exécuter là-bas.
...
augmentant la longueur. Les comptes de jetons varient selon le tokeniseur du modèle. Une estimation approximative de style OpenAI est d'environ 4 caractères/jeton, donc **97 caractères ≈ 24 jetons** par compétence plus vos longueurs de champ réelles.

## Cycle de vie des compétences gérées

OpenClaw est fourni avec un ensemble de base de compétences en tant que **compétences groupées** avec l'installation (package npm ou OpenClaw.app). `~/.openclaw/skills` existe pour les remplacements locaux - par exemple, épingler ou corriger une compétence sans changer la copie groupée. Les compétences de l'espace de travail sont détenues par l'utilisateur et remplacent les deux en cas de conflit de noms.

## Vous cherchez plus de compétences ?

Parcourez [https://clawhub.ai](https://clawhub.ai). Schéma de configuration complet : [Configuration des compétences](/fr/tools/skills-config).

## Connexes

- [ClawHub](/fr/clawhub) - registre public de compétences
- [Création de compétences](/fr/tools/creating-skills) - créer des compétences personnalisées
