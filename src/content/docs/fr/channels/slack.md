---
summary: "Configuration et comportement d'exécution de Slack (Socket Mode + URLs de requête HTTP)"
read_when:
...
<CardGroup cols={3}>
  <Card title="Pairing" icon="link" href="/fr/channels/pairing">
    Les MD Slack sont en mode d'appairage par défaut.
  </Card>
...
| Réseau sortant                                     | La WSS sortante vers `wss-primary.slack.com` doit être accessible                                                                                                                                    | Pas de WS sortant ; uniquement HTTPS entrant                                                                                                |
...
`plugins install` enregistre et active le plugin. Le plugin ne fait rien tant que vous n'avez pas configuré l'application Slack et les paramètres de canal ci-dessous. Consultez [Plugins](/fr/tools/plugin) pour le comportement général des plugins et les règles d'installation.
...
```
        ...
        <Note>
          **Recommended** correspond à l'ensemble complet des fonctionnalités du plugin Slack : App Home, commandes slash, fichiers, réactions, épingles, messages de groupe (DMs), et lectures d'émojis/groupes d'utilisateurs. Choisissez **Minimal** lorsque la stratégie de l'espace de travail restreint les portées — il couvre les DMs, l'historique des canaux/groupes, les mentions et les commandes slash mais supprime les fichiers, réactions, épingles, groupe-DM (`mpim:*`), `emoji:read`, et `usergroups:read`. Consultez la [Manifest and scope checklist](#manifest-and-scope-checklist) pour la justification par portée et les options additives comme des commandes slash supplémentaires.
        </Note>

        Une fois que Slack a créé l'application :

        - **Basic Information -> App-Level Tokens -> Generate Token and Scopes** : ajoutez `connections:write`, enregistrez, copiez le jeton de niveau application (App-Level Token).
...
```
        ...
        <Note>
          **Recommended** correspond à l'ensemble complet des fonctionnalités du plugin Slack ; **Minimal** supprime les fichiers, les réactions, les épingles, les DM de groupe (`mpim:*`), `emoji:read` et `usergroups:read` pour les espaces de travail restreints. Consultez la [checklist des manifestes et portées](#manifest-and-scope-checklist) pour la justification par portée.
        </Note>
        ...
        - **Install App -> Install to Workspace** : copiez le Jeton OAuth du Bot User.
...
N'utilisez ceci que pour les espaces de travail en mode Socket qui enregistrent des délais d'expiration de pong/ping de serveur websocket Slack ou qui s'exécutent sur des hôtes avec une famine connue de la boucle d'événements. Slack`clientPingTimeout` est le temps d'attente du pong après l'envoi d'un ping client par le SDK ; `serverPingTimeout`Slack est le temps d'attente pour les pings du serveur Slack. Les messages et événements de l'application restent dans l'état de l'application, et non des signaux de vivacité du transport.

Notes :

- `socketMode` est ignoré en mode URL de requête HTTP.
- Les paramètres de base `channels.slack.socketMode`Slack s'appliquent à tous les comptes Slack, sauf s'ils sont remplacés. Les remplacements par compte utilisent `channels.slack.accounts.<accountId>.socketMode` ; comme il s'agit d'un remplacement d'objet, incluez chaque champ de réglage de socket que vous souhaitez pour ce compte.
- Seul `clientPingTimeout`OpenClaw a une valeur par défaut OpenClaw (`15000`). `serverPingTimeout` et `pingPongLoggingEnabled`Slack sont transmis au SDK Slack uniquement lorsqu'ils sont configurés.
- Le délai de redémarrage en mode Socket commence autour de 2 secondes et plafonne autour de 30 secondes. Les échecs consécutifs récupérables de démarrage/d'attente de démarrage s'arrêtent après 12 tentatives ; après une connexion réussie, les déconnexions récupérables ultérieures commencent un nouveau cycle de tentatives. Les erreurs d'authentification Slack non récupérables telles que Slack`invalid_auth`, les jetons révoqués ou les portées manquantes échouent rapidement au lieu de réessayer indéfiniment.

## Liste de vérification du manifeste et des portées

Le manifeste de base de l'application Slack est le même pour le mode Socket et les URL de requête HTTP. Seul le bloc Slack`settings` (et la commande slash `url`) diffère.
...
Le manifeste par défaut active l'onglet **Home** de l'application Slack et s'abonne à Slack`app_home_opened`OpenClaw. Lorsqu'un membre de l'espace de travail ouvre l'onglet Home, OpenClaw publie une vue Home par défaut sécurisée avec `views.publish`SlackSlack ; aucune charge de conversation ou configuration privée n'est incluse. L'onglet **Messages** reste activé pour les DMs Slack. Le manifeste active également les fils de discussion de l'assistant Slack avec `features.assistant_view`, `assistant:write`, `assistant_thread_started` et `assistant_thread_context_changed`OpenClawSlack ; les fils de discussion de l'assistant sont acheminés vers leurs propres sessions de fil OpenClaw et gardent le contexte de fil fourni par Slack disponible pour l'agent.
...
- `userToken` est uniquement configuré (pas de repli d'env) et par défaut à un comportement en lecture seule (`userTokenReadOnly: true`).
...
<Tip>Pour les actions/lectures de répertoire, le jeton utilisateur peut être préféré lorsqu'il est configuré. Pour les écritures, le jeton bot reste préféré ; les écritures avec jeton utilisateur sont autorisées uniquement lorsque `userTokenReadOnly: false` et que le jeton bot n'est pas disponible.</Tip>
...
<Tabs>
  ...
  <Tab title="Politique de canal">
    ...
    <Warning>
    Les clés basées sur le nom (`#channel-name` ou `channel-name`) ne correspondent **pas** sous `groupPolicy: "allowlist"`. La recherche de canal est basée sur l'ID par défaut, donc une clé basée sur le nom ne routera jamais avec succès et tous les messages de ce canal seront bloqués silencieusement. Cela diffère de `groupPolicy: "open"`, où la clé de canal n'est pas requise pour le routage et une clé basée sur le nom semble fonctionner.

    Utilisez toujours l'ID de canal Slack comme clé. Pour le trouver : faites un clic droit sur le canal dans Slack → **Copy link** (Copier le lien) — l'ID (`C...`) apparaît à la fin de l'URL.
  ...
  <Tab title="Mentions et utilisateurs du channel">
    ...
    `allowBots` est conservateur pour les channels et les channels privés : les messages de salon rédigés par le bot sont acceptés uniquement lorsque le bot expéditeur est explicitement listé dans la liste d'autorisation `users` de ce salon, ou lorsqu'au moins un ID de propriétaire explicite Slack provenant de `channels.slack.allowFrom` est actuellement membre du salon. Les jokers et les entrées de propriétaire par nom d'affichage ne satisfont pas la présence du propriétaire. La présence du propriétaire utilise Slack `conversations.members` ; assurez-vous que l'application dispose de la portée de lecture correspondante pour le type de salon (`channels:read` pour les channels publics, `groups:read` pour les channels privés). Si la recherche de membre échoue, OpenClaw rejette le message de salon rédigé par le bot.

    Les messages Slack acceptés et rédigés par le bot utilisent une [protection commune contre les boucles de bots](/fr/channels/bot-loop-protection). Configurez `channels.defaults.botLoopProtection` pour le budget par défaut, puis remplacez avec `channels.slack.botLoopProtection` ou `channels.slack.channels.<id>.botLoopProtection` lorsqu'un espace de travail ou un channel nécessite une limite différente.
...
- Les messages de channel de niveau ordinaire restent sur la session par channel, même lorsque `replyToMode` est non `off`.
- Les réponses aux fils de discussion Slack utilisent le `thread_ts` Slack parent pour les suffixes de session (`:thread:<threadTs>`), même lorsque le threading des réponses sortantes est désactivé avec `replyToMode="off"`.
- OpenClaw amorce une racine de channel de niveau supérieur éligible dans `agent:<agentId>:slack:channel:<channelId>:thread:<rootTs>` lorsque cette racine est censée démarrer un fil de discussion visible Slack, de sorte que la racine et les réponses ultérieures du fil partagent une session OpenClaw. Cela s'applique aux événements `app_mention`, aux correspondances de bot explicites ou de modèles de mention configurés, et aux channels `requireMention: false` avec un `replyToMode` non `off`.
- Par défaut, `channels.slack.thread.historyScope` est `thread` ; par défaut, `thread.inheritParent` est `false`.
...
Pour les réponses explicites aux fils de discussion Slack provenant de l'outil `message`, définissez `replyBroadcast: true` avec `action: "send"` et `threadId` ou `replyTo` pour demander à Slack de diffuser également la réponse au fil vers le canal parent. Cela correspond au drapeau `reply_broadcast` `chat.postMessage` de Slack et n'est pris en charge que pour les envois de texte ou de Block Kit, et non pour les téléchargements de médias.

Lorsqu'un appel à l'outil `message` s'exécute dans un fil de discussion Slack et cible le même canal, OpenClaw hérite normalement du fil de discussion Slack actuel selon `replyToMode`. Définissez `topLevel: true` sur `action: "send"` ou `action: "upload-file"` pour forcer l'envoi d'un nouveau message dans le canal parent à la place. `threadId: null` est accepté comme la même option de refus de niveau supérieur.

<Note>
`replyToMode="off"` désactive le fil de discussion des réponses sortantes de Slack, y compris les balises explicites `[[reply_to_*]]`. Il n'aplatit pas les sessions de fil de discussion entrantes de Slack : les messages déjà publiés dans un fil de discussion Slack sont toujours routés vers la session `:thread:<threadTs>`. Cela diffère de Telegram, où les balises explicites sont toujours honorées en mode `"off"`. Les fils de discussion Slack masquent les messages du canal, tandis que les réponses Telegram restent visibles en ligne.
</Note>

## Réactions d'accusé de réception

`ackReaction` envoie un emoji d'accusé de réception pendant que OpenClaw traite un message entrant. `ackReactionScope` décide _quand_ cet emoji est réellement envoyé.
...
Le fournisseur Slack lit la portée depuis `messages.ackReactionScope` (par défaut `"group-mentions"`). Il n'y a aujourd'hui aucune substitution au niveau du compte Slack ou du canal Slack ; la valeur est globale pour la passerelle.
...
- `"group-mentions"` (par défaut) : réagir dans les groupes, mais seulement lorsque le bot est mentionné (ou dans les groupes mentionnables qui ont opté pour cela). **Les DMs sont exclus.**
- `"off"` / `"none"` : ne jamais réagir.

<Note>
  La portée par défaut (`"group-mentions"`) ne déclenche pas de réactions d'accusé de réception dans les messages directs. Pour voir la `ackReaction` configurée (par exemple `"eyes"`) sur les DM Slack entrants, définissez `messages.ackReactionScope` sur `"direct"` ou `"all"`. `messages.ackReactionScope` est lu au démarrage du fournisseur Slack, un redémarrage de la passerelle est donc nécessaire
...
- `progress` : afficher le texte d'état de la progression pendant la génération, puis envoyer le texte final.
- `streaming.preview.toolProgress` : lorsque l'aperçu de brouillon est actif, acheminer les mises à jour d'outil/progression dans le même message d'aperçu modifié (par défaut : `true`). Définissez `false` pour conserver des messages d'outil/progression distincts.
- `streaming.preview.commandText` / `streaming.progress.commandText` : définir sur `status` pour conserver des lignes de progression d'outil compactes tout en masquant le texte de commande/exécution brut (par défaut : `raw`).
...
Les cartes de tâches de progression natives Slack sont optionnelles pour le mode progression. Définissez `channels.slack.streaming.progress.nativeTaskCards` sur `true` avec `channels.slack.streaming.mode="progress"` pour envoyer une carte de plan/tâche native Slack pendant l'exécution du travail, puis mettre à jour la même carte de tâche à la fin. Sans cet indicateur, le mode progression conserve le comportement d'aperçu de brouillon portable.
...
- Les racines de canal, de conversation de groupe et de DM de premier niveau peuvent toujours utiliser l'aperçu de brouillon normal lorsque le flux natif n'est pas disponible ou qu'aucun fil de discussion de réponse n'existe.
- Les Slack DMs de premier niveau restent hors fil par défaut, ils n'affichent donc pas l'aperçu de flux/statut natif de style fil de Slack ; OpenClaw publie et modifie un aperçu de brouillon dans le DM à la place.
- Les médias et les charges utiles non textuelles reviennent à la livraison normale.
- Les finales médias/erreur annulent les modifications d'aperçu en attente ; les finales texte/bloc éligibles sont vidées uniquement lorsqu'elles peuvent modifier l'aperçu en place.
...
Activer les cartes de tâche de progression natives Slack :
...
- booléen `channels.slack.streaming` est un alias d'exécution hérité pour `channels.slack.streaming.mode` et `channels.slack.streaming.nativeTransport`.
- hérité `channels.slack.nativeStreaming` est un alias d'exécution pour `channels.slack.streaming.nativeTransport`.
- Exécutez `openclaw doctor --fix` pour réécrire la configuration de flux Slack persistante avec les clés canoniques.

## Solution de repli pour la réaction de frappe

`typingReaction` ajoute une réaction temporaire au message entrant Slack pendant que OpenClaw traite une réponse, puis la supprime une fois l'exécution terminée. Ceci est surtout utile en dehors des réponses en fil, qui utilisent un indicateur d'état « est en train d'écrire... » par défaut.
...
<AccordionGroup>
  ...
  <Accordion title="Texte et fichiers sortants">
    ...
    - l'envoi de fichiers utilise les API de téléchargement Slack et peut inclure des réponses de fils de discussion (`thread_ts`)
    - la limite de média sortant suit `channels.slack.mediaMaxMb` lorsque configuré ; sinon, les envois de channel utilisent les valeurs par défaut de type MIME du pipeline média
  ...
  <Accordion title="Cibles de livraison">
    ...
    Les DMs Slack contenant uniquement du texte ou des blocs peuvent être publiés directement sur les ID utilisateur ; les téléchargements de fichiers et les envois en fil de discussion ouvrent d'abord le DM via les API de conversation Slack car ces chemins nécessitent un ID de conversation concret.
...
Les menus d'arguments natifs utilisent une stratégie de rendu adaptatif qui affiche une modal de confirmation avant d'envoyer une valeur d'option sélectionnée :
...
Pour les nouveaux agents, CLI et les sorties de plugins, préférez les boutons
ou les blocs de sélection partagés `presentation`. Ils utilisent le même chemin d'interaction
...
Ces directives compilent en Slack Block Kit et acheminent les clics ou les sélections
...
Utilisez des payloads `presentation` et `buildSlackPresentationBlocks(...)` pour les nouveaux
...
- Il s'agit d'une interface utilisateur (UI) héritée spécifique à Slack. Les autres canaux ne traduisent pas les
  directives du Slack Block Kit en leurs propres systèmes de boutons.
...
- Si les blocs interactifs générés dépassent les limites du Slack Block Kit, OpenClaw revient à la réponse texte originale au lieu d'envoyer un payload de blocs non valide.
...
de cycle de vie modale `view_submission` et `view_closed` avant que OpenClaw ne compacte
le payload pour l'événement système visible par l'agent. Utilisez l'un de ces modèles de routage
...
Le gestionnaire reçoit `ctx.interaction.kind` en tant que `view_submission` ou
`view_closed`, `inputs` normalisé, et l'objet `stateValues` brut complet de
Slack. Un routage basé uniquement sur l'ID de rappel suffit pour invoquer le gestionnaire de plugin ; incluez
les champs de routage utilisateur/session `private_metadata` de la modale existante lorsque la
...
- Les approbations Exec et plugin peuvent être rendues sous forme de invites Block Kit natives Slack.
- `channels.slack.execApprovals.*` reste l'activation du client d'approbation natif exec et la config de routage DM/channel.
- Les DM d'approbation Exec utilisent `channels.slack.execApprovals.approvers` ou `commands.ownerAllowFrom`.
...
- Les DM d'approbation de plugin utilisent les approbateurs de plugin Slack provenant de `channels.slack.allowFrom`, `allowFrom` de compte nommé, ou la route par défaut du compte.
- L'autorisation de l'approbateur est toujours appliquée : les approbateurs exec-only ne peuvent pas approuver les demandes de plugin à moins qu'ils ne soient également des approbateurs de plugin.

Cela utilise la même surface de bouton d'approbation partagée que les autres canaux. Lorsque `interactivity` est activé dans vos paramètres d'application Slack, les invites d'approbation sont rendues sous forme de boutons Block Kit directement dans la conversation.
Lorsque ces boutons sont présents, ils constituent l'UX d'approbation principal ; OpenClaw
doit inclure uniquement une commande manuelle `/approve` lorsque le résultat de l'outil indique que les approbations de chat ne sont pas disponibles ou que l'approbation manuelle est le seul chemin.
...
Slack active automatiquement les approbations d'exécution natives lorsque Slack`enabled` n'est pas défini ou est `"auto"`SlackSlack et qu'au moins un approuveur d'exécution est résolu. Slack peut également gérer les approbations de plugins natifs via ce chemin native-client lorsque les approuveurs de plugins Slack sont résolus et que la demande correspond aux filtres native-client. Définissez `enabled: false`Slack pour désactiver explicitement Slack en tant que client d'approbation native. Définissez `enabled: true`SlackSlack pour forcer l'activation des approbations natives lorsque les approuveurs sont résolus. La désactivation des approbations d'exécution Slack ne désactive pas la livraison des approbations de plugins Slack natives qui est activée via `approvals.plugin`Slack ; la livraison des approbations de plugins utilise les approuveurs de plugins Slack à la place.
...
Une configuration native explicite de Slack n'est nécessaire que si vous souhaitez remplacer les approuveurs, ajouter des filtres ou opter pour la livraison origin-chat :
...
Le transfert partagé `approvals.exec` est distinct. Ne l'utilisez que lorsque les invites d'approbation d'exécution doivent également être acheminées vers d'autres discussions ou cibles hors bande explicites. Le transfert partagé `approvals.plugin`SlackSlack est également distinct ; la livraison native Slack ne supprime ce repli que lorsque Slack peut gérer la demande d'approbation du plugin de manière native.

Le `/approve`Slack same-chat fonctionne également dans les canaux et les DMs Slack qui prennent déjà en charge les commandes. Consultez [Exec approvals](/fr/tools/exec-approvals) pour connaître le modèle complet de transfert d'approbation.

## Événements et comportement opérationnel

- Les modifications/suppressions de messages sont mappées dans des événements système.
...
- Les événements d'arrivée/départ de membres, de création/renommage de canal et d'ajout/suppression d'épingles sont mappés dans des événements système.
...
- Le lanceur de fil de discussion et l'amorçage initial du contexte de l'historique du fil sont filtrés par les listes d'autorisation des expéditeurs configurées, le cas échéant.
- Les actions de bloc et les interactions modales émettent des `Slack interaction: ...` événements système structurés avec des champs de charge utile riches :
  - actions de bloc : valeurs sélectionnées, étiquettes, valeurs du sélecteur et métadonnées `workflow_*`
  - événements `view_submission` et `view_closed` modaux avec les métadonnées du canal routé et les entrées du formulaire

## Référence de configuration

Référence principale : [Référence de configuration - Slack](/fr/gateway/config-channels#slack).

<Accordion title="SlackChamps de signal élevée Slack">
...
- unfurls : `unfurlLinks` (par défaut : `false`), `unfurlMedia` pour `chat.postMessage` contrôle de prévisualisation des liens/medias ; définissez `unfurlLinks: true` pour réactiver les prévisualisations de liens
...
<AccordionGroup>
  <Accordion title="Pas de réponses dans les canaux">
    ...
    - `groupPolicy`
    - channel allowlist (`channels.slack.channels`) — **les clés doivent être des ID de channel** (`C12345678`), et non des noms (`#channel-name`). Les clés basées sur le nom échouent silencieusement sous `groupPolicy: "allowlist"`Slack car le routage des canaux est basé sur l'ID par défaut. Pour trouver un ID : clic droit sur le channel dans Slack → **Copy link** — la valeur `C...` à la fin de l'URL est l'ID du channel.
    - `requireMention`
    - allowlist `users` par channel
    - `messages.groupChat.visibleReplies` : les requêtes normales de groupe/channel sont `"automatic"` par défaut. Si vous avez opté pour `"message_tool"` et que les journaux montrent du texte d'assistant sans appel `message(action=send)`, le modèle a manqué le chemin visible de l'outil de message. Le texte final reste privé dans ce mode ; inspectez le journal détaillé de la passerelle pour les métadonnées de payload supprimées, ou définissez-le sur `"automatic"` si vous souhaitez que chaque réponse finale normale de l'assistant soit publiée via l'ancien chemin.
    - `messages.groupChat.unmentionedInbound` : s'il est `"room_event"`, les conversations non mentionnées des canaux autorisés sont un contexte ambiant et restent silencieuses à moins que l'agent n'appelle l'outil `message`. Voir [Ambient room events](/fr/channels/ambient-room-events).
...
```
  ...
  <Accordion title="DM messages ignored">
    ...
    - les approbations d'appairage / les entrées de liste d'autorisation (`dmPolicy: "open"` nécessite toujours `channels.slack.allowFrom: ["*"]`)
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    Le jeton de niveau application nécessite `connections:write`, et le jeton bot OAuth de l'utilisateur
    ...
    `appTokenStatus: "configured_unavailable"`, le compte Slack
    est configuré mais le runtime actuel n'a pas pu résoudre la valeur
    soutenue par SecretRef.

    Les journaux tels que `slack socket mode failed to start; retry ...` sont des échecs de démarrage
    récupérables. Les étendues manquantes, les jetons révoqués et les authentifications invalides échouent rapidement.
  ...
  <Accordion title="HTTP mode not receiving events">
    ...
    - les Slack Request URLs (Events + Interactivity + Slash Commands)
    - un `webhookPath` unique par compte HTTP
    - l'URL publique termine le TLS et transfère les demandes vers le chemin du Gateway
  ...
  <Accordion title="Native/slash commands not firing">
    ...
    Slack ne crée ni ne supprime automatiquement les commandes slash. `commands.native: "auto"` n'active pas les commandes natives de Slack ; utilisez `true` et créez les commandes correspondantes dans l'application Slack. En mode HTTP, chaque commande slash Slack doit inclure l'URL du Gateway. En mode Socket, les charges utiles de commande arrivent via le websocket et Slack ignore `slash_commands[].url`.
...
| Fichiers PDF                   | URL de fichier Slack         | Téléchargé et exposé en tant que contexte de fichier pour les outils tels que `download-file` ou `pdf`               | Slack entrant ne convertit pas automatiquement les PDF en entrée de vision par image            |
...
| Réponses de fil de discussion  | Fichiers de démarrage de fil | Les fichiers du message racine peuvent être hydratés en tant que contexte lorsque la réponse n'a pas de média direct | Les démarreurs composés uniquement de fichiers utilisent un espace réservé pour la pièce jointe |
| Messages multi-images          | Fichiers Slack multiples     | Chaque fichier est évalué indépendamment                                                                             | Le traitement Slack est plafonné à huit fichiers par message                                    |

### Pipeline entrant

Lorsqu'un message Slack avec des pièces jointes de fichiers arrive :

1. OpenClaw télécharge le fichier à partir de l'URL privée de Slack à l'aide du jeton du bot.
...
4. Les chemins de modèle/outil capables d'image peuvent utiliser les pièces jointes d'image à partir de ce contexte.
5. Les fichiers non-image restent disponibles en tant que métadonnées de fichier ou références de médias pour les outils capables de les gérer.
...
- Un message racine qui ne contient que des fichiers et aucun texte est représenté par un espace réservé de pièce jointe afin que la solution de repli puisse toujours inclure ses fichiers.

### Gestion des pièces jointes multiples

Lorsqu'un seul message Slack contient plusieurs fichiers joints :

- Chaque pièce jointe est traitée indépendamment via le pipeline média.
- Les références médias téléchargées sont agrégées dans le contexte du message.
...
- **Échecs de téléchargement** : Les fichiers que Slack ne peut pas servir, les URL expirées, les fichiers inaccessibles, les fichiers trop volumineux et les réponses HTML d'authentitation/connexion Slack sont ignorés au lieu d'être signalés comme des formats non pris en charge.
...
| Modèle de vision non configuré                | Les pièces jointes d'images sont stockées sous forme de références médias, mais ne sont pas analysées en tant qu'images | Configurez `agents.defaults.imageModel` ou utilisez un modèle de réponse compatible avec la vision |
...
| Pièces jointes transférées/partagées          | Le texte et les médias image/fichier hébergés par Slack sont traités au mieux effort                                    | Partagez à nouveau directement dans le fil OpenClaw                                                |
| Pièces jointes PDF                            | Stockées en tant que contexte fichier/média, non acheminées automatiquement via la vision par image                     | Utilisez `download-file` pour les métadonnées de fichier ou l'outil `pdf` pour l'analyse PDF       |
