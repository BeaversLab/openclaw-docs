...
- Sessions par groupe : les clés de session ressemblent à `agent:<agentId>:whatsapp:group:<jid>`, de sorte que les commandes telles que `/verbose on`, `/trace on` ou `/think high` (envoyées en tant que messages autonomes) sont limitées à ce groupe ; l'état du DM personnel reste inchangé. Les signaux de présence (heartbeats) sont ignorés pour les fils de groupe.
...
Seul le numéro du propriétaire (provenant de `channels.whatsapp.allowFrom`, ou le propre E.164 du bot s'il n'est pas défini) peut modifier cela. Envoyez `/status` comme un message autonome dans le groupe pour voir le mode d'activation actuel.
