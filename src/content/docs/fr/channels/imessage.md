...
<Note>
Pour les déploiements OpenClaw iMessage, utilisez `imsg` sur un hôte macOS Messages connecté. Si votre Gateway fonctionne sur Linux ou Windows, pointez `channels.imessage.cliPath` vers un wrapper SSH qui exécute `imsg` sur le Mac.

**Le rattrapage lors de l'indisponibilité du Gateway est optionnel.** Lorsqu'il est activé (`channels.imessage.catchup.enabled: true`), la passerine relit les messages entrants qui ont atterri dans `chat.db` pendant qu'elle était hors ligne (plantage, redémarrage, mise en veille du Mac) au prochain démarrage. Désactivé par défaut — voir [Catching up after gateway downtime](#catching-up-after-gateway-downtime). Corrige [openclaw#78649](https://github.com/openclaw/openclaw/issues/78649).

</Note>

<Warning>La prise en charge de BlueBubbles a été supprimée. Migrez les configurations `channels.bluebubbles` vers `channels.imessage` ; OpenClaw prend en charge iMessage uniquement via `imsg`.</Warning>

Statut : intégration externe native de CLI. Le Gateway lance `imsg rpc` et communique via JSON-RPC sur stdio (pas de démon/port distinct). Les actions avancées nécessitent `imsg launch` et une sonde réussie de l'API privée.
...
- Pour les actions avancées (réagir / modifier / annuler l'envoi / réponse en fil / effets / opérations de groupe), la protection de l'intégrité du système doit être désactivée — voir [Activation de l'API privée imsg](#enabling-the-imsg-private-api) ci-dessous. L'envoi et la réception de texte et de média basiques fonctionnent sans cela.
...
`imsg` fonctionne selon deux modes opérationnels :

- **Mode basique** (par défaut, aucune modification SIP nécessaire) : texte et média sortants via `send`, surveillance/historique entrants, liste de discussion. C'est ce que vous obtenez par défaut avec une nouvelle installation de `brew install steipete/tap/imsg` plus les permissions standard macOS ci-dessus.
- **Mode API privée** : API`imsg` injecte une dylib d'assistance dans `Messages.app` pour appeler les fonctions internes de `IMCore`. C'est ce qui déverrouille `react`, `edit`, `unsend`, `reply` (en fil de discussion), `sendWithEffect`, `renameGroup`, `setGroupIcon`, `addParticipant`, `removeParticipant`, `leaveGroup`, ainsi que les indicateurs de frappe et les accusés de réception.

Pour accéder à la surface d'action avancée documentée sur cette page de canal, vous avez besoin du mode API privée. Le README de API`imsg` est explicite quant à cette exigence :

> Les fonctionnalités avancées telles que `read`, `typing`, `launch`, l'envi enrichi pris en charge par un pont, la mutation de messages et la gestion des discussions sont optionnelles. Elles nécessitent que le SIP soit désactivé et qu'une dylib d'assistance soit injectée dans `Messages.app`. `imsg launch` refuse d'injecter lorsque le SIP est activé.

La technique d'injection d'assistance utilise la propre dylib de `imsg` pour atteindre les API privées de Messages. Il n'y a pas de serveur tiers ou d'exécution BlueBubbles dans le chemin OpenClaw iMessage.
...
1. **Installez (ou mettez à niveau) `imsg`** sur le Mac qui exécute Messages.app :
   ...
   La sortie `imsg status --json` signale `bridge_version`, `rpc_methods` et les `selectors` par méthode, afin que vous puissiez voir ce que la version actuelle prend en charge avant de commencer.
...
2. **Désactivez la protection de l'intégrité du système (System Integrity Protection).** Ceci est spécifique à la version de macOS car la condition sous-jacente d'Apple dépend du système d'exploitation et du matériel :
   - **macOS 10.13–10.15 (Sierra–Catalina) :** désactivez la validation de la bibliothèque via le Terminal, redémarrez en mode récupération, exécutez `csrutil disable`, puis redémarrez.
   - **macOS 11+ (Big Sur et versions ultérieures), Intel :** Mode récupération (ou récupération Internet), `csrutil disable`, redémarrez.
   - **macOS 11+, Apple Silicon :** séquence de démarrage via le bouton d'alimentation pour entrer en mode récupération ; sur les versions récentes de macOS, maintenez la touche **Maj gauche** lorsque vous cliquez sur Continuer, puis exécutez `csrutil disable`. Les configurations de machine virtuelle suivent un processus distinct — commencez par prendre un instantané de la VM.
   - **macOS 26 / Tahoe :** les stratégies de validation de bibliothèque et les vérifications d'entitlements privés de `imagent` se sont encore resserrées ; `imsg` peut avoir besoin d'une version mise à jour pour suivre. Si l'injection `imsg launch` ou des `selectors` spécifiques commencent à renvoyer false après une mise à niveau majeure de macOS, vérifiez les notes de version de `imsg` avant de supposer que l'étape SIP a réussi.
...
3. **Injectez l'assistant.** Avec SIP désactivé et Messages.app connecté :
   ...
   `imsg launch` refuse d'injecter tant que SIP est activé, ce qui sert également de confirmation que l'étape 2 a été effectuée.
...
4. **Vérifiez le pont depuis OpenClaw :**
   ...
   L'entrée iMessage doit signaler iMessage`works`, et `imsg status --json | jq '.selectors'` doit afficher `retractMessagePart: true`macOSOpenClaw ainsi que les sélecteurs d'édition, de frappe et de lecture que votre build macOS expose. Le filtrage par méthode du plugin OpenClaw dans `actions.ts` n'annonce que les actions dont le sélecteur sous-jacent est `true`, donc la surface d'action que vous voyez dans la liste des outils de l'agent reflète ce que le pont peut réellement faire sur cet hôte.

Si `openclaw channels status --probe` signale le canal comme `works`iMessage mais que des actions spécifiques renvoient « iMessage `<action>`API nécessite le pont de l'API privée imsg » au moment de l'envoi, exécutez `imsg launch` à nouveau — l'assistant peut se désactiver (redémarrage de Messages.app, mise à jour du système d'exploitation, etc.) et le statut `available: true` mis en cache continuera d'annoncer des actions jusqu'à la prochaine actualisation de la sonde.
...
- Vous pouvez faire fonctionner un Mac non Apple Silicon séparé (ou un Mac bot dédié) avec SIP désactivé pour la charge de travail iMessage, tout en gardant SIP activé sur vos appareils principaux. Voir [Utilisateur macOS de bot dédié (identité iMessage séparée)](iMessagemacOSiMessage#deployment-patterns) ci-dessous.
...
<Tabs>
  <Tab title="Stratégie de DM">
    ...
    Les entrées de la liste d'autorisation peuvent être des identifiants, des groupes d'accès d'expéditeurs statiques (`accessGroup:<name>`) ou des cibles de discussion (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`).
  ...
  <Tab title="Stratégie de groupe + mentions">
    ...
    Fallback à l'exécution : si `groupAllowFrom`iMessage n'est pas défini, les vérifications d'expéditeur de groupe iMessage reviennent à `allowFrom` si disponible.
    ...
    <Warning>
    Le routage de groupe possède **deux** portes de liste d'autorisation fonctionnant en série, et les deux doivent être validées :
    ...
    Si la porte 2 est vide, tous les messages de groupe sont supprimés. Le plugin émet deux signaux de niveau `warn` au niveau de journalisation par défaut :
    ...
    Si ces lignes `warn` apparaissent dans le journal de la passerelle, la porte 2 supprime les messages — ajoutez le bloc `groups`iMessage.
    </Warning>

    Filtrage des mentions pour les groupes :

    - iMessage ne possède pas de métadonnées de mention natives
    - la détection de mention utilise des motifs regex (`agents.list[].groupChat.mentionPatterns`, fallback `messages.groupChat.mentionPatterns`)
    ...
    Chaque entrée sous `channels.imessage.groups.*` accepte une chaîne `systemPrompt` facultative. La valeur est injectée dans le prompt système de l'agent à chaque tour traitant un message dans ce groupe. La résolution miroir la résolution de prompt par groupe utilisée par `channels.whatsapp.groups` :
  ...
  <Tab title="Sessions and deterministic replies">
    ...
    - Avec `session.dmScope=main`iMessage par défaut, les DMs iMessage fusionnent dans la session principale de l'agent.
    ...
    Si ce `chat_id` est explicitement configuré sous `channels.imessage.groups`OpenClaw, OpenClaw le traite comme du trafic de groupe (group gating + isolation de session de groupe).
...
Consultez [ACP Agents](/fr/tools/acp-agents) pour le comportement de liaison ACP partagé.
...
<AccordionGroup>
  ...
  <Accordion title="Formats d'adressage">
    ...
    Les cibles de gestion (handles) sont également prises en charge :
...
<AccordionGroup>
  <Accordion title="Actions disponibles">
    - **react** : Ajouter/supprimer des tapbacks iMessage (iMessage, `messageId`, `emoji`, `remove`). Les tapbacks pris en charge correspondent à love, like, dislike, laugh, emphasize et question.
    ...
    - **sendWithEffect** : Envoyer du texte avec un effet iMessage (iMessage, `text` ou `message`, `effect` ou `effectId`).
    - **edit** : Modifier un message envoyé sur les versions macOS/API privée prises en charge (macOS, API, `messageId`, `text` ou `newText`).
    - **unsend** : Rétracter un message envoyé sur les versions macOS/API privée prises en charge (macOS, API, `messageId`).
  ...
  <Accordion title="Accusés de lecture et indication de frappe">
    Lorsque le pont de l'API privée est actif, les conversations entrantes acceptées sont marquées comme lues avant répartition et une bulle de frappe est affichée à l'expéditeur pendant que l'agent génère sa réponse. Désactivez le marquage comme lu avec :
    ...
    Les anciennes versions de `imsg` antérieures à la liste des fonctionnalités par méthode désactiveront silencieusement la frappe/la lecture ; OpenClaw enregistre un avertissement unique par redémarrage afin que l'absence d'accusé de réception soit attribuable.
...
iMessage permet les écritures de configuration initiées par le canal par défaut (pour `/config set|unset` lorsque `commands.config: true`).
...
## Fusion des DMs d'envoi fractionné (commande + URL dans une seule composition)
...
Les deux lignes arrivent chez OpenClaw à environ 0,8-2,0 s d'intervalle sur la plupart des configurations. Sans fusion, l'agent reçoit la commande seul au tour 1, répond (souvent « envoyez-moi l'URL »), et ne voit l'URL qu'au tour 2 — moment auquel le contexte de la commande est déjà perdu. C'est le pipeline d'envoi d'Apple, et non quelque chose introduit par OpenClaw ou `imsg`.

`channels.imessage.coalesceSameSenderDms` permet à un DM de fusionner les lignes consécutives du même expéditeur en un seul tour d'agent. Les discussions de groupe continuent d'être réparties par message afin de préserver la structure des tours multi-utilisateurs.

<Tabs>
  <Tab title="Quand activer">
    Activer lorsque :

    - Vous diffusez des compétences qui attendent `command + payload` dans un seul message (dump, paste, save, queue, etc.).
    - Vos utilisateurs collent des URL, des images ou du contenu long à côté des commandes.
    - Vous pouvez accepter la latence supplémentaire du tour DM (voir ci-dessous).
  ...
  <Tab title="Activation">
    ...
    Avec le drapeau activé et sans `messages.inbound.byChannel.imessage` explicite, la fenêtre de débounce s'élargit à **2500 ms** (la valeur par défaut héritée est 0 ms — pas de débounce). La fenêtre élargie est nécessaire car le cadencement d'envoi fractionné d'Apple de 0,8 à 2,0 s ne rentre pas dans une valeur par défaut plus serrée.
  ...
  <Tab title="Compromis">
    - **Latence ajoutée pour les messages DM.** Avec le drapeau activé, chaque DM (y compris les commandes de contrôle autonomes et les suivis de texte unique) attend jusqu'à la fenêtre de débounce avant l'envoi, au cas où une ligne de payload arriverait. Les messages de chat de groupe conservent un envoi instantané.
    - **La sortie fusionnée est limitée.** Le texte fusionné est plafonné à 4000 caractères avec un marqueur `…[truncated]` explicite ; les pièces jointes sont plafonnées à 20 ; les entrées source sont plafonnées à 10 (le premier et les plus récents sont conservés au-delà). Chaque GUID source est suivi dans `coalescedMessageGuids` pour la télémétrie en aval.
    - **DM uniquement.** Les chats de groupe passent directement à l'envoi par message pour que le bot reste réactif lorsque plusieurs personnes écrivent.
...
| Utilisateur compose                                                                          | `chat.db` produit            | Drapeau désactivé (par défaut)                        | Drapeau activé + fenêtre de 2500 ms                                                |
| -------------------------------------------------------------------------------------------- | ---------------------------- | ----------------------------------------------------- | ---------------------------------------------------------------------------------- |
...
| Déluge rapide (>10 petits DM dans la fenêtre)                                                | N lignes                     | N tours                                               | Un tour, sortie limitée (premier + dernier, limites texte/pièce jointe appliquées) |
...
Lorsque la passerelle est hors ligne (plantage, redémarrage, mise en veille du Mac, machine éteinte), `imsg watch` reprend à partir de l'état actuel de `chat.db` une fois la passerelle de retour en ligne — tout ce qui est arrivé pendant l'intervalle est, par défaut, jamais vu. Le rattrapage rejoue ces messages au prochain démarrage pour que l'agent ne manque pas silencieusement le trafic entrant.
...
Une passe par démarrage de `monitorIMessageProvider`, séquencée comme `imsg launch` prêt → `watch.subscribe` → `performIMessageCatchup` → boucle de dispatch en direct. Le rattrapage utilise lui-même `chats.list` + `messages.history` par chat sur le même client JSON-RPC utilisé par `imsg watch`. Tout ce qui arrive pendant la passe de rattrapage passe normalement par le dispatch en direct ; le cache de déduplication entrant existant absorbe tout chevauchement avec les lignes rejouées.

Chaque ligne rejouée est acheminée via le chemin de dispatch en direct (`evaluateIMessageInbound` + `dispatchInboundMessage`), donc les listes autorisées, la stratégie de groupe, le débouncer, le cache d'écho et les accusés de réception se comportent de manière identique pour les messages rejoués et en direct.

### Sémantique du curseur et de la réessaye

Le rattrapage conserve un curseur par compte à `<openclawStateDir>/imessage/catchup/<account>__<hash>.json` (le répertoire d'état OpenClaw est `~/.openclaw` par défaut, remplaçable par `OPENCLAW_STATE_DIR`) :
...
- Le curseur avance à chaque répartition réussie et est bloqué lorsque la répartition d'une ligne échoue — le prochain démarrage réessaie la même ligne à partir du curseur bloqué.
- Après `maxFailureRetries` échecs consécutifs sur le même `guid`, le rattrapage enregistre un `warn` et force l'avancement du curseur au-delà du message bloqué afin que les démarrages suivants puissent progresser.
- Les guid déjà abandonnés sont ignorés à vue (sans tentative de répartition) lors des exécutions ultérieures et comptabilisés sous `skippedGivenUp` dans le résumé d'exécution.
...
Une ligne `WARN ... capped to perRunLimit` signifie qu'un seul démarrage n'a pas vidé l'intégralité de l'arriéré. Augmentez `perRunLimit` (max 500) si vos écarts dépassent régulièrement la passe par défaut de 50 lignes.
...
Lorsque vous activez le rattrapage, le premier démarrage sans curseur ne remonte que `firstRunLookbackMinutes` (30 min par défaut), et non la fenêtre complète `maxAgeMinutes` — cela évite de rejouer une longue histoire de messages pré-activation.
...
<AccordionGroup>
  <Accordion title="imsg introuvable ou RPC non pris en charge">
    ...
    Si la sonde indique que le RPC n'est pas pris en charge, mettez à jour `imsg`. Si les actions de l'API privée sont indisponibles, exécutez `imsg launch` dans la session utilisateur macOS connecté et sondez à nouveau. Si le Gateway ne fonctionne pas sur macOS, utilisez plutôt la configuration Mac à distance via SSH décrite ci-dessus au lieu du chemin local `imsg` par défaut.

  </Accordion>

  <Accordion title="GatewaymacOSLe Gateway ne fonctionne pas sur macOS">
    Le `cliPath: "imsg"`LinuxWindows par défaut doit s'exécuter sur le Mac connecté à Messages. Sur Linux ou Windows, définissez `channels.imessage.cliPath` sur un script wrapper qui SSH vers ce Mac et exécute `imsg "$@"`.
...
- [Configuration du Gateway](Gateway/en/gateway/configuration)
- [Appariement](/en/channels/pairing)

## Connexes

- [Aperçu des canaux](/en/channels) — tous les canaux pris en charge
- [Venant de BlueBubbles](/en/channels/imessage-from-bluebubbles) — tableau de traduction de configuration et bascule étape par étape
- [Jumelage](/en/channels/pairing) — authentification DM et flux de jumelage
