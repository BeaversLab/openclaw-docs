...
Les versions actuelles d'OpenClaw incluent BlueBubbles, donc les versions empaquetées standard n'ont pas besoin d'une étape `openclaw plugins install` séparée.
...
- Le jumelage/la liste d'autorisation fonctionne de la même manière que pour les autres canaux (`/channels/pairing` etc) avec `channels.bluebubbles.allowFrom` + codes de jumelage.
...
- L'authentification par webhook est toujours requise. OpenClaw rejette les requêtes webhook BlueBubbles à moins qu'elles n'incluent un mot de passe/guid correspondant à `channels.bluebubbles.password` (par exemple `?password=<password>` ou `x-password`), quelle que soit la topologie de bouclage/proxy.
...
- La première exécution peut déclencher des invites d'**Automatisation** de macOS (`osascript` → Messages). Approuvez-les dans la même session utilisateur qui exécute le LaunchAgent.
...
Les webhooks de groupe BlueBubbles n'incluent souvent que les adresses brutes des participants. Si vous souhaitez que le contexte `GroupMembers` affiche plutôt les noms des contacts locaux, vous pouvez activer l'enrichissement des Contacts locaux sur macOS :
...
## Liens de conversation ACP

Les discussions BlueBubbles peuvent être transformées en espaces de travail ACP durables sans changer la couche de transport.

Flux rapide pour l'opérateur :

- Exécutez `/acp spawn codex --bind here` dans le DM ou le discussion de groupe autorisé.
- Les futurs messages dans cette même conversation BlueBubbles sont acheminés vers la session ACP générée.
- `/new` et `/reset` réinitialisent la même session ACP liée sur place.
- `/acp close` ferme la session ACP et supprime le lien.

Les liens persistants configurés sont également pris en charge via des entrées `bindings[]` de premier niveau avec `type: "acp"` et `match.channel: "bluebubbles"`.
...
Pour des liens de groupe stables, préférez `chat_id:*` ou `chat_identifier:*`.
...
- **Indicateurs de frappe** : OpenClaw envoie des événements de début de frappe ; BlueBubbles efface automatiquement l'indication de frappe lors de l'envoi ou de l'expiration du délai (l'arrêt manuel via DELETE n'est pas fiable).
...
- **setGroupIcon** : Définir l'icône/photo d'une conversation de groupe (`chatGuid`, `media`) — instable sur macOS 26 Tahoe (l'API peut renvoyer un succès mais l'icône ne se synchronise pas).
...
- Les IDs courts sont en mémoire ; ils peuvent expirer lors d'un redémarrage ou d'une purge du cache.
- Les actions acceptent les IDs courts ou complets `messageId`, mais les IDs courts provoqueront une erreur s'ils ne sont plus disponibles.
...
- `channels.bluebubbles.enrichGroupParticipantsFromContacts` : Sur macOS, enrichissez facultativement les participants de groupe sans nom à partir des Contacts locaux après la validation. Par défaut : `false`.
...
- Handles directs : `+15555550123`, `user@example.com`
  - Si un handle direct n'a pas de conversation DM existante, OpenClaw en créera une via `POST /api/v1/chat/new`. Cela nécessite que l'API privée BlueBubbles soit activée.
...
- Si les événements de frappe/lecture cessent de fonctionner, vérifiez les journaux du webhook BlueBubbles et assurez-vous que le chemin de la passerelle correspond à `channels.bluebubbles.webhookPath`.
...
- Les mises à jour de l'icône de groupe peuvent être instables sur macOS 26 (Tahoe) : l'API peut renvoyer un succès mais la nouvelle icône ne se synchronise pas.
