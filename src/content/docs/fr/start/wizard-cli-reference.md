...
<Steps>
  ...
  <Step title="Modèle et authentification">
    - La matrice complète des options est dans [Auth and model options](#auth-and-model-options).
  </Step>
  <Step title="Workspace">
    ...
    - Organisation de l'espace de travail : [Espace de travail de l'agent](/en/concepts/agent-workspace).
  </Step>
  <Step title="Gateway">
  ...
  <Step title="Canaux">
    ...
    - [Signal](/en/channels/signal) : installation facultative de `signal-cli` + configuration de compte
    - [BlueBubbles](/en/channels/bluebubbles) : recommandé pour iMessage ; URL du serveur + mot de passe + webhook
    - [iMessage](/en/channels/imessage) : chemin du `imsg` CLI hérité + accès à la base de données
    - Sécurité des DM : la valeur par défaut est le jumelage (pairing). Le premier DM envoie un code ; approuvez via
      `openclaw pairing approve <channel> <code>` ou utilisez des listes d'autorisation (allowlists).
  </Step>
...
<Note>Si aucune interface graphique n'est détectée, l'assistant imprime les instructions de transfert de port SSH pour l'interface de contrôle au lieu d'ouvrir un navigateur. Si les ressources de l'interface de contrôle sont manquantes, l'assistant tente de les construire ; le repli est `pnpm ui:build` (installe automatiquement les dépendances de l'interface).</Note>
...
<Note>- Si la passerelle est uniquement en boucle locale, utilisez un tunnel SSH ou un tailnet. - Indices de découverte : - macOS : Bonjour (`dns-sd`) - Linux : Avahi (`avahi-browse`)</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Anthropic Claude CLI">
    Réutilise une connexion Claude CLI locale sur l'hôte de la passerelle et bascule la sélection du modèle
    vers `claude-cli/...`.
  ...
  <Accordion title="Jeton Anthropic (setup-token paste)">
  ...
  <Accordion title="xAI (Grok) clé API">
  ...
  <Accordion title="MiniMax">
    La configuration est écrite automatiquement. La valeur par défaut hébergée est `MiniMax-M2.7` ; `MiniMax-M2.5` reste disponible.
    Plus de détails : [MiniMax](/en/providers/minimax).
  </Accordion>
  <Accordion title="Synthétique (compatible Anthropic)">
    Invite pour `SYNTHETIC_API_KEY`.
  ...
  <Accordion title="Moonshot et Kimi Coding">
    ...
    Pour plus de détails : [Moonshot AI (Kimi + Kimi Coding)](/en/providers/moonshot).
  </Accordion>
...
- OAuth identifiants : `~/.openclaw/credentials/oauth.json`
...
- `--secret-input-mode ref` active le mode de référence au lieu du stockage de clé en texte brut.
  ...
  - réf de fournisseur configurée (`file` ou `exec`) avec l'alias et l'ID du fournisseur
...
- En mode non interactif, `--secret-input-mode ref` est basé uniquement sur les variables d'environnement.
  ...
  - Les indicateurs de clé en ligne (par exemple `--openai-api-key`) nécessitent que cette env var soit définie ; sinon, l'onboarding échoue rapidement.
...
<Note>Conseil pour les serveurs et les installations sans interface graphique : complétez OAuth sur une machine disposant d'un navigateur, puis copiez `~/.openclaw/credentials/oauth.json` (ou `$OPENCLAW_STATE_DIR/credentials/oauth.json`) vers l'hôte de la passerelle.</Note>
...
- Automatisation et scripts : [Automatisation CLI](/en/start/wizard-cli-automation)
