...
<Tip>Vérifiez votre version de Node avec `node --version`. **Utilisateurs Windows :** le Windows natif et WSL2 sont pris en charge. WSL2 est plus stable et recommandé pour une expérience complète. Voir [Windows](/fr/platforms/windows). Besoin d'installer Node ? Voir [Configuration de Node](/fr/install/node).</Tip>
