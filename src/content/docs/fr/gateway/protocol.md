...
- Les trames pré-connexion sont limitées à 64 Ko. Après une poignée de main réussie, les clients
  ...
  les tailles, les limites, les surfaces et les codes de raison sûrs. Ils ne conservent pas le corps du
...
Lorsqu'aucun jeton d'appareil n'est émis, `hello-ok.auth` peut tout de même rapporter les
...
Pendant le transfert du bootstrap de confiance, `hello-ok.auth` peut également inclure des entrées de rôle
...
Bootstrap de l'opérateur (`operator.approvals`, `operator.read`,
`operator.talk.secrets`, `operator.write`). Les vérifications de portée du bootstrap restent
...
`node.pair.approve` dispose également d'une vérification de portée supplémentaire au moment de l'approbation en plus de
la portée de méthode de base :
...
- `caps` : catégories de fonctionnalités de haut niveau.
- `commands` : liste d'autorisation de commande pour l'appel.
...
- **Les trames de chat, d'agent et de résultat d'outil** (y compris les événements `agent` diffusés et les résultats d'appels d'outils) nécessitent au moins `operator.read`. Les sessions sans `operator.read` ignorent entièrement ces trames.
...
Cette page n'est pas une sauvegarde complète générée, mais la surface publique WS est plus large
que les exemples de négociation/authentification ci-dessus. Voici les principales familles de méthodes que le
...
Traitez-la comme une découverte de fonctionnalités, et non comme une sauvegarde générée de chaque assistant callable
...
- `diagnostics.stability` renvoie l'enregistreur de stabilité diagnostique récent et limité. Il conserve des métadonnées opérationnelles telles que les noms d'événements, les comptes, les tailles en octets, les lectures de mémoire, l'état de la file d'attente/session, les noms de channel/plugin et les identifiants de session. Il ne conserve pas le texte des conversations, les corps de webhook, les sorties d'outils, les corps de requêtes ou de réponses brutes, les jetons, les cookies ou les valeurs secrètes. Le scope de lecture opérateur est requis.
- `status` renvoie le résumé de passerelle de style `/status` ; les champs sensibles ne sont inclus que pour les clients opérateurs avec un scope admin.
- `gateway.identity.get` renvoie l'identité de l'appareil de passerelle utilisé par les flux de relais et d'appariement.
...
- `last-heartbeat` renvoie le dernier événement de heartbeat persistant.
...
- `sessions.usage.timeseries` renvoie l'utilisation en série chronologique pour une session.
...
- `channels.logout` déconnecte un compte/channel spécifique lorsque le channel prend en charge la déconnexion.
- `web.login.start` lance un flux de connexion QR/web pour le provider web actuel compatible QR.
- `web.login.wait` attend que ce flux de connexion QR/web se termine et démarre le channel en cas de succès.
- `push.test` envoie une notification push de test APNs à un nœud iOS enregistré.
...
- `send` est la RPC de livraison sortante directe pour les envois ciblés sur channel/account/thread en dehors du chat runner.
- `logs.tail` retourne la fin du fichier journal de la passerelle configurée avec les contrôles curseur/limite et octet max.

### Talk et TTS

- `talk.config` retourne la charge utile de configuration Talk effective ; `includeSecrets`
...
- `talk.speak` synthétise la parole via le provider de parole Talk actif.
- `tts.status` retourne l'état activé du TTS, le provider actif, les providers de secours,
  et l'état de la configuration du provider.
- `tts.providers` retourne l'inventaire visible des providers TTS.
- `tts.enable` et `tts.disable` basculent l'état des préférences TTS.
- `tts.setProvider` met à jour le provider TTS préféré.
- `tts.convert` exécute une conversion synthèse vocale ponctuelle.

### Secrets, configuration, mise à jour et assistant

- `secrets.reload` résout à nouveau les SecretRefs actifs et échange l'état des secrets d'exécution
  uniquement en cas de succès complet.
- `secrets.resolve` résout les affectations de secrets cible de commande pour un ensemble
  commande/cible spécifique.
- `config.get` retourne l'instantané de configuration actuel et son hachage.
...
- `config.schema` renvoie la charge utile du schéma de configuration en direct utilisée par l'interface de contrôle et les outils CLI : schéma, `uiHints`, version et métadonnées de génération, y compris les métadonnées de schéma de plugin + de canal lorsque le runtime peut les charger. Le schéma comprend des métadonnées `title` / `description` dérivées des mêmes étiquettes et textes d'aide que ceux utilisés par l'interface, y compris les branches de composition d'objet imbriqué, de caractère générique, d'élément de tableau et de `anyOf` / `oneOf` / `allOf` lorsque la documentation du champ correspondant existe.
- `config.schema.lookup` renvoie une charge utile de recherche délimitée par un chemin pour un chemin de configuration : chemin normalisé, un nœud de schéma superficiel, indice correspondant + `hintPath`, et résumés des enfants immédiats pour le forage dans l'interface/CLI.
  - Les nœuds de schéma de recherche conservent la documentation orientée utilisateur et les champs de validation courants :
    ...
    limites numériques/chaîne/tableau/objet, et indicateurs booléens tels que
    `additionalProperties`, `deprecated`, `readOnly`, `writeOnly`.
  - Les résumés des enfants exposent `key`, `path` normalisé, `type`, `required`,
    `hasChildren`, ainsi que `hint` / `hintPath` correspondants.
...
- `agents.files.list`, `agents.files.get` et `agents.files.set` gèrent les
  fichiers d'amorçage de l'espace de travail exposés pour un agent.
...
- `sessions.subscribe` et `sessions.unsubscribe` activent ou désactivent les abonnements aux événements de
...
- `sessions.preview` renvoie des aperçus de transcription limités pour des clés de
  session spécifiques.
...
- l'exécution du chat utilise toujours `chat.history`, `chat.send`, `chat.abort` et
  `chat.inject`.
- `chat.history` est normalisé pour l'affichage sur les clients d'interface utilisateur : les balises de directive en ligne sont supprimées du texte visible, les charges utiles XML d'appels d'outil en texte brut (y compris `<tool_call>...</tool_call>`, `<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`, `<function_calls>...</function_calls>`, et les blocs d'appels d'outil tronqués) et les jetons de contrôle de model ASCII/full-width fuités sont supprimés, les lignes d'assistant purement silencieuses telles que exact `NO_REPLY` / `no_reply` sont omises, et les lignes trop volumineuses peuvent être remplacées par des espaces réservés.
...
- `node.canvas.capability.refresh` actualise les jetons de capacité de portée délimitée (scoped).
...
- `exec.approval.request`, `exec.approval.get`, `exec.approval.list` et
  `exec.approval.resolve` couvrent les demandes d'approbation d'exécution ponctuelles ainsi que la recherche et la relecture des approbations en attente.
...
- `plugin.approval.request`, `plugin.approval.list`,
  ...
  les flux d'approbation définis par des plugins.

#### Autres familles principales

- automatisation :
  - `wake` planifie une injection de texte de réveil immédiate ou lors du prochain battement de cœur
...
- `chat` : mises à jour du chat de l'interface utilisateur telles que `chat.inject` et autres événements de chat uniquement pour la transcription.
...
- `node.pair.requested` / `node.pair.resolved` : cycle de vie de l'appairage du nœud.
...
- `voicewake.changed` : configuration du déclencheur de mot d'éveil modifiée.
...
- Les opérateurs peuvent appeler `commands.list` (`operator.read`) pour récupérer l'inventaire des commandes
...
- Les opérateurs peuvent appeler `tools.catalog` (`operator.read`) pour récupérer le catalogue d'outils d'exécution pour un
  agent. La réponse comprend les outils groupés et les métadonnées de provenance :
...
- Les opérateurs peuvent appeler `tools.effective` (`operator.read`) pour récupérer l'inventaire des outils effectifs à l'exécution pour une session.
  ...
  - La réponse est limitée à la session et reflète ce que la conversation active peut utiliser maintenant, y compris les outils principaux, les plug-ins et les outils de channel.
...
- Les opérateurs peuvent appeler `skills.update` (`operator.admin`) dans deux modes :
  ...
  - Le mode de configuration modifie les valeurs `skills.entries.<skillKey>` telles que `enabled`, `apiKey` et `env`.
...
- Si un appelant modifie `command`, `rawCommand`, `cwd`, `agentId` ou
  `sessionKey` entre la préparation et la transmission finale approuvée `system.run`, la passerelle rejette l'exécution au lieu de faire confiance à la charge utile modifiée.

## Solution de secours pour la livraison de l'agent

- Les demandes `agent` peuvent inclure `deliver=true` pour demander la livraison sortante.
...
stables pour la version 3 du protocole et constituent la base attendue pour les clients tiers.
...
| Délai d'expiration de la préauth / du défi de connexion                 | `10_000` ms                                                 | `src/gateway/handshake-timeouts.ts` (plage `250`–`10_000`) |
...
plutôt que les valeurs par défaut préalables à la négociation.
...
- Les modes porteurs d'identité tels que Tailscale Serve
  (`gateway.auth.allowTailscale: true`) ou `gateway.auth.mode: "trusted-proxy"` non bouclant
...
- La reconnexion avec ce jeton d'appareil **stocké** doit également réutiliser l'ensemble
  d'étendues approuvées stocké pour ce jeton. Cela préserve l'accès en lecture/probe/état
  qui a déjà été accordé et évite de réduire silencieusement les reconnexions à une
  étendue implicite plus restreinte réservée aux administrateurs.
- Assemblage de l'authentification de connexion côté client (`selectConnectAuth` dans
  ...
  - La promotion automatique d'un jeton d'appareil stocké lors de la nouvelle tentative ponctuelle
    ...
    bouclage, ou `wss://` avec un `tlsFingerprint` épinglé. Le `wss://` public
...
- Les entrées `hello-ok.auth.deviceTokens` supplémentaires sont des jetons de transfert d'amorçage.
  Ne les conservez que lorsque la connexion a utilisé une authentification d'amorçage sur un transport approuvé
  tel que `wss://` ou le couplage bouclage/local.
- Si un client fournit un `deviceToken` **explicite** ou un `scopes` explicite, cet
  ensemble d'étendues demandé par l'appelant reste autoritaire ; les étendues mises en cache ne sont
  réutilisées que lorsque le client réutilise le jeton par appareil stocké.
- Les jetons d'appareil peuvent être révoqués/renouvelés via `device.token.rotate` et
  `device.token.revoke` (nécessite l'étendue `operator.pairing`).
- L'émission/le renouvellement de jetons reste limité à l'ensemble de rôles approuvés enregistré dans
  l'entrée de couplage de cet appareil ; le renouvellement d'un jeton ne peut pas étendre l'appareil à un
  rôle que l'approbation de couplage n'avait jamais accordé.
- Pour les sessions de jetons d'appareils couplés, la gestion des appareils est auto-scopée, sauf si
  l'appelant possède également `operator.admin` : les appelants non-administrateurs peuvent supprimer/révoquer/faire pivoter
  uniquement leur propre entrée d'appareil.
- `device.token.rotate` vérifie également l'ensemble des étendues d'opérateur demandées par rapport aux
  étendues de session actuelles de l'appelant. Les appelants non-administrateurs ne peuvent pas faire pivoter un jeton vers
  un ensemble d'étendues d'opérateur plus large que celui qu'ils détiennent déjà.
...
- Comportement du client pour `AUTH_TOKEN_MISMATCH` :
  ...
  - Si cette nouvelle tentative échoue, les clients doivent arrêter les boucles de reconnexion automatique et afficher des directives d'action pour l'opérateur.
...
- Les approbations de couplage sont requises pour les nouveaux ID d'appareil, sauf si l'auto-approbation
  locale est activée.
...
- OpenClaw dispose également d'un chemin étroit de connexion automatique local au backend/conteneur pour
  les flux d'assistance de confiance basés sur un secret partagé.
- Les connexions tailnet ou LAN sur le même hôte sont toujours traitées comme distantes pour le couplage et
...
Pour les clients hérités qui utilisent toujours le comportement de signature avant le défi, `connect` renvoie désormais
`DEVICE_AUTH_*` codes de détail sous `error.details.code` avec un `error.details.reason` stable.
...
les schémas TypeBox dans `src/gateway/protocol/schema.ts`.
