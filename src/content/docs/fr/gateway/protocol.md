...
Pour le modèle complet de portée d'opérateur, les vérifications au moment de l'approbation et la sémantique
...
<AccordionGroup>
  ...
  <Accordion title="Modèles et utilisation">
    - `models.list` renvoie le catalogue de modèles autorisés à l'exécution. Passez `{ "view": "configured" }` pour les modèles configurés de taille de sélecteur (`agents.defaults.models` d'abord, puis `models.providers.*.models`), ou `{ "view": "all" }` pour le catalogue complet.
    - `usage.status` renvoie les résumés des fenêtres d'utilisation du fournisseur / du quota restant.
  ...
  <Accordion title="Chaînes et assistants de connexion">
    - `channels.status` renvoie les résumés d'état des chaînes/plugins intégrés et regroupés.
    ...
    - `voicewake.get` renvoie les mots de réveil stockés.
    - `voicewake.set` met à jour les mots de réveil et diffuse la modification.

  </Accordion>

  <Accordion title="Messagerie et journaux">
    - `send` est le RPC de livraison sortante directe pour les envois ciblés sur chaîne/compte/fil en dehors du moteur de discussion.
    - `logs.tail` renvoie la fin du journal de fichiers de la passerelle configurée avec des contrôles de curseur/limite et d'octets maximum.

  </Accordion>

  <Accordion title="Talk et TTS">
    - `talk.catalog` renvoie le catalogue de fournisseurs Talk en lecture seule pour la synthèse vocale, la transcription en flux et la voix en temps réel. Il inclut les identifiants de fournisseur, les étiquettes, l'état configuré, les identifiants de modèle/voix exposés, les modes canoniques, les transports, les stratégies cérébrales et les indicateurs audio/capacité en temps réel sans renvoyer de secrets de fournisseur ni modifier la configuration globale.
    ...
    - `talk.session.create` crée une session Talk détenue par le Gateway pour `realtime/gateway-relay`, `transcription/gateway-relay` ou `stt-tts/managed-room`. Pour `stt-tts/managed-room`, les appelants `operator.write` qui passent `sessionKey` doivent également passer `spawnedBy` pour la visibilité de la clé de session délimitée ; la création non délimitée de `sessionKey` et `brain: "direct-tools"` nécessitent `operator.admin`.
    - `talk.session.join` valide un jeton de session de salle gérée, émet `session.ready` ou des événements `session.replaced` si nécessaire, et renvoie les métadonnées de salle/session ainsi que les événements Talk récents sans le jeton en texte clair ni le haché du jeton stocké.
    ...
    - `talk.session.cancelOutput` arrête la sortie audio de l'assistant, principalement pour les coupures activées par VAD dans les sessions de relais Gateway.
    - `talk.session.submitToolResult` termine un appel d'outil de fournisseur émis par une session de relai en temps réel détenue par le Gateway. Passez `options: { willContinue: true }` pour la sortie intermédiaire de l'outil lorsqu'un résultat final suivra, ou `options: { suppressResponse: true }` lorsque le résultat de l'outil doit satisfaire l'appel du fournisseur sans démarrer une autre réponse d'assistant en temps réel.
    - `talk.session.steer` envoie le contrôle vocal de la série active dans une session Talk soutenue par un agent détenue par le Gateway. Il accepte `{ sessionId, text, mode? }`, où `mode` est `status`, `steer`, `cancel` ou `followup` ; le mode omis est classé à partir du texte prononcé.
    - `talk.session.close` ferme une session de relai, de transcription ou de salle gérée détenue par le Gateway et émet des événements Talk terminaux.
    - `talk.mode` définit/diffuse l'état actuel du mode Talk pour les clients WebChat/Control UI.
    - `talk.client.create` crée une session de fournisseur en temps réel détenue par le client en utilisant `webrtc` ou `provider-websocket` pendant que le Gateway possède la configuration, les identifiants, les instructions et la stratégie d'outils.
    ...
    - `talk.client.steer` envoie le contrôle vocal de la série active pour les transports en temps réel détenus par le client. Le Gateway résout la série intégrée active à partir de `sessionKey` et renvoie un résultat structuré accepté/rejeté au lieu d'abandonner silencieusement la direction.
    ...
    - `tts.status` renvoie l'état d'activation du TTS, le fournisseur actif, les fournisseurs de secours et l'état de la configuration du fournisseur.
    ...
    - `tts.convert` exécute une conversion synthèse vocale ponctuelle.
  ...
  <Accordion title="Secrets, config, update, and wizard">
    ...
    - `secrets.resolve` résout les affectations de secrets pour une commande/cible spécifique.
    ...
    - `config.schema` renvoie la charge utile du schéma de configuration en direct utilisée par l'interface de contrôle et les outils CLI : schéma, `uiHints`, version et métadonnées de génération, y compris les métadonnées de schéma des plugins et canaux lorsque le runtime peut les charger. Le schéma inclut les métadonnées de champ `title` / `description` dérivées des mêmes étiquettes et textes d'aide utilisés par l'interface, y compris les branches de composition d'objet imbriqué, de caractère générique, d'élément de tableau et `anyOf` / `oneOf` / `allOf` lorsque la documentation correspondante existe.
    - `config.schema.lookup` renvoie une charge utile de recherche limitée au chemin pour un chemin de configuration : chemin normalisé, un nœud de schéma superficiel, indice correspondant + `hintPath`, `reloadKind` facultatif, et résumés enfants immédiats pour l'exploration interface/CLI. `reloadKind` est l'un des `restart`, `hot`, ou `none` et reflète le planificateur de rechargement de la configuration du Gateway pour le chemin demandé. Les nœuds de schéma de recherche conservent la documentation orientée utilisateur et les champs de validation courants (`title`, `description`, `type`, `enum`, `const`, `format`, `pattern`, limites numériques/chaîne/tableau/objet, et indicateurs tels que `additionalProperties`, `deprecated`, `readOnly`, `writeOnly`). Les résumés enfants exposent `key`, `path` normalisé, `type`, `required`, `hasChildren`, `reloadKind` facultatif, ainsi que `hint` / `hintPath` correspondants.
    - `update.run` exécute le flux de mise à jour de la passerelle et planifie un redémarrage uniquement lorsque la mise à jour elle-même a réussi ; les appelants avec une session peuvent inclure `continuationMessage` pour que le démarrage reprenne un tour d'agent de suivi via la file d'attente de continuation de redémarrage. Les mises à jour du gestionnaire de packages depuis le plan de contrôle utilisent un transfert de service géré détaché au lieu de remplacer l'arborescence des packages à l'intérieur du Gateway en cours d'exécution. Un transfert démarré renvoie `ok: true` avec `result.reason: "managed-service-handoff-started"` et `handoff.status: "started"` ; les transferts indisponibles ou ayant échoué renvoient `ok: false` avec `managed-service-handoff-unavailable` ou `managed-service-handoff-failed`, plus `handoff.command` lorsqu'une mise à jour manuelle du shell est requise. Lors d'un transfert démarré, la sentinelle de redémarrage peut brièvement signaler `stats.reason: "restart-health-pending"` ; la continuation est retardée jusqu'à ce que la CLI vérifie le Gateway redémarré et écrive la sentinelle finale `ok`.
  ...
  <Accordion title="Assistant et helpers d'espace de travail">
    - `agents.list` renvoie les entrées d'agent configurées, y compris les métadonnées effectives du modèle et d'exécution.
    ...
    - `artifacts.list`, `artifacts.get`, et `artifacts.download` exposent les résumés d'artefacts dérivés de transcriptions et les téléchargements pour une portée `sessionKey`, `runId`, ou `taskId` explicite. Les requêtes d'exécution et de tâche résolvent la session propriétaire côté serveur et ne renvoient que les médias de transcription avec une provenance correspondante ; les sources d'URL non sécurisées ou locales renvoient des téléchargements non pris en charge au lieu de récupérer côté serveur.
    - `environments.list` et `environments.status` exposent la découverte de l'environnement Gateway-local et de nœud en lecture seule pour les clients SDK.
  ...
  <Accordion title="Contrôle de session">
    ...
    - `sessions.abort` interrompt le travail actif pour une session. Un appelant peut passer `key` plus optionnellement `runId`, ou passer `runId`Gateway seul pour les exécutions actives que le Gateway peut résoudre en une session.
    - `sessions.patch` met à jour les métadonnées/surcharges de session et rapporte le modèle canonique résolu ainsi que les `agentRuntime` effectifs.
    ...
    - L'exécution du chat utilise toujours `chat.history`, `chat.send`, `chat.abort` et `chat.inject`. `chat.history` est normalisé pour l'affichage des clients d'interface utilisateur : les balises de directive en ligne sont supprimées du texte visible, les charges utiles XML d'appel d'outil en texte brut (y compris `<tool_call>...</tool_call>`, `<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`, `<function_calls>...</function_calls>` et les blocs d'appel d'outil tronqués) et les jetons de contrôle de modèle ASCII/pleine largeur divulgués sont supprimés, les lignes d'assistant silencieuses pures telles que exact `NO_REPLY` / `no_reply` sont omises, et les lignes trop volumineuses peuvent être remplacées par des espaces réservés.
  ...
  <Accordion title="Appareillage des nœuds, invocation et travail en attente">
    ...
    - `node.invoke.result` renvoie le résultat pour une demande d'invocation.
    - `node.event` transporte les événements provenant du nœud vers la passerelle.
  ...
  <Accordion title="Familles d'approbations">
    ...
    - `exec.approvals.get` et `exec.approvals.set` gèrent les instantanés de la politique d'approbation d'exécution de la passerelle.
    - `exec.approvals.node.get` et `exec.approvals.node.set` gèrent la politique d'approbation d'exécution locale au nœud via les commandes de relais du nœud.
  ...
  <Accordion title="Automatisation, compétences et outils">
    ...
    - `cron.run` reste un RPC de type mise en file d'attente pour les exécutions manuelles. Les clients qui ont besoin d'une sémantique d'achèvement doivent lire l'`runId` renvoyé et interroger `cron.runs`.
    - `cron.runs` accepte un filtre `runId` non vide facultatif pour que les clients puissent suivre une exécution manuelle mise en file d'attente sans entrer en concurrence avec d'autres entrées d'historique pour le même travail.
...
- `chat` : mises à jour du chat de l'interface utilisateur telles que `chat.inject` et autres événements de chat exclusifs à la transcription. Dans le protocole v4, les charges utiles delta transportent `deltaText` ; `message` reste l'instantané cumulatif de l'assistant. Les remplacements sans préfixe définissent `replace=true` et utilisent `deltaText` comme texte de remplacement.
...
- `tasks.list` nécessite `operator.read`.
  - Paramètres : `status` facultatif (`"queued"`, `"running"`, `"completed"`,
    ...
    `500`, et chaîne facultative `cursor`.
...
- Les opérateurs peuvent appeler `commands.list` (`operator.read`) pour récupérer l'inventaire
  ...
  - `scope` contrôle la surface cible de la `name` principale :
    - `text` renvoie le jeton de commande texte principal sans le `/` initial
    - `native` et le chemin `both` par défaut renvoient les noms natifs conscients du fournisseur
  ...
  - `provider` est facultatif et n'affecte que la dénomination native ainsi que la disponibilité des commandes de plugin
    natives.
...
- Les opérateurs peuvent appeler `tools.effective` (`operator.read`) pour récupérer l'inventaire d'outils effectif à l'exécution
  ...
  - `tools.effective` est en lecture seule pour MCP : il peut projeter un catalogue de session MCP « chaud » via la
    stratégie d'outil finale, mais il ne crée pas d'exécutions MCP, ne connecte pas de transports, ni n'émet
    de `tools/list`. Si aucun catalogue chaud correspondant n'existe, la réponse peut inclure un avis tel que
...
- Les opérateurs peuvent appeler `tools.invoke` (`operator.write`) pour invoquer un outil disponible via le
  ...
  - La réponse est une enveloppe orientée SDK avec `ok`, `toolName`, `output` facultatif et des champs `error` typés.
    Les refus d'approbation ou de stratégie renvoient `ok:false` dans la charge utile plutôt que
    de contourner le pipeline de stratégie des outils de la passerelle.
- Les opérateurs peuvent appeler `skills.status` (`operator.read`) pour récupérer l'inventaire
  des compétences visible pour un agent.
  - `agentId` est facultatif ; omettez-le pour lire l'espace de travail de l'agent par défaut.
...
- Les opérateurs peuvent appeler `skills.upload.begin`, `skills.upload.chunk` et
  ...
  avant de l'installer. Il s'agit d'un chemin de téléchargement administratif distinct pour les clients de confiance,
...
- Les opérateurs peuvent appeler `skills.install` (`operator.admin`) selon trois modes :
  - Mode ClawHub : ClawHub`{ source: "clawhub", slug, version?, force? }` installe un
  ...
  - Mode de téléchargement : `{ source: "upload", uploadId, slug, force?, sha256?, timeoutMs? }`
    ...
    `skills.install.allowUploadedArchives`ClawHub est activé. Ce paramètre n'affecte pas les installations ClawHub.
  - Mode installateur du Gateway : Gateway`{ name, installId, dangerouslyForceUnsafeInstall?, timeoutMs? }`
...
- Omis ou `"default"` : comportement à l'exécution actuel. Si `agents.defaults.models` est configuré, la réponse est le catalogue autorisé, incluant les modèles découverts dynamiquement pour les entrées `provider/*`Gateway. Sinon, la réponse est le catalogue complet du Gateway.
...
- `"all"`Gateway : catalogue complet du Gateway, en contournant `agents.defaults.models`. Utilisez ceci pour les diagnostics et les interfaces de découverte, et non pour les sélecteurs de model normaux.

## Approbations d'exécution

- Lorsqu'une demande d'exécution nécessite une approbation, la passerelle diffuse `exec.approval.requested`.
- Les clients opérateurs résolvent en appelant `exec.approval.resolve` (requiert le scope `operator.approvals`).
...
- `bestEffortDeliver=true` permet un repli vers une exécution en session uniquement lorsqu'aucune route livrable externe ne peut être résolue (par exemple pour les sessions internes/webchat ou les configurations multi-canal ambiguës).
...
| Délai d'expiration de préauth / défi de connexion                      | `15_000` ms                                                 | `src/gateway/handshake-timeouts.ts` (config/env peut augmenter le budget serveur/client couplé) |
...
- Après l'appariement, le Gateway émet un **jeton d'appareil** délimité au rôle +
  portées de connexion. Il est renvoyé dans `hello-ok.auth.deviceToken` et doit être
...
- Assemblage de l'authentification de connexion côté client (`selectConnectAuth` dans
  ...
  - `auth.token` est rempli par ordre de priorité : d'abord le jeton partagé explicite,
    puis un `deviceToken` explicite, puis un jeton par appareil stocké (clé par
    `deviceId` + `role`).
  - `auth.bootstrapToken` n'est envoyé que si aucun des éléments ci-dessus n'a résolu de
    `auth.token`. Un jeton partagé ou n'importe quel jeton d'appareil résolu le supprime.
  - La promotion automatique d'un jeton d'appareil stocké lors de la tentative unique
    ...
    boucle locale, ou `wss://` avec un `tlsFingerprint` épinglé. Les `wss://` publics
    sans épinglage ne sont pas éligibles.
- L'amorçage via le code de configuration intégré renvoie le `hello-ok.auth.deviceToken` du nœud principal
  plus un jeton d'opérateur limité dans
  `hello-ok.auth.deviceTokens` pour le transfert mobile de confiance. Le jeton d'opérateur
  inclut `operator.talk.secrets` pour la lecture de la configuration native Talk et
  exclut `operator.admin` et `operator.pairing`.
- Alors qu'un amorçage par code de configuration autre que celui de référence attend l'approbation, les détails `PAIRING_REQUIRED`
...
- Conserver `hello-ok.auth.deviceTokens` uniquement lorsque la connexion a utilisé une authentification par amorçage
  sur un transport de confiance tel que `wss://` ou un appariement en boucle locale (loopback).
- Si un client fournit une **explicite** `deviceToken` ou une explicite `scopes`, cet
...
- Les jetons d'appareil peuvent être révoqués/rotatifs via `device.token.rotate` et
  `device.token.revoke` (nécessite la portée `operator.pairing`). La rotation ou
  la révocation d'un nœud ou d'un autre rôle non-opérateur nécessite également `operator.admin`.
- `device.token.rotate` renvoie les métadonnées de rotation. Il ne renvoie le jeton de remplacement
  que pour les appels du même appareil déjà authentifiés avec
  ce jeton d'appareil, permettant ainsi aux clients basés sur le jeton de sauvegarder leur remplacement avant
  reconnexion. Les rotations partagées/administratives ne renvoient pas le jeton.
...
- Pour les sessions de jetons d'appareils couplés, la gestion des appareils est à portée autonome, sauf si l'appelant possède également `operator.admin` : les appelants non-administrateurs peuvent gérer uniquement le jeton d'opérateur pour leur propre entrée d'appareil. La gestion des jetons de nœud et d'autres non-opérateurs est réservée aux administrateurs, même pour l'appareil de l'appelant.
- `device.token.rotate` et `device.token.revoke` vérifient également l'ensemble des portées du jeton d'opérateur cible par rapport aux portées de la session actuelle de l'appelant. Les appelants non-administrateurs ne peuvent pas faire pivoter ou révoquer un jeton d'opérateur plus large que celui qu'ils possèdent déjà.
...
- Les clients WS incluent normalement l'identité `device` pendant `connect` (opérateur + nœud). Les seules exceptions d'opérateur sans appareil sont les chemins de confiance explicites :
  - `gateway.controlUi.allowInsecureAuth=true` pour la compatibilité HTTP non sécurisée localhost uniquement.
  - authentification `gateway.auth.mode: "trusted-proxy"` réussie de l'UI de contrôle de l'opérateur.
  ...
  - RPC backend `gateway-client` en boucle directe authentifiés avec le
    jeton/mot de passe partagé de la passerelle.
- L'omission de l'identité de l'appareil a des conséquences sur la portée. Lorsqu'une connexion UI de contrôle
  n'a pas d'identité d'appareil, `shouldClearUnboundScopesForMissingDeviceIdentity`
  efface les portées auto-déclarées pour un ensemble vide pour l'authentification par jeton, mot de passe et
  proxy de confiance. La connexion est autorisée sur les chemins de confiance explicites, mais
  les méthodes à portée restreinte échouent. L'exception concerne les sessions jeton/mot de passe UI de contrôle locales avec
  `allowInsecureAuth`, qui préservent les portées. Pour les autres cas,
  définissez `gateway.controlUi.dangerouslyDisableDeviceAuth=true` uniquement comme
  chemin de préservation de portée en cas de rupture de vitre.
...
- [Manuel de la passerelle](Gateway/en/gateway)
