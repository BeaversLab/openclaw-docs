...
La stratégie d'outil, les bascules expérimentales, la configuration des outils soutenus par un fournisseur, et la configuration
...
- `models.providers.*.localService` : gestionnaire de processus facultatif à la demande pour les serveurs de modèles locaux. OpenClaw sonde le point de terminaison de santé configuré, démarre le `command` absolu si nécessaire, attend la disponibilité, puis envoie la requête de modèle. Voir [Services de modèles locaux](/fr/gateway/local-model-services).
...
Les définitions de serveurs MCP gérées par OpenClaw se trouvent sous `mcp.servers` et sont consommées par le Pi intégré et autres adaptateurs d'exécution. Les commandes `openclaw mcp list`, `show`, `set` et `unset` gèrent ce bloc sans se connecter au serveur cible lors des modifications de configuration.
...
- `bundledDiscovery` : par défaut à `"allowlist"` pour les nouvelles configurations, donc un `plugins.allow` non vide
  verrouille également les plugins de fournisseur groupés, y compris les fournisseurs d'exécution de recherche web. Doctor écrit `"compat"` pour les configurations de liste d'autorisation héritées migrées afin de préserver le comportement des fournisseurs groupés existants jusqu'à ce que vous y optiez.
- `plugins.entries.<id>.apiKey` : champ de commodité pour la clé d'API au niveau du plugin (lorsqu'elle est prise en charge par le plugin).
- `plugins.entries.<id>.env` : mappage des variables d'environnement délimité au plugin.
- `plugins.entries.<id>.hooks.allowPromptInjection` : lorsque `false`, le cœur bloque `before_prompt_build` et ignore les champs de mutation de prompt provenant des `before_agent_start` hérités, tout en préservant les `modelOverride` et `providerOverride` hérités. S'applique aux hooks de plugins natifs et aux répertoires de hooks fournis par les bundles pris en charge.
...
- `plugins.entries.<id>.subagent.allowedModels` : liste d'autorisation (allowlist) facultative de cibles `provider/model` canoniques pour les substitutions de sous-agent de confiance. Utilisez `"*"` uniquement lorsque vous souhaitez intentionnellement autoriser n'importe quel modèle.
...
- `plugins.entries.<id>.llm.allowedModels` : liste d'autorisation (allowlist) facultative de cibles `provider/model` canoniques pour les substitutions de complétion LLM de plugins de confiance. Utilisez `"*"` uniquement lorsque vous souhaitez intentionnellement autoriser n'importe quel modèle.
- `plugins.entries.<id>.llm.allowAgentIdOverride` : accorder explicitement la confiance à ce plugin pour exécuter `api.runtime.llm.complete` par rapport à un identifiant d'agent non par défaut.
...
- Les paramètres de compte/exécution du plugin de canal se trouvent sous `channels.<id>` et doivent être décrits par les métadonnées `channelConfigs` du manifeste du plugin propriétaire, et non par un registre d'options central OpenClaw.
...
[Codex harness reference](/fr/plugins/codex-harness-reference) pour la surface de configuration
complète et [Codex harness](/fr/plugins/codex-harness) pour le modèle d'exécution.

`codexPlugins` s'applique uniquement aux sessions qui sélectionnent le harnais Codex natif.
Il n'active pas les plugins Codex pour Pi, les exécutions normales du fournisseur OpenAI, les
...
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions` :
  stratégie d'action destructrice par défaut pour les sollicitations d'applications de plugins migrées.
  Par défaut : `true`.
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.marketplaceName` :
  identité stable du marché. V1 prend uniquement en charge `"openai-curated"`.
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.allow_destructive_actions` :
  substitution d'action destructrice par plugin. Si omis, la valeur
...
explicites écrites par la migration constituent l'ensemble d'éligibilité durable pour l'installation et la réparation.
...
redémarrage de la passerelle après avoir modifié la configuration des plugins natifs.

- `plugins.entries.firecrawl.config.webFetch` : paramètres du fournisseur de récupération Web Firecrawl.
  - `apiKey` : clé Firecrawl API (accepte SecretRef). Revient à `plugins.entries.firecrawl.config.webSearch.apiKey`, `tools.web.fetch.firecrawl.apiKey` de l'héritage, ou à la env var `FIRECRAWL_API_KEY`.
...
- `plugins.entries.xai.config.xSearch` : paramètres de recherche Web xAI X (Grok).
...
- Les plugins de bundle Claude activés peuvent également contribuer des valeurs par défaut Pi intégrées à partir de `settings.json`OpenClawOpenClaw ; OpenClaw les applique en tant que paramètres d'agent nettoyés, et non en tant que correctifs de configuration bruts OpenClaw.
...
`commitments` contrôle la mémoire de suivi inférée : OpenClaw peut détecter les points d'avancement des tours de conversation et les diffuser via des exécutions de heartbeat.

- `commitments.enabled` : activez l'extraction, le stockage et la diffusion par heartbeat cachés LLM pour les engagements de suivi inférés. Par défaut : `false`.
- `commitments.maxPerDay` : nombre maximum d'engagements de suivi inférés diffusés par session d'agent sur un jour glissant. Par défaut : `3`.
...
- Définissez `ssrfPolicy.dangerouslyAllowPrivateNetwork: true` uniquement si vous faites intentionnellement confiance à la navigation du navigateur sur un réseau privé.
...
- `remoteCdpTimeoutMs` et `remoteCdpHandshakeTimeoutMs` s'appliquent à l'accessibilité CDP distante et
  `attachOnly` ainsi qu'aux demandes d'ouverture d'onglets. Les profils de boucle locale
  gérés conservent les paramètres CDP locaux par défaut.
- Si un service CDP géré de manière externe est accessible via le bouclage (loopback), définissez le `attachOnly: true` de ce profil ; sinon, OpenClaw considère le port de bouclage comme un profil de navigateur géré localement et peut signaler des erreurs de propriété de port local.
...
- Les profils `existing-session` conservent les limites d'itinéraire (route) actuelles de Chrome MCP :
  actions basées sur des instantanés/références au lieu d'un ciblage par sélecteur CSS, hooks de téléchargement d'un seul fichier,
...
- **Alias de liaison hérités** : utilisez les valeurs de mode de liaison dans `gateway.bind` (`auto`, `loopback`, `lan`, `tailnet`, `custom`), et non les alias d'hôte (`0.0.0.0`, `127.0.0.1`, `localhost`, `::`, `::1`Docker).
...
- **Auth** : requis par défaut. Les liaisons non-boucle locale nécessitent une authentification de passerelle. En pratique, cela signifie un jeton/mot de passe partagé ou un proxy inverse conscient de l'identité avec `gateway.auth.mode: "trusted-proxy"`. L'assistant d'intégration génère un jeton par défaut.
...
- `gateway.auth.mode: "trusted-proxy"` : déléguer l'authentification du navigateur/utilisateur à un proxy inverse conscient de l'identité et faire confiance aux en-têtes d'identité de `gateway.trustedProxies` (voir [Trusted Proxy Auth](/fr/gateway/trusted-proxy-auth)). Ce mode s'attend par défaut à une source proxy **non-boucle locale** ; les proxies inverses de boucle locale sur le même hôte nécessitent `gateway.auth.trustedProxy.allowLoopback = true` explicite. Les appelants internes sur le même hôte peuvent utiliser `gateway.auth.password` comme secours direct local ; `gateway.auth.token` reste mutuellement exclusif avec le mode de proxy de confiance.
- `gateway.auth.allowTailscale` : lorsque `true`Tailscale, les en-têtes d'identité Tailscale Serve peuvent satisfaire l'authentification Control UI/WebSocket (vérifiée via `tailscale whois`APITailscale). Les points de terminaison de l'API HTTP n'utilisent **pas** cette authentification d'en-tête Tailscale ; ils suivent plutôt le mode d'authentification HTTP normal de la passerelle. Ce flux sans jeton suppose que l'hôte de la passerelle est de confiance. Par défaut `true` lorsque `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit` : limiteur optionnel d'échec d'authentification. S'applique par IP client et par portée d'authentification (shared-secret et device-token sont suivis indépendamment). Les tentatives bloquées renvoient `429` + `Retry-After`Tailscale.
  - Sur le chemin asynchrone de l'interface utilisateur de contrôle Tailscale Serve, les tentatives échouées pour le même `{scope, clientIp}` sont sérialisées avant l'écriture de l'échec. Par conséquent, les mauvaises tentatives simultanées du même client peuvent déclencher le limiteur dès la deuxième demande au lieu que les deux passent comme des simples inadéquations.
  - `gateway.auth.rateLimit.exemptLoopback` par défaut `true` ; définissez `false` lorsque vous souhaitez volontairement limiter le débit du trafic localhost également (pour les configurations de test ou les déploiements de proxy stricts).
...
- En boucle locale, ces verrouillages d'origine navigateur sont isolés par valeur `Origin` normalisée,
...
- `tailscale.preserveFunnel` : lorsque `true` et `tailscale.mode = "serve"`OpenClaw, OpenClaw
  ...
  Par défaut `false`.
- `controlUi.allowedOrigins`Gateway : liste d'autorisation d'origine navigateur explicite pour les connexions WebSocket de la passerelle. Requis pour les origines de navigateur publiques non-boucle locale. Les chargements d'interface utilisateur privés de même origine LAN/Tailnet à partir de la boucle locale, RFC1918/link-local, `.local`, `.ts.net`Tailscale, ou les hôtes CGNAT Tailscale sont acceptés sans activer le secours de l'en-tête d'hôte.
- `controlUi.chatMessageMaxWidth` : largeur maximale optionnelle pour les messages de chat groupés de l'interface utilisateur de contrôle. Accepte les valeurs de largeur CSS contraintes telles que `960px`, `82%`, `min(1280px, 82%)` et `calc(100% - 2rem)`.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback` : mode dangereux qui active le secours d'origine d'en-tête d'hôte pour les déploiements qui s'appuient intentionnellement sur la politique d'origine d'en-tête d'hôte.
- `remote.transport` : `ssh` (par défaut) ou `direct` (ws/wss). Pour `direct`, `remote.url` doit être `wss://` pour les hôtes publics ; le texte brut `ws://` est accepté uniquement pour la boucle locale, le LAN, le lien local, `.local`, `.ts.net`Tailscale et les hôtes CGNAT Tailscale.
- `remote.remotePort` : port de passerelle sur l'hôte SSH distant. Par défaut `18789` ; utilisez ceci lorsque le port du tunnel local diffère du port de la passerelle distante.
...
- `gateway.push.apns.relay.baseUrl`iOSiOS : URL HTTPS de base pour le relais APNs externe utilisé par les versions officielles/TestFlight iOS après avoir publié des inscriptions soutenues par relais à la passerelle. Cette URL doit correspondre à l'URL du relais compilée dans la version iOS.
- `gateway.push.apns.relay.timeoutMs` : délai d'envoi passerelle-vers-relais en millisecondes. Par défaut `10000`iOS.
- Les inscriptions soutenues par relais sont déléguées à une identité de passerelle spécifique. L'application iOS jumelée récupère `gateway.identity.get`, inclut cette identité dans l'inscription au relais et transfère une subvention d'envoi délimitée par inscription à la passerelle. Une autre passerelle ne peut pas réutiliser cette inscription stockée.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true` : porte de secours de développement uniquement pour les URL de relais HTTP de boucle locale. Les URL de relais de production doivent rester en HTTPS.
- `gateway.handshakeTimeoutMs`Gateway : délai d'expiration de la négociation WebSocket de passerelle pré-auth en millisecondes. Par défaut : `15000`. `OPENCLAW_HANDSHAKE_TIMEOUT_MS` prend la priorité lorsqu'il est défini. Augmentez ceci sur les hôtes surchargés ou peu puissants où les clients locaux peuvent se connecter pendant que le réchauffement du démarrage est encore en cours.
...
- Les chemins d'appel de passerelle locale peuvent utiliser `gateway.remote.*` comme secours uniquement lorsque `gateway.auth.*` n'est pas défini.
- Si `gateway.auth.token` / `gateway.auth.password` est explicitement configuré via SecretRef et non résolu, la résolution échoue fermée (aucun masquage de secours distant).
- `trustedProxies`Tailscale : IP de proxy inverse qui terminent le TLS ou injectent des en-têtes de client transféré. Ne listez que les proxies que vous contrôlez. Les entrées de boucle locale sont toujours valides pour les configurations de proxy/détection locale sur le même hôte (par exemple Tailscale Serve ou un proxy inverse local), mais elles ne rendent **pas** les requêtes de boucle locale éligibles pour `gateway.auth.mode: "trusted-proxy"`.
...
- `autoGenerate` : génère automatiquement une paire de certificat/clé auto-signée locale lorsque des fichiers explicites ne sont pas configurés ; à usage local/dev uniquement.
- `certPath` : chemin du système de fichiers vers le fichier de certificat TLS.
- `keyPath` : chemin du système de fichiers vers le fichier de clé privée TLS ; gardez-le restreint en matière de permissions.
...
- `mode` : contrôle la manière dont les modifications de configuration sont appliquées lors de l'exécution.
  ...
  - `"restart"` : redémarre toujours le processus de la passerelle lors d'un changement de configuration.
...
- `debounceMs` : fenêtre d'anti-rebond (debounce) en ms avant que les modifications de configuration ne soient appliquées (entier non négatif).
- `deferralTimeoutMs` : durée maximale optionnelle en ms d'attente des opérations en cours avant de forcer un redémarrage ou un rechargement à chaud du channel. L'omettre pour utiliser l'attente bornée par défaut (`300000`) ; définir `0` pour attendre indéfiniment et enregistrer des avertissements périodiques pour les opérations toujours en attente.
...
- Si un mappage ou un préréglage utilise une `sessionKey` basée sur un modèle (template), définissez `hooks.allowedSessionKeyPrefixes` et `hooks.allowRequestSessionKey=true`. Les clés de mappage statiques ne nécessitent pas cette activation.

**Points de terminaison :**

- `POST /hooks/wake` → `{ text, mode?: "now"|"next-heartbeat" }`
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - `sessionKey` issue du payload de la requête n'est acceptée que lorsque `hooks.allowRequestSessionKey=true` (par défaut : `false`).
...
- `agentId` achemine vers un agent spécifique; les ID inconnus reviennent à celui par défaut.
...
- Si vous avez besoin de `hooks.allowRequestSessionKey: false`, remplacez la préréglage par un `sessionKey` statique au lieu du modèle par défaut.
...
- `full` : incluez `cliPath` + `sshPort` ; la publicité multidiffusion LAN nécessite toujours que le plugin `bonjour` inclus soit activé.
- `off` : supprime la publicité multidiffusion LAN sans modifier l'activation du plugin.
- Le plugin intégré `bonjour` démarre automatiquement sur les hôtes macOS et est facultatif sur Linux, Windows et les déploiements conteneurisés Gateway.
- Le nom d'hôte par défaut est le nom d'hôte du système lorsqu'il s'agit d'une étiquette DNS valide, avec repli sur `openclaw`. Remplacez-le par `OPENCLAW_MDNS_HOSTNAME`.
...
Écrit une zone DNS-SD de monodiffusion sous `~/.openclaw/dns/`. Pour la découverte inter-réseaux, associez à un serveur DNS (CoreDNS recommandé) + DNS split Tailscale.
...
- Fichiers `.env` : répertoire de travail actuel `.env` + `~/.openclaw/.env` (aucun ne remplace les variables existantes).
...
- Les chemins des providers File et exec échouent de manière fermée lorsque la vérification des ACL Windows n'est pas disponible. Définissez `allowInsecurePath: true` uniquement pour les chemins de confiance qui ne peuvent pas être vérifiés.
...
- Les profils en mode OAuth (`auth.profiles.<id>.mode = "oauth"`) ne prennent pas en charge les informations d'identification de profil d'authentification sauvegardées via SecretRef.
...
- `billingBackoffHoursByProvider` : substitutions facultatives par fournisseur pour les heures de délai de retour de facturation.
...
- `overloadedProfileRotations` : nombre maximum de rotations de profil d'authentification du même fournisseur pour les erreurs de surcharge avant de basculer vers le modèle de secours (par défaut : `1`). Les formes de fournisseur occupé telles que `ModelNotReadyException` atterrissent ici.
- `overloadedBackoffMs` : délai fixe avant de réessayer une rotation de profil/fournisseur surchargé (par défaut : `0`).
- `rateLimitedProfileRotations` : nombre maximum de rotations de profils d'authentification pour le même fournisseur en cas d'erreurs de limitation de débit avant de basculer vers le modèle de secours (par défaut : `1`). Ce compartiment de limitation de débit inclut le texte structuré par fournisseur tel que `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` et `resource exhausted`.
...
- `redactSensitive` / `redactPatterns` : masquage de « mieux que rien » pour la sortie console, les fichiers journaux, les enregistrements de journaux OTLP et le texte des transcriptions de session persistées. `redactSensitive: "off"` désactive uniquement cette stratégie générale de journalisation/transcription ; les surfaces de sécurité de l'interface utilisateur/outils/diagnostic masquent toujours les secrets avant émission.
...
- `enabled` : commutateur maître pour la sortie d'instrumentation (par défaut : `true`).
...
- `stuckSessionWarnMs` : seuil d'âge sans progression en ms pour classer les sessions de traitement de longue durée comme `session.long_running`, `session.stalled` ou `session.stuck`. La progression de la réponse, de l'outil, de l'état, du bloc et de l'ACP réinitialise la minuterie ; les diagnostics `session.stuck` répétés se désactivent progressivement tant qu'ils sont inchangés.
- `stuckSessionAbortMs` : seuil d'âge en ms sans progression avant que le travail actif éligible bloqué puisse être interrompu et drainé pour récupération. Si non défini, OpenClaw utilise la fenêtre d'exécution intégrée étendue plus sûre d'au moins 5 minutes et 3x `stuckSessionWarnMs`.
- `memoryPressureSnapshot` : capture un instantané de stabilité expurgé pré-OOM lorsque la pression mémoire atteint `critical` (par défaut : `false`). Définir sur `true` pour ajouter le scan/écriture du fichier bundle de stabilité tout en conservant les événements normaux de pression mémoire.
- `otel.enabled` : active le pipeline d'exportation OpenTelemetry (par défaut : `false`). Pour la configuration complète, le catalogue de signaux et le modèle de confidentialité, voir [Exportation OpenTelemetry](/fr/gateway/opentelemetry).
- `otel.endpoint` : URL du collecteur pour l'exportation OTel.
- `otel.tracesEndpoint` / `otel.metricsEndpoint` / `otel.logsEndpoint` : points de terminaison OTLP optionnels spécifiques aux signaux. Lorsqu'ils sont définis, ils remplacent `otel.endpoint` uniquement pour ce signal.
...
- `otel.captureContent` : capture optionnelle du contenu brut pour les attributs de span OTEL. Désactivé par défaut. Le booléen `true` capture le contenu des messages/outils non système ; la forme objet vous permet d'activer `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` et `systemPrompt` explicitement.
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental` : bascule d'environnement pour les derniers attributs de provider de span GenAI expérimentaux. Par défaut, les spans conservent l'attribut hérité `gen_ai.system` pour la compatibilité ; les métriques GenAI utilisent des attributs sémantiques bornés.
- `OPENCLAW_OTEL_PRELOADED=1` : bascule d'environnement pour les hôtes qui ont déjà enregistré un SDK OpenTelemetry global. OpenClaw ignore alors le démarrage/arrêt du SDK détenupar le plugin tout en maintenant les écouteurs de diagnostic actifs.
...
- `channel` : channel de publication pour les installations npm/git - `"stable"`, `"beta"` ou `"dev"`.
...
- `auto.stableDelayHours` : délai minimum en heures avant l'application automatique pour le canal stable (par défaut : `6` ; max : `168`).
- `auto.stableJitterHours` : fenêtre de délai supplémentaire de déploiement pour le canal stable en heures (par défaut : `12` ; max : `168`).
...
- `enabled` : interrupteur global de fonctionnalité ACP (par défaut : `true` ; définissez `false` pour masquer les fonctions de répartition et de génération ACP).
...
- `stream.coalesceIdleMs` : fenêtre de nettoyage de l'inactivité en ms pour le texte diffusé en flux.
...
- `cli.banner.taglineMode` contrôle le style de la bannière :
  - `"random"` (par défaut) : slogans amusants/saisonniers alternés.
...
- `webhook` : URL de webhook de secours héritée dépréciée (http/https) utilisée uniquement pour les tâches stockées qui ont encore `notify: true`.
...
- `includeSkipped` : compter les exécutions consécutives ignorées vers le seuil d'alerte (par défaut : `false`). Les exécutions ignorées sont suivies séparément et n'affectent pas l'attente exponentielle en cas d'erreur d'exécution.
...
_En rapport : [Configuration](/fr/gateway/configuration) · [Exemples de configuration](/fr/gateway/configuration-examples) · [Doctor](/fr/gateway/doctor)_
