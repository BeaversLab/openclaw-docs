...
OpenShell est un backend de bac à sable géré pour OpenClaw. Au lieu d'exécuter des conteneurs Docker localement, OpenClaw délègue le cycle de vie du bac à sable au CLI `openshell`, qui provisionne des environnements distants avec une exécution de commandes basée sur SSH.

Le plugin OpenShell réutilise le même transport SSH de base et le pont de système de fichiers distant que le [backend SSH générique](/fr/gateway/sandboxing#ssh-backend). Il ajoute un cycle de vie spécifique à OpenShell (`sandbox create/get/delete`, `sandbox ssh-config`) et un mode d'espace de travail `mirror` en option.

## Prérequis

- Le CLI `openshell` installé et disponible dans `PATH` (ou définissez un chemin personnalisé via `plugins.entries.openshell.config.command`)
- Un compte OpenShell avec accès au bac à sable
- OpenClaw Gateway exécuté sur l'hôte

## Quick start

1. Activez le plugin et définissez le backend du bac à sable :
...
2. Redémarrez la Gateway. Au prochain tour de l'agent, OpenClaw crée un bac à sable OpenShell et achemine l'exécution de l'outil via celui-ci.
...
- Vous modifiez des fichiers localement en dehors d'OpenClaw et souhaitez que ces modifications soient visibles dans le bac à sable automatiquement.
...
- Les lectures de média lors de l'invite fonctionnent toujours car les outils de fichiers et de médias lisent à travers
  le pont du sandbox.

Idéal pour :

- Le sandbox doit résider principalement sur le côté distant.
- Vous souhaitez une charge de synchronisation par tour plus faible.
- Vous ne voulez pas que les modifications locales écrasent silencieusement l'état du sandbox distant.

<Warning>Si vous modifiez des fichiers sur l'hôte en dehors de OpenClaw après l'amorçage initial, le sandbox distant ne voit **pas** ces modifications. Utilisez `openclaw sandbox recreate` pour réamorcer.</Warning>
...
| **Direction de synchro**        | Bidirectionnelle (chaque exec)               | Amorçage unique                      |
| **Charge par tour**             | Plus élevée (téléchargement + téléversement) | Plus faible (ops distantes directes) |
| **Modifs locales visibles ?**   | Oui, au prochain exec                        | Non, jusqu'à recréation              |
| **Idéal pour**                  | Flux de travail de développement             | Agents longue durée, CI              |

## Référence de configuration

Toute la configuration OpenShell réside sous `plugins.entries.openshell.config` :

| Clé                       | Type                     | Par défaut    | Description                                                                          |
| ------------------------- | ------------------------ | ------------- | ------------------------------------------------------------------------------------ |
| `mode`                    | `"mirror"` ou `"remote"` | `"mirror"`    | Mode de synchro de l'espace de travail                                               |
| `command`                 | `string`                 | `"openshell"` | Chemin ou nom du `openshell` CLI                                                     |
| `from`                    | `string`                 | `"openclaw"`  | Source du sandbox pour la création initiale                                          |
...
| `timeoutSeconds`          | `number`                 | `120`         | Délai d'expiration pour les opérations du `openshell` CLI                            |
...
Les sandboxes OpenShell sont gérés via le CLI de sandbox normal :
...
distant canonique pour cette portée. La prochaine utilisation initialise un nouvel espace de travail distant à partir
...
OpenShell fixe le fd du répertoire racine de l'espace de travail et vérifie à nouveau l'identité du Sandbox avant chaque
lecture, afin que les échanges de liens symboliques ou un espace de travail remonté ne puissent pas rediriger les lectures en dehors
de l'espace de travail distant prévu.
...
3. Core écrit la configuration SSH dans un fichier temporaire et ouvre une session SSH en utilisant le même pont de système de fichiers distant que le backend SSH générique.
