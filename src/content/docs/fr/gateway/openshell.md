...
<Warning>Si vous modifiez des fichiers sur l'hôte en dehors de OpenClaw après le premier amorçage, le bac à sable distant ne voit **pas** ces modifications. Utilisez `openclaw sandbox recreate` pour réamorcer.</Warning>
...
| `command`                 | `string`                 | `"openshell"` | Chemin ou nom de la `openshell` CLI                                                 |
...
| `timeoutSeconds`          | `number`                 | `120`         | Délai d'attente pour les opérations de la `openshell` CLI                           |
