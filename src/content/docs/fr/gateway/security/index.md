...
4. La réclamation n'est pas répertoriée dans [Out of Scope](https://github.com/openclaw/openclaw/blob/main/SECURITY.md#out-of-scope).
...
Définissez `contextVisibility` par canal ou par salon/conversation. Consultez [Group Chats](/en/channels/groups#context-visibility-and-allowlists) pour les détails de configuration.
...
| `fs.config_include.perms_writable`                            | critique               | Le fichier d'inclusion de configuration peut être modifié par d'autres personnes                                                                  | permissions des fichiers d'inclusion référencés depuis `openclaw.json`                                                                 | oui             |
| `fs.config_include.perms_group_readable`                      | avertissement          | Les utilisateurs du groupe peuvent lire les secrets/paramètres inclus                                                                             | permissions des fichiers d'inclusion référencés depuis `openclaw.json`                                                                 | oui             |
| `fs.config_include.perms_world_readable`                      | critique               | Les secrets/paramètres inclus sont lisibles par tous                                                                                              | include-file perms référencés à partir de `openclaw.json`                                                                              | oui             |
...
| `fs.credentials_dir.perms_writable`                           | critique               | D'autres personnes peuvent modifier l'état des identifiants/jumelage de canal                                                                     | filesystem perms sur `~/.openclaw/credentials`                                                                                         | yes             |
| `fs.credentials_dir.perms_readable`                           | avertissement          | Others can read channel credential state                                                                                                          | filesystem perms sur `~/.openclaw/credentials`                                                                                         | yes             |
...
| `gateway.http.no_auth`                                        | warn/critical          | Gateway HTTP accessibles avec `auth.mode="none"`                                                                                                  | `gateway.auth.mode`, `gateway.http.endpoints.*`                                                                                        | non             |
...
| `gateway.control_ui.allowed_origins_wildcard`                 | warn/critical          | `allowedOrigins=["*"]` désactive la liste blanche des origines du navigateur                                                                      | `gateway.controlUi.allowedOrigins`                                                                                                     | non             |
...
| `gateway.real_ip_fallback_enabled`                            | warn/critical          | Faire confiance au repli `X-Real-IP` peut activer l'usurpation d'IP source via une mauvaise configuration du proxy                                | `gateway.allowRealIpFallback`, `gateway.trustedProxies`                                                                                | non             |
...
| `hooks.path_root`                                             | critical               | Le chemin du hook est `/`, ce qui rend plus facile la collision ou le routage incorrect de l'ingress                                              | `hooks.path`                                                                                                                           | no              |
...
| `browser.remote_cdp_http`                                     | warn                   | Remote CDP over plain HTTP lacks transport encryption                                                                                             | profil de navigateur `cdpUrl`                                                                                                          | no              |
| `browser.remote_cdp_private_host`                             | warn                   | Remote CDP targets a private/internal host                                                                                                        | profil de navigateur `cdpUrl`, `browser.ssrfPolicy.*`                                                                                  | no              |
...
| `tools.exec.allowlist_interpreter_without_strict_inline_eval` | avertissement          | Les listes autorisées de l'interpréteur permettent l'évaluation en ligne sans réapprobation forcée                                                | `tools.exec.strictInlineEval`, `agents.list[].tools.exec.strictInlineEval`, liste blanche des approbations d'exécution                 | non             |
| `tools.exec.safe_bins_interpreter_unprofiled`                 | avertir                | Les bins interpréteur/runtime dans `safeBins` sans profils explicites élargissent le risque d'exécution                                           | `tools.exec.safeBins`, `tools.exec.safeBinProfiles`, `agents.list[].tools.exec.*`                                                      | non             |
...
| `skills.workspace.symlink_escape`                             | avertir                | Le `skills/**/SKILL.md` de l'espace de travail se résout en dehors de la racine de l'espace de travail (dérive de la chaîne de liens symboliques) | état du système de fichiers `skills/**` de l'espace de travail                                                                         | non             |
...
| `security.trust_model.multi_user_heuristic`                   | avertir                | La configuration semble multi-utilisateur alors que le modèle de confiance de la passerelle est assistant personnel                               | séparer les limites de confiance ou durcissement multi-utilisateur (`sandbox.mode`, refus d'outil/délimitation de l'espace de travail) | non             |
...
désactive entièrement les vérifications d'identité de l'appareil. Il s'agit d'une dégradation grave de la sécurité ;
gardez-la désactivée sauf si vous êtes en train de déboguer activement et pouvez revenir en arrière rapidement.

Indépendamment de ces indicateurs dangereux, une authentification `gateway.auth.mode: "trusted-proxy"`
réussie peut admettre des sessions de l'interface de contrôle **d'opérateur** sans identité d'appareil. Il s'agit d'un
comportement intentionnel du mode d'authentification, et non d'un raccourci `allowInsecureAuth`, et cela
ne s'étend toujours pas aux sessions de l'interface de contrôle avec le rôle de nœud.
...
`gateway.trustedProxies` pour une gestion correcte de l'IP du client transféré.
...
- les proxies inversés en boucle locale sur le même hôte peuvent toujours utiliser `gateway.trustedProxies` pour la détection de client local et la gestion des IP transférées
- pour les proxies inversés en boucle locale sur le même hôte, utilisez l'authentification par jeton/mot de passe au lieu de `gateway.auth.mode: "trusted-proxy"`
...
- `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` active le mode de secours d'origine via l'en-tête Host ; traitez-le comme une stratégie dangereuse sélectionnée par l'opérateur.
- Traitez le rebinding DNS et le comportement de l'en-tête proxy-host comme des problèmes de renforcement du déploiement ; gardez `trustedProxies` serré et évitez d'exposer directement la passerine à l'internet public.
...
Ceci est requis pour la continuité de session et (optionnellement) l'indexation de la mémoire de session, mais cela signifie également que **tout processus/utilisateur avec un accès au système de fichiers peut lire ces journaux**. Traitez l'accès au disque comme la frontière de confiance et verrouillez les permissions sur `~/.openclaw` (voir la section d'audit ci-dessous). Si vous avez besoin d'un isolement plus fort entre les agents, faites-les fonctionner sous des utilisateurs OS séparés ou des hôtes séparés.

## Exécution de nœud (system.run)

Si un nœud macOS est couplé, la Gateway peut invoquer `system.run` sur ce nœud. Il s'agit d'une **exécution de code à distance** sur le Mac :
...
- La stratégie `system.run` par nœud est le propre fichier d'approbations d'exécution du nœud (`exec.approvals.node.*`), qui peut être plus stricte ou plus souple que la stratégie globale d'ID de commande du passerelle.
- Un nœud exécuté avec `security="full"` et `ask="off"` suit le modèle par défaut d'opérateur de confiance. Traitez cela comme un comportement attendu, sauf si votre déploiement exige explicitement une approbation plus stricte ou une position de liste blanche.
...
- Pour `host=node`, les exécutions soutenues par une approbation stockent également un `systemRunPlan` préparé canonique ; les réacheminements approuvés ultérieurement réutilisent ce plan stocké, et la validation de la passerelle rejette les modifications de l'appelant sur le contexte de commande/cwd/session après la création de la demande d'approbation.
...
Les commandes slash et les directives sont honorées uniquement pour les **expéditeurs autorisés**. L'autorisation est dérivée des listes autorisatives/appariements de canal plus `commands.useAccessGroups` (voir [Configuration](/en/gateway/configuration) et [Slash commands](/en/tools/slash-commands)). Si une liste autorisée de canal est vide ou inclut `"*"`, les commandes sont effectivement ouvertes pour ce canal.
...
- Si vous installez ou mettez à jour des plugins (`openclaw plugins install <package>`, `openclaw plugins update <id>`), traitez cela comme l'exécution de code non fiable :
  ...
  - OpenClaw exécute une analyse de code dangereux intégrée avant l'installation/mise à jour. Les résultats `critical` bloquent par défaut.
...
- `disabled` : ignorez entièrement les DM entrants.
...
- **Liste d'autorisation DM** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom` ; héritage : `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`) : qui est autorisé à parler au bot en messages privés.
...
- **Liste blanche de groupe** (spécifique au channel) : les groupes/channels/guildes à partir desquels le bot acceptera les messages.
  - Modèles courants :
    - `channels.whatsapp.groups`, `channels.telegram.groups`, `channels.imessage.groups` : valeurs par défaut par groupe comme `requireMention` ; une fois défini, cela agit également comme une liste blanche de groupe (incluez `"*"` pour conserver le comportement « tout autoriser »).
    ...
    - `channels.discord.guilds` / `channels.slack.channels` : listes blanches par surface + valeurs par défaut de mention.
...
- Gardez ces éléments non définis ou sur faux en production.
...
- Les payloads de hooks sont du contenu non fiable, même lorsque la livraison provient de systèmes que vous contrôlez (les courriers/docs/contenu web peuvent transporter des injections de prompt).
- Les niveaux de modèles faibles augmentent ce risque. Pour l'automatisation pilotée par hooks, privilégiez les niveaux de modèles modernes et forts et gardez une politique d'outils stricte (`tools.profile: "messaging"` ou plus stricte), ainsi que du sandboxing lorsque cela est possible.

### L'injection de prompt ne nécessite pas de DMs publics

Même si **seulement vous** pouvez envoyer des messages au bot, l'injection de prompt peut toujours se produire via
...
- Pour les entrées URL OpenResponses (`input_file` / `input_image`), définissez des `gateway.http.endpoints.responses.files.urlAllowlist` strictes
  et des `gateway.http.endpoints.responses.images.urlAllowlist`, et gardez `maxUrlParts` faible.
...
- Pour les entrées de fichiers OpenResponses, le texte `input_file` décodé est toujours injecté comme
  **contenu externe non fiable**. Ne vous fiez pas au fait que le texte du fichier est fiable simplement parce que
  le Gateway l'a décodé localement. Le bloc injecté porte toujours des marqueurs explicites
  de limite `<<<EXTERNAL_UNTRUSTED_CONTENT ...>>>` ainsi que des métadonnées `Source: External`,
...
- Activation de la mise en bac à sable (sandboxing) et de listes d'autorisation d'outils strictes pour tout agent qui traite des entrées non fiables.
...
La résistance à l'injection de prompt n'est **pas** uniforme selon les niveaux de modèles. Les modèles plus petits/moins chers sont généralement plus susceptibles d'être utilisés à tort via les outils et de voir leurs instructions détournées, surtout sous des prompts adverses.
...
- **Use the latest generation, best-tier model** pour tout bot pouvant exécuter des outils ou accéder à des fichiers/réseaux.
- **Do not use older/weaker/smaller tiers** pour les agents avec outils ou les boîtes de réception non fiables ; le risque d'injection par prompt est trop élevé.
- If you must use a smaller model, **reduce blast radius** (read-only tools, strong sandboxing, minimal filesystem access, strict allowlists).
- When running small models, **enable sandboxing for all sessions** et **disable web_search/web_fetch/browser** sauf si les entrées sont strictement contrôlées.
...
n'était pas destiné à un channel public. Dans les contextes de groupe, considérez-les comme **debug
only** et gardez-les désactivés sauf si vous en avez explicitement besoin.

Consignes :

- Keep `/reasoning` et `/verbose` désactivés dans les salles publiques.
...
- Les liaisons non bouclées (`"lan"`, `"tailnet"`, `"custom"`) élargissent la surface d'attaque. Ne les utilisez qu'avec une authentification de passerelle (jeton/mot de passe partagé ou un proxy de confiance non bouclé correctement configuré) et un véritable pare-feu.

Règles empiriques :

- Privilégiez Tailscale Serve aux liaisons LAN (Serve garde la Gateway en bouclage, et Tailscale gère l'accès).
- Si vous devez vous lier au LAN, configurez le pare-feu du port avec une liste d'autorisation stricte d'adresses IP source ; ne redirigez pas le port largement.
...
(`-p HOST:CONTAINER` ou Compose `ports:`) sont acheminés via les chaînes de
transfert de Docker, et pas seulement les règles hôte `INPUT`.
...
varient selon les images VPS (`ens3`, `enp*`, etc.) et les incohérences pourraient accidentellement
...
Les ports externes attendus doivent être uniquement ceux que vous exposez intentionnellement (pour la plupart des configurations : SSH + vos ports de proxy inverse).

### 0.4.2) Découverte mDNS/Bonjour (divulgation d'informations)

Le Gateway diffuse sa présence via mDNS (`_openclaw-gw._tcp` sur le port 5353) pour la découverte d'appareils locaux. En mode complet, cela inclut des enregistrements TXT qui peuvent révéler des détails opérationnels :
...
### 0.5) Verrouillez le WebSocket du Gateway (authentification locale)

L'authentification du Gateway est **requise par défaut**. Si aucun chemin d'authentification de passerelle valide n'est configuré, le Gateway refuse les connexions WebSocket (échec sécurisé).

L'intégration génère un jeton par défaut (même pour le bouclage) afin que les clients locaux doivent s'authentifier.
...
Remarque : `gateway.remote.token` / `.password` sont des sources d'identifiants client. Ils ne protègent **pas** par eux-mêmes l'accès WS local.
...
Le `ws://` en texte clair est réservé au bouclage local par défaut. Pour les chemons de réseau privé de confiance, définissez `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` sur le processus client en mode bris de glace.

Jumelage d'appareils local :

- Le jumelage d'appareils est auto-approuvé pour les connexions en boucle locale directes afin de garder les clients sur le même hôte fluides.
- OpenClaw possède également un chemin étroit de connexion locale backend/conteneur pour les flux d'aide à secret partagé de confiance.
...
- `gateway.auth.mode: "trusted-proxy"` : faire confiance à un proxy inverse conscient de l'identité pour authentifier les utilisateurs et transmettre l'identité via les en-têtes (voir [Authentification Proxy de Confiance](/en/gateway/trusted-proxy-auth)).
...
Control UI/WebSocket. Tailscale vérifie l'identité en résolvant l'adresse
...
le bouclage local (loopback) et incluent `x-forwarded-for`, `x-forwarded-proto`, et `x-forwarded-host` tels
qu'injectés par OpenClaw.
Pour ce chemin de vérification d'identité asynchrone, les tentatives échouées pour le même `{scope, ip}`
sont sérialisées avant que le limiter n'enregistre l'échec. Les nouvelles tentatives simultanées
incorrectes d'un client Serve peuvent donc verrouiller la deuxième tentative immédiatement
au lieu de se concurrencer comme deux simples discordances.
Les points de terminaison de l'Tailscale HTTP (par exemple `/v1/*`, `/tools/invoke`, et `/api/channels/*`)
...
Note importante sur la limite de confiance (boundary) :
...
- Sur la surface HTTP compatible avec OpenAI, l'authentification Bearer par secret partagé restaure les étendues d'opérateur par défaut complètes (`operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`) et la sémantique de propriétaire pour les tours de l'agent ; des valeurs `x-openclaw-scopes` plus restreintes ne réduisent pas ce chemin par secret partagé.
- La sémantique d'étendue par requête sur HTTP ne s'applique que lorsque la requête provient d'un mode porteur d'identité tel que l'authentification par proxy de confiance ou `gateway.auth.mode="none"` sur une entrée privée.
- Dans ces modes porteurs d'identité, l'omission de `x-openclaw-scopes` revient à l'ensemble d'étendues par défaut de l'opérateur normal ; envoyez l'en-tête explicitement lorsque vous souhaitez un ensemble d'étendues plus restreint.
- `/tools/invoke` suit la même règle de secret partagé : l'authentification par porteur de jeton/mot de passe y est également considérée comme un accès complet à l'opérateur, tandis que les modes porteurs d'identité honorent toujours les portées déclarées.
...
sur la machine du navigateur et laissez la Gateway proxyer les actions du navigateur (voir [tool de navigateur](/en/tools/browser)).
...
- `openclaw.json` : la configuration peut inclure des jetons (passerelle, passerelle distante), les paramètres du fournisseur et les listes blanches.
- `credentials/**` : identifiants du canal (exemple : identifiants WhatsApp), listes blanches d'appariement, importations OAuth héritées.
...
- Privilégiez un compte utilisateur dédié sur le système d'exploitation pour le Gateway si l'hôte est partagé.

### 0.8) Journaux + transcriptions (rédaction et rétention)
...
Dans les discussions de groupe, répondre uniquement lorsqu'elle est explicitement mentionnée.
...
### 4) Mode lecture seule (via sandbox + tools)
...
Une configuration « par défaut sûre » qui maintient le Gateway privé, nécessite un appairage DM et évite les bots de groupe toujours actifs :
...
Ligne de base intégrée pour les tours d'agent pilotés par chat : les expéditeurs non propriétaires ne peuvent pas utiliser les tools `cron` ou `gateway`.
...
- **Bac à sable d'outils** (`agents.defaults.sandbox`, passerelle hôte + outils isolés par Docker) : [Sandboxing](/en/gateway/sandboxing)

Remarque : pour éviter l'accès inter-agent, maintenez `agents.defaults.sandbox.scope` à `"agent"` (par défaut)
...
- Les `sandbox.docker.binds` supplémentaires sont validés par rapport aux chemins source normalisés et canonisés. Les astuces de lien symbolique parent et les alias canoniques du répertoire personnel échouent toujours de manière fermée s'ils résolvent vers des racines bloquées telles que `/etc`, `/var/run`, ou les répertoires d'identifiants sous le répertoire personnel du système d'exploitation.

Important : `tools.elevated` est l'échappatoire de base global qui exécute exec en dehors du bac à sable. L'hôte effectif est `gateway` par défaut, ou `node` lorsque la cible exec est configurée sur `node`. Maintenez `tools.elevated.allowFrom` strict et ne l'activez pas pour les inconnus. Vous pouvez restreindre davantage les droits élevés par agent via `agents.list[].tools.elevated`. Voir [Elevated Mode](/en/tools/elevated).
...
- Gardez `agents.defaults.subagents.allowAgents` et toute substitution `agents.list[].subagents.allowAgents` par agent limités aux agents cibles connus comme sûrs.
...
Activer le contrôle du navigateur donne au model la capacité de piloter un vrai navigateur.
...
- L'API de contrôle du navigateur en boucle autonome ne respecte que l'authentification par secret partagé
  (authentification par porteur de jeton de passerelle ou mot de passe de passerelle). Elle ne consomme pas
...
- Pour les passerelles distantes, supposez que « le contrôle du navigateur » équivaut à « l'accès opérateur » à tout ce que ce profil peut atteindre.
- Gardez le Gateway et les hôtes de nœuds en accès tailnet uniquement ; évitez d'exposer les ports de contrôle du navigateur au réseau local ou à l'Internet public.
- Désactivez le routage proxy du navigateur lorsque vous n'en avez pas besoin (`gateway.nodes.browser.mode="off"`).
- Le mode de session existant Chrome MCP n'est **pas** plus « sûr » ; il peut agir en votre nom dans tout ce que le profil Chrome hôte peut atteindre.

### Stratégie SSRF du navigateur (stricte par défaut)

La stratégie de navigation du navigateur de OpenClaw est stricte par défaut : les destinations privées/internes restent bloquées à moins que vous ne vous y abonniez explicitement.
...
- La navigation est vérifiée avant la demande et revérifiée au mieux sur l'URL finale `http(s)` après navigation pour réduire les pivots basés sur les redirections.
...
3. **Gel de l'accès :** passez les DMs/groupes risqués en `dmPolicy: "disabled"` / exigez des mentions, et supprimez les entrées `"*"` autorisant tout si vous en aviez.

### Faire une rotation (supposer une compromission si des secrets ont fuité)

1. Faire une rotation de l'auth Gateway (`gateway.auth.token` / `OPENCLAW_GATEWAY_PASSWORD`) et redémarrez.
2. Faire une rotation des secrets de client distant (`gateway.remote.token` / `.password`) sur toute machine pouvant appeler le Gateway.
3. Faire une rotation des identifiants provider/API (identifiants WhatsApp, jetons Slack/Discord, clés model/API dans `auth-profiles.json`, et les valeurs de payload de secrets chiffrés lors de leur utilisation).

### Audit

1. Vérifiez les journaux du Gateway : `/tmp/openclaw/openclaw-YYYY-MM-DD.log` (ou `logging.file`).
2. Consultez la ou les transcription(s) pertinente(s) : `~/.openclaw/agents/<agentId>/sessions/*.jsonl`.
3. Examinez les récentes modifications de configuration (tout ce qui aurait pu élargir l'accès : `gateway.bind`, `gateway.auth`, stratégies de groupe/de messages directs, `tools.elevated`, modifications de plugins).
...
- La ou les transcription(s) de session + une fin de journal courte (après suppression des données sensibles)
- Ce que l'attaquant a envoyé + ce que l'agent a fait
- Si la Gateway a été exposée au-delà de la boucle locale (LAN/Tunnel Tailscale/Serve)

## Analyse de secrets (detect-secrets)

L'CI exécute le hook de pré-commit `detect-secrets` dans la tâche `secrets`.
Les envois vers `main` exécutent toujours une analyse sur tous les fichiers. Les demandes de tirage (pull requests) utilisent un chemin rapide pour les fichiers modifiés lorsqu'une base de référence est disponible, et reviennent à une analyse sur tous les fichiers sinon. Si cela échoue, il y a de nouveaux candidats qui ne sont pas encore dans la ligne de base.
...
5. Si vous avez besoin de nouvelles exclusions, ajoutez-les à `.detect-secrets.cfg` et régénérez la
   ligne de base avec les indicateurs correspondants `--exclude-files` / `--exclude-lines` (le fichier
   de configuration est uniquement pour référence ; detect-secrets ne le lit pas automatiquement).
