...
<CardGroup cols={2}>
  ...
  <Card title="Secrets plan contract" icon="shield-check" href="/fr/gateway/secrets-plan-contract">
    Règles exactes de `secrets apply` cible/chemin et comportement de profil d'authentification référence uniquement.
  </Card>
</CardGroup>
...
<Note>Le rechargement de la configuration du Gateway surveille le chemin du fichier de configuration actif (résolu à partir des valeurs par défaut du profil/état, ou `OPENCLAW_CONFIG_PATH` si défini). Le mode par défaut est `gateway.reload.mode="hybrid"`.</Note>
