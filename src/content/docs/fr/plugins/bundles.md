...
| Outils MCP              | Configuration MCP du bundle fusionnée dans les paramètres d'OpenClaw intégré ; serveurs stdio et HTTP pris en charge chargés             | Tous les formats |
| Serveurs LSP            | `.lsp.json` de Claude et `lspServers`OpenClaw déclarés dans le manifeste fusionnés dans les paramètres LSP par défaut d'OpenClaw intégré | Claude           |
| Paramètres              | `settings.json`OpenClaw de Claude importé en tant que paramètres par défaut d'OpenClaw intégré                                           | Claude           |
...
- OpenClaw fusionne la configuration MCP du bundle dans les paramètres effectifs d'OpenClaw intégré sous la forme
  OpenClawOpenClaw`mcpServers`
- OpenClaw expose les outils MCP du bundle pris en charge pendant les tours de l'agent OpenClaw intégré en
...
- les paramètres de l'agent intégré locaux au projet s'appliquent toujours après les valeurs par défaut du bundle, donc les
  paramètres de l'espace de travail peuvent remplacer les entrées MCP du bundle si nécessaire
...
- l'ordre final des outils exposés est déterministe par nom sécurisé pour maintenir la stabilité du cache des tours
  répétés de l'agent intégré
...
- Le `settings.json`OpenClaw de Claude est importé en tant que paramètres par défaut d'OpenClaw intégré lorsque le
  bundle est activé
...
- la configuration LSP du bundle est fusionnée dans les valeurs par défaut LSP effectives d'OpenClaw intégré
...
<AccordionGroup>
  ...
  <Accordion title="Claude bundles">
    ...
    - `.lsp.json` plus les chemins `lspServers` déclarés dans le manifeste chargent les valeurs par défaut LSP de OpenClaw intégré
    - `hooks/hooks.json` est détecté mais non exécuté
    - Les chemins des composants personnalisés dans le manifeste sont additifs (ils étendent les valeurs par défaut, ne les remplacent pas)
...
<Accordion title="Claude settings do not apply">Seuls les paramètres OpenClaw intégrés provenant de `settings.json` sont pris en charge. OpenClaw ne traite pas les paramètres de bundle comme des correctifs de configuration bruts.</Accordion>
