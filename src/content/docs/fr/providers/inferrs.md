...
[inferrs](https://github.com/ericcurtin/inferrs) peut servir des modèles locaux derrière une API compatible OpenAI `/v1`. OpenClaw fonctionne avec `inferrs` via le chemin générique `openai-completions`.

`inferrs` est actuellement mieux traité comme un backend personnalisé auto-hébergé compatible OpenAI, et non comme un plugin de fournisseur dédié OpenClaw.
