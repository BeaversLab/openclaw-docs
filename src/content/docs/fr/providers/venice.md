...
Venice AI fournit une **inférence IA axée sur la confidentialité** avec prise en charge des modèles non censurés et accès aux modèles propriétaires majeurs via leur proxy anonymisé. Toute inférence est privée par défaut — aucune formation sur vos données, aucune journalisation.
...
| **Anonymisé** | Acheminé via proxy Venice avec les métadonnées supprimées. Le fournisseur sous-jacent (OpenAI, Anthropic, Google, xAI) voit des demandes anonymisées. | Claude, GPT, Gemini, Grok                                     |

<Warning>Les modèles anonymisés ne sont **pas** entièrement privés. Venice supprime les métadonnées avant le transfert, mais le fournisseur sous-jacent (OpenAI, Anthropic, Google, xAI) traite toujours la demande. Choisissez les modèles **Privés** lorsque une confidentialité totale est requise.</Warning>
...
- **Appel de fonction** : Pris en charge sur certains modèles (vérifiez les capacités du modèle)
- **Vision** : Pris en charge sur les modèles avec capacité de vision
- **Pas de limites de débit strictes** : Une limitation d'utilisation équitable peut s'appliquer pour une utilisation extrême
  ...
  <Step title="Configurer OpenClaw">
    ...
    <Tabs>
      <Tab title="Interactif (recommandé)">
        ...
        Cela permettra :
        1. Demander votre clé API (ou utiliser la `VENICE_API_KEY` existante)
...
- **Option haute capacité** : `venice/claude-opus-4-6` pour le chemin anonymisé Venice le plus robuste.
...
| **Confidentialité + codage**  | `qwen3-coder-480b-a35b-instruct` | Modèle de codage privé avec un contexte large          |
...
| **Tâches privées complexes**  | `deepseek-v3.2`                  | Raisonnement fort, mais pas de support d'Venice Venice |
...
<AccordionGroup>
  <Accordion title="Modèles privés (26) — entièrement privés, sans journalisation">
    ...
    | `venice-uncensored`                    | Venice Uncensored (Dolphin-Mistral) | 32k      | Sans censure, outils désactivés |
  ...
  <Accordion title="VeniceModèles anonymisés (12) — via proxy Venice">
    | ID du modèle                     | Nom                            | Contexte | Fonctionnalités             |
    | ------------------------------- | ------------------------------ | -------- | --------------------------- |
    | `claude-opus-4-6`Venice               | Claude Opus 4.6 (via Venice)   | 1M       | Raisonnement, vision        |
    | `claude-sonnet-4-6`Venice             | Claude Sonnet 4.6 (via Venice) | 1M       | Raisonnement, vision        |
    | `openai-gpt-54`Venice                 | GPT-5.4 (via Venice)           | 1M       | Raisonnement, vision        |
    | `openai-gpt-53-codex`Venice           | GPT-5.3 Codex (via Venice)     | 400k     | Raisonnement, vision, code  |
    | `openai-gpt-52`Venice                 | GPT-5.2 (via Venice)           | 256k     | Raisonnement                |
    | `openai-gpt-52-codex`Venice           | GPT-5.2 Codex (via Venice)     | 256k     | Raisonnement, vision, code  |
    | `openai-gpt-4o-2024-11-20`Venice      | GPT-4o (via Venice)            | 128k     | Vision                      |
    | `openai-gpt-4o-mini-2024-07-18`Venice | GPT-4o Mini (via Venice)       | 128k     | Vision                      |
    | `gemini-3-1-pro-preview`Venice        | Gemini 3.1 Pro (via Venice)    | 1M       | Raisonnement, vision        |
    | `gemini-3-pro-preview`Venice          | Gemini 3 Pro (via Venice)      | 198k     | Raisonnement, vision        |
    | `gemini-3-flash-preview`Venice        | Gemini 3 Flash (via Venice)    | 256k     | Raisonnement, vision        |
    | `grok-41-fast`Venice                  | Grok 4.1 Fast (via Venice)     | 1M       | Raisonnement, vision        |
  </Accordion>
</AccordionGroup>
...
- **Private models** : Coût généralement inférieur
- **Anonymized models** : Similaire à la tarification directe de l'API + petits frais de Venice

### Venice (anonymisé) vs Venice direct

| Aspect              | Venice (Anonymisé)                              | API direct                 |
| ------------------- | ----------------------------------------------- | -------------------------- |
...
<CardGroup cols={2}>
  ...
  <Card%%PH:JSX_ATTR:43:da09ded0%Venice AI" href="https://venice.ai" icon="globe">
  ...
  <Card title="Documentation de l'API" href="https://docs.venice.ai" icon="book">
    Référence de l'Venice API et documentation développeur.
  </Card>
  <Card title="Pricing" href="https://venice.ai/pricing" icon="credit-card">
    Tarifs actuels et plans de crédit Venice.
  </Card>
</CardGroup>
