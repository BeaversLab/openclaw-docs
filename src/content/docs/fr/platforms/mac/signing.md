...
- **L'empaquetage utilise par défaut Node 24** : le script exécute les builds TS et le build de l'interface de contrôle. Node 22 LTS, actuellement `22.16+`, reste pris en charge pour la compatibilité.
...
L'onglet À propos lit ces clés pour afficher la version, la date de build, le commit git et s'il s'agit d'un build de débogage (via `#if DEBUG`). Exécutez l'empaqueteur pour actualiser ces valeurs après les modifications de code.

## Pourquoi

Les autorisations TCC sont liées à l'identifiant du bundle _et_ à la signature du code. Les builds de débogage non signés avec des UUID changeants faisaient en sorte que macOS oublie les autorisations après chaque重建. La signature des binaires (ad-hoc par défaut) et le maintien d'un identifiant/chemin de bundle fixe (`dist/OpenClaw.app`) préservent les autorisations entre les builds, correspondant à l'approche de VibeTunnel.
