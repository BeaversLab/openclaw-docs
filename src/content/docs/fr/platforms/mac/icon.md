...
- **En cours de fonctionnement (agent en cours d'exécution) :** `AppState.isWorking=true` pilote un micro-mouvement de "ruée de queue/pattes" : un mouvement plus rapide des pattes et un léger décalage pendant que le travail est en cours. Actuellement, activé/désactivé autour des exécutions de l'agent WebChat ; ajoutez le même interrupteur pour d'autres tâches longues lorsque vous les connecterez.
...
- La ruée utilise un mouvement de pattes jusqu'à environ 1,0 avec un petit gigotement horizontal ; cela s'ajoute à tout mouvement de repos existant.

Notes comportementales

- Pas d'interrupteur CLI/broker externe pour les oreilles/travail ; gardez-le interne aux signaux de l'application pour éviter les battements accidentels.
