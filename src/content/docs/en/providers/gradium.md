...
Among configured TTS providers, Gradium's auto-select order is `30`. See [Text-to-Speech](/en/tools/tts) for how OpenClaw picks the active provider when `messages.tts.provider` is not pinned.
