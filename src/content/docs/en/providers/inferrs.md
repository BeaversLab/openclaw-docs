...
<Note>
  `inferrs` is a custom self-hosted OpenAI-compatible backend, not a dedicated OpenClaw provider plugin: you configure it under `models.providers.inferrs` instead of picking an onboarding auth choice. For a bundled plugin with auto-discovery, see [SGLang](/en/providers/sglang) or [vLLM](/en/providers/vllm).
</Note>
...
`command` must be an absolute path. Run `which inferrs` on the Gateway host and use that path. Full field reference: [Local model services](/en/gateway/local-model-services).
...
<Tip>
For general help, see [Troubleshooting](/en/help/troubleshooting) and [FAQ](/en/help/faq).
</Tip>
