...
For cloud-only access without a local daemon, use `openclaw onboard --auth-choice ollama-cloud` and see [Ollama Cloud](/en/providers/ollama-cloud) — that path does not need `ollama signin` or a running server:
...
<AccordionGroup>
  ...
  <Accordion title="Small local model profile">
    Local Ollama models automatically use structured [Tool Search](/en/tools/tool-search)
...
<Note>
For full setup and behavior, see [Ollama Web Search](/en/tools/ollama-search).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Memory embeddings">
    ...
    [memory search](/en/concepts/memory). It uses the configured Ollama base URL
    ...
    see [Managed proxy](/en/security/network-proxy#gateway-loopback-mode).
