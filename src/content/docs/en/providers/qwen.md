...
Qwen Cloud is an official external OpenClaw provider plugin with canonical id `qwen`. It targets Qwen Cloud / Alibaba DashScope Standard and Coding Plan endpoints, exposes Token Plan as `qwen-token-plan`, keeps `modelstudio` as a compatibility alias, independently owns Alibaba's documented `bailian-token-plan` custom-provider id, and exposes the Qwen Portal token flow as [`qwen-oauth`](/en/providers/qwen-oauth).

| Property               | Value                                      |
| ---------------------- | ------------------------------------------ |
| Provider               | `qwen`                                     |
| Token Plan provider    | `qwen-token-plan`                          |
| Portal provider        | [`qwen-oauth`](/en/providers/qwen-oauth)      |
...
<Tabs>
  ...
  <Tab title="Qwen OAuth / Portal">
    **Best for:** a Qwen Portal token against `https://portal.qwen.ai/v1`.
    ...
    See [Qwen OAuth / Portal](/en/providers/qwen-oauth) for the dedicated provider
...
<CardGroup cols={2}>
  ...
  <Card title="Alibaba Model Studio" href="/en/providers/alibaba" icon="cloud">
