...
<Tabs>
  ...
  <Tab title="Gemini CLI runtime">
    **Advanced use only:** run a canonical `google/*` model through an installed
    ...
    See [CLI backends](/en/gateway/cli-backends) for its argv, JSONL dialect, and
...
[Gemini search](/en/tools/gemini-search) for the provider-specific tool behavior.
...
work; saved voice defaults stay unchanged. See [Discord voice changes](/en/channels/discord/voice-follow).
...
<CardGroup cols={2}>
  ...
  <Card title="Voice call plugin" href="/en/plugins/voice-call" icon="phone">
