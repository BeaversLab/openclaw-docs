...
<Note>
  OpenClaw pins all Fireworks Kimi models to `thinking: off` because Kimi on Fireworks can leak chain-of-thought into the visible reply unless the request explicitly disables thinking. Routing the same model through [Moonshot](/en/providers/moonshot) directly preserves Kimi reasoning output. See [thinking modes](/en/tools/thinking) for switching between providers.
</Note>
