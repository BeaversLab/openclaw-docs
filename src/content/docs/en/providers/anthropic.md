...
<Tabs>
  ...
  <Tab title="Claude CLI">
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Note>
    Setup and runtime details for the Claude CLI backend are in [CLI Backends](/en/gateway/cli-backends).
    </Note>
    ...
    [Claude CLI backend in Docker](/en/install/docker#claude-cli-backend-in-docker).
    Other container installs such as [Podman](/en/install/podman) do not mount host
    `~/.claude` into setup or runtime; use an Anthropic API key there, or choose
    a provider with OpenClaw-managed OAuth such as
    [OpenAI Codex](/en/providers/openai).
    </Warning>
    ...
    **Anthropic setup-token** under **Connect with an API key or token**, or use:
    ...
    [OpenAI Codex](/en/providers/openai), [Qwen Cloud](/en/providers/qwen),
    [MiniMax](/en/providers/minimax), and [Z.AI / GLM](/en/providers/zai).
    </Tip>
...
under [Tool calls and retained thinking](/en/providers/anthropic#tool-calls-and-retained-thinking).
...
[server-side tool-result clearing](/en/concepts/session-pruning#direct-anthropic-api-key-requests).
...
see [Multi-user mode](/en/concepts/multi-user).
...
See [Nodes: Claude sessions and transcripts](/en/nodes/session-catalogs#claude-sessions-and-transcripts)
...
2. Only a safety-classifier decline triggers the fallback. Rate limits,
   ...
   OpenClaw's normal [model failover](/en/concepts/model-failover).
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    ...
    Read-through is separate from copying: non-portable profiles can still be used from the shared store. Explicit copy flows follow the [agent copy portability policy](/en/auth-credential-semantics#agent-copy-portability).

    Native Claude CLI logins remain owned by Claude Code, not the shared OpenClaw auth store. For that route, use the [Claude CLI setup](/en/providers/anthropic#getting-started); do not copy native OAuth tokens into OpenClaw.
...
<CardGroup cols={2}>
  ...
  <Card title="Claude Max API proxy" href="/en/providers/claude-max-api-proxy" icon="shuffle">
    Community proxy exposing Claude subscription credentials as an OpenAI-compatible endpoint.
  </Card>
  <Card title="Anthropic plugin reference" href="/en/plugins/reference/anthropic" icon="plug">
