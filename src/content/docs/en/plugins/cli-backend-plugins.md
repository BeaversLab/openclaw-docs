...
<Info>
  ...
  [provider plugin](/en/plugins/sdk-provider-plugins) instead. If the upstream
  runtime owns complete agent sessions, tool events, compaction, or background
  task state, use an [agent harness](/en/plugins/sdk-agent-harness).
</Info>
...
<Steps>
  <Step title="Create package metadata">
    ...
    built JavaScript peer. See [Entry points](/en/plugins/sdk-entrypoints).
...
Rooted runs such as [Skill Workshop reviews](/en/tools/skill-workshop) also require
...
- [CLI backends](/en/gateway/cli-backends) - runtime selection and behavior
- [Building plugins](/en/plugins/building-plugins) - package and manifest basics
- [Plugin SDK overview](/en/plugins/sdk-overview) - registration API reference
- [Plugin manifest](/en/plugins/manifest) - `cliBackends` and setup descriptors
- [Agent harness](/en/plugins/sdk-agent-harness) - full external agent runtimes
