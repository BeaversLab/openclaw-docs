...
How plugin code reads the runtime config snapshot, persists config writes, and reuses the shared runtime utilities. Part of the [Plugin runtime helpers](/en/plugins/sdk-runtime) reference; the [`api.runtime.config` namespace](/en/plugins/sdk-runtime/state-and-system#api-runtime-config) holds the matching namespace entry.
...
This predicate does not enable or authorize [audit identity collection](/en/gateway/audit).
