...
the [local Codex configuration setup](/en/plugins/codex-harness/native-features#use-an-existing-local-configtoml).
...
Watched adopted Chats also participate in [session state awareness](/en/concepts/session-state).
...
these lists until its first own user turn. See [catalog hydration and bounds](/en/plugins/codex-harness).
...
See [native CLI creation](/en/web/control-ui/sessions-and-sidebar#start-a-native-coding-cli) for UI controls
...
[Fork a message in a supervised Chat](/en/plugins/codex-supervision#fork-a-message-in-a-supervised-chat).
...
[Codex harness reference](/en/plugins/codex-harness-reference#supervision).
...
- [Codex supervision architecture](/en/specs/codex-supervision)
- [Nodes](/en/nodes)
