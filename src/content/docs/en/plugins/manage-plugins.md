...
[`openclaw plugins`](/en/cli/plugins).
...
[CLI reload](/en/cli/plugins#reload) for that boundary. The separate **Reload plugin UI**
...
[Config hot reload](/en/gateway/configuration/hot-reload).
...
See [Install](/en/cli/plugins#install) for source selection and capability consent.
...
See [Plugin management RPCs](/en/gateway/protocol).
...
See [`openclaw plugins`](/en/cli/plugins#update) for the exact fallback and
pinning rules.
...
- [ClawHub publishing](/en/clawhub/publishing) explains owners, scopes,
  releases, review, package validation, and package transfer.
- [Building plugins](/en/plugins/building-plugins) shows the full plugin
  package shape (including `openclaw.plugin.json`) and first publish
  workflow.
- [Plugin manifest](/en/plugins/manifest) defines native plugin manifest
  fields.
...
- [Plugins](/en/tools/plugin) - install, configure, reload, and troubleshoot
- [`openclaw plugins`](/en/cli/plugins) - full CLI reference
- [Community plugins](/en/plugins/community) - public discovery and ClawHub publishing
- [ClawHub](/en/clawhub/cli) - registry CLI operations
- [Building plugins](/en/plugins/building-plugins) - create a plugin package
