...
Plugin config keys, the call owner, the full config reference table, and session scope. Part of the [Voice call plugin](/en/plugins/voice-call) guide.
...
change reloads the plugin automatically; see [Hot reload](/en/gateway/configuration/hot-reload).
Existing legacy default-agent selection is preserved; new multi-agent setups
should use an explicit owner. See [Agent configuration](/en/gateway/config-agents).
...
| `inboundPolicy`                 | `"disabled"` | `disabled` \| `allowlist` \| `pairing` \| `open`. See [Inbound calls](/en/plugins/voice-call/tts-and-inbound-calls#inbound-calls). |
...
| `staleCallReaperSeconds`        | `120`        | See [Stale call reaper](/en/plugins/voice-call/tts-and-inbound-calls#stale-call-reaper). `0` disables it.                          |
