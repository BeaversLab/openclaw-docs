...
[Control UI](/en/web/control-ui). It can also generate daily standup notes and
...
- The bundled Codex plugin enabled and authenticated. Codex provides
  ...
  [Codex harness](/en/plugins/codex-harness) for other auth paths.
...
[Hot reload](/en/gateway/configuration/hot-reload)). If the Gateway is offline,
...
See [Node troubleshooting](/en/nodes/troubleshooting) if the command is missing.
