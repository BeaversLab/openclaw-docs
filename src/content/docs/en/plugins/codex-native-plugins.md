...
Use this page after the base [Codex harness](/en/plugins/codex-harness) is
working.
...
[`openclaw migrate`](/en/cli/migrate) to point at a different one with `--from`.
...
[Automations](/en/automation/cron-jobs#codex-apps-in-scheduled-automations).
...
- [Configuration reference](/en/gateway/config-extensions#codex-harness-plugin-config)
- [Migrate CLI](/en/cli/migrate)
