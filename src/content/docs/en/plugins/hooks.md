...
Use [internal hooks](/en/automation/hooks) instead when you want a small
...
- `requireApproval` pauses the agent run and asks the user through plugin
  ...
  to the matching app-server approval request; see [Codex harness runtime](/en/plugins/codex-harness-runtime#hook-boundaries).
...
- **Plaintext channel envelopes** in `inbound_claim` and `message_received`
  ...
  [Plaintext channel envelopes → BodyForAgent](/en/plugins/sdk-migration#active-deprecations).
...
[Plugin SDK migration → Active deprecations](/en/plugins/sdk-migration#active-deprecations).
