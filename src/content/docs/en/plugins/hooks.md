...
- Native tool relays can have narrower contracts. Codex native tools support
  ...
  [Codex hook boundaries](/en/plugins/codex-harness-runtime#hook-boundaries).
- `requireApproval` pauses the agent run and asks the user through plugin
  ...
  [Codex harness runtime](/en/plugins/codex-harness-runtime#hook-boundaries).
...
See [Plugin permission requests](/en/plugins/plugin-permission-requests) for
...
[Codex transcript boundaries](/en/plugins/codex-harness-runtime#compaction-and-transcript-mirror).
...
- `before_prompt_build` with `{ requiresToolAuthority: true }`: runs in a
  ...
  tool. See [Authorized prompt enrichment](/en/plugins/hooks#authorized-prompt-enrichment).
...
[Permissions and scope](/en/plugins/hooks#permissions-and-scope) before enabling these hooks.
...
[migration note](/en/plugins/sdk-migration#deactivate-hook-alias).
