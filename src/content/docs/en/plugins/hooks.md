...
| Run an operator-installed script for `/new`, `/reset`, `/stop`, or bootstrap events | [Internal hooks](/en/automation/hooks): `HOOK.md` and colon event names such as `command:new` or `agent:bootstrap` |
| Trigger an agent from an external service over HTTP                                 | [Webhooks](/en/automation/cron-jobs#webhooks): Gateway HTTP endpoints                                              |
...
catalog](/en/plugins/hooks/reference#hook-catalog).
...
[Building plugins](/en/plugins/building-plugins) and [Plugin manifest](/en/plugins/manifest).
...
see [Troubleshooting](/en/plugins/hooks#troubleshooting).
...
[Codex hook boundaries](/en/plugins/codex-harness-runtime#hook-boundaries) and
...
- **Plaintext channel envelopes** in `inbound_claim` and `message_received`
  ...
  [Plaintext channel envelopes → BodyForAgent](/en/plugins/sdk-migration#removal-timeline).
...
[Plugin SDK migration → Active deprecations](/en/plugins/sdk-migration#removal-timeline).
...
[Hook reference](/en/plugins/hooks/reference) — Registration rules, execution contracts, per-handler budgets, and the complete typed hook catalog.

- <a id="registration-and-execution"></a>[Registration and execution](/en/plugins/hooks/reference#registration-and-execution)
- <a id="hook-catalog"></a>[Hook catalog](/en/plugins/hooks/reference#hook-catalog)
- <a id="skill-lifecycle-and-evaluation"></a>[Skill lifecycle and evaluation](/en/plugins/hooks/reference#skill-lifecycle-and-evaluation)
- <a id="channel-pairing-requests"></a>[Channel pairing requests](/en/plugins/hooks/reference#channel-pairing-requests)

### Tool call policy hooks

[Tool call policy hooks](/en/plugins/hooks/tool-policy) — Parameter rewrites, blocks, approvals, exec environment contributions, and transcript persistence.

- <a id="tool-call-policy"></a>[Tool call policy](/en/plugins/hooks/tool-policy#tool-call-policy)
- <a id="sender-aware-policy-in-one-file"></a>[Sender-aware policy in one file](/en/plugins/hooks/tool-policy#sender-aware-policy-in-one-file)
- <a id="exec-environment-hook"></a>[Exec environment hook](/en/plugins/hooks/tool-policy#exec-environment-hook)
- <a id="tool-result-persistence"></a>[Tool result persistence](/en/plugins/hooks/tool-policy#tool-result-persistence)

### Prompt and session hooks

[Prompt and session hooks](/en/plugins/hooks/prompt-and-session) — Model resolution, prompt construction, finalization, and durable plugin-owned session state.

- <a id="debug-runtime-hooks"></a>[Debug runtime hooks](/en/plugins/hooks/prompt-and-session#debug-runtime-hooks)
- <a id="prompt-and-model-hooks"></a>[Prompt and model hooks](/en/plugins/hooks/prompt-and-session#prompt-and-model-hooks)
- <a id="authorized-prompt-enrichment"></a>[Authorized prompt enrichment](/en/plugins/hooks/prompt-and-session#authorized-prompt-enrichment)
- <a id="session-extensions-and-next-turn-injections"></a>[Session extensions and next-turn injections](/en/plugins/hooks/prompt-and-session#session-extensions-and-next-turn-injections)

### Message and delivery hooks

[Message and delivery hooks](/en/plugins/hooks/messages) — Inbound interception, reply takeover, and outbound delivery policy.

- <a id="message-hooks"></a>[Message hooks](/en/plugins/hooks/messages#message-hooks)

### Gateway and install lifecycle hooks

[Gateway and install lifecycle hooks](/en/plugins/hooks/lifecycle) — Install-time checks, Gateway service lifecycle, and safe external cron projection.

- <a id="install-hooks"></a>[Install hooks](/en/plugins/hooks/lifecycle#install-hooks)
- <a id="gateway-lifecycle"></a>[Gateway lifecycle](/en/plugins/hooks/lifecycle#gateway-lifecycle)
- <a id="safe-external-cron-projection"></a>[Safe external cron projection](/en/plugins/hooks/lifecycle#safe-external-cron-projection)

## Related

- [Plugin SDK migration](/en/plugins/sdk-migration) - active deprecations and removal timeline
...
- [Internal hooks](/en/automation/hooks)
- [Webhooks](/en/automation/cron-jobs#webhooks)
- [Plugin architecture internals](/en/plugins/architecture-internals)
