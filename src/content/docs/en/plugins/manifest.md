...
[Enable custom plugin UI](/en/plugins/feature-plugins#enable-custom-plugin-ui) for
...
bindings below. See [Feature plugins](/en/plugins/feature-plugins) for authoring,
...
This field is publication-time authoring metadata, not a Gateway discovery hook. It does not add runtime network calls or hot reload; the existing [hosted catalog update lifecycle](/en/concepts/models#hosted-catalog-updates) is unchanged.
