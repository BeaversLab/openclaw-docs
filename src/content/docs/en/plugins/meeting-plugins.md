...
These plugins participate in meetings. They are separate from messaging channels such as the [Microsoft Teams channel](/en/channels/msteams) and from the [Voice call plugin](/en/plugins/voice-call).

## Choose a plugin

| Platform        | Plugin                                      | Accepted meeting links                                                                                      | Installation                                    | Participation paths                                      | Platform-specific capabilities                                                                                |
| --------------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Google Meet     | [`google-meet`](/en/plugins/google-meet)       | `meet.google.com/...`                                                                                       | Install from npm or ClawHub; enabled by default | Local Chrome, Chrome on a paired node, or Twilio dial-in | Can create meetings through the Meet API or a signed-in browser; can read supported Meet artifacts with OAuth |
| Microsoft Teams | [`teams-meetings`](/en/plugins/teams-meetings) | Work links under `teams.microsoft.com/l/meetup-join/...` and consumer links under `teams.live.com/meet/...` | Install from npm or ClawHub; enabled by default | Local Chrome or Chrome on a paired node                  | Guest join for work and consumer meetings                                                                     |
| Zoom            | [`zoom-meetings`](/en/plugins/zoom-meetings)   | `zoom.us/j/...` and account subdomains such as `example.zoom.us/j/...`                                      | Install from npm or ClawHub; enabled by default | Local Chrome or Chrome on a paired node                  | Guest join through the Zoom Web App                                                                           |
...
captions and writes the summary; use [`openclaw transcripts`](/en/cli/transcripts)
...
[Google Meet configuration example](/en/plugins/google-meet/config#gpt-live-with-cove)
...
These changes apply to a running Gateway automatically. If it is offline, start it before joining. Check the application result, then run the platform setup check below; see [Apply changes and inspect](/en/plugins/manage-plugins#apply-changes-and-inspect).
