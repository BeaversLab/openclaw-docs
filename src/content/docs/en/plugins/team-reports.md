...
the Gateway and adds a **Reports** tab to the [Control UI](/en/web/control-ui).
...
See [Secret management](/en/gateway/secrets) for other secret providers. If you
...
Use HTTPS, [Tailscale Serve](/en/gateway/tailscale), or a browser-trusted loopback
...
hybrid reload mode; see [Hot reload](/en/gateway/configuration/hot-reload). Secrets
...
[plugin LLM runtime](/en/plugins/sdk-runtime) for those policies.
...
[Gateway client options](/en/cli/gateway). Reading needs `operator.read`;
...
See also: [plugin configuration](/en/tools/plugin),
[Control UI](/en/web/control-ui), and [Secret management](/en/gateway/secrets).
