...
it to use the runtime integration. See [Apply changes and inspect](/en/plugins/manage-plugins#apply-changes-and-inspect).
...
The [Control UI](/en/web/control-ui) can browse the compiled wiki directly: open
...
still read another agent's directory. Use [sandboxing](/en/gateway/sandboxing) or
[separate Gateway profiles](/en/gateway/multiple-gateways) when agents do not trust
each other.
</Warning>
...
See [CLI: wiki](/en/cli/wiki) for the full command reference, including
