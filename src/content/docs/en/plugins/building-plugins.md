...
the package to [ClawHub](/en/clawhub) and users install it with:
...
```
    ...
    files. See [SDK entry points](/en/plugins/sdk-entrypoints) for the full entry
    point contract.
    ...
    For every manifest field, see [Plugin manifest](/en/plugins/manifest).
...
[plugin permission requests](/en/plugins/plugin-permission-requests) when a tool
...
For the full import map, see [Plugin SDK overview](/en/plugins/sdk-overview).
