...
Talk-back uses the shared [meeting-plugin audio setup](/en/plugins/meeting-plugins#prepare-chrome-and-audio): `BlackHole 2ch` plus SoX on macOS, or PipeWire-Pulse plus `pactl`/`pacat`/`parec` on Linux.
...
or export them with [`openclaw transcripts`](/en/cli/transcripts).
