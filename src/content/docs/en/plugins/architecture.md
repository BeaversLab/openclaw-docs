...
<CardGroup cols={2}>
  <Card title="Install and use plugins" icon="plug" href="/en/tools/plugin">
    End-user guide for adding, enabling, and troubleshooting plugins.
  </Card>
  <Card title="Building plugins" icon="rocket" href="/en/plugins/building-plugins">
    First-plugin tutorial with the smallest working manifest.
  </Card>
  <Card title="Channel plugins" icon="comments" href="/en/plugins/sdk-channel-plugins">
    Build a messaging channel plugin.
  </Card>
  <Card title="Provider plugins" icon="microchip" href="/en/plugins/sdk-provider-plugins">
    Build a model provider plugin.
  </Card>
  <Card title="SDK overview" icon="book" href="/en/plugins/sdk-overview">
...
Use `openclaw plugins inspect <id>` to see a plugin's shape and capability breakdown. See [CLI reference](/en/cli/plugins#inspect) for details.
...
The cache rule is documented in [Plugin architecture internals](/en/plugins/architecture-internals#plugin-cache-boundary): Gateway retains one cache generation, while explicit management operations use isolated generations of the same cache. There are no wall-clock TTLs for Gateway metadata.
...
[interrupted SQLite snapshots](/en/reference/database-schemas/integrity-and-recovery).
...
See [Plugin lifecycle and cleanup](/en/plugins/sdk-runtime#plugin-lifecycle-and-cleanup)
...
See [Plugin architecture internals](/en/plugins/architecture-internals) for the full startup sequence.
...
Need a concrete rollout checklist? See [Adding capabilities](/en/plugins/adding-capabilities).
...
For the load pipeline, registry model, provider runtime hooks, Gateway HTTP routes, message tool schemas, channel target resolution, provider catalogs, context engine plugins, and the guide to adding a new capability, see [Plugin architecture internals](/en/plugins/architecture-internals).
...
- [Context engines](/en/concepts/context-engine)
- [Plugin Runtime](/en/plugins/sdk-runtime) - the `api.runtime` helpers plugins call
