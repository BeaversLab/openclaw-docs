...
For features with native Control UI, use [Feature plugins](/en/plugins/feature-plugins):
...
| `api.registerAgentHarness(...)`                  | [Experimental](/en/plugins/sdk-agent-harness) native agent executor (Codex, Copilot) |
...
Use [`defineToolPlugin`](/en/plugins/tool-plugins) for simple tool-only plugins
...
[CLI backend plugins](/en/plugins/cli-backend-plugins).
...
See [Plugin hooks](/en/plugins/hooks) for examples, common hook names, and guard
semantics.
...
shutdown, see [Safe external cron projection](/en/plugins/hooks#safe-external-cron-projection).
