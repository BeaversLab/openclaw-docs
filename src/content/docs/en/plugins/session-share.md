...
Session Share is disabled by default. It publishes a read-only **OpenClaw sessions** catalog, not a second way to run agents on the source machine. For sanitized snapshots of external coding sessions without a paired node, see [Beam](/en/plugins/beam).
...
Check that the pairing request and connected node declare only `openclaw.sessions.list.v1` and `openclaw.sessions.read.v1`, with the `openclaw-sessions` capability. The catalog only recognizes nodes advertising both commands. See [Connect](/en/cli/connect) for join-code expiry and [Nodes](/en/nodes#restrict-the-node-command-surface) for the command allowlist.
...
Publication is shared with the receiver's permitted viewers, not just the named owner. Viewers need `operator.read`; on role-restricted Gateways, their profile's role must also permit viewing others' sessions (`sessions.others: "view"`, `"suggest"`, or `"write"`). Owner-only and unprofiled restricted viewers cannot see published rows. See [Operator scopes](/en/gateway/operator-scopes).
...
- [Connect a node](/en/cli/connect)
- [Node command allowlists](/en/nodes#restrict-the-node-command-surface)
- [Team setup](/en/start/teams)
- [Beam](/en/plugins/beam)
- [User model](/en/concepts/user-model)
