...
hooks](/en/automation/cron-jobs#webhooks). To react to internal agent events, use
[internal hooks](/en/automation/hooks). Those surfaces do not share this plugin's routes or authentication.
...
configuration changes apply automatically; see [Hot reload](/en/gateway/configuration/hot-reload).
...
[task inspection](/en/cli/tasks) for actual task status and delivery status. Keep
...
| Response              | Next check                                                                                                             |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `401`                 | Use the route secret. For an unresolved SecretRef, follow [route secret recovery](/en/plugins/webhooks#configure-routes). |
...
- [Hooks](/en/automation/hooks) - internal event-driven hooks vs. this HTTP-based TaskFlow bridge
- [Gateway webhooks (`hooks.*` config)](/en/automation/cron-jobs#webhooks) - separate generic Gateway HTTP endpoint feature. Not the same as this plugin's routes
