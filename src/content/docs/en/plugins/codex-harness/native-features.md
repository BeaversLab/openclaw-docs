...
Opt-in features that connect an OpenClaw agent to native Codex state and Codex-owned capabilities. Part of the [Codex harness](/en/plugins/codex-harness) guide; [Where each section moved](/en/plugins/codex-harness#where-each-section-moved) lists every section.
...
inherit the TOML model. See [branching behavior](/en/plugins/codex-supervision#branch-from-a-local-session).
...
copied between agents by default. See [Auth credential semantics](/en/auth-credential-semantics#agent-copy-portability)
and [Per-person model accounts](/en/concepts/multi-user#per-person-model-accounts).
...
[Codex auth and environment isolation](/en/plugins/codex-harness-reference/auth).
