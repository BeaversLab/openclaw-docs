...
Chrome, Chrome node, and Twilio transports, the Parallels macOS VM topology, and the host audio tools they need. Part of the [Google Meet plugin](/en/plugins/google-meet) guide.
...
Plugin enablement and config changes apply automatically with the default hybrid reload mode (see [Hot reload](/en/gateway/configuration/hot-reload)). If you changed the Gateway's process environment, restart it with those variables available. Verify:
