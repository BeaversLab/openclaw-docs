...
[paired-node limits](/en/plugins/codex-supervision#understand-paired-node-limits).

See [Codex supervision](/en/plugins/codex-supervision) for operator setup and the
...
see [Auth and environment isolation](/en/plugins/codex-harness-reference#auth-and-environment-isolation).
...
See [Sandboxed native execution](/en/plugins/codex-harness-reference#sandboxed-native-execution)
...
[Plugin permission requests](/en/plugins/plugin-permission-requests).
...
by default instead of steering. See [Steering queue](/en/concepts/queue-steering).
...
Gateway diagnostics export. See [Diagnostics export](/en/gateway/diagnostics) for
