...
Manifest fields that describe the models a provider plugin exposes: which shorthand ids it claims, the catalog rows core can read before runtime loads, model-id cleanup, and hosted pricing policy. Part of the [Plugin manifest](/en/plugins/manifest) reference; the [top-level field reference](/en/plugins/manifest#top-level-field-reference) lists every field.
...
The compiled OpenClaw Provider Index is retired. Model metadata comes from plugin manifests, provider-owned discovery, and the hosted model catalog, with configured overrides applied by model resolution. See [Model listing](/en/cli/models#list) for catalog sources and refresh behavior.
