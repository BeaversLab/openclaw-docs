...
Manifest fields that contribute a concrete host surface: an icon, a command, a server, a panel, a widget, a runner, a channel, or a backup resource. Part of the [Plugin manifest](/en/plugins/manifest) reference; the [top-level field reference](/en/plugins/manifest#top-level-field-reference) lists every field.
...
Settings → Appearance and the agent's [theme tool](/en/tools/theme). The same
...
colors from the [theme definition example](/en/tools/theme#create-and-apply-a-personal-theme),
...
These fields count toward the same 4096-byte normalized definition limit and are returned with the catalog descriptor. The [theme definition example](/en/tools/theme#create-and-apply-a-personal-theme) includes all four fields.
...
then fall back as described in [Plugin themes and hot reload](/en/tools/theme#plugin-themes-and-hot-reload).
...
[account lookup arguments](/en/plugins/sdk-channel-plugins/setup-and-config#stored-account-key-selection)
