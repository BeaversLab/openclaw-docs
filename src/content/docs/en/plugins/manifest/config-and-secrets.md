...
Manifest fields that generic core config helpers read: dangerous-flag detection, SecretRef migration targets, legacy path narrowing, and secret provider presets. Part of the [Plugin manifest](/en/plugins/manifest) reference; the [top-level field reference](/en/plugins/manifest#top-level-field-reference) lists every field.
