...
Manifest fields that tell core what a provider can do, and how to reach it, without importing the provider runtime. Part of the [Plugin manifest](/en/plugins/manifest) reference; the [top-level field reference](/en/plugins/manifest#top-level-field-reference) lists every field.
