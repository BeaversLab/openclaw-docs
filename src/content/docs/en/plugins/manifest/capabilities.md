...
Manifest fields that declare what a plugin owns and when the activation planner should load it. Part of the [Plugin manifest](/en/plugins/manifest) reference; the [top-level field reference](/en/plugins/manifest#top-level-field-reference) lists every field.
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for [`openclaw migrate`](/en/cli/migrate).                                                         |
...
Worker providers may set `allowsDesktopResize: true` when their desktop endpoints are safe to resize, such as dedicated virtual displays. Omission or `false` does not grant permission. Core carries this fact as optional `canResize` in desktop observation results for both SSH and node transports. It is permission to request a resize, not negotiated VNC support. The viewer must also hold control and complete authentication before requesting a resize.
