...
Where pre-runtime plugin metadata lives when it is not in the manifest, and how OpenClaw picks one manifest when two plugin roots share an id. Part of the [Plugin manifest](/en/plugins/manifest) reference; the [top-level field reference](/en/plugins/manifest#top-level-field-reference) lists every field.
...
| `openclaw.channel.cliAddOptions`                                                           | Plugin-owned [`openclaw channels add`](/en/cli/channels) options. Each entry declares `flags`, `description`, optional `defaultValue`, and optional `valueType` (`int` or `list`) for generic input coercion. |
