...
Manifest fields that setup, onboarding, and config UI surfaces read before plugin runtime loads. Part of the [Plugin manifest](/en/plugins/manifest) reference; the [top-level field reference](/en/plugins/manifest#top-level-field-reference) lists every field.
