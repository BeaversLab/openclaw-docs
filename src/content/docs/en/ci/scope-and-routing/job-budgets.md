...
Shard counts, concurrency and matrix budgets, lint memory policy, Android rows, and cache-key bounds. Part of the [CI scope and routing](/en/ci/scope-and-routing) index.
...
Ordinary full-scope manual validation retains six Android rows: the three test rows, `build-play`, `build-wear`, and Kotlin lint. The build rows own lint without repeating it in test rows. `build-play` assembles both phone flavors and the Wear shared module and compiles the benchmark; `build-wear` assembles Wear. `build-play-compat` retains Play-only packaging for older frozen targets. GitHub-hosted `build-play` gets a 35-minute job budget for its three memory-bounded Gradle invocations. Combined `test-play` and `test-third-party` unit tests and lint get 35 minutes on either runner route. Third-party Gradle invocations have completed in 18–19 minutes before checkout, toolchain setup, and cleanup. In an observed Play run, unit tests completed in 13 minutes 52 seconds, then lint was cancelled at the 20-minute job limit; this does not establish the full lint duration. Both test tasks without lint, Blacksmith `build-play`, and all other Android tasks retain 20 minutes. The budget follows the current attempt's runner route even when a retry reuses the original preflight matrix. Each current Gradle task has one protected sticky disk; PR jobs use disposable clones, while protected runs refresh content-addressed Gradle entries in place. Existing Android opt-in and npm qualification deferral are unchanged.
