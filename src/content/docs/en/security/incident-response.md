...
- [Contributing to the threat model](/en/security/CONTRIBUTING-THREAT-MODEL) — how to add a threat to the living threat model; not the path for reporting a live vulnerability.
- [Formal verification](/en/security/formal-verification) — machine-checked models of authorization, session isolation, and tool gating.
- [Operator incident response](/en/gateway/security/operator-incident-response) — contain, rotate, audit, and collect evidence after a suspected compromise of your own Gateway.
