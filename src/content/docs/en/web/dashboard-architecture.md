...
[Session Dashboards](/en/web/dashboards). For the widget authoring API, see
[Show widget](/en/tools/show-widget).
...
- **Static assets.** The shared widget CDN allowlist permits public scripts,
  ...
  contain private data. See [Libraries and fonts](/en/tools/show-widget#libraries-and-fonts).
- **Grants.** HTML and registered widgets declaring nothing render immediately
  ...
  [session permission mode](/en/gateway/permission-modes). **Full access** grants.
...
See [Show a website fullscreen](/en/web/dashboards#show-a-website-fullscreen).
...
interactive or inline content. See the [report schema and example](/en/tools/show-widget#native-dashboard-reports).
...
as success. See the
[authoring contract and example](/en/tools/show-widget#read-github-actions-runs).
...
`src/boards/sqlite-board-store.ts`. See [Database schemas](/en/reference/database-schemas)
