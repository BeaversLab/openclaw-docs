...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - Live `chat` events are delivery state, while `chat.history` is rebuilt from the durable session transcript. After tool-final events the Control UI reloads history and merges only a small optimistic tail; the transcript boundary is documented in [WebChat](/en/web/webchat).
...
See [Notifications](/en/web/notifications) for the browser and macOS setup steps.
...
<Note>
Web Push is independent of the iOS APNS relay path (see [Configuration](/en/gateway/configuration) for relay-backed push) and the `push.test` method, which targets native mobile pairing.
</Note>
