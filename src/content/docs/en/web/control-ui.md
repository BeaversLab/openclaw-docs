...
Once approved, the device is remembered and won't require re-approval unless you revoke it with `openclaw devices revoke --device <id> --role <role>`. See [Devices CLI](/en/cli/devices) for token rotation and revocation.
...
The page redacts credential-bearing URL-like values before rendering and quotes server names in command snippets so copied commands still work with spaces or shell metacharacters. The full CLI and config reference lives in [MCP](/en/cli/mcp).
...
<Note>
Web Push is independent of the iOS APNS relay path (see [Configuration](/en/gateway/configuration) for relay-backed push) and the existing `push.test` method, which target native mobile pairing.
</Note>
