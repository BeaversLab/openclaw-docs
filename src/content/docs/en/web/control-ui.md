...
and the [Tasks page](/en/automation/tasks#control-ui), outside sidebar navigation.
...
choices use the same [role templates](/en/cli/agents#role-templates) as the CLI;
...
page. See [Sidebar navigation](/en/web/control-ui/sessions-and-sidebar#sidebar-navigation)
...
- [Connect and pair](/en/web/control-ui/connect-and-pair) — pair a browser or phone, reach the UI over Tailscale, and fix a blank page.
- [Sessions and sidebar](/en/web/control-ui/sessions-and-sidebar) — sidebar zones, session menus, and the New session page.
- [Systems workspace](/en/web/control-ui/sessions-and-sidebar#systems-workspace) — contextual machine navigation and a desktop-first workspace.
- [Chat](/en/web/control-ui/chat) — composer controls, the session rail, transcript rendering, and hosted embeds.
- [Panels and docks](/en/web/control-ui/panels) — Ask OpenClaw, the Home dock, the operator terminal, and the browser panel.
- [Settings](/en/web/control-ui/settings) — identity, appearance, plugins, updates, MCP, activity, and meetings.
- [Feature and RPC reference](/en/web/control-ui/feature-reference) — every capability with the Gateway RPC behind it.
- [Offline and reconnect](/en/web/control-ui/offline-and-reconnect) — what survives a dropped connection.
- [Security model](/en/web/control-ui/security-model) — content security policy, media route auth, and approval links.
- [Build and develop](/en/web/control-ui/development) — build the UI and run the dev server against a Gateway.

Running the Gateway in Docker? See [Using the Control UI browser](/en/install/docker#using-the-control-ui-browser) for the browser-equipped image and setup requirements.
...
- <a id="session-rail-and-side-chat" />[Session rail and side chat](/en/web/control-ui/chat#session-rail-and-side-chat)
- <a id="session-links-in-messages" />[Session links in messages](/en/web/control-ui/chat#session-links-in-messages)
- <a id="composer-capability-menu" />[Composer capability menu](/en/web/control-ui/chat#composer-capability-menu)
- <a id="chat-behavior" />[Chat behavior](/en/web/control-ui/chat#chat-behavior)
- <a id="source-previews-and-copying-code" />[Source previews and copying code](/en/web/control-ui/chat#source-previews-and-copying-code)
- <a id="markdown-tables" />[Markdown tables](/en/web/control-ui/chat#markdown-tables)
- <a id="mermaid-diagrams" />[Mermaid diagrams](/en/web/control-ui/chat#mermaid-diagrams)
- <a id="hosted-embeds" />[Hosted embeds](/en/web/control-ui/chat#hosted-embeds)
- <a id="chat-transcript-layout" />[Chat transcript layout](/en/web/control-ui/chat#chat-transcript-layout)
- <a id="chat-message-width" />[Chat message width](/en/web/control-ui/chat#chat-message-width)
- <a id="send-and-history-semantics" />[send and history semantics](/en/web/control-ui/chat#send-and-history-semantics)
- <a id="talk-mode-browser-realtime" />[talk mode browser realtime](/en/web/control-ui/chat#talk-mode-browser-realtime)
- <a id="stop-and-abort" />[stop and abort](/en/web/control-ui/chat#stop-and-abort)
- <a id="abort-partial-retention" />[abort partial retention](/en/web/control-ui/chat#abort-partial-retention)
- <a id="strict" />[strict](/en/web/control-ui/chat#strict)
- <a id="scripts-default" />[scripts default](/en/web/control-ui/chat#scripts-default)
- <a id="trusted" />[trusted](/en/web/control-ui/chat#trusted)
...
- <a id="pair-a-mobile-device" />[Pair a mobile device](/en/web/control-ui/connect-and-pair#pair-a-mobile-device)
- <a id="runtime-config-endpoint" />[Runtime config endpoint](/en/web/control-ui/connect-and-pair#runtime-config-endpoint)
- <a id="pwa-install-and-web-push" />[PWA install and web push](/en/web/control-ui/connect-and-pair#pwa-install-and-web-push)
- <a id="tailnet-access-(recommended)" />[tailnet access (recommended)](</web/control-ui/connect-and-pair#tailnet-access-(recommended)>)
- <a id="insecure-http" />[Insecure HTTP](/en/web/control-ui/connect-and-pair#insecure-http)
- <a id="blank-control-ui-page" />[Blank Control UI page](/en/web/control-ui/connect-and-pair#blank-control-ui-page)
- <a id="device-pairing-first-connection" />[Device pairing (first connection)](/en/web/control-ui/connect-and-pair#device-pairing-first-connection)
- <a id="tailnet-access-recommended" />[Tailnet access (recommended)](/en/web/control-ui/connect-and-pair#tailnet-access-recommended)
- <a id="list-pending-requests" />[list pending requests](/en/web/control-ui/connect-and-pair#list-pending-requests)
- <a id="approve-by-request-id" />[approve by request id](/en/web/control-ui/connect-and-pair#approve-by-request-id)
- <a id="open-mobile-pairing" />[open mobile pairing](/en/web/control-ui/connect-and-pair#open-mobile-pairing)
- <a id="connect-the-phone" />[connect the phone](/en/web/control-ui/connect-and-pair#connect-the-phone)
- <a id="confirm-the-connection" />[confirm the connection](/en/web/control-ui/connect-and-pair#confirm-the-connection)
- <a id="trusted-proxy-note" />[trusted proxy note](/en/web/control-ui/connect-and-pair#trusted-proxy-note)
- <a id="build-and-develop-the-ui" />[Build and develop the UI](/en/web/control-ui/development#build-and-develop-the-ui)
- <a id="debugging%2Ftesting%3A-dev-server-%2B-remote-gateway" />[debugging%2Ftesting%3A dev server %2B remote gateway](/en/web/control-ui/development#debugging%2Ftesting%3A-dev-server-%2B-remote-gateway)
- <a id="debugging/testing-dev-server-+-remote-gateway" />[debugging/testing dev server + remote gateway](/en/web/control-ui/development#debugging/testing-dev-server-+-remote-gateway)
- <a id="start-the-ui-dev-server" />[start the ui dev server](/en/web/control-ui/development#start-the-ui-dev-server)
- <a id="connect-the-remote-gateway" />[connect the remote gateway](/en/web/control-ui/development#connect-the-remote-gateway)
- <a id="origin-security-notes" />[origin security notes](/en/web/control-ui/development#origin-security-notes)
- <a id="feature-and-rpc-reference" />[Feature and RPC reference](/en/web/control-ui/feature-reference#feature-and-rpc-reference)
- <a id="chat-and-talk" />[chat and talk](/en/web/control-ui/feature-reference#chat-and-talk)
- <a id="channels-sessions-memory" />[channels sessions memory](/en/web/control-ui/feature-reference#channels-sessions-memory)
- <a id="cron-tasks-plugins-skills-devices-exec-approvals" />[cron tasks plugins skills devices exec approvals](/en/web/control-ui/feature-reference#cron-tasks-plugins-skills-devices-exec-approvals)
- <a id="config" />[config](/en/web/control-ui/feature-reference#config)
- <a id="usage" />[usage](/en/web/control-ui/feature-reference#usage)
- <a id="debug-logs-update" />[debug logs update](/en/web/control-ui/feature-reference#debug-logs-update)
- <a id="automations-panel-notes" />[automations panel notes](/en/web/control-ui/feature-reference#automations-panel-notes)
- <a id="connection-loss-and-reconnect" />[Connection loss and reconnect](/en/web/control-ui/offline-and-reconnect#connection-loss-and-reconnect)
- <a id="openclaw-system-care" />[OpenClaw system care](/en/web/control-ui/panels#openclaw-system-care)
- <a id="home-dock" />[Home dock](/en/web/control-ui/panels#home-dock)
- <a id="operator-terminal" />[Operator terminal](/en/web/control-ui/panels#operator-terminal)
- <a id="browser-panel" />[Browser panel](/en/web/control-ui/panels#browser-panel)
- <a id="content-security-policy" />[Content security policy](/en/web/control-ui/security-model#content-security-policy)
- <a id="avatar-route-auth" />[Avatar route auth](/en/web/control-ui/security-model#avatar-route-auth)
- <a id="assistant-media-route-auth" />[Assistant media route auth](/en/web/control-ui/security-model#assistant-media-route-auth)
- <a id="approval-links" />[Approval links](/en/web/control-ui/security-model#approval-links)
- <a id="new-session-names" />[New session names](/en/web/control-ui/sessions-and-sidebar#new-session-names)
- <a id="new-session-preferences-and-recents" />[New-session preferences and recents](/en/web/control-ui/sessions-and-sidebar#new-session-preferences-and-recents)
- <a id="sidebar-navigation" />[Sidebar navigation](/en/web/control-ui/sessions-and-sidebar#sidebar-navigation)
- <a id="session-menu" />[Session menu](/en/web/control-ui/sessions-and-sidebar#session-menu)
- <a id="session-placement" />[Session placement](/en/web/control-ui/sessions-and-sidebar#session-placement)
- <a id="session-icons" />[Session icons](/en/web/control-ui/sessions-and-sidebar#session-icons)
- <a id="session-colors" />[Session colors](/en/web/control-ui/sessions-and-sidebar#session-colors)
- <a id="new-session-page" />[New session page](/en/web/control-ui/sessions-and-sidebar#new-session-page)
- <a id="start-a-native-coding-cli" />[Start a native coding CLI](/en/web/control-ui/sessions-and-sidebar#start-a-native-coding-cli)
- <a id="openclaw-chat-workspace-startup" />[OpenClaw Chat workspace startup](/en/web/control-ui/sessions-and-sidebar#openclaw-chat-workspace-startup)
- <a id="environment-identity" />[Environment identity](/en/web/control-ui/settings#environment-identity)
- <a id="community-invitation" />[Community invitation](/en/web/control-ui/settings#community-invitation)
- <a id="personal-identity" />[Personal identity](/en/web/control-ui/settings#personal-identity)
- <a id="gateway-host-status" />[Gateway host status](/en/web/control-ui/settings#gateway-host-status)
- <a id="language-support" />[Language support](/en/web/control-ui/settings#language-support)
- <a id="appearance-themes" />[Appearance themes](/en/web/control-ui/settings#appearance-themes)
- <a id="manage-plugins" />[Manage plugins](/en/web/control-ui/settings#manage-plugins)
- <a id="updates" />[Updates](/en/web/control-ui/settings#updates)
- <a id="apps-and-extensions" />[Apps and extensions](/en/web/control-ui/settings#apps-and-extensions)
- <a id="side-panel-keyboard-shortcuts" />[Side panel keyboard shortcuts](/en/web/control-ui/settings#side-panel-keyboard-shortcuts)
- <a id="this-mac-(macos-app)" />[This device (macOS and iOS apps)](/en/web/control-ui/settings#this-mac-macos-app)
- <a id="custom-plugin-ui" />[Custom plugin UI](/en/web/control-ui/settings#custom-plugin-ui)
- <a id="import-assistant-memory" />[Import assistant memory](/en/web/control-ui/settings#import-assistant-memory)
- <a id="mcp-page" />[MCP page](/en/web/control-ui/settings#mcp-page)
- <a id="activity-tab" />[Activity tab](/en/web/control-ui/settings#activity-tab)
- <a id="meetings-page" />[Meetings page](/en/web/control-ui/settings#meetings-page)
- <a id="this-mac-macos-app" />[This Mac (macOS app)](/en/web/control-ui/settings#this-mac-macos-app)
...
- [Codex session catalog and supervision](/en/plugins/codex-supervision) — the Native Session Discovery settings surface
