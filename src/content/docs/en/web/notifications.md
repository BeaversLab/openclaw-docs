...
This page covers those two surfaces. It does not control channel reaction notifications, Android notification forwarding, or iOS background push. The mobile apps register for push through their own node paths. See [iOS](/en/platforms/ios) and [Nodes](/en/nodes).
...
The Control UI asks for notification permission automatically the first time you send a chat message. That includes sends from **New Session** or **Start in background**. The prompt appears once per browser and origin. **Settings → Notifications** remains the manual path for enabling or repairing notifications, including after you deny the automatic prompt.
...
The recommended self-hosted setup is one Control UI service-worker scope per Gateway. Open or install each Gateway's PWA from its own HTTPS origin or base path. Enable notifications there. Reconnect once after upgrades. Each scope then owns an independent browser subscription, and approval links return to the Gateway that created them.
...
Either the browser lacks the required Web Push APIs or the Control UI is not connected to the Gateway. Try a current browser, check the Gateway connection, and reload the page.
...
For a single PWA that switches among Gateways, also check two things. Every Gateway must use the same VAPID keypair. Every Gateway must have a browser-reachable `gateway.publicOrigin`. Separate PWA origins or base-path scopes do not need to share VAPID keys.

### A mention is missing or produces no browser alert

Check that the sender selected your profile from the picker, rather than only typing your name. Check that the original message reached the transcript. Sign in with the same profile and check that you still have access to the session. Incognito, Goal, catalog, suggestion-only, and command-send modes do not support human mentions.

If the entry is in **Inbox → Mentions**, check this browser's subscription and **Someone mentions me**. Also check quiet hours, mute overrides, and the agent filter. An Inbox entry does not imply permission to show a browser notification. If the entry disappeared, it may have been dismissed from another browser using your profile. It may also have expired, been evicted, or become inaccessible after a session access change. Gateway restarts preserve retained entries and dismissals. The retained chat message is independent of the Inbox entry. Entries already lost before durable storage was introduced are not recovered from old messages.

## Related

- [Control UI PWA and Web Push](/en/web/control-ui/connect-and-pair#pwa-install-and-web-push)
- [iOS push delivery](/en/platforms/ios)
- [Node notification commands](/en/nodes)
