...
- If the session has a [goal](/en/tools/goal), the footer shows its compact state:
...
When the agent calls [`ask_user`](/en/tools/ask-user), the TUI opens a question
...
Gateway-connected [`secrets`](/en/tools/secrets) requests use a masked input that
...
[`agents.defaults.modelSelectionScope`](/en/gateway/config-agents/models#agentsdefaultsmodelselectionscope)
...
Other Gateway slash commands (for example, `/context`) are forwarded to the Gateway and shown as system output. See [Slash commands](/en/tools/slash-commands).
...
- [Control UI](/en/web/control-ui) — web-based control interface
- [Config](/en/cli/config) — inspect, validate, and edit `openclaw.json`
- [Doctor](/en/cli/doctor) — guided repair and migration checks
- [CLI Reference](/en/cli) — full CLI command reference
- [`openclaw resume`](/en/cli/resume) — attach the TUI to a recent Gateway session
- [`openclaw tui`](/en/cli/tui) — command reference and flags for the terminal UI
