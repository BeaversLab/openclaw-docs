...
Delivery is best-effort. The Inbox and replay bookkeeping are bounded and process-local; a restart clears them, and capacity limits can skip alerts. Notification failures do not retry or undo the posted chat message. Browser push does not provide exactly-once delivery. See [temporary Inbox retention](/en/concepts/multi-user#temporary-mentions-inbox) and [notification preferences](/en/web/notifications#receive-human-mention-alerts).
