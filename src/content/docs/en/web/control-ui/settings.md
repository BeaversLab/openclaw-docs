...
Themes can choose a neutral prompt mark instead of the lobster mascot and supply their own long-wait status vocabulary. They can also add occasional penguin or fedora visitors to the composer ledge and occasional hats on agent avatars from the `fedora`, `crown`, `santa`, `party`, and `pumpkin` catalog. A theme without the mascot hides the resident lobster and visiting lobster strangers while ordinary ledge traffic continues under the unchanged **Lobster visits** toggle. See the [theme definition fields](/en/tools/theme#create-and-apply-a-personal-theme) for the portable settings and limits.

Plugin themes can also bring their own SVG hats and composer visitors through [declared artwork](/en/plugins/manifest/surfaces#themes).
...
Appearance also carries the **Lobster visits** and **Lobster sounds** toggles and the Lobsterdex. Both toggles are browser-local. See [The Lobster](/en/web/lobster) for what the composer visitors do and how to turn them off for good.
...
[skill proposals](/en/tools/skill-workshop). **Learn from past conversations** opens
...
[Apply changes and inspect](/en/plugins/manage-plugins#apply-changes-and-inspect)
...
The Sessions view batches bursts of session-change events into a refresh, with at least one second between automatic refresh attempts during continuous activity. Event-driven refreshes pause while the browser tab is hidden and catch up once when you return. Changing filters or retrying a failed request still loads immediately. The sidebar's session capability also [reuses row snapshots and paces list reads](/en/web/control-ui/sessions-and-sidebar#sidebar-navigation). Activity links retain their search and people filters during initial loading and navigation.
...
use the [Transcripts CLI](/en/cli/transcripts). Archive access requires `operator.read`
...
cannot safely retry. See [capture configuration](/en/cli/transcripts#configuration).
...
populate this store. See the [Transcripts CLI](/en/cli/transcripts) for capture setup,
