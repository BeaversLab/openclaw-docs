...
See `gateway.auth` in [Gateway configuration](/en/gateway/configuration).
...
- **Not localhost**: use Tailscale Serve, a non-loopback shared-secret bind, a non-loopback identity-aware reverse proxy with `gateway.auth.mode: "trusted-proxy"`, or an SSH tunnel. HTTP APIs still use shared-secret auth unless you intentionally run private-ingress `gateway.auth.mode: "none"` or trusted-proxy HTTP auth. See [Web surfaces](/en/web).
...
- For token drift repair steps, see [Token drift recovery checklist](/en/cli/devices#token-drift-recovery-checklist).
...
- [`openclaw dashboard`](/en/cli/dashboard) — securely open the Control UI from the CLI
