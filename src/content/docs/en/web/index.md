...
When `hooks.enabled=true`, the Gateway also exposes a webhook endpoint on the same HTTP server. See `hooks` in [Gateway configuration reference](/en/gateway/config-hooks#hooks) for auth and payloads.
