...
widget when you need custom interactivity. See [Native dashboard reports](/en/tools/show-widget#native-dashboard-reports).
...
> Put [https://status.example.com](https://status.example.com) on this dashboard, expand it to fill the task,
...
[Browser side panel](/en/web/control-ui/panels#browser-panel). That opens the
...
Widgets that want **reach** must declare it. An explicit [session permission mode](/en/gateway/permission-modes)
...
- The security model, storage details, and design rationale live in
  [Dashboard Architecture](/en/web/dashboard-architecture), including the
