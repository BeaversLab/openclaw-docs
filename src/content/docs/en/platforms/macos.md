...
If you only need the CLI and Gateway, start with [Getting started](/en/start/getting-started).
...
app from source with [macOS dev setup](/en/platforms/mac/dev-setup).
...
| Remote | Another host runs the Gateway and this Mac should control it over SSH, LAN, or Tailnet. | [Remote control](/en/platforms/mac/remote)            |

Local mode requires an installed `openclaw` CLI. The app can install it, or you
can follow [Gateway on macOS](/en/platforms/mac/bundled-gateway).
