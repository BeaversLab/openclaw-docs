...
2. Open the [Control UI](/en/web/control-ui), select **Nodes**, and click
...
3. In the iOS app, open **Settings** -> **Gateway**, scan the QR code (or paste
   ...
   [`openclaw qr`](/en/cli/qr) — not a gateway join URL from
   [`openclaw devices join-code`](/en/cli/devices#openclaw-devices-join-code).
...
independent gates. See [HealthKit summaries](/en/platforms/ios-healthkit) for
...
These are [app-local SQLite journals](/en/reference/database-schemas#apple-companion-delivery-journals).
...
pairing requirements as [direct Watch node setup](/en/platforms/ios#optional-direct-apple-watch-node):
...
1. Configure realtime [Talk mode](/en/nodes/talk) on the Gateway with `webrtc`
...
- The setup code advertises a `wss://` Gateway endpoint with a certificate trusted
  ...
  pairing](/en/gateway/pairing) for endpoint configuration. Loopback, iPhone-only,
...
[standalone voice](/en/platforms/ios#standalone-voice); calls still require **Start**
...
If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `openclaw.internal.`) and Tailscale split DNS. See [Bonjour](/en/gateway/bonjour) for the CoreDNS example.
...
Agents can still operate the iOS app through OpenClaw by invoking node commands, but those calls go through the Gateway node protocol and follow iOS foreground/background limits. Use [Codex Computer Use](/en/plugins/codex-computer-use) for local desktop control and this page for iOS node capabilities.
...
- OpenAI realtime Talk uses client-owned WebRTC when `talk.realtime.transport` is `webrtc`; an explicit `gateway-relay` configuration remains Gateway-owned. See [Talk mode](/en/nodes/talk).
...
1. Open OpenClaw and [pair and connect to your Gateway](/en/platforms/ios#quick-start-pair-+-connect)
   first. Live voice uses your existing [Talk mode voice provider configuration](/en/nodes/talk);
...
- [Stable HTTPS URL](/en/gateway/stable-https-url) — give a loopback-only Gateway a stable, tailnet-only HTTPS URL with Tailscale Serve
