...
foreground/background limits. Use [Codex Computer Use](/en/plugins/codex-computer-use)
...
- `A2UI_HOST_NOT_CONFIGURED`: the Gateway did not advertise the Canvas plugin surface URL; check `plugins.entries.canvas.config.host` in [Gateway configuration](/en/gateway/configuration).
