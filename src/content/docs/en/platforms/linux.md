...
runtime installer. See [Chrome extension](/en/tools/chrome-extension) for approval,
...
regular browser for [Talk mode](/en/nodes/talk).
...
streaming only. Other device commands belong to the [CLI node host](/en/cli/node) and its
[Linux Node plugin](/en/platforms/linux#node-capabilities).

The [native macOS app](/en/platforms/macos) and [Windows Hub](/en/platforms/windows)
...
[Remote access](/en/gateway/remote) for secure Gateway configuration.
...
connection** on its Ollama card, then **Local only**. See [Ollama](/en/providers/ollama).
...
[OpenAI](/en/providers/openai) and [OAuth](/en/concepts/oauth).
...
signed installed-client proof; see [Linux companion publication](/en/reference/RELEASING#linux-companion-publication).
...
Full server guide: [Linux Server](/en/vps). Step-by-step VPS example:
...
For inline widgets in the Control UI, use [`show_widget`](/en/tools/show-widget).
The separate [macOS widget panel](/en/platforms/mac/canvas) requires a connected
...
- Optional: [Bun package workflow](/en/install/bun), [Nix](/en/install/nix), [Docker](/en/install/docker)
...
always-on hosts, lives in the [Gateway runbook](/en/gateway#supervision-and-service-lifecycle).
...
- [Linux server](/en/vps)
- [ChromeOS (Crostini)](/en/platforms/chromeos)
- [Raspberry Pi](/en/install/raspberry-pi)
