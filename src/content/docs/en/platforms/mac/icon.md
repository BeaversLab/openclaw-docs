...
A tool-activity badge (SF Symbol puck, e.g. `chevron.left.slash.chevron.right` for exec) can render on top of the same critter icon when a session has an active job or tool. That badge comes from `IconState`/`ActivityKind`; see [Menu bar](/en/platforms/mac/menu-bar) for the full state model.
...
- [Menu bar](/en/platforms/mac/menu-bar)
