...
`status` reports missing Screen Recording, Accessibility, or Event Synthesizing. The host never opens System
...
- The bridge checks **caller code signatures**. The production OpenClaw host accepts only the exact Peekaboo CLI
  ...
  sufficient.
- Prefer the signed bridge/app identity over a generic `node` runtime for Accessibility. Granting Accessibility to `node` lets any package launched by that Node executable inherit GUI automation access. See [macOS permissions](/en/platforms/mac/permissions#accessibility-grants-for-node-and-cli-runtimes).
...
`InMemorySnapshotManager` stores snapshots in memory. It applies a 10-minute validity window and a cap of 50 snapshots. Cleanup does not delete artifacts. If you need longer retention, re-capture from the client.
