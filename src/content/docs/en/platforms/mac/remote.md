...
Gateway. The Gateway's existing [automatic device approval policy](/en/gateway/trusted-proxy-auth#automatic-device-approval)
...
The Mac app's node combines native capabilities with system, browser, plugin, skill, and MCP commands from its bundled private worker. Connecting the app to a remote Gateway needs no external CLI or separate node-service installation on this Mac. An already-installed headless node service remains separate: the app preserves its start/stop and managed update/recovery behavior. Optional [cookie sync](/en/platforms/macos#sync-cookies-to-a-remote-computer) still uses an external CLI and reports a feature-specific error when it is missing.
...
- See [Security](/en/gateway/security) and [Tailscale](/en/gateway/tailscale).
