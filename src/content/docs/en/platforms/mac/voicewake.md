...
- Overlay dismissal, including manual dismiss via the X button, always resumes the recognizer: `VoiceSessionCoordinator.overlayDidDismiss` calls `VoiceWakeRuntime.refresh(state:)` on every dismiss path. See [Voice overlay](/en/platforms/mac/voice-overlay) for the session/token model.
...
- [Voice overlay](/en/platforms/mac/voice-overlay)
