...
[WebChat](/en/platforms/mac/webchat) for switching behavior and native chat features.
...
- **About OpenClaw** selects **About** in the native Connection window, with the app version, build information, resource links, and **Copy Build Info**. It works without a Gateway connection.
...
Manual Cron refreshes and successful job changes supersede older reads. An in-flight response therefore cannot restore a deleted job to the list.
...
- The root menu shows one "Context" row with a session count/status. That row opens a submenu (`MenuSessionsInjector`).
...
- Enable the developer toggle in **Dashboard → Settings → This Mac → Developer**. Then open **Connection… → Debug → Icon override**:
  ...
  - `Idle`
- Stored under `UserDefaults` key `openclaw.iconOverride`, and mapped to `IconState.overridden`.

## Testing checklist

- Trigger main session job: icon switches immediately and status row shows the main label.
- Trigger non-main session job while main is idle: icon/status shows the non-main session and stays stable until it finishes.
...
- [Menu bar icon](/en/platforms/mac/icon)
