...
[Chrome extension](/en/tools/chrome-extension).
...
argument. See [local hosting alongside a remote primary](/en/platforms/mac/remote#run-a-local-gateway-alongside-a-remote-primary).
...
- If the host loops with repeated `EADDRINUSE` or fast restarts, check for
  ...
  [Gateway troubleshooting](/en/gateway/troubleshooting#macos-launchd-supervisor-loop-with-duplicate-gateway%2Fnode-launchagents).
...
[environment variables](/en/help/environment#path-related-env-vars) and
...
[remote control](/en/platforms/mac/remote#macos-app-setup) for `primary set`,
