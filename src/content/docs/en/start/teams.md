...
A conversation that starts in the team channel can continue as a session the whole team can open, steer, and take over. [Multi-user mode](/en/concepts/multi-user) gives every session three layers of attribution - an immutable creator, an assignable owner (assign sessions like GitHub issues from the session context menu), and the history of people who actually prompted - plus live [presence](/en/concepts/presence): who is viewing, and who is typing, with drafts that never reach the model or the transcript.
