...
not imply support for its bundled npm as a source resolver. [Published/global installs](/en/install) do not
...
(Bare `openclaw setup`, without `--baseline`, opens an interactive OpenClaw chat on a configured system and falls through to guided onboarding on a fresh one. See [Setup CLI](/en/cli/setup) for the full routing order.)
...
- **Wrong developer CLI:** When `PATH` includes `node_modules/.bin`, `codex` can
  ...
  OpenClaw's [managed Codex app-server](/en/plugins/codex-harness-reference#app-server-transport)
...
- **Model auth profiles**: shared and agent-local SQLite auth stores; see [Auth credential semantics](/en/auth-credential-semantics#agent-copy-portability) for inheritance and legacy shared-store relocation
- **File-backed secrets payload (optional)**: `~/.openclaw/secrets.json`
- **Legacy OAuth import**: `~/.openclaw/credentials/oauth.json`
  More detail: [Security](/en/gateway/security/secrets-and-storage#credential-storage-map).
