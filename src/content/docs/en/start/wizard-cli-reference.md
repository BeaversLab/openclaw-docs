...
For a walkthrough, see [Onboarding (CLI)](/en/start/wizard). For the full CLI flag
reference (every `--flag`, non-interactive examples, provider-specific
commands), see [`openclaw onboard`](/en/cli/onboard).

## What the wizard does

Fresh local guided onboarding shows a one-line pointer to the
[security guide](/en/gateway/security) and one choice: **Quick start** or
...
See [Guided default](/en/start/wizard#guided-default).
...
described [above](/en/start/wizard-cli-reference#what-the-wizard-does).
...
<Steps>
  ...
  <Step title="Channels">
    ...
    - [Mattermost](/en/channels/mattermost): bot token + base URL
    ...
    - Other bundled or separately installed channel plugins can add their own
      onboarding steps. See the complete [channel catalog](/en/channels).
...
<Steps>
  <Step title="Discovery (optional)">
    ...
    configured. Docs: [Gateway discovery](/en/gateway/discovery), [Bonjour](/en/gateway/bonjour).
  </Step>
...
defaults move with the product baseline, so check [Models](/en/concepts/models) if you are
...
<AccordionGroup>
  ...
  <Accordion title="Anthropic setup token">
    ...
    **Anthropic setup-token** during onboarding, or manage it later with
    [`openclaw models auth`](/en/cli/models#auth-profiles).
  </Accordion>
  ...
  <Accordion title="StepFun">
    ...
    More detail: [StepFun](/en/providers/stepfun).
  </Accordion>
  ...
  <Accordion title="Ollama (Cloud and local open models)">
    ...
    More detail: [Ollama](/en/providers/ollama).
  </Accordion>
...
Paths respect `$OPENCLAW_STATE_DIR`. See [Auth credential semantics](/en/auth-credential-semantics#agent-copy-portability) for shared-store and agent-local behavior.
...
- In non-interactive mode, `--secret-input-mode ref` creates only env-backed references for new credentials.
  ...
  - Existing plaintext profile credentials remain unchanged; reference mode does not migrate them. Run `openclaw secrets configure --apply`, then `openclaw secrets audit --check`. See [Secrets management](/en/gateway/secrets).
...
on the Gateway host. See [Models CLI](/en/cli/models#auth-profiles) and
[OAuth](/en/concepts/oauth).

For unattended setup, use a provider API key with
[non-interactive onboarding](/en/cli/onboard#non-interactive-setup). If you use
...
- `tools.profile` (local onboarding selects `"full"` when unset, including on a rerun; explicit profiles and other tool policies are preserved). Full tool selection is not Full Access execution permissions. See [Tool profiles](/en/gateway/config-tools/tool-policy#tool-profiles).
...
examples, see [`openclaw onboard`](/en/cli/onboard). Provider-specific scripts live
in [CLI automation](/en/start/wizard-cli-automation).
