...
- The WhatsApp plugin installed, or WhatsApp chosen during onboarding. WhatsApp is an official plugin that installs on demand. See [WhatsApp](/en/channels/whatsapp)
...
- persona/instructions in [`SOUL.md`](/en/concepts/soul)
...
By default, OpenClaw runs a heartbeat every 30 minutes — or every hour when Anthropic OAuth/token auth is configured (including Claude CLI reuse). See [Heartbeat](/en/gateway/heartbeat) for the full defaults. The prompt is:
...
Set `agents.defaults.heartbeat.every: "0m"` to disable recurring cadence. Targeted event-driven wakes, such as background exec completion follow-ups, remain available and do not create a recurring schedule. Heartbeat checklists live in the monitor's cron scratch (see [Heartbeat](/en/gateway/heartbeat)); `openclaw doctor --fix` migrates a legacy workspace `HEARTBEAT.md` into it.
...
[Rich output protocol](/en/reference/rich-output-protocol#legacy-media-lines).
