...
The Gateway owns channel connections, config, credentials, and the [control-plane API](/en/gateway/protocol). It binds to loopback by default and refuses non-loopback binds without a working auth path ([architecture](/en/concepts/architecture), [network model](/en/network)).
...
[`tools.exec.host`](/en/tools/exec) resolves to the gateway host, a [sandbox](/en/gateway/sandboxing), or a paired [node](/en/nodes). While a sandbox runtime is active, per-call escapes to the host are rejected. An explicit `host=sandbox` with no runtime configured fails instead of silently running on the host. The backends are Docker and Podman, SSH, and [OpenShell](/en/gateway/openshell). The default Docker and Podman profile has no network, a read-only root, all capabilities dropped, and a non-root user. OpenShell installs as a plugin and registers through the same backend contract as Docker. If you run OpenShell already, OpenClaw uses its sandboxes. It does not need to be wrapped in one.
...
**Sandboxing is off by default.** Out of the box, OpenClaw is a personal assistant for one trusted operator, and exec runs on the gateway host without prompts. The enterprise posture requires explicit configuration, verifiable with two commands: [`openclaw sandbox explain`](/en/gateway/sandbox-vs-tool-policy-vs-elevated) prints the effective execution posture, and [`openclaw security audit`](/en/gateway/security/audit-checks) flags drift with stable check IDs you can alarm on.
