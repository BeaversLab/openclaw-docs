...
guided flow. `openclaw setup` runs the same flow ([Setup](/en/cli/setup) covers
the `--baseline` config-only variant). Windows desktop users can also start
from [Windows Hub](/en/platforms/windows).
...
Docs: [Dashboard](/en/web/dashboard).
</Info>
...
<Note>
`--json` does not imply non-interactive mode. For scripts, use `--non-interactive` (see [CLI automation](/en/start/wizard-cli-automation)).
</Note>
...
conversationally. Docs: [Web tools](/en/tools/web).
</Tip>

## Guided default

Fresh local interactive onboarding offers **Quick start** and **Custom setup**
after a one-line pointer to the [security guide](/en/gateway/security). Quick start
...
See [Team preset](/en/concepts/multi-agent#team-preset) for the delegation config and
[`agents team create`](/en/cli/agents#agents-team-create) to add a namespaced team
...
2. **Model/Auth** - pick a provider auth flow (API key, OAuth, or
   ...
   [Secrets management](/en/gateway/secrets). Interactive secret reference mode can
...
4. **Channels** - built-in and official plugin chat channels, including
   ...
   chat allowlists. See [command owner setup](/en/channels/pairing#set-up-an-owner-without-dm-pairing).
...
classic wizard instead of fresh setup; see [Migrate](/en/cli/migrate) and the migration guides under
[Install](/en/install/migrating-hermes). `openclaw onboard --modern` is a
compatibility alias for [OpenClaw](/en/cli/openclaw). It uses the same
...
- Non-interactive flags: `--role`, `--model`, `--agent-dir`, `--bind`, `--non-interactive`. With `--role`, `--workspace` can be omitted. See [Role templates](/en/cli/agents#role-templates).
...
For the full flag reference, see [`openclaw onboard`](/en/cli/onboard).
