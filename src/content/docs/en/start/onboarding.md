...
agent.
For CLI onboarding and a comparison of both paths, see [Onboarding Overview](/en/start/onboarding-overview).

<Tip>
Need the app first? [Download OpenClaw for macOS](/en/platforms/macos#download),
...
- Shared/multi-user setups need lock-down: split trust boundaries, keep tool access minimal, and follow [Security](/en/gateway/security).
- Local onboarding selects `tools.profile: "full"` when no profile is configured and preserves explicit profiles and other tool policies. Full tool selection does not grant execution permissions. See [Tool profiles](/en/gateway/config-tools/tool-policy#tool-profiles).
...
See [Gateway on macOS](/en/platforms/mac/bundled-gateway#automatic-setup).
</Step>
...
**Choose connection** → **Local only**. See [Ollama](/en/providers/ollama).
...
Location. See [macOS permissions](/en/platforms/mac/permissions) for grant and
recovery guidance.

See [Bootstrapping](/en/start/bootstrapping) for what happens on the Gateway host
...
- [Onboarding overview](/en/start/onboarding-overview)
