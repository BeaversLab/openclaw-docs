...
This is different from local [`exec`](/en/tools/exec):
...
Do **not** use it when you need local files, your shell, your repo, or paired devices. Use [`exec`](/en/tools/exec) for that.
...
- For fresh X data, use [`x_search`](/en/tools/web#x_search) first and pipe the result into `code_execution`.
...
<CardGroup cols={2}>
  ...
  <Card title="Exec approvals" href="/en/tools/exec-approvals" icon="shield">
    Allow/deny policy for shell execution.
  </Card>
  <Card title="Web tools" href="/en/tools/web" icon="globe">
    `web_search`, `x_search`, and `web_fetch`.
  </Card>
  <Card title="xAI provider" href="/en/providers/xai" icon="microchip">
