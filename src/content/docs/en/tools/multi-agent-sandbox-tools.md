...
<Tabs>
  ...
  <Tab title="Communication-only">
    ...
    Session tools default to Gateway-wide visibility (`all`) with agent-to-agent messaging on; this profile narrows the agent to its own session tree. See [`tools.sessions`](/en/gateway/config-tools#tools-sessions) and [`tools.agentToAgent`](/en/gateway/config-tools#tools-agenttoagent).
