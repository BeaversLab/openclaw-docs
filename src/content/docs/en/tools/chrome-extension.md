...
the [`user` profile](/en/tools/browser#profiles-openclaw-user-chrome) attaches over
...
oldest sessions under pressure; see [session maintenance](/en/reference/session-management-compaction#store-maintenance-and-disk-controls).
...
See also: [Browser](/en/tools/browser) for the full profile model and the
