...
surface instead of Tool Search controls, see [Code Mode](/en/tools/code-mode).
...
[Code Mode](/en/tools/code-mode) takes precedence, and Codex keeps its native surface.
...
in the embedded harness. The [Copilot harness](/en/plugins/copilot) instead maps
...
[Code Mode output contracts](/en/tools/code-mode/output#declared-output-contracts) and
[Tool plugins](/en/plugins/tool-plugins#output-contracts).
...
This is different from the [Code Mode](/en/tools/code-mode/configuration)
...
- [Multi-agent sandbox and tools](/en/tools/multi-agent-sandbox-tools)
