...
[Code Mode](/en/tools/code-mode) script. It is enabled by default, with an explicit
...
[fresh-session guidance](/en/tools/swarm#use-swarm-from-other-harnesses) below.
...
[Code Mode activation](/en/tools/code-mode/configuration#activation) and
...
[low-level tool flow](/en/tools/swarm#use-swarm-from-other-harnesses) needs both
...
When the [requirements](/en/tools/swarm#requirements) are met, Code Mode exposes
this guest API:
...
batches instead. See [Code Mode](/en/tools/code-mode/internals#nested-tool-execution)
...
tab, and the [Tasks page](/en/automation/tasks#control-ui). They have no session-sidebar
...
on the [Tasks page](/en/automation/tasks#control-ui) and retry cancellation for
...
directly. See [Sub-agent stopping](/en/tools/subagents#stopping) for exact-run and
...
For a [supervised, model-locked Chat](/en/plugins/codex-supervision#branch-from-a-local-session),
...
- [Code Mode](/en/tools/code-mode) for JavaScript executors and activation rules
- [Sub-agents](/en/tools/subagents) for child policy, isolation, and session behavior
- [Multi-agent sandbox tools](/en/tools/multi-agent-sandbox-tools) for per-agent restrictions
- [Tools overview](/en/tools) for tool profiles and policy routing
