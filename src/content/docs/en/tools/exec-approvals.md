...
evaluation. See the [strict inline-eval exceptions](/en/tools/exec#inline-eval-strictinlineeval).

For a mode-first overview of `deny`, `allowlist`, `ask`, `auto`, `full`,
Codex Guardian mapping, and ACPX harness permissions, see
[Permission modes](/en/tools/permission-modes).
...
[Native Bash and the exec allowlist](/en/gateway/cli-backends#native-bash-and-the-exec-allowlist)
...
- Gateway approval-backed commands bind every resolved command-segment executable before review and re-check it before launch. Node hosts capture these identities during local policy evaluation and re-check before dispatch. This does not cover inner shell executables across a remote human approval wait. Protected executables use resolved real-path identity only. Writable executables also use a content hash. A changed resolution during the bound window, including a new executable earlier on `PATH`, denies the run. Identity-only binding preserves otherwise eligible `allow-always` decisions. See [Interpreter/runtime commands](/en/tools/exec-approvals-advanced#interpreter%2Fruntime-commands).
...
<Note>
Per-session `/exec` overrides are not included. Run `/exec` in the relevant session to inspect its current defaults. See [session overrides](/en/tools/exec#session-overrides-%2Fexec).
</Note>

Full CLI reference (flags, JSON output, allowlist add/remove): [Approvals CLI](/en/cli/approvals).
...
<ParamField path="ask" type='"off" | "on-miss" | "always"'>
  ...
  [Exec tool](/en/tools/exec#parameters)) can only harden that baseline, and
...
remain denied.
See [Exec modes](/en/tools/exec#modes) for binding limits and escalation.
...
exception. See [Inline eval](/en/tools/exec#inline-eval-strictinlineeval).
...
[Codex tool approvals](/en/cli/mcp#codex-tool-approvals).
...
[Exec approvals - advanced](/en/tools/exec-approvals-advanced).
...
policy shape to edit them. See [Approvals CLI](/en/cli/approvals).

CLI: `openclaw approvals` supports gateway or node editing - see
[Approvals CLI](/en/cli/approvals).
...
- `/exec security=full <task>` is a current-turn request by an authorized operator, subject to effective session and host policy. To hard-block exec, deny the `exec` tool via tool policy. See [session overrides](/en/tools/exec#session-overrides-%2Fexec) for the full-access session exception to host approval floors.

## Related

<CardGroup cols={2}>
  <Card title="Exec approvals - advanced" href="/en/tools/exec-approvals-advanced" icon="gear">
    Safe bins, interpreter binding, and approval forwarding to chat.
  </Card>
  <Card title="Exec tool" href="/en/tools/exec" icon="terminal">
    Shell command execution tool.
  </Card>
  <Card title="Elevated mode" href="/en/tools/elevated" icon="shield-exclamation">
    Break-glass path that also skips approvals.
  </Card>
  <Card title="Sandboxing" href="/en/gateway/sandboxing" icon="box">
    Sandbox modes and workspace access.
  </Card>
  <Card title="Security" href="/en/gateway/security" icon="lock">
    Security model and hardening.
  </Card>
  <Card title="Sandbox vs tool policy vs elevated" href="/en/gateway/sandbox-vs-tool-policy-vs-elevated" icon="sliders">
    When to reach for each control.
  </Card>
  <Card title="Skills" href="/en/tools/skills" icon="sparkles">
