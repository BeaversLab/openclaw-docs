...
Choose **This session** for session-only enablement or **Everywhere** for global enablement. Either scope saves a global server definition; session policy is the per-session layer. See [Composer capability menu](/en/web/control-ui#composer-capability-menu) for the complete scope and tool-access behavior.
...
Override a server with `openclaw mcp configure <server> --approval approve|prompt|auto`; an explicit mode takes precedence over the posture-derived default. Stored grants apply only under `auto` or an unspecified server mode; explicit `prompt` keeps asking. Inspect or revoke grants through [MCP tool grants](/en/tools/exec-approvals#mcp-tool-grants). See [Codex tool approvals](/en/cli/mcp#codex-tool-approvals) for details and [Native approvals in Slack](/en/channels/slack#native-approvals-in-slack) for Slack button delivery.
...
- [Control UI](/en/web/control-ui#composer-capability-menu)
