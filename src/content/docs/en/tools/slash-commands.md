...
<AccordionGroup>
  <Accordion title="Directive behavior details">
    ...
    - In **directive-only** messages (the message is only directives), they
      ...
      the task. Use [session permission modes](/en/gateway/permission-modes) for
...
<AccordionGroup>
  <Accordion title="Sessions and runs">
    ...
    | `/compact [instructions]` | Compact the session context. See [Compaction](/en/concepts/compaction) |
  ...
  <Accordion title="Model and run controls">
    ...
    | `/exec host=<auto\|sandbox\|gateway\|node> security=<deny\|allowlist\|full> ask=<off\|on-miss\|always> node=<id>` | Show resolved exec defaults; persist host/node placement, apply security/ask to this message only. See [Session permission modes](/en/gateway/permission-modes) |
    ...
    | `/queue <mode>` | Manage active-run queue behavior. See [Queue](/en/concepts/queue) and [Queue steering](/en/concepts/queue-steering) |
    | `/steer <message>` | Inject guidance into the active run. Alias: `/tell`. See [Steer](/en/tools/steer) |
    ...
    <AccordionGroup>
      ...
      <Accordion title="Model switching details">
        - Prefer choosing the model when creating a session. Changing it in an established session is an advanced operation because model context limits, prompt/tool behavior, and prompt-cache behavior can differ. See [Choose a model for a session](/en/concepts/models#choose-a-model-for-a-session).
  ...
  <Accordion title="Discovery and status">
    ...
    | `/goal [status\|start\|edit\|pause\|resume\|complete\|block\|clear] ...` | Manage the current session's durable [goal](/en/tools/goal) |
...
user skill directly.
  ...
  <Accordion title="Skills, allowlists, approvals">
    ...
    | `/learn [request]` | Draft one reviewable skill from the current conversation or named sources through [Skill Workshop](/en/tools/skill-workshop) |
    ...
    | `/btw <question>` | Ask a side question without changing session context. Alias: `/side`. See [BTW](/en/tools/btw) |
  </Accordion>
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    [Updating from chat](/en/install/updating#from-chat).
  ...
  <Accordion title="Voice, TTS, channel control">
    | Command | Description |
    | --- | --- |
    | `/tts on\|off\|status\|chat\|latest\|provider\|limit\|summary\|audio\|help` | Control TTS. See [TTS](/en/tools/tts) |
...
| Command                                                                             | Description                                                                                                                                                                                    |
| ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/dreaming [on\|off\|status\|help]`                                                 | Toggle memory dreaming (owner or Gateway admin). See [Dreaming](/en/concepts/dreaming)                                                                                                            |
| `/pair [qr\|status\|pending\|approve\|cleanup\|notify]`                             | Manage device pairing. See [Pairing](/en/channels/pairing)                                                                                                                                        |
| [`/voice`](/en/nodes/talk#choose-a-talk-voice-from-chat) `status\|list\|set <voiceId>` | Manage Talk voice config. Discord native name: `/talkvoice`                                                                                                                                    |
| `/codex <action> ...`                                                               | Bind, steer, and inspect the Codex app-server harness (status, threads, resume, model, fast, permissions, compact, review, mcp, skills, and more). See [Codex harness](/en/plugins/codex-harness) |

LINE-only: `/card ...` (rich card presets, see [LINE](/en/channels/line))
...
Without a flag or that optional setting, `/model <model>` changes only the current session, including for owners/admins. Set `agents.defaults.modelSelectionScope` to `"agent"` or `"global"` only when you want unqualified selections to update that default. The setting does not grant permission to write configured defaults. See [Model selection scope](/en/gateway/config-agents/models#agentsdefaultsmodelselectionscope).
...
See [BTW side questions](/en/tools/btw) for the full behavior.
...
<AccordionGroup>
  <Accordion title="Session scoping per surface">
    ...
    - **`/login openrouter`** sends a browser sign-in action through the Gateway's managed HTTPS address. Approve access in your browser, then return to chat for the saved result. See [OpenRouter](/en/providers/openrouter#getting-started) for address requirements. Use `/login cancel` to cancel a pending sign-in.
...
<CardGroup cols={2}>
  <Card title="Skills" href="/en/tools/skills" icon="puzzle-piece">
    How skill slash commands are registered and gated.
  </Card>
  <Card title="Creating skills" href="/en/tools/creating-skills" icon="hammer">
    Build a skill that registers its own slash command.
  </Card>
  <Card title="BTW" href="/en/tools/btw" icon="comments">
    Side questions without changing session context.
  </Card>
  <Card title="Steer" href="/en/tools/steer" icon="compass">
    Guide the agent mid-run with `/steer`.
  </Card>
  <Card title="Configuration reference" href="/en/gateway/configuration-reference" icon="sliders">
    Settings that change how slash commands are resolved and gated.
  </Card>
  <Card title="OpenProse migration" href="/en/prose" icon="pen-nib">
