...
<AccordionGroup>
  <Accordion title="Sessions and runs">
    ...
    - `/compact [instructions]` compacts the session context. See [Compaction](/en/concepts/compaction).
  ...
  <Accordion title="Model and run controls">
    ...
    - `/queue <mode>` manages queue behavior (`steer`, legacy `queue`, `followup`, `collect`, `steer-backlog`, `interrupt`) plus options like `debounce:0.5s cap:25 drop:summarize`; `/queue default` or `/queue reset` clears the session override. See [Command queue](/en/concepts/queue) and [Steering queue](/en/concepts/queue-steering).
    - `/steer <message>` injects guidance into the active run for the current session, independent of `/queue` mode. It does not start a new run when the session is idle. Alias: `/tell`. See [Steer](/en/tools/steer).
  ...
  <Accordion title="Skills, allowlists, approvals">
    ...
    - `/btw <question>` asks a side question without changing future session context. Alias: `/side`. See [BTW](/en/tools/btw).
  ...
  <Accordion title="Subagents and ACP">
    ...
    - `/subagents steer <id|#> <message>` sends steering to a running sub-agent. See [Steer](/en/tools/steer).
  ...
  <Accordion title="Voice, TTS, channel control">
    - `/tts on|off|status|chat|latest|provider|limit|summary|audio|help` controls TTS. See [TTS](/en/tools/tts).
...
channel. See [Channel docking](/en/concepts/channel-docking) for setup,
...
- `/dreaming [on|off|status|help]` toggles memory dreaming. See [Dreaming](/en/concepts/dreaming).
- `/pair [qr|status|pending|approve|cleanup|notify]` manages device pairing/setup flow. See [Pairing](/en/channels/pairing).
...
- `/card ...` sends LINE rich card presets. See [LINE](/en/channels/line).
- `/codex status|models|threads|resume|compact|review|diagnostics|account|mcp|skills` inspects and controls the bundled Codex app-server harness. See [Codex harness](/en/plugins/codex-harness).
- QQBot-only commands:
  - `/bot-ping`
  - `/bot-version`
  - `/bot-help`
...
<AccordionGroup>
  ...
  <Accordion title="Channel-specific behavior">
    ...
    - ACP command reference and runtime behavior: [ACP agents](/en/tools/acp-agents).
  ...
  <Accordion title="Skill commands and native arguments">
    - **Skill commands:** `user-invocable` skills are exposed as slash commands. Names are sanitized to `a-z0-9_` (max 32 chars); collisions get numeric suffixes (e.g. `_2`).
      ...
      - Example: `/prose` (OpenProse plugin) — see [OpenProse](/en/prose).
...
See [BTW Side Questions](/en/tools/btw) for the full behavior and client UX details.
