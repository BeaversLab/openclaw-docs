...
- `/btw <question>` (ask an ephemeral side question about the current session without changing future session context; see [/tools/btw](/en/tools/btw))
...
- `/dreaming [on|off|status|help]` (toggle global dreaming or show status; see [Dreaming](/en/concepts/dreaming))

Text-only:

- `/compact [instructions]` (see [/concepts/compaction](/en/concepts/compaction))
