...
`show_widget` shows a self-contained HTML widget on the user's current surface. The Canvas plugin renders it inline in the Control UI, iOS, Android, and macOS chat transcripts; Linux uses the browser Control UI. In a Discord session with [Activities](/en/channels/discord-activities) enabled, the Discord plugin posts an **Open widget** button that launches it as an Activity.
...
- [Canvas plugin](/en/plugins/reference/canvas)
