...
`show_widget` is a core tool that shows a self-contained HTML widget on the user's current surface. OpenClaw renders it inline in the Control UI and in iOS, Android, macOS, and Linux Quick Chat transcripts; the Linux dashboard uses the browser Control UI. In a Discord session with [Activities](/en/channels/discord-activities) enabled, the Discord plugin posts an **Open widget** button that launches it as an Activity.
...
The iframe also follows [`gateway.controlUi.embedSandbox`](/en/web/control-ui/chat#hosted-embeds). The default `scripts` tier supports interactive widgets while preserving origin isolation.

The accepted WebRTC data-channel egress residual is documented in [Dashboard Architecture](/en/web/dashboard-architecture#modeled-residual-webrtc-data-channels).
...
- [Control UI hosted embeds](/en/web/control-ui/chat#hosted-embeds)
- [Discord Activities](/en/channels/discord-activities)
- [macOS widget panel](/en/platforms/mac/canvas)
- [Gateway protocol client capabilities](/en/gateway/protocol/handshake#client-capabilities)
