...
Sub-agents are background agent runs spawned from an existing agent run. They run in their own session (`agent:<agentId>:subagent:<uuid>`) and, when finished, **announce** their result back to the requester chat channel. Each sub-agent run is tracked as a [background task](/en/automation/tasks).
...
- For ACP harness sessions (Codex, Claude Code, Gemini CLI), use `sessions_spawn` with `runtime: "acp"` and see [ACP Agents](/en/tools/acp-agents), especially the [ACP delivery model](/en/tools/acp-agents#delivery-model) when debugging completions or agent-to-agent loops.
