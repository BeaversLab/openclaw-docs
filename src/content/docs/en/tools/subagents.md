...
- [Swarm collectors](/en/tools/swarm) return results through explicit collection,
...
When [execution identity auditing](/en/gateway/audit#run-identity-inspection) is
...
<AccordionGroup>
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - For ACP harness sessions (Claude Code, Gemini CLI, OpenCode, or explicit Codex ACP/acpx), use `sessions_spawn` with `runtime: "acp"` when the tool advertises that runtime. See [ACP delivery model](/en/tools/acp-agents#delivery-model) when debugging completions or agent-to-agent loops. When the `codex` plugin is enabled, Codex chat/thread control should prefer `/codex ...` over ACP unless the user explicitly asks for ACP/acpx.
...
[Subagent yield handoff](/en/concepts/subagent-yield-handoff) for lifecycle ownership
...
See [Configuration reference](/en/gateway/configuration-reference) and
[Slash commands](/en/tools/slash-commands) for current adapter details.
...
See [Stopping](/en/tools/subagents#stopping) for scope and incomplete-cancellation behavior.
...
run and their descendants, including ordinary sub-agents and [Swarm](/en/tools/swarm)
...
[background tasks](/en/automation/tasks#control-ui) and retry their cancellation;
