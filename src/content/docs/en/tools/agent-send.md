...
output projection, and process status, use [`openclaw agent exec`](/en/cli/agent#agent-exec).
...
- With `--json --deliver`, the JSON includes delivery status for sent,
  ...
  [JSON delivery status](/en/cli/agent#json-delivery-status).
...
<CardGroup cols={2}>
  ...
  <Card title="Reactions" href="/en/tools/reactions" icon="thumbs-up">
  ...
  <Card title="ACP agents" href="/en/tools/acp-agents" icon="robot">
