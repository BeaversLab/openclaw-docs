...
<Steps>
  ...
  <Step title="Skills teach the agent when and how">
    ...
    [Skills reference](/en/tools/skills) | [Creating skills](/en/tools/creating-skills)
  ...
  <Step title="Plugins package everything together">
    ...
    [Install and configure plugins](/en/tools/plugin) | [Build your own](/en/plugins/building-plugins)
...
| ------------------------------------------ | --------------------------------------------------------------------- | ------------------------------------------------------------ |
| `exec` / `process`                         | Run shell commands, manage background processes                       | [Exec](/en/tools/exec), [Exec Approvals](/en/tools/exec-approvals) |
| `code_execution`                           | Run sandboxed remote Python analysis                                  | [Code Execution](/en/tools/code-execution)                      |
| `browser`                                  | Control a Chromium browser (navigate, click, screenshot)              | [Browser](/en/tools/browser)                                    |
| `web_search` / `x_search` / `web_fetch`    | Search the web, search X posts, fetch page content                    | [Web](/en/tools/web), [Web Fetch](/en/tools/web-fetch)             |
...
| `image` / `image_generate`                 | Analyze or generate images                                            | [Image Generation](/en/tools/image-generation)                  |
| `music_generate`                           | Generate music tracks                                                 | [Music Generation](/en/tools/music-generation)                  |
| `video_generate`                           | Generate videos                                                       | [Video Generation](/en/tools/video-generation)                  |
| `tts`                                      | One-shot text-to-speech conversion                                    | [TTS](/en/tools/tts)                                            |
| `sessions_*` / `subagents` / `agents_list` | Session management, status, and sub-agent orchestration               | [Sub-agents](/en/tools/subagents)                               |
| `session_status`                           | Lightweight `/status`-style readback and session model override       | [Session Tools](/en/concepts/session-tool)                      |
...
For broader config docs, read [Configuration](/en/gateway/configuration) and
...
- [Canvas](/en/plugins/reference/canvas) — experimental bundled plugin for node Canvas control and A2UI rendering
- [Diffs](/en/tools/diffs) — diff viewer and renderer
- [LLM Task](/en/tools/llm-task) — JSON-only LLM step for structured output
- [Lobster](/en/tools/lobster) — typed workflow runtime with resumable approvals
- [Music Generation](/en/tools/music-generation) — shared `music_generate` tool with workflow-backed providers
- [OpenProse](/en/prose) — markdown-first workflow orchestration
- [Tokenjuice](/en/tools/tokenjuice) — compact noisy `exec` and `bash` tool results
...
[Tool Search](/en/tools/tool-search) is the compact surface
