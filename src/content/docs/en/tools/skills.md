...
<CardGroup cols={2}>
  ...
  <Card title="Skills config" href="/en/tools/skills-config" icon="gear">
    Full `skills.*` config schema and agent allowlists.
  </Card>
  <Card title="ClawHub" href="/en/clawhub" icon="cloud">
...
The release-versioned [Custodian skill library](/en/tools/custodian-skills) shares
...
files. See [Nodes](/en/nodes/mcp-and-skills#node-hosted-skills) for pairing and off-switches.
...
operations require an authenticated [Gateway profile](/en/concepts/user-model).
...
<Warning>
A shared Gateway remains [one trust domain](/en/start/teams#one-trust-boundary).
...
[Skills library CLI](/en/cli/skills#personal-skill-library), or the agent's
...
See [Plugins](/en/tools/plugin) and [Tools](/en/tools) for the full plugin system.
...
[Skill Workshop](/en/tools/skill-workshop) is a proposal queue between the agent
...
See [Skill Workshop](/en/tools/skill-workshop) for the full lifecycle, CLI
...
<Warning>
  ...
  [Sandboxing](/en/gateway/sandboxing) for agent-side controls.
</Warning>
...
[Snapshots and refresh](/en/tools/skills#snapshots-and-refresh).
...
<Warning>
  ...
  [Skills config](/en/tools/skills-config#sandboxed-skills-and-env-vars) for how
...
<CardGroup cols={2}>
  ...
  <Card title="Plugins" href="/en/tools/plugin" icon="plug">
