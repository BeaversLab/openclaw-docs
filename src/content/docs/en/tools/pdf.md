...
[Local media files](/en/tools/media-overview#local-media-files).
...
See [Configuration Reference](/en/gateway/config-agents#agent-defaults) for full field details.

## Output details

The tool returns analysis in both `content[0].text` and `details.text`, so [Code Mode](/en/tools/code-mode) and [Tool Search](/en/tools/tool-search) can read the same result.
...
- [Configuration Reference](/en/gateway/config-agents#agent-defaults) - `pdfMaxMb` and `pdfMaxPages` config
