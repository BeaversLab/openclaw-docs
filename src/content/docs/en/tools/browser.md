...
restart. See [Config hot reload](/en/gateway/configuration#config-hot-reload).
...
- `chrome`: built-in [Chrome extension](/en/tools/chrome-extension) profile for
...
[Browser control API](/en/tools/browser-control) for per-call snapshot options.
...
- Local `openclaw` profiles use a CDP port range starting 9 ports above the control port (default `18800`-`18899`). OpenClaw allocates from that range for
  ...
  `mcpArgs` override it; see [Custom Chrome MCP launch](/en/tools/browser#custom-chrome-mcp-launch).
...
- `driver: "extension"` drives your signed-in Chrome through the [OpenClaw Chrome extension](/en/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
[Browser control API](/en/tools/browser-control) for the full reference.
