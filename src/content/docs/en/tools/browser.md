...
is unavailable, jump to [Missing browser command or tool](/en/tools/browser#missing-browser-command-or-tool).
...
`OPENCLAW_HOME_VOLUME` or a bind mount. See [Docker](/en/install/docker).
...
- `snapshot`:
  ...
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/en/gateway/configuration-reference#browser)).
...
- For logins and anti-bot notes (X/Twitter, etc.), see [Browser login + X/Twitter posting](/en/tools/browser-login).
