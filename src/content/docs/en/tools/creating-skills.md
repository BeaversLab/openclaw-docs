...
For how skills are loaded and prioritized, see [Skills](/en/tools/skills).
...
- **Conditional activation** — use `requires.bins`, `requires.env`, or
  ...
  available. See [Skills reference: gating](/en/tools/skills#gating).
- **Environment and API-key wiring** — use `skills.entries.<name>.env` and
  ...
  turn. See [Skills reference: config wiring](/en/tools/skills#config-wiring).
- **Invocation control** — set `user-invocable: false` to hide a slash command,
  ...
  model prompt. See [Skills reference: frontmatter](/en/tools/skills#frontmatter).
...
- [Skills reference](/en/tools/skills) — loading, precedence, and gating rules
- [Skills config](/en/tools/skills-config) — `skills.*` config schema
- [ClawHub](/en/clawhub) — public skill registry
- [Building Plugins](/en/plugins/building-plugins) — plugins can ship skills
