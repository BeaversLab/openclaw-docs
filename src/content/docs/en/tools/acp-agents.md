...
to existing OpenClaw channel conversations, use [`openclaw mcp serve`](/en/cli/mcp)
instead of ACP.
...
- CLI backends are separate text-only local fallback runtimes. See [CLI Backends](/en/gateway/cli-backends).
