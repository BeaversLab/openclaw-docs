...
Each ACP session spawn is tracked as a [background task](/en/automation/tasks).
...
| Expose an OpenClaw Gateway session _as_ an ACP server for an editor or client                   | [`openclaw acp`](/en/cli/acp)            | Bridge mode. IDE/client talks ACP to OpenClaw over stdio/WebSocket                                                                                                                            |
...
[ACP agents - setup](/en/tools/acp-agents-setup) only when the harness
...
| `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`    | `permissionMode` blocks writes/exec in non-interactive ACP session.                                                    | Set `plugins.entries.acpx.config.permissionMode` to `approve-all` and restart gateway. See [Permission configuration](/en/tools/acp-agents-setup#permission-configuration). |
...
repeating `/new`. See [Codex harness troubleshooting](/en/plugins/codex-harness#troubleshooting).
</Note>
