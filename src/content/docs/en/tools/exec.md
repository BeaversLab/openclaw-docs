...
| `tools.exec.safeBins`                | unset                    | Stdin-only safe binaries that can run without explicit allowlist entries. See [Safe bins](/en/tools/exec-approvals-advanced#safe-bins-stdin-only).                    |
...
No-approval host exec is the default for gateway and node (`mode=full`) — this comes from the host-policy defaults, not from `host=auto`. If you want approvals/allowlist behavior, set `tools.exec.mode` and tighten the host approvals file. See [Exec approvals](/en/tools/exec-approvals#yolo-mode-no-approval). To force gateway or node routing regardless of sandbox state, set `tools.exec.host` or use `/exec host=...`.
...
Use `/exec ask=always` with a message to require human approval for that run. It does not persist to later messages. Use [session permission modes](/en/gateway/permission-modes) for session-wide policy.
...
Reviewer-approved unpinned execution requires the complete dispatch chain to be identity-bound: the authorization plan must be complete, use direct transports, and have a recorded executable operand for every wrapper and final executable. Eligible globs and chains run as written after executable-identity, mutable-file, and working-directory revalidation. Commands rebuilt with pinned executable paths retain their existing review behavior. Node-host auto-review accepts a prepared pinned direct command, including the node's own canonical POSIX shell transport around one direct absolute executable with static arguments. Bare executable names, unquoted globs, and user-supplied wrappers still require human approval when the gateway cannot inspect the node's in-memory binding. Node executable-identity revalidation covers local policy evaluation through dispatch. It does not preserve every inner shell executable's identity across a remote human approval wait. See [Interpreter/runtime commands](/en/tools/exec-approvals-advanced#interpreter%2Fruntime-commands) for that boundary.
...
These paths skip the approval owner that detects strict inline evaluation. Tightening a full session's ask mode restores that evaluation, even when the session still bypasses host approval-file floors. See [Session permission modes](/en/gateway/permission-modes) and [Elevated mode](/en/tools/elevated).
...
A standalone `/exec security=deny` acknowledges the run-only setting but starts no agent run and does not affect the next message. Use [session permission modes](/en/gateway/permission-modes) to keep a policy across messages.
...
For full policy details and examples, see [Exec approvals](/en/tools/exec-approvals-advanced#safe-bins-stdin-only) and [Safe bins versus allowlist](/en/tools/exec-approvals-advanced#safe-bins-versus-allowlist).
...
- [Exec Approvals](/en/tools/exec-approvals) — approval gates for shell commands
- [Sandboxing](/en/gateway/sandboxing) — running commands in sandboxed environments
- [Background Process](/en/gateway/background-process) — long-running exec and process tool
- [Security](/en/gateway/security) — tool policy and elevated access
- [Code Mode](/en/tools/code-mode) — an opt-in runtime where the model writes a program that calls the hidden tool catalog
- [`apply_patch`](/en/tools/apply-patch) — apply a structured edit instead of shelling out
- [Tokenjuice](/en/tools/tokenjuice) — compacting large command output
