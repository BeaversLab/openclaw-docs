...
[Configuration - tools and custom providers](/en/gateway/config-tools#mcp-and-plugin-tools-inside-sandbox-tool-policy).
...
[Swarm](/en/tools/swarm) adds `agents.run()`, `phase()`, and `log()` guest globals
...
- Do not set `restartSafe` on a new `exec`. Set it to `true` only when OpenClaw
  ...
  [restart recovery](/en/gateway/restart-recovery) can reconstruct an interrupted
...
See [Tool plugins](/en/plugins/tool-plugins#output-contracts) for plugin authoring
details.
...
See [Tool Search](/en/tools/tool-search) for the OpenClaw compact catalog bridge
...
tool batch fails before any call in that batch is dispatched. [Swarm](/en/tools/swarm)
