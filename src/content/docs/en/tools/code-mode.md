...
JavaScript executes outside the Gateway's main event loop. Node is the default
executor and uses `node:vm` in a worker thread for trusted execution; it is not
a security boundary. The bundled QuickJS executor provides hardened guest
isolation. Both use the same tool bridge, where permissions, approvals, and
session ownership remain with the Gateway. See
[Code Mode executors](/en/tools/code-mode/executors) before enabling it.

This page is an index. Code Mode is documented on nine pages, one per reader
job. Open the page that matches your task.

| Page                                                      | Read it when                                                                       |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| [Code Mode quickstart](/en/tools/code-mode/quickstart)       | You want to turn Code Mode on, override one model, and recover from tool errors.   |
| [Code Mode executors](/en/tools/code-mode/executors)         | You want to choose Node or QuickJS and understand their security boundaries.       |
| [Code Mode configuration](/en/tools/code-mode/configuration) | You need the configuration fields, the preferred-model list, and activation order. |
...
write short programs. Use [Tool Search](/en/tools/tool-search) when you want a
...
| Page                                                          | Read it when                                                                  |
| ------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| [Code Mode tool surface](/en/tools/code-mode/tool-surface)       | You need the `exec` and `wait` contracts, the hidden catalog, and collisions. |
| [Code Mode guest API](/en/tools/code-mode/guest-api)             | You are writing guest code and need its globals, handles, and MCP namespaces. |
| [Code Mode output](/en/tools/code-mode/output)                   | You need declared output contracts or the guest output API.                   |
| [Code Mode internals](/en/tools/code-mode/internals)             | You need scope, terms, nested execution, snapshots, or the security boundary. |
| [Code Mode troubleshooting](/en/tools/code-mode/troubleshooting) | You need error codes, telemetry fields, or the debug environment variables.   |
| [Code Mode maintainer notes](/en/tools/code-mode/maintainers)    | You are changing Code Mode source, validating it, or writing E2E coverage.    |
...
- <a id="quickstart" />[Quickstart](/en/tools/code-mode/quickstart)
- <a id="enable-code-mode" />[Enable code mode](/en/tools/code-mode/quickstart#enable-code-mode)
- <a id="override-one-model" />[Override one model](/en/tools/code-mode/quickstart#override-one-model)
- <a id="what-the-model-does" />[What the model does](/en/tools/code-mode/quickstart#what-the-model-does)
- <a id="recover-from-tool-errors" />[Recover from tool errors](/en/tools/code-mode/quickstart#recover-from-tool-errors)
- <a id="verify-the-active-surface" />[Verify the active surface](/en/tools/code-mode/quickstart#verify-the-active-surface)
- <a id="use-swarm-for-agent-fan-out" />[Use Swarm for agent fan-out](/en/tools/code-mode/quickstart#use-swarm-for-agent-fan-out)
- <a id="configuration" />[Configuration](/en/tools/code-mode/configuration#configuration)
- <a id="automatic-per-model-activation" />[Automatic per-model activation](/en/tools/code-mode/configuration#automatic-per-model-activation)
- <a id="the-compat.codemode-catalog-flag" /><a id="the-compat-codemode-catalog-flag" />[The compat catalog flag](/en/tools/code-mode/configuration#the-compat-codemode-catalog-flag)
- <a id="shipped-preferred-models" />[Shipped preferred models](/en/tools/code-mode/configuration#shipped-preferred-models)
- <a id="models-shipped-by-more-than-one-provider" />[Models shipped by more than one provider](/en/tools/code-mode/configuration#models-shipped-by-more-than-one-provider)
- <a id="choosing-when-to-enable" />[Choosing when to enable](/en/tools/code-mode/configuration#choosing-when-to-enable)
- <a id="activation" />[Activation](/en/tools/code-mode/configuration#activation)
- <a id="model-visible-tools" />[Model-visible tools](/en/tools/code-mode/tool-surface#model-visible-tools)
- <a id="exec" />[The exec tool](/en/tools/code-mode/tool-surface#exec)
- <a id="source-in-session-history" />[Source in session history](/en/tools/code-mode/tool-surface#source-in-session-history)
- <a id="wait" />[The wait tool](/en/tools/code-mode/tool-surface#wait)
- <a id="tool-catalog" />[Tool catalog](/en/tools/code-mode/tool-surface#tool-catalog)
- <a id="tool-search-interaction" />[Tool Search interaction](/en/tools/code-mode/tool-surface#tool-search-interaction)
- <a id="tool-names-and-collisions" />[Tool names and collisions](/en/tools/code-mode/tool-surface#tool-names-and-collisions)
- <a id="guest-runtime-api" />[Guest runtime API](/en/tools/code-mode/guest-api#guest-runtime-api)
- <a id="reading-paginated-file-data" />[Reading paginated file data](/en/tools/code-mode/guest-api#reading-paginated-file-data)
- <a id="declared-output-contracts" />[Declared output contracts](/en/tools/code-mode/output#declared-output-contracts)
- <a id="output-api" />[Output API](/en/tools/code-mode/output#output-api)
- <a id="runtime-status" />[Runtime status](/en/tools/code-mode/internals#runtime-status)
- <a id="scope" />[Scope](/en/tools/code-mode/internals#scope)
- <a id="terms" />[Terms](/en/tools/code-mode/internals#terms)
- <a id="nested-tool-execution" />[Nested tool execution](/en/tools/code-mode/internals#nested-tool-execution)
- <a id="run-and-snapshot-lifecycle" />[Run and snapshot lifecycle](/en/tools/code-mode/internals#run-and-snapshot-lifecycle)
- <a id="quickjs-wasi-runtime" />[QuickJS-WASI runtime](/en/tools/code-mode/internals#quickjs-wasi-runtime)
- <a id="typescript" />[TypeScript](/en/tools/code-mode/internals#typescript)
- <a id="security-boundary" />[Security boundary](/en/tools/code-mode/internals#security-boundary)
- <a id="error-codes" />[Error codes](/en/tools/code-mode/troubleshooting#error-codes)
- <a id="telemetry" />[Telemetry](/en/tools/code-mode/troubleshooting#telemetry)
- <a id="debugging" />[Debugging](/en/tools/code-mode/troubleshooting#debugging)
- <a id="implementation-layout" />[Implementation layout](/en/tools/code-mode/maintainers#implementation-layout)
- <a id="validation-checklist" />[Validation checklist](/en/tools/code-mode/maintainers#validation-checklist)
- <a id="e2e-test-plan" />[E2E test plan](/en/tools/code-mode/maintainers#e2e-test-plan)

## Related

- [Swarm](/en/tools/swarm) for fan-out agent orchestration from Code Mode scripts
- [Tool Search](/en/tools/tool-search)
