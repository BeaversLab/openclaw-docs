...
**Via environment:** set `PERPLEXITY_API_KEY` or `OPENROUTER_API_KEY` in the Gateway process environment. For a gateway install, put it in `~/.openclaw/.env` (or your service environment). See [Env vars](/en/help/faq#env-vars-and-env-loading).
...
<CardGroup cols={2}>
  <Card title="Web search overview" href="/en/tools/web" icon="globe">
    All providers and auto-detection rules.
  </Card>
  <Card title="Brave search" href="/en/tools/brave-search" icon="shield">
    Structured results with country and language filters.
  </Card>
  <Card title="Exa search" href="/en/tools/exa-search" icon="magnifying-glass">
    Neural search with content extraction.
  </Card>
  <Card title="Perplexity provider" href="/en/providers/perplexity-provider" icon="server">
