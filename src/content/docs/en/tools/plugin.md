...
examples, see [Manage plugins](/en/plugins/manage-plugins). For the full generated
...
deterministic source selection. See [`openclaw plugins`](/en/cli/plugins#install)
...
See [Skills config](/en/tools/skills-config#operator-install-policy-securityinstallpolicy)
...
[`openclaw plugins list --enabled --verbose`](/en/cli/plugins#list) or
[`openclaw plugins inspect <id>`](/en/cli/plugins#inspect) with the listed plugin
...
- Use `api.registerHook(...)` only when you want to participate in the internal
  hook system described in [Hooks](/en/automation/hooks). This is mainly for coarse
