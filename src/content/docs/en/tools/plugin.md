...
[Manage plugins](/en/plugins/manage-plugins). For the generated inventory of
bundled, official external, and source-only plugins, see
[Plugin inventory](/en/plugins/plugin-inventory).
...
<Steps>
  <Step title="Find the plugin">
    Search [ClawHub](/en/clawhub) for public plugin packages:
...
[`openclaw plugins`](/en/cli/plugins#install) for the full command contract.
...
See [Skills config](/en/tools/skills-config#operator-install-policy-security-installpolicy)
...
snippet. Run [`openclaw plugins list --enabled --verbose`](/en/cli/plugins#list)
or [`openclaw plugins inspect <id>`](/en/cli/plugins#inspect) on the listed
...
[Plugin bundles](/en/plugins/bundles) for the bundle compatibility boundary and
[Building plugins](/en/plugins/building-plugins) for native plugin authoring.
...
- `api.registerHook(...)` for the internal hook system described in
  [Hooks](/en/automation/hooks). This is mainly for coarse command/lifecycle side
...
the running process. See [Plugin hooks](/en/plugins/hooks#quick-start) for a complete
example.
...
changes; they do not need an extra reload. See [Reload](/en/cli/plugins#reload) for
...
| Config says a plugin is missing                                | Check [Plugin inventory](/en/plugins/plugin-inventory) for whether it is bundled, official external, or source-only                           | Install the external package, enable the bundled plugin, or remove stale config                                                  |
...
[Plugin dependency resolution](/en/plugins/dependency-resolution).

## Related

- [Manage plugins](/en/plugins/manage-plugins) - command examples for list, install, update, uninstall, and publish
- [`openclaw plugins`](/en/cli/plugins) - full CLI reference
- [Plugin inventory](/en/plugins/plugin-inventory) - generated bundled and external plugin list
- [Plugin reference](/en/plugins/reference) - generated per-plugin reference pages
- [Community plugins](/en/plugins/community) - ClawHub discovery and docs PR policy
- [Plugin dependency resolution](/en/plugins/dependency-resolution) - install roots, registry records, and runtime boundaries
- [Building plugins](/en/plugins/building-plugins) - native plugin authoring guide
- [Plugin SDK overview](/en/plugins/sdk-overview) - runtime registration, hooks, and API fields
- [Plugin manifest](/en/plugins/manifest) - manifest and package metadata
- [Context engines](/en/concepts/context-engine) - pluggable context assembly plugins
- [Diffs](/en/tools/diffs) - read-only diff viewer and file renderer (optional plugin tool)
- [ACP agents — setup](/en/tools/acp-agents-setup) - configuring a plugin-provided ACP agent
