...
- **Sandboxing note**: the default sandbox backend uses Docker when sandboxing is enabled, but sandboxing is off by default and does **not** require the full gateway to run in Docker. SSH and OpenShell sandbox backends are also available. See [Sandboxing](/en/gateway/sandboxing).
...
export](/en/gateway/opentelemetry). Collector auth headers are configured through
...
[Prometheus metrics](/en/gateway/prometheus).
...
For gotchas and troubleshooting, see [Bonjour discovery](/en/gateway/bonjour).
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    (source checkout) or the inline `docker build` command from [Sandboxing § Images and setup](/en/gateway/sandboxing#images-and-setup) (npm install),
