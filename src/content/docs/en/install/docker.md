...
| `OPENCLAW_DISABLE_BONJOUR`                      | Force Bonjour/mDNS advertising on (`0`) or off (`1`); see [Bonjour / mDNS](/en/install/docker#bonjour-%2F-mdns)      |
...
Use the published Gateway URL, Tailscale, or wide-area DNS-SD for Docker hosts otherwise. See [Bonjour discovery](/en/gateway/bonjour) for gotchas and troubleshooting.
...
For full VM persistence details, see [Docker VM Runtime - What persists where](/en/install/docker-vm-runtime#what-persists-where).
...
needed. See [Manual flow](/en/install/docker#manual-flow) for setup and extra mounts.
...
adjust the printed dashboard URL as described in [Containerized gateway](/en/install/docker#containerized-gateway).
Use [Health checks](/en/install/docker#health-checks) to verify the gateway and
[Update OpenClaw](/en/install/docker-vm-runtime#update-openclaw) for image updates.

Token setup belongs to the [Docker setup flow](/en/install/docker#containerized-gateway).
If you need the Control UI token, read `OPENCLAW_GATEWAY_TOKEN` privately from the
project `.env`. [`config get <path>`](/en/cli/config) redacts sensitive values; it
...
See [Hetzner (Docker VPS)](/en/install/hetzner) and [Docker VM Runtime](/en/install/docker-vm-runtime) for shared VM deployment steps including binary baking, persistence, and updates.
...
- [Sandboxing](/en/gateway/sandboxing) -- complete sandbox reference
- [OpenShell](/en/gateway/openshell) -- OpenShell-managed local or remote sandbox backend
- [Multi-Agent Sandbox and Tools](/en/tools/multi-agent-sandbox-tools) -- per-agent overrides
