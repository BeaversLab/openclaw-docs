...
<Info>The [openclaw-ansible](https://github.com/openclaw/openclaw-ansible) repo is the source of truth for Ansible deployment. This page is a quick overview.</Info>
...
<Note>The gateway runs directly on the host (not in Docker), but agent sandboxes use Docker for isolation. See [Sandboxing](/en/gateway/sandboxing) for details.</Note>

## Post-Install Setup

<Steps>
  <Step title="Switch to the openclaw user">```bash sudo -i -u openclaw ```</Step>
  <Step title="Run the onboarding wizard">The post-install script guides you through configuring OpenClaw settings.</Step>
  <Step title="Connect messaging providers">Log in to WhatsApp, Telegram, Discord, or Signal: ```bash openclaw channels login ```</Step>
  <Step title="Verify the installation">```bash sudo systemctl status openclaw sudo journalctl -u openclaw -f ```</Step>
...
Docker is installed for agent sandboxes (isolated tool execution), not for running the gateway itself. See [Multi-Agent Sandbox and Tools](/en/tools/multi-agent-sandbox-tools) for sandbox configuration.
...
The Ansible installer sets up OpenClaw for manual updates. See [Updating](/en/install/updating) for the standard update flow.
...
- [Docker](/en/install/docker) -- containerized gateway setup
- [Sandboxing](/en/gateway/sandboxing) -- agent sandbox configuration
- [Multi-Agent Sandbox and Tools](/en/tools/multi-agent-sandbox-tools) -- per-agent isolation
