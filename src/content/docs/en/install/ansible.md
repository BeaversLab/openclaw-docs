...
backend. See [Sandboxing](/en/gateway/sandboxing) for other backends.
</Note>
...
Docker is installed for agent sandboxes (isolated tool execution), not for running the gateway. See [Multi-Agent Sandbox and Tools](/en/tools/multi-agent-sandbox-tools) for sandbox configuration.
...
The Ansible installer sets up OpenClaw for manual updates; see [Updating](/en/install/updating) for the standard flow.
