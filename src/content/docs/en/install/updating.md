...
for inspection. See [Automatic rollback](/en/install/updating#automatic-schema-neutral-rollback).
...
See [Failed update recovery](/en/gateway/restart-recovery#recovery-after-a-failed-update).
...
the Gateway; use the [Git installer](/en/install/installer) when changing that
...
The hosted [installers](/en/install/installer) also support npm-owned temporary provisioning
...
directory must be comma-free. See [Installer path requirements](/en/install/installer).
...
troubleshooting](/en/install/update-troubleshooting) for reason codes, guided
...
afterward, or [install a managed service](/en/cli/gateway#manage-the-gateway-service) for
unattended updates.
...
[Usage telemetry and update checks](/en/gateway/telemetry) for the information
...
[From chat](/en/install/updating#from-chat).
...
the dialog and **Settings → Updates**. See [Control UI updates](/en/web/control-ui/settings#updates).
...
directly into the app. Failure details stay visible with Retry, [Update guide](/en/install/updating), and
...
remain until explicit [update cleanup](/en/cli/update#update-cleanup). These are
...
[Doctor recovery command](/en/cli/doctor#session-sqlite-migration) before cleanup.
...
state, restore the pre-update backup using [Restore a full archive](/en/install/backups#restore-a-full-archive).
...
`-wal` or `-shm` files from another database generation. See [Backup](/en/cli/backup)
...
[bounded unattended repair](/en/install/updating#unattended-repair-on-your-own-inference)
...
live state directory. See [Backup](/en/cli/backup) for included and intentionally
omitted files.
...
volume and the state volume. See [Session SQLite migration](/en/cli/doctor#session-sqlite-migration)
...
include a saved update-failure artifact. See [Triage](/en/cli/triage) for command
...
[Failed update recovery](/en/gateway/restart-recovery#recovery-after-a-failed-update).
...
again. See [Triage](/en/cli/triage#installation-target-and-embedded-handoff) for the
