...
the dialog and **Settings → Updates**. See [Control UI updates](/en/web/control-ui#updates).
...
[Downgrading after session SQLite migration](/en/cli/doctor#downgrading-after-session-sqlite-migration).
...
archived. See [Restore a full archive](/en/install/backups#restore-a-full-archive)
...
and an `openclaw triage` external handoff. See [Triage](/en/cli/triage#installation-target-and-embedded-handoff)
