...
[Upgrading container images](/en/install/docker#upgrading-container-images). The
...
[manual package-manager procedure](/en/install/updating/update-methods#alternative-manual-npm-pnpm-or-bun)
...
from an older release, check [Node requirements](/en/install/node) and the npm
prefix's permissions first; see [update troubleshooting](/en/install/update-troubleshooting#node-and-global-install-permissions).
...
results. See
[Validation and activation](/en/cli/update#validation-and-activation) for the checks.
...
See [Release channels](/en/install/development-channels) for channel semantics.
...
old-CLI limitation in [Database schemas](/en/reference/database-schemas#schema-bumps-and-older-updaters).
...
[Run history and reports](/en/cli/update#run-history-and-reports) for Gateway history
queries.

The sender must be in [`commands.ownerAllowFrom`](/en/tools/slash-commands#configuration)
or have a [verified channel link to a current Gateway administrator](/en/concepts/user-model#channel-identity-links).
...
setup and [pairing](/en/channels/pairing) distinguish owner access from chat access;
...
[expire automatically](/en/cli/update/status-and-history#run-history-and-reports)
...
repair after it exits. See [Update repair](/en/cli/update/repair-and-recovery#update-repair)
...
is not required for this ledger defect. See [Update run history](/en/cli/update#run-history-and-reports).
...
[Update cleanup](/en/cli/update#update-cleanup) for eligibility, JSON output, and
...
Migrates config, audits DM policies, and checks gateway health. Doctor also compares active official plugins with the OpenClaw package the managed service will load after restart. Resolve any plugin restart-readiness warning before continuing. Details: [Doctor](/en/gateway/doctor)
...
collect results. See [Background exec notifications](/en/gateway/background-process#disable-automatic-completion-turns)
...
<CardGroup cols={3}>
  <Card title="Other update methods" href="/en/install/updating/update-methods" icon="shuffle">
    Switching between npm and git installs, source servers, the installer, and manual package managers.
  </Card>
  <Card title="Automatic updates" href="/en/install/updating/automatic-updates" icon="clock">
    The auto-updater, per-channel behavior, and update campaigns.
  </Card>
  <Card title="Rollback and recovery" href="/en/install/updating/rollback-and-recovery" icon="rotate-left">
    Downgrades, automatic rollback, pre-update backups, and triage.
  </Card>
</CardGroup>

- <a id="switch-between-npm-and-git-installs" />[Switch between npm and git installs](/en/install/updating/update-methods#switch-between-npm-and-git-installs)
- <a id="source-checkout-servers-(reference-script)" /><a id="source-checkout-servers-reference-script" />[Source-checkout servers (reference script)](/en/install/updating/update-methods#source-checkout-servers-reference-script)
- <a id="alternative%3A-re-run-the-installer" /><a id="alternative-re-run-the-installer" />[Alternative: re-run the installer](/en/install/updating/update-methods#alternative-re-run-the-installer)
- <a id="alternative%3A-manual-npm%2C-pnpm%2C-or-bun" /><a id="alternative-manual-npm-pnpm-or-bun" />[Alternative: manual npm, pnpm, or bun](/en/install/updating/update-methods#alternative-manual-npm-pnpm-or-bun)
  - <a id="package-lifecycle-and-operator-state" />[Package lifecycle and operator state](/en/install/updating/update-methods#package-lifecycle-and-operator-state)
  - <a id="advanced-npm-install-topics" />[Advanced npm install topics](/en/install/updating/update-methods#advanced-npm-install-topics)
    - <a id="read-only-package-tree" />[Read-only package tree](/en/install/updating/update-methods#read-only-package-tree)
    - <a id="hardened-systemd-units" />[Hardened systemd units](/en/install/updating/update-methods#hardened-systemd-units)
    - <a id="disk-space-preflight" />[Disk-space preflight](/en/install/updating/update-methods#disk-space-preflight)
- <a id="auto-updater" />[Auto-updater](/en/install/updating/automatic-updates#auto-updater)
  - <a id="update-campaigns" />[Update campaigns](/en/install/updating/automatic-updates#update-campaigns)
- <a id="downgrade" />[Downgrade](/en/install/updating/rollback-and-recovery#downgrade)
  - <a id="automatic-checkpoint-recovery" />[Full-state recovery requires a backup](/en/install/updating/rollback-and-recovery#automatic-checkpoint-recovery)
  - <a id="automatic-schema-neutral-rollback" />[Automatic schema-neutral rollback](/en/install/updating/rollback-and-recovery#automatic-schema-neutral-rollback)
  - <a id="before-updating%3A-create-a-verified-backup" /><a id="before-updating-create-a-verified-backup" />[Before updating: create a verified backup](/en/install/updating/rollback-and-recovery#before-updating-create-a-verified-backup)
- <a id="if-you-are-stuck" />[If you are stuck](/en/install/updating/rollback-and-recovery#if-you-are-stuck)
  - <a id="unattended-repair-on-your-own-inference" />[Unattended repair on your own inference](/en/install/updating/rollback-and-recovery#unattended-repair-on-your-own-inference)
