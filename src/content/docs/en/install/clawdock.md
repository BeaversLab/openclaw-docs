...
If you have not set up Docker yet, start with [Docker](/en/install/docker).
...
ClawDock reads two separate `.env` files, matching the split described in [Docker](/en/install/docker):
...
<CardGroup cols={2}>
  <Card title="Docker" href="/en/install/docker" icon="docker">
    Canonical Docker install for OpenClaw.
  </Card>
  <Card title="Docker VM runtime" href="/en/install/docker-vm-runtime" icon="cube">
    Docker-managed VM runtime for hardened isolation.
  </Card>
  <Card title="Updating" href="/en/install/updating" icon="arrow-up-right-from-square">
