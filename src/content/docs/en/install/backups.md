...
it. See [Database schemas](/en/reference/database-schemas) for the exact layout.
...
When [cold transcript storage](/en/reference/session-management-compaction/maintenance#cold-transcript-storage)
...
<Warning>
  ...
  [Migrating between machines](/en/install/migrating) for the same rules applied
  to machine moves.
</Warning>
...
workspaces remain ordinary workspace files. [Backup CLI](/en/cli/backup)
...
[hosting guides](/en/install), a systemd timer is the natural fit. `--json`
...
[Backup CLI](/en/cli/backup#versioned-git-backups) for the exact list.
...
up](/en/cli/backup#what-gets-backed-up). Then verify and extract into a fresh
...
[Updating](/en/install/updating#rollback) for the rollback workflow.
...
[Database schemas](/en/reference/database-schemas#preflight-a-target-release).
