...
`~/.openclaw`). See [Database schemas](/en/reference/database-schemas) for the
...
Gateway runs. [Backup CLI](/en/cli/backup) documents every flag, the volatile
...
backup; see [Backup CLI](/en/cli/backup#versioned-git-backups) for the exact list.
...
workspace assets to their new paths. See [Updating](/en/install/updating#rollback)
