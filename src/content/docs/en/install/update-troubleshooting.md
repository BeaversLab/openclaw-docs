...
[triage](/en/cli/triage). Only an affirmative Yes proceeds. Enter, `n`, cancellation,
...
commands. See [automatic recovery](/en/cli/triage#automatic-failure-handoff). The original update failure and exit status remain authoritative;
...
installations, follow [Gateway service recovery](/en/cli/doctor/recovery#gateway-service-recovery)
...
Node release. See [Node.js](/en/install/node).
...
[user-writable npm prefix](/en/install/node#permission-errors-on-npm-install-g-linux).
...
[verified backup](/en/install/updating/rollback-and-recovery#before-updating-create-a-verified-backup).
...
[backup recovery](/en/install/updating/rollback-and-recovery#downgrade); changing
...
retrying, or upgrade the host. See [plugin source lifetime](/en/plugins/architecture#runtime-instance-and-source-lifetime).
...
[plugin source lifetime](/en/plugins/architecture#runtime-instance-and-source-lifetime).
...
- `preflight-insufficient-space`: free space on the filesystems containing
  ...
  package-manager stores. See [Git checkout flow](/en/cli/update#git-checkout-flow)
...
changed` when the updater's umask differs from the installed launcher's
  ...
  Re-run the [installer](/en/install/installer) if the package install is
  incomplete.
...
- `doctor-failed`: run `openclaw doctor` on the Gateway host, resolve its
  findings, then retry. See [Doctor](/en/cli/doctor) for the check list and
  `--fix` behavior.
...
retrying. See [session recovery](/en/cli/doctor/sqlite-maintenance).
...
[Updating](/en/install/updating#alternative-re-run-the-installer).
