...
Remove the Gateway service **before** deleting a checkout, launcher, or prefix. Inspect the resolved command and its target first; if ownership is unclear, leave it in place. Check [Installer internals](/en/install/installer) for custom checkout and prefix options.
...
If completion was installed, remove only its `# OpenClaw Completion` block and OpenClaw source line from the [selected shell profile](/en/cli/completion#install-flow). Remove a legacy `openclaw completion` source/eval line only if it contains no other command; preserve surrounding content.
...
- [Migration guide](/en/install/migrating)
- [`openclaw uninstall`](/en/cli/uninstall) — command reference and flags
