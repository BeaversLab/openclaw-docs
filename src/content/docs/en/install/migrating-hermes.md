...
<Tabs>
  ...
  <Tab title="CLI">
    Use `openclaw migrate` for scripted or repeatable runs. See [`openclaw migrate`](/en/cli/migrate) for the full reference.
...
<Steps>
  ...
  <Step title="Run doctor">
    ...
    [Doctor](/en/gateway/doctor) reapplies any pending config migrations and checks for issues introduced during the import.
...
- Accepting the prompt imports OpenCode OpenAI OAuth credentials from OpenCode `auth.json`, OpenCode and GitHub Copilot entries from OpenCode `auth.json`, and the [supported `.env` keys](/en/cli/migrate#supported-env-keys). Hermes `auth.json` OAuth entries are reported for manual OpenAI reauth or doctor repair.
...
<AccordionGroup>
  ...
  <Accordion title="API keys did not import">
    Interactive `openclaw migrate` imports API keys only when you accept the credential prompt. Non-interactive `--yes` runs require `--include-secrets`; onboarding imports require `--import-secrets`. Only the [supported `.env` keys](/en/cli/migrate#supported-env-keys) are recognized; other variables in `.env` are ignored.
  </Accordion>
</AccordionGroup>

## Related

- [`openclaw migrate`](/en/cli/migrate): full CLI reference, plugin contract, and JSON shapes.
- [Onboarding](/en/cli/onboard): wizard flow and non-interactive flags.
- [Migrating](/en/install/migrating): move an OpenClaw install between machines.
- [Doctor](/en/gateway/doctor): post-migration health check.
- [Agent workspace](/en/concepts/agent-workspace): where `SOUL.md`, `AGENTS.md`, and memory files live.
