...
**Telegram bot not responding** -- With DM pairing required, an unknown sender gets a short pairing code, not a reply. Approve it from the OpenClaw dashboard chat. You can also run `openclaw pairing approve telegram <CODE>` if you have shell access to the container. See [Pairing](/en/channels/pairing).
