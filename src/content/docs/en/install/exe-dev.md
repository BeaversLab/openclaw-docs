...
For remote hosts, prefer one `config patch` call over many SSH calls to `config set`. Keep real tokens in the VM environment or `~/.openclaw/.env`, and put only SecretRefs in `openclaw.json`. See [Secrets management](/en/gateway/secrets) for the full SecretRef contract.
...
See [Discord](/en/channels/discord) and [Slack](/en/channels/slack) for full channel config reference.
...
See [Updating](/en/install/updating) for channel switches and manual recovery.

## Related

- [Remote gateway](/en/gateway/remote)
