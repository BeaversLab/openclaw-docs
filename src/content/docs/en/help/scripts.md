...
General model auth is covered in [Authentication](/en/gateway/authentication). The scripts below are a separate, optional system. They monitor a **Claude Code CLI subscription token** on a remote/headless host. They also let you re-authenticate from a phone:
...
Use `scripts/gh-read` when you want `gh` to use a GitHub App installation token for repo-scoped read calls. This leaves normal `gh` on your personal login for write actions.
