...
<AccordionGroup>
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    ...
    **Option A - run the Gateway on a Mac (simplest)**. Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](/en/help/faq/gateway-ports-and-remote-mode#gateway-ports-already-running-and-remote-mode) or over Tailscale. Skills load normally because the Gateway host is macOS.
  ...
  <Accordion title="How do I use my existing signed-in Chrome with OpenClaw?">
    ...
    See [Browser](/en/tools/browser/existing-session#existing-session-via-chrome-devtools-mcp) for the full comparison.
