...
- Live Gateway concurrency: dispatch `OpenClaw Performance` on `main` with
  ...
  [Gateway concurrency benchmark](/en/ci/scheduled-workflows#gateway-concurrency-benchmark).
...
<Tip>
When you only need one failing case, prefer narrowing live tests via the [allowlist env vars](/en/help/testing/docker).
</Tip>
