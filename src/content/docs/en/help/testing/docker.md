...
- Skill install smoke: `pnpm test:docker:skill-install` installs the packed OpenClaw tarball globally in Docker and disables uploaded archive installs in config. With no nonempty `OPENCLAW_SKILL_INSTALL_E2E_QUERY`, `OPENCLAW_SKILL_INSTALL_E2E_SLUG`, or `OPENCLAW_SKILL_INSTALL_E2E_PREFERRED_SLUG` override, it resolves the maintained `@steipete/gifgrep` fixture from live search, installs version `1.0.1`, and verifies its reviewed `SKILL.md` bytes plus exact owner/version in `.clawhub` origin/lock metadata. Security refusal is terminal; the smoke does not run the skill or install its optional tools. Any nonempty override retains custom selection and the existing security-checked fallback behavior without the maintained fixture pins.
...
- Published upgrade survivor smoke: `pnpm test:docker:published-upgrade-survivor` installs `openclaw@latest` by default, seeds realistic existing-user files, configures that baseline with a baked command recipe, validates the resulting config, updates that published install to the candidate tarball, runs non-interactive doctor, writes `.artifacts/upgrade-survivor/summary.json`, then starts a loopback Gateway and checks configured intents, state preservation, startup, `/healthz`, `/readyz`, and RPC status budgets. Override one baseline with `OPENCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC`, ask the aggregate scheduler to expand exact local baselines with `OPENCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS` such as `openclaw@2026.5.2 openclaw@2026.4.23 openclaw@2026.4.15`, and expand issue-shaped fixtures with `OPENCLAW_UPGRADE_SURVIVOR_SCENARIOS` such as `reported-issues`; the reported-issues set includes `configured-plugin-installs` for automatic external OpenClaw plugin install repair. Package Acceptance exposes those as `published_upgrade_survivor_baseline`, `published_upgrade_survivor_baselines`, and `published_upgrade_survivor_scenarios`, resolves meta baseline tokens such as `last-stable-4` or `all-since-2026.4.23`, and Full Release Validation runs all `reported-issues` scenarios against the latest stable baseline, resolved once to an exact package before fanout. Historical matrices remain explicit manual overrides.
  ...
  Successful published-upgrade runs also include a host-redacted `summary.json` in the Docker artifacts. The receipt keeps the initial update output and its post-core observation separate from repair and recovery output. Missing initial post-core evidence stays unknown; a passed scenario can include capability-consent recovery.

- Session runtime context smoke: `pnpm test:docker:session-runtime-context` verifies hidden runtime context transcript persistence, Doctor repair of affected duplicated prompt-rewrite branches, and automatic transcript cold storage through a real Gateway running the installed package. Small, dated synthetic histories exercise the maintenance worker, changes to `session.maintenance.coldStorage.afterDays` without a Gateway restart, exact transcript restoration, explicit errors for missing archive files, Activity listing without rehydration, and history preservation across restart and portable backup recovery.
...
- OpenAI chat tools and archived-history recall: `pnpm test:docker:openai-chat-tools` verifies an external tool call through the chat-completions endpoint, then runs one live agent turn whose answer requires a nonce from an archived transcript. The recall prompt omits the nonce, so the response proves that restored history reaches the provider. This lane requires an OpenAI API key.
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, malformed npm package metadata, git moving refs, ClawHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  ...
  Set `OPENCLAW_PLUGINS_E2E_LIVE_CLAWHUB=1` to use live ClawHub with the published `@openclaw/kitchen-sink` package; otherwise the hermetic fixture remains the Docker/CI default, even when a ClawHub URL is set.
...
Full Release Validation includes both cold-storage cases in its stable and full
Docker release-path profiles: `session-runtime-context` belongs to `core`, and
`openai-chat-tools` belongs to `package-update-openai`. Both use the candidate
package installed in the shared functional image. These bounded cases complement
the storage unit tests by checking packaged workers, Gateway configuration and
