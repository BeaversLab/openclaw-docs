...
- `openclaw triage` writes a sanitized, agent-ready diagnosis and, when the Gateway is reachable, a support archive. See [Triage](/en/cli/triage) for agent handoff options.
...
Full profile/group table: [Tool profiles](/en/gateway/config-tools/tool-policy#tool-profiles).
...
3. Tiny direct calls work but larger OpenClaw prompts crash the backend: that
   ...
   [Local OpenAI-compatible backend passes direct probes but agent runs fail](/en/gateway/troubleshooting#local-openai-compatible-backend-passes-direct-probes-but-agent-runs-fail).
...
Reference: [Operator install policy](/en/tools/skills-config#operator-install-policy-security-installpolicy)
...
Deeper docs: [Blocked plugin path ownership](/en/tools/plugin#blocked-plugin-path-ownership), [Docker: Permissions and EACCES](/en/install/docker#permissions-and-eacces)
...
<AccordionGroup>
  <Accordion title="No replies">
    ...
    Deep pages: [No replies](/en/gateway/troubleshooting#no-replies), [Channel troubleshooting](/en/channels/troubleshooting), [Pairing](/en/channels/pairing)
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `too many failed authentication attempts (retry later)` → repeated failures from that browser `Origin` are temporarily locked out; other localhost origins use separate buckets. See [Dashboard/Control UI connectivity](/en/gateway/troubleshooting#dashboard-control-ui-connectivity) for the Tailscale Serve concurrent-retry nuance.
    - `gateway connect failed:` → UI targets the wrong URL/port, or the gateway is unreachable.

    Deep pages: [Dashboard/Control UI connectivity](/en/gateway/troubleshooting#dashboard-control-ui-connectivity), [Control UI](/en/web/control-ui), [Authentication](/en/gateway/authentication)
  ...
  <Accordion title="Gateway will not start or service installed but not running">
    ...
    Deep pages: [Gateway service not running](/en/gateway/troubleshooting#gateway-service-not-running), [Supervision and service lifecycle](/en/gateway#supervision-and-service-lifecycle), [Configuration](/en/gateway/configuration)
  ...
  <Accordion title="Channel connects but messages do not flow">
    ...
    Deep pages: [Channel connected, messages not flowing](/en/gateway/troubleshooting#channel-connected-messages-not-flowing), [Channel troubleshooting](/en/channels/troubleshooting)
  ...
  <Accordion title="Cron or heartbeat did not fire or did not deliver">
    ...
    Deep pages: [Cron and heartbeat delivery](/en/gateway/troubleshooting#cron-and-heartbeat-delivery), [Scheduled tasks: Troubleshooting](/en/automation/cron-jobs#troubleshooting), [Heartbeat](/en/gateway/heartbeat)
  ...
  <Accordion title="Node is paired but tool fails camera canvas screen exec">
    ...
    Deep pages: [Node paired, tool fails](/en/gateway/troubleshooting#node-paired-tool-fails), [Node troubleshooting](/en/nodes/troubleshooting), [Exec approvals](/en/tools/exec-approvals)
  ...
  <Accordion title="Exec suddenly asks for approval">
    ...
    Deep pages: [Exec](/en/tools/exec), [Exec approvals](/en/tools/exec-approvals), [Security: What the audit checks](/en/gateway/security/running-the-audit#what-the-audit-checks-high-level)
  ...
  <Accordion title="Browser tool fails">
    ...
    Deep pages: [Browser tool fails](/en/gateway/troubleshooting#browser-tool-fails), [Missing browser command or tool](/en/tools/browser/setup#missing-browser-command-or-tool), [Browser: Linux troubleshooting](/en/tools/browser-linux-troubleshooting), [Browser: WSL2/Windows remote CDP troubleshooting](/en/tools/browser-wsl2-windows-remote-cdp-troubleshooting)
...
- [Doctor](/en/gateway/doctor) — automated health checks and repairs
- [Channel Troubleshooting](/en/channels/troubleshooting) — channel connectivity issues
- [Scheduled tasks: Troubleshooting](/en/automation/cron-jobs#troubleshooting) — cron and heartbeat issues
