...
| `OPENCLAW_SQLITE_LIBRARY` | Override the SQLite library for [Bun on macOS](/en/install/bun-compatibility#sqlite-library-selection). |
...
| Variable                             | Purpose                                                                                      |
| ------------------------------------ | -------------------------------------------------------------------------------------------- |
| `OPENCLAW_CONFIG_READONLY`           | Set to `1` for [externally managed immutable config](/en/cli/config#externally-managed-config). |
...
See [Workspace `.env` files](/en/gateway/security/secrets-and-storage#workspace-env-files) for the security rationale.
...
See [Secrets Management](/en/gateway/secrets) and the
[SecretRef credential surface](/en/reference/secretref-credential-surface) for
supported fields.
...
See [Configuration: Env var substitution](/en/gateway/config-secrets-env#env-var-substitution) for full details.
