...
lives on the [First-run FAQ](/en/help/faq-first-run).
...
<AccordionGroup>
  ...
  <Accordion title="How can I use different models or settings for different tasks?">
    ...
    See [Cron jobs](/en/automation/cron-jobs), [Multi-Agent Routing](/en/concepts/multi-agent), [Configuration](/en/gateway/config-agents), and [Slash commands](/en/tools/slash-commands).
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across all local agents, use `openclaw skills install @owner/<skill-slug> --global` (or place them manually in `~/.openclaw/skills/<name>/SKILL.md`). If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/en/tools/skills), [Skills config](/en/tools/skills-config), and [ClawHub](/en/tools/clawhub).
...
<AccordionGroup>
  ...
  <Accordion title="Can I keep DMs personal but make groups public/sandboxed with one agent?">
    ...
    Key config reference: [Gateway configuration](/en/gateway/config-agents#agentsdefaultssandbox)
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    embedding models - see [Memory](/en/concepts/memory) for the setup details.
...
lives on the [Models FAQ](/en/help/faq-models).
...
<AccordionGroup>
  ...
  <Accordion title="Are ClawHub skills and third-party plugins safe to install?">
    ...
    Details: [Skills](/en/tools/skills), [Plugins](/en/tools/plugin),
...
<AccordionGroup>
  ...
  <Accordion title='Why does it feel like the bot "ignores" rapid-fire messages?'>
    ...
    Default mode is `steer`. You can add options like `debounce:0.5s cap:25 drop:summarize` for queued modes. See [Command queue](/en/concepts/queue) and [Steering queue](/en/concepts/queue-steering).
