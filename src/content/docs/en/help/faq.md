...
<AccordionGroup>
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.openclaw/skills/<name>/SKILL.md`. If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/en/tools/skills), [Skills config](/en/tools/skills-config), and [ClawHub](/en/clawhub).
...
<AccordionGroup>
  ...
  <Accordion title='Why does it feel like the bot "ignores" rapid-fire messages?'>
    ...
    Default mode is `steer`. You can add options like `debounce:0.5s cap:25 drop:summarize` for followup modes. See [Command queue](/en/concepts/queue) and [Steering queue](/en/concepts/queue-steering).
