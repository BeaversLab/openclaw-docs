...
<AccordionGroup>
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    See [Skills](/en/tools/skills), [Skills config](/en/tools/skills-config), [ClawHub](/en/tools/clawhub).
...
<AccordionGroup>
  ...
  <Accordion title="How do I completely reset OpenClaw but keep it installed?">
    ...
    Onboarding also offers **Reset** if it detects an existing config; see [Onboarding (CLI)](/en/start/wizard). If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (default `~/.openclaw-<profile>`). Dev-only reset: `openclaw gateway --dev --reset` wipes dev config, credentials, sessions, and workspace.
