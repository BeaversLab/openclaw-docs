...
<AccordionGroup>
  ...
  <Accordion title="Value proposition">
    ...
    Docs: [Gateway](/en/gateway), [Channels](/en/channels), [Multi-agent](/en/concepts/multi-agent), [Memory](/en/concepts/memory).
  ...
  <Accordion title="Is OpenClaw owned by OpenAI?">
    No. OpenClaw is stewarded by the [OpenClaw Foundation](https://openclaw.org), an independent 501(c)(3). OpenAI is one of several donors, and its creator works there. Donors do not own, control, or direct the project. Codex is one [agent harness](/en/concepts/agent-runtimes) plugin among several, and no lab's model is privileged in the code.

  </Accordion>

  <Accordion title="What does OpenClaw send to the Foundation?">
    By default, a daily update check carrying the OpenClaw version, OS, Node version, and CPU architecture: the same information any package registry sees. Optional anonymous feature statistics are off by default and carry no identifier. No prompts, messages, model names, keys, paths, or machine identifiers are ever sent to the Foundation. Set `update.checkOnStart: false` to send nothing at all. Traffic to the model providers and chat platforms you configure is separate and goes to them, as always; see "Is all data used with OpenClaw saved locally?" below. Details: [Usage telemetry and update checks](/en/gateway/telemetry).

  </Accordion>

  <Accordion title="How is OpenClaw funded, and how does that compare?">
    The Foundation is funded by donations and has no product to sell: no paid tier, no hosted service, no token. It is not venture-backed. Some other self-hosted agents are built by venture-funded companies that sell a subscription their agent offers during setup. That is a difference in incentives, not a judgment of their engineering; see the [governance comparison](/en/start/why-openclaw#governance).
...
<AccordionGroup>
  ...
  <Accordion title="How can I use different models or settings for different tasks?">
    ...
    you set the optional [model selection scope](/en/gateway/config-agents/models#agentsdefaultsmodelselectionscope).
    ...
    See [Cron jobs](/en/automation/cron-jobs), [Multi-Agent Routing](/en/concepts/multi-agent), [Configuration](/en/gateway/config-agents), [Slash commands](/en/tools/slash-commands).
  ...
  <Accordion title="The bot freezes while doing heavy work. How do I offload that?">
    ...
    Docs: [Sub-agents](/en/tools/subagents), [Background Tasks](/en/automation/tasks).
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    Docs: [Sub-agents](/en/tools/subagents), [Background Tasks](/en/automation/tasks), [Session Tools](/en/concepts/session-tool).
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    Docs: [Cron jobs](/en/automation/cron-jobs), [Automation](/en/automation).
  ...
  <Accordion title="Cron fired, but nothing was sent to the channel. Why?">
    ...
    Docs: [Cron jobs](/en/automation/cron-jobs), [Background Tasks](/en/automation/tasks).
  ...
  <Accordion title="Why did an isolated cron run switch models or retry once?">
    ...
    Docs: [Cron jobs](/en/automation/cron-jobs), [cron CLI](/en/cli/cron).
  ...
  <Accordion title="Can OpenClaw run tasks on a schedule or continuously in the background?">
    ...
    Docs: [Cron jobs](/en/automation/cron-jobs), [Automation](/en/automation), [Heartbeat](/en/gateway/heartbeat).
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    See [Skills](/en/tools/skills), [Skills config](/en/tools/skills-config), [ClawHub](/en/clawhub).
  ...
  <Accordion title="How do I use my existing signed-in Chrome with OpenClaw?">
    ...
    See [Browser](/en/tools/browser#existing-session-via-chrome-devtools-mcp) for the full comparison.
...
<AccordionGroup>
  <Accordion title="Is there a dedicated sandboxing doc?">
    Yes: [Sandboxing](/en/gateway/sandboxing). For Docker-specific setup (full gateway in Docker or sandbox images), see [Docker](/en/install/docker).
  </Accordion>
  ...
  <Accordion title="Can I keep DMs personal but make groups public/sandboxed with one agent?">
    ...
    Setup walkthrough: [Groups: personal DMs + public groups](/en/channels/groups#pattern-personal-dms-public-groups-single-agent). Key reference: [Gateway configuration](/en/gateway/config-agents/sandbox#agentsdefaultssandbox).
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    See [Sandboxing](/en/gateway/sandboxing#multiple-folders-for-one-agent) and [Sandbox vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check).
...
<AccordionGroup>
  ...
  <Accordion title="Where does OpenClaw store its data?">
    ...
    doctor relocates it; see [Auth credential semantics](/en/auth-credential-semantics#agent-copy-portability).
  ...
  <Accordion title="Can I make SOUL.md bigger?">
    ...
    See [Context](/en/concepts/context) and [Agent config](/en/gateway/config-agents).
  ...
  <Accordion title="Remote mode: where is the session store?">
    Session state is owned by the **gateway host**. In remote mode, the session store you care about is on the remote machine, not your local laptop. See [Session management](/en/concepts/session).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Docs: [Config](/en/cli/config), [Configure](/en/cli/configure), [Gateway troubleshooting](/en/gateway/troubleshooting#gateway-rejected-invalid-config), [Doctor](/en/gateway/doctor).
  ...
  <Accordion title="Can the OpenClaw browser run headless?">
    ...
    Default is `false` (headful). Headless is more likely to trigger anti-bot checks on some sites (X/Twitter often blocks headless sessions). It uses the same Chromium engine and works for most automation; the main difference is no visible browser window (use screenshots for visuals). See [Browser](/en/tools/browser).

  </Accordion>

  <Accordion title="How do I use Brave for browser control?">
    Set `browser.executablePath` to your Brave binary (or any Chromium-based browser) and restart the Gateway. See [Browser](/en/tools/browser#use-brave-or-another-chromium-based-browser).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="How can my agent access my computer if the Gateway is hosted remotely?">
    ...
    Security reminder: pairing a macOS node allows `system.run` on that machine. Only pair devices you trust; review [Security](/en/gateway/security).
  ...
  <Accordion title="Can two OpenClaw instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic)**: run a script that calls the other Gateway with `openclaw agent --message ... --deliver`, targeting a chat where the other bot listens. If one bot is on a remote VPS, point your CLI at that remote Gateway via SSH/Tailscale (see [Remote access](/en/gateway/remote)):
  ...
  <Accordion title="Do nodes run a gateway service?">
    No. Only **one gateway** should run per host unless you intentionally run isolated profiles (see [Multiple gateways](/en/gateway/multiple-gateways)). Nodes are peripherals that connect to the gateway (iOS/Android nodes, or macOS "node mode" in the menubar app). For headless node hosts and CLI control, see [Node host CLI](/en/cli/node).
...
<AccordionGroup>
  ...
  <Accordion title="How do I completely reset OpenClaw but keep it installed?">
    ...
    To reset and immediately re-run onboarding, pass `openclaw onboard --reset`; reset is a command flag, not a **Setup mode** menu choice. See [Onboarding (CLI)](/en/start/wizard). If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (default `~/.openclaw-<profile>`). Dev-only reset: `openclaw gateway --dev --reset` wipes dev config, credentials, sessions, and workspace.
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    Per-agent overrides use `agents.entries.*.heartbeat`. Docs: [Heartbeat](/en/gateway/heartbeat).
  ...
  <Accordion title="Can I run multiple bots or chats at the same time (Slack), and how should I set that up?">
    ...
    Docs: [Multi-Agent Routing](/en/concepts/multi-agent), [Slack](/en/channels/slack), [Browser](/en/tools/browser), [Nodes](/en/nodes).
...
<AccordionGroup>
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Still stuck: `openclaw status --all` plus [Troubleshooting](/en/gateway/troubleshooting). See [Dashboard](/en/web/dashboard) for auth details.
...
<AccordionGroup>
  ...
  <Accordion title="I closed my terminal on Windows - how do I restart OpenClaw?">
    ...
    **3) Native Windows CLI/Gateway**: runs directly in Windows.
    ...
    Docs: [Windows](/en/platforms/windows), [Gateway service runbook](/en/gateway).
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must use structured media fields such as `media`, `mediaUrl`, `path`, or `filePath`. See [OpenClaw assistant setup](/en/start/openclaw) and [Agent send](/en/tools/agent-send).
...
<AccordionGroup>
  ...
  <Accordion title="Is OpenClaw less safe because it uses TypeScript/Node instead of Rust/WASM?">
    ...
    Details: [Security](/en/gateway/security), [Sandboxing](/en/gateway/sandboxing).
  ...
  <Accordion title="I saw reports about exposed OpenClaw instances. What should I check?">
    ...
    Public binds without auth, open DMs/groups with tools, and exposed browser control are the findings to fix first. Details: [openclaw security audit](/en/gateway/security#openclaw-security-audit).
  ...
  <Accordion title="Are ClawHub skills and third-party plugins safe to install?">
    ...
    Details: [Skills](/en/tools/skills), [Plugins](/en/tools/plugin), [Security](/en/gateway/security).
  ...
  <Accordion title="Can I give it autonomy over my text messages and is that safe?">
    ...
    To experiment, do it on a dedicated, isolated account. See [Security](/en/gateway/security).

  </Accordion>

  <Accordion title="Can I use cheaper models for personal assistant tasks?">
    Yes, **if** the agent is chat-only and the input is trusted. Smaller tiers are more susceptible to instruction hijacking, so avoid them for tool-enabled agents or when reading untrusted content. If you must use a smaller model, lock down tools and run inside a sandbox. See [Security](/en/gateway/security).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="How do I stop/cancel a running task?">
    ...
    Most slash commands must be sent as a **standalone** message starting with `/`, but a few shortcuts (like `/status`) also work inline for allowlisted senders. See [Slash commands](/en/tools/slash-commands).
  ...
  <Accordion title='Why does it feel like the bot "ignores" rapid-fire messages?'>
    ...
    Add options to queued modes like `debounce:0.5s cap:25 drop:summarize`. See [Command queue](/en/concepts/queue) and [Steering queue](/en/concepts/queue-steering).
