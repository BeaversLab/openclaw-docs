...
<AccordionGroup>
  ...
  <Accordion title="How do I configure fast mode for GPT 5.5?">
    ...
    See [Thinking and fast mode](/en/tools/thinking) and [OpenAI fast mode](/en/providers/openai#fast-mode).
...
<AccordionGroup>
  <Accordion title="What is an auth profile?">
    ...
    To inspect saved profiles without dumping secrets, run `openclaw models auth list` (optionally `--provider <id>` or `--json`). See [Models CLI](/en/cli/models#auth-profiles) for details.
