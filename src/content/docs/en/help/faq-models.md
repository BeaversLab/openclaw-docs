...
<AccordionGroup>
  <Accordion title="What is an auth profile?">
    ...
    To inspect saved profiles without dumping secrets, run `openclaw models auth list` (optionally `--provider <id>` or `--json`). See [Models CLI](/en/cli/models#openclaw-models-auth-list) for details.
