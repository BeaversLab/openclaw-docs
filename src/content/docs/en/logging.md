...
See [Control UI](/en/web/control-ui) for how to open it.
...
requires no debug flag. See [Session pruning](/en/concepts/session-pruning#direct-anthropic-api-key-requests)
...
ordinary performance logs and do not change [audit collection](/en/gateway/audit),
authorization, cache behavior or deadlines. Their sanitized attributes may flow
through an already-enabled [OpenTelemetry log exporter](/en/gateway/opentelemetry/privacy-and-trace-context)
...
logs. They do not use or change [audit identity](/en/gateway/audit), decisions,
...
For shorter delays, [profiler flags](/en/diagnostics/flags#profiler-flags) lower the
...
[per-agent SQLite database](/en/reference/database-schemas#database-layout). Matching
...
- **OpenTelemetry export** — send metrics, traces, and logs over OTLP/HTTP to
  ...
  [OpenTelemetry export](/en/gateway/opentelemetry).
...
For OTLP export to a collector, see [OpenTelemetry export](/en/gateway/opentelemetry).
...
- [OpenTelemetry export](/en/gateway/opentelemetry) — OTLP/HTTP export, metric/span catalog, privacy model
...
- [`openclaw logs`](/en/cli/logs) — tail Gateway logs over RPC from the CLI
