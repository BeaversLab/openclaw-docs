...
runs it as named shards (`native-live-src-agents`, `native-live-src-gateway`,
`native-live-test`, `native-live-extensions-a-k`, and
`native-live-extensions-l-z`) through `scripts/test-live-shard.mjs` instead of
one serial job. That keeps the same file coverage while making slow live
provider failures easier to rerun and diagnose.
