...
See [Worker background processes](/en/gateway/background-process#worker-environments)
...
Prefer managed private [Tailscale Serve](/en/gateway/tailscale) when the operator and
...
The `portal` tool follows ordinary tool policy, described in [Tools configuration](/en/gateway/config-tools). The optional `gateway.portals.ingress` setting configures only the private wildcard ingress described above; it does not grant tool access.
