...
[Install the packages](/en/gateway/clients#install-the-packages) for exact-version
...
semantics, see [Operator scopes](/en/gateway/operator-scopes).
...
See [Active computer presence](/en/nodes/presence) for selection, privacy, model
...
[Audit history](/en/gateway/audit) for the data model and privacy semantics.
...
[`logging.audit.enabled`](/en/gateway/configuration-reference#audit). Message
...
Use [`openclaw audit`](/en/cli/audit) for text queries and bounded JSON exports.
...
- Final `agent` results may include `result.deliveryStatus` when delivery was
  ...
  [`openclaw agent --json --deliver`](/en/cli/agent#json-delivery-status).
