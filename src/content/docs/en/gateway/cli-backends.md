...
Use it as a safety net for "always works" text responses, not a primary path. For a full harness runtime with ACP session controls, background tasks, thread/conversation binding, and persistent external coding sessions, use [ACP Agents](/en/tools/acp-agents) instead. CLI backends are not ACP.

<Tip>
  Building a new backend plugin? See [CLI backend plugins](/en/plugins/cli-backend-plugins). This page covers configuring and operating an already-registered backend.
</Tip>
...
[CLI backend plugin](/en/plugins/cli-backend-plugins) instead of putting launch
...
Background work started inside a CLI is still part of that CLI subprocess. If the parent turn reaches its overall limit, OpenClaw stops the subprocess and its CLI-internal background tasks together. For durable long work, use a detached OpenClaw [sub-agent](/en/tools/subagents) or [ACP agent](/en/tools/acp-agents). Detached sub-agents have no run timeout by default.
...
The `openclaw agent` command also has its own request deadline. Its 600-second fallback default applies to that command invocation, not to ordinary Gateway turns. See [`openclaw agent`](/en/cli/agent).
...
against the agent's [exec allowlist](/en/tools/exec-approvals). For example:
...
Docker installs need Claude Code and the chosen credentials inside the persisted container home, not only on the host. See [Claude CLI backend in Docker](/en/install/docker#claude-cli-backend-in-docker).
...
A `claude-cli` attempt can fail over to a non-CLI candidate in [`agents.defaults.model.fallbacks`](/en/concepts/model-failover). OpenClaw then seeds the next attempt with a context prelude harvested from Claude Code's local JSONL transcript. That transcript lives under `~/.claude/projects/`, keyed per workspace. This supplies CLI-owned context that may not be present in OpenClaw's SQLite session transcript.
...
- [Thinking levels](/en/tools/thinking) — how the reasoning budget maps onto a CLI backend
