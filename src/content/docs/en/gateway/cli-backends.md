...
[ACP Agents](/en/tools/acp-agents) instead. CLI backends are not ACP.

<Tip>
  Building a new backend plugin? Use
  [CLI backend plugins](/en/plugins/cli-backend-plugins). This page is for users
...
[`agents.defaults.model.fallbacks`](/en/concepts/model-failover), OpenClaw seeds
