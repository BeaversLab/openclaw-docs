...
[Gateway protocol specification](/en/gateway/protocol).
...
versions together, and check the [wire-version rules](/en/gateway/clients#track-protocol-versions)
...
The [operator scopes reference](/en/gateway/operator-scopes)
...
[Devices CLI](/en/cli/devices) for approval, rotation, and
revocation commands.
...
1. Re-establish `sessions.subscribe` with your list parameters to receive the
   ...
   [below](/en/gateway/clients#subscribe-instead-of-polling-usage). Also re-establish
...
key. See [Session list bootstrap](/en/gateway/protocol/rpc-methods#session-list-bootstrap).
...
- [Gateway integrations for external apps](/en/gateway/external-apps)
