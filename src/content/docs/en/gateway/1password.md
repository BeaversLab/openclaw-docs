...
- **Config secrets:** any [SecretRef](/en/gateway/secrets) field in `openclaw.json` can resolve through the `op` CLI at runtime, so API keys never live in the config file.
...
See [Gateway secrets](/en/gateway/secrets) for resolution order, caching, and failure semantics, and [SecretRef Credential Surface](/en/reference/secretref-credential-surface) for every field that accepts SecretRefs.
...
[1Password for Claude](https://support.1password.com/1password-claude/) lets Claude request a login while the 1Password browser extension fills the credential directly into the page over an encrypted channel. The secret never enters the model context, the transcript, or OpenClaw. When OpenClaw runs the [`claude-cli` backend](/en/gateway/cli-backends#claude-cli-specifics) with Claude Code's Chrome integration enabled, agent tasks can use that flow for websites that need a real signed-in session.
...
- A [CLI backend plugin](/en/plugins/cli-backend-plugins) that adds `--chrome` to the Claude launch args; the bundled backend does not enable Chrome.
