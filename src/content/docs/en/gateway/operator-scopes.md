...
Related: [Security](/en/gateway/security), [Gateway protocol](/en/gateway/protocol),
[Gateway pairing](/en/gateway/pairing), [Devices CLI](/en/cli/devices).
...
[GitHub connections](/en/concepts/user-model#github-connections).
...
[`openclaw logs --follow`](/en/cli/logs). Existing unstamped sessions and new sessions
...
An administrator-attested [channel identity link](/en/concepts/user-model#channel-identity-links)
...
startup. See [Gateway pairing](/en/gateway/pairing) for details.
...
- [Trusted proxy auth](/en/gateway/trusted-proxy-auth) — how a trusted proxy supplies the operator identity these scopes attach to
- [Gateway protocol](/en/gateway/protocol) — the methods these scopes authorize
- [Cloud Workers](/en/gateway/cloud-workers) — worker dispatch, whose environment and session calls are authorized against these scopes
