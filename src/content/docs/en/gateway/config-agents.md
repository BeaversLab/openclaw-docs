...
- To cap active input for one model, set `models.providers.<provider>.models[].contextTokens`; use `contextWindow` on the same entry for its native window. See [OpenAI context window defaults](/en/providers/openai#context-window-defaults-and-long-context-opt-in).
- `models`: configured aliases and per-model settings. Each entry can include `alias` (shortcut) and `params` (provider-specific, for example `temperature`, `maxTokens`, `cacheRetention`, `context1m`, `anthropicServerCompaction`, `anthropicCompactThreshold`, `responsesServerCompaction`, `responsesCompactThreshold`, OpenRouter `provider` routing, `chat_template_kwargs`, `extra_body`/`extraBody`). Adding entries does not restrict model overrides.
  ...
  - Add `codeMode: true` or `codeMode: false` to an exact `provider/model` entry to override OpenClaw Code Mode activation. Omit it to inherit the global `tools.codeMode` default, including `"auto"`; agent-specific activation settings take precedence. This changes neither runtime selection nor Codex native Code Mode. The Control UI model editor offers **Default**, **On**, and **Off** alongside runtime settings. See [per-model Code Mode](/en/tools/code-mode#override-one-model) for precedence and an example.
...
See [Model selection in chat](/en/concepts/models#model-in-chat) for persistence,
...
as shown above. See [CLI backends](/en/gateway/cli-backends) for operations and
[building CLI backend plugins](/en/plugins/cli-backend-plugins) for command,
...
See [OpenAI GPT-5 prompt contribution](/en/providers/openai#gpt-5-prompt-contribution) for provider and native Codex behavior.
...
- `provider`: id of a registered compaction provider plugin. When set, the provider's `summarize()` is called instead of built-in LLM summarization. Falls back to built-in on failure. Setting a provider forces `mode: "safeguard"`. See [Compaction](/en/concepts/compaction).
...
Optional sandboxing for the embedded agent. See [Sandboxing](/en/gateway/sandboxing) for the full guide.
...
- OpenShell: `plugins.entries.openshell.config`; see [OpenShell](/en/gateway/openshell)
...
For npm installs without a source checkout, see [Sandboxing § Images and setup](/en/gateway/sandboxing#images-and-setup) for inline `docker build` commands.
...
the `tts` agent tool. See [Text-to-speech](/en/tools/tts#per-agent-voice-overrides)
...
- `cwd`: optional working directory for reply runs, separate from `workspace`. Overrides `agents.defaults.cwd`; see [Working directory](/en/gateway/config-agents#agents.defaults.cwd) for precedence and sandbox restrictions.
