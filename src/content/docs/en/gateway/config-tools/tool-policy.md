...
`coding` and `messaging` also include the [theme tool](/en/tools/theme) and implicitly
...
`pdf` belongs to both `group:media` and `group:openclaw`. Group denies also cover PDF and override an explicit `pdf` allow entry. If an existing configuration should keep PDF access, remove or narrow the conflicting group deny. Group grants do not bypass [PDF model and authentication requirements](/en/tools/pdf).
...
[Code Mode - automatic per-model activation](/en/tools/code-mode/configuration#automatic-per-model-activation).
...
[Code Mode executors](/en/tools/code-mode/executors) for the trust boundary and
...
Restricts tools for the current turn's originating requester. This is defense-in-depth on top of channel access control; sender values must come from the channel adapter, not message text. It does not authenticate other content in the model prompt; see [Requester-scoped controls and prompt context](/en/gateway/security/hardened-baseline#requester-scoped-controls-and-prompt-context).
