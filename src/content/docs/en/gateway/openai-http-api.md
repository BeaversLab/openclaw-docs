...
`POST /v1/responses` is enabled separately with `gateway.http.endpoints.responses.enabled`. See [OpenResponses API](/en/gateway/openresponses-http-api).
...
See [Operator scopes](/en/gateway/operator-scopes), [Security](/en/gateway/security), and [Remote access](/en/gateway/remote).

## Authentication

Uses the Gateway auth configuration (see [Trusted proxy auth](/en/gateway/trusted-proxy-auth) for that mode's details):
...
- For native mobile clients that connect directly to a remote gateway, prefer [WebChat](/en/web/webchat) or the [Gateway Protocol](/en/gateway/protocol) with the paired-device bootstrap/device-token flow, so the device does not need a shared HTTP token/password.
- Build a channel plugin instead when integrating an external messaging network with its own users, rooms, webhook delivery, or outbound transport. See [Building plugins](/en/plugins/building-plugins).
...
Security note: allowlisting a hostname does not bypass private/internal IP blocking. For internet-exposed gateways, apply network egress controls in addition to app-level guards. See [Security](/en/gateway/security).
...
Failed agent runs, including whole-agent timeouts, return an error instead of a successful completion. A streaming failure emits an `error` object followed by `[DONE]`; partial content may already have reached the client. Timeout settings follow the [agent loop](/en/concepts/agent-loop#timeouts).
