...
- For native mobile clients that connect directly to a remote gateway, prefer [WebChat](/en/web/webchat) or the [Gateway Protocol](/en/gateway/protocol) and implement the paired-device bootstrap/device-token flow so the device does not need a shared HTTP token/password.
- Build a channel plugin instead when you are integrating an external messaging network with its own users, rooms, webhook delivery, or outbound transport. See [Building plugins](/en/plugins/building-plugins).
