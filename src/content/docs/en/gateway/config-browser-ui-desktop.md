...
- Host-local opens with a stable native CDP target and browser identity are
  ...
  [Tab cleanup ownership](/en/tools/browser/configuration#tab-cleanup-ownership).
...
- `existing-session` profiles can set `cdpUrl` when Chrome is already running
  ...
  [Custom Chrome MCP launch](/en/tools/browser/existing-session#custom-chrome-mcp-launch).
...
Agent display names, emoji, and avatars belong to each agent's `identity` block under `agents.entries`; see [Agent configuration](/en/gateway/config-agents/entries-and-multi-agent#agentsentries-per-agent-overrides).
...
replacing the display. See [Computer use](/en/nodes/computer-use#gateway-desktop).
...
The [Tauri companion](/en/platforms/linux#desktop-sharing) has the same sharing
