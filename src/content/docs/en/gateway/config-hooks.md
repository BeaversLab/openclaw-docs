...
| `mappings`                  | `[]`                            | Ordered mapping list; first match wins. See [Mapping details](/en/gateway/config-hooks#mapping-details).                                                                                                             |
...
| `gmail`                     | unset                           | Gmail transport and processing defaults; see [Gmail integration](/en/gateway/config-hooks#gmail-integration).                                                                                                        |
...
| `POST /hooks/agent`  | [Agent payload](/en/gateway/config-hooks#hook-agent-payload). By default returns `200 { ok: true, runId }` after session/global placement admission. With `waitForCompletion: true`, waits for the admitted run and adds bounded terminal execution/delivery facts in `completion`.                                                                                                                       |
| `POST /hooks/<name>` | First matching mapping produces wake/agent actions. No matching mapping returns `404`; no actions returns `204`. Agent fan-out has the [batch response contract](/en/gateway/config-hooks#hook-retries-and-fan-out).                                                                                                                                                                                      |
...
retries](/en/gateway/config-hooks#hook-retries-and-fan-out).
