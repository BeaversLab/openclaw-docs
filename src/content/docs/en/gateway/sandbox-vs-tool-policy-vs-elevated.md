...
For a per-agent configuration with several host folders, access modes, and the external-source safety opt-in, see [Multiple folders for one agent](/en/gateway/sandboxing#multiple-folders-for-one-agent).
...
See [Elevated Mode](/en/tools/elevated).
...
- [Session permission modes](/en/gateway/permission-modes) -- session roots and escalation reviewers
...
- [OpenShell](/en/gateway/openshell) — a managed sandbox backend that runs the sandbox layer over SSH
