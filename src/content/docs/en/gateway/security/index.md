...
- For Gmail hooks, the built-in per-message session isolates conversation context but does not remove the target agent's tool or workspace permissions. Route untrusted mail to a dedicated reader agent, apply [per-agent sandbox and tool restrictions](/en/tools/multi-agent-sandbox-tools), and constrain any handoff to the main agent with [`tools.agentToAgent`](/en/gateway/config-tools#toolsagenttoagent). See [Gmail integration](/en/gateway/configuration-reference#gmail-integration).
