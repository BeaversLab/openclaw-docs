...
use the [Gateway exposure runbook](/en/gateway/security/exposure-runbook) as a
...
inspection checks, see [npm shrinkwrap](/en/gateway/security/shrinkwrap).
...
Details: [Secure file operations](/en/gateway/security/secure-file-operations).
...
Set `contextVisibility` per channel or per room/conversation. See [Group Chats](/en/channels/groups#context-visibility-and-allowlists) for setup details.
...
[Security audit checks](/en/gateway/security/audit-checks).
...
- Forwarded-header evidence on a loopback request disqualifies loopback
  ...
  [Gateway pairing](/en/gateway/pairing) for both rules.
...
"token"` or `"password"`) or [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth)
instead.
