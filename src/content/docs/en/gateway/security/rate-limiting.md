...
guessing credentials; see the [exposure runbook](/en/gateway/security/exposure-runbook).
...
Auth](/en/gateway/trusted-proxy-auth) and the [Gateway security
guide](/en/gateway/security/network-exposure#reverse-proxy-configuration).
...
behind one proxy share a budget. See [External apps](/en/gateway/external-apps) for
...
- When a client is behind a reverse proxy, the effective IP is the resolved
  ...
  auth](/en/gateway/trusted-proxy-auth) for how proxy headers are validated before
