...
These tool-visibility and exec rules describe OpenClaw-managed tools. Native harnesses can retain their own tool surface under their permission controls; see [Codex runtime policy](/en/plugins/codex-harness-runtime).
...
New sessions, including managed worktree sessions, inherit the configured global or per-agent tool/exec policy when no mode is specified. Exec and file-tool settings are independent: `apply_patch` stays workspace-contained by default even when `tools.exec.mode` is `full`. See [apply_patch containment](/en/tools/apply-patch#notes) for the file settings, session-mode precedence, and whole-agent worker defaults.
...
[OpenClaw operations and approval](/en/cli/openclaw#operations-and-approval).
...
For the independent sandbox, tool-policy, and elevated-exec controls, see [Sandbox vs tool policy vs elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated).
