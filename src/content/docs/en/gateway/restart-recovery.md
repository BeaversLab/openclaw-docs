...
See [Database schemas](/en/reference/database-schemas) for downgrade precautions.
...
the base unit preserves them. See [Linux services](/en/platforms/linux).
...
service recovery, then opens [`openclaw triage`](/en/cli/triage). Triage immediately
...
can enter [bounded unattended repair](/en/install/updating#unattended-repair-on-your-own-inference)
...
unavailable and use the [existing downgrade finalization path](/en/install/updating#roll-back-a-package-install).
...
wait reports the last observed startup phase. Standalone restarts use the [progress-gated readiness wait](/en/cli/gateway/restart-and-supervision#restart-the-gateway).
Verification facts and measured downtime are retained in the
[update run report](/en/cli/update#run-history-and-reports).
...
[Automatic rollback](/en/install/updating#automatic-schema-neutral-rollback).
...
[Updating and recovery](/en/install/updating). Restart sentinels report the outcome.
...
A recovered Control UI turn can finish [pinned dashboard widgets](/en/tools/show-widget)
...
OpenClaw can also reconstruct interrupted read-only [Code Mode](/en/tools/code-mode)
...
The [background task registry](/en/automation/tasks) is SQLite-backed and
...
channels.start --params '{"channel":"<id>"}'`
  ...
  See also [Gateway](/en/gateway) (safe mode paragraph) for the same control-plane
...
- **Metrics:** recovery activity is exported via
  [Prometheus](/en/gateway/prometheus) as `openclaw_session_recovery_total` and
