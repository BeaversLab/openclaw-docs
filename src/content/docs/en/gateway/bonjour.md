...
The macOS app treats the resolved endpoint as a hint too. Selecting **Nearby Gateways** opens the connection editor; it does not authenticate to the advertised destination, change a saved route, or import SSH details or a certificate pin. Supply a trusted address, SSH target, or owner-provided setup code, then save the connection. See [Configure in the app](/en/platforms/mac/remote#configure-in-the-app).
...
- Wide-area DNS-SD setup helper: [`openclaw dns`](/en/cli/dns)
