...
Owner page: [Gateway authentication](/en/gateway/authentication) — auth modes, token/password setup, and the operator-facing policy this wire contract enforces.
...
Owner page: [Gateway pairing](/en/gateway/pairing) — the approval flow, device records, and CLI surfaces behind these fields.
...
Owner page: [Remote access](/en/gateway/remote) — configuring `gateway.tls` and obtaining the fingerprint clients pin.
