...
<Note>
Looking for the step-by-step setup? See [Give your Gateway a stable HTTPS URL](/en/gateway/stable-https-url).
</Note>
...
Plain HTTP remains a downgraded transport: identity signing does not encrypt the page, shared secret, or Gateway traffic. Prefer Tailscale Serve for HTTPS. See [Insecure HTTP](/en/web/control-ui#insecure-http).
...
| `trusted-proxy`   | Identity-aware reverse proxy; see [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth) |
...
When token mode has no credential, loopback startup generates a runtime-only token without writing it to config. A separate `openclaw qr` command still needs usable configured or supplied token/password authentication. That startup-only token does not configure pairing. See [QR auth resolution](/en/cli/qr).
