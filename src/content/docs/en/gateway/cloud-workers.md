...
Each cloud session retains a session-owned [managed-worktree mirror](/en/concepts/managed-worktrees) on the Gateway for reconciliation, recovery, and publication. The Gateway creates this checkout before cloud dispatch. The default count of 100 is a cleanup target, not an admission cap: count alone does not block creating or restoring the mirror, but the Gateway's disk-space checks still apply.
