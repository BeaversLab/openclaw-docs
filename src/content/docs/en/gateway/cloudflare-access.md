...
This is one supported remote-access topology alongside [Tailscale](/en/gateway/tailscale)
and an [SSH tunnel](/en/gateway/remote). Choose it when you want a
...
- Familiarity with [trusted-proxy auth](/en/gateway/trusted-proxy-auth), which this topology uses.
...
See [Gateway profiles and GitHub credit](/en/concepts/user-model#gateway-profile-and-github-credit).
...
`gateway.cloudflareAccess.clientId` / `clientSecret`; see [Node CLI](/en/cli/node). The only
...
onboarding flow. See [Nodes](/en/nodes/node-host#gateway-deployments-that-cannot-host-nodes).
...
[Remote access](/en/gateway/remote#gateway-behind-an-identity-aware-proxy), then run
...
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gateway rejected websocket upgrade (HTTP 302)` from the CLI or TUI | Access intercepted the upgrade. Configure `gateway.remote.edgeAuth`; see [Remote access](/en/gateway/remote#gateway-behind-an-identity-aware-proxy). |
