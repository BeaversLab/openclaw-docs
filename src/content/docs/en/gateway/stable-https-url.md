...
If you need a public URL, use [Tailscale Funnel](/en/gateway/tailscale#public-internet-funnel-%2B-shared-password) instead. Funnel is public, and OpenClaw requires password auth for it.
...
It does not authenticate HTTP API endpoints, remove browser device identity requirements, authenticate node-role connections, or bypass node pairing. See [Tailscale identity headers](/en/gateway/tailscale#tailscale-identity-headers-serve-only) for the full contract.
...
See [iOS app setup](/en/platforms/ios) and [Android connection setup](/en/platforms/android#connection-runbook) for their pairing steps.
...
This publishes `https://openclaw.<tailnet>.ts.net`. The Gateway host must be an approved tagged node, and the Service may require admin-console approval before Serve can publish it. See [Tailscale Services](/en/gateway/tailscale#tailnet-only-serve) for the full setup constraints.
...
- [Tailscale reference](/en/gateway/tailscale)
