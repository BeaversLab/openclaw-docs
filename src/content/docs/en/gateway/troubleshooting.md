...
<Warning>
For an intentional downgrade, follow [Downgrade](/en/install/updating#downgrade).
...
- [Skills config](/en/tools/skills-config#symlinked-skill-roots)
...
- [OpenAI-compatible endpoints](/en/gateway/config-gateway#openai-compatible-endpoints)
- [Provider configuration](/en/providers)
- [Logs](/en/logging)
...
| `AUTH_TOKEN_MISMATCH`        | Shared token did not match gateway auth token.                                                                                                                                               | If `canRetryWithDeviceToken=true`, allow one trusted retry. Cached-token retries reuse stored approved scopes; explicit `deviceToken` / `scopes` callers keep requested scopes. If still failing, run the [token drift recovery checklist](/en/cli/devices#token-drift-recovery-checklist). |
...
- [macOS platform notes](/en/platforms/macos)
- [Logging](/en/logging)
- [Doctor](/en/gateway/doctor)
...
- [macOS platform notes](/en/platforms/mac/bundled-gateway)
- [Doctor](/en/gateway/doctor)
- [Gateway CLI](/en/cli/gateway)
...
require operator repair. See [Legacy config key migrations](/en/gateway/doctor#detailed-behavior-and-rationale).
