...
consumer is subscribed, preserving [OTLP code locations](/en/gateway/opentelemetry/privacy-and-trace-context).
...
[model transport diagnostic flags](/en/logging#targeted-model-transport-diagnostics)
...
payloads or credentials; see [secret egress proxy](/en/gateway/secrets/secret-store-and-egress#secret-egress-proxy).
...
- Trace logging also includes diagnostic timing summaries for selected hot paths, such as plugin tool factory preparation. See [/tools/plugin#slow-plugin-tool-setup](/en/tools/plugin#slow-plugin-tool-setup).
