...
- **User identity** (`identity: "user"`) posts and reads as the authorizing human. It requires `userToken` plus `appToken` in Socket Mode, or `userToken` plus `signingSecret` in HTTP mode. No bot token or bot user is required. See [User identity](/en/channels/slack#user-identity-post-as-a-real-person) for user scopes and event subscriptions.
