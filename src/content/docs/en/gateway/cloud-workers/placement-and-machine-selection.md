...
Public source preparation and prepared checkout adoption do not transfer GitHub credentials. Subsequent OpenClaw worker turns use the effective shared or native GitHub identity through the existing [per-turn credential binding](/en/gateway/config-tools/github-identity), when one is available.
