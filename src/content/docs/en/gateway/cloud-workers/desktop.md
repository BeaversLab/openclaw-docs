...
For a web app, the agent starts a server and opens a [portal](/en/gateway/portals)
...
or cleanup. See [conversation environment storage](/en/reference/database-schemas/layout#conversation-environments)
...
The app uses the existing [local CUA trust boundary](/en/nodes/computer-use#trust-model): processes running as the worker account can discover and use its desktop resources. Desktop profile and tool-policy settings govern managed OpenClaw entry points; they do not sandbox authorized shell code. Use a separate account or image without these desktop grants when isolation from that code is required.
