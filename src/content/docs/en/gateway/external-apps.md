...
<Note>
  ...
  [Building a Gateway client](/en/gateway/clients#install-the-packages). The install
...
| ------------------------------------------------------------- | --------------- | --------------------------------------------------------------------------------------------- |
| [Gateway client guide](/en/gateway/clients#install-the-packages) | Stable packages | npm packages, auth, reconnect, history, events, approvals, and version policy.                |
| [Embedding guide](/en/gateway/embedding)                         | Release train   | Child-process environment, readiness, lifecycle, recovery, RPC ownership, and packaging.      |
| [Gateway protocol](/en/gateway/protocol)                         | Ready           | WebSocket transport, connect handshake, auth scopes, protocol versioning, and events.         |
| [Gateway protocol RPC methods](/en/gateway/protocol/rpc-methods) | Ready           | Current Gateway methods for agents, sessions, tasks, models, tools, artifacts, and approvals. |
| [`openclaw agent`](/en/cli/agent)                                | Ready           | One-shot script integration when shelling out to the CLI is enough.                           |
| [`openclaw message`](/en/cli/message)                            | Ready           | Sending messages or channel actions from scripts.                                             |

## Recommended path

1. Run or discover a Gateway.
2. Connect over the [Gateway protocol](/en/gateway/protocol).
3. Call documented RPC methods from [Gateway protocol RPC methods](/en/gateway/protocol/rpc-methods).
...
call resume. The
[Admin HTTP RPC plugin](/en/plugins/admin-http-rpc) remains available for hosts
...
[Restart recovery](/en/gateway/restart-recovery#what-is-not-resumed).
...
<Tip>
  ...
  projection](/en/plugins/hooks/lifecycle#safe-external-cron-projection).
</Tip>
...
- [Gateway protocol RPC methods](/en/gateway/protocol/rpc-methods)
- [CLI agent command](/en/cli/agent)
- [CLI message command](/en/cli/message)
