...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - For custom/self-hosted providers, see [Custom providers](/en/gateway/config-tools#custom-providers-and-base-urls) in the reference.
  ...
  <Accordion title="Control who can message the bot">
    ...
    See the [full reference](/en/gateway/config-channels#dm-and-group-access) for per-channel details.
  ...
  <Accordion title="Set up group chat mention gating">
    ...
    - See [full reference](/en/gateway/config-channels#group-chat-mention-gating) for visible reply modes, per-channel overrides, and self-chat mode.
  ...
  <Accordion title="Restrict skills per agent">
    ...
    - See [Skills](/en/tools/skills), [Skills config](/en/tools/skills-config), and
      the [Configuration Reference](/en/gateway/config-agents/workspace-and-bootstrap#agents-defaults-skills).
  ...
  <Accordion title="Configure per-channel health monitoring">
    ...
    - See [Health Checks](/en/gateway/health) for operational debugging and the [full reference](/en/gateway/config-gateway#gateway) for all fields.
  ...
  <Accordion title="Configure sessions and resets">
    ...
    - See [full reference](/en/gateway/config-agents/sessions#session) for all fields.
  ...
  <Accordion title="Enable sandboxing">
    ...
    Build the image first - from a source checkout run `scripts/sandbox-setup.sh`, or from an npm install see the inline `docker build` command in [Sandboxing § Images and setup](/en/gateway/sandboxing#images-and-setup).

    See [Sandboxing](/en/gateway/sandboxing) for the full guide and [full reference](/en/gateway/config-agents/sandbox#agentsdefaultssandbox) for all options.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    ...
    See [iOS App](/en/platforms/ios#relay-backed-push-for-official-builds) for the end-to-end flow and [Authentication and trust flow](/en/platforms/ios#authentication-and-trust-flow) for the relay security model.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    See [full reference](/en/gateway/config-hooks#hooks) for all mapping options and Gmail integration.
  ...
  <Accordion title="Configure multi-agent routing">
    ...
    See [Multi-Agent](/en/concepts/multi-agent) and [full reference](/en/gateway/config-agents/entries-and-multi-agent#multi-agent-routing) for binding rules and per-agent access profiles.
...
doctor --fix` for repair. See [Gateway troubleshooting](/en/gateway/troubleshooting#gateway-rejected-invalid-config)
for the checklist.
...
it is stopped. Then run [`openclaw doctor --fix`](/en/cli/doctor) and restart.
...
The earlier `hot` and `restart` modes are retired; [`openclaw doctor --fix`](/en/cli/doctor) maps both to `hybrid`. Reload debounce is no longer configurable and runs behind a built-in default.
...
[Inbound debounce settings](/en/concepts/messages#inbound-debouncing) apply at the
...
See [Onboard](/en/cli/onboard) for storage choices and connecting without a shared secret.
...
directory until restart. See [Plugin metadata snapshots](/en/plugins/architecture#plugin-metadata-snapshot-and-lookup-table).
...
field-level docs and constraints. Use [Configuration reference](/en/gateway/configuration-reference)
...
`deviceId+clientIp`; see [Rate limiting](/en/gateway/security/rate-limiting). Restart
