...
[`openclaw doctor --fix`](/en/cli/doctor). Doctor repairs directly authored values
...
[Configuration reference](/en/gateway/config-observability#diagnostics).
...
[CPU pressure and event-loop delay](/en/gateway/health#cpu-pressure-and-event-loop-delay).
...
see [Prometheus event-loop windows](/en/gateway/prometheus#event-loop-observation-windows).
...
[GC duration coverage and correlation limits](/en/gateway/prometheus#garbage-collection-duration).
...
[subscribe to session changes instead of polling usage reports](/en/gateway/clients#subscribe-instead-of-polling-usage).
See the [Gateway RPC method reference](/en/gateway/protocol/rpc-methods#rpc-method-families)
...
- `gateway.rpc` - trusted request observations with phases `received`, `response`,
  ...
  in [Gateway RPC metrics](/en/gateway/opentelemetry#gateway-rpc).
