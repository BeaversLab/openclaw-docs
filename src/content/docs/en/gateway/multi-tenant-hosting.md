...
Use one cell per tenant so each trust domain has a separate Gateway process, container, persistent state tree, and Gateway credential. This follows the [Gateway security model](/en/gateway/security): do not co-locate mutually untrusted users in one OpenClaw process or one OS user.
...
Persistent Gateway state comes from `<state-dir>/fleet/cells/<tenant>/` and is mounted at `/home/node/.openclaw`. Current OAuth access, refresh, and ID-token values are stored as plaintext in SQLite beneath that ordinary state mount, so treat cell-state backups and copies as credentials. The separate `<state-dir>/fleet/auth-profile-secrets/<tenant>/` host path is mounted at `/home/node/.config/openclaw`, matching the official [Docker persistence layout](/en/install/docker#storage-and-persistence), but contains only the key for legacy encrypted-sidecar recovery. It does not encrypt current SQLite rows or protect a state-only backup or copy. Per-tenant channel accounts terminate inside the cell that owns them; Fleet does not provide a shared channel account or inbound message router.
...
See the [`openclaw fleet` CLI reference](/en/cli/fleet) for every command and option.
...
- [`openclaw fleet`](/en/cli/fleet)
- [Gateway security](/en/gateway/security)
- [Multiple gateways](/en/gateway/multiple-gateways)
