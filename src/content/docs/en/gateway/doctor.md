...
[manual update sequence](/en/install/updating#updating-from-2026.9.2-across-a-schema-bump)
from the refusal. See [Database schemas](/en/reference/database-schemas#schema-bumps-and-older-updaters)
...
<AccordionGroup>
  ...
  <Accordion title="Config and migrations">
    ...
    - Retired QMD memory config and derived workspace cleanup; see [Migrating from QMD](/en/concepts/memory-builtin#migrating-from-qmd).
  ...
  <Accordion title="Gateway, services, and supervisors">
    ...
    - WhatsApp responsiveness reports Gateway pressure and detected local TUI clients without attributing the pressure to those clients. Inspect [Gateway diagnostics](/en/gateway/diagnostics) before deciding whether to close clients; Doctor does not stop them.
...
<AccordionGroup>
  ...
  <Accordion title="7b. Plugin install cleanup">
    ...
    If an unreadable native definition also blocks installation or self-update, follow [native service recovery](/en/cli/gateway#recover-an-unreadable-native-service-definition). Preserve service-only environment values before rebuilding the launcher; configuration and plugin state do not need to be deleted.
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - On macOS, a same-label system LaunchDaemon blocks user LaunchAgent install, start, restart, and bootstrap repair. Doctor reports the system owner and stops service recovery; `--force` does not bypass this ownership boundary. See [Existing system LaunchDaemons](/en/gateway#existing-system-launchdaemons).
    ...
    - Doctor service repairs refuse to rewrite, stop, or restart a gateway service from an older OpenClaw binary when the config was last written by a newer version. See [Gateway troubleshooting](/en/gateway/troubleshooting#split-brain-installs-and-newer-config-guard).
  ...
  <Accordion title="19. Workspace tips (backup + memory system)">
    ...
    See [/concepts/agent-workspace](/en/concepts/agent-workspace) for a full guide to workspace structure and git backup (recommended private GitHub or GitLab).
