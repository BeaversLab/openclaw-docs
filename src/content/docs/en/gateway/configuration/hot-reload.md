...
writes use the same schema gate before writing (see [Strict validation](/en/gateway/configuration#strict-validation)
...
certificate pins remain operator-controlled; see [Gateway TLS](/en/gateway/config-gateway#gateway-tls).
...
and [Plugin metadata snapshots](/en/plugins/architecture#plugin-metadata-snapshot-and-lookup-table).
...
and releases channel reload pauses. Detailed [`/ready`](/en/gateway/health#http-probes)
...
the Gateway. See [Plugin lifecycle](/en/plugins/architecture#plugin-metadata-snapshot-and-lookup-table)
