...
<Note>
This content now lives in [Remote Access](/en/gateway/remote#macos-persistent-ssh-tunnel-via-launchagent). Use that page for the current guide; this page stays as a redirect target.
</Note>
...
1. Add an SSH config entry with `LocalForward 18789 127.0.0.1:18789` (see [Remote Access](/en/gateway/remote#macos-persistent-ssh-tunnel-via-launchagent) for the full config block).
...
For a tunnel that survives reboots and reconnects automatically, use the LaunchAgent setup on the [Remote Access](/en/gateway/remote#macos-persistent-ssh-tunnel-via-launchagent) page instead of a manual `ssh -N`.
