...
Agent display names, emoji, and avatars belong to each agent's `identity` block under `agents.list`; see [Agent configuration](/en/gateway/config-agents#agentslist-per-agent-overrides).
...
- For untrusted inboxes, route Gmail to a dedicated reader agent and restrict that agent with [per-agent sandbox and tool policy](/en/tools/multi-agent-sandbox-tools). If the reader must notify the main agent, constrain the handoff with [`tools.agentToAgent`](/en/gateway/config-tools#tools-agenttoagent). See [Prompt injection](/en/gateway/security#prompt-injection) for the recommended threat model and model tier.
- The setup wizard configures Gmail transport but does not create the reader agent or required session-key policy. Apply the complete [restricted Gmail reader configuration](/en/automation/cron-jobs#configure-a-restricted-gmail-reader-recommended) before running setup for untrusted mail.
