...
Field-level reference for `~/.openclaw/openclaw.json`: keys, defaults, and links to deeper subsystem pages. For task-oriented setup guidance, see [Configuration](/en/gateway/configuration). Channel- and plugin-owned command catalogs and deep memory knobs live on their own pages, not here.
...
- [Memory configuration reference](/en/reference/memory-config) for `memory.search.*`, `memory.citations`, and dreaming config under `plugins.entries.memory-core.config.dreaming`.
- [Slash commands](/en/tools/slash-commands) for the current built-in + bundled command catalog.
...
Per-channel config keys live in [Configuration - channels](/en/gateway/config-channels): `channels.*` for Slack, Discord, Telegram, WhatsApp, Matrix, iMessage, and other channel plugins (auth, access control, multi-account, mention gating).

## Agent defaults, multi-agent, sessions, and messages

See [Configuration - agents](/en/gateway/config-agents) for:
...
Optional global root directory for [managed worktree](/en/concepts/managed-worktrees) checkouts. Defaults to `<openclaw-state-dir>/worktrees`.
...
[Configuration - tools and custom providers](/en/gateway/config-tools).

## Models

Provider definitions, model allowlists, and custom provider setup live in
[Configuration - tools and custom providers](/en/gateway/config-tools#custom-providers-and-base-urls).
...
- `models.providers.*.localService`: optional on-demand process manager for
  ...
  request. See [Local model services](/en/gateway/local-model-services).
...
See [MCP](/en/cli/mcp#openclaw-as-an-mcp-client-registry) and
[CLI backends](/en/gateway/cli-backends#bundle-mcp-overlays) for runtime behavior.
...
- Skill Workshop autonomy/approval settings (`workshop.autonomous.mode`, `workshop.approvalPolicy`, `workshop.maxPending`, `workshop.maxSkillBytes`) are documented in [Skills configuration](/en/tools/skills-config).
...
[Codex harness reference](/en/plugins/codex-harness-reference) for the full config
surface and [Codex harness](/en/plugins/codex-harness) for the runtime model.
...
[Native Codex plugins](/en/plugins/codex-native-plugins) for app-server version and
...
- `plugins.entries.memory-core.config.dreaming`: memory dreaming settings. See [Dreaming](/en/concepts/dreaming) for phases and thresholds.
...
- Full memory config lives in [Memory configuration reference](/en/reference/memory-config):
  - `memory.search.*`
...
- Host-local opens with a stable native CDP target and browser identity are
  ...
  [Tab cleanup ownership](/en/tools/browser#tab-cleanup-ownership).
...
- `extension` profiles use the authenticated OpenClaw Chrome extension relay.
  ...
  `cdpUrl`. See [Chrome extension](/en/tools/chrome-extension).
...
- `existing-session` profiles can set `cdpUrl` when Chrome is already running
  ...
  [Custom Chrome MCP launch](/en/tools/browser#custom-chrome-mcp-launch).
...
Agent display names, emoji, and avatars belong to each agent's `identity` block under `agents.entries`; see [Agent configuration](/en/gateway/config-agents#agentsentries-per-agent-overrides).
...
- `gateway.auth.mode: "trusted-proxy"`: delegate browser/user auth to an identity-aware reverse proxy and trust identity headers from `gateway.trustedProxies` (see [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth)). This mode expects a **non-loopback** proxy source by default; same-host loopback reverse proxies require explicit `gateway.auth.trustedProxy.allowLoopback = true`. Internal same-host callers can use `gateway.auth.password` as a local direct fallback; `gateway.auth.token` remains mutually exclusive with trusted-proxy mode.
...
- Admin HTTP RPC: off by default as the `admin-http-rpc` plugin. Enable the plugin to register `POST /api/v1/admin/rpc`. See [Admin HTTP RPC](/en/plugins/admin-http-rpc).
...
- Optional response hardening header:
  - `gateway.http.securityHeaders.strictTransportSecurity` hot-applies to subsequent responses, including health probes. Set only for HTTPS origins you control; use `false` or remove the value to stop sending the header. See [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth#tls-termination-and-hsts).
...
The earlier `"restart"` and `"hot"` values are retired; [`openclaw doctor --fix`](/en/cli/doctor) maps both to `"hybrid"`.

Reload debounce and in-flight operation deferral are no longer configurable and run behind built-in defaults. [`openclaw doctor --fix`](/en/cli/doctor) removes the retired `debounceMs` and `deferralTimeoutMs` keys from older config files.
...
Provider support and backend-specific setup belong to [Crabbox](https://crabbox.sh/providers/index.html). Configure credentials, coordinator access, networking, and snapshots there rather than duplicating them in OpenClaw settings. The installed backend must satisfy OpenClaw's [cloud-worker lifecycle requirements](/en/gateway/cloud-workers#crabbox-provider-support).
...
request, see [Webhooks](/en/automation/cron-jobs#webhooks). This is separate from
[internal hooks](/en/automation/hooks) (`hooks.internal`, `HOOK.md`) and the
[TaskFlow Webhooks plugin](/en/plugins/webhooks) (`plugins.entries.webhooks`).
...
| `mappings`                  | `[]`                            | Ordered mapping list; first match wins. See [Mapping details](/en/gateway/configuration-reference#mapping-details).                                                                                                  |
...
| `gmail`                     | unset                           | Gmail transport and processing defaults; see [Gmail integration](/en/gateway/configuration-reference#gmail-integration).                                                                                             |
| `internal`                  | separate subsystem              | Internal event-hook configuration; see [Hooks](/en/automation/hooks). It does not enable HTTP ingress.                                                                                                               |
...
| `POST /hooks/agent`  | [Agent payload](/en/gateway/configuration-reference#hook-agent-payload). Returns `200 { ok: true, runId }` after session/global placement admission, not completion.                                                                                                                                                                                                                                      |
| `POST /hooks/<name>` | First matching mapping produces wake/agent actions. No matching mapping returns `404`; no actions returns `204`. Agent fan-out has the [batch response contract](/en/gateway/configuration-reference#hook-retries-and-fan-out).                                                                                                                                                                           |
...
or channel delivery. See [hook verification](/en/automation/cron-jobs#verify-and-troubleshoot-hook-requests).
...
configuration](/en/automation/cron-jobs#configure-a-restricted-gmail-reader-recommended)
...
retries](/en/gateway/configuration-reference#hook-retries-and-fan-out).
...
contain tokens; see the [CLI reference](/en/cli/webhooks).
...
[logs and its run output](/en/automation/cron-jobs#verify-the-reader-boundary). For a
reader-to-agent handoff, expose only the required tool and constrain the
default-on [`tools.agentToAgent`](/en/gateway/config-tools#tools-agenttoagent)
policy with `allow`, or set `enabled: false` when no handoff is needed; see also
[Prompt injection](/en/gateway/security#prompt-injection) and
[per-agent sandbox and tools](/en/tools/multi-agent-sandbox-tools).
...
See [Secret egress proxy](/en/gateway/secrets#secret-egress-proxy) for subprocess environment wiring, authentication, fail-closed behavior, and limitations.
...
[`openclaw audit`](/en/cli/audit) or the
[`audit.activity.list`](/en/gateway/protocol#audit-ledger-rpc) Gateway RPC. See
[Audit history](/en/gateway/audit) for the full data model, privacy semantics,
and coverage limits.
...
Run [`openclaw doctor --fix`](/en/cli/doctor) to move it to `logging.audit`.
...
- `consoleStyle`: `"pretty"` or `"json"`. The earlier `"compact"` value is retired; [`openclaw doctor --fix`](/en/cli/doctor) maps it to `"pretty"`.
...
- Redaction is always on and is no longer configurable. [`openclaw doctor --fix`](/en/cli/doctor) removes the retired switch from older config files; the runtime always applies `tools`-mode redaction to logs and transcripts. UI, tool, and diagnostic safety surfaces redact secrets independently of this policy.
...
- `otel.enabled`: enables the OpenTelemetry export pipeline (default: `false`). For the full configuration, signal catalog, and privacy model, see [OpenTelemetry export](/en/gateway/opentelemetry).
...
- `otel.protocol`: `"http/protobuf"` (default). gRPC export is retired; run [`openclaw doctor --fix`](/en/cli/doctor) to repair a persisted legacy value or get source-specific manual-edit guidance.
...
See [Usage telemetry and update checks](/en/gateway/telemetry) for the complete payload, privacy guarantees, and all opt-out controls.
...
See `agents.entries` identity fields under [Agent defaults](/en/gateway/config-agents#agent-defaults).
...
[`openclaw doctor --fix`](/en/cli/doctor).
...
See [Automations](/en/automation/cron-jobs). Isolated automation runs are tracked as [background tasks](/en/automation/tasks).
