...
- If methods fail with `missing scope` after a successful WebSocket connect, reload so the browser pairs its device identity, or approve the pending device request. See [Control UI insecure HTTP](/en/web/control-ui/connect-and-pair#insecure-http).
...
backend helper path, or the [admin HTTP RPC plugin](/en/plugins/admin-http-rpc)
...
[Cloudflare Tunnel and Access](/en/gateway/cloudflare-access).
...
Use a [separate Gateway cell](/en/gateway/multi-tenant-hosting) when one owner needs a
...
<AccordionGroup>
  ...
  <Accordion title="Connection succeeds but methods report missing scope">
    ...
    - For custom automation, use device identity/pairing, the reserved direct-local `gateway-client` backend helper path, or [admin HTTP RPC](/en/plugins/admin-http-rpc).
...
- [Operator scopes](/en/gateway/operator-scopes) — roles, scopes, and approval checks
...
- [Security audit checks](/en/gateway/security/audit-checks) — the catalog entry for the trusted-proxy findings
