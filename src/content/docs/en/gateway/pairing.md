...
- **Device pairing** (role `node`) gates the `connect` handshake. See
  ...
  below and [Channel pairing](/en/channels/pairing).
...
Gateway command policy and [node-local exec approvals](/en/tools/exec-approvals)
...
- Shell allowlist and ask policy for `system.run` live in the node's
  [exec approvals](/en/tools/exec-approvals), not in the pairing record.
...
[Trusted Proxy Auth](/en/gateway/trusted-proxy-auth) for the equivalent rule on
operator auth.
...
- [Gateway protocol auth](/en/gateway/protocol/auth) — the wire contract for device identity, pairing signatures, and device tokens
- [Nodes CLI](/en/cli/nodes)
- [Devices CLI](/en/cli/devices)
