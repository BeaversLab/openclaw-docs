...
channel converge on the agent's [main session](/en/concepts/main-session).
...
See [Broadcast groups](/en/channels/broadcast-groups) for selection, continuation
...
[Doctor](/en/cli/doctor#session-sqlite-migration) migration sequence for inspection
and validation.
