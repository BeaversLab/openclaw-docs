...
See [Slash commands](/en/tools/slash-commands) for the command catalog and behavior.
...
<AccordionGroup>
  <Accordion title="Introductions when joining a server">
    ...
    Introductions are enabled by default, apply only to newly joined servers, and never run in direct messages. Set `channels.discord.joinIntro: false` to disable them, or set `channels.discord.accounts.<accountId>.joinIntro` to override one account. See [group join introductions](/en/channels#group-join-introductions) for the history limits, target-channel selection, once-per-room behavior, and untrusted-content handling.
...
an existing `transcripts.autoStart` source. See [Transcripts CLI](/en/cli/transcripts#configuration)
...
Enable Discord voice, configure an authenticated [speech-to-text provider](/en/nodes/audio),
...
the `transcripts` tool, the [CLI](/en/cli/transcripts), or the Control UI Meetings
...
- Batch capture respects the largest applicable [audio input limit](/en/nodes/audio), including configured model and CLI fallbacks. Oversized captures stop with a warning to speak a shorter segment; partial audio is not transcribed. This limit does not buffer or truncate direct realtime streams.
