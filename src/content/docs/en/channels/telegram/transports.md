...
<AccordionGroup>
  <Accordion title="Long polling vs webhook">
    ...
    The listener reserves `/healthz` for health checks, so `webhookPath` must use a different route. If an existing setup uses `/healthz`, choose another route, update the path in `webhookUrl` and the reverse proxy mapping, then verify that [hot reload](/en/gateway/configuration/hot-reload) applied the listener change with `openclaw channels status --probe`.
...
[durable ingress and replay dedupe](/en/plugins/sdk-channel-plugins/durable-ingress#durable-ingress-and-replay-dedupe).
...
effect. See [transport retention](/en/plugins/sdk-channel-plugins/durable-ingress#transport-classes-and-retention),
[at-least-once side effects](/en/plugins/sdk-channel-plugins/durable-ingress#at-least-once-side-effects),
and [inbound dead letters](/en/cli/channels#inbound-dead-letters).
...
[message and delivery hooks](/en/plugins/hooks/messages) and the
[hook catalog](/en/plugins/hooks/reference#hook-catalog).
