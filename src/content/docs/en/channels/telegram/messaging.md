...
```
  ...
  <Accordion title="Ack reactions">
    ...
    **Scope (`messages.ackReactionScope`, default `"group-mentions"`; no Telegram-account or Telegram-channel override):**
    ...
    <Note>
    The default scope (`group-mentions`) does not fire ack reactions in DMs or ambient room events. Use `direct` or `all` for DMs; only `all` acknowledges ambient room events. Changes follow [hot reload](/en/gateway/configuration/hot-reload) and apply to subsequent messages. Each assembled turn keeps its captured value.
    </Note>
  ...
  <Accordion title="Retained group history">
    ...
    No separate observation setting is needed. Keep Telegram [group visibility](/en/channels/telegram/setup#privacy-mode-and-group-visibility) enabled so the bot receives ordinary messages. Enabling history does not change explicit `requireMention: false` activation.
