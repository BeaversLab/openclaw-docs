...
- `channels.msteams.graphMediaFallback`: opt into Graph message lookups when channel/group HTML omits file markers (default `false`; see [Channel/group file recovery](/en/channels/msteams#channel%2Fgroup-file-recovery-graphmediafallback)).
...
- See [/gateway/configuration](/en/gateway/configuration) for `dmPolicy` and allowlist gating.
