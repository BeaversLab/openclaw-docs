...
| `"quiet"`         | Same as `"partial"` but the message is a non-notifying notice. Recipients are notified once a per-user push rule matches the finalized edit (see [Self-hosted push rules for quiet finalized previews](/en/channels/matrix/rich-messages#self-hosted-push-rules-for-quiet-finalized-previews)). |
