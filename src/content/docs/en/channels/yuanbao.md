...
<Steps>
  ...
  Config changes follow [hot reload](/en/gateway/configuration/hot-reload). Check that the channel is ready:
  ```bash
...
3. Verify that [hot reload](/en/gateway/configuration/hot-reload) applied the new credentials with `openclaw channels status --probe`.
