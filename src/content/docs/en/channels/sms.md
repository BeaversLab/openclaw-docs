...
<CardGroup cols={3}>
  ...
  <Card title="Gateway security" icon="shield" href="/en/gateway/security">
...
One Twilio number can serve both SMS and [Voice Call](/en/plugins/voice-call) if it has both capabilities. The SMS webhook and Voice webhook are configured separately in Twilio and use separate Gateway paths; this page only covers the SMS webhook.
...
`authToken` can be a [SecretRef](/en/gateway/secrets) (`source: "env" | "file" | "exec" | "store"`). Use this when the Gateway should resolve the Twilio Auth Token from the OpenClaw secrets runtime instead of storing plaintext config:
...
- [Secrets management](/en/gateway/secrets) — resolving the Twilio auth token from a SecretRef
- [Configuration — gateway](/en/gateway/config-gateway) — `gateway.trustedProxies` and the other Gateway settings used above
