...
Check the [application result](/en/plugins/manage-plugins#apply-changes-and-inspect) after installing or updating the plugin.
...
agent. See [Channel routing](/en/channels/channel-routing) for matching precedence.
...
untrusted input, and configure that agent's [sandbox and tool policy](/en/gateway/sandbox-vs-tool-policy-vs-elevated)
...
identity prefixes. See [shared prefix behavior](/en/concepts/messages#prefixes-threading-and-replies)
...
`exec` SecretRef. See [Secrets management](/en/gateway/secrets) for provider setup.
...
3. Replace the configured private key and verify that [hot reload](/en/gateway/configuration/hot-reload) applies the change. Restart the Gateway if the key comes from a changed service environment.
...
- [Channel overview](/en/channels)
- [Channel access controls](/en/channels/groups)
