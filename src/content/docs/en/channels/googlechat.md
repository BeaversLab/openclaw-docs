...
4. Messages route by space:
   ...
   - DMs collapse into the agent's main session by default. Set `session.dmScope` for per-peer DM sessions (see [Session](/en/concepts/session)).
...
- Bot-authored messages are ignored by default. With `allowBots: true`, accepted bot messages use shared [bot loop protection](/en/channels/bot-loop-protection): configure `channels.defaults.botLoopProtection`, then override with `channels.googlechat.botLoopProtection` or `channels.googlechat.groups.<space>.botLoopProtection`.
...
2. **Plugin not enabled**: check plugin status:
   ...
   If it shows "disabled", run `openclaw plugins enable googlechat` and check the [application result](/en/plugins/manage-plugins#apply-changes-and-inspect).

3. **Configuration not applied**: check [hot reload](/en/gateway/configuration/hot-reload) status and Gateway logs. Start the Gateway if it is offline. If you changed the service environment, restart the Gateway to load it.
