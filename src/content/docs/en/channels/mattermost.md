...
<Steps>
  <Step title="Ensure plugin is available">
    Install `@openclaw/mattermost` with the command above. Check the [application result](/en/plugins/manage-plugins#apply-changes-and-inspect) before continuing.
  </Step>
...
`MATTERMOST_URL` cannot be set from a workspace `.env`; see [Workspace .env files](/en/gateway/security).
</Note>
...
- `channels.mattermost.allowFrom` accepts user ids (recommended) and `accessGroup:<name>` entries. See [Access groups](/en/channels/access-groups).

## Channels (groups)

- Default: `channels.mattermost.groupPolicy = "allowlist"` (mention-gated).
- Allowlist senders with `channels.mattermost.groupAllowFrom` (user IDs recommended).
- `channels.mattermost.groupAllowFrom` accepts `accessGroup:<name>` entries. See [Access groups](/en/channels/access-groups).
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    ...
    - See [Streaming](/en/concepts/streaming#preview-streaming-modes) for the channel-mapping matrix.
