...
Use `channels.twitch.accounts` with per-account credentials. See [Configuration](/en/gateway/configuration) for the shared pattern.
...
With [execution identity collection](/en/gateway/audit#run-identity-inspection) enabled,
