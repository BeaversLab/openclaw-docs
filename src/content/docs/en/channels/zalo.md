...
3. Check `openclaw channels status --probe`; start the Gateway if it is offline. Config changes follow [hot reload](/en/gateway/configuration/hot-reload). If you changed the service environment, restart the Gateway to load it.
...
- Pairing: unknown senders get a pairing code. Messages are ignored until approved. Codes expire after 1 hour.
  ...
  - Details: [Pairing](/en/channels/pairing)
