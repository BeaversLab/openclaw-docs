...
- an existing [OpenClaw Discord bot](/en/channels/discord)
...
<Steps>
  ...
  <Step title="Verify and test">
    ...
    Config changes follow [hot reload](/en/gateway/configuration/hot-reload). Restart the Gateway if you changed its service environment to provide the client secret. In a Discord conversation, ask the agent to show an interactive widget. The agent calls `show_widget`; click **Open widget** on the posted message.
  </Step>
</Steps>
...
- The widget can navigate within its own nested frame. The `sandbox="allow-scripts"` iframe blocks top-level navigation, popups, and same-origin access. Its Content Security Policy allows the shared [widget CDN scripts, stylesheets, and fonts](/en/tools/show-widget#libraries-and-fonts), while blocking API connections and other external resources. These controls are defense-in-depth, not a security boundary against the agent that authored the widget.
...
- confirm [hot reload](/en/gateway/configuration/hot-reload) applied the OpenClaw configuration; restart the Gateway if you changed its service environment
