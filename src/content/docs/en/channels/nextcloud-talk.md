...
Check the [application result](/en/plugins/manage-plugins#apply-changes-and-inspect) after installing.
...
5. Check `openclaw channels status --probe`; start the Gateway if it is offline. Config changes follow [hot reload](/en/gateway/configuration/hot-reload). If you changed the service environment, restart the Gateway to load it.
