...
Full troubleshooting: [WhatsApp troubleshooting](/en/channels/whatsapp#troubleshooting)
...
Full troubleshooting: [Telegram troubleshooting](/en/channels/telegram#troubleshooting)
...
| Typing/token usage but no Discord message                    | Check whether this is an ambient room event or an opted-in `message_tool` room where the model missed `message(action=send)` | Inspect the gateway verbose log for suppressed final payload metadata, verify `messages.groupChat.unmentionedInbound`, read [Ambient room events](/en/channels/ambient-room-events), or keep `messages.groupChat.visibleReplies: "automatic"` for normal group requests. |
| DM replies missing                                           | `openclaw pairing list discord`                                                                                              | Approve DM pairing or adjust DM policy.                                                                                                                                                                                                                               |
| Bot silent in channels that used to work                     | Check whether the guild entry gained a `channels` map                                                                        | A channel map is an allowlist: unlisted channels are denied. Add a `"*"` wildcard entry. See [Guild channel maps are allowlists](/en/channels/discord#guild-channel-maps-are-allowlists).                                                                                |
| Agent cannot see room history or attachments from other bots | Check the room's `requireMention` and the account's `allowBots`                                                              | `requireMention: true` drops unmentioned messages before they become room events, so there is no backlog. Bot-authored messages and their attachments need `allowBots` (`"mentions"` is the safer setting). See [Ambient room events](/en/channels/ambient-room-events). |
...
Full troubleshooting: [Discord troubleshooting](/en/channels/discord/troubleshooting#troubleshooting)
...
Full troubleshooting: [iMessage troubleshooting](/en/channels/imessage#troubleshooting)
...
Full troubleshooting: [Signal troubleshooting](/en/channels/signal#troubleshooting)
...
| Symptom                         | Fastest check                               | Fix                                                                                                                       |
| ------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| Bot replies "gone to Mars"      | Verify `appId` and `clientSecret` in config | Correct credentials, then check `openclaw channels status --probe` after [hot reload](/en/gateway/configuration/hot-reload). |
...
Full troubleshooting: [QQ Bot troubleshooting](/en/channels/qqbot#troubleshooting)
...
unclean boots, the [crash-loop breaker](/en/gateway/restart-recovery#safety-valves-and-observability)
