...
to a running Gateway; see [Apply changes and inspect](/en/plugins/manage-plugins#apply-changes-and-inspect).
...
instead. With the default [hot reload](/en/gateway/configuration/hot-reload),
...
Full walkthrough: [Telegram](/en/channels/telegram). Command reference:
[`openclaw channels`](/en/cli/channels).
...
- [A2A](/en/channels/a2a) - Connect external agents to OpenClaw through the A2A 1.0 JSON-RPC protocol (bundled plugin).
- [Buzz](/en/channels/buzz) - Connect OpenClaw agents to Buzz rooms (official plugin).
- [ClickClack](/en/channels/clickclack) - ClickClack bot-token channel setup and target syntax (official plugin).
- [Discord](/en/channels/discord) - Discord bot setup, config keys, components, voice, and troubleshooting (official plugin).
- [Feishu](/en/channels/feishu) - Feishu bot overview, features, and configuration (official plugin).
- [Google Chat](/en/channels/googlechat) - Google Chat app support status, capabilities, and configuration (official plugin).
- [iMessage](/en/channels/imessage) - Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new OpenClaw iMessage setups when host requirements fit (official plugin).
- [IRC](/en/channels/irc) - IRC plugin setup, access controls, and troubleshooting (official plugin).
- [LINE](/en/channels/line) - LINE Messaging API plugin setup, config, and usage (official plugin).
- [Matrix](/en/channels/matrix) - Matrix support status, setup, and configuration examples (official plugin).
- [Mattermost](/en/channels/mattermost) - Mattermost bot setup and OpenClaw config (official plugin).
- [Microsoft Teams](/en/channels/msteams) - Microsoft Teams bot support status, capabilities, and configuration (official plugin).
- [Nextcloud Talk](/en/channels/nextcloud-talk) - Nextcloud Talk support status, capabilities, and configuration (official plugin).
- [Nostr](/en/channels/nostr) - Nostr DM channel via NIP-04 encrypted messages (official plugin).
- [QQ bot](/en/channels/qqbot) - QQ Bot setup, config, and usage (official plugin).
- [Raft](/en/channels/raft) - Raft External Agent support through the Raft CLI wake bridge (official plugin).
- [Reef](/en/channels/reef) - Reef channel setup: guarded, end-to-end-encrypted messaging between OpenClaw agents of different people (bundled plugin).
- [Signal](/en/channels/signal) - Signal support via signal-cli (native daemon or bbernhard container), setup paths, and number model (official plugin).
- [Slack](/en/channels/slack) - Slack setup and runtime behavior (Socket Mode, HTTP Request URLs, and relay mode) (official plugin).
- [SMS](/en/channels/sms) - Twilio SMS/MMS setup, access controls, webhooks, and delivery status (official plugin).
- [Synology Chat](/en/channels/synology-chat) - Synology Chat webhook setup and OpenClaw config (official plugin).
- [Telegram](/en/channels/telegram) - Telegram bot support status, capabilities, and configuration (bundled plugin).
- [Tlon](/en/channels/tlon) - Tlon/Urbit support status, capabilities, and configuration (official plugin).
- [Twitch](/en/channels/twitch) - Twitch chat bot: install, credentials, access control, token refresh (official plugin).
- [WebChat](/en/web/webchat) - Native and Control UI WebChat usage over the Gateway WebSocket (included in core).
- [WeChat](/en/channels/wechat) - WeChat channel setup through the external openclaw-weixin plugin (external plugin).
- [WeCom](/en/channels/wecom) - Install the external WeCom plugin and find its versioned setup documentation (external plugin).
- [WhatsApp](/en/channels/whatsapp) - WhatsApp channel support, access controls, delivery behavior, and operations (official plugin).
- [Yuanbao](/en/channels/yuanbao) - Yuanbao bot overview, features, and configuration (external plugin).
- [Zalo](/en/channels/zalo) - Zalo bot support status, capabilities, and configuration (official plugin).
- [Zalo ClawBot](/en/channels/zaloclawbot) - Zalo ClawBot channel setup through the external openclaw-zaloclawbot plugin (external plugin).
- [Zalo personal](/en/channels/zalouser) - Zalo personal account support via native zca-js (QR login), capabilities, and configuration (official plugin).
