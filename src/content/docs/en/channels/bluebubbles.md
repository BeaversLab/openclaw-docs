...
<Tabs>
  <Tab title="DMs">
    ...
    - Pairing is the default token exchange. Details: [Pairing](/en/channels/pairing)
...
See [Configuration](/en/gateway/configuration) for template variables.
...
- There is no localhost bypass for BlueBubbles webhook auth. If you proxy webhook traffic, keep the BlueBubbles password on the request end-to-end. `gateway.trustedProxies` does not replace `channels.bluebubbles.password` here. See [Gateway security](/en/gateway/security#reverse-proxy-configuration).
