...
`plugins install` registers and enables the plugin; no separate `enable` step is needed. The channel still does nothing until configured below. See [Plugins](/en/tools/plugin) for general install rules.
...
Token and password SecretRefs follow the shared [source-specific provider-alias rules](/en/gateway/secrets#provider-config), including for named accounts. An explicit matching `env` provider still enforces its allowlist; an empty allowlist denies all variables.
...
See [group join introductions](/en/channels#group-join-introductions) for room
...
For account `ops`, names become `MATRIX_OPS_HOMESERVER`, `MATRIX_OPS_ACCESS_TOKEN`, and so on. `MATRIX_HOMESERVER` (and any `*_HOMESERVER` scoped variant) cannot be set from a workspace `.env`; see [Workspace `.env` files](/en/gateway/security).
...
Matrix uses the shared audio media provider under `tools.media.audio`, such as OpenAI `gpt-4o-mini-transcribe`. See [Media tools overview](/en/tools/media-overview) for provider setup and limits.
...
`streaming.mode: "quiet"` only notifies recipients once a block or turn is finalized - a per-user push rule must match the finalized preview marker. See [Matrix push rules for quiet previews](/en/channels/matrix-push-rules) for the full recipe.
...
- Accepted configured-bot messages use shared [bot loop protection](/en/channels/bot-loop-protection). Configure `channels.defaults.botLoopProtection`, then override per-account with `channels.matrix.botLoopProtection` or per-room with `channels.matrix.groups.<room>.botLoopProtection`.
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    See [Matrix migration](/en/channels/matrix-migration) for the full upgrade flow.
...
Related: [Exec approvals](/en/tools/exec-approvals).
...
See [Configuration reference](/en/gateway/config-channels#multi-account-all-channels) for the shared multi-account pattern.
