...
`plugins install` registers and enables the plugin, so no separate `openclaw plugins enable matrix` step is needed. The plugin still does nothing until you configure the channel below. See [Plugins](/en/tools/plugin) for general plugin behavior and install rules.
...
`MATRIX_HOMESERVER` cannot be set from a workspace `.env`; see [Workspace `.env` files](/en/gateway/security).
...
Matrix uses the shared audio media provider configured under `tools.media.audio`, such as OpenAI `gpt-4o-mini-transcribe`. See [Media tools overview](/en/tools/media-overview) for provider setup and limits.
...
`streaming: "quiet"` only notifies recipients once a block or turn is finalized - a per-user push rule has to match the finalized preview marker. See [Matrix push rules for quiet previews](/en/channels/matrix-push-rules) for the full recipe (recipient token, pusher check, rule install, per-homeserver notes).
...
- Accepted configured-bot messages use shared [bot loop protection](/en/channels/bot-loop-protection). Configure `channels.defaults.botLoopProtection`, then override with `channels.matrix.botLoopProtection` or `channels.matrix.groups.<room>.botLoopProtection` when one room needs a different budget.
...
- `accessToken`: access token for token-based auth. Plaintext and SecretRef values supported across env/file/exec providers ([Secrets Management](/en/gateway/secrets)).
