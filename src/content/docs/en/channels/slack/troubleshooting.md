...
```
  ...
  <Accordion title="Agent View DMs share one session">
    ...
    See [Agent View DMs](/en/channels/slack/threads-and-sessions#agent-view-dms).

  </Accordion>

  <Accordion title="Session status could not return to active">
    This normal-level diagnostic means Slack accepted `processing` but rejected the terminal `active` update. Enable [verbose logging](/en/logging) to inspect the Slack API failure. The diagnostic does not change the existing retry or cleanup behavior.
