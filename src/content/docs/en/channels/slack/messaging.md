...
Native commands require [additional manifest settings](/en/channels/slack/manifest-and-scopes#additional-manifest-settings) in your Slack app and are enabled with `channels.slack.commands.native: true` or `commands.native: true` in global configurations instead.
