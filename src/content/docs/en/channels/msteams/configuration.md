...
Key settings (see [/gateway/configuration](/en/gateway/configuration) for shared channel patterns):
...
- `channels.msteams.graphMediaFallback`: opt into Graph message lookups when channel/group HTML omits file markers (default `false`; see [Channel/group file recovery](/en/channels/msteams/manifest-and-permissions#channel%2Fgroup-file-recovery-graphmediafallback)).
...
- `channels.msteams.replyStyle`: `thread | top-level` (see [Reply style](/en/channels/msteams/messaging#reply-style-threads-vs-posts)).
...
- `channels.msteams.sharePointSiteId`: SharePoint site ID for file uploads in group chats/channels (see [Sending files in group chats](/en/channels/msteams/messaging#sending-files-in-group-chats)).
