...
Matrix migration runs when you run [`openclaw doctor --fix`](/en/gateway/doctor), and as a fallback when the Matrix client starts and still finds file-based sidecar state next to its SQLite store.
