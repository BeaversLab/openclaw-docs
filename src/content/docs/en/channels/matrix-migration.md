...
Matrix migration runs when you run [`openclaw doctor --fix`](/en/gateway/doctor). File-based sidecars next to the dedicated Matrix store retain their client-start fallback, but credential-file import is Doctor-only; runtime reads only canonical SQLite credential state.
...
- [Matrix](/en/channels/matrix): channel setup and config.
- [Matrix push rules](/en/channels/matrix-push-rules): notification routing.
- [Doctor](/en/gateway/doctor): health check and automatic migration trigger.
- [Migration guide](/en/install/migrating): all migration paths (machine moves, cross-system imports).
