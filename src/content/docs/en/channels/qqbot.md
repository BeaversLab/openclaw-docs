...
5. Check `openclaw channels status --probe`; start the Gateway if it is offline. Config changes follow [hot reload](/en/gateway/configuration/hot-reload).
...
Use a [SecretRef](/en/gateway/secrets/secretref-contract) to keep the AppSecret out
...
- `contextVisibility` controls quoted-message text that QQ supplies as
  ...
  while filtering other supplemental context. See [Groups](/en/channels/groups#context-visibility-and-allowlists).
...
[Secrets runtime model](/en/gateway/secrets/runtime-model).
...
| `toolsBySender`       | —                | Per-sender tool overrides; see [Groups](/en/channels/groups#group%2Fchannel-tool-restrictions-optional). |
