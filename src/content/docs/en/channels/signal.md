...
`@openclaw/signal` installs from npm first, then falls back to its declared ClawHub package only when the npm target is unavailable. Use `npm:` or `clawhub:` to force a source. `plugins install` registers and enables the plugin; no separate `enable` step is needed. See [Plugins](/en/tools/plugin) for general install rules.
...
Multi-account support: use `channels.signal.accounts` with per-account config and optional `name`. Each named account owns its `transport`; it does not inherit the top-level transport. The top-level transport belongs only to the implicit `default` account. See [Multi-account channels](/en/gateway/config-channels#multi-account-all-channels) for the shared pattern.
...
4. Configure OpenClaw and verify the channel. Config changes follow [hot reload](/en/gateway/configuration/hot-reload); start the Gateway if it is offline. Restart it if you changed the service's `PATH` to find `signal-cli`.
...
- `channels.signal.streaming.block.enabled`, `channels.signal.streaming.block.coalesce`: block-mode streaming controls shared across channels. See [Streaming](/en/concepts/streaming).
...
- [RPC adapters](/en/reference/rpc) - the signal-cli JSON-RPC-over-HTTP daemon pattern behind this channel
