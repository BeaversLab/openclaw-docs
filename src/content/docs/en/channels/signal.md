...
Multi-account support: use `channels.signal.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/en/gateway/config-channels#multi-account-all-channels) for the shared pattern.
