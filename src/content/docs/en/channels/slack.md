...
`plugins install` registers and enables the plugin. It does nothing until you configure the Slack app and channel settings below. See [Plugins](/en/tools/plugin) for general plugin install rules.
...
<AccordionGroup>
  <Accordion title="Optional native slash commands">
    ...
    Replace your existing `features.slash_commands` section with a subset of [available commands](/en/tools/slash-commands#command-list):
...
<Tabs>
  ...
  <Tab title="Mentions and channel users">
    ...
    Accepted bot-authored Slack messages use shared [bot loop protection](/en/channels/bot-loop-protection). Configure `channels.defaults.botLoopProtection` for the default budget, then override with `channels.slack.botLoopProtection` or `channels.slack.channels.<id>.botLoopProtection` when a workspace or channel needs a different limit.
...
- With default `session.dmScope=main`, ordinary Slack DMs collapse to the agent main session. Agent View roots and existing Assistant View threads remain isolated as `:thread:<threadTs>` sessions; see [Agent View DMs](/en/channels/slack#agent-view-dms).
...
Same-chat `/approve` also works in Slack channels and DMs that already support commands. See [Exec approvals](/en/tools/exec-approvals) for the full approval forwarding model.
...
```
  ...
  <Accordion title="Agent View DMs share one session">
    ...
    See [Agent View DMs](/en/channels/slack#agent-view-dms).
...
- [Media understanding pipeline](/en/nodes/media-understanding)
