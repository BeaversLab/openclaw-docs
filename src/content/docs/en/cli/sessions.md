...
and use the standard [CLI JSON failure envelope](/en/cli#json-failures) when `--json`
is set.
...
the cleanup command to that Gateway. See [Memory forget](/en/cli/memory#memory-forget)
...
([Configuration reference](/en/gateway/config-agents/sessions#session)):
...
- Scope note: `openclaw sessions cleanup` maintains session stores,
  ...
  history class as an additional ceiling ([Task maintenance](/en/automation/tasks#automatic-maintenance),
  [Cron configuration](/en/automation/cron-jobs#configuration)).
...
**Run now** action in
[Settings → Agent Defaults → Session](/en/gateway/config-agents/sessions#cold-storage).
...
[store maintenance and retention](/en/reference/session-management-compaction/maintenance)
...
- [Session config](/en/gateway/config-agents/sessions#session)
...
- [Cloud Workers](/en/gateway/cloud-workers) — sessions hosted on remote workers
