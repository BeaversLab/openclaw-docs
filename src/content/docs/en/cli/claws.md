...
`package.json` requirement. Use [`agents add --role`](/en/cli/agents#role-templates)
or `openclaw claws add docs/reference/templates/roles/<role>` through the
[preview and consent flow](/en/cli/claws#inspect-and-preview).
[`agents team create`](/en/cli/agents#agents-team-create) owns delegation wiring;
...
- [MCP configuration](/en/gateway/config-extensions#mcp)
