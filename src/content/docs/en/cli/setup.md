...
terminal hatch as `openclaw onboard --tui`. See [Onboard](/en/cli/onboard) and
[CLI automation](/en/start/wizard-cli-automation) for the full flag reference and
...
- If Hermes state is detected, interactive onboarding can offer migration automatically. Import onboarding requires a fresh setup; use [Migrate](/en/cli/migrate) for dry-run plans, backups, and overwrite mode outside onboarding.
