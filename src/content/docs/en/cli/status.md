...
[Queue warnings](/en/gateway/health#queue-warnings).
...
[Prometheus RPC timings](/en/gateway/prometheus) exclude CLI startup and connection
...
[Release channels](/en/install/development-channels#checking-current-status).
...
- Session status output separates `Execution:` from `Runtime:`. `Execution`
  ...
  [Agent runtimes](/en/concepts/agent-runtimes) for the provider/model/runtime
  distinction.
- When the current session snapshot is sparse, the `/status` chat command (see
  [Slash commands](/en/tools/slash-commands)) can backfill token and cache counters
...
- Update info surfaces in the Overview; if an update is available, status
  prints a hint to run `openclaw update` (see [Updating](/en/install/updating)).
...
- [`openclaw health`](/en/cli/health) — Gateway health snapshot over RPC
