...
Inspect and configure [internal hooks](/en/automation/hooks): handlers for command,
...
[Webhooks](/en/automation/cron-jobs#webhooks) or the typed `api.on(...)` hook catalog in
...
[reload mode](/en/gateway/configuration#reload-modes). `off` requires a manual
...
[Local, remote, and agent scope](/en/automation/hooks#local-remote-and-agent-scope)
...
[source policy](/en/automation/hooks/configuration#hook-discovery).
...
[verify its real side effect](/en/automation/hooks#quick-start).
</Note>
...
selection. See [Configuration](/en/automation/hooks/configuration#configuration).
...
owning plugin through [`openclaw plugins`](/en/cli/plugins).
...
[verify the handler's actual side effect](/en/automation/hooks#quick-start).
</Warning>
...
are in [Bundled hooks](/en/automation/hooks/bundled-hooks). This includes
...
use the standard [CLI JSON failure envelope](/en/cli#json-failures), and missing
