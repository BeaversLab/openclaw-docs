...
Related: [Agent send tool](/en/tools/agent-send)
...
When your tool policy enables `browser`, local browser control works without a Gateway. Explicit Gateway or node routing and sandbox restrictions still apply; see [Node browser proxy](/en/tools/browser/remote#node-browser-proxy-zero-config-default).
...
- `codeModeEngaged`: `true` only when [code mode](/en/tools/code-mode) actually owned the model tool surface for the run. `tools.codeMode.enabled=true` alone does not guarantee engagement, and harnesses that own their native tool surface always read `false` because OpenClaw code mode never owns their tools.
...
Failures keep the [CLI error envelope](/en/cli#json-failures): `ok: false` and
