...
Explicit `openclaw doctor --lint` is the deployment-preflight posture. Add `--json` for machine-readable output without changing lint's threshold-based exit code. Policy findings reported here are documented in [`openclaw policy`](/en/cli/policy).
...
plugin owner could not be inspected; see [Plugin repair warnings](/en/install/update-troubleshooting#plugin-repair-warnings).
