...
environment. Separately, [eligible installation drift](/en/cli/doctor/recovery#gateway-service-recovery)
...
| `--lint`                        | Run the [structured health checks](/en/cli/doctor/health-contract) in read-only mode and emit diagnostic findings.                                                                                                                                       |
...
| `--session-sqlite-store <path>` | With `--session-sqlite`: select a SQLite database or legacy `sessions.json` source, subject to the mode's [selection rules](/en/cli/doctor/sqlite-maintenance#session-sqlite-migration).                                                                 |
