...
`0`. For Nix installs, edit the Nix source instead; see [Nix](/en/install/nix).
...
`openclaw config schema`. With `--json`, both use the standard [CLI JSON failure envelope](/en/cli#json-failures)
...
<Warning>
SecretRef assignments are rejected on unsupported runtime-mutable surfaces (for example `hooks.token`, Discord thread-binding webhook tokens, and WhatsApp creds JSON). See [SecretRef Credential Surface](/en/reference/secretref-credential-surface).
</Warning>
...
See [Config hot reload](/en/gateway/configuration/hot-reload).
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. At startup, eligible single-file configs can receive deterministic legacy-key migrations if the complete result validates, with the previous config kept in the `.bak` ring. Other invalid direct edits fail startup; hot reload skips invalid edits without rewriting `openclaw.json`. Run `openclaw doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/en/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- [`openclaw configure`](/en/cli/configure) — guided editor for the same settings
