...
Use [`openclaw acp`](/en/cli/acp) instead when OpenClaw should host the coding runtime itself and keep the agent session inside OpenClaw.
...
For broader testing context, see [Testing](/en/help/testing).
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - a Gateway admits at most 256 OpenClaw-managed runtimes with server connections across sessions and requester partitions; sessions without available servers and sign-in-only catalogs do not consume this limit. Reaching the limit rejects new admissions until you stop or reset unused sessions. See [MCP configuration](/en/gateway/config-extensions#mcp) for details
...
See [MCP tool grants](/en/tools/exec-approvals#mcp-tool-grants) for the document
shape and export/edit workflow.

For approval delivery through Slack buttons, see
[Native approvals in Slack](/en/channels/slack/rich-messages#native-approvals-in-slack).
...
For a shorter setup walkthrough covering Settings, the composer path (**+** → **Connectors** → **Add MCP server…**) and its **This session** / **Everywhere** scopes, CLI, and direct config, see [Connect MCP servers](/en/tools/mcp).
