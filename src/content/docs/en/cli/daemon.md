...
Legacy alias for Gateway service management. `openclaw daemon ...` maps to the same service-control commands as `openclaw gateway ...`. Prefer [`openclaw gateway`](/en/cli/gateway) for current docs and examples.
...
- Running multiple Gateways on one host: isolate ports, config/state, and workspaces. See [Multiple gateways](/en/gateway#multiple-gateways-same-host).
