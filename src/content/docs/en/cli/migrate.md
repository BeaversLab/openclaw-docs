...
Import state from another agent system through a plugin-owned migration provider. Bundled providers cover Claude, Codex CLI, and [Hermes](/en/install/migrating-hermes). Plugins can register additional providers.

<Tip>
For user-facing walkthroughs, see [Migrating from Claude](/en/install/migrating-claude) and [Migrating from Hermes](/en/install/migrating-hermes). The [migration hub](/en/install/migrating) lists all paths.
</Tip>
...
<ParamField path="--include-secrets" type="boolean">
  Import supported credentials without an OpenClaw confirmation prompt. Codex may still request operating-system credential-store access, such as macOS Keychain access. Interactive apply asks before inspecting and importing auth credentials, with yes selected by default. Non-interactive `--yes` requires `--include-secrets` to import them.
</ParamField>
...
<Tip>
For a user-facing walkthrough, see [Migrating from Claude](/en/install/migrating-claude).
</Tip>
...
- [Migrating from Hermes](/en/install/migrating-hermes): user-facing walkthrough.
- [Migrating from Claude](/en/install/migrating-claude): user-facing walkthrough.
- [Migrating](/en/install/migrating): move OpenClaw to a new machine.
- [Doctor](/en/gateway/doctor): health check after applying a migration.
- [Plugins](/en/tools/plugin): plugin install and registration.
