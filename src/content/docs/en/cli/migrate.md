...
Import state from another agent system through a plugin-owned migration provider. Bundled providers cover Codex CLI state, [Claude](/en/install/migrating-claude), and [Hermes](/en/install/migrating-hermes); third-party plugins can register additional providers.
