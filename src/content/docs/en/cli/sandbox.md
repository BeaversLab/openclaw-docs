...
[`openclaw agent exec`](/en/cli/agent#agent-exec) does not use these configured runtimes. Its isolated implicit policy config turns the agent sandbox off, allows full Gateway-host execution, and restricts filesystem tools to `--cwd`.
...
- [Doctor](/en/gateway/doctor): checks sandbox setup.
- [OpenShell](/en/gateway/openshell) — a managed sandbox backend driven through the `openshell` CLI
