...
An agent whose database belongs to another agent appears as **degraded**, with the refusal reason and repair guidance. The Gateway can continue serving healthy agents when a secondary agent is refused. Follow [Doctor's database recovery guidance](/en/gateway/doctor/state-and-sessions), then restart the Gateway after repairing the files.
...
Bundled roles are [Claw sources](/en/cli/claws): each role directory contains a
...
checkout. Follow the [Claw preview and consent flow](/en/cli/claws#inspect-and-preview)
...
change global delegation defaults or tool policy. See [Team preset](/en/concepts/multi-agent#team-preset).
...
If you also want different visible skills per agent, configure `agents.defaults.skills` and `agents.entries.*.skills` in `openclaw.json`. See [Skills config](/en/tools/skills-config) and [Configuration reference](/en/gateway/config-agents/workspace-and-bootstrap#agents-defaults-skills).
...
- [Skills config](/en/tools/skills-config): skill visibility configuration.
