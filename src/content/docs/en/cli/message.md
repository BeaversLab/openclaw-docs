...
[System Agent](/en/gateway/config-agents/heartbeat-compaction-and-streaming#agents.defaults.systemagent)
...
| `react`         | Discord, Matrix, Nextcloud Talk, Signal, Slack, Telegram, WhatsApp                                              | `--message-id`, `--target`                                     | `--emoji`, `--remove` (needs `--emoji`; omit it to clear own reactions where supported, see [Reactions](/en/tools/reactions)). WhatsApp: `--participant`, `--from-me`. Signal group reactions require `--target-author` or `--target-author-uuid`. Nextcloud Talk only adds reactions; `--remove` errors. |
...
- `--media <path-or-url>`: attach image/audio/video/document (local path or
  ...
  photos into [albums](/en/channels/telegram/media#photo-albums).
- `--presentation <json>`: shared payload with `text`, `context`, `divider`,
  `chart`, `table`, `buttons`, and `select` blocks, rendered per channel
  capability. See [Message Presentation](/en/plugins/message-presentation).
...
- [Message Presentation](/en/plugins/message-presentation)
