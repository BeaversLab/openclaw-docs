...
Channel switching, update validation, the restart handoff, and the Git checkout flow. Part of the [`openclaw update`](/en/cli/update) reference.
...
[Status and history](/en/cli/update/status-and-history) to inspect recorded warnings.
...
still need the [manual upgrade recovery path](/en/install/updating#alternative-re-run-the-installer).
...
installation's [manual update method](/en/install/updating/update-methods), then run
...
See [Unattended repair](/en/install/updating#unattended-repair-on-your-own-inference).
...
[independent verified backup](/en/install/updating#before-updating-create-a-verified-backup).
...
post-failure [Triage](/en/cli/triage) starts only after update ownership and service
...
see [Rollback](/en/install/updating#rollback).
...
code runs. Use your installation's [manual update method](/en/install/updating/update-methods),
