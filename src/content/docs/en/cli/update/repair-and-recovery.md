...
What happens when an update fails, and the subcommands that finish the job. Part of the [`openclaw update`](/en/cli/update) reference.
...
Choosing **Diagnose update failure** opens [Triage](/en/cli/triage), which starts the
...
failures can start [post-failure triage](/en/install/updating#unattended-repair-on-your-own-inference)
...
failed verification can [restore the previous package](/en/cli/update/how-updates-run#validation-and-activation)
...
and any recovery journals or backups. Follow [backup guidance](/en/install/backups)
