...
Manage sandbox runtimes for isolated agent execution. See [/cli/sandbox](/en/cli/sandbox).
...
Look up self, peer, and group IDs for channels that expose a directory surface. See [`openclaw directory`](/en/cli/directory).
...
Generate a mobile pairing QR and setup code from the current Gateway config. See [`openclaw qr`](/en/cli/qr).
...
Legacy alias namespace. Currently supports `openclaw clawbot qr`, which maps to [`openclaw qr`](/en/cli/qr).
...
Gmail Pub/Sub hook setup + runner. See [Gmail Pub/Sub](/en/automation/cron-jobs#gmail-pubsub-integration).
...
See [`acp`](/en/cli/acp) for full behavior, security notes, and examples.
...
List and manage [background task](/en/automation/tasks) runs across agents.
...
Legacy alias for the Gateway service-management commands. See [/cli/daemon](/en/cli/daemon).
...
Plugin-provided voice-call utilities. Only appears when the voice-call plugin is installed and enabled. See [`openclaw voicecall`](/en/cli/voicecall).
