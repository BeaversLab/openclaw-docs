...
- Emits `security.trust_model.cross_agent_session_access_default` for cross-agent session reach that no config narrows.
  ...
  - Fix: the detail lists each agent's reach and allowed session tools. Narrow [session visibility or agent-to-agent access](/en/gateway/config-tools#tools-agenttoagent) for persona separation.
