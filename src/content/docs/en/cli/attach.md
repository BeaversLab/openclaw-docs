...
[Devices](/en/cli/devices). Session URLs must stay credential-free: userinfo and
sensitive query or fragment parameters such as `token` and `password` are
rejected.

Target resolution uses the same [session target error matrix](/en/cli/tui#session-target-errors)
as `openclaw tui`.
...
[Ask user](/en/tools/ask-user).

See also: [Control UI URLs](/en/web/urls), [Devices](/en/cli/devices), [Gateway CLI](/en/cli/gateway), [MCP CLI](/en/cli/mcp), and [ACP CLI](/en/cli/acp).
