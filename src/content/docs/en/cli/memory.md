...
Related: [Memory](/en/concepts/memory) concept, [Dreaming](/en/concepts/dreaming),
[Memory config reference](/en/reference/memory-config), [Memory Wiki](/en/plugins/memory-wiki),
[wiki](/en/cli/wiki), [Plugins](/en/tools/plugin).
...
[CLI JSON failure envelope](/en/cli#json-failures) and exit with code 1. The failure
...
[Dreaming](/en/concepts/dreaming) for scheduling details.
...
to rebuild, or [`memory reset`](/en/cli/memory#memory-reset) to clear the derived index and
embedding cache; see
[Safe index recovery](/en/concepts/memory-builtin#safe-index-recovery).
</Warning>
...
it. To reclaim disk space, follow [disk-space recovery](/en/concepts/memory-builtin#reclaim-disk-space)
before rebuilding. It is not a privacy purge: use [`memory forget`](/en/cli/memory#memory-forget) to remove
...
[Memory provenance and deletion](/en/concepts/memory-provenance) for the
...
[session deletion](/en/cli/sessions#delete-sessions) ordinarily retains a
...
[`memory forget`](/en/cli/memory#memory-forget) remain durably excluded, including
...
Full key list and phase details: [Dreaming](/en/concepts/dreaming),
[Memory config reference](/en/reference/memory-config#dreaming).
