...
If `memory status` shows `Dreaming status: blocked`, the managed dreaming cron is enabled but the heartbeat that drives it is not firing for the default agent. See [Dreaming never runs](/en/concepts/dreaming#dreaming-never-runs-status-shows-blocked) for the two common causes.
...
- See [Dreaming](/en/concepts/dreaming) for full phase descriptions and configuration reference.
