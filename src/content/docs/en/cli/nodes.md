...
These commands drive the gateway-owned `node.pair.*` store, separate from device pairing (`openclaw devices approve`) that gates the node's WS `connect` handshake. See [Nodes](/en/nodes) for how the two relate.
...
Camera and Canvas commands have their own docs: [Camera nodes](/en/nodes/camera), [Canvas](/en/platforms/mac/canvas). Canvas is implemented by the bundled experimental Canvas plugin; core keeps `openclaw nodes canvas` as a compatibility mount point.
