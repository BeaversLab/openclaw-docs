...
Approve or inspect DM pairing requests for channels that support pairing (chat DMs only - node/device pairing uses [`openclaw devices`](/en/cli/devices)).

Related: [Pairing flow](/en/channels/pairing)
...
If you approved a sender before the first-owner bootstrap shipped in 2026.4.29, run [`openclaw doctor`](/en/cli/doctor). It warns when no command owner is configured. It also shows the exact `openclaw config set commands.ownerAllowFrom ...` command to fix it.

## Related

- [CLI reference](/en/cli)
- [Channel pairing](/en/channels/pairing)
- [`openclaw qr`](/en/cli/qr) — mobile/device bootstrap QR and setup code, not a channel DM pairing code
