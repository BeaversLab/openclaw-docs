...
Related: [Browser tool](/en/tools/browser)
...
Detailed guidance: [Browser troubleshooting](/en/tools/browser/troubleshooting#cdp-startup-failure-vs-navigation-ssrf-block)
...
Related: [Browser tool](/en/tools/browser/setup#missing-browser-command-or-tool)
...
Setup, security model, and recovery steps: [Chrome extension](/en/tools/chrome-extension).
...
`openclaw browser batch` sends a `kind="batch"` `/act` request with nested `BrowserActRequest` actions (`wait`, `click`, `type`, `evaluate`, ...) — not `open`/`navigate`/`snapshot`/`screenshot`, which are CLI subcommands, not `/act` kinds. `--continue` sets `stopOnError=false` (default stops on first error). `--target-id` scopes the whole batch to one tab. A failed nested action makes the command exit nonzero. Use `--json` to retain the ordered `results` response. See [Browser batch CLI](/en/tools/browser-control#browser-batch-cli) for the full contract (ref lifecycle, target id conflicts, error summary). `batch` is not supported on `profile="user"` / existing-session profiles.
