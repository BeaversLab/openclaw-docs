...
Related: [Secrets Management](/en/gateway/secrets) · [1Password plugin](/en/plugins/onepassword) · [SecretRef Credential Surface](/en/reference/secretref-credential-surface) · [Security](/en/gateway/security)
...
Entries also arrive from **Settings -> Secrets** in the Control UI, and from the agent's [`secrets` tool](/en/tools/secrets). That tool asks you to type a credential into a masked prompt. It stores the credential without the value reaching the model.
...
re-pair devices. See [Gateway token recovery](/en/cli/doctor/recovery#invalid-gateway-tokens).
...
- Targets secret-bearing fields in `openclaw.json` plus the selected agent's auth profile store. The canonical supported surface is [SecretRef Credential Surface](/en/reference/secretref-credential-surface).
...
Plan contract details (allowed target paths, validation rules, failure semantics): [Secrets Apply Plan Contract](/en/gateway/secrets-plan-contract).
...
- [Vault SecretRefs](/en/plugins/vault)
- [1Password plugin](/en/plugins/onepassword)
