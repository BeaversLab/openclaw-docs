...
<CardGroup cols={2}>
  <Card title="CLI onboarding hub" href="/en/start/wizard" icon="rocket">
    Walkthrough of the interactive CLI flow.
  </Card>
  <Card title="Onboarding overview" href="/en/start/onboarding-overview" icon="map">
    How OpenClaw onboarding fits together.
  </Card>
  <Card title="CLI setup reference" href="/en/start/wizard-cli-reference" icon="book">
    Outputs, internals, and per-step behavior.
  </Card>
  <Card title="CLI automation" href="/en/start/wizard-cli-automation" icon="terminal">
    Non-interactive flags and scripted setups.
  </Card>
  <Card title="macOS app onboarding" href="/en/start/onboarding" icon="apple">
...
later from the Control UI [Memory import page](/en/web/control-ui), which offers
the same memory-only scope. (A full [`openclaw migrate`](/en/cli/migrate) run is
...
be disabled with [`wizard.appRecommendations`](/en/gateway/config-runtime#wizard).
...
[Capability consent](/en/plugins/manage-plugins#capability-consent) and the
[automation guide](/en/start/wizard-cli-automation#review-required-plugins).
...
`--auth-choice llama-cpp` selects the managed local server instead. `--llama-server-api-key` is optional; if omitted, onboarding checks `LLAMA_SERVER_API_KEY` in env. See [llama.cpp](/en/plugins/llama-cpp) for endpoint-replacement and auth-profile behavior.
...
- [`openclaw setup`](/en/cli/setup) — the system-agent entry point; bare `setup` is interactive, and falls through to guided onboarding on a fresh system
