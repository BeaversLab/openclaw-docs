...
IDs. For a [Session Share](/en/plugins/session-share) node:
...
Use [`openclaw node status`](/en/cli/node#service-background) to inspect the
installed service.
...
- To revoke a normal enrolled machine, remove its paired device with
  [`openclaw devices remove <deviceId>`](/en/cli/devices#openclaw-devices-remove-%3Cdeviceid%3E).
...
See [Node](/en/cli/node) for service management, explicit connection flags, node
