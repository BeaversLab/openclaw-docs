...
The database and migration sources respect `$OPENCLAW_STATE_DIR`. Full reference: [/gateway/config-secrets-env#auth-storage](/en/gateway/config-secrets-env#auth-storage)
...
[personal-account CLI setup](/en/cli/models#personal-model-accounts).
...
[Per-person model accounts](/en/concepts/multi-user#per-person-model-accounts).
...
- [Model failover](/en/concepts/model-failover) (rotation + cooldown rules)
- [Slash commands](/en/tools/slash-commands) (command surface)
...
- [Configuration Reference](/en/gateway/config-secrets-env#auth-storage) - auth config keys
