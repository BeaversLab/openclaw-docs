...
- [Honcho memory](/en/concepts/memory-honcho) - a memory plugin a context engine can draw on
- [Plugin Architecture](/en/plugins/architecture) - registering context engine plugins
- [Plugin manifest](/en/plugins/manifest) - plugin manifest fields
- [Plugins](/en/tools/plugin) - plugin overview
- [Session management deep dive](/en/reference/session-management-compaction) - the session store, transcript events, and auto-compaction internals
- [System prompt](/en/concepts/system-prompt) - what OpenClaw assembles into the system prompt for every agent run, and the layers it renders from
