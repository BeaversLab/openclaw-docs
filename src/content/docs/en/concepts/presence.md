...
[Active computer presence](/en/nodes/presence).
...
- `ip`: best-effort IP address; the [geolocation plugin](/en/plugins/geolocation) resolves it to a coarse city where one is available
...
node event described in [Active computer presence](/en/nodes/presence) owns it. The
...
from a session's live viewers. The [people card](/en/concepts/multi-user#people-cards) keeps online duration
...
<CardGroup cols={2}>
  <Card title="Active computer presence" href="/en/nodes/presence" icon="computer-mouse">
    How physical Mac input selects an active node and routes connection alerts.
  </Card>
  <Card title="Typing indicators" href="/en/concepts/typing-indicators" icon="ellipsis">
    When typing indicators are sent and how to tune them.
  </Card>
  <Card title="Streaming and chunking" href="/en/concepts/streaming" icon="bars-staggered">
    Outbound streaming, chunking, and per-channel formatting.
  </Card>
  <Card title="Gateway architecture" href="/en/concepts/architecture" icon="diagram-project">
    Gateway components and the WebSocket protocol that drives presence updates.
  </Card>
  <Card title="Gateway protocol" href="/en/gateway/protocol" icon="plug">
