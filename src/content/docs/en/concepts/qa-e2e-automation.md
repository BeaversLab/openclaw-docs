...
- `qa/`: repo-backed seed assets for the kickoff task and baseline QA
  scenarios.
- [Mantis](/en/concepts/mantis): before and after live verification for bugs that
...
| `qa matrix`                                         | Live transport lane against a disposable Tuwunel homeserver. See [Matrix QA](/en/concepts/qa-matrix).                                                                                                                                                                      |
...
| `qa mantis`                                         | Before and after verification runner for live transport bugs, with Discord status-reactions evidence, Crabbox desktop/browser smoke, and Slack-in-VNC smoke. See [Mantis](/en/concepts/mantis) and [Mantis Slack Desktop Runbook](/en/concepts/mantis-slack-desktop-runbook). |
...
handling steps live in [Mantis Slack Desktop Runbook](/en/concepts/mantis-slack-desktop-runbook).
...
Matrix has a [dedicated page](/en/concepts/qa-matrix) because of its scenario count and Docker-backed homeserver provisioning. Telegram, Discord, Slack, and WhatsApp run against pre-existing real transports, so their reference lives here.
...
Operational env vars and the Convex broker endpoint contract live in [Testing → Shared Telegram credentials via Convex](/en/help/testing#shared-telegram-credentials-via-convex-v1) (the section name predates the multi-channel pool; the lease semantics are shared across kinds).
...
For scorecard context, see [Maturity scorecard](/en/maturity/scorecard).
...
- [Matrix QA](/en/concepts/qa-matrix)
