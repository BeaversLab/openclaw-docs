...
- **Group activity.** Under `session.groupScope: "per-group"` (the default),
  ...
  (the default), the main session can use [session tools](/en/concepts/session-tool) across the Gateway,
...
- **Memory recall across conversations** lets the agent recall content from
  ...
  opt in explicitly. See [Memory configuration](/en/reference/memory-config).
...
- By default there is no automatic reset; compaction keeps the active context
  ...
  opt-in (see [Session management](/en/concepts/session)). On `/new` and `/reset`,
...
- When the per-agent store's physical database, WAL, and session artifacts
  ...
  see [Session maintenance](/en/reference/session-management-compaction/maintenance)),
...
covered in [Session management](/en/concepts/session) and [Channel routing](/en/channels/channel-routing).
...
the originating room. See [Session management](/en/concepts/session#group-and-room-routing).

## Related

- [Session management](/en/concepts/session) — routing, scopes, resets
- [Channel routing](/en/channels/channel-routing) — how agents and sessions are selected
- [Memory](/en/concepts/memory) — durable memory layers
- [Multi-agent](/en/concepts/multi-agent) — running several isolated agents
- [Session management deep dive](/en/reference/session-management-compaction) — the disk budget, archival, and compaction keys
