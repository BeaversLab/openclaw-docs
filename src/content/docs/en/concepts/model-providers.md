...
<AccordionGroup>
  ...
  <Accordion title="OpenAI provider/runtime split">
    ...
    See [OpenAI](/en/providers/openai) and [Codex harness](/en/plugins/codex-harness). If the provider/runtime split is confusing, read [Agent runtimes](/en/concepts/agent-runtimes) first.
...
<Note>
Provider runtime `capabilities` is shared runner metadata (provider family, transcript/tooling quirks, transport/cache hints). It is not the same as the [public capability model](/en/plugins/architecture#public-capability-model), which describes what a plugin registers (text inference, speech, etc.).
</Note>
...
OpenClaw uses LM Studio's native `/api/v1/models` and `/api/v1/models/load` for discovery + auto-load, with `/v1/chat/completions` for inference by default. See [/providers/lmstudio](/en/providers/lmstudio) for setup and troubleshooting.
