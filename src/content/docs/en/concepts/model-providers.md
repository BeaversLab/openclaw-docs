...
- OpenAI-family routes are prefix-specific: `openai/<model>` uses the direct
  ...
  uses the native Codex app-server harness. See [OpenAI](/en/providers/openai)
  and [Codex harness](/en/plugins/codex-harness). If the provider/runtime split is
  confusing, read [Agent runtimes](/en/concepts/agent-runtimes) first.
...
- [Qwen Cloud](/en/providers/qwen): Qwen Cloud provider surface plus Alibaba DashScope and Coding Plan endpoint mapping
- [MiniMax](/en/providers/minimax): MiniMax Coding Plan OAuth or API key access
- [GLM models](/en/providers/glm): Z.AI Coding Plan or general API endpoints
...
See [/providers/lmstudio](/en/providers/lmstudio) for setup and troubleshooting.
...
`openclaw onboard` and the model picker. See [/providers/ollama](/en/providers/ollama)
