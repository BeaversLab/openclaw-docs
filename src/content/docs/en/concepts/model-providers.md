...
<AccordionGroup>
  <Accordion title="Model refs and CLI helpers">
    ...
    - Fallback rules, cooldown probes, and session-override persistence: [Model failover](/en/concepts/model-failover).
  ...
  <Accordion title="OpenAI provider/runtime split">
    ...
    See [OpenAI implicit agent runtime](/en/providers/openai#implicit-agent-runtime) and [Codex harness](/en/plugins/codex-harness). If the provider/runtime split is confusing, read [Agent runtimes](/en/concepts/agent-runtimes) first.
...
The full list of provider-SDK hooks and bundled-plugin examples lives in [Provider plugins](/en/plugins/sdk-provider-plugins). A provider that needs a totally custom request executor is a separate, deeper extension surface.
...
- Native Codex app-server harness docs: [Codex harness](/en/plugins/codex-harness)
- Astra (`openai/gpt-6-astra`) defaults to `low` reasoning effort to limit routine budget use. The [OpenAI provider default](/en/providers/openai#gpt-6-astra) is shared by model controls and both runtimes; explicit thinking settings take precedence.
...
- The shared `/fast` toggle, configured defaults, and valid model-scoped Fast params resolve through one runtime-control policy. See [Thinking levels](/en/tools/thinking#fast-mode-%2Ffast) for precedence.
...
<CardGroup cols={3}>
  ...
  <Card title="Qwen Cloud" href="/en/providers/qwen">
...
| [Ollama Cloud](/en/providers/ollama-cloud) | `ollama-cloud`                   | `OLLAMA_API_KEY`                                     | `ollama-cloud/kimi-k2.6`                               |
...
Bundled and catalog-known routes take their `compat` capabilities from the owning provider plugin. A config `compat` block is for a custom provider/model or a different `api`/`baseUrl` route whose endpoint contract you have verified; see the [custom-provider capability guide](/en/gateway/config-tools#custom-provider-capability-declarations). Doctor removes legacy values that merely repeat the catalog and leaves divergent values visible for operator review.
...
See [Moonshot AI (Kimi + Kimi Coding)](/en/providers/moonshot) for the full setup guide.
...
Both use `llama-cpp/<model>` references. See [llama.cpp](/en/plugins/llama-cpp) for setup,
...
Ollama is detected locally at `http://127.0.0.1:11434` when you opt in with `OLLAMA_API_KEY`, and the bundled provider plugin adds Ollama directly to `openclaw onboard` and the model picker. See [/providers/ollama](/en/providers/ollama) for onboarding, cloud/local mode, and custom configuration.
...
See [/providers/sglang](/en/providers/sglang) for details.
