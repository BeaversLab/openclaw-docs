...
This page covers queue-mode steering for normal inbound messages in `steer` mode. In `followup` or `collect` mode, normal messages skip this path and wait until the active run finishes. For the explicit `/steer <message>` command, see [Steer](/en/tools/steer).
...
[Personal `USER.md` context](/en/concepts/user-model#personal-user-files-on-a-shared-gateway)
...
- [Codex harness runtime](/en/plugins/codex-harness-runtime) - `turn/steer` behavior on the native Codex harness
