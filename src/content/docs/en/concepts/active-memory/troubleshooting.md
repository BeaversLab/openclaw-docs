...
<AccordionGroup>
  <Accordion title="Registered recall tools return `status=policy-disabled`">
    ...
    - Check the selected agent's profile and explicit `tools.alsoAllow` grants.
      ...
      alone does not authorize them. Use the scoped [Lossless Claw example](/en/concepts/active-memory/memory-tools#lossless-claw).
  ...
  <Accordion title="First recall after gateway restart returns `status=timeout`">
    ...
    See [Cold-start grace](/en/concepts/active-memory/recommended-setup#cold-start-grace) under Recommended setup for the
