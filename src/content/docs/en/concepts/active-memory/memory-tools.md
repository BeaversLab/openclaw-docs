...
the [Lossless Claw example](/en/concepts/active-memory/memory-tools#lossless-claw). These grants also give the parent
...
After [installing and configuring LanceDB](/en/plugins/memory-lancedb), Active
...
a context engine first; see [Context engine](/en/concepts/context-engine). Then
...
[Tool policy](/en/gateway/config-tools) before adapting an existing layered allowlist:
