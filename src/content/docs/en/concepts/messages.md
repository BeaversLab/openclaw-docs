...
See [Configuration](/en/gateway/configuration) for the full schema.
...
Details: [Command queue](/en/concepts/queue) and [Steering queue](/en/concepts/queue-steering).
...
- [Ambient room events](/en/channels/ambient-room-events) and internal helper turns can remain silent. In `message_tool` visible-reply mode, an optional turn stays silent by not calling `message(action=send)`.
...
Reply requirements come from admission, not model control tokens. Confirmed delivery or still-owned delivery prevents duplicate recovery; see [Reply shaping](/en/concepts/agent-loop#reply-shaping).

## Related

- [Channel inbound API](/en/plugins/sdk-channel-inbound) - receive orchestration and acknowledgment policy
- [Channel outbound API](/en/plugins/sdk-channel-outbound) - durable sends and delivery receipts
- [Streaming](/en/concepts/streaming) - real-time message delivery
- [Retry](/en/concepts/retry) - message delivery retry behavior
- [Queue](/en/concepts/queue) - message processing queue
- [Channels](/en/channels) - messaging platform integrations
