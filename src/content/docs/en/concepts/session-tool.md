...
[`gateway call --timeout`](/en/cli/gateway/query#gateway-call-method).
...
See [Session state awareness](/en/concepts/session-state) for the full model: event kinds, watcher registration, the anti-spam notice protocol, reconciliation flow, and current limits.

`sessions_yield` intentionally ends the current turn so the next message can be an announced child completion event. Use it for announcing sub-agents, not [Swarm collectors](/en/tools/swarm): collectors require explicit result collection through `agents_wait` or an awaited `agents.run()` in OpenClaw Code Mode, and send no completion notification.
...
- `visible: true` to create a persistent dashboard session instead of a hidden sub-agent session. Visible spawns support an explicit sidebar `group`, model, working directory, same-agent transcript fork, and an optional [managed worktree](/en/concepts/managed-worktrees); see [Sub-agents](/en/tools/subagents#tool-parameters) for the exact compatibility limits. The accepted result is a receipt: it includes the child session key, run id, a Control UI `sessionUrl` (omitted when the Control UI is disabled), and an `owner` record naming the requesting agent. When acknowledging the spawn in a channel, put the session URL on the first line and `Owner: <label>` on the second. The spawned session is attributed to the requesting agent in the sidebar; see [Multi-user mode](/en/concepts/multi-user#agent-spawned-sessions).
...
For ACP-specific behavior, see [ACP Agents](/en/tools/acp-agents).
...
- [Session Management](/en/concepts/session): routing, lifecycle, maintenance
- [Session pruning](/en/concepts/session-pruning)
- [Sub-agents](/en/tools/subagents): child-session lifecycle and delivery
- [ACP Agents](/en/tools/acp-agents): external harness spawning
- [Multi-agent](/en/concepts/multi-agent): multi-agent architecture
- [Goal](/en/tools/goal) — durable per-session objectives, read and updated through the dedicated `get_goal`, `create_goal`, and `update_goal` tools
- [Gateway Configuration](/en/gateway/configuration): session tool config knobs
