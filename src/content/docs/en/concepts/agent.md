...
harness process. Each configured agent (see [Multi-agent routing](/en/concepts/multi-agent)
...
`/queue interrupt` aborts the active run instead. See [Queue](/en/concepts/queue)
and [Steering queue](/en/concepts/queue-steering) for queue and boundary behavior.
...
- A sender allowlist for every channel you enable (strongly recommended) — see
  [Access groups](/en/channels/access-groups)
...
- [Group chats](/en/channels/group-messages)
