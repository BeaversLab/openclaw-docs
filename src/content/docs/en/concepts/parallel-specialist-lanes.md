...
through the [command queue](/en/concepts/queue). Specialist lanes add policy on
...
to return results without further delegation. See [Team preset](/en/concepts/multi-agent#team-preset).

### Phase 1: lane contracts + background heavy work

Give every lane a written contract in its [agent workspace](/en/concepts/agent-workspace)
...
[sub-agent delegation](/en/tools/subagents/tool-reference) for what each value does,
and [command queue](/en/concepts/queue) for `mode`, `cap`, and `drop`.
