...
target. See the [TUI CLI reference](/en/cli/tui#notes) for local-mode behavior.
...
See [Session management](/en/concepts/session) for routing, isolation, lifecycle,
and storage details.
...
guessing. See [Control UI URLs](/en/web/urls) for the complete literal encoding and
stability contract.
...
not clone the transcript or create a new session. See [TUI](/en/cli/tui) for target
...
until its TTL expires. See [Attach CLI](/en/cli/attach) for grant lifetime and
launch options.
...
See [Devices](/en/cli/devices), [Remote access](/en/gateway/remote), and
[Gateway security](/en/gateway/security) for approval, rotation, revocation, and
network guidance.
