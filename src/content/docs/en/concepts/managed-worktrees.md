...
See [Workspace access](/en/gateway/sandboxing/workspace-access#managed-project-workspaces)
...
See [Configuration reference](/en/gateway/config-runtime#worktreeroot) for the option's default and scope.
...
Transient fetch failures, including an incomplete object transfer, retry once after one second within the original fetch timeout. Cancellation and expired workspace authority stop recovery; a second failure surfaces the Git error. See [Retry policy](/en/concepts/retry#managed-git-operations).
...
The Place picker's **Projects** section can start the same worktree flow from a registered project ID. The Gateway resolves the recorded checkout path, so this path remains available at [`operator.write`](/en/gateway/operator-scopes); selecting an arbitrary host folder still requires `operator.admin`.
...
The bundled [Workboard plugin](/en/plugins/workboard) can materialize a card workspace as a managed worktree:
...
- [Workboard plugin](/en/plugins/workboard) — cards that materialize a managed worktree
- [Configuration reference](/en/gateway/config-runtime#worktreeroot) — where `worktreeRoot` is set
