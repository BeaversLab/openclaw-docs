...
<Info>
Before compacting, OpenClaw automatically reminds the agent to save important notes to [memory](/en/concepts/memory) files. This helps preserve durable context.
</Info>
...
Configure compaction under `agents.defaults.compaction` in your `openclaw.json`. The most common knobs are listed below; for the full reference, see [Session management deep dive](/en/reference/session-management-compaction).
...
The memory-flush model override is exact and does not inherit the active session fallback chain. See [Memory](/en/concepts/memory) for details and config.
...
A [worker placement](/en/gateway/cloud-workers) cannot transfer ownership to a different session identity during compaction. Custom engines must keep the current identity while the placement owns the session, or the operator must move the session back to the Gateway before retrying. A rejected transition leaves the original session and worker claim intact.
...
[Session pruning](/en/concepts/session-pruning) is a lighter-weight complement that trims tool output without summarizing.

## Troubleshooting

**Compacting too often?** The model's context window may be small, or tool outputs may be large. Try enabling [session pruning](/en/concepts/session-pruning).

**Context feels stale after compaction?** Use `/compact Focus on <topic>` to guide the summary, or enable the [memory flush](/en/concepts/memory) so notes survive.

**Need a clean slate?** `/new` starts a fresh session without compacting.

For advanced configuration (reserve tokens, identifier preservation, custom context engines, OpenAI server-side compaction), see the [Session management deep dive](/en/reference/session-management-compaction).

## Related

- [Agent loop](/en/concepts/agent-loop): the turn lifecycle that triggers auto-compaction.
...
- [Context engines](/en/concepts/context-engine): pluggable context assembly.
- [Hooks](/en/automation/hooks/event-types): internal compaction events (`session:compact:before`, `session:compact:after`).
- [Plugin hooks](/en/plugins/hooks/reference#hook-catalog): typed compaction hooks (`before_compaction`, `after_compaction`).
