...
This extends [Multi-Agent Routing](/en/concepts/multi-agent) from personal use into organizational deployments.
...
- Follows **[standing orders](/en/automation/standing-orders)**: rules in the agent's `AGENTS.md` that define what it may do autonomously vs. what needs human approval. [Cron Jobs](/en/automation/cron-jobs) drive scheduled execution.
...
Combines Tier 2 permissions with [Cron Jobs](/en/automation/cron-jobs) and [Standing Orders](/en/automation/standing-orders).
...
See [Sandboxing](/en/gateway/sandboxing) and [Multi-Agent Sandbox & Tools](/en/tools/multi-agent-sandbox-tools).
...
Route inbound messages to the delegate agent using [Multi-Agent Routing](/en/concepts/multi-agent) bindings:
...
The login writes the delegate's SQLite auth store at `~/.openclaw/agents/delegate/agent/openclaw-agent.sqlite`; its local profiles override the shared read-through base. Never share the main agent's `agentDir` with the delegate. See [Auth credential semantics](/en/auth-credential-semantics#agent-copy-portability) and [Multi-Agent Routing](/en/concepts/multi-agent) for auth isolation details.
...
The delegate's `AGENTS.md` defines its autonomous authority - what it may do without asking, what needs approval, and what is forbidden. [Cron Jobs](/en/automation/cron-jobs) drive its daily schedule.
...
4. **Define [standing orders](/en/automation/standing-orders)** for autonomous operations.
