...
[`memory forget`](/en/cli/memory#memory-forget).
...
[Chunk provenance](/en/concepts/memory-architecture#provenance-every-memory-knows-where-it-came-from)
...
When [dreaming](/en/concepts/dreaming) merges or supersedes tracked entries,
...
data. See [Memory config](/en/reference/memory-config#memory-admission-policy)
...
[command reference](/en/cli/memory#memory-forget) describes the counters and
selection limits.
...
- **Original transcripts and archives.** Memory cleanup leaves them in the
  session store. [Session deletion](/en/cli/sessions#delete-sessions) is a
...
- [`memory forget` command reference](/en/cli/memory#memory-forget)
- [Memory admission policy configuration](/en/reference/memory-config#memory-admission-policy)
- [Memory architecture](/en/concepts/memory-architecture)
- [Dreaming](/en/concepts/dreaming)
- [Built-in memory](/en/concepts/memory-builtin)
