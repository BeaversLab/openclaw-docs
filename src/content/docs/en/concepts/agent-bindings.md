...
Older configurations may still contain one `default: true` marker. [Doctor migration](/en/gateway/doctor/config-migrations) carries that ownership into explicit bindings and service targets while preserving existing explicit choices. The marker cannot be combined with `agents.ownership: "explicit"`.

Valid binding changes apply automatically under the default `hybrid` [reload mode](/en/gateway/configuration/hot-reload). If `gateway.reload.mode` is `off`, restart the Gateway to apply them. Then verify the roster and channel accounts:
...
Top-level `bindings` also accepts `type: "acp"` entries for persistent ACP conversations. Those require a concrete `match.peer.id` and follow the ACP conversation identity contract instead of ordinary route precedence; see [ACP agents](/en/tools/acp-agents) when that is what you need.
...
- [Agent configuration](/en/gateway/config-agents)
