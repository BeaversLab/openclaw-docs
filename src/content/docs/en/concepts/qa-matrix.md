...
For broader QA framework context, see [QA overview](/en/concepts/qa-e2e-automation).
...
Matrix is one of three live transport lanes (Matrix, Telegram, Discord) that share a single contract checklist defined in [QA overview → Live transport coverage](/en/concepts/qa-e2e-automation#live-transport-coverage). `qa-channel` remains the broad synthetic suite and is intentionally not part of that matrix.

## Related

- [QA overview](/en/concepts/qa-e2e-automation) — overall QA stack and live transport contract
- [QA Channel](/en/channels/qa-channel) — synthetic channel adapter for repo-backed scenarios
- [Testing](/en/help/testing) — running tests and adding QA coverage
- [Matrix](/en/channels/matrix) — the channel plugin under test
