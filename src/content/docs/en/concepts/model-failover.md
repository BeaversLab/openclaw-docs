...
policy described in [Codex harness runtime behavior](/en/plugins/codex-harness/runtime-behavior#automatic-daybreak-escalation).
...
More detail: [OAuth](/en/concepts/oauth)
...
The embedded runtime's existing session setting `retry.provider.maxRetries` overrides its recovery retry budget. `0` disables retries, and rate limits remain capped at 10 total attempts. It is not an `openclaw.json` key and does not change a native harness's internal request retries. Native harnesses may finish their own request retries before OpenClaw starts continuation recovery. The reply runner does not add another whole-turn replay loop. See [Retry policy](/en/concepts/retry) for pacing and exclusions.
...
<Tabs>
  ...
  <Tab title="Does not continue on">
    ...
    - final provider refusals. Eligible Anthropic direct API-key requests handle refusal fallback within the provider request instead (see [Anthropic](/en/providers/anthropic#safety-refusal-fallback-claude-opus-5-and-fable-5))
...
- [`openclaw models`](/en/cli/models) — inspect resolved defaults and fallbacks

See [Models](/en/concepts/models) for the broader model selection and fallback overview.

See [Model providers](/en/concepts/model-providers) for provider setup, credentials, and per-provider model catalogs.
