...
- Telegram selected quote replies are an exception: when `replyToMode` is not `"off"` and selected quote text is present, OpenClaw skips the answer preview stream for that turn so tool-progress preview lines cannot render. Current-message replies without selected quote text still keep preview streaming. See [Telegram channel docs](/en/channels/telegram) for details.
...
- [Message lifecycle refactor](/en/concepts/message-lifecycle-refactor) - target shared preview, edit, stream, and finalization design
