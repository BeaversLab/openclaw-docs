...
single-file config meets the [startup migration conditions](/en/gateway/doctor#detailed-behavior-and-rationale).
...
- Telegram selected quote replies are an exception: when `replyToMode` is not
  ...
  [Telegram channel docs](/en/channels/telegram) for details.
...
- When a parent yields to accepted subagents, `progress` mode can transfer its
  ...
  [Subagent yield handoff](/en/concepts/subagent-yield-handoff#progress-after-yield).
...
[Progress cards](/en/tools/progress-card) replace the complete plan state in
...
- **`streaming.progress.commentary`** - render the model's pre-tool
  **commentary** (a short "I'll check... then..." narration) interleaved with
  ...
  [Progress drafts](/en/concepts/progress-drafts#status-headline).
...
- [Agent loop](/en/concepts/agent-loop) - the turn lifecycle that emits these stream events
- [Channel outbound API](/en/plugins/sdk-channel-outbound) - shared preview, durable send, and finalization APIs
- [Progress drafts](/en/concepts/progress-drafts) - visible work-in-progress messages that update during long turns
- [Messages](/en/concepts/messages) - message lifecycle and delivery
- [Retry](/en/concepts/retry) - retry behavior on delivery failure
- [Typing indicators](/en/concepts/typing-indicators) - typing state shown while a turn is in flight
- [Channels](/en/channels) - per-channel streaming support
