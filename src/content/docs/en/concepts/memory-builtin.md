...
Consider [Honcho](/en/concepts/memory-honcho) if you want cross-session memory
...
[llama.cpp provider](/en/plugins/llama-cpp), then point `local.modelPath` at a
GGUF file:
...
[Memory provenance and deletion](/en/concepts/memory-provenance).
...
- **Index compatibility:** changing the embedding provider, model, settings,
  ...
  See [provider selection](/en/reference/memory-config#provider-selection).
...
is retained. Use the [memory index command](/en/cli/memory#memory-index) for
memory-only repair.
...
[database versioning and rollback contract](/en/reference/database-schemas/versioning)
...
[configuration reference](/en/reference/memory-config#additional-memory-paths).
</Info>
...
install the [llama.cpp provider](/en/plugins/llama-cpp) and set
...
[llama.cpp](/en/plugins/llama-cpp) setup once with `openclaw onboard`, confirm the
...
[`memory reset`](/en/cli/memory#memory-reset):
...
its sidecars, retain the verbose error, and [create and verify a backup](/en/cli/backup)
...
the [restore workflow](/en/install/backups#restore-a-full-archive).
...
[backup](/en/cli/backup), then stop the Gateway through its deployment owner and
...
- [Active memory](/en/concepts/active-memory)
