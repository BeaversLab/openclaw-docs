...
For a focused setup guide with account and conversation examples, see [Agent bindings](/en/concepts/agent-bindings).
...
[Memory Wiki per-agent vaults](/en/concepts/multi-agent#per-agent-memory-wiki-vaults)
when personas must not share compiled wiki knowledge.

<Note>
**Workspace note:** each agent's workspace is the **default cwd**, not a hard sandbox. Relative paths resolve inside the workspace, but absolute paths can reach other host locations unless sandboxing is enabled. See [Sandboxing](/en/gateway/sandboxing).
</Note>
...
The bundled roles are [Claw sources](/en/cli/claws), sharing the portable
...
the [Claw preview and consent flow](/en/cli/claws#inspect-and-preview).
...
Explicit targets and [routing bindings](/en/concepts/agent-bindings) take precedence.
...
[`agents team create`](/en/cli/agents#agents-team-create) for flags and examples,
or use the team choice during [onboarding](/en/start/wizard#choose-one-agent-or-a-team).
...
<Steps>
  ...
  <Step title="Create channel accounts">
    ...
    See channel guides: [Discord](/en/channels/discord), [Telegram](/en/channels/telegram), [WhatsApp](/en/channels/whatsapp).
...
[Memory Wiki per-agent vaults](/en/plugins/memory-wiki#per-agent-vaults) for bridge
...
upgrade path, see [Migrating from QMD](/en/concepts/memory-builtin#migrating-from-qmd).
...
DM access control (pairing/allowlist) is global per WhatsApp account, not per agent. For shared groups, bind the group to one agent or use [Broadcast groups](/en/channels/broadcast-groups).

## Routing rules

Bindings are deterministic and most-specific wins. See [Channel routing](/en/channels/channel-routing#routing-rules-how-an-agent-is-chosen) for the full tier order (exact peer, parent peer, peer wildcard, guild+roles, guild, team, account, channel, default agent). A few rules worth calling out here:
...
- Direct chats collapse to `agent:<agentId>:main` by default (the per-agent [main session](/en/concepts/main-session)).
...
See [Multi-agent sandbox and tools](/en/tools/multi-agent-sandbox-tools) for detailed examples.

## Related

- [ACP agents](/en/tools/acp-agents) — running external coding harnesses
- [Channel routing](/en/channels/channel-routing) — how messages route to agents
- [Parallel specialist lanes](/en/concepts/parallel-specialist-lanes) — splitting one job across role-scoped agents
- [Presence](/en/concepts/presence) — agent presence and availability
- [Session](/en/concepts/session) — session isolation and routing
- [Sub-agents](/en/tools/subagents) — spawning background agent runs
- [`openclaw agents`](/en/cli/agents) — create and inspect agents from the CLI
