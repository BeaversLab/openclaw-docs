...
<CardGroup cols={2}>
  ...
  <Card title="Agent runtimes" href="/en/concepts/agent-runtimes">
...
Model refs choose a provider and model. They do not usually choose the low-level agent runtime. For example, `openai/gpt-5.5` can run through the normal OpenAI provider path or through the Codex app-server runtime, depending on `agents.defaults.agentRuntime.id`. See [Agent runtimes](/en/concepts/agent-runtimes).
...
<AccordionGroup>
  <Accordion title="Related model surfaces">
    ...
    - Per-agent defaults can override `agents.defaults.model` via `agents.list[].model` plus bindings (see [Multi-agent routing](/en/concepts/multi-agent)).
...
Provider configuration examples (including OpenCode) live in [OpenCode](/en/providers/opencode).
</Note>
...
- Live probes require OpenRouter API key from auth profiles or `OPENROUTER_API_KEY` (see [Environment variables](/en/help/environment))
...
- [Agent runtimes](/en/concepts/agent-runtimes) — PI, Codex, and other agent loop runtimes
