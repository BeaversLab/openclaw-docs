...
HTTP endpoints are rejected. See [OpenAI implicit agent runtime](/en/providers/openai#implicit-agent-runtime).
...
- `agents.defaults.modelSelectionScope` optionally chooses the scope of chat commands and Gateway session model updates without an explicit scope. Omit it to preserve existing behavior; see [Model selection scope](/en/gateway/config-agents#agentsdefaultsmodelselectionscope).
...
[`modelCatalog.modelsDev`](/en/plugins/manifest#modelcatalog-reference). Each mapping
...
[configuration reference](/en/gateway/configuration-reference#models).
