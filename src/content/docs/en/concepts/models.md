...
<CardGroup cols={2}>
  <Card title="Model failover" href="/en/concepts/model-failover">
    Auth profile rotation, cooldowns, and how that interacts with fallbacks.
  </Card>
  <Card title="Model providers" href="/en/concepts/model-providers">
    Quick provider overview and examples.
  </Card>
  <Card title="Models CLI reference" href="/en/cli/models">
    Full `openclaw models` command and flag reference.
  </Card>
  <Card title="Configuration reference" href="/en/gateway/config-agents#agent-defaults">
...
HTTP endpoints are rejected. See [OpenAI implicit agent runtime](/en/providers/openai/runtimes#implicit-agent-runtime).
...
[Agent runtimes](/en/concepts/agent-runtimes) and
[GitHub Copilot agent runtime](/en/plugins/copilot).
...
- `agents.defaults.modelSelectionScope` chooses the scope of chat commands and Gateway session model updates without an explicit scope. The default is the current session. See [Model selection scope](/en/gateway/config-agents/models#agentsdefaultsmodelselectionscope).
...
- Per-agent `agents.entries.*.model` (plus bindings) overrides `agents.defaults.model` — see [Multi-agent routing](/en/concepts/multi-agent).

Full key reference, defaults, and JSON5 examples: [Configuration reference](/en/gateway/config-agents#agent-defaults).

For the typed decision model class, available models, rubrics, and plugin API,
see [Decision models](/en/concepts/decision-models).
...
Full mechanics: [Model failover](/en/concepts/model-failover).
...
health probes remain independent of model execution. Use [Models status](/en/cli/models)
and [Doctor](/en/gateway/doctor) to diagnose the configured route.
...
See [Anthropic](/en/providers/anthropic#tool-calls-and-retained-thinking).
...
Full command behavior and config: [Slash commands](/en/tools/slash-commands).
...
`openclaw models` with no subcommand is a shortcut for `models status`, which also surfaces OAuth expiry for auth-store profiles (warns within 24h by default). Full flags, JSON shapes, and auth-profile subcommands: [Models CLI reference](/en/cli/models).
...
[`modelCatalog.modelsDev`](/en/plugins/manifest/models#modelcatalog-reference). Each mapping
...
remain available. See [DeepInfra price estimates](/en/providers/deepinfra#price-estimates).
...
[configuration reference](/en/gateway/config-runtime#models).
...
- [Agent runtimes](/en/concepts/agent-runtimes) — OpenClaw, Codex, and other agent loop runtimes
- [Configuration reference](/en/gateway/config-agents#agent-defaults) — model config keys
- [Image generation](/en/tools/image-generation) — image model configuration
- [Model failover](/en/concepts/model-failover) — fallback chains
- [Model providers](/en/concepts/model-providers) — provider routing and auth
- [Models CLI reference](/en/cli/models) — full command and flag reference
- [Music generation](/en/tools/music-generation) — music model configuration
- [Video generation](/en/tools/video-generation) — video model configuration
- [`openclaw infer`](/en/cli/infer) — infer-first CLI for provider-backed model, media, and embedding workflows
