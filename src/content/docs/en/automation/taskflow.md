...
Task Flow (formerly ClawFlow) is the orchestration layer above [background tasks](/en/automation/tasks). A flow is a durable record of multi-step work with its own status, JSON state, revision counter, and linked task records. Flows survive gateway restarts; individual tasks remain the unit of detached work.
...
Use the returned identities and current owner-visible task facts, not invented status/timing. A child can finish before linkage; `runTask` does not replay past terminal events. Do not create a running projection of completed work. See the [SDK Tasks contract](/en/plugins/sdk-runtime) for the launch, synchronous pre-link check, result handling and revision rules.
