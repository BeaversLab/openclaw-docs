...
only act for a particular agent. See [Hook discovery](/en/automation/hooks#hook-discovery).
...
[Reply delivery](/en/automation/hooks#reply-delivery) for the exact boundary.
...
| `context`    | Event-specific data described under [Event context highlights](/en/automation/hooks#event-context-highlights) |
...
[typed plugin hook](/en/plugins/hooks).
...
[Install and update hook packs](/en/cli/hooks#install-and-update-hook-packs).
...
and sender identifiers it records. See [Log inspection](/en/cli/hooks#command-logger-log-file).
...
[session transcript indexing](/en/reference/memory-config#session-memory-search)
...
shutdown. Typed `session_end` drain behavior belongs to [Plugin hooks](/en/plugins/hooks).
...
[Hook discovery](/en/automation/hooks#hook-discovery).
...
`hookKey` entry and [reload mode](/en/gateway/configuration#reload-modes). A `ready`
...
under [Reply delivery](/en/automation/hooks#reply-delivery), not just enablement.
