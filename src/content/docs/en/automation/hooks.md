...
lifecycle events. OpenClaw includes [bundled hooks](/en/automation/hooks/bundled-hooks)
...
| Modify prompts, intercept tools, control replies, or use lifecycle contracts with priorities and return values | **[Plugin hooks](/en/plugins/hooks)** through `api.on(...)`        |
| Let another service start work through an HTTP request                                                         | **[Webhooks](/en/automation/cron-jobs#webhooks)**                  |
| Export telemetry rather than change behavior                                                                   | **[Diagnostic events](/en/logging#diagnostics-and-opentelemetry)** |
...
The default `hybrid` [reload mode](/en/gateway/configuration#reload-modes) applies
...
only act for a particular agent. See [Hook discovery](/en/automation/hooks/configuration#hook-discovery).
...
[Plugin hooks](/en/plugins/hooks) reference.
...
See [`openclaw hooks`](/en/cli/hooks) for every public report and toggle option,
...
| Page                                                                | Read it when                                                                            |
| ------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| [Writing hooks](/en/automation/hooks/writing-hooks)                    | You are writing a hook and need the file layout, handler contract, or `HOOK.md` fields. |
| [Hook configuration and discovery](/en/automation/hooks/configuration) | You are enabling hooks, narrowing the selection, or tracing discovery across sources.   |
| [Bundled hooks](/en/automation/hooks/bundled-hooks)                    | You want a shipped hook and need its behavior, options, and checks.                     |
| [Hook event types and context](/en/automation/hooks/event-types)       | You need an event key's trigger, wait behavior, or context fields.                      |
| [Hook troubleshooting](/en/automation/hooks/troubleshooting)           | A hook is not discovered, not eligible, or not executing.                               |
...
- <a id="writing-hooks" />[Writing hooks](/en/automation/hooks/writing-hooks#writing-hooks)
- <a id="hook-structure" />[Hook structure](/en/automation/hooks/writing-hooks#hook-structure)
- <a id="handler-implementation" />[Handler implementation](/en/automation/hooks/writing-hooks#handler-implementation)
- <a id="reply-delivery" />[Reply delivery](/en/automation/hooks/writing-hooks#reply-delivery)
- <a id="hook.md-format" /><a id="hook-md-format" />[HOOK.md format](/en/automation/hooks/writing-hooks#hook-md-format)
- <a id="configuration" />[Configuration](/en/automation/hooks/configuration#configuration)
- <a id="hook-discovery" />[Hook discovery](/en/automation/hooks/configuration#hook-discovery)
- <a id="hook-packs" />[Hook packs](/en/automation/hooks/configuration#hook-packs)
- <a id="bundled-hooks" />[Bundled hooks](/en/automation/hooks/bundled-hooks#bundled-hooks)
- <a id="boot-md" />[boot-md](/en/automation/hooks/bundled-hooks#boot-md)
- <a id="boot-md-details" />[boot-md details](/en/automation/hooks/bundled-hooks#boot-md-details)
- <a id="bootstrap-extra-files" />[bootstrap-extra-files](/en/automation/hooks/bundled-hooks#bootstrap-extra-files)
- <a id="bootstrap-extra-files-config" />[bootstrap-extra-files config](/en/automation/hooks/bundled-hooks#bootstrap-extra-files-config)
- <a id="command-logger" />[command-logger](/en/automation/hooks/bundled-hooks#command-logger)
- <a id="command-logger-details" />[command-logger details](/en/automation/hooks/bundled-hooks#command-logger-details)
- <a id="compaction-notifier" />[compaction-notifier](/en/automation/hooks/bundled-hooks#compaction-notifier)
- <a id="compaction-notifier-details" />[compaction-notifier details](/en/automation/hooks/bundled-hooks#compaction-notifier-details)
- <a id="session-memory" />[session-memory](/en/automation/hooks/bundled-hooks#session-memory)
- <a id="session-memory-details" />[session-memory details](/en/automation/hooks/bundled-hooks#session-memory-details)
- <a id="event-types" />[Event types](/en/automation/hooks/event-types#event-types)
- <a id="event-context-highlights" />[Event context highlights](/en/automation/hooks/event-types#event-context-highlights)
- <a id="message-context" />[Message context](/en/automation/hooks/event-types#message-context)
- <a id="troubleshooting" />[Troubleshooting](/en/automation/hooks/troubleshooting#troubleshooting)
- <a id="hook-not-discovered" />[Hook not discovered](/en/automation/hooks/troubleshooting#hook-not-discovered)
- <a id="hook-not-eligible" />[Hook not eligible](/en/automation/hooks/troubleshooting#hook-not-eligible)
- <a id="hook-not-executing" />[Hook not executing](/en/automation/hooks/troubleshooting#hook-not-executing)
...
- [Standing orders](/en/automation/standing-orders)
