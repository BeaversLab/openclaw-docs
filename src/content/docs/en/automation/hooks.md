...
- **Webhooks**: external HTTP endpoints that let other systems trigger work in OpenClaw. See [Webhooks](/en/automation/cron-jobs#webhooks).
...
plugin hook `before_agent_finalize` instead. See [Plugin hooks](/en/plugins/hooks).
...
For the complete plugin hook reference, see [Plugin hooks](/en/plugins/hooks).
...
- [Plugin hooks](/en/plugins/hooks) — in-process plugin lifecycle hooks
