...
<Note>
Looking for scheduling? See [Automation](/en/automation) for choosing the right mechanism. This page is the activity ledger for background work, not the scheduler.
</Note>
...
For a strict, ephemeral one-shot agent run in CI or a script, use [`openclaw agent exec`](/en/cli/agent#agent-exec) instead of managed background work.
...
<AccordionGroup>
  ...
  <Accordion title="What does not create tasks">
    - Heartbeat turns - main-session; see [Heartbeat](/en/gateway/heartbeat)
...
[Progress after yield](/en/concepts/subagent-yield-handoff#progress-after-yield).
...
The flow lookup token accepts a flow id or owner key. Use these when the orchestrating [Task Flow](/en/automation/taskflow) is the thing you care about rather than one individual background task record.
...
[Task Flow](/en/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `openclaw tasks` to inspect individual task records and `openclaw tasks flow` to inspect the orchestrating flow.
...
See [Automations](/en/automation/cron-jobs).
...
- [CLI: Tasks](/en/cli/tasks) - CLI command reference
- [Heartbeat](/en/gateway/heartbeat) - periodic main-session turns
- [Automations](/en/automation/cron-jobs) - scheduling background work
- [Task Flow](/en/automation/taskflow) - flow orchestration above tasks
- [Sub-agents](/en/tools/subagents) - child agents spawned from a session
- [Music generation](/en/tools/music-generation) - generate music via `music_generate` across the supported provider workflows
