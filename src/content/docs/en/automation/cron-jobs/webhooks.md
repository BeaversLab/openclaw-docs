...
Gateway HTTP hooks: how an external service calls OpenClaw to wake an agent or submit an agent turn. Part of the [Automations](/en/automation/cron-jobs) guide.
...
[Hooks reference](/en/gateway/config-hooks#hooks) lists payload fields,
...
<AccordionGroup>
  ...
  <Accordion title="POST /hooks/agent">
    Submit an agent turn with a required `message`. Optional routing, model, thinking, timeout, and idempotency fields are documented in the [payload reference](/en/gateway/config-hooks#hook-agent-payload).
  ...
  <Accordion title="Mapped hooks (POST /hooks/<name>)">
    Custom paths resolve through `hooks.mappings`. The first matching mapping wins, ahead of presets. Templates or trusted local JS/TS transforms turn the payload into `wake` or `agent` actions; a transform returning `null` produces HTTP `204` without a run. See [Mapping details](/en/gateway/config-hooks#mapping-details).
...
[reference](/en/gateway/config-hooks#hook-retries-and-fan-out) explains its
