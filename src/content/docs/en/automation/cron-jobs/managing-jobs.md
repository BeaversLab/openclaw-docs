...
Day-to-day operation of stored jobs: copy-ready CLI examples, the management commands, run history semantics, and the `cron.*` configuration keys. Part of the [Automations](/en/automation/cron-jobs) guide.
...
The webhook URL remains subject to the [strict outbound policy](/en/automation/cron-jobs/delivery#delivery-and-output); configure `cron.webhookSsrfPolicy` for an intentional local or private receiver.

Clearing **Timeout (seconds)** in the Control UI and saving removes the saved override, restoring the [default runtime budget](/en/automation/cron-jobs/how-it-works). For API clients, a `cron.update` payload patch sets a timeout with a number, clears it with `timeoutSeconds: null`, and preserves the saved value when `timeoutSeconds` is omitted. New jobs omit the field to use the default; `null` is only an update instruction.
