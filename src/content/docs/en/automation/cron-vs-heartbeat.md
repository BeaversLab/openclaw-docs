...
All cron job executions (main and isolated) create [task records](/en/automation/tasks). Heartbeat turns do not. Main-session cron tasks use `silent` notify policy by default so they do not generate notifications.
...
- **No task record**: heartbeat turns stay in main-session history (see [Background Tasks](/en/automation/tasks)).
...
- **Task tracking**: isolated jobs create [background task](/en/automation/tasks) records visible in `openclaw tasks` and `openclaw tasks audit`.
...
| [Tasks](/en/automation/tasks) | No task record                  | Task record (silent)     | Task record (visible in `openclaw tasks`)       |
...
- [Background Tasks](/en/automation/tasks) — task ledger, audit, and lifecycle
