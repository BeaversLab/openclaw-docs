...
[`openclaw doctor --fix`](/en/cli/doctor) owns file-to-SQLite migrations and records a
...
[legacy state migration](/en/cli/doctor/state-migrations) for how to resolve it.
...
| [Database access in workers](/en/reference/database-schemas/worker-access)                        | Moving runtime reads and writes off the Gateway main thread while preserving their owners.               |
| [Worker migration inventory](/en/reference/database-schemas/worker-access-inventory)              | Reproducing the synchronous-access inventory and choosing the next migration.                            |
...
- [`openclaw doctor`](/en/cli/doctor) — CLI reference for the command that runs those migrations
- [`openclaw update`](/en/cli/update) — CLI reference for the updater that preflights schema support
