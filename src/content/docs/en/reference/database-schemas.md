...
The [mentions Inbox](/en/concepts/multi-user#temporary-mentions-inbox) uses existing
...
cannot recover history already evicted by an older writer. See [ACP CLI](/en/cli/acp).
...
storage. See [Transcripts CLI](/en/cli/transcripts).
...
Structured [Goal controls](/en/tools/goal#gateway-requests-and-retries) use a lazy
...
updater version, and [manual update commands](/en/install/updating#updating-from-2026.9.2-across-a-schema-bump).
...
See the accepted [personal GitHub ownership and publication design](https://github.com/openclaw/openclaw/issues/133590) and the operator-facing [GitHub connections guide](/en/concepts/user-model#github-connections).
...
See [Per-person model accounts](/en/concepts/multi-user#per-person-model-accounts) for connection, cancellation, session billing, and unlink behavior.
...
delivery protocol. See [Watch voice and chat](/en/platforms/ios#apple-watch-voice-and-chat)
...
replace a complete backup. See [Downgrade](/en/install/updating#downgrade).
...
Follow [Downgrade](/en/install/updating#downgrade) for the managed rollback path,
