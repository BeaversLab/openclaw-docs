...
3. Run `pnpm release:readiness`. Its independent generated, plugin metadata, package, temporary-path, and workflow gates run concurrently and write timing evidence when called by the candidate controller. Freeze the product-complete pre-changelog commit as the **Code SHA**, then use `node scripts/full-release-validation-at-sha.mjs --sha <code-sha> --target-ref release/YYYY.M.PATCH`. This pins trusted workflow tooling while the full Vitest, Docker, QA, package, and performance matrix targets the exact Code SHA.
...
- Cross-OS install and upgrade runtime validation runs through public `OpenClaw Release Checks`, which calls `.github/workflows/openclaw-cross-os-release-checks-reusable.yml`; `Full Release Validation` forwards its release profile into Release Checks. This split is intentional: keep the real npm release path short, deterministic, and artifact-focused, while slower live checks stay in their own lane so they do not stall or block publish.
- Packaged-upgrade lanes restore an installed baseline prefix from an immutable OS/architecture/Node/package cache and cap each install or updater stage at five minutes. Beta keeps packaged-upgrade failures visible as advisory evidence; stable keeps them blocking. Fresh package and installer lanes remain blocking for both profiles.
- Use `pnpm release:evidence-impact -- --base <sha> --head <sha>` while iterating. It selects the smallest diagnostic rerun group for changelog-only, release-tooling-only, plugin-only, or product changes. Diagnostic selection never weakens final publish proof: every non-changelog product or tooling SHA still needs full validation before publication.
...
See [Full release validation](/en/reference/full-release-validation) for the complete stage matrix, exact workflow job names, stable versus full profile differences, artifacts, and focused rerun handles.
...
The canonical checklist for update and plugin validation is [Testing updates and plugins](/en/help/testing-updates-plugins). Use it when deciding which local, Docker, Package Acceptance, or release-check lane proves a plugin install/update, doctor cleanup, or published-package migration change. Exhaustive published update migration from every stable `2026.4.23+` package is a separate manual `Update Migration` workflow, not part of Full Release CI.
