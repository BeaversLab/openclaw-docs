...
<CardGroup cols={2}>
  <Card title="Memory overview" href="/en/concepts/memory">
    How memory works.
  </Card>
  <Card title="Builtin engine" href="/en/concepts/memory-builtin">
    Default SQLite backend.
  </Card>
  <Card title="Memory search" href="/en/concepts/memory-search">
    Search pipeline and tuning.
  </Card>
  <Card title="Active memory" href="/en/concepts/active-memory">
...
See [Active Memory](/en/concepts/active-memory) for both activation paths,
...
an API-key or bearer-token profile saved with [`openclaw models auth`](/en/cli/models#auth-profiles),
...
it and the active profile or container hint. See [memory index](/en/cli/memory#memory-index).
...
[`openclaw memory forget`](/en/cli/memory#memory-forget) are also durably excluded,
...
<Note>
The [session-memory hook](/en/automation/hooks/bundled-hooks#session-memory) saves conversation
...
[`tools.sessions.visibility`](/en/gateway/config-tools#tools-sessions). The default
...
agent's indexed corpus; use [`sessions_search`](/en/concepts/session-search) for
...
For Bun on macOS, install Homebrew SQLite to enable extension loading; see [Bun SQLite setup](/en/install/bun-compatibility#sqlite-library-selection) for automatic discovery and the `OPENCLAW_SQLITE_LIBRARY` library override.
...
[Memory provenance and deletion](/en/concepts/memory-provenance) for coverage and
deletion workflows.
...
Sessions selected by [`memory forget`](/en/cli/memory#memory-forget) are checked
...
[deletion boundaries](/en/concepts/memory-provenance#what-deletion-does-not-cover)
...
For conceptual behavior and slash commands, see [Dreaming](/en/concepts/dreaming).
...
- [Memory overview](/en/concepts/memory)
