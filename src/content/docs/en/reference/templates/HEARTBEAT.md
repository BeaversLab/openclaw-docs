...
For due-only checks instead of a plain checklist, use a structured `tasks:` block with per-task `interval` and `prompt` fields; see [HEARTBEAT.md](/en/gateway/heartbeat#heartbeatmd-optional) for the format and behavior.

## Related

- [Heartbeat](/en/gateway/heartbeat)
- [Heartbeat config](/en/gateway/config-agents)
