...
See [Scheduled Tasks (Cron) vs Heartbeat](/en/automation#scheduled-tasks-cron-vs-heartbeat) for the full decision table. Short version: heartbeat batches periodic checks with full session context on approximate timing (default every 30 minutes); cron is for exact timing, isolated runs, a different model, or one-shot reminders.
...
- [Default AGENTS.md](/en/reference/AGENTS.default)
- [Scheduled tasks vs heartbeat](/en/automation#scheduled-tasks-cron-vs-heartbeat)
