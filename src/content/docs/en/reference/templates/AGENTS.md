...
See [Scheduled Tasks (Cron) vs Heartbeat](/en/automation#automations-vs-heartbeat) for the full decision table. Short version: heartbeat batches periodic checks with full session context on approximate timing (default every 30 minutes); cron is for exact timing, isolated runs, a different model, or one-shot reminders.
...
- [Scheduled tasks vs heartbeat](/en/automation#automations-vs-heartbeat)
