...
rows. See [incremental canonical validation](/en/reference/database-schemas/agent-schema-history#incremental-canonical-session-validation)
...
policy described in [Integrity checks](/en/reference/database-schemas/integrity-and-recovery#integrity-checks).
...
Diagnostic paths that prepare their own private read-only snapshots use the size-derived child-process budget described under [Integrity checks](/en/reference/database-schemas#integrity-checks).
