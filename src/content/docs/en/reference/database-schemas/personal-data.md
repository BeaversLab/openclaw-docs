...
The companion table leaves both existing local request-table definitions and their receipt digests unchanged. State schema 18 adds `requester_authority_json` to this companion and to repository publication receipts so shared requests retain their original requesting authority. Historical rows stay unbound; terminal receipts remain readable. These optional tables require a version bump because shipped schema validators reject additional columns even when nullable. See [state schema 18](/en/reference/database-schemas/state-schema-history#state-schema-18) for migration and rollback.
