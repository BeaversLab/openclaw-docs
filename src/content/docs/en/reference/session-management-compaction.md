...
follow the [Doctor migration sequence](/en/cli/doctor#session-sqlite-migration)
...
A `sessionKey` identifies which conversation bucket you are in (routing + isolation). Canonical rules: [/concepts/session](/en/concepts/session).
...
More on limits: [/reference/token-use](/en/reference/token-use).
...
Config (`agents.defaults.compaction.memoryFlush`), full reference at [/gateway/config-agents](/en/gateway/config-agents/heartbeat-compaction-and-streaming#agents-defaults-compaction):
