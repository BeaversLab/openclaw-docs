...
- Direct chats collapse into the agent's `main` session by default; groups and channels/rooms get their own session keys. See [Channel routing](/en/channels/channel-routing) for the exact key formats. Heartbeats keep background tasks alive.
