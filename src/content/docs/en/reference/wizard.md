...
For a high-level overview, see [Onboarding (CLI)](/en/start/wizard). For step-by-step
behavior and outputs, see [CLI setup reference](/en/start/wizard-cli-reference).
...
<Steps>
  ...
  <Step title="Channels">
    ...
    - [Signal](/en/channels/signal) (plugin): optional `signal-cli` install + account config.
    ...
    - Discord, Feishu, Microsoft Teams, QQ Bot, Slack, and other channels ship as
      plugins onboarding can install for you. Full catalog: [Channels](/en/channels).
...
- `session.dmScope` (local onboarding defaults this to `"per-channel-peer"` when unset; existing explicit values are preserved. Details: [CLI Setup Reference](/en/start/wizard-cli-reference#outputs-and-internals))
...
- CLI setup reference: [CLI setup reference](/en/start/wizard-cli-reference)
