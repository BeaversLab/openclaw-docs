...
Pending pairing requests expire 5 minutes after the device's last retry — a device that keeps reconnecting keeps its one pending request (and `requestId`) alive instead of minting a new prompt every few minutes; see [Node pairing](/en/gateway/pairing) for the full request/approve lifecycle. If a node retries with changed auth details (role/scopes/public key), the prior pending request is superseded and a new `requestId` is created — clients get a `device.pair.resolved` event for the superseded request, and you should re-run `openclaw devices list` before approving.
...
- A connected native Mac can opt in to coalesced physical-input activity from
  **Settings -> Permissions -> Active computer detection**. Accessibility is
  ...
  [Active computer presence](/en/nodes/presence) for setup, privacy, timing, and
  troubleshooting.
...
omitted. See
[Node host stats](/en/gateway/protocol/presence#node-host-stats) for the wire contract.
...
[Node pairing](/en/gateway/pairing#one-paste-node-pairing).
...
[Identity and pairing state](/en/cli/node#identity-and-pairing-state) for the
...
See [Supervise Codex sessions](/en/plugins/codex-supervision) for setup,
...
[Multi-user mode](/en/concepts/multi-user).
...
See [Anthropic: Claude sessions across computers](/en/providers/anthropic#claude-sessions-across-computers)
...
The [File Transfer plugin](/en/plugins/reference/file-transfer) provides independently
...
Dangerous or privacy-heavy commands require a one-time persistent opt-in with `gateway.nodes.commands.allow`, even if a node declares them: `camera.snap`, `camera.clip`, `camera.ptz.control`, `desktop.stream`, `screen.record`, `contacts.add`, `calendar.add`, `reminders.add`, `health.summary`, `sms.send`, `sms.search`. `gateway.nodes.commands.deny` always wins over defaults and extra allowlist entries. See [Paired node desktops](/en/gateway/config-browser-ui-desktop#paired-node-desktops), [HealthKit summaries](/en/platforms/ios-healthkit), and [Computer use](/en/nodes/computer-use) for the local enablement, pairing, capability, and tool-policy gates around desktop access.
...
- The agent-facing path is [`show_widget`](/en/tools/show-widget) with
  `presentation.target: "node_panel"`; use the CLI helpers for direct operator
  control.
- A2UI renders on [session dashboards](/en/web/dashboards), not through node
  Canvas commands.
...
- Nodes clamp clip duration to keep the base64 payload manageable (see [Camera capture](/en/nodes/camera) for exact per-platform limits). The `nodes` agent tool additionally caps requested `durationMs` at 300000 (5 minutes) before forwarding the call; the node itself enforces the tighter limit.
...
- Full parameter/response shape and error codes: [Location command](/en/nodes/location-command).
...
- Exec approvals are enforced locally via
  `~/.openclaw/state/openclaw.sqlite#exec_approvals_config` (see [Exec approvals](/en/tools/exec-approvals)).
