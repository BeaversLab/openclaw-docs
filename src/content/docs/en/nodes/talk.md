...
Apple Watch also retains **Talk to Claw**, the separate [one-turn companion flow](/en/platforms/ios#talk-to-claw-with-the-iphone): native dictation, text relayed through the iPhone, and system-voice readback. **Talk on Watch** is the realtime path included in normal Watch setup; see [standalone voice setup](/en/platforms/ios#standalone-voice).
...
Raw archives and [session exports](/en/tools/slash-commands) remain lossless. Existing
...
honor `session.scope` and the configured [main session](/en/concepts/main-session) key. A shared fixed store retains
...
For model capability limits, see [Discord voice policies](/en/channels/discord#voice-channels)
...
Watch background behavior. See [Watch setup and limits](/en/platforms/ios#standalone-voice).
