...
Apple Watch also retains **Talk to Claw**, the separate [one-turn companion flow](/en/platforms/ios#talk-to-claw-with-the-iphone). That flow uses native dictation, text relayed through the iPhone, and system-voice readback. **Talk on Watch** is the realtime path included in normal Watch setup. See [standalone voice setup](/en/platforms/ios#standalone-voice).
...
| Page                                                                   | Read it when                                                                          |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| [Talk realtime sessions and delegation](/en/nodes/talk/realtime-sessions) | You are wiring realtime Talk: voice selection, delegation, steering, and transcripts. |
| [Talk session ownership](/en/nodes/talk/session-ownership)                | You need agent and session resolution, control authority, or close semantics.         |
| [Talk on macOS and the Gateway relay](/en/nodes/talk/macos-relay)         | You run Talk on macOS or enable the streamed realtime Gateway relay.                  |
| [Talk client UI](/en/nodes/talk/client-ui)                                | You need the macOS, Apple Watch, or Android client controls and behavior.             |
...
- <a id="choose-a-talk-voice-from-chat" />[Choose a Talk voice from chat](/en/nodes/talk/realtime-sessions#choose-a-talk-voice-from-chat)
- <a id="session-ownership" />[Session ownership](/en/nodes/talk/session-ownership#session-ownership)
- <a id="behavior-(macos)" />[Behavior (macOS)](</nodes/talk/macos-relay#behavior-(macos)>)
- <a id="behavior-macos" />[Behavior (macOS)](/en/nodes/talk/macos-relay#behavior-macos)
...
- <a id="realtime-talk-over-the-gateway-relay-macos" />[Realtime Talk over the Gateway relay (macOS)](/en/nodes/talk/macos-relay#realtime-talk-over-the-gateway-relay-macos)
- <a id="when-realtime-cannot-start" />[When realtime cannot start](/en/nodes/talk/macos-relay#when-realtime-cannot-start)
- <a id="macos-ui" />[macOS UI](/en/nodes/talk/client-ui#macos-ui)
- <a id="apple-watch-ui" />[Apple Watch UI](/en/nodes/talk/client-ui#apple-watch-ui)
- <a id="android-ui" />[Android UI](/en/nodes/talk/client-ui#android-ui)
...
to `marin` and has its own [voice list](/en/providers/openai/voice-and-speech).
...
See [GPT-Live in Discord](/en/channels/discord/voice-channels#gpt-live-in-discord)
...
For model capability limits, see [Discord voice policies](/en/channels/discord/voice-channels#voice-channels)
and [Voice Call tools](/en/plugins/voice-call#realtime-voice-conversations).
...
- [Google Meet plugin](/en/plugins/google-meet)
