...
- Groq setup details: [Groq](/en/providers/groq).
- Deepgram picks up `DEEPGRAM_API_KEY` when `provider: "deepgram"` is used. Setup details: [Deepgram](/en/providers/deepgram).
- Mistral setup details: [Mistral](/en/providers/mistral).
- SenseAudio picks up `SENSEAUDIO_API_KEY` when `provider: "senseaudio"` is used. Setup details: [SenseAudio](/en/providers/senseaudio).
