...
- Media understanding (configured via `tools.media.*` or shared `tools.media.models`) runs before templating and can insert `[Image]`, `[Audio]`, and `[Video]` blocks into `Body`.
  ...
  - Native-vision models can skip the `[Image]` summary block. See [Rules and behavior](/en/nodes/media-understanding#rules-and-behavior) for the rule and its MiniMax exception.
