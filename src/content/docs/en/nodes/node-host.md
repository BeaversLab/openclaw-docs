...
- **Node onboarding support:** Join-code creation and `/j` redemption are core Gateway operations. They do not require enabling the `device-pair` plugin, even though its retained `publicUrl` configuration field can supply an endpoint. See [Join codes](/en/cli/devices#openclaw-devices-join-code) for the printed `npx openclaw connect <url>` command.
...
Gateway command policy and the node host's [exec approvals](/en/tools/exec-approvals)
...
command IDs. For example, a [Session Share](/en/plugins/session-share) node can
...
[Headless node updates](/en/install/updating/automatic-updates#headless-node-updates)
...
declared and allowed can still run. [Gateway command policy](/en/nodes/command-policy)
...
- Device pairing and command-surface approval are still required; follow [Pair + name](/en/nodes/node-host#pair-+-name) through both stages.
