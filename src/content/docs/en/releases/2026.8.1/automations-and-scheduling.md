...
[Automations](/en/automation) now bring scheduled work together under one name across the agent, Control UI, command line, docs, and supported native apps, while existing Cron commands, settings, jobs, and schedule syntax continue to work.
...
[Automations](/en/automation/cron-jobs) is now the user-facing name in the agent tool, Control UI, command line, and docs, while `openclaw automations` offers the same command family as `openclaw cron`. The old command, `/cron` route, `cron.*` settings and RPC names, schedule expressions, identifiers, and stored jobs continue to work.
