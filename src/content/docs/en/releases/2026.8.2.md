...
Forwarded messages in the [Control UI](/en/web/control-ui) now show which agent sent them through that agent's avatar or stable initials, including in direct conversations where the identity used to disappear.
...
In [Discord](/en/channels/discord) and [Slack](/en/channels/slack), long turns are calmer too, with one stable acknowledgement, useful milestones, and prompt approvals or failures instead of a running list of every tool action.
...
[iMessage](/en/channels/imessage) direct replies and actions such as polls now stay with the conversation that requested them when the service setting is unset, while a redacted display value is no longer retried as though it were a real destination. In multi-bot [Discord](/en/channels/discord) rooms, the opt-in other-mention filter also keeps OpenClaw out of replies aimed at another bot unless someone explicitly mentions it.

[Telegram](/en/channels/telegram) keeps rich-button labels in reply context and keeps its model picker usable after a configuration reload, while long [WhatsApp](/en/channels/whatsapp) replies keep their formatting and quote context across split messages. Slack Enterprise Grid plugin approvals now stay with the intended workspace from the direct message through the approval click.
...
[LINE](/en/channels/line) now stays quiet when another connected service controls the chat, and a bot joining a group or room responds only where it is allowed to serve. Shared group rules also carry into named groups and rooms unless that conversation has its own setting.
...
Shared [Beam](/en/plugins/beam) transcripts now keep questions before their answers and show the verified person who published the snapshot instead of borrowing the viewer's identity.
...
- Autoreview no longer requires a local credential scan before dispatch. [#133626](https://github.com/openclaw/openclaw/pull/133626) — thanks to [@Patrick-Erichsen](https://github.com/Patrick-Erichsen)
