...
The [managed llama.cpp setup](/en/plugins/llama-cpp) now recommends a model using the available memory, supported graphics hardware, and disk space on the computer running OpenClaw. It shows you that computer's name, the model, and the download size before asking to proceed, which matters when you are opening setup in a browser connected to a different machine. Before changing your default, it checks both a response and a real file-read tool result; a failed or cancelled activation leaves the previous model selected.

The [local-model recommendations](/en/gateway/local-models) begin with an 8 GiB memory floor for the smallest recipe, with larger choices needing more memory and, in some cases, graphics acceleration. Actual fit and speed still depend on available resources and the workload. CPU-only replies can still take several minutes, and setup now distinguishes a response-check timeout from a tool-use-check timeout with the failed check named in the error.

Managed setup also applies [lean tools for local models](/en/concepts/experimental-features#local-model-lean-mode) before checking the candidate, reducing the large tool list a smaller model has to process while keeping the agent's ordinary instructions in normal chats. An explicit opt-out is preserved. On a Gateway with several agents, setting up just one local agent can still change shared lean-tool settings used by other agents, so review those settings after targeted setup.

![Conceptual 2026.9.2 explainer: Inspect host — Memory, graphics and free disk — Gateway host, not the browser; Confirm download — See the recipe and its size — Approve before downloading; Verify tools — Response + real file-read check — Only then change the default. Failed or cancelled activation keeps the previous model. Fit and speed depend on the host.](https://github.com/user-attachments/assets/40c7c9b9-1c96-4812-8d91-342b4a6d3dd3)

_A local-model setup journey, not a speed comparison. Conceptual illustration, not a screenshot. Failed or cancelled activation keeps the previous model. Fit and speed depend on the host._
...
Before [local configuration changes](/en/cli/configure), OpenClaw also checks the installed Gateway's recorded data and configuration paths, even when the service is stopped. A confirmed mismatch stops the wizard before it writes to the wrong location. If those paths cannot be verified, configuration remains available with a warning and a pointer to `openclaw gateway status --deep`.
...
Clean, environment-only [agent runs](/en/cli/agent#agent-exec) using an OpenAI API key can now start through the built-in OpenClaw runtime when the optional Codex runtime is absent. This removes a setup failure before the agent could answer or begin its task, while explicitly chosen runtimes and existing session pins continue to require their selected runtime.
...
When Doctor or onboarding cannot update your shell profile, [completion setup](/en/cli/completion) now provides a command that loads the prepared completions in your current shell instead of sending you back into the same permission failure. Run the complete command it displays; enabling completion for future shells still requires fixing the reported permission or read-only error and retrying installation. PowerShell recovery commands also handle paths containing typographic apostrophes.
...
![Published 2026.9.2 interface: Four saved Harbor dashboards, with actual gallery previews and owner controls. Fictional Harbor data; feature-focused crop.](https://github.com/user-attachments/assets/68ba4ea8-8da8-41e1-868d-d4bc8030d03a)

_Four saved Harbor dashboards, with actual gallery previews and owner controls. Published 2026.9.2 interface with fictional Harbor data; no live task execution._
...
![Published 2026.9.2 interface: Open the draft file without losing the request that explains it. Fictional Harbor data; feature-focused crop.](https://github.com/user-attachments/assets/df3995fa-a5f1-4a99-b346-3774d14adc75)

_Open the draft file without losing the request that explains it. Published 2026.9.2 interface with fictional Harbor data; no live task execution._
...
![Published 2026.9.2 Side chat beside a fictional Harbor garden conversation, explaining the east gate meeting point.](https://github.com/user-attachments/assets/0a913719-e76f-4aec-88cc-457a59e2f8ee)

_Keep the welcome brief in view while asking about the meeting point. Published 2026.9.2 interface with fictional Harbor data; no live task execution._
...
![Published 2026.9.2 interface: The brief and workspace picker stay together before starting the session. Fictional Harbor data; feature-focused crop.](https://github.com/user-attachments/assets/c4f51d00-d735-423f-9f97-1ac46db355c8)

_The brief and workspace picker stay together before starting the session. Published 2026.9.2 interface with fictional Harbor data; no live task execution._
...
![Conceptual 2026.9.2 explainer: Before restart — The attempt records its steps; Reconnect — Return to the same saved attempt; After restart — Read outcome and verification. A running label is not proof of a live updater. Missing verification remains unverified.](https://github.com/user-attachments/assets/0c88dd6a-92c1-4f4a-843d-0b259d6f07e6)

_Follow one saved update attempt across a restart. Conceptual illustration, not a screenshot. A running label is not proof of a live updater. Missing verification remains unverified._
...
![Conceptual 2026.9.2 explainer: Read saved context — Recover the original request — Harbor welcome brief; Check current access — Use fresh authorized tools — Not expired approvals; Continue eligible work — Inspect what already happened — Avoid uncertain repeats. Interrupted calls and expired handles are not replayed automatically; uncertain delivery needs attention.](https://github.com/user-attachments/assets/bdde1594-ff9e-49c8-97f6-39b14ca9facf)

_Recovery for eligible interrupted Full Access tasks. Conceptual illustration, not a screenshot. Interrupted calls and expired handles are not replayed automatically; uncertain delivery needs attention._
...
[Skill Workshop](/en/tools/skill-workshop) now identifies conflicting support-file locations before saving a proposal, and refuses proposals or evaluations that would silently omit unreadable or excessively nested content. When agents sharing a workspace cannot start a collection review because a provider credential is missing, the error names the affected provider and agent so the operator can fix authentication without guessing at model settings.

Restoring an older collection backup also protects later edits and deliberate deletions when the relevant files cannot be verified. Some older backups need [manual recovery](/en/tools/skill-workshop#when-an-older-backup-cannot-be-restored-automatically); keep complete private copies of both the backup and current files while choosing what to restore, and do not change saved hashes or flatten live files to force an automatic restore.
...
A failure to delete disposable skill files no longer changes a successful worker result into a failure or hides the original cancellation or timeout. [Cleanup warnings](/en/cli/worker#runtime-boundary) identify files that may remain; wait for the worker or session and its processes to stop before removing them manually. Credential cleanup remains separate and strict. Cloud startup also reports whether a download failed during connection, TLS, the HTTP response, or body transfer, helping operators investigate the relevant part of setup.
...
The [Skills interface](/en/tools/skills#installer-specs) now selects the installation recipe advertised for your operating system when a skill leaves recipe IDs implicit. That corrects cases where the button showed one platform's dependency but selected another platform's recipe.
...
![Conceptual 2026.9.2 explainer: Initial request — Draft the Harbor welcome brief; Mid-response correction — Use the east gate, not the north gate; Continuation — Accepted direction stays in context. Official OpenAI API-key WebSocket + built-in runtime. SSE queues normally; native Codex is separate.](https://github.com/user-attachments/assets/5e4b5e99-9e3d-485f-84ff-225a45e1067f)

_A conceptual example of supported Astra response steering. Conceptual illustration, not a screenshot. Official OpenAI API-key WebSocket + built-in runtime. SSE queues normally; native Codex is separate._
...
OpenClaw now supports [Meta's Muse Spark 1.3](/en/providers/meta), with Standard and Contributor choices for accounts that have access. Both accept text and image input and expose reasoning controls, a 1,048,576-token context window, and up to 131,072 output tokens. Select `meta/muse-spark-1.3` for Standard or `meta/muse-spark-1.3-contributor` for Contributor.
...
![Published 2026.9.2 interface: Saved fictional accounts and the preference for new chats, without changing existing chats. Fictional Harbor data; feature-focused crop.](https://github.com/user-attachments/assets/50939811-cf38-4785-91bb-7d8a56faaf66)

_Saved fictional accounts and the preference for new chats, without changing existing chats. Published 2026.9.2 interface with fictional Harbor data; no live task execution._
...
The local-server guide now uses [llmman](/en/providers/llmman), the maintained replacement for Inferrs, with corrected startup commands, readiness checks, and troubleshooting for requests that work directly but fail during a larger agent task. Existing Inferrs documentation links redirect to the new guide. Keep its unauthenticated service on loopback or behind trusted access controls.
...
Ending a Gateway-controlled OpenAI Realtime call now reports a failed provider hangup and retains it for bounded automatic cleanup retries. If those attempts are exhausted, cleanup remains incomplete and the call still reserves capacity; pressing End again is not the retry mechanism. The [OpenAI cleanup guidance](/en/providers/openai#gateway-controlled-realtime-call-cleanup) explains recovery, including that a restart can lose the in-memory cleanup obligation and does not prove the provider call ended.
...
![Conceptual 2026.9.2 explainer: Omitted settings broaden access — Session visibility defaults to all — Agent-to-agent access defaults on; Narrow deliberately — Use agent or self as appropriate — Review allowed agent pairs. These are one-Gateway controls, not admin isolation. Mutually untrusted users need separate Gateways.](https://github.com/user-attachments/assets/15297f0a-ad0f-4bc2-b31b-08e3be8cb5dd)

_Review conversation visibility before upgrading a shared Gateway. Conceptual illustration, not a screenshot. These are one-Gateway controls, not admin isolation. Mutually untrusted users need separate Gateways._
