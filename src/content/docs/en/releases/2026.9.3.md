...
<Note>
Upgrade Node before OpenClaw if you use an older runtime. This release requires Node 24.16.0 or newer on 24.x, or Node 26.1.0 or newer. Node 26 is recommended. See the [Node requirements](/en/install/node) for supported hosts.
</Note>
...
![Published 2026.9.3 interface: The real Session sharing menu shows public access and its disable control. Fictional Harbor data; feature-focused crop.](https://github.com/user-attachments/assets/36c88eee-e444-4177-85a9-45c481904de2)

_The real Session sharing menu shows public access and its disable control. Published 2026.9.3 interface with fictional Harbor data; no live task execution._
...
![Published 2026.9.3 Sessions page showing four fictional Harbor conversations and the transcript-search field.](https://github.com/user-attachments/assets/9cc2dd61-f187-4489-8c04-420a94403540)

_Named tasks make a growing project easier to revisit. Published 2026.9.3 interface with fictional Harbor data; no live task execution._
...
![Published 2026.9.3 conversation showing a fictional Harbor garden plan with a workstream table and a welcome brief.](https://github.com/user-attachments/assets/c5e47d33-273c-4513-af94-3335297d53ef)

_A planning table and clear next decisions in the shipped interface. Published 2026.9.3 interface with fictional Harbor data; no live task execution._
...
![Conceptual 2026.9.3 explainer: Choose repository — harbor-volunteer-guide — Fictional project; Remote checkout + setup — Managed cloud or paired node — Current worker bundle required; Retain accepted changes — Stop keeps checkpoints — Move to Gateway is explicit. Checkpoints are not full Git history and do not back up unaccepted changes.](https://github.com/user-attachments/assets/4146442d-3e0a-4b22-807e-5fb059b5755c)

_Start repository work without a preliminary Gateway clone. Conceptual illustration, not a screenshot. Checkpoints are not full Git history and do not back up unaccepted changes._
...
![Conceptual 2026.9.3 explainer: Private candidate — Check core and enabled plugins — Before downtime; Independent validation — Candidate must pass — Config edits stay with you; Activate and verify — Inspect the running installation — Recovery is not update success. Conceptual flow, not a completed update. Rollback does not replace a verified data backup.](https://github.com/user-attachments/assets/f6f68f1a-a5b9-4021-9054-822e720fa9cc)

_Supported updates separate candidate checks from activation. Conceptual illustration, not a screenshot. Conceptual flow, not a completed update. Rollback does not replace a verified data backup._
...
Eligible updates driven by 2026.9.2 can cross a shared-state schema change while the older updater finishes reading its update history. Agent-schema migrations, missing state metadata and failed state migrations still require the documented external-install and fresh-Doctor recovery path. See [schema-bump recovery](/en/reference/database-schemas#schema-bumps-and-older-updaters) if your update is refused.
...
![Conceptual 2026.9.3 explainer: Search transcript — "east gate" — Find the relevant discussion; Read in context — Rowan: meet beside the step-free route — Fictional excerpt, not a recording; Export selected meeting — Markdown or JSONL — Search does not trim the export. Web downloads are limited to 4 MiB. Larger exports require the documented host CLI path.](https://github.com/user-attachments/assets/33383c58-6cc0-4986-a9a1-3440c3348937)

_Fictional Harbor planning notes illustrate transcript search. Conceptual illustration, not a screenshot. Web downloads are limited to 4 MiB. Larger exports require the documented host CLI path._
...
![Conceptual 2026.9.3 explainer: Literal question — Where did we note the east gate? — Fictional Harbor planning query; Keyword index — Match words in saved notes — Optional embeddings unavailable; Relevant note — access-checklist.md — Read the original context. Explicitly required embedding providers still report failure. Keyword fallback supplies no semantic vectors.](https://github.com/user-attachments/assets/6dc0ac69-54c3-47e1-bbe2-c5be026c9508)

_Keyword recall is useful fallback, not semantic search in disguise. Conceptual illustration, not a screenshot. Explicitly required embedding providers still report failure. Keyword fallback supplies no semantic vectors._
...
A skill your agent learns now belongs to that agent’s [Workshop](/en/tools/skill-workshop), so moving between workspaces no longer means moving the collection along with it. Installed skills and suggestions have separate views, keeping what the agent can already use distinct from changes awaiting review.
...
![Published 2026.9.3 interface: Four fictional workspace skills, with readiness and enable controls. Fictional Harbor data; feature-focused crop.](https://github.com/user-attachments/assets/6708ac0f-5b21-415c-9f10-bfff88f30ed7)

_Four fictional workspace skills, with readiness and enable controls. Published 2026.9.3 interface with fictional Harbor data; no live task execution._
...
![Published 2026.9.3 interface: A project account is preferred; the personal account remains available. Fictional Harbor data; feature-focused crop.](https://github.com/user-attachments/assets/0f7368cf-299c-4a10-b462-f13acf947f0c)

_A project account is preferred; the personal account remains available. Published 2026.9.3 interface with fictional Harbor data; no live task execution._
...
[LM Studio](/en/providers/lmstudio) now budgets against the context of the loaded model instance, and with preload enabled sends chat to an instance that has enough room. If a new instance is loaded, the request uses its returned identity while your configured model reference stays the same. This avoids budgeting for a larger instance and then sending the conversation to a smaller one.
...
The [TaskFlow examples](/en/automation/taskflow) now use supported [Lobster](/en/tools/lobster) approval and resume flows, with child work linked through its authoritative owner. These are bounded examples using prepared synthetic batches; connecting a real inbox or pull-request source still requires its own adapter.
...
The [Browser panel](/en/tools/browser) shows page repaints as they happen, so you can follow a page loading, scrolling or responding without waiting for the next screenshot. Existing click, type, scroll, Annotate and Inspect controls remain available.
...
![Conceptual 2026.9.3 explainer: Page loads — Fictional volunteer sign-up page; Page scrolls — Follow the visible movement; Page responds — See the next repaint. Paired nodes, existing Chrome MCP sessions and unavailable streams keep screenshot fallback.](https://github.com/user-attachments/assets/9c78ed7e-33a3-4a7f-9c07-f5387371d95c)

_Browser repaints can reach the panel without waiting for another screenshot. Conceptual illustration, not a screenshot. Paired nodes, existing Chrome MCP sessions and unavailable streams keep screenshot fallback._
...
[Code Mode](/en/tools/code-mode) can keep a program running through fast tool replies and optionally check composed TypeScript calls before execution. Long conversations, session lists, delegated work, and terminal diagnostics also receive improvements that make their results easier to retain and inspect.
