...
- 将 `transport: "streamable-http"` 用于可流式传输的 HTTP MCP 服务器。`openclaw mcp set` 还将 CLI 原生的 `type: "http"` 标准化为相同的规范配置形状，以确保兼容性。
...
<Note>这些命令仅管理已保存的配置。它们不会启动渠道桥接、打开实时的 MCP 客户端会话，也不会验证目标服务器是否可达。</Note>
