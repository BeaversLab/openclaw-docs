...
`openclaw update` 没有 `--verbose` 标志。请使用 `--dry-run` 预览
计划的 渠道/tag/install/restart 操作，使用 `--json` 获取机器可读
结果，并在只需要 渠道 和
...
<Note>在 Nix 模式 (`OPENCLAW_NIX_MODE=1`) 下，禁用了变更 `openclaw update` 运行的功能。请改为更新此安装的 Nix 源码或 flake 输入；对于 nix-openclaw，请使用 agent-first [快速开始](https://github.com/openclaw/nix-openclaw#quick-start)。`openclaw update status` 和 `openclaw update --dry-run` 保持只读状态。</Note>
...
<Steps>
  ...
  <Step title="安装依赖项">使用仓库包管理器。对于 pnpm 检出，更新器会按需自举 `pnpm`（首先通过 `corepack`，然后是临时的 `npm install pnpm@11` 回退），而不是在 pnpm 工作区内运行 `npm run build`。</Step>
...
在 beta 更新渠道上，跟踪的 npm 和 ClawHub 插件安装如果遵循默认/最新主线，会首先尝试插件的 `@beta` 发布版本。如果插件没有 beta 发布版本，OpenClaw 会回退到记录的默认/最新规范并将其报告为警告。对于 npm 插件，如果 beta 包存在但安装验证失败，OpenClaw 也会回退。这些插件回退警告不会导致核心更新失败。确切的版本和显式标记不会被重写。

<Warning>如果精确固定的 npm 插件更新解析为某个构件，且其完整性与存储的安装记录不同，npm`openclaw update` 将中止该插件构件的更新，而不是安装它。仅在确认您信任新构件后，才重新安装或显式更新该插件。</Warning>

<Note>
更新后插件同步失败，如果范围限定在托管插件且同步路径可以绕过（例如，对于非必要插件无法访问 npm 注册表），则在核心更新成功后作为警告报告。JSON 结果保留顶级更新 `status: "ok"`，并使用 `openclaw doctor --fix` 和 `openclaw plugins inspect <id> --runtime --json` 指导报告 `postUpdate.plugins.status: "warning"`。意外的更新程序或同步异常仍会导致更新结果失败。修复插件安装或更新错误，然后重新运行 `openclaw doctor --fix` 或 `openclaw update`。

在每个插件同步步骤之后，`openclaw update` 在网关重启之前运行强制性的**核心后收敛** 传递：它修复丢失的配置插件有效负载，验证磁盘上的每个*活动*跟踪安装记录，并静态验证其 `package.json` 是否可解析（以及任何显式声明的 `main` 是否存在）。此传递中的失败——以及无效的 OpenClaw 配置快照——返回 `postUpdate.plugins.status: "error"` 并将顶级更新 `status` 翻转为 `"error"`，因此 `openclaw update` 以非零退出，并且网关*不会*使用未验证的插件集重启。错误包括结构化的 `postUpdate.plugins.warnings[].guidance` 行，指向 `openclaw doctor --fix` 和 `openclaw plugins inspect <id> --runtime --json` 以供后续跟进。此处跳过已禁用的插件条目和非受信任源链接的官方同步目标记录，这与丢失有效负载检查使用的 `skipDisabledPlugins` 策略相镜像，因此过时的已禁用插件记录无法阻止其他有效的更新。

当更新的 Gateway(网关) 启动时，插件加载仅为验证：启动不运行包管理器或更改依赖树。包管理器 `update.run` 重启在包树交换后绕过正常的空闲延迟和重启冷却，因此旧进程无法继续延迟加载已删除的块。

如果 pnpm bootstrap 仍然失败，更新程序将提前停止并显示特定于包管理器的错误，而不是尝试 `npm run build` 在检出版本内。

</Note>

## `--update` 简写

`openclaw --update` 会被重写为 `openclaw update` （对 Shell 和启动器脚本很有用）。
