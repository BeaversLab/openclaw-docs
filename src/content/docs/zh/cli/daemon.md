...
- 如果令牌身份验证未实际生效（显式 `gateway.auth.mode` 为 `password`/`none`/`trusted-proxy`，或者未设置模式且密码可能胜出而没有令牌候选者可能胜出），令牌漂移检查将跳过配置令牌解析。
...
有关最新的文档和示例，请使用 [`openclaw gateway`](/en/cli/gateway)。
