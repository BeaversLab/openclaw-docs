...
- `status --deep` 添加了尽最大努力进行的系统级服务扫描。当发现其他类似网关的服务时，人类可读的输出会打印清理提示，并警告每台机器运行一个网关仍然是常规建议。
...
- 如果令牌身份验证未有效激活（显式 `gateway.auth.mode` 为 `password`/`none`/`trusted-proxy`，或者模式未设置，即密码可能胜出而没有令牌候选者可以胜出），令牌漂移检查将跳过配置令牌解析。
