...
- 会话状态输出将 `Execution:` 与 `Runtime:` 分开。`Execution` 是沙盒路径（`direct`、`docker/*`），而 `Runtime` 告诉你会话是否使用 `OpenClaw Pi Default`、`OpenAI Codex`CLI、CLI 后端或 ACP 后端（如 `codex (acp/acpx)`）。有关提供商/模型/运行时区别的信息，请参阅 [Agent runtimes](/zh/concepts/agent-runtimes)。
...
- 对于提示词大小统计，当会话元数据缺失或较小时，记录回退优先选择较大的面向提示词的总数，以便自定义提供商会话不会退化为 `0` 令牌显示。
