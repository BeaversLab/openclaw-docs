...
- 使用 `/status` 进行快速诊断。
- 使用 `/config` 进行持久化配置更改。
- 使用 `/debug` 进行仅运行时配置覆盖（内存中，非磁盘；需要 `commands.debug: true`）。
...
- 如果不使用 `--install` 或 `--write-state`，`completion` 会将脚本打印到标准输出。
...
- `--wizard`：运行新手引导。
- `--non-interactive`：运行新手引导，不显示提示。
- `--mode <local|remote>`：引导模式。
- `--remote-url <url>`：远程 Gateway(网关) URL。
- `--remote-token <token>`：远程 Gateway(网关) 令牌。

当存在任何引导标志时，新手引导将自动运行（`--non-interactive`、`--mode`、`--remote-url`、`--remote-token`）。
...
- `--workspace <dir>`
- `--reset`（在引导之前重置配置 + 凭证 + 会话）
- `--reset-scope <config|config+creds+sessions|full>`（默认为 `config+creds+sessions`；使用 `full` 也可以删除工作区）
- `--non-interactive`
- `--mode <local|remote>`
...
- Qwen 注意：`qwen-*` 是规范的 auth-choice 系列。`modelstudio-*`
  id 仍被接受仅为传统兼容性的别名。
- `--secret-input-mode <plaintext|ref>`（默认 `plaintext`；使用 `ref` 存储提供商默认环境引用而非明文密钥）
...
- `--node-manager <npm|pnpm|bun>`（设置/新手引导技能的节点管理器；推荐使用 pnpm，也支持 bun）
- `--json`
...
非交互式配置辅助工具（get/set/unset/file/schema/validate）。在不带子命令的情况下运行 `openclaw config` 将启动向导。
...
- `config set --dry-run --json`：发出机器可读的试运行输出（检查 + 完整性信号、操作、已检查/跳过的引用、错误）。
- `config set --strict-json`：要求对路径/值输入进行 JSON5 解析。`--json` 仍然是试运行输出模式之外严格解析的遗留别名。
...
- 对于由 SecretRef 管理的网关令牌，`dashboard` 会打印或打开非令牌化的 URL，而不是在终端输出或浏览器启动参数中暴露该密钥。
...
- `channels status`：检查网关可达性和渠道健康状况（当网关可达时，`--probe` 会运行实时的逐账户探测/审计检查；如果不可达，则会回退到仅配置的渠道摘要。请使用 `openclaw health` 或 `openclaw status --deep` 进行更广泛的网关健康探测）。
- 提示：当 `channels status` 检测到常见的错误配置时，会打印带有建议修复的警告（然后指向 `openclaw doctor`）。
- `channels logs`：显示网关日志文件中的近期渠道日志。
- `channels add`：未传递标志时为向导式设置；标志切换到非交互模式。
  - 当向仍在使用单账户顶层配置的渠道添加非默认账户时，OpenClaw 会在写入新账户之前将账户范围的值提升到渠道账户映射中。大多数渠道使用 `accounts.default`；Matrix 可以保留现有的匹配命名/默认目标。
...
- `skills install <slug>`：从 ClawHub 将技能安装到当前工作区。
...
- `skills check --json`：在标准输出上输出机器可读的就绪状态信息。
...
提示：对于 ClawHub 托管的技能，请使用 `openclaw skills search`、`openclaw skills install` 和 `openclaw skills update`。
...
- 如果仅配置了一个支持配对的渠道，则还允许使用 `pairing approve <code>`。
...
根据当前的 Gateway(网关) 配置生成移动配对二维码和设置代码。请参阅 [`openclaw qr`](/en/cli/qr)。
...
- 内置引导移交会将主节点令牌保留在 `scopes: []`。
- 任何移交的操作员引导令牌都仅限于 `operator.approvals`、`operator.read`、`operator.talk.secrets` 和 `operator.write`。
...
- 无法通过 `openclaw hooks` 启用或禁用插件管理的钩子；请改为启用或禁用所属插件。
- `hooks install` 和 `hooks update` 仍作为兼容性别名起作用，但它们会打印弃用警告并转发到插件命令。

### `webhooks`

Webhook 辅助工具。当前内置界面为 Gmail Pub/Sub 设置 + 运行程序：
...
Gmail Pub/Sub 钩子设置 + 运行程序。请参阅 [Gmail Pub/Sub](/en/automation/cron-jobs#gmail-pubsub-integration)。
...
- `run` 使用可选的运行时覆盖启动本地 Gmail 监视/续订循环。
...
- `--apply` 目前仅支持通过 Homebrew 安装 CoreDNS 的 macOS。
...
通过 Gateway(网关)（或嵌入式 `--local`）运行一轮代理。
...
- `--local` 仍会预加载插件注册表，因此插件提供的提供商、工具和渠道在嵌入式运行期间仍然可用。
- `--channel`、`--reply-channel` 和 `--reply-account` 影响回复传送，而非路由。
...
添加一个新的隔离代理。除非传递了标志（或 `--non-interactive`），否则运行向导；在非交互模式下需要 `--workspace`。
...
绑定规范使用 `channel[:accountId]`。当省略 `accountId` 时，OpenClaw 可能会通过渠道默认值/插件挂钩解析账号范围；否则这是没有显式账号范围的渠道绑定。
传递任何显式添加标志会将命令切换到非交互路径。`main` 已被保留，不能用作新代理 ID。
...
- 当未提供显式身份字段时，该命令将读取 `IDENTITY.md`。

### `acp`

运行将 IDE 连接到 Gateway(网关) 的 ACP 网桥。
...
管理已保存的 MCP 服务器定义，并通过 MCP stdio 公开 OpenClaw 渠道。

#### `mcp serve`

通过 MCP stdio 公开已路由的 OpenClaw 渠道对话。
...
使用文件或 stdin 中的 JSON 替换 exec 批准。
...
- 概述在可用时包含 Gateway(网关) + 节点主机服务状态。
- `--usage` 将标准化的提供商使用时间窗口打印为 `X% left`。

### 使用情况跟踪

OpenClaw 可在 OAuth/API 凭据可用时显示提供商使用情况/配额。
...
- MiniMax 备注：原始 `usage_percent` / `usagePercent` 表示剩余配额，因此 OpenClaw 在显示前会将其反转；如果存在基于计数的字段，则仍优先使用。`model_remains` 响应优先选择聊天模型条目，在需要时从时间戳推导窗口标签，并在计划标签中包含模型名称。
- 使用情况身份验证在可用时来自提供商特定的钩子；否则 OpenClaw 会回退到从身份验证配置文件、环境变量或配置中匹配 OAuth/API 密钥凭据。如果未解析到任何凭据，使用情况将被隐藏。
...
- 默认 `health` 可以返回一个新的缓存网关快照。
- `health --verbose` 强制进行实时探测，并在所有配置的帐户和代理上展开可读输出。
...
- `sessions cleanup` 还支持 `--fix-missing` 以清除转录文件已消失的条目。
...
列出并管理各代理上的 [后台任务](/en/automation/tasks) 运行。
...
- `gateway status` 默认使用服务解析的端口/配置探测 Gateway(网关) RPC（可使用 `--url/--token/--password` 覆盖）。
- `gateway status` 支持用于脚本编写的 `--no-probe`、`--deep`、`--require-rpc` 和 `--json`。
- `gateway status` 还会在检测到旧版或额外的网关服务时将其显示出来（`--deep` 会添加系统级扫描）。以配置文件命名的 OpenClaw 服务被视为一流服务，不会被标记为“额外”。
- 即使本地 CLI 配置缺失或无效，`gateway status` 仍可用于诊断。
- `gateway status` 会打印解析后的文件日志路径、CLI 与服务的配置路径/有效性快照，以及解析后的探测目标 URL。
- 如果网关认证 SecretRefs 在当前命令路径中未解析，则 `gateway status --json` 仅在探测连接性/认证失败时报告 `rpc.authWarning`（探测成功时将抑制警告）。
...
Gateway(网关) 服务管理命令的旧版别名。请参阅 [/cli/daemon](/en/cli/daemon)。
...
- `gateway status --deep` 添加系统级服务扫描。使用 `gateway probe`、
  `health --verbose` 或顶级 `status --deep` 以获取更深入的运行时探测详细信息。

常见 RPC：

- `config.schema.lookup` （使用浅层架构节点检查一个配置子树、匹配的提示元数据和直接子项摘要）
...
提示：对于部分编辑，请先使用 `config.schema.lookup` 检查，并优先使用 `config.patch`。
提示：这些配置写入 RPC 会对提交的配置负载中的 SecretRef 引用进行预检查，以解析活动的 SecretRef，并在有效的活动提交的引用未解析时拒绝写入。
提示：仅所有者拥有的 `gateway` 运行时工具仍然拒绝重写 `tools.exec.ask` 或 `tools.exec.security`；旧的 `tools.bash.*` 别名会规范化为相同的受保护执行路径。

## 模型

有关回退行为和扫描策略，请参阅 [/concepts/models](/en/concepts/models)。

Anthropic 注意：Anthropic 员工告知我们，再次允许 OpenClaw 风格的 Claude CLI 使用，因此 OpenClaw 将 Claude CLI 重用和 `claude -p` 使用视为对此集成的认可，除非 Anthropic 发布新策略。对于生产环境，首选 Anthropic API 密钥或其他支持的订阅制提供商（例如 OpenAI Codex、阿里云模型工作室编码计划、OpenAI 编码计划或 Z.AI / MiniMax 编码计划）。

Anthropic setup-token 仍然是一种受支持的令牌认证路径，但 OpenClaw 现在更倾向于在可用时重用 Claude CLI 和 `claude -p`。
...
预计探测状态如 `ok`、`auth`、`rate_limit`、`billing`、`timeout`、
...
- `setup-token` 和 `paste-token` 是暴露令牌身份验证方法的提供商的通用令牌命令。
- `setup-token` 需要交互式 TTY 并运行提供商的令牌身份验证方法。
- `paste-token` 提示输入令牌值，当省略 `--profile-id` 时默认为身份验证配置文件 ID `<provider>:manual`。
- Anthropic `setup-token` / `paste-token` 仍作为受支持的 OpenClaw 令牌路径可用，但如果可用，OpenClaw 现在更倾向于 Claude CLI 重用和 `claude -p`。
...
将系统事件加入队列并选择性地触发心跳（Gateway(网关) RPC）。
...
- `cron add`（别名：`create`；需要 `--name` 以及 `--at`、`--every` 或 `--cron` 中的 exactly one，以及 `--system-event` 或 `--message` 中的一个有效负载）
- `cron edit <id>`（修补字段）
...
`cron add|edit --model ...` 使用为该作业选择的允许模型。如果该模型不被允许，cron 会发出警告并回退到作业的代理/默认模型选择。配置的回退链仍然适用，但没有显式按作业回退列表的普通模型覆盖不再将代理主要选项追加为隐藏的额外重试目标。
...
- `node` 从环境/配置解析网关身份验证（无 `--token`/`--password` 标志）：`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`，然后是 `gateway.auth.*`。在本地模式下，节点主机故意忽略 `gateway.remote.*`；在 `gateway.mode=remote` 中，`gateway.remote.*` 根据远程优先级规则参与。
- 节点主机身份验证解析仅遵守 `OPENCLAW_GATEWAY_*` 环境变量。

## 节点

`nodes` 与 Gateway(网关) 通信并以配对的节点为目标。请参阅 [/nodes](/en/nodes)。
...
插件提供的语音通话实用工具。仅在安装并启用了语音通话插件时显示。参见[`openclaw voicecall`](/en/cli/voicecall)。
