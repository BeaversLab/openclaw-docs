...
管理用于隔离代理执行的沙箱运行时。请参阅 [/cli/sandbox](/en/cli/sandbox)。
...
查找公开目录界面的渠道的自有 ID、对等方 ID 和组 ID。请参阅 [`openclaw directory`](/en/cli/directory)。
...
从当前的 Gateway(网关) 配置生成移动配对二维码和设置代码。请参阅 [`openclaw qr`](/en/cli/qr)。
...
Gmail Pub/Sub 挂钩设置 + 运行器。请参阅 [Gmail Pub/Sub](/en/automation/cron-jobs#gmail-pubsub-integration)。
...
列出并管理跨代理的[后台任务](/en/automation/tasks)运行。
...
Gateway(网关) 服务管理命令的遗留别名。请参阅 [/cli/daemon](/en/cli/daemon)。
...
插件提供的语音通话工具。仅在安装并启用语音通话插件时显示。请参阅 [`openclaw voicecall`](/en/cli/voicecall)。
