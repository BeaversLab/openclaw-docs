...
管理沙箱运行时以进行隔离的代理执行。请参阅 [/cli/sandbox](/zh/cli/sandbox)。
...
查找公开目录界面的频道的自身、对等方和组 ID。请参阅 [`openclaw directory`](/zh/cli/directory)。
...
从当前 Gateway(网关) 配置生成移动配对二维码和设置代码。请参阅 [`openclaw qr`](/zh/cli/qr)。
...
Gmail Pub/Sub 钩子设置 + 运行器。请参阅 [Gmail Pub/Sub](/zh/automation/cron-jobs#gmail-pubsub-integration)。
...
广域网发现 DNS 助手（CoreDNS + Tailscale）。请参阅 [/gateway/discovery](/zh/gateway/discovery)。
...
跨代理列出和管理 [background task](/zh/automation/tasks) 运行。
...
Gateway 服务管理命令的旧式别名。请参阅 [/cli/daemon](/zh/cli/daemon)。
...
`--all` 包含捆绑的提供商拥有的静态目录行，在配置身份验证之前。在存在匹配的提供商凭据之前，这些行将保持不可用。
...
- `setup-token` 和 `paste-token` 是为公开令牌身份验证方法的提供商提供的通用令牌命令。
- `setup-token` 需要交互式 TTY 并运行提供商的令牌身份验证方法。
- `paste-token` 会提示输入令牌值，并在省略 `--profile-id` 时默认使用身份验证配置文件 ID `<provider>:manual`。
- Anthropic `setup-token` / `paste-token` 仍然作为受支持的 OpenClaw 令牌路径可用，但如果可用，OpenClaw 现在更倾向于重用 Claude CLI 和 `claude -p`。
...
将系统事件加入队列，并选择性地触发心跳（Gateway RPC）。
...
管理定时任务（Gateway RPC）。请参阅 [/automation/cron-jobs](/zh/automation/cron-jobs)。

子命令：

- `cron status [--json]`
- `cron list [--all] [--json]`（默认为表格输出；使用 `--json` 获取原始输出）
- `cron add` （别名：`create`；需要 `--name` 并且需要 `--at` | `--every` | `--cron` 中的恰好一个，以及 `--system-event` | `--message` 中的恰好一个有效负载）
- `cron edit <id>` （补丁字段）
...
`cron add|edit --model ...` 使用为任务选定的允许模型。如果该模型不被允许，cron 会发出警告并回退到任务的代理/默认模型选择。已配置的回退链仍然适用，但不包含明确每任务回退列表的纯模型覆盖不再将代理主节点作为隐藏的额外重试目标追加。
...
- `node` 从环境变量/配置中解析 Gateway(网关) 身份验证（没有 `--token`/`--password` 标志）：`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`，然后是 `gateway.auth.*`。在本地模式下，节点主机有意忽略 `gateway.remote.*`；在 `gateway.mode=remote` 中，`gateway.remote.*` 根据远程优先级规则参与解析。
- 节点主机身份验证解析仅遵从 `OPENCLAW_GATEWAY_*` 环境变量。

## 节点

`nodes` 与 Gateway(网关) 通信并针对已配对的节点。请参阅 [/nodes](/zh/nodes)。
...
插件提供的语音通话实用程序。仅在安装并启用了语音通话插件时出现。请参阅 [`openclaw voicecall`](/zh/cli/voicecall)。
