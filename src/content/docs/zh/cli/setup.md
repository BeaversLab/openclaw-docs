...
<Note>`openclaw setup` 适用于可变配置安装。在 Nix 模式（`OPENCLAW_NIX_MODE=1`）下，OpenClaw 拒绝写入设置，因为配置文件由 Nix 管理。请使用官方的 [nix-openclaw 快速开始](https://github.com/openclaw/nix-openclaw#quick-start) 或其他 Nix 软件包的等效源配置。</Note>
...
只要明确存在以下任一标志，即使没有 `--wizard`，`openclaw setup` 也会运行向导：
