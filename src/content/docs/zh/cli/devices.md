...
轮换特定角色的设备令牌（可选择更新作用域）。目标角色必须已存在于该设备的已批准配对合约中；轮换无法创建新的未批准角色。如果您省略 `--scope`，稍后使用存储的轮换令牌重新连接时将重用该令牌缓存的已批准作用域。如果您传递显式的 `--scope` 值，这些值将成为未来缓存令牌重新连接的存储作用域集。非管理员配对设备调用者只能轮换他们**自己的**设备令牌。此外，任何显式的 `--scope` 值必须保持在调用者会话自己的操作员作用域内；轮换无法创建比调用者已有的更广泛的操作员令牌。
...
非管理员配对设备调用者只能撤销他们**自己的**设备令牌。撤销其他设备的令牌需要 `operator.admin`。
...
- 受信任的 `AUTH_TOKEN_MISMATCH` 恢复可以针对这一次有界的重试，临时同时发送共享令牌和存储的设备令牌。
