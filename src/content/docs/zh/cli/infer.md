...
| 描述图片文件或 URL | `openclaw infer image describe --file ./image.png --prompt "..." --json`                      | `--model` 必须是具备图像功能的 `<provider/model>` |
...
- 常规的本地路径不需要网关正在运行。
- 本地 `model run` 是一种精简的一次性提供商补全。它解析配置的代理模型和身份验证，但不会启动聊天代理轮次、加载工具或打开捆绑的 MCP 服务器。
- `model run --file` 接受图像文件，检测其 MIME 类型，并将其与提供的提示一起发送给选定的模型。对多个图像重复 `--file`。
- `model run --file` 拒绝非图像输入。对于音频文件使用 `infer audio transcribe`，对于视频文件使用 `infer video describe`。
- `model run --gateway`Gateway(网关) 执行 Gateway(网关) 路由、保存的身份验证、提供商选择和嵌入式运行时，但仍作为原始模型探针运行：它发送提供的提示和任何图像附件，而无需先前的会话记录、引导/AGENTS 上下文、上下文引擎组装、工具或捆绑的 MCP 服务器。
- `model run --gateway --model <provider/model>`Gateway(网关) 需要受信任的操作员网关凭据，因为该请求要求 Gateway(网关) 运行一次性提供商/模型覆盖。
- 本地 `model run --thinking` 使用精简的提供商补全路径；特定于提供商的级别（例如 `adaptive` 和 `max`）被映射到最接近的可移植简单补全级别。
...
使用完整的 `<provider/model>`Gateway(网关) 引用来对特定提供商进行冒烟测试，而无需
启动 Gateway(网关) 或加载完整的 agent 工具 surface：
