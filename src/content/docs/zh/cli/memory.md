...
如果 `memory status` 显示 `Dreaming status: blocked`，则说明受管理的梦境处理 cron 已启用，但驱动它的心跳针对默认代理程序未触发。有关两个常见原因，请参阅 [Dreaming never runs](/zh/concepts/dreaming#dreaming-never-runs-status-shows-blocked)。
...
- 使用 `dreaming.frequency` 调整计划的扫描频率。深度提升策略通常是内部的，但 `dreaming.phases.deep.maxPromotedSnippetTokens`CLI 除外，它在保持来源可见的同时限制提升的片段长度。当需要一次性手动覆盖阈值时，请在 `memory promote` 上使用 CLI 标志。
- `memory rem-harness --path <file-or-dir> --grounded` 预览历史每日笔记中的基于事实的 `What Happened`、`Reflections` 和 `Possible Lasting Updates`，而无需写入任何内容。
- `memory rem-backfill --path <file-or-dir>` 将可逆的基于事实的日记条目写入 `DREAMS.md` 以供 UI 审查。
- `memory rem-backfill --path <file-or-dir> --stage-short-term` 还会将基于事实的持久候选内容植入到实时的短期提升存储中，以便正常的深度阶段可以对它们进行排名。
- `memory rem-backfill --rollback` 移除先前写入的基于事实的日记条目，而 `memory rem-backfill --rollback-short-term` 移除先前暂存的基于事实的短期候选内容。
