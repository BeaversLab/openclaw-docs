---
summary: "CLI 参考，用于 `openclaw memory` (status/index/search/promote)"
read_when:
...
- `--agent <id>`：将范围限定为单个代理。如果没有它，这些命令将为每个配置的代理运行；如果未配置代理列表，它们将回退到默认代理。
...
- 使用来自记忆回溯和每日摄取传递的短期信号，加上浅层/REM 阶段增强信号。
- 启用做梦时，`memory-core` 会自动管理一个 cron 任务，该任务在后台运行全面扫描（`light -> REM -> deep`）（无需手动 `openclaw cron add`）。
- `--agent <id>`：范围限定为单个智能体（默认：默认智能体）。
- `--limit <n>`：要返回/应用的最大候选项数。
- `--min-score <n>`：最低加权提升分数。
- `--min-recall-count <n>`：候选项所需的最低回溯次数。
- `--min-unique-queries <n>`：候选项所需的最低不同查询数。
- `--apply`：将选定的候选项追加到 `MEMORY.md` 并将其标记为已提升。
- `--include-promoted`：在输出中包含已提升的候选项。
- `--json`：打印 JSON 输出。

## 做梦（实验性）

做梦是后台记忆巩固系统，具有三个协作阶段：**浅层**（排序/暂存短期材料）、**深层**（将持久事实提升到 `MEMORY.md`）和 **REM**（反思并呈现主题）。

- 使用 `plugins.entries.memory-core.config.dreaming.enabled: true` 启用。
- 从聊天中使用 `/dreaming on|off` 切换（或使用 `/dreaming status` 检查）。
- 做梦在一个管理的扫描计划（`dreaming.frequency`）上运行，并按顺序执行阶段：浅层、REM、深层。
- 只有深层阶段会将持久记忆写入 `MEMORY.md`。
- 人类可读的阶段输出和日记条目被写入 `DREAMS.md`（或现有的 `dreams.md`），并在 `memory/dreaming/<phase>/YYYY-MM-DD.md` 中提供可选的每阶段报告。
- 排名使用加权信号：召回频率、检索相关性、查询多样性、时间近度、跨日整合以及衍生概念的丰富度。
- 在写入 `MEMORY.md` 之前，提升操作会重新读取实时每日笔记，因此已编辑或已删除的短期片段不会从过时的召回存储快照中被提升。
- 除非您传递 CLI 阈值覆盖参数，否则定期和手动 `memory promote` 运行共享相同的深度阶段默认值。
- 自动运行会在配置的内存工作区中展开。
...
- 如果有效活动的内存远程 API 密钥字段被配置为 SecretRefs，该命令会从活动网关快照中解析这些值。如果网关不可用，该命令将快速失败。
- Gateway(网关) 版本偏差说明：此命令路径需要支持 `secrets.resolve` 的网关；较旧的网关会返回未知方法错误。
- 使用 `dreaming.frequency` 调整定期扫描频率。深度提升策略在内部是固定的；当需要一次性手动覆盖时，请使用 CLI 标志于 `memory promote`。
