...
Crestodian 使用与普通代理相同的 OpenClaw 设备发现引用。在 Git 检出中，它指向本地 OpenClaw`docs/`npm 和本地源代码树。在 npm 包安装中，它使用捆绑的包文档并链接到 [https://github.com/openclaw/openclaw](https://github.com/openclaw/openclaw)，并明确指导在文档不足时查看源代码。
...
- Claude Code CLI：CLI`claude-cli/claude-opus-4-7`
- Codex app-server harness：`openai/gpt-5.5`
- Codex CLI：CLI`codex-cli/gpt-5.5`

模型辅助规划器无法直接更改配置。它必须将请求转换为 Crestodian 的类型化命令之一，然后应用正常的批准和审计规则。Crestodian 在运行任何内容之前会打印它使用的模型和解释的命令。无配置的回退规划器轮转是临时的，在运行时支持的情况下禁用工具，并使用临时工作区/会话。

消息渠道救援模式不使用模型辅助规划器。远程救援保持确定性，因此损坏或被劫持的正常代理路径不能被用作配置编辑器。
...
`openclaw tui`、`openclaw chat` 和 `openclaw terminal`TUI 仍然直接打开正常的代理 TUI。它们不会启动 Crestodian。

切换到正常的 TUI 后，使用 TUI`/crestodian` 返回 Crestodian。您可以包含后续请求：
...
在 TUI 内部的代理切换会留下一个面包屑，指示 TUI`/crestodian` 可用。

## 消息救援模式

消息救援模式是 Crestodian 的消息渠道入口点。它适用于正常代理已死机，但受信任的渠道（如 WhatsApp）仍然接收命令的情况。
...
远程救援模式是一个管理界面。必须将其视为远程配置
修复，而不是正常聊天。

远程救援的安全约定：

- 当沙箱隔离激活时禁用。如果代理/会话处于沙箱隔离中，
  Crestodian 必须拒绝远程救援并说明需要本地 CLI
  修复。
- 默认有效状态为 `auto`：仅允许在受信任的 YOLO
  操作中进行远程救援，此时运行时已具有非沙箱隔离的本地权限。
- 需要明确的所有者身份。救援不得接受通配符发送者
  规则、开放组策略、未经身份验证的 Webhook 或匿名渠道。
- 默认仅限所有者私信。组/渠道救援需要明确选择加入。
- 插件搜索和列表是只读的。插件安装默认仅限本地，
  因为它会下载可执行代码。当救援策略允许持久写入时，
  可以允许插件卸载作为已批准的修复操作。
- 远程救援无法打开本地 TUI 或切换到交互式代理
  会话。请使用本地 `openclaw` 进行代理移交。
- 即使在救援模式下，持久写入仍然需要批准。
- 审核每个应用的救援操作。消息渠道救援记录渠道、
  账户、发送方和源地址元数据。配置变更操作还会
  记录前后的配置哈希。
- 绝不回显机密。SecretRef 检查应报告可用性，而非
  值。
- 如果 Gateway(网关) 存活，首选 Gateway(网关) 类型的操作。如果 Gateway(网关)
  死亡，请仅使用不依赖
  正常代理循环的最小本地修复界面。
...
一个可选的实时渠道命令界面冒烟测试 `/crestodian status` 加上通过救援处理程序的持久批准往返：

```bash
pnpm test:live:crestodian-rescue-channel
```

通过 Crestodian 进行全新的无配置设置由以下内容覆盖：

```bash
pnpm test:docker:crestodian-first-run
```

该车道以空状态目录开始，将裸 `openclaw`Discord 路由到 Crestodian，设置默认模型，创建额外的代理，通过插件启用加令牌 SecretRef 配置 Discord，验证配置，并检查审计日志。QA Lab 也有针对同一 Ring 0 流程的仓库支持场景：
