...
有关快速入门、QA运行程序、单元/集成套件和Docker流程，请参阅[测试](/zh/help/testing)。本页面涵盖**实时**（网络接触）测试套件：模型矩阵、CLI后端、ACP和媒体提供商实时测试，以及凭证处理。
...
- 完整的 Android 设置详情：[Android 应用程序](/zh/platforms/android)

## 实时：模型冒烟（配置文件密钥）

实时测试分为两层，以便我们能够隔离故障：

- “直接模型”告诉我们提供商/模型是否可以使用给定的密钥进行回答。
- “Gateway(网关) smoke”告诉我们完整的 gateway+agent 管道对该模型（会话、历史记录、工具、沙箱策略等）有效。
...
- How to select models:
  - `OPENCLAW_LIVE_MODELS=modern` 运行 modern allowlist（Opus/Sonnet 4.6+、GPT-5.2 + Codex、Gemini 3、DeepSeek V4、GLM 4.7、MiniMax M2.7、Grok 4）
  - `OPENCLAW_LIVE_MODELS=all` 是 modern allowlist 的别名
  - 或 `OPENCLAW_LIVE_MODELS="openai/gpt-5.2,openai-codex/gpt-5.2,anthropic/claude-opus-4-6,..."`（逗号 allowlist）
...
- 注：
  - 该通道使用带有管理员专用的合成 originating-route 字段的网关 `chat.send` 接口，以便测试可以在不伪装外部投递的情况下附加消息渠道上下文。
  - 当 `OPENCLAW_LIVE_ACP_BIND_AGENT_COMMAND` 未设置时，测试使用嵌入的 `acpx` 插件的内置代理注册表来获取所选的 ACP harness 代理。
  - 绑定会话 cron MCP 创建默认情况下是尽力而为的，因为外部 ACP harness 可以在绑定/镜像证明通过后取消 MCP 调用；设置 `OPENCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1` 以使该绑定后 cron 探测变为严格模式。
...
- 默认情况下，它依次针对聚合的实时 CLI 代理运行 ACP bind smoke：`claude`、`codex`，然后是 `gemini`。
...
- 它从 `~/.profile` 获取资源，将匹配的 CLI 认证材料暂存到容器中，然后安装请求的实时 CLI（`@anthropic-ai/claude-code`、`@openai/codex`、通过 `https://app.factory.ai/cli` 的 Factory Droid、`@google/gemini-cli` 或 `opencode-ai`）（如果缺失）。ACP 后端本身是 `acpx` 插件中捆绑的嵌入式 `acpx/runtime` 包。
- Droid Docker 变体暂存 `~/.factory` 作为设置，转发 `FACTORY_API_KEY`，并且需要该 API 密钥，因为本地 Factory OAuth/keyring 认证无法移植到容器中。它使用 ACPX 的内置 `droid exec --output-format acp` 注册表条目。
- OpenCode Docker 变体是一个严格单代理回归通道。它在获取 `~/.profile` 后，从 `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL`（默认 `opencode/kimi-k2.6`）写入一个临时的 `OPENCODE_CONFIG_CONTENT` 默认模型，并且 `pnpm test:docker:live-acp-bind:opencode` 需要一个绑定的助手转录记录，而不是接受通用的绑定后跳过。
- 直接 `acpx` CLI 调用只是在 Gateway 之外比较行为的一种手动/变通方法。Docker ACP 绑定冒烟测试会练习 OpenClaw 的嵌入式 `acpx` 运行时后端。

## Live：Codex 应用服务器 套具冒烟测试

- 目标：通过正常的 gateway `agent` 方法验证插件拥有的 Codex 套具：
  - 加载捆绑的 `codex` 插件
  - 选择 `OPENCLAW_AGENT_RUNTIME=codex`
  - 向 `openai/gpt-5.2` 发送第一个 Gateway 代理轮次，并强制使用 Codex 套具
  - 向同一个 OpenClaw 会话发送第二个轮次，并验证应用服务器线程可以恢复
  - 通过同一个 gateway 命令路径运行 `/codex status` 和 `/codex models`
  - （可选）运行两个 Guardian 审查的升级 shell 探针：一个应该被批准的良性命令，和一个应该被拒绝的虚假机密上传，以便代理反向询问
...
- 该冒烟测试设置了 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`，因此损坏的 Codex 套具无法通过静默回退到 PI 来通过测试。
- 认证：来自本地 Codex 订阅登录的 Codex 应用服务器认证。Docker 冒烟测试还可以在适用时为非 Codex 探针提供 `OPENAI_API_KEY`，以及可选复制的 `~/.codex/auth.json` 和 `~/.codex/config.toml`。
...
- 它获取已挂载的 `~/.profile`，传递 `OPENAI_API_KEY`，在存在时复制 Codex CLI 身份验证文件，将 `@openai/codex` 安装到可写的已挂载 npm 前缀中，暂存源代码树，然后仅运行 Codex-harness 实时测试。
- Docker 默认启用镜像、MCP/工具和 Guardian 探测。当您需要更窄的调试运行时，请设置 `OPENCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` 或 `OPENCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=0` 或 `OPENCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0`。
- Docker 还导出 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`，以匹配实时测试配置，这样旧的别名或 PI 回退就不会掩盖 Codex harness 的回归问题。
...
没有固定的“CI 模型列表”（实时测试是可选加入的），但这些是在带有密钥的开发机器上定期覆盖的**推荐**模型。
...
在 `OPENCLAW_LIVE_GATEWAY_MODELS` 中至少包含一个支持图片的模型 (Claude/Gemini/OpenAI 支持视觉的变体等)，以测试图片探测功能。
...
<Tip>不要在文档中硬编码“所有模型”。权威列表是 `discoverModels(...)` 在你的机器上返回的内容加上任何可用的密钥。</Tip>
...
- 如果实时测试显示“no creds”（无凭证），请像调试 `openclaw models list` / 模型选择那样进行调试。

- 按代理划分的身份验证配置文件：`~/.openclaw/agents/<agentId>/agent/auth-profiles.json`（这就是“profile keys”在实时测试中的含义）
- 配置：`~/.openclaw/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
- 旧版状态目录：`~/.openclaw/credentials/`（如果存在，会复制到暂存的实时主目录中，但不是主要的配置文件密钥存储）
- 实时本地运行默认会将活动配置、按代理划分的 `auth-profiles.json` 文件、旧版 `credentials/` 以及支持的外部 CLI 身份验证目录复制到临时测试主目录中；暂存的实时主目录会跳过 `workspace/` 和 `sandboxes/`，并且 `agents.*.workspace` / `agentDir` 路径覆盖会被剥离，以便探针不会干扰你真实的主机工作区。

如果你想依赖环境变量密钥（例如在你的 `~/.profile` 中导出），请在 `source ~/.profile` 之后运行本地测试，或使用下面的 Docker 运行器（它们可以将 `~/.profile` 挂载到容器中）。
...
- 范围：
  ...
  - 默认情况下优先使用实时/环境 API 密钥而非存储的身份验证配置文件，因此 `auth-profiles.json` 中的陈旧测试密钥不会掩盖真实的 Shell 凭据
...
- 可选身份验证行为：
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` 以强制配置文件存储身份验证并忽略仅环境变量的覆盖

对于已发布的 CLI 路径，在提供商/运行时直播测试通过后添加 `infer` 冒烟测试：
...
这涵盖了 CLI 参数解析、配置/默认代理解析、捆绑插件激活、按需捆绑运行时依赖项修复、共享图像生成运行时以及实时提供商请求。
...
- 范围：
  ...
  - 如果可用，运行两种声明的运行时模式：
    - `generate` 仅提示输入
    - 当提供商声明 `capabilities.edit.enabled` 时，`edit`
  - 当前共享通道覆盖范围：
...
- 可选身份验证行为：
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` 强制使用配置文件存储的身份验证并忽略仅环境的覆盖
...
- 范围：
  - 练习共享捆绑视频生成提供商路径
  - 默认为发布安全的冒烟测试路径：非 FAL 提供商，每个提供商一个文本转视频请求，一秒龙虾提示，以及来自 `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` 的每个提供商操作上限（默认为 `180000`）
  - 默认跳过 FAL，因为提供商端的队列延迟可能会主导发布时间；传递 `--video-providers fal` 或 `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` 以显式运行它
  - 在探测之前，从您的登录 shell 加载提供商环境变量（`~/.profile`）
  - 默认优先使用实时/环境的 API 密钥而非已存储的身份验证配置文件，因此 `auth-profiles.json` 中过时的测试密钥不会掩盖真实的 Shell 凭证
  - 跳过没有可用身份验证/配置文件/模型的提供商
  - 默认仅运行 `generate`
  - 设置 `OPENCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` 以在可用时同时运行声明的转换模式：
    - 当提供商声明 `capabilities.imageToVideo.enabled` 且选定的提供商/模型在共享扫描中接受支持缓冲区的本地图像输入时，`imageToVideo`
    - 当提供商声明 `capabilities.videoToVideo.enabled` 且选定的提供商/模型在共享扫描中接受支持缓冲区的本地视频输入时，`videoToVideo`
  - 共享扫描中当前已声明但跳过的 `imageToVideo` 提供商：
    - `vydra`，因为内置的 `veo3` 仅支持文本，且内置的 `kling` 需要远程图像 URL
  - 特定于提供商的 Vydra 覆盖范围：
    - `OPENCLAW_LIVE_TEST=1 OPENCLAW_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
    - 该文件默认运行 `veo3` 文本生成视频，外加一条默认使用远程图像 URL 固定装置的 `kling` 通道
  - 当前的 `videoToVideo` 实时覆盖范围：
    - 仅当选定的模型是 `runway/gen4_aleph` 时 `runway`
  - 共享扫描中当前已声明但跳过的 `videoToVideo` 提供商：
    - `alibaba`、`qwen`、`xai`，因为这些路径当前需要远程 `http(s)` / MP4 参考 URL
    - `google`，因为当前的共享 Gemini/Veo 通道使用本地支持缓冲区的输入，而该路径在共享扫描中不被接受
    - `openai`，因为当前的共享通道缺乏特定于组织的视频重绘/混合访问保证
- 可选的缩小范围：
  - `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="google,openai,runway"`
  - `OPENCLAW_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `OPENCLAW_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` 以将默认扫描中的每个提供商（包括 FAL）都包含在内
  - `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` 以降低每个提供商的操作上限，从而进行激进的冒烟测试运行
- 可选的身份验证行为：
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` 强制使用配置文件存储的身份验证并忽略仅基于环境变量的覆盖
...
- 用途：
  ...
  - 默认情况下，自动将每个测试套件的范围缩小至当前具有可用身份验证的提供商
