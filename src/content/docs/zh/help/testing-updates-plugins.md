...
- 每个插件 npm 依赖项都安装在一个受管理的 npm 项目中，在信任前进行扫描，并在卸载期间通过 npm 删除，以避免提升的依赖项残留。
...
- 依赖项布局或清理行为：同时断言运行时执行和文件系统边界。npm 依赖项可能会在插件的受管理 npm 项目中被提升，因此测试应证明该项目已被扫描/清理，而不是仅假设插件包本地的 `node_modules` 树。
