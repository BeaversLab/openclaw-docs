...
针对现实环境（本地开发、VPS、多智能体、OAuth/API 密钥、模型故障转移）的快速解答和更深入的故障排除。有关运行时诊断，请参阅 [故障排除](/zh/gateway/troubleshooting)。有关完整的配置参考，请参阅 [配置](/zh/gateway/configuration)。
...
6. **运行医生（修复）**
   ...
   修复/迁移配置/状态并运行健康检查。请参阅 [诊断工具](/zh/gateway/doctor)。
...
<AccordionGroup>
  ...
  <Accordion title="Value proposition">
    OpenClaw 不仅仅是一个“Claude 包装器”。它是一个**本地优先的控制平面**，让您能够在**您自己的硬件**上运行一个
    能干的助手，可以从您已经使用的聊天应用程序访问，并具有有状态会话、记忆和工具 - 而无需将工作流程的控制权交给托管
    的 SaaS。
    ...
    - **真实渠道，而非 Web 沙盒：** WhatsApp/Telegram/Slack/Discord/Signal/iMessage/等，
      加上支持的平台上的移动语音和 Canvas。
    - **模型无关：** 使用 Anthropic、OpenAI、MiniMax、OpenRouter 等，并具有按代理路由
      和故障转移功能。
    - **仅限本地选项：** 运行本地模型，这样如果您愿意，**所有数据都可以保留在您的设备上**。
    - **多代理路由：** 按渠道、账户或任务分离代理，每个代理都有自己的
      工作区和默认设置。
    - **开源且可扩展：** 检查、扩展和自托管，没有供应商锁定。

    文档：[Gateway(网关)](/zh/gateway)、[Channels](/zh/channels)、[Multi-agent](/zh/concepts/multi-agent)、
  ...
  <Accordion title="OpenClawOpenClaw 能否协助 SaaS 进行潜在客户开发、外联、广告和博客撰写？"OpenClaw>
    可以协助进行 **调研、资格审查和起草**。它可以扫描网站、建立候选名单、
    总结潜在客户信息，并撰写外联或广告文案草稿。

    对于 **外联或广告投放**，请保持人工介入。避免垃圾信息，遵守当地法律
    和平台政策，并在发送前审核所有内容。最安全的模式是让
    OpenClaw 起草，然后由您审批。
  ...
  <Accordion title="与 Claude Code 相比，在 Web 开发方面有哪些优势？"OpenClawOpenClawWhatsAppTelegramTUIWebChatGateway(网关)>
    OpenClaw 是一个**个人助手**和协调层，而非 IDE 的替代品。请使用
...
<AccordionGroup>
  ...
  <Accordion title="我如何为不同的任务使用不同的模型？">
    目前支持的模式有：
  ...
  <Accordion title="机器人在执行繁重任务时卡顿。如何卸载这些任务？">
    对长时间或并行任务使用**子代理**。子代理在各自的会话中运行，
    返回摘要，并保持您的主聊天响应迅速。

    让您的机器人“为此任务生成一个子代理”或使用 `/subagents`。
    在聊天中使用 `/status` 查看 Gateway(网关) 当前正在做什么（以及它是否忙碌）。

    令牌提示：长时间任务和子代理都会消耗令牌。如果关注成本，请通过 `agents.defaults.subagents.model` 为子代理设置更便宜的模型。

    文档：[子代理](/zh/tools/subagents)，[后台任务](/zh/automation/tasks)。

  </Accordion>

  <Accordion title="DiscordDiscord 上绑定线程的子代理会话如何工作？"Discord>
    ...
    - 生成时自动绑定：`channels.discord.threadBindings.spawnSessions` 默认为 `true`；将其设置为 `false` 以禁用绑定线程的会话生成。
  ...
  <Accordion title="子代理已完成，但完成更新发送到了错误的位置或从未发布。我应该检查什么？"OpenClaw>
    ...
    - 完成模式的子代理投递优先使用任何绑定的线程或会话路由（如果存在）。
    - 如果完成源仅携带渠道，OpenClaw 将回退到请求者会话的存储路由（`lastChannel` / `lastTo` / `lastAccountId`），以便直接投递仍能成功。
    - 如果既没有绑定路由也没有可用的存储路由，直接投递可能会失败，结果将回退到排队会话投递，而不是立即发布到聊天中。
    - 无效或过时的目标仍可能导致队列回退或最终投递失败。
    - 如果子代的最后可见助手回复是确切的静默令牌 `NO_REPLY` / `no_reply`，或者完全是 `ANNOUNCE_SKIP`OpenClaw，OpenClaw 将有意抑制公告，而不是发布过时的早期进度。
    - 如果子代仅在工具调用后超时，公告可以将其折叠为简短的部分进度摘要，而不是重播原始工具输出。
  ...
  <Accordion title="Cron 已触发，但没有任何内容发送到渠道。为什么？">
    ...
    - 渠道认证失败（`unauthorized`, `Forbidden`）意味着运行器尝试投递但凭据阻止了它。
    - 静默的隔离结果（仅 `NO_REPLY` / `no_reply`）被视为故意不可投递，因此运行器也会抑制排队的回退投递。

    对于隔离的 cron 任务，当聊天路由可用时，代理仍然可以使用 `message` 工具直接发送。`--announce` 仅控制代理尚未发送的最终文本的运行器回退路径。
    ...
    文档：[Cron 任务](/zh/automation/cron-jobs), [后台任务](/zh/automation/tasks)。
  ...
  <Accordion title="为什么隔离的 cron 运行会切换模型或重试一次？">
    ...
    隔离的 cron 可以持久化运行时模型切换，并在活动运行抛出 `LiveSessionModelSwitchError` 时进行重试。重试会保留切换后的提供商/模型，并且如果切换携带了新的身份验证配置文件覆盖，cron 也会在重试之前将其持久化。
  ...
  <Accordion title="Linux如何在 Linux 上安装 Skills？">
    ...
    目录。仅当您想要发布或同步您自己的 skills 时，才安装单独的 `clawhub`CLI CLI。若要在代理之间共享安装，请将 skill 置于
    `~/.openclaw/skills` 之下，并使用 `agents.defaults.skills` 或
  ...
  <Accordion title="macOSLinux我可以从 Linux 运行仅限 Apple macOS 的技能吗？"macOS>
    不能直接运行。macOS 技能受 `metadata.openclaw.os`Gateway(网关)Linux 及所需二进制文件的限制，并且只有在 **Gateway(网关) 主机**上符合条件时，这些技能才会出现在系统提示中。在 Linux 上，除非您覆盖限制条件，否则 `darwin` 专属技能（如 `apple-notes`、`apple-reminders`、`things-mac`Gateway(网关)Gateway(网关)macOSLinux）将无法加载。

    您有三种支持的模式：

    **选项 A - 在 Mac 上运行 Gateway（最简单）。**
    在存在 macOS 二进制文件的地方运行 Gateway，然后通过 [远程模式](#gateway-ports-already-running-and-remote-modeTailscaleGateway(网关)macOSmacOSGateway(网关)LinuxmacOSOpenClawmacOS) 或通过 Tailscale 从 Linux 进行连接。由于 Gateway 主机是 macOS，技能会正常加载。

    **选项 B - 使用 macOS 节点（无需 SSH）。**
    在 Linux 上运行 Gateway，配对一个 macOS 节点（菜单栏应用），并在 Mac 上将 **节点运行命令** 设置为“始终询问”或“始终允许”。当节点上存在所需的二进制文件时，OpenClaw 可以将仅限 macOS 的技能视为符合条件。代理会通过 `nodes`macOSGateway(网关)LinuxCLILinux 工具运行这些技能。如果您选择“始终询问”，在提示中批准“始终允许”会将该命令添加到允许列表中。
  ...
  <Accordion title="你们有 Notion 或 HeyGen 集成吗？"API>
    目前尚未内置。

    选项：

    - **自定义 skill / 插件：** 最适合可靠的 API 访问（Notion/HeyGen 均提供 API）。
    - **浏览器自动化：** 无需代码即可运行，但速度较慢且稳定性较低。

    如果你想为每个客户端（代理机构工作流）保留上下文，一个简单的模式是：
    ...
    原生安装会进入活动工作区 `skills/` 目录。对于 agents 之间共享的 skills，请将它们放置在 `~/.openclaw/skills/<name>/SKILL.md` 中。如果只有部分 agents 应该看到共享安装，请配置 `agents.defaults.skills` 或 `agents.list[].skills`LinuxLinux。某些 skills 需要通过 Homebrew 安装的二进制文件；在 Linux 上这意味着 Linuxbrew（请参阅上方的 Homebrew Linux 常见问题条目）。参见 [Skills](/zh/tools/skills)、[Skills 配置](/zh/tools/skills-configClawHub) 和 [ClawHub](/zh/clawhub)。
...
<AccordionGroup>
  <Accordion title="是否有专门的沙箱隔离文档？">
    有的。请参阅 [沙箱隔离](/en/gateway/sandboxing)。有关 DockerDocker 特定的设置（Docker 中的完整网关或沙箱镜像），请参阅 [Docker](/en/install/docker)。
  </Accordion>
  ...
  <Accordion title="DockerDocker 感觉受限 - 如何启用完整功能？">
    ...
    - 使用 `OPENCLAW_DOCKER_APT_PACKAGES`CLI 将系统依赖项烘焙到镜像中。
  ...
  <Accordion title="我可以保持私信的私密性，但使用同一个 Agent 让群组公开/处于沙箱隔离状态吗？">
    可以 - 如果您的私人流量是 **私信** 而公共流量是 **群组**。

    使用 `agents.defaults.sandbox.mode: "non-main"` 以便群组/渠道会话（非主键）在配置的沙箱后端中运行，而主私信会话保持在主机上。如果您不选择后端，Docker 是默认后端。然后通过 `tools.sandbox.tools` 限制沙箱会话中可用的工具。

    设置演练 + 示例配置：[群组：个人私信 + 公共群组](/en/channels/groups#pattern-personal-dms-public-groups-single-agentGateway(网关))

    关键配置参考：[Gateway(网关) 配置](/en/gateway/config-agents#agentsdefaultssandbox)

  </Accordion>

  <Accordion title="如何将主机文件夹绑定到沙箱？">
    将 `agents.defaults.sandbox.docker.binds` 设置为 `["host:path:mode"]`（例如，`"/home/user/src:/src:ro"`）。全局绑定与每个代理的绑定会合并；当设置 `scope: "shared"` 时，将忽略每个代理的绑定。对于任何敏感内容，请使用 `:ro`OpenClaw，并请记住绑定会绕过沙箱文件系统隔离墙。

    OpenClaw 会根据标准化路径和通过最深层的现有祖先解析的规范路径来验证绑定源。这意味着即使最后一个路径段尚不存在，通过符号链接父目录进行的逃逸仍然会被封闭拒绝，并且在解析符号链接后，允许的根目录检查仍然适用。

    有关示例和安全说明，请参阅 [沙箱隔离](/en/gateway/sandboxing#custom-bind-mounts) 和 [沙箱 vs 工具策略 vs 提权](/en/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check)。
  ...
  <Accordion title="How does memory work?"OpenClaw>
    ...
    - `MEMORY.md`OpenClaw 中的精选长期笔记（仅限主会话/私有会话）
  ...
  <Accordion title="Memory keeps forgetting things. How do I make it stick?">
    请机器人**将事实写入记忆**。长期笔记应放入 `MEMORY.md`，
    短期上下文则进入 `memory/YYYY-MM-DD.md`Gateway(网关)。

    这仍是我们正在改进的领域。提醒模型存储记忆会有所帮助；
    它会知道该怎么做。如果它一直忘记，请验证 Gateway(网关) 在每次运行时使用的是相同的
    工作区。
  ...
  <Accordion title="内存会永久保留吗？有什么限制？">
    内存文件存储在磁盘上，会一直保留直到你将其删除。限制在于你的
  ...
  <Accordion title="OpenAIAPI语义记忆搜索是否需要 OpenAI API 密钥？"OpenAIOAuthOAuthCLIOpenAIAPI>
    ...
    仍然需要真正的 API 密钥（`OPENAI_API_KEY` 或 `models.providers.openai.apiKey`OpenClawAPI）。

    如果您没有显式设置提供商，当 OpenClaw
    可以解析 API 密钥（auth 配置文件、`models.providers.*.apiKey`OpenAIOpenAIOpenClaw 或环境变量）时，它会自动选择提供商。
    如果解析到 OpenAI 密钥，它优先使用 OpenAI，否则如果解析到 Gemini 密钥则使用 Gemini，
    然后是 Voyage，接着是 Mistral。如果没有可用的远程密钥，内存
    搜索将保持禁用状态，直到您对其进行配置。如果您有已配置且存在的本地模型路径，OpenClaw
    优先使用 `local`Ollama。当您显式设置
...
<AccordionGroup>
  <Accordion title="使用 OpenClaw 的所有数据都会保存在本地吗？">
    ...
    - **默认本地：**会话、内存文件、配置和工作区位于 Gateway(网关) 主机上
      (`~/.openclaw` + 您的工作区目录)。
    - **必要远程：**您发送给模型提供商（Anthropic/OpenAI/等）的消息会发送到
  ...
  <Accordion title="AGENTS.md / SOUL.md / USER.md / MEMORY.md 应该放在哪里？">
    ...
    - **Workspace (per agent)（工作区，每个代理）**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      ...
      小写的根 `memory.md` 仅用于遗留修复输入；当两个文件都存在时，`openclaw doctor --fix`
      可以将其合并到 `MEMORY.md` 中。
    ...
    如果机器人“忘记”了之前的内容，请确认 Gateway(网关) 在每次启动时使用的是
    相同的工作区（请记住：远程模式使用的是 **gateway host's（网关主机）**
    的工作区，而不是你本地笔记本电脑的工作区）。

    提示：如果你希望行为或偏好能够持久保存，请让机器人将其**写入
  ...
  <Accordion title="推荐的备份策略"GitHub>
    将您的 **agent workspace** 放在一个 **私有** 的 git 仓库中，并将其备份到某个私有位置（例如 GitHub 私有仓库）。这会捕获内存以及 AGENTS/SOUL/USER 文件，并允许您稍后恢复助手的“思维”。
n
    请 **勿** 提交 `~/.openclaw` 下的任何内容（凭据、会话、令牌或加密的机密负载）。
...
<Accordion title="OpenClaw如何完全卸载 OpenClaw？">请参阅专用指南：[Uninstall](/en/install/uninstall)。</Accordion>
  ...
  <Accordion title="Remote mode: where is the 会话 store?">
    会话状态由 **网关主机 (gateway host)** 拥有。如果您处于远程模式，您关心的会话存储位于远程机器上，而不是您的本地笔记本电脑。请参阅[会话管理](/zh/concepts/session)。
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="如何启用 Web 搜索（以及 Web 获取）？">
    ...
    - 基于 API 的 提供商（例如 Brave、Exa、Firecrawl、Gemini、Grok、Kimi、MiniMax Search、Perplexity 和 Tavily）需要其常规的 API 密钥设置。
    - Ollama Web Search 是免密的，但它使用您配置的 Ollama 主机并需要 `ollama signin`。
    - DuckDuckGo 是免密的，但它是一个非官方的基于 HTML 的集成。
    - SearXNG 是免密/自托管的；请配置 `SEARXNG_BASE_URL` 或 `plugins.entries.searxng.config.webSearch.baseUrl`。

    **推荐：** 运行 `openclaw configure --section web` 并选择一个 提供商。
    环境变量替代方案：
    ...
    特定于 提供商 的 Web 搜索配置现在位于 `plugins.entries.<plugin>.config.webSearch.*` 下。
    旧的 `tools.web.search.*` 提供商 路径为了兼容性暂时仍会加载，但不应用于新配置。
    Firecrawl 的 web 获取回退配置位于 `plugins.entries.firecrawl.config.webFetch.*` 下。

    注意事项：

    - 如果您使用 allowlists（允许列表），请添加 `web_search`/`web_fetch`/`x_search` 或 `group:web`。
    - `web_fetch` 默认启用（除非被明确禁用）。
    - 如果省略了 `tools.web.fetch.provider`，OpenClaw 会根据可用的凭据自动检测第一个就绪的获取回退 提供商。目前内置的 提供商 是 Firecrawl。
  ...
  <Accordion title="config.apply 清除了我的配置。如何恢复并避免这种情况？">
    ...
    - 无效或具有破坏性的 OpenClaw 拥有的写入将被拒绝，并保存为 `openclaw.json.rejected.*`。
    - 如果直接编辑导致启动或热重载失败，Gateway(网关) 将会失败关闭或跳过重载；它不会重写 `openclaw.json`。
    - `openclaw doctor --fix` 负责修复，并可以在将被拒绝的文件保存为 `openclaw.json.clobbered.*` 的同时，恢复最后已知的有效配置。
    ...
    - 如果你没有最后已知的有效配置或被拒绝的有效负载，请从备份还原，或重新运行 `openclaw doctor` 并重新配置通道/模型。
    - 如果这是意外发生的，请提交错误报告并附上你最后已知的配置或任何备份。
    - 本地编码代理通常可以从日志或历史记录中重构一个可用的配置。

    避免这种情况：

    - 使用 `openclaw config set` 进行小改动。
    - 使用 `openclaw configure` 进行交互式编辑。
    - 当你不确定确切的路径或字段形状时，请先使用 `config.schema.lookup`；它返回一个浅层架构节点以及用于下钻的直接子摘要。
    - 使用 `config.patch` 进行部分 RPC 编辑；仅将 `config.apply` 用于全配置替换。
    - 如果你正在使用仅限所有者的 `gateway` 工具进行代理运行，它仍将拒绝写入 `tools.exec.ask` / `tools.exec.security`（包括规范化为同一受保护执行路径的传统 `tools.bash.*` 别名）。

    文档：[配置](/zh/cli/config)、[配置指南](/zh/cli/configure)、[Gateway(网关) 故障排除](/zh/gateway/troubleshooting#gateway-rejected-invalid-config)、[Doctor](/zh/gateway/doctor)。

  </Accordion>

  <Accordion title="Gateway(网关)如何跨设备运行中央 Gateway(网关) 并配备专用工作程序？"Gateway(网关)Raspberry PiGateway(网关)SignalWhatsAppiOSAndroid>
    常见模式是 **一个 Gateway(网关)**（例如 Raspberry Pi）加上 **节点** 和 **代理**：

    - **Gateway(网关)（中央）：** 拥有通道（Signal/WhatsApp）、路由和会话。
    - **节点（设备）：** Mac/iOS/Android 作为外设连接并公开本地工具（`system.run`、`canvas`、`camera`HetznerTUIGateway(网关)）。
  ...
  <Accordion title="OpenClawOpenClaw 浏览器可以以无头模式运行吗？">
    可以的。这是一个配置选项：
    ...
    默认是 `false` (有头模式)。无头模式在某些网站上更容易触发反机器人检查。请参阅 [浏览器](/zh/tools/browser)。

    无头模式使用**相同的 Chromium 引擎**，并适用于大多数自动化任务（表单、点击、抓取、登录）。主要区别在于：
...
<AccordionGroup>
  ...
  <Accordion title="Gateway(网关)如果 Gateway(网关) 托管在远程，我的 Agent 如何访问我的计算机？"Gateway(网关)>
    简短回答：**将您的计算机配对为节点**。Gateway(网关) 在其他地方运行，但它可以通过 Gateway(网关) WebSocket 调用您本地机器上的 `node.*`Gateway(网关)Gateway(网关)Gateway(网关)Gateway(网关)macOSGateway(网关) 工具（屏幕、摄像头、系统）。
    ...
    4. 在本地打开 macOS 应用程序，并以 **Remote over SSH** 模式（或直接通过 tailnet）连接，以便它注册为节点。
    ...
    文档：[节点](/en/nodesGateway(网关))，[Gateway(网关) 协议](/en/gateway/protocolmacOS)，[macOS 远程模式](/en/platforms/mac/remote)，[安全性](/en/gateway/security)。

  </Accordion>

  <Accordion title="TailscaleTailscale 已连接但我没有收到回复。怎么办？"Gateway(网关)>
    检查基础信息：
  ...
  <Accordion title="OpenClaw两个 OpenClaw 实例可以互相通信吗（本地 + VPS）？"TelegramSlackWhatsAppCLIGateway(网关)>
    可以。没有内置的“机器人对机器人”桥接器，但你可以通过以下几种可靠的方式连接它们：

    **最简单的方式：** 使用两个机器人都可以访问的正常聊天渠道（Telegram/Slack/WhatsApp）。
    让机器人 A 向机器人 B 发送消息，然后让机器人 B 像往常一样回复。

    **CLI 桥接器（通用）：** 运行一个脚本，使用
    `openclaw agent --message ... --deliver`CLIGateway(网关)Tailscale 调用另一个 Gateway，
    目标指向另一个机器人监听的聊天。如果一个机器人位于远程 VPS 上，可以通过 SSH/Tailscale 将你的 CLI 指向该远程 Gateway（请参阅 [远程访问](/en/gateway/remoteGateway(网关))）。

    示例模式（在可以访问目标 Gateway 的机器上运行）：

    ```bash
    openclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```

    提示：添加一个防护措施，防止两个机器人无限循环（仅限提及、渠道白名单，或“不回复机器人消息”规则）。
  ...
  <Accordion title="与从 VPS 使用 SSH 相比，在我的个人笔记本电脑上使用节点有什么好处吗？"Gateway(网关)Gateway(网关)macOSLinuxWindowsWSL2Raspberry PiGateway(网关)>
    是的——节点是从远程 Gateway(网关) 连接到您的笔记本电脑的首选方式，而且它们提供的功能不仅仅是 Shell 访问。Gateway(网关) 运行在 macOS/Linux（Windows 通过 WSL2）上，并且非常轻量（小型 VPS 或 Raspberry Pi 级别的设备即可；4 GB 内存绰绰有余），因此常见的设置是一台始终在线的主机加上作为节点的笔记本电脑。
    ...
    SSH 适用于临时 Shell 访问，但对于持续的代理工作流和设备自动化，节点更为简单。
  ...
  <Accordion title="节点会运行网关服务吗？">
    不会。除非您有意运行隔离的配置文件（参见 [Multiple gateways](/zh/gateway/multiple-gateways)），否则每台主机应仅运行 **一个网关**。节点是连接到网关的外设（iOS/Android 节点，或菜单栏应用中的 macOS “节点模式”）。有关无头节点主机和 CLI 控制，请参阅 [Node host CLI](/zh/cli/node)。
  ...
  <Accordion title="Tailscale如何在 VPS 上设置 Tailscale 并从我的 Mac 连接？">
    ...
    2. **在 Mac 上安装 + 登录**
       - 使用 Tailscale 应用并登录到同一个 tailnet 网络。
    3. **启用 MagicDNS（推荐）**
    ...
    如果你不想使用 SSH 而想使用控制 UI，请在 VPS 上使用 Tailscale Serve：

    ```bash
    openclaw gateway --tailscale serve
    ```TailscaleTailscale

    这将使网关绑定到回环地址，并通过 Tailscale 暴露 HTTPS。参见 [Tailscale](/en/gateway/tailscale)。

  </Accordion>

  <Accordion title="Gateway(网关)Tailscale如何将 Mac 节点连接到远程 Gateway(网关) (Tailscale Serve)？"Gateway(网关)Gateway(网关)macOSGateway(网关)>
    Serve 暴露了 **Gateway(网关) Control UI + WS**。节点通过同一个 Gateway(网关) WS 端点进行连接。
  ...
  <Accordion title="Should I install on a second laptop or just add a node?">
    如果你只需要在第二台笔记本电脑上使用 **本地工具**（屏幕/摄像头/exec），请将其作为一个
    **节点** 添加。这样可以保持单一的 Gateway(网关) 并避免重复的配置。本地节点工具目前
    仅支持 macOS，但我们计划将其扩展到其他操作系统。

    仅当你需要 **硬隔离** 或两个完全独立的机器人时，才安装第二个 Gateway(网关)。
...
<AccordionGroup>
  ...
  <Accordion title='我设置了 COPILOT_GITHUB_TOKEN，但模型状态显示 "Shell env: off." 为什么？'>
    `openclaw models status`OpenClawGateway(网关) 报告是否启用了 **shell env import**（Shell 环境导入）。"Shell env: off"
    并 **不** 意味着您的环境变量丢失——它只是意味着 OpenClaw 不会
    自动加载您的登录 shell。

    如果 Gateway 作为服务运行（launchd/systemd），它将不会继承您的 shell
    环境。请通过以下方式之一进行修复：
...
<AccordionGroup>
  ...
  <Accordion title="OpenClaw有没有办法组建一个 OpenClaw 实例团队（一个 CEO 和多个代理）？">
    是的，通过**多代理路由**和**子代理**。你可以创建一个协调者代理和多个具有自己工作空间和模型的工作者代理。

    也就是说，这最好被视为一个**有趣的实验**。它非常消耗 token，而且通常比使用具有独立会话的一个机器人效率低。我们设想的典型模式是你与一个机器人对话，使用不同的会话进行并行工作。该机器人在需要时也可以生成子代理。
  ...
  <Accordion title="如何在保留安装的情况下完全重置 OpenClaw？">
    ...
    - 如果检测到现有配置，**新手引导** (新手引导) 也会提供 **重置** (Reset) 选项。请参阅 [新手引导 (CLI)](/zh/start/wizard)。
  ...
  <Accordion title="OpenClaw为什么 OpenClaw 不在群组中回复？">
    ...
    - 提及门控已开启（默认）。您必须 @提及 该机器人（或匹配 `mentionPatterns`）。
    - 您配置了 `channels.whatsapp.groups` 但没有配置 `"*"`，且该群组未被列入白名单。

    请参阅 [群组](/en/channels/groups) 和 [群组消息](/en/channels/group-messages)。

  </Accordion>

<Accordion title="Do groups/threads share context with 私信?">直接聊天默认折叠到主会话。群组/频道拥有各自的会话密钥，且 Telegram 主题 / Discord 线程是独立的会话。参阅 [Groups](/en/channels/groups) 和 [Group messages](/en/channels/group-messages)。</Accordion>
  ...
  <Accordion title="我可以同时运行多个机器人或聊天（Slack），应该如何设置？">
    可以。使用 **Multi-Agent Routing** 运行多个隔离的代理，并按渠道/账户/对等方路由传入消息。Slack 支持作为渠道，可以绑定到特定的代理。

    浏览器访问功能强大，但并非“能做人类能做的任何事情”——反机器人、验证码 (CAPTCHA) 和多因素认证 (MFA) 仍可能阻止自动化。为了最可靠的浏览器控制，请在主机上使用本地 Chrome MCP，或者在实际运行浏览器的机器上使用 CDP。
...
位于 [Models 常见问题](/en/help/faq-models)。
...
<AccordionGroup>
  ...
  <Accordion title='控制界面显示“未授权”（或不断重新连接）。怎么办？'>
    您的网关认证路径与 UI 的认证方法不匹配。

    事实（来自代码）：

    - 控制界面将令牌保存在 `sessionStorage` 中，用于当前浏览器标签页会话和选定的网关 URL，因此同标签页刷新可以继续工作，而无需恢复长期的 localStorage 令牌持久性。
    ...
    - 该缓存的令牌重试现在重用与设备令牌一起存储的缓存批准范围。显式 `deviceToken` / 显式 `scopes` 调用方仍然保留其请求的范围集，而不是继承缓存的范围。
    - 在该重试路径之外，连接认证优先级首先是显式共享令牌/密码，然后是显式 `deviceToken`，然后是存储的设备令牌，最后是引导令牌。
    - 引导令牌范围检查带有角色前缀。内置引导操作员允许列表仅满足操作员请求；节点或其他非操作员角色仍需要在其自己的角色前缀下拥有范围。

    修复方法：

    - 最快的方法：`openclaw dashboard`（打印 + 复制仪表板 URL，尝试打开；如果是无头模式，则显示 SSH 提示）。
    - 如果您还没有令牌：`openclaw doctor --generate-gateway-token`。
    - 如果是远程访问，先建立隧道：`ssh -N -L 18789:127.0.0.1:18789 user@host` 然后打开 `http://127.0.0.1:18789/`。
    - 共享密钥模式：设置 `gateway.auth.token` / `OPENCLAW_GATEWAY_TOKEN` 或 `gateway.auth.password` / `OPENCLAW_GATEWAY_PASSWORD`，然后在控制界面设置中粘贴匹配的密钥。
    - Tailscale Serve 模式：确保启用了 `gateway.auth.allowTailscale`，并且您打开的是 Serve URL，而不是绕过 Tailscale 身份标头的原始回环/tailnet URL。
    - 受信任代理模式：确保您通过配置的感知身份代理访问，而不是原始网关 URL。同主机回环代理也需要 `gateway.auth.trustedProxy.allowLoopback = true`。
    - 如果在一次重试后不匹配仍然存在，请轮换/重新批准配对的设备令牌：
      - `openclaw devices list`
      - `openclaw devices rotate --device <id> --role operator`
    - 如果该轮换调用显示被拒绝，请检查两件事：
      - 配对设备会话只能轮换它们**自己的**设备，除非它们也拥有 `operator.admin`
  ...
  <Accordion title="我设置了 gateway.bind tailnet 但无法绑定，没有任何内容在监听">
    `tailnet`TailscaleTailscaleTailscale bind 会从你的网络接口中选择一个 Tailscale IP (100.64.0.0/10)。如果该机器未连接 Tailscale（或接口已关闭），则没有可绑定的地址。
    ...
    注意：`tailnet` 是显式的。 `auto` 优先使用环回地址；当你希望仅绑定到 tailnet 时，请使用 `gateway.bind: "tailnet"`。

  </Accordion>

  <Accordion title="我可以在同一主机上运行多个 Gateway 吗？">
    通常不行——一个 Gateway(网关) 可以运行多个消息通道和代理。仅当您需要冗余（例如：救援机器人）或硬隔离时，才使用多个 Gateway。
  ...
  <Accordion title='“invalid handshake”（无效握手） / 代码 1008 是什么意思？'Gateway(网关)>
    ...
    **代码 1008**（策略违规）关闭连接。
    ...
    3. 如果开启了身份验证，请将令牌/密码包含在 `connect`CLITUI 帧中。
...
<AccordionGroup>
  ...
  <Accordion title="WindowsOpenClaw我在Windows上关闭了终端 - 如何重启OpenClaw？"WindowsWSL2Gateway(网关)Linux>
    **有两种Windows安装模式**：

    **1) WSL2（推荐）：** Gateway(网关)在Linux内运行。

    打开PowerShell，进入WSL，然后重启：
    ...
    **2) 原生Windows（不推荐）：** Gateway(网关)直接在Windows中运行。
    ...
    文档：[Windows (WSL2)](/en/platforms/windowsGateway(网关))，[Gateway(网关)服务手册](/en/gateway)。
  ...
  <Accordion title="Gateway(网关)Gateway(网关)已启动，但从未收到回复。我应该检查什么？">
    ...
    - 渠道配对/允许列表阻止了回复（检查渠道配置和日志）。
    - WebChat/仪表板未使用正确的令牌打开。
  ...
  <Accordion title='“从Gateway(网关)断开连接：无原因” - 现在该怎么办？'Gateway(网关)>
  ...
  <Accordion title="TelegramTelegram setMyCommands 失败。我应该检查什么？">
    ...
    - `BOT_COMMANDS_TOO_MUCH`TelegramOpenClawTelegram：Telegram 菜单项过多。OpenClaw 已经将命令修剪到 Telegram 限制并使用较少的命令重试，但仍需要删除一些菜单项。减少插件/技能/自定义命令，或者如果您不需要菜单，请禁用 `channels.telegram.commands.native`。
    - `TypeError: fetch failed`、`Network request for 'setMyCommands' failed!` 或类似的网络错误：如果您在 VPS 上或位于代理后面，请确认允许出站 HTTPS 并且 `api.telegram.org`Gateway(网关)Gateway(网关)Telegram 的 DNS 解析正常。
  ...
  <Accordion title="TUITUI 显示无输出。我应该检查什么？"Gateway(网关)>
    首先确认 Gateway(网关) 是可达的，且代理可以运行：
    ...
    在 TUI 中，使用 `/status` 查看当前状态。如果您在聊天渠道中期待回复，
    请确保已启用投递 (`/deliver on`TUI)。
...
<AccordionGroup>
  <Accordion title="我的技能生成了图片/PDF，但没有任何内容被发送">
    来自代理的出站附件必须包含一个 `MEDIA:<path-or-url>`OpenClaw 行（单独占一行）。请参阅 [OpenClaw 助手设置](/en/start/openclaw) 和 [代理发送](/en/tools/agent-sendCLI)。
    ...
    - 目标渠道支持出站媒体且未被允许列表阻止。
    - 文件在提供商的大小限制范围内（图片会被调整大小至最大 2048px）。
    - `tools.fs.workspaceOnly=true` 将本地路径发送限制在工作区、temp/media-store 和沙盒验证的文件内。
    - `tools.fs.workspaceOnly=false` 允许 `MEDIA:` 发送代理已可读取的主机本地文件，但仅限于媒体和安全的文档类型（图片、音频、视频、PDF 和 Office 文档）。纯文本和类似机密的文件仍然被阻止。
...
<AccordionGroup>
  ...
  <Accordion title="提示词注入仅是公共机器人需要关注的问题吗？">
    不是。提示词注入关乎的是**不受信任的内容**，而不仅仅是谁能给机器人发私信。
  ...
  <Accordion title="GitHub我的机器人应该拥有自己的电子邮件、GitHub 账户还是电话号码？">
    是的，对于大多数设置而言。使用单独的账户和电话号码隔离机器人
    可以在出现问题时减小影响范围。这也使得轮换
    凭据或撤销访问权限变得更容易，而不会影响您的个人账户。
  ...
  <Accordion title="我可以让它自主管理我的短信吗？这安全吗？">
    ...
    如果您想进行实验，请在专用帐户上进行，并保持其隔离。请参阅
    [安全性](/zh/gateway/security)。

  </Accordion>

<Accordion title="我可以在个人助手任务中使用更便宜的模型吗？">是的，**如果**代理仅用于聊天且输入是受信任的。较低层级的模型更容易受到指令劫持，因此对于启用了工具的代理或在读取不受信任的内容时，应避免使用它们。如果您必须使用较小的模型，请锁定工具并在沙盒中运行。请参阅 [安全](/zh/gateway/security)。</Accordion>
...
<AccordionGroup>
  <Accordion title="如何阻止内部系统消息在聊天中显示？">
    大多数内部消息或工具消息仅在该会话启用了 **verbose**、**trace** 或 **reasoning** 时才会显示。
    ...
    如果仍然很嘈杂，请在控制 UI 中检查会话设置并将 verbose 设置为 **inherit**。同时确认您未使用在配置中将 `verboseDefault` 设置为 `on` 的机器人配置文件。
  ...
  <Accordion title="如何停止/取消正在运行的任务？">
    ...
    大多数命令必须作为以 `/` 开头的 **独立** 消息发送，但少数快捷方式（如 `/status`）也可以在白名单发送者的行内消息中工作。
  ...
  <Accordion title='为什么感觉机器人“忽略”了连续发送的消息？'>
    队列模式控制新消息如何与正在运行的交互。使用 `/queue` 来更改模式：

    - `steer` - 将所有待处理的引导操作排队，以便在当前运行的下一个模型边界处执行
    - `queue` - 旧版的一次引导一个操作
    - `followup` - 一次运行一条消息
    - `collect` - 批量处理消息并回复一次
    ...
    默认模式是 `steer`。你可以添加像 `debounce:0.5s cap:25 drop:summarize` 这样的选项作为后续模式。参见 [Command queue](/zh/concepts/queue) 和 [Steering queue](/zh/concepts/queue-steering)。
...
<AccordionGroup>
  <Accordion title="AnthropicAPI使用 API 密钥时 Anthropic 的默认模型是什么？" OpenClaw>
    在 OpenClaw 中，凭据和模型选择是分开的。设置 `ANTHROPIC_API_KEY`AnthropicAPI（或在身份验证配置文件中存储 Anthropic API 密钥）可以启用身份验证，但实际的默认模型取决于您在 `agents.defaults.model.primary` 中配置的内容（例如，`anthropic/claude-sonnet-4-6` 或 `anthropic/claude-opus-4-6`）。如果您看到 `No credentials found for profile "anthropic:default"`Gateway(网关)Anthropic，这意味着
...
- [Troubleshooting](/zh/help/troubleshooting) — 基于症状的初步排查
