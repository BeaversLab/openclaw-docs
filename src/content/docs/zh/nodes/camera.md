...
- **iOS 节点**（通过 Gateway(网关) 网关 配对）：通过 `node.invoke` 捕获**照片** (`jpg`) 或**短视频片段** (`mp4`，可选音频)。
- **Android 节点**（通过 Gateway(网关) 网关 配对）：通过 `node.invoke` 捕获**照片** (`jpg`) 或**短视频片段** (`mp4`，可选音频)。
- **macOS 应用**（通过 Gateway(网关) 网关 作为节点）：通过 `node.invoke` 捕获**照片** (`jpg`) 或**短视频片段** (`mp4`，可选音频)。
