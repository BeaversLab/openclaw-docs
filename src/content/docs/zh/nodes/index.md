...
**节点**是一种配套设备（macOS/iOS/Android/headless），它使用 `role: "node"` 连接到 Gateway(网关) **WebSocket**（与操作员端口相同），并通过 `node.invoke` 暴露命令界面（例如 `canvas.*`、`camera.*`、`device.*`、`notifications.*`、`system.*`）。协议详情：[Gateway(网关) protocol](/zh/gateway/protocol)。
...
- 执行批准通过 `~/.openclaw/exec-approvals.json`
  在本地强制执行（参见 [执行批准](/zh/tools/exec-approvals)）。
