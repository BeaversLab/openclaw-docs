...
**节点** 是一个配套设备（macOS/iOS/Android/headless），通过 `role: "node"` 连接到 Gateway(网关) **WebSocket**（与操作员端口相同），并通过 `node.invoke` 暴露命令表面（例如 `canvas.*`、`camera.*`、`device.*`、`notifications.*`、`system.*`）。协议详情：[Gateway(网关) protocol](/en/gateway/protocol)。

传统传输：[Bridge protocol](/en/gateway/bridge-protocol) (TCP JSONL; 对于当前节点已弃用/移除)。
...
- Shell 执行现在通过带有 `host=node` 的 `exec` 工具进行；`nodes` 仍然是用于显式节点命令的 direct-RPC 表面。
- `nodes invoke` 不暴露 `system.run` 或 `system.run.prepare`；这些仅保留在 exec 路径上。
- `system.notify` 遵循 macOS 应用程序上的通知权限状态。
- 无法识别的节点 `platform` / `deviceFamily` 元数据使用保守的默认允许列表，该列表排除了 `system.run` 和 `system.which`。如果您故意需要针对未知平台的这些命令，请通过 `gateway.nodes.allowCommands` 显式添加它们。
- `system.run` 支持 `--cwd`、`--env KEY=VAL`、`--command-timeout` 和 `--needs-screen-recording`。
- 对于 shell 包装器（`bash|sh|zsh ... -c/-lc`），请求范围的 `--env` 值被减少为显式允许列表（`TERM`、`LANG`、`LC_*`、`COLORTERM`、`NO_COLOR`、`FORCE_COLOR`）。
- 在允许列表模式下，对于“始终允许”的决策，已知的调度包装器（`env`、`nice`、`nohup`、`stdbuf`、`timeout`）会持久化内部可执行文件路径而不是包装器路径。如果解包不安全，则不会自动持久化允许列表条目。
...
- 在 macOS 节点模式下，`system.run` 受 macOS 应用程序中的执行批准限制（设置 → 执行批准）。
  Ask/allowlist/full 的行为与无头节点主机相同；被拒绝的提示返回 `SYSTEM_RUN_DENIED`。
- 在无头节点主机上，`system.run` 受执行批准（`~/.openclaw/exec-approvals.json`）的限制。

## Exec 节点绑定

当有多个节点可用时，您可以将 exec 绑定到特定节点。
这会为 `exec host=node` 设置默认节点（并且可以按代理覆盖）。
...
节点可以在 `node.list` / `node.describe` 中包含 `permissions` 映射，以权限名称（例如 `screenRecording`、`accessibility`）为键，布尔值（`true` = 已授予）。
