...
OpenClawDiscordGoogle ChatiMessageMatrixMicrosoft TeamsSignalSlackTelegramWhatsAppZaloGateway(网关) 是一个 **自托管 Gateway(网关)**，可将您喜爱的聊天应用和渠道（包括内置渠道以及捆绑或外部渠道插件，例如 Discord、Google Chat、iMessage、Matrix、Microsoft Teams、Signal、Slack、Telegram、WhatsApp、Zalo 等等）连接到 AI 编码代理。您在自己的机器（或服务器）上运行单个 Gateway(网关) 进程，它就成为您的消息应用与随时可用的 AI 助手之间的桥梁。
...
**你需要什么？** Node 24（推荐）或 Node 22 LTS (`22.19+`API) 以确保兼容性，来自您选择的提供商的 API 密钥，以及 5 分钟时间。为了获得最佳质量和安全性，请使用可用的最强大的最新模型。
...
<Columns>
  <Card title="多渠道网关" icon="network" href="/zh/channels">
    通过单个 Discord 进程支持 iMessage、Signal、Slack、Telegram、WhatsApp、WebChat、Gateway(网关) 等更多渠道。
  </Card>
  <Card title="Plugin channels" icon="plug" href="/zh/tools/plugin">
    捆绑插件在常规当前版本中添加了 Matrix、Nostr、Twitch、Zalo 等支持。
  </Card>
  <Card title="Multi-agent routing" icon="route" href="/zh/concepts/multi-agent">
    针对每个代理、工作区或发送者的隔离会话。
  </Card>
  <Card title="Media support" icon="image" href="/zh/nodes/images">
    发送和接收图片、音频以及文档。
  </Card>
  <Card title="Web Control UI" icon="monitor" href="/zh/web/control-ui">
    用于聊天、配置、会话和节点的浏览器仪表板。
  </Card>
  <Card title="Mobile nodes" icon="smartphone" href="/zh/nodes">
    配对 iOS 和 Android 节点，用于 Canvas、摄像头和语音启用的工作流。
  </Card>
</Columns>
...
<Steps>
  ...
  <Step title="Chat">
    ...
    或连接一个渠道（[Telegram](/zh/channels/telegram) 最快）并从您的手机聊天。
...
- 如果您 **不做任何操作**，OpenClaw 将使用捆绑的 OpenClaw 代理运行时和基于发送方的会话。
...
<Columns>
  ...
  <Card title="Nodes" href="/zh/nodes" icon="smartphone">
    支持 iOS 和 Android 的节点，具有配对、Canvas、相机和设备操作功能。
  </Card>
...
<Columns>
  ...
  <Card title="关于与致谢" href="/zh/reference/credits" icon="info">
    项目起源、贡献者和许可证。
  </Card>
</Columns>
