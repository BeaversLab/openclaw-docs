---
summary: "最常用的 OpenClaw 文档精选链接。"
read_when:
  - You want quick access to key docs pages
title: "文档目录"
---

<Note>此页面是精选索引。如果您是新手，请从[入门指南](/zh/start/getting-started)开始。 如需文档的完整地图，请参阅[文档中心](/zh/start/hubs)。</Note>

## 从这里开始

- [文档中心（所有页面链接）](/zh/start/hubs)
- [帮助](/zh/help)
- [配置](/zh/gateway/configuration)
- [配置示例](/zh/gateway/configuration-examples)
- [斜杠命令](/zh/tools/slash-commands)
- [多代理路由](/zh/concepts/multi-agent)
- [更新和回滚](/zh/install/updating)
- [配对（私信和节点）](/zh/channels/pairing)
- [Nix 模式](/zh/install/nix)
- [OpenClaw 助手设置](/zh/start/openclaw)
- [Skills](/zh/tools/skills)
- [Skills 配置](/zh/tools/skills-config)
- [工作区模板](/zh/reference/templates/AGENTS)
- [RPC 适配器](/zh/reference/rpc)
- [Gateway(网关) 运维手册](/zh/gateway)
- [节点（iOS 和 Android）](/zh/nodes)
- [Web 界面（控制 UI）](/zh/web)
- [设备发现和传输](/zh/gateway/discovery)
- [远程访问](/zh/gateway/remote)

## 提供商和用户体验

- [WebChat](/zh/web/webchat)
- [控制 UI（浏览器）](/zh/web/control-ui)
- [Telegram](/zh/channels/telegram)
- [Discord](/zh/channels/discord)
- [Mattermost](/zh/channels/mattermost)
- [BlueBubbles (iMessage)](/zh/channels/bluebubbles)
- [QQ 机器人](/zh/channels/qqbot)
- [iMessage（旧版）](/zh/channels/imessage)
- [群组](/zh/channels/groups)
- [WhatsApp 群消息](/zh/channels/group-messages)
- [媒体图片](/zh/nodes/images)
- [媒体音频](/zh/nodes/audio)

## 配套应用

- [macOS 应用](/zh/platforms/macos)
- [iOS 应用](/zh/platforms/ios)
- [Android 应用](/zh/platforms/android)
- [Windows (WSL2)](/zh/platforms/windows)
- [Linux 应用](/zh/platforms/linux)

## 运维和安全

- [会话](/zh/concepts/session)
- [Cron 作业](/zh/automation/cron-jobs)
- [Webhooks](/zh/automation/cron-jobs#webhooks)
- [Gmail 钩子 (Pub/Sub)](/zh/automation/cron-jobs#gmail-pubsub-integration)
- [安全性](/zh/gateway/security)
- [故障排除](/zh/gateway/troubleshooting)

## 相关

- [入门指南](/zh/start/getting-started)
- [文档中心](/zh/start/hubs)
