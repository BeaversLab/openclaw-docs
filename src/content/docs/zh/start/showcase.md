...
<Info>**想被展示？** 在 [Discord 上的 #showcase](https://discord.gg/clawd) 分享你的项目，或者在 X 上 [艾特 @openclaw](https://x.com/openclaw)。</Info>

## 🎥 OpenClaw 实战演示

由 VelvetShark 制作的全套设置演练（28分钟）。
...
OpenCode 完成更改 → 打开 PR → OpenClaw 审查差异并在 Telegram 中回复“次要建议”以及明确的合并裁决（包括需要先应用的关键修复）。

  <img src="/assets/showcase/pr-review-telegram.jpg" alt="OpenClaw PR 审查反馈在 Telegram 中送达" />
</Card>

<Card title="几分钟内打造酒窖技能" icon="wine-glass" href="https://x.com/i/status/2010916352454791216">
  **@prades_maxime** • `skills` `local` `csv`

要求“Robby”(@openclaw) 提供一个本地酒窖技能。它请求导出示例 CSV 以及存储位置，然后快速构建/测试该技能（示例中为 962 瓶酒）。
...
热键选中屏幕区域 → Gemini 视觉 → 剪贴板中即时生成 Markdown。
...
通过 ParentPay 自动预订英国学校餐食。使用鼠标坐标以确保可靠的表格单元格点击。
...
构建了一个完整的包含地图和语音录制的 iOS 应用，完全通过 Telegram 聊天部署到 TestFlight。
...
在一个网关下管理 14+ 个代理，通过 Opus 4.5 编排器委派给 Codex 工作节点。包含 Dream Team 名单、模型选择、沙箱隔离、webhooks、心跳和委派流程的全面[技术文章](https://github.com/adam91holt/orchestrated-ai-articles)。用于代理沙箱隔离的 [Clawdspace](https://github.com/adam91holt/clawdspace)。[博客文章](https://adams-ai-journey.ghost.io/2026-the-year-of-the-orchestrator/)。
...
与代理工作流（Claude Code、OpenClaw）集成的 Linear CLI。从终端管理 issue、项目和工作流。第一个外部 PR 已合并！
...
通过 Beeper Desktop 读取、发送和归档消息。使用 Beeper 本地 MCP API，让代理可以在一个地方管理您所有的聊天（iMessage、WhatsApp 等）。
...
Claude Code 发现并确认了净化器控制，然后由 OpenClaw 接管以管理室内空气质量。
...
由屋顶摄像头触发：让 OpenClaw 在天空看起来很美时拍照——它设计了一个技能并完成了拍摄。
...
每天早上，计划中的提示词通过 OpenClaw 人设生成一张“场景”图片（包含天气、任务、日期、最喜欢的帖子/引用）。

</Card>

<Card title="Padel Court Booking" icon="calendar-check" href="https://github.com/joshp123/padel-cli">
  **@joshp123** • `automation` `booking` `cli` Playtomic 可用性检查器 + 预订 CLI。再也不错过任何开放球场。
  <img src="/assets/showcase/padel-screenshot.jpg" alt="padel-cli screenshot" />
</Card>

<Card title="Accounting Intake" icon="file-invoice-dollar">
  **Community** • `automation` `email` `pdf` 从邮件中收集 PDF，为税务顾问准备文件。每月会计核算自动化。
</Card>
...
搜索职位列表，匹配简历关键词，并返回带有链接的相关机会。使用 JSearch API 在 30 分钟内构建完成。
...
OpenClaw 连接到 Jira，然后即时生成了一项新技能（在 ClawHub 上存在之前）。
...
通过浏览器自动化登录 TradingView，截取图表，并根据需求执行技术分析。不需要 API——只需浏览器控制。
...
监视公司 Slack 渠道，提供有益回复，并将通知转发到 Telegram。在未被要求的情况下，自主修复了已部署应用程序中的生产错误。
...
<Card title="xuezh Chinese Learning" icon="language" href="https://github.com/joshp123/xuezh">
  **@joshp123** • `learning` `voice` `skill` 带有发音反馈和 通过 OpenClaw 进行学习流程的中文学习引擎。
  <img src="/assets/showcase/xuezh-pronunciation.jpeg" alt="xuezh pronunciation feedback" />
</Card>

<Card title="WhatsApp Memory Vault" icon="vault">
  **社区** • `memory` `transcription` `indexing` 摄入完整的 WhatsApp 导出数据，转录 1000+ 条语音备忘录，与 git 日志交叉核对，输出链接的 markdown 报告。
</Card>
...
<Card title="Inside-Out-2 Memory" icon="brain">
  **社区** • `memory` `beliefs` `self-model` 独立的内存管理器，将会话文件 转换为记忆 → 信念 → 演进的自模型。
</Card>
...
<Card title="Clawdia Phone Bridge" icon="phone" href="https://github.com/alejandroOPI/clawdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge` Vapi 语音助手 ↔ OpenClaw HTTP 网桥。与您的代理进行 近实时的电话通话。
</Card>
...
<Card title="Home Assistant Add-on" icon="home" href="https://github.com/ngutman/openclaw-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi` 在 Home Assistant OS 上运行的 OpenClaw 网关，支持 SSH 隧道和持久化状态。
</Card>
...
<Card title="Nix Packaging" icon="snowflake" href="https://github.com/openclaw/nix-openclaw">
  **@openclaw** • `nix` `packaging` `deployment` 开箱即用的 nix 化 OpenClaw 配置，用于可复现部署。
</Card>
...
<Card title="GoHome Automation" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana` Nix 原生家庭自动化，以 OpenClaw 作为界面，以及美观的 Grafana 仪表板。
...
<Card title="StarSwap Marketplace" icon="star" href="https://star-swap.com/">
  **社区** • `marketplace` `astronomy` `webapp` 完整的天文器材市场。基于 OpenClaw 生态系统构建。
</Card>
...
<Steps>
  <Step title="分享它">在 [#showcase on Discord](https://discord.gg/clawd) 中发帖或 [发推文 @openclaw](https://x.com/openclaw)</Step>
  <Step title="包含详情">告诉我们它的用途，链接到仓库/演示，如果有截图也请分享</Step>
