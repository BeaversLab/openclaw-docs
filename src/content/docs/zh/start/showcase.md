...
OpenCode 完成更改 → 打开 PR → OpenClaw 审查差异并在 Telegram 中回复“微小的建议”以及明确的合并裁定（包括需要先应用的关键修复）。
...
向“Robby” (@openclaw) 请求一个本地酒窖技能。它请求一个 CSV 示例导出以及存储位置，然后快速构建/测试该技能（示例中包含 962 瓶酒）。
...
快捷键选定屏幕区域 → Gemini 视觉 → 剪贴板中即时生成 Markdown。
...
<Card title="维也纳交通" icon="train" href="https://clawhub.com/hjanuschka/wienerlinien">
...
<Card title="ParentPay 学校膳食" icon="utensils" href="#">
  **@George5562** • `automation` `browser` `parenting`

通过 ParentPay 自动预订英国学校膳食。使用鼠标坐标以可靠地点击表格单元格。

</Card>

<Card title="R2 上传" icon="cloud-arrow-up" href="https://clawhub.com/skills/r2-upload">
...
<Card title="通过 iOS 构建 Telegram 应用" icon="mobile" href="#">
  **@coard** • `ios` `xcode` `testflight`

构建了一个包含地图和语音录制的完整 iOS 应用，完全通过 Telegram 聊天部署到 TestFlight。
...
14 个以上智能体在一个网关下，由 Opus 4.5 编排器委派给 Codex 工作人员。涵盖梦之队名单、模型选择、沙箱隔离、Webhooks、心跳和委派流程的综合性[技术文章](https://github.com/adam91holt/orchestrated-ai-articles)。用于智能体沙箱隔离的 [Clawdspace](https://github.com/adam91holt/clawdspace)。[博客文章](https://adams-ai-journey.ghost.io/2026-the-year-of-the-orchestrator/)。
...
与智能体工作流（Claude Code、CLI）集成的 Linear OpenClaw。在终端管理问题、项目和工作流。首个外部 PR 已合并！
...
通过 Beeper Desktop 读取、发送和归档消息。使用 Beeper 本地 MCP API，以便智能体在一个地方管理您的所有聊天（iMessage、WhatsApp 等）。
...
由屋顶摄像头触发：每当天空看起来很美时，就要求 OpenClaw 拍摄一张天空照片——它设计了一个技能并拍摄了照片。
...
通过定时的提示，每天早上生成一张单一的“场景”图片（天气、任务、日期、最喜欢的帖子/引用），由 OpenClaw 角色驱动。

</Card>

<Card title="Padel 球场预订" icon="calendar-check" href="https://github.com/joshp123/padel-cli">
  **@joshp123** • `automation` `booking` `cli` Playtomic 可用性检查器 + 预订 CLI。不再错过空闲球场。
  <img src="/assets/showcase/padel-screenshot.jpg" alt="padel-cli screenshot" />
</Card>

<Card title="会计资料收集" icon="file-invoice-dollar">
  **社区** • `automation` `email` `pdf` 从邮件中收集 PDF，为税务顾问准备文件。每月会计核算自动完成。
</Card>

<Card title="沙发土豆开发者模式" icon="couch" href="https://davekiss.com">
  **@davekiss** • `telegram` `website` `migration` `astro`

在观看 Netflix 的同时，通过 Telegram 重建了整个个人网站 —— Notion → Astro，迁移了 18 篇文章，DNS 切换到 Cloudflare。从未打开过笔记本电脑。
...
搜索职位列表，根据简历关键词进行匹配，并返回相关机会及链接。使用 JSearch API 在 30 分钟内构建完成。
...
通过浏览器自动化登录 TradingView，截图图表，并按要求进行技术分析。不需要 API——只需浏览器控制。
...
监视公司 Slack 渠道，提供有益回复，并将通知转发到 Telegram。在未接到指令的情况下，自主修复了已部署应用中的生产环境错误。
...
<Card title="WhatsApp Memory Vault" icon="vault">
  **Community** • `memory` `transcription` `indexing` 摄入完整的 WhatsApp 导出数据，转录 1000+ 条语音笔记，与 git 日志交叉比对，输出关联的 markdown 报告。
</Card>
...
<Card title="Inside-Out-2 Memory" icon="brain">
  **Community** • `memory` `beliefs` `self-model` 独立的内存管理器，将会话文件转化为记忆 → 信念 → 演进中的自我模型。
</Card>
...
<Card title="Clawdia Phone Bridge" icon="phone" href="https://github.com/alejandroOPI/clawdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge` Vapi 语音助手 ↔ OpenClaw HTTP 桥接。与您的智能体进行准实时电话通话。
</Card>
...
<Card title="GoHome Automation" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana` 基于Nix的原生家庭自动化，以OpenClaw为接口，并配有精美的 Grafana 仪表板。
...
<Card title="StarSwap Marketplace" icon="star" href="https://star-swap.com/">
  **Community** • `marketplace` `astronomy` `webapp` 完整的天文装备市场。基于 OpenClaw 生态系统构建。
</Card>
