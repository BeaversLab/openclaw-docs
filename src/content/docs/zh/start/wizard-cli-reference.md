...
<Steps>
  ...
  <Step title="模型和身份验证">
    - 完整的选项矩阵位于 [身份验证和模型选项](#auth-and-model-options) 中。
  ...
  <Step title="Channels"WhatsApp>
    - [WhatsApp](/zh/channels/whatsappTelegram)：可选的二维码登录
    - [Telegram](/zh/channels/telegramDiscord)：机器人令牌
    - [Discord](/en/channels/discordGoogle Chat)：机器人令牌
    - [Google Chat](/zh/channels/googlechatMattermost)：服务账户 JSON + Webhook 受众
    - [Mattermost](/zh/channels/mattermostSignal)：机器人令牌 + 基础 URL
    - [Signal](/zh/channels/signal)：可选的 `signal-cli`iMessage 安装 + 账户配置
    - [iMessage](/zh/channels/imessage)：`imsg`CLIGateway(网关) CLI 路径 + Messages 数据库访问；当 Gateway 在 Mac 以外运行时，请使用 SSH 封装器
...
<AccordionGroup>
  ...
  <Accordion title="OAuthxAI (Grok) OAuth"OpenClaw>
    适用于符合条件的 SuperGrok 或 X 高级账户的浏览器登录。对于大多数用户来说，这是推荐的 xAI 路径。OpenClaw 会为 Grok 模型、`x_search` 和 `code_execution` 存储生成的身份验证配置文件。
  </Accordion>
  ...
  <Accordion title="APIxAI (Grok) API 密钥">
    提示输入 `XAI_API_KEY`APIOAuth 并将 xAI 配置为模型提供商。当您需要 xAI 控制台 API 密钥而不是订阅 OAuth 时，请使用此选项。
  </Accordion>
  ...
  <Accordion title="VercelGateway(网关)Vercel AI 网关">
    提示输入 `AI_GATEWAY_API_KEY`VercelGateway(网关)。
    更多详细信息：[Vercel AI 网关](/zh/providers/vercel-ai-gateway)。
  </Accordion>
  <Accordion title="Gateway(网关)Cloudflare AI Gateway">
    提示输入账户 ID、Gateway(网关) ID 和 `CLOUDFLARE_AI_GATEWAY_API_KEY`Gateway(网关)。
  ...
  <Accordion title="OllamaOllama (Cloud and local open models)">
...
- 当新手引导从提供商身份验证选择开始时，模型选择器会自动首选该提供商。对于 Volcengine 和 BytePlus，同样的首选项也匹配其编码计划变体（`volcengine-plan/*`，
  `byteplus-plan/*`）。
...
<Note>
Headless and server 提示：在有浏览器的机器上完成 OAuth，然后
将该 agent 的 `auth-profiles.json`（例如
