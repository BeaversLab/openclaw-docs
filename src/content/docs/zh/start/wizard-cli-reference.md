...
<Steps>
  ...
  <Step title="模型和认证">
    - 完整选项矩阵位于 [认证和模型选项](#auth-and-model-options)。
  ...
  <Step title="频道">
    ...
    - [Mattermost](/zh/channels/mattermost)：Bot 令牌 + 基础 URL
    - [Signal](/zh/channels/signal)：可选的 `signal-cli` 安装 + 账号配置
    - [iMessage](/zh/channels/imessage)：`imsg` CLIGateway(网关) 路径 + 消息数据库访问权限；当 Gateway(网关) 在非 Mac 设备上运行时，请使用 SSH 包装器
    - 私信安全：默认为配对。首次私信会发送代码；通过
...
<AccordionGroup>
  ...
  <Accordion title="xAI (Grok) OAuth"OpenClaw>
    适用于符合条件的 SuperGrok 或 X Premium 账号的浏览器登录。对于大多数用户来说，这是推荐的 xAI 路径。OpenClaw 会存储生成的认证
  ...
  <Accordion title="APIxAI (Grok) API 密钥">
    提示输入 `XAI_API_KEY`APIOAuth 并将 xAI 配置为模型提供商。当您希望使用 xAI Console API 密钥而不是订阅 OAuth 时，请使用此选项。
  </Accordion>
  ...
  <Accordion title="VercelGateway(网关)Vercel AI Gateway(网关)">
    提示输入 `AI_GATEWAY_API_KEY`VercelGateway(网关)。
    更多详情：[Vercel AI Gateway(网关)](/zh/providers/vercel-ai-gateway)。
  </Accordion>
  <Accordion title="Gateway(网关)Cloudflare AI Gateway(网关)">
    提示输入账户 ID、网关 ID 和 `CLOUDFLARE_AI_GATEWAY_API_KEY`Gateway(网关)。
    更多详情：[Cloudflare AI Gateway(网关)](/zh/providers/cloudflare-ai-gateway)。
  </Accordion>
  <Accordion title="MiniMaxMiniMax">
    配置是自动写入的。托管默认值是 `MiniMax-M2.7`API；API 密钥设置使用
    `minimax/...`OAuth，OAuth 设置使用 `minimax-portal/...`MiniMax。
    更多详情：[MiniMax](/zh/providers/minimax)。
  </Accordion>
  <Accordion title="StepFun">
    针对中国或全球端点上的 StepFun 标准版或 Step Plan 自动写入配置。
    标准版目前包括 `step-3.5-flash`，Step Plan 还包括 `step-3.5-flash-2603`。
    更多详情：[StepFun](/zh/providers/stepfun)。
  </Accordion>
  <Accordion title="AnthropicSynthetic (Anthropic 兼容)">
    提示输入 `SYNTHETIC_API_KEY`。
    更多详情：[Synthetic](/zh/providers/synthetic)。
  </Accordion>
  <Accordion title="OllamaOllama（云端和本地开源模型）">
    首先提示输入 `Cloud + Local`、`Cloud only` 或 `Local only`。
    `Cloud only` 将 `OLLAMA_API_KEY` 与 `https://ollama.com` 结合使用。
    主机支持的模式会提示输入基础 URL（默认为 `http://127.0.0.1:11434`），发现可用模型并建议默认值。
    `Cloud + Local`OllamaOllama 还会检查该 Ollama 主机是否已登录以进行云端访问。
    更多详情：[Ollama](/zh/providers/ollama)。
  </Accordion>
  <Accordion title="MoonshotMoonshot 和 Kimi Coding"MoonshotMoonshot>
...
- 在非交互模式下，`--secret-input-mode ref` 仅支持环境变量后端。
  ...
  - 在该自定义提供商的情况下，`--custom-api-key` 要求设置 `CUSTOM_API_KEY`；否则新手引导会快速失败。
...
- 传递 `--skip-bootstrap` 时 `agents.defaults.skipBootstrap`
- `agents.defaults.model` / `models.providers`（如果选择了 Minimax）
- `tools.profile`（本地新手引导未设置时默认为 `"coding"`；保留现有的显式值）
- `gateway.*`（mode、bind、auth、tailscale）
- `session.dmScope`（本地新手引导未设置时将其默认为 `per-channel-peer`；保留现有的显式值）
...
- `skills.install.nodeManager`
  ...
  - 手动配置仍然可以在以后设置 `skills.install.nodeManager: "yarn"`。
- `wizard.lastRunAt`
