...
本文档描述了**当前**的首次运行设置流程。目标是提供流畅的“第 0 天”体验：选择 Gateway(网关) 的运行位置，连接身份验证，运行向导，然后让代理自行引导。
有关新手引导路径的总体概述，请参阅 [新手引导 Overview](<Gateway(网关)/en/start/onboarding-overview>)。
...
<Step title="本地与远程">
<Frame>
<img src="/assets/macos-onboarding/04-choose-gateway.png"Gateway(网关)Gateway(网关) alt="" />
</Frame>

**Gateway(网关)** 在哪里运行？

- **此 Mac (仅本地)：** 新手引导可以配置身份验证并在本地写入凭据。
- **远程 (通过 SSH/Tailnet)：** 新手引导**不会**配置本地身份验证；凭据必须存在于 gateway 主机上。
- **稍后配置：** 跳过设置并保持应用未配置状态。

<Tip>
**Gateway(网关) 身份验证提示：**

- 该向导现在即使对于环回也会生成一个 **token**，因此本地 WS 客户端必须进行身份验证。
...
<Step title="新手引导聊天 (专用会话)">
  设置完成后，应用会打开一个专用的新手引导聊天会话，以便代理可以自我介绍并指导后续步骤。这使首次运行指导与您的正常对话保持分开。有关第一次运行代理期间在 gateway 主机上发生的情况，请参阅 [Bootstrapping](/zh/start/bootstrapping)。
</Step>
</Steps>
