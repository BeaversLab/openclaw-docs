...
<Note>如果您是第一次进行设置，请从 [入门指南](/en/start/getting-started) 开始。 有关新手引导的详细信息，请参阅 [新手引导 (CLI)](/en/start/wizard)。</Note>
...
对于常驻或多用户服务器，请考虑使用 **系统**服务而不是用户服务（不需要 lingering）。有关 systemd 的说明，请参阅 [Gateway(网关) runbook](/en/gateway)。

## 相关文档

- [Gateway(网关) runbook](/en/gateway)（标志、监管、端口）
