...
GitHub Copilot 也可以充当[记忆搜索](/zh/concepts/memory-search)的嵌入提供商。如果您拥有 Copilot 订阅并已登录，OpenClaw 可以使用它进行嵌入，而无需单独的 API 密钥。

### 自动检测

当 `memorySearch.provider` 为 `"auto"`（默认值）时，GitHub Copilot 将在优先级 15 尝试使用——在本地嵌入之后，但在 OpenAI 和其他付费提供商之前。如果存在 GitHub 令牌，OpenClaw 会从 Copilot API 发现可用的嵌入模型并自动选择最佳的一个。
