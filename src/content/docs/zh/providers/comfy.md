---
summary: "OpenClaw 中 ComfyUI 工作流图像、视频和音乐生成的设置"
title: "ComfyUI"
read_when:
...
OpenClaw 附带了一个捆绑的 `comfy` 插件，用于驱动工作流的 ComfyUI 运行。该插件完全由工作流驱动，因此 OpenClaw 不会尝试将通用的 `size`、`aspectRatio`、`resolution`、`durationSeconds` 或 TTS 风格的控制映射到您的图上。
...
| 身份验证 | 本地 ComfyUI 不需要；Comfy Cloud 需要 `COMFY_API_KEY` 或 `COMFY_CLOUD_API_KEY` |
...
<Tabs>
  ...
  <Tab title="Comfy Cloud">
    **最适用于：** 在 Comfy Cloud 上运行工作流，无需管理本地 GPU 资源。

    <Steps>
      <Step title="获取 API 密钥">
        在 [comfy.org](https://comfy.org) 注册，并从您的账户仪表板生成一个 API 密钥。
      </Step>
...
| `apiKey`              | string                 | 可选的内联键，是 `COMFY_API_KEY` / `COMFY_CLOUD_API_KEY` 环境变量的替代方案。 |
| `allowPrivateNetwork` | boolean                | 允许在云模式下使用私有/LAN `baseUrl`。                                        |
...
| 键                    | 必需                 | 默认      | 描述                        |
| --------------------- | -------------------- | --------- | --------------------------- |
| `inputImageNodeId`    | 是（传递参考图片时） | --        | 接收上传参考图片的节点 ID。 |
| `inputImageInputName` | 否                   | `"image"` | 图片节点上的输入名称。      |

## 工作流详细信息

<AccordionGroup>
  <Accordion title="图片工作流">
  ...
  <Accordion title="视频工作流">
    ...
    <Note>
    OpenClaw 不会将输入视频传递到 Comfy 工作流中。仅支持文本提示和单个参考图片作为输入。
    </Note>
  ...
  <Accordion title="向后兼容性">
    ...
    <Tip>
    如果您仅使用图像生成，旧的扁平配置和新的嵌套 `image` 部分在功能上是等效的。
    </Tip>
