...
[inferrs](https://github.com/ericcurtin/inferrs) 可以在 OpenAI 兼容的 `/v1` API 后面服务本地模型。OpenClaw 通过通用的 `openai-completions` 路径与 `inferrs` 配合使用。

目前最好将 `inferrs` 视为自定义的自托管 OpenAI 兼容后端，而不是专用的 OpenClaw 提供商插件。
