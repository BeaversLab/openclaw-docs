...
| `speakerVoice`          | `messages.tts.providers.azure-speech.speakerVoice`          | Azure 语音 ShortName（默认为 `en-US-JennyNeural`）。旧版别名：`voice`。                    |
| `lang`                  | `messages.tts.providers.azure-speech.lang`                  | SSML 语言代码（默认为 `en-US`）。                                                          |
| `outputFormat`          | `messages.tts.providers.azure-speech.outputFormat`          | 音频文件输出格式（默认为 `audio-24khz-48kbitrate-mono-mp3`）。                             |
| `voiceNoteOutputFormat` | `messages.tts.providers.azure-speech.voiceNoteOutputFormat` | 语音笔记输出格式（默认为 `ogg-24khz-16bit-mono-opus`）。                                   |
...
<AccordionGroup>
  <Accordion title="身份验证">
    ...
    作为 `Ocp-Apim-Subscription-Key` 发送；除非您
    提供 `endpoint` 或 `baseUrl`，否则 OpenClaw 会
    从 `region` 推导 `https://<region>.tts.speech.microsoft.com`。
  </Accordion>
  <Accordion title="语音名称">
    使用 Azure Speech 语音 `ShortName` 值，例如
    `en-US-JennyNeural`。捆绑的提供商可以通过同一语音资源列出语音，并过滤标记为已弃用或已退休的语音。
  </Accordion>
  <Accordion title="音频输出">
    ...
    会为 `voice-note` 目标请求 Ogg/Opus 格式，以便频道可以发送原生语音气泡而无需额外的 MP3 转换。
  </Accordion>
  <Accordion title="Alias">
    `azure` 被接受作为现有 PR 和用户配置的提供商别名，
...
<CardGroup cols={2}>
  <Card title="Text-to-speech" href="/zh/tools/tts" icon="waveform-lines">
