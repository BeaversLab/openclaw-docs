...
`qwen` 扩展还在 **Standard** DashScope 端点（而非 Coding Plan 端点）上公开了多模态功能：
...
<AccordionGroup>
  ...
  <Accordion title="能力计划">
    ...
    - **文本/聊天模型：** 现已内置
    - **工具调用、结构化输出、思维：** 继承自与 OpenAI 兼容的传输层
    - **图像生成：** 计划在提供商插件层实现
    - **图像/视频理解：** 现已在标准端点上内置
    - **语音/音频：** 计划在提供商插件层实现
...
<CardGroup cols={2}>
  ...
  <Card title="阿里巴巴 (ModelStudio)" href="/en/providers/alibaba" icon="cloud">
