...
<Tabs>
  <Tab title="Coding Plan (subscription)">
    **适用于：** 通过 Qwen Coding Plan 进行的基于订阅的访问。
    ...
    仍然作为兼容性别名有效，但新的设置流程应优先使用规范的
    `qwen-*` auth-choice id 和 `qwen/...` 模型引用。如果您使用其他
    `api` 值定义了精确的自定义 `models.providers.modelstudio` 条目，
    那么该自定义提供商将拥有 `modelstudio/...` 引用，而不是 Qwen 兼容性别名。
    </Note>

  </Tab>

  <Tab title="Standard (pay-as-you-go)">
    **Best for:** 通过 Standard Model Studio 端点进行按量付费访问，包括在 Coding Plan 上可能不可用的模型，如 `qwen3.6-plus`。
    ...
    <Note>
    旧版 `modelstudio-*` 身份验证选项 ID 和 `modelstudio/...` 模型引用仍
    可作为兼容性别名使用，但新的设置流程应首选规范化的
    `qwen-*` 身份验证选项 ID 和 `qwen/...` 模型引用。如果您定义了具有其他 `api` 值的精确自定义 `models.providers.modelstudio` 条目，则
    该自定义提供商拥有 `modelstudio/...` 引用，而不是 Qwen 兼容性
    别名。
    </Note>
...
<AccordionGroup>
  ...
  <Accordion title="Qwen 3.6 Plus 可用性">
    `qwen3.6-plus` 在标准版（按量付费）Model Studio 端点上可用：

    - 中国：`dashscope.aliyuncs.com/compatible-mode/v1`
    - 全球：`dashscope-intl.aliyuncs.com/compatible-mode/v1`

    如果编码计划端点针对 `qwen3.6-plus` 返回“不支持的模型”错误，请切换到标准版（按量付费），而不是使用编码计划端点/密钥对。

  </Accordion>

  <Accordion title="能力计划">
    `qwen` 插件正被定位为完整 Qwen Cloud 表面的供应商主页，而不仅仅是编码/文本模型。

    - **文本/聊天模型：** 现已内置
    - **工具调用、结构化输出、思维：** 继承自 OpenAI 兼容传输层
    - **图像生成：** 计划在提供商插件层实现
    - **图像/视频理解：** 现已在标准端点内置
    - **语音/音频：** 计划在提供商插件层实现
  ...
  <Accordion title="视频生成详情">
    对于视频生成，OpenClaw 会在提交作业之前，将配置的 Qwen 区域映射到匹配的
    DashScope AIGC 主机：

    - Global/Intl：`https://dashscope-intl.aliyuncs.com`
    - China：`https://dashscope.aliyuncs.com`

    这意味着一个指向 Coding Plan 或标准 Qwen 主机的普通 `models.providers.qwen.baseUrl`
    仍然会将视频生成保持在正确的区域性 DashScope 视频端点上。
  ...
  <Accordion title="流式使用兼容性">
    原生 Model Studio 端点在共享的 `openai-completions` 传输上通告了流式使用兼容性。
    OpenClaw 密钥现在会关闭端点功能，因此针对同一原生主机的 DashScope 兼容自定义提供商 ID
    会继承相同的流式使用行为，而无需专门使用内置的 `qwen` 提供商 ID。
  ...
  <Accordion title="环境和守护程序设置">
    如果 Gateway(网关)作为守护程序（launchd/systemd）运行，请确保 `QWEN_API_KEY` 对该进程可用（例如，在 `~/.openclaw/.env` 中或通过 `env.shellEnv`）。
  </Accordion>
</AccordionGroup>
