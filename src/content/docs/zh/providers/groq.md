...
[Groq](https://groq.com) 使用定制的 LPU 硬件，在开源权重模型（Llama、Gemma、Kimi、Qwen、GPT OSS 等）上提供超快的推理。OpenClaw 包含一个捆绑的 Groq 插件，该插件注册了一个兼容 OpenAI 的聊天提供商和一个音频媒体理解提供商。
...
OpenClaw 附带了一个基于清单的 Groq 目录，其中包含推理和非推理条目。运行 `openclaw models list --provider groq` 以查看您所安装版本的捆绑行，或查看 [console.groq.com/docs/models](https://console.groq.com/docs/models) 了解 Groq 的权威列表。
...
<Tip>目录随每个 OpenClaw 版本发布而演进。`openclaw models list --provider groq` 显示您已安装版本已知的行；与 [console.groq.com/docs/models](https://console.groq.com/docs/models) 对照以检查新增或已弃用的模型。</Tip>
...
有关共享的 `/think` 级别以及 OpenClaw 如何针对每个提供商进行转换的信息，请参阅[思维模式](/zh/tools/thinking)。
...
<AccordionGroup>
  <Accordion title="Environment availability for the daemon">
    如果 Gateway(网关) 作为托管服务（launchd、systemd、Docker）运行，`GROQ_API_KEY` 对该进程必须可见——而不仅仅是对您的交互式 shell 可见。

    <Warning>
      仅在交互式 shell 中导出的密钥对 launchd 或 systemd 守护进程没有帮助，除非该环境也在那里被导入。在 `~/.openclaw/.env` 中设置密钥或通过 `env.shellEnv` 设置，以便网关进程可以读取它。
    </Warning>

  </Accordion>

  <Accordion title="Custom Groq 模型 ids">
    OpenClaw 在运行时接受任何 Groq 模型 ID。请使用 Groq 显示的确切 ID，并为其加上 `groq/` 前缀。捆绑的目录涵盖了常见情况；未编目的 ID 会回退到默认的 OpenAI 兼容模板。
