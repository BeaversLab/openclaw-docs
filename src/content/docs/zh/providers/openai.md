...
| 通过 OpenClaw 进行 ChatGPT/Codex 订阅身份验证 | `openai/gpt-5.5` 加上提供商/模型运行时 `openclaw`      | 为兼容路由选择一个 `openai-codex` 身份验证配置文件。                     |
...
<Note>OpenAI 代理模型轮次需要捆绑的 Codex 应用服务器插件。显式 OpenClaw 运行时配置仍然作为可选兼容性路径可用。当 OpenClaw 被 `openai-codex` 身份验证配置文件显式选中时，OpenClaw 会将 公共模型引用保留为 `openai/*`，并通过 Codex-auth 传输在内部进行路由。运行 `openclaw doctor --fix` 以修复过时的 `openai-codex/*`、`codex-cli/*` 或并非来自 显式运行时配置的旧运行时会话固定项。</Note>
...
<Note>
  OpenAI Realtime voice（被 Voice Call 的 `realtime.provider: "openai"` 和
  Control UI 的“使用 `talk.realtime.provider: "openai"` 对话”所使用）通过公开的
  **OpenAI Platform Realtime API** 进行，其计费基于 OpenAI
  Platform 额度而非 Codex/ChatGPT 订阅配额。一个拥有有效的 Codex OAuth 且能够无问题地运行 `openai-codex/*` 聊天模型的账户，如果同一 OpenAI 组织未设置
  Platform 计费，在首次 Realtime 轮次中仍可能遇到 `insufficient_quota` / "You exceeded your current
  quota" 错误。

修复方法：在
[platform.openai.com/account/billing](https://platform.openai.com/account/billing)
为支持您的 realtime 凭证的组织充值 Platform 额度。Realtime 接受 Platform `OPENAI_API_KEY`（通过 `talk.realtime.providers.openai.apiKey`
为 Control UI 的“对话”配置，或通过 `plugins.entries.voice-call.config.realtime.providers.openai.apiKey`
为 Voice Call 配置）或其基础组织设有 Platform 计费的 `openai-codex` OAuth 个人资料——这两种方式均通过 Platform API 生成 Realtime 客户端密钥，
因此无论哪种方式，该组织都需要有资金的 Platform
额度。对于聊天轮次，您仍可针对同一
OpenClaw 安装使用 `openai-codex/*`；Realtime 是唯一需要 Platform 计费的路径。

</Note>

## Memory embeddings

OpenClaw 可以使用 OpenAI 或兼容 OpenAI 的 embedding 端点来处理
`memory_search` 索引和查询 embeddings：
...
对于需要非对称嵌入标签的 OpenAI 兼容端点，请在 `memorySearch` 下设置 `queryInputType` 和 `documentInputType`。OpenClaw 会将这些作为特定于提供商的 `input_type` 请求字段进行转发：查询嵌入使用 `queryInputType`；索引的内存块和批量索引使用 `documentInputType`。有关完整示例，请参阅 [Memory configuration reference](/zh/reference/memory-config#provider-specific-config)。
...
<Tabs>
  <Tab title="APIOpenAIAPI 密钥 (OpenAI Platform)"API>
    **最适用于：** 直接 API 访问和按使用量计费。
    ...
    | 模型引用              | 运行时配置             | 路由                       | 认证             |
    | ---------------------- | -------------------------- | --------------------------- | ---------------- |
    | `openai/gpt-5.5`      | 省略 / 提供商/模型 `agentRuntime.id: "codex"`OpenAI | Codex 应用服务器连接 | Codex 兼容的 OpenAI 配置文件 |
    | `openai/gpt-5.4-mini` | 省略 / 提供商/模型 `agentRuntime.id: "codex"`OpenAI | Codex 应用服务器连接 | Codex 兼容的 OpenAI 配置文件 |
    | `openai/gpt-5.5`      | 提供商/模型 `agentRuntime.id: "openclaw"`OpenClaw              | OpenClaw 嵌入式运行时      | `openai` 配置文件或选定的 `openai-codex` 配置文件 |

    <Note>
    `openai/*`APIAPI 代理模型使用 Codex 应用服务器连接。要对代理模型使用 API 密钥
    认证，请创建 Codex 兼容的 API 密钥配置文件，并使用 `auth.order.openai` 对其进行排序；`OPENAI_API_KEY`OpenAIAPI 仍然是
    非 Agent OpenAI API 表面的直接回退。较旧的 `auth.order.openai-codex` 条目仍然
    有效。
    </Note>
    ...
    模型，并推荐 `gpt-5.5`API 用于生产 API 使用，因此
    请保持 `openai/gpt-5.5` 为稳定的默认值，除非您明确需要该
    别名行为。该别名当前仅接受 `medium`OpenClawOpenAIOpenClaw 文本详细程度，因此
    OpenClaw 会针对此模型规范不兼容的 OpenAI 文本详细程度覆盖项。

    <Warning>
    OpenClaw **不** 公开 `openai/gpt-5.3-codex-spark`OpenAIAPI。实时的 OpenAI API 请求会拒绝该直接提供商路由。仅当 Codex 目录为您的登录账户公开它时，才使用 `openai-codex/gpt-5.3-codex-spark`。
    </Warning>

  </Tab>

  <Tab title="Codex subscription"API>
    **最适用于：** 使用你的 ChatGPT/Codex 订阅配合原生 Codex 应用服务器执行，而不是单独的 API 密钥。Codex 云端需要 ChatGPT 登录。
    ...
    <Steps>
      ...
      <Step title="OpenAI使用规范的 OpenAI 模型路由">
        ...
        默认路径不需要运行时配置。OpenAI 代理会自动
        选择原生 Codex 应用服务器运行时，并且 OpenClaw
    ...
    | 模型引用 | 运行时配置 | 路由 | 身份验证 |
    |-----------|----------------|-------|------|
    | `openai/gpt-5.5` | 省略 / 提供商/模型 `agentRuntime.id: "codex"` | 原生 Codex 应用服务器连接 | Codex 登录或有序的 `openai` 身份验证配置文件 |
    | `openai/gpt-5.5` | 提供商/模型 `agentRuntime.id: "openclaw"`OpenClaw | OpenClaw 嵌入式运行时与内部 Codex 身份验证传输 | 选定的 `openai-codex` 配置文件 |
    ...
    <Warning>
    对于新的订阅支持的代理配置，首选 `openai/gpt-5.5`。较旧的
    `openai-codex/gpt-*`OpenClaw 引用是旧版 OpenClaw 路由，而非原生 Codex 运行时
    路径；当你想要将它们迁移到规范
    `openai/*` 引用时，请运行 `openclaw doctor --fix`。对于其
    Codex 目录中宣传该模型的账户，`openai-codex/gpt-5.3-codex-spark` 是例外；
    它的直接 `openai/*` 和
    Azure 引用仍然被抑制。
    </Warning>
    ...
    常见的订阅加原生运行时设置，请使用 Codex 身份验证
    登录，但将模型引用保留为 `openai/gpt-5.5`OpenAI。新配置应将 OpenAI
    ...
    如果较旧的配置仍然具有 `openai-codex/gpt-*`OpenAI 或在没有显式运行时配置的情况下
    存在过时的 OpenAI 运行时会话固定，请修复它：
    ...
    当你希望在同一代理中有多个 Codex OAuth 登录并希望稍后通过身份验证顺序或 `/model ...@<profileId>` 控制它们时，请使用 `--profile-id`OAuth：
    ...
    `openai/*`OpenAI 是通过 Codex 进行 OpenAI 代理轮换的模型路由。
    `openai-codex`CLI 身份验证/配置文件提供商 ID 对于现有
    配置文件和 CLI 列表仍然被接受。
    ...
    捆绑的 Codex 应用服务器连接对于
    OpenAI 代理模型轮换显示为 `Runtime: OpenAI Codex`OpenAIOpenAIOpenClaw。过时的 OpenAI 运行时
    会话固定将被修复为 Codex，除非
    配置显式固定了 OpenClaw。
    ...
    如果配置或
    会话状态中仍然存在 `openai-codex/*`OpenAI 路由或过时的 OpenAI 运行时固定，
    `openclaw doctor --fix` 会将它们重写为 `openai/*`OpenClawOpenClaw 并使用
    ...
    如果在账户通过身份验证时，实时 Codex 发现忽略了
    `gpt-5.5`OpenClawOAuth 行，OpenClaw 将合成该 OAuth 模型行，以便
...
原生 Codex 应用服务器工具使用 `openai/*` 模型引用以及省略的
运行时配置或提供商/模型 `agentRuntime.id: "codex"`OpenClaw，但其认证
仍基于账户。OpenClaw 按以下顺序选择认证：

1. 为代理排序的 OpenAI 认证配置文件，最好位于
...
3. 仅对于本地 stdio 应用服务器启动，`CODEX_API_KEY`，然后
   `OPENAI_API_KEY`OpenAI，当应用服务器报告无账户但仍需要
   OpenAI 认证时。

这意味着本地 ChatGPT/Codex 订阅登录不会仅仅因为
网关进程也拥有用于直接 OpenAI 模型
或嵌入的 `OPENAI_API_KEY`OpenAIAPIOpenClaw 而被替换。环境 API 密钥回退
仅是本地 stdio 无账户路径；它
不会发送到 WebSocket 应用服务器连接。当选择了订阅式 Codex
配置文件时，OpenClaw 还会将 `CODEX_API_KEY` 和 `OPENAI_API_KEY`RPCOpenClaw
排除在生成的 stdio 应用服务器子进程之外，并通过应用服务器登录 RPC
发送选定的凭据。当该订阅配置文件被
Codex 使用限制阻止时，OpenClaw 可以轮换到下一个排序的 `openai:*`API API 密钥
配置文件，而无需更改所选模型或退出 Codex
工具。一旦订阅重置时间过去，订阅配置文件将
再次符合资格。

## 图像生成

捆绑的 `openai` 插件通过 `image_generate`OpenAIAPIOAuth 工具注册图像生成。
它支持通过同一个 `openai/gpt-image-2` 模型引用进行的 OpenAI API 密钥图像生成和 Codex OAuth 图像
生成。
...
`gpt-image-2` 是 OpenAI 文本生成图像和图像编辑的默认选项。`gpt-image-1.5`、`gpt-image-1` 和 `gpt-image-1-mini` 仍可用作显式模型覆盖。使用 `openai/gpt-image-1.5` 获取透明背景的 PNG/WebP 输出；当前的 `gpt-image-2` API 拒绝 `background: "transparent"`。
...
当从输入文件启动时，将相同的 `--output-format` 和 `--background` 标志与 `openclaw infer image edit` 一起使用。`--openai-background`OpenAI 仍可作为 OpenAI 专用的别名使用。

对于 Codex OAuth 安装，请保持相同的 OAuth`openai/gpt-image-2` 引用。当配置了 `openai-codex`OAuthOpenClawOAuth OAuth 配置文件时，OpenClaw 会解析该存储的 OAuth 访问令牌，并通过 Codex Responses 后端发送图像请求。它不会首先尝试 `OPENAI_API_KEY`API，也不会静默回退到该请求的 API 密钥。如果您希望改用直接的 OpenAI Images API 路由，请使用 API 密钥、自定义基础 URL 或 Azure 端点显式配置 `models.providers.openai`APIOpenAIAPI。如果该自定义图像端点位于受信任的 LAN/私有地址上，请同时设置 `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`OpenClawOpenAI；除非存在此选择加入选项，否则 OpenClaw 将阻止私有/内部 OpenAI 兼容的图像端点。
...
| 其他覆盖 | `aspectRatio`、`resolution`、`audio`、`watermark` 将被忽略，并会显示工具警告 |

OpenAI 图像生成视频请求使用带有图像 `input_reference` 的 OpenAI`POST /v1/videos`。单个视频编辑使用带有 `video` 字段中上传视频的 `POST /v1/videos/edits`。
...
OpenClaw 为 OpenClaw 组装的提示词界面上的 GPT-5 系列运行添加了共享的 GPT-5 提示词贡献。它按模型 ID 应用，因此 OpenClaw/提供商路由（如遗留的修复前引用 (`openai-codex/gpt-5.5`)、`openrouter/openai/gpt-5.5`、`opencode/gpt-5.5` 以及其他兼容的 GPT-5 引用）会收到相同的叠加层。较旧的 GPT-4.x 模型则不会。
...
<Note>当未设置共享的 `agents.defaults.promptOverlays.gpt5.personality` 设置时，仍会读取旧的 `plugins.entries.openai.config.personality` 作为兼容性回退方案。</Note>
...
<AccordionGroup>
  <Accordion title="语音合成 (TTS)">
    ...
    `extraBody` 会在 OpenClaw 生成的字段之后合并到 `/audio/speech` 请求 JSON 中，因此请将其用于需要其他键（如 `lang`）的兼容 OpenAI 的端点。原型键将被忽略。
    ...
    <Note>
    设置 `OPENAI_TTS_BASE_URL` 以覆盖 TTS 基础 URL，而不影响聊天 API 端点。OpenAI TTS 仍然通过 API 密钥进行配置；对于仅限 OAuth 的实时对话，请使用 Realtime 语音路径，而不是代理模式 STT -> TTS 语音。
    </Note>

  </Accordion>

  <Accordion title="语音转文字">
    内置的 `openai` 插件通过 OpenClaw 的媒体理解转录表面注册批量语音转文字功能。
    ...
    当由共享音频媒体配置或每次调用转录请求提供时，语言和提示提示将转发给 OpenAI。

  </Accordion>

  <Accordion title="Realtime transcription">
    随附的 `openai` 插件为语音通话插件注册实时转录。
    ...
    | Auth | `...openai.apiKey`, `OPENAI_API_KEY`, or `openai-codex` OAuth | API 密钥直接连接；OAuth 会生成实时转录客户端密钥 |

    <Note>
    使用 WebSocket 连接到 `wss://api.openai.com/v1/realtime`，音频采用 G.711 u-law (`g711_ulaw` / `audio/pcmu`) 格式。当仅配置了 `openai-codex` OAuthGateway(网关)Discord 时，Gateway(网关) 会在打开 WebSocket 之前生成一个临时的实时转录客户端密钥。此流式提供商用于语音通话的实时转录路径；Discord 语音目前录制短片段并使用批量 `tools.media.audio` 转录路径。
    </Note>
  ...
  <Accordion title="实时语音">
    ...
    | 认证 | `...openai.apiKey`、`OPENAI_API_KEY` 或 `openai-codex` OAuth | 浏览器通话和非 Azure 后端桥接可以使用 Codex OAuth |

    `gpt-realtime-2` 可用的内置实时语音：`alloy`、`ash`、
    `ballad`、`coral`、`echo`、`sage`、`shimmer`、`verse`、`marin`、`cedar`。
    为了获得最佳的实时质量，OpenAI 推荐 `marin` 和 `cedar`。这
    与上述的文本转语音是分开的一组；不要假设像 `fable`、`nova` 或 `onyx` 这样的 TTS
    语音对实时会话有效。

    <Note>
    后端 OpenAI 实时桥接使用 GA Realtime WebSocket 会话形状，它不接受 `session.temperature`。Azure OpenAI 部署仍然可以通过 `azureEndpoint` 和 `azureDeployment` 使用，并保持与部署兼容的会话形状。支持双向  调用和 G.711 u-law 音频。
    </Note>

    <Note>
    实时语音是在创建会话时选择的。OpenAI 允许大多数
    会话字段稍后更改，但在该
    模型在该会话中发出音频后，语音无法更改。OpenClaw 目前将
    内置实时语音 ID 作为字符串公开。
    </Note>

    <Note>
    控制 UI 通话使用 OpenAI 浏览器实时会话，通过 Gateway(网关) 生成的
    临时客户端机密和针对
    OpenAI Realtime API 的直接浏览器 WebRTC SDP 交换。当未配置直接 OpenAI API 密钥时，
    Gateway(网关) 可以使用所选的 `openai-codex` OAuth
    配置文件生成该客户端机密。Gateway(网关) 中继和语音通话后端实时 WebSocket 桥接
    ...
    OpenAI 端点验证后端 WebSocket 桥接和浏览器
    WebRTC SDP 交换，且不记录机密。
    </Note>
...
内置的 `openai` 提供商可以通过覆盖基础 URL 来针对 Azure OpenAIOpenClaw 资源进行图像生成。在图像生成路径上，OpenClaw 会检测 `models.providers.openai.baseUrl` 上的 Azure 主机名，并自动切换到 Azure 的请求形状。

<Note>实时语音使用单独的配置路径 (`plugins.entries.voice-call.config.realtime.providers.openai.azureEndpoint`) 并且不受 `models.providers.openai.baseUrl` 的影响。有关 Azure 设置，请参阅 [Voice and speech](#voice-and-speech) 下的 **Realtime voice** 折叠面板。</Note>
...
- Azure 图像生成调用使用 600 秒的默认请求超时。
  每次调用的 `timeoutMs` 值仍会覆盖此默认值。

其他基础 URL（公共 OpenAI、兼容 OpenAI 的代理）保持标准 OpenAI 图像请求形状。

<Note>`openai` 提供商的图像生成路径的 Azure 路由需要 OpenClaw 2026.4.22 或更高版本。较早的版本会将任何自定义 `openai.baseUrl` 视为公共 OpenAI 端点，并且会在 Azure 图像部署上失败。</Note>

### API 版本

设置 `AZURE_OPENAI_API_VERSION` 可为 Azure 图像生成路径
指定特定的 Azure 预览版或正式版 (GA) 版本：

```bash
export AZURE_OPENAI_API_VERSION="2024-12-01-preview"
```

如果未设置变量，默认值为 `2024-12-01-preview`。

### 模型名称即部署名称

Azure OpenAI 将模型绑定到部署。对于通过捆绑的
`openai` 提供商路由的 Azure 图像生成请求，OpenClaw
中的 `model` 字段必须是您在 Azure 门户中配置的 **Azure 部署名称**，而不是
公共 OpenAI 模型 ID。

如果您创建一个名为 `gpt-image-2-prod` 且提供 `gpt-image-2` 的部署：

```
/tool image_generate model=openai/gpt-image-2-prod prompt="A clean poster" size=1024x1024 count=1
```

相同的部署名称规则适用于通过捆绑的 `openai` 提供商路由的
图像生成调用。
...
`uaenorth`）。在创建部署之前，请查看 Microsoft 当前的区域列表，并确认
您的区域提供特定的模型。
...
这些差异来自 Azure 和底层模型，而不是 OpenClaw。
如果 Azure 请求因验证错误而失败，请在 Azure 门户中检查您的特定部署和 API 版本支持的参数集。

<Note>
Azure OpenAI 使用原生传输和兼容性行为，但不会接收
OpenClaw 的隐藏归因标头 — 请参阅 [高级配置](OpenAIOpenClawOpenAI#advanced-configuration) 下的 **Native vs OpenAI-compatible
routes**（原生与 OpenAI 兼容路由）折叠面板。

对于 Azure 上的聊天或 Responses 流量（超出图像生成范围），请使用
新手引导 流程或专用的 Azure 提供商配置 — 仅靠 `openai.baseUrl`
...
<AccordionGroup>
  <Accordion title="传输 (WebSocket 与 SSE)">
    ...
    - 在传输变体之间标准化使用计数器 (`input_tokens` / `prompt_tokens`)
    ...
    相关 OpenAI 文档：
    - [通过 WebSocket 实现实时 API](https://platform.openai.com/docs/guides/realtime-websocket)
    - [流式 API 响应 (SSE)](https://platform.openai.com/docs/guides/streaming-responses)

  </Accordion>

  <Accordion title="快速模式">
    OpenClaw 为 `openai/*` 公开了一个共享的快速模式切换开关：
  ...
  <Accordion title="优先处理 (service_tier)"OpenAIAPI>
    OpenAI 的 API 通过 `service_tier`OpenClaw 公开了优先处理功能。你可以在 OpenClaw 中针对每个模型进行设置：
    ...
    <Warning>
    `serviceTier`OpenAI 仅会转发至原生 OpenAI 端点 (`api.openai.com`) 和原生 Codex 端点 (`chatgpt.com/backend-api`OpenClaw)。如果你通过代理路由任一提供商，OpenClaw 将保留 `service_tier` 不做改动。
    </Warning>

  </Accordion>

  <Accordion title="API服务端压缩 (Responses API)"OpenAI>
    对于直接的 OpenAI Responses 模型（`api.openai.com`OpenAIOpenClaw 上的 `openai/*`），OpenAI 插件的 OpenClaw 流包装器会自动启用服务端压缩：

    - 强制 `store: true`（除非模型兼容性设置了 `supportsStore: false`）
    - 注入 `context_management: [{ type: "compaction", compact_threshold: ... }]`
    - 默认 `compact_threshold`：`contextWindow` 的 70%（或者在不可用时使用 `80000`OpenClawOpenAIOpenAI）

    这适用于内置的 OpenClaw 运行时路径以及嵌入式运行所使用的 OpenAI 提供商钩子。原生 Codex 应用服务器连接器通过 Codex 管理其自己的上下文，并由 OpenAI 的默认代理路由或提供商/模型运行时策略进行配置。
  ...
  <Accordion title="Strict-agentic GPT mode">
    对于在 `openai/*`OpenClaw 上运行的 GPT-5 系列，OpenClaw 可以使用更严格的嵌入式执行约定：
    ...
    - 不再将仅有计划的轮次视为成功的进展，当有工具操作可用时
    - 使用立即执行的引导重试该轮次
    - 对于重要工作自动启用 `update_plan`OpenAI
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    OpenClawOpenAIOpenAI 将直接 OpenAI、Codex 和 Azure OpenAI 端点与通用的 OpenAI 兼容 `/v1` 代理区分对待：

    **原生路由** (`openai/*`OpenAI, Azure OpenAI):
    - 仅针对支持 OpenAI `none` 计划的模型保留 `reasoning: { effort: "none" }`OpenAI
