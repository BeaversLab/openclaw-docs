...
<Note>OpenClaw 默认将 Mistral 实时 STT 设置为 8 kHz 的 `pcm_mulaw`，以便 Voice Call can forward Twilio media frames directly. 仅当您的上游流已经是原始 PCM 时才使用 `encoding: "pcm_s16le"` 和 matching `sampleRate`.</Note>

## 高级配置

<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    `mistral/mistral-small-latest` 映射到 Mistral Small 4，并支持通过 `reasoning_effort` 在 Chat Completions API 上进行 [adjustable reasoning](https://docs.mistral.ai/capabilities/reasoning/adjustable)（`none` 最大程度减少输出中的额外思考；`high` 在最终答案之前显示完整的思考轨迹）。
  ...
  <Accordion title="Memory embeddings">
    Mistral 可以通过 `/v1/embeddings` 提供内存嵌入（默认模型：`mistral-embed`）。
...
<CardGroup cols={2}>
  ...
  <Card title="Media understanding" href="/zh/nodes/media-understanding" icon="microphone">
    音频转录设置和提供商选择。
  </Card>
</CardGroup>
