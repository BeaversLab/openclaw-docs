    ...
    **最适合：** 全面控制云端或本地设置。
    ...
    <Steps>
      ...
      <Step title="Enable Ollama for OpenClaw">
        对于 `Cloud only`，请使用您真实的 `OLLAMA_API_KEY`。对于基于主机的设置，任何占位符值均可：
...
<Accordion title="模型成本" Ollama>
  ...
  <Accordion title="Memory embeddings">
    捆绑的 Ollama 插件为
    [memory search](/en/concepts/memory) 注册了一个 memory embedding 提供商。它使用配置的 Ollama 基础 URL
    和 API 密钥，调用 Ollama 当前的 `/api/embed` 端点，并在可能时将
    多个 memory chunks 批处理到一个 `input` 请求中。

    | 属性      | 值               |
    | ------------- | ------------------- |
    | 默认模型 | `nomic-embed-text`  |
    | 自动拉取     | 是 — 如果本地不存在，embedding 模型将自动拉取 |

    查询时的 embeddings 对需要或推荐使用检索前缀的模型使用检索前缀，包括 `nomic-embed-text`、`qwen3-embedding` 和 `mxbai-embed-large`。Memory document 批次保持原始格式，因此现有索引不需要格式迁移。
    ...
    对于远程 embedding 主机，请将 auth 限制在该主机范围内：
  ...
  <Accordion title="流式配置"OpenClawOllamaOllamaAPI>
    OpenClaw 的 Ollama 集成默认使用 **Ollama 原生 API** (`/api/chat`)，它完全支持同时进行流式传输和工具调用。无需特殊配置。

    对于原生 `/api/chat`OpenClawOllama 请求，OpenClaw 还会将思维控制直接转发给 Ollama：`/think off` 和 `openclaw agent --thinking off` 发送顶层的 `think: false`，除非配置了显式的模型 `params.think`/`params.thinking` 值，而 `/think low|medium|high` 则发送匹配的顶层 `think` 努力度字符串。`/think max`Ollama 映射到 Ollama 的最高原生努力度，`think: "high"`OpenAIOpenAI。
...
<AccordionGroup>
  <Accordion title="WSL2WSL2 崩溃循环（反复重启）"WSL2OllamaLinux>
    在带有 NVIDIA/CUDA 的 WSL2 上，官方 Ollama Linux 安装程序会创建一个带有 `Restart=always`WSL2OllamaWindowsWSL2OllamaWSL2Windows 的 `ollama.service` systemd 单元。如果该服务在 WSL2 启动期间自动启动并加载了 GPU 支持的模型，Ollama 可能会在模型加载时锁定主机内存。Hyper-V 内存回收机制并不总是能回收这些被锁定的页面，因此 Windows 可能会终止 WSL2 虚拟机，systemd 随后再次启动 Ollama，从而导致循环重复。

    常见迹象：

    - Windows 侧反复重启或终止 WSL2
    - WSL2 启动后不久，`app.slice` 或 `ollama.service`WSL2LinuxOpenClawWSL2 的 CPU 占用率很高
    - 收到来自 systemd 的 SIGTERM 信号，而不是 Linux OOM-killer 事件

    当 OpenClaw 检测到 WSL2、启用了带有 `Restart=always` 的 `ollama.service` 以及可见的 CUDA 标记时，会记录一条启动警告。
    ...
    将此添加到 Windows 侧的 `%USERPROFILE%\.wslconfig`Windows 中，然后运行 `wsl --shutdown`：
    ...
    在 Ollama 服务环境中设置更短的保持活动时间，或者仅在需要时手动启动 Ollama：
  ...
  <Accordion title="Ollama未检测到 Ollama"Ollama>
  ...
  <Accordion title="没有可用的模型">
    如果未列出您的模型，请在本地拉取该模型，或在 `models.providers.ollama` 中显式定义它。
  ...
  <Accordion title="OpenClaw远程主机可用 curl 但 OpenClaw 不可用"Gateway(网关)>
    ...
    - 该 URL 使用了 `/v1`OpenAIOllamaOllama，这会选择 OpenAI 兼容行为，而不是 Ollama 原生行为。
    - 远程主机需要在 Ollama 一侧更改防火墙或局域网绑定设置。
    - 该模型存在于您笔记本电脑的守护进程上，但不存在于远程守护进程上。

  </Accordion>

  <Accordion title="模型将工具 JSON 输出为文本"OpenAIOllama>
  ...
  <Accordion title="GLMKimi 或 GLM 返回乱码符号"GLM>
    托管的 Kimi/GLM 响应如果是长串的非语言符号，将被视为提供商输出失败，而不是成功的助手回答。这使得正常的重试、回退或错误处理机制能够接管，而不会将损坏的文本持久化到会话中。

    如果这种情况反复发生，请记录原始模型名称、当前会话文件，以及运行是否使用了 `Cloud + Local` 或 `Cloud only`，然后尝试一个新会话和一个回退模型：
  ...
  <Accordion title="本地冷模型超时">
    大型本地模型在开始流式传输前可能需要很长的首次加载时间。请将超时范围限制在 Ollama 提供商，并可选择要求 Ollama 在对话之间保持模型加载状态：
    ...
    如果主机本身接受连接的速度很慢，`timeoutSeconds` 也会延长针对此提供商受保护的 Undici 连接超时时间。

  </Accordion>

  <Accordion title="大上下文模型太慢或内存不足">
    许多 Ollama 模型宣传的上下文大小可能超出了您硬件的舒适运行范围。原生 Ollama 使用 Ollama 自有的运行时上下文默认值，除非您设置了 `params.num_ctx`。当您希望获得可预测的首字延迟时，请同时限制 OpenClaw 的预算和 Ollama 的请求上下文：
    ...
    如果 OpenClaw 发送的提示过多，请先降低 `contextWindow`。如果 Ollama 正在加载对该机器而言过大的运行时上下文，请降低 `params.num_ctx`。如果生成时间过长，请降低 `maxTokens`。
