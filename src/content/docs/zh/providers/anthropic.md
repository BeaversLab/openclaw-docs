...
OpenClaw 的共享 `/fast` 切换开关也支持直接面向公共 Anthropic 的流量，包括发送到 `api.anthropic.com` 的使用 API 密钥和 OAuth 认证的请求。
...
- 显式的 Anthropic `serviceTier` 或 `service_tier` 模型参数会在两者均已设置时覆盖 `/fast` 的默认值。
- Anthropic 在响应中的 `usage.service_tier` 下报告有效层级。在不具有优先级层级容量的账户上，`service_tier: "auto"` 可能仍会解析为 `standard`。
...
- Anthropic API 密钥智能默认值也会在未设置显式值时为 Bedrock 上的 Claude 模型引用植入 `cacheRetention: "short"`。
...
请求中包含 `extended-cache-ttl-2025-04-11` beta 标志；如果您覆盖提供商头部（请参阅 [/gateway/configuration](/en/gateway/configuration)），请保留它。

## 1M 上下文窗口 (Anthropic beta)

Anthropic 的 1M 上下文窗口处于 beta 限制阶段。在 OpenClaw 中，为受支持的 Opus/Sonnet 模型
...
3. 后续回合通过 `--resume <sessionId>` 重用已存储的 Claude 会话。
...
- 当您的配置显式引用 `claude-cli/...` 或 `claude-cli` 后端配置时，
...
如果您目前使用 setup-token 或 API 密钥配合 `anthropic/...`，并希望
...
- 移除主默认模型/允许列表路径之外的每一个旧的 `anthropic/...` 配置引用
...
- 如果您在 Claude 订阅上看到 “OAuth token refresh failed …”，请使用 setup-token 重新进行身份验证。参见 [/gateway/故障排除](/en/gateway/troubleshooting)。
- 身份验证详情和重用规则位于 [/concepts/oauth](/en/concepts/oauth)。
