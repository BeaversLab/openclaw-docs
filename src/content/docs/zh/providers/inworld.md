...
<Steps>
  <Step title="设置您的 API 密钥">
    从您的 Inworld 仪表板复制凭据（Workspace > API Keys），
    并将其设置为环境变量。该值将作为 HTTP Basic
    凭据逐字发送，因此请勿再次对其进行 Base64 编码或将其转换为不记名
    令牌。
  ...
  <Step title="发送消息">
    通过任何连接的渠道发送回复。OpenClaw 将
    使用 Inworld 合成音频，并将其作为 MP3 传送（当渠道
    预期为语音笔记时则为 OGG_OPUS）。
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="身份验证">
    Inworld 使用带有单个 Base64 编码凭据字符串的 HTTP Basic 认证。请直接从 Inworld 仪表板复制它。提供商会将其作为 `Authorization: Basic <apiKey>` 发送，而无需进一步编码，因此请勿自行进行 Base64 编码，也不要传递不记名样式（bearer-style）的令牌。有关相同的说明，请参阅 [TTS 身份验证说明](/zh/tools/tts#inworld-primary)。
  </Accordion>
  <Accordion title="模型">
  ...
  <Accordion title="音频输出">
    ...
    时，OpenClaw 会请求 Inworld 提供 `OGG_OPUS`，以便音频作为原生语音气泡播放。电话合成使用 22050 Hz 的原始 `PCM` 来馈送电话桥接。
  </Accordion>
