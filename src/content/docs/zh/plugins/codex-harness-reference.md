...
| `codexPlugins`             | disabled              | 对已迁移的源码安装精选插件的 Codex 原生插件/应用支持。请参阅 [Native Codex plugins](/zh/plugins/codex-native-plugins)。 |
| `computerUse`              | disabled              | Codex 计算机使用设置。请参阅 [Codex Computer Use](/zh/plugins/codex-computer-use)。                                     |
...
- 对于媒体理解 `image` 工具，`tools.media.image.timeoutSeconds`
  已转换为毫秒，或者为默认的 60 秒媒体值。
- 默认为 30 秒的动态工具值。

动态工具预算上限为 600000 毫秒。超时时，如果支持，OpenClaw 会中止
工具信号并向 Codex 返回失败的动态工具响应，
以便回合可以继续，而不是将会话置于 `processing`。

在 Codex 接受回合后，以及在 OpenClaw 响应回合范围的
应用服务器请求后，harness 预期 Codex 取得当前回合进展，
并最终使用 `turn/completed` 完成本机回合。如果应用服务器
保持静默 `appServer.turnCompletionIdleTimeoutMs`，OpenClaw 会尽力
中断 Codex 回合，记录诊断超时，并释放
OpenClaw 会话通道，以便后续聊天消息不会排在
陈旧的本机回合之后。

同一回合的大多数非终止通知会解除该短监视计时器，
因为 Codex 已证明该回合仍然活跃。原始 `custom_tool_call_output`
完成会使工具后的短监视计时器保持启用，因为它们是
回合范围的工具结果移交。已完成的 `agentMessage` 项和工具前的原始
助手 `rawResponseItem/completed` 项会启用助手输出释放：如果
Codex 随后在没有 `turn/completed` 的情况下保持静默，OpenClaw 会尽力中断
本机回合并释放会话通道。工具后的原始助手进度
会继续等待 `turn/completed` 或终止监视计时器。超时诊断
包括最后一个应用服务器通知方法，以及对于原始助手响应
项，包括项类型、角色、id 和有界的助手文本预览。

## 模型发现

默认情况下，Codex 插件会向应用服务器请求可用的模型。模型的可用性由 Codex 应用服务器决定，因此当 OpenClaw 升级捆绑的 `@openai/codex``appServer.command` 版本，或者当部署指向不同的 Codex 二进制文件时，列表可能会发生变化。可用性也可能取决于账户范围。在运行中的网关上使用 `/codex models` 来查看该适配器和账户的实时目录。

如果发现失败或超时，OpenClaw 将针对以下内容使用捆绑的回退目录：

- GPT-5.5
- GPT-5.4 mini
- GPT-5.2

当前捆绑的适配器是 `@openai/codex` `0.130.0`。针对该捆绑应用服务器的 `model/list` 探测返回了：
...
应用服务器目录可以为内部或专用流程返回隐藏的模型，但它们不是正常的模型选择器选项。
...
当您希望启动过程避免探测 Codex 并仅使用回退目录时，请禁用发现：
...
Codex 通过原生项目文档发现自行处理 `AGENTS.md`。OpenClaw 不会写入合成的 Codex 项目文档文件，也不会依赖 Codex 的回退文件名来查找角色文件，因为 Codex 回退仅在 `AGENTS.md` 缺失时适用。

为了与 OpenClaw 工作区保持一致，Codex harness 会解析其他启动
文件，包括 `SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、
`HEARTBEAT.md`、`BOOTSTRAP.md` 和 `MEMORY.md`（如果存在），并通过 `thread/start` 和 `thread/resume` 上的 Codex 开发者说明将它们
转发出去。
这使得工作区角色和配置文件上下文在原生 Codex
行为塑造通道上保持可见，而无需复制 `AGENTS.md`。
...
`OPENCLAW_CODEX_APP_SERVER_GUARDIAN=1` 已被移除。请
改用 `plugins.entries.codex.config.appServer.mode: "guardian"`，或
使用 `OPENCLAW_CODEX_APP_SERVER_MODE=guardian` 进行一次性本地测试。对于可重复的部署，首选配置，因为它将插件行为保留在与 Codex harness 设置的其余部分相同的已审核文件中。
