...
- `realtime.agentContext.enabled` 默认为关闭。启用后，语音通话插件会在会话设置时将有界的代理身份和选定的工作区文件封装注入到实时提供商指令中。
...
<Tabs>
  <Tab title="Google Gemini Live"API>
    ...
    `sessionResumption` 和 `contextWindowCompression` 默认开启，以实现更长的、可重新连接的通话。使用 `silenceDurationMs`、`startSensitivity` 和
    `endSensitivity` 来调整电话音频上的更快轮流对话。
