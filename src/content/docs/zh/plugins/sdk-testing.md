...
**通道测试辅助导入：** `openclaw/plugin-sdk/channel-test-helpers`
...
**提供者合约导入：** `openclaw/plugin-sdk/provider-test-contracts`

**提供者 HTTP 模拟导入：** `openclaw/plugin-sdk/provider-http-test-mocks`
...
对于新的插件测试，首选下面特定的子路径。宽泛的
`openclaw/plugin-sdk/testing` 桶文件仅用于旧版兼容性。
代码库防护机制拒绝从 `plugin-sdk/testing` 和
`plugin-sdk/test-utils` 进行新的实际导入；这些名称仅作为外部插件和兼容性记录测试的弃用兼容性接口保留。
...
| 导出                                                 | 用途                                                                                                                  |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `createTestPluginApi`                                | 为直接注册单元测试构建最小的插件 API 模拟。从 API`plugin-sdk/plugin-test-api` 导入                                    |
| `AUTH_PROFILE_RUNTIME_CONTRACT`                      | 用于原生 agent 运行时适配器的共享身份配置文件合约 fixture。从 `plugin-sdk/agent-runtime-test-contracts` 导入          |
| `DELIVERY_NO_REPLY_RUNTIME_CONTRACT`                 | 用于原生 agent 运行时适配器的共享投递抑制合约 fixture。从 `plugin-sdk/agent-runtime-test-contracts` 导入              |
| `OUTCOME_FALLBACK_RUNTIME_CONTRACT`                  | 用于原生 agent 运行时适配器的共享回退分类合约 fixture。从 `plugin-sdk/agent-runtime-test-contracts` 导入              |
| `createParameterFreeTool`                            | 为原生运行时合约测试构建动态工具 schema fixture。从 `plugin-sdk/agent-runtime-test-contracts` 导入                    |
| `expectChannelInboundContextContract`                | 断言渠道入站上下文形状。从 `plugin-sdk/channel-contract-testing` 导入                                                 |
| `installChannelOutboundPayloadContractSuite`         | 安装渠道出站负载契约用例。从 `plugin-sdk/channel-contract-testing` 导入                                               |
| `createStartAccountContext`                          | 构建渠道帐户生命周期上下文。从 `plugin-sdk/channel-test-helpers` 导入                                                 |
...
| `expectDirectoryIds`                                 | 断言目录列表函数中的渠道目录 ID。从 `plugin-sdk/channel-test-helpers` 导入                                            |
| `assertBundledChannelEntries`                        | 断言打包的渠道入口点暴露了预期的公共契约。从 `plugin-sdk/channel-test-helpers` 导入                                   |
...
| `runRealtimeSttLiveTest`                             | 使用共享音频固件运行实时 STT 提供商测试。从 `plugin-sdk/provider-test-contracts` 导入                                 |
| `normalizeTranscriptForMatch`                        | 在模糊断言之前标准化实时转录输出。从 `plugin-sdk/provider-test-contracts` 导入                                        |
| `expectExplicitVideoGenerationCapabilities`          | 断言视频提供商声明了明确的生成模式能力。从 `plugin-sdk/provider-test-contracts` 导入                                  |
| `expectExplicitMusicGenerationCapabilities`          | 断言音乐提供商声明了明确的生成/编辑能力。从 `plugin-sdk/provider-test-contracts` 导入                                 |
| `mockSuccessfulDashscopeVideoTask`                   | 安装一个成功的 DashScope 兼容视频任务响应。从 `plugin-sdk/provider-test-contracts` 导入                               |
| `getProviderHttpMocks`                               | 访问可选的提供商 HTTP/身份验证 Vitest 模拟。从 `plugin-sdk/provider-http-test-mocks` 导入                             |
...
| `setActivePluginRegistry`                            | 安装用于插件运行时测试的注册表 fixture。从 `plugin-sdk/plugin-test-runtime` 或 `plugin-sdk/channel-test-helpers` 导入 |
| `createRequestCaptureJsonFetch`                      | 在媒体辅助程序测试中捕获 JSON 获取请求。从 `plugin-sdk/test-env` 导入                                                 |
...
| `withFetchPreconnect`                                | 在安装了预连接钩子的情况下运行获取测试。从 `plugin-sdk/test-env` 导入                                                 |
| `withEnv` / `withEnvAsync`                           | 临时修补环境变量。从 `plugin-sdk/test-env` 导入                                                                       |
| `createTempHomeEnv` / `withTempHome` / `withTempDir` | 创建隔离的文件系统测试 fixture。从 `plugin-sdk/test-env` 导入                                                         |
| `createMockServerResponse`                           | 创建最小的 HTTP 服务器响应 mock。从 `plugin-sdk/test-env` 导入                                                        |
...
| `bundledPluginRoot` / `bundledPluginFile`            | 解析捆绑插件的源码或 dist fixture 路径。从 `plugin-sdk/test-fixtures` 导入                                            |
...
| `writeSkill`                                         | 编写技能 fixture。从 `plugin-sdk/test-fixtures` 导入                                                                  |
| `makeAgentAssistantMessage`                          | 构建代理记录消息 fixture。从 `plugin-sdk/test-fixtures` 导入                                                          |
...
| `setProviderWizardProvidersResolverForTest`          | 为独立测试注入提供商向导提供商                                                                                        |
| `createProviderUsageFetch`                           | 构建提供商使用获取固件                                                                                                |
...
捆绑插件合约套件也使用 SDK 测试子路径来处理仅限测试的注册表、清单、公共工件和运行时固件助手。依赖捆绑 OpenClaw 清单的核心套件保留在 `src/plugins/contracts` 下。
将新的扩展测试保留在已记录的、集中的 SDK 子路径上，例如 `plugin-sdk/plugin-test-api`、`plugin-sdk/channel-contract-testing`、
...
将手写的 `api` mock 传递给 `register(api)` 的单元测试不会执行 OpenClaw 的加载器接受门控。请为您插件依赖的每个注册表面添加至少一个由加载器支持的冒烟测试，特别是 hooks 和独占功能（如内存）。

当缺少必需的元数据或插件调用了其不拥有的功能 API 时，真实的加载器会导致插件注册失败。例如，`api.registerHook(...)` 需要 hook 名称，而 `api.registerMemoryCapability(...)` 要求插件清单或导出的入口声明 `kind: "memory"`。

### 测试运行时配置访问

在测试内置渠道插件时，请优先使用来自 `openclaw/plugin-sdk/channel-test-helpers` 的共享插件运行时 mock。其已弃用的 `runtime.config.loadConfig()` 和 `runtime.config.writeConfigFile(...)` mock 默认会抛出错误，以便测试能够捕获对兼容性 的新使用。仅当测试明确覆盖旧版兼容性行为时，才覆盖这些 mock。
...
1. **禁止单体根导入** -- `openclaw/plugin-sdk` 根 barrel 会被拒绝
2. **禁止直接导入 `src/`** -- 插件不能直接导入 `../../src/`
3. **禁止自导入** -- 插件无法导入其自身的 `plugin-sdk/<name>` 子路径
