...
有关公共能力模型、插件形状以及所有权/执行契约，请参阅 [Plugin architecture](</en/plugins/architectureGateway(网关)>)。本页面是关于内部机制的参考：加载管道、注册表、运行时挂钩、Gateway(网关) HTTP 路由、导入路径和架构表。
...
- **Config-time hooks**：`catalog`（旧版 `discovery`）以及
  `applyConfigDefaults`。
- **运行时挂钩**：涵盖身份验证、模型解析、流包装、思考级别、重放策略和使用端点的 40 多个可选挂钩。请参阅 [Hook order and usage](#hook-order-and-usage) 下的完整列表。
...
捆绑的提供商插件结合了上述钩子，以适应每个供应商的目录、身份验证、思考、重放和使用需求。权威的钩子集位于每个插件下的 `extensions/` 中；本页面展示了形状，而不是镜像列表。

<AccordionGroup>
  <Accordion title="Pass-through catalog providers">OpenRouter、Kilocode、Z.AI、xAI 注册 `catalog` 以及 `resolveDynamicModel` / `prepareDynamicModel`，以便它们可以在 OpenClaw 的静态目录之前展示上游模型 ID。</Accordion>
  <Accordion title="OAuth and usage endpoint providers">GitHub Copilot、Gemini CLI、ChatGPT Codex、MiniMax、Xiaomi、z.ai 将 `prepareRuntimeAuth` 或 `formatApiKey` 与 `resolveUsageAuth` + `fetchUsageSnapshot` 配对，以拥有令牌交换和 `/usage` 集成。</Accordion>
  <Accordion title="重放和转录清理系列">共享命名系列（`google-gemini`、`passthrough-gemini`、 `anthropic-by-model`、`hybrid-anthropic-openai`）允许提供商通过 `buildReplayPolicy` 选择加入 转录策略，而不是让每个插件重新实现清理。</Accordion>
  ...
  <Accordion title="Anthropic 专用流助手">Beta 头、`/fast` / `serviceTier` 和 `context1m` 位于 Anthropic 插件公共 `api.ts` / `contract-api.ts` 接缝 （`wrapAnthropicProviderStream`、`resolveAnthropicBetas`、 `resolveAnthropicFastMode`、`resolveAnthropicServiceTier`）内，而不是在通用 SDK 中。</Accordion>
</AccordionGroup>
...
- `textToSpeech` 返回用于文件/语音笔记界面的常规核心 TTS 输出载荷。
...
- 使用语音提供商来处理供应商拥有的合成行为。
- 旧版 Microsoft `edge` 输入会被标准化为 `microsoft` 提供商 ID。
- 首选的所有权模型是以公司为导向的：一个供应商插件可以拥有文本、语音、图像以及未来的媒体提供商，正如 OpenClaw 添加这些能力合约时那样。
...
- `extractStructuredWithModel(...)`OpenClaw 是有界的、提供商拥有的、以图像为优先的提取的插件面向接缝。至少包含一个图像输入；文本输入是补充上下文。
  产品插件拥有其路由和架构，而 OpenClaw 拥有提供商/运行时边界。
...
- OpenClaw 仅针对可信调用者遵守这些覆盖字段。
- 对于插件拥有的后备运行，操作员必须使用 `plugins.entries.<id>.subagent.allowModelOverride: true` 选择加入。
- 使用 `plugins.entries.<id>.subagent.allowedModels` 将可信插件限制为特定的规范 `provider/model` 目标，或者使用 `"*"` 显式允许任何目标。
- 不受信任的插件子代理运行仍然有效，但覆盖请求将被拒绝，而不是静默回退。
- 插件创建的子代理会话会标记创建者的插件 ID。后备 `api.runtime.subagent.deleteSession(...)` 只能删除这些拥有的会话；任意会话的删除仍然需要管理员范围的 Gateway(网关) 请求。

对于 Web 搜索，插件可以使用共享的运行时辅助工具，而无需深入到代理工具的连接中：
...
- 使用 Web 搜索提供商处理特定供应商的搜索传输。
- `api.runtime.webSearch.*` 是需要搜索行为但不依赖代理工具封装的功能/渠道插件的首选共享接口。
...
- 具有不同 `auth` 级别的重叠路由将被拒绝。请仅在相同的身份验证级别上保留 `exact`/`prefix` 透传链。
- `auth: "plugin"`Gateway(网关) 路由**不会**自动接收操作员运行时作用域。它们用于由插件管理的 Webhook/签名验证，而非特权 Gateway(网关) 辅助调用。
- `auth: "gateway"`Gateway(网关) 路由在 Gateway(网关) 请求运行时作用域内运行，但该作用域是有意保守的：
  ...
  - 承载受信任身份的 HTTP 模式（例如私有入口上的 `trusted-proxy` 或 `gateway.auth.mode = "none"`）仅在标头显式存在时才遵守 `x-openclaw-scopes`
  - 如果在那些承载身份的插件路由请求中缺少 `x-openclaw-scopes`，运行时作用域将回退到 `operator.write`
- 实用规则：不要假设网关身份验证插件路由是隐式的管理员操作面。如果您的路由需要仅管理员行为，请要求使用承载身份的身份验证模式，并记录显式的 `x-openclaw-scopes` 标头约定。

## Plugin SDK 导入路径

在编写新插件时，请使用狭窄的 SDK 子路径，而不是单一的 `openclaw/plugin-sdk` 根桶。核心子路径：
...
渠道插件从一系列狭缝 `channel-setup`、
`setup-runtime`、`setup-tools`、`channel-pairing`、
`channel-contract`、`channel-feedback`、`channel-inbound`、`channel-outbound`、
`command-auth`、`secret-input`、`webhook-ingress`、
`channel-targets` 和 `channel-actions` 中进行选择。审批行为应合并
到一个 `approvalCapability` 协议上，而不是在不相关的插件字段之间混合。请参阅 [渠道插件](/zh/plugins/sdk-channel-plugins)。

运行时和配置辅助工具位于匹配的专用 `*-runtime` 子路径下
(`approval-runtime`、`agent-runtime`、`lazy-runtime`、`directory-runtime`、
`text-runtime`、`runtime-store`、`system-event-runtime`、`heartbeat-runtime`、
`channel-activity-runtime` 等)。首选 `config-contracts`、
`plugin-config-runtime`、`runtime-config-snapshot` 和 `config-mutation`，
而不是宽泛的 `config-runtime` 兼容性桶。

<Info>`openclaw/plugin-sdk/channel-runtime`、`openclaw/plugin-sdk/channel-lifecycle`、 小型渠道辅助工具门面、`openclaw/plugin-sdk/outbound-runtime`、 `openclaw/plugin-sdk/outbound-send-deps`、`openclaw/plugin-sdk/config-runtime` 和 `openclaw/plugin-sdk/infra-runtime` 是已弃用的兼容性垫片，用于 旧插件。新代码应改为导入更窄的通用原语。</Info>

仓库内部入口点（位于每个打包的插件包根目录）：

- `index.js` — 打包插件入口
- `api.js` — 辅助/类型桶（helper/types barrel）
- `runtime-api.js` — 仅运行时桶（runtime-only barrel）
- `setup-entry.js` — 设置插件入口（setup plugin entry）

外部插件应仅导入 `openclaw/plugin-sdk/*` 子路径。切勿
从核心或从其他插件导入另一个插件包的 `src/*`。
Facade 加载的入口点优先使用现有的活动运行时配置快照，然后
回退到磁盘上解析的配置文件。

特定功能的子路径（如 `image-generation`、`media-understanding`
和 `speech`）之所以存在，是因为捆绑插件目前使用了它们。它们并
非自动长期冻结的外部契约——依赖它们时请查阅相关的 SDK
参考页面。

## 消息工具模式（Message 工具 schemas）

插件应拥有针对反应、已读和投票等非消息原语的特定渠道 `describeMessageTool(...)` 模式
贡献。共享发送呈现应使用通用 `MessagePresentation` 契约，
而不是提供商原生的按钮、组件、块或卡片字段。
有关契约、
回退规则、提供商映射和插件作者清单，请参阅 [消息呈现](/zh/plugins/message-presentation)。

具备发送能力的插件通过消息功能声明它们可以呈现的内容：

- `presentation` 用于语义呈现块（`text`、`context`、`divider`、`buttons`、`select`）
- `delivery-pin` 用于固定传递请求（pinned-delivery requests）

核心决定是原生呈现呈现内容还是将其降级为文本。
不要从通用消息工具中暴露提供商原生的 UI 逃生舱（escape hatches）。
用于遗留原生模式的已弃用 SDK 辅助函数仍会为现有的
第三方插件导出，但新插件不应使用它们。

## 渠道目标解析（Channel target resolution）

渠道插件应拥有特定渠道的目标语义。保持共享
出站主机的通用性，并使用消息适配器表面（adapter surface）来处理提供商规则：

- `messaging.inferTargetChatType({ to })` 决定在目录查找之前，是否应将标准化目标视为 `direct`、`group` 或 `channel`。
- `messaging.targetResolver.looksLikeId(raw, normalized)` 告诉核心输入是否应跳过目录搜索，直接进行类似 ID 的解析。
- `messaging.targetResolver.resolveTarget(...)` 是插件回退机制，当核心在标准化后或目录未命中后需要最终提供商拥有的解析时使用。
- 一旦解析出目标，`messaging.resolveOutboundSessionRoute(...)` 负责提供商特定的会话路由构造。

建议拆分：

- 使用 `inferTargetChatType` 进行应在搜索对等/组之前进行的类别决策。
- 使用 `looksLikeId` 进行“将其视为显式/原生目标 ID”的检查。
- 使用 `resolveTarget` 进行提供商特定的标准化回退，而不是用于广泛的目录搜索。
- 将提供商原生 ID（如聊天 ID、线程 ID、JID、句柄和房间 ID）保留在 `target` 值或提供商特定参数中，而不是通用 SDK 字段中。

## 基于配置的目录

从配置派生目录条目的插件应将该逻辑保留在插件中，并重用 `openclaw/plugin-sdk/directory-runtime` 中的共享辅助工具。

当渠道需要基于配置的对等/组时使用此功能，例如：
...
`catalog.run(...)`OpenClaw 返回与 OpenClaw 写入 `models.providers` 相同的形状：

- 用于一个提供商条目的 `{ provider }`
- 用于多个提供商条目的 `{ providers }`

当插件拥有特定于提供商的模型 ID、基础 URL 默认值或通过身份验证的模型元数据时，请使用 `catalog`。

`catalog.order` 控制插件的目录相对于 OpenClaw 内置隐式提供商的合并时机：

- `simple`：普通 API 密钥或环境驱动的提供商
- `profile`：在身份验证配置文件存在时出现的提供商
- `paired`：合成多个相关提供商条目的提供商
- `late`：最后一遍，在其他隐式提供商之后

如果键发生冲突，后面的提供商会胜出，因此插件可以使用相同的提供商 ID 有意覆盖内置提供商条目。

插件还可以通过 `api.registerModelCatalogProvider({ 提供商, kinds, staticCatalog, liveCatalog
})` 发布只读模型行。这是列表/帮助/选择器界面的前向路径，支持 `text`、`image_generation`、`video_generation` 和 `music_generation` 行。
提供商插件仍然拥有实时端点调用、令牌交换和供应商响应映射；核心拥有通用行形状、源标签和媒体工具帮助格式。媒体生成提供商注册会自动从 `defaultModel`、`models` 和 `capabilities` 合成静态目录行。

兼容性：

- `discovery` 作为旧别名仍然有效，但会发出弃用警告
- 如果同时注册了 `catalog` 和 `discovery`，OpenClaw 将使用 `catalog`
- `augmentModelCatalog` 已弃用；捆绑提供商应通过 `registerModelCatalogProvider` 发布补充行
...
- `resolveAccount(...)` 是运行时路径。它允许假定凭据已完全具体化，并且在缺少所需机密时可以快速失败。
...
这使得只读命令可以报告“已配置但在当前命令路径中不可用”，而不是崩溃或将帐户错误地报告为未配置。
...
安全防护措施：解析符号链接后，每个 `openclaw.extensions` 条目都必须保留在插件
目录内。转义出包目录的条目将被拒绝。

安全说明：`openclaw plugins install` 会使用项目本地的 `npm install --omit=dev --ignore-scripts`npm 安装插件依赖（无生命周期脚本，运行时无 dev 依赖），并忽略继承的全局 npm 安装设置。请保持插件依赖树为“纯 JS/TS”，并避免需要 `postinstall` 构建的包。
...
可选：`openclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen` 可以让渠道插件在网关的预监听启动阶段选择进入相同的 `setupEntry` 路径，即使该渠道已经配置也是如此。

仅当 `setupEntry` 完全覆盖了网关开始监听之前必须存在的启动界面时，才使用此选项。实际上，这意味着设置入口必须注册启动所依赖的每个渠道拥有的能力，例如：

- 渠道注册本身
- 网关开始监听之前必须可用的任何 HTTP 路由
- 在同一窗口期间必须存在的任何网关方法、工具或服务

如果您的完整入口仍然拥有任何必需的启动能力，请不要启用此标志。请保持插件处于默认行为，并让 OpenClaw 在启动期间加载完整入口。

打包的渠道还可以发布仅用于设置的合约界面辅助工具，核心可以在加载完整的渠道运行时之前进行查询。当前的设置升级界面为：

- `singleAccountKeysToMove`
- `namedAccountPromotionKeys`
- `resolveSingleAccountPromotionTarget(...)`

当 Core 需要将旧的单账户渠道配置提升为 `channels.<id>.accounts.*`Matrix 而不加载完整插件入口时，会使用该接口。Matrix 是当前内置的示例：当已存在命名账户时，它仅将身份验证/引导密钥移动到命名提升账户中，并且可以保留已配置的非规范 default-account 密钥，而不是总是创建 `accounts.default`。

这些设置修补适配器使内置合约接口的发现保持惰性。导入时间保持短暂；升级接口仅在首次使用时加载，而不是在模块导入时重新进入内置渠道启动流程。

当这些启动接口包含网关 RPC 方法时，请将它们放在特定于插件的前缀下。Core 管理命名空间（`config.*`、`exec.approvals.*`、`wizard.*`、`update.*`）保留专用，并且始终解析为 `operator.admin`，即使插件请求了更窄的作用域。
...
渠道插件可以通过 `openclaw.channel` 宣传设置/发现元数据，并通过 `openclaw.install` 提供安装提示。这使得核心目录保持无数据状态。
...
- `selectionDocsPrefix`、`selectionDocsOmitLabel`、`selectionExtras`：选择界面的文本控件
...
- `quickstartAllowFrom`：将该渠道加入标准快速启动 `allowFrom` 流程
- `forceAccountBinding`：即使只存在一个账户，也要求显式绑定账户
- `preferSessionLookupForAnnounceTarget`：在解析公告目标时优先使用会话查找

OpenClaw 还可以合并**外部渠道目录**（例如，MPM 注册表导出文件）。将 JSON 文件放置在以下位置之一：
...
生成的渠道目录条目和提供商安装目录条目会在原始 `openclaw.install`npmnpm 块旁边公开
标准化的安装源信息。标准化信息标识了 npm 规范是精确版本还是浮动
选择器、是否存在预期的完整性元数据，以及本地
源路径是否可用。当目录/包身份已知时，如果解析出的 npm 包名称与该身份不符，
标准化信息会发出警告。此外，当 `defaultChoice`npmnpm 无效或指向不可用的
源，以及存在 npm 完整性元数据却缺少有效的 npm
源时，它们也会发出警告。使用者应将 `installSource` 视为附加的可选字段，以便
手动构建的条目和目录垫片无需合成该字段。
这使得新手引导和诊断无需
导入插件运行时即可解释源平面状态。

官方的外部 npm 条目最好使用确切的 `npmSpec` 加上
`expectedIntegrity`。为了兼容性，裸包名和 dist-tags 仍然可用，但它们会提示源平面警告，以便目录可以在不破坏现有插件的情况下转向固定、经过完整性检查的安装。当从本地目录路径进行新手引导安装时，它会在可能的情况下记录一个带有 `source: "path"` 和工作区相对 `sourcePath` 的托管插件索引条目。绝对的操作加载路径保留在 `plugins.load.paths` 中；安装记录避免将本地工作站路径复制到长期配置中。这使得本地开发安装对源平面诊断可见，而不会增加第二个原始文件系统路径泄露面。持久化的 `plugins/installs.json` 插件索引是安装的真实来源，可以在不加载插件运行时模块的情况下刷新。它的 `installRecords` 映射即使在插件清单丢失或无效时也是持久的；它的 `plugins` 数组是一个可重建的清单视图。

## 上下文引擎插件

上下文引擎插件拥有用于摄取、组装和压缩的会话上下文编排。使用
`api.registerContextEngine(id, factory)` 从您的插件注册它们，然后使用
`plugins.slots.contextEngine` 选择活动引擎。

当您的插件需要替换或扩展开箱即用的上下文管道，而不仅仅是添加内存搜索或钩子时，请使用此功能。
...
工厂 `ctx` 公开了用于构造时初始化的可选 `config`、`agentDir` 和 `workspaceDir`
值。

当活动的线束具有持久化的后端线程时，`assemble()` 可能会返回 `contextProjection`。对于传统的逐轮投影，请省略它。当组装好的上下文应该被注入到后端线程一次并重复使用，直到周期变更时，返回 `{ mode: "thread_bootstrap", epoch }`。在引擎的语义上下文变更后（例如在引擎拥有的压缩过程之后），更改周期。宿主可能会在线程引导投影中保留工具调用元数据、输入形状和经过编辑的工具结果，以便新的后端线程在不复制原始携带机密的负载的情况下保持工具连续性。

如果你的引擎**不**拥有压缩算法，请保留 `compact()` 的实现并显式地委托它：
...
2. 添加类型化的插件注册/运行时接口
   使用最小有用的类型化功能接口扩展 `OpenClawPluginApi` 和/或 `api.runtime`。
3. 连接核心 + 渠道/功能使用者
...
这就是 OpenClaw 在不硬编码于某个提供商的世界观的情况下保持其主张的方式。有关具体的文件清单和操作示例，请参阅[功能开发手册](/zh/tools/capability-cookbook)。

### 功能清单

当你添加一个新功能时，实现通常应该同时涉及这些接口：

- `src/<capability>/types.ts` 中的核心契约类型
- `src/<capability>/runtime.ts` 中的核心运行器/运行时辅助程序
- `src/plugins/types.ts` 中的插件 API 注册接口
- 插件注册表连接位于 `src/plugins/registry.ts`
- 当功能/渠道插件需要使用时，插件运行时暴露在 `src/plugins/runtime/*`
- `src/test-utils/plugin-registration.ts` 中的捕获/测试辅助工具
- `src/plugins/contracts/registry.ts` 中的所有权/契约断言
- `docs/` 中的操作员/插件文档

如果缺少其中任何一个接口，通常意味着该功能尚未完全集成。
...
- [插件架构](/zh/plugins/architecture) — 公共功能模型和形态
