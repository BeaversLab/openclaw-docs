...
有关公共能力模型、插件形状以及所有权/执行契约，请参阅 [Plugin architecture](</en/plugins/architectureGateway(网关)>)。本页面是内部机制的参考文档：加载管道、注册表、运行时钩子、Gateway(网关) HTTP 路由、导入路径和架构表。
...
- **Config-time hooks**：`catalog`（旧版 `discovery`）以及
  `applyConfigDefaults`。
- **Runtime hooks**：40 多个可选的 hooks，涵盖身份验证、模型解析、
  流式包装、思考级别、重放策略和使用端点。请参阅
