...
当提供商拥有基于环境变量的凭据，且通用的身份验证/状态/模型选择器路径在未加载插件运行时应看到这些凭据时，请使用清单 `providerAuthEnvVars`。当一个提供商 ID 应重用另一个提供商 ID 的环境变量、身份验证配置文件、支持配置的身份验证以及 API 密钥的新手引导选择时，请使用清单 `providerAuthAliases`。当新手引导/身份验证选择的 CLI 界面应知道提供商的选择 ID、组标签和简单的单标志身份验证连线，而无需加载提供商运行时时，请使用清单 `providerAuthChoices`。将提供商运行时 `envVars` 保留用于面向操作员的提示，例如新手引导标签或 OAuth 客户端 ID/客户端密钥设置变量。

当渠道拥有由环境驱动的身份验证或设置，且通用的 shell-env 回退、配置/状态检查或设置提示在未加载渠道运行时应看到这些信息时，请使用清单 `channelEnvVars`。
...
| 7   | `resolveConfigApiKey`             | 在运行时身份验证加载之前，为配置提供商解析环境标记身份验证                                            | 提供商拥有提供商拥有的环境标记 API 密钥解析；`amazon-bedrock` 在此处也有一个内置的 AWS 环境标记解析器      |
| 8   | `resolveSyntheticAuth`            | 提供本地/自托管或配置支持的身份验证，而不持久化明文                                                   | 提供商可以使用合成/本地凭据标记运行                                                                        |
| 9   | `resolveExternalAuthProfiles`     | 覆盖提供商拥有的外部身份验证配置文件；默认的 `persistence` 是 `runtime-only`，用于 CLI/应用拥有的凭据 | 提供商重用外部身份验证凭据，而不持久化复制的刷新令牌                                                       |
...
| 34  | `supportsXHighThinking`           | `xhigh` 所选模型的推理支持                                                                            | 提供商希望仅在模型子集上 `xhigh`                                                                           |
| 35  | `supportsAdaptiveThinking`        | `adaptive` 所选模型的思考支持                                                                         | 提供商希望仅针对具有提供商管理自适应思考的模型显示 `adaptive`                                              |
| 36  | `supportsMaxThinking`             | `max` 所选模型的推理支持                                                                              | 提供商希望仅针对具有提供商最大思考的模型显示 `max`                                                         |
| 37  | `resolveDefaultThinkingLevel`     | 特定模型系列的默认 `/think` 级别                                                                      | 提供商拥有模型系列的默认 `/think` 策略                                                                     |
| 38  | `isModernModelRef`                | 用于实时配置文件过滤和烟雾选择的现代模型匹配器                                                        | 提供商拥有实时/烟雾首选模型匹配                                                                            |
| 39  | `prepareRuntimeAuth`              | 在推理之前立即将配置的凭据交换为实际的运行时令牌/密钥                                                 | 提供商需要令牌交换或短期请求凭据                                                                           |
| 40  | `resolveUsageAuth`                | 解析 `/usage` 的使用/计费凭据及相关状态表面                                                           | 提供商需要自定义使用/配额令牌解析或不同的使用凭据                                                          |
| 41  | `fetchUsageSnapshot`              | 在身份验证解析后，获取并规范化特定提供商的使用情况/配额快照                                           | 提供商需要特定于提供商的使用情况端点或有效负载解析器                                                       |
| 42  | `createEmbeddingProvider`         | 为内存/搜索构建一个提供商拥有的嵌入适配器                                                             | 内存嵌入行为属于提供商插件                                                                                 |
| 43  | `buildReplayPolicy`               | 返回一个控制提供商脚本处理的重放策略                                                                  | 提供商需要自定义脚本策略（例如，移除思考块）                                                               |
| 44  | `sanitizeReplayHistory`           | 在通用脚本清理之后重写重放历史记录                                                                    | 提供商需要超出共享压缩辅助功能的特定于提供商的重写重写                                                     |
| 45  | `validateReplayTurns`             | 在嵌入式运行器之前进行最终的重放回合验证或重塑                                                        | 提供商传输在通用清理之后需要更严格的回合验证                                                               |
| 46  | `onModelSelected`                 | 运行提供商拥有的选择后副作用                                                                          | 提供商需要在模型激活时进行遥测或拥有提供商拥有的状态                                                       |

`normalizeModelId`、`normalizeTransport` 和 `normalizeConfig` 首先检查匹配的提供商插件，然后回退到其他支持钩子的提供商插件，直到其中一个真正更改了模型 ID 或传输/配置。这使得别名/兼容提供商填充层能够正常工作，而无需调用者知道哪个捆绑插件拥有重写权限。如果没有提供商钩子重写支持的 Google 系列配置条目，捆绑的 Google 配置规范化器仍然应用该兼容性清理。

如果提供商需要完全自定义的线路协议或自定义请求执行器，那是另一类扩展。这些钩子适用于仍在 OpenClaw 正常推理循环上运行的提供商行为。
...
- Anthropic 使用 `resolveDynamicModel`、`capabilities`、`buildAuthDoctorHint`、
  ...
  默认/自适应思考策略，以及用于 beta 标头的 Anthropic 专用流塑形、
  `/fast` / `serviceTier` 和 `context1m`。
- Anthropic 的 Claude 专用流助手目前保留在捆绑插件自己的
  公共 `api.ts` / `contract-api.ts` 接缝中。该包表面
  导出 `wrapAnthropicProviderStream`、`resolveAnthropicBetas`、
  `resolveAnthropicFastMode`、`resolveAnthropicServiceTier` 和较低级别的
  Anthropic 包装器构建器，而不是围绕一个
  提供商的 beta 标头规则来扩展通用 SDK。
- OpenAI 使用 `resolveDynamicModel`、`normalizeResolvedModel` 和
  ...
  `openai-completions` -> `openai-responses` 标准化、感知 Codex 的身份验证
  提示、Spark 抑制、合成 OpenAI 列表行以及 GPT-5 思考 /
  实时模型策略；`openai-responses-defaults` 流系列拥有
  共享的原生 OpenAI Responses 包装器，用于属性标头、
  `/fast`/`serviceTier`、文本详细度、原生 Codex 网络搜索、
  推理兼容负载塑形以及 Responses 上下文管理。
- OpenRouter 使用 `catalog` 加上 `resolveDynamicModel` 和
  `prepareDynamicModel`，因为该提供商是透传的，并且可能会在 OpenClaw 的静态目录更新之前暴露新的模型 ID；它还使用
  `capabilities`、`wrapStreamFn` 和 `isCacheTtlEligible` 将特定于提供商的请求标头、路由元数据、推理补丁和提示缓存策略排除在核心之外。其重放策略来自
  `passthrough-gemini` 系列，而 `openrouter-thinking` 流系列拥有代理推理注入，而不受支持的模型 / `auto` 则会跳过。
- GitHub Copilot 使用 `catalog`、`auth`、`resolveDynamicModel` 和
  `capabilities`，再加上 `prepareRuntimeAuth` 和 `fetchUsageSnapshot`，因为它
  需要提供商拥有的设备登录、模型回退行为、Claude 转录怪癖、GitHub 令牌到 Copilot 令牌的交换，以及提供商拥有的使用端点。
- OpenAI Codex 使用 `catalog`、`resolveDynamicModel`、
  ...
  仍在核心 OpenAI 传输上运行，但拥有其传输/基本 URL 标准化、OAuth 刷新回退策略、默认传输选择、
...
- Google AI Studio 和 Gemini CLI OAuth 使用 `resolveDynamicModel`、
  ...
  `google-gemini` 重放系列拥有 Gemini 3.1 向前兼容回退、
  原生 Gemini 重放验证、启动重放清理、标记
  推理输出模式和现代模型匹配，而
  `google-thinking` 流系列拥有 Gemini 思维负载规范化；
  Gemini CLI OAuth 也使用 `formatApiKey`、`resolveUsageAuth` 和
  `fetchUsageSnapshot` 进行令牌格式化、令牌解析和配额端点
  连接。
- Anthropic Vertex 使用 `buildReplayPolicy` 通过
  `anthropic-by-model` 重放系列，以便针对 Claude 的重放清理
  保持在 Claude ID 的范围内，而不是每个 `anthropic-messages` 传输。
- Amazon Bedrock 使用 `buildReplayPolicy`、`matchesContextOverflowError`、
  ...
  针对托管在 Bedrock 上的 Anthropic 流量的特定于 Bedrock 的
  节流/未就绪/上下文溢出错误分类；
  其重放策略仍然共享相同的
...
- MiniMax 使用 `buildReplayPolicy` 通过
  ...
  思维块删除，同时将推理
  输出模式覆盖回原生模式，并且 `minimax-fast-mode` 流系列拥有
  共享流路径上的快速模式模型重写。
- Moonshot 使用 `catalog` 加上 `wrapStreamFn`，因为它仍然使用共享的 Moonshot 传输，但需要提供商拥有的思考负载标准化；`moonshot-thinking` 流系列将配置加上 `/think` 状态映射到其原生二进制思考负载。
- Kilocode 使用 `catalog`、`capabilities`、`wrapStreamFn` 和 `isCacheTtlEligible`，因为它需要提供商拥有的请求标头、推理负载标准化、Gemini 脚本提示和 Anthropic 缓存 TTL 限制；`kilocode-thinking` 流系列在共享代理流路径上保持 Kilo 思考注入，同时跳过 `kilo/auto` 和其他不支持显式推理负载的代理模型 ID。
- Z.AI 使用 `resolveDynamicModel`、`prepareExtraParams`、`wrapStreamFn`、`isCacheTtlEligible`、`isBinaryThinking`、`isModernModelRef`、`resolveUsageAuth` 和 `fetchUsageSnapshot`，因为它拥有 GLM-5 回退、`tool_stream` 默认值、二进制思考 UX、现代模型匹配以及使用身份验证 + 配额获取；`tool-stream-default-on` 流系列使默认开启的 `tool_stream` 包装器免于每个提供商的手写胶水代码。
- xAI 使用 `normalizeResolvedModel`、`normalizeTransport`、`contributeResolvedModelCompat`、`prepareExtraParams`、`wrapStreamFn`、`resolveSyntheticAuth`、`resolveDynamicModel` 和 `isModernModelRef`，因为它拥有原生 xAI Responses 传输标准化、Grok 快速模式别名重写、默认 `tool_stream`、严格工具/推理负载清理、插件拥有工具的回退身份验证重用、向前兼容的 Grok 模型解析，以及提供商拥有的兼容性补丁，例如 xAI 工具架构配置文件、不支持的架构关键字、原生 `web_search` 和 HTML 实体工具调用参数解码。
- Mistral、OpenCode Zen 和 OpenCode Go 仅使用 `capabilities`，以便将 transcript/tooling 的怪癖排除在核心之外。
- 仅限目录捆绑的提供商，例如 `byteplus`、`cloudflare-ai-gateway`、
  `huggingface`、`kimi-coding`、`nvidia`、`qianfan`、
  `synthetic`、`together`、`venice`、`vercel-ai-gateway` 和 `volcengine`，
  仅使用 `catalog`。
- Qwen 对其文本提供商使用 `catalog`，并为其多模态表面使用共享的媒体理解和视频生成注册。
- MiniMax 和 Xiaomi 使用 `catalog` 加上 usage hooks，因为即使推​​理仍然通过共享传输运行，它们的 `/usage` 行为仍由插件拥有。
...
- `textToSpeech` 返回用于文件/语音备注表面的常规核心 TTS 输出负载。
- 使用核心 `messages.tts` 配置和提供商选择。
- 返回 PCM 音频缓冲区 + 采样率。插件必须为提供商重新采样/编码。
- 每个提供商的 `listVoices` 是可选的。将其用于供应商拥有的语音选择器或设置流程。
- 语音列表可以包含更丰富的元数据，例如区域设置、性别和个性标签，用于提供商感知的选择器。
...
- 首选的所有权模型是以公司为导向的：一个供应商插件可以拥有文本、语音、图像和未来的媒体提供商，因为 OpenClaw 添加了这些能力合约。

对于图像/音频/视频理解，插件注册一个类型化的媒体理解提供商，而不是通用的键值包：
...
- `provider` 和 `model` 是可选的每次运行覆盖项，而不是持久的会话更改。
...
- 不受信任的插件子代理运行仍然有效，但覆盖请求将被拒绝而不是静默回退。

对于网络搜索，插件可以使用共享的运行时助手，而不是深入到代理工具连接中：
...
- `api.runtime.webSearch.*` 是需要搜索行为但不依赖于代理工具包装器的功能/渠道插件的首选共享表面。
...
- `auth: "plugin"` 路由**不会**自动接收操作员运行时范围。它们用于插件管理的 Webhook/签名验证，而非特权 Gateway(网关) 辅助调用。
- `auth: "gateway"` 路由在 Gateway(网关) 请求运行时范围内运行，但该范围是有意保守的：
  - 共享密钥不记名认证（`gateway.auth.mode = "token"` / `"password"`）将插件路由运行时范围固定到 `operator.write`，即使调用者发送了 `x-openclaw-scopes`
  - 受信任的身份承载 HTTP 模式（例如私有入口上的 `trusted-proxy` 或 `gateway.auth.mode = "none"`）仅在标头明确存在时才遵守 `x-openclaw-scopes`
  - 如果那些承载身份的插件路由请求中缺少 `x-openclaw-scopes`，运行时范围将回退到 `operator.write`
- 实践规则：不要假设网关身份验证（gateway-auth）插件路由是一个隐式的管理界面。如果您的路由需要仅限管理员的行为，请要求携带身份的验证模式并记录显式的 `x-openclaw-scopes` 标头约定。
...
- `openclaw/plugin-sdk/core` 用于通用的共享插件端约定。
...
- 稳定的渠道原语，如 `openclaw/plugin-sdk/channel-setup`、
  ...
  `openclaw/plugin-sdk/webhook-ingress`，用于共享的设置/身份验证/回复/网络钩子
  连接。`channel-inbound` 是防抖、提及匹配、
  入站提及策略助手、信封格式化和入站信封
  上下文助手的共享基地。
  `channel-setup` 是狭隘的可选安装设置接缝。
  `setup-runtime` 是 `setupEntry` 使用 /
  延迟启动的运行时安全设置表面，包括导入安全的设置补丁适配器。
  `setup-adapter-runtime` 是环境感知的帐户设置适配器接缝。
  `setup-tools` 是小型的 CLI/归档/文档助手接缝（`formatCliCommand`、
...
- Domain subpaths such as `openclaw/plugin-sdk/channel-config-helpers`,
  ...
  `telegram-command-config` 是用于 Telegram 自定义
  命令规范化/验证的狭窄公共接口，即使捆绑的
  Telegram 协议表面暂时不可用，它仍然可用。
  `text-runtime` 是共享的文本/markdown/日志接口，包括
  助手可见文本剥离、markdown 渲染/分块辅助工具、编辑
  辅助工具、指令标签辅助工具和安全文本实用程序。
- 特定于审批的渠道接口应优先选择插件上的一个 `approvalCapability`
  协议。核心随后通过该单一能力读取审批授权、交付、渲染、
  原生路由和惰性原生处理程序行为，而不是将审批行为混合到
  不相关的插件字段中。
- `openclaw/plugin-sdk/channel-runtime` 已被弃用，仅作为
  较旧插件的兼容性垫片保留。新代码应改为导入
  更狭窄的通用基元，且仓库代码不应添加
  该垫片的新导入。
- 捆绑的扩展内部细节保持私有。外部插件应仅
  使用 `openclaw/plugin-sdk/*` 子路径。OpenClaw 核心/测试代码可以使用插件包根目录下的仓库
  公共入口点，例如 `index.js`、`api.js`、
  `runtime-api.js`、`setup-entry.js`，以及范围狭窄的文件，例如
  `login-qr-api.js`。切勿从核心或
  另一个扩展导入插件包的 `src/*`。
- 仓库入口点拆分：
  `<plugin-package-root>/api.js` 是辅助/类型 桶，
  `<plugin-package-root>/runtime-api.js` 是仅运行时 桶，
  `<plugin-package-root>/index.js` 是打包插件入口，
  而 `<plugin-package-root>/setup-entry.js` 是设置插件入口。
- 当前打包的提供商示例：
  - Anthropic 使用 `api.js` / `contract-api.js` 作为 Claude 流辅助工具，
    例如 `wrapAnthropicProviderStream`、beta-header 辅助工具和
    `service_tier` 解析。
  - OpenAI 使用 `api.js` 获取提供商构建器、默认模型辅助工具和
    实时提供商构建器。
  - OpenRouter 使用 `api.js` 获取其提供商构建器以及新手引导/配置
    辅助工具，而 `register.runtime.js` 仍然可以重新导出通用
    `plugin-sdk/provider-stream` 辅助工具以供仓库本地使用。
- Facade 加载的公共入口点优先使用现有的活动运行时配置快照，
  当 OpenClaw 尚未提供运行时快照时，再回退到磁盘上的已解析配置文件。
- 通用共享原语仍然是首选的公共 SDK 契约。一小部分
  保留的兼容性打包渠道品牌辅助程序接口仍然存在。
  将这些视为打包维护/兼容性接口，而不是新的
  第三方导入目标；新的跨渠道契约仍应位于
...
- 首先首选狭窄的稳定原语。较新的 setup/pairing/reply/
  feedback/contract/inbound/threading/command/secret-input/webhook/infra/
  allowlist/status/message-工具 子路径是新的
  打包和外部插件工作的预期契约。
  目标解析/匹配属于 `openclaw/plugin-sdk/channel-targets`。
  消息操作门控和反应消息 ID 辅助工具属于
  `openclaw/plugin-sdk/channel-actions`。
- 捆绑的扩展特定的辅助程序桶默认情况下是不稳定的。如果一个辅助程序仅由捆绑的扩展需要，请将其保留在扩展的本地 `api.js` 或 `runtime-api.js` 接缝之后，而不是将其提升到 `openclaw/plugin-sdk/<extension>` 中。
- 新的共享辅助程序接缝应该是通用的，而不是渠道品牌的。共享目标解析属于 `openclaw/plugin-sdk/channel-targets`；渠道特定的内部内容保留在拥有插件的本地 `api.js` 或 `runtime-api.js` 接缝之后。
- 特定于功能的子路径（如 `image-generation`、`media-understanding` 和 `speech`）之所以存在，是因为捆绑/原生插件目前使用了它们。它们的存在本身并不意味着每个导出的辅助程序都是长期冻结的外部契约。

## 消息工具架构

插件应拥有特定于渠道的 `describeMessageTool(...)` 架构贡献。请将特定于提供商的字段保留在插件中，而不是在共享核心中。

对于共享的可移植架构片段，请重用通过 `openclaw/plugin-sdk/channel-actions` 导出的通用辅助程序：

- `createMessageToolButtonsSchema()` 用于按钮网格样式的有效负载
- `createMessageToolCardSchema()` 用于结构化卡片有效负载

如果架构形状仅对一种提供商有意义，请在该插件自己的源中定义它，而不是将其提升到共享 SDK 中。
...
- `messaging.targetResolver.looksLikeId(raw, normalized)` 告诉核心输入是否应直接跳过类似 ID 的解析，而不是目录搜索。
- `messaging.targetResolver.resolveTarget(...)` 是当核心在规范化或目录未命中后需要最终的提供商拥有的解析时的插件后备。
- 一旦解析了目标，`messaging.resolveOutboundSessionRoute(...)` 就拥有特定于提供商的会话路由构造。

建议的拆分：

- 在搜索对等方/组之前，使用 `inferTargetChatType` 进行类别决策。
...
当插件拥有特定于提供商的模型 ID、基础 URL 默认值或身份验证网管的模型元数据时，使用 `catalog`。
...
- `late`：在其它隐式提供商之后的最后一遍

键发生冲突时，后出现的提供商获胜，因此插件可以使用相同的提供商 ID 有意覆盖内置的提供商条目。
...
如果您的插件注册了一个渠道，建议与 `resolveAccount(...)` 一起实现
...
- 您无需返回原始令牌值即可报告只读
  可用性。对于状态样式命令，返回 `tokenStatus: "available"`（以及匹配的 source
  字段）就足够了。
- 当凭证通过 SecretRef 配置但在当前命令路径中
  不可用时，请使用 `configured_unavailable`。
...
每个条目都会成为一个插件。如果包组列出了多个扩展，插件 ID
将变为 `name/<fileBase>`。
...
安全说明：`openclaw plugins install` 使用 `npm install --omit=dev --ignore-scripts`（无生命周期脚本，运行时无开发依赖项）安装插件依赖项。保持插件依赖
...
可以在网关的预侦听启动阶段将渠道插件选择加入相同的 `setupEntry` 路径，即使该渠道已配置。

仅当 `setupEntry` 完全覆盖了网关开始侦听之前必须存在的启动界面时，才使用此选项。实际上，这意味着设置入口
必须注册启动所依赖的每个渠道拥有的功能，例如：

- 渠道注册本身
- 任何在网关开始侦听之前必须可用的 HTTP 路由
- 任何在同一窗口期间必须存在的网关方法、工具或服务

如果您的完整入口仍拥有任何必需的启动功能，请不要启用
此标志。保持插件处于默认行为，并让 OpenClaw 在启动期间加载
完整入口。

打包的渠道还可以发布仅限设置的合约表面助手，核心
可以在加载完整的渠道运行时之前查询这些助手。当前的设置
提升表面是：
...
当 Core 需要将旧的单账户渠道配置提升为 `channels.<id>.accounts.*` 而不加载完整插件条目时，会使用该接口。Matrix 是当前绑定的示例：当已存在命名账户时，它仅将 auth/bootstrap 键移动到命名的提升账户，并且它可以保留已配置的非规范 default-account 键，而不是总是创建 `accounts.default`。

这些设置修补适配器使绑定的契约接口发现保持懒惰。导入时间保持轻量；提升接口仅在首次使用时加载，而不是在模块导入时重新进入绑定的渠道启动。

当这些启动接口包含网关 RPC 方法时，请将它们保持在特定于插件的前缀上。Core 管理命名空间（`config.*`、`exec.approvals.*`、`wizard.*`、`update.*`）保留，并且始终解析为 `operator.admin`，即使插件请求更窄的范围。
...
渠道插件可以通过 `openclaw.channel` 宣传设置/发现元数据，并通过 `openclaw.install` 提供安装提示。这使核心目录保持无数据。
...
除了最小示例之外，有用的 `openclaw.channel` 字段：

- `detailLabel`：用于更丰富的目录/状态界面的辅助标签
- `docsLabel`：覆盖文档链接的链接文本
- `preferOver`：此目录条目应压过的较低优先级插件/渠道 ID
- `selectionDocsPrefix`、`selectionDocsOmitLabel`、`selectionExtras`：选择界面副本控制
- `markdownCapable`：将渠道标记为支持 markdown，以便进行出站格式化决策
- `exposure.configured`：当设置为 `false` 时，从已配置渠道列表界面中隐藏该渠道
- `exposure.setup`：当设置为 `false` 时，从交互式设置/配置选择器中隐藏该渠道
- `exposure.docs`：将渠道标记为内部/私有，用于文档导航界面
- `showConfigured` / `showInSetup`：为了兼容性仍然接受的旧别名；推荐使用 `exposure`
- `quickstartAllowFrom`：选择将渠道纳入标准的快速启动 `allowFrom` 流程
- `forceAccountBinding`：即使只存在一个账户，也要求显式绑定账户
- `preferSessionLookupForAnnounceTarget`：在解析公告目标时优先查找会话
...
上下文引擎插件拥有用于摄入、组装和压缩的会话上下文编排。使用 `api.registerContextEngine(id, factory)` 从你的插件注册它们，然后使用 `plugins.slots.contextEngine` 选择活动引擎。

当你的插件需要替换或扩展默认的上下文管道，而不仅仅是添加内存搜索或钩子时，请使用此功能。
...
当插件需要的行为不适合当前的 API 时，不要绕过插件系统进行私有调用。请添加缺失的功能。
...
4. 注册供应商实现
   然后供应商插件针对该功能注册其后端。
5. 添加合约覆盖
   添加测试，以便所有权和注册形式随时间推移保持明确。

这就是 OpenClaw 在不硬编码于某个
提供商的世界观下保持自以为是通过的方式。请参阅 [Capability Cookbook](/zh/tools/capability-cookbook)
以获取具体文件清单和实际示例。

### 功能清单

当您添加新功能时，实现通常应同时涉及这些
方面：
