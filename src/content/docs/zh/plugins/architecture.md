...
<Info>这是**深度架构参考**。如需实用指南，请参阅： - [安装和使用 插件](/zh/tools/plugin) — 用户指南 - [入门指南](/zh/plugins/building-plugins) — 第一个 插件教程 - [渠道插件](/zh/plugins/sdk-channel-plugins) — 构建消息渠道 - [提供商插件](/zh/plugins/sdk-provider-plugins) — 构建模型提供商 - [SDK 概述](/zh/plugins/sdk-overview) — 导入映射和注册 API</Info>
