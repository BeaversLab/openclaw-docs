...
<Info>这是**深度架构参考**。有关实用指南，请参阅： - [安装和使用插件](/zh/tools/plugin) — 用户指南 - [入门指南](/zh/plugins/building-plugins) — 第一个插件教程 - [渠道插件](/zh/plugins/sdk-channel-plugins) — 构建消息渠道 - [提供商插件](/zh/plugins/sdk-provider-plugins) — 构建模型提供商 - [SDK 概述](/zh/plugins/sdk-overview) — 导入映射和注册 API</Info>
...
使用 `openclaw plugins inspect <id>` 查看插件的形状和功能细分。有关详细信息，请参阅 [CLI 参考](/zh/cli/plugins#inspect)。
...
1. **清单 + 设备发现**
   ...
   全局扩展根目录和捆绑扩展中查找候选插件。设备发现首先读取本地
   `openclaw.plugin.json` 清单和支持的捆绑清单。
2. **启用 + 验证**
...
需要具体的发布检查清单？请参阅
[能力开发手册](/zh/tools/capability-cookbook)。
...
当提供商拥有基于环境的凭据，且通用的 auth/status/模型-picker 路径应该在不加载插件运行时的情况下看到这些凭据时，请使用清单 `providerAuthEnvVars`。当一个提供商 id 应该重用另一个提供商 id 的环境变量、auth 配置文件、支持配置的 auth 以及 API 密钥新手引导选择时，请使用清单 `providerAuthAliases`。当新手引导/auth-choice CLI 界面应该在不加载提供商运行时的情况下了解提供商的选择 id、分组标签和简单的单标志 auth 连线时，请使用清单 `providerAuthChoices`。请将提供商运行时 `envVars` 保留用于面向操作员的提示，例如新手引导标签或 OAuth client-id/client-secret 设置变量。

当渠道拥有由环境驱动的 auth 或设置，且通用的 shell-env 回退、config/status 检查或设置提示应该在不加载渠道运行时的情况下看到这些信息时，请使用清单 `channelEnvVars`。
...
| 7   | `resolveConfigApiKey`                                      | 在运行时身份验证加载之前，为配置提供商解析环境标记身份验证                                          | 提供商拥有提供商自己的环境标记 API 密钥解析；`amazon-bedrock` 此处也有内置的 AWS 环境标记解析器            |
| 8   | `resolveSyntheticAuth`                                     | 提供本地/自托管或配置支持的身份验证，而不持久化明文                                                 | 提供商可以使用合成/本地凭据标记运行                                                                        |
| 9   | `resolveExternalAuthProfiles`                              | 覆盖提供商拥有的外部身份验证配置文件；对于 CLI/应用拥有的凭据，默认 `persistence` 是 `runtime-only` | 提供商重用外部身份验证凭据，而不持久化复制的刷新令牌                                                       |
...
| 24  | `formatApiKey`                                             | 认证配置文件格式化程序：存储的配置文件变为运行时 `apiKey` 字符串                                    | 提供商存储额外的认证元数据，并且需要自定义运行时令牌形状                                                   |
...
| 32  | `augmentModelCatalog`                                      | 发现后附加的合成/最终目录行                                                                         | 提供商需要在 `models list` 和选择器中使用合成向前兼容的行                                                  |
| 33  | `isBinaryThinking`                                         | 针对二元思维提供商的推理开关                                                                        | 提供商仅公开二元思维的开/关                                                                                |
| 34  | `supportsXHighThinking`                                    | `xhigh` 所选模型的推理支持                                                                          | 提供商希望仅在模型的子集上启用 `xhigh`                                                                     |
...
| 37  | `prepareRuntimeAuth`                                       | 在推理之前将配置的凭据交换为实际的运行时令牌/密钥                                                   | 提供商需要进行令牌交换或使用短期请求凭据                                                                   |
| 38  | `resolveUsageAuth`                                         | 解析 `/usage` 及相关状态表面的使用/计费凭据                                                         | 提供商需要自定义使用/配额令牌解析或不同的使用凭据                                                          |
| 39  | `fetchUsageSnapshot`                                       | 在解析身份验证后获取并规范化特定于提供商的使用/配额快照                                             | 提供商需要特定于提供商的使用端点或有效负载解析器                                                           |
| 40  | `createEmbeddingProvider`                                  | 为内存/搜索构建提供商拥有的嵌入适配器                                                               | 内存嵌入行为属于提供商插件                                                                                 |
| 41  | `buildReplayPolicy`                                        | 返回控制提供商转录处理的重放策略                                                                    | 提供商需要自定义转录策略（例如，思考块剥离）                                                               |
| 42  | `sanitizeReplayHistory`                                    | 在通用转录清理后重写重放历史                                                                        |
| }   | 提供商需要在共享压缩辅助工具之外进行特定于提供商的重放重写 |
| 43  | `validateReplayTurns`                                      | 在嵌入式运行器之前进行最终的重放轮次验证或重塑                                                      | 提供商传输在通用清理后需要更严格的轮次验证                                                                 |
| 44  | `onModelSelected`                                          | 运行提供商拥有的选择后副作用                                                                        | 当模型激活时，提供商需要遥测或提供商拥有的状态                                                             |

`normalizeModelId`、`normalizeTransport` 和 `normalizeConfig` 首先检查匹配的提供商插件，然后回退到其他支持钩子的提供商插件，直到有一个实际更改了模型 ID 或传输/配置。这使得别名/兼容性提供商垫片（shims）能够正常工作，而无需调用者知道哪个捆绑插件拥有该重写。如果没有提供商钩子重写受支持的 Google 系列配置条目，捆绑的 Google 配置规范化器仍将应用该兼容性清理。

如果提供商需要完全自定义的线路协议或自定义请求执行器，那是另一类扩展。这些钩子用于仍在 OpenClaw 正常推理循环上运行的提供商行为。
...
- Anthropic 使用 `resolveDynamicModel`、`capabilities`、`buildAuthDoctorHint`、`resolveUsageAuth`、`fetchUsageSnapshot`、`isCacheTtlEligible`、`resolveDefaultThinkingLevel`、`applyConfigDefaults`、`isModernModelRef` 和 `wrapStreamFn`，因为它拥有 Claude 4.6 前向兼容性、提供商系列提示、身份验证修复指导、使用端点集成、提示缓存资格、感知身份验证的配置默认值、Claude 默认/自适应思维策略，以及针对 beta 标头、`/fast` / `serviceTier` 和 `context1m` 的 Anthropic 特定流式处理塑形。
- Anthropic 针对 Claude 的特定流辅助函数目前仍保留在打包插件自己的 public `api.ts` / `contract-api.ts` 接口中。该包表面导出了 `wrapAnthropicProviderStream`、`resolveAnthropicBetas`、`resolveAnthropicFastMode`、`resolveAnthropicServiceTier` 以及底层的 Anthropic 包装构建器，而不是围绕单一提供商的 beta-header 规则来扩展通用 SDK。
- OpenAI 使用 `resolveDynamicModel`、`normalizeResolvedModel` 和 `capabilities` 加上 `buildMissingAuthMessage`、`suppressBuiltInModel`、`augmentModelCatalog`、`supportsXHighThinking` 和 `isModernModelRef`，因为它拥有 GPT-5.4 的前向兼容性、直接的 OpenAI `openai-completions` -> `openai-responses` 规范化、Codex 感知的身份验证提示、Spark 抑制、合成的 OpenAI 列表行以及 GPT-5 思维/实时模型策略；`openai-responses-defaults` 流家族拥有用于归因标头、`/fast`/`serviceTier`、文本详细程度、原生 Codex 网络搜索、推理兼容负载塑形以及 Responses 上下文管理的共享原生 OpenAI Responses 包装器。
- OpenRouter 使用 `catalog` 加上 `resolveDynamicModel` 和 `prepareDynamicModel`，因为该提供商是透传的，并且可能会在 OpenClaw 的静态目录更新之前暴露新的模型 ID；它还使用 `capabilities`、`wrapStreamFn` 和 `isCacheTtlEligible`，以便将提供商特定的请求标头、路由元数据、推理补丁和提示缓存策略保留在核心之外。其重放策略来自 `passthrough-gemini` 系列，而 `openrouter-thinking` 流系列则拥有代理推理注入和不支持的模型/`auto` 跳过逻辑。
- GitHub Copilot 使用 `catalog`、`auth`、`resolveDynamicModel` 和
  `capabilities` 以及 `prepareRuntimeAuth` 和 `fetchUsageSnapshot`，因为它
  需要提供商拥有的设备登录、模型回退行为、Claude 记录
...
- OpenAI Codex 使用 `catalog`、`resolveDynamicModel`、
  ...
  与直接 OpenAI 共享相同的 `openai-responses-defaults` 流族。
- Google AI Studio 和 Gemini CLI OAuth 使用 `resolveDynamicModel`、
  ...
  `google-gemini` 重放族拥有 Gemini 3.1 前向兼容回退、
  原生 Gemini 重放验证、引导重放清理、标记的
  推理输出模式和现代模型匹配，而
  `google-thinking` 流族拥有 Gemini 思维负载规范化；
  Gemini CLI OAuth 还使用 `formatApiKey`、`resolveUsageAuth` 和
  `fetchUsageSnapshot` 进行令牌格式化、令牌解析和配额端点
  连接。
- Anthropic Vertex 通过
  `anthropic-by-model` 重放族使用 `buildReplayPolicy`，以便特定于 Claude 的重放清理保持
  限定于 Claude ID，而不是每个 `anthropic-messages` 传输。
- Amazon Bedrock 使用 `buildReplayPolicy`、`matchesContextOverflowError`、`classifyFailoverReason` 和 `resolveDefaultThinkingLevel`，因为它拥有针对 Anthropic-on-Bedrock 流量的 Bedrock 专用节流/未就绪/上下文溢出错误分类；其重放策略仍共享同一个仅限 Claude 的 `anthropic-by-model` 守卫。
- OpenRouter、Kilocode、Opencode 和 Opencode Go 通过 `passthrough-gemini` 重放系列使用 `buildReplayPolicy`，因为它们通过 OpenAI 兼容传输代理 Gemini 模型，并且需要 Gemini 思考签名清理，而不需要原生 Gemini 重放验证或引导重写。
- MiniMax 通过 `hybrid-anthropic-openai` 重放系列使用 `buildReplayPolicy`，因为一个提供商同时拥有 Anthropic 消息和 OpenAI 兼容语义；它在 Anthropic 端保持仅限 Claude 的思考块删除，同时将推理输出模式覆盖回原生，并且 `minimax-fast-mode` 流系列在共享流路径上拥有快速模式模型重写。
- Moonshot 使用 `catalog` 加上 `wrapStreamFn`，因为它仍使用共享的 OpenAI 传输，但需要提供商拥有的思考负载规范化；`moonshot-thinking` 流系列将配置加上 `/think` 状态映射到其原生二进制思考负载。
- Kilocode 使用 `catalog`、`capabilities`、`wrapStreamFn` 和 `isCacheTtlEligible`，因为它需要提供商拥有的请求头、推理负载规范化、Gemini 转录提示和 Anthropic 缓存 TTL 门控；`kilocode-thinking` 流系列在共享代理流路径上保持 Kilo 思考注入，同时跳过 `kilo/auto` 和其他不支持显式推理负载的代理模型 id。
- Z.AI 使用 `resolveDynamicModel`、`prepareExtraParams`、`wrapStreamFn`、
  ...
  使用授权 + 配额获取；`tool-stream-default-on` 流系列使
  n 默认启用的 `tool_stream` 包装器免于逐提供商的手写胶水代码。
- xAI 使用 `normalizeResolvedModel`、`normalizeTransport`、
  ...
  n 清理、插件拥有工具的回退授权重用、前向兼容 Grok
  模型解析，以及提供商拥有的兼容性补丁，例如 xAI 工具架构
  配置文件、不支持的架构关键字、原生 `web_search` 和 HTML 实体
  n 工具调用参数解码。
- Mistral、OpenCode Zen 和 OpenCode Go 仅使用 `capabilities` 将
  副本/工具特性保留在核心之外。
- 仅目录捆绑提供商（例如 `byteplus`、`cloudflare-ai-gateway`、
  `huggingface`、`kimi-coding`、`nvidia`、`qianfan`、
  `synthetic`、`together`、`venice`、`vercel-ai-gateway` 和 `volcengine`）仅
  n 使用 `catalog`。
- Qwen 使用 `catalog` 作为其文本提供商，加上针对其多模态表面的共享媒体理解和视频生成注册。
- MiniMax 和 Xiaomi 使用 `catalog` 加上 usage hooks，因为它们的 `/usage` 行为由插件拥有，尽管推理仍然通过共享传输运行。
...
- `textToSpeech` 返回用于文件/语音留言表面的普通核心 TTS 输出负载。
...
- 视频生成已经遵循相同的模式：
  - core 拥有能力合约和 runtime helper
  - vendor plugins 注册 `api.registerVideoGenerationProvider(...)`
  - feature/渠道 plugins 消费 `api.runtime.videoGeneration.*`
...
- 插件路由必须显式声明 `auth`。
- 完全相同的 `path + match` 冲突将被拒绝，除非设置了 `replaceExisting: true`，且一个插件不能替换另一个插件的路由。
- 具有不同 `auth` 级别的重叠路由将被拒绝。请将 `exact`/`prefix` 传递链保持在相同的身份验证级别上。
...
- `auth: "gateway"` 路由在 Gateway(网关) 请求运行时作用域内运行，但该作用域是有意保守的：
  - 共享密钥承载者身份验证（`gateway.auth.mode = "token"` / `"password"`）将插件路由运行时作用域限定为 `operator.write`，即使调用方发送了 `x-openclaw-scopes`
  - 受信任的携带身份的 HTTP 模式（例如私有入口上的 `trusted-proxy` 或 `gateway.auth.mode = "none"`）仅在标头明确存在时才遵守 `x-openclaw-scopes`
  - 如果在那些携带身份的插件路由请求中缺少 `x-openclaw-scopes`，运行时作用域将回退到 `operator.write`
- 实用规则：不要假设网关认证插件路由是隐式的管理界面。如果您的路由需要仅限管理员的行为，请要求携带身份的认证模式并记录显式的 `x-openclaw-scopes` header 协定。
...
- `openclaw/plugin-sdk/core` 用于通用的共享面向插件的协定。
...
- Domain subpaths such as `openclaw/plugin-sdk/channel-config-helpers`,
  ...
  `telegram-command-config` 是 Telegram 自定义命令
  标准化/验证的窄型公共接口，即使捆绑的
  Telegram 契约表面暂时不可用，它也保持可用。
  `text-runtime` 是共享文本/Markdown/日志接口，包括
  助手可见文本剥离、Markdown 渲染/分块辅助工具、
  编辑辅助工具、指令标签辅助工具和安全文本实用程序。
- 特定于审批的渠道接口应优先在插件上使用一个 `approvalCapability`
  契约。然后，Core 通过该单一功能读取
  审批身份验证、交付、渲染、原生路由和延迟
  原生处理程序行为，而不是将审批行为混合到
  不相关的插件字段中。
- `openclaw/plugin-sdk/channel-runtime` 已被弃用，仅作为
  旧版插件的兼容性填充层保留。新代码应改为导入
  范围更窄的通用原语，且仓库代码不应
  添加对该填充层的新导入。
- 捆绑扩展的内部细节保持私有。外部插件应仅使用
  ...
  另一个扩展中导入插件包的 `src/*`。
- 仓库入口点拆分：
  `<plugin-package-root>/api.js` 是辅助/类型汇总桶（barrel），
  `<plugin-package-root>/runtime-api.js` 是仅运行时汇总桶，
  `<plugin-package-root>/index.js` 是打包插件入口，
  而 `<plugin-package-root>/setup-entry.js` 是设置插件入口。
- 当前打包的提供商示例：
  - Anthropic 使用 `api.js` / `contract-api.js` 来获取 Claude 流辅助功能（如
    `wrapAnthropicProviderStream`）、beta-header 辅助功能以及 `service_tier`
    解析。
  - OpenAI 使用 `api.js` 来获取提供商构建器、默认模型辅助功能和
    实时提供商构建器。
  - OpenRouter 使用 `api.js` 来获取其提供商构建器以及新手引导/配置
    辅助功能，而 `register.runtime.js` 仍然可以重新导出通用的
    `plugin-sdk/provider-stream` 辅助功能以供仓库本地使用。
- 外观加载的公共入口点在存在时首选活动的运行时配置快照，然后当
  OpenClaw 尚未提供运行时快照时，回退到磁盘上已解析的配置文件。
- 通用共享原语仍然是首选的公共 SDK 契约。一小部分保留的、特定于打包品牌的辅助程序接口仍然存在。
  请将这些视为打包维护/兼容性接口，而不是新的第三方导入目标；新的跨渠道契约仍应落在
  通用 `plugin-sdk/*` 子路径或插件本地的 `api.js` /
  `runtime-api.js` 汇总桶上。

兼容性说明：

- 对于新代码，请避免使用根 `openclaw/plugin-sdk` 汇总桶。
- 首选狭义的稳定原语。较新的 setup/pairing/reply/
  feedback/contract/inbound/threading/command/secret-input/webhook/infra/
  allowlist/status/message-工具 子路径是新的打包和外部插件工作的预期契约。
  Target 解析/匹配属于 `openclaw/plugin-sdk/channel-targets`。
  消息操作门控和反应消息 ID 辅助功能属于
  `openclaw/plugin-sdk/channel-actions`。
- 打包的扩展专用辅助模块默认是不稳定的。如果一个辅助函数仅由打包的扩展需要，请将其保留在该扩展的本地 `api.js` 或 `runtime-api.js` 接缝之后，而不是将其提升到 `openclaw/plugin-sdk/<extension>` 中。
- 新的共享辅助接缝应该是通用的，而不是特定于渠道的。共享目标解析属于 `openclaw/plugin-sdk/channel-targets`；特定于渠道的内部实现保留在所属插件的本地 `api.js` 或 `runtime-api.js` 接缝之后。
- 特定于功能的子路径（如 `image-generation`、`media-understanding` 和 `speech`）之所以存在，是因为打包/原生插件当前正在使用它们。它们的存在本身并不意味着每个导出的辅助函数都是长期冻结的外部契约。
...
- 用于按钮网格样式负载的 `createMessageToolButtonsSchema()`
- 用于结构化卡片负载的 `createMessageToolCardSchema()`

如果架构形状仅对一个提供商有意义，请在该插件的源代码中定义它，而不是将其提升到共享 SDK 中。
...
- `messaging.targetResolver.resolveTarget(...)` 是当核心在规范化或目录未命中后需要最终的提供商拥有的解析时的插件回退机制。
- `messaging.resolveOutboundSessionRoute(...)` 负责在目标解析后构建特定于提供商的会话路由。

建议的拆分：

- 使用 `inferTargetChatType` 进行应在搜索对等/群组之前发生的类别决策。
...
从配置派生目录条目的插件应将该逻辑保留在插件内，并复用 `openclaw/plugin-sdk/directory-runtime` 中的共享辅助工具。

当渠道需要基于配置的对等/群组时使用此选项，例如：
...
`catalog.run(...)` 返回与 OpenClaw 写入 `models.providers` 的形状相同的形状：

- `{ provider }` 用于单个提供商条目
- `{ providers }` 用于多个提供商条目

当插件拥有提供商特定的模型 id、基本 URL 默认值或受身份验证保护的模型元数据时，请使用 `catalog`。

`catalog.order` 控制插件目录相对于 OpenClaw 内置隐式提供商标合并的时间：
...
后面的提供商在键冲突时胜出，因此插件可以故意使用相同的提供商 id 覆盖内置提供商条目。
...
如果您的插件注册了一个渠道，建议在实现 `resolveAccount(...)` 的同时也实现
...
- 诸如 `openclaw status`、`openclaw status --all`、
  ...
  修复流程之类的只读命令路径，不应仅仅为了描述配置而具体化运行时凭据。
...
- 您不需要仅为了报告只读可用性而返回原始令牌值。返回 `tokenStatus: "available"`（以及匹配的源
  字段）对于状态式命令来说就足够了。
- 当通过 SecretRef 配置了凭据但在当前命令路径中不可用时，请使用 `configured_unavailable`。

这使得只读命令可以报告“在此命令路径中已配置但不可用”，
而不是崩溃或错误地将帐户报告为未配置。
...
每个条目都会成为一个插件。如果集合列出了多个扩展，则插件 ID
将变为 `name/<fileBase>`。

如果您的插件导入了 npm 依赖项，请在该目录中安装它们，以便
`node_modules` 可用（`npm install` / `pnpm install`）。

安全护栏：解析符号链接后，每个 `openclaw.extensions` 条目必须保留在插件
目录内。转义出包目录的条目将被
拒绝。

安全说明：`openclaw plugins install` 使用
`npm install --omit=dev --ignore-scripts` 安装插件依赖项（运行时无生命周期脚本，无开发依赖项）。保持插件依赖
...
可以使渠道插件在网关的
预监听启动阶段进入相同的 `setupEntry` 路径，即使该渠道已配置。

仅当 `setupEntry` 完全覆盖网关开始监听之前必须存在的
启动界面时，才使用此选项。实际上，这意味着设置入口
...
此标志。保持插件的默认行为，并让 OpenClaw 在启动期间加载
完整入口。

捆绑的渠道还可以发布仅设置的合约界面辅助工具，核心
可以在加载完整的渠道运行时之前查阅这些工具。当前的设置
提升界面是：
...
当 Core 需要将旧的单一账户渠道配置提升为 `channels.<id>.accounts.*` 而不加载完整的插件入口时，会使用该接口。Matrix 是当前内置的示例：当命名账户已存在时，它仅将 auth/bootstrap 密钥移动到命名提升账户，并且它可以保留已配置的非规范 default-account 密钥，而不是总是创建 `accounts.default`。

那些设置补丁适配器使内置合约接口的发现保持惰性。导入时间保持轻量；提升接口仅在首次使用时加载，而不是在模块导入时重新进入内置渠道启动。

当这些启动接口包含网关 RPC 方法时，请将它们保留在特定于插件的前缀上。Core 管理命名空间（`config.*`、`exec.approvals.*`、`wizard.*`、`update.*`）保留专用，并且始终解析为 `operator.admin`，即使插件请求更窄的范围。
...
- `preferOver`：此目录条目应超过的较低优先级插件/渠道 ID
...
- `showConfigured` / `showInSetup`：仍接受的遗留别名以保持兼容性；首选 `exposure`
- `quickstartAllowFrom`：选择将渠道加入标准快速启动 `allowFrom` 流程
- `forceAccountBinding`：即使仅存在一个账户，也需要显式的账户绑定
- `preferSessionLookupForAnnounceTarget`：在解析公告目标时首选会话查找

OpenClaw 也可以合并 **外部渠道目录**（例如，MPM 注册表导出）。将 JSON 文件放置在以下位置之一：
...
上下文引擎插件拥有摄取、组装和压缩的会话上下文编排。通过 `api.registerContextEngine(id, factory)` 从插件中注册它们，然后使用 `plugins.slots.contextEngine` 选择活动引擎。

当您的插件需要替换或扩展默认上下文流程而不仅仅是添加内存搜索或钩子时，请使用此方法。
...
如果您的引擎 **不** 拥有压缩算法，请保持 `compact()` 的实现并显式委托它：
...
当插件需要的行为不符合当前的 API 时，不要绕过插件系统使用私有的内部侵入。添加缺失的功能。
...
4. 注册供应商实现
   供应商插件随后针对该功能注册其后端。
5. 增加契约覆盖
   添加测试，以确保所有权和注册结构随着时间的推移保持明确。

这就是 OpenClaw 在不硬编码于单一
提供商世界观的情况下保持鲜明立场的方式。有关具体文件清单和实际示例，请参阅 [Capability Cookbook](/zh/tools/capability-cookbook)。
...
- 当功能/渠道
  插件需要使用时，在 `src/plugins/runtime/*` 中暴露插件运行时
