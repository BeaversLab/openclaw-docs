...
| `providerAuthEnvVars`               | 否   | `Record<string, string[]>`       | 廉价的提供商身份验证环境元数据，OpenClaw 可以在无需加载插件代码的情况下进行检查。                                                  |
...
| `activation`                        | 否   | `object`                         | 针对提供商、命令、渠道、路由和功能触发加载的廉价激活提示。仅限元数据；插件运行时仍拥有实际行为。                                   |
...
当插件拥有一个运行时命令名称，而用户可能将其错误地放入 `plugins.allow` 或尝试作为根 CLI 命令运行时，请使用 `commandAliases`。CLI 使用此元数据进行诊断，而无需导入插件运行时代码。
...
| `cliCommand` | 否   | `string`          | 相关的根 CLI 命令，用于 CLI 操作建议（如果存在）。 |
...
当插件在共享 `openclaw qa` 根下提供一个或多个传输运行程序时，请使用 `qaRunners`。保持此元数据低成本且静态；插件运行时仍拥有通过轻量级 `runtime-api.ts` 接口导出 `qaRunnerCliRegistrations` 的实际 CLI 注册。
...
此块仅包含元数据。它不注册运行时行为，也不替代 `register(...)`、`setupEntry` 或其他运行时/插件入口点。当前的使用者在更广泛的插件加载之前将其用作缩小提示，因此缺少激活元数据通常只会损失性能；只要遗留清单所有权回退仍然存在，就不应影响正确性。
...
| 字段             | 必填 | 类型                                                 | 含义                                 |
| ---------------- | ---- | ---------------------------------------------------- | ------------------------------------ |
| `onProviders`    | 否   | `string[]`                                           | 被请求时应激活此插件的提供商 ID。    |
| `onCommands`     | 否   | `string[]`                                           | 应激活此插件的命令 ID。              |
| `onChannels`     | 否   | `string[]`                                           | 应激活此插件的渠道 ID。              |
| `onRoutes`       | 否   | `string[]`                                           | 应激活此插件的路由种类。             |
| `onCapabilities` | 否   | `Array<"provider" \| "channel" \| "tool" \| "hook">` | 控制平面激活规划使用的广泛能力提示。 |

当前的实时使用者：

- 由命令触发的 CLI 规划会回退到传统的
  `commandAliases[].cliCommand` 或 `commandAliases[].name`
- 由渠道触发的 setup/渠道规划会回退到传统的 `channels[]`
  所有权，当缺少显式的渠道激活元数据时
- 由提供商触发的 setup/运行时规划会回退到传统的
  `providers[]` 和顶层 `cliBackends[]` 所有权，当缺少显式的提供商
  激活元数据时

## setup 参考

当 setup 和新手引导界面在运行时加载之前需要廉价的插件拥有的元数据时，请使用 `setup`。
...
顶层 `cliBackends` 保持有效，并继续描述 CLI 推理后端。`setup.cliBackends` 是针对控制平面/setup 流程的特定于 setup 的描述符表面，应保持仅为元数据。

如果存在，`setup.providers` 和 `setup.cliBackends` 是用于 setup 发现的优先的描述符优先查找表面。如果描述符仅缩小了候选插件的范围，而 setup 仍需要更丰富的 setup 时的运行时钩子，请设置 `requiresRuntime: true` 并保持 `setup-api` 作为后备执行路径。

由于 setup 查找可以执行插件拥有的 `setup-api` 代码，规范化的 `setup.providers[].id` 和 `setup.cliBackends[]` 值必须在发现的插件之间保持唯一。模糊的所有权将失败关闭，而不是根据发现顺序选择获胜者。

### setup.providers 参考

| 字段          | 必需 | 类型       | 含义                                                                  |
| ------------- | ---- | ---------- | --------------------------------------------------------------------- |
| `id`          | 是   | `string`   | 在设置或新手引导期间公开的提供商 ID。保持标准化 ID 在全局范围内唯一。 |
| `authMethods` | 否   | `string[]` | 此提供商支持的设置/身份验证方法 ID，无需加载完整运行时。              |
| `envVars`     | 否   | `string[]` | 通用设置/状态界面在加载插件运行之前可以检查的环境变量。               |
...
| `cliBackends`      | 否   | `string[]` | 用于描述符优先设置查找的设置时后端 ID。保持标准化 ID 在全局范围内唯一。 |
| `configMigrations` | 否   | `string[]` | 由此插件的设置界面拥有的配置迁移 ID。                                   |
| `requiresRuntime`  | 否   | `boolean`  | 在描述符查找后，设置是否仍需执行 `setup-api`。                          |
...
当 OpenClaw 应该从简写模型 ID（如 `gpt-5.4` 或 `claude-sonnet-4.6`）推断您的提供商插件，且在插件运行时加载之前时，请使用 `modelSupport`。
...
| 文件                   | 用于                                                                               |
| ---------------------- | ---------------------------------------------------------------------------------- |
| `openclaw.plugin.json` | 设备发现、配置验证、认证选择元数据，以及插件代码运行前必须存在的 UI 提示           |
| `package.json`         | npm 元数据、依赖项安装，以及用于入口点、安装控制、设置或目录元数据的 `openclaw` 块 |
...
一些运行前的插件元数据故意位于 `package.json` 下的
...
| `openclaw.channel.persistedAuthState`                             | 轻量级持久化身份验证检查器元数据，可以在不加载完整渠道运行时的情况下回答“是否已有任何内容登录？”。 |
...
`openclaw.install.minHostVersion` 在安装和清单注册表加载期间强制执行。无效的值将被拒绝；在旧主机上，更新但有效的值将跳过该插件。

`openclaw.install.allowInvalidConfigRecovery` 被有意限制得较窄。它不会使任意损坏的配置变得可安装。目前它仅允许安装流程从特定的陈旧捆绑插件升级失败中恢复，例如缺少的捆绑插件路径或该捆绑插件的陈旧 `channels.<id>` 条目。不相关的配置错误仍然会阻止安装并将操作员引导至 `openclaw doctor --fix`。
...
当设置、诊断或配置状态流程需要在完整的渠道插件加载之前进行廉价的“是/否”身份验证探测时，请使用它。目标导出应该是一个仅读取持久化状态的小函数；不要将其通过完整的渠道运行时桶进行路由。
...
当渠道可以从环境变量或其他微小的非运行时输入获取配置状态时，请使用它。如果检查需要完整的配置解析或真实的渠道运行时，请将该逻辑保留在插件 `config.hasConfiguredState` 挂钩中。
...
- 如果插件已安装但其清单或架构损坏或缺失，验证将失败，并且 Doctor 会报告插件错误。
- 如果插件配置存在但插件已**禁用**，则保留该配置，并在 Doctor 和日志中显示**警告**。

有关完整的 `plugins.*` 架构，请参阅 [配置参考](/en/gateway/configuration)。

## 注意

- 对于原生 OpenClaw 插件（包括本地文件系统加载），清单是**必需的**。
- 运行时仍然会单独加载插件模块；清单仅用于发现和验证。
- 原生清单使用 JSON5 解析，因此只要最终值仍然是对象，就允许注释、尾随逗号和未加引号的键。
- 清单加载器仅读取文档化的清单字段。请避免在此处添加自定义顶级键。
- `providerAuthEnvVars` 是用于身份验证探测、环境标记验证和类似提供商身份验证表面的廉价元数据路径，这些表面不应仅为了检查环境名称而启动插件运行时。
- `providerAuthAliases` 允许提供商变体重用另一个提供商的身份验证环境变量、身份验证配置文件、支持配置的身份验证以及 API 密钥新手引导选择，而无需在核心中硬编码该关系。
- `channelEnvVars` 是用于 shell 环境回退、设置提示和类似渠道表面的廉价元数据路径，这些表面不应仅为了检查环境名称而启动插件运行时。
- `providerAuthChoices` 是用于身份选择器、
  `--auth-choice` 解析、首选提供商映射以及提供商运行时加载前的简单新手引导
...
- [Building Plugins](/en/plugins/building-plugins) — 插件入门
- [Plugin Architecture](/en/plugins/architecture) — 内部架构
- [SDK Overview](/en/plugins/sdk-overview) — 插件 SDK 参考
