...
| `contracts`                          | 否   | `object`                         | 用于外部身份验证钩子、嵌入、语音、实时转录、实时语音、媒体理解、图像生成、音乐生成、视频生成、网络获取、网络搜索和工具所有权的静态功能所有权快照。                      |
...
每个 `providerAuthChoices`OpenClaw 条目描述一个新手引导或身份验证选项。
OpenClaw 会在提供商运行时加载之前读取此内容。
提供商设置列表使用这些清单选项、描述符派生的设置选项以及安装目录元数据，而无需加载提供商运行时。
...
当插件拥有用户可能会错误地放入 `plugins.allow`CLIOpenClaw 或尝试作为根 CLI 命令运行的运行时命令名称时，请使用 `commandAliases`。OpenClaw 使用此元数据进行诊断，而无需导入插件运行时代码。
...
此块是规划器元数据，而不是生命周期 API。它不注册运行时行为，不替换 API`register(...)`，也不承诺插件代码已执行。激活规划器使用这些字段在回退到现有的清单所有权元数据（例如 `providers`、`channels`、`commandAliases`、`setup.providers`、`contracts.tools` 和钩子）之前缩小候选插件范围。
...
| `onAgentHarnesses` | 否   | `string[]`                                           | 嵌入式代理 harness 运行时 ID，这些 ID 应在激活/加载计划中包含此插件。使用顶级 `cliBackends`CLI 作为 CLI 后端别名。                           |
...
- agent-runtime 启动规划将 `activation.onAgentHarnesses` 用于
  嵌入式 harness，将顶级 `cliBackends[]`CLI 用于 CLI 运行时别名
- 渠道触发的设置/渠道规划会回退到旧的 `channels[]`
  所有权，当缺少显式渠道激活元数据时
- 启动插件规划使用 `activation.onConfigPaths` 处理非渠道根
  配置层面，例如捆绑的浏览器插件的 `browser` 块
- 提供商触发的设置/运行时规划会回退到旧的
  `providers[]` 和顶级 `cliBackends[]` 所有权，当缺少显式提供商
  激活元数据时
...
OpenClaw 还在通用提供商身份验证和环境变量查找中包含 OpenClaw`setup.providers[].envVars`。在弃用窗口期内，`providerAuthEnvVars` 通过兼容性适配器继续获得支持，但仍在使用它的非打包插件会收到清单诊断。新插件应将设置/状态环境元数据放在 `setup.providers[].envVars` 上。

当没有可用的设置条目，或者 `setup.requiresRuntime: false` 声明设置运行时不需要时，OpenClaw 也可以从 OpenClaw`setup.providers[].authMethods` 推导简单的设置选择。对于自定义标签、CLI 标志、新手引导范围和助手元数据，显式的 `providerAuthChoices`CLI 条目仍然是首选。

仅当这些描述符足以满足设置界面时，才设置 `requiresRuntime: false`OpenClaw。OpenClaw 将显式的 `false` 视为仅描述符契约，不会执行 `setup-api` 或 `openclaw.setupEntry`OpenClaw 进行设置查找。如果一个仅描述符插件仍然提供了这些设置运行时条目之一，OpenClaw 会报告一个附加诊断并继续忽略它。省略 `requiresRuntime` 会保留传统的回退行为，以便那些添加了描述符但没有设置该标志的现有插件不会中断。
...
当设置运行时确实执行时，如果 `setup-api`CLI 注册了清单描述符未声明的提供商或 CLI 后端，或者描述符没有匹配的运行时注册，则设置注册表诊断会报告描述符偏差。这些诊断是附加的，不会拒绝传统插件。
...
仅将 `contracts`OpenClaw 用于 OpenClaw 可以在无需导入插件运行时的情况下读取的静态能力所有权元数据。
...
| `embeddingProviders`             | `string[]` | 此插件拥有的用于内存之外的可重用向量嵌入用途的通用嵌入提供商 ID。           |
...
| `meetingNotesSourceProviders`    | `string[]` | 此插件拥有的会议记录源提供商 ID。                                           |
...
实现 `resolveExternalAuthProfiles` 的提供程序插件应声明
`contracts.externalAuthProviders`。未声明的插件仍然会
通过已弃用的兼容性回退运行，但该回退速度较慢，
并将在迁移窗口后被移除。

打包的内存嵌入提供程序应
为其公开的每个适配器 ID 声明
`contracts.memoryEmbeddingProviders`，包括
内置适配器，例如 `local`。独立 CLI 路径使用此清单
协定，以便在完整的 Gateway(网关) 运行时注册
提供程序之前，仅加载所属插件。

通用嵌入提供程序应为其在
`api.registerEmbeddingProvider(...)` 中注册的每个适配器声明 `contracts.embeddingProviders`。当向量旨在供多个功能、
工具或插件使用时，请使用通用协定。请保留
`contracts.memoryEmbeddingProviders` 用于形状和生命周期特定于 OpenClaw 内存索引的适配器。

`contracts.gatewayMethodDispatch` 目前接受
`"authenticated-request"`。这是原生插件 HTTP
路由的 API 卫生网关，用于故意在进程内调度 Gateway(网关) 控制平面方法，而
不是针对恶意原生插件的沙箱。仅将其用于已经需要 Gateway(网关) HTTP 身份验证的严格审查的打包/运营商界面。

## mediaUnderstandingProviderMetadata 参考

当媒体理解提供商具有默认模型、自动身份验证后备优先级或通用核心助手在运行时加载之前所需的本地文档支持时，请使用 `mediaUnderstandingProviderMetadata`。键还必须在 `contracts.mediaUnderstandingProviders` 中声明。
...
| `autoPriority`         | `Record<string, number>`            | 对于基于凭据的自动提供商后备，数字越小排序越靠前。 |
| `nativeDocumentInputs` | `"pdf"[]`                           | 提供商支持的本地文档输入。                         |

## channelConfigs 参考

当渠道插件需要在运行时加载之前获取廉价的配置元数据时，请使用 `channelConfigs`。当没有可用的设置条目时，或者当 `setup.requiresRuntime: false` 声明设置运行时不需要时，只读渠道设置/状态发现可以直接将此元数据用于配置的外部渠道。

`channelConfigs` 是插件清单元数据，而不是一个新的顶级用户配置部分。用户仍然在 `channels.<channel-id>`OpenClaw 下配置渠道实例。OpenClaw 读取清单元数据，以便在插件运行时代码执行之前确定哪个插件拥有该配置的渠道。
...
声明 `channels[]` 的非捆绑插件还应声明匹配的 `channelConfigs`OpenClaw 条目。如果没有它们，OpenClaw 仍然可以加载插件，但在插件运行时执行之前，冷路径配置架构、设置和控制 UI 界面无法知道渠道拥有的选项形状。

`channelConfigs.<channel-id>.commands.nativeCommandsAutoEnabled` 和
`nativeSkillsAutoEnabled` 可以声明静态 `auto` 默认值，用于在渠道运行时加载之前运行的命令配置
检查。打包的渠道也可以通过 `package.json#openclaw.channel.commands` 发布
相同的默认值，与其其他的包拥有的渠道目录元数据一起。
...
| 字段          | 类型                     | 含义                                                                 |
| ------------- | ------------------------ | -------------------------------------------------------------------- |
| `schema`      | `object`                 | `channels.<id>` 的 JSON Schema。对于每个声明的渠道配置条目是必需的。 |
...
| `preferOver`  | `string[]`               | 在选择界面中，该渠道应超越的旧版或较低优先级的插件 ID。              |

### 替换另一个渠道插件

当您的插件是另一个插件也可以提供的渠道 ID 的首选所有者时，请使用 `preferOver`。常见情况包括重命名的插件 ID、取代打包插件的独立插件，或者为了配置兼容性而保持相同渠道 ID 的维护分支。
...
配置 `channels.chat` 后，OpenClaw 会同时考虑渠道 ID 和首选插件 ID。如果选择较低优先级的插件仅是因为它是打包的或默认启用的，OpenClaw 会在有效的运行时配置中禁用它，以便一个插件拥有该渠道及其工具。显式的用户选择仍然优先：如果用户明确启用了这两个插件，OpenClaw 将保留该选择，并报告重复的渠道/工具诊断信息，而不是静默更改请求的插件集。

请将 `preferOver` 限制在确实可以提供相同渠道的插件 ID 范围内。
这不是一个通用的优先级字段，它不会重命名用户配置键。

## modelSupport 参考

当 OpenClaw 应在插件运行时加载之前从简写模型 ID（如 `gpt-5.5` 或 `claude-sonnet-4.6`）推断您的提供商插件时，请使用 `modelSupport`OpenClaw。
...
当 OpenClaw 应在加载插件运行时之前了解提供商模型元数据时，请使用 `modelCatalog`OpenClaw。这是清单拥有的固定目录行、提供商别名、抑制规则和发现模式的来源。运行时刷新仍属于提供商运行时代码，但清单会告知核心何时需要运行时。
...
| `suppressions`   | `object[]`                                               | 来自其他源且被此插件出于提供商特定原因而抑制的模型行。                 |
| `discovery`      | `Record<string, "static" \| "refreshable" \| "runtime">` | 提供商目录是否可以从清单元数据读取、刷新到缓存，还是需要运行时。       |
| `runtimeAugment` | `boolean`                                                | 仅当提供商运行时必须在清单/配置规划之后追加目录行时，才设置为 `true`。 |

`aliases` 参与模型目录规划的提供商所有权查找。
别名目标必须是由同一插件拥有的顶级提供商。当
提供商过滤列表使用别名时，OpenClaw 可以读取所属清单并
应用别名 API/基础 URL 覆盖，而无需加载提供商运行时。
别名不会扩展未过滤的目录列表；广泛的列表仅发出所属
规范提供商行。

`suppressions` 取代了旧的提供商运行时 `suppressBuiltInModel` 挂钩。
仅当提供商由插件拥有或被声明为
针对拥有提供商的 `modelCatalog.aliases` 键时，抑制条目才会被遵守。运行时
抑制挂钩不再在模型解析期间被调用。
...
| `status`        | `"available"` \| `"preview"` \| `"deprecated"` \| `"disabled"` | 列表状态。仅当该行根本不得出现时才进行抑制。            |
...
请勿在 `modelCatalog` 中放入仅运行时的数据。仅当清单行足够完整，使提供商过滤的列表和选择器界面能够跳过注册表/运行时发现时，才使用 `static`。当清单行是可用的可列出种子或补充项，但刷新/缓存稍后可以添加更多行时，使用 `refreshable`；可刷新的行本身不具有权威性。当 OpenClaw 必须加载提供商运行时以了解列表时，使用 `runtime`。

## modelIdNormalization 参考

将 `modelIdNormalization` 用于必须在提供商运行时加载之前发生的低成本的提供商拥有的模型 ID 清理。这将短模型名称、提供商本地的旧式 ID 和代理前缀规则等别名保留在所有者插件清单中，而不是核心模型选择表中。
...
| `stripPrefixes`                      | `string[]`              | 在别名查找之前要删除的前缀，适用于旧式提供商/模型重复的情况。             |
| `prefixWhenBare`                     | `string`                | 当规范化的模型 ID 尚未包含 `/` 时要添加的前缀。                           |
| `prefixWhenBareAfterAliasStartsWith` | `object[]`              | 别名查找之后的有条件的纯 ID 前缀规则，按 `modelPrefix` 和 `prefix` 键控。 |
...
| `hostSuffixes`                 | `string[]` | 映射到端点类的主机后缀。以前缀 `.` 表示仅匹配域后缀。                       |
| `baseUrls`                     | `string[]` | 映射到端点类的确切标准化 HTTP(S) 基础 URL。                                 |
| `googleVertexRegion`           | `string`   | 用于确切全局主机的静态 Google Vertex 区域。                                 |
| `googleVertexRegionHostSuffix` | `string`   | 要从匹配主机中剥离以显示 Google Vertex 区域前缀的后缀。                     |

## providerRequest 参考

使用 `providerRequest` 获取通用请求策略在不加载提供商运行时所需的低廉请求兼容性元数据。将特定行为的有效负载重写保留在提供商运行时钩子或共享提供商系列帮助器中。
...
| `compatibilityFamily` | `"moonshot"` | 用于共享请求帮助器的可选提供商系列兼容性桶。                       |
| `openAICompletions`   | `object`     | OpenAI 兼容的补全请求标志，目前为 OpenAI`supportsStreamingUsage`。 |

## modelPricing 参考

在提供商需要运行时加载之前的控制面定价行为时，请使用 `modelPricing`Gateway(网关)。Gateway(网关) 定价缓存会在不导入提供商运行时代码的情况下读取此元数据。
...
| 字段         | 类型              | 含义                                                                                  |
| ------------ | ----------------- | ------------------------------------------------------------------------------------- |
| `external`   | `boolean`         | 为不应获取 OpenRouter 或 LiteLLM 定价的本地/自托管提供商设置 `false`OpenRouter。      |
...
| 字段                       | 类型               | 含义                                                                                       |
| -------------------------- | ------------------ | ------------------------------------------------------------------------------------------ |
| `provider`                 | `string`           | 当外部目录提供商 ID 与 OpenClaw 提供商 ID 不同时使用，例如 OpenClaw`z-ai` 提供商的 `zai`。 |
| `passthroughProviderModel` | `boolean`          | 将包含斜杠的模型 ID 视为嵌套的提供商/模型引用，这对于 OpenRouter 等代理提供商非常有用。    |
| `modelIdTransforms`        | `"version-dots"[]` | 额外的外部目录模型 ID 变体。`version-dots` 会尝试带点的版本 ID，如 `claude-opus-4.6`。     |

### OpenClaw 提供商索引

OpenClaw 提供商索引是 OpenClaw 拥有的预览元数据，针对那些插件可能尚未安装的提供商。它不属于插件清单的一部分。插件清单仍然是已安装插件的权威来源。提供商索引是内部后备合约，当未安装提供商插件时，未来的可安装提供商和预安装模型选择器界面将使用该合约。
...
提供商索引不得包含机密信息、启用状态、运行时挂钩或实时特定于帐户的模型数据。其预览目录使用与插件清单相同的 `modelCatalog` 提供商行形状，但应仅限于稳定的显示元数据，除非 `api`、`baseUrl`、定价或兼容性标志等运行时适配器字段有意与已安装的插件清单保持一致。具有实时 `/models` 发现功能的提供商应通过显式模型目录缓存路径写入刷新的行，而不是进行正常的列表或新手引导调用提供商 API。

提供商索引条目可能还会为插件已移出核心或尚未安装的提供商携带可安装插件元数据。此元数据反映了渠道目录模式：包名称、npm 安装规范、预期的完整性和廉价的身份验证选择标签足以显示可安装的设置选项。一旦安装了插件，其清单将优先，并且该提供商的提供商索引条目将被忽略。

旧版顶级功能键已弃用。使用 `openclaw doctor --fix` 将 `speechProviders`、`realtimeTranscriptionProviders`、`realtimeVoiceProviders`、`mediaUnderstandingProviders`、`imageGenerationProviders`、`videoGenerationProviders`、`webFetchProviders` 和 `webSearchProviders` 移至 `contracts` 下；正常的清单加载不再将这些顶级字段视为功能所有权。
...
| 文件                   | 用于                                                                             |
| ---------------------- | -------------------------------------------------------------------------------- |
| `openclaw.plugin.json` | 设备发现、配置验证、身份验证选择元数据以及插件代码运行之前必须存在的 UI 提示     |
...
- 如果它是关于打包、入口文件或 npm 安装行为的，请将其放在 `package.json` 中

### 影响发现的 package. 字段

一些运行前插件元数据有意位于 `package.json` 的
`openclaw` 块下，而不是 `openclaw.plugin.json` 中。
`openclaw.bundle` 和 `openclaw.bundle.json` 不是 OpenClaw 插件合约；
原生插件必须使用 `openclaw.plugin.json` 加上以下支持的
...
| `openclaw.runtimeSetupEntry`                                                               | 声明已安装软件包的构建 JavaScript 设置入口点。需要 `setupEntry`，必须存在，并且必须保留在插件包目录内。     |
| `openclaw.channel`                                                                         | 廉价的渠道目录元数据，如标签、文档路径、别名和选择副本。                                                    |
| `openclaw.channel.commands`                                                                | 配置、审计和命令列表界面在加载渠道运行时之前使用的静态原生命令和原生技能自动默认元数据。                    |
...
| `openclaw.install.clawhubSpec` / `openclaw.install.npmSpec` / `openclaw.install.localPath` | 关于捆绑和外部发布插件的安装/更新提示。                                                                     |
| `openclaw.install.defaultChoice`                                                           | 当有多个安装源可用时的首选安装路径。                                                                        |
| `openclaw.install.minHostVersion`                                                          | 支持的最低 OpenClaw 主机版本，使用 semver 下限，例如 `>=2026.3.22` 或 `>=2026.5.1-beta.1`。                 |
| `openclaw.install.expectedIntegrity`                                                       | 预期的 npm dist 完整性字符串，例如 `sha512-...`；安装和更新流程会根据它验证获取的工件。                     |
| `openclaw.install.allowInvalidConfigRecovery`                                              | 当配置无效时，允许狭义的捆绑插件重新安装恢复路径。                                                          |
| `openclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | 允许仅设置的渠道界面在启动期间在完整的渠道插件之前加载。                                                    |

清单元数据决定了在运行时加载之前，新手引导中显示哪些提供商/渠道/设置选择。`package.json#openclaw.install` 告诉新手引导当用户选择这些选项之一时如何获取或启用该插件。不要将安装提示移至 `openclaw.plugin.json`。

对于非捆绑插件源，`openclaw.install.minHostVersion` 在安装和清单注册表加载期间被强制执行。无效的值会被拒绝；较新但有效的值会导致外部插件在旧版主机上被跳过。捆绑源插件被假定为与主机检出版本一致。

官方的按需安装元数据应在插件发布到 ClawHub 时使用 `clawhubSpec`；新手引导将其视为首选的远程源，并在安装后记录 ClawHub 工件事实。`npmSpec` 仍然是尚未迁移到 ClawHub 的包的兼容性后备方案。

精确的 npm 版本锁定已经存在于 npm`npmSpec` 中，例如
`"npmSpec": "@wecom/wecom-openclaw-plugin@1.2.3"`。官方外部目录
条目应将精确规范与 `expectedIntegrity`npmnpm 配对，以便如果获取的 npm 构件不再匹配锁定的版本，更新流程将
失败。交互式新手引导仍然提供可信的注册表 npm 规范，包括裸包名称
和分发标签，以保持兼容性。目录诊断可以区分精确、浮动、完整性锁定、缺失完整性、包名称不匹配
和无效默认选择来源。当 `expectedIntegrity`npm 存在但没有有效的 npm 源可以锁定时，它们也会发出警告。
当 `expectedIntegrity` 存在时，
安装/更新流程会强制执行它；当它被省略时，注册表解析结果将被记录
而没有完整性锁定。

当状态、渠道列表
或 SecretRef 扫描需要在不加载完整运行时的情况下识别已配置的帐户时，渠道插件应提供 `openclaw.setupEntry`。设置条目应公开渠道元数据以及设置安全的配置、
状态和密钥适配器；将网络客户端、网关侦听器
和传输运行时保留在主扩展入口点中。

运行时入口点字段不会覆盖源入口点字段的包边界检查。例如，`openclaw.runtimeExtensions` 无法使
转义的 `openclaw.extensions` 路径可加载。

`openclaw.install.allowInvalidConfigRecovery` 故意很窄。它
不会使任意损坏的配置可安装。目前它只允许安装流程从特定的过时捆绑插件升级失败中恢复，例如
缺少捆绑插件路径或针对同一
捆绑插件的过时 `channels.<id>` 条目。无关的配置错误仍然会阻止安装，并将操作员发送至
`openclaw doctor --fix`。

`openclaw.channel.persistedAuthState` 是一个小型检查器
模块的包元数据：
...
在设置、诊断、状态或只读存在流需要廉价的
是/否身份验证探针，且位于完整渠道插件加载之前时使用它。持久化身份验证状态
不是已配置的渠道状态：不要使用此元数据来自动启用插件、
修复运行时依赖项，或决定是否应加载渠道运行时。
目标导出应该是一个仅读取持久化状态的小函数；请
不要通过完整的渠道运行时集合来路由它。

`openclaw.channel.configuredState` 遵循相同的形状，用于廉价的仅环境
已配置检查：
...
当渠道可以从环境或其他微小的
非运行时输入回答已配置状态时使用它。如果检查需要完整的配置解析或真实的
...
2. **内置** — 随 OpenClaw 一起附带的插件
3. **全局安装** — 安装到全局 OpenClaw 插件根目录的插件
4. **工作区** — 相对于当前工作区发现的插件

含义：

- 位于工作区中的内置插件的分叉或陈旧副本不会覆盖内置构建。
- 要使用本地插件实际覆盖内置插件，请通过 `plugins.entries.<id>` 固定它，使其通过优先级获胜，而不是依赖工作区发现。
- 重复项的丢弃会被记录下来，以便诊断和启动诊断程序可以指向被丢弃的副本。
- 配置选择的重复覆盖在诊断中被表述为显式覆盖，但仍会发出警告，以便陈旧的分叉和意外的覆盖保持可见。
...
- 当使用新的配置键扩展或派生打包插件时，请同时更新该插件的 `openclaw.plugin.json` `configSchema`。打包插件的架构很严格，因此在用户配置中添加 `plugins.entries.<id>.config.myNewKey` 而不在 `configSchema.properties` 中添加 `myNewKey` 将在插件运行时加载之前被拒绝。
...
- 如果插件已安装但清单或架构损坏或缺失，验证将失败，Doctor 会报告插件错误。
...
- `providerCatalogEntry` 必须保持轻量级，不应导入广泛的运行时代码；将其用于静态提供商目录元数据或狭义的发现描述符，而不是请求时执行。`providerDiscoveryEntry` 是旧式拼写，仍然适用于现有插件。
- 通过 `plugins.slots.*` 选择互斥的插件种类：`kind: "memory"` 通过 `plugins.slots.memory`，`kind: "context-engine"` 通过 `plugins.slots.contextEngine`（默认为 `legacy`）。
- 在此清单中声明互斥的插件种类。运行时入口 `OpenClawPluginDefinition.kind` 已弃用，仅作为旧版插件的兼容性回退方案保留。
- 环境变量元数据（`setup.providers[].envVars`、已弃用的 `providerAuthEnvVars` 和 `channelEnvVars`）仅用于声明。状态、审计、cron 传递验证和其他只读接口在将环境变量视为已配置之前，仍会应用插件信任和有效激活策略。
