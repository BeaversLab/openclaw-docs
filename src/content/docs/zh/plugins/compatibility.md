...
- 传统 `api.on("deactivate", ...)` 清理挂钩名称，同时插件迁移到
  `gateway_stop`
- 传统 `activate(api)` 插件入口点，同时插件迁移到
  `register(api)`
- 传统 SDK 别名，如 `openclaw/extension-api`、
  `openclaw/plugin-sdk/channel-runtime`、`openclaw/plugin-sdk/command-auth`
  状态构建器、`openclaw/plugin-sdk/test-utils`（已被专用的
  `openclaw/plugin-sdk/*` 测试子路径取代）以及 `ClawdbotConfig` /
  `OpenClawSchemaType` 类型别名
- 捆绑插件允许列表和启用行为
- 传统提供商/渠道环境变量清单元数据
- 传统提供商插件挂钩和类型别名，同时提供商迁移到
  显式目录、身份验证、思考、重播和传输挂钩
- 传统运行时别名，如 `api.runtime.taskFlow`、
  `api.runtime.subagent.getSession`、`api.runtime.stt` 以及已弃用的
  `api.runtime.config.loadConfig()` / `api.runtime.config.writeConfigFile(...)`
- 传统内存插件分离注册，同时内存插件迁移到
  `registerMemoryCapability`
- 传统的内存特定嵌入提供商注册，而嵌入提供商正在迁移到 `api.registerEmbeddingProvider(...)` 和 `contracts.embeddingProviders`
- 用于原生消息架构、提及门控、入站信封格式和审批功能嵌套的传统渠道 SDK 助手
- 传统的渠道路由密钥和可比较目标助手别名，而插件正在迁移到 `openclaw/plugin-sdk/channel-route`
- 正在被清单贡献所有权取代的激活提示
- `setup-api` 运行时回退，而设置描述符正在迁移到冷 `setup.requiresRuntime: false` 元数据
- 提供商 `discovery` 钩子，而提供商目录钩子正在迁移到 `catalog.run(...)`
- 渠道 `showConfigured` / `showInSetup` 元数据，而渠道包正在迁移到 `openclaw.channel.exposure`
- 传统的运行时策略配置密钥，而 doctor 正在将操作符迁移到 `agentRuntime`
- 生成的捆绑渠道配置元数据回退，同时基于注册表优先的 `channelConfigs` 元数据落地
- 持久化的插件注册表禁用和安装迁移环境标志，同时修复流程正在将操作符迁移到 `openclaw plugins registry --refresh` 和 `openclaw doctor --fix`
- 传统的插件拥有的网络搜索、网络获取和 x_search 配置路径，同时 doctor 正在将它们迁移到 `plugins.entries.<plugin>.config`
- 传统的 `plugins.installs` 编写的配置和捆绑插件加载路径别名，同时安装元数据正迁移到状态管理的插件分类账中

新插件代码应优先使用注册表和特定迁移指南中列出的替代项。现有插件可以继续使用兼容性路径，直到文档、诊断和发布通知宣布移除窗口。

## 发布说明

发布说明应包含即将到来的插件弃用信息，包括目标日期和迁移文档链接。该警告需要在兼容性路径迁移到 `removal-pending` 或 `removed` 之前发出。
