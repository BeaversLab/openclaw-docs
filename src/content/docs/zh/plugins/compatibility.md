...
- 用于原生消息架构、提及控制、入站信封格式化和审批功能嵌套的传统渠道 SDK 辅助工具
- 传统渠道路由键和可比较目标辅助别名，同时插件
  迁移到 `openclaw/plugin-sdk/channel-route`
- 正在被清单贡献所有权取代的激活提示
- `setup-api` 运行时回退，同时设置描述符迁移到冷
  `setup.requiresRuntime: false` 元数据
- 提供商 `discovery` 挂钩，同时提供商目录挂钩迁移到
  `catalog.run(...)`
- 渠道 `showConfigured` / `showInSetup` 元数据，同时渠道包迁移
  到 `openclaw.channel.exposure`
- 传统运行时策略配置键，同时 doctor 将操作员迁移到
  `agentRuntime`
- 生成的捆绑渠道配置元数据回退，同时基于注册表优先的
  `channelConfigs` 元数据落地
- 持久化的插件注册表禁用和安装迁移环境标志，同时
  修复流程将操作员迁移到 `openclaw plugins registry --refresh` 和
  `openclaw doctor --fix`
- 传统插件拥有的 web search、web fetch 和 x_search 配置路径，同时 doctor 会将它们迁移到 `plugins.entries.<plugin>.config`
- 传统 `plugins.installs` 编写的配置和捆绑插件加载路径别名，同时安装元数据将移至状态管理的插件账本

新插件代码应首选注册表中列出的替换项以及特定的迁移指南。现有插件可以继续使用兼容性路径，直到文档、诊断和发布通知宣布移除时间窗口。

## 发行说明

发行说明应包含即将弃用的插件，包括目标日期和迁移文档的链接。在兼容性路径移至 `removal-pending` 或 `removed` 之前，需要发出该警告。
