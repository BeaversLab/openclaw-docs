...
使用 `api.registerEmbeddingProvider(...)` 注册的嵌入提供商也必须在插件清单的 `contracts.embeddingProviders` 中列出。这是用于可重用向量生成的通用嵌入表面。仅内存适配器仍然使用 `api.registerMemoryEmbeddingProvider(...)` 和 `contracts.memoryEmbeddingProviders`。
...
结构化 `surfaces` 可以包含 `pi_main`、`codex_app_server`、`cli_backend`、`acp_backend` 或 `subagent`。对于有意的全表面指导，请省略 `surfaces`。不要传递空的 `surfaces` 数组；它会被拒绝，以防止意外的范围丢失变为全局提示文本。

原生 Codex 应用服务器开发者指令比其他提示表面更严格：只有明确限定范围为 `codex_app_server` 的指导才会被提升到该更高优先级通道。为了保持兼容性，旧式字符串指导和未限定范围的结构化指导仍然可用于非 Codex 提示表面。
...
| `api.registerNodeCliFeature(registrar, opts?)` | 位于 `openclaw nodes` 下的节点特性 CLI |
...
等效的扁平方法仍作为已弃用的兼容性别名供现有插件使用。请勿添加直接调用
...
`scheduleSessionTurn(...)`Gateway(网关) 是 Gateway(网关) Cron 调度器的会话作用域便捷封装。Cron 负责计时并在轮次运行时创建后台任务记录；Plugin SDK 仅限制目标会话、插件拥有的命名和清理。当工作本身需要持久的多步骤任务流状态时，请在计划的轮次内使用 `api.runtime.tasks.managedFlows`。

这些契约有意划分了权限：

- 外部插件可以拥有会话扩展、UI 描述符、命令、工具元数据、下一轮次注入和普通钩子。
- 受信任的工具策略在普通的 `before_tool_call` 钩子之前运行，并且仅限于捆绑包，因为它们参与主机安全策略。
- 保留的命令所有权仅限于捆绑包。外部插件应使用自己的命令名称或别名。
- `allowPromptInjection=false` 禁用了提示词变更挂钩，包括
  `agent_turn_prepare`、`before_prompt_build`、`heartbeat_prompt_contribution`、
  来自旧版 `before_agent_start` 的提示词字段以及
...
<Note>保留的核心管理员命名空间 (`config.*`, `exec.approvals.*`, `wizard.*`, `update.*`) 始终保持 `operator.admin`，即使插件尝试分配 更窄的网关方法范围。对于 插件拥有的方法，首选特定于插件的前缀。</Note>

<Accordion title="何时使用工具结果中间件">
  捆绑插件可以在需要在执行后重写工具结果并在运行时将该结果反馈回模型之前使用 `api.registerAgentToolResultMiddleware(...)`。这是受信任的与运行时无关的接缝，用于 tokenjuice 等异步输出缩减器。

捆绑插件必须为每个目标运行时声明 `contracts.agentToolResultMiddleware`，例如 `["pi", "codex"]`。外部插件无法注册此中间件；对于不需要模型前工具结果计时的任务，请保留正常的 OpenClaw 插件钩子。旧的仅限 Pi 的嵌入式扩展工厂注册路径已被移除。

</Accordion>

### Gateway(网关) 发现注册

`api.registerGatewayDiscoveryService(...)` 允许插件在本地发现传输（如 mDNS/Gateway(网关)）上通告活动的 Bonjour。当启用本地发现时，OpenClaw 会在 Gateway(网关) 启动期间调用该服务，传递当前 Gateway(网关) 端口和非机密 TXT 提示数据，并在 Gateway(网关) 关闭期间调用返回的 `stop` 处理程序。
...
- `commands`：由注册器拥有的显式命令名称
- `descriptors`：用于 CLI 帮助、路由和延迟插件 CLI 注册的解析时命令描述符
- `parentPath`：嵌套命令组的可选父命令路径，例如 `["nodes"]`

对于配对节点功能，请首选 `api.registerNodeCliFeature(registrar, opts?)`。它是 `api.registerCli(..., { parentPath: ["nodes"] })` 的一个轻量级包装器，并将 `openclaw nodes canvas` 等命令设为显式的插件拥有的节点功能。

如果您希望插件命令在常规根 CLI 路径中保持延迟加载，请提供 `descriptors`，覆盖该注册器公开的每个顶级命令根。
...
仅当您不需要延迟根 CLI 注册时，才单独使用 `commands`。该急切兼容性路径仍然受支持，但不会安装描述符支持的占位符以用于解析时延迟加载。
...
- 用户配置仍然优先。OpenClaw 在运行 CLI 之前，会将 `agents.defaults.cliBackends.<id>` 合并到插件默认配置之上。
- 当后端在合并后需要兼容性重写（例如规范化旧标志的结构）时，请使用 `normalizeConfig`。
- 对于属于 CLI 方言的请求范围 argv 重写（例如将 OpenClaw 思考级别映射到原生 effort 标志），请使用 `resolveExecutionArgs`。
...
| 方法                                       | 注册内容                                                                                                                |
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `api.registerContextEngine(id, factory)`   | 上下文引擎（一次激活一个）。`assemble()` 回调接收 `availableTools` 和 `citationsMode`，以便引擎可以调整提示词添加内容。 |
...
- `registerMemoryCapability` 也可以暴露 `publicArtifacts.listArtifacts(...)`，以便配套插件可以通过 `openclaw/plugin-sdk/memory-host-core` 使用导出的记忆工件，而无需深入特定记忆插件的私有布局。
- `registerMemoryPromptSection`、`registerMemoryFlushPlan` 和
  `registerMemoryRuntime` 是与旧版兼容的专用内存插件 API。
- `MemoryFlushPlan.model` 可以将 flush 回合固定到精确的 `provider/model`
  引用（例如 `ollama/qwen3:8b`），而不继承活动的后备
  链。
