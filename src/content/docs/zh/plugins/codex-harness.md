...
有关认证优先级、环境隔离、自定义 app-server 命令、模型发现以及所有配置字段，请参阅 [Codex harness reference](/zh/plugins/codex-harness-reference)。
...
本页其余部分涵盖了用户必须在常见变体之间进行选择的内容：部署形态、故障封闭路由、监护人审批策略、原生 Codex 插件以及计算机使用。有关完整的选项列表、默认值、枚举、发现、环境隔离、超时和应用服务器传输字段，请参阅 [Codex harness 参考](/zh/plugins/codex-harness-reference)。
...
`/codex status` 报告应用服务器连接性、帐户、速率限制、MCP 服务器和技能。`/codex models` 列出了该 harness 和帐户的实时 Codex 应用服务器目录。如果 `/status` 令人意外，请参阅 [故障排除](#troubleshooting)。
...
| 通过 PI 直接传输 OpenAI API 密钥流量       | 提供商或模型 `agentRuntime.id: "pi"` 和正常的 OpenAI 认证 | `/status` 显示 PI 运行时               | 仅当有意使用 PI 时使用  |
| 旧版配置                                   | `openai-codex/gpt-*`                                      | `openclaw doctor --fix` 重写它         | 不要以此方式编写新配置  |
| ACP/acpx Codex 适配器                      | ACP `sessions_spawn({ runtime: "acp" })`                  | ACP 任务/会话状态                      | 与原生 Codex 适配器分离 |

`agents.defaults.imageModel` 遵循相同的前缀拆分。对正常的 OpenAI 路由使用 `openai/gpt-*`，仅当图像理解应通过有界的 Codex 应用服务器轮次运行时使用 `codex/gpt-*`。不要使用 `openai-codex/gpt-*`；医生将该旧版前缀重写为 `openai/gpt-*`。
...
对于 OpenAI 代理轮次，当捆绑插件可用时，`openai/gpt-*` 已解析为 Codex。当您需要书写的失效闭合规则时，添加显式运行时策略：
...
`sandbox: "danger-full-access"`OpenClawOpenClawOpenClaw。如果本地 Codex 要求不允许这种
隐式的 YOLO 姿态，OpenClaw 将改为选择允许的守护者权限。
当为会话启用 OpenClaw 沙箱时，OpenClaw 会将 Codex
`danger-full-access` 缩小为 Codex `workspace-write`OpenClawDocker，以便原生 Codex 代码模式轮次
保持在沙箱隔离的工作区内。Codex 轮次网络标志遵循
OpenClaw 沙箱出口策略：Docker `network: "none"` 保持离线，而
`network: "bridge"`Docker 或自定义 Docker 网络则允许出站访问。
显式的 Codex `workspace-write` 轮次使用相同的由出口策略派生的网络标志。
...
守护者模式扩展为 Codex 应用服务器审批，通常包括
`approvalPolicy: "on-request"`、`approvalsReviewer: "auto_review"` 和
`sandbox: "workspace-write"`，前提是本地要求允许这些值。

有关每个应用服务器字段、身份验证顺序、环境隔离、发现和
超时行为的详细信息，请参阅 [Codex harness 参考](/zh/plugins/codex-harness-reference)。

## 命令和诊断

捆绑插件将 `/codex`OpenClaw 注册为斜杠命令，适用于任何支持
OpenClaw 文本命令的渠道。

常用形式：

- `/codex status` 检查应用服务器连接性、模型、帐户、速率限制、
  MCP 服务器和技能。
- `/codex models` 列出实时的 Codex 应用服务器模型。
- `/codex threads [filter]` 列出最近的 Codex 应用服务器线程。
- `/codex resume <thread-id>`OpenClaw 将当前的 OpenClaw 会话附加到
  现有的 Codex 线程。
...
对于大多数支持报告，请在发生错误的对话中首先使用 `/diagnostics [note]`Gateway(网关)。它会创建一份 Gateway(网关) 诊断报告，并且对于 Codex harness 会话，它会请求批准以发送相关的 Codex 反馈包。有关隐私模型和群组聊天行为，请参阅 [诊断导出](/zh/gateway/diagnostics)。

仅当您特别想要上传当前附加线程的 Codex 反馈而不需要完整的 Gateway(网关) 诊断包时，才使用 `/codex diagnostics [note]`Gateway(网关)。
...
1. 为代理提供排序的 OpenAI 身份验证配置文件，最好位于 OpenAI`auth.order.openai` 下。现有的 `openai-codex:*` 配置文件 ID 仍然有效。
2. 该代理的 Codex 主目录中应用服务器的现有帐户。
3. 仅对于本地 stdio app-server 启动，当不存在 app-server 账户且仍需要 OpenAI 身份验证时，请执行 `CODEX_API_KEY`，然后执行 `OPENAI_API_KEY`OpenAI。

当 OpenClaw 看到 ChatGPT 订阅风格的 Codex 身份验证配置文件时，它会从生成的 Codex 子进程中移除 OpenClaw`CODEX_API_KEY` 和 `OPENAI_API_KEY`Gateway(网关)APIOpenAIAPIAPIGateway(网关)API。这可以使 Gateway(网关) 级 API 密钥可用于嵌入或直接 OpenAI 模型，而不会使本机 Codex app-server 轮次意外通过 API 计费。显式 Codex API 密钥配置文件和本地 stdio env-key 回退使用 app-server 登录，而不是继承的子进程环境。WebSocket app-server 连接不会接收 Gateway(网关) 环境 API 密钥回退；请使用显式身份验证配置文件或远程 app-server 自己的账户。

如果订阅配置文件达到 Codex 使用限制，OpenClaw 会在 Codex 报告时记录重置时间，并为相同的 Codex 运行尝试下一个有序的身份验证配置文件。当重置时间过去后，订阅配置文件将再次符合资格，而无需更改所选的 OpenClaw`openai/gpt-*` 模型或 Codex 运行时。

对于本地 stdio 应用服务器启动，OpenClaw 将 OpenClaw`CODEX_HOME` 设置为每个代理的目录，因此 Codex 配置、身份验证/帐户文件、插件缓存/数据和本机线程状态默认不会读取或写入操作员的个人 `~/.codex`OpenClaw。OpenClaw 保留正常的进程 `HOME`；Codex 运行的子进程仍然可以找到用户主目录配置和令牌，并且 Codex 可能会发现共享的 `$HOME/.agents/skills` 和 `$HOME/.agents/plugins/marketplace.json` 条目。
...
`appServer.clearEnv`OpenClaw 仅影响生成的 Codex 应用服务器子进程。OpenClaw 在本地启动规范化期间从此列表中删除 `CODEX_HOME` 和 `HOME`：`CODEX_HOME` 保持每个代理，而 `HOME` 保持继承，以便子进程可以使用正常的用户主目录状态。

Codex 动态工具默认为 `searchable`OpenClaw 加载。OpenClaw 不公开重复 Codex 原生工作区操作的动态工具：`read`、`write`、`edit`、`apply_patch`、`exec`、`process` 和 `update_plan`OpenClaw。大多数剩余的 OpenClaw 集成工具（如消息传递、媒体、cron、浏览器、节点、网关、`heartbeat_respond` 和 `web_search`）均可通过 Codex 工具搜索在 `openclaw` 命名空间下获得，从而使初始模型上下文更小。
`sessions_yield` 和仅消息工具源回复保持直接，因为它们是轮次控制合约。`sessions_spawn` 保持可搜索状态，因此 Codex 的原生 `spawn_agent`OpenClaw 仍然是主要的 Codex 子代理界面，而通过 `openclaw` 动态工具命名空间进行显式 OpenClaw 或 ACP 委托仍然可用。Heartbeat 协作指令指示 Codex 在结束心跳轮次之前（如果该工具尚未加载）搜索 `heartbeat_respond`。

仅在连接到无法搜索延迟动态工具的自定义 Codex 应用服务器或调试完整工具有效载荷时，才设置 `codexDynamicToolsLoading: "direct"`。
...
| `clearEnv`                    | `[]`                                      | 从生成的 stdio 应用服务器进程中移除的额外环境变量名称，这些变量是在 OpenClaw 构建其继承环境之后被移除的。对于本地启动，OpenClaw 会保留每个代理 `CODEX_HOME` 和继承的 `HOME`。                                                                               |
| `requestTimeoutMs`            | `60000`                                   | 应用服务器控制平面调用的超时时间。                                                                                                                                                                                                                          |
| `turnCompletionIdleTimeoutMs` | `60000`                                   | 在 Codex 接受一轮或在一轮范围内的应用服务器请求之后，OpenClaw 等待 `turn/completed` 时的静默窗口。如果工具后或仅状态合成阶段较慢，请增大此值。                                                                                                              |
| `mode`                        | `"yolo"`，除非本地 Codex 要求不允许 YOLO  | 用于 YOLO 或守护者审查执行的预设。省略 `danger-full-access`、`never` 批准或 `user` 审查者的本地 stdio 要求将使隐式默认守护者生效。                                                                                                                          |
| `approvalPolicy`              | `"never"` 或允许的守护者批准策略          | 发送到线程启动/恢复/轮次的本机 Codex 批准策略。如果允许，守护者默认优先选择 `"on-request"`。                                                                                                                                                                |
| `sandbox`                     | `"danger-full-access"` 或允许的守护者沙箱 | 发送到线程启动/恢复的本机 Codex 沙箱模式。如果允许，守护者默认优先选择 `"workspace-write"`，否则选择 `"read-only"`。当 OpenClaw 沙箱处于活动状态时，`danger-full-access` 轮次使用具有从 OpenClaw 沙箱出口设置派生的网络访问权限的 Codex `workspace-write`。 |
| `approvalsReviewer`           | `"user"` 或允许的守护者审查者             | 如果允许，请使用 `"auto_review"` 让 Codex 审查原生审批提示，否则使用 `guardian_subagent` 或 `user`。`guardian_subagent` 仍是一个传统别名。                                                                                                                  |
| `serviceTier`                 | 未设置                                    | 可选的 Codex 应用服务器服务层级。`"priority"` 启用快速模式路由，`"flex"` 请求弹性处理，`null` 清除覆盖设置，传统的 `"fast"` 被接受为 `"priority"`。                                                                                                         |

OpenClaw 拥有的动态工具调用与 `appServer.requestTimeoutMs` 独立绑定：Codex `item/tool/call` 请求默认使用 30 秒的 OpenClaw 看门狗。正数的每次调用 `timeoutMs` 参数会延长或缩短该特定工具预算。`image_generate` 工具也使用 `agents.defaults.imageGenerationModel.timeoutMs`，当工具调用未提供其自己的超时时，并且媒体理解 `image` 工具使用 `tools.media.image.timeoutSeconds` 或其 60 秒的媒体默认值。动态工具预算上限为 600000 毫秒。超时时，OpenClaw 将中止工具信号（如果支持），并向 Codex 返回失败的动态工具响应，以便轮次可以继续，而不是让 OpenClaw 处于 `processing`。

在 Codex 接受一个轮次，且 OpenClaw 响应了一个轮次范围的 app-server 请求后，harness 预期 Codex 取得当前轮次的进展，并最终使用 OpenClaw`turn/completed` 完成原生轮次。如果 app-server 安静了 `appServer.turnCompletionIdleTimeoutMs`OpenClawOpenClaw，OpenClaw 会尽力中断 Codex 轮次，记录诊断超时，并释放 OpenClaw 会话通道，以便后续聊天消息不会排在过时的原生轮次后面。针对同一轮次的大多数非终止通知会解除该短看门狗，因为 Codex 已证明该轮次仍然存活；原始 `custom_tool_call_output` 补全会使短看门狗（工具后）保持警戒，因为它们是轮次范围的工具结果移交。全局 app-server 通知（例如速率限制更新）不会重置轮次空闲进度。已完成的 `agentMessage` 项和工具前原始助手 `rawResponseItem/completed` 项会启用助手输出释放：如果 Codex 随后在未使用 `turn/completed`OpenClaw 的情况下变得安静，OpenClaw 会尽力中断原生轮次并释放会话通道。工具后原始助手进展会继续等待 `turn/completed` 或终止看门狗。超时诊断包括最后一个 app-server 通知方法，对于原始助手响应项，还包括项类型、角色、id 和有界的助手文本预览。
...
`OPENCLAW_CODEX_APP_SERVER_GUARDIAN=1` 已被移除。请改用 `plugins.entries.codex.config.appServer.mode: "guardian"`，或者使用 `OPENCLAW_CODEX_APP_SERVER_MODE=guardian` 进行一次性本地测试。对于可重复部署，首选配置，因为它将插件行为与 Codex harness 设置的其余部分保持在同一已审核的文件中。
...
有关迁移资格、应用清单、破坏性操作策略、引导 (elicitation) 和原生插件诊断，请参阅 [Native Codex plugins](/zh/plugins/codex-native-plugins)。

## 计算机使用 (Computer Use)

计算机使用 (Computer Use) 在其各自的设置指南中进行了介绍：[Codex Computer Use](/zh/plugins/codex-computer-use)。
...
- 除非活动的 OpenClaw 上下文引擎声明 `ownsCompaction: true`，否则 Codex 拥有原生压缩。OpenClaw 会保留一份脚本副本用于渠道历史记录、搜索、`/new`、`/reset` 以及未来的模型或挂载切换。
- 媒体生成、媒体理解、TTS、批准和消息传递工具输出继续通过匹配的 OpenClaw 提供商/模型设置进行。
- `tool_result_persist` 适用于 OpenClaw 拥有的脚本工具结果，而不适用于 Codex 原生工具结果记录。

有关 Hook 层、支持的 V1 表面、原生权限处理、队列引导、Codex 反馈上传机制和压缩详细信息，请参阅 [Codex harness runtime](/zh/plugins/codex-harness-runtime)。

## 故障排除

**Codex 未作为普通的 `/model` 提供商出现：** 对于新配置来说，这是预期的。选择一个 `openai/gpt-*` 模型，启用 `plugins.entries.codex.enabled`，并检查 `plugins.allow` 是否排除了 `codex`。

**OpenClaw 使用 PI 而不是 Codex：** 请确保模型引用是官方 OpenClaw 提供商上的 OpenAI`openai/gpt-*`，并且已安装并启用 Codex 插件。如果在测试时需要严格的证明，请设置提供商或模型 `agentRuntime.id: "codex"`。强制执行的 Codex 运行时将失败，而不是回退到 PI。
...
有用的摘录通常包括 `openai/gpt-5.5` 或 `openai/gpt-5.4`、`Runtime: OpenAI Codex`、`agentRuntime.id` 或 `harnessRuntime`、`candidateProvider: "openai"`，以及 `401`、`Incorrect API key` 或 `No API key` 结果。更正后的运行应显示 `openai-codex` OAuth 路径，而不是普通的 OpenAI API 密钥失败。

**遗留的 `openai-codex/*` 配置仍然存在：** 运行 `openclaw doctor --fix`。Doctor 会将遗留的模型引用重写为 `openai/*`，移除过时的会话和全代理运行时固定，并保留现有的身份验证配置文件覆盖。

**应用服务器被拒绝：** 请使用 Codex 应用服务器 `0.125.0` 或更新版本。同版本的预发布版本或带有构建后缀的版本（例如 `0.125.0-alpha.2` 或 `0.125.0+custom`）会被拒绝，因为 OpenClaw 会测试稳定的 `0.125.0` 协议底线。
...
**WebSocket 传输立即失败：**检查 `appServer.url``authToken`、%%PH:INLINE_CODE:292:0099c4e%%、
标头，以及远程应用服务器是否讲相同的 Codex 应用服务器
协议版本。

**非 Codex 模型使用 PI：**除非提供商或模型运行时
策略将其路由到另一个线束，否则这是预期的。纯非 OpenAI 提供商引用在 `auto` 模式下保持在其正常的提供商路径上。

**Computer Use 已安装但工具未运行：**从新的
`/codex computer-use status` 检查 `/codex computer-use status`。如果工具报告
`Native hook relay unavailable`，请使用 `/new` 或 `/reset`；如果持续存在，请重启
网关以清除过时的本机钩子注册。请参阅
