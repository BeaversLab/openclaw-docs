...
<Info>这是针对 OpenClaw 核心开发者的**贡献者指南**。如果您正在 构建外部插件，请参阅[构建插件](/zh/plugins/building-plugins) 。有关深入的架构参考（功能模型、所有权、 加载管道、运行时助手），请参阅[插件内部机制](/zh/plugins/architecture)。</Info>

当 OpenClaw 需要一个新的共享领域（例如图像生成、视频生成或某些未来的供应商支持的功能领域）时，请使用本指南。
...
如果 PR 跳过了功能层并将供应商行为硬编码到渠道/工具中，请将其退回并先定义契约。

## 相关

- [插件内部](/zh/plugins/architecture) — 功能模型、所有权、加载管道、运行时辅助器。
- [构建插件](/zh/plugins/building-plugins) — 第一个插件教程。
- [SDK 概述](/zh/plugins/sdk-overviewAPI) — 导入映射和注册 API 参考。
