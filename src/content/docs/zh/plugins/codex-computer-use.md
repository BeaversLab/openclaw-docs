...
当 OpenClaw 已在使用原生 Codex harness 时，请使用此页面。有关运行时设置本身，请参阅 [Codex harness](/zh/plugins/codex-harness)。
...
当您希望 Peekaboo.app 作为 OpenClaw Peekaboo 自动化的权限感知主机时，请使用 [CLI bridge](/zh/platforms/mac/peekaboo)。当 Codex 模式的 OpenClaw 代理应该在回合开始之前拥有 Codex 的原生 `computer-use` MCP 插件时，请使用此页面。
...
当您希望代理通过网关控制 iPhone 节点时，请使用 [iOS](/zh/platforms/ios)。当 Codex 模式代理应通过 Codex 的原生 Computer Use 插件控制本地 macOS 桌面时，请使用此页面。
...
当 Codex 模式回合必须在线程开始之前准备好 Computer Use 时，设置 `plugins.entries.codex.config.computerUse`。`autoInstall: true` 会选用 Computer Use，并让 OpenClaw 在回合之前安装或重新启用它：
...
使用此配置，OpenClaw 会在每次 Codex 模式回合之前检查 Codex app-server。如果缺少 Computer Use，但 Codex app-server 已发现可安装的 marketplace，OpenClaw 会要求 Codex app-server 安装或重新启用该插件并重新加载 MCP 服务器。在 macOS 上，当没有注册匹配的 marketplace 且存在标准 Codex app bundle 时，OpenClaw 也会尝试在失败之前从 `/Applications/Codex.app/Contents/Resources/plugins/openai-bundled` 注册捆绑的 Codex marketplace。如果设置仍无法使 MCP 服务器可用，则回合在线程开始之前失败。

更改 Computer Use 配置后，请在受影响的聊天中使用 `/new` 或 `/reset`，然后再测试现有的 Codex 线程是否已启动。

## 命令

在任何 `codex` 插件命令界面可用的聊天界面中使用 `/codex computer-use` 命令。这些是 OpenClaw 聊天/运行时命令，而不是 `openclaw codex ...` CLI 子命令：
...
`status` 是只读的。它不添加市场源，不安装插件，也不
启用 Codex 插件支持。如果没有配置项启用 Computer Use，`status` 可以
在执行一次性安装命令后仍报告为已禁用。

`install` 启用 Codex app-server 插件支持，可选地添加配置的
市场源，通过 Codex app-server 安装或重新启用配置的插件，重新加载 MCP 服务器，并验证 MCP 服务器是否暴露了工具。

## Marketplace choices

OpenClaw 使用 Codex 本身公开的相同 app-server API。
市场字段用于选择 Codex 应在何处查找 OpenClawAPI`computer-use`。

| 字段                | 使用时机                                    | 安装支持                                         |
| ------------------- | ------------------------------------------- | ------------------------------------------------ |
| 无市场字段          | 您希望 Codex 应用服务器使用它已知的市场。   | 是，当应用服务器返回本地市场时。                 |
| `marketplaceSource` | 您有一个应用服务器可以添加的 Codex 市场源。 | 是的，对于明确的 `/codex computer-use install`。 |
| `marketplacePath`   | 您已经知道主机上的本地市场文件路径。        | 是，用于显式安装和轮次开始时的自动安装。         |
| `marketplaceName`   | 您希望按名称选择一个已注册的市场。          | 仅当所选市场具有本地路径时为是。                 |

全新的 Codex 主目录可能需要短暂时间来初始化其官方市场。
在安装过程中，OpenClaw 轮询 OpenClaw`plugin/list` 最长
`marketplaceDiscoveryTimeoutMs` 毫秒。默认值为 60 秒。

如果多个已知市场包含 Computer Use，OpenClaw 优先选择
OpenClaw`openai-bundled`，然后是 `openai-curated`，再然后是 `local`。未知的模糊匹配
将以失败告终并要求您设置 `marketplaceName` 或 `marketplacePath`。
...
`computer-use`OpenClaw 的市场时，OpenClaw 会尝试自动添加标准捆绑的
市场根目录：
...
如果您使用非标准的 Codex 应用路径，请运行 `/codex computer-use install
--source <marketplace-root>` once or set `computerUse.marketplacePath` 指向
本地市场文件路径。仅当您拥有
市场 JSON 文件路径而非捆绑的市场根目录时，才使用 `--marketplace-path`。

## 远程目录限制

Codex 应用服务器可以列出并读取仅限远程的目录条目，但目前不支持远程 `plugin/install`。这意味着 `marketplaceName` 可以选择仅限远程的 marketplace 进行状态检查，但安装和重新启用仍需通过 `marketplaceSource` 或 `marketplacePath` 使用本地 marketplace。
...
| `marketplaceSource`             | 未设置         | 传递给 Codex 应用服务器的源字符串 `marketplace/add`。         |
...
回合开始时的自动安装会故意拒绝已配置的 `marketplaceSource` 值。添加新源是一个显式的设置操作，因此请使用 `/codex computer-use install --source <marketplace-source>` 一次，然后让 `autoInstall` 处理从已发现的本地 marketplace 进行的未来重新启用。回合开始时的自动安装可以使用已配置的 `marketplacePath`，因为这已经是主机上的本地路径。
...
- `computer-use` MCP 服务器出现在 Codex 应用服务器 MCP 状态中。
- macOS 已授予桌面控制应用所需的权限。
- 当前主机会话可以访问正在控制的桌面。

OpenClaw 故意在 OpenClaw`computerUse.enabled` 为 true 时以失败关闭。在缺少配置所需的本机桌面工具的情况下，Codex 模式回合不应静默继续。

## 故障排除

**状态显示未安装。** 运行 `/codex computer-use install`。如果未发现市场，请传递 `--source` 或 `--marketplace-path`。

**状态显示已安装但已禁用。** 再次运行 `/codex computer-use install`。Codex 应用服务器安装会将插件配置重新写为已启用。
...
**状态或探测在 `computer-use.list_apps`OpenClaw 上超时。** 插件和 MCP 服务器存在，但本地 Computer Use 网桥未响应。退出或重启 Codex Computer Use，如有需要请重新启动 Codex Desktop，然后在新的 OpenClaw 会话中重试。

**Computer Use 工具显示 `Native hook relay unavailable`OpenClawGateway(网关)OpenClaw。** Codex 原生工具挂钩无法通过本地网桥或 Gateway(网关) 备选到达活跃的 OpenClaw 中继。使用 `/new` 或 `/reset` 启动一个新的 OpenClaw 会话。如果它成功一次但在随后的工具调用中再次失败，`/new`OpenClawGateway(网关) 只是清除当前尝试；重启 Codex 应用服务器或 OpenClaw Gateway(网关) 以便丢弃旧线程和挂钩注册，然后在新的会话中重试。

**回合开始时的自动安装拒绝某个来源。** 这是故意的。首先使用显式的 `/codex computer-use install --source <marketplace-source>` 添加来源，然后未来的回合开始自动安装可以使用已发现的本地市场。
