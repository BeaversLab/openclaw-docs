...
<Info>如果您之前未构建过任何 OpenClaw 插件，请先阅读 [入门指南](/zh/plugins/building-plugins) 以了解基本的包 结构和清单设置。</Info>
...
新的渠道插件还应暴露一个来自 `openclaw/plugin-sdk/channel-outbound` 的 `message` 适配器，其中包含 `defineChannelMessageAdapter`。该适配器声明原生传输实际支持的持久最终发送能力，并将文本/媒体发送指向与旧版 `outbound`APIAPI 适配器相同的传输函数。仅当合同测试证明原生副作用和返回回执时，才声明某项能力。
有关完整的 API 合同、示例、能力矩阵、回执规则、实时预览最终化、接收确认策略、测试和迁移表，请参阅[渠道出站 API](/zh/plugins/sdk-channel-outbound)。
如果现有的 `outbound` 适配器已经具有正确的发送方法和能力元数据，请使用 `createChannelMessageAdapterFromOutbound(...)` 来派生 `message` 适配器，而不是手动编写另一个桥接层。
适配器发送应返回 `MessageReceipt` 值。当兼容性代码仍然需要旧版 id 时，请使用 `listMessageReceiptPlatformIds(...)` 或 `resolveMessageReceiptPrimaryId(...)` 来派生它们，而不是在新的生命周期代码中维护并行的 `messageIds` 字段。
支持预览的渠道还应声明 `message.live.capabilities`，其中包含它们拥有的确切实时生命周期，例如 `draftPreview`、`previewFinalization`、`progressUpdates`、`nativeStreaming` 或 `quietFinalization`。就地最终化草稿预览的渠道还应声明 `message.live.finalizer.capabilities`，例如 `finalEdit`、`normalFallback`、`discardPending`、`previewReceipt` 和 `retainOnAmbiguousFailure`，并将运行时逻辑通过 `defineFinalizableLivePreviewAdapter(...)` 加上 `deliverWithFinalizableLivePreviewAdapter(...)` 进行路由。保持这些能力由 `verifyChannelMessageLiveCapabilityAdapterProofs(...)` 和 `verifyChannelMessageLiveFinalizerProofs(...)` 测试支持，以便原生预览、进度、编辑、回退/保留、清理和回执行为不会静默偏离。
延迟平台确认的入站接收器应声明 `message.receive.defaultAckPolicy` 和 `supportedAckPolicies`，而不是将确认时机隐藏在监视器本地状态中。使用 `verifyChannelMessageReceiveAckPolicyAdapterProofs(...)` 覆盖每个声明的策略。
...
迁移入站授权的渠道可以使用运行时接收路径中的实验性 `openclaw/plugin-sdk/channel-ingress-runtime` 子路径。该子路径将平台查找和副作用保留在插件中，同时共享允许列表状态解析、路由/发送者/命令/事件/激活决策、编辑诊断和准入映射。在传递给解析器的描述符中保持插件标识规范化；不要序列化来自已解析状态或决策的原始匹配值。有关 API 设计、所有权边界和测试预期，请参阅 [渠道入口 API](/zh/plugins/sdk-channel-ingress)。
...
- 当渠道同时支持会话发起的原生交付和显式批准转发目标时，请使用 `openclaw/plugin-sdk/approval-native-runtime` 中的 `createNativeApprovalChannelRouteGates`。该帮助器集中了批准配置选择、`mode` 处理、代理/会话过滤器、账户绑定、会话目标匹配和目标列表匹配，而调用者仍拥有渠道 ID、默认转发模式、账户查找、传输启用检查、目标规范化和轮源目标解析。不要使用它来创建核心拥有的渠道策略默认值；显式传递渠道文档记录的默认模式。
- `createChannelNativeOriginTargetResolver` 默认对 `{ to, accountId, threadId }` 目标使用共享渠道路由匹配器。仅当渠道具有提供商特定的等效规则（例如 Slack 时间戳前缀匹配）时，才传递 `targetsMatch`。
- 当渠道需要在默认路由匹配器或自定义 `targetsMatch` 回调运行之前对提供商 ID 进行规范化，同时保留用于交付的原始目标时，请将 `normalizeTargetForMatch` 传递给 `createChannelNativeOriginTargetResolver`。仅当已解析的交付目标本身需要规范化时，才使用 `normalizeTarget`。
- `availability` - 账户是否已配置以及是否应处理请求
- `presentation` - 将共享批准视图模型映射到待处理/已解析/已过期的原生负载或最终操作
- `transport` - 准备目标并发送/更新/删除原生审批消息
- `interactions` - 用于原生按钮或反应的可选 bind/unbind/clear-action 钩子，以及一个可选的 `cancelDelivered` 钩子。当 `deliverPending` 注册进程内或持久化状态（例如反应目标存储）时，请实现 `cancelDelivered`，以便在处理程序停止取消 `bindPending` 运行之前的投递，或者当 `bindPending` 未返回句柄时，可以释放该状态。
- `observe` - 可选的投递诊断钩子
- 如果渠道需要运行时拥有的对象（例如客户端、令牌、Bolt 应用或 Webhook 接收器），请通过 `openclaw/plugin-sdk/channel-runtime-context` 进行注册。通用的运行时上下文注册表允许核心从渠道启动状态引导由功能驱动的处理程序，而无需添加特定于审批的包装粘合代码。
- 仅当由功能驱动的接口尚不够具有表现力时，才使用较低级别的 `createChannelApprovalHandler` 或 `createChannelNativeApprovalRuntime`。
- 原生审批渠道必须通过这些辅助程序路由 `accountId` 和 `approvalKind`。`accountId` 将多帐户审批策略限定在正确的机器人帐户范围内，而 `approvalKind` 使执行与插件审批的行为对渠道可用，而无需在核心中进行硬编码分支。
- 核心现在也拥有审批重定向通知。渠道插件不应从 `createChannelNativeApprovalRuntime` 发送自己的“审批已转至私信或其他渠道”的后续消息；相反，应通过共享的审批功能辅助程序暴露准确的源 + 审批者私信路由，并让核心在向发起聊天发布任何通知之前汇总实际投递情况。
- 端到端保留已投递的审批 ID 类型。原生客户端不应
  根据渠道本地状态猜测或重写执行与插件审批的路由。
- 不同的审批类型可以有意识地暴露不同的原生界面。
  当前捆绑的示例：
  - Slack 为 exec 和 plugin ID 保留了原生审批路由。
  - Matrix 为 exec 和插件审批保留了相同的原生私信/渠道路由和用户体验，同时仍允许根据审批类型使用不同的身份验证。
- `createApproverRestrictedNativeApprovalAdapter` 仍然作为兼容性包装器存在，但新代码应该首选 capability 构建器并在插件上公开 `approvalCapability`。

对于热渠道入口点，当您只需要该系列中的一部分时，首选更窄的运行时子路径：
...
同样，当您不需要更广泛的通用接口时，首选 `openclaw/plugin-sdk/setup-runtime`、
...
- `openclaw/plugin-sdk/setup-runtime` 涵盖了运行时安全的设置助手：
  ...
  设置代理构建器
- `openclaw/plugin-sdk/setup-runtime` 包括用于 `createEnvPatchedAccountSetupAdapter` 的环境感知适配器接缝
- `openclaw/plugin-sdk/channel-setup` 涵盖了可选安装的设置
  构建器以及一些设置安全的原语：
  `createOptionalChannelSetupSurface`、`createOptionalChannelSetupAdapter`、

如果您的渠道支持环境驱动的设置或身份验证，并且通用启动/配置流程应该在运行时加载之前知道这些环境名称，请使用 `channelEnvVars` 在插件清单中声明它们。保持渠道运行时 `envVars` 或用于面向操作员副本的本地常量。

如果您的渠道可以在插件运行时启动之前出现在 `status`、`channels list`、`channels status` 或 SecretRef 扫描中，请在 `package.json` 中添加 `openclaw.setupEntry`。该入口点在只读命令路径中导入应该是安全的，并且应返回渠道元数据、设置安全的配置适配器、状态适配器以及这些摘要所需的渠道密钥目标元数据。请勿从设置入口启动客户端、监听器或传输运行时。

也要保持主渠道入口导入路径狭窄。设备发现可以评估入口和渠道插件模块以注册功能，而无需激活渠道。诸如 `channel-plugin-api.ts` 之类的文件应导出渠道插件对象，而无需导入设置向导、传输客户端、套接字监听器、子进程启动器或服务启动模块。将这些运行时部件放在从 `registerFull(...)`、运行时设置器或惰性功能适配器加载的模块中。
...
如果您的渠道只想在设置界面中宣传“先安装此插件”，请首选 `createOptionalChannelSetupSurface(...)`。生成的
适配器/向导在配置写入和完成时默认失败，并且它们在验证、完成和文档链接
副本中重用相同的需要安装的消息。

对于其他热渠道路径，请首选狭窄的助手而非更广泛的遗留
界面：
...
- `openclaw/plugin-sdk/outbound-media` 用于媒体加载，以及
  `openclaw/plugin-sdk/channel-outbound` 用于出站身份/发送代理
  和负载规划
- 从 `openclaw/plugin-sdk/channel-core` 获取 `buildThreadAwareOutboundSessionRoute(...)`，当出站路由应保留
  显式 `replyToId`/`threadId` 或在基础会话密钥仍匹配时
  恢复当前 `:thread:` 会话。当提供商平台的
  线程传递语义具有原生支持时，提供商插件可以覆盖
  优先级、后缀行为和线程 ID 标准化。
- `openclaw/plugin-sdk/thread-bindings-runtime` 用于线程绑定生命周期
  和适配器注册
- `openclaw/plugin-sdk/agent-media-payload` 仅当仍然需要
  传统的代理/媒体负载字段布局时
- `openclaw/plugin-sdk/telegram-command-config`Telegram 用于 Telegram 自定义命令
  标准化、重复/冲突验证以及一个回退稳定的命令
  配置契约

仅认证通道通常可以停留在默认路径：核心处理审批，插件只需暴露出站/认证功能。原生审批通道（如 Matrix、Slack、Telegram 和自定义聊天传输）应使用共享的原生辅助工具，而不是自行实现审批生命周期。
...
仅在需要更广泛的入站
辅助工具集合时使用 `openclaw/plugin-sdk/channel-inbound`。
...
`api.runtime.channel.mentions` 为已依赖运行时注入的内置渠道插件公开了相同的共享提及辅助程序：
...
`openclaw/plugin-sdk/channel-mention-gating` 导入，以避免加载无关的入站运行时辅助程序。
...
<Steps>
  ...
  <Step title="包和清单">
    ...
    验证 `channels.acme-chat`，并且是配置架构、设置和 UI 表面在插件运行时加载之前使用的冷路径源。
  ...
  <Step title="构建渠道插件对象">
    ...
    对于既接受规范顶级私信密钥又接受传统嵌套密钥的渠道，请使用 `plugin-sdk/channel-config-helpers` 中的辅助函数：`resolveChannelDmAccess`、`resolveChannelDmPolicy`、`resolveChannelDmAllowFrom` 和 `normalizeChannelDmPolicy` 将账户本地值置于继承的根值之前。通过 `normalizeLegacyDmAliases` 将相同的解析器与 doctor 修复配对，以便运行时和迁移读取相同的契约。

    <Accordion title="createChatChannelPlugin 为您做什么">
      无需手动实现低级适配器接口，您只需传递声明式选项，构建器会将它们组合起来：
      ...
      例如 `maxLinesPerMessage`；请在发送前应用它，以便回复线程和分块边界由共享的出站传递解析一次。
      当解析了原生回复目标时，发送上下文还包括 `replyToIdSource` (`implicit` 或 `explicit`)，
      因此负载助手可以保留显式回复标签，而不会消耗隐式的一次性回复槽。
    </Accordion>
  ...
  <Step title="连接入口点">
    ...
    将渠道拥有的 CLI 描述符放在 `registerCliMetadata(...)`OpenClaw 中，以便 OpenClaw
    可以在不激活完整渠道运行时的情况下在根帮助中显示它们，而正常的完整加载仍然会获取相同的描述符以进行实际命令注册。保留 `registerFull(...)` 用于仅运行时的工作。
    如果 `registerFull(...)`RPC 注册了网关 RPC 方法，请使用
    特定于插件的前缀。核心管理员命名空间（`config.*`、
    `exec.approvals.*`、`wizard.*`、`update.*`）保持保留状态并始终
    解析为 `operator.admin`。
  ...
  <Step title="添加设置入口">
    ...
    当渠道被禁用或未配置时，OpenClaw 会加载此入口而不是完整入口。它避免了在设置流程中引入繁重的运行时代码。有关详细信息，请参阅 [设置和配置](/en/plugins/sdk-setup#setup-entry)。

    将设置安全的导出拆分到 Sidecar 模块中的打包工作区渠道，如果它们还需要一个显式的设置时运行时设置器，可以使用
    来自 `openclaw/plugin-sdk/channel-entry-contract` 的 `defineBundledChannelSetupEntry(...)`。

  </Step>

  <Step title="处理入站消息">
    您的插件需要从平台接收消息并将其转发到
    OpenClaw。典型的模式是使用一个验证请求并通过您的渠道的入站处理程序进行分发的 webhook：
...
在 `src/channel.test.ts` 中编写同地测试：
    ...
    有关共享测试帮助程序，请参阅 [Testing](/zh/plugins/sdk-testing)。
...
<CardGroup cols={2}>
  ...
  <Card title="运行时帮助程序" icon="settings" href="/zh/plugins/sdk-runtime">
    通过 api.runtime 实现的 TTS, STT, media, subagent
  </Card>
  <Card title="渠道入站 API" icon="bolt" href="/zh/plugins/sdk-channel-inbound">
    共享的入站事件生命周期：ingest, resolve, record, dispatch, finalize
  </Card>
</CardGroup>

<Note>部分捆绑的辅助接口（helper seams）仍然存在，用于捆绑插件的维护和兼容性。它们不是新渠道插件的推荐模式；除非您直接维护该捆绑插件系列，否则请优先使用来自通用 SDK 表面的通用 渠道/setup/reply/runtime 子路径。</Note>
...
- [代理插件](/zh/plugins/sdk-agent-harness)
