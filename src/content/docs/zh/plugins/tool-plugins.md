...
工具插件向 OpenClaw 添加 agent 可调用的工具，而无需添加渠道、模型提供商、钩子、服务或设置后端。当插件拥有固定的工具列表，并且您希望 OpenClaw 生成清单元数据以使这些工具在无需加载运行时代码的情况下可被发现时，请使用 OpenClaw`defineToolPlugin`OpenClaw。
...
3. 构建 JavaScript。
4. 使用
   `openclaw plugins build` 生成 `openclaw.plugin.json` 和 `package.json` 元数据。
5. 在发布或安装之前验证生成的元数据。

对于提供商、渠道、钩子、服务或混合功能插件，请从
[构建插件](/zh/plugins/building-plugins)、[渠道插件](/zh/plugins/sdk-channel-plugins)
或 [提供商插件](/zh/plugins/sdk-provider-plugins) 开始。
...
- `openclaw >=2026.5.17`OpenClaw，第一个导出
  `openclaw/plugin-sdk/tool-plugin` 的 OpenClaw 版本。
- 一个可以包含 `dist/`、`openclaw.plugin.json` 和
  `package.json` 的包根目录。
...
`defineToolPlugin` 接受插件标识、可选的配置架构以及
...
`openclaw plugins build` 会写入匹配的 `toolMetadata.<tool>.optional`
...
工厂仍然用于固定的工具名称。当插件动态计算工具名称或将工具与钩子、
...
- 当您希望模型看到格式化的 JSON
  并且希望 OpenClaw 在 `details` 中保留原始值时，返回 JSON 兼容值。
...
OpenClaw 从冷元数据中发现已安装的插件。它必须能够在导入插件运行时代码之前读取插件清单。OpenClaw`defineToolPlugin` 因此会公开静态元数据，而 `openclaw plugins build` 会将该元数据写入包中。
...
`contracts.tools`OpenClaw 是重要的发现契约。它告诉 OpenClaw 哪个插件拥有每个工具，而无需加载每个已安装的插件运行时。如果清单过时，工具可能会在发现中丢失，或者注册错误可能会归咎于错误的插件。
...
- `package.json` 将 `openclaw.extensions` 指向选定的运行时条目。
...
- [插件 CLI](/zh/cli/plugins)
- [ClawHub 发布](/zh/clawhub/publishing)
