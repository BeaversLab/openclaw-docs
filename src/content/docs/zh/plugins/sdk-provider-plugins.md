...
<Tip>提供商插件将模型添加到 OpenClaw 的常规推理循环中。如果模型 必须通过拥有线程、压缩或工具事件的原生代理守护进程运行，请将提供商与 [agent harness](/zh/plugins/sdk-agent-harness) 配对，而不是将守护进程协议详细信息放入核心中。</Tip>
...
<Steps>
  ...
  <Step title="注册提供商">
    ...
    `output` 在 OpenClaw 解析自己的控制标记或渠道传递之前，
    重写助手文本增量和最终文本。

    对于仅注册一个带有 API 密钥认证的文本提供商
    以及单个支持目录的运行时的捆绑提供商，首选更狭窄的
    ...
    和代理默认模型，请使用来自 `openclaw/plugin-sdk/provider-onboard` 的预设辅助函数。
    最狭窄的辅助函数是 `createDefaultModelPresetAppliers(...)`、
    `createDefaultModelsPresetAppliers(...)` 和
    `createModelCatalogPresetAppliers(...)`。

    当提供商的原生端点在常规 `openai-completions` 传输上
    支持流式使用块时，请优先使用 `openclaw/plugin-sdk/provider-catalog-shared` 中的
    共享目录辅助函数，而不是硬编码提供商 ID 检查。
    `supportsNativeStreamingUsageCompat(...)` 和
    `applyProviderNativeStreamingUsageCompat(...)` 从端点能力图中
    检测支持，因此即使是插件使用自定义提供商 ID，
    原生的 Moonshot/DashScope 风格端点仍然会选择加入。
  ...
  <Step title="添加动态模型解析">
    ...
    预热 —— `resolveDynamicModel` 在完成后会再次运行。
  ...
  <Step title="添加运行时钩子（根据需要）">
    ...
    | 系列 | 它连接的内容 |
    | --- | --- |
    | `openai-compatible` | 适用于 OpenAI 兼容传输的共享 OpenAI 风格重放策略，包括工具调用 ID 清理、助手优先顺序修复以及传输需要的通用 Gemini 轮次验证 |
    | `anthropic-by-model` | 由 `modelId` 选择的感知 Claude 的重放策略，因此 Anthropic 消息传输仅在解析的模型实际上是 Claude ID 时才获得特定于 Claude 的思维块清理 |
    ...
    | `hybrid-anthropic-openai` | 适用于在一个插件中混合 Anthropic 消息和 OpenAI 兼容模型表面的提供商的混合策略；可选的仅 Claude 思维块删除仅限于 Anthropic 端 |
    ...
    | `moonshot-thinking` | Moonshot 二进制原生思维负载映射，来自配置 + `/think` 级别 |
    | `minimax-fast-mode` | 共享流路径上的 MiniMax 快速模式模型重写 |
    | `openai-responses-defaults` | 共享原生 OpenAI/Codex Responses 包装器：归属标头、`/fast`/`serviceTier`、文本详细程度、原生 Codex 网络搜索、推理兼容负载塑造以及 Responses 上下文管理 |
    | `openrouter-thinking` | 适用于代理路由的 OpenRouter 推理包装器，不支持模型/`auto` 跳过由中央处理 |
    | `tool-stream-default-on` | 适用于像 Z.AI 这样除非明确禁用否则希望工具流传输的提供商的默认启用 `tool_stream` 包装器 |
    ...
    `openclaw/plugin-sdk/provider-model-shared` 还导出重放系列枚举以及构建这些系列所基于的共享辅助工具。常见的公共导出包括：
    ...
    - Gemini 重放辅助工具，例如 `sanitizeGoogleGeminiReplayHistory(...)`
      和 `resolveTaggedReasoningOutputMode()`
    - 端点/模型辅助工具，例如 `resolveProviderEndpoint(...)`、
      `normalizeProviderId(...)`、`normalizeGooglePreviewModelId(...)` 和
      `normalizeNativeXaiModelId(...)`

    `openclaw/plugin-sdk/provider-stream` 暴露了系列构建器以及这些系列重用的公共包装器辅助工具。常见的公共导出包括：
    ...
    一些流辅助工具有意保留为提供商本地。当前的捆绑示例：`@openclaw/anthropic-provider` 从其公共 `api.ts` /
    `contract-api.ts` 接缝导出
    `wrapAnthropicProviderStream`、`resolveAnthropicBetas`、
    `resolveAnthropicFastMode`、`resolveAnthropicServiceTier` 以及
    较低级别的 Anthropic 包装器构建器。这些辅助工具仍然是特定于 Anthropic 的，因为
    它们还编码了 Claude OAuth beta 处理和 `context1m` 限制。

    其他捆绑提供商还在行为未在各系列间清晰共享时，保留特定于传输的包装器为本地。当前示例：
    捆绑的 xAI 插件在其自己的 `wrapStreamFn` 中保留原生 xAI Responses 塑形，包括 `/fast` 别名重写、默认 `tool_stream`、
    不支持的严格工具清理以及 xAI 特定的推理负载
    删除。
    ...
    - `normalizeConfig` 首先检查匹配的提供商，然后检查其他
      具有钩子能力的提供商插件，直到有一个实际更改了配置。
      如果没有提供商钩子重写受支持的 Google 系列配置条目，则
      捆绑的 Google 配置规范化器仍然适用。
    - `resolveConfigApiKey` 在公开时使用提供商钩子。捆绑的
      `amazon-bedrock` 路径在这里还有一个内置的 AWS 环境标记解析器，
      尽管 Bedrock 运行时身份验证本身仍使用 AWS SDK 默认
      链。
      | 13 | `contributeResolvedModelCompat` | 适用于另一个兼容传输之后的供应商模型的兼容标志 |
      | 14 | `capabilities` | 遗留静态功能包；仅兼容性 |
      | 15 | `normalizeToolSchemas` | 注册之前的提供商拥有的工具模式清理 |
      ...
      | 22 | `resolveWebSocketSessionPolicy` | 原生 WS 会话标头/冷却期 |
      ...
      | 34 | `supportsAdaptiveThinking` | 自适应思维支持 |
      | 35 | `supportsMaxThinking` | `max` 推理支持 |
      | 36 | `resolveDefaultThinkingLevel` | 默认 `/think` 策略 |
      | 37 | `isModernModelRef` | 实时/冒烟模型匹配 |
      | 38 | `prepareRuntimeAuth` | 推理前的令牌交换 |
      | 39 | `resolveUsageAuth` | 自定义使用情况凭据解析 |
      | 40 | `fetchUsageSnapshot` | 自定义使用情况端点 |
      | 41 | `createEmbeddingProvider` | 提供商拥有的用于内存/搜索的嵌入适配器 |
      | 42 | `buildReplayPolicy` | 自定义记录重放/压缩策略 |
      | 43 | `sanitizeReplayHistory` | 通用清理之后的提供商特定重放重写 |
      | 44 | `validateReplayTurns` | 嵌入式运行程序之前的严格重放轮验证 |
      | 45 | `onModelSelected` | 选择后回调（例如遥测） |

      提示调整说明：

      - `resolveSystemPromptContribution` 允许提供商为模型系列注入
        具有缓存感知能力的系统提示指导。当该行为属于一个提供商/模型
        系列并且应保留稳定/动态缓存划分时，优先使用它而不是
        `before_prompt_build`。
  ...
  <Step title="添加额外功能（可选）">
    ...
    提供商插件可以注册语音、实时转录、实时语音、媒体理解、图像生成、视频生成、Web 抓取和 Web 搜索，以及文本推理：
    ...
    对于视频生成，首选上述模式感知的功能形状：
    `generate`、`imageToVideo` 和 `videoToVideo`。扁平聚合字段如
    `maxInputImages`、`maxInputVideos` 和 `maxDurationSeconds` 不足以清晰地通告转换模式支持或禁用模式。

    音乐生成提供商应遵循相同的模式：
    `generate` 用于仅提示词生成，`edit` 用于基于参考图像的
    生成。扁平聚合字段如 `maxInputImages`、
    `supportsLyrics` 和 `supportsFormat` 不足以通告编辑支持；显式的 `generate` / `edit` 块是预期的契约。
...
`catalog.order` 控制您的目录相对于内置提供商的合并时间：
...
- [插件内部原理](/zh/plugins/architecture#provider-runtime-hooks) — Hook 详细信息和捆绑示例
