...
<Info>如果您之前尚未构建过任何 OpenClaw 插件，请先阅读 [入门指南](/en/plugins/building-plugins) 以了解基本包 结构和清单设置。</Info>
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    清单声明了 `providerAuthEnvVars`，以便 OpenClaw 能够在
    不加载插件运行时的情况下检测凭据。如果您在 ClawHub 上
    发布该提供商，则 `package.json` 中的 `openclaw.compat` 和 `openclaw.build` 字段
    是必填的。

  </Step>

  <Step title="Register the 提供商">
    最小的提供商需要一个 `id`、`label`、`auth` 和 `catalog`：
  ...
  <Step title="添加额外功能（可选）">
    ...
    OpenClaw 将此归类为**混合功能**（hybrid-capability）插件。这是
