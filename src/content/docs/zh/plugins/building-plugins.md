...
您无需将您的插件添加到 OpenClaw 代码库中。发布到
[ClawHub](OpenClawClawHub/en/clawhub)，用户使用
`openclaw plugins install clawhub:<package-name>`npm 进行安装。在启动切换期间，原始包规范仍会从 npm 安装。
...
<Steps>
  <Step title="创建包和清单">
    ...
    每个插件都需要一个清单，即使没有配置。运行时注册的工具必须在 `contracts.tools` 中列出，以便 OpenClaw 无需加载每个插件运行时即可发现所属插件。插件还应有意声明 `activation.onStartup`。本示例将其设置为 `true`。有关完整架构，请参阅 [Manifest](/zh/plugins/manifest)。规范的 ClawHub 发布代码片段位于 `docs/snippets/plugin-publish/` 中。
  ...
  <Step title="编写入口点">
    ...
    `definePluginEntry` 适用于非渠道插件。对于渠道，请使用 `defineChannelPluginEntry` - 请参阅 [渠道插件](/zh/plugins/sdk-channel-plugins)。
...
| 频道 / 消息传递 | `api.registerChannel(...)`                       | [频道插件](/zh/plugins/sdk-channel-plugins)                                  |
...
| HTTP 路由       | `api.registerHttpRoute(...)`                     | [内部实现](/zh/plugins/architecture-internals#gateway-http-routes)           |
| CLI 子命令      | `api.registerCli(...)`                           | [入口点](/zh/plugins/sdk-entrypoints)                                        |

有关完整的注册 API，请参阅 [SDK 概述](API/en/plugins/sdk-overview#registration-api)。
...
有关示例和 hook 参考，请参阅 [Plugin hooks](/zh/plugins/hooks)。
...
OpenClaw 会捕获并缓存来自已注册工具的经过验证的描述符，因此插件不会在清单中重复 OpenClaw`description` 或架构数据。清单合约仅声明所有权和发现；执行仍然调用实时的已注册工具实现。为使用 `api.registerTool(..., { optional: true })`OpenClaw 注册的工具设置 `toolMetadata.<tool>.optional: true`，以便 OpenClaw 可以避免加载该插件运行时，直到该工具被明确加入允许列表。
...
- 注册对象格式错误的工具（包括缺少 `parameters`）将被跳过，并在插件诊断中报告，而不是中断 agent 运行
- 对于具有副作用或额外二进制要求的工具，请使用 `optional: true`
- 用户可以通过将插件 ID 添加到 `tools.allow` 来启用插件中的所有工具

## 注册 CLI 命令

插件可以使用 `api.registerCli` 添加根 `openclaw` 命令组。请为每个顶级命令根提供 `descriptors`OpenClaw，以便 OpenClaw 可以显示并路由该命令，而无需急切加载每个插件运行时。
...
在插件内部，请使用本地桶文件（`api.ts`，`runtime-api.ts`）进行内部导入——切勿通过其 SDK 路径导入自己的插件。

对于提供商插件，除非接口确实是通用的，否则请将特定于提供商的辅助函数保留在这些包根桶文件中。当前捆绑的示例：

- Anthropic：Claude 流包装器和 Anthropic`service_tier` / beta 辅助函数
- OpenAI：提供商构建器、默认模型辅助函数、实时提供商
- OpenRouter：提供商构建器以及新手引导/配置辅助函数

如果某个辅助函数仅在一个捆绑的提供商包中有用，请将其保留在该包根接口处，而不是将其提升到 `openclaw/plugin-sdk/*` 中。

某些生成的 `openclaw/plugin-sdk/<bundled-id>` 辅助接口仍然存在，用于捆绑插件的维护（当它们有跟踪的拥有者使用时）。将这些视为保留接口，而不是新的第三方插件的默认模式。
...
1. 请关注 [openclaw/openclaw](https://github.com/openclaw/openclaw/releases) 上的 GitHub 发布标签，并通过 `Watch` > `Releases` 订阅。Beta 标签看起来像 `v2026.3.N-beta.1`。您也可以开启官方 OpenClaw X 账号 [@openclaw](https://x.com/openclaw) 的通知以获取发布公告。
2. 在 Beta 标签出现后，立即针对它测试您的插件。稳定版发布前的窗口期通常只有几个小时。
3. 在使用 `all good` 或发现的问题进行测试后，请在 `plugin-forum` Discord 频道的插件主题帖中发布反馈。如果您还没有主题帖，请创建一个。
4. 如果出现问题，请打开或更新一个标题为 `Beta blocker: <plugin-name> - <summary>` 的问题，并应用 `beta-blocker` 标签。将问题链接放在您的主题帖中。
5. 向 `main` 提交一个标题为 `fix(<plugin-id>): beta blocker - <summary>` 的 PR，并在 PR 和您的 Discord 主题帖中都链接到该问题。贡献者无法给 PR 打标签，因此标题是维护者和自动化工具在 PR 一侧的信号。带有 PR 的阻碍性问题会被合并；没有 PR 的阻碍性问题可能会照常发布。维护者会在 Beta 测试期间关注这些主题帖。
