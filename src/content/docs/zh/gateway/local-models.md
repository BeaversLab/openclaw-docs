...
- 一些较小或较严格的本地后端在 OpenClaw 的完整
  agent-runtime 提示形状下不稳定，尤其是包含工具架构时。如果
  后端适用于微小的直接 `/v1/chat/completions` 调用，但在正常的
...
- Gateway(网关) 能到达代理吗？ `curl http://127.0.0.1:1234/v1/models`。
- LM Studio 模型已卸载？重新加载；冷启动是常见的“挂起”原因。
- 当检测到的上下文窗口低于 **32k** 时，OpenClaw 会发出警告，低于 **16k** 时会阻止。如果您遇到该预检，请提高服务器/模型的上下文限制或选择更大的模型。
- 上下文错误？降低 `contextWindow` 或提高您的服务器限制。
- OpenAI 兼容的服务器返回 `messages[].content ... expected a string`？
...
- 微小的直接 `/v1/chat/completions` 调用有效，但 `openclaw infer model run`
  ...
  `compat.supportsTools: false` 禁用工具架构，然后重新测试。如果服务器仍然仅在
  较大的 OpenClaw 提示下崩溃，则将其视为上游服务器/模型的局限性。
- 安全性：本地模型跳过提供商端的过滤器；请保持代理狭窄并启用压缩，以限制提示词注入的破坏范围。
