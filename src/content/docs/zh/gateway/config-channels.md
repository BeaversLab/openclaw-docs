...
- 带有 `type: "acp"` 的顶层 `bindings[]` 条目用于配置论坛话题的持久化 ACP 绑定（在 `match.peer.id` 中使用规范 `chatId:topic:topicId`）。字段语义在 [ACP 代理](/zh/tools/acp-agents#persistent-channel-bindings) 中共享。
...
**Thread 会话隔离：** `thread.historyScope` 是按线程隔离（默认）还是在渠道间共享。`thread.inheritParent` 会将父渠道的记录复制到新线程中。

- Slack 原生流式传输以及 Slack 助手风格的“正在输入...”主题状态需要一个回复主题目标。顶级 Slack 默认保持在主题之外，因此它们仍可以通过 Slack 草稿发布和编辑预览进行流式传输，而不是显示主题风格的原生流/状态预览。
- `typingReaction` 会在运行回复时向传入的 Slack 消息添加临时反应，然后在完成时将其移除。使用 Slack 表情符号短代码，例如 `"hourglass_flowing_sand"`。
- `channels.slack.execApprovals`： Slack 原生执行审批交付和审批人授权。架构与 Discord 相同： `enabled` （`true`/`false`/`"auto"`）， `approvers` （Slack 用户 ID）， `agentFilter`， `sessionFilter` 和 `target` （`"dm"`， `"channel"` 或 `"both"`）。

| 动作组     | 默认   | 注释                |
| ---------- | ------ | ------------------- |
| reactions  | 已启用 | React + 列出反应    |
...
Mattermost 作为当前 OpenClaw 版本中的捆绑插件提供。旧版本或自定义构建可以通过 `openclaw plugins install @openclaw/mattermost` 安装当前的 npm 包。在固定版本之前，请检查 [npmjs.com/package/@openclaw/mattermost](https://www.npmjs.com/package/@openclaw/mattermost) 以获取当前的 dist-tags。
...
- `commands.callbackUrl` 必须解析到 OpenClaw 网关端点，并且能够从 Mattermost 服务器访问。
- 原生斜杠回调使用 Mattermost 在斜杠命令注册期间返回的每个命令的令牌进行身份验证。如果注册失败或没有激活任何命令，OpenClaw 将拒绝带有 `Unauthorized: invalid command token.` 的回调
- 对于私有/tailnet/内部回调主机，Mattermost 可能需要 `ServiceSettings.AllowedUntrustedInternalConnections` 来包含回调主机/域名。
  使用主机/域名值，而不是完整的 URL。
- `channels.mattermost.configWrites`：允许或拒绝 Mattermost 发起的配置写入。
- `channels.mattermost.requireMention`：在渠道中回复之前需要 `@mention`。
- `channels.mattermost.groups.<channelId>.requireMention`：每个渠道的提及拦截覆盖（默认为 `"*"`）。
- 当匹配到已配置的帐户 ID 时，可选的 `channels.mattermost.defaultAccount` 会覆盖默认的帐户选择。
...
- 当匹配到已配置的帐户 ID 时，可选的 `channels.signal.defaultAccount` 会覆盖默认的帐户选择。

### iMessage

OpenClaw 会生成 `imsg rpc`（通过 stdio 进行 JSON-RPC）。不需要守护进程或端口。当主机可以授予 Messages 数据库和自动化权限时，这是新的 OpenClaw iMessage 设置的首选路径。

BlueBubbles 支持已被移除。请将 BlueBubbles`channels.bluebubbles` 配置迁移到 `channels.imessage`OpenClawiMessage；OpenClaw 仅通过 `imsg` 支持 iMessage。
...
- 可选的 `channels.imessage.defaultAccount` 会在匹配已配置的账户 ID 时覆盖默认的账户选择。

- 需要对 Messages 数据库拥有完全磁盘访问权限。
- 优先使用 `chat_id:<id>` 目标。使用 `imsg chats --limit 20` 列出聊天。
- `cliPath` 可以指向 SSH 封装脚本；设置 `remoteHost` (`host` 或 `user@host`) 以通过 SCP 获取附件。
...
- 带有 `type: "acp"``chat_id:*`iMessage 的顶级 `bindings[]` 条目可以将 iMessage 会话绑定到持久的 ACP 会话。在 `match.peer.id` 中使用规范化的句柄或显式聊天目标（%%PH:INLINE_CODE:267:7f2194b%%、`chat_guid:*`、`chat_identifier:*`）。共享字段语义：[ACP Agents](/zh/tools/acp-agents#persistent-channel-bindings)。

<Accordion title="iMessageiMessage SSH 封装脚本示例">
...
Matrix 由插件支持，配置位于 Matrix`channels.matrix` 之下。
...
- `channels.matrix.proxy` 将 Matrix HTTP 流量通过显式的 HTTP(S) 代理进行路由。命名账户可以使用 `channels.matrix.accounts.<id>.proxy` 覆盖此项。
...
- `channels.matrix.execApprovals`：Matrix 原生执行审批传递和审批人授权。
  - `enabled`：`true`、`false` 或 `"auto"`（默认）。在自动模式下，当可以从 `approvers` 或 `commands.ownerAllowFrom` 解析出审批人时，执行审批将被激活。
...
- 当匹配已配置的帐户 ID 时，可选的 `channels.irc.defaultAccount` 会覆盖默认帐户选择。
- 完整的 IRC 渠道配置（主机/端口/TLS/渠道/允许列表/提及门控）记录在 [IRC](/zh/channels/irc) 中。

### 多帐户（所有渠道）

为每个渠道运行多个帐户（每个都有自己的 `accountId`）：
...
- Env 令牌仅适用于 **默认** 帐户。
- 基本渠道设置适用于所有帐户，除非按帐户进行了覆盖。
- 使用 `bindings[].match.accountId` 将每个帐户路由到不同的代理。
- 如果您仍在使用单帐户顶级渠道配置时通过 `openclaw channels add`（或渠道新手引导）添加非默认帐户，OpenClaw 会首先将帐户范围的顶级单帐户值提升到渠道帐户映射中，以便原始帐户继续工作。大多数渠道将它们移入 `channels.<channel>.accounts.default`；Matrix 可以改为保留现有的匹配命名/默认目标。
- 现有的仅渠道绑定（没有 `accountId`）保持匹配默认帐户；帐户范围绑定仍然是可选的。
- `openclaw doctor --fix` 还会通过将账户范围的顶级单账户值移动到为该渠道选择的提升账户来修复混合形状。大多数渠道使用 `accounts.default`；Matrix 可以改为保留现有的匹配命名/默认目标。

### 其他插件渠道

许多插件渠道配置为 `channels.<id>`，并在其专门的渠道页面中进行了记录（例如飞书、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
...
可见回复单独控制。群组/渠道房间默认为 `messages.groupChat.visibleReplies: "message_tool"`：OpenClaw 仍然处理回合，但正常的最终回复保持私密，可见的房间输出需要 `message(action=send)`。仅当您希望将正常回复发回到房间的旧行为时，才设置 `"automatic"`。要对直接聊天也应用相同的仅工具可见回复行为，请设置 `messages.visibleReplies: "message_tool"`；Codex 约束工具也将该仅工具行为用作其未设置的直接聊天默认值。

仅工具可见回复需要能够可靠调用工具的模型/运行时。如果会话日志显示带有 `didSendViaMessagingTool: false` 的助手文本，则模型生成了私有的最终答案而不是调用消息工具。切换到该渠道的更强的工具调用模型，或设置 `messages.groupChat.visibleReplies: "automatic"` 以恢复传统的可见最终回复。

如果消息工具在当前工具策略下不可用，OpenClaw 将回退到自动可见回复，而不是静默抑制响应。`openclaw doctor` 会针对此不匹配发出警告。

网关在文件保存后热重载 `messages` 配置。仅当在部署中禁用文件监视或配置重载时才需要重启。

**提及类型：**

- **元数据提及**：平台原生的 @提及。在 WhatsApp 自聊模式下会被忽略。
- **文本模式**：`agents.list[].groupChat.mentionPatterns` 中的安全正则表达式模式。无效的模式和不安全的嵌套重复将被忽略。
- 仅当可以进行检测时（原生提及或至少一种模式），才会强制执行提及门控。
...
`messages.visibleReplies` 是全局源轮次默认值；`messages.groupChat.visibleReplies` 会为群组/渠道源轮次覆盖它。当未设置 `messages.visibleReplies` 时，工具线可以提供自己的直接/源默认值；Codex 工具线默认为 `message_tool`。渠道允许列表和提及门控仍然决定是否处理轮次。
...
- 此块配置命令界面。有关当前内置 + 捆绑的命令目录，请参阅 [Slash Commands](/zh/tools/slash-commands)。
- 此页面是 **config-key reference**（配置键参考），并非完整的命令目录。渠道/插件拥有的命令，如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、device-pair `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice`，均在其各自的渠道/插件页面以及 [Slash Commands](/zh/tools/slash-commands) 中进行了说明。
- 文本命令必须是带有前缀 `/` 的 **standalone**（独立）消息。
- `native: "auto"`DiscordTelegramSlack 为 Discord/Telegram 开启原生命令，Slack 保持关闭。
- `nativeSkills: "auto"`DiscordTelegramSlack 为 Discord/Telegram 开启原生技能命令，Slack 保持关闭。
- 按渠道覆盖：`channels.discord.commands.native`（布尔值或 `"auto"`Discord）。对于 Discord，`false` 会在启动期间跳过原生命令的注册和清理。
...
- `config: true` 启用 `/config`（读取/写入 `openclaw.json`）。对于 gateway `chat.send` 客户端，持久化 `/config set|unset` 写入还需要 `operator.admin`；只读 `/config show` 对普通写入作用域的操作员客户端仍然可用。
- `mcp: true` 为 `mcp.servers` 下的 OpenClaw 托管 MCP 服务器配置启用 `/mcp`OpenClaw。
...
- `allowFrom` 是按提供商区分的。设置后，它是 **only**（唯一）的授权源（渠道允许列表/配对和 `useAccessGroups` 将被忽略）。
- 当未设置 `allowFrom` 时，`useAccessGroups: false` 允许命令绕过访问组策略。
- 命令文档映射：
  - built-in + bundled catalog（内置 + 捆绑目录）：[Slash Commands](/zh/tools/slash-commands)
  - 渠道-specific command surfaces（特定于渠道的命令界面）：[Channels](/zh/channels)
  - QQ Bot commands（QQ Bot 命令）：[QQ Bot](/zh/channels/qqbot)
  - pairing commands（配对命令）：[Pairing](/zh/channels/pairing)
  - LINE card command（LINE 卡片命令）：[LINE](/zh/channels/line)
  - memory dreaming（记忆梦境）：[Dreaming](/zh/concepts/dreaming)
