...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目为论坛主题配置持久的 ACP 绑定（在 `match.peer.id` 中使用规范 `chatId:topic:topicId`）。字段语义在 [ACP 代理](/zh/tools/acp-agents#persistent-channel-bindings) 中共享。
...
- 支持机器人撰写入站消息的渠道可以使用共享的 [机器人循环保护](/zh/channels/bot-loop-protection)。设置 `channels.defaults.botLoopProtection` 以获取基准配对预算，然后仅当某个界面需要不同的限制时才覆盖渠道或账户设置。
...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目为渠道和线程配置持久的 ACP 绑定（在 `match.peer.id` 中使用渠道/线程 ID）。字段语义在 [ACP 代理](/zh/tools/acp-agents#persistent-channel-bindings) 中共享。
...
Mattermost 作为捆绑插件包含在当前的 OpenClaw 版本中。旧版本或自定义构建可以使用 MattermostOpenClawnpm`openclaw plugins install @openclaw/mattermost` 安装当前的 npm 包。在固定版本之前，请检查 [npmjs.com/package/@openclaw/mattermost](https://www.npmjs.com/package/@openclaw/mattermost) 以获取当前的 dist-tags。
...
BlueBubbles 支持已被移除。在当前的 OpenClaw 中，BlueBubbles`channels.bluebubbles`OpenClaw 不是受支持的运行时配置界面。将旧配置迁移到 `channels.imessage`BlueBubblesiMessage；有关简短版本，请使用 [BlueBubbles 移除和 imsg iMessage 路径](/zh/announcements/bluebubbles-imessageBlueBubbles)，有关完整转换表，请使用 [从 BlueBubbles 迁移](/zh/channels/imessage-from-bluebubbles)。
...
- 具有 `type: "acp"` 的顶级 `bindings[]` 条目可以将 iMessage 会话绑定到持久的 ACP 会话。在 `match.peer.id` 中使用规范的句柄或显式聊天目标（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义：[ACP Agents](/zh/tools/acp-agents#persistent-channel-bindings)。
...
许多插件渠道配置为 `channels.<id>` 并记录在其专门的渠道页面中（例如飞书、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
请查看完整的渠道索引：[Channels](/zh/channels)。

### 群组聊天提及控制

群组消息默认为 **需要提及**（元数据提及或安全的正则表达式模式）。适用于 WhatsApp、Telegram、Discord、Google Chat 和 iMessage 群组聊天。

可见回复单独控制。普通群组、渠道和内部 WebChat 直接请求默认为自动最终交付：最终助手文本通过旧版可见回复路径发布。当可见输出应仅在代理调用 `message(action=send)` 后发布时，请选择 `messages.visibleReplies: "message_tool"` 或 `messages.groupChat.visibleReplies: "message_tool"`。如果在选择了仅工具模式的情况下，模型返回最终文本但未调用消息工具，则该最终文本将保持私密，并且网关详细日志会记录已抑制的负载元数据。

仅工具可见回复需要能够可靠调用工具的模型/运行时，并且推荐在 GPT 5.5 等最新模型上用于共享常驻房间。如果会话日志显示带有 `didSendViaMessagingTool: false` 的助手文本，则该模型生成了私有最终文本而不是调用消息工具。请为该渠道切换到更强的工具调用模型，检查网关详细日志中的被抑制负载摘要，或者设置 `messages.groupChat.visibleReplies: "automatic"` 以对每个群组/渠道请求使用可见的最终回复。

如果在当前工具策略下消息工具不可用，OpenClaw 将回退到自动可见回复，而不是静默抑制响应。`openclaw doctor` 会针对此不匹配发出警告。
...
原因：群组/渠道的可见回复模式解析为 `"message_tool"`，因此 OpenClaw 会运行该轮次，但除非代理调用 `message(action=send)`，否则将抑制最终的助手文本。没有报错，因为抑制是配置的行为。正常的群组和渠道轮次默认为 `"automatic"`，因此此症状仅在 `messages.groupChat.visibleReplies`（或全局 `messages.visibleReplies`）被显式设置为 `"message_tool"` 时才会出现。Harness `defaultVisibleReplies` 不适用于此处——群组/渠道解析器会忽略它；它仅影响直接/源聊天（Codex harness 会以这种方式抑制直接聊天的最终内容）。

修复方法：选择更强的工具调用模型，移除显式的 `"message_tool"` 覆盖以回退到 `"automatic"` 默认值，或者设置 `messages.groupChat.visibleReplies: "automatic"` 以强制对每个群组/渠道请求使用可见回复。网关会在文件保存后热重新加载 `messages` 配置；仅在部署中禁用了文件监视或配置重新加载时才需要重启网关。

**提及类型：**

- **元数据提及**：原生平台 @提及。在 WhatsApp 自聊模式下会被忽略。
- **文本模式**：`agents.list[].groupChat.mentionPatterns` 中安全的正则表达式模式。无效的模式和不安全的嵌套重复将被忽略。
...
`messages.groupChat.unmentionedInbound: "room_event"` 将未提及的常驻群组/渠道消息作为静默房间上下文提交，适用于支持的渠道。被提及的消息、命令和直接消息仍作为用户请求。有关 Discord、Slack 和 Telegram 的完整示例，请参阅 [Ambient room events](/zh/channels/ambient-room-events)。

`messages.visibleReplies` 是全局源事件默认值；`messages.groupChat.visibleReplies` 会为群组/渠道源事件覆盖它。当未设置 `messages.visibleReplies` 时，直接/源聊天使用所选的运行时或工具默认值，但内部 WebChat 直接回合使用自动最终交付以实现 Pi/Codex 提示符奇偶校验。设置 `messages.visibleReplies: "message_tool"` 以故意要求 `message(action=send)` 进行可见输出。渠道允许列表和提及提及门控仍然决定是否处理事件。
...
- 此块配置命令界面。有关当前内置 + 捆绑的命令目录，请参阅 [斜杠命令](/zh/tools/slash-commands)。
- 此页面是 **配置键参考**，而非完整的命令目录。渠道/插件拥有的命令，如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、device-pair `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice`，均记录在其各自的渠道/插件页面以及 [斜杠命令](/zh/tools/slash-commands) 中。
- 文本命令必须是带有前缀 `/` 的 **独立** 消息。
- `native: "auto"` 为 Discord/Telegram 启用原生命令，Slack 则保持关闭。
- `nativeSkills: "auto"` 为 Discord/Telegram 启用原生技能命令，Slack 则保持关闭。
- 按渠道覆盖：`channels.discord.commands.native`（布尔值或 `"auto"`）。对于 Discord，`false` 将在启动期间跳过原生命令注册和清理。
...
- `config: true` 启用 `/config`（读/写 `openclaw.json`）。对于 gateway `chat.send` 客户端，持久化 `/config set|unset` 写入也需要 `operator.admin`；只读 `/config show` 对普通写入作用域的操作员客户端仍然可用。
...
- `ownerAllowFrom` 是仅限所有者命令和所有者门控渠道操作的显式所有者允许列表。它与 `allowFrom` 是分开的。
...
- [配置参考](/zh/gateway/configuration-reference) — 顶层键
- [配置 — agents](/zh/gateway/config-agents)
- [通道概述](/zh/channels)
