...
- 支持机器人发起的入站消息的渠道可以使用共享的 [bot loop protection](/zh/channels/bot-loop-protection)。设置 `channels.defaults.botLoopProtection` 作为基准对预算，然后仅当某个界面需要不同的限制时才覆盖该渠道或账户。
...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目为渠道和线程配置持久的 ACP 绑定（在 `match.peer.id` 中使用渠道/线程 id）。字段语义在 [ACP Agents](/zh/tools/acp-agents#persistent-channel-bindings) 中共享。
...
Mattermost 作为当前 OpenClaw 版本的内置插件提供。旧版本或自定义构建可以使用 MattermostOpenClawnpm`openclaw plugins install @openclaw/mattermost` 安装当前的 npm 包。在固定版本之前，请检查 [npmjs.com/package/@openclaw/mattermost](https://www.npmjs.com/package/@openclaw/mattermost) 以获取当前的 dist-tags。
...
已移除对 BlueBubbles 的支持。`channels.bluebubbles` 不是当前 OpenClaw 上支持的运行时配置界面。请将旧配置迁移到 `channels.imessage`；请参阅 [BlueBubbles 移除和 imsg iMessage 路径](/zh/announcements/bluebubbles-imessage) 了解简短版本，并参阅 [从 BlueBubbles 迁移](/zh/channels/imessage-from-bluebubbles) 了解完整的转换表。
...
- 当与已配置的账户 ID 匹配时，可选的 `channels.imessage.defaultAccount` 会覆盖默认账户选择。
...
- `channels.imessage.includeAttachments` 默认为关闭；在期望代理轮次中有传入媒体之前，请将其设置为 `true`。
- `channels.imessage.catchup.enabled`Gateway(网关)：选择重放 Gateway 关闭期间到达的传入消息。
- `channels.imessage.groups`：群组注册表和每群组设置。使用 `groupPolicy: "allowlist"`，配置显式的 `chat_id` 键或 `"*"` 通配符条目，以便群组消息可以通过注册表检查。
- 具有 `type: "acp"`iMessage 的顶级 `bindings[]` 条目可以将 iMessage 对话绑定到持久的 ACP 会话。在 `match.peer.id` 中使用规范化的句柄或显式聊天目标（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义：[ACP Agents](/zh/tools/acp-agents#persistent-channel-bindings)。
...
Matrix 由插件支持，并在 Matrix`channels.matrix` 下进行配置。
...
- `channels.matrix.proxy`Matrix 将 Matrix HTTP 流量通过显式 HTTP(S) 代理路由。命名账户可以使用 `channels.matrix.accounts.<id>.proxy` 覆盖它。
- `channels.matrix.network.dangerouslyAllowPrivateNetwork` 允许私有/内部家庭服务器。`proxy` 和此网络选择加入是独立的控制项。
- `channels.matrix.defaultAccount` 在多账户设置中选择首选账户。
- `channels.matrix.autoJoin` 默认为 `off`，因此被邀请的房间和新的私信 邀请将被忽略，直到您使用 `autoJoinAllowlist` 或 `autoJoin: "always"` 设置 `autoJoin: "allowlist"`。
- `channels.matrix.execApprovals`Matrix：Matrix 原生的执行审批传递和审批人授权。
  - `enabled`：`true`、`false` 或 `"auto"`（默认）。在自动模式下，当可以从 `approvers` 或 `commands.ownerAllowFrom` 解析出审批人时，执行审批将激活。
  - `approvers`Matrix：允许批准执行请求的 Matrix 用户 ID（例如 `@owner:example.org`）。
  - `agentFilter`：可选的代理 ID 允许列表。省略以转发所有代理的审批。
  - `sessionFilter`：可选的会话键模式（子字符串或正则表达式）。
  - `target`：发送审批提示的位置。`"dm"`（默认）、`"channel"`（来源房间）或 `"both"`。
  - 每个账户的覆盖：`channels.matrix.accounts.<id>.execApprovals`。
- `channels.matrix.dm.sessionScope`Matrix 控制 Matrix 私信 如何分组为会话：`per-user`（默认）按路由对等方共享，而 `per-room` 隔离每个私信房间。
...
- 完整的 Teams 配置（凭据、webhook、私信/群组策略、每个团队/渠道的覆盖）记录在 [Microsoft Teams](Microsoft Teams/en/channels/msteams) 中。
...
- 完整的 IRC 渠道配置（主机/端口/TLS/渠道/允许列表/提及限制）在 [IRC](/zh/channels/irc) 中有详细文档说明。
...
- 如果您在仍使用单账户顶级渠道配置时，通过 `openclaw channels add`（或渠道新手引导）添加非默认账户，OpenClaw 会首先将账户范围的顶级单账户值提升到渠道账户映射中，以便原始账户继续工作。大多数渠道会将它们移动到 `channels.<channel>.accounts.default` 中；Matrix 可以保留现有的匹配命名/默认目标。
- 现有的仅渠道绑定（无 `accountId`）将继续匹配默认账户；账户范围绑定保持可选。
- `openclaw doctor --fix` 还通过将账户范围的顶级单账户值移动到为该渠道选择的提升账户中来修复混合形状。大多数渠道使用 `accounts.default`；Matrix 可以保留现有的匹配命名/默认目标。

### 其他插件渠道

许多插件渠道配置为 `channels.<id>` 并在其专属渠道页面中有文档说明（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
...
可见回复单独控制。普通群组和渠道请求默认为自动最终投递：最终助手文本通过传统可见回复路径发布。某些适配器（包括 Codex）将直接/源聊天默认为消息工具投递，因此只有当代理调用 `message(action=send)` 后，可见输出才会发布。如果模型在没有调用消息工具的情况下返回最终文本，则该最终文本保持私有，并且网关详细日志会记录被抑制的负载数据。

仅工具可见回复需要可靠调用工具的模型/运行时，建议在最新一代模型（如 GPT 5.5）的共享环境房间中使用。如果会话日志显示助手文本带有 `didSendViaMessagingTool: false`，则表明模型生成了私有最终文本而不是调用消息工具。请为该渠道切换到更强大的工具调用模型，检查网关详细日志中的被抑制负载摘要，或设置 `messages.groupChat.visibleReplies: "automatic"` 以对每个群组/渠道请求使用可见最终回复。

如果在当前工具策略下消息工具不可用，OpenClaw 将回退到自动可见回复，而不是静默抑制响应。`openclaw doctor` 会警告这种不匹配。

**故障排除：群组 @提及触发正在输入随后静默（无错误）**

症状：群组/渠道 @提及 显示正在输入指示器，并且网关日志报告 `dispatch complete (queuedFinal=false, replies=0)`，但没有消息发送到房间中。发给同一代理的私信正常回复。

原因：群组/渠道可见回复模式解析为 `"message_tool"`OpenClaw，因此 OpenClaw 运行该轮次，但除非代理调用 `message(action=send)`，否则会抑制最终的助手文本。没有错误，因为抑制是已配置的行为。普通的群组和渠道轮次默认为 `"automatic"`，因此仅当明确设置 `messages.groupChat.visibleReplies`（或全局 `messages.visibleReplies`）为 `"message_tool"` 时，才会出现此症状。此处不适用 Harness `defaultVisibleReplies` —— 群组/渠道解析器会忽略它；它仅影响直接/源聊天（Codex harness 会以这种方式抑制直接聊天的最终输出）。

修复方法：要么选择功能更强的工具调用模型，要么移除显式的 `"message_tool"` 覆盖以恢复 `"automatic"` 默认值，要么设置 `messages.groupChat.visibleReplies: "automatic"` 以强制每个群组/渠道请求都进行可见回复。保存文件后，网关会热重载 `messages` 配置；仅在部署中禁用文件监视或配置重载时才需要重启网关。

**提及类型：**

- **元数据提及**：原生平台 @提及。在 WhatsApp 自聊模式下会被忽略。
- **文本模式**：`agents.list[].groupChat.mentionPatterns` 中的安全正则表达式模式。无效的模式和不安全的嵌套重复将被忽略。
- 仅在可以进行检测时（原生提及或至少一种模式），才会强制执行提及控制。
...
`messages.groupChat.unmentionedInbound: "room_event"` 将未被提及的常开群组/渠道消息作为静默房间上下文提交到支持的渠道。被提及的消息、命令和直接消息仍保持为用户请求。有关 Discord、Slack 和 Telegram 的完整示例，请参阅 [Ambient room events](/zh/channels/ambient-room-eventsDiscordSlackTelegram)。

`messages.visibleReplies` 是全局源事件的默认设置；`messages.groupChat.visibleReplies` 会覆盖群组/渠道源事件的设置。当未设置 `messages.visibleReplies` 时，直接/源聊天将使用所选运行时或默认线束设置。Codex 线束默认将直接/源聊天设置为 message-工具 投递；设置 `messages.visibleReplies: "automatic"` 可使用自动最终投递。渠道白名单和提及门控仍然决定是否处理事件。
...
- 此块配置命令界面。有关当前内置 + 捆绑命令目录，请参阅 [斜杠命令](/zh/tools/slash-commands)。
- 此页面是**配置键参考**，不是完整的命令目录。渠道/插件拥有的命令，例如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、device-pair `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice` 记录在其各自的渠道/插件页面以及 [斜杠命令](/zh/tools/slash-commands) 中。
- 文本命令必须是带有前导 `/` 的**独立**消息。
- `native: "auto"` 为 Discord/Telegram 启用原生命令，保持 Slack 关闭。
- `nativeSkills: "auto"` 为 Discord/Telegram 启用原生技能命令，保持 Slack 关闭。
- 每个渠道覆盖：`channels.discord.commands.native`（布尔值或 `"auto"`）。对于 Discord，`false` 跳过启动期间的原生命令注册和清理。
- 使用 `channels.<provider>.commands.nativeSkills` 覆盖每个渠道的原生技能注册。
...
- `config: true` 启用 `/config`（读/写 `openclaw.json`）。对于网关 `chat.send` 客户端，持久 `/config set|unset` 写入还需要 `operator.admin`；只读 `/config show` 对普通写入范围的运营商客户端保持可用。
- `mcp: true` 为 OpenClaw 管理的 `mcp.servers` 下的 MCP 服务器配置启用 `/mcp`。
- `plugins: true` 为插件发现、安装和启用/禁用控制启用 `/plugins`。
- `channels.<provider>.configWrites` 对每个渠道的配置更改进行控制（默认：true）。
- 对于多账户渠道，`channels.<provider>.accounts.<id>.configWrites` 还会对针对该账户的写入操作进行控制（例如 `/allowlist --config --account <id>` 或 `/config set channels.<provider>.accounts.<id>...`）。
- `restart: false` 禁用 `/restart` 和网关重启工具操作。默认：`true`。
- `ownerAllowFrom` 是仅限所有者的命令和所有者控制的渠道操作的显式所有者白名单。它与 `allowFrom` 分开。
- `ownerDisplay: "hash"` 对系统提示中的所有者 ID 进行哈希处理。设置 `ownerDisplaySecret` 以控制哈希处理。
- `allowFrom` 是按提供商区分的。设置后，它是**唯一**的授权源（渠道白名单/配对和 `useAccessGroups` 将被忽略）。
- 当未设置 `allowFrom` 时，`useAccessGroups: false` 允许命令绕过访问组策略。
- 命令文档映射：
  - 内置 + 捆绑目录：[斜杠命令](/zh/tools/slash-commands)
  - 特定于渠道的命令界面：[渠道](/zh/channels)
  - QQ Bot 命令：[QQ Bot](/zh/channels/qqbot)
  - 配对命令：[配对](/zh/channels/pairing)
  - LINE 卡片命令：[LINE](/zh/channels/line)
  - 内存梦境：[梦境](/zh/concepts/dreaming)
