...
- 对于直接连接到远程 Gateway(网关) 的原生移动客户端，首选 [WebChat](<WebChat/en/web/webchatGateway(网关)>) 或 [Gateway(网关) 协议](/zh/gateway/protocol)，并实施配对设备引导/设备令牌流程，以便设备不需要共享的 HTTP 令牌/密码。
...
当设置了任一 Token 上限字段时，该值会通过代理流参数通道转发给上游提供商。发送给上游提供商的实际线路字段名称由提供商传输决定：OpenAI 系列端点使用 `max_completion_tokens`OpenAI，而仅接受旧版名称的提供商（例如 Mistral 和 Chutes）则使用 `max_tokens`。采样字段（`temperature`, `top_p`）遵循相同的流参数通道；基于 ChatGPT 的 Codex Responses 后端会在服务器端将其剥离，因为它使用固定采样。
...
- `tool_choice: "required"`（尚未在运行时强制执行；将在实施硬性强制执行后支持）
...
如果 `stream_options.include_usage=true`，则会在 `[DONE]` 之前发出一个尾随的使用情况块。
...
- 先前的助手工具调用消息
- 一条或多条带有匹配 `tool_call_id` 的 `role: "tool"` 消息
...
- macOS 上 Docker 的 Base URL: DockermacOS`http://host.docker.internal:18789/v1`
- API 密钥：您的 Gateway(网关) 不记名令牌
- 模型: `openclaw/default`
...
在后续调用中重复使用相同的 `user` 值，以延续同一个代理会话。
...
- 后端提供商/模型覆盖应位于 `x-openclaw-model`OpenAI 中，而不是 OpenAI `model` 字段中。
