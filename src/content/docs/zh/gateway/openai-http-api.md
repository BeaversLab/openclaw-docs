...
- 对于直接连接到远程网关的原生移动客户端，首选[WebChat](<WebChat/en/web/webchatGateway(网关)>)或[网关协议](/zh/gateway/protocol)，并实现配对设备引导/设备令牌流程，以便设备不需要共享的 HTTP 令牌/密码。
- 当您集成具有自己的用户、房间、Webhook 传递或出站传输的外部消息传递网络时，请改为构建渠道插件。请参阅[构建插件](/zh/plugins/building-plugins)。
...
- `max_completion_tokens`OpenAI: 数字；单次调用总补全 Token 数的上限（包括推理 Token）。当前的 OpenAI Chat Completions 字段名称；当同时发送 `max_completion_tokens` 和 `max_tokens` 时优先使用此项。
- `max_tokens`: 数字；为向后兼容而接受的旧版别名。如果同时存在 `max_completion_tokens` 则忽略此项。
- `temperature`: 数字；通过代理流参数通道尽最大努力转发给上游提供商的采样温度。
- `top_p`: 数字；通过代理流参数通道尽最大努力转发给上游提供商的核采样。
- `frequency_penalty`：number；通过代理流参数渠道尽力而为地转发给上游提供商的频率惩罚。验证范围：-2.0 到 2.0。对于超出范围的值，返回 `400 invalid_request_error`。
- `presence_penalty`：number；通过代理流参数渠道尽力而为地转发给上游提供商的存在惩罚。验证范围：-2.0 到 2.0。对于超出范围的值，返回 `400 invalid_request_error`。
- `seed`：number（整数）；通过代理流参数渠道尽力而为地转发给上游提供商的种子。对于非整数值，返回 `400 invalid_request_error`。
- `stop`：字符串或最多 4 个字符串的数组；尽力而为的停止序列，通过代理流参数渠道转发给上游提供商。如果序列超过 4 个或条目为非字符串/空，则返回 `400 invalid_request_error`。

当设置了任一令牌上限字段时，该值将通过代理流参数渠道转发给上游提供商。发送给上游提供商的实际线路字段名称由提供商传输选择：OpenAI 系列端点使用 `max_completion_tokens`OpenAI，仅接受旧名称的提供商（如 Mistral 和 Chutes）使用 `max_tokens`。采样字段（`temperature`、`top_p`、`frequency_penalty`、`presence_penalty`、`seed`）遵循相同的流参数渠道；基于 ChatGPT 的 Codex Responses 后端会在服务器端将其剥离，因为它使用固定的采样。`stop` 也通过流参数渠道传递，并映射到传输的 stop 字段（Chat Completions 后端为 `stop`，Anthropic 为 `stop_sequences`AnthropicOpenAIAPI）；OpenAI Responses API 没有 stop 参数，因此 `stop` 不应用于 Responses 支持的模型。
...
- `tool_choice: "required"`（尚未在运行时强制执行；一旦实施强制执行将受支持）
- `tool_choice: { "type": "function", "function": { "name": "..." } }`（原因与 `required` 相同）
...
- 包含以下内容的 `choices[0].message.tool_calls[]` 条目：
  - `id`
  - `type: "function"`
  - `function.name`
...
- macOS 上的 Docker Base URL: DockermacOS`http://host.docker.internal:18789/v1`
...
- 如果您想为该代理指定特定的后端提供商/模型，请设置代理的正常默认模型或发送 `x-openclaw-model`
...
在该对话的后续调用中重用相同的 `user` 值，以延续同一个代理会话。
