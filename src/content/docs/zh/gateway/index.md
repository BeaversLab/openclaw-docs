...
<CardGroup cols={2}>
  <Card title="深度故障排除" icon="siren" href="/en/gateway/troubleshooting">
    基于症状的诊断，包含确切的命令阶梯和日志特征。
  </Card>
  <Card title="配置" icon="sliders" href="/en/gateway/configuration">
    面向任务的设置指南 + 完整配置参考。
  </Card>
  <Card title="密钥管理" icon="key-round" href="/en/gateway/secrets">
    SecretRef 协约、运行时快照行为以及迁移/重新加载操作。
  </Card>
  <Card title="密钥计划协议" icon="shield-check" href="/en/gateway/secrets-plan-contract">
    确切的 `secrets apply` 目标/路径规则以及仅引用的身份配置文件行为。
  </Card>
</CardGroup>
...
- 许多 RAG 和内存管线期望 `/v1/embeddings`。
- 原生 Agent 客户端越来越倾向于使用 `/v1/responses`。

规划说明：

- `/v1/models` 是 Agent 优先的：它返回 `openclaw`、`openclaw/default` 和 `openclaw/<agentId>`。
- `openclaw/default` 是一个稳定的别名，始终映射到配置的默认 Agent。
- 当您想要后端提供商/模型覆盖时，请使用 `x-openclaw-model`；否则所选 Agent 的常规模型和嵌入设置将保持控制权。
...
<Warning>如果配置了 Gateway 身份验证，客户端即使在通过 SSH 隧道连接时也必须发送身份验证信息 (`token`/`password`)。</Warning>

参见：[远程 Gateway(网关)](/en/gateway/remote)、[身份验证](/en/gateway/authentication)、[Tailscale](/en/gateway/tailscale)。
...
请参阅：[多网关](/en/gateway/multiple-gateways)。
...
2. 最终完成响应（`status:"ok"|"error"`），中间包含流式 `agent` 事件。

查看完整协议文档：[Gateway(网关) 协议](/en/gateway/protocol)。
...
如需完整的诊断步骤，请使用 [Gateway(网关) 故障排除](/en/gateway/troubleshooting)。
