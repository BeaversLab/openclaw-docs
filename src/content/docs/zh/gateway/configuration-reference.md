...
`~/.openclaw/openclaw.json` 中可用的每个字段。如需面向任务的概述，请参阅 [Configuration](/en/gateway/configuration)。
...
- 具有 `type: "acp"` 的顶级 `bindings[]` 条目为论坛主题配置持久的 ACP 绑定（在 `match.peer.id` 中使用规范的 `chatId:topic:topicId`）。字段语义在 [ACP Agents](/en/tools/acp-agents#channel-specific-settings) 中共享。
...
OpenClaw 生成 `imsg rpc`（基于 stdio 的 JSON-RPC）。不需要守护进程或端口。
...
- `cliPath` 可以指向 SSH 包装器；为 SCP 附件获取设置 `remoteHost`（`host` 或 `user@host`）。
...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目可以将 iMessage 对话绑定到持久的 ACP 会话。在 `match.peer.id` 中使用规范化的句柄或显式聊天目标（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义：[ACP Agents](/en/tools/acp-agents#channel-specific-settings)。
...
Microsoft Teams 由扩展提供支持，并在 `channels.msteams` 下进行配置。
...
- 完整的 Teams 配置（凭据、webhook、私信/组策略、每个团队/每个频道的覆盖）记录在 [Microsoft Teams](/en/channels/msteams) 中。

### IRC

IRC 由扩展提供支持，并在 `channels.irc` 下进行配置。
...
- 如果您在仍处于单账户顶级频道配置时，通过 `openclaw channels add`（或频道新手引导）添加非默认账户，OpenClaw 会首先将账户范围的顶级单账户值移动到 `channels.<channel>.accounts.default` 中，以便原始账户继续工作。
- 现有的仅渠道绑定（无 `accountId`）继续匹配默认账号；作用域为账号的绑定保持可选。
- 当存在命名账号但缺少 `default` 时，`openclaw doctor --fix` 还会通过将作用域为账号的顶级单账号值移动到 `accounts.default` 来修复混合形状。

### 其他扩展渠道

许多扩展渠道配置为 `channels.<id>` 并在其各自的渠道页面中记录（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
...
- 仅在检测可行时（原生提及或至少一种模式）才会强制执行提及控制。
...
`messages.groupChat.historyLimit` 设置全局默认值。渠道可以使用 `channels.<channel>.historyLimit`（或针对每个账号）进行覆盖。设置 `0` 以禁用。
...
- 每个渠道覆盖：`channels.discord.commands.native`（布尔值或 `"auto"`）。`false` 清除以前注册的命令。
...
- `config: true` 启用 `/config`（读/写 `openclaw.json`）。对于网关 `chat.send` 客户端，持久的 `/config set|unset` 写入还需要 `operator.admin`；只读 `/config show` 对普通写入范围的 Operator 客户端仍然可用。
...
- `allowFrom` 是针对每个提供商的。设置后，它是**唯一**的授权来源（渠道允许列表/配对和 `useAccessGroups` 被忽略）。
...
系统提示词 Runtime 行中显示的可选仓库根目录。如果未设置，OpenClaw 会通过从工作区向上遍历自动检测。
...
在调用提供商之前，脚本/工具图块中最长图像边的最大像素大小。
默认值：`1200`。

较低的值通常会减少包含大量截图的运行中的视觉令牌使用量和请求负载大小。
较高的值则保留更多视觉细节。
...
- `imageGenerationModel`：接受字符串（`"provider/model"`）或对象（`{ primary, fallbacks }`）。
  - 由共享的图像生成功能以及任何生成图像的未来工具/插件接口使用。
  ...
  - 如果直接选择提供商/模型，还需要配置匹配的提供商 auth/API 密钥（例如对于 `google/*` 配置 `GEMINI_API_KEY` 或 `GOOGLE_API_KEY`，对于 `openai/*` 配置 `OPENAI_API_KEY`，对于 `fal/*` 配置 `FAL_KEY`）。
  - 如果省略，`image_generate` 仍可从兼容的、支持认证的图像生成提供商中推断出尽力而为的提供商默认值。
...
- `model.primary`：格式 `provider/model`（例如 `anthropic/claude-opus-4-6`）。如果省略提供商，OpenClaw 假设为 `anthropic`（已弃用）。
- `models`：为 `/model` 配置的模型目录和允许列表。每个条目可以包含 `alias`（快捷方式）和 `params`（特定于提供商，例如 `temperature`、`maxTokens`、`cacheRetention`、`context1m`）。
- `params` 合并优先级（配置）：`agents.defaults.models["provider/model"].params` 是基础，然后 `agents.list[].params`（匹配的代理 ID）按键进行覆盖。
- 更改这些字段的配置编写器（例如 `/models set`、`/models set-image` 和回退添加/删除命令）会保存规范的对象形式，并尽可能保留现有的回退列表。
...
用于仅文本回退运行（无工具调用）的可选 CLI 后端。当 API 提供商出现故障时，可作为备份使用。
...
- `isolatedSession`：为 true 时，每次心跳都在没有先前对话历史记录的新会话中运行。与 cron `sessionTarget: "isolated"` 的隔离模式相同。将每次心跳的令牌成本从约 100K 降低到约 2-5K 个令牌。
- 按代理：设置 `agents.list[].heartbeat`。当任何代理定义 `heartbeat` 时，**只有那些代理**运行心跳。
...
- `identifierPolicy`：`strict`（默认）、`off` 或 `custom`。`strict` 在压缩摘要期间在开头添加内置的不透明标识符保留指导。
- `identifierInstructions`：当 `identifierPolicy=custom` 时使用的可选自定义标识符保留文本。
- `postCompactionSections`：要在压缩后重新注入的可选 AGENTS.md H2/H3 节名称。默认为 `["Session Startup", "Red Lines"]`；设置 `[]` 可禁用重新注入。如果未设置或显式设置为该默认对，则较旧的 `Every Session`/`Safety` 标题也会被接受作为传统回退。
- `model`：仅用于压缩摘要的可选 `provider/model-id` 覆盖。当主会话应保留一个模型但压缩摘要应在另一个模型上运行时使用；如果未设置，压缩使用会话的主模型。
- `memoryFlush`：自动压缩前的静默智能体轮次，用于存储持久化记忆。当工作区为只读时跳过。

### `agents.defaults.contextPruning`

在发送到 LLM 之前，从内存上下文中修剪**旧工具结果**。**不**修改磁盘上的会话历史。
...
- 修剪首先对过大的工具结果进行软修剪，然后在需要时对较旧的工具结果进行硬清除。

**Soft-trim** 保留开头 + 结尾，并在中间插入 `...`。
...
- 图像块永远不会被修剪/清除。
- 比率是基于字符的（近似），而不是精确的 token 计数。
- 如果少于 `keepLastAssistants` 条助手消息，则跳过修剪。

</Accordion>

有关行为详细信息，请参阅 [Session Pruning](/en/concepts/session-pruning)。
...
有关行为和分块详细信息，请参阅 [Streaming](/en/concepts/streaming)。
...
嵌入代理的可选沙箱隔离。完整指南请参阅[沙箱隔离](/en/gateway/sandboxing)。
...
- `workspaceRoot`：用于每个作用域工作空间的绝对远程根目录
- `identityFile` / `certificateFile` / `knownHostsFile`：传递给 OpenSSH 的现有本地文件
- `identityData` / `certificateData` / `knownHostsData`：内联内容或 SecretRef，由 OpenClaw 在运行时实例化为临时文件
...
- 在创建或重新创建后对远程工作空间进行一次初始化
- 然后保持远程 SSH 工作空间为权威来源
...
- `none`：位于 `~/.openclaw/sandboxes` 下的每个作用域沙箱工作空间
- `ro`：位于 `/workspace` 的沙箱工作空间，代理工作空间以只读方式挂载于 `/agent`
- `rw`：代理工作空间以读/写方式挂载于 `/workspace`

**作用域：**

- `session`：每个会话的容器 + 工作空间
...
- `mirror`：执行前从本地初始化远程，执行后同步回本地；本地工作空间保持为权威来源
- `remote`：在创建沙箱时初始化远程一次，然后保持远程工作空间为权威来源
...
**`docker.binds`** 挂载其他主机目录；全局和每个代理的绑定将被合并。
...
- `sandbox.browser.binds` 将额外的主机目录挂载到沙箱浏览器容器中。当设置时（包括 `[]`），它将替换浏览器容器的 `docker.binds`。
- 启动默认值在 `scripts/sandbox-browser-entrypoint.sh` 中定义，并针对容器主机进行了优化：
  ...
  - 如果您的工作流
    依赖扩展，`OPENCLAW_BROWSER_DISABLE_EXTENSIONS=0` 会重新启用它们。
...
浏览器沙箱隔离和 `sandbox.docker.binds` 目前仅限于 Docker。
...
- `id`：稳定的代理 ID（必需）。
- `default`：如果设置了多个，则以第一个为准（会记录警告）。如果未设置，则默认为列表中的第一个条目。
- `model`：字符串形式仅覆盖 `primary`；对象形式 `{ primary, fallbacks }` 覆盖两者（`[]` 禁用全局回退）。仅覆盖 `primary` 的 Cron 作业仍会继承默认回退，除非您设置了 `fallbacks: []`。
- `params`：合并到 `agents.defaults.models` 中选定模型条目的每个代理流参数。使用它进行特定于代理的覆盖，如 `cacheRetention`、`temperature` 或 `maxTokens`，而无需复制整个模型目录。
...
- `reasoningDefault`：可选的每个代理默认推理可见性（`on | off | stream`）。当未设置每条消息或会话推理覆盖时适用。
- `fastModeDefault`：可选的每个代理快速模式默认值（`true | false`）。当未设置每条消息或会话快速模式覆盖时适用。
- `runtime`：可选的每个代理运行时描述符。当代理应默认为 ACP 驱动会话时，使用带有 `runtime.acp` 默认值（`agent`、`backend`、`mode`、`cwd`）的 `type: "acp"`。
- `identity.avatar`：工作区相对路径、`http(s)` URL 或 `data:` URI。
- `identity` 派生默认值：`ackReaction` 源自 `emoji`，`mentionPatterns` 源自 `name`/`emoji`。
- `subagents.allowAgents`：`sessions_spawn` 的代理 ID 白名单（`["*"]` = 任意；默认：仅限同一代理）。
- 沙箱继承守卫：如果请求者会话处于沙箱隔离状态，`sessions_spawn` 将拒绝以非沙箱方式运行的目标。

---

## 多代理路由

在一个 Gateway(网关) 内运行多个隔离的代理。参见 [Multi-Agent](/en/concepts/multi-agent)。
...
对于 `type: "acp"` 条目，OpenClaw 根据确切的会话标识（`match.channel` + 账户 + `match.peer.id`）进行解析，并且不使用上述路由绑定层级顺序。
...
有关优先级详情，请参阅 [多智能体沙箱与工具](/en/tools/multi-agent-sandbox-tools)。
...
- **`reset`**: 主要重置策略。`daily` 在 `atHour` 本地时间重置；`idle` 在 `idleMinutes` 后重置。当两者都配置时，以先到期者为准。
...
- **`mainKey`**: 旧字段。运行时现在始终将 `"main"` 用于主直接聊天桶。
- **`sendPolicy`**: 按 `channel`、`chatType` (`direct|group|channel`，带有旧版 `dm` 别名)、`keyPrefix` 或 `rawKeyPrefix` 匹配。优先匹配拒绝项。
- **`maintenance`**: 会话存储清理 + 保留控制。
  ...
  - `maxDiskBytes`: 可选的会话目录磁盘预算。在 `warn` 模式下它记录警告；在 `enforce` 模式下它首先删除最旧的工件/会话。
...
- `removeAckAfterReply`：在回复后移除确认响应（仅限 Slack/Discord/Telegram/Google Chat）。

### 入站防抖

将同一发送者快速发送的纯文本消息合并为单个代理轮次。媒体/附件立即刷新。控制命令绕过防抖。
...
- `silenceTimeoutMs` 控制 Talk 模式在用户停止说话后等待发送文本记录的时长。未设置则保留平台默认暂停窗口 (`700 ms on macOS and Android, 900 ms on iOS`)。
...
控制哪些会话可以由会话工具 (`sessions_list`, `sessions_history`, `sessions_send`) 定位。
...
- 沙箱限制：当当前会话是沙箱隔离且 `agents.defaults.sandbox.sessionToolsVisibility="spawned"` 时，即使 `tools.sessions.visibility="all"`，可见性也会被强制为 `tree`。
...
- 文件被实例化到子工作区的 `.openclaw/attachments/<uuid>/` 中，并带有 `.manifest.json`。
- 附件内容会从脚本持久化中自动编辑。
...
- `model`: 生成子代理的默认模型。如果省略，子代理将继承调用者的模型。
...
- 匹配提供商 ID 的合并优先级：
  ...
  - 非空的 agent `apiKey` 值仅在该提供商在当前配置/auth-profile 上下文中不由 SecretRef 管理时才优先。
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`：可选的兼容性提示。对于具有非空非原生 `baseUrl`（主机不是 `api.openai.com`）的 `api: "openai-completions"`，OpenClaw 会在运行时将其强制设为 `false`。空或省略的 `baseUrl` 将保留默认的 OpenAI 行为。
...
<Accordion title="合成 (兼容Anthropic)">
...
如果您更喜欢旧版文本模型，`MiniMax-M2.5` 和 `MiniMax-M2.5-highspeed` 仍然可用。

</Accordion>

<Accordion title="本地模型 (LM Studio)">

参见 [本地模型](/en/gateway/local-models)。简而言之：在强大的硬件上通过 LM Studio Responses MiniMax 运行 API M2.5；保持托管模型合并以用于回退。
...
- `allowBundled`：仅适用于内置技能的可选允许列表（托管/工作区技能不受影响）。
- `entries.<skillKey>.enabled: false` 即使已内置/安装也会禁用技能。
- `entries.<skillKey>.apiKey`：为声明了主要环境变量的技能提供的便捷方式（纯文本字符串或 SecretRef 对象）。
...
- `plugins.entries.<id>.hooks.allowPromptInjection`：当 `false` 时，核心会阻止 `before_prompt_build` 并忽略旧版 `before_agent_start` 中的提示突变字段，同时保留旧版 `modelOverride` 和 `providerOverride`。适用于原生插件钩子和支持的包提供的钩子目录。
...
- 启用的 Claude 包插件还可以从 `settings.json` 贡献嵌入的 Pi 默认值；OpenClaw 将这些作为清理过的代理设置应用，而不是作为原始 OpenClaw 配置补丁。
- `plugins.slots.memory`：选择活动内存插件 ID，或选择 `"none"` 以禁用内存插件。
- `plugins.slots.contextEngine`：选择活动上下文引擎插件 ID；除非您安装并选择了另一个引擎，否则默认为 `"legacy"`。
- `plugins.installs`：由 `openclaw plugins update` 使用的 CLI 管理的安装元数据。
  ...
  - 将 `plugins.installs.*` 视为受管状态；相比手动编辑，优先使用 CLI 命令。
...
- 设置 `ssrfPolicy.dangerouslyAllowPrivateNetwork: false` 以强制实施仅限公开的浏览器导航。
- 在严格模式下，远程 CDP 配置文件端点（`profiles.*.cdpUrl`）在可达性/发现检查期间也会受到相同的专用网络阻止限制。
...
- `seamColor`：原生应用 UI 外观的强调色（通话模式气泡色调等）。
...
- `mode`: `local`（运行网关）或 `remote`（连接到远程网关）。除非是 `local`，否则 Gateway(网关) 拒绝启动。
...
- **Docker 注意事项**：默认的 `loopback` 绑定侦听容器内部的 `127.0.0.1`。使用 Docker 桥接网络（`-p 18789:18789`）时，流量到达 `eth0`，因此网关无法访问。请使用 `--network host`，或设置 `bind: "lan"`（或配合 `customBindHost: "0.0.0.0"` 使用 `bind: "custom"`）以侦听所有接口。
- **Auth**：默认情况下需要。非本地回环绑定需要共享令牌/密码。新手引导向导默认生成令牌。
...
- `gateway.auth.allowTailscale`: 当 `true` 时，Tailscale Serve 身份标头可以满足控制 UI/WebSocket 身份验证（通过 `tailscale whois` 验证）；HTTP API 端点仍需要令牌/密码身份验证。这种无令牌流程假定网关主机受信任。当 `tailscale.mode = "serve"` 时，默认为 `true`。
...
- `controlUi.allowedOrigins`: Gateway(网关) WebSocket 连接的显式浏览器源允许列表。当预期浏览器客户端来自非回环源时必需。
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: 危险模式，为有意依赖 Host-header 源策略的部署启用 Host-header 源回退。
- `remote.transport`: `ssh`（默认）或 `direct`（ws/wss）。对于 `direct`，`remote.url` 必须是 `ws://` 或 `wss://`。
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: 客户端端紧急覆盖，允许纯文本 `ws://` 连接到受信任的专用网络 IP；纯文本的默认设置仍为仅回环。
- `gateway.remote.token` / `.password` 是远程客户端凭据字段。它们本身不配置网关身份验证。
- `gateway.push.apns.relay.baseUrl`: 官方/TestFlight iOS 版本在向网关发布基于中继的注册后使用的外部 APNs 中继的基本 HTTPS URL。此 URL 必须与编译到 iOS 版本中的中继 URL 匹配。
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: 仅用于开发的回环 HTTP 中继 URL 紧急通道。生产中继 URL 应保持 HTTPS。
- `gateway.channelHealthCheckMinutes`: 渠道健康监控间隔（以分钟为单位）。设置 `0` 可全局禁用健康监控重启。默认值：`5`。
- `gateway.channelStaleEventThresholdMinutes`: 旧套接字阈值（以分钟为单位）。请保持此值大于或等于 `gateway.channelHealthCheckMinutes`。默认值：`30`。
- `gateway.channelMaxRestartsPerHour`: 每个渠道/帐户在一个滚动小时内最多可进行的健康监控重启次数。默认值：`10`。
...
- 本地网关调用路径仅当 `gateway.auth.*` 未设置时才能使用 `gateway.remote.*` 作为回退。
...
- `allowRealIpFallback`: 当 `true` 时，如果缺少 `X-Forwarded-For`，网关接受 `X-Real-IP`。默认 `false` 表示失败关闭行为。
...
- 聊天补全：默认禁用。通过 `gateway.http.endpoints.chatCompletions.enabled: true` 启用。
...
- `transform` 可以指向返回 hook 动作的 JS/TS 模块。
  - `transform.module` 必须是相对路径，并且必须保持在 `hooks.transformsDir` 内（绝对路径和遍历路径将被拒绝）。
...
- 不要在 Gateway(网关) 之外单独运行 `gog gmail watch serve`。
...
- 通过 HTTP 在 Gateway(网关) 端口下提供可由 agent 编辑的 HTML/CSS/JS 和 A2UI：
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- 仅限本地：保持 `gateway.bind: "loopback"`（默认）。
- 非环回绑定：画布路由需要 Gateway(网关) 认证（令牌/密码/受信任代理），与其他 Gateway(网关) HTTP 表面相同。
- 节点 WebViews 通常不发送认证头；在节点配对并连接后，Gateway(网关) 会发布用于画布/A2UI 访问的节点范围功能 URL。
...
- `file` 提供商 支持 `mode: "json"` 和 `mode: "singleValue"`（在 singleValue 模式下 `id` 必须是 `"value"`）。
- `exec` 提供商 需要一个绝对的 `command` 路径，并在 stdin/stdout 上使用协议 payloads。
...
- Secret refs 在激活时解析为内存快照，然后请求路径仅读取该快照。
- 激活期间应用活动表面过滤：已启用表面上未解析的 refs 将导致启动/重新加载失败，而非活动表面将被跳过并生成诊断信息。
...
- `auth-profiles.json` 支持值级别的引用（`keyRef` 用于 `api_key`，`tokenRef` 用于 `token`）。
- 静态运行时凭据来自内存中解析的快照；旧版静态 `auth.json` 条目在发现时会被清除。
- 从 `~/.openclaw/credentials/oauth.json` 导入旧版 OAuth。
- 参见 [OAuth](/en/concepts/oauth)。
- 密钥的运行时行为和 `audit/configure/apply` 工具：[密钥管理](/en/gateway/secrets)。
...
当前版本不再包含 TCP 桥接。节点通过 Gateway(网关) WebSocket 连接。`bridge.*` 键不再是配置架构的一部分（验证失败直到移除；`openclaw doctor --fix` 可以剥离未知键）。

<Accordion title="传统网桥配置（历史参考）">
...
- `sessionRetention`：在从 `sessions.json` 中清除之前，保留已完成的独立 cron 运行会话的时间。同时也控制已归档的已删除 cron 脚本的清理。默认值：`24h`；设置为 `false` 可禁用。
- `runLog.maxBytes`：清除前每个运行日志文件 (`cron/runs/<jobId>.jsonl`) 的最大大小。默认值：`2_000_000` 字节。
- `runLog.keepLines`：触发运行日志清除时保留的最新行数。默认值：`2000`。
- `webhookToken`：用于 cron webhook POST 传递 (`delivery.mode = "webhook"`) 的不记名令牌 (bearer token)，如果省略则不发送身份验证标头。
- `webhook`：已弃用的传统回退 webhook URL (http/https)，仅用于仍然具有 `notify: true` 的存储作业。

参见 [Cron 作业](/en/automation/cron-jobs)。
