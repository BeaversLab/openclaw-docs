...
`~/.openclaw/openclaw.json` 中可用的每个字段。有关面向任务概览，请参阅[配置](/en/gateway/configuration)。
...
如果完全缺少提供商块（`channels.<provider>` 不存在），运行时组策略将回退到 `allowlist`（故障关闭）并显示启动警告。
</Note>

### 渠道模型覆盖

使用 `channels.modelByChannel` 将特定渠道 ID 固定到模型。值接受 `provider/model` 或配置的模型别名。当会话尚未具有模型覆盖（例如，通过 `/model` 设置）时，应用渠道映射。
...
使用 `channels.defaults` 跨提供商实现共享组策略和心跳行为：
...
- 可选的 `channels.whatsapp.defaultAccount` 会在匹配到配置的账号 id 时覆盖该回退默认账号选择。
...
- 可选的 `channels.telegram.defaultAccount` 会在匹配到配置的账号 id 时覆盖默认账号选择。
- 在多账号设置（2 个或以上账号 id）中，请设置显式默认值（`channels.telegram.defaultAccount` 或 `channels.telegram.accounts.default`）以避免回退路由；`openclaw doctor` 会在缺失或无效时发出警告。
- `configWrites: false` 阻止 Telegram 发起的配置写入（超级群组 ID 迁移，`/config set|unset`）。
- 具有 `type: "acp"` 的顶级 `bindings[]` 条目为论坛主题配置持久的 ACP 绑定（在 `match.peer.id` 中使用规范 `chatId:topic:topicId`）。字段语义在 [ACP 代理](/en/tools/acp-agents#channel-specific-settings) 中共享。
- Telegram 流预览使用 `sendMessage` + `editMessageText`（适用于直接和群组聊天）。
- 重试策略：请参阅 [重试策略](/en/concepts/retry)。
...
- 令牌：`channels.discord.token`，其中 `DISCORD_BOT_TOKEN` 作为默认帐户的后备。
- 提供显式 Discord `token` 的直接出站调用使用该令牌进行调用；帐户重试/策略设置仍来自活动运行时快照中所选的帐户。
- 可选的 `channels.discord.defaultAccount` 在匹配配置的帐户 ID 时覆盖默认帐户选择。
- 使用 `user:<id>` (私信) 或 `channel:<id>` (guild 渠道) 作为传递目标；拒绝纯数字 ID。
- Guild slugs 为小写，空格替换为 `-`；频道键使用 slugged 名称（无 `#`）。建议使用 Guild ID。
- 默认情况下忽略 Bot 发送的消息。`allowBots: true` 启用它们；使用 `allowBots: "mentions"` 仅接受提及该 bot 的 bot 消息（自己的消息仍被过滤）。
- `channels.discord.guilds.<id>.ignoreOtherMentions`（以及频道覆盖）会丢弃提及另一用户或角色但未提及 bot 的消息（不包括 @everyone/@here）。
- `maxLinesPerMessage`（默认为 17）拆分长消息，即使少于 2000 个字符。
- `channels.discord.threadBindings` 控制 Discord 线程绑定路由：
  - `enabled`：Discord 覆盖线程绑定会话功能（`/focus`、`/unfocus`、`/agents`、`/session idle`、`/session max-age` 以及绑定传递/路由）
  - `idleHours`：覆盖 Discord 非活动自动取消聚焦的小时数（`0` 表示禁用）
  - `maxAgeHours`：覆盖 Discord 硬性最大存在时间的小时数（`0` 表示禁用）
  - `spawnSubagentSessions`：`sessions_spawn({ thread: true })` 自动线程创建/绑定的选择性加入开关
...
- OpenClaw 还会在重复解密失败后尝试通过退出并重新加入语音会话来恢复语音接收。
...
- `channels.discord.dangerouslyAllowNameMatching` 重新启用可变名称/标签匹配（紧急情况兼容模式）。
- `channels.discord.execApprovals`：Discord 原生执行审批传递和审批人授权。
  ...
  - `target`：发送审批提示的位置。`"dm"`（默认）发送到审批者的私信，`"channel"` 发送到原始渠道，`"both"` 发送到两者。当目标包含 `"channel"` 时，按钮仅可供已解析的审批者使用。
...
- `channels.googlechat.dangerouslyAllowNameMatching` 重新启用可变的电子邮件主体匹配（应急玻璃兼容模式）。
...
- **HTTP 模式**需要 `botToken` 以及 `signingSecret`（在根级别或每个账号级别）。
- `botToken`、`appToken`、`signingSecret` 和 `userToken` 接受纯文本
  字符串或 SecretRef 对象。
- Slack 账户快照公开了每个凭据的源/状态字段，例如 `botTokenSource`、`botTokenStatus`、`appTokenStatus`，以及在 HTTP 模式下的 `signingSecretStatus`。`configured_unavailable` 表示该账户是通过 SecretRef 配置的，但当前的命令/运行时路径无法解析密钥值。
- `configWrites: false` 会阻止 Slack 发起的配置写入。
- 可选的 `channels.slack.defaultAccount` 会在匹配到已配置的账户 ID 时覆盖默认的账户选择。
- `channels.slack.streaming` 是规范的流模式键。旧式的 `streamMode` 和布尔 `streaming` 值会自动迁移。
- 使用 `user:<id>` (私信) 或 `channel:<id>` 作为投递目标。

**反应通知模式：** `off`、`own`（默认）、`all`、`allowlist`（来自 `reactionAllowlist`）。

**线程会话隔离：** `thread.historyScope` 是按线程隔离（默认）还是跨渠道共享。`thread.inheritParent` 会将父渠道的记录复制到新线程中。

- `typingReaction` 会在回复运行时向入站的 Slack 消息添加一个临时的反应，然后在完成后将其移除。使用 Slack 表情符号短代码，例如 `"hourglass_flowing_sand"`。
- `channels.slack.execApprovals`：Slack 原生执行审批投递和审批者授权。架构与 Discord 相同：`enabled` (`true`/`false`/`"auto"`)、`approvers`（Slack 用户 ID）、`agentFilter`、`sessionFilter` 和 `target`（`"dm"`、`"channel"` 或 `"both"`）。
...
- 原生斜杠回调通过 Mattermost 在斜杠命令注册期间返回的每命令令牌进行身份验证。如果注册失败或未激活任何命令，OpenClaw 将拒绝回调并显示 `Unauthorized: invalid command token.`
- 对于私有/tailnet/内部回调主机，Mattermost 可能要求 `ServiceSettings.AllowedUntrustedInternalConnections` 包含回调主机/域名。使用主机/域名值，而不是完整的 URL。
- `channels.mattermost.configWrites`：允许或拒绝 Mattermost 发起的配置写入。
- `channels.mattermost.requireMention`：在渠道中回复之前需要 `@mention`。
- `channels.mattermost.groups.<channelId>.requireMention`：逐渠道提及覆盖（`"*"` 为默认值）。
...
- 可选的 `channels.bluebubbles.defaultAccount` 会在匹配到已配置的账号 ID 时覆盖默认的账号选择。
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目可以将 BlueBubbles 会话绑定到持久的 ACP 会话。在 `match.peer.id` 中使用 BlueBubbles 句柄或目标字符串（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义：[ACP Agents](/en/tools/acp-agents#channel-specific-settings)。
- 完整的 BlueBubbles 渠道配置文档请参阅 [BlueBubbles](/en/channels/bluebubbles)。
...
- 可选的 `channels.imessage.defaultAccount` 会在匹配到已配置的账号 ID 时覆盖默认的账号选择。

- 需要对 Messages 数据库进行完全磁盘访问。
- 首选 `chat_id:<id>` 目标。使用 `imsg chats --limit 20` 列出聊天。
- `cliPath` 可以指向 SSH 封装程序；设置 `remoteHost`（`host` 或 `user@host`）以便通过 SCP 获取附件。
...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目可以将 iMessage 会话绑定到持久的 ACP 会话。在 `match.peer.id` 中使用规范化的句柄或显式聊天目标（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义：[ACP Agents](/en/tools/acp-agents#channel-specific-settings)。
...
- `channels.matrix.allowPrivateNetwork` 允许私有/内部主服务器。`proxy` 和 `allowPrivateNetwork` 是独立的控制项。
- `channels.matrix.defaultAccount` 在多账户设置中选择首选账户。
- `channels.matrix.execApprovals`：Matrix 原生执行审批交付和审批者授权。
  - `enabled`：`true`、`false` 或 `"auto"`（默认）。在自动模式下，当可以从 `approvers` 或 `commands.ownerAllowFrom` 解析出审批者时，执行审批即激活。
  ...
  - `target`：发送审批提示的位置。`"dm"`（默认）、`"channel"`（起始房间）或 `"both"`。
  - 按账户覆盖：`channels.matrix.accounts.<id>.execApprovals`。
- `channels.matrix.dm.sessionScope` 控制 Matrix 私信如何归组到会话中：`per-user`（默认）按路由对端共享，而 `per-room` 隔离每个私信房间。
...
- 完整的 Microsoft Teams 配置（凭据、webhook、私信/群组策略、针对团队/渠道的覆盖）记录在 [Microsoft Teams](/en/channels/msteams) 中。
...
- 可选的 `channels.irc.defaultAccount` 在与已配置的账户 ID 匹配时，会覆盖默认的账户选择。
...
- 当省略 `accountId` 时（CLI + 路由），将使用 `default`。
- 环境令牌仅适用于**默认**账户。
- 基本渠道设置适用于所有账户，除非在特定账户中被覆盖。
- 使用 `bindings[].match.accountId` 将每个账户路由到不同的代理。
- 如果您在仍使用单账户顶级渠道配置的情况下，通过 `openclaw channels add`（或渠道新手引导）添加非默认账户，OpenClaw 会首先将账户范围的顶级单账户值提升到渠道账户映射中，以便原始账户继续工作。大多数渠道会将它们移动到 `channels.<channel>.accounts.default` 中；Matrix 则可以改为保留现有的匹配的命名/默认目标。
- 现有的仅渠道绑定（无 `accountId`）继续匹配默认账户；账户范围的绑定仍然是可选的。
- `openclaw doctor --fix` 还会通过将账户范围的顶级单账户值移动到为该渠道选择的提升后的账户中来修复混合形状。大多数渠道使用 `accounts.default`；Matrix 则可以改为保留现有的匹配的命名/默认目标。

### 其他扩展渠道

许多扩展渠道配置为 `channels.<id>`，并在其专门的渠道页面中进行了文档说明（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
请查看完整的渠道索引：[Channels](/en/channels)。

### 群聊提及控制

群消息默认为**需要提及**（元数据提及或安全的正则模式）。适用于 WhatsApp、Telegram、Discord、Google Chat 和 iMessage 群聊。

**提及类型：**

- **元数据提及**：平台原生的 @-提及。在 WhatsApp 自聊模式下忽略。
- **文本模式**：`agents.list[].groupChat.mentionPatterns` 中的安全正则模式。无效的模式和不安全的嵌套重复将被忽略。
- 只有在可以检测到提及（原生提及或至少一种模式）时，才会强制执行提及控制。
...
- `bash: true` 为主机 Shell 启用 `! <cmd>`。需要 `tools.elevated.enabled` 并且发送者在 `tools.elevated.allowFrom.<channel>` 中。
- `config: true` 启用 `/config`（读取/写入 `openclaw.json`）。对于网关 `chat.send` 客户端，持久化 `/config set|unset` 写入还需要 `operator.admin`；只读 `/config show` 对普通写入范围的操作员客户端仍然可用。
- `channels.<provider>.configWrites` 限制每个渠道的配置变更（默认：true）。
- 对于多账户渠道，`channels.<provider>.accounts.<id>.configWrites` 还限制针对该账户的写入（例如 `/allowlist --config --account <id>` 或 `/config set channels.<provider>.accounts.<id>...`）。
...
系统提示运行时行中显示的可选存储库根目录。如果未设置，OpenClaw 会通过从工作区向上遍历来自动检测。
...
在调用提供商之前，脚本/工具图像块中图像最长边的最大像素尺寸。
默认值：`1200`。

较低的值通常会减少包含大量屏幕截图的运行中的视觉令牌（vision-token）使用量和请求负载大小。
较高的值可保留更多视觉细节。
...
- `imageGenerationModel`：接受字符串 (`"provider/model"`) 或对象 (`{ primary, fallbacks }`)。
  ...
  - 如果您直接选择提供商/模型，请同时配置匹配的提供商 auth/API 密钥（例如，对于 `google/*`，配置 `GEMINI_API_KEY` 或 `GOOGLE_API_KEY`；对于 `openai/*`，配置 `OPENAI_API_KEY`；对于 `fal/*`，配置 `FAL_KEY`）。
  - 如果省略，`image_generate` 仍然可以推断出具有身份验证支持的默认提供商。它首先尝试当前的默认提供商，然后按提供商 ID 的顺序尝试剩余的注册图像生成提供商。
- `musicGenerationModel`：接受字符串 (`"provider/model"`) 或对象 (`{ primary, fallbacks }`)。
  ...
  - 如果省略，`music_generate` 仍然可以推断出具有身份验证支持的默认提供商。它首先尝试当前的默认提供商，然后按提供商 ID 的顺序尝试剩余的注册音乐生成提供商。
  - 如果您直接选择提供商/模型，请同时配置匹配的提供商 auth/API 密钥。
- `videoGenerationModel`：接受字符串 (`"provider/model"`) 或对象 (`{ primary, fallbacks }`)。
  - 由共享视频生成功能和内置的 `video_generate` 工具使用。
  - 典型值：`qwen/wan2.6-t2v`、`qwen/wan2.6-i2v`、`qwen/wan2.6-r2v`、`qwen/wan2.6-r2v-flash` 或 `qwen/wan2.7-r2v`。
  - 如果省略，`video_generate` 仍然可以推断支持身份验证的提供商默认值。它首先尝试当前的默认提供商，然后按提供商 ID 顺序尝试剩余的已注册视频生成提供商。
  - 如果您直接选择提供商/模型，请同时配置匹配的提供商身份验证/API 密钥。
  - 捆绑的 Qwen 视频生成提供商目前支持最多 1 个输出视频、1 个输入图像、4 个输入视频、10 秒时长以及提供商级别的 `size`、`aspectRatio`、`resolution`、`audio` 和 `watermark` 选项。
...
- `model.primary`：格式 `provider/model`（例如 `openai/gpt-5.4`）。如果您省略提供商，OpenClaw 会首先尝试别名，然后为该确切的模型 ID 尝试唯一匹配的配置提供商，只有在这种情况下才会回退到配置的默认提供商（已弃用的兼容性行为，因此最好显式指定 `provider/model`）。如果该提供商不再暴露配置的默认模型，OpenClaw 将回退到第一个配置的提供商/模型，而不是显示陈旧的已移除提供商的默认值。
- `models`：为 `/model` 配置的模型目录和允许列表。每个条目可以包括 `alias`（快捷方式）和 `params`（特定于提供商，例如 `temperature`、`maxTokens`、`cacheRetention`、`context1m`）。
...
- 改变这些字段的配置编写器（例如 `/models set`、`/models set-image` 和回退添加/删除命令）会保存规范的对象形式，并在可能的情况下保留现有的回退列表。
- `maxConcurrent`：跨会话的最大并行代理运行数（每个会话仍然串行化）。默认值：4。
...
Z.AI 模型在工具调用流式传输时默认启用 `tool_stream`。将 `agents.defaults.models["zai/<model>"].params.tool_stream` 设置为 `false` 以将其禁用。
...
- `isolatedSession`：为 true 时，每次心跳在新的会话中运行，没有之前的对话历史。与 cron `sessionTarget: "isolated"` 具有相同的隔离模式。将每次心跳的 token 成本从约 100K 减少到约 2-5K token。
...
- `mode`： `default` 或 `safeguard`（针对长历史的分块摘要）。参见 [Compaction](/en/concepts/compaction)。
- `timeoutSeconds`：在 OpenClaw 中止单次压缩操作之前允许的最大秒数。默认值： `900`。
- `identifierPolicy`： `strict`（默认值）、 `off` 或 `custom`。 `strict` 会在压缩摘要期间预置内置的不透明标识符保留指导。
- `identifierInstructions`：当使用 `identifierPolicy=custom` 时使用的可选自定义标识符保留文本。
- `postCompactionSections`：压缩后重新注入的可选 AGENTS.md H2/H3 部分名称。默认为 `["Session Startup", "Red Lines"]`；设置为 `[]` 可禁用重新注入。当未设置或显式设置为该默认值对时，较旧的 `Every Session`/`Safety` 标题也将作为旧版回退被接受。
- `model`：仅用于压缩摘要的可选 `provider/model-id` 覆盖。当主会话应保留一个模型但压缩摘要应在另一个模型上运行时使用此选项；如果未设置，压缩将使用会话的主要模型。
- `notifyUser`：当为 `true` 时，在压缩开始时向用户发送简短通知（例如，“Compacting context...”）。默认禁用以保持压缩静默。
- `memoryFlush`：自动压缩前的静默智能体轮次，用于存储持久记忆。当工作区为只读时跳过。
...
- `mode: "cache-ttl"` 启用清理遍历。
- `ttl` 控制清理再次运行的频率（在最后一次缓存访问之后）。
- 清理首先对过大的工具结果进行软修剪，然后在需要时对较旧的工具结果进行硬清除。
...
- `identityData` / `certificateData` / `knownHostsData`：内联内容或 SecretRefs，OpenClaw 会在运行时将其具象化为临时文件
...
- SecretRef 支持的 `*Data` 值在沙箱会话开始之前从活动机密运行时快照中解析
...
- `mirror`：在执行前从本地初始化远程，在执行后同步回；本地工作区保持权威
- `remote`：在创建沙箱时初始化远程一次，然后保持远程工作区为权威

在 `remote` 模式下，在 OpenClaw 之外进行的主机本地编辑在初始化步骤之后不会自动同步到沙箱中。
...
`"host"` 被阻止。除非您明确设置
`sandbox.docker.dangerouslyAllowContainerNamespaceJoin: true`（应急），否则 `"container:<id>"` 默认被阻止。

**入站附件** 被暂存到活动工作区中的 `media/inbound/*`。

**`docker.binds`** 挂载其他主机目录；全局和每个代理的绑定会被合并。

**沙箱浏览器**（`sandbox.browser.enabled`）：容器中的 Chromium + CDP。noVNC URL 注入到系统提示中。不需要 `browser.enabled` 在 `openclaw.json` 中。
...
- `cdpSourceRange` 可选择地将容器边缘的 CDP 入站限制为 CIDR 范围（例如 `172.21.0.1/32`）。
- `sandbox.browser.binds` 将其他主机目录仅挂载到沙箱浏览器容器中。设置后（包括 `[]`），它将替换浏览器容器的 `docker.binds`。
- 启动默认值在 `scripts/sandbox-browser-entrypoint.sh` 中定义，并针对容器主机进行了调整：
  ...
  - `OPENCLAW_BROWSER_DISABLE_EXTENSIONS=0` 如果您的工作流依赖扩展，则重新启用它们。
...
- `id`：稳定的代理 ID（必需）。
- `default`：当设置了多个时，第一个生效（会记录警告）。如果未设置，则第一个列表项为默认值。
- `model`：字符串形式仅覆盖 `primary`；对象形式 `{ primary, fallbacks }` 覆盖两者（`[]` 禁用全局回退）。仅覆盖 `primary` 的 Cron 作业仍会继承默认回退，除非您设置了 `fallbacks: []`。
- `params`：与 `agents.defaults.models` 中所选模型条目合并的每个代理流参数。将其用于代理特定的覆盖，如 `cacheRetention`、`temperature` 或 `maxTokens`，而无需复制整个模型目录。
- `skills`：可选的每个代理技能允许列表。如果省略，代理将继承已设置的 `agents.defaults.skills`；显式列表会替换默认值而不是合并，并且 `[]` 表示没有技能。
...
- `reasoningDefault`：可选的每个代理默认推理可见性（`on | off | stream`）。当未设置每条消息或会话推理覆盖时应用。
- `fastModeDefault`：可选的每个代理快速模式默认值（`true | false`）。当未设置每条消息或会话快速模式覆盖时应用。
- `runtime`：可选的每个代理运行时描述符。当代理应默认为 ACP 驱动会话时，将 `type: "acp"` 与 `runtime.acp` 默认值（`agent`、`backend`、`mode`、`cwd`）一起使用。
...
- 沙箱隔离继承保护：如果请求者会话处于沙箱隔离状态，`sessions_spawn` 将拒绝以非沙箱隔离方式运行的目标。
...
- `type`（可选）：`route` 用于普通路由（缺失类型默认为 route），`acp` 用于持久化 ACP 会话绑定。
- `match.channel`（必填）
...
对于 `type: "acp"` 条目，OpenClaw 根据确切的对话标识（`match.channel` + 账户 + `match.peer.id`）进行解析，而不使用上述路由绑定层顺序。
...
- **`reset`**: 主要重置策略。`daily` 在 `atHour` 本地时间重置；`idle` 在 `idleMinutes` 后重置。当两者都配置时，以先过期者为准。
...
- **`mainKey`**: 旧版字段。运行时现在始终为主直接聊天存储桶使用 `"main"`。
- **`agentToAgent.maxPingPongTurns`**: 代理之间在代理到代理交换期间的最大回复轮次（整数，范围：`0`–`5`）。`0` 禁用乒乓链式连接。
- **`sendPolicy`**: 通过 `channel`，`chatType`（`direct|group|channel`，带有旧版 `dm` 别名），`keyPrefix` 或 `rawKeyPrefix` 进行匹配。先拒绝者优先。
- **`maintenance`**: 会话存储清理 + 保留控制。
  ...
  - `maxDiskBytes`：可选的会话目录磁盘预算。在 `warn` 模式下它记录警告；在 `enforce` 模式下它首先删除最旧的人工制品/会话。
...
解析顺序（最具体者优先）：账户 → 渠道 → 全局。`""` 禁用并停止级联。`"auto"` 派生自 `[{identity.name}]`。
...
- `messages.statusReactions.enabled`：在 Slack、Discord 和 Telegram 上启用生命周期状态反应。
  在 Slack 和 Discord 上，未设置会在确认反应处于活动状态时保持启用状态反应。
  在 Telegram 上，将其显式设置为 `true` 以启用生命周期状态反应。

### 入站防抖

将来自同一发件人的快速纯文本消息批处理为单个代理轮次。媒体/附件立即刷新。控制命令绕过防抖。
...
- `auto` 控制自动 TTS。`/tts off|always|inbound|tagged` 会覆盖每次会话的设置。
...
- `openai.baseUrl` 覆盖 OpenAI TTS 端点。解析顺序为配置，然后是 `OPENAI_TTS_BASE_URL`，接着是 `https://api.openai.com/v1`。
- 当 `openai.baseUrl` 指向非 OpenAI 端点时，OpenClaw 会将其视为 OpenAI 兼容的 TTS 服务器并放宽模型/声音验证。
...
- 当配置了多个 Talk 提供商时，`talk.provider` 必须匹配 `talk.providers` 中的键。
- 旧的扁平 Talk 键（`talk.voiceId`、`talk.voiceAliases`、`talk.modelId`、`talk.outputFormat`、`talk.apiKey`）仅用于兼容性，并会自动迁移到 `talk.providers.<provider>` 中。
- 声音 ID 会回退到 `ELEVENLABS_VOICE_ID` 或 `SAG_VOICE_ID`。
- `providers.*.apiKey` 接受纯文本字符串或 SecretRef 对象。
- `ELEVENLABS_API_KEY` 回退仅在未配置 Talk API 密钥时适用。
- `providers.*.voiceAliases` 允许 Talk 指令使用友好名称。
- `silenceTimeoutMs` 控制 Talk 模式在用户静默后发送转录文本之前等待的时间。未设置则保留平台默认的暂停窗口（`700 ms on macOS and Android, 900 ms on iOS`）。
...
- 提升的 `exec` 绕过沙箱并使用配置的逃逸路径（默认为 `gateway`，当执行目标是 `node` 时则为 `node`）。
...
- `detectors.pingPong`：对交替的无进展对模式发出警告/阻断。
...
- `asyncCompletion.directSend`：当为 `true` 时，已完成的异步 `music_generate`
  和 `video_generate` 任务首先尝试直接渠道投递。默认值：`false`
  （旧版请求者-会话唤醒/模型投递路径）。
...
- 文件在 `.openclaw/attachments/<uuid>/` 处被具体化到子工作空间中，并带有 `.manifest.json`。
- 附件内容会从脚本持久化中自动编辑掉。
...
- 启用后，系统提示还会添加使用指南，以便模型仅将其用于实质性工作，并且最多保留一个步骤 `in_progress`。
...
- 使用 `OPENCLAW_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`，即旧版环境变量别名）覆盖代理配置根目录。
- 匹配提供商 ID 的合并优先级：
  - 非空的代理 `models.json` `baseUrl` 值优先。
  - 仅当该提供商在当前配置/身份验证配置文件上下文中不由 SecretRef 管理时，非空的代理 `apiKey` 值才优先。
  - SecretRef 管理的提供商 `apiKey` 值将从源标记刷新（环境引用为 `ENV_VAR_NAME`，文件/执行引用为 `secretref-managed`），而不是持久化已解析的机密。
  - SecretRef 管理的提供商标头值将从源标记刷新（环境引用为 `secretref-env:ENV_VAR_NAME`，文件/执行引用为 `secretref-managed`）。
  - 空或缺失的代理 `apiKey`/`baseUrl` 将回退到配置中的 `models.providers`。
  - 匹配的模型 `contextWindow`/`maxTokens` 使用显式配置和隐式目录值之间的较大值。
  - 匹配的模型 `contextTokens` 会在存在时保留显式的运行时上限；使用它可以在不更改本机模型元数据的情况下限制有效上下文。
  - 当您希望配置完全重写 `models.json` 时，请使用 `models.mode: "replace"`。
  - 标记持久化是以源为权威的：标记是从活动的源配置快照（解析前）写入的，而不是从解析后的运行时密钥值写入的。
...
- `models.providers.*.request`：模型提供商 HTTP 请求的传输覆盖设置。
  ...
  - `request.auth`：认证策略覆盖。模式：`"provider-default"`（使用提供商的内置认证）、`"authorization-bearer"`（带有 `token`）、`"header"`（带有 `headerName`、`value`、可选的 `prefix`）。
...
- `models.providers.*.models.*.contextTokens`：可选的运行时上下文上限。当您希望有效上下文预算小于模型的原生 `contextWindow` 时使用此项。
- `models.providers.*.models.*.compat.supportsDeveloperRole`：可选的兼容性提示。对于具有非空非原生 `baseUrl`（主机不是 `api.openai.com`）的 `api: "openai-completions"`，OpenClaw 会在运行时强制将其设为 `false`。为空或省略 `baseUrl` 将保留默认的 OpenAI 行为。
...
原生 Moonshot 端点在共享的 `openai-completions` 传输上宣传流式使用兼容性，OpenClaw 现在根据端点功能而不仅仅是内置提供商 ID 来对此进行匹配。
...
模型目录现在默认仅包含 M2.7。
在兼容 Anthropic 的流式传输路径上，除非您显式自行设置 `thinking`，否则 OpenClaw 默认会禁用 MiniMax 思考功能。`/fast on` 或
...
参见 [本地模型](/en/gateway/local-models)。简而言之：在强力硬件上通过 LM Studio Responses API 运行大型本地模型；保持托管模型已合并以作为回退。
...
- `install.nodeManager`：`metadata.openclaw.install`
  规范的节点安装程序首选项（`npm` | `pnpm` | `yarn` | `bun`）。
- `entries.<skillKey>.enabled: false` 即使在已捆绑/已安装的情况下也会禁用 Skill。
- `entries.<skillKey>.apiKey`：为声明了主环境变量的 Skills 提供的便利设置（纯文本字符串或 SecretRef 对象）。
...
- `plugins.entries.<id>.hooks.allowPromptInjection`：当 `false` 时，核心会阻止 `before_prompt_build` 并忽略来自旧版 `before_agent_start` 的提示突变字段，同时保留旧版 `modelOverride` 和 `providerOverride`。适用于原生插件钩子和支持的捆绑包提供的钩子目录。
- `plugins.entries.<id>.subagent.allowModelOverride`：显式信任此插件，以为后台子代理运行请求每次运行的 `provider` 和 `model` 覆盖。
...
- `plugins.entries.memory-core.config.dreaming`：内存梦境（实验性）设置。有关阶段和阈值，请参阅 [Dreaming](/en/concepts/dreaming)。
  - `enabled`：主梦境开关（默认为 `false`）。
  - `frequency`：每次完整梦境扫描的 cron 频率（默认为 `"0 3 * * *"`）。
  - 阶段策略和阈值是实现细节（不是面向用户的配置键）。
- 已启用的 Claude 捆绑插件还可以从 `settings.json` 提供嵌入式 Pi 默认值；OpenClaw 将这些作为清理后的代理设置应用，而不是作为原始 OpenClaw 配置补丁。
...
- 在严格模式下，远程 CDP 配置文件端点 (`profiles.*.cdpUrl`) 在可达性/发现检查期间也会受到同样的私有网络阻止限制。
- `ssrfPolicy.allowPrivateNetwork` 作为旧别名仍然受支持。
- 在严格模式下，使用 `ssrfPolicy.hostnameAllowlist` 和 `ssrfPolicy.allowedHostnames` 作为明确的例外。
...
- `seamColor`：本机应用 UI 铬 (对话模式气泡色调等) 的强调色。
- `assistant`：控制 UI 身份覆盖。回退到当前代理的身份。
...
<Accordion title="Gateway(网关) field details">

- `mode`: `local` (运行网关) 或 `remote` (连接到远程网关)。除非设置为 `local`，否则 Gateway(网关) 拒绝启动。
- `port`: 用于 WS + HTTP 的单个多路复用端口。优先级：`--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port` > `18789`。
- `bind`: `auto`、`loopback`（默认）、`lan`（`0.0.0.0`）、`tailnet`（仅 Tailscale IP）或 `custom`。
- **Legacy bind aliases**: 在 `gateway.bind`（`auto`、`loopback`、`lan`、`tailnet`、`custom`）中使用绑定模式值，而不是主机别名（`0.0.0.0`、`127.0.0.1`、`localhost`、`::`、`::1`）。
- **Docker note**: 默认的 `loopback` 绑定监听容器内部的 `127.0.0.1`。在使用 Docker 网桥网络（`-p 18789:18789`）时，流量到达 `eth0`，因此网关无法访问。请使用 `--network host`，或设置 `bind: "lan"`（或在 `customBindHost: "0.0.0.0"` 中设置 `bind: "custom"`）以监听所有接口。
- **Auth**: 默认情况下需要。非 local loopback 绑定需要网关认证。实际上，这意味着共享令牌/密码或具有 `gateway.auth.mode: "trusted-proxy"` 的感知身份的反向代理。新手引导向导默认会生成一个令牌。
...
- `gateway.auth.mode: "trusted-proxy"`: 将认证委托给感知身份的反向代理，并信任来自 `gateway.trustedProxies` 的身份标头（请参阅 [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth)）。此模式期望 **非 local loopback** 代理源；同主机 loopback 反向代理不满足 trusted-proxy 认证。
- `gateway.auth.allowTailscale`: 当 `true` 时，Tailscale Serve 身份标头可以满足控制 UI/WebSocket 认证（通过 `tailscale whois` 验证）。HTTP API 端点 **不** 使用该 Tailscale 标头认证；它们遵循网关的正常 HTTP 认证模式。此无令牌流程假设网关主机是受信任的。当 `tailscale.mode = "serve"` 时，默认为 `true`。
- `gateway.auth.rateLimit`: 可选的失败认证限制器。适用于每个客户端 IP 和每个认证范围（共享密钥和设备令牌独立跟踪）。受阻的尝试返回 `429` + `Retry-After`。
  - 在异步 Tailscale Serve 控制 UI 路径上，同一 `{scope, clientIp}` 的失败尝试在失败写入之前被序列化。因此，来自同一客户端的并发错误尝试可能会在第二个请求时触发限制器，而不是像普通不匹配那样竞争通过。
  - `gateway.auth.rateLimit.exemptLoopback` 默认为 `true`；如果您希望也对 localhost 流量进行速率限制（用于测试设置或严格的代理部署），请设置 `false`。
- 浏览器源 WS 认证尝试始终受到限制，并禁用 loopback 豁免（深度防御基于浏览器的 localhost 暴力破解）。
- 在 loopback 上，这些浏览器源锁定是按标准化的 `Origin`
  值隔离的，因此来自一个 localhost 源的重复失败不会自动
  锁定不同的源。
- `tailscale.mode`: `serve`（仅 tailnet，loopback 绑定）或 `funnel`（公开，需要认证）。
- `controlUi.allowedOrigins`: Gateway(网关) WebSocket 连接的显式浏览器源允许列表。当预期来自非 loopback 源的浏览器客户端时，这是必需的。
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: 危险模式，为有意依赖 Host 标头源策略的部署启用 Host 标头源回退。
- `remote.transport`: `ssh`（默认）或 `direct` (ws/wss)。对于 `direct`，`remote.url` 必须是 `ws://` 或 `wss://`。
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: 客户端应急覆盖，允许明文 `ws://` 连接到受信任的专用网络 IP；对于明文，默认仍仅限于 loopback。
- `gateway.remote.token` / `.password` 是远程客户端凭据字段。它们本身不配置网关认证。
- `gateway.push.apns.relay.baseUrl`: 官方/TestFlight iOS 构建版在将基于中继的注册发布到网关后使用的外部 APNs 中继的基本 HTTPS URL。此 URL 必须与 iOS 构建中编译的中继 URL 匹配。
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: 仅用于 loopback HTTP 中继 URL 的开发逃生口。生产中继 URL 应保持 HTTPS。
- `gateway.channelHealthCheckMinutes`: 渠道健康监视器间隔（以分钟为单位）。设置 `0` 可全局禁用健康监视器重启。默认值：`5`。
- `gateway.channelStaleEventThresholdMinutes`: 陈旧套接字阈值（以分钟为单位）。保持此值大于或等于 `gateway.channelHealthCheckMinutes`。默认值：`30`。
- `gateway.channelMaxRestartsPerHour`: 每个渠道/帐户在滚动小时内的最大健康监视器重启次数。默认值：`10`。
- `channels.<provider>.healthMonitor.enabled`: 健康监视器重启的每个渠道退出选择，同时保持全局监视器启用。
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: 多帐户渠道的每个帐户覆盖。设置后，它优先于渠道级覆盖。
- 本地网关调用路径仅当 `gateway.auth.*` 未设置时，才能将 `gateway.remote.*` 作为回退使用。
- 如果 `gateway.auth.token` / `gateway.auth.password` 是通过 SecretRef 显式配置且未解析，则解析失败（无远程回退掩码）。
- `trustedProxies`: 终止 TLS 或注入转发客户端标头的反向代理 IP。仅列出您控制的代理。Loopback 条目对于同主机代理/本地检测设置（例如 Tailscale Serve 或本地反向代理）仍然有效，但它们 **不** 使 loopback 请求符合 `gateway.auth.mode: "trusted-proxy"` 的条件。
- `allowRealIpFallback`: 当 `true` 时，如果 `X-Forwarded-For` 缺失，网关接受 `X-Real-IP`。默认为 `false` 以实现失败关闭行为。
...
- 可选的响应加固标头：
  - `gateway.http.securityHeaders.strictTransportSecurity`（仅为您控制的 HTTPS 源设置；参见[受信任代理认证](/en/gateway/trusted-proxy-auth#tls-termination-and-hsts)）
...
- `autoGenerate`：当未配置显式文件时，自动生成本地自签名证书/密钥对；仅限本地/开发使用。
...
- `deferralTimeoutMs`: 在强制重启前等待进行中操作的最大毫秒时间（默认：`300000` = 5 分钟）。
...
- `hooks.token` 必须与 `gateway.auth.token` **不同**；复用 Gateway(网关) 令牌将被拒绝。
...
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - 仅当 `hooks.allowRequestSessionKey=true` 时，才会接受请求负载中的 `sessionKey`（默认：`false`）。
...
- `transform` 可以指向返回 hook 操作的 JS/TS 模块。
  - `transform.module` 必须是相对路径，并且必须保持在 `hooks.transformsDir` 内部（拒绝绝对路径和路径遍历）。
- `agentId` 路由到特定的 agent；未知的 ID 会回退到默认值。
- `allowedAgentIds`：限制显式路由（`*` 或省略 = 允许所有，`[]` = 拒绝所有）。
- `defaultSessionKey`：用于在没有显式 `sessionKey` 的情况下运行 hook agent 的可选固定会话键。
...
- 非环回绑定：画布路由需要 Gateway(网关) 认证（令牌/密码/可信代理），与其他 Gateway(网关) HTTP 表面相同。
- Node WebViews 通常不发送认证头；在节点配对并连接后，Gateway(网关) 会为画布/A2UI 访问通告节点范围的功能 URL。
- 功能 URL 绑定到活动节点 WS 会话并快速过期。不使用基于 IP 的回退。
- 将实时重载客户端注入到提供的 HTML 中。
- 如果为空，则自动创建初始 `index.html`。
- 同时在 `/__openclaw__/a2ui/` 提供 A2UI 服务。
- 更改需要重启网关。
...
在 `~/.openclaw/dns/` 下写入单播 DNS-SD 区域。对于跨网络发现，请将其与 DNS 服务器（推荐 CoreDNS）+ Tailscale 分离 DNS 搭配使用。
...
- 有关完整优先级，请参阅 [Environment](/en/help/environment)。
...
- `exec` 子进程环境默认为最小化；使用 `passEnv` 显式传递所需变量。
- Secret 引用在激活时解析为内存快照，然后请求路径仅读取该快照。
- 激活期间应用活动表面过滤：已启用表面上的未解析引用会导致启动/重新加载失败，而非活动表面则会被跳过并输出诊断信息。
...
- `auth-profiles.json` 支持值级别引用（`keyRef` 表示 `api_key`，`tokenRef` 表示 `token`），适用于静态凭证模式。
- OAuth 模式配置文件（`auth.profiles.<id>.mode = "oauth"`）不支持基于 SecretRef 的身份验证配置文件凭证。
- 静态运行时凭证来自内存中解析的快照；传统的静态 `auth.json` 条目在发现时会被清除。
- 从 `~/.openclaw/credentials/oauth.json` 传统的 OAuth 导入。
...
- `billingBackoffHours`：当配置文件因真实的计费/余额不足错误而失败时的基本退避时间，以小时为单位（默认值：`5`）。即使是在 `401`/`403` 响应上，显式的计费文本也可能落入此处，但特定于提供商的文本匹配器仍仅限于拥有它们的提供商（例如 OpenRouter `Key limit exceeded`）。可重试的 HTTP `402` 使用量窗口或组织/工作区支出限制消息将保留在 `rate_limit` 路径中。
- `billingBackoffHoursByProvider`：针对计费退避小时数的可选的每个提供商覆盖值。
...
- `overloadedProfileRotations`：在切换到模型回退之前，针对超载错误进行同一提供商身份配置文件轮换的最大次数（默认：`1`）。提供商繁忙类型的错误（如 `ModelNotReadyException`）属于此类。
- `overloadedBackoffMs`：重试过载的提供商/配置文件轮换之前的固定延迟（默认：`0`）。
- `rateLimitedProfileRotations`：在切换到模型回退之前，针对速率限制错误进行同一提供商身份配置文件轮换的最大次数（默认：`1`）。该速率限制桶包括提供商特定的文本，例如 `Too many concurrent requests`、`ThrottlingException`、`concurrency limit reached`、`workers_ai ... quota limit exceeded` 和 `resource exhausted`。
...
- `maxFileBytes`：抑制写入之前的最大日志文件大小（以字节为单位，正整数；默认：`524288000` = 500 MB）。对于生产部署，请使用外部日志轮换。
...
- `enabled`：检测输出的主控开关（默认：`true`）。
- `flags`：用于启用目标日志输出的标志字符串数组（支持通配符，如 `"telegram.*"` 或 `"*"`）。
- `stuckSessionWarnMs`：当会话保持处理状态时，发出卡死会话警告的时间阈值（单位：毫秒）。
...
- `auto.stableJitterHours`：额外的稳定版分批发布时间窗口（小时）（默认：`12`；最大值：`168`）。
- `auto.betaCheckIntervalHours`：测试版检查运行频率（小时）（默认：`1`；最大值：`24`）。
...
- `dispatch.enabled`：ACP 会话轮次分发的独立开关（默认：`true`）。设置为 `false` 可在阻止执行时保持 ACP 命令可用。
- `backend`: 默认 ACP 运行时后端 ID（必须匹配已注册的 ACP 运行时插件）。
- `defaultAgent`: 当生成 未指定显式目标时的备用 ACP 目标代理 ID。
...
- `stream.hiddenBoundarySeparator`: 隐藏工具事件之后可见文本之前的分隔符（默认：`"paragraph"`）。
...
请参阅 [Agent defaults](#agent-defaults) 下的 `agents.list` 标识字段。

---

## Bridge（旧版，已移除）

当前版本不再包含 TCP 桥接。节点通过 Gateway(网关) WebSocket 连接。`bridge.*` 键不再是配置架构的一部分（移除前验证会失败；`openclaw doctor --fix` 可以剥离未知键）。
...
- `sessionRetention`：在从 `sessions.json` 中修剪之前保留已完成的隔离 cron 运行会话的时间。还控制已归档的已删除 cron 副本的清理。默认值：`24h`；设置为 `false` 以禁用。
- `runLog.maxBytes`：修剪前每个运行日志文件的最大大小 (`cron/runs/<jobId>.jsonl`)。默认值：`2_000_000` 字节。
- `runLog.keepLines`：触发运行日志修剪时保留的最新行数。默认值：`2000`。
- `webhookToken`：用于 cron webhook POST 传递 (`delivery.mode = "webhook"`) 的 bearer 令牌，如果省略，则不发送 auth 标头。
...
仅适用于一次性 cron 作业。周期性作业使用单独的错误处理。
...
- `accountId`：用于限定警报传递范围的可选帐号或渠道 ID。
...
- 所有任务的 cron 失败通知的默认目标。
- `mode`：`"announce"` 或 `"webhook"`；当存在足够的目标数据时，默认为 `"announce"`。
- `channel`：通告传递的渠道覆盖。`"last"` 重用最后已知的传递渠道。
- `to`：显式通告目标或 webhook URL。Webhook 模式所必需。
- `accountId`：用于传递的可选帐号覆盖。
- 每个任务的 `delivery.failureDestination` 会覆盖此全局默认值。
- 当未设置全局或每任务的失败目标时，已经通过 `announce` 传递的任务在失败时会回退到该主要通告目标。
- `delivery.failureDestination` 仅受 `sessionTarget="isolated"` 任务支持，除非任务的主要 `delivery.mode` 是 `"webhook"`。

参见 [Cron Jobs](/en/automation/cron-jobs)。隔离的 cron 执行作为 [background tasks](/en/automation/tasks) 进行跟踪。
...
- 兄弟键：在包含后合并（覆盖包含的值）。
- 嵌套包含：最多 10 层深度。
- 路径：相对于包含文件解析，但必须保持在顶级配置目录（`dirname` 的 `openclaw.json`）内。仅当解析结果仍在该边界内时，才允许使用绝对路径/`../` 形式。
- 错误：针对缺失文件、解析错误和循环包含提供明确信息。
