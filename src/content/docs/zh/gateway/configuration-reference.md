...
`~/.openclaw/openclaw.json` 的核心配置参考。有关面向任务的概述，请参阅 [Configuration](/zh/gateway/configuration)。
...
Agent 查找路径：在编辑之前，请使用 `gateway` 工具操作 `config.schema.lookup` 获取精确的字段级文档和约束。请使用 [Configuration](/zh/gateway/configuration) 获取面向任务的指导，并使用本页面获取更广泛的字段映射、默认值以及指向子系统参考的链接。

专用深入参考：

- `agents.defaults.memorySearch.*`、`memory.qmd.*`、`memory.citations` 以及 `plugins.entries.memory-core.config.dreaming` 下的梦境配置的 [Memory configuration reference](/zh/reference/memory-config)
...
每个渠道的配置键已移至专用页面 - 请参阅
[Configuration - channels](/zh/gateway/config-channels) 以获取 `channels.*`SlackDiscordTelegramWhatsAppMatrixiMessage，
...
已移至专用页面 - 请参阅
[Configuration - agents](/zh/gateway/config-agents) 以获取：
...
[Configuration - tools and custom providers](/zh/gateway/config-tools#custom-providers-and-base-urls)。
...
- `models.pricing.enabled`Gateway(网关)：控制在 sidecar 和通道到达 Gateway 就绪路径后启动的
  后台定价引导。当 `false`Gateway(网关)OpenRouter 时，
  Gateway 会跳过 OpenRouter 和 LiteLLM 定价目录获取；配置的
  `models.providers.*.models[].cost` 值仍可用于本地成本估算。

## MCP

OpenClaw 托管的 MCP 服务器定义位于 OpenClaw`mcp.servers` 之下，并
...
- `mcp.servers`：为暴露已配置 MCP 工具的运行时提供的
  命名 stdio 或远程 MCP 服务器定义。
...
- `load.allowSymlinkTargets`：受信任的实际目标根目录，当 Skills 符号链接位于其配置的源根目录之外时，
  可以解析到这些目录。
- `install.preferBrew`：如果为 true，则在 `brew` 可用时
  优先使用 Homebrew 安装程序，然后再回退到其他类型的安装程序。
...
- `install.allowUploadedArchives`：允许受信任的 `operator.admin`Gateway(网关) Gateway(网关)
  客户端安装通过 `skills.upload.*`ClawHub 暂存的私有 zip 归档
  （默认值：false）。这仅启用了上传归档的路径；正常的 ClawHub
  安装不需要它。
- `entries.<skillKey>.enabled: false` 禁用技能，即使该技能已被打包/安装。
- `entries.<skillKey>.apiKey`：为声明主环境变量的技能提供便利（纯文本字符串或 SecretRef 对象）。
...
- `bundledDiscovery`：对于新配置，默认值为 `"allowlist"`，因此非空的
  `plugins.allow` 也会限制打包的提供商插件，包括网络搜索
  运行时提供商。对于迁移的旧版允许列表配置，Doctor 会写入 `"compat"`
  以保留现有的打包提供商行为，直到您选择加入。
...
- `plugins.entries.<id>.hooks.allowConversationAccess`：当 `true` 时，受信任的非捆绑插件可以从类型化挂钩（例如 `llm_input`、`llm_output`、`before_model_resolve`、`before_agent_reply`、`before_agent_run`、`before_agent_finalize` 和 `agent_end`）读取原始对话内容。
...
- `plugins.entries.<id>.llm.allowModelOverride`：显式信任此插件以请求针对 `api.runtime.llm.complete` 的模型覆盖。
- `plugins.entries.<id>.llm.allowedModels`：用于受信任插件 LLM 完成覆盖的规范 `provider/model`LLM 目标的可选允许列表。仅当您有意允许任何模型时才使用 `"*"`。
- `plugins.entries.<id>.llm.allowAgentIdOverride`：显式信任此插件以针对非默认代理 ID 运行 `api.runtime.llm.complete`。
- `plugins.entries.<id>.config`OpenClaw：插件定义的配置对象（如果可用，由原生 OpenClaw 插件架构验证）。
- 通道插件帐户/运行时设置位于 `channels.<id>` 下，应由所属插件的清单 `channelConfigs`OpenClaw 元数据描述，而不是由中央 OpenClaw 选项注册表描述。

### Codex harness 插件配置

捆绑的 `codex` 插件拥有 `plugins.entries.codex.config` 下的原生 Codex 应用服务器 harness 设置。有关完整的配置表面，请参阅 [Codex harness 参考](/zh/plugins/codex-harness-reference)，有关运行时模型，请参阅 [Codex harness](/zh/plugins/codex-harness)。
...
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.enabled`：当全局 `codexPlugins.enabled` 也为 true 时，启用
  已迁移的插件条目。显式条目的默认值为 `true`。
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.marketplaceName`：
  稳定的市场标识。V1 仅支持 `"openai-curated"`。
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.pluginName`：从迁移而来的稳定
  Codex 插件标识，例如 `"google-calendar"`。
- `plugins.entries.codex.config.codexPlugins.plugins.<key>.allow_destructive_actions`：
  每个插件的破坏性操作覆盖设置。如果省略，则使用
...
- `plugins.entries.memory-core.config.dreaming`：记忆做梦 设置。有关阶段和阈值，请参阅 [Dreaming](/zh/concepts/dreaming)。
...
- 启用的 Claude 包插件还可以从 `settings.json`OpenClawOpenClaw 提供嵌入式 Pi 默认值；OpenClaw 会将这些作为经过清理的代理设置应用，而不是作为原始的 OpenClaw 配置补丁。
...
`commitments` 控制推断的后续记忆：OpenClaw 可以检测对话轮次中的签到（check-ins），并通过心跳运行传递它们。

- `commitments.enabled`：启用推断的后续承诺的隐藏 LLM 提取、存储和心跳传递。默认值：`false`。
- `commitments.maxPerDay`：在每个滚动日内，每个代理会话传递的最大推断后续承诺数量。默认值：`3`。

参见 [推断的承诺](/zh/concepts/commitments)。
...
- `ssrfPolicy.dangerouslyAllowPrivateNetwork` 在未设置时被禁用，因此浏览器导航默认保持严格模式。
- 仅在您有意信任私有网络浏览器导航时，才设置 `ssrfPolicy.dangerouslyAllowPrivateNetwork: true`。
...
- 本地托管配置文件在进程启动后使用 `browser.localLaunchTimeoutMs` 进行 Chrome CDP HTTP
  发现，并在启动后使用 `browser.localCdpReadyTimeoutMs` 等待
  CDP websocket 就绪。在 Chrome 成功启动但就绪检查与启动过程冲突的较慢主机上，请增加这些值。这两个值都必须是
...
- `browser.executablePath` 和 `browser.profiles.<name>.executablePath` 都
  在 Chromium 启动前接受 `~` 和 `~/...` 作为您操作系统的主目录。
  `existing-session` 配置文件上每个配置文件的 `userDataDir` 也会进行波浪号扩展。
...
- `seamColor`：原生应用 UI chrome 的强调色（对话模式气泡色调等）。
...
- `mode`：`local`（运行网关）或 `remote`Gateway(网关)（连接到远程网关）。除非设置为 `local`，否则Gateway(网关)拒绝启动。
...
- **身份验证**：默认情况下需要。非本地环回绑定需要网关身份验证。实际上，这意味着共享令牌/密码或具有 `gateway.auth.mode: "trusted-proxy"` 的感知身份的反向代理。新手引导向导默认生成令牌。
...
- `gateway.auth.mode: "trusted-proxy"`：将浏览器/用户身份验证委托给感知身份的反向代理，并信任来自 `gateway.trustedProxies` 的身份标头（请参阅 [Trusted Proxy Auth](/zh/gateway/trusted-proxy-auth)）。此模式默认期望**非本地环回**代理源；同主机本地环回反向代理需要显式的 `gateway.auth.trustedProxy.allowLoopback = true`。内部同主机调用者可以将 `gateway.auth.password` 作为本地直接回退；`gateway.auth.token` 与受信任代理模式仍然互斥。
- `gateway.auth.allowTailscale`：当 `true`Tailscale 时，Tailscale Serve 身份标头可以满足控制 UI/WebSocket 身份验证（通过 `tailscale whois`APITailscale 验证）。HTTP API 端点**不**使用该 Tailscale 标头身份验证；它们遵循网关的正常 HTTP 身份验证模式。此无令牌流程假设网关主机受信任。当 `tailscale.mode = "serve"` 时，默认为 `true`。
- `gateway.auth.rateLimit`：可选的身份验证失败限制器。适用于每个客户端 IP 和每个身份验证范围（共享密钥和设备令牌独立跟踪）。被阻止的尝试返回 `429` + `Retry-After`Tailscale。
  - 在异步 Tailscale Serve 控制 UI 路径上，相同的 `{scope, clientIp}` 的失败尝试在失败写入之前被序列化。因此，来自同一客户端的并发错误尝试可能会在第二个请求时触发限制器，而不是两者都作为简单的不匹配竞争通过。
  - `gateway.auth.rateLimit.exemptLoopback` 默认为 `true`；当您有意希望也对本地主机流量进行速率限制时（对于测试设置或严格的代理部署），设置 `false`。
- 浏览器源 WS 身份验证尝试始终受到限制，并禁用本地环回豁免（针对基于浏览器的本地主机暴力破解的纵深防御）。
- 在本地环回上，这些浏览器源锁定是按标准化的 `Origin`
  值隔离的，因此来自一个本地主机源的重复失败不会自动
  锁定不同的源。
- `tailscale.mode`：`serve`（仅限 tailnet，本地环回绑定）或 `funnel`（公共，需要身份验证）。
- `tailscale.preserveFunnel`：当 `true` 和 `tailscale.mode = "serve"`OpenClaw 时，OpenClaw
  在启动时重新应用 Serve 之前检查 `tailscale funnel status`，如果外部配置的 Funnel 路由已覆盖网关端口，则跳过
  它。默认 `false`。
- `controlUi.allowedOrigins`Gateway(网关)：Gateway(网关) WebSocket 连接的显式浏览器源允许列表。当预期来自非本地环回源的浏览器客户端时，这是必需的。
- `controlUi.chatMessageMaxWidth`：分组的控制 UI 聊天消息的可选最大宽度。接受受约束的 CSS 宽度值，例如 `960px`、`82%`、`min(1280px, 82%)` 和 `calc(100% - 2rem)`。
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`：客户端进程环境
  应急覆盖，允许将纯文本 `ws://` 用于受信任的专用网络
  IP；纯文本的默认值仍为仅限本地环回。没有 `openclaw.json`
  等效项，并且浏览器专用网络配置（例如
  `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork`Gateway(网关)）不影响 Gateway(网关)
  WebSocket 客户端。
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`：仅用于开发的本地环回 HTTP 中继 URL 的应急手段。生产环境中继 URL 应保持 HTTPS。
- `gateway.handshakeTimeoutMs`Gateway(网关)：预身份验证 Gateway(网关) WebSocket 握手超时（以毫秒为单位）。默认值：`15000`。设置 `OPENCLAW_HANDSHAKE_TIMEOUT_MS` 时优先。在负载较重或低性能主机上增加此值，以便在启动预热仍在进行时本地客户端可以连接。
...
- `gateway.channelMaxRestartsPerHour`：每个渠道/账户在一个滚动小时内的最大健康监控重启次数。默认值：`10`。
- `channels.<provider>.healthMonitor.enabled`：在保持全局监控启用的同时，针对每个渠道选择退出健康监控重启。
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`：多账户渠道的每个账户覆盖。设置后，它优先于渠道级别的覆盖。
- 本地网关调用路径仅当 `gateway.auth.*` 未设置时才能将 `gateway.remote.*` 用作回退。
- 如果 `gateway.auth.token` / `gateway.auth.password` 通过 SecretRef 显式配置且未解析，则解析失败关闭（无远程回退掩码）。
- `trustedProxies`Tailscale：终止 TLS 或注入转发的客户端标头的反向代理 IP。仅列出您控制的代理。本地环回条目对于同主机代理/本地检测设置（例如 Tailscale Serve 或本地反向代理）仍然有效，但它们并**不**使本地环回请求有资格进行 `gateway.auth.mode: "trusted-proxy"`。
- `allowRealIpFallback`：当 `true` 时，如果 `X-Forwarded-For` 缺失，网关接受 `X-Real-IP`。默认 `false` 以实现故障关闭行为。
- `gateway.nodes.pairing.autoApproveCidrs`WebChat：用于自动批准首次节点设备配对（无请求范围）的可选 CIDR/IP 允许列表。如果未设置，则禁用。这不会自动批准操作员/浏览器/控制 UI/WebChat 配对，也不会自动批准角色、范围、元数据或公钥升级。
- `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`：配对和平台允许列表评估后，声明的节点命令的全局允许/拒绝塑造。使用 `allowCommands` 选择加入危险的节点命令，例如 `camera.snap`、`camera.clip` 和 `screen.record`；`denyCommands` 移除命令，即使平台默认或显式允许否则会包含它。节点更改其声明的命令列表后，拒绝并重新批准该设备配对，以便网关存储更新的命令快照。
...
- Responses API：API`gateway.http.endpoints.responses.enabled`。
- Responses URL 输入强化：
...
- `enabled`：在网关监听器上启用 TLS 终结（HTTPS/WSS）（默认：`false`）。
...
- `deferralTimeoutMs`：在强制重启或渠道热重载之前，等待进行中操作的可选最长时间（毫秒）。省略此项以使用默认的有界等待（`300000`）；设置 `0` 以无限期等待并定期记录仍待处理的警告。
...
- `hooks.token` 必须与 `gateway.auth.token`Gateway(网关) **不同**；重用 Gateway 令牌将被拒绝。
...
- `POST /hooks/<name>` → 通过 `hooks.mappings` 解析
  - 模板渲染的映射 `sessionKey` 值被视为外部提供的，并且还需要 `hooks.allowRequestSessionKey=true`。
...
- `transform` 可以指向一个返回 hook 操作的 JS/TS 模块。
  - `transform.module` 必须是相对路径，并且必须位于 `hooks.transformsDir` 内（绝对路径和遍历操作将被拒绝）。
  - 将 `hooks.transformsDir` 保持在 `~/.openclaw/hooks/transforms` 之下；工作区技能目录将被拒绝。如果 `openclaw doctor` 报告此路径无效，请将转换模块移动到 hooks transforms 目录中或删除 `hooks.transformsDir`。
...
- `allowedSessionKeyPrefixes`：用于显式 `sessionKey` 值（请求 + 映射）的可选前缀允许列表，例如 `["hook:"]`。当任何映射或预设使用模板化的 `sessionKey` 时，它将变为必需项。
- `deliver: true` 将最终回复发送到渠道；`channel` 默认为 `last`。
- `model`LLM 覆盖此 hook 运行的 LLM（如果设置了模型目录，则必须允许）。
...
- 如果您保留该每条消息的路由，请设置 `hooks.allowRequestSessionKey: true` 并将 `hooks.allowedSessionKeyPrefixes` 限制为匹配 Gmail 命名空间，例如 `["hook:", "hook:gmail:"]`。
...
- Node WebViews 通常不发送身份验证标头；在节点配对并连接后，Gateway(网关) 会通告用于 canvas/A2UI 访问的节点作用域功能 URL。
...
- `full`：包含 `cliPath` + `sshPort`；LAN 组播广告仍需要启用捆绑的 `bonjour` 插件。
- `off`：在不更改插件启用状态的情况下禁止 LAN 组播广告。
- 内置的 `bonjour`macOSLinuxWindowsGateway(网关) 插件在 macOS 主机上自动启动，在 Linux、Windows 和容器化的 Gateway(网关) 部署中是可选的。
...
在 `~/.openclaw/dns/`Tailscale 下写入单播 DNS-SD 区域。用于跨网络发现时，需配合 DNS 服务器（推荐 CoreDNS）+ Tailscale 分离 DNS 使用。
...
- 当 Windows ACL 验证不可用时，文件和 exec 提供商路径将以失败关闭（fail closed）方式处理。仅对无法验证的受信任路径设置 `allowInsecurePath: true`。
- `exec` 提供商需要绝对的 `command` 路径，并在 stdin/stdout 上使用协议负载。
- 默认情况下，拒绝符号链接命令路径。设置 `allowSymlinkCommand: true` 以在验证解析后的目标路径时允许符号链接路径。
- 如果配置了 `trustedDirs`，则受信任目录检查将应用于解析后的目标路径。
- `exec` 子环境默认是最小化的；使用 `passEnv` 显式传递所需的变量。
- Secret 引用在激活时解析为内存快照，然后请求路径仅读取该快照。
- 活动表面过滤在激活期间应用：已启用表面上的未解析引用会导致启动/重载失败，而未激活的表面将被跳过并生成诊断信息。
...
- 对于静态凭证模式，`auth-profiles.json` 支持值级引用（`api_key` 的 `keyRef`，`token` 的 `tokenRef`）。
...
- 静态运行时凭据来自内存中解析的快照；发现时会清除传统的静态 `auth.json` 条目。
- 从 OAuth`~/.openclaw/credentials/oauth.json` 传统的 OAuth 导入。
...
- `billingBackoffHours`：当配置文件因真实的计费/信用不足错误而失败时，以小时为单位的基本退避时间（默认值：`5`）。即使对于 `401`/`403`OpenRouter 响应，明确的计费文本仍可能出现在此处，但特定于提供商的文本匹配器仍限定于拥有它们的提供商（例如 OpenRouter `Key limit exceeded`）。可重试的 HTTP `402` 使用量窗口或组织/工作区支出限制消息将改为保留在 `rate_limit` 路径中。
- `billingBackoffHoursByProvider`：可选的每个提供商计费退避小时数覆盖。
- `billingMaxHours`：计费退避指数增长的小时数上限（默认值：`24`）。
- `authPermanentBackoffMinutes`：高置信度 `auth_permanent` 失败的以分钟为单位的基本退避时间（默认值：`10`）。
- `authPermanentMaxMinutes`：`auth_permanent` 退避增长的分钟数上限（默认值：`60`）。
- `failureWindowHours`：用于退避计数器的以小时为单位的滚动窗口（默认值：`24`）。
- `overloadedProfileRotations`：在切换到模型回退之前，针对超载错误的最大同一提供商身份验证配置文件轮换次数（默认值：`1`）。提供商繁忙类型（例如 `ModelNotReadyException`）会归类于此。
- `overloadedBackoffMs`：重试过载的提供商/配置轮换之前的固定延迟（默认：`0`）。
- `rateLimitedProfileRotations`：在切换到模型回退之前，针对速率限制错误的最大同提供商身份验证配置轮换次数（默认：`1`）。该速率限制存储桶包括提供商形状的文本，例如 `Too many concurrent requests`、`ThrottlingException`、`concurrency limit reached`、`workers_ai ... quota limit exceeded` 和 `resource exhausted`。
...
- `maxFileBytes`：轮换前活动日志文件的最大大小（以字节为单位，正整数；默认：`104857600`OpenClaw = 100 MB）。OpenClaw 在活动文件旁最多保留五个编号的存档。
- `redactSensitive` / `redactPatterns`：针对控制台输出、文件日志、OTLP 日志记录和持久化会话记录文本的尽力屏蔽。`redactSensitive: "off"` 仅禁用此常规日志/记录策略；UI/工具/诊断安全界面在发出之前仍会编辑密钥。
...
- `flags`：用于启用定向日志输出的标志字符串数组（支持像 `"telegram.*"` 或 `"*"` 这样的通配符）。
- `stuckSessionWarnMs`：用于将长时间运行的会话分类为 `session.long_running`、`session.stalled` 或 `session.stuck` 的无进展时间阈值（以毫秒为单位）。回复、工具、状态、块和 ACP 进度会重置计时器；如果未更改，重复的 `session.stuck` 诊断会退避。
- `stuckSessionAbortMs`OpenClaw: 在符合条件的停滞活动工作可能被中止排出以进行恢复之前的无进度时间阈值（毫秒）。如果未设置，OpenClaw 将使用更安全的扩展嵌入式运行窗口，该窗口至少为 10 分钟且为 `stuckSessionWarnMs` 的 5 倍。
...
- `otel.tracesEndpoint` / `otel.metricsEndpoint` / `otel.logsEndpoint`: 可选的特定信号 OTLP 端点。设置后，它们将仅覆盖该信号的 `otel.endpoint`。
...
- `otel.captureContent`: 选择加入用于 OTEL 跨度属性的原始内容捕获。默认为关闭。布尔值 `true` 捕获非系统消息/工具内容；对象形式允许您显式启用 `inputMessages`、`outputMessages`、`toolInputs`、`toolOutputs` 和 `systemPrompt`。
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: 用于最新实验性 GenAI 跨度提供商属性的环境切换开关。默认情况下，为了兼容性，跨度保留旧的 `gen_ai.system` 属性；GenAI 指标使用有界语义属性。
- `OPENCLAW_OTEL_PRELOADED=1`：针对已注册全局 OpenTelemetry SDK 的主机的环境切换开关。OpenClaw 然后会跳过插件拥有的 SDK 启动/关闭，同时保持诊断侦听器处于活动状态。
...
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`：控制缓存跟踪输出中包含的内容（所有默认值均为 `true`）。
...
- `auto.stableJitterHours`：额外的稳定版发布推广窗口小时数（默认：`12`；最大值：`168`）。
- `auto.betaCheckIntervalHours`：测试版检查的运行频率，以小时为单位（默认：`1`；最大值：`24`）。
...
- `enabled`：全局 ACP 功能开关（默认：`true`；设置 `false` 以隐藏 ACP 调度和生成功能）。
- `dispatch.enabled`：用于 ACP 会话轮次调度的独立开关（默认：`true`）。设置 `false` 以在阻止执行的同时保持 ACP 命令可用。
- `backend`：默认的 ACP 运行时后端 ID（必须与已注册的 ACP 运行时插件匹配）。
  请先安装后端插件，如果设置了 `plugins.allow`，请包含后端插件 ID（例如 `acpx`），否则 ACP 后端将无法加载。
- `defaultAgent`：当生成未指定明确目标时的后备 ACP 目标代理 ID。
...
请参阅 [Agent 默认值](/zh/gateway/config-agents#agent-defaults) 下的 `agents.list` 身份字段。

---

## 桥接（已弃用，已移除）

当前版本不再包含 TCP 桥接。节点通过 Gateway(网关) WebSocket 连接。Gateway(网关)`bridge.*` 键不再是配置架构的一部分（验证将失败，直到将其删除；`openclaw doctor --fix` 可以去除未知键）。
...
- `sessionRetention`：在从 `sessions.json` 中清理之前，保留已完成的隔离 Cron 运行会话的时间。还控制已归档的已删除 Cron 副本的清理。默认值：`24h`；设置为 `false` 以禁用。
...
- `webhook`：已弃用的旧版回退 webhook URL (http/https)，仅用于仍然具有 `notify: true` 的存储作业。
...
- `retryOn`：触发重试的错误类型 - `"rate_limit"`、`"overloaded"`、`"network"`、`"timeout"`、`"server_error"`。如果省略，则重试所有瞬时类型。

仅适用于一次性 cron 作业。周期性作业使用单独的错误处理机制。
...
- `cooldownMs`：针对同一作业重复警报之间的最小毫秒数（非负整数）。
- `includeSkipped`：将连续跳过的运行计入警报阈值（默认：`false`）。跳过的运行会被单独跟踪，并且不会影响执行错误的退避策略。
...
- `channel`：通告传递的渠道覆盖。`"last"` 重用上次已知的传递渠道。
- `to`：显式的通告目标或 webhook URL。webhook 模式下必填。
- `accountId`：用于传递的可选账户覆盖。
- 每个作业的 `delivery.failureDestination` 会覆盖此全局默认设置。
- 当未设置全局或每个作业的失败目标时，已经通过 `announce` 进行传递的作业在失败时将回退到该主要通告目标。
- `delivery.failureDestination` 仅当作业的主要 `delivery.mode` 为 `"webhook"` 时，才支持 `sessionTarget="isolated"` 作业。

请参阅 [Cron Jobs](/zh/automation/cron-jobs)。隔离的 cron 执行作为 [后台任务](/zh/automation/tasks) 进行跟踪。
...
- 兄弟键：在包含后合并（覆盖包含的值）。
- 嵌套包含：最多 10 层深。
- 路径：相对于包含文件解析，但必须位于顶级配置目录（`openclaw.json` 的 `dirname`）内。仅当解析结果仍在该边界内时，才允许使用绝对/`../` 形式。
- 由 OpenClaw 拥有且仅更改由单文件包含支持的一个顶级部分的写入操作，将直接写入到该包含文件中。例如，`plugins install` 会更新 `plugins.json5` 中的 `plugins: { $include: "./plugins.json5" }`，并保持 `openclaw.json` 不变。
- 对于由 OpenClaw 拥有的写入操作，根包含、包含数组以及带有同级覆盖项的包含为只读；这些写入操作将失败（fail closed），而不是扁平化配置。
- 错误：针对文件缺失、解析错误和循环包含提供明确的消息。
