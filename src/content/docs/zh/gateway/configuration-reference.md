...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目可以将 BlueBubbles 对话绑定到持久的 ACP 会话。在 `match.peer.id` 中使用 BlueBubbles 句柄或目标字符串（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义：[ACP Agents](/zh/tools/acp-agents#channel-specific-settings)。
...
- 带有 `type: "acp"` 的顶级 `bindings[]` 条目可以将 iMessage 对话绑定到持久的 ACP 会话。在 `match.peer.id` 中使用规范化句柄或显式聊天目标（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义：[ACP Agents](/zh/tools/acp-agents#channel-specific-settings)。
...
- 完整的 Teams 配置（凭据、webhook、私信/群组策略、每个团队/每个渠道的覆盖设置）记录在 [Microsoft Teams](/zh/channels/msteams) 中。
...
- 此页面是**配置键参考**，而非完整的命令目录。渠道/插件拥有的命令，如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、device-pair `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice`，均在其各自的渠道/插件页面以及 [Slash Commands](/zh/tools/slash-commands) 中进行了说明。
- 文本命令必须是带有前缀 `/` 的**独立**消息。
...
- `bash: true` 为主机 Shell 启用 `! <cmd>`。需要 `tools.elevated.enabled` 和发送者在 `tools.elevated.allowFrom.<channel>` 中。
- `config: true` 启用 `/config`（读取/写入 `openclaw.json`）。对于网关 `chat.send` 客户端，持久 `/config set|unset` 写入还需要 `operator.admin`；只读 `/config show` 对普通写入范围的 Operator 客户端仍然可用。
- `mcp: true` 为 OpenClaw 管理的位于 `mcp.servers` 下的 MCP 服务器配置启用 `/mcp`。
- `plugins: true` 启用 `/plugins` 以进行插件发现、安装以及启用/禁用控制。
...
- `ownerAllowFrom` 是仅限所有者命令/工具的明确所有者允许列表。它与 `allowFrom` 是分开的。
...
- 命令文档映射：
  ...
  - 记忆梦游：[Dreaming](/zh/concepts/dreaming)
...
- `provider`：已注册压缩提供商 插件的 ID。设置后，将调用提供商的 `summarize()`，而不是内置的 LLM 摘要。失败时回退到内置。设置提供商会强制执行 `mode: "safeguard"`。请参阅 [Compaction](/zh/concepts/compaction)。
...
- `notifyUser`：当设置为 `true` 时，在压缩开始和完成时向用户发送简短通知（例如，“Compacting context...” 和 “Compaction complete”）。默认情况下禁用，以保持压缩静默。
...
有关行为 + 分块详细信息，请参阅 [Streaming](/zh/concepts/streaming)。
...
对于中国区域端点：`baseUrl: "https://api.moonshot.cn/v1"` 或 `openclaw onboard --auth-choice moonshot-api-key-cn`。

原生 Moonshot 端点在共享的
`openai-completions` 传输上通告流式使用兼容性，并且 OpenClaw 键使用端点能力
而不是仅使用内置的提供商 ID。
...
请参阅 [Local Models](/zh/gateway/local-models)。TL;DR：在性能强大的硬件上通过 LM Studio Responses API 运行大型本地模型；保持托管模型合并以便进行回退。
...
- `plugins.entries.xai.config.xSearch`: xAI X 搜索（Grok 网页搜索）设置。
  - `enabled`: 启用 X 搜索提供商。
  - `model`: 用于搜索的 Grok（例如 `"grok-4-1-fast"`）。
- `plugins.entries.memory-core.config.dreaming`：记忆梦幻设置。有关阶段和阈值，请参阅 [Dreaming](/zh/concepts/dreaming)。
...
- `mode`: `local` (运行网关) 或 `remote` (连接到远程网关)。除非 `local`，否则 Gateway(网关) 拒绝启动。
...
- **Docker 注意事项**：默认的 `loopback` 绑定在容器内的 `127.0.0.1` 上监听。使用 Docker 桥接网络（`-p 18789:18789`）时，流量到达 `eth0`，因此网关无法访问。请使用 `--network host`，或设置 `bind: "lan"`（或带有 `customBindHost: "0.0.0.0"` 的 `bind: "custom"`）以在所有接口上监听。
- **认证**：默认需要。非环回绑定需要网关认证。实际上，这意味着共享令牌/密码或具有 `gateway.auth.mode: "trusted-proxy"` 的身份感知反向代理。新手引导向导默认生成令牌。
- 如果同时配置了 `gateway.auth.token` 和 `gateway.auth.password`（包括 SecretRefs），请将 `gateway.auth.mode` 显式设置为 `token` 或 `password`。当两者都配置但未设置模式时，启动和服务安装/修复流程将失败。
- `gateway.auth.mode: "none"`：显式无认证模式。仅用于受信任的本地环回设置；新手引导提示有意不提供此选项。
- `gateway.auth.mode: "trusted-proxy"`：将认证委托给身份感知反向代理，并信任来自 `gateway.trustedProxies` 的身份标头（请参阅 [Trusted Proxy Auth](/zh/gateway/trusted-proxy-auth)）。此模式期望**非环回**代理源；同主机环回反向代理不满足受信任代理认证。
- `gateway.auth.allowTailscale`：当 `true` 时，Tailscale Serve 身份标头可以满足控制 UI/WebSocket 认证（通过 `tailscale whois` 验证）。HTTP API 端点**不**使用该 Tailscale 标头认证；它们遵循网关的正常 HTTP 认证模式。此无令牌流程假定网关主机受信任。当 `tailscale.mode = "serve"` 时，默认为 `true`。
- `gateway.auth.rateLimit`：可选的认证失败限制器。适用于每个客户端 IP 和每个认证范围（共享密钥和设备令牌独立跟踪）。被阻止的尝试返回 `429` + `Retry-After`。
  - 在异步 Tailscale Serve 控制 UI 路径上，同一 `{scope, clientIp}` 的失败尝试在失败写入之前被序列化。因此，来自同一客户端的并发不良尝试可能会在第二个请求时触发限制器，而不是两者都作为普通不匹配通过。
  - `gateway.auth.rateLimit.exemptLoopback` 默认为 `true`；当您有意希望本地主机流量也受到速率限制时（用于测试设置或严格的代理部署），请设置 `false`。
- 浏览器源 WS 认证尝试始终受到限制，并禁用环回豁免（针对基于浏览器的本地主机暴力破解的深度防御）。
- 在环回上，这些浏览器源锁定是按标准化的 `Origin`
  值隔离的，因此来自一个本地主机源的重复失败不会自动
  锁定不同的源。
- `tailscale.mode`: `serve`（仅 tailnet，环回绑定）或 `funnel`（公共，需要认证）。
- `controlUi.allowedOrigins`：Gateway(网关) WebSocket 连接的显式浏览器源允许列表。当预期来自非环回源的浏览器客户端时需要。
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`：危险模式，为故意依赖 Host 标头源策略的部署启用 Host 标头源回退。
- `remote.transport`: `ssh`（默认）或 `direct` (ws/wss)。对于 `direct`，`remote.url` 必须是 `ws://` 或 `wss://`。
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`：客户端紧急覆盖，允许对受信任的专用网络 IP 进行纯文本 `ws://`；纯文本的默认设置仍为仅环回。
- `gateway.remote.token` / `.password` 是远程客户端凭据字段。它们本身不配置网关认证。
- `gateway.push.apns.relay.baseUrl`：官方/TestFlight iOS 构建版本在将中继支持的注册发布到网关后使用的外部 APNs 中继的基本 HTTPS URL。此 URL 必须与编译到 iOS 构建版本中的中继 URL 匹配。
- `gateway.push.apns.relay.timeoutMs`：网关到中继的发送超时（以毫秒为单位）。默认为 `10000`。
- 中继支持的注册被委托给特定的网关身份。配对的 iOS 应用获取 `gateway.identity.get`，将该身份包含在中继注册中，并向网关转发注册范围的发送授权。另一个网关无法重用该存储的注册。
- `OPENCLAW_APNS_RELAY_BASE_URL` / `OPENCLAW_APNS_RELAY_TIMEOUT_MS`：上述中继配置的临时环境覆盖。
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`：仅用于开发的环回 HTTP 中继 URL 的应急方案。生产中继 URL 应保持 HTTPS。
...
- `gateway.channelMaxRestartsPerHour`：每渠道/账户在滚动一小时内的最大健康监控重启次数。默认值：`10`。
- `channels.<provider>.healthMonitor.enabled`：每渠道选择退出健康监控重启，同时保持全局监控启用。
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`：多账户渠道的每账户覆盖。设置后，它优先于渠道级别的覆盖。
...
- `trustedProxies`：终止 TLS 或注入转发客户端标头的反向代理 IP。仅列出您控制的代理。环回条目对于同主机代理/本地检测设置（例如 Tailscale Serve 或本地反向代理）仍然有效，但它们**不**使环回请求符合 `gateway.auth.mode: "trusted-proxy"` 的条件。
- `allowRealIpFallback`：当 `true` 时，如果缺少 `X-Forwarded-For`，网关接受 `X-Real-IP`。默认 `false` 以实现失败关闭行为。
- `gateway.tools.deny`：被阻止用于 HTTP `POST /tools/invoke` 的额外工具名称（扩展默认拒绝列表）。
...
- `allowedSessionKeyPrefixes`：用于显式 `sessionKey` 值（请求 + 映射）的可选前缀允许列表，例如 `["hook:"]`。当任何映射或预设使用模板化的 `sessionKey` 时，它将成为必填项。
...
- 节点 WebViews 通常不发送身份验证标头；在节点配对并连接后，Gateway(网关) 会公布用于 canvas/A2UI 访问的节点范围能力 URL。
...
- `shellEnv`: 从登录 shell 配置文件中导入缺少的预期键名。
...
- `exec` 子环境默认情况下是极简的；请使用 `passEnv` 显式传递所需的变量。
- Secret 引用在激活时被解析为内存中的快照，然后请求路径仅读取该快照。
- 激活期间应用活动表面过滤：启用表面上的未解析引用将导致启动/重新加载失败，而非活动表面则会被跳过并输出诊断信息。
...
- 对于静态凭证模式，`auth-profiles.json` 支持值级引用（用于 `api_key` 的 `keyRef`，用于 `token` 的 `tokenRef`）。
- OAuth 模式配置文件 (`auth.profiles.<id>.mode = "oauth"`) 不支持由 SecretRef 支持的身份验证配置文件凭证。
- 静态运行时凭证来自内存解析的快照；遗留的静态 `auth.json` 条目在发现时会被清除。
...
- `billingBackoffHours`：当配置文件因真实的计费/余额不足错误而失败时，以小时为单位的基准退避时间（默认：`5`）。明确的计费文本即使出现在 `401`/`403` 响应中也可能会落在这里，但特定于提供商的文本匹配器仍限于拥有它们的提供商（例如 OpenRouter `Key limit exceeded`）。可重试的 HTTP `402` 使用量窗口或组织/工作区支出限额消息则会保留在 `rate_limit` 路径中。
- `billingBackoffHoursByProvider`：可选的按提供商覆盖的计费退避小时数。
- `billingMaxHours`：计费退避指数增长的上限，以小时为单位（默认：`24`）。
- `authPermanentBackoffMinutes`：针对高置信度 `auth_permanent` 故障的基础退避时间，以分钟为单位（默认：`10`）。
- `authPermanentMaxMinutes`：`auth_permanent` 退避增长的上限，以分钟为单位（默认：`60`）。
- `failureWindowHours`：用于退避计数器的滚动窗口，以小时为单位（默认：`24`）。
- `overloadedProfileRotations`：在切换到模型回退之前，针对过载错误允许的最大同一提供商身份配置轮换次数（默认：`1`）。提供商繁忙类型的错误（例如 `ModelNotReadyException`）属于此类。
- `overloadedBackoffMs`：在重试过载的提供商/配置轮换之前的固定延迟（默认：`0`）。
- `rateLimitedProfileRotations`：在切换到模型回退之前，针对速率限制错误允许的最大同一提供商身份配置轮换次数（默认：`1`）。该速率限制分组包括提供商特征的文本，例如 `Too many concurrent requests`、`ThrottlingException`、`concurrency limit reached`、`workers_ai ... quota limit exceeded` 和 `resource exhausted`。
...
- 设置 `logging.file` 以获取稳定的路径。
- 当 `--verbose` 时，`consoleLevel` 会提升至 `debug`。
- `maxFileBytes`：在停止写入之前的最大日志文件大小（字节）（正整数；默认：`524288000` = 500 MB）。对于生产环境部署，请使用外部日志轮转。
...
- `stuckSessionWarnMs`：发出卡住会话警告的年龄阈值（毫秒），此时会话仍处于处理状态。
...
- `cacheTrace.includeMessages` / `includePrompt` / `includeSystem`：控制缓存追踪输出中包含的内容（均默认：`true`）。
...
- `auto.stableJitterHours`：额外的稳定渠道（stable-渠道）发布分布窗口，单位为小时（默认：`12`；最大：`168`）。
- `auto.betaCheckIntervalHours`：Beta 渠道检查运行的频率，单位为小时（默认：`1`；最大：`24`）。
...
- `defaultAgent`：当生成实例未指定显式目标时使用的 ACP 回退目标代理 ID。
...
- `stream.deliveryMode`：`"live"` 逐渐流式传输；`"final_only"` 缓冲直到轮次终止事件。
...
- `stream.tagVisibility`：针对流式事件的标签名到布尔可见性覆盖的记录。
- `runtime.ttlMinutes`：ACP 会话工作程序的空闲 TTL，单位为分钟，之后符合清理条件。
...
请参阅 [Agent defaults](#agent-defaults) 下的 `agents.list` 身份标识字段。

---

## Bridge（旧版，已移除）

当前构建版本不再包含 TCP 网桥。节点通过 Gateway(网关) WebSocket 进行连接。`bridge.*` 键不再是配置架构的一部分（验证失败直到将其删除；`openclaw doctor --fix` 可以去除未知键）。
...
- `sessionRetention`：在从 `sessions.json` 中修剪之前保留已完成的隔离 cron 运行会话的时间。同时也控制已归档的已删除 cron 副本的清理。默认值：`24h`；设置为 `false` 以禁用。
...
- `backoffMs`：每次重试尝试的退避延迟数组，单位毫秒（默认：`[30000, 60000, 300000]`；1–10 项）。
...
- 当既未设置全局也未设置每个作业的失败目标时，已经通过 `announce` 交付的作业在失败时会回退到该主要通告目标。
- `delivery.failureDestination` 仅支持 `sessionTarget="isolated"` 作业，除非作业的主要 `delivery.mode` 是 `"webhook"`。

参见 [Cron Jobs](/zh/automation/cron-jobs)。隔离的 cron 执行被作为 [后台任务](/zh/automation/tasks) 跟踪。
...
- 同级键：在包含之后合并（覆盖包含的值）。
- 嵌套包含：最多 10 层深。
- 路径：相对于包含文件解析，但必须保持在顶级配置目录（`openclaw.json` 的 `dirname`）内。仅当它们仍在该边界内解析时，才允许绝对/`../` 形式。
- 错误：针对丢失文件、解析错误和循环包含提供明确消息。
