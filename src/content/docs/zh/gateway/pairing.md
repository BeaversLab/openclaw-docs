...
- 批准**始终**会生成一个新的令牌；永远不会从 `node.pair.request` 返回任何令牌。
...
<Warning>**重大变更：** 从 `2026.3.31` 开始，节点命令在节点配对获得批准之前处于禁用状态。仅靠设备配对已不再足以暴露声明的节点命令。</Warning>
...
<Warning>**重大变更：** 节点发起的运行现在保持在缩小的可信表面上。</Warning>
