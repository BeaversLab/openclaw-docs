...
- **传输**：Gateway(网关)（网关）WS 端点转发请求但不决定成员资格。（已移除旧版 TCP 网桥支持。）
...
当已配对的设备重新连接时，如果仅包含非敏感的元数据更改（例如，显示名称或客户端平台提示），OpenClaw 会将其视为 `metadata-upgrade` 并自动批准重新连接，而不会提示。范围升级（从只读到写入/管理员）和公钥更改**不符合**元数据升级自动批准的条件——它们仍保留为显式的重新批准请求。

## 二维码配对辅助工具

`/pair qr` 将配对负载渲染为结构化媒体，以便移动端和浏览器客户端可以直接扫描。设备删除现在还会清除同一设备 ID 的过时期待配对请求，因此 `nodes pending` 在撤销后不再显示孤立行。

## 位置与转发标头

当原始套接字和任何上游代理证据一致时，Gateway(网关) 配对才会将连接视为回环。如果请求到达回环但携带指向非本地源的 `X-Forwarded-For` / `X-Forwarded-Host` / `X-Forwarded-Proto` 头，则该转发头证据使回环本地性声明失效。配对路径随后需要显式批准，而不是静默地将请求视为同主机连接。有关操作员身份验证的等效规则，请参阅[Trusted Proxy Auth](/zh/gateway/trusted-proxy-auth)。
...
- 如果 Gateway(网关) 离线或配对已禁用，节点将无法配对。
