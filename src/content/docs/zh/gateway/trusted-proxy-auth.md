...
- **Forwarded-header 标头证据会覆盖环回本地性。** 如果请求到达环回接口，但携带指向非本地来源的 `X-Forwarded-For` / `X-Forwarded-Host` / `X-Forwarded-Proto` 标头，则该证据将使环回本地性声明失效。出于配对、受信任代理身份验证和控制 UI 设备身份筛选的目的，该请求将被视为远程请求。这可以防止同主机环回代理将转发的标头身份洗白为受信任代理身份验证。
...
| `gateway.auth.trustedProxy.requiredHeaders` | 否   | 使请求受信任必须存在的附加标头                         |
| `gateway.auth.trustedProxy.allowUsers`      | 否   | 用户身份白名单。留空表示允许所有经过身份验证的用户。   |
...
当您的反向代理处理 `https://control.example.com` 的 HTTPS 时，请在该域名的代理上设置
`Strict-Transport-Security`。

- 适合面向互联网的部署。
- 将证书和 HTTP 加固策略集中在一处。
- OpenClaw 可以位于代理后的回环 HTTP 上。
...
`strictTransportSecurity` 接受字符串标头值，或者接受 `false` 以显式禁用。

### 滚动指导

- 在验证流量时，首先使用较短的最长期限（例如 `max-age=300`）。
- 只有在确信度较高后，才增加到长期的值（例如 `max-age=31536000`）。
...
OpenClaw 会拒绝同时启用了 `gateway.auth.token`（或 `OPENCLAW_GATEWAY_TOKEN`）和 `trusted-proxy` 模式的这种歧义配置。混合令牌配置可能导致回环请求在错误的身份验证路径上静默验证。

如果在启动时看到 `mixed_trusted_proxy_token` 错误：

- 在使用 trusted-proxy 模式时移除共享令牌，或者
- 如果打算使用基于令牌的身份验证，请将 `gateway.auth.mode` 切换为 `"token"`。

回环 trusted-proxy 身份验证也会以失败关闭：同主机调用者必须通过受信任的代理提供配置的身份标头，而不是被静默验证。

## Operator scopes 标头

Trusted-proxy 认证是一种**承载身份**的 HTTP 模式，因此调用方可以选择使用 `x-openclaw-scopes` 声明操作员范围。
...
- 当请求头存在时，OpenClaw 会遵守声明的范围集。
- 当请求头存在但为空时，该请求声明**没有**操作员范围。
- 当请求头不存在时，普通的承载身份 HTTP API 会回退到标准操作员默认范围集。
- Gateway-auth **插件 HTTP 路由**默认范围更窄：当 `x-openclaw-scopes` 不存在时，其运行时范围会回退到 `operator.write`。
- 即使 trusted-proxy 认证成功，浏览器来源的 HTTP 请求仍必须通过 `gateway.controlUi.allowedOrigins`（或故意的 Host-header 回退模式）。

实用规则：

- 当您希望受信任代理请求比默认值更窄，或者当 gateway-auth 插件路由需要比写入作用域更强的权限时，请显式发送 `x-openclaw-scopes`。
...
- [ ] **无回环代理源**：对于源自回环地址的请求，trusted-proxy auth 将以失败（拒绝访问）作为默认处理
...
- [ ] **allowUsers is set**（推荐）：限制为已知用户，而不是允许任何通过身份验证的用户
- [ ] **No mixed token config**：不要同时设置 `gateway.auth.token` 和 `gateway.auth.mode: "trusted-proxy"`

## 安全审计

`openclaw security audit` 会将 trusted-proxy auth 标记为 **critical**（严重）级别的发现。这是有意为之——目的是提醒您正在将安全性委托给代理设置。
...
- 缺少 `userHeader` 配置
- `allowUsers` 为空（允许任何通过身份验证的用户）
- 在暴露的 Control UI 表面上存在通配符或缺失的浏览器源策略
...
- 您是否试图通过同主机环回反向代理使用受信任代理身份验证？

修复方法：

- 对于同主机环回代理设置，请使用令牌/密码身份验证，或者
...
- 您的代理关于这些特定 header 的配置
- 链路中的某处是否剥离了 headers

### "trusted_proxy_user_not_allowed"

用户已通过身份验证，但不在 `allowUsers` 中。请将用户添加进去或移除允许列表。

### "trusted_proxy_origin_not_allowed"

受信任代理身份验证成功，但浏览器的 `Origin` header 未通过控制 UI 源检查。

检查：

- `gateway.controlUi.allowedOrigins` 包含确切的浏览器源
- 除非您有意想要“全部允许”的行为，否则不要依赖通配符源
- 如果您有意使用 Host header 回退模式，`gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` 是否已明确设置
...
- [Security](/zh/gateway/security) — 完整安全指南
- [Configuration](/zh/gateway/configuration) — 配置参考
- [Remote Access](/zh/gateway/remote) — 其他远程访问模式
