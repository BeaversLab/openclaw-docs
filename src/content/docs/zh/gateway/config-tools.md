...
<AccordionGroup>
  <Accordion title="可见性范围">
    ...
    - 沙箱限制：当当前会话处于沙箱隔离状态且为 `agents.defaults.sandbox.sessionToolsVisibility="spawned"` 时，即使设置为 `tools.sessions.visibility="all"`，可见性也会被强制为 `tree`。
...
<AccordionGroup>
  <Accordion title="附件说明">
    ...
    - 文件被具体化到子工作空间的 `.openclaw/attachments/<uuid>/` 中，并带有 `.manifest.json`。
    - 附件内容会从转录持久化中自动编辑。
...
- 默认值：`false`，除非针对 OpenAI 或 OpenAI Codex GPT-5 系列的运行将 `agents.defaults.embeddedPi.executionContract`（或每个代理的覆盖设置）设置为 `"strict-agentic"`。将 `true` 设置为在该范围之外强制启用该工具，或将 `false` 设置为即使在严格的代理式 GPT-5 运行中也保持关闭状态。
- 启用后，系统提示词还会添加使用指南，以便模型仅将其用于实质性工作，并且最多保持一个步骤 `in_progress`。
...
- `allowAgents`：当请求代理未设置其自己的 `subagents.allowAgents` 时，`sessions_spawn` 的已配置目标代理 ID 的默认允许列表（`["*"]` = 任何已配置的目标；默认值：仅限同一代理）。已过时且代理配置已被删除的条目将被 `sessions_spawn` 拒绝，并从 `agents_list` 中省略；运行 `openclaw doctor --fix` 以将其清理。
- `runTimeoutSeconds`：当工具调用省略 `runTimeoutSeconds` 时，`sessions_spawn` 的默认超时（秒）。`0` 表示没有超时。
- `announceTimeoutMs`：网关 `agent` 公告传递尝试的每次调用超时（毫秒）。默认值：`120000`。瞬态重试可能会使总公告等待时间超过一个配置的超时时间。
...
配置自定义/本地提供商 `baseUrl`OpenClaw 也是针对模型 HTTP 请求的狭义网络信任决策：OpenClaw 允许该确切的 `scheme://host:port` 源通过受保护的获取路径，而无需添加单独的配置选项或信任其他私有源。
...
<AccordionGroup>
  <Accordion title="身份验证和合并优先级">
    ...
    - 匹配提供商 ID 的合并优先级：
      ...
      - 非空代理 `apiKey` 值仅在该提供商在当前配置/身份验证配置文件上下文中不受 SecretRef 管理时优先。
      - 受 SecretRef 管理的提供商 `apiKey` 值从源标记（环境变量引用为 `ENV_VAR_NAME`，文件/exec 引用为 `secretref-managed`）刷新，而不是持久化已解析的密钥。
      - 受 SecretRef 管理的提供商标头值从源标记（环境变量引用为 `secretref-env:ENV_VAR_NAME`，文件/exec 引用为 `secretref-managed`）刷新。
      ...
      - 标记持久性以源为准：标记是从活动源配置快照（解析前）写入的，而不是从已解析的运行时密钥值写入的。
...
<AccordionGroup>
  ...
  <Accordion title="Provider connection and auth">
    - `models.providers.*.api`: 请求适配器（`openai-completions`、`openai-responses`、`anthropic-messages`、`google-generative-ai` 等）。对于 MLX、vLLM、SGLang 等自托管 `/v1/chat/completions` 后端以及大多数与 OpenAI 兼容的本地服务器，请使用 `openai-completions`。具有 `baseUrl` 但没有 `api` 的自定义提供商默认为 `openai-completions`；仅在后端支持 `/v1/responses` 时才设置 `openai-responses`。
    - `models.providers.*.apiKey`: 提供商凭证（建议使用 SecretRef/env 替换）。
    ...
    - `models.providers.*.timeoutSeconds`: 可选的按提供商模型的 HTTP 请求超时时间（秒），包括连接、头部、正文和总请求中止处理。
    - `models.providers.*.injectNumCtxForOpenAICompat`: 对于 Ollama + `openai-completions`，将 `options.num_ctx` 注入到请求中（默认：`true`）。
    - `models.providers.*.authHeader`: 需要时，强制在 `Authorization` 头中传输凭证。
  ...
  <Accordion title="请求传输覆盖">
    ...
    - `request.headers`: 额外的请求头（与提供商默认值合并）。值支持 SecretRef。
    - `request.auth`: 认证策略覆盖。模式：`"provider-default"`（使用提供商内置认证），`"authorization-bearer"`（带 `token`），`"header"`（带 `headerName`、`value`，可选 `prefix`）。
    - `request.proxy`: HTTP 代理覆盖。模式：`"env-proxy"`（使用 `HTTP_PROXY`/`HTTPS_PROXY` 环境变量），`"explicit-proxy"`（带 `url`）。这两种模式均接受可选的 `tls` 子对象。
    - `request.tls`: 直连的 TLS 覆盖。字段：`ca`、`cert`、`key`、`passphrase`（均接受 SecretRef）、`serverName`、`insecureSkipVerify`。
    - `request.allowPrivateNetwork`: 当设为 `true` 时，允许模型提供商的 HTTP 请求通过提供商 HTTP 获取守卫访问私有、CGNAT 或类似网段。自定义/本地提供商基础 URL 已信任确切配置的源，但元数据/链路本地源除外，后者在未明确选择加入的情况下仍将被阻止。将其设置为 `false` 可选择退出确切源信任。WebSocket 对请求头/TLS 使用相同的 `request`，但不使用该获取 SSRF 守门。默认值为 `false`。

  </Accordion>
  <Accordion title="模型目录条目">
    - `models.providers.*.models`：显式提供商模型目录条目。
    - `models.providers.*.models.*.input`：模型输入模态。对于仅文本模型使用 `["text"]`，对于原生图像/视觉模型使用 `["text", "image"]`。只有当所选模型被标记为具备图像能力时，图像附件才会被注入到对话轮次中。
    - `models.providers.*.models.*.contextWindow`：原生模型上下文窗口元数据。这会覆盖该模型提供商级别的 `contextWindow`。
    - `models.providers.*.models.*.contextTokens`：可选运行时上下文上限。这会覆盖提供商级别的 `contextTokens`；当你想要比模型原生 `contextWindow` 更小的有效上下文预算时使用它；当两个值不同时，`openclaw models list` 会显示这两个值。
    - `models.providers.*.models.*.compat.supportsDeveloperRole`：可选兼容性提示。对于具有非空非原生 `baseUrl`（主机不是 `api.openai.com`）的 `api: "openai-completions"`，OpenClaw 会在运行时强制将其设为 `false`。为空/省略的 `baseUrl` 将保留默认 OpenAI 行为。
    - `models.providers.*.models.*.compat.requiresStringContent`：可选兼容性提示，用于仅限字符串的 OpenAI 兼容聊天端点。当 `true` 时，OpenClaw 会在发送请求之前将纯文本 `messages[].content` 数组展平为纯字符串。
    - `models.providers.*.models.*.compat.strictMessageKeys`：可选兼容性提示，用于严格 OpenAI 兼容聊天端点。当 `true` 时，OpenClaw 会在发送请求之前将传出的 Chat Completions 消息对象剥离为 `role` 和 `content`。
    - `models.providers.*.models.*.compat.thinkingFormat`：可选思考负载提示。对于 Together 风格的 `reasoning.enabled` 使用 `"together"`，对于顶级 `enable_thinking` 使用 `"qwen"`，或者对于支持请求级别 chat-template kwargs 的 Qwen 系列 OpenAI 兼容服务器（例如 vLLM）上的 `chat_template_kwargs.enable_thinking` 使用 `"qwen-chat-template"`。

  </Accordion>
  <Accordion title="Amazon BedrockAmazon Bedrock 发现">
    - `plugins.entries.amazon-bedrock.config.discovery`: Bedrock 自动发现设置的根。
...
交互式自定义提供商新手引导会为常见的视觉模型 ID（例如 GPT-4o、Claude、Gemini、Qwen-VL、LLaVA、Pixtral、InternVL、Mllama、MiniCPM-V 和 GLM-4V）推断图像输入，并跳过已知纯文本系列的额外问题。未知的模型 ID 仍会提示图像支持。非交互式新手引导使用相同的推断；传递 `--custom-image-input` 以强制支持图像的元数据，或传递 `--custom-text-input` 以强制仅文本的元数据。

### 提供商示例

<AccordionGroup>
  <Accordion title="Cerebras (GLM 4.7 / GPT OSS)">
    捆绑的 `cerebras` 提供商插件可以通过 `openclaw onboard --auth-choice cerebras-api-key` 进行配置。仅在覆盖默认值时使用显式提供商配置。
  ...
  <Accordion title="Local models (LM Studio)">
    参见 [Local Models](/zh/gateway/local-models)。TL;DR：通过强大的硬件使用 LM Studio Responses API 运行大型本地模型；保持托管模型合并以作为回退。
  </Accordion>
  <Accordion title="MiniMaxMiniMax M2.7 (direct)">
    ...
    设置 `MINIMAX_API_KEY`。快捷方式：`openclaw onboard --auth-choice minimax-global-api` 或 `openclaw onboard --auth-choice minimax-cn-api`AnthropicOpenClawMiniMax。模型目录默认仅限于 M2.7。在 Anthropic 兼容的流式传输路径上，除非您显式自行设置 `thinking`，否则 OpenClaw 默认禁用 MiniMax 思考。`/fast on` 或 `params.fastMode: true` 会将 `MiniMax-M2.7` 重写为 `MiniMax-M2.7-highspeed`。
  ...
  <Accordion title="MoonshotMoonshot AI (Kimi)">
    ...
    对于中国区域端点：`baseUrl: "https://api.moonshot.cn/v1"` 或 `openclaw onboard --auth-choice moonshot-api-key-cn`Moonshot。

    原生 Moonshot 端点在共享的 `openai-completions`OpenClaw 传输上通告了流式使用兼容性，并且 OpenClaw 密钥侧重于端点功能，而不仅仅是内置的提供商 id。
