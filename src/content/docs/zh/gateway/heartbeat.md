...
> **Heartbeat vs Cron?** 有关何时使用哪个的指导，请参阅 [自动化与任务](/zh/automation)。

心跳在主会话中运行**周期性的代理轮次**，以便模型可以
呈现需要注意的事项，而不会通过垃圾消息打扰您。

Heartbeat 是一个定期的主会话轮次——它并**不**创建 [后台任务](/zh/automation/tasks) 记录。
...
- **人工检查**：“在白天偶尔检查你的人类”会偶尔触发轻量的“你需要什么吗？”消息，但通过使用你配置的本地时区来避免夜间打扰（请参阅 [/concepts/timezone](/zh/concepts/timezone)）。
...
- 分离的[后台任务](/zh/automation/tasks)可以加入系统事件并在主会话应快速注意某些事情时唤醒心跳。该唤醒不会使心跳运行后台任务。
...
- [自动化与任务](/zh/automation) — 一目了然的所有自动化机制
