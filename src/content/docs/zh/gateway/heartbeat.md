...
<ParamField path="includeReasoning" type="boolean" default="false">
  启用后，当可用时也会传递单独的 `Reasoning:` 消息（形状与 `/reasoning on` 相同）。
</ParamField>
<ParamField path="lightContext" type="boolean" default="false">
  为 true 时，Heartbeat 运行使用轻量级引导上下文，并仅保留工作区引导文件中的 `HEARTBEAT.md`。
</ParamField>
<ParamField path="isolatedSession" type="boolean" default="false">
  为 true 时，每次 Heartbeat 在没有先前对话历史的新会话中运行。使用与 cron `sessionTarget: "isolated"` 相同的隔离模式。大幅降低每次 Heartbeat 的 token 成本。结合 `lightContext: true` 使用以实现最大程度的节省。传递路由仍使用主会话上下文。
</ParamField>
<ParamField path="skipWhenBusy" type="boolean" default="false">
  为 true 时，Heartbeat 运行会在该代理的额外繁忙通道上延迟：其自身的会话键子代理或嵌套命令工作。即使没有此标志，Cron 通道也总是会延迟 Heartbeat，因此本地模型主机不会同时运行 cron 和 heartbeat 提示。
</ParamField>
