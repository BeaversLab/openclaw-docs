...
<Note>**Heartbeat 与 cron？** 请参阅[自动化](/zh/automation)以获取有关何时使用每种功能的指导。</Note>

Heartbeat 在主会话中运行**定期的 agent 轮次**，以便模型可以引起您对需要注意的事项的注意，而不会对您造成信息轰炸。

Heartbeat 是一个计划的主动话话轮 — 它**不会**创建[后台任务](/zh/automation/tasks)记录。任务记录用于分离的工作（ACP 运行、子代理、独立的 cron 作业）。
...
- 当 cron 工作处于活动状态或排队中时，Heartbeat 会自动延迟。设置 `heartbeat.skipWhenBusy: true` 也可以使该代理在其自身的会话键子代理或嵌套命令通道上延迟；兄弟代理不会仅仅因为另一个代理有子代理工作正在进行而暂停。
...
- **人工检查**：“白天偶尔检查你的使用者”会提示偶尔发送一条轻量级的“你需要什么？”消息，但通过使用你配置的本地时区来避免夜间骚扰（请参阅[时区](/zh/concepts/timezone)）。

Heartbeat 可以对已完成的[后台任务](/zh/automation/tasks)做出反应，但 Heartbeat 运行本身不会创建任务记录。
...
<ParamField path="includeReasoning" type="boolean" default="false">
  启用后，如果有单独的 `Thinking` 消息可用，也将其投递（形状与 `/reasoning on` 相同）。
</ParamField>
<ParamField path="lightContext" type="boolean" default="false">
  为 true 时，心跳运行使用轻量级引导上下文，并仅保留来自工作区引导文件的 `HEARTBEAT.md`。
</ParamField>
<ParamField path="isolatedSession" type="boolean" default="false">
  为 true 时，每次心跳在新的会话中运行，没有先前的对话历史。使用与 cron `sessionTarget: "isolated"` 相同的隔离模式。大幅降低每次心跳的 token 成本。结合 `lightContext: true` 可实现最大程度的节省。投递路由仍使用主会话上下文。
</ParamField>
<ParamField path="skipWhenBusy" type="boolean" default="false">
  为 true 时，心跳运行会在该代理的额外繁忙通道上让步：其自身的带会话密钥的子代理或嵌套命令工作。Cron 通道总是让步心跳，即使没有此标志，因此本地模型主机不会同时运行 cron 和心跳提示。
</ParamField>
...
<AccordionGroup>
  <Accordion title="Session and target routing">
    - Heartbeats run in the agent's main 会话 by default (`agent:<id>:<mainKey>`), or `global` when `session.scope = "global"`. Set `session` to override to a specific 渠道 会话 (Discord/WhatsApp/etc.).
    - `session` only affects the run context; delivery is controlled by `target` and `to`.
    - To deliver to a specific 渠道/recipient, set `target` + `to`. With `target: "last"`, delivery uses the last external 渠道 for that 会话.
    - Heartbeat deliveries allow direct/私信 targets by default. Set `directPolicy: "block"` to suppress direct-target sends while still running the heartbeat turn.
    - If the main queue, target 会话 lane, cron lane, or an active cron job is busy, the heartbeat is skipped and retried later.
    - If `skipWhenBusy: true`, this agent's 会话-keyed subagent and nested lanes also defer heartbeat runs. Other agents' busy lanes do not defer this agent.
  ...
  <Accordion title="Session lifecycle and audit">
    - 仅心跳的回复**不会**保持会话活跃。心跳元数据可能会更新会话行，但空闲过期使用的是来自最后一个真实用户/渠道消息的 `lastInteractionAt`，而每日过期使用的是 `sessionStartedAt`。
    - 控制UI和WebChat历史记录会隐藏心跳提示和仅OK的确认信息。底层的会话记录仍然可以包含这些轮次以供审计/重放。
    - 分离的[后台任务](/zh/automation/tasks)可以排队一个系统事件并在主会话需要快速注意某些事情时唤醒心跳。该唤醒不会导致心跳运行后台任务。
...
如果工作区中存在 `HEARTBEAT.md` 文件，默认提示会告诉代理去读取它。可以将其视为您的“心跳检查清单”：小巧、稳定，并且每30分钟考虑一次是安全的。
...
`HEARTBEAT.md` 还支持一个小型的结构化 `tasks:` 块，用于在心跳内部进行基于间隔的检查。
...
<Warning>不要将机密信息（API 密钥、电话号码、私人令牌）放入 API`HEARTBEAT.md` 中 — 它会成为提示上下文的一部分。</Warning>

## 手动唤醒 (按需)

您可以排队一个系统事件并通过以下方式触发即时心跳：

```bash
openclaw system event --text "Check for urgent follow-ups" --mode now
```

如果配置了多个代理的 `heartbeat`，手动唤醒会立即运行每个代理的心跳。

使用 `--mode next-heartbeat` 等待下一次计划周期。

## 推理交付 (可选)

默认情况下，心跳仅传递最终的 "answer" 有效载荷。
...
如果心跳之前将现有会话留在了较小的本地模型上，例如具有 32k 窗口的 Ollama 模型，并且下一个主会话轮次报告了上下文溢出，则会将会话运行时模型重置回配置的主模型。当最后的运行时模型与配置的 `heartbeat.model` 匹配时，OpenClaw 的重置消息会指明这一点。
