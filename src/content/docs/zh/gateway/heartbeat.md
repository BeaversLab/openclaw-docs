...
> **Heartbeat vs Cron?** 有关何时使用哪个的指导，请参阅 [自动化与任务](/en/automation)。

心跳在主会话中运行**周期性的代理轮次**，以便模型可以
呈现需要注意的事项，而不会通过垃圾消息打扰您。

Heartbeat 是一个定期的主会话轮次——它并**不**创建 [后台任务](/en/automation/tasks) 记录。
任务记录用于分离式工作（ACP 运行、子代理、独立的 cron 作业）。
...
- **人工检查**：“在白天偶尔检查你的人类”会偶尔触发轻量的“你需要什么吗？”消息，但通过使用你配置的本地时区来避免夜间打扰（请参阅 [/concepts/timezone](/en/concepts/timezone)）。
...
- `session`：心跳运行的可选会话密钥。
  - `main`（默认值）：代理主会话。
  - 显式会话密钥（从 `openclaw sessions --json` 或 [sessions CLI](/en/cli/sessions) 复制）。
  - 会话密钥格式：请参阅 [会话](/en/concepts/session) 和 [组](/en/channels/groups)。
- `target`：
  - `last`：发送到最后使用的外部渠道。
...
- 仅心跳回复**不会**保持会话活跃；会恢复最后的 `updatedAt`，以便空闲超时正常工作。
- 分离的[后台任务](/en/automation/tasks)可以加入系统事件并在主会话应快速注意某些事情时唤醒心跳。该唤醒不会使心跳运行后台任务。

## 可见性控制

默认情况下，传递警报内容时会抑制 `HEARTBEAT_OK` 确认。您可以按渠道或按帐户进行调整：
...
如果工作区中存在 `HEARTBEAT.md` 文件，默认提示会告知代理读取它。可以将其视为您的“心跳检查清单”：内容精简、稳定，且每 30 分钟包含一次是安全的。

在正常运行中，只有为默认代理启用心跳指导时，才会注入 `HEARTBEAT.md`。使用 `0m` 禁用心跳节奏或设置 `includeSystemPromptSection: false` 会将其从正常引导上下文中省略。

如果 `HEARTBEAT.md` 存在但实际上为空（只有空行和像 `# Heading` 这样的 markdown 标题），OpenClaw 将跳过心跳运行以节省 API 调用。该跳过行为被报告为 `reason=empty-heartbeat-file`。如果文件丢失，心跳仍会运行，模型会决定做什么。
...
`HEARTBEAT.md` 还支持一个结构化的小型 `tasks:` 代码块，用于心跳内部基于间隔的检查。
...
- `HEARTBEAT.md` 中的非任务内容将被保留，并作为额外上下文附加在到期任务列表之后。
- 任务上次运行的时间戳存储在会话状态（`heartbeatTaskState`）中，因此间隔在正常重启后仍然有效。
- 只有在心跳运行完成其正常回复路径后，任务时间戳才会推进。被跳过的 `empty-heartbeat-file` / `no-tasks-due` 运行不会将任务标记为已完成。

当您希望一个心跳文件包含多个定期检查，而不想为每次运行都支付所有检查的费用时，任务模式非常有用。

### 代理可以更新 HEARTBEAT.md 吗？

可以——如果您要求它这样做。

`HEARTBEAT.md` 只是代理工作区中的一个普通文件，因此您可以（在普通聊天中）告诉代理类似这样的话：

- “更新 `HEARTBEAT.md` 以添加每日日历检查。”
- “重写 `HEARTBEAT.md` 使其更简短，并专注于收件箱后续跟进。”

如果您希望这种情况主动发生，您还可以在心跳提示中包含一行明确的内容，例如：“如果清单过时，请用更好的清单更新 HEARTBEAT.md。”

安全提示：请勿将机密信息（API 密钥、电话号码、私人令牌）放入 `HEARTBEAT.md` —— 它将成为提示上下文的一部分。

## 手动唤醒（按需）

您可以加入系统事件并使用以下命令立即触发心跳：
...
启用后，心跳还将传递一条前缀为 `Reasoning:` 的单独消息（形状与 `/reasoning on` 相同）。当代理正在管理多个会话/代码库并且您希望查看它决定联系您的原因时，这很有用 —— 但这也可能会泄露比您想要的更多的内部细节。建议在群聊中将其保持关闭。
...
- [自动化与任务](/en/automation) — 一目了然的所有自动化机制
