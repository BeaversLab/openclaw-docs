...
- 网关在启动时立即使用独占 TCP 监听器绑定 WebSocket 监听器（默认为 `ws://127.0.0.1:18789`）。
- 如果绑定失败并返回 `EADDRINUSE`，启动时会抛出 `GatewayLockError("another gateway instance is already listening on ws://127.0.0.1:<port>")`。
...
- 如果端口被*另一个*进程占用，错误信息是一样的；请释放端口或使用 `openclaw gateway --port <port>` 选择另一个端口。
- macOS 应用在生成网关之前仍会维护自己的轻量级 PID 守护；运行时锁由 WebSocket 绑定强制执行。
