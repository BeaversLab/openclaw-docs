...
OpenClaw 的 Gateway(网关) 可以提供一个与 OpenResponses 兼容的 `POST /v1/responses` 端点。
...
- 使用匹配的 Gateway(网关) HTTP 身份验证路径：
  ...
  - trusted-proxy 身份验证（`gateway.auth.mode="trusted-proxy"`）：来自配置的非环回可信代理源的身份感知代理头
  - private-ingress open 身份验证（`gateway.auth.mode="none"`）：无身份验证头
- 将该端点视为对网关实例的完全操作员访问权限
- 对于 shared-secret 身份验证模式（`token` 和 `password`），忽略较窄的 bearer 声明的 `x-openclaw-scopes` 值，并恢复正常的完全操作员默认值
- 对于承载可信身份的 HTTP 模式（例如 trusted proxy 身份验证或 `gateway.auth.mode="none"`），当存在 `x-openclaw-scopes` 时予以遵循，否则回退到正常的操作员默认作用域集
...
- 使用 `x-openclaw-session-key` 进行显式会话 路由
...
- 受信任的承载身份的 HTTP 模式（例如受信任的代理身份验证，或专用入口上的 `gateway.auth.mode="none"`）
  ...
  - 仅当调用者明确缩小范围并省略 `operator.admin` 时，才会丢失所有者语义
...
有关代理目标模型、`openclaw/default`、嵌入直通和后端模型覆盖如何组合在一起的权威说明，请参阅 [OpenAI Chat Completions](/zh/gateway/openai-http-api#agent-first-model-contract) 和 [Model list and agent routing](/zh/gateway/openai-http-api#model-list-and-agent-routing)。
...
- `previous_response_id`：当请求保持在相同的代理/用户/请求会话范围内时，OpenClaw 会重用之前的响应会话。
...
- 最近的 `user` 或 `function_call_output` item 将成为“当前消息”。
...
如果代理决定调用工具，响应将返回一个 `function_call` 输出 item。
然后您发送一个带有 `function_call_output` 的后续请求以继续该回合。
...
- 此文件输入路径有意省略了长的 `SECURITY NOTICE:` 标语以
  保留提示词预算；边界标记和元数据仍然保留。
- PDF 首先被解析为文本。如果发现的文本很少，前几页将被
  光栅化为图像并传递给模型，并且注入的文件块使用
  占位符 `[PDF content rendered to images]`。

PDF 解析由捆绑的 `document-extract` 插件提供，该插件使用
对 Node 友好的 `pdfjs-dist` 旧版构建（无 worker）。现代的 PDF.js 构建
期望浏览器 worker/DOM 全局变量，因此未在 Gateway(网关) 中使用。
...
- HEIC/HEIF `input_image` 源会被接受并在发送给提供商之前转换为 JPEG。

安全提示：

- URL 允许列表在获取之前和重定向跳转时强制执行。
- 将主机名列入允许列表不会绕过私有/内部 IP 阻止。
- 对于暴露于互联网的网关，除了应用程序级别的防护外，还要应用网络出口控制。
  参见[安全性](/zh/gateway/security)。
...
OpenClaw 会在这些计数器到达下游状态/会话表面之前规范化常见的 OpenAI 风格别名，包括 `input_tokens` / `output_tokens`
