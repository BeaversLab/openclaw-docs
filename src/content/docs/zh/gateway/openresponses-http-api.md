...
操作行为与 [OpenAI Chat Completions](OpenAI/en/gateway/openai-http-api) 一致：
...
- 使用匹配的 Gateway(网关) HTTP 身份验证路径：
  ...
  - private-ingress open auth (`gateway.auth.mode="none"`)：无 auth 标头
- 将该端点视为对网关实例的完全操作员访问权限
- 对于共享密钥认证模式（`token` 和 `password`），忽略较窄的 bearer 声明的 `x-openclaw-scopes` 值，并恢复正常完整的操作员默认值
- 对于承载可信身份的 HTTP 模式（例如 trusted proxy auth 或 `gateway.auth.mode="none"`），当存在 `x-openclaw-scopes` 时予以遵守，否则回退到正常的操作员默认范围集
...
- 使用 `x-openclaw-session-key` 进行显式的会话路由
...
- 承载可信身份的 HTTP 模式（例如 trusted proxy auth，或在专用入口上的 `gateway.auth.mode="none"`）
  - 当标头存在时遵守 `x-openclaw-scopes`
  - 当标头不存在时，回退到正常的操作员默认范围集
  - 仅当调用方显式缩小范围并省略 `operator.admin` 时，才会丢失所有者语义
...
关于代理目标模型、`openclaw/default`OpenAI、嵌入直通以及后端模型覆盖如何组合在一起的权威说明，请参阅 [OpenAI Chat Completions](/zh/gateway/openai-http-api#agent-first-model-contract) 和 [模型列表和代理路由](/zh/gateway/openai-http-api#model-list-and-agent-routing)。

## 会话行为

默认情况下，该端点是**每个请求无状态的**（每次调用都会生成一个新的会话密钥）。

如果请求包含 OpenResponses OpenResponses`user`Gateway(网关) 字符串，Gateway 会从中派生一个稳定的会话密钥，以便重复调用可以共享代理会话。
...
- `previous_response_id`OpenClaw：当请求保持在同一代理/用户/请求的会话范围内时，OpenClaw 会重用先前的响应会话。
...
- `system` 和 `developer` 会被附加到系统提示词。
- 最近的 `user` 或 `function_call_output` 项将成为“当前消息”。
- 之前的用户/助手消息将作为上下文历史被包含在内。
...
为了架构兼容性而接受，但在构建提示词时会被忽略。

## 工具 (客户端函数工具)

使用 `tools: [{ type: "function", function: { name, description?, parameters? } }]` 提供工具。

如果 Agent 决定调用工具，响应将返回一个 `function_call` 输出项。然后，你需要发送一个带有 `function_call_output` 的后续请求以继续对话。
...
- 此文件输入路径有意省略了冗长的 `SECURITY NOTICE:` 横幅，以
  保留提示词预算；边界标记和元数据仍然保留。
- 首先解析 PDF 中的文本。如果发现文本很少，首页将
  被光栅化为图像并传递给模型，注入的文件块使用
  占位符 `[PDF content rendered to images]`。

PDF 解析由捆绑的 `document-extract` 插件提供，该插件使用
对 Node 友好的 `pdfjs-dist`Gateway(网关) 旧版构建（无 worker）。现代 PDF.js 构建
需要浏览器 worker/DOM 全局变量，因此不在 Gateway(网关) 中使用。
...
- 支持针对每种输入类型（`files.urlAllowlist`、`images.urlAllowlist`）的可选主机名允许列表。
...
- 将主机名列入允许列表不会绕过私有/内部 IP 阻止。
- 对于面向互联网的网关，除了应用级保护措施外，还应应用网络出口控制。
...
OpenClaw 会在这些计数器到达下游状态/会话表面之前，规范化常见的 OpenAI 风格别名，包括 `input_tokens` / `output_tokens`
