...
操作行为与 [OpenAI 聊天补全](OpenAI/en/gateway/openai-http-api) 相匹配：
...
有关面向代理的模型、`openclaw/default`OpenAI、嵌入直通和后端模型覆盖如何协同工作的权威说明，请参阅 [OpenAI 聊天补全](/zh/gateway/openai-http-api#agent-first-model-contract) 和 [模型列表和代理路由](/zh/gateway/openai-http-api#model-list-and-agent-routing)。
...
PDF 解析由捆绑的 `document-extract` 插件提供，它使用
Node 友好的 `pdfjs-dist` 旧版构建（无 worker）。现代 PDF.js 构建
...
- 对于暴露在互联网上的网关，除了应用程序级别的防护外，还应应用网络出口控制。
  参见 [安全](/zh/gateway/security)。
...
OpenClaw 会在这些计数器到达下游状态/会话表面之前标准化常见的 OpenAI 风格别名，包括 `input_tokens` / `output_tokens`
