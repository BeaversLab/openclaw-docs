...
为每个渠道或每个房间/对话设置 `contextVisibility`。有关设置详细信息，请参阅[群聊](/en/channels/groups#context-visibility-and-allowlists)。
...
- 详细的部署指南请参阅[受信任代理认证](/en/gateway/trusted-proxy-auth#tls-termination-and-hsts)。
...
渠道允许列表/配对以及 `commands.useAccessGroups`（请参阅[配置](/en/gateway/configuration)
...
详情及磁盘文件：[配对](/en/channels/pairing)
...
- `gateway.auth.mode: "trusted-proxy"`：信任一个支持身份识别的反向代理来对用户进行身份验证并通过请求头传递身份（请参阅 [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth)）。
...
"token"` or `"password"`）或 [可信代理认证](/en/gateway/trusted-proxy-auth)
代替。
...
- **工具沙箱** (`agents.defaults.sandbox`，主机 Gateway + Docker-隔离工具)：[沙箱隔离](/en/gateway/sandboxing)
...
重要提示：`tools.elevated` 是全局基准逃生舱口，用于在沙箱之外运行 exec。有效主机默认为 `gateway`，或者当 exec 目标配置为 `node` 时为 `node`。请严格控制 `tools.elevated.allowFrom`，不要为陌生人启用它。您可以通过 `agents.list[].tools.elevated` 进一步限制每个代理的提权。请参阅[提权模式](/en/tools/elevated)。
...
有关完整信息和优先级规则，请参阅[多智能体沙箱与工具](/en/tools/multi-agent-sandbox-tools)。
