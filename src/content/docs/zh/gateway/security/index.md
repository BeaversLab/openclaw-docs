...
按渠道或按房间/对话设置 `contextVisibility`。有关设置详细信息，请参阅 [群聊](/en/channels/groups#context-visibility-and-allowlists)。
...
- 详细的部署指南请参阅 [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth#tls-termination-and-hsts)。
...
斜杠命令和指令仅被 **授权发送者** 尊重。授权源自渠道允许列表/配对加上 `commands.useAccessGroups`（请参阅 [Configuration](/en/gateway/configuration) 和 [Slash commands](/en/tools/slash-commands)）。如果渠道允许列表为空或包含 `"*"`，则该渠道的命令实际上处于开放状态。
...
详细信息 + 磁盘文件：[配对](/en/channels/pairing)
...
`/reasoning` 和 `/verbose` 可能会暴露本不应出现在公开渠道中的内部推理或工具输出。在群组设置中，请将它们视为**仅限调试**，除非明确需要，否则请保持关闭状态。
...
- 非环回绑定（`"lan"`、`"tailnet"`、`"custom"`）会扩大攻击面。请仅在配置了网关身份验证（共享令牌/密码或正确配置的非环回受信任代理）和真正的防火墙时使用它们。
...
如果您在 VPS 上使用 Docker 运行 OpenClaw，请记住发布的容器端口
...
在最小模式下，Gateway(网关) 仍广播足够的信息用于设备发现（`role`, `gatewayPort`, `transport`），但省略 `cliPath` 和 `sshPort`。需要 CLI 路径信息的应用程序可以通过经过身份验证的 WebSocket 连接获取它。
...
注意：`gateway.remote.token` / `.password` 是客户端凭证来源。它们自身**不**保护本地 WS 访问。
本地调用路径仅当 `gateway.auth.*` 未设置时，才能将 `gateway.remote.*` 作为回退。
如果 `gateway.auth.token` / `gateway.auth.password` 通过 SecretRef 显式配置但未解析，解析将失败关闭（不存在远程回退屏蔽）。
...
- `gateway.auth.mode: "trusted-proxy"`：信任一个具有身份感知能力的反向代理来验证用户并通过标头传递身份（请参阅[受信任的代理认证](/en/gateway/trusted-proxy-auth)）。
...
解析 `x-forwarded-for` 地址并将其与标头匹配来验证身份。这仅针对命中环回
...
**不**使用 Tailscale 身份标头身份验证。它们仍遵循 Gateway
配置的 HTTP 身份验证模式。
...
- 在 OpenAI 兼容的 HTTP 表面上，共享机密持有者身份验证会恢复完整的默认操作员范围 (`operator.admin`、`operator.approvals`、`operator.pairing`、`operator.read`、`operator.talk.secrets`、`operator.write`) 和代理轮次的所有者语义；更窄的 `x-openclaw-scopes` 值不会减少该共享机密路径。
- HTTP 上的每请求范围语义仅在请求来自具有身份的模式（如受信任的代理身份验证或专用入口上的 `gateway.auth.mode="none"`）时才适用。
- 在那些具有身份的模式中，省略 `x-openclaw-scopes` 将回退到正常的操作员默认范围集；当您想要更窄的范围集时，请显式发送标头。
- `/tools/invoke` 遵循相同的共享密钥规则：令牌/密码持有者认证在那里也被视为完全的操作员访问权限，而承载身份的模式仍然遵循声明的范围。
- 请勿与不受信任的调用者共享这些凭据；建议每个信任边界使用单独的 Gateway（网关）。

**信任假设：** 无令牌的 Serve 认证假设 Gateway（网关）主机是受信任的。
不要将其视为针对本地敌对进程的保护。如果不受信任的
本地代码可能在 Gateway（网关）主机上运行，请禁用 `gateway.auth.allowTailscale`
并要求使用 `gateway.auth.mode: "token"` 或
`"password"` 进行显式的共享密钥认证。

**安全规则：** 不要从您自己的反向代理转发这些标头。如果您
在 Gateway（网关）前终止 TLS 或进行代理，请禁用
`gateway.auth.allowTailscale` 并使用共享密钥认证（`gateway.auth.mode:
"token"` or `"password"`）或 [受信任的代理认证](/en/gateway/trusted-proxy-auth)
代替。

受信任的代理：

- 如果您在 Gateway（网关）之前终止 TLS，请将 `gateway.trustedProxies` 设置为您的代理 IP。
- OpenClaw 将信任来自这些 IP 的 `x-forwarded-for`（或 `x-real-ip`）来确定用于本地配对检查以及 HTTP 认证/本地检查的客户端 IP。
...
并让 Gateway（网关）代理浏览器操作（请参阅 [浏览器工具](/en/tools/browser)）。
请将节点配对视为管理员访问权限。
...
- `secrets.json`（可选）：由 `file` SecretRef 提供程序使用的文件支持的机密负载（`secrets.providers`）。
- `agents/<agentId>/agent/auth.json`：旧版兼容文件。当发现静态 `api_key` 条目时，会将其清除。
...
- 共享诊断信息时，首选 `openclaw status --all`（可粘贴，已编辑机密），而不是原始日志。
...
- `agents.defaults.sandbox.workspaceAccess: "ro"` （或 `"none"` 以便不访问工作区）
- 阻止 `write`、`edit`、`apply_patch`、`exec`、`process` 等的工具允许/拒绝列表。

额外的加固选项：

- `tools.exec.applyPatch.workspaceOnly: true` （默认）：确保即使在关闭沙箱隔离的情况下，`apply_patch` 也无法在工作区目录之外进行写入/删除操作。仅当您有意希望 `apply_patch` 接触工作区之外的文件时，才设置为 `false`。
- `tools.fs.workspaceOnly: true` （可选）：将 `read`/`write`/`edit`/`apply_patch` 路径和本机提示图像自动加载路径限制在工作区目录内（如果您今天允许绝对路径并希望设置单一的防护栏，则非常有用）。
...
专门文档：[沙箱隔离](/en/gateway/sandboxing)

两种互补的方法：

- **在 Gateway(网关) 中运行完整的 Docker**（容器边界）：[Docker](/en/install/docker)
- **工具沙箱**（`agents.defaults.sandbox`，主机网关 + Docker 隔离工具）：[沙箱隔离](/en/gateway/sandboxing)

注意：为了防止跨代理访问，请将 `agents.defaults.sandbox.scope` 保持在 `"agent"`（默认值），或使用 `"session"` 以实现更严格的每个会话隔离。`scope: "shared"` 使用单个容器/工作区。

同时考虑沙箱内的代理工作区访问权限：

- `agents.defaults.sandbox.workspaceAccess: "none"`（默认）将代理工作区设为禁区；工具针对 `~/.openclaw/sandboxes` 下的沙箱工作区运行
...
- 额外的 `sandbox.docker.binds` 会根据规范化和标准化的源路径进行验证。如果父级符号链接技巧和标准主目录别名解析到 `/etc`、`/var/run` 或操作系统主目录下的凭据目录等被阻止的根目录，它们仍然会失败并关闭。

重要提示：`tools.elevated` 是在沙箱外部运行 exec 的全局基准逃生舱。默认情况下，有效主机是 `gateway`，或者当 exec 目标配置为 `node` 时为 `node`。请严格控制 `tools.elevated.allowFrom`，不要为陌生人启用它。您可以通过 `agents.list[].tools.elevated` 进一步限制每个代理的提权。请参阅[提权模式](/en/tools/elevated)。
...
- 请将 `agents.defaults.subagents.allowAgents` 以及任何针对特定 Agent 的 `agents.list[].subagents.allowAgents` 覆盖配置限制在已知安全的目标 Agent 上。
...
- 在严格模式下，使用 `hostnameAllowlist`（如 `*.example.com` 等模式）和 `allowedHostnames`（精确的主机例外，包括像 `localhost` 这样的被阻止名称）来设置显式例外。
- 导航会在请求前进行检查，并在导航后对最终的 `http(s)` URL 进行尽力而为的重新检查，以减少基于重定向的跳转攻击。
...
使用此功能为每个代理授予 **完全访问权限**、**只读权限** 或 **无访问权限**。
有关完整详细信息
...
2. **关闭暴露：** 设置 `gateway.bind: "loopback"`（或禁用 Tailscale Funnel/Serve），直到你弄清楚发生了什么。
3. **冻结访问：** 将有风险的私信/群组切换到 `dmPolicy: "disabled"` / 要求提及，并删除 `"*"` 允许所有的条目（如果有的话）。
...
3. 轮转提供商/API 凭证（WhatsApp 凭证、Slack/Discord 令牌、`auth-profiles.json` 中的模型/API 密钥，以及使用时的加密密钥负载值）。

### 审计

1. 检查 Gateway(网关) 日志：`/tmp/openclaw/openclaw-YYYY-MM-DD.log`（或 `logging.file`）。
2. 查看相关的记录：`~/.openclaw/agents/<agentId>/sessions/*.jsonl`。
...
推送到 `main` 始终运行全文件扫描。当有基础提交可用时，拉取请求使用已更改文件的快速路径，否则回退到全文件扫描。如果失败，说明有新的候选条目尚未添加到基线中。
...
5. 如果需要新的排除项，请将它们添加到 `.detect-secrets.cfg` 并使用匹配的 `--exclude-files` / `--exclude-lines` 标志重新生成基线（该配置文件仅供参考；detect-secrets 不会自动读取它）。
