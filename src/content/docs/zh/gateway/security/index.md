...
- 相比 LAN 绑定，优先使用 Tailscale Serve（Serve 使 Gateway 保持在环回地址上，并由 Tailscale 处理访问）。
...
Gateway 通过 mDNS（端口 5353 上的 `_openclaw-gw._tcp`）广播其存在以供本地设备发现。在完整模式下，这包括可能暴露运行细节的 TXT 记录：
...
在最小模式下，Gateway 仍会广播足够的信息以进行设备发现（`role`, `gatewayPort`, `transport`），但会省略 `cliPath` 和 `sshPort`。需要 CLI 路径信息的应用程序可以通过经过身份验证的 WebSocket 连接来获取。

### 0.5) 锁定 Gateway WebSocket（本地身份验证）

Gateway 身份验证**默认是必需的**。如果未配置有效的 Gateway 身份验证路径，
Gateway 将拒绝 WebSocket 连接（故障关闭）。
...
- 会话记录 + 简短的日志尾部（脱敏后）
- 攻击者发送的内容 + 代理执行的操作
- Gateway 是否暴露在回环地址之外（LAN/Tailscale Funnel/Serve）
