...
此仓库通过在专用主机（台式机/服务器）上运行单个 Gateway(网关)（主控端）并将客户端连接到它来支持“通过 SSH 远程访问”。
...
- macOS 应用“节点模式”只是一个通过 Gateway(网关) WebSocket 的节点客户端。
...
当网关仅限环回时，请将 URL 保留在 `ws://127.0.0.1:18789`macOS 并首先打开 SSH 隧道。
在 macOS 应用的 SSH 隧道传输中，发现的网关主机名属于
...
- 在 macOS 上，首选应用的“Remote over SSH”模式，该模式会自动管理隧道。
...
- **Trusted-proxy** 身份验证默认期望非环回的、具有身份感知的代理设置。
  同主机环回反向代理需要显式的 `gateway.auth.trustedProxy.allowLoopback = true`。
