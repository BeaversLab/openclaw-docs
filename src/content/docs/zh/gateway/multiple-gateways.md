...
- `gateway status --deep` 有助于发现旧版本安装中残留的 launchd/systemd/schtasks 服务。
- 仅当您有意运行多个隔离的 Gateway 时，才会预期出现 `gateway probe` 警告文本，例如 `multiple reachable gateways detected`。
