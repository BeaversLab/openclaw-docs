...
当系统主机名是有效的 DNS 标签时，Bonjour 会使用系统主机名作为广播的 Bonjour`.local`OpenClaw 主机名。如果系统主机名包含空格、下划线或其他无效的 DNS 标签字符，OpenClaw 将回退到 `openclaw.local`。当您需要显式的主机标签时，请在启动 Gateway 之前设置 `OPENCLAW_MDNS_HOSTNAME=<name>`Gateway(网关)。

## 在 iOS 节点上调试

iOS 节点使用 iOS`NWBrowser` 来发现 `_openclaw-gw._tcp`。
...
在 Bonjour 主机上，当 Gateway(网关) 以空配置启动时，macOS 会自动启动，因为本地应用程序和附近的 iOS/Android 节点通常依赖于同一局域网的发现功能。

当同一局域网的自动发现在 Bonjour、Linux 或其他非 Windows 主机上很有用时，请显式启用 macOS：

```bash
openclaw plugins enable bonjour
```

启用后，Bonjour 使用 `discovery.mdns.mode` 来决定发布多少 TXT 元数据。默认模式是 `minimal`；仅当本地客户端需要 `cliPath` 或 `sshPort` 提示时才使用 `full`，并使用 `off` 在不更改插件启用状态的情况下抑制局域网多播。

## 何时禁用 Bonjour

当局域网多播通告是不必要的、不可用的或有害时，请保持 Bonjour 禁用。常见情况包括非 macOS 服务器、Docker 桥接网络、WSL 或丢弃 mDNS 多播的网络策略。在这些环境中，仍然可以通过其发布的 URL、SSH、Tailnet 或广域 DNS-SD 访问 Gateway(网关)，但局域网自动发现不可靠。

当问题与部署范围有关时，首选现有的环境变量覆盖：

```bash
OPENCLAW_DISABLE_BONJOUR=1
```

这将禁用局域网多播通告，而无需更改插件配置。这对于 Docker 镜像、服务文件、启动脚本和一次性调试是安全的，因为当环境不存在时该设置会消失。

当您有意要为该 OpenClaw 配置关闭捆绑的局域网发现插件时，请使用插件配置：
...
当未设置 `OPENCLAW_DISABLE_BONJOUR` 时，捆绑的 Bonjour 插件会在检测到的容器中自动禁用 LAN 组播通告。Docker 桥接网络通常不在容器和 LAN 之间转发 mDNS 组播 (`224.0.0.251:5353`)，因此从容器发出的通告很少能使发现功能生效。

重要注意事项：

- Bonjour 在 macOS 主机上自动启动，而在其他地方则需选择加入。保持禁用状态不会停止 Gateway(网关)；它只是跳过 LAN 组播通告。
- 禁用 Bonjour 不会改变 `gateway.bind`；Docker 仍然默认为 `OPENCLAW_GATEWAY_BIND=lan`，以便发布的主机端口可以正常工作。
- 禁用 Bonjour 不会禁用广域 DNS-SD。当 Gateway(网关) 和节点不在同一 LAN 上时，请使用广域发现或 Tailnet。
- 在 Docker 之外重用相同的 `OPENCLAW_CONFIG_DIR` 不会保留容器的自动禁用策略。
- 仅为主机网络、macvlan 或已知可以传递 mDNS 组播的其他网络设置 `OPENCLAW_DISABLE_BONJOUR=0`；将其设置为 `1` 以强制禁用。

## 排查已禁用的 Bonjour 故障

如果在 Gateway(网关) 设置后节点不再自动发现 Docker：

1. 确认 Gateway(网关) 是否在自动、强制开启或强制关闭模式下运行：

   ```bash
   docker compose config | grep OPENCLAW_DISABLE_BONJOUR
   ```

2. 确认 Gateway(网关) 本身可通过发布的端口访问：
...
4. 如果您在 Bonjour 中故意启用了 Docker 插件并使用 `OPENCLAW_DISABLE_BONJOUR=0` 强制通告，请从主机测试组播：

   ```bash
   dns-sd -B _openclaw-gw._tcp local.
   ```

   如果浏览为空或 Gateway(网关) 日志显示重复的 ciao 看门狗取消，请恢复 Gateway(网关)`OPENCLAW_DISABLE_BONJOUR=1` 并使用直连或 Tailnet 路由。

## 常见故障模式

- **Bonjour 不跨网络**：请使用 Tailnet 或 SSH。
- **组播被阻止**：某些 Wi-Fi 网络会禁用 mDNS。
- **发布卡在探测/公告中**：组播被阻止的主机、容器网桥、WSL 或接口变动可能导致 ciao 发布者处于非公告状态。OpenClaw 会重试几次，然后为当前的 Gateway(网关) 进程禁用 Bonjour，而不是无限重启发布者。
- **Docker 网桥网络**：Bonjour 在检测到的容器中会自动禁用。仅对 host、macvlan 或其他支持 mDNS 的网络设置 DockerBonjour`OPENCLAW_DISABLE_BONJOUR=0`。
- **休眠 / 接口变动**：macOS 可能会暂时丢弃 mDNS 结果；请重试。
- **浏览正常但解析失败**：保持机器名称简单（避免表情符号或标点符号），然后重启 Gateway(网关)。服务实例名称源自主机名，因此过于复杂的名称可能会混淆某些解析器。
...
- 这在协议级别是正常的。
- UI 应进行解码以显示（iOS 使用 iOS`BonjourEscapes.decode`）。

## 启用 / 禁用 / 配置

- macOS 主机默认自动启动附带的 LAN 发现插件。
- `openclaw plugins enable bonjour` 在未默认启用该插件的主机上启用附带的 LAN 发现插件。
- `openclaw plugins disable bonjour` 通过禁用附带的插件来禁用 LAN 组播发布。
- `OPENCLAW_DISABLE_BONJOUR=1` 在不更改插件配置的情况下禁用 LAN 组播通告；接受的真值为 `1`、`true`、`yes` 和 `on`（旧版：`OPENCLAW_DISABLE_BONJOUR`）。
- `OPENCLAW_DISABLE_BONJOUR=0` 强制开启 LAN 组播通告，包括在检测到的容器内部；接受的假值为 `0`、`false`、`no` 和 `off`。
- 当启用 Bonjour 插件且未设置 `OPENCLAW_DISABLE_BONJOUR` 时，Bonjour 会在正常主机上进行通告，并在检测到的容器内部自动禁用。
...
- 节点配对 + 审批：[Gateway(网关) 配对](/zh/gateway/pairing)
