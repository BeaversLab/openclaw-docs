...
OpenClaw 使用 Bonjour (mDNS / DNS‑SD) 来发现活动的 Gateway(网关) (WebSocket endpoint)。
组播 `local.` 浏览是一种**仅限局域网的便利功能**。对于跨网络设备发现，
相同的信标也可以通过配置的广域 DNS-SD 域发布。设备发现
仍然是尽力而为的，并**不**替代基于 SSH 或 Tailnet 的连接。
...
一旦客户端接受 tailnet DNS，iOS 节点和 CLI 设备发现就可以在不使用组播的情况下浏览
您设备发现域中的 `_openclaw-gw._tcp`。

### Gateway(网关) listener security (recommended)

Gateway(网关) WS 端口 (默认 `18789`) 默认绑定到环回地址。对于局域网/tailnet
访问，请显式绑定并保持身份验证已启用。
...
只有 Gateway(网关) 会发布 `_openclaw-gw._tcp`。
...
- TLS pinning 绝不能允许通告的 `gatewayTlsSha256` 覆盖之前存储的 pin。
...
- Settings → Gateway(网关) → Advanced → **设备发现 Logs** → 复现问题 → **Copy**
...
- **休眠 / 接口变更**：macOS 可能会暂时丢失 mDNS 结果；请重试。
- **浏览正常但解析失败**：保持机器名称简单（避免表情符号或
  标点符号），然后重启 Gateway(网关)。服务实例名称派生自
  主机名，因此过于复杂的名称可能会使某些解析器混淆。
...
- 节点配对 + 审批：[Gateway(网关) 配对](/en/gateway/pairing)
