...
- `timeout` (秒，默认 `tools.exec.timeoutSec`)：在此超时后终止进程；仅设置 `timeout: 0` 以禁用该调用的 exec 进程超时
- `elevated` (bool)：如果启用了/允许提升模式，则在沙箱外运行 (默认 `gateway`，或者当 exec 目标为 `node` 时为 `node`)
...
- 前台运行直接返回输出。
- 当后台运行（显式或超时）时，工具返回 `status: "running"` + `sessionId` 和一个简短的尾部。
...
- 对于现在开始且长时间运行的工作，启动一次，并在启用自动完成唤醒且命令发出输出或失败时依赖它。
- 如果自动完成唤醒不可用，或者你需要干净退出且无输出的命令的静默成功确认，请使用 `process`
  来确认完成。
- 不要用 `sleep` 循环或重复
  轮询来模拟提醒或延迟跟进；将 cron 用于未来的工作。

## 子进程桥接

在 exec/process 工具之外生成长时间运行的子进程（例如，CLI 重生或网关助手）时，附加子进程桥接助手，以便转发终止信号并在退出/错误时分离侦听器。这可以避免 systemd 上的孤立进程，并使关闭行为在不同平台上保持一致。
...
- `tools.exec.notifyOnExit`（默认 true）：当后台 exec 退出时，将系统事件加入队列 + 请求心跳。
- `tools.exec.notifyOnExitEmptySuccess`（默认 false）：如果为 true，也为成功执行但未产生任何输出后台运行加入完成事件。
...
- 只有后台会话会被列出并持久保存在内存中。
- 进程重启时会丢失会话（无磁盘持久化）。
- 仅当您运行 `process poll/log` 且工具结果被记录时，会话日志才会保存到聊天历史记录。
- `process` 的范围限定为每个 agent；它只能看到由该 agent 启动的会话。
