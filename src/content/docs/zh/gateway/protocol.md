...
<AccordionGroup>
  ...
  <Accordion title="模型和用法">
    ...
    - `usage.cost` 返回日期范围内的聚合成本使用摘要。
      传递 `agentId` 针对单个代理，或传递 `agentScope: "all"` 以聚合已配置的代理。
    - `doctor.memory.status` 返回活动默认代理工作区的向量内存/缓存嵌入就绪状态。仅当调用方明确需要实时嵌入提供商 ping 时，才传递 `{ "probe": true }` 或 `{ "deep": true }`。
    - `doctor.memory.remHarness` 返回一个有界的、只读 REM 约束预览，供远程控制平面客户端使用。它可能包含工作区路径、内存片段、渲染的基于 Markdown 的内容以及深度提升候选者，因此调用方需要 `operator.read`。
    - `sessions.usage` 返回每个会话的使用摘要。传递 `agentId` 针对单个
      代理，或传递 `agentScope: "all"` 以列出一个配置的代理。
  ...
  <Accordion title="Channels and login helpers">
    ...
    - `channels.logout` 注销特定的渠道/账户，前提是该渠道支持注销。
  ...
  <Accordion title="Messaging and logs">
    - `send`RPC 是直接出站交付 RPC，用于在聊天运行器之外针对渠道/账户/线程目标的发送。
    - `logs.tail` 返回配置的网关文件日志尾部，包含游标/限制和最大字节控制。

  </Accordion>

  <Accordion title="Talk and TTS">
    - `talk.catalog` 返回用于语音、流式转录和实时语音的只读 Talk 提供商目录。它包括提供商 ID、标签、配置状态、公开的模型/语音 ID、规范模式、传输方式、大脑策略以及实时音频/能力标志，但不返回提供商机密或修改全局配置。
    - `talk.config` 返回有效的 Talk 配置负载；`includeSecrets` 需要 `operator.talk.secrets`（或 `operator.admin`）。
    - `talk.session.create` 为 `realtime/gateway-relay`、`transcription/gateway-relay` 或 `stt-tts/managed-room` 创建一个 Gateway(网关) 所有的 Talk 会话。对于 `stt-tts/managed-room`，传递 `sessionKey` 的 `operator.write` 调用者还必须传递 `spawnedBy` 以实现范围内的会话密钥可见性；非范围的 `sessionKey` 创建和 `brain: "direct-tools"` 需要 `operator.admin`。
    - `talk.session.join` 验证托管房间会话令牌，根据需要发出 `session.ready` 或 `session.replaced` 事件，并返回房间/会话元数据以及最近的 Talk 事件，但不包含明文令牌或存储的令牌哈希。
    - `talk.session.appendAudio` 将 base64 PCM 输入音频附加到 Gateway(网关) 所有的实时中继和转录会话。
    ...
    - `talk.session.submitToolResult` 完成由 Gateway(网关) 所有的实时中继会话发出的提供商工具调用。当最终结果随后出现时，传递 `options: { willContinue: true }` 以获取临时工具输出；或者当工具结果应该满足提供商调用而不启动另一个实时助手响应时，传递 `options: { suppressResponse: true }`。
    - `talk.session.steer` 将活动运行的语音控制发送到 Gateway(网关) 所有的由代理支持的 Talk 会话。它接受 `{ sessionId, text, mode? }`，其中 `mode` 是 `status`、`steer`、`cancel` 或 `followup`；省略的模式将从口语文本中分类。
    - `talk.session.close` 关闭 Gateway(网关) 所有的中继、转录或托管房间会话，并发出终端 Talk 事件。
    - `talk.mode` 为 WebChat/Control UI 客户端设置/广播当前的 Talk 模式状态。
    - `talk.client.create` 使用 `webrtc` 或 `provider-websocket` 创建客户端所有的实时提供商会话，同时 Gateway(网关) 拥有配置、凭据、指令和工具策略。
    - `talk.client.toolCall` 允许客户端所有的实时传输将提供商工具调用转发给 Gateway(网关) 策略。第一个支持的工具是 `openclaw_agent_consult`；客户端在提交特定于提供商的工具结果之前，会收到一个运行 ID 并等待正常的聊天生命周期事件。
    - `talk.client.steer` 为客户端所有的实时传输发送活动运行的语音控制。Gateway(网关) 从 `sessionKey` 解析活动的嵌入式运行，并返回结构化的接受/拒绝结果，而不是静默丢弃控制。
  ...
  <Accordion title="Secrets, config, update, and wizard">
    - `secrets.reload` 仅在完全成功时重新解析活动 SecretRefs 并交换运行时 secret 状态。
    ...
    - `config.schema.lookup` 返回一个配置路径的范围查找负载：标准化路径、浅层架构节点、匹配的提示 + `hintPath`、可选的 `reloadKind`，以及用于 UI/CLI 向下钻取的直接子摘要。`reloadKind` 是 `restart`、`hot` 或 `none` 之一，并反映所请求路径的 Gateway(网关) 配置重新加载计划器。查找架构节点保留面向用户的文档和通用验证字段（`title`、`description`、`type`、`enum`、`const`、`format`、`pattern`、数字/字符串/数组/对象边界，以及像 `additionalProperties`、`deprecated`、`readOnly`、`writeOnly` 这样的标志）。子摘要公开 `key`、标准化的 `path`、`type`、`required`、`hasChildren`、可选的 `reloadKind`，加上匹配的 `hint` / `hintPath`。
    - `update.run` 运行网关更新流程，并仅在更新本身成功时安排重启；拥有会话的调用者可以包含 `continuationMessage`，以便通过重启继续队列恢复一个后续代理轮次。来自控制平面包管理器更新使用分离的托管服务移交，而不是替换活动 Gateway(网关) 内部的包树。已启动的移交返回 `ok: true`，其中包含 `result.reason: "managed-service-handoff-started"` 和 `handoff.status: "started"`；不可用或失败的移交返回 `ok: false`，其中包含 `managed-service-handoff-unavailable` 或 `managed-service-handoff-failed`，当需要手动 Shell 更新时，还会加上 `handoff.command`。在已启动的移交期间，重启哨兵可能会短暂报告 `stats.reason: "restart-health-pending"`；继续操作将延迟，直到 CLI 验证已重启的 Gateway(网关) 并写入最终的 `ok` 哨兵。
    - `update.status` 返回最新的缓存更新重启哨兵，包括可用时重启后的运行版本。
    - `wizard.start`、`wizard.next`、`wizard.status` 和 `wizard.cancel` 通过 WS RPC 公开新手向导。
  ...
  <Accordion title="Agent and workspace helpers">
    ...
    - `artifacts.list`、`artifacts.get` 和 `artifacts.download` 公开源自脚本的工件摘要和下载，适用于特定的 `sessionKey`、`runId` 或 `taskId` 作用域。Run 和 task 查询在服务端解析所属会话，并且仅返回具有匹配来源的脚本媒体；不安全或本地 URL 源返回不支持的下载，而不是在服务端获取。
  ...
  <Accordion title="设备配对和设备令牌">
    ...
    - `device.token.rotate` 在其批准的角色和调用者范围界限内轮换配对设备令牌。
    - `device.token.revoke` 在其批准的角色和调用者范围界限内撤销配对设备令牌。
  ...
  <Accordion title="自动化、Skills 和工具">
    - 自动化：`wake` 调度立即或下一次心跳的唤醒文本注入；`cron.get`、`cron.list`、`cron.status`、`cron.add`、`cron.update`、`cron.remove`、`cron.run` 和 `cron.runs` 管理计划的工作。
    - `cron.run`RPC 仍然是一种用于手动运行的入队式 RPC。需要完成语义的客户端应读取返回的 `runId` 并轮询 `cron.runs`。
    - `cron.runs` 接受一个可选的非空 `runId` 过滤器，以便客户端可以跟踪一个排队的手动运行，而不会与同一作业的其他历史条目产生竞争。
...
- `chat`：UI 聊天更新，例如 `chat.inject` 和其他仅限记录的聊天事件。在协议 v4 中，增量负载携带 `deltaText`；`message` 仍然是累积的助手快照。非前缀替换设置 `replace=true` 并使用 `deltaText` 作为替换文本。
- `session.message`、`session.operation` 和 `session.tool`：已订阅会话的记录、进行中的会话操作和事件流更新。
...
`TaskSummary` 包括 `id`、`status` 以及可选的元数据，如 `kind`、
...
- 操作员可以调用 `commands.list` (`operator.read`) 来获取代理的
  ...
  - 当存在提供商感知的原生命令名称时，`nativeName` 会携带该名称。
...
- 操作员可以调用 `tools.effective` (`operator.read`) 来获取会话的运行时有效工具清单。
  ...
  - `tools.effective` 对 MCP 而言是只读的：它可以通过最终工具策略投影热会话 MCP 目录，但它不会创建 MCP 运行时、连接传输或发出 `tools/list`。如果不存在匹配的热目录，响应可能包含诸如 `mcp-not-yet-connected`、`mcp-not-yet-listed` 或 `mcp-stale-catalog` 之类的通知。
  - 有效的工具条目使用 `source="core"`、`source="plugin"`、`source="channel"` 或 `source="mcp"`。
- 操作员可以调用 `tools.invoke` (`operator.write`) 以通过与 `/tools/invoke` 相同的网关策略路径来调用一个可用工具。
  ...
  - 响应是一个面向 SDK 的信封，包含 `ok`、`toolName`、可选的 `output` 和类型化
    `error` 字段。批准或策略拒绝会在有效载荷中返回 `ok:false`，而不是
    绕过网关工具策略管道。
...
- 操作员可以调用 `skills.upload.begin`、`skills.upload.chunk` 和
  `skills.upload.commit` (`operator.admin`) 以在安装私有技能存档之前
  对其进行暂存。这是一个供受信任客户端使用的单独管理员上传路径，
  而不是正常的 ClawHub 技能安装流程，并且默认处于禁用状态，除非
  ...
  - `skills.upload.commit({ uploadId, sha256? })` 验证最终大小和
    SHA-256。提交仅完成上传；它不会安装该技能。
  - 上传的技能存档是包含 `SKILL.md` 根目录的 zip 存档。
    存档的内部目录名称从不选择安装目标。
- 操作员可以通过三种模式调用 `skills.install` (`operator.admin`)：
  - ClawHub 模式：ClawHub`{ source: "clawhub", slug, version?, force? }` 将一个技能文件夹安装到默认 agent workspace 的 `skills/` 目录中。
  - 上传模式：`{ source: "upload", uploadId, slug, force?, sha256?, timeoutMs? }`
    将已提交的上传内容安装到默认 agent workspace 的 `skills/<slug>`
    目录中。slug 和 force 值必须与原始
    `skills.upload.begin` 请求匹配。除非
    `skills.install.allowUploadedArchives`ClawHub 已启用，否则将拒绝此模式。该设置不影响 ClawHub 安装。
  - Gateway(网关) 安装程序模式：Gateway(网关)`{ name, installId, dangerouslyForceUnsafeInstall?, timeoutMs? }`
...
- `"configured"`：选择器大小的行为。如果配置了 `agents.defaults.models`，它仍然优先，包括 `provider/*` 条目的 提供商-scoped 发现。如果没有允许列表，响应使用显式的 `models.providers.*.models` 条目，仅当不存在配置的模型行时才回退到完整目录。
- `"all"`Gateway(网关): 完整的Gateway(网关)目录，绕过 `agents.defaults.models`。将此用于诊断和发现用户界面，而非普通的模型选择器。

## 执行审批

- 当执行请求需要审批时，Gateway(网关)会广播 `exec.approval.requested`。
- Operator 客户端通过调用 `exec.approval.resolve` 来解决（需要 `operator.approvals` scope）。
...
- 如果调用者在准备阶段和最终批准的 `system.run` 转发之间改变了 `command`、`rawCommand`、`cwd`、`agentId` 或
  `sessionKey`，Gateway(网关)将拒绝运行，而不是信任被更改的有效负载。

## Agent 投递回退

- `agent` 请求可以包含 `deliver=true` 以请求出站投递。
- `bestEffortDeliver=false` 保持严格行为：未解析或仅限内部的投递目标返回 `INVALID_REQUEST`。
- 当无法解析外部可投递路由时（例如内部/webchat 会话或不明确的多渠道配置），`bestEffortDeliver=true` 允许回退到仅会话执行。
- 当请求投递时，最终的 `agent` 结果可能包含 `result.deliveryStatus`，
  使用为 [`openclaw agent --json --deliver`](/zh/cli/agent#json-delivery-status) 记录的相同 `sent`、`suppressed`、`partial_failed` 和 `failed`
  状态。
...
- 共享密钥 Gateway 身份验证使用 `connect.params.auth.token` 或
  `connect.params.auth.password`，具体取决于配置的身份验证模式。
- 承载身份的模式（如 Tailscale Serve
  (`gateway.auth.allowTailscale: true`) 或非环回
  `gateway.auth.mode: "trusted-proxy"`）通过
  请求头满足连接身份验证检查，而不是 `connect.params.auth.*`。
- 私有入口 `gateway.auth.mode: "none"` 完全跳过共享密钥连接身份验证；
  请勿在公共/不受信任的入口上公开该模式。
- 配对后，Gateway(网关) 会颁发一个范围为连接
  角色 + 范围的 **device token**。它会在 `hello-ok.auth.deviceToken` 中返回，客户端
  应将其持久化以供将来连接使用。
- 客户端应在任何
...
- 使用该 **存储的** device token 重新连接也应重用为该令牌存储的
  ...
  并避免将重新连接静默折叠为
  更窄的隐式仅管理员范围。
- 客户端连接身份验证组装（`selectConnectAuth` 在
  ...
  - `auth.token` 按优先级顺序填充：首先是显式共享令牌，
    ...
    `deviceId` + `role` 键控）。
  - 仅当上述均未解析出
    `auth.token` 时才发送 `auth.bootstrapToken`。共享令牌或任何已解析的设备令牌会抑制它。
  - 在一次性 `AUTH_TOKEN_MISMATCH` 重试中，存储的设备令牌的自动提升仅限于**受信任的端点**——
    环回，或带有固定 `tlsFingerprint` 的 `wss://`。没有固定的公共 `wss://`
    不符合条件。
- 内置的设置代码引导返回主节点
  `hello-ok.auth.deviceToken` 加上 `hello-ok.auth.deviceTokens` 中的有界操作员令牌，
  用于受信任的移动端移交。操作员令牌
  包含用于原生 Talk 配置读取的 `operator.talk.secrets`，
...
- 当非基线设置代码引导正在等待批准时，`PAIRING_REQUIRED`
  ...
  引导令牌重新连接，直到请求被批准或令牌变得无效。
- 仅当连接在 `wss://` 或环回/本地配对等受信任传输上
  使用引导身份验证时，才持久化 `hello-ok.auth.deviceTokens`。
- 如果客户端提供了**显式** `deviceToken` 或显式 `scopes`，该
  调用者请求的范围集将保持权威；仅当
  客户端重用存储的每设备令牌时，才会重用缓存的范围。
- 设备令牌可以通过 `device.token.rotate` 和
  `device.token.revoke` 轮换/撤销（需要 `operator.pairing` 范围）。轮换或
  撤销节点或其他非操作员角色还需要 `operator.admin`。
- `device.token.rotate` 返回轮换元数据。它仅针对已使用
  该设备令牌进行身份验证的同一设备调用回显替换
  持有者令牌，以便仅令牌客户端可以在重新连接之前持久化其替换项。
  共享/管理员轮换不回显持有者令牌。
- 令牌的颁发、轮换和撤销始终受限于记录在
  该设备配对条目中的已批准角色集；令牌变更
  不能扩展或定位配对批准从未授予的设备角色。
- 对于已配对设备的令牌会话，设备管理是自限域的，除非调用者也拥有 `operator.admin`：非管理员调用者只能管理其**自身**设备条目的操作员令牌。节点和其他非操作员令牌的管理仅限管理员，即使对于调用者自己的设备也是如此。
- `device.token.rotate` 和 `device.token.revoke` 也会检查目标操作员令牌域集与调用者当前会话域的匹配情况。非管理员调用者不能轮换或撤销比其当前持有的更广泛的操作员令牌。
...
- `AUTH_SCOPE_MISMATCH` 表示设备令牌已被识别，但不涵盖请求的角色/域。客户端不应将其显示为错误令牌；应提示操作员重新配对或批准更窄/更宽的域约定。
...
- WS 客户端通常在 `connect`（操作员 + 节点）期间包含 `device` 身份。唯一的无设备操作员例外是显式信任路径：
  - `gateway.controlUi.allowInsecureAuth=true` 用于仅限本地的不安全 HTTP 兼容。
  - 成功的 `gateway.auth.mode: "trusted-proxy"` 运营商控制 UI 身份验证。
  - `gateway.controlUi.dangerouslyDisableDeviceAuth=true`（破罐子破摔，严重的安全降级）。
  - 使用共享网关令牌/密码进行身份验证的直接回环 `gateway-client` 后端 RPC。
- 省略设备身份会影响范围。当控制 UI 连接缺少设备身份时，`shouldClearUnboundScopesForMissingDeviceIdentity` 会将针对令牌、密码和受信任代理身份验证的自声明范围清除为空集。该连接允许在显式信任路径上，但受范围限制的方法会失败。例外情况是具有 `allowInsecureAuth` 的本地控制 UI 令牌/密码会话，它们会保留范围。对于其他情况，仅将 `gateway.controlUi.dangerouslyDisableDeviceAuth=true` 作为破罐子破摔的范围保留路径进行设置。
- 所有连接必须对服务器提供的 `connect.challenge` 随机数进行签名。

### 设备身份验证迁移诊断

对于仍使用预挑战签名行为的旧版客户端，`connect` 现在在 `error.details.code` 下返回带有稳定的 `error.details.reason` 的 `DEVICE_AUTH_*` 详细代码。

常见的迁移失败：

| 消息                        | details.code                     | details.reason           | 含义                                          |
| --------------------------- | -------------------------------- | ------------------------ | --------------------------------------------- |
| `device nonce required`     | `DEVICE_AUTH_NONCE_REQUIRED`     | `device-nonce-missing`   | 客户端省略了 `device.nonce`（或发送为空白）。 |
...
- 请始终等待 `connect.challenge`。
- 对包含服务器 nonce 的 v2 载荷进行签名。
- 在 `connect.params.device.nonce` 中发送相同的 nonce。
- 首选的签名载荷是 `v3`，它除了绑定设备/客户端/角色/范围/令牌/nonce 字段外，还绑定了 `platform` 和 `deviceFamily`。
- 出于兼容性考虑，仍接受旧的 `v2` 签名，但在重新连接时，配对设备元数据固定仍然控制命令策略。

## TLS + 证书固定

- WS 连接支持 TLS。
- 客户端可以选择固定网关证书指纹（请参阅 `gateway.tls` 配置以及 `gateway.remote.tlsFingerprint` 或 CLI CLI `--tls-fingerprint`）。

## 范围

此协议暴露了**完整的网关 API**（状态、频道、模型、聊天、代理、会话、节点、审批等）。具体的接口由 `packages/gateway-protocol/src/schema.ts` 中的 TypeBox 架构定义。

## 相关

- [桥接协议](/zh/gateway/bridge-protocol)
- [网关运维手册](<Gateway(网关)/en/gateway>)
