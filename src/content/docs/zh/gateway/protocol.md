...
操作员交接特意设计为有界的，以便 QR 新手引导可以在不授予 `operator.admin`、`operator.pairing` 或
...
有关完整的操作员范围模型、审批时检查和共享密钥语义，请参阅 [Operator scopes](/zh/gateway/operator-scopes)。
...
<AccordionGroup>
  ...
  <Accordion title="Messaging and logs">
    - `send`RPC 是用于在聊天运行器之外进行针对渠道/账户/线程发送的直接出站交付 RPC。
    - `logs.tail` 返回配置的网关文件日志尾部，并包含游标/限制和最大字节控制。

  </Accordion>

  <Accordion title="Talk and TTS">
    - `talk.catalog` 返回用于语音、流式转录和实时语音的只读 Talk 提供商目录。它包括提供商 ID、标签、配置状态、公开的模型/语音 ID、规范模式、传输、大脑策略和实时音频/功能标志，而不返回提供商机密或更改全局配置。
    - `talk.config` 返回有效的 Talk 配置负载；`includeSecrets` 需要 `operator.talk.secrets`（或 `operator.admin`）。
    - `talk.session.create`Gateway(网关) 为 `realtime/gateway-relay`、`transcription/gateway-relay` 或 `stt-tts/managed-room` 创建 Gateway(网关)拥有的 Talk 会话。对于 `stt-tts/managed-room`，传递 `sessionKey` 的 `operator.write` 调用方还必须传递 `spawnedBy` 以实现作用域会话密钥可见性；无作用域的 `sessionKey` 创建和 `brain: "direct-tools"` 需要 `operator.admin`。
    - `talk.session.join` 验证托管房间会话令牌，根据需要发出 `session.ready` 或 `session.replaced` 事件，并返回房间/会话元数据和最近的 Talk 事件，但不返回明文令牌或存储的令牌哈希。
    - `talk.session.appendAudio`Gateway(网关) 将 base64 PCM 输入音频附加到 Gateway(网关)拥有的实时中继和转录会话。
    - `talk.session.startTurn`、`talk.session.endTurn` 和 `talk.session.cancelTurn` 驱动托管房间的轮次生命周期，并在清除状态之前拒绝过时的轮次。
    - `talk.session.cancelOutput`Gateway(网关) 停止助手音频输出，主要用于 Gateway(网关)中继会话中的 VAD 门控插话。
    - `talk.session.submitToolResult`Gateway(网关) 完成由 Gateway(网关)拥有的实时中继会话发出的提供商工具调用。传递 `options: { willContinue: true }` 用于中间工具输出（当最终结果紧随其后时），或传递 `options: { suppressResponse: true }` 当工具结果应满足提供商调用而不启动另一个实时助手响应时。
    - `talk.session.steer`Gateway(网关) 将活动运行语音控制发送到 Gateway(网关)拥有的代理支持的 Talk 会话中。它接受 `{ sessionId, text, mode? }`，其中 `mode` 是 `status`、`steer`、`cancel` 或 `followup`；省略的模式根据口语文本进行分类。
    - `talk.session.close`Gateway(网关) 关闭 Gateway(网关)拥有的中继、转录或托管房间会话，并发出终端 Talk 事件。
    ...
    - `talk.client.toolCall`Gateway(网关) 允许客户端拥有的实时传输将提供商工具调用转发到 Gateway(网关)策略。第一个支持的工具是 `openclaw_agent_consult`；客户端在提交特定于提供商的工具结果之前接收运行 ID 并等待正常的聊天生命周期事件。
    - `talk.client.steer`Gateway(网关) 为客户端拥有的实时传输发送活动运行语音控制。Gateway(网关) 从 `sessionKey` 解析活动的嵌入式运行，并返回结构化的接受/拒绝结果，而不是静默丢弃转向。
  ...
  <Accordion title="Secrets, config, update, and wizard">
    ...
    - `config.schema` 返回控制 UI 和 CLI 工具使用的实时配置架构负载：架构、`uiHints`、版本和生成元数据，包括运行时可以加载时的插件 + 渠道架构元数据。该架构包括从 UI 使用的相同标签和帮助文本派生的字段 `title` / `description` 元数据，包括嵌套对象、通配符、数组项以及 `anyOf` / `oneOf` / `allOf` 组合分支（当存在匹配的字段文档时）。
    - `config.schema.lookup` 返回一个配置路径的范围限定查找负载：标准化路径、浅层架构节点、匹配的提示 + `hintPath`、可选的 `reloadKind`，以及用于 UI/CLI 向下钻取的直接子项摘要。`reloadKind` 是 `restart`、`hot` 或 `none` 之一，并反映所请求路径的 Gateway(网关) 配置重新加载规划器。查找架构节点保留面向用户的文档和通用验证字段（`title`、`description`、`type`、`enum`、`const`、`format`、`pattern`、数字/字符串/数组/对象边界，以及诸如 `additionalProperties`、`deprecated`、`readOnly`、`writeOnly` 等标志）。子项摘要公开 `key`、标准化的 `path`、`type`、`required`、`hasChildren`、可选的 `reloadKind`，以及匹配的 `hint` / `hintPath`。
    - `update.run` 运行网关更新流程，并仅在更新本身成功时安排重新启动；具有会话的调用者可以包含 `continuationMessage`，以便启动过程通过重新启动延续队列恢复一个后续代理轮次。来自控制平面的包管理器更新使用分离的托管服务切换，而不是替换活动 Gateway(网关) 内的包树。已启动的切换返回 `ok: true`，其中包含 `result.reason: "managed-service-handoff-started"` 和 `handoff.status: "started"`；不可用或失败的切换返回 `ok: false`，其中包含 `managed-service-handoff-unavailable` 或 `managed-service-handoff-failed`，以及需要手动 shell 更新时的 `handoff.command`。在已启动的切换期间，重新启动哨兵可能会短暂报告 `stats.reason: "restart-health-pending"`；延续操作会延迟，直到 CLI 验证重新启动的 Gateway(网关) 并写入最终的 `ok` 哨兵为止。
    - `update.status` 返回最新的缓存更新重新启动哨兵，包括可用时重新启动后的运行版本。
  ...
  <Accordion title="Agent and workspace helpers">
    ...
    - `agents.create`、`agents.update` 和 `agents.delete` 管理 agent 记录和工作空间连接。
    - `agents.files.list`、`agents.files.get` 和 `agents.files.set` 管理为 agent 公开的引导工作空间文件。
    - `tasks.list`、`tasks.get` 和 `tasks.cancel` 向 SDK 和操作员客户端公开 Gateway(网关) 任务账本。
    - `artifacts.list`、`artifacts.get` 和 `artifacts.download` 为特定的 `sessionKey`、`runId` 或 `taskId` 范围公开从脚本派生的工件摘要和下载。运行和任务查询在服务端解析所属会话，并仅返回具有匹配来源的脚本媒体；不安全或本地 URL 源返回不支持的下载，而不是在服务端获取。
    - `environments.list` 和 `environments.status` 向 SDK 客户端公开只读的 Gateway(网关) 本地和节点环境发现信息。
  ...
  <Accordion title="会话控制">
    - `sessions.list` 返回当前会话索引，当配置了代理运行时后端时，包含逐行的 `agentRuntime` 元数据。
    - `sessions.subscribe` 和 `sessions.unsubscribe` 切换当前 WS 客户端的会话变更事件订阅。
    - `sessions.messages.subscribe` 和 `sessions.messages.unsubscribe` 切换单个会话的记录/消息事件订阅。
    - `sessions.preview` 返回特定会话键的有界记录预览。
    - `sessions.describe` 返回精确会话键的一个 Gateway(网关) 会话行。
    - `sessions.resolve` 解析或规范化会话目标。
    - `sessions.create` 创建新会话条目。
    - `sessions.send` 向现有会话发送消息。
    - `sessions.steer` 是活动会话的中断和引导变体。
    - `sessions.abort` 中止会话的活跃工作。调用方可以传递 `key` 加上可选的 `runId`，或者单独传递 `runId`，用于 Gateway(网关) 可以解析为会话的活跃运行。
    - `sessions.patch` 更新会话元数据/覆盖项，并报告解析的规范化模型和有效的 `agentRuntime`。
    - `sessions.reset`、`sessions.delete` 和 `sessions.compact` 执行会话维护。
    - `sessions.get` 返回完整的存储会话行。
    - 聊天执行仍使用 `chat.history`、`chat.send`、`chat.abort` 和 `chat.inject`。`chat.history` 针对客户端 UI 进行了显示规范化：内联指令标签从可见文本中剥离，纯文本工具调用 XML 载荷（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 和截断的工具调用块）以及泄露的 ASCII/全角模型控制令牌被剥离，纯静默令牌助手行（如精确的 `NO_REPLY` / `no_reply`）被省略，过大的行可以用占位符替换。
  ...
  <Accordion title="设备配对和设备令牌">
    ...
    - `device.token.rotate` 在其已批准的角色和调用者范围内轮换配对设备令牌。
    - `device.token.revoke` 在其已批准的角色和调用者范围内撤销配对设备令牌。
  ...
  <Accordion title="Approval families">
    - `exec.approval.request`、`exec.approval.get`、`exec.approval.list` 和 `exec.approval.resolve` 涵盖一次性执行审批请求以及待处理审批的查找/重放。
  ...
  <Accordion title="Automation, skills, and tools">
    - Automation：`wake` 调度立即或下一次心跳唤醒文本注入；`cron.get`、`cron.list`、`cron.status`、`cron.add`、`cron.update`、`cron.remove`、`cron.run` 和 `cron.runs` 管理计划工作。
    ...
    - Skills 和 tools：`commands.list`、`skills.*`、`tools.catalog`、`tools.effective`、`tools.invoke`。
...
- `chat`: UI 聊天更新，例如 `chat.inject` 和其他仅限脚本的聊天
  事件。在协议 v4 中，增量载荷携带 `deltaText`；`message` 仍然
  是累积的助手快照。非前缀替换设置 `replace=true`
  并使用 `deltaText` 作为替换文本。
- `session.message`、`session.operation` 和 `session.tool`: 脚本、
  进行中的会话操作以及已订阅会话的事件流更新。
...
- `exec.approval.requested` / `exec.approval.resolved`: exec 批准
  生命周期。
...
`TaskSummary` 包括 `id`、`status` 以及可选的元数据，例如 `kind`、
...
- 操作员可以调用 `skills.search` 和 `skills.detail` (`operator.read`ClawHub) 来获取 ClawHub 发现元数据。
- 操作员可以调用 `skills.upload.begin`、`skills.upload.chunk` 和 `skills.upload.commit` (`operator.admin`ClawHub) 以在安装私有技能归档之前对其进行暂存。这是一个适用于受信任客户端的独立管理员上传路径，并非正常的 ClawHub 技能安装流程，并且默认情况下处于禁用状态，除非启用了 `skills.install.allowUploadedArchives`。
  - `skills.upload.begin({ kind: "skill-archive", slug, sizeBytes, sha256?, force?, idempotencyKey? })` 创建一个绑定到该 slug 和 force 值的上传。
  - `skills.upload.chunk({ uploadId, offset, dataBase64 })` 在确切的解码偏移量处追加字节。
  - `skills.upload.commit({ uploadId, sha256? })` 验证最终大小和 SHA-256。提交仅完成上传；它不会安装技能。
  - 上传的技能归档是包含 `SKILL.md` 根目录的 zip 归档。归档的内部目录名称从不选择安装目标。
- 操作员可以通过三种模式调用 `skills.install` (`operator.admin`)：
  - ClawHub 模式：ClawHub`{ source: "clawhub", slug, version?, force? }` 将技能文件夹安装到默认代理工作区 `skills/` 目录中。
  - 上传模式：`{ source: "upload", uploadId, slug, force?, sha256?, timeoutMs? }` 将已提交的上传安装到默认代理工作区 `skills/<slug>` 目录中。Slug 和 force 值必须与原始 `skills.upload.begin` 请求匹配。除非启用了 `skills.install.allowUploadedArchives`ClawHub，否则将拒绝此模式。该设置不影响 ClawHub 安装。
  - Gateway(网关) 安装程序模式：Gateway(网关)`{ name, installId, dangerouslyForceUnsafeInstall?, timeoutMs? }` 在网关主机上运行已声明的 `metadata.openclaw.install` 操作。
...
- 省略或 `"default"`：当前运行时行为。如果配置了 `agents.defaults.models`，则响应为允许的目录，包括针对 `provider/*`Gateway(网关) 条目动态发现的模型。否则响应为完整的 Gateway 目录。
- `"configured"`：选择器大小的行为。如果配置了 `agents.defaults.models`，它仍然优先，包括针对 `provider/*` 条目的提供商范围发现。如果没有允许列表，响应将使用显式的 `models.providers.*.models` 条目，仅当不存在配置的模型行时才回退到完整目录。
- `"all"`Gateway(网关)：完整的 Gateway 目录，绕过 `agents.defaults.models`。将此用于诊断和发现界面，而非正常的模型选择器。
...
- 批准后，转发的 `node.invoke system.run` 调用将重用该规范化的
  `systemRunPlan` 作为权威的命令/工作目录/会话上下文。
- 如果调用者在准备和最终批准的 `system.run` 转发之间更改了 `command`、`rawCommand`、`cwd`、`agentId` 或 `sessionKey`，网关将拒绝运行，而不是信任更改后的有效负载。

## 代理投递回退

- `agent` 请求可以包含 `deliver=true` 以请求出站投递。
- `bestEffortDeliver=false` 保持严格行为：未解析或仅限内部的投递目标返回 `INVALID_REQUEST`。
- 当无法解析外部可投递路由时（例如内部/webchat 会话或多渠道配置模棱两可），`bestEffortDeliver=true` 允许回退到仅限会话的执行。
- 最终 `agent` 结果可能包含 `result.deliveryStatus`（如果请求了投递），使用为 [`openclaw agent --json --deliver`](/zh/cli/agent#json-delivery-status) 记录的相同 `sent`、`suppressed`、`partial_failed` 和 `failed` 状态。

## 版本控制

- `PROTOCOL_VERSION` 位于 `src/gateway/protocol/version.ts` 中。
- 客户端发送 `minProtocol` + `maxProtocol`；服务器会拒绝不包含其当前协议的范围。当前的客户端和服务器需要协议 v4。
...
| 预认证 / 连接质询超时                 | `15_000` ms                                         | `src/gateway/handshake-timeouts.ts` (配置/环境可以提高配对的服务器/客户端预算) |
...
| 在 `terminate()` 之前的强制停止宽限期 | `250` 毫秒                                          | `FORCE_STOP_TERMINATE_GRACE_MS`                                                |
| `stopAndWait()` 默认超时              | `1_000` 毫秒                                        | `STOP_AND_WAIT_TIMEOUT_MS`                                                     |
| 默认心跳间隔（在 `hello-ok` 之前）    | `30_000` 毫秒                                       | `src/gateway/client.ts`                                                        |
...
- 承载身份的模式，例如 Tailscale Serve
  ...
  `gateway.auth.mode: "trusted-proxy"`，通过请求头
  满足连接认证检查，而不是 `connect.params.auth.*`。
- 私有入口 `gateway.auth.mode: "none"` 完全跳过共享密钥连接认证；
  不要在公共/不受信任的入口上公开该模式。
- 配对后，Gateway(网关) 会发布一个限定于连接
  角色 + 范围的 **设备令牌**。它在 `hello-ok.auth.deviceToken` 中返回，客户端
  应将其持久化以用于将来的连接。
- 客户端应在任何
...
- 客户端连接认证组装（`selectConnectAuth` 在
  ...
  - `auth.password` 是正交的，设置后始终被转发。
  - `auth.token` 按优先级顺序填充：首先是显式共享令牌，
    然后是显式 `deviceToken`，最后是存储的每设备令牌（键为
    `deviceId` + `role`）。
  - 仅当上述方法均未解析出
    `auth.token` 时，才会发送 `auth.bootstrapToken`。共享令牌或任何已解析的设备令牌会抑制其发送。
  - 在单次 `AUTH_TOKEN_MISMATCH` 重试期间自动提升存储的设备令牌的操作仅限于 **受信任的端点** —
    环回地址，或具有固定 `tlsFingerprint` 的 `wss://`。不具有固定配置的公共 `wss://`
    不符合条件。
- 内置的设置代码引导会返回主节点
  `hello-ok.auth.deviceToken` 以及位于 `hello-ok.auth.deviceTokens` 中的有界操作员令牌，用于受信任的移动端移交。该操作员令牌
  排除 `operator.admin`、`operator.pairing` 和 `operator.talk.secrets`。
- 当非基线设置代码引导正在等待批准时，`PAIRING_REQUIRED`
  ...
  引导令牌重新连接，直到请求被批准或令牌失效。
- 仅当连接在 `wss://` 或环回/本地配对等受信任传输上使用了引导身份验证时，
  才应持久化 `hello-ok.auth.deviceTokens`。
- 如果客户端提供了 **显式** `deviceToken` 或显式 `scopes`，该
  调用方请求的作用域集将保持权威性；仅当客户端重用存储的每设备令牌时，才会重用缓存的作用域。
- 可以通过 `device.token.rotate` 和
  `device.token.revoke` 轮换/撤销设备令牌（需要 `operator.pairing` 作用域）。
- `device.token.rotate` 返回轮换元数据。它仅在已使用该设备令牌进行身份验证的同一设备调用中回显替换的 Bearer 令牌，以便仅使用令牌的客户端可以在重新连接之前持久化其替换令牌。共享/管理员轮换不会回显 Bearer 令牌。
- 令牌的签发、轮换和撤销始终受限于记录在该设备配对条目中的已批准角色集；令牌变更不能扩展或定位到配对批准从未授予的设备角色。
- 对于配对设备令牌会话，设备管理是自限范围的，除非调用者还拥有 `operator.admin`：非管理员调用者只能移除/撤销/轮换他们**自己的**设备条目。
- `device.token.rotate` 和 `device.token.revoke` 还会根据调用者当前的会话范围检查目标操作员令牌范围集。非管理员调用者不能轮换或撤销比其当前持有的更宽泛的操作员令牌。
...
- `AUTH_SCOPE_MISMATCH` 意味着设备令牌已被识别，但不涵盖请求的角色/范围。客户端不应将此显示为错误的令牌；应提示操作员重新配对或批准更窄/更宽的范围协议。
...
- WS 客户端通常在 `connect`（操作员 + 节点）期间包含 `device` 标识。唯一无设备的操作员例外是显式信任路径：
  - `gateway.controlUi.allowInsecureAuth=true` 用于仅限本地主机的不安全 HTTP 兼容性。
  - 成功的 `gateway.auth.mode: "trusted-proxy"` 操作员控制 UI 身份验证。
  - `gateway.controlUi.dangerouslyDisableDeviceAuth=true`（打破隔离，严重的安全降级）。
  - 使用共享网关令牌/密码进行身份验证的直接环回 `gateway-client` 后端 RPC。
- 省略设备标识会产生范围后果。当控制 UI 连接缺少设备标识时，`shouldClearUnboundScopesForMissingDeviceIdentity` 会针对令牌、密码和受信任代理认证将自声明的范围清除为空集。该连接在显式信任路径上是被允许的，但受范围限制的方法会失败。唯一的例外是具有 `allowInsecureAuth` 的本地控制 UI 令牌/密码会话，它们会保留范围。对于其他情况，仅将 `gateway.controlUi.dangerouslyDisableDeviceAuth=true` 设置为一种应急的范围保留路径。
...
- 首选签名负载是 `v3`，它除了设备/客户端/角色/范围/令牌/随机数字段外，还绑定了 `platform` 和 `deviceFamily`。
- 为了兼容性，遗留的 `v2` 签名仍然被接受，但在重新连接时，配对设备的元数据固定仍然控制着命令策略。

## TLS + 固定（pinning）

- WS 连接支持 TLS。
- 客户端可以选择固定 Gateway 证书指纹（请参阅 `gateway.tls` 配置以及 `gateway.remote.tlsFingerprint` 或 CLI CLI `--tls-fingerprint`）。

## 范围（Scope）

此协议暴露了 **完整的 Gateway API**（状态、频道、模型、聊天、代理、会话、节点、审批等）。确切的范围由 `src/gateway/protocol/schema.ts` 中的 TypeBox schema 定义。
