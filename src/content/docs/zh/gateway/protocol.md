...
有关完整的操作员范围模型、批准时检查以及共享密钥语义，请参阅[操作员范围](/zh/gateway/operator-scopes)。
...
<AccordionGroup>
  ...
  <Accordion title="自动化、Skills 和工具">
    - 自动化：`wake` 调度立即或下一次心跳唤醒文本注入；`cron.get`、`cron.list`、`cron.status`、`cron.add`、`cron.update`、`cron.remove`、`cron.run`、`cron.runs` 管理调度工作。
...
- `chat`：UI 聊天更新，例如 `chat.inject` 以及其他仅针对文本记录的聊天事件。在协议 v4 中，增量载荷携带 `deltaText`；`message` 仍然是累积的助手快照。非前缀替换设置 `replace=true` 并使用 `deltaText` 作为替换文本。
- `session.message` 和 `session.tool`：已订阅会话的文字记录/事件流更新。
...
`TaskSummary` 包含 `id`、`status` 以及可选的元数据，例如 `kind`、
...
- 运营商可以调用 `tools.invoke` (`operator.write`) 通过与 `/tools/invoke` 相同的网关策略路径来调用一个可用的工具。
  - `name` 是必需的。`args`、`sessionKey`、`agentId`、`confirm` 和 `idempotencyKey` 是可选的。
  - 如果同时存在 `sessionKey` 和 `agentId`，则解析出的会话代理必须匹配
    `agentId`。
  - 响应是一个面向 SDK 的信封，包含 `ok`、`toolName`、可选的 `output` 和类型化的 `error` 字段。批准或策略拒绝会在有效载荷中返回 `ok:false`，而不是绕过网关工具策略管道。
...
- 操作员可以调用 `skills.upload.begin`、`skills.upload.chunk` 和
  `skills.upload.commit` (`operator.admin`) 来暂存私有技能档案
  以便在安装前进行准备。这是一条供受信任客户端使用的独立管理员上传路径，
  而不是正常的 ClawHub 技能安装流程，并且默认处于禁用状态，除非
  `skills.install.allowUploadedArchives` 已启用。
  - `skills.upload.begin({ kind: "skill-archive", slug, sizeBytes, sha256?, force?, idempotencyKey? })`
    创建一个绑定到该 slug 和 force 值的上传。
  - `skills.upload.chunk({ uploadId, offset, dataBase64 })` 在精确的解码偏移量处
    追加字节。
  - `skills.upload.commit({ uploadId, sha256? })` 验证最终大小和
    SHA-256。Commit 仅完成上传；它不会安装该 skill。
  - 上传的技能档案是包含 `SKILL.md` 根目录的 zip 档案。档案的内部目录名称从不决定安装目标。
- 操作员可以三种模式调用 `skills.install` (`operator.admin`)：
  ...
  - Upload mode: `{ source: "upload", uploadId, slug, force?, sha256?, timeoutMs? }`
    将已提交的上传内容安装到默认代理工作区 `skills/<slug>`
    目录中。slug 和 force 值必须与原始
    `skills.upload.begin` 请求匹配。除非启用了
    `skills.install.allowUploadedArchives`，否则将拒绝此模式。此设置不影响
    ClawHub 安装。
...
- 如果省略或为 `"default"`：当前运行时行为。如果配置了 `agents.defaults.models`，响应为允许的目录，包括 `provider/*`Gateway(网关) 条目的动态发现的模型。否则响应为完整的 Gateway(网关) 目录。
- `"configured"`：选择器大小的行为。如果配置了 `agents.defaults.models`，它仍然优先，包括对 `provider/*` 条目的提供商范围发现。如果没有允许列表，响应使用显式的 `models.providers.*.models` 条目，仅当不存在配置的模型行时才回退到完整目录。
- `"all"`Gateway(网关)：完整的 Gateway(网关) 目录，绕过 `agents.defaults.models`。将此用于诊断和发现 UI，而非正常的模型选择器。

## 执行批准

- 当执行请求需要批准时，Gateway(网关)会广播 `exec.approval.requested`。
- 操作员客户端通过调用 `exec.approval.resolve`（需要 `operator.approvals` 范围）进行解析。
- 对于 `host=node`，`exec.approval.request` 必须包含 `systemRunPlan`（规范 `argv`/`cwd`/`rawCommand`/会话元数据）。缺少 `systemRunPlan` 的请求将被拒绝。
- 批准后，转发的 `node.invoke system.run` 调用将重用该规范 `systemRunPlan` 作为权威的 command/cwd/会话 上下文。
- 如果调用者在 prepare 和最终批准的 `system.run` 转发之间更改了 `command`、`rawCommand`、`cwd`、`agentId` 或
  `sessionKey`，网关将拒绝运行，而不是信任被更改的负载。
...
- 最终的 `agent` 结果在请求传送时可能包含 `result.deliveryStatus`，使用与 [`openclaw agent --json --deliver`](/zh/cli/agent#json-delivery-status) 中记录的相同的 `sent`、`suppressed`、`partial_failed` 和 `failed` 状态。
...
| 预认证 / 连接质询超时                 | `15_000` 毫秒                                       | `src/gateway/handshake-timeouts.ts`（配置/环境变量可以提高配对的服务器/客户端预算） |
...
| 心跳超时关闭                          | 当静默超过 `tickIntervalMs * 2` 时，关闭代码 `4000` | `src/gateway/client.ts`                                                             |
...
- Private-ingress `gateway.auth.mode: "none"` 完全跳过 shared-secret 连接身份验证；请勿在公共/不受信任的入口上暴露该模式。
- 配对后，Gateway(网关) 会颁发一个针对连接角色 + 范围的 **device token**。该令牌在 `hello-ok.auth.deviceToken` 中返回，客户端应将其持久化存储以供未来连接使用。
- 客户端应在任何成功连接后持久化主 `hello-ok.auth.deviceToken`。
- 使用该**已存储**设备令牌重新连接也应重用为该令牌存储的已批准范围集。这保留了已授予的读取/探测/状态访问权限，并避免将重新连接静默地收窄为隐式仅管理员范围。
- 客户端连接认证组装（位于 `src/gateway/client.ts` 中的 `selectConnectAuth`）：
  - `auth.password` 是正交的，并且在设置时始终被转发。
  - `auth.token` 按优先级顺序填充：首先是显式共享令牌，然后是显式 `deviceToken`，最后是存储的每设备令牌（由 `deviceId` + `role` 键控）。
  - 仅当上述所有方式均未解析出 `auth.token` 时，才会发送 `auth.bootstrapToken`。共享令牌或任何已解析的设备令牌都会阻止发送它。
  - 在一次性的 `AUTH_TOKEN_MISMATCH` 重试中，存储的设备令牌的自动提升仅限于 **受信任的端点** —— 环回，或者带有固定 `tlsFingerprint` 的 `wss://`。没有固定的 `wss://` 不符合条件。
- 额外的 `hello-ok.auth.deviceTokens` 条目是引导交接令牌。
  仅当连接使用了受信任传输（如 `wss://` 或环回/本地配对）上的引导身份验证时，才应持久化这些令牌。
- 如果客户端提供了**显式**的 `deviceToken` 或显式的 `scopes`，则该
  调用方请求的作用域集合保持权威；仅当客户端复用存储的每设备令牌时，才会复用
  缓存的作用域。
- 设备令牌可以通过 `device.token.rotate` 和
  `device.token.revoke` 进行轮换/撤销（需要 `operator.pairing` 作用域）。
- `device.token.rotate` 返回轮换元数据。它仅在已使用该设备令牌进行身份验证的同一设备调用中回显替换后的持有者令牌，以便仅使用令牌的客户端可以在重新连接之前保存其替换令牌。共享/管理员轮换不会回显持有者令牌。
- 令牌的颁发、轮换和撤销受限于该设备配对条目中记录的已批准角色集；令牌变更不能扩展或针对配对批准从未授予的设备角色。
- 对于已配对设备的令牌会话，设备管理默认处于自限定范围，除非调用者还拥有 `operator.admin`：非管理员调用者只能移除/撤销/轮换其**自己**的设备条目。
- `device.token.rotate` 和 `device.token.revoke` 还会检查目标操作员令牌的范围集与调用者当前会话的范围集。非管理员调用者轮换或撤销的操作员令牌范围不能大于其当前持有的范围。
- 身份验证失败包括 `error.details.code` 以及恢复提示：
  - `error.details.canRetryWithDeviceToken`（布尔值）
  - `error.details.recommendedNextStep` (`retry_with_device_token`, `update_auth_configuration`, `update_auth_credentials`, `wait_then_retry`, `review_auth_configuration`)
- 针对 `AUTH_TOKEN_MISMATCH` 的客户端行为：
  - 受信任的客户端可以使用缓存的各种设备令牌尝试一次有界重试。
  - 如果重试失败，客户端应停止自动重连循环并向操作员显示操作指导。
- `AUTH_SCOPE_MISMATCH` 表示设备令牌已被识别，但不覆盖所请求的角色/范围。客户端不应将其显示为错误的令牌；应提示操作员重新配对或批准更窄/更广的范围合约。

## 设备身份 + 配对

- 节点应包含一个稳定的设备标识 (`device.id`)，该标识源于密钥对的指纹。
- 网关会为每个设备和角色颁发令牌。
- 除非启用了本地自动批准，否则新的设备 ID 需要配对批准。
- 配对自动批准以直接本地回环连接为中心。
- OpenClaw 还有一条狭窄的后端/容器本地自连接路径，用于受信任的共享密钥辅助流。
- 同主机 tailnet 或局域网连接在配对时仍被视为远程连接，并需要批准。
- WS 客户端通常在 `connect` 期间包含 `device` 身份（operator + node）。唯一的无设备 operator 例外是显式信任路径：
  - `gateway.controlUi.allowInsecureAuth=true` 用于仅限本地的不安全 HTTP 兼容。
  - 成功的 `gateway.auth.mode: "trusted-proxy"` operator 控制台 UI 身份验证。
  - `gateway.controlUi.dangerouslyDisableDeviceAuth=true` （break-glass，严重的安全降级）。
  - 直接环回 `gateway-client` 后端 RPC，使用共享的网关令牌/密码进行身份验证。
...
- 首选签名载荷是 `v3`，除了设备/客户端/角色/范围/令牌/随机数字段外，它还绑定了 `platform` 和 `deviceFamily`。
- 传统的 `v2` 签名仍然被接受以保持兼容性，但配对设备元数据固定仍然在重连时控制命令策略。
...
该协议暴露了 **完整的 Gateway(网关) API**（状态、频道、模型、聊天、代理、会话、节点、审批等）。确切的接口由 TypeBox 模式在 `src/gateway/protocol/schema.ts` 中定义。
