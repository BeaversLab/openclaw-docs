...
- 任何明确允许在沙箱外运行的工具（例如 `tools.elevated`）。
  ...
  - 如果关闭了沙箱隔离，`tools.elevated` 不会改变执行方式（已经在主机上）。请参阅 [提升模式](/en/tools/elevated)。
...
- OpenClaw 会在 `sandbox.ssh.workspaceRoot` 下为每个范围创建一个远程根目录。
- 在创建或重建后首次使用时，OpenClaw 会从本地工作区向该远程工作区进行一次种子同步。
- 之后，`exec`、`read`、`write`、`edit`、`apply_patch`、提示媒体读取以及入站媒体暂存会通过 SSH 直接针对远程工作区运行。
- OpenClaw 不会自动将远程更改同步回本地工作区。

认证材料：

- `identityFile`、`certificateFile`、`knownHostsFile`：使用现有的本地文件并通过 OpenSSH 配置传递它们。
- `identityData`、`certificateData`、`knownHostsData`：使用内联字符串或 SecretRefs。OpenClaw 通过正常的机密运行时快照解析它们，使用 `0600` 将它们写入临时文件，并在 SSH 会话结束时删除它们。
- 如果为同一项目同时设置了 `*File` 和 `*Data`，则对于该 SSH 会话，`*Data` 生效。
...
- `openclaw sandbox recreate` 删除每个作用域的远程根目录，并在下次使用时从本地重新播种。
- SSH 后端不支持浏览器沙箱隔离。
- `sandbox.docker.*` 设置不适用于 SSH 后端。

### OpenShell 后端

当您希望 OpenClaw 在由 OpenShell 管理的远程环境中隔离工具时，请使用 `backend: "openshell"`。有关完整的设置指南、配置参考和工作区模式比较，请参阅专用的 [OpenShell 页面](/en/gateway/openshell)。
...
- `mirror`（默认）：本地工作区保持权威。OpenClaw 在执行前将本地文件同步到 OpenShell 中，并在执行后将远程工作区同步回来。
- `remote`：创建沙箱后，OpenShell 工作区即为权威。OpenClaw 从本地工作区向远程工作区播种一次，然后文件工具和执行直接针对远程沙箱运行，而不将更改同步回来。
...
- Docker 专用运行时配置（位于 `sandbox.docker.*` 下）仍仅适用于 Docker 后端
...
- 使用 `scope: "agent"` 或 `scope: "shared"` 时，该远程工作区在该相同范围内共享。
...
对于 `mirror` 模式，重新创建主要是重置远程执行环境，因为本地工作区无论如何都保持规范。
...
传入的媒体被复制到活动的沙箱工作区（`media/inbound/*`）中。
Skills 注意：`read` 工具是基于沙箱根目录的。使用 `workspaceAccess: "none"` 时，
OpenClaw 会将符合条件的 Skills 镜像到沙箱工作区（`.../skills`）中，以便
读取它们。使用 `"rw"` 时，可以从 `/workspace/skills` 读取工作区 Skills。
...
- OpenClaw 也会阻止常见的家目录凭据根路径，例如 `~/.aws`、`~/.cargo`、`~/.config`、`~/.docker`、`~/.gnupg`、`~/.netrc`、`~/.npm` 和 `~/.ssh`。
- Bind 验证不仅仅是字符串匹配。OpenClaw 会规范化源路径，然后通过最深的现有祖先再次解析它，然后再检查被阻止的路径和允许的根目录。
- 这意味着即使最终叶子节点尚不存在，通过符号链接父级进行的逃逸仍然会失败并关闭。例如：如果 `run-link` 指向那里，`/workspace/run-link/new-file` 仍然会解析为 `/var/run/...`。
- 允许的源根目录以相同的方式进行规范化处理，因此如果在解析符号链接之前，某个路径仅在允许列表内查看，它仍会被拒绝为 `outside allowed roots`。
- 敏感挂载（机密、SSH 密钥、服务凭据）应 `:ro`，除非绝对必要。
- 如果您只需要对工作区进行读取访问，请结合 `workspaceAccess: "ro"` 使用；bind 模式保持独立。
- 关于 bind 如何与工具策略和提升执行交互，请参阅 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)。
...
注意：默认镜像**不**包含 Node。如果技能需要 Node（或其他运行时），可以烘焙自定义镜像或通过
`sandbox.docker.setupCommand` 安装（需要网络出口 + 可写根目录 +
root 用户）。

如果您想要一个具有常用工具（例如
`curl`、`jq`、`nodejs`、`python3`、`git`）的功能更全面的沙箱镜像，请构建：
...
默认情况下，Docker 沙箱容器在**无网络**的情况下运行。
可以使用 `agents.defaults.sandbox.docker.network` 覆盖。

捆绑的沙盒浏览器镜像还为容器化工作负载应用了保守的 Chromium 启动默认值。当前的容器默认值包括：
...
- 当 `noSandbox` 启用时，`--no-sandbox` 和 `--disable-setuid-sandbox`。
...
- `--disable-extensions` 默认启用，对于依赖扩展的工作流，可以通过
...
如果您需要不同的运行时配置文件，请使用自定义浏览器镜像并提供您自己的入口点。对于本地（非容器）Chromium 配置文件，请使用
...
`OPENCLAW_DOCKER_SOCKET` 覆盖套接字位置。完整的设置和环境参考：[Docker](/en/install/docker#agent-sandbox)。

## setupCommand（一次性容器设置）

`setupCommand` 在沙箱容器创建后运行**一次**（而非每次运行）。
它通过 `sh -lc` 在容器内执行。
...
- `docker.network: "container:<id>"` 需要 `dangerouslyAllowContainerNamespaceJoin: true`，并且仅限紧急情况下使用。
...
- 沙箱执行**不**继承主机 `process.env`。请使用
  `agents.defaults.sandbox.docker.env`（或自定义镜像）来存放技能 API 密钥。

## 工具策略 + 逃生舱口

工具允许/拒绝策略在沙箱规则之前仍然适用。如果某个工具在全局或按代理被拒绝，沙箱隔离也不会将其恢复。

`tools.elevated` 是一个显式的逃生舱口，用于在沙箱外部运行 `exec`（默认为 `gateway`，或者当执行目标是 `node` 时为 `node`）。
`/exec` 指令仅适用于授权发送者并按会话持久化；要彻底禁用
`exec`，请使用工具策略拒绝（请参阅 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)）。

调试：

- 使用 `openclaw sandbox explain` 来检查有效的沙箱模式、工具策略和修复配置键。
- 参阅 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated) 以了解“为什么被阻止？”这一心智模型。
  请保持锁定状态。
...
参阅 [Multi-Agent 沙箱 & Tools](/en/tools/multi-agent-sandbox-tools) 了解优先级。
...
- [OpenShell](/en/gateway/openshell) -- 托管式沙箱后端设置、工作区模式和配置参考
- [沙箱 Configuration](/en/gateway/configuration-reference#agentsdefaultssandbox)
- [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated) -- 调试“为什么被阻止？”
- [Multi-Agent 沙箱 & Tools](/en/tools/multi-agent-sandbox-tools) -- 每个代理的覆盖和优先级
