...
- 任何被明确允许在沙箱之外运行的工具（例如 `tools.elevated`）。
  ...
  - 如果沙箱隔离已关闭，`tools.elevated` 不会改变执行方式（已在主机上）。请参阅[提升模式](/zh/tools/elevated)。
...
如果您将 OpenClaw Gateway(网关) 本身部署为 Docker 容器，它会使用主机的 Docker 套接字（DooD）来编排同级沙箱容器。这引入了一个特定的路径映射约束：

- **配置需要主机路径**：DockerDockerOpenClawGateway(网关)DockerDocker`openclaw.json` `workspace` 配置必须包含**主机的绝对路径**（例如 `/home/user/.openclaw/workspaces`Gateway(网关)OpenClawDockerGateway(网关)OpenClawGateway(网关)），而不是内部 Gateway(网关) 容器路径。当 OpenClaw 请求 Docker 守护进程生成沙箱时，守护进程是相对于主机操作系统命名空间（而非 Gateway(网关) 命名空间）来评估路径的。
- **FS 桥接对等（相同的卷映射）**：OpenClaw Gateway(网关) 原生进程也会将心跳和桥接文件写入 `workspace`Gateway(网关)Gateway(网关) 目录。由于 Gateway(网关) 在其自身的容器化环境中评估的是完全相同的字符串（即主机路径），因此 Gateway(网关) 的部署必须包含一个相同的卷映射，以原生方式链接主机命名空间（`-v /home/user/.openclaw:/home/user/.openclaw`OpenClaw）。

如果您在内部映射路径而没有绝对的主机对等性，OpenClaw 会原生地抛出一个 `EACCES` 权限错误，因为它试图在容器环境中写入其心跳，而该完全限定的路径字符串在原生环境中并不存在。

</Warning>

### SSH 后端

当您希望 OpenClaw 在任意可通过 SSH 访问的机器上对 `exec`、文件工具和媒体读取进行沙箱化处理时，请使用 `backend: "ssh"`OpenClaw。
...
<AccordionGroup>
  <Accordion title="工作原理"OpenClaw>
    ...
    - 在创建或重建后首次使用时，OpenClaw 会从本地工作区将该远程工作区播种一次。
    - 在此之后，`exec`、`read`、`write`、`edit`、`apply_patch`OpenClaw、提示媒体读取和入站媒体暂存将直接通过 SSH 对远程工作区运行。
    - OpenClaw 不会自动将远程更改同步回本地工作区。

  </Accordion>
  <Accordion title="身份验证资料">
    - `identityFile`、`certificateFile`、`knownHostsFile`：使用现有的本地文件并通过 OpenSSH 配置传递它们。
    - `identityData`、`certificateData`、`knownHostsData`OpenClaw：使用内联字符串或 SecretRefs。OpenClaw 通过正常的机密运行时快照解析它们，使用 `0600` 将它们写入临时文件，并在 SSH 会话结束时删除这些文件。
    - 如果为同一项目同时设置了 `*File` 和 `*Data`，则 `*Data` 在该 SSH 会话中优先。

  </Accordion>
  <Accordion title="远程规范化的后果"OpenClaw>
    这是一个 **远程规范化** 模型。在初始播种之后，远程 SSH 工作区将成为真正的沙箱状态。

    - 在播种步骤之后，在 OpenClaw 之外进行的主机本地编辑在重新创建沙箱之前远程不可见。
    - `openclaw sandbox recreate` 会删除每个作用域的远程根目录，并在下次使用时从本地重新播种。
    - SSH 后端不支持浏览器沙箱隔离。
...
当您希望 `backend: "openshell"`OpenClaw 在由 OpenShell 管理的远程环境中对工具进行沙盒化时，请使用它。有关完整的设置指南、配置参考和工作区模式比较，请参阅专用的 [OpenShell page](/zh/gateway/openshell)。
...
- `mirror`OpenClaw（默认）：本地工作区保持权威。OpenClaw 在执行前将本地文件同步到 OpenShell 中，并在执行后将远程工作区同步回来。
- `remote`OpenClaw：创建沙盒后，OpenShell 工作区即为权威。OpenClaw 从本地工作区向远程工作区一次性播种，随后文件工具和执行直接针对远程沙盒运行，而不同步更改。
...
<AccordionGroup>
  <Accordion title="Remote transport details"OpenClaw>
    ...
    - 在 `mirror` 模式下，仅生命周期有所不同：执行前将本地同步到远程，然后在执行后同步回来。
...
<Tabs>
  <Tab title="mirror (local canonical)">
    ...
    - 文件工具仍然通过沙箱桥进行操作，但本地工作区在回合之间仍然是事实来源。

    在以下情况下使用此模式：

    - 您在 OpenClaw 之外本地编辑文件，并希望这些更改自动显示在沙箱中
    - 您希望 OpenShell 沙箱的行为尽可能类似于 Docker 后端
    - 您希望主机工作区在每次执行回合后反映沙箱的写入

    权衡：执行前后有额外的同步开销。

  </Tab>
  <Tab title="remote (OpenShell canonical)">
    当您希望 **OpenShell 工作区成为权威来源** 时，请使用 `plugins.entries.openshell.config.mode: "remote"`OpenClaw。

    行为：

    - 当沙箱首次创建时，OpenClaw 会从本地工作区向远程工作区进行一次播种。
    - 在此之后，`exec`、`read`、`write`、`edit` 和 `apply_patch`OpenClaw 将直接对远程 OpenShell 工作区进行操作。
    - OpenClaw **不会**在执行后将远程更改同步回本地工作区。
    - 提示时的媒体读取仍然有效，因为文件和媒体工具是通过沙箱桥接读取，而不是假设本地主机路径。
    - 传输方式是通过 SSH 连接到由 `openshell sandbox ssh-config`OpenClaw 返回的 OpenShell 沙箱。

    重要后果：

    - 如果您在播种步骤之后，在 OpenClaw 之外的主机上编辑文件，远程沙箱将**不会**自动看到这些更改。
...
对于 `mirror` 模式，重新创建主要会重置远程执行环境，因为本地工作区仍然是权威来源。
...
<Tabs>
  ...
  <Tab title="ro">在 `/agent` 以只读方式挂载代理工作区（禁用 `write`/`edit`/`apply_patch`）。</Tab>
  <Tab title="rw">在 `/workspace` 以读写方式挂载代理工作区。</Tab>
</Tabs>

使用 OpenShell 后端时：

- `mirror` 模式在执行轮次之间仍然使用本地工作区作为权威来源
- `remote` 模式在初始植入后使用远程 OpenShell 工作区作为权威来源
- `workspaceAccess: "ro"` 和 `"none"` 仍然以相同的方式限制写入行为

传入媒体会被复制到活动沙箱工作区（`media/inbound/*`）中。

<Note>**Skills 注意事项：** `read` 工具是沙箱根目录化的。使用 `workspaceAccess: "none"`OpenClaw 时，OpenClaw 会将符合条件的 Skills 镜像到沙箱工作区（`.../skills`）以便读取。使用 `"rw"` 时，可以从 `/workspace/skills` 读取工作区 Skills。</Note>

## 自定义绑定挂载

`agents.defaults.sandbox.docker.binds` 将额外的主机目录挂载到容器中。格式：`host:container:mode`（例如，`"/home/user/source:/source:rw"`）。

全局和每个代理的绑定是 **合并** 的（而不是替换）。在 `scope: "shared"` 下，每个代理的绑定将被忽略。
...
- 绑定会绕过沙箱文件系统：它们会以您设置的任何模式（`:ro` 或 `:rw`OpenClaw）公开主机路径。
- OpenClaw 会阻止危险的绑定源（例如：`docker.sock`、`/etc`、`/proc`、`/sys`、`/dev`OpenClaw 以及会暴露它们的父挂载）。
- OpenClaw 还会阻止常见的主目录凭证根目录，例如 `~/.aws`、`~/.cargo`、`~/.config`、`~/.docker`、`~/.gnupg`、`~/.netrc`、`~/.npm` 和 `~/.ssh`OpenClaw。
- 绑定验证不仅仅是字符串匹配。OpenClaw 会规范化源路径，然后通过最深的现有祖先再次解析它，然后再重新检查受阻路径和允许的根。
- 这意味着即使最终叶节点尚不存在，通过符号链接父级进行的逃逸尝试也会失败（安全关闭）。例如：如果 `run-link` 指向那里，`/workspace/run-link/new-file` 仍将解析为 `/var/run/...`。
- 允许的源根目录以相同方式进行规范化，因此在符号链接解析前仅看起来在允许列表内的路径仍将被拒绝为 `outside allowed roots`。
- 除非绝对必要，否则敏感挂载（机密、SSH 密钥、服务凭证）应该是 `:ro`。
- 如果您只需要对工作区进行读取访问，请结合使用 `workspaceAccess: "ro"`；绑定模式保持独立。
- 有关绑定如何与工具策略和提升执行交互的信息，请参阅 [沙箱 vs Tool Policy vs Elevated](/zh/gateway/sandbox-vs-tool-policy-vs-elevated)。
...
<Steps>
  <Step title="构建默认镜像">
    ...
    当缺少 `openclaw-sandbox:bookworm-slim` 时，OpenClaw 不会静默替换普通的 `debian:bookworm-slim`。针对默认镜像的沙箱运行会快速失败并给出构建说明，直到您构建它，因为捆绑镜像携带了用于沙箱写入/编辑辅助的 `python3`。

  </Step>
  <Step title="可选：构建通用镜像">
    要获取一个包含常用工具（例如 `curl`、`jq`、`nodejs`、`python3`、`git`）的、功能更齐全的沙箱镜像：
    ...
    对于 npm 安装，请先构建默认镜像（见上文），然后使用仓库中的 [`scripts/docker/sandbox/Dockerfile.common`](https://github.com/openclaw/openclaw/blob/main/scripts/docker/sandbox/Dockerfile.common) 在其基础上构建通用镜像。
...
默认情况下，Docker 沙箱容器以**无网络**模式运行。可以使用 Docker`agents.defaults.sandbox.docker.network` 覆盖此设置。
...
Docker 安装和容器化网关位于此处：[Docker](DockerDocker/en/install/docker)

对于 Docker 网关部署，Docker`scripts/docker/setup.sh` 可以引导沙箱配置。设置 `OPENCLAW_SANDBOX=1`（或 `true`/`yes`/`on`）以启用该路径。您可以使用 `OPENCLAW_DOCKER_SOCKET`Docker 覆盖套接字位置。完整设置和环境变量参考：[Docker](/en/install/docker#agent-sandbox)。

## setupCommand（一次性容器设置）

`setupCommand` 在沙箱容器创建后运行**一次**（而非每次运行）。它通过 `sh -lc` 在容器内执行。
...
`tools.elevated` 是一个显式的后门，用于在沙箱之外运行 `exec`（默认为 `gateway`，当执行目标是 `node` 时则为 `node`）。`/exec` 指令仅适用于经过授权的发送者，并在每个会话中持续有效；若要彻底禁用 `exec`，请使用工具策略拒绝（参见 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)）。

调试：

- 使用 `openclaw sandbox explain` 来检查有效的沙箱模式、工具策略以及修复配置键。
- 请参阅 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated) 以了解“为什么被阻止？”的思维模型。

保持锁定状态。

## 多代理覆盖

每个代理可以覆盖沙箱和工具：`agents.list[].sandbox` 和 `agents.list[].tools`（加上用于沙箱工具策略的 `agents.list[].tools.sandbox.tools`）。有关优先级，请参阅 [Multi-Agent 沙箱 & Tools](/en/tools/multi-agent-sandbox-tools)。
