...
- `"docker"`（启用沙箱隔离时的默认设置）：本地由 Docker 支持的沙箱运行时。
...
默认情况下关闭沙箱隔离。如果您启用沙箱隔离且未选择后端，OpenClaw 将使用 Docker 后端。它通过 Docker 守护进程套接字 (`/var/run/docker.sock`) 在本地执行工具和沙箱浏览器。沙箱容器隔离由 Docker 命名空间决定。
