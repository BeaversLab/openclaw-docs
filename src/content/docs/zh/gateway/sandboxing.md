...
- 任何明确允许在主机上运行的工具（例如 `tools.elevated`）。
  ...
  - 如果沙箱隔离已关闭，`tools.elevated` 不会改变执行方式（已在主机上运行）。请参阅[提升模式](/en/tools/elevated)。
...
[OpenShell 页面](/en/gateway/openshell)。
...
- 请参阅 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated) 了解绑定如何与工具策略和提升执行交互。
...
设置 `OPENCLAW_SANDBOX=1`（或 `true`/`yes`/`on`）以启用该路径。你可以
通过 `OPENCLAW_DOCKER_SOCKET` 覆盖 socket 位置。完整的设置和环境
变量参考：[Docker](/en/install/docker#agent-sandbox)。
...
`exec`，请使用工具策略拒绝（参见 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)）。

调试：

- 使用 `openclaw sandbox explain` 来检查有效的沙箱模式、工具策略和修复配置键。
- 有关“为什么被阻止？”的心智模型，请参阅 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)。
  请保持锁定状态。
...
有关优先级，请参阅 [Multi-Agent 沙箱 & Tools](/en/tools/multi-agent-sandbox-tools)。
...
- [OpenShell](/en/gateway/openshell) -- 托管沙箱后端设置、工作区模式和配置参考
