...
- 任何被明确允许在沙箱之外运行的工具（例如 `tools.elevated`）。
  ...
  - 如果沙箱隔离已关闭，`tools.elevated` 不会改变执行方式（已在主机上）。请参阅 [Elevated Mode](/zh/tools/elevated)。
...
当您希望 OpenClaw 在 OpenShell 托管的远程环境中对工具进行沙盒隔离时，请使用 `backend: "openshell"`。有关完整的设置指南、配置参考和工作区模式比较，请参阅专门的 [OpenShell 页面](/zh/gateway/openshell)。
...
- OpenClaw 还会阻止常见的主目录凭证根路径，例如 `~/.aws`、`~/.cargo`、`~/.config`、`~/.docker`、`~/.gnupg`、`~/.netrc`、`~/.npm` 和 `~/.ssh`OpenClaw。
- 绑定验证不仅仅是字符串匹配。OpenClaw 会规范化源路径，然后通过最深的现有祖先再次解析它，然后再检查阻止的路径和允许的根路径。
- 这意味着即使最终的叶子节点尚不存在，符号链接父级逃逸仍然会失败关闭。例如：如果 `run-link` 指向那里，`/workspace/run-link/new-file` 仍然解析为 `/var/run/...`。
- 允许的源根路径以相同方式进行规范化，因此在符号链接解析之前仅看起来在允许列表内的路径仍将被拒绝为 `outside allowed roots`。
- 敏感挂载（机密、SSH 密钥、服务凭证）应该是 `:ro`，除非绝对需要。
- 如果您只需要对工作区进行读取访问，请将其与 `workspaceAccess: "ro"` 结合使用；绑定模式保持独立。
- 请参阅 [沙箱 vs Tool Policy vs Elevated](/zh/gateway/sandbox-vs-tool-policy-vs-elevated) 了解绑定如何与工具策略和 elevated exec 交互。
...
<Steps>
  ...
  <Step title="可选：构建通用镜像">
    若要使用常用工具构建功能更齐全的沙箱镜像（例如 `curl`、`jq`、`nodejs`、`python3`、`git`）：
    ...
    从 npm 安装，请先构建默认镜像（见上文），然后使用仓库中的 [`scripts/docker/sandbox/Dockerfile.common`](https://github.com/openclaw/openclaw/blob/main/scripts/docker/sandbox/Dockerfile.common) 在其之上构建通用镜像。
...
Docker 安装和容器化网关位于此处：[Docker](/en/install/docker)

对于 Docker 网关部署，Docker`scripts/docker/setup.sh` 可以引导沙箱配置。设置 `OPENCLAW_SANDBOX=1`（或 `true`/`yes`/`on`）以启用该路径。您可以使用 `OPENCLAW_DOCKER_SOCKET`Docker 覆盖套接字位置。完整的设置和环境变量参考：[Docker](/en/install/docker#agent-sandbox)。
...
<AccordionGroup>
  <Accordion title="常见陷阱">
    ...
    - `docker.network: "container:<id>"` 需要 `dangerouslyAllowContainerNamespaceJoin: true`，且仅供紧急情况使用。
    - `readOnlyRoot: true` 阻止写入；请设置 `readOnlyRoot: false` 或构建自定义镜像。
    - 为了安装软件包，`user` 必须是 root（省略 `user` 或设置 `user: "0:0"`）。
    - 沙箱执行**不**继承主机 `process.env`。请使用 `agents.defaults.sandbox.docker.env`（或自定义镜像）来存放技能 API 密钥。
...
`tools.elevated` 是一个显式的逃逸机制，用于在沙箱之外运行 `exec`（默认为 `gateway`，或者当执行目标是 `node` 时为 `node`）。`/exec` 指令仅适用于已授权的发送者，并在每个会话中持续存在；若要彻底禁用 `exec`，请使用工具策略拒绝（参见 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)）。

调试：

- 使用 `openclaw sandbox explain` 来检查有效的沙箱模式、工具策略和修复配置键。
- 有关“为什么被阻止？”的思维模型，请参阅 [沙箱 vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)。

保持锁定状态。

## 多代理覆盖

每个代理可以覆盖沙箱和工具设置：`agents.list[].sandbox` 和 `agents.list[].tools`（以及用于沙箱工具策略的 `agents.list[].tools.sandbox.tools`）。有关优先级，请参阅 [多代理沙箱和工具](/en/tools/multi-agent-sandbox-tools)。
...
- [沙箱与工具策略与提权](/zh/gateway/sandbox-vs-tool-policy-vs-elevated) — 调试“为什么这被阻止了？”
