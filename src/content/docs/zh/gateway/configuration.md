...
<AccordionGroup>
  ...
  <Accordion title="设置群组聊天提及控制">
    群组消息默认为**需要提及**。为每个代理配置触发模式，并保持可见的房间回复在默认消息工具路径上，除非您有意想要遗留的自动最终回复：
    ...
    - **可见回复**：`messages.visibleReplies` 可以全局要求消息工具发送；`messages.groupChat.visibleReplies` 会针对群组/渠道覆盖该设置。
