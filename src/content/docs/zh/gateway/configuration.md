...
<AccordionGroup>
  ...
  <Accordion title="设置群聊提及门槛">
    群组消息默认为**需要提及**。请为每个代理配置触发模式，并保持可见的房间回复使用默认的 message-工具 路径，除非您有意希望每个正常的群组回复都使用旧的自动最终回复路径：
    ...
    - **可见回复**：`messages.visibleReplies` 可以全局要求 message-工具 发送；`messages.groupChat.visibleReplies` 会针对群组/渠道覆盖该设置。
    - 有关可见回复模式、逐渠道覆盖和自聊模式，请参阅[完整参考](/zh/gateway/config-channels#group-chat-mention-gating)。
...
doctor --fix` 进行修复。请参阅 [Gateway 故障排除](/en/gateway/troubleshooting#gateway-rejected-invalid-config)
查看检查清单。
