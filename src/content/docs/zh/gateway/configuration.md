...
<Tip>**配置新手？** 从 `openclaw onboard` 开始进行交互式设置，或者查看[配置示例](/en/gateway/configuration-examples)指南获取完整的复制粘贴配置。</Tip>
...
<Tabs>
  ...
  <Tab title="Control UI">打开 [http://127.0.0.1:18789](http://127.0.0.1:18789) 并使用 **Config** 选项卡。 Control UI 根据配置架构渲染一个表单，并提供 **Raw JSON** 编辑器作为备选方案。</Tab>
  <Tab title="Direct edit">直接编辑 `~/.openclaw/openclaw.json`。Gateway(网关) 会监视该文件并自动应用更改（请参阅[热重载](#config-hot-reload)）。</Tab>
</Tabs>

## 严格验证

<Warning>OpenClaw 仅接受完全符合模式的配置。未知的键、格式错误的类型或无效的值会导致 Gateway(网关) **拒绝启动**。唯一的根级例外是 `$schema`（字符串），以便编辑器可以附加 JSON 模式元数据。</Warning>
...
<AccordionGroup>
  <Accordion title="Set up a 渠道 (WhatsApp, Telegram, Discord, etc.)">
  ...
  <Accordion title="选择并配置模型">
    ...
    - 请参阅 [模型 CLI](/en/concepts/models) 以在聊天中切换模型，并参阅 [模型故障转移](/en/concepts/model-failover) 以了解身份轮换和备用行为。
  ...
  <Accordion title="控制谁可以向机器人发送消息">
    ...
    请参阅 [完整参考文档](/en/gateway/configuration-reference#dm-and-group-access) 了解每个渠道的详细信息。
  ...
  <Accordion title="设置群组聊天提及控制">
    ...
    - 请参阅 [完整参考文档](/en/gateway/configuration-reference#group-chat-mention-gating) 了解渠道特定覆盖和自聊模式。
  ...
  <Accordion title="调整网关渠道健康监控">
    ...
    - 使用 `channels.<provider>.healthMonitor.enabled` 或 `channels.<provider>.accounts.<id>.healthMonitor.enabled` 为单个渠道或账户禁用自动重启，而不禁用全局监控器。
    - 参见 [Health Checks](/en/gateway/health) 了解操作调试，以及 [full reference](/en/gateway/configuration-reference#gateway) 了解所有字段。
  ...
  <Accordion title="配置会话和重置">
    ...
    - 参见 [Session Management](/en/concepts/session) 了解范围、身份链接和发送策略。
  ...
  <Accordion title="启用沙箱隔离">
    ...
    参见 [沙箱隔离](/en/gateway/sandboxing) 了解完整指南，以及 [full reference](/en/gateway/configuration-reference#agentsdefaultssandbox) 了解所有选项。
  ...
  <Accordion title="设置心跳（定期检查）">
    ...
    - 查看 [Heartbeat](/en/gateway/heartbeat) 了解完整指南。
  ...
  <Accordion title="配置定时任务">
    ...
    - 查看 [Cron jobs](/en/automation/cron-jobs) 了解功能概述和 CLI 示例。
  ...
  <Accordion title="设置 Webhooks (hooks)">
    ...
    查看 [full reference](/en/gateway/configuration-reference#hooks) 了解所有映射选项和 Gmail 集成。
  ...
  <Accordion title="配置多代理路由">
    ...
    有关绑定规则和每个代理的访问配置文件，请参阅 [Multi-Agent](/en/concepts/multi-agent) 和 [完整参考](/en/gateway/configuration-reference#multi-agent-routing)。
...
<Note>控制平面写入 RPC (`config.apply`、`config.patch`、`update.run`) 受到速率限制，每个 `deviceId+clientIp` 每 60 秒 **3 个请求**。当受到限制时，RPC 返回 `UNAVAILABLE` 并带有 `retryAfterMs`。</Note>
...
SecretRef 的详细信息（包括 `secrets.providers` 用于 `env`/`file`/`exec`）请参阅 [Secrets Management](/en/gateway/secrets)。
