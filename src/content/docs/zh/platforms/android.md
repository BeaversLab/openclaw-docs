...
<Note>
  官方 Android 应用可通过 [Google Play](https://play.google.com/store/apps/details?id=ai.openclaw.app&hl=en_IN) 获取。它是一个伴随节点，需要一个正在运行的 OpenClaw Gateway(网关)。源代码也可在 [OpenClaw 代码库](https://github.com/openclaw/openclaw) 中 `apps/android` 下找到；有关构建说明，请参阅 [apps/android/README.md](https://github.com/openclaw/openclaw/blob/main/apps/android/README.md)。
</Note>
...
- 安装：通过 [Google Play](https://play.google.com/store/apps/details?id=ai.openclaw.app&hl=en_IN) 安装应用，通过 [入门指南](/zh/start/getting-started) 安装 Gateway(网关)，然后进行 [配对](/zh/channels/pairing)。
...
- 在私有 LAN 地址 / `.local` 主机上，加上 `localhost`、`127.0.0.1` 以及 Android 模拟器网桥 (`10.0.2.2`)，仍然支持明文 `ws://`
...
这为 Android 提供了一个安全的 `wss://` / `https://` 端点。仅使用普通的 `gateway.bind: "tailnet"` 设置不足以进行首次远程 Android 配对，除非您另外单独终止 TLS。
...
这会一次性显示 `local.` 以及配置的广域域名，并使用解析后的服务端点，而不仅仅是 TXT 提示。
...
仅靠设备发现不足以进行 tailnet/公网 Android 配对。发现的路由仍然需要一个安全端点（`wss://` 或 Tailscale Serve）：
...
- 打开 **连接** 选项卡。
- 使用 **设置代码** 或 **手动** 模式。
- 如果设备发现被阻止，请在 **Advanced controls** 中使用手动主机/端口。对于私有 LAN 主机，`ws://` 仍然有效。对于 Tailscale/公网主机，请开启 TLS 并使用 `wss://` / Tailscale Serve 端点。
...
在经过身份验证的节点会话连接后，以及当应用移动到后台但前台服务仍处于连接状态时，Android 会调用 `node.event` 并传入 `event: "node.presence.alive"`。网关仅在知晓经过身份验证的节点设备身份后，才会将其记录为配对节点/设备元数据上的 `lastSeenAtMs`/`lastSeenReason`。

仅当网关响应包含 `handled: true` 时，应用才会将信标视为成功记录。旧版网关可能会使用 `{ "ok": true }` 确认 `node.event`；该响应是兼容的，但不被视为持久的上次所见更新。
...
默认情况下禁用此项。它仅适用于没有请求范围的全新 `role: node` 配对。操作员/浏览器配对以及任何角色、范围、元数据或公钥更改仍然需要手动批准。
...
Android 聊天选项卡支持会话选择（默认 Android`main`，加上其他现有会话）：

- 历史记录：`chat.history`（显示标准化；内联指令标签从可见文本中剥离，纯文本工具调用 XML 载荷（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 和截断的工具调用块）以及泄露的 ASCII/全角模型控制令牌被剥离，精确的 `NO_REPLY` / `no_reply` 之类的纯静默令牌助手行被省略，过大的行可以用占位符替换）
- 发送：`chat.send`
...
<Note>节点从 Gateway HTTP 服务器加载画布（与 Gateway(网关)`gateway.port` 相同的端口，默认 `18789`）。</Note>
...
Tailnet（可选）：如果两台设备都在 Tailscale 上，请使用 MagicDNS 名称或 tailnet IP 代替 Tailscale`.local`，例如 `http://<gateway-magicdns>:18789/__openclaw__/canvas/`。
...
- Talk Mode 会在捕获开始前将现有的前台服务从 `dataSync` 提升为 `dataSync|microphone`，然后在 Talk Mode 停止时将其降级。Android 14+ 需要 `FOREGROUND_SERVICE_MICROPHONE` 声明、`RECORD_AUDIO` 运行时授权以及运行时的麦克风服务类型。
...
- 其他 Android 命令系列（可用性取决于设备 + 权限）：
  ...
  - `notifications.list`, `notifications.actions`（参见下面的 [Notification forwarding](#notification-forwarding)）
  - `photos.latest`
...
| `notifications.denyPackages`     | string[]        | 绝不转发来自这些包名称的通知。在 `allowPackages` 之后应用。  |
