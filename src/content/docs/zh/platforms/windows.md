...
- 如果 `schtasks` 本身卡死或停止响应，OpenClaw 现在会快速中止该路径并进行回退，而不是无限期挂起
...
- 远程节点必须指向一个**可访问的** Gateway URL（而非 `127.0.0.1`）；请使用
  `openclaw status --all` 进行确认。
- 使用 `listenaddress=0.0.0.0` 进行局域网访问；`127.0.0.1` 将其保持为仅本地访问。
...
对于 WSL 内的正常首次设置，请遵循 Linux 入门指南流程：
...
我们还没有 Windows 伴侣应用。如果您希望通过贡献来实现它，我们欢迎您的贡献。
