...
如果 Gateway(网关) 反复消失数分钟到数小时，并且只有在您触摸控制 UI 或通过 SSH 登录到主机时才恢复，请参阅关于 macOS 维护睡眠 / macOS`ENETDOWN`Gateway(网关) 崩溃以及 launchd 重生保护门的 [Gateway(网关) 故障排除](/zh/gateway/troubleshooting#macos-gateway-silently-stops-responding-then-resumes-when-you-touch-the-dashboard) 中的说明。
...
该节点报告一个 `permissions` 映射，以便代理可以决定允许执行什么操作。

节点服务 + 应用 IPC：

- 当无头节点主机服务正在运行时（远程模式），它会作为节点连接到 Gateway(网关) WS。
- `system.run`macOS 在 macOS 应用（UI/TCC 上下文）中通过本地 Unix 套接字执行；提示和输出保留在应用内。
...
`system.run`macOS 由 macOS 应用中的 **执行批准**（Settings → Exec approvals）控制。
Security + ask + allowlist 存储在 Mac 本地的以下位置：
...
- `system.run` 环境覆盖项会被过滤（剔除 `PATH`、`DYLD_*`、`LD_*`、`NODE_OPTIONS`、`NODE_REDIRECT_WARNINGS`、`NODE_REPL_EXTERNAL_MODULE`、`NODE_REPL_HISTORY`、`NODE_V8_COVERAGE`、`PYTHON*`、`PERL*`、`RUBYOPT`、`SHELLOPTS`、`PS4`），然后与应用的环境合并。
- 对于 Shell 包装器（`bash|sh|zsh ... -c/-lc`），请求范围的环境覆盖项会缩减为一个小的显式允许列表（`TERM`、`LANG`、`LC_*`、`COLORTERM`、`NO_COLOR`、`FORCE_COLOR`）。
- 对于允许列表模式下的“始终允许”决策，已知的分派包装器（`env`、`nice`、`nohup`、`stdbuf`、`timeout`）会持久化内部可执行文件路径，而不是包装器路径。如果解包不安全，则不会自动持久化允许列表条目。
...
触发 Gateway(网关) Gateway(网关)`agent` 请求。
...
- 如果具有有效的 `key`，则运行是无监控的（旨在用于个人自动化）。

## 新手引导流程（典型）

1. 安装并启动 **OpenClaw.app**。
2. 完成权限检查清单（TCC 提示）。
3. 确保 **Local** 模式处于激活状态，并且 Gateway(网关) 正在运行。
4. 如果需要终端访问，请安装 CLI。

## 状态目录位置（macOS）

避免将 OpenClaw 状态目录放在 iCloud 或其他云同步文件夹中。
同步支持的路径可能会增加延迟，并偶尔导致会话和凭据的文件锁定/同步冲突。
...
使用调试 CLI 来执行与 Gateway(网关) 应用相同的 macOS WebSocket 握手和设备发现逻辑，而无需启动应用。
...
- `--include-local`：包含会被过滤为“local”的网关
- `--timeout <ms>`：整体发现窗口（默认：`2000`）
- `--json`：用于比对的的结构化输出

<Tip>与 `openclaw gateway discover --json` 进行比较，以查看 macOS 应用的发现管道（`local.` 加上配置的广域域名，以及广域域和 Tailscale Serve 备选）是否与 Node CLI 基于 `dns-sd` 的发现有所不同。</Tip>

## 远程连接管道 (SSH 隧道)

当 macOS 应用在 **远程 (Remote)** 模式下运行时，它会打开一个 SSH 隧道，以便本地 UI 组件可以与远程 Gateway(网关) 通信，就像该网关位于 localhost 上一样。

### 控制隧道 (Gateway(网关) WebSocket 端口)

- **用途：** 健康检查、状态、Web 聊天、配置和其他控制平面调用。
- **本地端口：** Gateway(网关) 端口（默认为 `18789`），始终稳定。
- **远程端口：** 远程主机上相同的 Gateway(网关) 端口。
- **行为：** 没有随机本地端口；应用会重用现有的健康隧道，或者在需要时重新启动它。
- **SSH 形状：** 带有 BatchMode +
  ExitOnForwardFailure + keepalive 选项的 `ssh -N -L <local>:127.0.0.1:<remote>`。
- **IP 报告：** SSH 隧道使用环回地址，因此网关会将节点 IP 视为 `127.0.0.1`。如果您希望显示真实的客户端 IP，请使用 **直接 (ws/wss)** 传输（请参阅 [macOS 远程访问](/zh/platforms/mac/remote)）。
