...
- 您仍然可以在 [Gateway(网关) health](/en/gateway/health) (`openclaw status`, `openclaw status --deep`, `openclaw health --json`) 中使用 CLI 流程，并查看 `/tmp/openclaw/openclaw-*.log` 的尾部，以查找 `web-heartbeat` / `web-reconnect`。
