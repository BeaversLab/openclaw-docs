...
- **工作中（代理运行中）：** `AppState.isWorking=true`WebChat 驱动“尾巴/腿部急促移动”的微动作：在工作进行时腿部摆动更快且有轻微偏移。目前围绕 WebChat 代理运行进行切换；当你连接其他长任务时，请添加相同的切换。
...
- 急促移动时腿部摆动幅度最高可达约 1.0，并伴随微小的水平抖动；它会叠加在任何现有的空闲摆动之上。

行为说明

- 没有用于耳朵/工作状态的外部 CLI/代理切换开关；请将其保持在应用程序自身信号的内部，以避免意外的抖动。
- 保持 TTL 较短（&lt;10s），以便在作业挂起时图标快速恢复到基线。

## 相关

- [菜单栏](/zh/platforms/mac/menu-bar)
