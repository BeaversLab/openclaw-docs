...
macOS 可能还会对终端/后台进程的桌面、文档和下载进行限制。如果文件读取或目录列表挂起，请向执行文件操作的同一进程上下文（例如 Terminal/iTerm、LaunchAgent 启动的应用或 SSH 进程）授予访问权限。

变通方法：如果您想避免针对每个文件夹的授权，请将文件移动到 OpenClaw 工作区（`~/.openclaw/workspace`）。

如果您正在测试权限，请务必使用真实证书进行签名。临时签名仅适用于权限无关紧要的快速本地运行。
