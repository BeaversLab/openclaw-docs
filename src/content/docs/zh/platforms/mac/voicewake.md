...
- 触发器仅在唤醒词与下一个词之间存在**有意义的停顿**（约 0.55 秒的间隔）时才会触发。覆盖层/提示音可以在命令开始之前的停顿期间启动。
...
以前，如果覆盖层卡住可见并且您手动关闭了它，语音唤醒可能会显示为“无响应”，因为运行时的重启尝试可能会被覆盖层可见性阻止，并且没有安排随后的重启。
...
- **声音**：在检测到触发和发送时播放提示音；默认为 macOS 的“Glass”系统音效。您可以为每个事件选择任何 macOS`NSSound` 可加载文件（例如 MP3/WAV/AIFF），或选择**无声音**。
