...
1. 登录到 [Oracle Cloud 控制台](https://cloud.oracle.com/)
...
此处的 `gateway.trustedProxies=["127.0.0.1"]` 仅用于本地 Tailscale Serve 代理的转发 IP/本地客户端处理。它**不是** `gateway.auth.mode: "trusted-proxy"`。在此设置中，Diff 查看器路由保持故障关闭行为：没有转发代理标头的原始 `127.0.0.1` 查看器请求可能会返回 `Diff not found`。使用 `mode=file` / `mode=both` 处理附件，或者如果需要可共享的查看器链接，请有意启用远程查看器并设置 `plugins.entries.diffs.config.viewerBaseUrl`（或传递代理 `baseUrl`）。
...
现在一切正常，锁定 VCN 以阻止除 Tailscale 之外的所有流量。OCI 的虚拟云网络充当网络边缘的防火墙——流量在到达您的实例之前被阻止。
...
这将在网络边缘阻止端口 22 上的 SSH、HTTP、HTTPS 以及其他所有内容。从现在起，您只能通过 Tailscale 连接。

---

## 访问控制 UI

从您 Tailscale 网络上的任何设备：
...
- 从您 tailnet 上的任何设备（笔记本电脑、手机等）访问

---

## 安全性：VCN + Tailscale（推荐基线）

由于 VCN 已锁定（仅开放 UDP 41641）且 Gateway(网关) 绑定到环回地址，您获得了强大的纵深防御：公共流量在网络边缘被阻止，而管理员访问通过您的 tailnet 进行。

此设置通常消除了额外基于主机的防火墙规则的*需求*，纯粹是为了阻止全范围的 SSH 暴力破解——但您仍应保持操作系统更新，运行 `openclaw security audit`，并验证您没有意外地在公共接口上监听。

### 已受保护

| 传统步骤       | 需要？     | 原因                                             |
| -------------- | ---------- | ------------------------------------------------ |
| UFW 防火墙     | 否         | VCN 在流量到达实例之前进行拦截                   |
| fail2ban       | 否         | 如果在 VCN 阻止了端口 22，则不存在暴力破解       |
| sshd 加固      | 否         | Tailscale SSH 不使用 sshd                        |
| 禁用 root 登录 | 否         | Tailscale 使用 Tailscale 身份，而不是系统用户    |
| SSH 密钥认证   | 否         | Tailscale 通过您的 tailnet 进行身份验证          |
...
- `~/.openclaw/workspace/` — 工作区（SOUL.md，内存，产物）
