...
1. **进程环境**（Gateway(网关) 网关 进程从父 shell/守护进程已获取的环境）。
2. **当前工作目录中的 `.env`**（dotenv 默认；不覆盖现有值）。
3. 位于 `~/.openclaw/.env` 的 **全局 `.env`**（又名 `$OPENCLAW_STATE_DIR/.env`；不覆盖现有值）。
...
请参阅[配置：环境变量替换](/en/gateway/configuration#env-var-substitution-in-config)了解详情。

## 相关

- [Gateway(网关) 网关配置](/en/gateway/configuration)
- [常见问题：环境变量和 .env 加载](/en/help/faq#env-vars-and-env-loading)
- [模型概览](/en/concepts/models)
