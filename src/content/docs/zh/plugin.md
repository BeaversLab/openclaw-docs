...
插件只是一个**小型代码模块**，它通过额外功能（命令、工具和 Gateway(网关) 网关 RPC）来扩展 OpenClaw。
...
3. 重启 Gateway(网关) 网关，然后在 `plugins.entries.<id>.config` 下进行配置。

有关具体的插件示例，请参阅 [语音通话](/en/plugins/voice-call)。
...
- [语音通话](/en/plugins/voice-call) — `@openclaw/voice-call`
...
OpenClaw 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**，而是使用插件清单和 JSON Schema。请参阅 [插件清单](/en/plugins/manifest)。

插件可以注册：

- Gateway(网关) 网关 RPC 方法
- Gateway(网关) 网关 HTTP 处理程序
- Agent 工具
- CLI 命令
- 后台服务
- 可选配置验证
...
插件与 Gateway(网关) 网关 **同进程** 运行，因此请将其视为受信任的代码。
工具编写指南：[插件代理工具](/en/plugins/agent-tools)。
...
请参阅专用指南：[插件 Agent 工具](/en/plugins/agent-tools)。
...
请参阅 [语音通话](/en/plugins/voice-call) 和 `extensions/voice-call/README.md` 了解设置和用法。
