...
请参阅[Gateway(网关) 配置](/en/gateway/configuration)中的 `gateway.auth`。
...
- 有关令牌偏差修复步骤，请遵循[令牌偏差恢复检查清单](/en/cli/devices#token-drift-recovery-checklist)。
- 从网关主机检索或提供共享密钥：
