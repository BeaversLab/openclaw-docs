...
外观面板保留了内置的 Claw、Knot 和 Dash 主题，以及一个浏览器本地的 tweakcn 导入槽。要导入主题，请打开 [tweakcn 编辑器](https://tweakcn.com/editor/theme)，选择或创建一个主题，点击 **Share**，然后将复制的主题链接粘贴到外观中。导入器还接受 `https://tweakcn.com/r/themes/<id>` 注册表 URL、类似 `https://tweakcn.com/editor/theme?theme=amethyst-haze` 的编辑器 URL、相对 `/themes/<id>` 路径、原始主题 ID 以及默认主题名称，例如 `amethyst-haze`。
...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - 聊天上传接受图片和非视频文件。图片保留原生图片路径；其他文件存储为托管媒体，并在历史记录中显示为附件链接。
    - 使用相同的 `idempotencyKey` 重新发送，在运行时返回 `{ status: "in_flight" }`，完成后返回 `{ status: "ok" }`。
    - `chat.history` 响应为了 UI 安全受到大小限制。当对话记录条目太大时，Gateway(网关) 可能会截断长文本字段，省略繁重的元数据块，并用占位符替换过大的消息 (`[chat.history omitted: message too large]`)。
    - 助手/生成的图片作为托管媒体引用持久保存，并通过经过身份验证的 Gateway(网关) 媒体 URL 返回，因此重新加载不依赖于保留在聊天历史记录响应中的原始 base64 图像负载。
    - 渲染 `chat.history` 时，Control UI 会从可见的助手文本中剥离仅用于显示的内联指令标签（例如 `[[reply_to_*]]` 和 `[[audio_as_voice]]`），纯文本工具调用 XML 负载（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 和截断的工具调用块），以及泄漏的 ASCII/全角模型控制令牌，并省略其整个可见文本仅为精确静默令牌 `NO_REPLY` / `no_reply` 或心跳确认令牌 `HEARTBEAT_OK` 的助手条目。
    - 在活动发送和最终历史记录刷新期间，如果 `chat.history` 短暂返回较旧的快照，聊天视图会保持本地乐观的用户/助手消息可见；一旦 Gateway(网关) 历史记录赶上，标准对话记录将替换这些本地消息。
    - 实时 `chat` 事件是传递状态，而 `chat.history` 是从持久会话对话记录重建的。在工具最终事件之后，Control UI 重新加载历史记录并仅合并一小部分乐观尾部；对话记录边界在 [WebChat](/zh/web/webchat) 中有记录。
    - `chat.inject` 将助手注释附加到会话对话记录，并广播 `chat` 事件以进行仅 UI 更新（无代理运行，无渠道传递）。
    - 聊天标题在会话选择器之前显示代理过滤器，并且会话选择器由所选代理限定范围。切换代理仅显示与该代理关联的会话，并且当该代理尚未保存的仪表板会话时，回退到该代理的主会话。
    - 在桌面宽度上，聊天控件保持在一个紧凑行中，并在向下滚动对话记录时折叠；向上滚动、返回顶部或到达底部会恢复控件。
    - 连续重复的纯文本消息呈现为一个带有计数徽章的气泡。携带图片、附件、工具输出或画布预览的消息不会被折叠。
    - 聊天标题模型和思维选择器通过 `sessions.patch` 立即修补活动会话；它们是持久的会话覆盖，而非仅限单轮的发送选项。
    - 如果在针对同一会话的模型选择器更改仍在保存时发送消息，编辑器将等待该会话修补完成，然后再调用 `chat.send`，以便发送使用所选模型。
    - 在 Control UI 中键入 `/new` 会创建并切换到与“新聊天”相同的全新仪表板会话，除非配置了 `session.dmScope: "main"` 并且当前父级是代理的主会话；在这种情况下，它会原地重置主会话。键入 `/reset` 保留 Gateway(网关) 对当前会话的显式原地重置。
    - 聊天模型选择器请求 Gateway(网关) 的配置模型视图。如果存在 `agents.defaults.models`，该允许列表驱动选择器，包括 `provider/*` 条目，这些条目保持提供商范围的目录动态。否则，选择器显示显式 `models.providers.*.models` 条目加上具有可用身份验证的提供商。完整目录通过带有 `view: "all"` 的调试 `models.list` RPC 保持可用。
    - 当全新的 Gateway(网关) 会话使用报告包含当前上下文令牌时，聊天编辑器区域会显示一个紧凑的上下文使用指示器。它在高上下文压力时切换到警告样式，并在建议的压缩级别显示一个运行正常会话压缩路径的紧凑按钮。过时的令牌快照将被隐藏，直到 Gateway(网关) 再次报告新的使用情况。
...
<Note>Web Push 独立于 iOS APNS 中继路径（请参阅 [Configuration](/zh/gateway/configuration) 了解基于中继的推送）以及现有的 `push.test` 方法，后两者针对的是原生移动端配对。</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Trusted-proxy note">
    - 成功的受信任代理认证可以在没有设备身份的情况下允许 **operator**（操作员）控制 UI 会话。
    - 这**不**适用于具有节点角色的控制 UI 会话。
    - 同主机环回反向代理仍不满足受信任代理认证；请参阅 [Trusted proxy auth](/zh/gateway/trusted-proxy-auth)。

  </Accordion>
</AccordionGroup>

有关 HTTPS 设置指南，请参阅 Tailscale[Tailscale](/zh/gateway/tailscale)。
...
如果浏览器加载了空白仪表板且 DevTools 未显示有用的错误，则可能是扩展程序或早期内容脚本阻止了 JavaScript 模块应用的运行。静态页面包含一个纯 HTML 恢复面板，当启动后未注册 `<openclaw-app>` 时，该面板将会出现。

更改浏览器环境后，请使用面板上的**重试**操作，或在完成这些检查后手动重新加载：

- 禁用会注入到所有页面的扩展程序，尤其是具有 `<all_urls>` 内容脚本的扩展程序。
- 尝试使用隐私窗口、干净的浏览器配置文件或另一个浏览器。
- 保持 Gateway(网关) 运行，并在更改浏览器后验证相同的仪表板 URL。

## 调试/测试：开发服务器 + 远程 Gateway(网关)

Control UI 是静态文件；WebSocket 目标是可配置的，并且可以与 HTTP 源不同。当你在本地需要 Vite 开发服务器但 Gateway(网关) 运行在其他地方时，这非常有用。
...
<AccordionGroup>
  <Accordion title="注意事项">
    ...
    - 只要可能，应通过 URL 片段 (`#token=...`) 传递 `token`。片段不会发送到服务器，从而避免了请求日志和 Referer 泄露。出于兼容性原因，仍会导入一次传统的 `?token=` 查询参数，但这仅作为后备，并且在启动后立即被剥离。
    - `password` 仅保存在内存中。
    ...
    - 仅在顶级窗口（非嵌入式）中接受 `gatewayUrl`，以防止点击劫持。
    - 非环回 Control UI 部署必须显式设置 `gateway.controlUi.allowedOrigins`（完整来源）。这包括远程开发设置。
    - Gateway(网关) 启动可能会根据有效的运行时绑定和端口设定本地来源（如 `http://localhost:<port>` 和 `http://127.0.0.1:<port>`），但远程浏览器来源仍需要显式条目。
    - 不要使用 `gateway.controlUi.allowedOrigins: ["*"]`，除非是严格控制的本地测试。这意味着允许任何浏览器来源，而不是“匹配我正在使用的任何主机”。
...
远程访问设置详情：[远程访问](/zh/gateway/remote)。

## 相关

- [仪表板](/zh/web/dashboard) — 网关仪表板
- [健康检查](/zh/gateway/health) — 网关健康监控
