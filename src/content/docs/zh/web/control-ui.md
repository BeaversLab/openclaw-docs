...
仪表板设置面板会保留当前浏览器标签页会话的令牌
以及所选的 Gateway URL；密码不会被持久化。新手引导通常
会在首次连接时为共享密钥身份验证生成 Gateway 令牌，但当
`gateway.auth.mode` 为 `"password"` 时，密码
身份验证也可以使用。
...
如果浏览器使用更改的身份验证详细信息（角色/范围/公
钥）重试配对，之前的挂起请求将被取代，并创建一个新的 `requestId`。
请在批准之前重新运行 `openclaw devices list`。

一旦获得批准，设备将被记住，除非您
使用 `openclaw devices revoke --device <id> --role <role>` 撤销它，否则不需要重新批准。请参阅
[Devices CLI](/en/cli/devices) 了解令轮换和撤销。

**注意：**

- 直接本地回环浏览器连接（`127.0.0.1` / `localhost`）
  会被自动批准。
...
- 会话：列表 + 每个会话的模型/思考/快速/详细/推理覆盖 (`sessions.list`、`sessions.patch`)
- 梦想：做梦状态、启用/禁用切换以及梦境日记阅读器 (`doctor.memory.status`、`doctor.memory.dreamDiary`、`config.patch`)
...
- 配置写入 (`config.set`/`config.apply`/`config.patch`) 还会对提交的配置负载中的引用进行活动 SecretRef 解析的预检；未解析的活动提交引用将在写入前被拒绝
- 配置架构 + 表单渲染 (`config.schema` / `config.schema.lookup`,
  包括字段 `title` / `description`, 匹配的 UI 提示, 直接子级
  摘要, 嵌套对象/通配符/数组/组合节点上的文档元数据,
  以及可用时的插件 + 渠道架构); 仅当快照具有安全的原始往返时,
  原始 JSON 编辑器才可用
...
- 日志：网关文件日志的实时跟踪，带有筛选/导出 (`logs.tail`) 功能
...
- 设置 `cron.webhookToken` 以发送专用不记名令牌，如果省略，则 webhook 将在不带身份验证标头的情况下发送。
- 已弃用的回退：存储的具有 `notify: true` 的旧版任务在迁移之前仍可使用 `cron.webhook`。

## 聊天行为

- `chat.send` 是 **非阻塞的**：它立即使用 `{ runId, status: "started" }` 进行确认，并且响应通过 `chat` 事件流式传输。
- 使用相同的 `idempotencyKey` 重新发送，在运行时返回 `{ status: "in_flight" }`，完成后返回 `{ status: "ok" }`。
- 出于 UI 安全考虑，`chat.history` 响应受大小限制。当转录条目过大时，Gateway(网关) 可能会截断长文本字段，省略繁重的元数据块，并使用占位符（`[chat.history omitted: message too large]`）替换过大的消息。
- `chat.history` 还会从可见的助手文本中去除仅用于显示的内联指令标签（例如 `[[reply_to_*]]` 和 `[[audio_as_voice]]`）、纯文本工具调用 XML 有效负载（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 和截断的工具调用块）以及泄露的 ASCII/全角模型控制令牌，并省略其整个可见文本仅为精确静默令牌 `NO_REPLY` / `no_reply` 的助手条目。
- `chat.inject` 会将助手笔记附加到会话记录中，并广播 `chat` 事件以进行仅 UI 更新（无代理运行，无渠道传递）。
- 聊天标题中的模型和思维选择器会通过 `sessions.patch` 立即修补活动会话；它们是持久的会话覆盖，而非仅限单次发送的选项。
- 停止：
  - 点击 **停止**（调用 `chat.abort`）
...
将 Gateway(网关) 保持在本地环回，并让 Tailscale Serve 使用 HTTPS 进行代理：
...
默认情况下，当 `gateway.auth.allowTailscale` 为 `true` 时，Control UI/WebSocket Serve 请求可以通过 Tailscale 身份标头（`tailscale-user-login`）进行身份验证。OpenClaw 通过使用 `tailscale whois` 解析 `x-forwarded-for` 地址并将其与标头匹配来验证身份，并且仅当请求通过 Tailscale 的 `x-forwarded-*` 标头到达本地环回时才接受这些请求。如果您想要求显式的共享密钥凭证（即使是 Serve 流量），请设置 `gateway.auth.allowTailscale: false`。然后使用 `gateway.auth.mode: "token"` 或 `"password"`。对于该异步 Serve 身份验证路径，来自同一客户端 IP 和身份验证范围的失败身份验证尝试会在速率限制写入之前进行序列化。因此，来自同一浏览器的并发错误重试可能会在第二个请求上显示 `retry later`，而不是两个普通的匹配错误并行竞争。无令牌 Serve 身份验证假设网关主机是受信任的。如果不受信任的本地代码可能在该主机上运行，请要求令牌/密码身份验证。
...
如果您通过纯 HTTP（`http://<lan-ip>` 或 `http://<tailscale-ip>`）打开仪表板，浏览器将在**非安全上下文**中运行并阻止 WebCrypto。默认情况下，如果没有设备身份，OpenClaw 会**阻止** Control UI 连接。

记录的例外情况：

- 仅限本地主机的不安全 HTTP 与 `gateway.controlUi.allowInsecureAuth=true` 的兼容性
- 通过 `gateway.auth.mode: "trusted-proxy"` 成功进行操作员 Control UI 身份验证
- 打破玻璃 `gateway.controlUi.dangerouslyDisableDeviceAuth=true`
...
- 它允许本地 Control UI 会话在不安全的 HTTP 上下文中无需设备身份即可继续。
- 它不会绕过配对检查。
...
`dangerouslyDisableDeviceAuth` 禁用 Control UI 设备身份检查，会严重降低安全性。紧急使用后请快速恢复。

受信任代理说明：

- 成功的受信任代理身份验证可以允许 **operator** Control UI 会话无需设备身份即可进入
- 这 **不** 延伸到节点角色 Control UI 会话
...
Control UI 是静态文件；WebSocket 目标是可配置的，并且可以与 HTTP 源不同。当您希望本地使用 Vite 开发服务器但 Gateway(网关) 在其他地方运行时，这非常方便。
...
- 只要有可能，应通过 URL 片段（`#token=...`）传递 `token`。片段不会发送到服务器，从而避免了请求日志和 Referer 泄露。出于兼容性考虑，传统的 `?token=` 查询参数仍会被导入一次，但这仅作为后备手段，且会在启动后立即被清除。
- `password` 仅保存在内存中。
...
- 非环回的 Control UI 部署必须显式设置 `gateway.controlUi.allowedOrigins`
  （完整的源）。这包括远程开发设置。
- 除非进行严格控制下的
  本地测试，否则不要使用 `gateway.controlUi.allowedOrigins: ["*"]`。这意味着允许任何浏览器源，而不是“匹配我正在使用的任何主机”。
- `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` 启用了
  Host 头部源回退模式，但这是一种危险的安全模式。
