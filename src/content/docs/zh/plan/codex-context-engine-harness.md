...
使捆绑的 Codex app-server harness 遵守与嵌入式 OpenClaw turns 已遵守的相同的 OpenClawOpenClaw 上下文引擎生命周期合约。

使用 提供商/模型 `agentRuntime.id: "codex"` 或 `codex/*` 模型的会话，仍应允许所选的上下文引擎插件（例如 `lossless-claw`）控制上下文组装、轮次后摄取、维护以及 OpenClaw 级别的压缩策略，其限制范围由 Codex app-server 边界决定。
...
如今可复用的生命周期辅助工具位于嵌入式代理运行器之下：

- `src/agents/embedded-agent-runner/run/attempt.context-engine-helpers.ts`
- `src/agents/embedded-agent-runner/run/attempt.prompt-helpers.ts`
- `src/agents/embedded-agent-runner/context-engine-maintenance.ts`

Codex 应导入与 harness 无关的辅助工具，而不是深入探究运行器的实现细节。
...
此顺序让通用提示钩子能看到 Codex 将接收的相同提示。如果我们需要严格的 OpenClaw 一致性，请在钩子组合之前运行上下文引擎组装，因为内置 harness 在其提示管道之后将上下文引擎 `systemPromptAddition` 应用于最终系统提示。重要的不变量是上下文引擎和钩子都获得确定性的、有文档记录的顺序。
...
- 配置 提供商/模型 `agentRuntime.id = "codex"`
- 断言测试引擎已观察到：
  - bootstrap
  - assemble
  - afterTurn 或 ingest
...
3. `before_prompt_build` 应该在上下文引擎组装之前还是之后运行？
   ...
   建议：在为 Codex 进行上下文引擎投影之后，以便通用
   挂钩能够看到 Codex 将接收的实际提示/开发者指令。如果
   内置线束奇偶校验要求相反的顺序，请在测试中
   对所选顺序进行编码并在此处记录。
