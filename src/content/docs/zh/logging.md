...
如果隐式本地 loopback Gateway(网关) 请求配对、在连接期间关闭，
或者在 `logs.tail` 回答之前超时，`openclaw logs` 将自动回退到
配置的 Gateway(网关) 文件日志。显式 `--url` 目标不使用
此回退。
...
控制 UI 的 **Logs（日志）** 选项卡使用 `logs.tail` 实时追踪同一个文件。
...
- `agent_id`：当日志调用携带代理上下文时的活动代理 ID。
...
所有日志记录配置均位于 `~/.openclaw/openclaw.json` 中的 `logging` 之下。
...
您可以通过 **`OPENCLAW_LOG_LEVEL`** 环境变量（例如 `OPENCLAW_LOG_LEVEL=debug`）覆盖这两者。环境变量的优先级高于配置文件，因此您可以在不编辑 `openclaw.json`CLI 的情况下为单次运行提高详细程度。您还可以传递全局 CLI 选项 **`--log-level <level>`**（例如，`openclaw --log-level debug gateway run`），这将覆盖该命令的环境变量。
...
- `OPENCLAW_DEBUG_MODEL_PAYLOAD=summary`：在模型请求日志中包含有界的请求负载摘要。
- `OPENCLAW_DEBUG_MODEL_PAYLOAD=tools`：在负载摘要中包含所有面向模型的工具名称。
- `OPENCLAW_DEBUG_MODEL_PAYLOAD=full-redacted`：包含经过编辑和限制的 JSON 负载快照。仅在调试时使用；机密信息已被编辑，但提示和消息文本可能仍然存在。
- `OPENCLAW_DEBUG_SSE=events`：发出首个事件和流完成的时间。
- `OPENCLAW_DEBUG_SSE=peek`：同时发出前五个经过编辑的 SSE 事件负载，每个事件均有限制。
- `OPENCLAW_DEBUG_CODE_MODE=1`：发出代码模式模型表面诊断信息，包括当原生提供商工具因代码模式拥有工具表面而被隐藏时的情况。

这些标志通过正常的 OpenClaw 日志进行记录，因此 OpenClaw`openclaw logs --follow` 和控制 UI 日志选项卡会显示它们。如果没有这些标志，相同的诊断信息仍可在 `debug` 级别获取。

### 追踪关联

文件日志是 JSONL 格式。当日志调用携带有效的诊断追踪上下文时，OpenClaw 会将追踪字段作为顶级 JSON 键（OpenClaw`traceId`、`spanId`、`parentSpanId`、`traceFlags`）写入，以便外部日志处理器可以将该行与 OTEL 跨度和提供商 `traceparent` 传播相关联。

Gateway HTTP 请求和 Gateway WebSocket 帧建立一个内部请求追踪范围。在该异步范围内发出的日志和诊断事件，如果未传递显式追踪上下文，则会继承请求追踪。Agent 运行和模型调用追踪将成为活动请求追踪的子级，因此本地日志、诊断快照、OTEL 跨度和受信任的提供商 Gateway(网关)Gateway(网关)`traceparent` 标头可以通过 `traceId` 进行关联，而无需记录原始请求或模型内容。
...
- `logging.redactPatterns`：用于覆盖默认集的正则表达式字符串列表。自定义模式在控制 UI 工具负载的内置默认值之上应用，因此添加模式绝不会削弱已被默认值捕获的值的编辑力度。
...
`logging.redactSensitive: "off"`OpenClaw 仅禁用此通用日志/记录策略。OpenClaw 仍然会编辑可以被 UI 客户端、支持包、诊断观察者、批准提示或代理工具显示的安全边界负载。示例包括 Control UI 工具调用事件、`sessions_history`Gateway(网关) 输出、诊断支持导出、提供商错误观察、exec 批准命令显示以及 Gateway WebSocket 协议日志。自定义 `logging.redactPatterns` 仍可以在这些表面上添加项目特定的模式。
...
- **OpenTelemetry 导出** — 通过 OTLP/HTTP 将指标、追踪和日志发送到任何兼容 OpenTelemetry 的收集器或后端（Grafana、Datadog、Honeycomb、New Relic、Tempo 等）。完整配置、信号目录、指标/span 名称、环境变量和隐私模型位于专用页面：[OpenTelemetry 导出](/zh/gateway/opentelemetry)。
- **诊断标志** — 有针对性的调试日志标志，用于将额外的日志路由到 `logging.file` 而不提高 `logging.level`。标志不区分大小写并支持通配符（`telegram.*`、`*`）。在 `diagnostics.flags` 下配置或通过 `OPENCLAW_DIAGNOSTICS=...` 环境变量覆盖。完整指南：[诊断标志](/zh/diagnostics/flags)。
...
要通过 OTLP 导出到收集器，请参阅 [OpenTelemetry 导出](/zh/gateway/opentelemetry)。

## 故障排除提示

- **无法连接 Gateway？** 首先运行 Gateway(网关)`openclaw doctor`。
- **日志为空？** 检查 Gateway 是否正在运行并写入 Gateway(网关)`logging.file` 中的文件路径。
- **需要更多详细信息？** 将 `logging.level` 设置为 `debug` 或 `trace` 并重试。

## 相关

- [OpenTelemetry 导出](/zh/gateway/opentelemetry) — OTLP/HTTP 导出、指标/span 目录、隐私模型
- [诊断标志](/zh/diagnostics/flags) — 针对性的调试日志标志
- [Gateway(网关) 日志内部机制](/zh/gateway/logging) — WS 日志样式、子系统前缀和控制台捕获
