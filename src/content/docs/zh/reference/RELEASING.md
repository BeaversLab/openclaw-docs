...
- 当您仅需要对发布候选版本进行完整的常规 CI
  覆盖时，直接运行手动 `CI` 工作流。手动 CI 调度会绕过变更范围界定，并强制执行 Linux Node 分片、bundled-plugin 分片、渠道
  契约、Node 22 兼容性、`check`、`check-additional`、构建冒烟测试、
  文档检查、Python 技能、Windows、macOS、Android 和 Control UI i18n
  通道。
...
对于有界恢复，请将 `rerun_group` 传递给 umbrella。`all` 是实际的候选发布运行，`ci` 仅运行正常的 CI 子项，`plugin-prerelease` 仅运行仅发布的插件子项，`release-checks` 运行每个发布 box，而较窄的发布组是 `install-smoke`、`cross-os`、`live-e2e`、`package`、`qa`、`qa-parity`、`qa-live` 和 `npm-telegram`。有针对性的 `npm-telegram` 重跑需要 `release_package_spec` 或 `npm_telegram_package_spec`；带有 `release_profile=full` 的完整/全部运行使用 release-checks package 构件。有针对性的跨 OS 重跑可以添加 `cross_os_suite_filter=windows/packaged-upgrade` 或其他 OS/套件过滤器。QA release-check 失败是建议性的；仅 QA 的失败不会阻止发布验证。

### Vitest

Vitest box 是手动 `CI`Linux 子工作流。手动 CI 故意绕过更改范围，并强制为候选发布执行正常的测试图：Linux Node 分片、bundled-plugin 分片、渠道 契约、Node 22 兼容性、`check`、`check-additional`WindowsmacOSAndroid、构建冒烟测试、文档检查、Python 技能、Windows、macOS、Android 和 Control UI i18n。
