...
- Skills 列表（仅元数据；指令通过 `read` 按需加载）。
  Native Codex 轮次接收紧凑的 skills 块作为轮次范围的协作开发者指令；其他 harness 在正常提示表面接收它。其受 `skills.limits.maxSkillsPromptChars` 限制，并可通过 `agents.list[].skillsLimits.maxSkillsPromptChars` 进行可选的每个代理覆盖。
- 自我更新指令
- 工作区 + 引导文件（`AGENTS.md`、`SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、`HEARTBEAT.md`、`BOOTSTRAP.md`（如果是新的），外加 `MEMORY.md`（如果存在））。Native Codex 轮次不会从配置的代理工作区粘贴原始 `MEMORY.md`（当该工作区有可用的内存工具时）；它们在轮次范围的协作开发者指令中包含一个小的内存指针，并按需使用内存工具。如果工具被禁用、内存搜索不可用，或者活动工作区与代理内存工作区不同，`MEMORY.md` 将使用正常的有界轮次上下文路径。小写的根 `memory.md` 不会被注入；当与 `MEMORY.md` 配对时，它是 `openclaw doctor --fix` 的遗留修复输入。大型注入文件会被 `agents.defaults.bootstrapMaxChars` 截断（默认：12000），并且总引导注入量受 `agents.defaults.bootstrapTotalMaxChars` 上限限制（默认：60000）。`memory/*.md` 每日文件不是正常引导提示的一部分；它们在普通轮次中通过内存工具保持按需调用，但在重置/启动模型运行时，可以在第一轮前置一个包含最近每日内存的一次性启动上下文块。裸聊天 `/new` 和 `/reset` 命令会被确认而不调用模型。启动前奏由 `agents.defaults.startupContext` 控制。压缩后的 AGENTS.md 摘录是分开的，并且需要明确的 `agents.defaults.compaction.postCompactionSections` 选择加入。
- 时间 (UTC + 用户时区)
- 回复标签 + 心跳行为
