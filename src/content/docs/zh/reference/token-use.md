...
- 自我更新指令
- 工作区 + 引导文件（新建时包括 `AGENTS.md`、`SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、`HEARTBEAT.md`、`BOOTSTRAP.md`，如果存在还包括 `MEMORY.md`）。原生 Codex 轮次不会在针对该工作区提供记忆工具时，从配置的代理工作区粘贴原始 `MEMORY.md`；它们会在轮次范围的协作开发者指令中包含一个小的记忆指针，并按需使用记忆工具。如果工具被禁用、记忆搜索不可用，或者活动工作区与代理记忆工作区不同，`MEMORY.md` 将使用正常的有界轮次上下文路径。小写根 `memory.md` 不会被注入；它是 `openclaw doctor --fix` 与 `MEMORY.md` 配对时的遗留修复输入。大型注入文件会被 `agents.defaults.bootstrapMaxChars` 截断（默认值：20000），且总引导注入量受 `agents.defaults.bootstrapTotalMaxChars` 限制（默认值：60000）。`memory/*.md` 每日文件不是正常引导提示的一部分；它们在普通轮次中通过记忆工具按需保留，但重置/启动模型运行可以在第一轮前置包含近期每日记忆的一次性启动上下文块。纯聊天 `/new` 和 `/reset` 命令在不调用模型的情况下被确认。启动前序由 `agents.defaults.startupContext` 控制。压缩后的 AGENTS.md 摘录是分开的，需要明确选择加入 `agents.defaults.compaction.postCompactionSections`。
- 时间 (UTC + 用户时区)
- 回复标签 + 心跳行为
