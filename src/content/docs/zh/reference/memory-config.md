...
- [内置引擎](/en/concepts/memory-builtin) -- 默认的 SQLite 后端
- [QMD 引擎](/en/concepts/memory-qmd) -- 本地优先的 sidecar
- [内存搜索](/en/concepts/memory-search) -- 搜索管道和调优
- [活动内存](/en/concepts/active-memory) -- 为交互式会话启用内存子代理
...
请参阅 [活动内存](/en/concepts/active-memory) 了解激活模型、
...
有关概念行为和斜杠命令，请参阅 [梦境](/en/concepts/dreaming)。
