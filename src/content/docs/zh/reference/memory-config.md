...
- [QMD 引擎](/en/concepts/memory-qmd) -- 本地优先的附属进程
- [内存搜索](/en/concepts/memory-search) -- 搜索管道和调优

除非另有说明，所有内存搜索设置均位于 `agents.defaults.memorySearch` 中的
`openclaw.json` 之下。
...
常青文件（`MEMORY.md`，`memory/` 中的无日期文件）永远不会衰减。
...
路径可以是绝对路径或相对于工作区的路径。目录将被递归扫描以查找 `.md` 文件。符号链接的处理取决于活动的后端：内置引擎忽略符号链接，而 QMD 遵循底层 QMD 扫描器行为。

对于代理范围的跨代理转录本搜索，请使用
`agents.list[].memorySearch.qmd.extraCollections` 而不是 `memory.qmd.paths`。
这些额外的集合遵循相同的 `{ path, name, pattern? }` 形状，但
...
仅适用于 `extraPaths` 中的文件。默认内存根目录保持仅限 Markdown。
需要 `gemini-embedding-2-preview`。`fallback` 必须是 `"none"`。
...
会话索引是可选的，并异步运行。结果可能会略有延迟。
会话日志位于磁盘上，因此请将文件系统访问视为信任边界。
...
OpenClaw 优先使用当前的 QMD 集合和 MCP 查询形状，但通过回退到旧版 `--mask` 集合标志和较旧的 MCP 工具名称，使旧的 QMD 版本能够继续工作。

QMD 模型覆盖保留在 QMD 一侧，而不是 OpenClaw 配置中。如果您需要全局覆盖 QMD 的模型，请在网关运行时环境中设置环境变量，例如 `QMD_EMBED_MODEL`、`QMD_RERANK_MODEL` 和 `QMD_GENERATE_MODEL`。
...
默认为仅私信。`match.keyPrefix` 匹配标准化的会话键；
`match.rawKeyPrefix` 匹配包括 `agent:<id>:` 在内的原始键。
...
梦境是在 `plugins.entries.memory-core.config.dreaming` 下配置的，
而不是在 `agents.defaults.memorySearch` 下。

梦境作为一次计划的扫描运行，并使用内部的浅睡/深睡/REM 阶段作为
一个实现细节。

有关概念性行为和斜杠命令，请参阅 [梦境](/en/concepts/dreaming)。
...
- 梦境将人类可读的叙述输出写入 `DREAMS.md`（或现有的 `dreams.md`）。
