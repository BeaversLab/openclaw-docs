...
- [内存概述](/zh/concepts/memory) -- 内存的工作原理
- [内置引擎](/zh/concepts/memory-builtin) -- 默认的 SQLite 后端
- [QMD 引擎](/zh/concepts/memory-qmd) -- 本地优先的 sidecar
- [内存搜索](/zh/concepts/memory-search) -- 搜索管道和调优
- [活动内存](/zh/concepts/active-memory) -- 为交互式会话启用内存子代理
...
请参阅 [活动内存](/zh/concepts/active-memory) 了解激活模型、
插件拥有的配置、对话记录持久化和安全推出模式。
...
2. `github-copilot` -- 如果可以解析 GitHub Copilot 令牌（环境变量或身份验证配置文件）。
3. `openai` -- 如果可以解析 OpenAI 密钥。
4. `gemini` -- 如果可以解析 Gemini 密钥。
5. `voyage` -- 如果可以解析 Voyage 密钥。
6. `mistral` -- 如果可以解析 Mistral 密钥。
7. `bedrock` -- 如果 AWS SDK 凭证链解析成功（实例角色、访问密钥、配置文件、SSO、Web 身份或共享配置）。

支持 `ollama` 但不会自动检测（需显式设置）。

### API 密钥解析

远程嵌入需要 API 密钥。Bedrock 改用 AWS SDK 默认
凭证链（实例角色、SSO、访问密钥）。
...
<Warning>更改模型或 `outputDimensionality` 会触发自动完全重新索引。</Warning>
...
所有配置均位于 `memorySearch.query.hybrid` 下：
...
| `candidateMultiplier` | `number`  | `4`    | 候选项池大小倍数         |
...
| 键                           | 类型      | 默认值  | 描述            |
| ---------------------------- | --------- | ------- | --------------- |
| `temporalDecay.enabled`      | `boolean` | `false` | 启用近期性增强  |
...
路径可以是绝对路径或相对于工作区的路径。目录将递归扫描 `.md` 文件。
符号链接的处理取决于活动的后端：内置引擎会忽略符号链接，而 QMD 则遵循底层的 QMD
扫描器行为。

对于代理范围的跨代理记录搜索，请使用 `agents.list[].memorySearch.qmd.extraCollections` 而不是 `memory.qmd.paths`。
这些额外的集合遵循相同的 `{ path, name, pattern? }` 结构，但它们按代理合并，并且当路径指向当前
工作区之外时，可以保留显式的共享名称。
...
仅适用于 `extraPaths` 中的文件。默认内存根目录保持仅 Markdown 格式。
...
适用于 `openai`、`gemini` 和 `voyage`。OpenAI 批处理通常是大批量回填最快且最便宜的方式。

---

## 会话内存搜索（实验性）

为会话转录建立索引并通过 `memory_search` 展示它们：
...
会话索引是可选加入（opt-in）的，并且异步运行。结果可能会略有滞后。会话日志存储在磁盘上，因此请将文件系统访问视为信任边界。
...
OpenClaw 优先使用当前的 QMD 集合和 MCP 查询形状，但在必要时会回退到旧版 `--mask` 集合标志和较旧的 MCP 工具名称，以保持旧版 QMD 版本正常工作。

QMD 模型覆盖保留在 QMD 端，而不是 OpenClaw 配置中。如果需要全局覆盖 QMD 的模型，请在网关运行时环境中设置环境变量，例如 `QMD_EMBED_MODEL`、`QMD_RERANK_MODEL` 和 `QMD_GENERATE_MODEL`。
...
| `update.waitForBootSync`  | `boolean` | `false` | 在刷新完成前阻止启动 |
...
控制哪些会话可以接收 QMD 搜索结果。架构与
[`session.sendPolicy`](/zh/gateway/configuration-reference#session) 相同：
...
梦境配置在 `plugins.entries.memory-core.config.dreaming` 下，
而不是在 `agents.defaults.memorySearch` 下。

梦境作为一个计划好的扫描任务运行，并使用内部的浅层/深层/REM 阶段作为
实现细节。

有关概念行为和斜杠命令，请参阅 [梦境](/zh/concepts/dreaming)。
...
- 浅层/深层/REM 阶段策略和阈值是内部行为，而非面向用户的配置。
