...
<Steps>
  ...
  <Step title="Channels">
    ...
    - 私信安全性：默认为配对。第一条私信会发送一个代码；通过 `openclaw pairing approve <channel> <code>` 批准或使用允许列表。
  </Step>
...
<Note>如果未检测到 GUI，新手引导将打印控制 UI 的 SSH 端口转发指令，而不是打开浏览器。 如果控制 UI 资源缺失，新手引导将尝试构建它们；回退方案是 `pnpm ui:build` （自动安装 UI 依赖）。</Note>
...
<Note>`--json` **并不**意味着非交互模式。请使用 `--non-interactive` （以及 `--workspace`） 用于脚本。</Note>
...
- 配置参考：[Gateway(网关) configuration](/en/gateway/configuration)
