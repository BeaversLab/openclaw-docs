...
<Steps>
  <Step title="创建 OCI 实例">
    ...
    <Tip>
    如果实例创建失败并显示“Out of capacity”（容量不足），请尝试不同的可用性域或稍后重试。免费层的容量有限。
    </Tip>
  ...
  <Step title="验证">
    ...
    将 `<tailnet-name>` 替换为您的 tailnet 名称（可在 `tailscale status` 中看到）。
...
- [Channels](/en/channels) -- 连接 Telegram、WhatsApp、Discord 等平台
