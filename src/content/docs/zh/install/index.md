...
- **macOS、Linux 或 Windows** - 同时支持原生 Windows 和 WSL2；WSL2 更稳定。请参阅 [Windows](macOSLinuxWindowsWindowsWSL2WSL2Windows/en/platforms/windows)。
- `pnpm` 仅在从源码构建时需要
...
它默认支持 npm 安装，以及在相同前缀流程下的 git-checkout 安装。完整参考：[Installer internals](/zh/install/installer#install-clish)。
...
<Tabs>
  ...
  <Tab title="pnpm">
    ...
    <Note>
    pnpm 需要对带有构建脚本的软件包进行显式批准。在首次安装后运行 `pnpm approve-builds -g`。
    </Note>
...
或者跳过链接并从仓库内部使用 `pnpm openclaw ...`。有关完整的开发工作流程，请参阅[设置](/zh/start/setup)。
...
如果 `$(npm prefix -g)/bin` 不在你的 `$PATH` 中，请将其添加到你的 shell 启动文件（`~/.zshrc` 或 `~/.bashrc`）中：
