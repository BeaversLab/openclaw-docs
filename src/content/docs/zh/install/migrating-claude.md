...
<AccordionGroup>
  <Accordion title="指令和记忆">- 项目 `CLAUDE.md` 和 `.claude/CLAUDE.md` 内容会被复制或追加到 OpenClaw 代理工作区 `AGENTS.md` 中。 - 用户 `~/.claude/CLAUDE.md` 内容会被追加到工作区 `USER.md` 中。</Accordion>
  <Accordion title="MCP 服务器">MCP 服务器定义将从项目 `.mcp.json`、Claude Code `~/.claude.json` 和 Claude Desktop `claude_desktop_config.json` 导入（如果存在）。</Accordion>
  <Accordion title="Skills and commands">- 带有 `SKILL.md` 文件的 Claude skills 会被复制到 OpenClaw workspace skills 目录中。 - 位于 `.claude/commands/` 或 `~/.claude/commands/` 下的 Claude 命令 Markdown 文件会被转换为带有 `disable-model-invocation: true` 的 OpenClaw skills。</Accordion>
</AccordionGroup>
