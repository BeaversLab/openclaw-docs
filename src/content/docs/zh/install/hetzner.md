...
请参阅 [安全](/zh/gateway/security) 和 [VPS 托管](/zh/vps)。
...
<Steps>
  ...
  <Step title="配置环境变量">
    ...
    除非您明确希望通过
    `.env` 进行管理，否则请将 `OPENCLAW_GATEWAY_TOKEN` 留空；
    OpenClaw 会在首次启动时将随机网关令牌写入配置。
    生成一个密钥环密码并将其粘贴到
  ...
  <Step title="Docker Compose 配置">
    ...
    `--allow-unconfigured` 仅是为了引导方便，它不是适当网关配置的替代品。仍需设置认证（`gateway.auth.token` 或密码）并为您的部署使用安全的绑定设置。
  ...
  <Step title="共享 Docker VM 运行时步骤">
    ...
    - [将必需的二进制文件烘焙到镜像中](/zh/install/docker-vm-runtime#bake-required-binaries-into-the-image)
...
共享持久化映射位于 [Docker VM Runtime](/zh/install/docker-vm-runtime#what-persists-where) 中。
