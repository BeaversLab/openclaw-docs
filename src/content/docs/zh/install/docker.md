...
查看 [ClawDock](/en/install/clawdock) 获取完整的帮助指南。
...
<Accordion title="共享网络安全说明">`openclaw-cli` 使用 `network_mode: "service:openclaw-gateway"`，以便 CLI 命令可以通过 `127.0.0.1` 访问网关。请将此视为共享 信任边界。Compose 配置会丢弃 `NET_RAW`/`NET_ADMIN` 并 在 `openclaw-cli` 上启用 `no-new-privileges`。</Accordion>
  ...
  <Accordion title="高级用户容器选项">
    ...
    2. **预构建系统依赖**：`export OPENCLAW_DOCKER_APT_PACKAGES="git curl jq"`
    3. **安装 Playwright 浏览器**：
       ```bash
...
<Accordion title="OpenAI Codex OAuth (无头 Docker)">如果您在向导中选择 OpenAI Codex OAuth，它会打开一个浏览器 URL。在 Docker 或无头环境中，复制您跳转到的完整重定向 URL 并将其粘贴回向导中以完成身份验证。</Accordion>

  <Accordion title="基础镜像元数据">
    主要的 Docker 镜像使用 `node:24-bookworm` 并发布 OCI 基础镜像注解，包括 `org.opencontainers.image.base.name`、
...
[Docker VM Runtime](/en/install/docker-vm-runtime) 以了解共享 VM 部署步骤，
包括二进制文件构建、持久化和更新。
...
（Shell、文件读/写等），而
网关本身则保留在主机上。这为不受信任或
多租户代理会话提供了一道坚实的屏障，而无需将整个网关容器化。

沙箱作用域可以是每个代理（默认）、每个会话或共享的。每个作用域
都会获得挂载在 `/workspace` 的自己的工作区。您还可以配置
...
<Accordion title="沙箱中的权限错误">将 `docker.user` 设置为与您挂载的工作区所有权相匹配的 UID:GID， 或者对工作区文件夹执行 chown。</Accordion>

<Accordion title="沙箱中找不到自定义工具">OpenClaw 使用 `sh -lc`（登录 Shell）运行命令，该 Shell 会导入 `/etc/profile` 并可能会重置 PATH。设置 `docker.env.PATH` 以在前面添加您的 自定义工具路径，或者在 Dockerfile 中的 `/etc/profile.d/` 下添加脚本。</Accordion>

<Accordion title="镜像构建期间 OOM-killed（退出代码 137）">虚拟机至少需要 2 GB RAM。使用更大规格的机器并重试。</Accordion>
...
- [安装概览](/en/install) — 所有安装方法
- [Podman](/en/install/podman) — Docker 的 Podman 替代方案
