...
- **沙箱注意事项**：启用沙箱隔离时，默认的沙箱后端使用 Docker，但沙箱隔离默认处于关闭状态，并且**不**要求整个网关都在 Docker 中运行。此外还提供 SSH 和 OpenShell 沙箱后端。请参阅[沙箱隔离](/zh/gateway/sandboxing)。
...
官方 Docker 镜像不附带 Homebrew。在 OpenClaw 容器中运行且没有 `brew` 期间，Linux 会隐藏仅限 brew 的技能依赖项安装程序；这些依赖项必须由自定义镜像提供或手动安装。对于可从 Debian 软件包获得的依赖项，请在镜像构建期间使用 `OPENCLAW_DOCKER_APT_PACKAGES`。

维护人员可以通过将一个插件源目录挂载到其打包的源路径（例如 `OPENCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`），来针对打包镜像测试捆绑的插件源。该挂载的源目录会覆盖同一插件 ID 的匹配编译 `/app/dist/extensions/synology-chat` 包。
...
在启用导出之前，请从 ClawHub 在打包的 Docker 安装中安装官方 `@openclaw/diagnostics-otel` 插件。自定义源构建的镜像仍然可以使用 `OPENCLAW_EXTENSIONS=diagnostics-otel` 包含本地插件源。要启用导出，请在配置中允许并启用 `diagnostics-otel` 插件，然后设置 `diagnostics.otel.enabled=true` 或使用 [OpenTelemetry 导出](/zh/gateway/opentelemetry) 中的配置示例。收集器身份验证标头通过 `diagnostics.otel.headers` 配置，而不是通过 Docker 环境变量。

Prometheus 指标使用已发布的 Gateway(网关) 端口。安装 `clawhub:@openclaw/diagnostics-prometheus`，启用 `diagnostics-prometheus` 插件，然后抓取：

```text
http://<gateway-host>:18789/api/diagnostics/prometheus
```

该路由受 Gateway(网关) 身份验证保护。请勿公开单独的公共 `/metrics` 端口或未经身份验证的反向代理路径。请参阅 [Prometheus 指标](/zh/gateway/prometheus)。
...
Docker 镜像包含一个内置的 Docker`HEALTHCHECK`，用于对 `/healthz`Docker 执行 ping 操作。
...
`http://127.0.0.1:18789`Docker 可以通过 Docker 端口发布正常工作。

- `lan`CLI（默认）：主机浏览器和主机 CLI 可以访问已发布的网关端口。
- `loopback`：仅容器网络命名空间内的进程可以直接访问
  网关。
...
当 OpenClaw 在 Docker 中运行时，容器内部的 OpenClawDocker`127.0.0.1` 是容器
...
的新手引导默认值，并且 DockerOllama`docker-compose.yml` 将 `host.docker.internal`DockerLinuxDockerDockermacOSWindows 映射到
Linux Docker 引擎的 Docker 主机网关。Docker Desktop 已在 macOS 和 Windows 上
提供相同的主机名。
...
Docker 网桥网络通常无法可靠转发 Bonjour/mDNS 多播
(`224.0.0.251:5353`)。因此，捆绑的 Compose 设置默认使用
`OPENCLAW_DISABLE_BONJOUR=1`，以免当网桥丢弃多播流量时，Gateway(网关) 发生崩溃循环或重复
重启通告服务。

对于 Gateway(网关) 主机，请使用已发布的 Tailscale URL、Docker 或广域 DNS-SD。
仅当使用主机网络、macvlan 或其他已知 mDNS 多播可用的网络时，才设置 `OPENCLAW_DISABLE_BONJOUR=0`。
...
`OPENCLAW_AUTH_PROFILE_SECRET_DIR` 绑定挂载到 `/home/node/.config/openclaw`，以便这些
路径在容器替换后得以保留。当任何变量未设置时，捆绑的
`docker-compose.yml` 会回退到 `${HOME}` 下，或者当 `HOME` 本身
...
- `.env` 用于基于环境的运行时机密，例如 `OPENCLAW_GATEWAY_TOKEN`

auth-profile 密钥目录存储用于
OAuth 支持的 auth-profile 令牌材料的本地加密密钥。请将其与您的 Docker 主机状态一起保存，
但需与 `OPENCLAW_CONFIG_DIR` 分开存放。
...
有关完整的辅助工具指南，请参阅 [ClawDock](/zh/install/clawdock)。
...
<Accordion title="共享网络安全说明">`openclaw-cli` 使用 `network_mode: "service:openclaw-gateway"`CLI 以便 CLI 命令可以通过 `127.0.0.1` 访问网关。请将此视为共享的 信任边界。Compose 配置丢弃了 `NET_RAW`/`NET_ADMIN` 并在 `openclaw-gateway` 和 `openclaw-cli` 上启用了 `no-new-privileges`。</Accordion>

  <Accordion title="DockerDocker Desktop 在 openclaw-cli 中的 DNS 故障"Docker>
    在丢弃 `NET_RAW` 后，某些 Docker Desktop 设置无法从共享网络
    `openclaw-cli` sidecar 进行 DNS 查找，这会在 npm 支持的命令（如 `openclaw plugins install`）期间显示为
    `EAI_AGAIN`。对于正常的网关操作，请保留默认的加固 compose 文件。
    下面的本地覆盖通过恢复 Docker 的默认功能放宽了 CLIDocker 容器的安全姿态，因此仅将其用于需要访问包注册表的一次性 CLI
    命令，而不是作为您默认的 Compose
    调用方式：
    ...
    如果您已经创建了一个长时间运行的 `openclaw-cli` 容器，请使用相同的覆盖重新创建它。
  ...
  <Accordion title="权限和 EACCES">
    ...
    已挂载的插件目录所有者不一致。建议以默认 uid 1000 运行容器
    并修复绑定挂载的所有权。仅当您有意长期
    以 root 身份运行 OpenClaw 时，才将
    `/path/to/openclaw-config/npm` chown 为 `root:root`。

  </Accordion>

  <Accordion title="更快的重新构建">
    对您的 Dockerfile 进行排序，以便缓存依赖层。除非锁定文件发生变化，否则这可以避免重新运行
    `pnpm install`：
  ...
  <Accordion title="高级用户容器选项">
    默认镜像以安全为首要考虑，并以非 root `node` 身份运行。对于一个
    功能更全面的容器：

    1. **持久化 `/home/node`**：`export OPENCLAW_HOME_VOLUME="openclaw_home"`
    2. **构建系统依赖**：`export OPENCLAW_DOCKER_APT_PACKAGES="git curl jq"`
    3. **构建 Playwright Chromium**：`export OPENCLAW_INSTALL_BROWSER=1`
    ...
    5. **持久化浏览器下载**：使用 `OPENCLAW_HOME_VOLUME` 或
       `OPENCLAW_EXTRA_MOUNTS`。OpenClaw 会自动检测 Docker 镜像在 Linux 上由 Playwright 管理的 Chromium。
...
<Accordion title="OpenAIOAuthDockerOpenAI Codex OAuth (无头 Docker)" OpenAIOAuthDocker>
  ...
  <Accordion title="基础镜像元数据"Docker>
    ...
    `org.opencontainers.image.source`Docker 等。Node 基础摘要通过 Dependabot Docker 基础镜像 PR 进行更新；发布版本构建不运行
    发行版升级层。请参阅
...
在隔离的 Docker 容器内运行 agent 工具执行（shell、文件读/写等），而网关本身保留在主机上。这为您提供了围绕不受信任或多租户 agent 会话的硬性隔离，而无需将整个
网关容器化。

沙箱作用域可以是每个 agent（默认）、每个会话或共享的。每个作用域
都有自己的工作空间，挂载于 `/workspace`。您还可以配置
...
<AccordionGroup>
  <Accordion title="镜像缺失或沙箱容器未启动">
    ...
    （源代码检出）或 [沙箱隔离 § 镜像和设置](/zh/gateway/sandboxing#images-and-setup) 中的内联 `docker build` 命令（npm 安装）构建沙箱镜像，
    或将 `agents.defaults.sandbox.docker.image` 设置为您的自定义镜像。
    容器会根据需求按会话自动创建。
  </Accordion>

<Accordion title="沙箱中的权限错误">将 `docker.user` 设置为与您挂载的工作区所有权匹配的 UID:GID， 或对工作区文件夹执行 chown。</Accordion>

<Accordion title="沙箱中找不到自定义工具">OpenClaw 使用 `sh -lc`（登录 shell）运行命令，它会 引用 `/etc/profile` 并且可能会重置 PATH。设置 `docker.env.PATH` 以前置您的 自定义工具路径，或在您的 Dockerfile 中 `/etc/profile.d/` 下添加脚本。</Accordion>
...
- [Podman](/zh/install/podman) — Podman 替代 Docker
