...
- **沙箱注意事项**：启用沙箱隔离时，默认的沙箱后端使用 Docker，但沙箱隔离默认是关闭的，并且**不**要求整个网关都在 Docker 中运行。也可以使用 SSH 和 OpenShell 沙箱后端。请参阅[沙箱隔离](DockerDocker/en/gateway/sandboxing)。
...
- 足够的磁盘空间用于镜像和日志
- 如果在 VPS/公共主机上运行，请查看
  [网络安全暴露的加固措施](/zh/gateway/securityDocker)，
...
该路由受 Gateway(网关) 身份验证保护。不要暴露单独的
公共 Gateway(网关)`/metrics` 端口或未经身份验证的反向代理路径。请参阅
...
有关完整的辅助指南，请参阅 [ClawDock](/zh/install/clawdock)。
...
沙箱设置无法完成，该脚本会将 `agents.defaults.sandbox.mode`
重置为 `off`。
...
<Accordion title="共享网络安全说明">`openclaw-cli` 使用 `network_mode: "service:openclaw-gateway"`，以便 CLI 命令可以通过 `127.0.0.1` 访问网关。请将此视为共享信任边界。Compose 配置会丢弃 `NET_RAW`/`NET_ADMIN` 并在 `openclaw-gateway` 和 `openclaw-cli` 上启用 `no-new-privileges`。</Accordion>
  ...
  <Accordion title="openclaw-cli 中的 Docker Desktop DNS 故障">
    ...
    `openclaw-cli` 边车进行 DNS 查找，这在 npm 支持的命令（如 `openclaw plugins install`）期间显示为
    `EAI_AGAIN`。请保留默认的强化 compose 文件以进行正常的网关操作。
    下面的本地覆盖通过恢复 CLI 的默认功能，放宽了 Docker 容器的安全姿态，因此请仅将其用于需要包注册表访问的一次性 CLI
    命令，而不要将其作为默认的 Compose
    调用：
  ...
  <Accordion title="权限和 EACCES">
    该镜像以 `node` (uid 1000) 运行。如果您在 `/home/node/.openclaw` 上看到权限错误，请确保您的主机绑定挂载由 uid 1000 拥有：

    ```bash
    sudo chown -R 1000:1000 /path/to/openclaw-config /path/to/openclaw-workspace
    ```

    同样的不匹配可能显示为插件警告，例如 `blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root)`
    后跟 `plugin present but blocked`。这意味着进程 uid 和挂载的插件目录所有者不一致。建议容器以默认 uid 1000 运行并修复绑定挂载的所有权。仅当您打算长期以 root 身份运行 OpenClaw 时，才将 `/path/to/openclaw-config/npm` chown 为 `root:root`OpenClaw。

  </Accordion>

  <Accordion title="更快的重新构建">
    对 Dockerfile 进行排序，以便缓存依赖层。这避免了除非 lockfile 更改，否则重新运行 `pnpm install`：
  ...
  <Accordion title="高级用户容器选项">
    默认镜像优先考虑安全性，并以非 root `node` 运行。对于功能更全面的容器：

    1. **持久化 `/home/node`**：`export OPENCLAW_HOME_VOLUME="openclaw_home"`
    2. **构建系统依赖项**：`export OPENCLAW_DOCKER_APT_PACKAGES="git curl jq"`
    3. **安装 Playwright 浏览器**：
       ```bash
       docker compose run --rm openclaw-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    4. **持久化浏览器下载**：使用 `OPENCLAW_HOME_VOLUME` 或
       `OPENCLAW_EXTRA_MOUNTS`OpenClawDockerLinux。OpenClaw 会自动检测 Docker 镜像在 Linux 上由 Playwright 管理的 Chromium。
...
<Accordion title="OpenAIOAuthDockerOpenAI Codex OAuth (无头 Docker)" OpenAIOAuthDocker>
  ...
  <Accordion title="Base image metadata"Docker>
    主要的 Docker 运行时镜像使用 `node:24-bookworm-slim` 并包含 `tini` 作为入口点初始化进程 (PID 1)，以确保在长期运行的容器中回收僵尸进程并正确处理信号。它发布了 OCI 基础镜像注解，包括 `org.opencontainers.image.base.name`、
    `org.opencontainers.image.source`Docker 等。Node 基础摘要
    通过 Dependabot Docker 基础镜像 PR 进行更新；发布构建不运行
    发行版升级层。请参阅
...
关于共享 VM 部署步骤（包括二进制文件制作、持久化和更新），请参阅 [Hetzner (Docker VPS)](HetznerDocker/en/install/hetznerDocker) 和
[Docker VM Runtime](/zh/install/docker-vm-runtime)。

## Agent 沙箱

当使用 Docker 后端启用 `agents.defaults.sandbox`DockerDocker 时，网关
会在独立的 Docker 容器内运行 agent 工具执行（shell、文件读/写等），而网关本身则保留在主机上。这为您提供了一个硬性隔离墙，
用于保护不受信任或多租户 agent 会话，而无需将整个
网关容器化。

沙箱范围可以是每个 agent（默认）、每个会话或共享的。每个范围
都有其自己的工作区，挂载于 `/workspace`。您还可以配置
...
对于没有源码检出（source checkout）的 npm 安装，请参阅 [沙箱隔离 § Images and setup](npm/en/gateway/sandboxing#images-and-setup) 中的内联 `docker build` 命令。
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    （源码检出）或 [沙箱隔离 § Images and setup](/zh/gateway/sandboxing#images-and-setupnpm) 中的内联 `docker build` 命令（npm 安装）构建沙箱镜像，
    或者将 `agents.defaults.sandbox.docker.image` 设置为您的自定义镜像。
    容器会按需在每个会话中自动创建。
  </Accordion>

<Accordion title="Permission errors in sandbox">将 `docker.user` 设置为与您挂载的工作区所有权匹配的 UID:GID， 或者对工作区文件夹执行 chown 命令。</Accordion>

<Accordion title="Custom tools not found in sandbox">OpenClaw 使用 `sh -lc` (login shell) 运行命令，这会调用 `/etc/profile` 并可能重置 PATH。设置 `docker.env.PATH` 以在前面添加您的 自定义工具路径，或者在您的 Dockerfile 中的 `/etc/profile.d/` 下添加脚本。</Accordion>
...
- [Podman](/zh/install/podmanDocker) — Podman 作为 Docker 的替代方案
- [ClawDock](/zh/install/clawdockDocker) — Docker Compose 社区配置
