...
- **沙箱注意事项**：agent 沙箱隔离也使用 Docker，但它**不**要求整个网关在 Docker 中运行。请参阅 [沙箱隔离](/en/gateway/sandboxing)。
...
<Note>从仓库根目录运行 `docker compose`。如果您启用了 `OPENCLAW_EXTRA_MOUNTS` 或 `OPENCLAW_HOME_VOLUME`，安装脚本会写入 `docker-compose.extra.yml`； 请使用 `-f docker-compose.yml -f docker-compose.extra.yml` 将其包含在内。</Note>

<Note>由于 `openclaw-cli` 与 `openclaw-gateway` 共享网络命名空间，它是一个 启动后工具。在 `docker compose up -d openclaw-gateway` 之前，请通过 `openclaw-gateway` 使用 `--no-deps --entrypoint node` 运行新手引导和设置时配置写入。</Note>
...
`http://127.0.0.1:18789` 的访问可以配合 Docker 端口发布使用。

- `lan`（默认值）：主机浏览器和主机 CLI 可以访问已发布的网关端口。
- `loopback`：只有容器网络命名空间内的进程可以直接访问
  网关。

<Note>请在 `gateway.bind` 中使用绑定模式值（`lan` / `loopback` / `custom` / `tailnet` / `auto`），而不要使用像 `0.0.0.0` 或 `127.0.0.1` 这样的主机别名。</Note>
...
[Docker VM Runtime - 什么持久化在哪里](/en/install/docker-vm-runtime#what-persists-where)。
...
<AccordionGroup>
  <Accordion title="为 Docker 网关启用 Agent 沙箱">
    ```bash
...
<Accordion title="共享网络安全注意事项">`openclaw-cli` 使用 `network_mode: "service:openclaw-gateway"`，以便 CLI 命令可以通过 `127.0.0.1` 访问网关。请将其视为共享 信任边界。Compose 配置会丢弃 `NET_RAW`/`NET_ADMIN` 并在 `openclaw-cli` 上启用 `no-new-privileges`。</Accordion>
  ...
  <Accordion title="更快地重建">
    请对 Dockerfile 进行排序，以便缓存依赖层。这样可以避免重新运行
    `pnpm install`，除非锁定文件发生变化：
  ...
  <Accordion title="Power-user container options">
    默认镜像以安全为重，并以非 root `node` 运行。若要使用
    功能更全的容器：
...
<Accordion title="OpenAI Codex OAuth (headless Docker)">如果您在向导中选择 OpenAI Codex OAuth，它会打开一个浏览器 URL。在 Docker 或无头（headless）环境中，请复制您重定向到的完整 URL，并将其粘贴回向导以完成认证。</Accordion>
...
（Shell、文件读/写等），而
网关本身仍驻留在主机上。这为不受信任的或
多租户代理会话提供了严格的隔离屏障，而无需将整个网关容器化。
...
- [沙箱隔离](/en/gateway/sandboxing) -- 沙箱完整参考
- [OpenShell](/en/gateway/openshell) -- 沙箱容器的交互式 Shell 访问
- [多智能体沙箱和工具](/en/tools/multi-agent-sandbox-tools) -- 每个智能体的覆盖设置
...
<AccordionGroup>
  <Accordion title="镜像缺失或沙箱容器无法启动">
    ...
    构建沙箱镜像，或者将 `agents.defaults.sandbox.docker.image` 设置为您的自定义镜像。
    容器会根据需求在每个会话中自动创建。
  </Accordion>

<Accordion title="沙箱中的权限错误">将 `docker.user` 设置为与您挂载的工作区所有权相匹配的 UID:GID， 或者修改工作区文件夹的所有者。</Accordion>

<Accordion title="在沙箱中找不到自定义工具">OpenClaw 使用 `sh -lc` (登录 shell) 运行命令，这会执行 `/etc/profile` 并且可能会重置 PATH。设置 `docker.env.PATH` 以在前面添加您的 自定义工具路径，或者在您的 Dockerfile 中 `/etc/profile.d/` 下添加脚本。</Accordion>

<Accordion title="镜像构建期间发生 OOM 终止（退出代码 137）">虚拟机至少需要 2 GB RAM。使用更大规格的机器并重试。</Accordion>
  ...
  <Accordion title="控制 UI 中未授权或需要配对">
    ...
    更多详情：[控制面板](/en/web/dashboard)、[设备](/en/cli/devices)。
