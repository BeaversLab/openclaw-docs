...
- **沙箱注意事项**：当启用沙箱隔离时，默认的沙箱后端使用 Docker，但沙箱隔离默认是关闭的，并且**不**要求完整的网关在 Docker 中运行。SSH 和 OpenShell 沙箱后端也可用。参见[沙箱隔离](/zh/gateway/sandboxing)。
...
<Steps>
  ...
  <Step title="配置渠道（可选）">
    ...
    文档：[CLI](/zh/channels/whatsapp)、[WhatsApp](/zh/channels/telegram)、[Telegram](/zh/channels/discord)
...
请参阅 [ClawDock](/zh/install/clawdock) 了解完整的辅助指南。
...
当使用 Docker 后端启用 `agents.defaults.sandbox` 时，网关在隔离的 Docker 容器内运行代理工具执行（shell、文件读/写等），而网关本身则保留在主机上。这为您的不受信任或多租户代理会话提供了一道坚固的屏障，而无需将整个网关容器化。
...
<Accordion title="镜像构建期间 OOM-killed（退出代码 137）">虚拟机至少需要 2 GB RAM。使用更大的机器类型并重试。</Accordion>
  ...
  <Accordion title="控制 UI 中未授权或需要配对">
    ...
    更多详细信息：[仪表板](/zh/web/dashboard)、[设备](/zh/cli/devices)。
