...
- 无根模式下的 **Podman**
- 主机上已安装 **OpenClaw CLI**
- **可选：** 如果您需要 Quadlet 管理的自动启动，请 `systemd --user`
- **可选：** 仅当您需要在无头主机上实现开机持久化时才 `sudo` `loginctl enable-linger "$(whoami)"`
...
- `./scripts/podman/setup.sh` 默认在您的 rootless Podman 存储中构建 `openclaw:local`，或者如果您设置了 `OPENCLAW_IMAGE` / `OPENCLAW_PODMAN_IMAGE`，则使用它们。
- 如果缺失，它会使用 `gateway.mode: "local"` 创建 `~/.openclaw/openclaw.json`。
- 如果缺失，它会使用 `OPENCLAW_GATEWAY_TOKEN` 创建 `~/.openclaw/.env`。
- 对于手动启动，辅助脚本仅从 `~/.openclaw/.env` 读取一小部分与 Podman 相关的键，并将显式的运行时环境变量传递给容器；它不会将整个环境文件交给 Podman。
...
- `OPENCLAW_EXTENSIONS` —— 在构建时预先安装扩展依赖项
...
在 macOS 上，Podman machine 可能会使浏览器对网关来说显示为非本地。
如果控制界面在启动后报告设备身份验证错误，请优先使用 [macOS Podman SSH 隧道](#macos-podman-ssh-tunnel) 中的 SSH 隧道流程。对于
远程 HTTPS 访问，请使用
[Podman + Tailscale](#podman--tailscale) 中的 Tailscale 指南。

## macOS Podman SSH 隧道

在 macOS 上，即使发布的端口仅在 `127.0.0.1` 上，Podman machine 也可能使浏览器对网关来说显示为非本地。
...
- `18789` 通常已被 Mac 主机上 Podman 发布的网关端口占用，因此该隧道使用 `28889` 作为本地浏览器端口。
...
- 对于不使用 HTTPS 的本地 macOS 浏览器访问，请优先使用上面的 SSH 隧道部分。
...
如果您运行了 `./scripts/podman/setup.sh --quadlet`，安装程序会在以下位置安装一个 Quadlet 文件：
...
为了在 SSH/无头主机上实现开机持久运行，请为您当前的用户启用 lingering：
...
默认情况下，这些是主机目录，而不是匿名的容器状态，因此配置和工作区在容器替换后仍然保留。
Podman 设置还会在发布的网关端口上为 `127.0.0.1` 和 `localhost` 播种 `gateway.controlUi.allowedOrigins`，以便本地仪表板与容器的非环回绑定一起工作。
...
手动启动器在最终确定容器/镜像默认值之前会读取 `~/.openclaw/.env`，因此您可以在此处持久化这些设置。
...
- 生成的 Quadlet 服务有意保持固定的、经过强化的默认形状：`127.0.0.1` 发布端口、容器内的 `--bind lan` 以及 `keep-id` 用户命名空间。
- 它仍然读取 `~/.openclaw/.env` 以获取 Gateway(网关) 运行时环境变量，例如 `OPENCLAW_GATEWAY_TOKEN`，但它不会使用手动启动器中 Podman 特定的覆盖允许列表。
...
- **从主机 CLI 打开面板 URL：** `openclaw dashboard --no-open`
- **通过主机 CLI 查看健康/状态：** `openclaw gateway status --deep`
...
- **容器 CLI 命令作用到错误的目标：** 请显式使用 `openclaw --container <name> ...`，或在您的 shell 中导出 `OPENCLAW_CONTAINER=<name>`。
- **`openclaw update` 失败并提示 `--container`：** 这是预期行为。重新构建/拉取镜像，然后重启容器或 Quadlet 服务。
...
- **SELinux 阻止绑定挂载：** 请保持默认挂载行为不变；当 SELinux 处于强制或宽容模式时，启动器会在 Linux 上自动添加 `:Z`。
