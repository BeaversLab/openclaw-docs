...
在 macOS 上，Podman machine 可能会导致浏览器对网关来说显示为非本地。
如果在启动后控制界面报告设备身份验证错误，请使用
...
- 在 macOS 上，如果本地浏览器设备身份验证上下文不可靠，请使用 Tailscale 访问，而不是临时的本地隧道变通方法。
...
为了在 SSH/无头主机上实现启动持久化，请为您当前用户启用 linger：
...
默认情况下，这些是宿主机目录，而不是匿名的容器状态，因此
`openclaw.json`、每个代理的 `auth-profiles.json`、渠道/提供商状态、
会话和工作空间在容器替换后仍然保留。
Podman 设置还会在发布的网关端口上为 `127.0.0.1` 和 `localhost` 播种 `gateway.controlUi.allowedOrigins`，以便本地仪表板与容器的非环回绑定配合使用。
...
- `OPENCLAW_PODMAN_PUBLISH_HOST` -- 已发布端口的宿主机接口；默认为 `127.0.0.1`
- `OPENCLAW_GATEWAY_BIND` -- 容器内的网关绑定模式；默认为 `lan`
- `OPENCLAW_PODMAN_USERNS` -- `keep-id`（默认）、`auto` 或 `host`

手动启动器在确定容器/镜像默认值之前会读取 `~/.openclaw/.env`，因此您可以在那里持久化这些设置。
...
- 生成的 Quadlet 服务有意保持固定的、加固的默认形状：`127.0.0.1` 发布的端口、容器内的 `--bind lan` 以及 `keep-id` 用户命名空间。
...
- 它将 `~/.openclaw/.env` 作为诸如 `OPENCLAW_GATEWAY_TOKEN` 等值的运行时 `EnvironmentFile` 读取，但它不使用手动启动器的 Podman 特定覆盖允许列表。
...
- **配置或工作区权限被拒绝 (EACCES)：** 容器默认以 `--userns=keep-id` 和 `--user <your uid>:<your gid>` 运行。请确保主机配置/工作区路径归您当前用户所有。
...
- **容器 CLI 命令命中了错误的目标：** 显式使用 `openclaw --container <name> ...`，或在您的 shell 中导出 `OPENCLAW_CONTAINER=<name>`。
