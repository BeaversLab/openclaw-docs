...
目标：在 exe.dev 虚拟机上运行 OpenClaw Gateway，可通过以下方式从您的笔记本电脑访问：`https://<vm-name>.exe.xyz`
...
Shelley，即 [exe.dev](https://exe.dev) 的代理，可以通过我们的提示词立即安装 OpenClaw。使用的提示词如下：
...
<Tip>保持此虚拟机 **有状态（stateful）**。OpenClaw 将 `openclaw.json`、每个代理的 `auth-profiles.json`、会话以及渠道/提供商状态存储在 `~/.openclaw/` 下，并将工作区存储在 `~/.openclaw/workspace/` 下。</Tip>
...
并且附加风格的 `X-Forwarded-For` 链被视为一种加固风险。
