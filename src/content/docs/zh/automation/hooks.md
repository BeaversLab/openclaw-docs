...
Hooks 也可以打包在插件内部；请参阅 [Plugin hooks](/en/plugins/architecture#provider-runtime-hooks)。`openclaw hooks list` 会显示独立的 hook 和插件管理的 hook。
...
- **💾 会话-memory**：当您发出 `/new` 或 `/reset` 时，将会话上下文保存到您的代理工作区（默认 `~/.openclaw/workspace/memory/`）
...
2. **插件钩子**：捆绑在已安装插件内部的钩子（请参阅 [插件钩子](/en/plugins/architecture#provider-runtime-hooks)）
3. **托管钩子**：`~/.openclaw/hooks/`（用户安装的，在工作区之间共享；可以覆盖捆绑和插件钩子）。通过 `hooks.internal.load.extraDirs` 配置的**额外钩子目录**也被视为托管钩子，并共享相同的覆盖优先级。
...
Hook packs 是标准的 npm 包，通过 `openclaw.hooks` 在
...
每个条目指向一个包含 `HOOK.md` 和处理程序文件的钩子目录。加载程序依次尝试 `handler.ts`、`handler.js`、`index.ts`、`index.js`。
...
- **`tool_result_persist`**：在将工具结果写入会话记录之前转换它们。必须是同步的；返回更新后的工具结果负载或 `undefined` 以保持原样。请参阅 [代理循环](/en/concepts/agent-loop)。
...
- **`origin`**：可用的标准化安装来源（例如 `openclaw-bundled`、`openclaw-workspace`、`plugin-bundle`、`plugin-package` 或 `plugin-file`）
- **`request`**：安装请求的出处，包括 `kind`、`mode` 和可选的 `requestedSpecifier`
- **`builtinScan`**：内置扫描器的结构化结果，包括 `status`、摘要计数、发现的问题以及可选的 `error`
- **`skill`**：当 `targetType` 为 `skill` 时的技能安装元数据，包括 `installId` 和所选的 `installSpec`
- **`plugin`**：当 `targetType` 为 `plugin` 时的插件安装元数据，包括规范的 `pluginId`、标准化的 `contentType`、可选的 `packageName` / `manifestId` / `version` 以及 `extensions`
...
技能安装使用相同的事件结构，但使用 `targetType: "skill"` 和 `skill` 对象来代替 `plugin`。
...
<Note>两个钩子 (`tool_result_persist` 和 `before_message_write`) **仅同步** — 它们绝不能返回 Promise。如果这些钩子返回 Promise，会在运行时被捕获，结果将被丢弃并发出警告。</Note>

有关完整的处理程序签名和上下文类型，请参阅[插件架构](/en/plugins/architecture)。

### 未来事件

以下事件类型已计划用于内部 hook 事件流。
请注意，`session_start` 和 `session_end` 已作为[插件 Hook API](/en/plugins/architecture#provider-runtime-hooks) hook 存在
...
当 Gateway(网关) 启动时（通道启动之后）运行 `BOOT.md`。
...
- [Bundled Hooks 自述文件](https://github.com/openclaw/openclaw/tree/main/src/hooks/bundled)
