...
> **Cron 与 Heartbeat 对比？** 请参阅 [Cron vs Heartbeat](/en/automation/cron-vs-heartbeat) 以获取关于何时使用每种方法的指导。
...
- 在下一次成功运行后，退避会自动重置。
- 单次（`at`）作业会使用退避策略重试临时错误（速率限制、过载、网络、server_error）最多 3 次；永久错误会立即禁用。请参阅[重试策略](/en/automation/cron-jobs#retry-policy)。
