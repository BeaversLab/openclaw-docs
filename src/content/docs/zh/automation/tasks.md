...
> **正在寻找调度功能？** 请参阅 [Automation & Tasks](/zh/automation) 以选择合适的机制。本页面介绍的是对后台工作的**跟踪**，而非调度。
...
对于 ACP 和子代理任务，这将终止子会话。对于 CLI 跟踪的任务，取消操作会记录在任务注册表中（没有单独的子运行时句柄）。状态转换为 `cancelled`，并在适用时发送传递通知。
...
- 子代理完成交付优先使用最新的可见助手文本；如果为空，则回退到经过清理的最新工具/toolResult文本，且仅超时的工具调用运行可能会折叠为简短的部分进度摘要。终端失败的运行会宣布失败状态，而不会重播捕获的回复文本。
- 清理失败不会掩盖真实的任务结果。
...
[Task Flow](/zh/automation/taskflow) 是位于后台任务之上的流程编排层。单个流程在其生命周期内可以使用托管或镜像同步模式来协调多个任务。使用 `openclaw tasks` 检查单个任务记录，使用 `openclaw tasks flow` 检查编排流程。

详情请参阅 [Task Flow](/zh/automation/taskflow)。

### Tasks and cron

Cron 作业**定义**存储在 `~/.openclaw/cron/jobs.json` 中；运行时执行状态存储在其旁边的 `~/.openclaw/cron/jobs-state.json` 中。**每次** cron 执行都会创建一个任务记录 —— 包括主会话和隔离的。主会话 cron 任务默认采用 `silent` 通知策略，以便它们在不生成通知的情况下进行跟踪。
...
任务可以引用 `childSessionKey`（工作运行的位置）和 `requesterSessionKey`（启动它的人）。会话是对话上下文；任务是建立在对话上下文之上的活动跟踪。
...
- [Task Flow](/zh/automation/taskflow) —— 位于任务之上的流程编排
- [Scheduled Tasks](/zh/automation/cron-jobs) —— 调度后台工作
- [Heartbeat](/zh/gateway/heartbeat) — 周期性主会话轮次
