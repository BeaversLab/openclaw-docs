...
> **正在寻找调度功能？** 请参阅 [自动化与任务](/zh/automation) 以选择正确的机制。本页面涵盖**跟踪**后台工作，而非对其进行调度。
...
[Task Flow](/zh/automation/taskflow) 是位于后台任务之上的流程编排层。单个流程在其生命周期内可以使用托管或镜像同步模式协调多个任务。使用 `openclaw tasks` 检查单个任务记录，使用 `openclaw tasks flow` 检查编排流程。
