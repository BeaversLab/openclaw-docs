...
<Tip>将常驻指令放入 `AGENTS.md` 以确保它们在每个会话中都被加载。工作区 引导程序会自动注入 `AGENTS.md`、`SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、 `HEARTBEAT.md`、`BOOTSTRAP.md` 和 `MEMORY.md` — 但不会自动注入子目录中的任意文件。</Tip>
...
- [Cron Jobs](/zh/automation/cron-jobs) — 为长期命令安排执行
- [Agent Workspace](/zh/concepts/agent-workspace) — 长期命令的所在地，包括自动注入的引导文件的完整列表（AGENTS.md、SOUL.md 等）
