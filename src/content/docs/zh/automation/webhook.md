...
Gateway(网关) 网关 可以暴露一个小型的 HTTP webhook 端点用于外部触发。
...
- `channel` 可选（字符串）：用于投递的消息渠道。使用 `last` 或任何已配置的渠道或插件 ID，例如 `discord`、`matrix`、`telegram` 或 `whatsapp`。默认为 `last`。
...
- `thinking` 可选（字符串）：覆盖思考级别（例如，`low`、`medium`、`high`）。
...
`/hooks/agent` payload `sessionKey` 覆盖默认禁用。
...
自定义挂钩名称通过 `hooks.mappings` 解析（请参阅配置）。映射可以将任意负载转换为 `wake` 或 `agent` 操作，并带有可选模板或代码转换。

映射选项（摘要）：

- `hooks.presets: ["gmail"]` 启用内置的 Gmail 映射。
- `hooks.mappings` 允许您在配置中定义 `match`、`action` 和模板。
- `hooks.transformsDir` + `transform.module` 会加载一个用于自定义逻辑的 JS/TS 模块。
  - `hooks.transformsDir` （如果设置）必须位于您的 OpenClaw 配置目录下的 transforms 根目录中（通常是 `~/.openclaw/hooks/transforms`）。
  - `transform.module` 必须在有效的转换目录内解析（拒绝遍历/转义路径）。
- 使用 `match.source` 来保留一个通用的接入端点（由负载驱动的路由）。
...
- 当未提供显式密钥时，`hooks.defaultSessionKey` 为钩子代理运行设置默认会话。
...
- `200` 针对 `/hooks/wake`
- `200` 针对 `/hooks/agent`（接受异步运行）
- 在身份验证失败时返回 `401`
- 同一客户端反复身份验证失败后返回 `429`（检查 `Retry-After`）
- 在负载无效时返回 `400`
- 超大载荷上的 `413`
...
如果您强制执行 `agents.defaults.models`，请确保覆盖的模型包含在其中。
...
- 最好使用专用的 hook agent 并配合严格的 `tools.profile` 和沙箱隔离，这样 hook 入侵的影响范围更小。
- 每个客户端地址会限制重复身份验证失败的速率，以减缓暴力破解尝试。
- 如果您使用多代理路由，请设置 `hooks.allowedAgentIds` 以限制显式 `agentId` 选择。
- 请保留 `hooks.allowRequestSessionKey=false`，除非您需要调用方选择的会话。
- 如果启用请求 `sessionKey`，请限制 `hooks.allowedSessionKeyPrefixes`（例如，`["hook:"]`）。
- 避免在 Webhook 日志中包含敏感的原始负载。
- Hook payloads 默认被视为不受信任，并使用安全边界进行包裹。
  如果必须为特定的 hook 禁用此功能，请在该 hook 的映射中设置 `allowUnsafeExternalContent: true`（危险）。
