...
上述所有内容也都遵守 `$OPENCLAW_STATE_DIR`（状态目录覆盖）。完整参考：[/gateway/configuration](/zh/gateway/configuration-reference#auth-storage)
...
<Warning>
Anthropic 的公开 Claude Code 文档指出，直接使用 Claude Code 仍在 Claude 订阅限制范围内，且 AnthropicOpenClaw 工作人员告知我们，OpenClaw 风格的 Claude CLI 使用再次获得许可。因此，除非 OpenClaw 发布新政策，否则 CLI 将 Claude Anthropic 的重用和 `claude -p` 使用视为对此集成是经批准的。

有关 Anthropic 当前的直接 Claude Code 计划文档，请参阅 [在 Pro 或 Max
计划中使用 Claude Code](https://support.claude.com/en/articles/11145838-using-claude-code-with-your-pro-or-max-plan)
和 [在 Team 或 Enterprise
计划中使用 Claude Code](https://support.anthropic.com/en/articles/11845131-using-claude-code-with-your-team-or-enterprise-plan/)。

如果您希望在 OpenClaw 中使用其他订阅式选项，请参阅 [OpenAI
Codex](/zh/providers/openai)、[Qwen 云编程计划](/zh/providers/qwen)、[MiniMax 编程计划](/zh/providers/minimax)
以及 [Z.AI / GLM 编程计划](/zh/providers/glm)。
...
- 例外：某些外部 CLI 凭据保持外部管理；OpenClaw 会重新读取那些 CLI 身份验证存储，而不是消耗复制的刷新令牌。Codex CLI 引导 intentionally 范围更窄：它播种一个空的 `openai-codex:default` 配置文件，然后由 OpenClaw 拥有的刷新操作保持本地配置文件的权威性。
...
- [身份验证](/zh/gateway/authentication) - 模型提供商身份验证概述
