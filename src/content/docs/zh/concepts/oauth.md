...
- 外部 CLI 复用取决于提供商：Codex CLI 可以引导一个空的 `openai:default` 配置文件，但一旦 OpenClaw 拥有本地 OAuth 配置文件，本地刷新令牌即为权威。如果本地刷新令牌被拒绝，OpenClaw 可以使用可用的同账户 Codex CLI 令牌作为仅运行时的后备；其他集成可以保持外部管理，并重新读取其 CLI 认证存储
...
OpenClaw 的交互式登录流程在 OpenClaw`openclaw/plugin-sdk/llm` 中实现，并连接到向导/命令。
...
向导路径为 `openclaw onboard` → 认证选择 `openai`。
...
- 例外：某些外部 CLI 凭证保持外部管理；OpenClaw 会重新读取那些 CLI 认证存储，而不是消耗已复制的刷新令牌。Codex CLI 引导故意范围更窄：它为一个空的 `openai:default` 配置文件提供种子，然后 OpenClaw 拥有的刷新使本地配置文件保持权威。如果本地 Codex 刷新失败，并且 Codex CLI 拥有同一账户的可用令牌，OpenClaw 可能会将该令牌用于当前的运行时请求，而不会将其写回 `auth-profiles.json`。
