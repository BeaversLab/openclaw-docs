...
这将[多智能体路由](/zh/concepts/multi-agent)从个人使用扩展到组织部署。
...
- 遵循**[常备指令](/zh/automation/standing-orders)** —— 在智能体 `AGENTS.md` 中定义的规则，指定其可以自主执行哪些操作、哪些操作需要人工批准（有关计划执行，请参见[Cron Jobs](/zh/automation/cron-jobs)）。
...
此层级结合了第 2 层级的权限与[Cron Jobs](/zh/automation/cron-jobs)和[Standing Orders](/zh/automation/standing-orders)。
...
请参见[沙箱隔离](/zh/gateway/sandboxing)和[Multi-Agent 沙箱 & Tools](/zh/tools/multi-agent-sandbox-tools)。
...
注册一个具有 `Mail.Read` 和 `Calendars.Read` 应用程序权限的 Azure AD 应用程序。**在使用该应用程序之前**，请使用[应用程序访问策略](https://learn.microsoft.com/graph/auth-limit-mailbox-access)限制访问范围，使其仅限于代表和主体邮箱：
...
使用[多智能体路由](/zh/concepts/multi-agent)绑定将入站消息路由到代表智能体：
...
切勿将主智能体的 `agentDir` 与代表共享。有关身份验证隔离的详细信息，请参见[多智能体路由](/zh/concepts/multi-agent)。
...
代表的 `AGENTS.md` 定义了其自主权限 —— 即它可以不经询问执行的操作、需要批准的操作以及禁止的操作。[Cron Jobs](/zh/automation/cron-jobs) 驱动其日常计划。

如果您授予 `sessions_history`，请记住这是一个受安全过滤限制的
有限召回视图。OpenClaw 会编辑类似凭据/令牌的文本，截断长
...
令牌/来自助手召回的格式错误的 MiniMax 工具调用 XML，并且可以
...
1. **为每个组织创建一个委托代理**。
2. **优先加固** —— 工具限制、沙箱、硬性阻止、审计跟踪。
3. **通过身份提供商授予限定权限**（最小权限原则）。
4. **定义[常备指令](/zh/automation/standing-orders)**以进行自主操作。
5. **为定期任务安排 cron 作业**。
6. **随着信任度的建立，审查并调整**能力层级。

多个组织可以使用多代理路由共享一个 Gateway(网关) 服务器 —— 每个组织获得其独立的代理、工作区和凭证。
