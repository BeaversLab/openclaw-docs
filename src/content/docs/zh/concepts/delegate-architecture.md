...
目标：将 OpenClaw 作为**命名委派**（named delegate）运行——这是一个具有自身身份的代理，代表组织内的人员行动。该代理从不冒充人类。它在拥有明确委派权限的自己的账户下进行发送、读取和调度。
...
- 遵循**[长期指令](/zh/automation/standing-orders)**（standing orders）——在代理的 `AGENTS.md` 中定义的规则，指定它可以自主执行哪些操作以及哪些操作需要人工批准（有关计划执行，请参阅 [Cron Jobs](/zh/automation/cron-jobs)）。

委派模型直接对应于行政助理的工作方式：他们拥有自己的凭据，“代表”其委托人发送邮件，并遵循既定的职权范围。

## 为什么要使用委派？

OpenClaw 的默认模式是**个人助理**（personal assistant）——一个人，一个代理。委派将其扩展到组织：
...
此层级仅需要来自身份提供商的读取权限。代理不会向任何邮箱或日历写入内容——草稿和提案通过聊天发送，由人类采取行动。
...
使用每个代理的 工具 策略 (v2026.1.6+) 在 Gateway(网关) 级别强制执行边界。此操作独立于代理的人格配置文件运行——即使指示代理绕过其规则，Gateway(网关) 也会阻止 工具 调用：
...
委派需要在您的身份提供商中拥有自己的帐户，并具有明确的委派权限。**应用最小权限原则**——从第 1 层（只读）开始，仅当用例要求时才升级。
...
委托的 `AGENTS.md` 定义了其自主权限——即它无需询问即可执行的操作、需要批准的操作以及被禁止的操作。[Cron Jobs](/zh/automation/cron-jobs) 驱动其日常日程。
...
多个组织可以使用多代理路由共享一个 Gateway(网关) 服务器 — 每个组织都有自己的独立代理、工作区和凭证。
