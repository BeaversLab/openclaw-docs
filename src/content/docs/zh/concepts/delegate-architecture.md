...
这将 [多代理路由](/en/concepts/multi-agent) 从个人使用扩展到组织部署。
...
- 遵循**[常驻指令](/en/automation/standing-orders)**——在代理的 `AGENTS.md` 中定义的规则，指定了它可以自主执行哪些操作以及哪些操作需要人工批准（有关计划执行，请参阅 [Cron 作业](/en/automation/cron-jobs)）。
...
此层级结合了层级 2 的权限与 [Cron Jobs](/en/automation/cron-jobs) 和 [Standing Orders](/en/automation/standing-orders)。
...
请参阅 [沙箱隔离](/en/gateway/sandboxing) 和 [多代理沙箱与工具](/en/tools/multi-agent-sandbox-tools)。
...
使用 [Multi-Agent Routing](/en/concepts/multi-agent) 绑定将入站消息路由到委托代理：
...
切勿与委托共享主代理的 `agentDir`。有关身份验证隔离的详细信息，请参阅 [Multi-Agent Routing](/en/concepts/multi-agent)。
...
委托的 `AGENTS.md` 定义了其自主权限——即它可以未经许可执行的操作、需要批准的操作以及被禁止的操作。[Cron Jobs](/en/automation/cron-jobs) 驱动其日常计划。
...
4. **为自主操作定义 [长期命令](/en/automation/standing-orders)**。
5. **为周期性任务安排 cron 作业**。
