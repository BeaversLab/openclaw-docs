...
这将从个人用途到组织部署扩展了 [多智能体路由](/zh/concepts/multi-agent)。
...
- 遵循 **[常驻命令](/zh/automation/standing-orders)** - 在智能体的 `AGENTS.md` 中定义的规则，指定它可以自主执行哪些操作与哪些操作需要人工批准（有关计划执行，请参阅 [Cron 作业](/zh/automation/cron-jobs)）。
...
此层级结合了第 2 层级的权限与 [Cron 作业](/zh/automation/cron-jobs) 和 [常驻命令](/zh/automation/standing-orders)。
...
请参阅 [沙箱隔离](/zh/gateway/sandboxing) 和 [多智能体沙箱与工具](/zh/tools/multi-agent-sandbox-tools)。

### 审计跟踪

在委派代理处理任何真实数据之前配置日志记录：

- Cron 运行历史记录：OpenClaw 共享 SQLite 状态数据库
...
为委托创建一个专用的用户帐户（例如，`delegate@[organization].org`）。
...
注册一个具有 `Mail.Read` 和 `Calendars.Read` 应用程序权限的 Azure AD 应用程序。**在使用应用程序之前**，请使用[应用程序访问策略](https://learn.microsoft.com/graph/auth-limit-mailbox-access) 限定访问范围，将应用程序限制为仅访问委托和主体邮箱：
...
<Warning>如果没有应用程序访问策略，`Mail.Read` 应用程序权限将授予对 **租户中每个邮箱** 的访问权限。始终在应用程序读取任何邮件之前创建访问策略。通过确认应用程序为安全组之外的邮箱返回 `403` 来进行测试。</Warning>
...
使用 [多智能体路由](/zh/concepts/multi-agent) 绑定将入站消息路由到委托智能体：
...
切勿将主代理的 `agentDir` 与委托人共享。有关身份验证隔离的详细信息，请参阅 [Multi-Agent Routing](/zh/concepts/multi-agent)。
...
委托人的 `AGENTS.md` 定义了其自主权限——即它可以不经询问执行的操作、需要批准的操作以及禁止的操作。[Cron Jobs](/zh/automation/cron-jobs) 驱动其日常计划。

如果您授予 `sessions_history`OpenClaw，请记住这是一个受限的、经过安全过滤的
回溯视图。OpenClaw 会编辑凭据/令牌类文本，截断长
内容，去除思考标签 / `<relevant-memories>` 脚手架 / 纯文本
工具调用 XML 载荷（包括 `<tool_call>...</tool_call>`、
`<function_call>...</function_call>`、 `<tool_calls>...</tool_calls>`、
`<function_calls>...</function_calls>`MiniMax 以及截断的工具调用块）/
降级的工具调用脚手架 / 泄露的 ASCII/全角模型控制
令牌 / 助手回溯中格式错误的 MiniMax 工具调用 XML，并可以
...
4. **定义 [长期指令](/zh/automation/standing-orders)** 以实现自主操作。
5. **为周期性任务安排 cron 作业**。
