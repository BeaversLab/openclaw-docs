...
这将[多智能体路由](/en/concepts/multi-agent)从个人使用扩展到组织部署。
...
- 遵循**[常备指令](/en/automation/standing-orders)** —— 在智能体 `AGENTS.md` 中定义的规则，指定其可以自主执行哪些操作、哪些操作需要人工批准（有关计划执行，请参见[Cron Jobs](/en/automation/cron-jobs)）。
...
此层级结合了第 2 层级的权限与[Cron Jobs](/en/automation/cron-jobs)和[Standing Orders](/en/automation/standing-orders)。
...
请参见[沙箱隔离](/en/gateway/sandboxing)和[Multi-Agent 沙箱 & Tools](/en/tools/multi-agent-sandbox-tools)。
...
使用[多智能体路由](/en/concepts/multi-agent)绑定将入站消息路由到代表智能体：
...
切勿将主智能体的 `agentDir` 与代表共享。有关身份验证隔离的详细信息，请参见[多智能体路由](/en/concepts/multi-agent)。
...
代表的 `AGENTS.md` 定义了其自主权限 —— 即它可以不经询问执行的操作、需要批准的操作以及禁止的操作。[Cron Jobs](/en/automation/cron-jobs) 驱动其日常计划。
...
4. **定义[常备指令](/en/automation/standing-orders)**以进行自主操作。
5. **为定期任务安排 cron 作业**。
