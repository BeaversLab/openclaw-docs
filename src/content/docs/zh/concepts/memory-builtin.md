...
默认情况下，内置引擎使用 OpenAI 嵌入。如果您已经配置了 `OPENAI_API_KEY` 或 `models.providers.openai.apiKey`，则向量搜索无需额外的内存配置即可工作。
...
| 本地           | `local`             | 可选 `node-llama-cpp` 运行时        |
| Mistral        | `mistral`           |                                     |
...
设置 `memorySearch.provider` 以切换离开 OpenAI。

## 索引如何工作

OpenClaw 将 `MEMORY.md` 和 `memory/*.md` 索引为分块（约 400 个 token，具有 80 个 token 的重叠），并将它们存储在每个代理的 SQLite 数据库中。

- **索引位置：** `~/.openclaw/memory/<agentId>.sqlite`
- **存储维护：** SQLite WAL 附属文件通过定期和关闭检查点进行限制。
- **文件监视：** 对内存文件的更改会触发防抖重新索引（1.5秒）。
- **自动重新索引：** 当嵌入提供商、模型或分块配置更改时，整个索引将自动重建。
...
如果您需要重排序（reranking）、查询扩展（query expansion），或者想要索引工作区之外的目录，请考虑切换到 [QMD](/zh/concepts/memory-qmd)。

如果您需要带有自动用户建模的跨会话记忆，请考虑使用 [Honcho](/zh/concepts/memory-honcho)。

## 故障排除

**记忆搜索被禁用？** 请检查 `openclaw memory status`API。如果未检测到提供商，请显式设置一个或添加 API 密钥。

**未检测到本地提供商？** 请确认本地路径存在并运行：
...
独立的 CLI 命令和 Gateway 使用相同的 CLIGateway(网关)`local` 提供商 ID。当您需要本地嵌入时，请设置 `memorySearch.provider: "local"`。

**结果过时？** 运行 `openclaw memory index --force` 以重建索引。监视器在极少数边缘情况下可能会错过更改。

**sqlite-vec 未加载？** OpenClaw 会自动回退到进程内余弦相似度计算。OpenClaw`openclaw memory status --deep` 会将本地向量存储与嵌入提供商分开报告，因此 `Vector store: unavailable` 指向 sqlite-vec 的加载情况，而 `Embeddings: unavailable` 指向提供商/身份验证或模型就绪状态。请检查日志以获取具体的加载错误。
...
- [主动记忆](/zh/concepts/active-memory)
