...
- 消息传递通道可以选择队列模式（collect/steer/followup）来馈送到此车道系统。
...
- 强制执行特定于模型的限制和压缩保留令牌。
- 请参阅 [System prompt](/en/concepts/system-prompt) 了解模型看到的内容。
...
- 消息工具重复项会从最终负载列表中删除。
- 如果没有剩余可渲染的负载且工具出错，则会发出回退工具错误回复
  （除非消息工具已发送用户可见的回复）。
...
- Agent 运行时：`agents.defaults.timeoutSeconds` 默认为 172800s（48 小时）；在 `runEmbeddedPiAgent` 中止计时器中强制执行。
