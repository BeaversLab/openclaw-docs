...
- 强制执行特定于模型的限制和压缩保留令牌。
- 请参阅 [System prompt](/zh/concepts/system-prompt) 以了解模型看到的内容。
...
请参阅 [Plugin hooks](/zh/plugins/architecture#provider-runtime-hooks) 了解 hook API 和注册详细信息。
...
- 请参阅 [Streaming](/zh/concepts/streaming) 了解分块和块回复行为。
...
- 请参阅 [Compaction](/zh/concepts/compaction) 了解压缩管道。
...
- [Hooks](/zh/automation/hooks) — 由代理生命周期事件触发的事件驱动脚本
- [Compaction](/zh/concepts/compaction) — 长对话如何被总结
