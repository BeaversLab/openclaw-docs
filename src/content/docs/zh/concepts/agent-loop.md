...
2. `agentCommand` 运行 Agent：
   ...
   - 加载技能快照
   - 调用 `runEmbeddedAgent`OpenClaw（OpenClaw 代理运行时）
   - 如果嵌入的循环未发出事件，则发出 **生命周期结束/错误**
3. `runEmbeddedAgent`：
   - 通过每个会话 + 全局队列串行化运行
   - 解析模型 + 身份验证配置文件并构建 OpenClaw 会话
   - 订阅运行时事件并流式传输助手/工具增量
   - 强制执行超时 -> 如果超时则中止运行
   - 对于 Codex app-server 轮次，中止已接受的轮次，该轮次在终止事件之前停止产生 app-server 进度
   - 返回 payloads + usage metadata
4. `subscribeEmbeddedAgentSession`OpenClaw 将代理运行时事件桥接到 OpenClaw `agent` 流：
...
- 助手增量从代理运行时流式传输，并作为 `assistant` 事件发出。
...
- `lifecycle`：由 `subscribeEmbeddedAgentSession` 发出（作为后备，也由 `agentCommand` 发出）
...
- 代理运行时：`agents.defaults.timeoutSeconds` 默认 172800s（48 小时）；在 `runEmbeddedAgent` 中止计时器中强制执行。
- Cron 运行时：孤立的 agent-turn `timeoutSeconds` 归 cron 所有。调度器在执行开始时启动该计时器，在配置的截止时间中止底层运行，然后在记录超时之前运行有界的清理工作，以确保过时的子会话不会阻塞通道。
- 会话活跃度诊断：启用诊断后，`diagnostics.stuckSessionWarnMs` 将未观察到回复、工具、状态、块或 ACP 进度的长时间 `processing` 会话进行分类。活动的嵌入式运行、模型调用和工具调用报告为 `session.long_running`；无近期进度报告的活动工作报告为 `session.stalled`；`session.stuck` 保留用于可恢复的陈旧会话记账，包括具有无主陈旧模型/工具活动的空闲排队会话。陈旧会话记账在恢复通过通过后立即释放受影响的会话通道；仅在 `diagnostics.stuckSessionAbortMs`（默认：至少 5 分钟和警告阈值的 3 倍）之后才中止排空停滞的嵌入式运行，以便队列工作可以恢复而不切断仅仅是缓慢的运行。恢复发出结构化的请求/完成结果，且仅当相同的处理世代仍然当前时，诊断状态才被标记为空闲。重复的 `session.stuck` 诊断在会话保持不变时退避。
