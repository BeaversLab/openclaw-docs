...
- [会话修剪](/en/concepts/session-pruning) -- 裁剪工具结果
- [压缩](/en/concepts/compaction) -- 总结长对话
- [会话工具](/en/concepts/session-tool) -- 用于跨会话工作的代理工具
- [会话管理深度剖析](/en/reference/session-management-compaction) --
  存储架构、记录、发送策略、源元数据和高级配置
- [多代理](/en/concepts/multi-agent) — 跨代理的路由和会话隔离
- [后台任务](/en/automation/tasks) — 分离的工作如何创建带有会话引用的任务记录
- [频道路由](/en/channels/channel-routing) — 入站消息如何路由到会话
