...
- 您可以使用 `openclaw security audit` 验证您的私信设置（参见 [security](/zh/cli/security)）。
...
当会话接近自动压缩时，OpenClaw 可以运行一个 **静默内存刷新** 回合，提醒模型将持久化笔记写入磁盘。这仅在工作区可写时运行。请参阅 [Memory](/zh/concepts/memory) 和
...
- 将 `/compact`（可选指令）作为独立消息发送，以汇总较旧的上下文并释放窗口空间。请参阅 [/concepts/compaction](/zh/concepts/compaction)。
