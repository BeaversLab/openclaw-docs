...
预览流式传输还可以包含 **工具进度** 更新——例如“正在搜索网络”、“正在读取文件”或“正在调用工具”等简短状态行——这些内容会在工具运行期间显示在同一条预览消息中，并在最终回复之前出现。这让多步骤的交互过程在视觉上保持活跃，而不是在首次思考预览和最终答案之间保持沉默。

支持的平台：

- **Discord**、**Slack**、**Telegram** 和 **Matrix** 在预览流式传输激活时，默认会将工具进度流式传输到实时预览编辑中。Microsoft Teams 在个人聊天中使用其原生进度流。
