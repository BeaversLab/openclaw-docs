...
3. **按渠道渲染**
   - **Slack：** mrkdwn 标记（粗体/斜体/删除线/代码），链接显示为 `<url|label>`。
...
Markdown 表格在聊天客户端之间支持不一致。使用 `markdown.tables` 来控制每个渠道（以及每个账户）的转换。

- `code`：将表格渲染为代码块（大多数渠道的默认设置）。
- `bullets`：将每一行转换为要点（Signal + WhatsApp 的默认设置）。
...
如果您需要了解更多关于跨渠道的分块行为，请参阅
[Streaming + chunking](/zh/concepts/streaming)。

## 链接策略

- **Slack:** `[label](url)` -> `<url|label>`; 裸 URL 保持不变。解析期间禁用自动链接以避免重复链接。
- **Telegram:** `[label](url)` -> `<a href="url">label</a>` (HTML 解析模式)。
- **Signal:** `[label](url)` -> `label (url)`，除非标签与 URL 匹配。

## 剧透

剧透标记 (`||spoiler||`) 仅针对 Signal 解析，它们映射到
SPOILER 样式范围。其他渠道将其视为纯文本。

## 如何添加或更新渠道格式化程序

1. **解析一次：** 使用共享的 `markdownToIR(...)` 辅助函数，并配合适合渠道的
   选项（自动链接、标题样式、引用块前缀）。
2. **渲染：** 实现一个带有 `renderMarkdownWithMarkers(...)` 的渲染器以及
   一个样式标记映射（或 Signal 样式范围）。
