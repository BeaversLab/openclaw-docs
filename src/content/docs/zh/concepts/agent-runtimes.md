...
4. 如果旧版配置仍包含 **`openai-codex/*` 模型引用**，请使用 `openclaw doctor --fix` 将其修复为 `openai/<model>`；doctor 通过添加提供商/模型范围的 `agentRuntime.id: "codex"` 来保留 Codex 认证路由，其中旧的模型引用暗示了这一点。
5. 如果用户明确提到 **ACP**、**acpx** 或 **Codex ACP 适配器**，请使用带有 `runtime: "acp"` 和 `agentId: "codex"` 的 ACP。
...
有关 OpenAI 系前缀拆分的信息，请参阅 [OpenAI](/zh/providers/openai) 和
[模型提供商](/zh/concepts/model-providers)。有关 Codex 运行时支持
合约，请参阅 [Codex harness runtime](/zh/plugins/codex-harness-runtime#v1-support-contract)。
...
2. 接下来是提供商范围的运行时策略，位于
   `models.providers.<provider>.agentRuntime`。
3. 在 `auto` 模式下，已注册的插件运行时可以声明支持的提供商/模型
   组合。
4. 如果在 `auto` 模式下没有运行时声明一个轮次，OpenClaw 将使用 PI 作为
   兼容运行时。当运行必须严格时，请使用显式的运行时 id。
...
显式的提供商/模型插件运行时将会失败并关闭。例如，在提供商或模型上 `agentRuntime.id: "codex"` 意味着使用 Codex 或出现明确的选择/运行时错误；它绝不会被静默路由回 PI。
...
诸如 `claude-cli/claude-opus-4-7` 之类的旧版引用仍为了兼容性而受到支持，但新配置应保持提供商/模型的规范性，并将执行后端置于提供商/模型运行时策略中。

对于大多数提供商，`auto`OpenAI 模式是有意保守的。OpenAI 代理模型是例外：未设置运行时和 `auto` 都解析为 Codex 驱动程序。显式的 PI 运行时配置仍然是 `openai/*` 代理轮次的可选兼容性路径；当与选定的 `openai-codex`OpenClaw 身份验证配置文件配对时，OpenClaw 在内部通过旧版 Codex-auth 传输路由 PI，同时将公共模型引用保持为 `openai/*`OpenAI。过时的 OpenAI PI 会话固定被运行时选择忽略，并可以使用 `openclaw doctor --fix` 清理。

如果 `openclaw doctor` 警告启用了 `codex` 插件，而
配置中仍然保留了 `openai-codex/*`，请将其视为遗留路由状态。运行
...
| 动态工具钩子（hooks）是否有效？ | 插件期望 `before_tool_call`、`after_tool_call` 以及 OpenClaw 拥有的工具周围有中间件。 |
...
- 诸如 `codex` 之类的运行时 ID 会告诉你哪个循环正在执行该轮次。
