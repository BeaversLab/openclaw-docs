...
`SOUL.md` 是你的代理声音的所在之处。

OpenClaw 会在常规会话中注入它，因此它具有实际分量。如果你的代理听起来平淡无奇、闪烁其词，或者带有奇怪的企业腔，这通常就是你需要修改的文件。
...
- 提示词工程指南指出，高层级行为、语气、目标和示例应属于高优先级指令层，而不应埋没在用户轮次中。
- 同一指南建议将提示词视为需要迭代、固定和评估的对象，而不是写一次就忘记的神奇散文。

对于 OpenClaw，`SOUL.md` 就是那个层。

如果你想要更好的个性，请编写更强的指令。如果你想要稳定的个性，请保持简短并进行版本控制。
...
将其粘贴到你的代理中，让它重写 `SOUL.md`。

路径已针对 OpenClaw 工作空间进行修正：使用 `SOUL.md`，而不是 `http://SOUL.md`。
...
保留 `AGENTS.md` 用于操作规则。保留 `SOUL.md` 用于声音、立场和风格。如果你的代理在共享频道、公开回复或客户界面中工作，请确保语气仍然适合场合。
...
- [SOUL.md 模板](/zh/reference/templates/SOUL)
