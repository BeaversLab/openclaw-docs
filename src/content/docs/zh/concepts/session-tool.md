...
当路由元数据可用时，`session_status` 还包含一个可见的 `Route context` JSON 块和匹配的结构化 `details` 字段。这些字段将会话密钥与当前处理实时运行的路由区分开来：

- `origin` 是会话的创建位置，或者是当较旧状态缺乏存储的源元数据时，从可传递的会话密钥前缀推断出的提供商。
- `active` 是当前的实时运行路由。它仅针对正在处理的实时或当前会话进行报告。
- `deliveryContext` 是存储在会话上的持久化传递路由，即使当前活跃的表面发生变化，OpenClaw 也可以重复使用它进行后续传递。

`sessions_yield` 会故意结束当前轮次，以便下一条消息可以是您正在等待的后续事件。在生成子代理后使用它，当您希望完成结果作为下一条消息到达而不是构建轮询循环时。

`subagents` 是已生成的 OpenClaw 子代理的可见性助手。它支持 `action: "list"` 来检查活跃/最近的运行。

## 生成子代理

`sessions_spawn` 默认为后台任务创建一个隔离的会话。它总是非阻塞的——它立即返回一个 `runId` 和 `childSessionKey`。原生子代理运行在子会话的第一个可见 `[Subagent Task]` 消息中接收委托的任务，而系统提示仅携带子代理运行时规则和路由上下文。

关键选项：

- 用于外部工具代理的 `runtime: "subagent"`（默认）或 `"acp"`。
- 子会话的 `model` 和 `thinking` 覆盖。
- `thread: true` 将生成绑定到聊天线程（Discord、Slack 等）。
- `sandbox: "require"` 以对子代强制执行沙箱隔离。
- `context: "fork"` 适用于原生子代理需要当前
  请求者记录的情况；省略它或使用 `context: "isolated"` 以获得干净的子代理。
  线程绑定的原生子代理默认为 `context: "fork"`，除非
  `threadBindings.defaultSpawnContext` 另有说明。

默认的叶子子代理不会获得会话工具。当
`maxSpawnDepth >= 2` 时，深度为 1 的编排子代理还会获得
`sessions_spawn`、`subagents`、`sessions_list` 和 `sessions_history`，以便它们
能够管理自己的子代理。叶子运行仍然不会获得递归
的编排工具。

完成后，一个公告步骤会将结果发布到请求者的渠道。
完成交付在可用时会保留绑定的线程/主题路由，如果
完成源仅标识了一个渠道，OpenClaw 仍然可以复用
...
默认为 `tree`。沙箱隔离会话被限制为 `tree`，而无论
配置如何。

## 延伸阅读

- [Session Management](/zh/concepts/session) -- 路由、生命周期、维护
- [ACP Agents](/zh/tools/acp-agents) -- 外部工具派生
- [Multi-agent](/zh/concepts/multi-agent) -- 多代理架构
- [Gateway(网关) Configuration](<Gateway(网关)/en/gateway/configuration>) -- 会话工具配置选项
