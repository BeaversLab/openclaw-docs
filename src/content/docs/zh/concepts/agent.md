...
如果启用了 `agents.defaults.sandbox`，非主会话可以使用 `agents.defaults.sandbox.workspaceRoot` 下的每个会话的工作区来覆盖此设置（请参阅 [Gateway(网关) configuration](/zh/gateway/configuration)）。
...
当队列模式为 `followup` 或 `collect` 时，传入消息将被保留，直到当前轮次结束，然后新的代理轮次将使用排队的负载开始。有关模式 + 防抖/上限行为，请参阅 [Queue](/zh/concepts/queue)。
