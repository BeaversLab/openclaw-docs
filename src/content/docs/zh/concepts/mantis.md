...
值是配置的 Mantis R2/S3 构件前缀下的相对路径。发布程序拒绝路径遍历，并在可选预览或视频不可用时跳过标记为 `"required": false`
的条目。

支持的制品类型：

- `timeline`：确定性场景截图，通常用于之前/之后对比。
- `desktopScreenshot`：VNC/浏览器桌面截图。
- `motionPreview`：从桌面录像生成的内联动画 GIF。
- `motionClip`：经过运动修剪的 MP4，移除了静态的开头和结尾。
- `fullVideo`：用于深度检查的完整 MP4 录像。
- `metadata`：JSON/日志附属文件。
- `report`：Markdown 报告。

可重用的发布程序是 `scripts/mantis/publish-pr-evidence.mjs`。工作流使用清单、目标 PR、构件目标根目录、评论标记、Actions 构件 URL、运行 URL 和请求源来调用它。它会将声明的构件上传到配置的 Mantis R2/S3 存储桶，构建一个以摘要为优先的 PR 评论，其中包含内联图片/预览和链接的视频，然后更新现有的标记评论或创建一个新评论。工作流发布到 `openclaw-crabbox-artifacts`，并在 `https://artifacts.openclaw.ai` 下提供公共 URL。它们直接提供存储桶、区域和公共 URL 值。可重用的发布程序需要：
...
`telegram-status-command`。维护人员可以覆盖 `candidate=...`、
...
第一个命令是明确的，并且专注于场景。第二个命令稍后可以将 PR 或问题映射
到推荐的 Mantis 场景，这些场景基于标签、更改的文件和
ClawSweeper 审查结果。
...
11. 运行相同的场景并捕获候选证据。
12. 比较预言机 结果和视觉证据。
13. 编写 Markdown、JSON、日志、屏幕截图和可选的跟踪产物。
...
- **Bug 复现**：基准以预期的方式失败。
- **Harness 故障**：在 bug 预言机有意义之前，环境设置、凭据、Discord API、浏览器或
  提供商已失败。

最终报告必须区分这些情况，以便维护人员不会将不稳定的环境
与产品行为混淆。

## Discord MVP

第一个场景应针对公会频道 中的 Discord 状态反应，其中
源回复传递模式为 `message_tool_only`。

为什么它是 Mantis 的一个很好的种子：

- 它在 Discord 中作为触发消息上的反应是可见的。
- 它通过 Discord 消息反应状态拥有强大的 REST 预言机。
- 它演练了一个真正的 OpenClaw Gateway(网关)、Discord bot auth、消息分发、源回复传递模式、状态反应状态和模型轮次生命周期。
- 它的范围足够窄，可以保持第一次实现的诚实。
...
可执行的第一部分是 opt-in Discord live QA 场景：
...
它使用始终开启的 guild 处理、`visibleReplies:
"message_tool"`, `DiscordackReaction: "👀"` 和显式状态反应来配置 SUT。Oracle 轮询真正的 Discord 触发消息，并期望观察到的序列 `👀 -> 🤔 -> 👍`。制品包括 `discord-qa-reaction-timelines.json`、
`discord-status-reactions-tool-only-timeline.html` 和
`discord-status-reactions-tool-only-timeline.png`。

## 现有的 QA 部件

Mantis 应该建立在现有的私有 QA 堆栈上，而不是从头开始：

- `pnpm openclaw qa discord`Discord 已经运行了一个带有 driver 和
  SUT bot 的 live Discord 通道。
- Live transport runner 已经在 `.artifacts/qa-e2e/` 下写入报告和观察到的消息
  制品。
- Convex 凭证租约已经提供了对共享 live
  传输凭证的独占访问。
- 浏览器控制服务已经支持截图、快照、
  无头托管配置文件和远程 CDP 配置文件。
- QA Lab 已经有一个用于传输形状测试的调试器 UI 和总线。

第一个 Mantis 实现可以是基于这些部件的薄型前后运行器，外加一个可视化证据层。
...
`mantis-summary.json` 应该是机器可读的单一事实来源。Markdown
报告用于 PR 评论和人工审查。
...
屏幕截图是证据，而非机密。它们仍需遵守删减规则：可能会出现私密渠道名称、用户名或消息内容。对于公开 PR，在删减机制更强之前，首选 GitHub Actions 产物链接而非内联图片。

## 浏览器和 VNC

浏览器通道有两种模式：

- **无头自动化**：CI 的默认模式。Chrome 在启用 CDP 的情况下运行，并且 Playwright 或 OpenClaw 浏览器控制会捕获屏幕截图。
- **VNC 救援**：当登录、MFA、Discord 反自动化或可视化调试需要人工干预时，在同一 VM 上启用。

Discord 观察者浏览器配置文件应足够持久，以避免每次运行都要登录，但应与个人浏览器状态隔离。配置文件属于 Mantis 机器池，而不是开发人员的笔记本电脑。
...
首次私有部署可以将这些消息发布到现有的操作员渠道，然后移动到专用的 Mantis 渠道。

## 机器

对于首次远程实现，Mantis 应首选通过 Crabbox 使用 AWS。Crabbox 为我们提供预热机器、租约跟踪、水合、日志、结果和清理。如果 AWS 容量太慢或不可用，请在同一机器接口后面添加 Hetzner 提供商。
...
- 足够的 CPU 和内存，用于运行一个 OpenClaw Gateway(网关)、一个浏览器和一个模型运行
- 对 Discord、GitHub、模型提供商和凭证代理的出站访问权限

除了预期的凭证或浏览器配置文件存储外，VM 不应保留长期的原始机密。

## 机密

Secrets 位于 GitHub 组织或仓库的 secrets 中，用于远程运行；对于本地运行，则位于本地操作员控制的 secret 文件中。
...
- `OPENCLAW_QA_REDACT_PUBLIC_METADATA=1` 用于公共 GitHub 构件上传
- `OPENCLAW_QA_CONVEX_SITE_URL`
- `OPENCLAW_QA_CONVEX_SECRET_CI`
- `OPENCLAW_QA_MANTIS_CRABBOX_COORDINATOR`
- `OPENCLAW_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`

从长远来看，Convex 凭证池应该仍然是实时传输凭证的正常来源。GitHub secrets 为代理和备用通道提供引导。Discord status-reactions 工作流将 Mantis Crabbox secrets 映射回 Crabbox CLI 所期望的 `CRABBOX_COORDINATOR` 和 `CRABBOX_COORDINATOR_TOKEN` 环境变量。简单的 `CRABBOX_*` GitHub secret 名称仍然被接受作为兼容性回退。
...
公共构件上传还应编辑 Discord 目标元数据，如 bot、guild、渠道 和 message ids。GitHub smoke 工作流为此启用了 `OPENCLAW_QA_REDACT_PUBLIC_METADATA=1`。

如果 token 意外粘贴到 issue、PR、聊天或日志中，请在存储新 secret 后轮换它。

## GitHub artifacts 和 PR comments

Mantis 工作流应将完整的证据包作为短期 Actions 构件上传。当为 bug 报告或修复 PR 运行工作流时，它还应将编辑过的内联媒体发布到配置的 Mantis R2/S3 存储桶，并使用内联的前后截图在该 bug 或修复 PR 上插入一条评论。不要仅在通用的 QA 自动化 PR 上发布主要证明。原始日志、观察到的消息和其他庞大的证据保留在 Actions 构件中。

生产工作流应该使用 Mantis GitHub App 来发布这些评论，而不是使用 `github-actions[bot]`。将 app id 和 private key 存储为 `MANTIS_GITHUB_APP_ID` 和 `MANTIS_GITHUB_APP_PRIVATE_KEY` GitHub Actions 密钥。工作流使用隐藏标记作为更新键，当令牌可以编辑该评论时更新它，而当较旧的机器人拥有的标记无法编辑时，则创建一个新的 Mantis 拥有的评论。
...
当运行因工具失败而失败时，评论必须说明这一点，而不是暗示候选项失败。

## 私有部署说明

私有部署可能已经有一个 Mantis Discord 应用程序。当它具有正确的机器人权限并且可以安全轮换时，请重用该应用程序，而不是创建另一个应用程序。

通过密钥或部署配置设置初始操作员通知渠道。它可以首先指向现有的维护人员或操作渠道，然后一旦存在专用的 Mantis 渠道，就移动到该渠道。

请勿将公会 ID、渠道 ID、机器人令牌、浏览器 Cookie 或 VNC 密码放在本文档中。将它们存储在 GitHub 密钥、凭证代理或操作员的本地密钥存储中。
...
- 当 UI 是唯一可靠的观察对象时，使用浏览器截图

视觉检查应该是补充性的。如果平台 API 可以证明该 Bug，请使用 API 作为通过/失败预言机，并保留截图以供人工确信。

## Provider expansion

在 Discord 之后，同一运行器可以添加：

- Slack：表情反应、消息线程、应用提及、模态框、文件上传。
- 电子邮件：在连接器不足以满足需求时，使用 `gog` 进行 Gmail 身份验证和消息线程处理。
- WhatsApp：二维码登录、重新识别、消息投递、媒体、表情反应。
- Telegram：群组提及限制、命令、以及支持情况下的表情反应。
- Matrix：加密房间、线程或回复关系、重启恢复。

每种传输方式都应包含一个低成本的冒烟测试场景和一个或多个 Bug 类场景。高成本的可视化场景应保持可选。

## 未决问题

- 当复用现有的 Mantis 机器人时，哪个 Discord 机器人应该作为驱动程序，哪个应该作为被测系统（SUT）？
- 观察者浏览器登录应使用人类 Discord 账户、测试账户，还是在第一阶段仅使用机器人可读的 REST 证据？
- GitHub 应该为 PR 保留 Mantis 制品多久？
- ClawSweeper 何时应自动推荐 Mantis，而不是等待维护者命令？
- 在上传到公共 PR 之前，屏幕截图是否应被编辑或裁剪？
