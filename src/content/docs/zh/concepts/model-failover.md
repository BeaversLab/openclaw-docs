...
对 ChatGPT/Codex OAuth 配置文件和 OpenAI API-key 配置文件均使用 `openai:*`。当订阅达到 Codex 使用限制时，OpenClaw 会在 Codex 提供时记录精确的重置时间，尝试下一个有序的认证配置文件，并保持运行在 Codex 框架内。一旦重置时间过去，订阅配置文件将再次符合资格，下一次自动选择可以恢复使用它。
...
<AccordionGroup>
  <Accordion title="归入速率限制/超时类别的项目">
    该速率限制类别比单纯的 `429` 更广泛：它还包括提供商消息，例如 `Too many concurrent requests`、`ThrottlingException`、`concurrency limit reached`、`workers_ai ... quota limit exceeded`、`throttled`、`resource exhausted`，以及周期性使用窗口限制，例如 `weekly/monthly limit reached`。

    格式/无效请求错误通常是致命的，因为重试相同的负载会以同样的方式失败，因此 OpenClaw 会直接显示这些错误，而不是轮换身份验证配置文件。已知的重试修复路径可以显式选择加入：例如，Cloud Code Assist 工具调用 ID 验证失败会被清理，并通过 `allowFormatRetry` 策略重试一次。OpenAI 兼容的停止原因错误（如 `Unhandled stop reason: error`、`stop reason: error` 和 `reason: error`）被归类为超时/故障转移信号。

    当源匹配已知的瞬态模式时，通用服务器文本也可能归入该超时类别。例如，裸露的模型运行时流包装器消息 `An unknown error occurred` 对于每个提供商都被视为值得进行故障转移，因为当提供商流以 `stopReason: "aborted"` 或 `stopReason: "error"` 结束而没有具体细节时，共享模型运行时会发出此消息。包含瞬态服务器文本（例如 `internal server error`、`unknown error, 520`、`upstream error` 或 `backend error`）的 JSON `api_error` 负载也被视为值得进行故障转移的超时。

    OpenRouter 特定的通用上游文本（例如裸露的 `Provider returned error`）仅在提供商上下文实际为 OpenRouter 时才被视为超时。通用内部回退文本（例如 `LLM request failed with an unknown error.`）保持保守，不会自行触发故障转移。

  </Accordion>
  <Accordion title="SDK retry-after caps">
    某些提供商 SDK 可能会在将控制权返回给 OpenClaw 之前休眠一个很长的 `Retry-After` 窗口。对于基于 Stainless 的 SDK，例如 Anthropic 和 OpenAI，OpenClaw 默认将 SDK 内部的 `retry-after-ms` / `retry-after` 等待时间上限设定为 60 秒，并立即暴露更长的可重试响应，以便运行此故障转移路径。使用 `OPENCLAW_SDK_RETRY_MAX_WAIT_SECONDS` 调整或禁用该上限；请参阅 [Retry behavior](/zh/concepts/retry)。
  </Accordion>
  <Accordion title="Model-scoped cooldowns">
    速率限制冷却也可以限定于模型范围：

    - 当失败的模型 id 已知时，OpenClaw 会为速率限制失败记录 `cooldownModel`。
    - 当冷却限定于不同的模型时，同一提供商上的兄弟模型仍可尝试。
...
<Note>
并非每个计费形式的响应都是 `402`，也并非每个 HTTP `402` 都会落在此处。即使提供商返回 `401` 或 `403`，OpenClaw 仍会将明确的计费文本保留在计费通道中，但特定于提供商的匹配器仅限于拥有它们的提供商（例如 OpenRouter `403 Key limit exceeded`）。

同时，临时的 `402` 使用量窗口和组织/工作区支出限制错误会在消息看起来可重试时（例如 `weekly usage limit exhausted`、`daily limit reached, resets tomorrow` 或 `organization spending limit exceeded`）被归类为 `rate_limit`。这些错误将停留在短冷却/故障转移路径上，而不是长计费禁用路径上。
...
如果某个提供商的所有配置文件均失败，OpenClaw 将切换到 `agents.defaults.model.fallbacks` 中的下一个模型。这适用于身份验证失败、速率限制以及耗尽了配置文件轮换的超时（其他错误不会触发后备）。未暴露足够细节的提供商错误在后备状态下仍会被精确标记：`empty_response` 表示提供商未返回可用的消息或状态，`no_error_details` 表示提供商显式返回了 `Unknown error (no error details in response)`，而 `unclassified` 表示 OpenClaw 保留了原始预览，但尚未有分类器匹配它。

过载和速率限制错误的处理比计费冷却更激进。默认情况下，OpenClaw 允许进行一次同一提供商的身份验证配置文件重试，随后立即切换到下一个配置的模型后备，无需等待。提供商繁忙信号（例如 `ModelNotReadyException`）会被归入该过载类别。您可以使用 `auth.cooldowns.overloadedProfileRotations`、`auth.cooldowns.overloadedBackoffMs` 和 `auth.cooldowns.rateLimitedProfileRotations` 对此进行调整。

当运行从配置的默认主模型、定时任务主模型、具有显式后备的代理主模型或自动选择的后备覆盖启动时，OpenClaw 可以遍历匹配的配置后备链。没有显式后备的代理主模型和显式用户选择（例如 `/model ollama/qwen3.5:27b`、模型选择器 `sessions.patch`，或一次性 CLI 提供商/模型覆盖）是严格的：如果该提供商/模型无法访问或在生成回复之前失败，OpenClaw 将报告失败，而不是使用不相关的后备进行回复。

### 候选链规则

OpenClaw 根据当前请求的 `provider/model` 以及配置的后备来构建候选列表。

<AccordionGroup>
  <Accordion title="规则"OpenClaw>
    - 请求的模型始终排在第一位。
    - 显式配置的回退模型会被去重，但不会受模型允许列表的过滤。它们被视为显式的操作者意图。
    - 如果当前运行已经在同一提供商系列中的某个已配置回退模型上，OpenClaw 将继续使用完整的配置链。
    - 当未提供显式回退覆盖时，即使请求的模型使用不同的提供商，也会在配置的主模型之前尝试配置的回退模型。
    - 当未向回退运行器提供显式回退覆盖时，配置的主模型会被附加到末尾，以便在早期的候选项耗尽后，链路可以回落到正常的默认值。
    - 当调用者提供 `fallbacksOverride` 时，运行器将仅使用请求的模型以及该覆盖列表。空列表将禁用模型回退，并防止将配置的主模型作为隐藏的重试目标进行附加。
...
<Tabs>
  <Tab title="继续于">
    ...
    - 超时形式的故障转移错误
    - 计费禁用
    - `LiveSessionModelSwitchError`，它被规范化为故障转移路径，以便过时的持久化模型不会创建外层重试循环
    - 当仍有剩余候选项时的其他未识别错误
...
<AccordionGroup>
  <Accordion title="Per-candidate decisions">
    - 持续的认证失败会立即跳过整个提供商。
    - 计费禁用通常会被跳过，但可能会在限流状态下探测主要候选，以便在不重新启动的情况下恢复。
    - 可能会在冷却期即将到期时探测主要候选，并带有针对每个提供商的限流。
    - 当故障看起来是瞬时的（`rate_limit`、`overloaded` 或未知）时，即使处于冷却期，也可以尝试同一提供商下的后备同级模型。当速率限制特定于模型且同级模型可能立即恢复时，这一点尤为重要。
    - 每次后备运行中，针对每个提供商的瞬时冷却探测仅限一次，以免单个提供商拖延跨提供商的后备。
...
- 只有显式的用户驱动的模型更改才会标记挂起的实时切换。这包括 `/model`、`session_status(model=...)` 和 `sessions.patch`。
...
- 自动后备覆盖会在后续轮次中保持选中状态，这样 OpenClaw 就不会在每条消息时探测已知不良的主要模型。OpenClaw 会定期重新探测配置的原始源，并在其恢复时清除自动覆盖；`/new`、`/reset` 和 `sessions.reset` 会立即清除自动来源的覆盖。
- 用户回复会在每次状态更改时公布故障转移转换和已清除故障转移的恢复。粘性故障转移轮次不会重复该通知。
- `/status` 显示所选模型，并且在后备状态不同时，显示活动后备模型及其原因。
...
结构化 `model_fallback_decision` 日志在候选对象失败、被跳过或后续故障转移成功时，还包含扁平的 `fallbackStep*` 字段。这些字段明确显示了尝试的转换（`fallbackStepFromModel`、`fallbackStepToModel`、`fallbackStepFromFailureReason`、`fallbackStepFromFailureDetail`、`fallbackStepFinalOutcome`），以便日志和诊断导出器即使最终的故障转移也失败了，也能重构主要故障。

当所有候选对象都失败时，OpenClaw 会抛出 `FallbackSummaryError`。外部回复运行程序可以使用该错误来构建更具体的消息，例如“所有模型暂时都受到速率限制”，并在已知的情况下包含最近的冷却到期时间。
...
有关更广泛的模型选择和故障转移概述，请参阅 [Models](/zh/concepts/models)。
