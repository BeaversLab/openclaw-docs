...
- 故障转移运行时规则、冷却探测和会话覆盖持久性
  记录在 [/concepts/模型-failover](/zh/concepts/model-failover) 中。
...
- 注意：提供商运行时 `capabilities` 是共享的运行器元数据（提供商系列、转录/工具怪癖、传输/缓存提示）。它不同于 [public capability 模型](/zh/plugins/architecture#public-capability-model)，后者描述插件注册的内容（文本推理、语音等）。
...
OpenClaw 使用 LM Studio 原生的 `/api/v1/models` 和 `/api/v1/models/load` 进行发现和自动加载，默认使用 `/v1/chat/completions` 进行推理。有关设置和故障排除，请参阅 [/providers/lmstudio](/zh/providers/lmstudio)。
...
当您选择使用 `OLLAMA_API_KEY` 时，Ollama 会在本地 `http://127.0.0.1:11434` 被检测到，并且捆绑的提供商插件会将 Ollama 直接添加到 `openclaw onboard` 和模型选择器中。有关新手引导、云端/本地模式和自定义配置，请参阅 [/providers/ollama](/zh/providers/ollama)。
...
另请参阅：[/gateway/configuration](/zh/gateway/configuration) 以获取完整配置示例。
