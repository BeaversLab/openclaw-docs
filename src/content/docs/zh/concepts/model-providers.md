...
- 注意：提供商运行时 `capabilities` 是共享的运行器元数据（提供商系列、转录/工具怪癖、传输/缓存提示）。它不同于 [public capability 模型](/en/plugins/architecture#public-capability-model)，后者描述插件注册的内容（文本推理、语音等）。
- 捆绑的 `codex` 提供商与捆绑的 Codex 代理工具搭配使用。
  当您需要 Codex 拥有的登录、模型发现、原生会话恢复和应用服务器执行时，请使用 `codex/gpt-*`。普通的 `openai/gpt-*` 引用继续使用 OpenAI 提供商和普通的 OpenClaw 提供商传输。
  仅 Codex 部署可以通过 `agents.defaults.embeddedHarness.fallback: "none"` 禁用自动 PI 回退；请参阅
...
- `normalizeTransport`: 提供商在通用模型组装之前规范化传输系列 `api` / `baseUrl`；OpenClaw 首先检查匹配的提供商，然后检查其他支持挂钩的提供商插件，直到其中一个实际更改传输
- `normalizeConfig`: 提供商在运行时使用 `models.providers.<id>` 配置之前对其进行规范化；OpenClaw 首先检查匹配的提供商，然后检查其他支持挂钩的提供商插件，直到其中一个实际更改配置。如果没有提供商挂钩重写配置，捆绑的 Google 系列辅助工具仍会规范化支持的 Google 提供商条目。
- `applyNativeStreamingUsageCompat`: 提供商为配置提供商应用端点驱动的原生流式使用兼容性重写
- `resolveConfigApiKey`: 提供商解析配置提供商的环境标记身份验证，而无需强制进行完整的运行时身份验证加载。`amazon-bedrock` 此处还有一个内置的 AWS 环境标记解析器，尽管 Bedrock 运行时身份验证使用 AWS SDK 默认链。
- `resolveSyntheticAuth`: 提供商可以暴露本地/自托管或其他配置支持的身份验证可用性，而无需持久化明文密钥
- `shouldDeferSyntheticProfileAuth`: 提供商可以将存储的合成配置文件占位符标记为比环境/配置支持的身份验证优先级更低
- `resolveDynamicModel`: 提供商接受本地静态目录中尚未出现的模型 ID
- `prepareDynamicModel`: 提供商在重试动态解析之前需要元数据刷新
- `normalizeResolvedModel`: 提供商需要传输或基础 URL 重写
- `contributeResolvedModelCompat`：提供商为其供应商模型提供兼容性标志，即使它们是通过另一个兼容的传输方式到达的
- `capabilities`：提供商发布转录/工具/提供商系列的特性
- `normalizeToolSchemas`：提供商在嵌入式运行器看到之前清理工具架构
- `inspectToolSchemas`：提供商在规范化之后显示特定于传输的架构警告
- `resolveReasoningOutputMode`：提供商选择原生或标记的推理输出合约
...
- `createEmbeddingProvider`：当内存嵌入行为属于提供商插件而不是核心嵌入交换板时，提供商拥有该行为的所有权
- `formatApiKey`：提供商将存储的身份验证配置文件格式化为传输所期望的运行时 `apiKey` 字符串
...
- `classifyFailoverReason`：提供商将特定于提供商的原始传输/API 错误映射到故障转移原因，例如速率限制或过载
- `isCacheTtlEligible`：提供商决定哪些上游模型 ID 支持 prompt-cache TTL
- `buildMissingAuthMessage`：提供商使用特定于提供商的恢复提示替换通用身份验证存储错误
- `suppressBuiltInModel`：提供商隐藏过时的上游行，并可以为直接解析失败返回供应商拥有的错误
- `augmentModelCatalog`：提供商在发现和配置合并后附加合成/最终目录行
- `isBinaryThinking`：提供商拥有二进制开/关思考 UX
- `supportsXHighThinking`：提供商选择加入所选模型到 `xhigh`
- `resolveDefaultThinkingLevel`：提供商拥有模型系列的默认 `/think` 策略
- `applyConfigDefaults`：提供商在配置具体化期间根据身份验证模式、环境或模型系列应用特定于提供商的全局默认值
...
- `anthropic`：Claude 4.6 前向兼容回退、身份修复提示、使用端点获取、缓存 TTL/提供商系列元数据以及感知身份的全局配置默认值
- `amazon-bedrock`：提供商拥有的上下文溢出匹配和针对 Bedrock 特定的限流/未就绪错误的故障转移原因分类，以及用于 Anthropic 流量上的仅 Claude 重播策略防护的共享 `anthropic-by-model` 重播系列
- `anthropic-vertex`：Anthropic 消息流量上的仅 Claude 重播策略防护
- `openrouter`：透传模型 ID、请求包装器、提供商功能提示、代理 Gemini 流量上的 Gemini 思考签名清理、通过 `openrouter-thinking` 流系列的代理推理注入、路由元数据转发以及缓存 TTL 策略
- `github-copilot`: 新手引导/设备登录，模型的前向兼容回退，
  Claude 思考记录提示，运行时令牌交换，以及使用端点
  获取
- `openai`: GPT-5.4 前向兼容回退，直接 OpenAI 传输
  规范化，识别 Codex 的缺失身份验证提示，Spark 抑制，合成
  OpenAI/Codex 目录行，思考/实时模型策略，使用令牌别名
  规范化（`input` / `output` 和 `prompt` / `completion` 系列），
  用于原生 OpenAI/Codex 封装的共享 `openai-responses-defaults` 流系列，提供商系列元数据，
  用于 `gpt-image-1` 的捆绑图像生成提供商
  注册，以及用于 `sora-2` 的捆绑视频生成提供商
  注册
- `google` 和 `google-gemini-cli`：Gemini 3.1 前向兼容回退，
  原生 Gemini 重放验证，引导重放清理，标记
  推理输出模式，现代模型匹配，用于 Gemini 图像预览模型的捆绑图像生成
  提供商注册，以及用于 Veo 模型的捆绑
  视频生成提供商注册；Gemini CLI OAuth 还
  拥有用于使用界面的身份验证配置文件令牌格式化、使用令牌解析和配额端点
  获取
- `moonshot`: 共享传输，插件拥有的思考负载规范化
- `kilocode`: 共享传输，插件拥有的请求头，推理负载
  规范化，代理 Gemini 思考签名清理，以及缓存 TTL
  策略
- `zai`: GLM-5 前向兼容回退，`tool_stream` 默认值，缓存 TTL
  策略，二进制思考/实时模型策略，以及使用身份验证 + 配额获取；
  未知的 `glm-5*` id 从捆绑的 `glm-4.7` 模板合成
- `xai`：原生 Responses 传输标准化，`/fast` 针对 Grok 快速变体的别名重写，默认 `tool_stream`，xAI 特定的工具模式 / 推理负载清理，以及针对 `grok-imagine-video` 的捆绑视频生成提供商注册
- `mistral`：插件拥有的能力元数据
- `opencode` 和 `opencode-go`：插件拥有的能力元数据以及代理 Gemini 思维签名清理
- `alibaba`：插件拥有的视频生成目录，用于直接引用 Wan 模型（例如 `alibaba/wan2.6-t2v`）
- `byteplus`：插件拥有的目录，以及针对 Seedance 文本生成视频/图像生成视频模型的捆绑视频生成提供商注册
- `fal`：针对托管第三方图像生成提供商 FLUX 图像模型的捆绑视频生成提供商注册，以及针对托管第三方视频模型的捆绑视频生成提供商注册
- `cloudflare-ai-gateway`、`huggingface`、`kimi`、`nvidia`、`qianfan`、
  `stepfun`、`synthetic`、`venice`、`vercel-ai-gateway` 和 `volcengine`：
  仅包含插件拥有的目录
- `qwen`：用于文本模型的插件拥有目录，以及为其多模态界面共享的媒体理解和视频生成提供商注册；Qwen 视频生成使用标准 DashScope 视频端点，并捆绑诸如 `wan2.6-t2v` 和 `wan2.7-r2v` 等 Wan 模型
- `runway`：针对原生 Runway 基于任务的模型（例如 `gen4.5`）的插件拥有视频生成提供商注册
...
捆绑的 `openai` 插件现在拥有两个提供商 ID：`openai` 和 `openai-codex`。
...
- 密钥选择顺序保留优先级并对值进行去重。
- 仅在速率限制响应时才使用下一个密钥重试请求（例如 `429`、`rate_limit`、`quota`、`resource exhausted`、`Too many
concurrent requests`, `ThrottlingException`, `concurrency limit reached`、`workers_ai ... quota limit exceeded` 或定期使用限制消息）。
- 非速率限制的失败会立即失败；不会尝试密钥轮换。
- 当所有候选密钥都失败时，将返回最后一次尝试的最终错误。

## 内置提供商（pi-ai 目录）

OpenClaw 随附 pi-ai 目录。这些提供商**不**需要 `models.providers` 配置；只需设置认证并选择一个模型。
...
- 隐藏的 OpenClaw 归属标头（`originator`、`version`、
  `User-Agent`）仅适用于发送至 `api.openai.com` 的原生 OpenAI 流量，而不适用于
  通用的 OpenAI 兼容代理
...
- Anthropic 说明：Anthropic 员工告知我们 OpenClaw 风格的 Claude CLI 使用再次被允许，因此除非 Anthropic 发布新政策，否则 OpenClaw 将此集成中的 Claude CLI 复用和 `claude -p` 使用视为已获批准。
- Anthropic setup-token 仍然作为受支持的 OpenClaw 令牌路径可用，但在可用时 OpenClaw 现在更倾向于复用 Claude CLI 和 `claude -p`。
...
- 提供商：`openai-codex`
- 身份验证：OAuth (ChatGPT)
- 模型示例：`openai-codex/gpt-5.4`
- CLI：`openclaw onboard --auth-choice openai-codex` 或 `openclaw models auth login --provider openai-codex`
- 默认传输方式是 `auto`（优先使用 WebSocket，SSE 作为回退）
...
- 隐藏的 OpenClaw 归因头部（`originator`、`version`、
  `User-Agent`）仅附加到 `chatgpt.com/backend-api` 的原生 Codex 流量上，
  而非通用的 OpenAI 兼容代理
...
- [Qwen Cloud](/en/providers/qwen)：Qwen Cloud 提供商界面以及阿里云 DashScope 和 Coding Plan 端点映射
...
- 直接的 Gemini 运行也接受 `agents.defaults.models["google/<model>"].params.cachedContent`
...
- Gemini CLI OAuth 作为捆绑的 `google` 插件的一部分提供。
  ...
  - 注意：**不要**将客户端 ID 或密钥粘贴到 `openclaw.json` 中。CLI 登录流程会将
    令牌存储在网关主机上的身份验证配置文件中。
...
- `kilocode/kilo/auto` 背后的精确上游路由由 Kilo Gateway(网关) 拥有，而非在 OpenClaw 中硬编码。
...
- OpenClaw 仅当请求实际针对 `openrouter.ai` 时，才会应用 OpenRouter 记录的应用归因标头
- OpenRouter 特有的 Anthropic `cache_control` 标记同样仅限于已验证的 OpenRouter 路由，而非任意的代理 URL
- OpenRouter 仍保留在代理风格的 OpenAI 兼容路径上，因此原生 OpenAI 专用的请求整形（`serviceTier`、Responses `store`、提示缓存提示、OpenAI 推理兼容负载）不会被转发
- Gemini 支持的 OpenRouter 引用仅保留代理 Gemini 的思维签名清理；
  原生 Gemini 重放验证和引导重写保持关闭状态
- Kilo Gateway(网关)：`kilocode` (`KILOCODE_API_KEY`)
- 示例模型：`kilocode/kilo/auto`
- 由 Gemini 支持的 Kilo 引用保留相同的代理 Gemini 思维签名清理路径；`kilocode/kilo/auto` 和其他不支持代理推理的提示将跳过代理推理注入
...
- MiniMax 新手引导/API 密钥设置会通过 `input: ["text", "image"]` 写入明确的 M2.7 模型定义；捆绑提供商目录在具体提供商配置实例化之前，会保持聊天引用为纯文本
...
## 通过 `models.providers` (自定义/基础 URL) 的提供商

使用 `models.providers` (或 `models.json`) 添加 **自定义** 提供商或
OpenAI/Anthropic 兼容的代理。

下方的许多捆绑提供商插件已发布默认目录。仅当您需要覆盖
默认基础 URL、请求头或模型列表时，才使用显式的 `models.providers.<id>` 条目。

### Moonshot AI (Kimi)

Moonshot 作为捆绑提供商插件提供。默认使用内置提供商，
仅当您需要覆盖基础 URL 或模型元数据时，才添加显式的 `models.providers.moonshot` 条目：
...
在新手引导/配置模型选择器中，Volcengine 认证选项优先显示 `volcengine/*` 和 `volcengine-plan/*` 行。如果这些模型尚未加载，OpenClaw 将回退到未过滤的目录，而不是显示空的提供商范围选择器。
...
新手引导默认使用编程界面，但通用的 `byteplus/*` 目录也会同时注册。
...
在 MiniMax 的 Anthropic 兼容流式路径上，OpenClaw 默认情况下禁用思考，除非您明确设置它，并且 `/fast on` 会将
...
- 图像理解在两个 MiniMax 认证路径上均为插件拥有的 `MiniMax-VL-01`
- 网络搜索保留在提供商 id `minimax` 上

### LM Studio

LM Studio 作为捆绑的提供商插件提供，该插件使用原生 API：
...
OpenClaw 使用 LM Studio 原生的 `/api/v1/models` 和 `/api/v1/models/load` 进行发现和自动加载，默认使用 `/v1/chat/completions` 进行推理。有关设置和故障排除，请参阅 [/providers/lmstudio](/en/providers/lmstudio)。

### Ollama

Ollama 作为捆绑提供商插件提供，并使用 Ollama 的原生 Ollama：
...
当您选择使用 `OLLAMA_API_KEY` 时，Ollama 会在本地 `http://127.0.0.1:11434` 被检测到，并且捆绑的提供商插件会将 Ollama 直接添加到 `openclaw onboard` 和模型选择器中。有关新手引导、云端/本地模式和自定义配置，请参阅 [/providers/ollama](/en/providers/ollama)。

### vLLM

vLLM 作为捆绑提供商插件提供，用于本地/自托管的 OpenAI 兼容服务器：
...
SGLang 作为捆绑提供商插件提供，用于快速自托管的 OpenAI 兼容服务器：
...
- 对于非原生端点上的 `api: "openai-completions"`（任何主机不是 `api.openai.com` 的非空 `baseUrl`），OpenClaw 强制执行 `compat.supportsDeveloperRole: false` 以避免因不支持的 `developer` 角色而导致的提供商 400 错误。
- 代理风格的 OpenAI 兼容路由也会跳过原生 OpenAI 专用的请求整形：无 `service_tier`，无响应 `store`，无提示缓存提示，无 OpenAI 推理兼容负载整形，且无隐藏的 OpenClaw 归属标头。
- 如果 `baseUrl` 为空/被省略，OpenClaw 将保留默认的 OpenAI 行为（该行为解析为 `api.openai.com`）。
- 出于安全考虑，显式的 `compat.supportsDeveloperRole: true` 在非原生 `openai-completions` 端点上仍会被覆盖。
...
另请参阅：[/gateway/configuration](/en/gateway/configuration) 以获取完整配置示例。
