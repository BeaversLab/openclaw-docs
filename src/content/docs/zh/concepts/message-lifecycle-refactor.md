...
本页面旨在设计一个统一且持久的消息生命周期，以替代分散的渠道入站、回复分发、预览流式传输和出站交付辅助工具。
...
`thread.post()`OpenClaw 风格的 Promise 对于 OpenClaw 来说是不够的。它们隐藏了决定发送是否可恢复的事务边界。
...
`state` 拥有持久意图存储、回执、幂等性、恢复、锁定和去重功能。
...
第一个必需的用途是网关故障输出。人类仍应看到诸如“Agent failed before reply”或“Missing API key”之类的消息，但在启用 APIOpenClaw`allowBots` 时，标记为 OpenClaw 运营输出的消息绝不能在共享房间中被接受为机器人编写的输入。
...
`ReceiveAckPolicy` 仅控制传输或轮询确认。不得将其重新用于已读回执或状态反应。
...
此丢弃操作是基于标签的，而非基于文本的。一条具有相同可见网关故障文本但没有 OpenClaw 来源元数据的机器人编写的房间消息，仍需经过正常的 OpenClaw`allowBots` 授权。
...
Telegram 轮询现在使用接收上下文确认策略作为其持久化的重启水印。追踪器仍然在更新进入中间件链时观察 grammY 更新，但 OpenClaw 仅在成功分发后持久化安全的已完成的更新 ID，使得失败或较低的待处理更新在重启后可重放。Telegram 的上游 TelegramgrammYOpenClawTelegram`getUpdates`OpenClaw 获取偏移量仍由轮询库控制，因此如果我们需要超越 OpenClaw 重启水印的平台级重新投递，剩下的更深层次的改进是完全持久的轮询源。Webhook 平台可能需要立即的 HTTP 确认，但它们仍然需要入站去重和持久的出站发送意图，因为 webhook 可以重新投递。
...
危险的边界是在平台成功之后和回执提交之前。如果进程在那里死亡，OpenClaw 无法知道平台消息是否存在，除非适配器提供原生幂等性或回执对账路径。这些尝试必须在 OpenClaw`unknown_after_send` 中恢复，而不是盲目重放。没有对账功能的渠道可以选择至少一次重放，前提是重复的可见消息对于该渠道和关系来说是一个可接受的、有记录的权衡。当前的 SDK 对账桥接器要求适配器声明 `reconcileUnknownSend`，然后要求 `durableFinal.reconcileUnknownSend` 将未知条目分类为 `sent`、`not_sent` 或 `unresolved`；只有 `not_sent` 允许重放，未解决的条目保持终端状态或仅重试对账检查。
...
`required` 意味着当核心无法写入持久化意图时必须失败关闭。`best_effort` 可以在持久化不可用时通过。`disabled` 保持旧的直接发送行为。在迁移期间，遗留包装器和公共兼容性助手默认为 `disabled`；它们绝不能仅因为渠道具有通用出站适配器这一事实就推断 `required`。
...
发送辅助函数必须将回执一直返回给调用者。持久化包装器不能吞掉消息 ID 或用 `undefined` 替换渠道传递结果；缓冲调度器使用这些 ID 作为线程锚点、后续编辑、预览完成以及去重处理。
...
当此类回退是持久化的时，整个投影批次必须由一个持久化发送意图或另一个原子批次计划来表示。逐一记录每个有效负载是不够的：有效负载之间的崩溃可能会留下部分可见的回退，而没有剩余有效负载的持久化记录。恢复过程必须知道哪些单元已有回执，并仅重放缺失的单元或将批次标记为 `unknown_after_send`，直到适配器协调它。
...
在飞行前授权之前，核心必须在 `origin.decode` 返回 OpenClaw 源元数据时运行共享的 OpenClaw 回显谓词。接收适配器提供平台事实，例如机器作者和房间形状；核心拥有丢弃决策和排序，以便渠道不重新实现文本过滤器。
...
核心设置 `MessageOrigin`。渠道仅将其转换为本机传输元数据。Slack 将其映射到 `chat.postMessage({ metadata })` 和入站 `message.metadata`；Matrix 可以将其映射到额外的事件内容；没有本机元数据的渠道可以在这是最佳可用近似值时使用回执/出站注册表。
...
捆绑的插件在迁移期间可以通过保留的运行时子路径保留内部辅助导入。公共文档应引导插件作者在 `plugin-sdk/channel-outbound` 存在时使用它。

## 与渠道入站的关系

`runtime.channel.inbound.*` 是迁移期间的运行时桥梁。
...
`channel.inbound.runPreparedReply` 最初也应保留：
...
旧的 `channel.turn` 运行时接口已被移除。运行时调用者使用 `channel.inbound.*`；渠道文档和 SDK 子路径使用入站/消息名词。
...
- `channel.inbound.run` 和 `dispatchChannelInboundReply` 使用渠道的交付回调，除非该渠道明确提供了经过审计的持久化策略/选项对象。
- `channel.inbound.runPreparedReply` 在准备好的分发程序显式调用发送上下文之前，一直由渠道拥有。
- 公共兼容性助手（如 `recordInboundSessionAndDispatchReply`、`dispatchInboundReplyWithBase`）和直接私信助手绝不在调用者提供的 `deliver` 或 `reply` 回调之前注入通用的持久化交付。

对于迁移桥接类型，`durable: undefined` 意味着“非持久化”。持久化路径仅由显式的策略/选项值启用。`durable: false` 可以保留为一种兼容性拼写，但实现不应要求每个未迁移的渠道都添加它。

当前的桥接代码必须保持持久化决策的明确性：

- 持久化最终交付返回一个可区分的状态。`handled_visible` 和 `handled_no_send` 是终态的；`unsupported` 和 `not_applicable` 可能回退到渠道拥有的交付；`failed` 传播发送失败。
- 通用持久化最终投递受适配器能力（如静默投递、回复目标保留、原生引用保留
  和消息发送钩子）的限制。如果缺少功能对等性，应选择渠道拥有的投递，
  而不是会改变用户可见行为的通用发送。
- 由队列支持的持久化发送公开一个交付意图引用。现有的 `pendingFinalDelivery*` 会话字段可以在过渡期间携带意图 ID；最终状态是一个 `MessageSendIntent` 存储，而不是冻结的回复文本加上临时上下文字段。
...
- 直接私信助手可以具有一个回复回调，该回调是唯一正确的传输目标。通用出站绝不能根据 `OriginatingTo` 或 `To` 进行猜测并跳过该回调。
- OpenClaw 网关失败输出必须对人类保持可见，但带有标记的机器人编写的房间回声必须在 OpenClaw`allowBots` 授权之前被丢弃。渠道不得使用可见文本前缀过滤器来实现这一点，除非作为短期的应急权宜之计；持久化契约是结构化的来源元数据。
...
- 除非存在渲染回退 (render fallback)，否则不要重试 `invalid_payload`。
- 在配置更改之前，不要重试 `auth` 或 `permission`。
- 对于 `not_found`，当渠道声明其为安全时，允许实时完成 (live finalization) 从编辑回退到全新发送。
- 对于 `conflict`，使用回执/幂等 (receipt/idempotency) 规则来确定消息是否已存在。
- 适配器可能已完成平台 I/O 但在提交回执之前的任何错误都将变成 `unknown_after_send`，除非适配器能证明平台操作未发生。
...
| Slack           | 发送适配器处理正常的聊天发布。当线程形状支持时，实时适配器选择原生流，否则选择草稿预览。回执保留线程时间戳。源适配器将 OpenClaw 网关故障映射到 Slack `chat.postMessage.metadata`，并在 `allowBots` 授权之前丢弃标记的机器人房间回显。          |
| WhatsApp        | 发送适配器负责具有持久化最终意图的文本/媒体发送。接收适配器处理群组提及和发送者身份。在 WhatsApp 拥有可编辑的传输之前，实时适配器可以保持缺席。                                                                                                |
| Matrix          | 实时适配器负责草稿事件编辑、完成、删除、加密媒体约束以及回复目标不匹配回退。接收适配器负责加密事件补充 (hydration) 和去重。源适配器应将 OpenClaw 网关失败的来源编码到 Matrix 事件内容中，并在 `allowBots` 处理之前丢弃已配置机器人房间的回显。 |
...
| Google Chat     | 简单的接收加发送适配器，将线程关系映射到空间和线程 ID。审查 `allowBots=true` 房间对标记的 OpenClaw 网关失败回显的行为。                                                                                                                        |
...
- 为消息、目标、关系、来源、回执、能力、持久意图、接收上下文、发送上下文、实时上下文和故障类别添加 `src/channels/message/*` 类型。
- 将 `origin?: MessageOrigin` 添加到当前回复传递所使用的迁移桥接负载类型中，然后在重构替换回复负载时，将该字段移至 `ChannelMessage` 和已渲染消息类型。
...
- 在适配器声明重放安全后，在新的消息生命周期中，将最终发送的持久性设为默认值，并在无法写入持久性意图时以失败关闭（fail closed）处理。在此阶段，现有的入站运行器和 SDK 兼容性路径默认保持为直接发送。
- 一致地记录回执。
- 将回执和投递结果返回给原始调度器调用者，而不是将持久化发送视为终端副作用。
- 通过持久化发送意图保留消息来源，以便恢复、重放和分块发送保留 OpenClaw 的操作来源。

### 阶段 3：渠道入站桥接（Channel Inbound Bridge）
...
- 将 `durable: false` 作为兼容性逃生通道，用于那些完成原生编辑且尚无法安全重放的路径，但不要依赖 `false` 标记来保护未迁移的渠道。
- 仅在新消息生命周期中默认组装轮次的持久性，且需在渠道映射证明通用发送路径保留了旧的渠道投递语义之后。

### 阶段 4：准备好的调度器桥接（Prepared Dispatcher Bridge）

- 将 `deliverDurableInboundReplyPayload` 替换为发送上下文桥接（send-context bridge）。
- 保留旧的辅助程序作为包装器。
...
- 保持 `recordInboundSessionAndDispatchReply`、直接私信（direct-私信）辅助函数和类似的公共兼容性辅助函数的行为不变。它们稍后可能会提供显式的发送上下文（send-context）选择加入（opt-in），但在调用者拥有的交付回调之前，不得自动尝试通用的持久化交付。

### 阶段 5：统一的实时生命周期

- 使用两个验证适配器（proof adapters）构建 `messages.live`：
...
- 在渠道出站（渠道-outbound）SDK 表面中包含 `MessageOrigin`、来源编码/解码钩子（origin encode/decode hooks）以及共享的 `shouldDropOpenClawEcho` 谓词。
- 为旧的子路径保留兼容性包装器。
- 在捆绑插件迁移后，在文档中将命名的 reply SDK 辅助函数标记为已弃用。

### 阶段 7：所有发送方

将所有非回复出站生产者迁移到 `messages.send` 上：
...
### 阶段 8：移除 Turn-Named 兼容性

- 将入站/消息命名（inbound/message-named）包装器保留为兼容性窗口。
- 发布迁移说明。
- 针对旧导入运行插件 SDK 兼容性测试。
...
- 幂等性密钥重用和重复抑制。
- 回执提交和重放跳过。
- 当适配器支持协调（reconciliation）时，`unknown_after_send` 恢复会在重放之前进行协调。
- 故障分类策略。
- 接收确认策略排序。
- 针对回复、跟进、系统和广播发送的关系映射。
- 网关失败的来源工厂（origin factory）和 Gateway(网关)`shouldDropOpenClawEcho` 谓词。
- 通过负载规范化、分块、持久化队列序列化和恢复来保留来源。

集成测试：

- `channel.inbound.run` 简单适配器仍然会记录并发送。
- 除非渠道明确选择加入，否则传统的组装事件传递不会变得持久。
- `channel.inbound.runPreparedReply` 桥接仍然会记录并完成。
...
- 带有已标记 OpenClaw 网关失败消息的 Slack 保持出站可见，已标记的机器人房间回声（bot-room echoes）在 SlackOpenClaw`allowBots` 之前丢弃，且具有相同可见文本的未标记机器人消息仍然遵循正常的机器人授权。
- Slack 原生流在顶级私信中回退到草稿预览。
- Matrix 预览定稿和撤回回退。
- Matrix 标记的来自配置机器人账户的 OpenClaw 网关故障房间回显会在 MatrixOpenClaw`allowBots` 处理之前丢弃。
- Discord 和 Google Chat 共享房间网关故障级联审计在声明通用保护之前涵盖 DiscordGoogle Chat`allowBots` 模式。
...
- 在 Testbox 中使用 `pnpm check:changed` 测试完整的变更范围。
- 在落地完整的重构或公共 SDK/导出更改之前，在 Testbox 中进行更广泛的 `pnpm check`。
...
- 在 `plugin-sdk/channel-outbound` 发布后，兼容性包装器在文档中保留的时间长度。
- 第三方插件是应该直接实现接收适配器，还是仅通过 `defineChannelMessageAdapter` 提供规范化/发送/实时钩子。
...
- 每个捆绑的消息渠道通过 `messages.send` 发送最终可见的输出。
- 每个入站消息渠道通过 `messages.receive` 或已记录的兼容性包装器进入。
- 每个预览/编辑/流渠道使用 `messages.live` 进行草稿状态和最终确定。
...
- 旧版 SDK 兼容性助手默认为直接渠道拥有的交付；通用持久发送仅显式选择加入。
...
- [渠道入站 API](API/en/plugins/sdk-channel-inbound)
