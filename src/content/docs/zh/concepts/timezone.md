...
有关完整的行为和示例，请参阅[日期与时间](/zh/date-time)。

## 相关

- [Heartbeat](/zh/gateway/heartbeat) — 活跃时段使用时区进行调度
- [Cron Jobs](/zh/automation/cron-jobs) — cron 表达式使用时区进行调度
- [Date & Time](/zh/date-time) — 完整的日期/时间行为和示例
