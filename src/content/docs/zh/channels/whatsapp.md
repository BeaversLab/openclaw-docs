...
- 在接受入站消息后立即发送（回复前）
- 如果存在 `ackReaction` 但没有 `emoji`WhatsApp，WhatsApp 将使用路由代理的身份表情符号，回退到“👀”；省略 `ackReaction` 或设置 `emoji: ""` 以不发送确认反应
- 失败会被记录，但不会阻止正常回复的发送
- 群组模式 `mentions` 对提及触发的轮次做出反应；群组激活 `always` 充当此检查的旁路
- WhatsApp 使用 WhatsApp`channels.whatsapp.ackReaction`（此处不使用旧版 `messages.ackReaction`）

## 生命周期状态反应

设置 `messages.statusReactions.enabled: true`WhatsAppOpenClaw 以便 WhatsApp 在一轮中替换确认反应，而不是留下静态的接收表情符号。启用后，OpenClaw 对排队、思考、工具活动、压缩、完成和错误等生命周期状态使用相同的入站消息反应槽位。
...
- 排队状态反应使用与普通确认反应相同的有效确认表情符号。
- WhatsApp 每条消息只有一个机器人反应槽，因此生命周期更新会就地替换当前反应。
...
<AccordionGroup>
  ...
  <Accordion title="凭证路径和旧版兼容性">
    ...
    - 备份文件：`creds.json.bak`
    - `~/.openclaw/credentials/` 中的旧版默认认证对于默认账户流程仍会被识别/迁移
  ...
  <Accordion title="登出行为">
    ...
    当 Gateway(网关) 可达时，登出首先停止所选账户的实时 WhatsApp 监听器，以便关联会话不会在下次重启前继续接收消息。`openclaw channels remove --channel whatsapp` 也会在禁用或删除账户配置之前停止实时监听器。
...
<AccordionGroup>
  ...
  <Accordion title="已链接但断开连接 / 重连循环">
    症状：已链接的帐户出现重复断开连接或重连尝试。

    安静的帐户可以保持连接超过正常的消息超时时间；当 WhatsApp Web 传输活动停止、套接字关闭或应用程序级别的活动在更长的安全窗口内保持静默时，监视程序会重新启动。

    如果日志显示重复的 `status=408 Request Time-out Connection was lost`，请在 `web.whatsapp` 下调整 Baileys 套接字计时。首先缩短 `keepAliveIntervalMs` 使其低于网络的空闲超时时间，并在缓慢或易丢包的链路上增加 `connectTimeoutMs`：
    ...
    如果在修复主机连接和计时问题后循环仍然存在，请备份帐户认证目录并重新链接该帐户：
    ...
    如果 `~/.openclaw/logs/whatsapp-health.log` 显示 `Gateway inactive` 但 `openclaw gateway status` 和 `openclaw channels status --probe` 显示网关和 WhatsApp 状态正常，请运行 `openclaw doctor`。在 Linux 上，doctor 会警告遗留的 crontab 条目仍然调用 `~/.openclaw/bin/ensure-whatsapp.sh`；请使用 `crontab -e` 删除那些过时的条目，因为 cron 可能缺少 systemd user-bus 环境并导致该旧脚本错误报告网关健康状况。
  ...
  <Accordion title="代理后二维码登录超时">
    症状：`openclaw channels login --channel whatsapp` 在显示可用的二维码之前失败，并提示 `status=408 Request Time-out` 或 TLS 套接字断开连接。

    WhatsApp Web 登录使用网关主机的标准代理环境（`HTTPS_PROXY`、`HTTP_PROXY`、小写变体和 `NO_PROXY`）。请验证网关进程是否继承了代理环境，并且 `NO_PROXY` 不匹配 `mmg.whatsapp.net`。
  ...
  <Accordion title="回复显示在记录中但未显示在 WhatsApp 中">
    记录行记录了代理生成的内容。WhatsApp 投递是单独检查的：OpenClaw 仅在 Baileys 针对至少一次可见文本或媒体发送返回出站消息 ID 后，才将自动回复视为已发送。

    Ack 反应是独立的预回复回执。成功的反应并不证明后续的文本或媒体回复已被 WhatsApp 接受。
  ...
  <Accordion title="群组消息意外被忽略">
    ...
    - `openclaw.json` (JSON5) 中的重复键：后面的条目会覆盖前面的条目，因此每个作用域请保持单个 `groupPolicy`

    如果存在 `channels.whatsapp.groups`，WhatsApp 仍可观察来自其他群组的消息，但 OpenClaw 会在会话路由前将其丢弃。将群组 JID 添加到 `channels.whatsapp.groups` 或添加 `groups["*"]` 以允许所有群组，同时将发送方授权控制在 `groupPolicy` 和 `groupAllowFrom` 之下。
...
WhatsApp 支持通过 `groups` 和 `direct` 映射，为群组和私信聊天提供 Telegram 风格的系统提示词。

群组消息的解析层级：

首先确定有效的 `groups` 映射：如果账号定义了自己的 `groups`，它将完全替换根 `groups` 映射（不进行深度合并）。然后对生成的单个映射运行提示词查找：

1. **群组特定系统提示词** (`groups["<groupId>"].systemPrompt`)：当映射中存在特定群组条目**并且**定义了其 `systemPrompt` 键时使用。如果 `systemPrompt` 是空字符串 (`""`)，则通配符被抑制且不应用系统提示词。
2. **群组通配符系统提示词** (`groups["*"].systemPrompt`)：当映射中完全不存在特定群组条目，或者虽然存在但未定义 `systemPrompt` 键时使用。

私信的解析层级：

首先确定有效的 `direct` 映射：如果账号定义了自己的 `direct`，它将完全替换根 `direct` 映射（不进行深度合并）。然后对生成的单个映射运行提示词查找：

1. **特定对象系统提示词** (`direct["<peerId>"].systemPrompt`)：当映射中存在特定对象条目**并且**定义了其 `systemPrompt` 键时使用。如果 `systemPrompt` 是空字符串 (`""`)，则通配符被抑制且不应用系统提示词。
2. **对象通配符系统提示词** (`direct["*"].systemPrompt`)：当映射中完全不存在特定对象条目，或者虽然存在但未定义 `systemPrompt` 键时使用。

<Note>
`dms` 仍然是轻量级的每条私信历史记录覆盖存储桶 (`dms.<id>.historyLimit`)。提示词覆盖位于 `direct` 下。
</Note>

**与 Telegram 多账号行为的区别：** 在 Telegram 中，在多账号设置中，会故意为所有账号抑制根 TelegramTelegram`groups` — 即使是那些没有定义自己的 `groups`WhatsApp 的账号 — 以防止机器人接收到其不属于的群组的群组消息。WhatsApp 不应用此防护措施：对于未定义账号级别覆盖的账号，总是会继承根 `groups` 和根 `direct`WhatsApp，无论配置了多少个账号。在多账号 WhatsApp 设置中，如果您希望每个账号都有单独的群组或直接提示，请在每个账号下明确定义完整的映射，而不是依赖根级别的默认值。

重要行为：

- `channels.whatsapp.groups` 既是一个每个群组的配置映射，也是聊天级别的群组允许列表。无论是在根作用域还是账号作用域，`groups["*"]` 都意味着“允许所有群组”进入该作用域。
- 仅当您已经希望该作用域允许所有群组时，才添加通配符群组 `systemPrompt`。如果您仍然希望只有一组固定的群组 ID 符合条件，请不要对提示默认值使用 `groups["*"]`。相反，应在每个明确列入允许列表的群组条目上重复该提示。
- 群组准入和发送者授权是分开的检查。`groups["*"]` 扩展了可以到达群组处理的群组集合，但它本身并不授权这些群组中的每个发送者。发送者访问仍然由 `channels.whatsapp.groupPolicy` 和 `channels.whatsapp.groupAllowFrom` 单独控制。
- `channels.whatsapp.direct` 对私信没有相同的副作用。`direct["*"]` 仅在私信已被 `dmPolicy` 加上 `allowFrom` 或配对存储规则准许后，提供默认的直接聊天配置。
