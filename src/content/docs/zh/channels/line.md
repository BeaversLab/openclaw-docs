...
状态：内置插件。支持私信、群聊、媒体、位置、Flex 消息、模板消息和快速回复。不支持回应和话题串。

## 内置插件

LINE 在当前的 OpenClaw 版本中作为内置插件提供，因此普通的打包版本无需单独安装。
...
- LINE 签名验证依赖于正文（对原始正文进行 HMAC），因此 OpenClaw 在验证之前会应用严格的前置身份验证正文限制和超时。
- OpenClaw 处理来自验证过的原始请求字节的 webhook 事件。出于签名完整性的安全考虑，上游中间件转换后的 `req.body` 值将被忽略。
...
- `channels.line.groupAllowFrom`：群组的 LINE 用户 ID 白名单
...
- 文本在 5000 个字符处进行分块。
- Markdown 格式会被去除；代码块和表格在可能的情况下会转换为 Flex 卡片。
- 流式响应会被缓冲；当代理工作时，LINE 会接收带有加载动画的完整分块。
- 媒体下载受到 `channels.line.mediaMaxMb` 限制（默认为 10）。
...
- 已配置的 ACP 绑定和活动的对话绑定 ACP 会话在 LINE 上的工作方式与其他对话频道类似。

有关详细信息，请参阅 [ACP 代理](/zh/tools/acp-agents)。

## 出站媒体

LINE 插件支持通过代理消息工具发送图片、视频和音频文件。媒体通过 LINE 特定的传递路径发送，并具有适当的预览和跟踪处理：

- **图片**：作为 LINE 图片消息发送，并自动生成预览。
- **视频**：发送时带有明确的预览和内容类型处理。
- **音频**：作为 LINE 音频消息发送。

出站媒体 URL 必须是公共 HTTPS URL。OpenClaw 在将 URL 交给 LINE 之前会验证目标主机名，并拒绝回环、链路本地和专用网络目标。
