...
有关完整的升级流程、限制、恢复命令和常见迁移信息，请参阅 [Matrix migration](/zh/install/migrating-matrix)。
...
有关提及限制和允许列表行为，请参阅 [Groups](/zh/channels/groups)。
...
相关文档：[Exec approvals](/zh/tools/exec-approvals)

## 斜杠命令

Matrix 斜杠命令（例如 `/new`、`/reset`、`/model`）直接在私信中工作。在房间中，OpenClaw 还能识别以机器人自己的 Matrix 提及为前缀的斜杠命令，因此 `@bot:server /new` 会触发命令路径而无需自定义提及正则表达式。这使机器人能够响应用户在键入命令前通过 Tab 补全提及机器人时，Element 及类似客户端发出的房间风格 `@mention /command` 帖子。
...
顶层的 `channels.matrix` 值充当命名帐户的默认值，除非某个帐户覆盖了它们。
您可以使用 `groups.<room>.account` 将继承的房间条目限定为一个 Matrix 帐户。
没有 `account` 的条目在所有 Matrix 帐户之间保持共享，而有 `account: "default"` 的条目在直接于顶层 `channels.matrix.*` 上配置默认帐户时仍然有效。
部分共享的身份验证默认值本身不会创建单独的隐式默认帐户。仅当该默认值具有新的身份验证（`homeserver` 加上 `accessToken`，或 `homeserver` 加上 `userId` 和 `password`）时，OpenClaw 才会合成顶层 `default` 帐户；当缓存的凭据稍后满足身份验证时，命名帐户仍然可以通过 `homeserver` 加上 `userId` 被发现。
如果 Matrix 已经只有一个命名帐户，或者 `defaultAccount` 指向现有的命名帐户密钥，则单帐户到多帐户的修复/设置升级会保留该帐户，而不是创建新的 `accounts.default` 条目。只有 Matrix 身份验证/引导密钥会移动到该升级的帐户中；共享的传递策略密钥保持在顶层。
当您希望 OpenClaw 为隐式路由、探测和 Matrix 操作优先使用一个命名 CLI 帐户时，请设置 `defaultAccount`。
如果配置了多个 Matrix 帐户，并且有一个帐户 ID 为 `default`，则即使未设置 `defaultAccount`，OpenClaw 也会隐式使用该帐户。
...
有关共享的多帐户模式，请参阅[配置参考](/zh/gateway/configuration-reference#multi-account-all-channels)。

## 私有/LAN 家庭服务器

默认情况下，为了进行 SSRF 防护，OpenClaw 会阻止私有/内部 Matrix 主服务器，除非您针对每个账户明确选择加入。
...
命名账户可以使用 `channels.matrix.accounts.<id>.proxy` 覆盖顶级默认值。OpenClaw 对运行时 Matrix 流量和账户状态探测使用相同的代理设置。

## 目标解析

只要 OpenClaw 要求您提供房间或用户目标，Matrix 都可以在任何地方接受以下目标形式：
...
- 房间查找直接接受明确的房间 ID 和别名，然后回退到为该账户搜索已加入的房间名称。
- 已加入房间名称查找是尽力而为的。如果房间名称无法解析为 ID 或别名，它将在运行时允许列表解析中被忽略。
...
- `network.dangerouslyAllowPrivateNetwork`：允许此 Matrix 账户连接到私有/内部主服务器。当主服务器解析为 `localhost`、局域网/Tailscale IP 或内部主机（例如 `matrix-synapse`）时，请启用此选项。
- `proxy`：用于 Matrix 流量的可选 HTTP(S) 代理 URL。命名账户可以使用其自己的 `proxy` 覆盖顶级默认设置。
- `userId`：完整的 Matrix 用户 ID，例如 `@bot:example.org`。
- `accessToken`：用于基于令牌的身份验证的访问令牌。env/file/exec 提供程序支持 `channels.matrix.accessToken` 和 `channels.matrix.accounts.<id>.accessToken` 的纯文本值和 SecretRef 值。请参阅[机密管理](/zh/gateway/secrets)。
- `password`：用于基于密码登录的密码。支持纯文本值和 SecretRef 值。
...
- `encryption`：启用 E2EE。
- `allowlistOnly`：当为 `true` 时，将 `open` 房间策略升级为 `allowlist`，并强制所有活动的私信策略（除 `disabled` 外，包括 `pairing` 和 `open`）变为 `allowlist`。不影响 `disabled` 策略。
...
- `groupAllowFrom`：房间流量的用户 ID 白名单。完整的 Matrix 用户 ID 是最安全的；完全匹配的目录项会在启动时以及监控运行期间白名单更改时进行解析。无法解析的名称将被忽略。
- `historyLimit`：作为群组历史上下文包含的最大房间消息数。回退至 `messages.groupChat.historyLimit`；如果两者均未设置，有效默认值为 `0`。设置 `0` 可禁用。
- `replyToMode`：`off`、`first`、`all` 或 `batched`。
- `markdown`：用于传出 Matrix 文本的可选 Markdown 渲染配置。
- `streaming`：`off`（默认）、`"partial"`、`"quiet"`、`true` 或 `false`。`"partial"` 和 `true` 启用通过正常 Matrix 文本消息进行预览优先的草稿更新。`"quiet"` 对自托管推送规则设置使用非通知预览通知。`false` 等效于 `"off"`。
- `blockStreaming`：`true` 在草稿预览流式传输处于活动状态时，为已完成的助手块启用单独的进度消息。
- `threadReplies`：`off`、`inbound` 或 `always`。
- `threadBindings`：用于绑定线程的会话路由和生命周期的按渠道覆盖设置。
- `startupVerification`：启动时的自动自我验证请求模式（`if-unverified`、`off`）。
- `startupVerificationCooldownHours`：重试自动启动验证请求之前的冷却时间。
- `textChunkLimit`: 以字符为单位的出站消息块大小（当 `chunkMode` 为 `length` 时适用）。
- `chunkMode`: `length` 按字符数分割消息；`newline` 在行边界分割。
- `responsePrefix`: 附加到此渠道所有出站回复的可选字符串。
- `ackReaction`: 此渠道/帐户的可选确认表情符号覆盖。
- `ackReactionScope`: 可选的确认表情符号范围覆盖（`group-mentions`, `group-all`, `direct`, `all`, `none`, `off`）。
- `reactionNotifications`: 入站表情符号通知模式（`own`, `off`）。
- `mediaMaxMb`: 用于出站发送和入站媒体处理的媒体大小上限（MB）。
- `autoJoin`: 邀请自动加入策略（`always`, `allowlist`, `off`）。默认值：`off`。适用于所有 Matrix 邀请，包括私信样式的邀请。
- `autoJoinAllowlist`: 当 `autoJoin` 为 `allowlist` 时允许的房间/别名。别名条目在邀请处理期间解析为房间 ID；OpenClaw 不信任被邀请房间声明的别名状态。
- `dm`: 私信策略块（`enabled`, `policy`, `allowFrom`, `sessionScope`, `threadReplies`）。
- `dm.policy`: 在 OpenClaw 加入房间并将其归类为私信后，控制私信访问。它不会更改是否自动加入邀请。
- `dm.allowFrom`: 私信流量的用户 ID 白名单。完整的 Matrix 用户 ID 最安全；确切的目录匹配项在启动时以及监控程序运行时白名单更改时进行解析。无法解析的名称将被忽略。
- `dm.sessionScope`: `per-user`（默认）或 `per-room`。当您希望每个 Matrix 私信房间即使对方相同也保持单独上下文时，请使用 `per-room`。
- `dm.threadReplies`: 仅限私信的会话策略覆盖 (`off`, `inbound`, `always`)。它覆盖顶层 `threadReplies` 设置，用于私信中的回复放置和会话隔离。
...
- `accounts`: 命名的每个账户覆盖。顶层 `channels.matrix` 值作为这些条目的默认值。
- `groups`: 每个房间的策略映射。优先使用房间 ID 或别名；运行时将忽略未解析的房间名称。会话/群组身份在解析后使用稳定的房间 ID。
- `groups.<room>.account`: 在多账户设置中，将一个继承的房间条目限制为特定的 Matrix 账户。
- `groups.<room>.allowBots`: 配置的机器人发送者的房间级别覆盖 (`true` 或 `"mentions"`)。
- `groups.<room>.users`: 每个房间的发送者允许列表。
- `groups.<room>.tools`: 每个房间的工具允许/拒绝覆盖。
- `groups.<room>.autoReply`: 房间级别的提及门控覆盖。`true` 禁用该房间的提及要求；`false` 强制重新启用。
