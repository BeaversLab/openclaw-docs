...
如果启动发现损坏的引导状态并配置了 `channels.matrix.password`，OpenClaw 可以尝试更严格的修复路径。
...
请参阅 [Groups](/en/channels/groups) 了解提及限制和允许列表行为。
...
请参阅 [Pairing](/en/channels/pairing) 了解共享的私信配对流程和存储布局。
...
顶层 `channels.matrix` 值充当命名帐户的默认值，除非帐户覆盖了它们。
您可以使用 `groups.<room>.account` 将继承的房间条目限定到一个 Matrix 帐户。
没有 `account` 的条目在所有 Matrix 帐户之间保持共享，带有 `account: "default"` 的条目在直接在顶层 `channels.matrix.*` 上配置默认帐户时仍然有效。
部分共享的身份验证默认值本身不会创建单独的隐式默认帐户。OpenClaw 仅在该默认值具有新的身份验证（`homeserver` 加上 `accessToken`，或 `homeserver` 加上 `userId` 和 `password`）时才合成顶层 `default` 帐户；当缓存的凭据稍后满足身份验证时，命名帐户仍可从 `homeserver` 加上 `userId` 中被发现。
如果 Matrix 已经恰好有一个命名帐户，或者 `defaultAccount` 指向现有的命名帐户密钥，则单帐户到多帐户的修复/设置升级将保留该帐户，而不是创建新的 `accounts.default` 条目。只有 Matrix 身份验证/引导密钥会移动到该升级后的帐户中；共享的传递策略密钥保留在顶层。
当您希望 OpenClaw 为隐式路由、探测和 CLI 操作优先使用一个命名的 Matrix 帐户时，请设置 `defaultAccount`。
如果您配置了多个命名帐户，请设置 `defaultAccount` 或为依赖隐式帐户选择的 CLI 命令传递 `--account <id>`。
当您想要针对一个命令覆盖该隐式选择时，请将 `--account <id>` 传递给 `openclaw matrix verify ...` 和 `openclaw matrix devices ...`。

有关共享的多帐户模式，请参阅 [配置参考](/en/gateway/configuration-reference#multi-account-all-channels)。
...
此选项仅允许受信任的私有/内部目标。公共明文主服务器（例如 `http://matrix.example.org:8008`）仍然被阻止。尽可能首选 `https://`。
...
- `network.dangerouslyAllowPrivateNetwork`：允许此 Matrix 账户连接到私有/内部主服务器。当主服务器解析为 `localhost`、LAN/Tailscale IP 或内部主机（例如 `matrix-synapse`）时，请启用此项。
- `proxy`：Matrix 流量的可选 HTTP(S) 代理 URL。命名账户可以使用自己的 `proxy` 覆盖顶级默认值。
- `userId`：完整的 Matrix 用户 ID，例如 `@bot:example.org`。
- `accessToken`：基于令牌的身份验证的访问令牌。env/file/exec 提供程序支持 `channels.matrix.accessToken` 和 `channels.matrix.accounts.<id>.accessToken` 的明文值和 SecretRef 值。请参阅 [Secrets Management](/en/gateway/secrets)。
- `password`：基于密码的登录密码。支持明文值和 SecretRef 值。
- `deviceId`：显式的 Matrix 设备 ID。
- `deviceName`：密码登录的设备显示名称。
- `avatarUrl`：存储的用于个人资料同步和 `profile set` 更新的自我头像 URL。
- `initialSyncLimit`：启动同步期间获取的最大事件数。
- `encryption`：启用 E2EE。
- `allowlistOnly`：当为 `true` 时，将 `open` 房间策略升级为 `allowlist`，并强制除 `disabled`（包括 `pairing` 和 `open`）之外的所有活动私信策略变为 `allowlist`。不影响 `disabled` 策略。
...
- `groupAllowFrom`：房间流量的用户 ID 允许列表。条目应为完整的 Matrix 用户 ID；未解析的名称在运行时将被忽略。
- `historyLimit`：包含在组历史上下文中的最大房间消息数。回退到 `messages.groupChat.historyLimit`；如果两者均未设置，则有效默认值为 `0`。设置 `0` 以禁用。
- `replyToMode`：`off`、`first`、`all` 或 `batched`。
- `markdown`：用于发出的 Matrix 文本的可选 Markdown 渲染配置。
- `streaming`：`off`（默认）、`"partial"`、`"quiet"`、`true` 或 `false`。`"partial"` 和 `true` 使用普通的 Matrix 文本消息启用“先预览后草稿”更新。`"quiet"` 为自托管推送规则设置使用非通知预览通知。`false` 等同于 `"off"`。
- `blockStreaming`：当草稿预览流式传输处于活动状态时，`true` 为已完成的助手模块启用单独的进度消息。
- `threadReplies`：`off`、`inbound` 或 `always`。
- `threadBindings`：用于线程绑定会话路由和生命周期的逐渠道覆盖。
- `startupVerification`：启动时的自动自我验证请求模式（`if-unverified`、`off`）。
- `startupVerificationCooldownHours`：重试自动启动验证请求之前的冷却时间。
- `textChunkLimit`：出站消息块大小（字符）（当 `chunkMode` 为 `length` 时适用）。
- `chunkMode`：`length` 按字符数拆分消息；`newline` 在行边界处拆分。
- `responsePrefix`：附加到此渠道的所有出站回复之前的可选字符串。
- `ackReaction`：此渠道/帐户的可选确认回应覆盖。
- `ackReactionScope`：可选的确认回应范围覆盖 (`group-mentions`、`group-all`、`direct`、`all`、`none`、`off`)。
- `reactionNotifications`：入站回应通知模式 (`own`、`off`)。
- `mediaMaxMb`：出站发送和入站媒体处理的媒体大小上限（MB）。
- `autoJoin`：邀请自动加入策略 (`always`、`allowlist`、`off`)。默认值：`off`。适用于所有 Matrix 邀请，包括私信风格的邀请。
...
- `dm.policy`：控制 OpenClaw 加入房间并将其分类为私信后的私信访问权限。它不改变是否自动加入邀请。
- `dm.allowFrom`：条目应为完整的 Matrix 用户 ID，除非您已通过实时目录查找解析了它们。
- `dm.sessionScope`：`per-user`（默认）或 `per-room`。当您希望每个 Matrix 私信房间即使对方相同也保持独立上下文时，请使用 `per-room`。
- `dm.threadReplies`：仅限私信的线程策略覆盖 (`off`、`inbound`、`always`)。它会覆盖顶层的 `threadReplies` 设置，以控制私信中的回复放置和会话隔离。
- `execApprovals`：Matrix 原生执行审批传递方式（`enabled`、`approvers`、`target`、`agentFilter`、`sessionFilter`）。
- `execApprovals.approvers`：被允许批准执行请求的 Matrix 用户 ID。如果 `dm.allowFrom` 已经标识了审批者，则为可选项。
- `execApprovals.target`：`dm | channel | both`（默认值：`dm`）。
- `accounts`：命名的按账户覆盖项。顶层 `channels.matrix` 值作为这些条目的默认值。
- `groups`：按房间策略映射。优先使用房间 ID 或别名；运行时会忽略未解析的房间名称。会话/群组标识在解析后使用稳定的房间 ID。
- `groups.<room>.account`：在多账户设置中，将一个继承的房间条目限制为特定的 Matrix 账户。
- `groups.<room>.allowBots`：针对已配置机器人发送者（`true` 或 `"mentions"`）的房间级覆盖设置。
- `groups.<room>.users`：按房间发送者允许列表。
- `groups.<room>.tools`：按房间工具允许/拒绝覆盖设置。
- `groups.<room>.autoReply`：房间级提及门槛覆盖设置。`true` 禁用该房间的提及要求；`false` 强制重新启用。
- `groups.<room>.skills`：可选的房间级技能过滤器。
- `groups.<room>.systemPrompt`：可选的房间级系统提示片段。
- `rooms`：`groups` 的旧别名。
- `actions`：按动作工具门控（`messages`、`reactions`、`pins`、`profile`、`memberInfo`、`channelInfo`、`verification`）。

## 相关内容

- [通道概述](/en/channels) —— 所有支持的通道
- [配对](/en/channels/pairing) —— 私信身份验证和配对流程
- [组](/en/channels/groups) — 群聊行为和提及控制
