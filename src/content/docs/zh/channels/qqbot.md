...
当 QQ Bot 执行审批使用默认的同聊回退机制时，原生审批按钮点击遵循相同的显式非通配符命令白名单。要在没有更广泛的命令访问权限的情况下授予仅审批访问权限，请配置 `channels.qqbot.execApprovals.approvers`。

## Engine architecture

QQ Bot 作为插件内的独立引擎提供：

- 每个账户拥有一个独立的资源栈（WebSocket 连接、API 客户端、令牌缓存、媒体存储根目录），以 `appId` 为键。账户从不共享入站/出站状态。
- 多账户记录器使用所属账户标记日志行，以便您在一个网关下运行多个机器人时，诊断信息保持分离。
- 入站、出站和网关桥接路径在 `~/.openclaw/media` 下共享单个媒体有效载荷根目录，因此上传、下载和转码缓存落在一个受保护的目录下，而不是每个子系统的树状结构中。
- 富媒体传递通过用于 C2C 和群组目标的 `sendMedia` 路径进行。超过大文件阈值的本地文件和缓冲区使用 QQ 的分块上传端点，而较小的有效载荷使用一次性媒体 API。
- 凭据可以作为标准 OpenClaw 凭据快照的一部分进行备份和恢复；引擎在恢复时会重新附加每个账户的资源栈，而无需重新进行二维码配对。

## QR-code 新手引导

除了手动粘贴 `AppID:AppSecret` 之外，引擎还支持二维码引导流程，用于将 QQ Bot 连接到 OpenClaw：

1. 运行 QQ Bot 设置路径（例如 `openclaw channels add --channel qqbot`）并在提示时选择二维码流程。
2. 使用与目标 QQ Bot 绑定的手机应用扫描生成的二维码。
3. 在手机上批准配对。OpenClaw 将返回的凭据持久化到正确账户作用域下的 `credentials/` 中。

由机器人本身生成的批准提示（例如，QQ Bot API 公开的“允许此操作？”流程）会显示为原生 OpenClaw 提示，您可以使用 `/bot-approve` 接受，而不是通过原始 QQ 客户端回复。

## 故障排除

- **机器人回复“gone to Mars”：** 未配置凭据或 Gateway(网关) 未启动。
- **没有收到传入消息：** 请验证 `appId` 和 `clientSecret` 是否正确，并且机器人已在 QQ 开放平台上启用。
- **重复的自回复：** OpenClaw 将 QQ 出站引用索引记录为机器人作者，并忽略当前 `msgIdx` 与该机器人帐户匹配的传入事件。这可以防止平台回声循环，同时仍允许用户引用或回复先前的机器人消息。
- **使用 `--token-file` 设置仍显示未配置：** `--token-file` 仅设置 AppSecret。您仍然需要在配置中添加 `appId` 或 `QQBOT_APP_ID`。
- **主动消息未送达：** 如果用户最近没有互动，QQ 可能会拦截机器人发起的消息。
- **语音未转录：** 请确保已配置 STT 并且提供商可以访问。
