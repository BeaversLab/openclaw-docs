...
状态：支持文本 + `sharePointSiteId` 附件；渠道/群组文件发送需要 `sharePointSiteId` + Graph 权限（请参阅[在群组聊天中发送文件](#sending-files-in-group-chats)）。投票通过自适应卡片发送。消息操作为文件优先发送暴露显式的 `upload-file`。

## 捆绑插件

Microsoft Teams 作为当前 OpenClaw 版本中的捆绑插件提供，因此在正常的打包构建中无需单独安装。

如果您使用的是较旧的构建或排除了捆绑 Teams 的自定义安装，请手动安装：
...
2. 创建一个 **Azure 机器人**（App ID + 客户端密码 + 租户 ID）。
3. 使用这些凭据配置 OpenClaw。
...
注意：群组聊天默认被阻止（`channels.msteams.groupPolicy: "allowlist"`）。要允许群组回复，请设置 `channels.msteams.groupAllowFrom`（或使用 `groupPolicy: "open"` 以允许任何成员，通过提及控制）。

## 目标

- 通过 Teams OpenClaw、群组聊天或渠道与 OpenClaw 交谈。
- 保持路由确定性：回复总是返回到它们到达的渠道。
...
- 默认：`channels.msteams.dmPolicy = "pairing"`。未知发件人将被忽略，直到获得批准。
...
- 默认值：`channels.msteams.groupPolicy = "allowlist"`（除非添加 `groupAllowFrom`，否则会被阻止）。当未设置时，使用 `channels.defaults.groupPolicy` 覆盖默认值。
- `channels.msteams.groupAllowFrom` 控制在群组聊天/渠道中哪些发件人可以触发（回退到 `channels.msteams.allowFrom`）。
...
- 通过在 `channels.msteams.teams` 下列出团队和渠道来限定群组/渠道的回复范围。
- 键应使用稳定的团队 ID 和渠道对话 ID。
- 当存在 `groupPolicy="allowlist"` 和团队允许列表时，仅接受列出的团队/渠道（受提及限制）。
- 配置向导接受 `Team/Channel` 条目并为您存储它们。
- 启动时，OpenClaw 会将团队/渠道和用户允许列表名称解析为 ID（当 Graph 权限允许时）并记录映射；未解析的团队/渠道名称将按输入保留，但默认情况下会被路由忽略，除非启用了 `channels.msteams.dangerouslyAllowNameMatching: true`。
...
配置 OpenClaw 之前，您需要创建一个 Azure Bot 资源。
...
除了手动创建清单 ZIP 包，你可以使用 [Teams Developer Portal](https://dev.teams.microsoft.com/apps)：
...
7. 在 Teams 中：**Apps** → **Manage your apps** → **Upload a custom app** → 选择该 ZIP 包
...
2. 发送一条消息 - 你应该会收到回复
3. 这会在 Teams 设置之前确认你的 Webhook 终结点正常工作
...
6. **运行网关**
   - 当内置或手动安装的插件可用且存在包含凭据的 `msteams` 配置时，Teams 渠道会自动启动。

## 成员信息操作

OpenClaw 为 Microsoft Teams 提供了一个基于 Graph 支持的 `member-info` 操作，以便代理和自动化可以直接从 Microsoft Graph 解析渠道成员详细信息（显示名称、电子邮件、角色）。
...
该操作受 `channels.msteams.actions.memberInfo` 限制（默认：当 Graph 凭据可用时启用）。

## 历史上下文

- `channels.msteams.historyLimit` 控制将多少条最近的渠道/群组消息包含到提示中。
- 回退到 `messages.groupChat.historyLimit`。设置 `0` 以禁用（默认 50）。
- 获取的线程历史记录会根据发件人允许列表 (`allowFrom` / `groupAllowFrom`) 进行过滤，因此线程上下文植入仅包含来自允许发件人的消息。
- 引用的附件上下文（从 Teams 回复 HTML 派生的 `ReplyTo*`）目前按接收到的原样传递。
- 换句话说，允许列表决定了谁可以触发代理；目前仅过滤特定的补充上下文路径。
- 私信历史记录可以用 `channels.msteams.dmHistoryLimit`（用户轮次）来限制。每用户覆盖：`channels.msteams.dms["<user_id>"].historyLimit`。

## 当前的 Teams RSC 权限（清单）

这些是我们的 Teams 应用清单中 **现有的 resourceSpecific 权限**。它们仅适用于安装了应用的团队/聊天内部。
...
- 如果您希望进行渠道流量交互，`authorization.permissions.resourceSpecific` 必须包含渠道读/写权限。
...
3. **重新打包**包含图标的清单文件（`manifest.json`、`outline.png`、`color.png`）
4. 上传新的 zip 文件：
...
**底线：** RSC 用于实时监听；Graph API 用于历史访问。若要在离线时补看错过的消息，您需要带有 `ChannelMessage.Read.All` 的 Graph API（需要管理员同意）。

## 启用 Graph 的媒体 + 历史（渠道必需）

如果您需要 **渠道** 中的图片/文件或想要获取 **消息历史**，则必须启用 Microsoft Graph 权限并授予管理员同意。
...
2. **授予租户的管理员同意**。
3. 更新 Teams 应用的**清单版本**，重新上传，并**在 Teams 中重新安装该应用**。
4. **完全退出并重新启动 Teams** 以清除缓存的应用元数据。

**用户提及的其他权限：** 对于对话中的用户，用户 @提及功能开箱即用。但是，如果您想动态搜索并提及**不在当前对话中**的用户，请添加 `User.Read.All` (Application) 权限并授予管理员同意。
...
OpenClaw 通过快速返回并发送主动回复来处理此问题，但响应非常缓慢仍可能导致问题。
...
- `channels.msteams.allowFrom`：私信允许列表（推荐使用 AAD 对象 ID）。当可使用 Graph 访问权限时，向导会在设置期间将名称解析为 ID。
...
- `channels.msteams.teams.<teamId>.toolsBySender`：默认按团队按发件人工具策略覆盖（支持 `"*"` 通配符）。
...
- 会话键遵循标准代理格式（请参阅 [/concepts/会话](/en/concepts/session)）：
...
Teams 最近在相同的数据模型之上引入了两种渠道 UI 样式：

| 样式                   | 描述                                 | 推荐的 `replyStyle` |
| ---------------------- | ------------------------------------ | ------------------- |
| **帖子**（经典）       | 消息以卡片形式显示，下方带有线程回复 | `thread`（默认）    |
| **主题**（类似 Slack） | 消息线性流动，更类似于 Slack         | `top-level`         |

**问题：** Teams API 不会公开渠道使用的 UI 样式。如果您使用了错误的 `replyStyle`：

- 在主题样式的渠道中使用 `thread` → 回复会以尴尬的方式嵌套显示
- 在帖子样式的渠道中使用 `top-level` → 回复显示为单独的顶级帖子，而不是在线程内
...
- **渠道/群组：** 附件驻留在 M365 存储中。Webhook 载荷仅包含 HTML 存根，而不包含实际文件字节。下载渠道附件**需要 Graph API 权限**。
...
默认情况下，OpenClaw 仅从 Microsoft/Teams 主机名下载媒体。可以使用 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任何主机）。
授权标头仅附加到 `channels.msteams.mediaAuthAllowHosts` 中的主机（默认为 Graph + Bot Framework 主机）。请严格限制此列表（避免多租户后缀）。

## 在群组聊天中发送文件

Bot 可以使用 FileConsentCard 流程（内置）在私信中发送文件。但是，**在群聊/频道中发送文件**需要额外的设置：

| 上下文                 | 文件发送方式                          | 所需设置                             |
| ---------------------- | ------------------------------------- | ------------------------------------ |
| **私信**               | FileConsentCard → 用户接受 → bot 上传 | 开箱即用                             |
| **群聊/频道**          | 上传至 SharePoint → 分享链接          | 需要 `sharePointSiteId` + Graph 权限 |
| **图片（任何上下文）** | Base64 编码内联                       | 开箱即用                             |

### 为什么群聊需要 SharePoint

Bot 没有个人 OneDrive 驱动器（`/me/drive` Graph API 端点不适用于应用程序标识）。要在群聊/频道中发送文件，bot 会上传到 **SharePoint 网站** 并创建共享链接。
...
| 权限                                    | 共享行为                                       |
| --------------------------------------- | ---------------------------------------------- |
| 仅 `Sites.ReadWrite.All`                | 组织范围内的共享链接（组织内的任何人都可访问） |
| `Sites.ReadWrite.All` + `Chat.Read.All` | 每个用户的共享链接（仅聊天成员可访问）         |

每个用户的共享更安全，因为只有聊天参与者可以访问该文件。如果缺少 `Chat.Read.All` 权限，bot 将回退到组织范围内的共享。
...
注意：如果没有 `user:` 前缀，名称默认解析为组/团队。当通过显示名称定位人员时，请始终使用 `user:`。

## 主动消息

- 主动消息仅在用户进行交互**之后**才可能，因为我们在此时存储对话引用。
...
- **频道中无响应：** 默认需要提及；设置 `channels.msteams.requireMention=false` 或针对每个团队/频道进行配置。
- **版本不匹配（Teams 仍显示旧清单）：** 移除并重新添加应用，然后完全退出 Teams 以刷新。
- **来自 Webhook 的 401 Unauthorized：** 在没有 Azure JWT 的情况下手动测试时预期会出现此错误 - 表示端点可达但身份验证失败。请使用 Azure Web Chat 进行正确测试。

### 清单上传错误

- **“Icon file cannot be empty”：** 清单引用了大小为 0 字节的图标文件。创建有效的 PNG 图标（`outline.png` 为 32x32，`color.png` 为 192x192）。
- **“webApplicationInfo.Id already in use”：** 该应用仍安装在其他团队/聊天中。请先找到并卸载它，或等待 5-10 分钟以传播更改。
- **上传时出现“Something went wrong”：** 请改为通过 [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) 上传，打开浏览器开发人员工具 (F12) → 网络选项卡，并检查响应正文以获取实际错误。
...
4. 确认你使用的范围正确：团队使用 `ChannelMessage.Read.Group`，群组聊天使用 `ChatMessage.Read.Chat`
...
- [配对](/en/channels/pairing) — 私信身份验证和配对流程
- [群组](/en/channels/groups) — 群组聊天行为和提及限制
