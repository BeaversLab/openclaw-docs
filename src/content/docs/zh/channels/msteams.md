...
状态：支持文本和私信附件；渠道/群组文件发送需要 `sharePointSiteId` + Graph 权限（参见 [在群聊中发送文件](#sending-files-in-group-chats)）。投票通过自适应卡片发送。消息操作为文件优先发送公开了明确的 `upload-file`。
...
对于生产部署，建议使用 [联合身份验证](#federated-authentication-certificate--managed-identity)（证书或托管标识）代替客户端密钥。

注意：群聊默认被阻止（`channels.msteams.groupPolicy: "allowlist"`）。要允许群聊回复，请设置 `channels.msteams.groupAllowFrom`（或使用 `groupPolicy: "open"` 允许任何成员，但受提及限制）。
...
- `channels.msteams.groupAllowFrom` 控制哪些发件人可以在群聊/渠道中触发（回退到 `channels.msteams.allowFrom`）。
- 设置 `groupPolicy: "open"` 以允许任何成员（默认仍受提及限制）。
- 要允许**不启用任何渠道**，请设置 `channels.msteams.groupPolicy: "disabled"`。
...
- 当 `groupPolicy="allowlist"` 和 Teams 允许列表存在时，仅接受列出的团队/渠道（提及限制）。
- 配置向导接受 `Team/Channel` 条目并为您存储它们。
- 启动时，OpenClaw 会将团队/渠道和用户允许列表名称解析为 ID（当 Graph 权限允许时）
  并记录映射；未解析的团队/渠道名称将按输入保留，但在默认情况下会被忽略路由，除非启用了 `channels.msteams.dangerouslyAllowNameMatching: true`。
...
> **弃用通知：** 2025-07-31 之后，创建新的多租户机器人已被弃用。新机器人请使用 **单租户**。
...
3. 点击 **管理密码** → 转到应用注册
4. 在 **证书和机密** 下 → **新的客户端机密** → 复制 **值** → 这是您的 `appPassword`
5. 转到 **概览** → 复制 **目录（租户）ID** → 这是您的 `tenantId`

### 步骤 3：配置消息传送终结点

1. 在 Azure Bot → **配置** 中
2. 将 **消息传送终结点** 设置为您的 Webhook URL：
...
3. 在运行时，OpenClaw 使用 `@azure/identity` 从 Azure IMDS 终结点 (`169.254.169.254`) 获取令牌。
...
- 从 pod/虚拟机到 IMDS (`169.254.169.254:80`) 的网络访问权限
...
5. **确保对 IMDS (`169.254.169.254`) 的网络访问** — 如果使用 NetworkPolicy，请添加一条允许流量通过 80 端口访问 `169.254.169.254/32` 的出口规则。
...
无需手动创建清单 ZIP，你可以使用 [Teams 开发者门户](https://dev.teams.microsoft.com/apps)：
...
1. **确保 Microsoft Teams 插件可用**
   ...
   - 较旧或自定义的安装可以手动添加：
     ...
     - 从本地检出安装： `openclaw plugins install ./path/to/local/msteams-plugin`
...
6. **运行网关**
   - 当捆绑或手动安装的插件可用并且 `msteams` 配置中包含凭据时，Teams 渠道会自动启动。
...
- 对于跨团队查找： `User.Read.All` Graph Application 权限且需管理员同意

该操作受 `channels.msteams.actions.memberInfo` 限制（默认：当 Graph 凭证可用时启用）。

## 历史上下文

- `channels.msteams.historyLimit` 控制将多少条最近的频道/群组消息包装到提示中。
- 回退到 `messages.groupChat.historyLimit`。设置 `0` 以禁用（默认为 50）。
- 获取的线程历史记录会按发件人允许列表（`allowFrom` / `groupAllowFrom`）进行过滤，因此线程上下文植入仅包含来自被允许发件人的消息。
- 引用的附件上下文（从 Teams 回复 HTML 派生的 `ReplyTo*`）目前按原样传递。
- 换句话说，允许列表控制谁能触发智能体；目前只有特定的补充上下文路径会被过滤。
- 私信历史记录可以用 `channels.msteams.dmHistoryLimit`（用户轮次）限制。每个用户的覆盖设置：`channels.msteams.dms["<user_id>"].historyLimit`。
...
- 个人作用域中的文件处理需要 `bots[].supportsFiles: true`。
...
**底线：** RSC 用于实时监听；Graph API 用于历史访问。若要在离线时补读错过的消息，您需要带有 `ChannelMessage.Read.All` 的 Graph API（需要管理员同意）。
...
- `channels.msteams.dangerouslyAllowNameMatching`：紧急情况切换开关，用于重新启用可变的 UPN/显示名称匹配以及直接的团队/渠道名称路由。
...
- 在主题串样式的渠道中使用 `thread` → 回复会以尴尬的嵌套方式显示
...
默认情况下，OpenClaw 仅从 Microsoft/Teams 主机名下载媒体文件。可以通过 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任何主机）。
...
机器人没有个人的 OneDrive 驱动器（`/me/drive` Graph API 端点不适用于应用程序标识）。要在群聊/渠道中发送文件，机器人会上传到 **SharePoint 站点** 并创建共享链接。
...
| 群组聊天 + 文件 + 没有 `sharePointSiteId`   | 尝试上传到 OneDrive（可能会失败），仅发送文本    |
...
有关卡片架构和示例，请参阅[自适应卡片文档](https://adaptivecards.io/)。有关目标格式的详细信息，请参阅下方的[目标格式](#target-formats)。
...
注意：如果没有 `user:` 前缀，名称默认解析为群组/团队。当通过显示名称指定人员时，请始终使用 `user:`。
...
- **渠道中无响应：** 默认需要提及；请设置 `channels.msteams.requireMention=false` 或针对每个团队/渠道进行配置。
