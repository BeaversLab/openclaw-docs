...
`plugins install` 会注册并启用该插件。在您配置下方的 Slack 应用和渠道设置之前，该插件仍然不会执行任何操作。有关插件的一般行为和安装规则，请参阅 [Plugins](/zh/tools/plugin)。
...
```
        ...
        <Note>
          **Recommended（推荐）** 匹配 Slack 插件的完整功能集：App Home、斜杠命令、文件、表情反应、固定消息、群组私信以及表情符号/用户组的读取。当工作区策略限制范围时，请选择 **Minimal（最小）** —— 它涵盖私信、渠道/群组历史记录、提及和斜杠命令，但会丢弃文件、表情反应、固定消息、群组私信 (`mpim:*`)、`emoji:read` 和 `usergroups:read`。有关每个范围的理由和附加选项（如额外的斜杠命令），请参阅 [Manifest and scope checklist](#manifest-and-scope-checklist)。
        </Note>
  ...
  <Tab title="HTTP 请求 URL">
    <Steps>
      <Step title="创建新的 Slack 应用">
        打开 [api.slack.com/apps](https://api.slack.com/apps/new) → **Create New App** → **From a manifest** → 选择你的工作区 → 粘贴以下清单之一 → 将 `https://gateway-host.example.com/slack/events` 替换为你的公共 Gateway(网关) URL → **Next** → **Create**.
...
```
        ...
        <Note>
          **Recommended** 匹配 Slack 插件的完整功能集；**Minimal** 去除了文件、回应、固定消息、群组  (`mpim:*`)、`emoji:read` 和 `usergroups:read`，以适应受限的工作区。有关每个作用域的理由，请参阅 [Manifest and scope checklist](#manifest-and-scope-checklist)。
        </Note>

        <Info>
          三个 URL 字段（`slash_commands[].url`、`event_subscriptions.request_url` 和 `interactivity.request_url` / `message_menu_options_url`）都指向同一个 OpenClaw 端点。Slack 的清单架构要求它们分别命名，但 OpenClaw 根据负载类型进行路由，因此一个 `webhookPath`（默认 `/slack/events`）就足够了。没有 `slash_commands[].url` 的斜杠命令在 HTTP 模式下将静默无操作。
        </Info>

        Slack 创建应用后：

        - **Basic Information → App Credentials**：复制 **Signing Secret** 以用于请求验证。
        - **Install App → Install to Workspace**：复制 `xoxb-...` Bot User OAuth Token。
...
```
        ...
        <Note>
        在多账户 HTTP 中使用唯一的 Webhook 路径

        为每个账户指定不同的 `webhookPath`（默认 `/slack/events`），以免注册冲突。
        </Note>
...
<AccordionGroup>
  <Accordion title="可选的原生斜杠命令">
    ...
    - 使用 `/agentstatus` 而不是 `/status`，因为 `/status` 命令是被保留的。
    - 一次可用的斜杠命令不能超过 25 个。

    使用 [可用命令](/zh/tools/slash-commands#command-list) 的子集替换现有的 `features.slash_commands` 部分：
...
```
      ...
      <Tab title="HTTP 请求 URL">
        使用与上面的 Socket 模式相同的 `slash_commands` 列表，并为每个条目添加 `"url": "https://gateway-host.example.com/slack/events"`。示例：
...
<Tabs>
  ...
  <Tab title="提及和渠道用户">
    ...
    - 隐式回复机器人线程的行为（当 `thread.requireExplicitMention` 为 `true` 时禁用）
    ...
    `allowBots` 对渠道和私有渠道采用保守策略：仅当发送机器人明确列在该房间的 `users` 允许列表中，或者当 `channels.slack.allowFrom` 中的至少一个明确的 Slack 所有者 ID 当前是房间成员时，才会接受机器人发送的房间消息。通配符和显示名称所有者条目不满足所有者存在条件。所有者存在使用 Slack `conversations.members`；确保应用具有该房间类型的匹配读取范围（公共渠道为 `channels:read`，私有渠道为 `groups:read`）。如果成员查找失败，OpenClaw 将丢弃该机器人发送的房间消息。

    接受的机器人发送的 Slack 消息使用共享的 [bot loop protection](/zh/channels/bot-loop-protection)。配置 `channels.defaults.botLoopProtection` 以设置默认预算，然后当工作区或渠道需要不同的限制时，使用 `channels.slack.botLoopProtection` 或 `channels.slack.channels.<id>.botLoopProtection` 进行覆盖。
...
本机命令需要在您的 Slack 应用中进行[额外的清单设置](#additional-manifest-settingsSlack)，并在全局配置中使用 `channels.slack.commands.native: true` 或 `commands.native: true` 启用。
...
同一聊天 `/approve`Slack 也适用于已经支持命令的 Slack 渠道和私信。有关完整的审批转发模型，请参阅 [Exec approvals](/zh/tools/exec-approvals)。
...
<AccordionGroup>
  <Accordion title="No replies in channels">
    ...
    - `groupPolicy`
    - 渠道允许列表 (`channels.slack.channels`) — **键必须是渠道 ID** (`C12345678`)，而不是名称 (`#channel-name`)。在 `groupPolicy: "allowlist"`Slack 下，基于名称的键会静默失败，因为默认情况下渠道路由优先考虑 ID。要查找 ID：在 Slack 中右键单击渠道 → **复制链接** — URL 末尾的 `C...` 值即为渠道 ID。
    - `requireMention`
    - 每个渠道的 `users` 允许列表
    - `messages.groupChat.visibleReplies`：如果是 `"message_tool"` 且日志显示助手文本但没有 `message(action=send)` 调用，则模型错过了可见的消息-工具路径。在此模式下，最终文本保持私密；检查网关详细日志以获取被抑制的负载元数据，或者如果您希望每个正常的助手最终回复都通过旧路径发布，请将其设置为 `"automatic"`。
    - `messages.groupChat.unmentionedInbound`：如果是 `"room_event"`，未提及的允许渠道聊天将成为环境上下文并保持静默，除非代理调用 `message` 工具。请参阅 [Ambient room events](/zh/channels/ambient-room-events)。
...
```
  ...
  <Accordion title="私信 messages ignored">
    ...
    - 群组私信使用 MPIM 处理；启用 `channels.slack.dm.groupEnabled`，并且如果已配置，在 `channels.slack.dm.groupChannels` 中包含 MPIM
    - Slack Assistant 私信事件：提及 `drop message_changed` 的详细日志
      通常表示 Slack 发送了一个已编辑的 Assistant 线程事件，且消息元数据中没有可恢复的人类发送者
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    `appTokenStatus: "configured_unavailable"`，则表示已配置 Slack 账户，但当前运行时无法解析由 SecretRef 支持的值。

    诸如 `slack socket mode failed to start; retry ...` 之类的日志属于可恢复的启动失败。缺失作用域、令牌被撤销和身份验证无效会直接快速失败。`slack token mismatch ...` 日志表示 bot 令牌和 app 令牌似乎属于不同的 Slack 应用；请修复 Slack 应用凭据。

  </Accordion>

  <Accordion title="HTTP 模式未接收到事件"Slack>
    请验证：
    ...
    如果帐户快照中出现了 `signingSecretStatus: "configured_unavailable"`，说明 HTTP 帐户已配置，但当前运行时无法解析由 SecretRef 支持的签名密钥。

    重复出现的 `slack: webhook path ... already registered` 日志意味着两个 HTTP 帐户正在使用相同的 `webhookPath`；请为每个帐户指定不同的路径。
  ...
  <Accordion title="Native/slash commands not firing">
    ...
    Slack 不会自动创建或删除斜杠命令。`commands.native: "auto"` 不会启用 Slack 原生命令；请使用 `true` 并在 Slack 应用中创建匹配的命令。在 HTTP 模式下，每个 Slack 斜杠命令必须包含 Gateway(网关) URL。在 Socket 模式下，命令负载通过 websocket 到达，Slack 会忽略 `slash_commands[].url`。

    还要检查 `commands.useAccessGroups`、私信授权、渠道允许列表，
    以及每个渠道的 `users` 允许列表。对于
...
| 媒体类型                     | 来源            | 当前行为                                                        | 备注                                                    |
| ---------------------------- | --------------- | --------------------------------------------------------------- | ------------------------------------------------------- |
| JPEG / PNG / GIF / WebP 图像 | Slack 文件 URL  | 已下载并附加到轮次以进行支持视觉的处理                          | 每个文件上限：`channels.slack.mediaMaxMb`（默认 20 MB） |
| PDF 文件                     | Slack 文件 URL  | 已下载并作为文件上下文公开给工具（如 `download-file` 或 `pdf`） | Slack 入站不会自动将 PDF 转换为图像视觉输入             |
...
- **视觉模型**：图像分析使用当前激活的回复模型（如果该模型支持视觉功能），或者在 `agents.defaults.imageModel` 中配置的图像模型。
...
| 未配置视觉模型               | 图像附件作为媒体引用存储，但未作为图像进行分析    | 配置 `agents.defaults.imageModel` 或使用支持视觉的回复模型          |
| 非常大的图像（默认 > 20 MB） | 因大小限制被跳过                                  | 如果 Slack 允许，增加 `channels.slack.mediaMaxMb`                   |
