...
<AccordionGroup>
  <Accordion title="可选的原生斜杠命令">
    ...
    使用[可用命令](/zh/tools/slash-commands#command-list)的子集替换您现有的 `features.slash_commands` 部分：
...
同聊 `/approve` 也适用于已经支持命令的 Slack 渠道和私信。请参阅 [Exec approvals](/zh/tools/exec-approvals) 了解完整的审批转发模型。
