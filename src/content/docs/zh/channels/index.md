...
- [Synology Chat](/en/channels/synology-chat) — 通过传出和传入 Webhook 连接的 Synology NAS Chat（插件，需单独安装）。
...
- [Voice Call](/en/plugins/voice-call) — 通过 Plivo 或 Twilio 提供的电话服务（插件，需单独安装）。
- [WebChat](/en/web/webchat) — 基于 WebSocket 的 Gateway(网关) WebChat 界面。
...
- [Zalo](/en/channels/zalo) — Zalo Bot API；越南流行的通讯工具（插件，需单独安装）。
- [Zalo Personal](/en/channels/zalouser) — 通过二维码登录的 Zalo 个人账号（插件，需单独安装）。

## 备注

- 渠道可以同时运行；配置多个渠道后，OpenClaw 将根据聊天进行路由。
- 最快的设置通常是 **Telegram**（简单的 Bot 令牌）。WhatsApp 需要二维码配对，并且在磁盘上存储更多状态。
...
- 模型提供商另有文档说明；请参阅 [Model Providers](/en/providers/models)。
