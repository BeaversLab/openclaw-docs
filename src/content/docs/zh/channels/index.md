...
- [Synology Chat](/zh/channels/synology-chat) - 通过传出和传入 Webhook 连接的 Synology NAS Chat（附带插件）。
- [Telegram](/zh/channels/telegram) - 通过 API 实现的 Bot grammY；支持群组。
- [Tlon](/zh/channels/tlon) - 基于 Urbit 的通讯工具（附带插件）。
...
- [Zalo](/zh/channels/zalo) - Zalo Bot API；越南流行的通讯工具（附带插件）。
- [Zalo Personal](/zh/channels/zalouser) - 通过二维码登录的 Zalo 个人账号（附带插件）。
...
- 为了安全起见，强制执行私信配对和允许列表；请参阅[安全](/zh/gateway/security)。
- 故障排除：[渠道故障排除](/zh/channels/troubleshooting)。
- 模型提供商有单独的文档；请参阅[模型提供商](/zh/providers/models)。
