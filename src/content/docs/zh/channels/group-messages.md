...
- 激活模式：`mention`（默认）或 `always`。`mention` 需要收到一次 ping（通过 `mentionedJids` 发起的真实 WhatsApp @提及、安全的正则表达式模式，或文本中任何位置出现的机器人 E.164 号码）。`always` 会在每条消息时唤醒代理，但它应仅在能提供有意义价值时回复；否则将返回静默令牌 `NO_REPLY`。默认值可在配置（`channels.whatsapp.groups`）中设置，并可通过 `/activation` 针对每个群组进行覆盖。当设置了 `channels.whatsapp.groups` 时，它也作为群组白名单生效（包含 `"*"` 以允许所有群组）。
...
- 当有人点击联系人时，WhatsApp 仍会通过 `mentionedJids` 发送标准提及，因此号码备用方案很少需要，但它是一个有用的安全网。
...
只有所有者号码（来自 `channels.whatsapp.allowFrom`，或未设置时的机器人自身 E.164 号码）可以更改此项。在群组中发送 `/status` 作为一条独立消息，以查看当前的激活模式。

## 如何使用

1. 将您的 WhatsApp 账户（运行 OpenClaw 的那个）添加到群组中。
2. 说 `@openclaw …`（或包含号码）。除非您设置了 `groupPolicy: "open"`，否则只有白名单发送者可以触发它。
3. 代理提示将包含最近的群组上下文以及尾随的 `[from: …]` 标记，以便它能称呼正确的人。
...
- 会话存储条目将显示为会话存储中的 `agent:<agentId>:whatsapp:group:<jid>`（默认为 `~/.openclaw/agents/<agentId>/sessions/sessions.json`）；缺少条目仅意味着该群组尚未触发运行。
