...
2. 登录（QR 码，在 Gateway 机器上）：
   - `openclaw channels login --channel zalouser`
   - 使用 Zalo 手机应用程序扫描 QR 码。
3. 启用渠道：
...
- 专为 Zalo Bot API 不可用的“个人账户”用例而设计。

## 命名

渠道 ID 为 `zalouser`，以明确表明这是自动化 **个人 Zalo 用户账号**（非官方）。我们保留 `zalo` 用于未来可能的官方 Zalo API 集成。
...
`channels.zalouser.allowFrom` 接受用户 ID 或名称。在设置过程中，名称将使用插件进程内的联系人查找解析为 ID。
...
- 群组允许列表匹配默认仅基于 ID。除非启用了 `channels.zalouser.dangerouslyAllowNameMatching: true`，否则未解析的名称在身份验证中将被忽略。
- `channels.zalouser.dangerouslyAllowNameMatching: true` 是一种应急兼容模式，可重新启用可变群组名称匹配。
...
- 引用机器人消息被视为用于激活群组的隐式提及。
- 授权的控制命令（例如 `/new`）可以绕过提及限制。
- 当由于需要提及而跳过群组消息时，OpenClaw 会将其存储为待处理的群组历史记录，并将其包含在下一条处理的群组消息中。
- 群组历史记录限制默认为 `messages.groupChat.historyLimit`（回退值 `50`）。您可以使用 `channels.zalouser.historyLimit` 为每个帐户进行覆盖。
...
账户映射到 OpenClaw 状态中的 `zalouser` 个人资料。示例：
...
- 渠道动作支持针对 `zalouser` 的消息反应动作 `react`。
  - 使用 `remove: true` 从消息中移除特定的反应表情符号。
  - 反应语义：[Reactions](/zh/tools/reactions)
- 对于包含事件元数据的入站消息，OpenClaw 会发送送达 + 已读回执（尽力而为）。
