...
您需要创建一个包含机器人 的新应用程序，将其添加到您的服务器，并将其与 OpenClaw 配对。我们建议将您的机器人添加到您自己的私人服务器。如果您还没有，请[先创建一个](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server)（选择 **Create My Own > For me and my friends**）。

<Steps>
  <Step title="Create a Discord application and bot">
    前往 [Discord Developer Portal](https://discord.com/developers/applications) 并点击 **New Application**。将其命名为类似 "OpenClaw" 的名称。

    在侧边栏点击 **Bot**。将 **Username** 设置为您称呼 OpenClaw 代理的名称。
  ...
  <Step title="Generate an invite URL and add the bot to your server">
    ...
    **Text Permissions**
    ...
    这是普通文本渠道 的基准设置。如果您计划在 Discord 话题中发布，包括创建或继续话题的论坛或媒体渠道工作流，请同时启用 **Send Messages in Threads**。
    复制底部生成的 URL，将其粘贴到您的浏览器中，选择您的服务器，然后点击 **Continue** 进行连接。您现在应该在 Discord 服务器中看到您的机器人了。
...
```
  ...
  <Step title="配置 OpenClaw 并配对">
    ...
    <Tabs>
      <Tab title="询问您的代理">
        在任何现有渠道（例如 OpenClaw）上与您的 Telegram 代理聊天，并告知它。如果 Discord 是您的第一个渠道，请改用 CLI / config 选项卡。

        > “我已在 config 中设置了 Discord bot 令牌。请使用用户 ID `<user_id>` 和服务器 ID `<server_id>` 完成 Discord 设置。”
      </Tab>
...
```
        ...
        支持纯文本 `token` 值。对于 env/file/exec 提供商的 `channels.discord.token`，也支持 SecretRef 值。请参阅[机密管理](/zh/gateway/secrets)。
...
`/model` 和 `/models` 斜杠命令会打开一个交互式模型选择器，其中包含提供商和模型下拉菜单以及一个提交步骤。除非 `commands.modelsWrite=false`，否则 `/models add` 也支持从聊天中添加新的提供商/模型条目，并且新添加的模型会在不重启网关的情况下显示。选择器回复是临时的，只有调用的用户可以使用它。
...
<Tabs>
  <Tab title="私信 policy">
    ...
    除非提供明确的用户/渠道目标类型，否则纯数字 ID 存在歧义且会被拒绝。
  ...
  <Tab title="Guild policy">
    ...
    - 可选的发送方允许列表：`users`（推荐使用稳定 ID）和 `roles`（仅限角色 ID）；如果配置了其中任何一个，当发送方匹配 `users` 或 `roles` 时即被允许
    - 直接名称/标签匹配默认禁用；仅将 `channels.discord.dangerouslyAllowNameMatching: true` 作为应急兼容模式启用
...
```
    ...
    如果您仅设置 `DISCORD_BOT_TOKEN` 而未创建 `channels.discord` 块，则运行时回退为 `groupPolicy="allowlist"`（并在日志中发出警告），即使 `channels.defaults.groupPolicy` 是 `open`。
  ...
  <Tab title="提及和群组私信">
    ...
    `requireMention` 是按公会/渠道配置的 (`channels.discord.guilds...`)。
...
使用 `bindings[].match.roles` 根据 角色 ID 将 Discord 公会成员路由到不同的 代理。基于角色的绑定仅接受角色 ID，并且在同级或父级同级绑定之后、仅公会绑定之前进行评估。如果绑定还设置了其他匹配字段（例如 `peer` + `guildId` + `roles`），则所有配置的字段都必须匹配。
...
<AccordionGroup>
  ...
  <Accordion title="OAuth 范围和基线权限">
    ...
    **Text Permissions**
    ...
    这是普通文本渠道的基线集。如果您计划在 Discord 线程中发布，包括创建或继续线程的论坛或媒体渠道工作流，也请启用 **Send Messages in Threads**。
...
- 针对每个渠道的覆盖设置：`channels.discord.commands.native`。
...
<AccordionGroup>
  <Accordion title="回复标签和原生回复">
    ...
    `first` 始终将隐式原生回复引用附加到该轮次的第一个 Discord 出站消息。
    `batched` 仅当入站轮次是多个消息的去抖动批量时，才附加 Discord 的隐式原生回复引用。如果您主要针对模棱两可的突发聊天而不是每个单消息轮次使用原生回复，这非常有用。

    消息 ID 会显示在上下文/历史记录中，以便智能体能够针对特定消息。
  ...
  <Accordion title="Live stream preview">
    ...
    - 默认保持 `off`，因为 Discord 预览编辑可能会很快达到速率限制，尤其是当多个机器人或网关共享同一账户或频道流量时。
    - 接受 `progress` 以保持跨渠道一致性，并映射到 Discord 上的 `partial`。
    - `channels.discord.streamMode` 是旧版别名，会自动迁移。
    - `partial` 随着令牌的到来编辑单个预览消息。
    - `block` 发出草稿大小的块（使用 `draftChunk` 调整大小和断点）。
    - 媒体、错误和显式回复最终会在正常交付之前取消挂起的预览编辑，而不会刷新临时草稿。
    - `streaming.preview.toolProgress` 控制工具/进度更新是否重用同一草稿预览消息（默认：`true`）。设置 `false` 以保持单独的工具/进度消息。
...
```
  ...
  <Accordion title="历史、上下文和线程行为">
    ...
    - 通过 `channels.discord.thread.inheritParent` 可选择将父转录记录继承到新创建的自动线程中（默认 `false`）。当 `false` 时，新创建的 Discord 线程会话与父渠道转录记录隔离开始；当 `true` 时，父渠道历史记录将作为新线程会话的种子
    - 每个账户的覆盖项位于 `channels.discord.accounts.<id>.thread.inheritParent` 下
    - 消息工具反应可以解析 `user:<id>` 私信目标以及渠道目标
    - `channels.discord.guilds.<guild>.channels.<channel>.requireMention: false` 在回复阶段激活回退期间得以保留，因此配置为始终开启的渠道即使在运行回复阶段回退时也会保持始终开启状态

    渠道话题作为**不受信任的**上下文（而非系统提示）注入。
    回复和引用消息上下文当前保持原样接收。
    Discord 白名单主要控制谁可以触发代理，而不是完整的补充上下文编辑边界。

  </Accordion>

  <Accordion title="Thread-bound sessions for subagents">
    Discord 可以将主题绑定到会话目标，以便该主题中的后续消息继续路由到同一会话（包括子代理会话）。

    命令：

    - `/focus <target>` 将当前/新主题绑定到子代理/会话目标
    - `/unfocus` 移除当前主题绑定
    - `/agents` 显示活动运行和绑定状态
    - `/session idle <duration|off>` 检查/更新焦点绑定的非活动自动取消聚焦设置
...
```
    ...
    - `spawnSubagentSessions` 必须为 true 才能为 `sessions_spawn({ thread: true })` 自动创建/绑定主题。
    - `spawnAcpSessions` 必须为 true 才能为 ACP（`/acp spawn ... --thread ...` 或 `sessions_spawn({ runtime: "acp", thread: true })`）自动创建/绑定主题。
    - 如果某个账户禁用了主题绑定，则 `/focus` 及相关主题绑定操作将不可用。
...
```
    ...
    - `/acp spawn codex --bind here` 会将当前的 Discord 渠道或主题就地绑定，并确保未来的消息路由到同一个 ACP 会话。
    - 这仍然可能意味着“启动一个新的 Codex ACP 会话”，但它本身不会创建新的 Discord 主题。现有渠道保持为聊天界面。
    - Coded 仍可能在其自己的 `cwd` 或磁盘上的后端工作区中运行。该工作区是运行时状态，而不是 Discord 主题。
    - 主题消息可以继承父渠道的 ACP 绑定。
    - 在已绑定的渠道或主题中，`/new` 和 `/reset` 会就地重置同一个 ACP 会话。
    - 临时主题绑定仍然有效，并且在活动期间可以覆盖目标解析。
    - 只有当 OpenClaw 需要通过 `--thread auto|here` 创建/绑定子主题时，才需要 `spawnAcpSessions`。对于当前渠道中的 `/acp spawn ... --bind here`，则不需要。
...
```
  ...
  <Accordion title="Presence configuration">
    当您设置状态或活动字段，或者启用自动在线状态时，会应用在线状态更新。
...
```
  ...
  <Accordion title="Discord 中的审批">
    Discord 支持在私信中通过基于按钮的审批处理，并可以选择在原始渠道中发布审批提示。
    ...
    当 `enabled` 未设置或为 `"auto"` 并且至少可以解析出一个审批人（无论是从 `execApprovals.approvers` 还是从 `commands.ownerAllowFrom`）时，Discord 会自动启用原生执行审批。Discord 不会从渠道 `allowFrom`、旧版 `dm.allowFrom` 或私信 `defaultTo` 推断执行审批人。设置 `enabled: false` 以明确禁用 Discord 作为原生审批客户端。

    当 `target` 为 `channel` 或 `both` 时，审批提示在渠道中可见。只有已解析的审批人可以使用这些按钮；其他用户会收到临时的拒绝通知。审批提示包含命令文本，因此请仅在受信任的渠道中启用渠道传递。如果无法从会话密钥派生渠道 ID，OpenClaw 将回退到私信传递。

    Discord 还会呈现其他聊天渠道使用的共享审批按钮。原生 Discord 适配器主要增加了审批人私信路由和渠道分发。
    当存在这些按钮时，它们是主要的审批用户体验；仅当工具结果指示
    聊天审批不可用或手动审批是唯一途径时，OpenClaw
    才应包含手动 `/approve` 命令。

    此处理程序的 Gateway(网关) 身份验证使用与其他 Gateway(网关) 客户端相同的共享凭据解析契约：

    - 环境优先的本地身份验证 （`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD` 然后是 `gateway.auth.*`）
    - 在本地模式下，仅当 `gateway.auth.*` 未设置时，`gateway.remote.*` 才能用作回退；已配置但未解析的本地 SecretRef 将以失败关闭
    - 通过 `gateway.remote.*` 支持远程模式（如果适用）
    ...
    - Discord 此处不会执行额外的执行到插件回退跳转；ID 前缀决定了它调用的 Gateway(网关) 方法。

    执行审批默认在 30 分钟后过期。如果审批因
    未知的审批 ID 失败，请验证审批人解析、功能启用，
    以及已传递的审批 ID 类型是否与待处理请求匹配。
...
`event-create` 动作接受一个可选的 `image` 参数（URL 或本地文件路径）来设置预定活动的封面图片。
...
OpenClaw 使用 Discord 组件 v2 进行执行审批和跨上下文标记。Discord 消息动作也可以接受 `components` 用于自定义 UI（高级；需要通过 discord 工具构造组件负载），而旧版 `embeds` 仍然可用，但不推荐使用。
...
- 语音转录回合从 Discord `allowFrom`（或 `dm.allowFrom`）派生所有者状态；非所有者发言人无法访问仅限所有者的工具（例如 `gateway` 和 `cron`）。
- 默认情况下启用语音；设置 `channels.discord.voice.enabled=false` 以将其禁用。
- `voice.daveEncryption` 和 `voice.decryptionFailureTolerance` 会传递到 `@discordjs/voice` 加入选项。
...
Discord 语音消息显示波形预览，并需要 OGG/Opus 音频和元数据。OpenClaw 会自动生成波形，但这需要网关主机上有 `ffmpeg` 和 `ffprobe` 来检查和转换音频文件。
...
<AccordionGroup>
  ...
  <Accordion title="Guild messages blocked unexpectedly">
    ...
    - 验证 `groupPolicy`
    - 验证 `channels.discord.guilds` 下的允许列表
...
```
    ...
    使用 `eventQueue.listenerTimeout` 进行慢速监听器设置，并且仅当您希望为排队代理轮次设置单独的安全阀时才使用 `inboundWorker.runTimeoutMs`。
...
- 入站工作进程: `inboundWorker.runTimeoutMs`
...
- 媒体/重试: `mediaMaxMb`, `retry`
  - `mediaMaxMb` 限制出站 Discord 上传（默认值：`100MB`）
- 操作: `actions.*`
...
- 授予 Discord 最小权限。
- 如果 command 部署/状态过时，请重启网关并使用 `openclaw channels status --probe` 重新检查。
