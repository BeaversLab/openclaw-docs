...
````
  ...
  <Step title="允许不通过 @提及 进行回复">
    默认情况下，您的代理仅在收到 @提及 时才会在频道中响应。对于私人服务器，您可能希望它响应每条消息。

    在频道中，可见的 Discord 输出默认应使用 `message` 工具，以便代理可以潜伏，仅在认为频道回复有用时才发布。除非工具发送信息，否则环境房间事件保持静默。有关完整的潜伏模式配置，请参阅 [环境房间事件](/zh/channels/ambient-room-events)。

    这意味着所选的模型应能可靠地调用工具。如果 Discord 显示正在输入且日志显示正在使用 token 但没有发布消息，请检查该回合是否被配置为环境房间事件，或使用下面的配置恢复针对正常群组请求的传统自动最终回复。
    ...
    <Tabs>
      ...
      <Tab title="配置">
        在您的频道配置中设置 `requireMention: false`：
...
```
        ...
        要恢复针对群组/频道房间的传统自动最终回复，请设置 `messages.groupChat.visibleReplies: "automatic"`。
...
```
  ...
  <Accordion title="Bot to bot loops">
    ...
    建议使用 `channels.discord.allowBots="mentions"`OpenClaw 来仅接受提及了该机器人的机器人消息。

    OpenClaw 还提供了共享的 [机器人循环保护](/zh/channels/bot-loop-protection)。每当 `allowBots`Discord 允许机器人发送的消息到达分发时，Discord 会将传入事件映射到 `(account, channel, bot pair)`Discord 事实，通用配对守卫会在其超过配置的事件预算后抑制该配对。该守卫可以防止以前必须由 Discord 速率限制来阻止的失控的双机器人循环；它不会影响保持在预算之内的单机器人部署或一次性机器人回复。

    默认设置（当设置 `allowBots` 时激活）：

    - `maxEventsPerWindow: 20` -- 机器人配对可以在滑动窗口内交换 20 条消息
    - `windowSeconds: 60` -- 滑动窗口长度
    - `cooldownSeconds: 60` -- 一旦预算触发，每个方向的额外机器人到机器人消息将被丢弃一分钟

    在 `channels.defaults.botLoopProtection`Discord 下配置一次共享默认值，然后当合法工作流需要更多空间时覆盖 Discord。优先顺序为：
