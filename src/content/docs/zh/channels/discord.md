...
您需要创建一个包含 bot 的新应用程序，将 bot 添加到您的服务器，并将其与 OpenClaw 配对。我们建议将您的 bot 添加到您自己的私人服务器。如果您还没有私人服务器，请先[创建一个](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server)（选择 **Create My Own > For me and my friends**）。
...
<Steps>
  <Step title="创建 Discord 应用程序和 bot">
    ...
    点击侧边栏上的 **Bot**。将 **Username** 设置为您称呼您的 OpenClaw 代理的名称。
...
```
        ...
        支持纯文本 `token` 值。跨 env/file/exec 提供程序也支持 SecretRef 值，用于 `channels.discord.token`。请参阅 [Secrets Management](/en/gateway/secrets)。
...
```
    ...
    - 如果为帐户禁用了线程绑定，则 `/focus` 和相关线程绑定操作将不可用。
  ...
  <Accordion title="Persistent ACP 渠道 bindings">
    对于稳定的“常驻”ACP工作区，请配置针对Discord对话的顶层类型化ACP绑定。
...
```
    ...
    - `/acp spawn codex --bind here` 会将当前的Discord频道或子版块就地绑定，并将未来的消息路由至同一个ACP会话。
    - 这仍然可能意味着“启动一个新的Codex ACP会话”，但它本身不会创建一个新的Discord子版块。现有频道保持为聊天界面。
    - Codex 仍可能在其自身的 `cwd` 或磁盘上的后端工作区中运行。该工作区是运行时状态，而非Discord子版块。
    - 子版块消息可以继承父级频道的ACP绑定。
    - 在已绑定的频道或子版块中，`/new` 和 `/reset` 会就地重置同一个ACP会话。
    - 临时子版块绑定仍然有效，并且可以在激活时覆盖目标解析。
    - 仅当OpenClaw需要通过 `--thread auto|here` 创建/绑定子子版块时，才需要 `spawnAcpSessions`。对于当前频道中的 `/acp spawn ... --bind here`，则不需要。
...
```
  ...
  <Accordion title="Discord 中的审批">
    ...
    当 `enabled` 未设置或为 `"auto"` 并且至少可以解析出一个审批者（无论是从 `execApprovals.approvers` 还是从 `commands.ownerAllowFrom`）时，Discord 会自动启用原生执行审批。Discord 不会从渠道 `allowFrom`、旧版 `dm.allowFrom` 或直接消息 `defaultTo` 推断执行审批者。设置 `enabled: false` 以明确禁用 Discord 作为原生审批客户端。

    当 `target` 为 `channel` 或 `both` 时，审批提示在渠道中可见。只有已解析的审批者可以使用按钮；其他用户会收到临时拒绝消息。审批提示包含命令文本，因此仅可在受信任的渠道中启用渠道投递。如果无法从会话密钥派生渠道 ID，OpenClaw 将回退到私信投递。

    Discord 还会呈现其他聊天渠道使用的共享审批按钮。原生 Discord 适配器主要增加了审批者私信路由和渠道分发。
    当存在这些按钮时，它们是主要的审批用户体验；OpenClaw
    仅应在工具结果指出聊天审批不可用或手动审批是唯一路径时才包含手动 `/approve` 命令。

    此处理程序的 Gateway(网关) 身份验证与其他 Gateway(网关) 客户端使用相同的共享凭据解析约定：
    ...
    - Discord 不会在此处进行额外的执行到插件回退跳转；ID
      前缀决定它调用哪种 gateway 方法。

    执行审批默认在 30 分钟后过期。如果审批因
    未知的审批 ID 失败，请验证审批者解析、功能启用以及
    投递的审批 ID 类型是否与待处理请求匹配。
...
OpenClaw 使用 Discord 组件 v2 进行执行审批和跨上下文标记。Discord 消息操作还可以接受 `components` 用于自定义 UI（高级；需要通过 discord 工具构造组件负载），而传统的 `embeds` 仍然可用但不建议使用。
...
- 语音转录回合根据 Discord `allowFrom`（或 `dm.allowFrom`）确定所有者状态；非所有者发言者无法访问所有者专属工具（例如 `gateway` 和 `cron`）。
...
- 如果接收日志反复显示 `DecryptionFailed(UnencryptedWhenPassthroughDisabled)`，这可能是 [discord.js #11419](https://github.com/discordjs/discord.js/issues/11419) 中追踪的上游 `@discordjs/voice` 接收错误。

## 语音消息

Discord 语音消息显示波形预览，并且需要 OGG/Opus 音频和元数据。OpenClaw 会自动生成波形，但它需要网关主机上有 `ffmpeg` 和 `ffprobe` 可用来检查和转换音频文件。
...
```
    ...
    使用 `eventQueue.listenerTimeout` 进行慢速监听器设置，并 `inboundWorker.runTimeoutMs`
    仅当您希望为排队代理回合设置单独的安全阀时使用。
...
- 请将 bot token 视为机密（在受监管环境中首选 `DISCORD_BOT_TOKEN`）。
- 授予最小权限 Discord 权限。
