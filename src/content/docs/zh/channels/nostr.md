...
- 新手引导（`openclaw onboard`）和 `openclaw channels add` 仍然会从共享渠道目录中展示 Nostr。
- 如果您的构建排除了捆绑的 Nostr，请手动安装它。
...
个人资料数据作为 NIP-01 `kind:0` 事件发布。您可以通过控制 UI（渠道 -> Nostr -> 个人资料）管理它，或直接在配置中设置。
...
- 配对回复的发送不经过处理原始私信正文的过程。
- 入站私信受到速率限制，超大的载荷在解密前会被丢弃。
...
| NIP-17 | 计划中 | 礼品包装的私信 (Gift-wrapped 私信) |
| NIP-44 | 计划中 | 版本化加密                         |
...
- 使用多个中继时属于预期情况。
- 消息按事件 ID 进行去重；仅首次投递会触发响应。

## 安全性

- 切勿提交私钥。
- 使用环境变量存储密钥。
- 对于生产环境的 bot，请考虑使用 `allowlist`。
- 签名会在发送者策略之前验证，且发送者策略会在解密之前强制执行，因此伪造的事件会被尽早拒绝，且未知发送者无法强制进行完整的加密运算。
