...
- 渠道探测显示传输已连接，且在支持的情况下，`works` 或 `audit ok`
...
| 机器人在线但群组无响应         | 验证提及要求和机器人隐私模式             | 禁用隐私模式以实现群组可见性或提及机器人。                                                 |
| 发送失败并伴随网络错误         | 检查日志中 Telegram API 调用失败的情况   | 修复到 `api.telegram.org` 的 DNS/IPv6/代理路由。                                           |
| 轮询停滞或重连缓慢             | `openclaw logs --follow` 用于轮询诊断    | 升级；如果重启是误报，请调整 `pollingStallThresholdMs`。持续停滞仍指向代理/DNS/IPv6 问题。 |
...
| 症状                      | 最快检查                           | 修复                                                                                                                                |
| ------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Socket 模式已连接但无响应 | `openclaw channels status --probe` | 验证 app token + bot token 和所需范围；在 SecretRef 支持的设置上留意 `botTokenStatus` / `appTokenStatus = configured_unavailable`。 |
...
| 症状                          | 最快检查                                                                | 修复                                         |
| ----------------------------- | ----------------------------------------------------------------------- | -------------------------------------------- |
| 无传入事件                    | 验证 webhook/服务器可达性和应用权限                                     | 修复 webhook URL 或 BlueBubbles 服务器状态。 |
| 在 macOS 上可以发送但无法接收 | 检查 macOS 隐私权限以进行消息自动化                                     | 重新授予 TCC 权限并重启频道进程。            |
...
| 症状                   | 最快检查                               | 修复                                                |
| ---------------------- | -------------------------------------- | --------------------------------------------------- |
| Bot 回复“gone to Mars” | 验证配置中的 `appId` 和 `clientSecret` | 设置凭证或重启网关。                                |
| 无入站消息             | `openclaw channels status --probe`     | 在 QQ 开放平台上验证凭证。                          |
...
| 症状                 | 最快检查                               | 修复                                                                  |
| -------------------- | -------------------------------------- | --------------------------------------------------------------------- |
| 已登录但忽略房间消息 | `openclaw channels status --probe`     | 检查 `groupPolicy`、房间白名单和提及门槛。                            |
...
| 备份恢复挂起/失败    | `openclaw matrix verify backup status` | 运行 `openclaw matrix verify backup restore` 或使用恢复密钥重新运行。 |
| 跨签名/引导异常      | `openclaw matrix verify bootstrap`     | 一次性修复密钥存储、跨签名和备份状态。                                |
