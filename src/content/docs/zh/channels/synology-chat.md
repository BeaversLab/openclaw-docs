...
`SYNOLOGY_CHAT_INCOMING_URL` 无法从工作区 `.env` 中设置；请参阅 [工作区 `.env` 文件](/zh/gateway/security)。

## 私信策略和访问控制

- `dmPolicy: "allowlist"` 是推荐的默认设置。
- `allowedUserIds` 接受 Synology 用户 ID 列表（或逗号分隔的字符串）。
- 在 `allowlist` 模式下，空的 `allowedUserIds` 列表被视为配置错误，webhook 路由将不会启动（请使用带有 `allowedUserIds: ["*"]` 的 `dmPolicy: "open"` 以允许所有）。
- 仅当 `allowedUserIds` 包含 `"*"` 时，`dmPolicy: "open"` 才允许公开私信；如果有限制性条目，则只有匹配的用户可以聊天。
- `dmPolicy: "disabled"` 阻止私信。
- 回复收件人绑定默认保持稳定的数字 `user_id`。`channels.synology-chat.dangerouslyAllowNameMatching: true` 是一种紧急兼容模式，它重新启用了可变的用户名/昵称查找以进行回复传递。
- 配对批准适用于：
...
出站文件 URL 必须使用 `http` 或 `https`OpenClaw，私有或被阻止的网络目标将在 OpenClaw 将 URL 转发给 NAS webhook 之前被拒绝。

## 多账户

`channels.synology-chat.accounts` 下支持多个 Synology Chat 账户。
每个账户可以覆盖令牌、传入 URL、webhook 路径、私信策略和限制。
私信会话按账户和用户隔离，因此两个不同 Synology 账户上相同的数字 `user_id` 不会共享记录状态。
为每个启用的账户指定一个不同的 `webhookPath`OpenClaw。OpenClaw 现在拒绝重复的完全相同的路径，并拒绝启动在多账户设置中仅继承共享 webhook 路径的命名账户。
如果您有意为命名账户保留旧式继承，请在该账户或 `channels.synology-chat` 上设置 `dangerouslyAllowInheritedWebhookPath: true`，但重复的完全相同的路径仍将被故障关闭方式拒绝。最好使用显式的每账户路径。
...
- 请保密 `token`，如果泄露请轮换它。
- 保留 `allowInsecureSsl: false`，除非您明确信任自签名的本地 NAS 证书。
...
- 保持 `dangerouslyAllowInheritedWebhookPath` 关闭，除非您在多账户设置中明确接受共享路径路由的风险。
...
- `User not authorized`：
  - 发送方的数字 `user_id` 不在 `allowedUserIds` 中
