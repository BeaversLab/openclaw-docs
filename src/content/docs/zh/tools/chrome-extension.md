...
如果您想要 Chrome 官方的 DevTools MCP 附加流程，而不是 OpenClaw 扩展中继，请改用 `existing-session` 浏览器配置文件。请参阅 [Browser](/en/tools/browser#chrome-existing-session-via-mcp)。有关 Chrome 自己的设置文档，请参阅 [Chrome for Developers: Use Chrome DevTools MCP with your browser 会话](https://developer.chrome.com/blog/chrome-devtools-mcp-debug-your-browser-session) 和 [Chrome DevTools MCP README](https://github.com/ChromeDevTools/chrome-devtools-mcp)。
