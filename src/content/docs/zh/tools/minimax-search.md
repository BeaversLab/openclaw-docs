...
Coding Plan search API 实现。它返回包含标题、URL、
摘要和相关查询的结构化搜索结果。
...
已指向 coding-plan 令牌时，仍会读取它作为兼容性回退方案。
...
如果未设置 `plugins.entries.minimax.config.webSearch.region`，OpenClaw 将
按以下顺序解析区域：
...
这意味着中国区新手引导或 `MINIMAX_API_HOST=https://api.minimaxi.com/...`
会自动将 MiniMax 搜索保留在中国主机上。

即使您通过 OAuth `minimax-portal` 路径认证了 MiniMax，
...
- [Web 搜索概述](/zh/tools/web) -- 所有提供商和自动检测
