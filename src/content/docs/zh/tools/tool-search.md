---
summary: "OpenClaw工具搜索：通过搜索、描述和调用将大型 OpenClaw 工具目录紧凑化"
title: "工具搜索"
read_when:
...
工具搜索是一项实验性的 OpenClaw 代理运行时功能。它为代理提供了一种
紧凑的方式来发现和调用大型工具目录。当运行中有许多可用工具，但模型可能只需要其中少数几个时，
这非常有用。

本页面记录了 OpenClaw 工具搜索。它不是 Codex 原生工具
搜索或 dynamic-tools 表面。Codex 原生代码模式、工具搜索、延迟
dynamic 工具和嵌套工具调用是稳定的 Codex harness 表面，不
依赖于 OpenClaw`tools.toolSearch`。

当为 OpenClaw 运行启用时，模型默认会收到一个 OpenClaw`tool_search_code` 工具。
该工具在隔离的 Node 子进程中运行一个简短的 JavaScript 主体，并带有
`openclaw.tools` 桥接：
...
在规划阶段，OpenClaw 嵌入式运行程序将为运行构建有效目录：
...
这两种模式都是实验性的。对于小型 OpenClaw 工具目录，首选直接工具公开，
对于 Codex harness 运行，首选 Codex 原生稳定表面。
...
网关 E2E 运行程序通过 OpenClaw 运行时验证这两个路径：
