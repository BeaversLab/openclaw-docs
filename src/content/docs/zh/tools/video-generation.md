...
- 当请求包含一张或多张参考图像时使用 `imageToVideo`
...
代理会自动调用 `video_generate`。无需将工具加入白名单。
...
重复预防：如果当前会话的视频任务已经是 `queued` 或 `running`，`video_generate` 将返回现有任务状态而不是启动新任务。使用 `action: "status"` 进行明确检查而不触发新的生成。
...
某些提供商接受额外或替代的 API 密钥环境变量。有关详细信息，请参阅各个 [提供商页面](#related)。
...
| Google   | 是         | 是             | 是             | `generate`、`imageToVideo`；跳过共享的 `videoToVideo`，因为当前基于缓冲区的 Gemini/Veo 扫描不接受该输入       |
| MiniMax  | 是         | 是             | 否             | `generate`、`imageToVideo`                                                                                    |
| OpenAI   | 是         | 是             | 是             | `generate`、`imageToVideo`；跳过共享的 `videoToVideo`，因为此组织/输入路径目前需要提供商端的重绘/混合访问权限 |
| Qwen     | 是         | 是             | 是             | `generate`、`imageToVideo`；跳过 `videoToVideo`，因为此提供商需要远程 `http(s)` 视频地址                      |
| Runway   | 是         | 是             | 是             | `generate`、`imageToVideo`；仅当所选模型为 `runway/gen4_aleph` 时才运行 `videoToVideo`                        |
| Together | 是         | 是             | 否             | `generate`、`imageToVideo`                                                                                    |
| Vydra    | 是         | 是             | 否             | `generate`；跳过共享的 `imageToVideo`，因为捆绑的 `veo3` 仅支持文本，而捆绑的 `kling` 需要远程图片地址        |
...
| 参数     | 类型   | 描述                                              |
| -------- | ------ | ------------------------------------------------- |
| `prompt` | string | 待生成视频的文本描述（`action: "generate"` 必需） |
...
工具结果会报告应用的设置。当 OpenClaw 在提供商备用期间重新映射持续时间或几何形状时，返回的 `durationSeconds`、`size`、`aspectRatio` 和 `resolution` 值反映的是提交的内容，而 `details.normalization` 则捕获了从请求到应用的转换过程。
...
| 提供商   | 说明                                                                                                                   |
| -------- | ---------------------------------------------------------------------------------------------------------------------- |
| 阿里云   | 使用 DashScope/Model Studio 异步端点。参考图片和视频必须是远程 `http(s)` URL。                                         |
| BytePlus | 仅支持单张图片参考。                                                                                                   |
| ComfyUI  | 工作流驱动的本地或云端执行。通过配置的图表支持文生视频和图生视频。                                                     |
| fal      | 对长时间运行的任务使用队列支持流。仅支持单张图片参考。                                                                 |
| Google   | 使用 Gemini/Veo。支持一张图片或一个视频参考。                                                                          |
| MiniMax  | 仅支持单张图片参考。                                                                                                   |
| OpenAI   | 仅 `size` 覆盖项会被转发。其他样式覆盖项（`aspectRatio`、`resolution`、`audio`、`watermark`）将被忽略并发出警告。      |
| Qwen     | 与阿里云使用相同的 DashScope 后端。参考输入必须是远程 `http(s)` URL；本地文件会被直接拒绝。                            |
| Runway   | 通过数据 URI 支持本地文件。视频生视频需要 `runway/gen4_aleph`。纯文本运行会暴露 `16:9` 和 `9:16` 宽高比。              |
| Together | 仅支持单张图片参考。                                                                                                   |
| Vydra    | 直接使用 `https://www.vydra.ai/api/v1` 以避免丢失身份验证的重定向。`veo3` 仅打包为文生视频；`kling` 需要远程图片 URL。 |
| xAI      | 支持文生视频、图生视频以及远程视频编辑/扩展流程。                                                                      |

## 提供商能力模式

共享的视频生成合同现在允许提供商声明特定模式的功能，而不仅仅是平面的聚合限制。新的提供商实现应优先使用显式模式块：
...
诸如 `maxInputImages` 和 `maxInputVideos` 之类的平面聚合字段不足以宣传转换模式支持。提供商应显式声明 `generate`、`imageToVideo` 和 `videoToVideo`，以便实时测试、合同测试和共享 `video_generate` 工具可以确定性地验证模式支持。
...
此实时文件从 `~/.profile` 加载缺失的提供商环境变量，默认优先使用 live/env API 密钥而非存储的身份验证配置文件，并运行其可以安全地通过本地媒体执行的声明模式：

- 针对扫描中的每个提供商 `generate`
- 当 `capabilities.imageToVideo.enabled` 时 `imageToVideo`
- 当 `capabilities.videoToVideo.enabled` 且提供商/模型
  在共享扫描中接受基于缓冲区的本地视频输入时 `videoToVideo`

如今，共享的 `videoToVideo` 实时通道涵盖：

- 仅当您选择 `runway/gen4_aleph` 时 `runway`
