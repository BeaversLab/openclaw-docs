...
<ParamField path="timeoutMs" type="number">可选的提供商操作超时（毫秒）。</ParamField>
<ParamField path="providerOptions" type="object">
  提供商特定选项，作为 JSON 对象（例如 `{"seed": 42, "draft": true}`）。
  声明了类型化架构的提供商会验证键和类型；在回退期间，未知的
  键或类型不匹配将跳过该候选项。没有声明架构的提供商将按原样接收选项。
  运行 `video_generate action=list` 以查看每个提供商接受的内容。
</ParamField>

<Note>并非所有提供商都支持所有参数。OpenClaw 会将持续时间标准化为 提供商支持的最接近值，并在回退提供商公开不同的 控制面时，重新映射已转换的几何提示（例如尺寸到宽高比）。 真正不支持的覆盖项将被尽力忽略，并在工具结果中作为警告报告。 硬性能力限制（例如参考输入过多）会在提交前失败。 工具结果会报告已应用的设置；`details.normalization` 捕获任何 从请求值到应用值的转换。</Note>
...
- 任何图像参考 → `imageToVideo`
- 任何视频参考 → `videoToVideo`
- 参考音频输入**不会**改变解析出的模式；它们应用在
  图像/视频参考所选择的任何模式之上，并且仅与声明了
  `maxInputAudios` 的提供商一起工作。
...
- 当请求包含音频引用时，未声明 `maxInputAudios`（或 `0`）的活动候选将被跳过；尝试下一个候选。
- 活动候选的 `maxDurationSeconds` 低于请求的 `durationSeconds` 且未声明 `supportedDurationSeconds` 列表 → 被跳过。
- 请求包含 `providerOptions` 且活动候选显式声明了类型化 `providerOptions` 架构 → 如果提供的键不在架构中或值类型不匹配，则跳过。未声明架构的提供商按原样接收选项（向后兼容的透传）。提供商可以通过声明空架构（`capabilities.providerOptions: {}`）来选择退出所有提供商选项，这会导致与类型不匹配相同的跳过结果。

请求中的第一个跳过原因记录在 `warn`，以便操作员了解其主要提供商何时被跳过；后续跳过记录在 `debug`，以保持冗长的回退链安静。如果每个候选都被跳过，则聚合错误包含每个候选的跳过原因。
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus (1.0)">
    ...
    T2V 模型（`*-t2v-*`）不接受图像输入；I2V 模型和通用 `*-pro-*` 模型支持单张参考图像（第一帧）。按位置传递图像或设置 `role: "first_frame"`。
    当提供图像时，T2V 模型 ID 会自动切换到相应的 I2V 变体。
  ...
  <Accordion title="BytePlus Seedance 1.5">
    ...
    `aspectRatio: "adaptive"` 自动从输入图像检测纵横比。
  ...
  <Accordion title="BytePlus Seedance 2.0">
    ...
    `aspectRatio: "adaptive"` 自动从输入图像检测比例。
    `audio: true` 映射到 `generate_audio`。`providerOptions.seed`
    （数字）会被转发。
  ...
  <Accordion title="MiniMaxMiniMax"MiniMax>
    ...
    分辨率；诸如 `720P` 的请求会在提交前被标准化为最接近的
    支持值。
  </Accordion>
  <Accordion title="OpenAIOpenAI">
    仅 `size` 覆盖会被转发。其他样式覆盖
  ...
  <Accordion title="OpenRouterOpenRouter"OpenRouter>
    ...
    任务，轮询 `polling_url`，并下载 `unsigned_urls` 或
    文档记录的任务内容端点。内置的 `google/veo-3.1-fast` 默认
    设置支持 4/6/8 秒时长、`720P`/`1080P` 分辨率以及
    `16:9`/`9:16` 纵横比。
  </Accordion>
  <Accordion title="QwenQwen">
    使用与阿里巴巴相同的 DashScope 后端。参考输入必须是远程
    `http(s)` URL；本地文件会被直接拒绝。
  </Accordion>
  <Accordion title="Runway">
    通过 data URI 支持本地文件。视频转视频需要
    `runway/gen4_aleph`。纯文本运行提供 `16:9` 和 `9:16` 纵横
    比。
  </Accordion>
  ...
  <Accordion title="Vydra">
    直接使用 `https://www.vydra.ai/api/v1` 以避免导致身份验证丢失的
    重定向。`veo3` 仅作为文本生成视频捆绑；`kling` 需要
    远程图像 URL。
  </Accordion>
  <Accordion title="xAI">
    支持文本生成视频、单首帧图像生成视频、通过 xAI `reference_images` 输入最多 7
    个 `reference_image`，以及远程
    视频编辑/扩展流程。
  </Accordion>
</AccordionGroup>
...
诸如 `maxInputImages` 和 `maxInputVideos` 之类的单一总体字段
**不**足以表明对转换模式的支持。提供商应
显式声明 `generate`、`imageToVideo` 和 `videoToVideo`，以便实时
测试、合约测试和共享 `video_generate` 工具能够
确定性验证模式支持。
...
此实时文件从 `~/.profile` 加载缺失的提供商环境变量，默认情况下
优先使用 live/env API 密钥而非存储的身份验证配置文件，并默认
运行发布安全的冒烟测试：
