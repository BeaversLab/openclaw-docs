...
如需进行设置、配置和故障排除，请参阅 [Browser](/zh/tools/browser)。
...
- `--efficient`（或 `--mode efficient`）：紧凑型角色快照预设。设置 `browser.snapshotDefaults.mode: "efficient"` 可将其设为默认值（请参阅 [Gateway(网关) configuration](/zh/gateway/configuration-reference#browser)）。
