...
有关设置、配置和故障排除，请参阅 [浏览器](/zh/tools/browser)。
...
`OPENCLAW_HOME_VOLUME` 或绑定挂载进行持久化。OpenClaw 会自动检测 Linux 上持久化的
...
- `upload` 和 `dialog` 是**准备**（arming）调用；在触发选择器/对话框的点击/按压操作之前运行它们。如果某个操作打开了模态框，该操作响应将包含 `blockedByDialog` 和 `browserState.dialogs.pending`；传递该 `dialogId` 以直接响应。在 OpenClaw 之外处理的对话框将显示在 `browserState.dialogs.recent` 下。
- `click`/`type`/etc 需要来自 `snapshot` 的 `ref`（数字 `12`、角色引用 `e12` 或可操作的 ARIA 引用 `ax12`）。出于意图，操作不支持 CSS 选择器。当可见视口位置是唯一可靠的目标时，请使用 `click-coords`。
- 下载和跟踪路径受限于 OpenClaw 临时根目录：`/tmp/openclaw{,/downloads}`（回退：`${os.tmpdir()}/openclaw/...`）。
- `upload` 接受来自 OpenClaw 临时上传根目录和
  OpenClaw 托管的入站媒体的文件。托管入站媒体可以引用为
  `media://inbound/<id>`、相对于沙箱的 `media/inbound/<id>` 或托管入站媒体目录内的已解析
  路径。嵌套媒体引用、
  遍历、符号链接、硬链接和任意本地路径仍将被拒绝。
- `upload` 也可以通过 `--input-ref` 或 `--element` 直接设置文件输入。

当 OpenClaw 能够证明替换选项卡时（例如相同的 URL，或者表单提交后单个旧选项卡变为
单个新选项卡），稳定的选项卡 ID 和标签在 Chromium 原始目标替换中得以保留。原始目标 ID 仍然是不稳定的；在脚本中优先使用
来自 `tabs` 的 `suggestedTargetId`。

快照标志一览：

- `--format ai`（Playwright 的默认值）：带有数字引用（`aria-ref="<n>"`）的 AI 快照。
- `--format aria`：带有 `axN`OpenClaw 引用的辅助功能树。当 Playwright 可用时，OpenClaw 会将引用与后端 DOM ID 绑定到实时页面，以便后续操作可以使用它们；否则，将输出视为仅用于检查。
- `--efficient`（或 `--mode efficient`）：紧凑的角色快照预设。设置 `browser.snapshotDefaults.mode: "efficient"`Gateway(网关) 可将其设为默认值（参见 [Gateway(网关) configuration](/zh/gateway/configuration-reference#browser)）。
- `--interactive`、`--compact`、`--depth`、`--selector` 强制使用带有 `ref=e12` 引用的角色快照。`--frame "<iframe>"` 将角色快照范围限定在 iframe 内。
- `--labels` 添加一张仅包含视口的屏幕截图，上面叠加了引用标签，并打印保存的路径。
...
- **角色快照（如 `e12` 的角色引用）**：`openclaw browser snapshot --interactive`（或 `--compact`、`--depth`、`--selector`、`--frame`）
  ...
  - 添加 `--labels` 以包含一张视口屏幕截图，上面叠加了 `e12` 标签。
  - 当链接文本有歧义且代理需要具体的导航目标时，请添加 `--urls`。
...
- 如果 Playwright 不可用，ARIA 快照对于检查仍然有用，但引用可能无法执行。当您需要操作引用时，请使用 `--format ai` 或 `--interactive` 重新生成快照。
- raw-CDP 回退路径的 Docker 验证：`pnpm test:docker:browser-cdp-snapshot` 通过 CDP 启动 Chromium，运行 `browser doctor --deep`，并验证角色快照是否包含链接 URL、光标提升的可点击元素以及 iframe 元数据。

引用行为：

- 引用在导航之间**不稳定**；如果操作失败，请重新运行 `snapshot` 并使用新的引用。
- 当 `/act` 能够证明替换选项卡时，它会在操作触发的替换后返回当前原始 `targetId`。对于后续命令，请继续使用稳定的选项卡 ID/标签。
- 如果角色快照是使用 `--frame` 获取的，则角色引用的作用域限定在该 iframe，直到下一个角色快照。
- 未知或过时的 `axN` 引用会快速失败，而不是回退到 Playwright 的 `aria-ref` 选择器。发生这种情况时，请在同一选项卡上运行新的快照。
...
3. 如果仍然失败：`openclaw browser highlight <ref>` 以查看 Playwright 的目标是什么
4. 如果页面表现异常：
   - `openclaw browser errors --clear`
   - `openclaw browser requests --filter api --clear`
5. 对于深度调试：录制一个追踪（trace）：
   - `openclaw browser trace start`
   - 重现该问题
...
- `browser act kind=evaluate` / `openclaw browser evaluate` 和 `wait --fn`
  在页面上下文中执行任意 JavaScript。提示注入可能会引导此行为。
  如果不需要，请使用 `browser.evaluateEnabled=false` 将其禁用。
- 当页面端函数可能需要比默认评估超时更长的时间时，
  使用 `openclaw browser evaluate --timeout-ms <ms>`。
- 有关登录和反机器人注意事项（X/Twitter 等），请参阅 [Browser login + X/Twitter posting](/zh/tools/browser-login)。
- 请保持 Gateway(网关)/node 主机私密（仅限环回或 tailnet）。
...
- [浏览器](/zh/tools/browser) - 概述、配置、配置文件、安全性
- [浏览器登录](/zh/tools/browser-login) - 签录到网站
- [浏览器 Linux 故障排除](Linux/en/tools/browser-linux-troubleshooting)
- [浏览器 WSL2 故障排除](WSL2/en/tools/browser-wsl2-windows-remote-cdp-troubleshooting)
