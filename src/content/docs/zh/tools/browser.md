...
默认的 `browser` 工具现在是一个默认启用的内置插件。这意味着您可以禁用或替换它，而无需删除 OpenClaw 插件系统的其余部分：
...
在安装另一个提供相同 `browser` 工具名称的插件之前，请先禁用内置插件。默认的浏览器体验需要同时具备：

- `plugins.entries.browser.enabled` 未禁用
- `browser.enabled=true`

如果您仅关闭插件，内置的浏览器 CLI (`openclaw browser`)、Gateway 方法 (`browser.request`)、代理工具和默认的浏览器控制服务将一起消失。您的 `browser.*` 配置将保持完整，供替换插件重复使用。

内置的浏览器插件现在也拥有浏览器运行时实现。Core 仅保留共享的 Plugin SDK 助手以及用于较旧的内部导入路径的兼容性重新导出。实际上，删除或替换 `extensions/browser` 会移除浏览器功能集，而不是留下第二个 Core 拥有的运行时。

浏览器配置更改仍然需要重启 Gateway，以便内置插件可以使用新设置重新注册其浏览器服务。
...
- 当现有的登录会话很重要且用户在计算机旁边以点击/批准任何附加提示时，首选 `profile="user"`。
...
- 浏览器控制服务绑定到从 `gateway.port` 派生的端口上的环回地址（默认值：`18791`，即 gateway + 2）。
- 如果您覆盖 Gateway(网关) 端口（`gateway.port` 或 `OPENCLAW_GATEWAY_PORT`），派生的浏览器端口将进行偏移以保持在同一“系列”中。
- `cdpUrl` 在未设置时默认为托管的本地 CDP 端口。
- `remoteCdpTimeoutMs` 适用于远程（非环回）CDP 可达性检查。
- `remoteCdpHandshakeTimeoutMs` 适用于远程 CDP WebSocket 可达性检查。
- 浏览器导航/打开选项卡在导航之前会受到 SSRF 保护，并且在导航之后会对最终的 `http(s)` URL 进行尽力的重新检查。
...
- `attachOnly: true` 意味着“从不启动本地浏览器；仅当它已在运行时才附加”。
- `color` + 每个配置文件的 `color` 会为浏览器 UI 着色，以便您可以看到哪个配置文件处于活动状态。
- 默认配置文件是 `openclaw`（由 OpenClaw 托管的独立浏览器）。使用 `defaultProfile: "user"` 选择加入已登录用户浏览器。
- 自动检测顺序：如果基于 Chromium，则为系统默认浏览器；否则为 Chrome → Brave → Edge → Chromium → Chrome Canary。
- 本地 `openclaw` 配置文件会自动分配 `cdpPort`/`cdpUrl` — 仅需为远程 CDP 设置这些。
...
- **本地控制（默认）：** Gateway(网关) 启动回环控制服务并可以启动本地浏览器。
...
- 如果您设置了 `nodeHost.browserProxy.allowProfiles`，OpenClaw 会将其视为最小权限边界：仅允许定位列入白名单的配置文件，并且持久化配置文件的创建/删除路由将在代理表面上被阻止。
- 如果您不想要它，请禁用：
...
CDP 连接 URL。OpenClaw 可以使用任何一种形式，但对于远程浏览器配置文件，最简单的选项是使用 Browserless 连接文档中的直接 WebSocket URL。
...
- 如果 Browserless 提供了 HTTPS 基础 URL，您可以将其转换为
  `wss://` 以进行直接的 CDP 连接，或者保留 HTTPS URL 并让 OpenClaw
  发现 `/json/version`。

## 直接的 WebSocket CDP 提供商
...
- **WebSocket 端点** (`ws://` / `wss://`) — OpenClaw 直接连接，
  跳过 `/json/version`。将此用于诸如
  [Browserless](https://browserless.io)、
  [Browserbase](https://www.browserbase.com) 之类的服务，或任何向您提供
  WebSocket URL 的提供商。
...
- 浏览器控制仅限本地回环；访问流经 Gateway(网关) 的认证或节点配对。
- 如果启用了浏览器控制且未配置认证，OpenClaw 会在启动时自动生成 `gateway.auth.token` 并将其持久化到配置中。
...
- **existing 会话**：通过 Chrome DevTools MCP 自动连接你现有的 Chrome 配置文件
...
## 通过 Chrome DevTools MCP 实现现有会话

OpenClaw 还可以通过官方的 Chrome DevTools MCP 服务器附加到正在运行的基于 Chromium 的浏览器配置文件。这将重用该浏览器配置文件中已打开的标签页和登录状态。
...
- 内置的 `user` 配置文件使用 Chrome MCP 自动连接，它针对的是
...
- `openclaw doctor` 会迁移旧的基于扩展的浏览器配置，并检查
  是否在本地安装了 Chrome 以用于默认自动连接配置文件，但它无法
  为您启用浏览器端的远程调试
...
- OpenClaw 不会为此驱动程序启动浏览器；它仅附加到
  现有会话。
- OpenClaw 在此处使用官方的 Chrome DevTools MCP `--autoConnect` 流程。如果设置了 `userDataDir`，OpenClaw 会将其传递给目标 Chromium 用户数据目录。
- 现有会话屏幕截图支持从快照中进行页面捕获和 `--ref` 元素捕获，但不支持 CSS `--element` 选择器。
- 现有会话 `wait --url` 支持与其他浏览器驱动程序类似的精确、子字符串和 glob 模式。`wait --load networkidle` 尚不支持。
- 某些功能仍需要托管浏览器路径，例如 PDF 导出和下载拦截。
- 现有会话是主机本地的。如果 Chrome 位于不同的机器或不同的网络命名空间上，请使用远程 CDP 或节点主机。
...
仅适用于本地集成，Gateway(网关) 会公开一个小型环回 HTTP API：
...
某些功能（导航/操作/AI 快照/角色快照、元素截图、PDF）需要 Playwright。如果未安装 Playwright，这些端点将返回明确的 501 错误。ARIA 快照和基本截图在 OpenClaw 管理的 Chrome 上仍然可用。

如果您看到 `Playwright is not available in this gateway build`，请安装完整的 Playwright 包（而不是 `playwright-core`）并重启 Gateway(网关)，或者重新安装带有浏览器支持的 OpenClaw。
...
要持久化浏览器下载，请设置 `PLAYWRIGHT_BROWSERS_PATH`（例如，`/home/node/.cache/ms-playwright`）并确保通过 `OPENCLAW_HOME_VOLUME` 或绑定挂载持久化 `/home/node`。请参阅 [Docker](/en/install/docker)。
...
- 下载和追踪输出路径被限制在 OpenClaw 临时根目录下：
  - 追踪： `/tmp/openclaw` （后备： `${os.tmpdir()}/openclaw`）
...
- `snapshot`：
  ...
  - `--efficient` （或 `--mode efficient`）：紧凑角色快照预设（交互 + 紧凑 + 深度 + 更低的 maxChars）。
  - 配置默认值（仅限工具/ CLI）：设置 `browser.snapshotDefaults.mode: "efficient"` 以便在调用方未传递模式时使用高效快照（参见 [ Gateway(网关) 配置](/en/gateway/configuration-reference#browser)）。
  ...
  - `--labels` 添加一个仅限视口的屏幕截图，并在上面叠加引用标签（打印 `MEDIA:<path>`）。
...
- **角色快照（类似 `e12` 的角色引用）**：`openclaw browser snapshot --interactive`（或 `--compact`，`--depth`，`--selector`，`--frame`）
  ...
  - 在内部，引用通过 `getByRole(...)` 解析（对于重复项，额外使用 `nth()`）。
  - 添加 `--labels` 以包含带有叠加 `e12` 标签的视口屏幕截图。

引用行为：

- 引用在导航之间**不稳定**；如果出现故障，请重新运行 `snapshot` 并使用新的引用。
- 如果角色快照是使用 `--frame` 拍摄的，则角色引用的作用域仅限于该 iframe，直到拍摄下一个角色快照。
...
JSON 格式的角色快照包含 `refs` 以及一个小型的 `stats` 块（行/字符/引用/交互），以便工具可以推断负载大小和密度。
...
- `browser act kind=evaluate` / `openclaw browser evaluate` 和 `wait --fn`
  在页面上下文中执行任意 JavaScript。提示注入可能会操纵此行为。如果不需要，请使用 `browser.evaluateEnabled=false` 将其禁用。
