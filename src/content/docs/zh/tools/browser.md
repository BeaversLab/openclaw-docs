...
如果完全没有 `openclaw browser`，或者代理说浏览器工具不可用，请跳转到[缺失浏览器命令或工具](/zh/tools/browser#missing-browser-command-or-tool)。
...
[Browserless](https://browserless.io) 是一项托管的 Chromium 服务，通过 HTTPS 和 WebSocket 公开 CDP 连接 URL。OpenClaw 可以使用任一种形式，但对于远程浏览器配置文件，最简单的选项是来自 Browserless 连接文档的直接 WebSocket URL。
...
- **裸 WebSocket 根路径** —— `ws://host[:port]` 或 `wss://host[:port]` 且不带
  ...
  [Browserbase](https://www.browserbase.com)）。OpenClaw 会首先尝试 HTTP
  `/json/version` 发现（将协议规范化为 `http`/`https`）；
  如果发现返回了 `webSocketDebuggerUrl`，则使用它，否则 OpenClaw
  会回退到在裸根路径进行直接的 WebSocket 握手。这涵盖了
  风格的远程调试端口和仅支持 WebSocket 的提供商。
...
如果您看到 `Playwright is not available in this gateway build`，请修复
捆绑的浏览器插件运行时依赖项，以便安装 `playwright-core`，
然后重启网关。对于打包安装，请运行 `openclaw doctor --fix`。
对于 Docker，还需安装 Chromium 浏览器二进制文件，如下所示。
...
- 对于仅附加和远程 CDP 配置文件，`openclaw browser stop` 仍然是测试后正确的清理命令。它会关闭活动的控制会话
  并清除临时的模拟覆盖，而不是终止底层的浏览器。
...
- `snapshot`:
  ...
  - Config default (工具/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway(网关) configuration](/zh/gateway/configuration-reference#browser)).
  - 角色快照选项（`--interactive`、`--compact`、`--depth`、`--selector`）强制使用基于角色的快照，并包含诸如 `ref=e12` 的引用。
  - `--frame "<iframe selector>"` 将角色快照范围限定在 iframe 内（与诸如 `e12` 的角色引用配对使用）。
...
- **角色快照（角色引用，如 `e12`）**：`openclaw browser snapshot --interactive`（或 `--compact`、`--depth`、`--selector`、`--frame`）
  - 输出：一个基于角色的列表/树，其中包含 `[ref=e12]`（以及可选的 `[nth=1]`）。
  - 操作：`openclaw browser click e12`、`openclaw browser highlight e12`。
  - 在内部，引用通过 `getByRole(...)` 解析（针对重复项使用 `nth()`）。
  - 添加 `--labels` 以包含带有叠加 `e12` 标签的视口截图。

Ref 行为：

- 引用在导航过程中**不稳定**；如果操作失败，请重新运行 `snapshot` 并使用新的引用。
- 如果角色快照是使用 `--frame` 拍摄的，则角色引用将限定在该 iframe 范围内，直到拍摄下一个角色快照。
...
JSON 中的角色快照包含 `refs` 加上一个小的 `stats` 块（行/字符/引用/交互），以便工具可以推断负载大小和密度。
...
- Headers: `set headers --headers-json '{"X-Debug":"1"}'` (传统的 `set headers --json '{"X-Debug":"1"}'` 仍受支持)
...
- `browser act kind=evaluate` / `openclaw browser evaluate` 和 `wait --fn`
  在页面上下文中执行任意 JavaScript。提示注入可能会影响
  此功能。如果不需要，请使用 `browser.evaluateEnabled=false` 将其禁用。
- 有关登录和反机器人注意事项（X/Twitter 等），请参阅 [浏览器登录 + X/Twitter 发布](/zh/tools/browser-login)。
...
有关 WSL2 Gateway(网关) + Windows Chrome 分宿主机设置，请参阅
...
- 导航 SSRF 阻止：
  - `open`、`navigate`、快照或打开标签页流程因浏览器/网络策略错误而失败，而 `start` 和 `tabs` 仍然工作正常
...
- 如果 `start` 成功但 `tabs` 失败，则控制平面仍然不健康。请将此视为 CDP 可达性问题，而不是页面导航问题。
- 如果 `start` 和 `tabs` 成功，但 `open` 或 `navigate` 失败，则浏览器控制平面已启动，故障出在导航策略或目标页面上。
- 如果 `start`、`tabs` 和 `open` 全部成功，则基本的托管浏览器控制路径是健康的。

重要行为细节：

- 即使您没有配置 `browser.ssrfPolicy`，浏览器配置默认也会使用一个失效关闭（fail-closed）的 SSRF 策略对象。
- 对于本地回环 `openclaw` 托管配置文件，CDP 健康检查有意跳过 OpenClaw 自身本地控制平面的浏览器 SSRF 可达性强制执行。
- 导航保护是分开的。成功的 `start` 或 `tabs` 结果并不表示允许后续的 `open` 或 `navigate` 目标。

安全指南：

- 默认情况下**不要**放宽浏览器 SSRF 策略。
- 优先使用 `hostnameAllowlist` 或 `allowedHostnames` 等窄主机例外，而不是广泛的私有网络访问。
- 仅在经过审查且需要私有网络浏览器访问的故意受信任环境中使用 `dangerouslyAllowPrivateNetwork: true`。
...
- `start` 失败并报错 `not reachable after start`
- `tabs` 也失败或无法运行
