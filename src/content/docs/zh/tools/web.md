...
<Info>`web_search` 是一个轻量级 HTTP 工具，而非浏览器自动化工具。对于 重度依赖 JS 的网站或登录操作，请使用 [Web Browser](/zh/tools/browser)。对于 获取特定 URL，请使用 [Web Fetch](/zh/tools/web-fetch)。</Info>
...
当启用 OpenClaw 网络搜索且未固定托管提供商时，直连 OpenAI Responses 模型会自动使用 OpenAI 托管的 `web_search` 工具。这是捆绑 OpenAI 插件中由提供商拥有的行为，仅适用于原生 OpenAI API 流量，不适用于 OpenAI 兼容的代理基础 URL 或 Azure 路由。将 `tools.web.search.provider` 设置为另一个提供商（例如 `brave`）以为 OpenAI 模型保留托管的 `web_search` 工具，或者设置 `tools.web.search.enabled: false` 以同时禁用托管搜索和原生 OpenAI 搜索。

## 原生 Codex 网络搜索

具备 Codex 功能的模型可以选择使用提供商原生的 Responses `web_search` 工具，而不是 OpenClaw 的托管 `web_search` 函数。

- 在 `tools.web.search.openaiCodex` 下进行配置
- 它仅对支持 Codex 的模型（`openai-codex/*` 或使用 `api: "openai-codex-responses"` 的提供商）激活
...
如果未设置 `provider`，OpenClaw 将按此顺序检查提供商，并使用第一个准备就绪的提供商：
...
10. **DuckDuckGo** -- 免密钥 HTML 回退，无需帐户或 API 密钥（优先级 100）
11. **Ollama Web Search** -- 通过您配置的 Ollama 主机进行免密钥回退；要求 Ollama 可访问并使用 `ollama signin` 登录，如果主机需要，可以重用 Ollama 提供商的 Bearer 认证（优先级 110）
12. **SearXNG** -- `SEARXNG_BASE_URL` 或 `plugins.entries.searxng.config.webSearch.baseUrl`（优先级 200）

如果未检测到提供商，它将回退到 Brave（您将收到缺少密钥的错误提示，要求您配置一个）。

<Note>
  所有提供商密钥字段均支持 SecretRef 对象。无论提供商是通过 `tools.web.search.provider` 显式选择还是通过自动检测选择，`plugins.entries.<plugin>.config.webSearch.apiKey` 下的插件范围 SecretRefs 都会针对内置的 Exa、Firecrawl、Gemini、Grok、Kimi、Perplexity 和 Tavily 提供商进行解析。在自动检测模式下，OpenClaw 仅解析所选提供商的密钥——未选择的 SecretRefs 保持不活动状态，因此您可以配置多个提供商，而无需为未使用的提供商支付解析成本。
</Note>
...
`plugins.entries.<plugin>.config.webSearch.*` 下。请参阅提供商页面获取
示例。

`web_fetch` 回退提供商的选择是独立的：

- 通过 `tools.web.fetch.provider` 进行选择
- 或省略该字段，让 OpenClaw 从可用的凭证中自动检测第一个准备就绪的网页抓取提供商
- 目前捆绑的网页抓取提供商是 Firecrawl，在 `plugins.entries.firecrawl.config.webFetch.*` 下进行配置

当您在 `openclaw onboard` 或 `openclaw configure --section web` 期间选择 **Kimi** 时，OpenClaw 还可能询问：

- Moonshot API 地区（`https://api.moonshot.ai/v1` 或 `https://api.moonshot.cn/v1`）
- 默认的 Kimi 网络搜索模型（默认为 `kimi-k2.6`）

对于 `x_search`，配置 `plugins.entries.xai.config.xSearch.*`。它使用与
Grok 网络搜索相同的 `XAI_API_KEY` 回退机制。
传统的 `tools.web.x_search.*` 配置会由 `openclaw doctor --fix` 自动迁移。
当您在 `openclaw onboard` 或 `openclaw configure --section web` 期间选择 Grok 时，
OpenClaw 也可以提供使用相同密钥的可选 `x_search` 设置。
这是 Grok 路径内一个单独的后续步骤，不是一个单独的顶层
网络搜索提供商选择。如果您选择其他提供商，OpenClaw 不会
显示 `x_search` 提示。
...
<Tabs>
  ...
  <Tab title="环境变量">
    ...
    参见 [环境变量](/zh/help/faq#env-vars-and-env-loading)。
...
<Warning>
  并非所有参数都适用于所有提供商。Brave `llm-context` 模式 会拒绝 `ui_lang`、`freshness`、`date_after` 和 `date_before`。 Gemini、Grok 和 Kimi 返回一个带有引用的综合答案。它们 接受 `count` 以实现共享工具兼容性，但这不会改变 基于事实的答案形态。 当您使用 Sonar/Perplexity 兼容性路径（`plugins.entries.perplexity.config.webSearch.baseUrl` / `model` 或 `OPENROUTER_API_KEY`）时，OpenRouter 的行为相同。
  SearXNG 仅对受信任的专用网络或环回主机接受 `http://`； 公共 SearXNG 端点必须使用 `https://`。 Firecrawl 和 Tavily 仅通过 `web_search` 支持 `query` 和 `count` -- 请使用它们专用工具进行高级选项设置。
</Warning>

## x_search

`x_search` 使用 xAI 查询 X（前 Twitter）帖子，并返回带有引用的 AI 综合答案。它接受自然语言查询和可选的结构化过滤器。OpenClaw 仅在服务于此次工具调用的请求上启用内置的 xAI `x_search` 工具。

<Note>xAI 文档描述 `x_search` 支持关键词搜索、语义搜索、用户搜索和帖子串获取。对于每篇帖子的互动数据（如转发、回复、书签或浏览量），建议针对确切的帖子 URL 或状态 ID 进行定向查找。广泛的关键词搜索可能会找到正确的帖子，但返回的每篇帖子元数据可能不够完整。一个好的模式是：先定位帖子，然后运行第二个 `x_search` 查询，专注于该特定帖子。</Note>
...
- [Web Browser](/zh/tools/browser) -- 针对 JS 密集型网站的完整浏览器自动化
- [Grok Search](/zh/tools/grok-search) -- 将 Grok 作为 `web_search` 提供商
- [Ollama Web Search](/zh/tools/ollama-search) -- 通过您的 Ollama 主机进行免密网络搜索
