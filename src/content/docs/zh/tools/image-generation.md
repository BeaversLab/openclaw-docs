---
summary: "通过 image_generate 在 OpenAI、Google、fal、MiniMax、ComfyUI、OpenRouter、LiteLLM、xAI、Vydra 之间生成和编辑图像"
read_when:
...
<Steps>
  ...
  <Step title="询问代理">
    ...
    代理会自动调用 `image_generate`。无需将工具列入允许列表 — 当提供商可用时，它默认启用。
...
同一个 `image_generate` 工具处理文生图和参考图像
编辑。请使用 `image` 处理单个参考，或使用 `images` 处理多个参考。
提供商支持的输出提示（如 `quality`、`outputFormat` 和
`background`）在可用时会被转发，在提供商不支持时会被报告为忽略。
内置的透明背景支持是 OpenAI 特有的；如果其后端发射 alpha 通道，其他提供商可能仍保留 PNG alpha。

## 支持的提供商

| 提供商     | 默认模型                                | 编辑支持                   | 身份验证                                              |
| ---------- | --------------------------------------- | -------------------------- | ----------------------------------------------------- |
| ComfyUI    | `workflow`                              | 是（1 张图片，工作流配置） | `COMFY_API_KEY` 或 `COMFY_CLOUD_API_KEY` 用于云端     |
...
| 编辑 / 参考       | 1 张图片（工作流） | 1 张图片    | 最多 5 张图片 | 1 张图片（主体参考） | 最多 5 张图片 | —     | 最多 5 张图片 |
| 尺寸控制          | —                  | ✓           | ✓             | —                    | 最高 4K       | —     | —             |
| 宽高比            | —                  | ✓（仅生成） | ✓             | ✓                    | —             | —     | ✓             |
| 分辨率 (1K/2K/4K) | —                  | ✓           | ✓             | —                    | —             | —     | 1K, 2K        |
...
<ParamField path="model" type="string">
  提供商/模型覆盖（例如 `openai/gpt-image-2`）。使用 `openai/gpt-image-1.5` 获取透明的 OpenAI 背景。
</ParamField>
<ParamField path="image" type="string">
  用于编辑模式的单个参考图像路径或 URL。
</ParamField>
<ParamField path="images" type="string[]">
  用于编辑模式的多个参考图像（支持的提供商最多支持 5 个）。
</ParamField>
...
<Note>并非所有提供商都支持所有参数。当备用提供商支持接近的几何选项而不是所请求的确切选项时，OpenClaw 会在提交之前重新映射到最接近的受支持大小、纵横比或分辨率。 对于未声明支持的提供商，不支持的输出提示将被丢弃，并在工具结果中报告。工具结果报告已应用的设置；`details.normalization` 捕获从所请求到已应用的任何转换。</Note>
...
<AccordionGroup>
  <Accordion title="每次调用模型覆盖均为精确匹配">每次调用 `model` 覆盖仅尝试该提供商/模型，并且不会继续尝试配置的主要/备用或自动检测到的提供商。</Accordion>
  <Accordion title="自动检测具有身份验证感知能力">只有当 OpenClaw 实际上可以对该提供商进行身份验证时，该提供商默认值才会进入候选项列表。设置 `agents.defaults.mediaGenerationAutoProviderFallback: false` 以仅使用显式的 `model`、`primary` 和 `fallbacks` 条目。</Accordion>
  <Accordion title="超时">为缓慢的图像后端设置 `agents.defaults.imageGenerationModel.timeoutMs`。每次调用的 `timeoutMs` 工具参数会覆盖配置的默认值。</Accordion>
  <Accordion title="Inspect at runtime">使用 `action: "list"` 检查当前注册的提供商、 其默认模型和授权环境变量提示。</Accordion>
</AccordionGroup>

### 图像编辑

OpenAI、OpenRouter、Google、fal、MiniMax、ComfyUI 和 xAI 支持编辑
参考图像。传入参考图像路径或 URL：
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2（和 gpt-image-1.5）">
    OpenAI 图像生成默认为 `openai/gpt-image-2`。如果配置了 `openai-codex` OAuth 配置文件，OpenClaw 会重用 Codex 订阅聊天模型所使用的同一 OAuth 配置文件，并通过 Codex Responses 后端发送图像请求。旧的 Codex 基础 URL（例如 `https://chatgpt.com/backend-api`）会被规范化为 `https://chatgpt.com/backend-api/codex` 用于图像请求。对于该请求，OpenClaw **不会**静默回退到 `OPENAI_API_KEY`——要强制直接路由到 OpenAI Images API，请使用 API 密钥、自定义基础 URL 或 Azure 端点显式配置 `models.providers.openai`。

    仍然可以显式选择 `openai/gpt-image-1.5`、`openai/gpt-image-1` 和 `openai/gpt-image-1-mini` 模型。使用 `gpt-image-1.5` 以获得透明背景的 PNG/WebP 输出；当前的 `gpt-image-2` API 会拒绝 `background: "transparent"`。

    `gpt-image-2` 通过同一个 `image_generate` 工具支持文生图生成和参考图像编辑。OpenClaw 会将 `prompt`、`count`、`size`、`quality`、`outputFormat` 以及参考图像转发给 OpenAI。OpenAI **不会**直接接收 `aspectRatio` 或 `resolution`；尽可能的情况下，OpenClaw 会将这些映射为支持的 `size`，否则工具会将其报告为已忽略的覆盖参数。
    ...
    `openai.background` 接受 `transparent`、`opaque` 或 `auto`；透明输出需要 `outputFormat` `png` 或 `webp` 以及支持透明度的 OpenAI 图像模型。OpenClaw 将默认 `gpt-image-2` 透明背景请求路由到 `gpt-image-1.5`。`openai.outputCompression` 适用于 JPEG/WebP 输出。

    顶层 `background` 提示是提供商无关的，当选择 OpenAI 提供商时，目前映射到同一个 OpenAI `background` 请求字段。未声明背景支持的提供商会在 `ignoredOverrides` 中返回它，而不是接收不支持的参数。

    要通过 Azure OpenAI 部署而不是 `api.openai.com` 路由 OpenAI 图像生成，请参阅
  ...
  <Accordion title="OpenRouter image models">
    ...
    Current built-in OpenRouter image 模型 shortcuts include
  ...
  <Accordion title="xAI grok-imagine-image">
    捆绑的 xAI 提供商针对仅提示词请求使用 `/v1/images/generations`，当存在 `image` 或 `images` 时使用 `/v1/images/edits`。

    - Models: `xai/grok-imagine-image`, `xai/grok-imagine-image-pro`
    - Count: 最多 4 张
    - References: 一个 `image` 或最多五个 `images`
    - Aspect ratios: `1:1`, `16:9`, `9:16`, `4:3`, `3:4`, `2:3`, `3:2`
    - Resolutions: `1K`, `2K`
    - Outputs: 作为 OpenClaw 托管的图像附件返回

    OpenClaw 故意不公开 xAI 原生的 `quality`、`mask`、
    `user` 或额外的仅原生纵横比，直到共享的跨提供商 `image_generate` 契约中存在这些控件。
...
`openclaw infer image edit`；`--openai-background` 仍然是
OpenAI 专用的别名。除 OpenAI 之外的捆绑提供商目前未声明
明确的背景控制，因此对它们而言，`background: "transparent"` 被报告为
已忽略。
