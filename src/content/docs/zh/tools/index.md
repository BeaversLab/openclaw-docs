...
<Steps>
  ...
  <Step title="Skills 教授智能体何时以及如何">
    Skill 是注入到系统提示词中的 markdown 文件 (`SKILL.md`)。
    Skills 为智能体提供上下文、约束以及有效使用工具的逐步指导。Skills 存在于您的工作区中、共享文件夹中，或打包在插件内。
  ...
  <Step title="Plugins 将所有内容打包在一起">
    插件是一个可以注册任意功能组合的包：
    channels、模型提供者、tools、skills、语音、图像生成等等。
    有些插件是 **核心**（随 OpenClaw 一起提供），其他是 **外部**
    （由社区发布在 npm 上）。
...
| `message`                               | 跨所有渠道发送消息                       | [Agent 发送](/en/tools/agent-send)   |
| `canvas`                                | 驱动节点 Canvas（演示、评估、快照）      |                                      |
| `nodes`                                 | 发现并定位配对的设备                     |                                      |
| `cron` / `gateway`                      | 管理定时任务、重启网关                   |                                      |
| `image` / `image_generate`              | 分析或生成图像                           |                                      |
| `sessions_*` / `agents_list`            | 会话管理，子代理                         | [子代理](/en/tools/subagents)        |

对于图像工作，请使用 `image` 进行分析，使用 `image_generate` 进行生成或编辑。如果您使用 `openai/*`、`google/*`、`fal/*` 或其他非默认图像提供商，请先配置该提供商的 auth/API 密钥。

### 插件提供的工具

插件可以注册额外的工具。一些示例：

- [Lobster](/en/tools/lobster) — 带有可恢复审批的类型化工作流运行时
...
通过配置中的 `tools.allow` / `tools.deny` 来控制代理可以调用哪些工具。
拒绝列表始终优先于允许列表。
...
每个代理的覆盖项：`agents.list[].tools.profile`。
