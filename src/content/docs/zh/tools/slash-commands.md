...
<AccordionGroup>
  <Accordion title="会话和运行">
    - `/new [model]` 启动一个新的会话；`/reset` 是重置别名。
    - 控制界面会拦截输入的 `/new` 以创建并切换到一个全新的仪表板会话，除非配置了 `session.dmScope: "main"` 且当前父级是代理的主会话；在这种情况下，`/new` 会就地重置主会话。输入的 `/reset`Gateway(网关) 仍然会执行 Gateway(网关) 的就地重置。
    ...
    - `/stop` 中止当前运行。
    - `/session idle <duration|off>` 和 `/session max-age <duration|off>` 管理线程绑定的过期。
    - `/export-session [path]` 将当前会话导出为 HTML。别名：`/export`。
    - `/export-trajectory [path]` 请求执行批准，然后为当前会话导出 JSONL [轨迹包](/zh/tools/trajectoryOpenClaw)。当您需要 OpenClaw 会话的提示、工具和记录时间轴时使用它。在群组聊天中，批准提示和导出结果将私下发送给所有者。别名：`/trajectory`。

  </Accordion>
  <Accordion title="模型和运行控制">
    - `/think <level|default>` 设置思考级别或清除会话覆盖。选项来自当前模型的提供商配置文件；常见级别包括 `off`、`minimal`、`low`、`medium` 和 `high`，以及自定义级别（如 `xhigh`、`adaptive`、`max`），或仅在支持的场合使用二进制 `on`。别名：`/thinking`、`/t`。
    ...
    - `/models [provider] [page] [limit=<n>|size=<n>|all]` 列出已配置/已授权可用的提供商或某个提供商的模型；添加 `all` 可浏览该提供商的完整目录。`agents.defaults.models` 中的 `provider/*` 条目使 `/model` 和 `/models` 仅显示这些提供商发现的模型。
    - `/queue <mode>` 管理队列行为（`steer`、旧版 `queue`、`followup`、`collect`、`steer-backlog`、`interrupt`）以及 `debounce:0.5s cap:25 drop:summarize` 等选项；`/queue default` 或 `/queue reset` 清除会话覆盖。参见 [Command queue](/zh/concepts/queue) 和 [Steering queue](/zh/concepts/queue-steering)。
    - `/steer <message>` 向当前会话的活动运行注入指导，独立于 `/queue` 模式。当会话空闲时，它不会启动新的运行。别名：`/tell`。参见 [Steer](/zh/tools/steer)。
  ...
  <Accordion title="设备发现和状态">
    ...
    - `/status` 显示执行/运行时状态、Gateway(网关) 和系统正常运行时间，以及在可用时显示提供商使用量/配额。
    - `/diagnostics [note]` 是仅所有者专用的支持报告流程，用于 Gateway(网关) 错误和 Codex 驱动程序运行。每次运行 `openclaw gateway diagnostics export --json` 之前，它都会请求明确的执行批准；不要使用允许所有规则批准诊断。批准后，它会发送一份可粘贴的报告，其中包含本地捆绑包路径、清单摘要、隐私说明和相关会话 ID。在群组聊天中，批准提示和报告将私下发送给所有者。当活动会话使用 OpenAI Codex 驱动程序时，相同的批准也会将相关的 Codex 反馈发送到 OpenAI 服务器，并且完成的回复将列出 OpenClaw 会话 ID、Codex 线程 ID 和 `codex resume <thread-id>` 命令。请参阅[诊断导出](/zh/gateway/diagnostics)。
  ...
  <Accordion title="Skills, allowlists, approvals">
    ...
    - `/approve <id> <decision>` 处理执行审批提示。
    - `/btw <question>` 提出旁注问题而不更改未来的 会话 上下文。别名：`/side`。参见 [BTW](/zh/tools/btw)。
  ...
  <Accordion title="Subagents and ACP">
    ...
    - `/subagents steer <id|#> <message>` 向正在运行的子代理发送导向。请参阅 [Steer](/zh/tools/steer)。

  </Accordion>
  <Accordion title="仅所有者写入和管理员">
    - `/config show|get|set|unset` 读取或写入 `openclaw.json`。仅所有者。需要 `commands.config: true`。
    - `/mcp show|get|set|unset` 读取或写入 OpenClaw 管理的 MCP 服务器配置，位于 `mcp.servers` 下。仅所有者。需要 `commands.mcp: true`。
    - `/plugins list|inspect|show|get|install|enable|disable` 检查或修改插件状态。`/plugin` 是别名。写入操作仅限所有者。需要 `commands.plugins: true`。
    - `/debug show|set|unset|reset` 管理仅限运行时的配置覆盖。仅所有者。需要 `commands.debug: true`。
  ...
  <Accordion title="语音、TTS、渠道控制">
    ...
    - `/bash <command>` 运行主机 shell 命令。仅限文本。别名：`! <command>`。需要 `commands.bash: true` 和 `tools.elevated` 白名单。
...
Dock 命令将当前会话的回复路由切换到另一个关联的渠道。请参阅 [Channel docking](/zh/concepts/channel-docking) 了解设置、示例和故障排除。
...
Dock 命令需要 `session.identityLinks`。源发送者和目标对等体必须位于同一身份组中，例如 `["telegram:123", "discord:456"]`。如果 ID 为 `123` 的 Telegram 用户发送 `/dock_discord`，OpenClaw 会将 `lastChannel: "discord"` 和 `lastTo: "456"` 存储在活动会话中。如果发送者未链接到 Discord 对等体，该命令将回复设置提示，而不是转入正常聊天。

对接仅更改活动会话路由。它不会创建渠道帐户、授予访问权限、绕过渠道允许列表，或将对话记录历史移动到另一个会话。使用 `/dock-telegram`、`/dock-slack`、`/dock-mattermost` 或其他生成的对接命令再次切换路由。
...
- `/card ...` 发送 LINE 富卡预设。参见 [LINE](/zh/channels/line)。
- `/codex status|models|threads|resume|compact|review|diagnostics|account|mcp|skills` 检查并控制捆绑的 Codex 应用服务器线束。参见 [Codex harness](/zh/plugins/codex-harness)。
- 仅限 QQBot 的命令：
  - `/bot-ping`
  - `/bot-version`
  - `/bot-help`
...
- 当技能/插件注册它们时，技能也可能显示为直接命令，如 `/prose`。
- 原生技能命令注册由 `commands.nativeSkills` 和 `channels.<provider>.commands.nativeSkills` 控制。
- 对于支持本地化描述的原生界面（包括 Discord），命令规范可以提供 `descriptionLocalizations`。

<AccordionGroup>
  <Accordion title="Argument and parser notes">
    - 命令接受命令和参数之间的可选 `:`（例如 `/think: high`、`/send: on`、`/help:`）。
    - `/new <model>` 接受模型别名、`provider/model` 或提供商名称（模糊匹配）；如果没有匹配项，则该文本被视为消息正文。
    - 有关完整的提供商使用细分，请使用 `openclaw status --usage`。
    - `/allowlist add|remove` 需要 `commands.config=true` 并尊重渠道 `configWrites`。
    - 在多账户渠道中，以配置为目标的 `/allowlist --account <id>` 和 `/config set channels.<provider>.accounts.<id>...` 也尊重目标账户的 `configWrites`。
    - `/usage` 控制每次响应的使用页脚；`/usage cost` 从 OpenClaw 会话日志打印本地成本摘要。
    - `/restart` 默认启用；设置 `commands.restart: false` 以禁用它。
    - `/plugins install <spec>` 接受与 `openclaw plugins install` 相同的插件规范：本地路径/存档、npm 包、`git:<repo>` 或 `clawhub:<pkg>`，然后请求 Gateway(网关) 重启，因为插件源模块已更改。
    - `/plugins enable|disable` 更新插件配置并触发 Gateway(网关) 插件重新加载以进行新的代理轮次。

  </Accordion>
  <Accordion title="Channel-specific behavior">
    - Discord-only native command: `/vc join|leave|status` controls voice channels (not available as text). `join` requires a guild and selected voice/stage 渠道. Requires `channels.discord.voice` and native commands.
    ...
    - ACP command reference and runtime behavior: [ACP agents](/zh/tools/acp-agents).

  </Accordion>
  <Accordion title="Verbose / trace / fast / reasoning safety">
    - `/verbose` 用于调试和增加可见性；在正常使用中请保持 **关闭**。
    - `/trace` 比 `/verbose` 范围更窄：它仅显示插件拥有的跟踪/调试行，并保持正常的详细工具输出静默。
    - `/fast on|off` 持久化会话覆盖设置。使用会话 UI 中的 `inherit` 选项来清除它并恢复为配置默认值。
    - `/fast` 特定于提供商：OpenAI/OpenAI Codex 在原生响应端点上将其映射为 `service_tier=priority`，而发送到 `api.anthropic.com` 的直接公共 Anthropic 请求（包括 OAuth 认证的流量）将其映射为 `service_tier=auto` 或 `standard_only`。请参阅 [OpenAI](/zh/providers/openai) 和 [Anthropic](/zh/providers/anthropic)。
    - 工具失败摘要仍会在相关时显示，但详细的失败文本仅在 `/verbose` 为 `on` 或 `full` 时才会包含。
    - `/reasoning`、`/verbose` 和 `/trace` 在群组设置中具有风险：它们可能会泄露您不打算公开的内部推理、工具输出或插件诊断信息。建议保持关闭状态，尤其是在群组聊天中。

  </Accordion>
  <Accordion title="模型切换">
    - `/model`OpenClawTUI 立即保存新的会话模型。
    - 如果代理处于空闲状态，下一次运行将立即使用它。
    - 如果运行已经在进行中，OpenClaw 会将实时切换标记为待处理，并仅在清洁的重试点重启到新模型。
    - 如果工具活动或回复输出已经开始，待处理的切换可以保持排队状态，直到稍后的重试机会或下一次用户轮次。
    - 在本地 TUI 中，`/crestodian [request]`TUI 从普通代理 TUI 返回到 Crestodian。这与消息渠道救援模式不同，且不授予远程配置权限。

  </Accordion>
  <Accordion title="快速路径和内联快捷方式">
    - **快速路径：** 来自白名单发送者的仅命令消息会立即被处理（绕过队列和模型）。
    - **群组提及门控：** 来自白名单发送者的仅命令消息会绕过提及要求。
    - **内联快捷方式（仅限白名单发送者）：** 某些命令在嵌入到普通消息中时也有效，并且在模型看到剩余文本之前会被剥离。
  ...
  <Accordion title="Skill commands and native arguments">
    - **Skill commands:** `user-invocable` Skills 作为斜杠命令公开。名称会被清理为 `a-z0-9_`（最多 32 个字符）；冲突会添加数字后缀（例如 `_2`）。
      - `/skill <name> [input]` 按名称运行 Skill（当原生命令限制阻止为每个 Skill 设置命令时很有用）。
      - 默认情况下，Skill 命令作为普通请求转发给模型。
      - Skills 可以选择声明 `command-dispatch: tool` 以将命令直接路由到工具（确定性，无模型）。
      - 示例：`/prose`OpenProseOpenProse (OpenProse 插件) — 见 [OpenProse](/zh/proseDiscordTelegramSlack)。
    - **Native command arguments:** Discord 使用自动补全来处理动态选项（当你省略必需参数时显示按钮菜单）。当命令支持选项且你省略参数时，Telegram 和 Slack 会显示按钮菜单。动态选项是根据目标会话模型解析的，因此模型特定的选项（如 `/think` 级别）遵循该会话的 `/model` 覆盖设置。
...
`/tools` 回答的是一个运行时问题，而不是配置问题：**该智能体在此对话中此刻可以使用哪些内容**。
...
- **提供商使用量/配额**（例如：“Claude 剩余 80%”）会在启用使用量跟踪时显示在当前模型提供商的 `/status` 中。OpenClaw 会将提供商窗口标准化为 `% left`；对于 MiniMax，仅剩百分比字段会在显示前进行反转，而 `model_remains` 响应优先使用聊天模型条目以及带有模型标签的计划标签。
- 当实时会话快照稀疏时，`/status` 中的 **Token/cache lines** 可以回退到最新的脚本使用条目。现有的非零实时值仍然优先，并且当存储的总数缺失或较小时，脚本回退还可以恢复活动的运行时模型标签以及更大的面向提示的总数。
- **执行与运行时：** `/status` 报告有效沙箱路径的 `Execution` 以及实际运行会话的人的 `Runtime`：`OpenClaw Pi Default`、`OpenAI Codex`、一个 CLI 后端或 ACP 后端。
...
- `/model` 和 `/model list` 显示一个紧凑的带编号的选择器（模型系列 + 可用提供商）。
- 在 Discord 上，Discord`/model` 和 `/models` 会打开一个交互式选择器，其中包含提供商和模型下拉菜单以及一个提交步骤。该选择器遵循 `agents.defaults.models`，包括 `provider/*`Discord 条目，因此提供商范围的发现可以使选择器保持在 Discord 的 25 个选项组件限制内。
- `/model <#>` 从该选择器中进行选择（并尽可能优先使用当前提供商）。
...
`/trace` 让您可以在不开启完整详细模式的情况下切换**会话范围的插件跟踪/调试行**。
...
- `/trace off` 再次将其禁用。
- 插件跟踪行可以出现在 `/status` 中，并作为正常助手回复之后的后续诊断消息。
- `/trace` 不会取代 `/debug`；`/debug` 仍然管理仅运行时的配置覆盖。
- `/trace` 不会替换 `/verbose`；常规的详细工具/状态输出仍属于 `/verbose`。
...
<Note>配置在写入前会进行验证；无效的更改会被拒绝。`/config` 更新会在重启后保留。</Note>
...
- 启用和禁用的更改会为新的 Agent 轮次热重载 Gateway(网关) 插件运行时表面；安装请求 Gateway(网关) 重启，因为插件源模块已更改。
...
<AccordionGroup>
  <Accordion title="Sessions per surface">
    ...
    - **`/stop`** 定位当前活动的聊天会话，以便中止当前的运行。

  </Accordion>
  <Accordion title="SlackSlack specifics">
    `channels.slack.slashCommand` 仍支持用于单条 `/openclaw` 风格的命令。如果启用了 `commands.native`Slack，则必须为每个内置命令创建一个 Slack 斜杠命令（名称与 `/help`SlackSlack 相同）。Slack 的命令参数菜单将以临时 Block Kit 按钮的形式提供。
...
`/btw` 是关于当前会话的快速**旁注问题**。`/side` 是一个别名。
...
有关完整行为和客户端 UX 详细信息，请参阅 [BTW 附加问题](/zh/tools/btw)。
