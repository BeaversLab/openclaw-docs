...
- `/btw <question>` 提出附带问题而不更改未来的会话上下文。参见 [/tools/btw](/en/tools/btw)。
...
- `/dreaming [on|off|status|help]` 切换记忆梦境功能。请参阅 [梦境](/en/concepts/dreaming)。
- `/pair [qr|status|pending|approve|cleanup|notify]` 管理设备配对/设置流程。请参阅[配对](/en/channels/pairing)。
...
- `/plugins enable|disable` 会更新插件配置并可能会提示重启。
...
- `/fast on|off` 会持久化会话覆盖。使用会话 UI `inherit` 选项来清除它并回退到配置默认值。
- `/fast` 是特定于提供商的：OpenAI/OpenAI Codex 在原生 Responses 端点上将其映射到 `service_tier=priority`，而直接发送到 `api.anthropic.com` 的公共 Anthropic 请求（包括 OAuth 身份验证的流量）将其映射到 `service_tier=auto` 或 `standard_only`。请参阅 [OpenAI](/en/providers/openai) 和 [Anthropic](/en/providers/anthropic)。
- 在相关时仍会显示工具故障摘要，但仅当 `/verbose` 为 `on` 或 `full` 时才包含详细的故障文本。
- `/reasoning`（和 `/verbose`）在群组设置中有风险：它们可能会暴露您不想公开的内部推理或工具输出。建议保持关闭状态，尤其是在群组聊天中。
- `/model` 会立即持久化新的会话模型。
- 如果代理处于空闲状态，下一次运行将立即使用它。
- 如果一个运行已经激活，OpenClaw 会将实时切换标记为待处理，并且只在干净的重试点重新启动到新模型。
- 如果工具活动或回复输出已经开始，待处理的切换可以保持排队，直到稍后的重试机会或下一个用户回合。
- **快速路径：** 来自白名单发送者的纯命令消息会立即被处理（绕过队列 + 模型）。
- **群组提及门控：** 来自白名单发送者的纯命令消息绕过提及要求。
- **内联快捷方式（仅限白名单发送者）：** 某些命令在嵌入普通消息时也有效，并在模型看到剩余文本之前被剥离。
  - 例如：`hey /status` 触发状态回复，剩余文本继续正常流程。
- 目前包括：`/help`、`/commands`、`/status`、`/whoami` (`/id`)。
- 未经授权的纯命令消息将被静默忽略，内联 `/...` 标记将被视为纯文本。
- **Skill 命令：** `user-invocable` Skills 作为斜杠命令公开。名称会被清理为 `a-z0-9_`（最多 32 个字符）；冲突会附加数字后缀（例如 `_2`）。
  ...
  - 例如：`/prose` (OpenProse plugin) — 见 [OpenProse](/en/prose)。
- **原生命令参数：** Discord 对动态选项使用自动完成（当你省略必需参数时显示按钮菜单）。当命令支持选项且你省略参数时，Telegram 和 Slack 会显示按钮菜单。

## `/tools`

`/tools` 回答的是运行时问题，而不是配置问题：**此代理此时在此对话中可以使用什么**。

- 默认 `/tools` 是紧凑的，并针对快速扫描进行了优化。
- `/tools verbose` 添加简短描述。
- 支持参数的原生命令界面会暴露与 `compact|verbose` 相同的模式切换。
- 结果范围限定在会话内，因此更改代理、渠道、线程、发送者授权或模型可能会改变输出。
...
- 当启用使用跟踪时，**提供商使用量/配额**（例如：“Claude 剩余 80%”）会显示在 `/status` 中，针对当前模型提供商。OpenClaw 将提供商窗口标准化为 `% left`；对于 MiniMax，仅剩百分比的字段在显示前会反转，并且 `model_remains` 响应首选聊天模型条目加上带有模型标签的计划标签。
- `/status` 中的 **Token/缓存行** 当实时会话快照稀疏时，可以回退到最新的转录使用条目。现有的非零实时值仍然优先，并且当存储的总数缺失或较小时，转录回退也可以恢复活动的运行时模型标签加上更大的面向提示的总数。
- **每次响应的 Token/成本** 由 `/usage off|tokens|full` 控制（附加到正常回复中）。
- `/model status` 是关于 **模型/认证/端点**，而不是使用量。
...
- `/model <#>` 从该选择器中进行选择（并在可能时首选当前提供商）。
- `/model status` 显示详细视图，包括配置的提供商端点（`baseUrl`）和 API 模式（`api`，如果可用）。

## 调试覆盖

`/debug` 允许您设置 **仅运行时** 配置覆盖（内存，非磁盘）。仅限所有者。默认禁用；使用 `commands.debug: true` 启用。
...
`/mcp` 在 `mcp.servers` 下写入由 OpenClaw 管理的 MCP 服务器定义。仅限所有者。默认禁用；使用 `commands.mcp: true` 启用。
...
- **Slack：** `channels.slack.slashCommand` 仍支持单个 `/openclaw` 风格的命令。如果启用 `commands.native`，则必须为每个内置命令创建一个 Slack 斜杠命令（与 `/help` 同名）。Slack 的命令参数菜单以临时 Block Kit 按钮的形式提供。
  - Slack 原生例外：注册 `/agentstatus` （而非 `/status`），因为 Slack 保留了 `/status`。在 Slack 消息中，文本 `/status` 仍然有效。
...
这使得 `/btw` 在你想在主任务继续进行时获得临时澄清非常有用。
...
有关完整行为和客户端 UX 的详细信息，请参阅 [BTW Side Questions](/en/tools/btw)。
