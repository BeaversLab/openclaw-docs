---
summary: "使用 ACP 运行时会话运行 Codex、Claude Code、Cursor、Gemini CLI、OpenClaw ACP 以及其他工具代理"
read_when:
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) 会话允许 OpenClaw 通过 ACP 后端插件运行外部编码工具（例如 Pi、Claude Code、Codex、Cursor、Copilot、OpenClaw ACP、OpenCode、Gemini CLI 以及其他受支持的 ACPX 工具）。
...
- `--bind here` 仍然可以创建新的 ACP 会话，如果您正在生成新工作。绑定将该会话附加到当前对话。
...
- 子线程/主题：仅由 `--thread ...` 创建的可选附加消息界面
- runtime workspace：harness 运行的文件系统位置（`cwd`、repo checkout、后端工作区）

示例：

- `/acp spawn codex --bind here`：保留此聊天，生成或连接 Codex ACP 会话，并将此处的后续消息路由到该会话
- `/acp spawn codex --thread auto`：OpenClaw 可能会创建一个子线程/主题，并在其中绑定 ACP 会话
- `/acp spawn codex --bind here --cwd /workspace/repo`：与上述聊天绑定相同，但 Codex 在 `/workspace/repo` 中运行

当前对话绑定支持：

- 广告支持当前对话绑定的聊天/消息渠道可以通过共享的对话绑定路径使用 `--bind here`。
...
- 在 Discord 上，`--bind here` 原地绑定当前渠道或线程。仅当 OpenClaw 需要为 `--thread auto|here` 创建子线程时，才需要 `spawnAcpSessions`。
...
- `acp.enabled=true`
- `acp.dispatch.enabled` 默认处于开启状态（设置 `false` 以暂停 ACP 调度）
...
- 如果省略 `agentId`，OpenClaw 会在配置时使用 `acp.defaultAgent`。
...
- `resumeSessionId` （可选）：恢复现有的 ACP 会话，而不是创建新的会话。Agent 通过 `session/load` 重放其对话历史记录。需要 `runtime: "acp"`。
- `streamTo` （可选）：`"parent"` 将初始 ACP 运行进度摘要作为系统事件流式传回请求者会话。
  - 如果有可用的响应，接受的响应包括 `streamLogPath`，指向会话范围的 JSONL 日志（`<sessionId>.acp-stream.jsonl`），您可以对其进行跟踪以获取完整的中继历史记录。

### 恢复现有会话

使用 `resumeSessionId` 来继续之前的 ACP 会话，而不是重新开始。Agent 通过 `session/load` 重放其对话历史记录，因此它能够完全了解之前发生的事情。
...
- `resumeSessionId` 恢复上游 ACP 对话历史记录；`thread` 和 `mode` 仍然正常适用于您正在创建的新 OpenClaw 会话，因此 `mode: "session"` 仍然需要 `thread: true`。
...
1. 验证目标主机上部署的网关版本/提交。
2. 确认部署的源代码包含
   `src/gateway/sessions-patch.ts` （`subagent:* or acp:* sessions`）中的 ACP 血统接受。
3. 打开到实时 Agent（例如
   `razor(main)` 上的 `jpclawhq`）的临时 ACPX 桥接会话。
4. 要求该 Agent 调用 `sessions_spawn` 并带有以下内容：
   - `runtime: "acp"`
   - `agentId: "codex"`
   - `mode: "run"`
...
- 除非你正在故意测试
  线程绑定的持久 ACP 会话，否则请在 `mode: "run"` 上保留此冒烟测试。
- 不要为基础网关要求 `streamTo: "parent"`。该路径取决于
  请求者/会话的功能，并且是一个单独的集成检查。
- 将线程绑定的 `mode: "session"` 测试视为来自真实的 Discord 线程或 Telegram 主题的第二次、更丰富的集成
  通过。
...
当需要从聊天中进行显式的操作员控制时，请使用 `/acp spawn`。
...
1. 显式目标参数（或 `--session` 用于 `/acp steer`）
   - 尝试键
   - 然后是 UUID 形状的会话 ID
   - 然后是标签
...
如果未解析任何目标，OpenClaw 会返回明确的错误（`Unable to resolve session target: ...`）。
...
- `--bind here` 是“让此渠道或聊天支持 Codex”的最简单操作员路径。
- `--bind here` 不会创建子线程。
...
`/acp status` 显示有效的运行时选项，如果可用，还会显示运行时级别和后端级别的会话标识符。
...
`/acp sessions` 读取当前绑定或请求者会话的存储。接受 `session-key`、`session-id` 或 `session-label` 令牌的命令通过网关会话发现解析目标，包括每个代理的自定义 `session.store` 根目录。
...
直接使用 acpx CLI 也可以通过 `--agent <command>` 定位任意适配器，但这种原始的应急方法是 acpx CLI 的一项功能（而非正常的 OpenClaw `agentId` 路径）。
...
ACP 会话以非交互方式运行 — 没有 TTY 来批准或拒绝文件写入和 Shell 执行权限提示。acpx 插件提供了两个配置键来控制处理权限的方式：
...
控制当应该显示权限提示但没有可用的交互式 TTY 时发生的情况（对于 ACP 会话来说总是这种情况）。
...
> **重要：** OpenClaw 当前默认为 `permissionMode=approve-reads` 和 `nonInteractivePermissions=fail`。在非交互式 ACP 会话中，任何触发权限提示的写入或执行操作都可能会因 `AcpRuntimeError: Permission prompt unavailable in non-interactive mode` 而失败。
>
...
| 症状                                                                        | 可能原因                                                       | 修复                                                                                                                                               |
| --------------------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ACP runtime backend is not configured`                                     | 后端插件丢失或已禁用。                                         | 安装并启用后端插件，然后运行 `/acp doctor`。                                                                                                       |
...
| `Conversation bindings are unavailable for <channel>.`                      | 适配器缺乏当前会话 ACP 绑定功能。                              | 在支持的地方使用 `/acp spawn ... --thread ...`，配置顶层的 `bindings[]`，或者切换到支持的渠道。                                                    |
| `--thread here requires running /acp spawn inside an active ... thread`     | 在线程上下文之外使用了 `--thread here`。                       | 移动到目标线程或使用 `--thread auto`/`off`。                                                                                                       |
| `Only <user-id> can rebind this channel/conversation/thread.`               | 另一位用户拥有当前的绑定目标。                                 | 作为所有者重新绑定，或使用不同的对话或线程。                                                                                                       |
| `Thread bindings are unavailable for <channel>.`                            | 适配器缺少线程绑定能力。                                       | 使用 `--thread off` 或移动到支持的适配器/渠道。                                                                                                    |
| `Sandboxed sessions cannot spawn ACP sessions ...`                          | ACP 运行时位于宿主端；请求者会话处于沙箱隔离状态。             | 从沙箱隔离的会话中使用 `runtime="subagent"`，或者从非沙箱隔离的会话运行 ACP 生成。                                                                 |
| `sessions_spawn sandbox="require" is unsupported for runtime="acp" ...`     | ACP 运行时请求了 `sandbox="require"`。                         | 使用 `runtime="subagent"` 进行所需的沙箱隔离，或者从非沙箱隔离的会话结合 `sandbox="inherit"` 使用 ACP。                                            |
| 绑定会话缺少 ACP 元数据                                                     | 陈旧/已删除的 ACP 会话元数据。                                 | 使用 `/acp spawn` 重新创建，然后重新绑定/聚焦线程。                                                                                                |
| `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`    | `permissionMode` 在非交互式 ACP 会话中阻止了写入/执行。        | 将 `plugins.entries.acpx.config.permissionMode` 设置为 `approve-all` 并重启网关。参见[权限配置](#permission-configuration)。                       |
| ACP 会话过早失败且输出很少                                                  | 权限提示被 `permissionMode`/`nonInteractivePermissions` 阻止。 | 检查网关日志中的 `AcpRuntimeError`。要获得完整权限，请设置 `permissionMode=approve-all`；要实现优雅降级，请设置 `nonInteractivePermissions=deny`。 |
| ACP 会话在完成工作后无限期停滞                                              | Harness 进程已完成，但 ACP 会话未报告完成。                    | 使用 `ps aux \| grep acpx` 监控；手动终止陈旧的进程。                                                                                              |
