...
对于异步工具，OpenClaw 向提供商提交请求，立即返回任务
ID，并在任务账本中跟踪该任务。在任务运行期间，代理会继续响应其他消息。当提供商完成处理时，
OpenClaw 会唤醒代理，并附带生成的媒体路径，以便代理告知
用户并通过消息工具中继结果。如果请求会话
处于非活动状态或其主动唤醒失败，并且部分生成的媒体仍然
缺失于消息工具的传递中，OpenClaw 将发送仅包含缺失媒体的幂等直接
回退。已经通过消息工具传递的媒体不会再次发布。
