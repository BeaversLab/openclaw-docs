...
对于异步工具，OpenClaw 会向提供商提交请求，立即返回任务 ID，并在任务账本中跟踪该作业。在作业运行时，代理会继续响应其他消息。当提供商完成处理时，OpenClaw 会使用生成的媒体路径唤醒代理，以便它可以通过会话的常规可见回复模式告知用户：如果已配置，则自动发送最终回复，或者在会话需要消息工具时发送 OpenClawOpenClaw`message(action="send")`OpenClaw。如果请求者会话处于非活动状态或其主动唤醒失败，并且完成回复中仍然缺少某些生成的媒体，OpenClaw 将发送一个仅包含缺失媒体的幂等直接回退。完成回复已发送的媒体不会再次发布。

## 语音转文本和语音通话

Deepgram、DeepInfra、ElevenLabs、Mistral、OpenAI、OpenRouter、SenseAudio 和 xAI 在配置后均可通过批量 DeepgramOpenAIOpenRouter`tools.media.audio` 路径转录入站音频。对语音备注进行提及门控或命令解析预检的通道插件会在入站上下文中标记转录的附件，因此共享的媒体理解过程会重用该转录记录，而不会对同一音频进行第二次 STT 调用。

Deepgram、ElevenLabs、Mistral、OpenAI 和 xAI 还注册了语音通话流式 STT 提供商，因此实时电话音频可以转发到选定的供应商，而无需等待完整的录音。

对于实时用户对话，请首选 [Talk 模式](</en/nodes/talkGateway(网关)>)。批量音频附件保留在媒体路径上；浏览器实时、原生按住通话、电话和会议音频应使用 Talk 事件和 Gateway 返回的会话范围目录。
