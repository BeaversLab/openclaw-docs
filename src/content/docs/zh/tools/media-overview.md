...
对于异步工具，OpenClaw 会向提供商提交请求，立即返回任务 ID，并在任务账本中跟踪该任务。在任务运行时，代理会继续响应其他消息。当提供商完成处理后，OpenClaw 会使用生成的媒体路径唤醒代理，以便代理可以告知用户并通过消息工具传递结果。如果请求者会话处于非活动状态，且某些生成的媒体仍然缺失于消息工具的交付中，OpenClaw 将发送一个仅包含缺失媒体的幂等直接回退。已经通过消息工具交付的媒体不会再次发布。
