...
- 经过 Gateway 身份验证的调用者是该 Gateway 的受信任操作员。
...
对于直接调用 Gateway 的 Telegram 和其他原生审批客户端，此回退机制有针对性地限定为“未找到审批”的失败情况。真正的 exec 审批拒绝/错误不会作为插件审批静默重试。
...
常见问题：[为什么聊天审批有两个 exec 审批配置？](/en/help/faq#why-are-there-two-exec-approval-configs-for-chat-approvals)
...
- Slack 原生按钮保留批准 id 类型，因此 `plugin:` id 可以解析插件批准
  而不需要第二层 Slack 本地回退层
- Matrix 原生私信/渠道路由仅限于 exec；Matrix 插件批准保留在共享的
  同聊天 `/approve` 和可选的 `approvals.plugin` 转发路径上
- 请求者不需要是审批人
- 当原始聊天已经支持命令和回复时，原始聊天可以使用 `/approve` 直接批准
- 原生 Discord 批准按钮按批准 id 类型路由：`plugin:` id 直接
  进入插件批准，其他所有内容进入 exec 批准
...
Telegram 默认为审批人私信 (`target: "dm"`)。当您
希望批准提示也出现在原始 Telegram 聊天/主题中时，您可以切换到 `channel` 或 `both`。对于 Telegram 论坛
主题，OpenClaw 会为批准提示和批准后后续操作保留该主题。
...
在节点报告事件后，这些消息会发布到代理的会话中。
Gateway-host exec 批准在命令完成时（以及可选地在运行时间超过阈值时）发出相同的生命周期事件。
需要批准的 exec 在这些消息中重用批准 id 作为 `runId`，以便于关联。
...
- `/exec security=full` 是授权操作员在会话级别的便利功能，设计上会跳过批准。
  若要硬性阻止主机 exec，请将批准安全性设置为 `deny` 或通过工具策略拒绝 `exec` 工具。
...
- [沙箱与工具策略与提权](/en/gateway/sandbox-vs-tool-policy-vs-elevated) — 何时使用每一种
