...
对于基于会话的代理运行，OpenClaw 将音乐生成为后台任务，在任务分类账中跟踪它，然后在曲目准备就绪时再次唤醒代理，以便代理可以告知用户并附加完成的音频。生成的媒体完成项由代理通过消息工具传递。如果请求者会话处于非活动状态或其活动唤醒失败，并且消息工具传递中仍然缺少一些生成的音频，OpenClaw 将发送一个仅包含缺失音频的幂等直接回退。完成唤醒明确警告代理，此路由的正常最终回复是私有的。
