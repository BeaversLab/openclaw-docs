...
当需要批准时，exec 工具会立即返回一个批准 ID。使用该 ID来关联后续批准运行的系统事件（`Exec finished`，以及配置时的 `Exec running`OpenClaw）。如果在超时之前未做出决定，该请求将被视为批准超时，并作为终端主机命令拒绝显示。对于具有原始会话的代理异步批准，OpenClaw 还会通过内部后续操作恢复该会话，以便代理观察到命令未运行，而不是稍后修复缺失的结果。

### 后续交付行为

批准的异步 exec 完成后，OpenClaw 会向同一会话发送后续 OpenClaw`agent` 轮次。被拒绝的异步批准使用相同的主会话后续路径来传递拒绝状态，但它们不会注册提升的运行时交接，也不会运行该命令。没有可恢复主会话的拒绝要么被抑制，要么在存在安全直接路由时通过该路由报告。
