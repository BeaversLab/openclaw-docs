...
OpenClaw 使用与 **[AgentSkills](https://agentskills.io)** 兼容的技能文件夹来教代理如何使用工具。每个技能都是一个包含 `SKILL.md` 的目录，其中包含 YAML frontmatter 和指令。OpenClaw 加载**捆绑技能**以及可选的本地覆盖，并根据环境、配置和二进制文件的存在在加载时对其进行过滤。
...
对于捆绑的 `claude-cli` 后端，OpenClaw 还将相同的合格快照实现为临时 Claude Code 插件，并使用 `--plugin-dir` 传递它。Claude Code 然后可以使用其原生技能解析器，而 OpenClaw 仍然拥有优先级、每代理允许列表、门控和 `skills.entries.*` env/API 密钥注入。其他 CLI 后端仅使用提示目录。

## 会话快照（性能）

OpenClaw 在**会话开始时**对合格技能进行快照，并在同一会话的后续轮次中重用该列表。对技能或配置的更改将在下一个新会话中生效。

当启用 Skills 监视器或出现新的符合条件的远程节点时（见下文），Skills 也可以在会话期间刷新。可以将这视为一种**热重新加载**：刷新后的列表将在下一次代理轮次中被采用。

如果该会话的有效代理 Skills 允许列表发生变化，OpenClaw 会刷新快照，以便可见的 Skills 与当前代理保持一致。

## 远程 macOS 节点（Linux Gateway）

如果 Gateway(网关) 在 Linux 上运行，但连接了一个 **macOS 节点**且**允许 `system.run`**（Exec approvals 安全性未设置为 `deny`），则当该节点上存在所需的二进制文件时，OpenClaw 可以将仅限 macOS 的 Skills 视为符合条件。代理应通过 `exec` 工具并使用 `host=node` 来执行这些 Skills。

这依赖于节点报告其命令支持以及通过 `system.run` 进行二进制探测。如果 macOS 节点稍后离线，Skills 仍然可见；在节点重新连接之前，调用可能会失败。

## Skills 监视器（自动刷新）

默认情况下，OpenClaw 会监视 Skills 文件夹，并在 `SKILL.md` 文件更改时更新 Skills 快照。在 `skills.load` 下配置此功能：
...
当 Skills 符合条件时，OpenClaw 会将可用 Skills 的精简 XML 列表注入系统提示（通过 `formatSkillsForPrompt` 中的 `pi-coding-agent`）。成本是确定的：

- **基本开销（仅当 ≥1 个 Skill 时）：** 195 个字符。
- **每个 Skill：** 97 个字符 + 经过 XML 转义的 `<name>`、`<description>` 和 `<location>` 值的长度。
...
- Token 计数因模型分词器而异。粗略的 OpenAI 风格估算约为 4 个字符/token，因此 **97 个字符 ≈ 24 个 token**，外加您的实际字段长度。

## 托管 Skills 生命周期

OpenClaw 在安装（npm 包或 OpenClaw.app）过程中附带了一组作为 **bundled skills** 的基准 Skills。`~/.openclaw/skills` 用于本地覆盖（例如，固定/修补 Skill 而不更改捆绑的副本）。Workspace Skills 归用户所有，并且在名称冲突时覆盖两者。
