...
- 对于网关 + 节点，默认为无审批的主机执行。如果您需要审批/允许列表行为，请同时收紧 `tools.exec.*` 和主机 `~/.openclaw/exec-approvals.json`；参见 [Exec approvals](/zh/tools/exec-approvals#yolo-mode-no-approval)。
...
- `tools.exec.strictInlineEval`（默认值：false）：当为 true 时，内联解释器 eval 表单（例如 `python -c`、`node -e`、`ruby -e`、`perl -e`、`php -r`、`lua -e` 和 `osascript -e`）需要审核者或明确审批。在 `mode=auto` 中，正常的 exec 审批路径可能会让原生自动审核者允许明显低风险的一次性命令；直接的主机托管 `system.run` 调用仍然需要明确审批，因为它们无法将命令传递给人工审批路由。如果审核者提出要求，请求将发送给人工。`allow-always` 仍然可以持续执行良性解释器/脚本调用，但内联 eval 表单不会成为持久的允许规则。
- `tools.exec.commandHighlighting`（默认值：false）：当为 true 时，审批提示可以在命令文本中突出显示解析器派生的命令范围。全局设置或按代理设置为 `true`，以便在不更改 exec 审批策略的情况下启用命令文本突出显示。
- `tools.exec.pathPrepend`：要在 exec 运行时添加到 `PATH` 前面的目录列表（仅限网关 + 沙箱）。
- `tools.exec.safeBins`：仅限 stdin 的安全二进制文件，无需明确的允许列表条目即可运行。有关行为详细信息，请参阅 [Safe bins](/zh/tools/exec-approvals-advanced#safe-bins-stdin-only)。
- `tools.exec.safeBinTrustedDirs`：用于 `safeBins` 路径检查的额外受信任显式目录。`PATH` 条目永远不会被自动信任。内置默认值为 `/bin` 和 `/usr/bin`。
...
- `host=gateway`：将您的登录 Shell `PATH` 合并到 exec 环境中。`env.PATH` 覆盖在
  主机执行期间会被拒绝。守护进程本身仍然使用最小的 `PATH` 运行：
  - macOS：`/opt/homebrew/bin`、`/usr/local/bin`、`/usr/bin`、`/bin`
  - Linux：`/usr/local/bin`、`/usr/bin`、`/bin`
    - 为了防止用户 Shell 配置（如 `~/.zshenv` 或 `/etc/zshenv`）在启动期间覆盖优先路径，`tools.exec.pathPrepend` 条目会在执行前安全地添加到 Shell 命令内部的最终 `PATH` 之前。
- `host=sandbox`：在容器内运行 `sh -lc`（登录 Shell），因此 `/etc/profile` 可能会重置 `PATH`。
  OpenClaw 通过内部环境变量（无 Shell 插值）在配置文件加载后添加 `env.PATH`；
  `tools.exec.pathPrepend` 也适用于此。
- `host=node`：只有您传递的未被阻止的环境变量覆盖才会发送到节点。`env.PATH` 覆盖
  在主机执行期间会被拒绝，并被节点主机忽略。如果您在节点上需要额外的 PATH 条目，
...
它**仅更新会话状态**，不写入配置。授权的外部渠道发送者可以
设置这些会话默认值。内部 Gateway/webchat 客户端需要 `operator.admin` 才能持久化它们。
要彻底禁用 exec，请通过工具策略（`tools.deny: ["exec"]` 或每个代理）拒绝它。除非您显式设置
...
(`Exec running` / `Exec finished`)。被拒绝或超时的批准是终态的，不会
通过拒绝系统事件唤醒代理会话。
在具有原生批准卡片/按钮的渠道上，代理应首先依赖该
原生 UI，并且仅在工具结果明确说明聊天批准不可用或手动批准是
唯一路径时，才包含手动 `/approve` 命令。

## 白名单 + 安全回收站

手动允许列表执行会匹配已解析的二进制路径通配符和纯命令名通配符。纯名称仅匹配通过 PATH 调用的命令，因此当命令是 `rg` 时，`rg` 可以匹配 `/opt/homebrew/bin/rg`，但不能匹配 `./rg` 或 `/tmp/rg`。
当 `security=allowlist` 时，仅当管道中的每个段都在允许列表中或是安全二进制文件时，shell 命令才会被自动允许。在允许列表模式下，除非每个顶级段都满足允许列表（包括安全二进制文件），否则将拒绝链接（`;`、`&&`、`||`）和重定向。重定向仍然不受支持。
持久的 `allow-always` 信任不能绕过该规则：链接的命令仍然要求每个顶级段都匹配。

`autoAllowSkills` 是 exec 批准中一条独立的便捷路径。它并不等同于手动路径允许列表条目。为了严格的显式信任，请保持 `autoAllowSkills` 禁用状态。
...
不要将 `safeBins` 视为通用白名单，并且不要添加解释器/运行时二进制文件（例如 `python3`、`node`、`ruby`、`bash`）。如果您需要这些，请使用显式白名单条目并保持批准提示启用。
`openclaw security audit` 会在解释器/运行时 `safeBins` 条目缺少显式配置时发出警告，而 `openclaw doctor --fix` 可以构建缺失的自定义 `safeBinProfiles` 条目。
当您明确将具有广泛行为特性的二进制文件（如 `jq`）重新添加到 `safeBins` 时，`openclaw security audit` 和 `openclaw doctor` 也会发出警告。
如果您明确将解释器列入白名单，请启用 `tools.exec.strictInlineEval`，以便内联代码求值表单仍然需要审查员或显式批准。
...
对于 OpenAI 和 OpenAI Codex 模型，它默认启用。仅在您想要禁用它或将其限制为特定模型时才使用配置：
...
- `deny: ["write"]` 不会拒绝 `apply_patch`；当补丁写入也应被阻止时，请明确拒绝 `apply_patch` 或使用 `deny: ["group:fs"]`。
- 配置位于 `tools.exec.applyPatch` 下。
- `tools.exec.applyPatch.enabled` 默认为 `true`；将其设置为 `false` 可为 OpenAI 模型禁用该工具。
- `tools.exec.applyPatch.workspaceOnly` 默认为 `true`（包含在工作空间内）。仅当您有意希望 `apply_patch` 在工作空间目录之外进行写入/删除操作时，才将其设置为 `false`。
...
- [Security](/zh/gateway/security) — 工具策略和提升的访问权限
