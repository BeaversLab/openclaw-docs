...
<Steps>
  <Step title="选择提供商"OpenAI>
    OpenAI 和 ElevenLabs 是最可靠的托管选项。Microsoft 和
    Local CLI 无需 API 密钥即可工作。请参阅 [提供商矩阵](#supported-providers)
    以获取完整列表。
  </Step>
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="provider" type="string"OpenClaw>
      语音提供商 ID。未设置时，OpenClaw 将按注册表自动选择顺序使用第一个配置的提供商。旧的 `provider: "edge"` 会被 `openclaw doctor --fix` 重写为 `"microsoft"`。
    </ParamField>
    <ParamField path="persona" type="string">
      来自 `personas` 的活动 persona ID。已标准化为小写。
    </ParamField>
    ...
    <ParamField path="prefsPath" type="string">
      覆盖本地首选项 JSON 路径（提供商/limit/summary）。默认 `~/.openclaw/settings/tts.json`。
    </ParamField>
  </Accordion>
...
- [OpenAI text-to-speech 指南](OpenAIhttps://platform.openai.com/docs/guides/text-to-speech)
...
- [Xiaomi MiMo speech synthesis](Xiaomi/en/providers/xiaomi#text-to-speech)
...
- [Media overview](/zh/tools/media-overview)
- [Music generation](/zh/tools/music-generation)
- [Video generation](/zh/tools/video-generation)
