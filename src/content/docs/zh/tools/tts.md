...
5. 此主机的本地 `/tts` 偏好设置
6. 当启用[模型 overrides](#model-driven-directives)时，内联 `[[tts:...]]` 指令

渠道和账户覆盖使用与 `messages.tts` 相同的结构，
并与之前的层级进行深度合并，因此共享的提供商凭据可以保留在
`messages.tts` 中，而渠道或机器人账户仅更改说话人声音、模型、人设
或自动模式：
...
单一命令 `/tts`。在 Discord 上，OpenClaw 也注册了 `/voice`，因为
...
<Note>命令需要授权的发送者（应用允许列表/所有者规则）且必须启用 `commands.text` 或原生命令注册。</Note>

行为说明：

- `/tts on` 将本地 TTS 偏好写入 `always`；`/tts off` 将其写入 `off`。
- `/tts chat on|off|default` 为当前聊天写入会话范围的自动 TTS 覆盖设置。
- `/tts persona <id>` 写入本地人设偏好；`/tts persona off` 清除它。
- `/tts latest` 从当前会话记录中读取最新的助手回复，并将其作为音频发送一次。它仅在该会话条目上存储该回复的哈希值，以抑制重复的语音发送。
- `/tts audio` 生成一次性音频回复（**不**开启 TTS）。
- `limit` 和 `summary` 存储在 **本地首选项** 中，而非主配置。
- `/tts status` 包含最新尝试的回退诊断信息 —— `Fallback: <primary> -> <used>`、`Attempts: ...` 以及每次尝试的详细信息（`provider:outcome(reasonCode) latency`）。
- 当启用 TTS 时，`/status` 会显示活动的 TTS 模式以及配置的提供商、模型、语音和经过清理的自定义端点元数据。

## 每个用户的偏好设置

斜杠命令会将本地覆盖写入 `prefsPath`。默认为
...
| `summarize` | 摘要切换（默认 `true`）                 |

这些设置会覆盖 `messages.tts` 的有效配置以及该主机活动的
`agents.list[].tts` 块。

## 输出格式（固定）

TTS 语音交付由渠道能力驱动。渠道插件会通告语音风格的 TTS 是否应向提供商请求原生 `voice-note` 目标，
还是保持常规 `audio-file` 合成并仅标记兼容输出以进行语音
交付。

- **支持语音笔记的渠道**：语音笔记回复优先使用 Opus（来自 ElevenLabs 的 `opus_48000_64`，来自 OpenAI 的 `opus`）。
  - 48kHz / 64kbps 是语音消息的一个良好折衷方案。
- **飞书 / WhatsApp**：当语音笔记回复生成为 MP3/WebM/WAV/M4A 或其他可能的音频文件时，渠道插件会在发送原生语音消息之前使用 WhatsApp`ffmpeg`WhatsAppBaileys 将其转码为 48kHz 的 Ogg/Opus 格式。WhatsApp 通过 Baileys `audio` 载荷配合 `ptt: true` 和 `audio/ogg; codecs=opus`WhatsApp 发送结果。如果转换失败，飞书会收到原始文件作为附件；WhatsApp 发送会失败，而不是发布不兼容的 PTT 载荷。
- **其他渠道**：MP3（来自 ElevenLabs 的 `mp3_44100_128`，来自 OpenAI 的 `mp3`OpenAI）。
  - 44.1kHz / 128kbps 是语音清晰度的默认平衡设置。
- **MiniMax**：普通音频附件为 MP3（MiniMax`speech-2.8-hd`OpenClawMiniMax 模型，32kHz 采样率）。对于渠道通告的语音笔记目标，当渠道通告支持转码时，OpenClaw 会在交付前使用 `ffmpeg` 将 MiniMax MP3 转码为 48kHz Opus。
- **Xiaomi MiMo**：默认为 MP3，或者根据配置为 WAV。对于渠道通告的语音笔记目标，当渠道通告支持转码时，OpenClaw 会在交付前使用 XiaomiOpenClawXiaomi`ffmpeg` 将 Xiaomi 输出转码为 48kHz Opus。
- **Local CLI**：使用配置的 CLI`outputFormat`。语音笔记目标被转换为 Ogg/Opus，电话输出被转换为带有 `ffmpeg` 的原始 16 kHz 单声道 PCM。
...
- **Microsoft**：使用 `microsoft.outputFormat`（默认 `audio-24khz-48kbitrate-mono-mp3`）。
  - 内置传输接受 `outputFormat`，但并非所有格式都可从该服务获取。
  - 输出格式值遵循 Microsoft 语音输出格式（包括 Ogg/WebM Opus）。
  - Telegram Telegram`sendVoice`OpenAI 接受 OGG/MP3/M4A；如果您需要
...
启用 `messages.tts.auto`OpenClaw 后，OpenClaw：
...
- 将生成的音频附加到回复中。
- 在 `mode: "final"` 中，文本流完成后仍为流式最终回复
  发送仅音频 TTS；生成的媒体会经历与普通回复附件相同的
  渠道 媒体标准化。

如果回复超过 `maxLength` 且摘要关闭（或没有摘要模型的 API 密钥），则跳过音频并发送普通文本回复。
...
| 目标                                  | 格式                                                                                                                                |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Feishu / Matrix / Telegram / WhatsApp | 语音笔记回复首选 **Opus**（来自 ElevenLabs 的 `opus_48000_64`，来自 OpenAI 的 `opus`OpenAI）。48 kHz / 64 kbps 平衡了清晰度和大小。 |
| 其他渠道                              | **MP3**（来自 ElevenLabs 的 `mp3_44100_128`，来自 OpenAI 的 `mp3`OpenAI）。44.1 kHz / 128 kbps 为语音默认值。                       |
| Talk / 电话                           | 提供商原生 **PCM**（Inworld 22050 Hz，Google 24 kHz），或来自 Gradium 的 `ulaw_8000` 用于电话。                                     |

各提供商说明：

- **Feishu / WhatsApp 转码：** 当语音笔记回复为 MP3/WebM/WAV/M4A 时，渠道 插件会使用 `ffmpeg` 将其转码为 48 kHz Ogg/Opus。WhatsApp 通过 Baileys 发送，并使用 `ptt: true` 和 `audio/ogg; codecs=opus`。如果转换失败：Feishu 会回退到附加原始文件；WhatsApp 发送会失败，而不是发布不兼容的 PTT 载荷。
- **MiniMax / Xiaomi MiMo:** 默认 MP3（MiniMax MiniMaxXiaomiMiniMax`speech-2.8-hd` 为 32 kHz）；通过 `ffmpeg` 转码为 48 kHz Opus 以用于语音消息目标。
- **Local CLI:** 使用配置的 CLI`outputFormat`。语音消息目标转换为 Ogg/Opus，电话输出转换为原始 16 kHz 单声道 PCM。
...
- **xAI:** 默认为 MP3；`responseFormat` 可能 `mp3|wav|pcm|mulaw|alaw`。使用 xAI 的批量 REST 端点——**不**使用流式 WebSocket TTS。**不**支持原生 Opus 语音消息格式。
- **Microsoft:** 使用 `microsoft.outputFormat`（默认 `audio-24khz-48kbitrate-mono-mp3`Telegram）。Telegram `sendVoice`OpenAIOpenClaw 接受 OGG/MP3/M4A；如果需要确保 Opus 语音消息，请使用 OpenAI/ElevenLabs。如果配置的 Microsoft 格式失败，OpenClaw 将重试使用 MP3。
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    <ParamField path="auto" type='"off" | "always" | "inbound" | "tagged"'>
      自动 TTS 模式。`inbound` 仅在收到入站语音消息后发送音频；`tagged` 仅在回复包含 `[[tts:...]]` 指令或 `[[tts:text]]` 代码块时发送音频。
    </ParamField>
    <ParamField path="enabled" type="boolean" deprecated>
      旧版切换开关。`openclaw doctor --fix` 会将其迁移到 `auto`。
    </ParamField>
    <ParamField path="mode" type='"final" | "all"' default="final">
      `"all"` 除了最终回复外，还包括工具/代码块回复。
    </ParamField>
    <ParamField path="provider" type="string"OpenClaw>
      语音提供商 ID。如果未设置，OpenClaw 将按注册表自动选择顺序使用第一个配置的提供商。旧版 `provider: "edge"` 会被 `openclaw doctor --fix` 重写为 `"microsoft"`。
    </ParamField>
    <ParamField path="persona" type="string">
      来自 `personas` 的活跃人设 ID。已规范化为小写。
    </ParamField>
    <ParamField path="personas.<id>" type="object">
      稳定的语音身份。字段：`label`、`description`、`provider`、`fallbackPolicy`、`prompt`、`providers.<provider>`。参见 [人设](#personas)。
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      由语音提供商 ID 键入的提供商自有设置。旧版直接代码块（`messages.tts.openai`、`.elevenlabs`、`.microsoft`、`.edge`）由 `openclaw doctor --fix` 重写；请仅提交 `messages.tts.providers.<id>`。
    </ParamField>
...
<Accordion title="Azure 语音">
  ...
  <ParamField path="speakerVoice" type="string">
    Azure 语音 ShortName。默认值 `en-US-JennyNeural`。旧别名：`voice`。
  </ParamField>
  <ParamField path="lang" type="string">
    SSML 语言代码。默认值 `en-US`。
  </ParamField>
  <ParamField path="outputFormat" type="string">
    标准音频的 Azure `X-Microsoft-OutputFormat`。默认值 `audio-24khz-48kbitrate-mono-mp3`。
  </ParamField>
  <ParamField path="voiceNoteOutputFormat" type="string">
    语音备注输出的 Azure `X-Microsoft-OutputFormat`。默认值 `ogg-24khz-16bit-mono-opus`。
  </ParamField>
</Accordion>
...
<Accordion title="ElevenLabs">
  ...
  <ParamField path="languageCode" type="string">
    双字母 ISO 639-1 代码（例如 `en`、`de`）。
  </ParamField>
  <ParamField path="seed" type="number">
    整数 `0..4294967295`，用于尽最大努力实现确定性。
  </ParamField>
  <ParamField path="baseUrl" type="string" API>
...
<Accordion title="Gradium">
  ...
  <Accordion title="Inworld">
    ...
    <ParamField path="modelId" type="string">默认值 `inworld-tts-1.5-max`. 另支持： `inworld-tts-1.5-mini`, `inworld-tts-1-max`, `inworld-tts-1`.</ParamField>
    <ParamField path="speakerVoiceId" type="string">默认值 `Sarah`. 旧别名： `voiceId`.</ParamField>
    <ParamField path="temperature" type="number">采样温度 `0..2`.</ParamField>

  </Accordion>

<Accordion title="CLI本地 CLI (tts-local-cli)">
  <ParamField path="command" type="string" CLI>
    CLI TTS 的本地可执行文件或命令字符串。
  </ParamField>
  <ParamField path="args" type="string[]">
    命令参数。支持 `{{ Text }}`, `{{ OutputPath }}`, `{{ OutputDir }}`, `{{ OutputBase }}` 占位符。
  </ParamField>
  <ParamField path="outputFormat" type='"mp3" | "opus" | "wav"' CLI>
    预期的 CLI 输出格式。音频附件的默认值为 `mp3`。
  </ParamField>
  <ParamField path="timeoutMs" type="number">
...
<Accordion title="APIMicrosoft (无 API 密钥)">
  ...
  <ParamField path="speakerVoice" type="string">
    Microsoft 神经语音名称（例如 `en-US-MichelleNeural`）。旧别名：`voice`。
  </ParamField>
  <ParamField path="lang" type="string">
    语言代码（例如 `en-US`）。
  </ParamField>
  <ParamField path="outputFormat" type="string">
    Microsoft 输出格式。默认为 `audio-24khz-48kbitrate-mono-mp3`。并非所有格式都受随附的 Edge 支持传输支持。
  </ParamField>
  ...
  <ParamField path="edge.*" type="object" deprecated>
    旧别名。运行 `openclaw doctor --fix` 以将持久化配置重写为 `providers.microsoft`。
  </ParamField>
</Accordion>
...
<Accordion title="MiniMaxMiniMax">
  ...
  <ParamField path="speakerVoiceId" type="string">
    默认值 `English_expressive_narrator`。环境变量：`MINIMAX_TTS_VOICE_ID`。旧版别名：`voiceId`。
  </ParamField>
...
<Accordion title="OpenAIOpenAI">
  ...
  <ParamField path="extraBody / extra_body" type="Record<string, unknown>">
    在生成的 OpenAI TTS 字段之后合并到 `/audio/speech`OpenAIOpenAI 请求正文中的额外 JSON 字段。将其用于需要特定于提供商的密钥（如 `lang`）的 OpenAI 兼容端点（如 Kokoro）；不安全的原型键将被忽略。
  </ParamField>
...
<Accordion title="OpenRouterOpenRouter">
  ...
  <ParamField path="baseUrl" type="string">
    默认 `https://openrouter.ai/api/v1`。旧版 `https://openrouter.ai/v1` 会被规范化。
  </ParamField>
  <ParamField path="model" type="string">
    默认 `hexgrad/kokoro-82m`。别名：`modelId`。
  </ParamField>
  <ParamField path="speakerVoice" type="string">
    默认 `af_alloy`。旧版别名：`voice`、`voiceId`。
  </ParamField>
...
<Accordion title="Volcengine (BytePlus Seed Speech)">
  ...
  <ParamField path="appId / token / cluster" type="string" deprecated>
    旧版 Volcengine 语音控制台字段。环境变量：`VOLCENGINE_TTS_APPID`，`VOLCENGINE_TTS_TOKEN`，`VOLCENGINE_TTS_CLUSTER`（默认值 `volcano_tts`）。
  </ParamField>
</Accordion>
...
<Accordion title="xAI">
  ...
  <ParamField path="speakerVoiceId" type="string">
    默认值 `eve`。可用语音：`ara`、`eve`、`leo`、`rex`、`sal`、`una`。旧别名：`voiceId`。
  </ParamField>
  ...
  <Accordion title="Xiaomi小米 MiMo">
    ...
    <ParamField path="model" type="string">默认 `mimo-v2.5-tts`。环境变量：`XIAOMI_TTS_MODEL`。同时也支持 `mimo-v2-tts`。</ParamField>
    <ParamField path="speakerVoice" type="string">默认 `mimo_default`。环境变量：`XIAOMI_TTS_VOICE`。旧版别名：`voice`。</ParamField>
...
`tts`MatrixTelegramWhatsAppWhatsApp 工具将文本转换为语音并返回音频附件，用于回复投递。在飞书、Matrix、Telegram 和 WhatsApp 上，音频会作为语音消息而非文件附件发送。当 `ffmpeg` 可用时，飞书和 WhatsApp 可以在此路径上转码非 Opus 格式的 TTS 输出。

WhatsApp 通过 Baileys 将音频作为 PTT 语音笔记（WhatsAppBaileys`audio` 并带有 `ptt: true`）发送，并将可见文本与 PTT 音频**分开**发送，因为客户端在语音笔记上不能一致地渲染字幕。

该工具接受可选的 `channel` 和 `timeoutMs` 字段；`timeoutMs` 是以毫秒为单位的每次调用提供商请求超时时间。每次调用的值会覆盖 `messages.tts.timeoutMs`；配置的 TTS 超时会覆盖任何插件编写的提供商默认值。
