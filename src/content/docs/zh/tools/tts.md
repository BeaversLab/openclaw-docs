...
OpenClaw 可以使用 ElevenLabs、Microsoft 或 OpenAI 将回复转换为音频。
在 OpenClaw 可以发送音频的任何地方，它都有效。
...
捆绑的 Microsoft 语音提供商目前通过 `node-edge-tts` 库使用 Microsoft Edge 的在线神经 TTS 服务。这是一种托管服务（非本地），使用 Microsoft 端点，不需要 API 密钥。
...
如果配置了多个提供商，则优先使用选定的提供商，其他作为备用选项。
...
不。默认情况下自动 TTS 处于**关闭**状态。在配置中使用
`messages.tts.auto` 启用它，或按会话使用 `/tts always`（别名：`/tts on`）启用。
...
完整架构请参阅 [Gateway(网关) 配置](/en/gateway/configuration)。
...
- `providers.openai.baseUrl`：覆盖 OpenAI TTS 端点。
  ...
  - 非默认值将被视为 OpenAI 兼容的 TTS 端点，因此接受自定义模型和语音名称。
...
- `providers.elevenlabs.seed`：整数 `0..4294967295`（尽力确保确定性）
- `providers.microsoft.enabled`: 允许使用 Microsoft 语音（默认为 `true`；不需要 API 密钥）。
...
- `providers.microsoft.outputFormat`: Microsoft 输出格式（例如 `audio-24khz-48kbitrate-mono-mp3`）。
  - 有关有效值，请参阅 Microsoft 语音输出格式；并非所有格式都受内置 Edge 支持的传输支持。
...
启用后，模型可以发出 `[[tts:...]]` 指令来覆盖单个回复的语音，还可以添加一个可选的 `[[tts:text]]...[[/tts:text]]` 块来提供仅应在音频中出现的表达性标签（笑声、歌唱提示等）。
...
如果回复超过 `maxLength` 且摘要功能已关闭（或者没有摘要 API 的 API 密钥），则跳过音频并发送正常的文本回复。
...
只有一个命令：`/tts`。
有关启用详细信息，请参阅 [Slash commands](/en/tools/slash-commands)。

Discord 说明：`/tts` 是 Discord 的内置命令，因此 OpenClaw 在那里将 `/voice` 注册为本地命令。文本 `/tts ...` 仍然有效。
...
- 命令需要经过授权的发送者（允许列表/所有者规则仍然适用）。
- 必须启用 `commands.text` 或本地命令注册。
- `off|always|inbound|tagged` 是基于会话的开关（`/tts on` 是 `/tts always` 的别名）。
- `limit` 和 `summary` 存储在本地首选项中，而不是主配置中。
- `/tts audio` 生成一次性音频回复（不会开启 TTS）。

## Agent 工具

`tts` 工具将文本转换为语音并返回音频附件以供回复。当渠道为 Feishu、Matrix、Telegram 或 WhatsApp 时，音频将以语音消息的形式传递，而不是文件附件。
