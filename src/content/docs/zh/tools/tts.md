...
- [ElevenLabs 语音合成](https://elevenlabs.io/docs/api-reference/text-to-speech)
...
完整架构请参考 [Gateway(网关) 配置](/en/gateway/configuration)。
...
- `mode`：`"final"`（默认）或 `"all"`（包括工具/模块回复）。
- `provider`：语音提供商 ID，例如 `"elevenlabs"`、`"microsoft"`、`"minimax"` 或 `"openai"`（后备是自动的）。
- 如果未设置 `provider`，OpenClaw 将使用注册表自动选择顺序中的第一个已配置语音提供商。
...
- `modelOverrides`：允许模型发出 TTS 指令（默认开启）。
  - `allowProvider` 默认为 `false`（提供商切换为可选项）。
- `providers.<id>`：按语音提供商 ID 键控的提供商自有设置。
- 传统的直接提供商块（`messages.tts.openai`、`messages.tts.elevenlabs`、`messages.tts.microsoft`、`messages.tts.edge`）在加载时会自动迁移到 `messages.tts.providers.<id>`。
...
默认情况下，模型**可以**针对单条回复发出 TTS 指令。
当 `messages.tts.auto` 为 `tagged` 时，必须存在这些指令才能触发音频。

启用后，模型可以发出 `[[tts:...]]` 指令来覆盖单条回复的语音，
以及一个可选的 `[[tts:text]]...[[/tts:text]]` 块，用于
提供只应出现在音频中的表达性标签（笑声、歌唱提示等）。
...
- **MiniMax**：MP3 (`speech-2.8-hd` 模型，32kHz 采样率)。语音备忘录格式并非原生支持；请使用 OpenAI 或 ElevenLabs 以确保获得 Opus 语音消息。
- **Microsoft**：使用 `microsoft.outputFormat` (默认 `audio-24khz-48kbitrate-mono-mp3`)。
  - 捆绑的传输接受 `outputFormat`，但并非所有格式都能从该服务获得。
...
- 跳过非常短的回复（< 10 个字符）。
- 当使用 `agents.defaults.model.primary` (或 `summaryModel`) 启用时，对长回复进行摘要。
- 将生成的音频附加到回复中。

如果回复超过了 `maxLength` 并且摘要已关闭（或没有用于摘要模型的 API 密钥），将跳过音频并发送正常的文本回复。
...
Discord 注意：`/tts` 是内置的 Discord 命令，因此 OpenClaw 在那里将 `/voice` 注册为本地命令。文本 `/tts ...` 仍然有效。
...
`tts` 工具将文本转换为语音，并返回音频附件以进行回复传递。当渠道是 Feishu、Matrix、Telegram 或 WhatsApp 时，音频将作为语音消息而不是文件附件传递。
