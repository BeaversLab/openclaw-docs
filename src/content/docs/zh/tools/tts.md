...
5. 此主机的本地 `/tts` 偏好设置
6. 内联 `[[tts:...]]` 指令（当启用[模型覆盖](#model-driven-directives)时）
...
- 将生成的音频附加到回复中。
- 在 `mode: "final"` 中，文本流完成后，仍会为流式最终回复
  发送仅音频的 TTS；生成的媒体会经过与普通回复附件相同的
  渠道媒体标准化处理。

如果回复超过 `maxLength`API 且摘要已关闭（或没有用于摘要模型的 API 密钥），
则跳过音频并发送普通文本回复。
...
| 目标                                  | 格式                                                                                                                                          |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Feishu / Matrix / Telegram / WhatsApp | 语音笔记回复首选 **Opus**（来自 ElevenLabs 的 `opus_48000_64`，来自 OpenAI 的 `opus`OpenAI）。48 kHz / 64 kbps 在清晰度和大小之间取得了平衡。 |
| 其他渠道                              | **MP3**（来自 ElevenLabs 的 `mp3_44100_128`，来自 OpenAI 的 `mp3`OpenAI）。语音默认为 44.1 kHz / 128 kbps。                                   |
| Talk / 电话                           | 提供商原生 **PCM**（Inworld 22050 Hz，Google 24 kHz），或来自 Gradium 的 `ulaw_8000` 用于电话。                                               |

各提供商说明：

- **飞书 / WhatsApp 转码：** 当语音笔记回复为 MP3/WebM/WAV/M4A 格式时，渠道插件会使用 `ffmpeg` 将其转码为 48 kHz Ogg/Opus。WhatsApp 通过 Baileys 发送，并附带 `ptt: true` 和 `audio/ogg; codecs=opus`。如果转换失败：飞书会回退以附加原始文件；WhatsApp 发送将失败，而不是发布不兼容的 PTT 载荷。
- **MiniMax / Xiaomi MiMo：** 默认为 MP3（MiniMax 的 `speech-2.8-hd` 为 32 kHz）；通过 `ffmpeg` 为语音笔记目标转码为 48 kHz Opus。
- **本地 CLI：** 使用配置的 `outputFormat`。语音笔记目标将转换为 Ogg/Opus，电话输出将转换为原始 16 kHz 单声道 PCM。
- **Google Gemini：** 返回原始 24 kHz PCM 格式。OpenClaw 将其封装为 WAV 格式用于附件，为语音笔记目标转码为 48 kHz Opus 格式，并为 Talk/电话直接返回 PCM。
- **Inworld：** MP3 附件，原生 `OGG_OPUS` 语音笔记，用于 Talk/电话的原始 `PCM` 22050 Hz。
- **xAI：** 默认为 MP3；`responseFormat` 可以是 `mp3|wav|pcm|mulaw|alaw`。使用 xAI 的批量 REST 端点——不使用流式 WebSocket TTS。不支持原生 Opus 语音笔记格式。
- **Microsoft：** 使用 `microsoft.outputFormat`（默认为 `audio-24khz-48kbitrate-mono-mp3`）。Telegram `sendVoice` 接受 OGG/MP3/M4A；如果需要保证 Opus 语音消息，请使用 OpenAI/ElevenLabs。如果配置的 Microsoft 格式失败，OpenClaw 将重试使用 MP3。
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    <ParamField path="auto" type='"off" | "always" | "inbound" | "tagged"'>
      自动 TTS 模式。`inbound` 仅在接收到语音消息后发送音频；`tagged` 仅在回复包含 `[[tts:...]]` 指令或 `[[tts:text]]` 块时发送音频。
    </ParamField>
    ...
    <ParamField path="provider" type="string"OpenClaw>
      语音提供商 ID。如果未设置，OpenClaw 将使用注册表自动选择顺序中第一个配置的提供商。旧的 `provider: "edge"` 会被 `openclaw doctor --fix` 重写为 `"microsoft"`。
    </ParamField>
    <ParamField path="persona" type="string">
      来自 `personas` 的活跃 persona ID。已规范化为小写。
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      按语音提供商 ID 键入的提供商自有设置。旧的直接块（`messages.tts.openai`、`.elevenlabs`、`.microsoft`、`.edge`）由 `openclaw doctor --fix` 重写；仅提交 `messages.tts.providers.<id>`。
    </ParamField>
...
<Accordion title="Azure 语音">
  <ParamField path="apiKey" type="string">
    Env: `AZURE_SPEECH_KEY`, `AZURE_SPEECH_API_KEY`, 或 `SPEECH_KEY`。
  </ParamField>
  <ParamField path="region" type="string">
    Azure 语音区域（例如 `eastus`）。Env: `AZURE_SPEECH_REGION` 或 `SPEECH_REGION`。
  </ParamField>
  <ParamField path="endpoint" type="string">
    可选的 Azure 语音端点覆盖（别名 `baseUrl`）。
  </ParamField>
  <ParamField path="speakerVoice" type="string">
    Azure 语音 ShortName。默认为 `en-US-JennyNeural`。旧别名：`voice`。
  </ParamField>
...
<Accordion title="ElevenLabs">
  ...
  <ParamField path="languageCode" type="string">
    2字母 ISO 639-1（例如 `en`、`de`）。
  </ParamField>
  <ParamField path="seed" type="number">
    整数 `0..4294967295`，用于尽力确保确定性。
  </ParamField>
...
<Accordion title="Google Gemini">
  ...
  <ParamField path="speakerName" type="string">
    当您的提示词使用命名说话者时，附加在口语文本之前的可选说话者标签。
  </ParamField>
  <ParamField path="promptTemplate" type='"audio-profile-v1"'>
    设置为 `audio-profile-v1`，以便将活跃的角色提示词字段封装在确定性的 Gemini TTS 提示词结构中。
  </ParamField>
  <ParamField path="personaPrompt" type="string">
    附加到模板“导演备注”之后的 Google 特定额外角色提示词文本。
  </ParamField>
  <ParamField path="baseUrl" type="string">
...
<Accordion title="Gradium">
  ...
  <Accordion title="Inworld">
    ...
    <ParamField path="modelId" type="string">默认值 `inworld-tts-1.5-max`。也可用： `inworld-tts-1.5-mini`、 `inworld-tts-1-max`、 `inworld-tts-1`。</ParamField>
...
<Accordion title="Microsoft（无 API 密钥）">
  ...
  <ParamField path="outputFormat" type="string">
    Microsoft 输出格式。默认 `audio-24khz-48kbitrate-mono-mp3`。并非所有格式都受打包的 Edge 支持传输支持。
  </ParamField>
...
<Accordion title="OpenAIOpenAI">
  ...
  <ParamField path="extraBody / extra_body" type="Record<string, unknown>">
    在生成的 OpenAI TTS 字段之后合并到 `/audio/speech`OpenAIOpenAI 请求正文中的额外 JSON 字段。将此用于需要提供商特定键（如 `lang`）的 OpenAI 兼容端点（如 Kokoro）；不安全原型键将被忽略。
  </ParamField>
...
<Accordion title="OpenRouterOpenRouter">
  <ParamField path="apiKey" type="string">
    Env: `OPENROUTER_API_KEY`. 可复用 `models.providers.openrouter.apiKey`。
  </ParamField>
  <ParamField path="baseUrl" type="string">
...
<Accordion title="xAI">
  ...
  <ParamField path="speakerVoiceId" type="string">
    默认值 `eve`。可用语音： `ara`、 `eve`、 `leo`、 `rex`、 `sal`、 `una`。旧版别名： `voiceId`。
  </ParamField>
  ...
  <Accordion title="Xiaomi小米 MiMo">
    ...
    <ParamField path="speakerVoice" type="string">默认 `mimo_default`。环境变量: `XIAOMI_TTS_VOICE`。旧别名: `voice`。</ParamField>
...
`tts`MatrixTelegramWhatsAppWhatsApp 工具将文本转换为语音并返回音频附件以用于回复发送。在飞书、Matrix、Telegram 和 WhatsApp 上，音频以语音消息而非文件附件的形式发送。当 `ffmpeg` 可用时，飞书和 WhatsApp 可以在此路径上转码非 Opus 的 TTS 输出。

WhatsApp 通过 Baileys 将音频作为 PTT 语音笔记（带有 `ptt: true` 的 WhatsAppBaileys`audio`）发送，并将可见文本与 PTT 音频**分开**发送，因为客户端在语音笔记上渲染字幕时行为不一致。

该工具接受可选的 `channel` 和 `timeoutMs` 字段；`timeoutMs` 是以毫秒为单位的每次调用提供商请求超时时间。每次调用的值会覆盖 `messages.tts.timeoutMs`；已配置的 TTS 超时会覆盖任何插件创建的提供商默认值。
...
- [Azure 语音 REST 文字转语音](https://learn.microsoft.com/azure/ai-services/speech-service/rest-text-to-speech)
- [Azure 语音提供商](/zh/providers/azure-speech)
...
- [火山引擎 TTS HTTP API](/zh/providers/volcengine#text-to-speech)
- [Xiaomi MiMo 语音合成](/zh/providers/xiaomi#text-to-speech)
