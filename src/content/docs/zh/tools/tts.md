...
- `providers.microsoft.outputFormat`: Microsoft 输出格式（例如 `audio-24khz-48kbitrate-mono-mp3`）。
  - 有关有效值，请参阅 Microsoft Speech 输出格式；并非所有格式都受内置的 Edge 支持传输支持。
...
如果回复超过 `maxLength` 且摘要功能关闭（或者没有用于摘要API的 API 密钥），将跳过音频并发送正常的文本回复。
...
Discord 注意：`/tts` 是 Discord 的内置命令，因此 OpenClaw 在那里注册 `/voice` 作为原生命令。文本 `/tts ...` 仍然有效。
...
- `off|always|inbound|tagged` 是每次会话的切换开关（`/tts on` 是 `/tts always` 的别名）。
...
- OpenAI 和 ElevenLabs API 失败现在包括已解析的提供商错误详情和请求 ID（当由提供商返回时），这些信息会显示在 TTS 错误/日志中。

## Agent 工具

`tts` 工具将文本转换为语音，并返回音频附件以供回复发送。当渠道是飞书、Matrix、Telegram 或 WhatsApp 时，音频将作为语音消息而不是文件附件发送。
