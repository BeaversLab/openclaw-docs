...
否。自动 TTS 默认为**关闭**。可以在配置中通过 `messages.tts.auto` 启用，或者在每次会话中通过 `/tts always`（别名：`/tts on`）启用。
...
- `/tts status` 包括针对最新尝试的回退可见性：
  - 成功回退：`Fallback: <primary> -> <used>` 加上 `Attempts: ...`
  - 故障：`Error: ...` 加上 `Attempts: ...`
  - 详细诊断：`Attempt details: provider:outcome(reasonCode) latency`
- OpenAI 和 ElevenLabs API 故障现在包含已解析的提供商错误详情和请求 ID（当提供商返回时），这些信息会在 TTS 错误/日志中显示。

## Agent 工具

`tts` 工具将文本转换为语音，并返回一个音频附件用于
回复发送。当渠道为 Feishu、Matrix、Telegram 或 WhatsApp 时，
音频将以语音消息而非文件附件的形式发送。
