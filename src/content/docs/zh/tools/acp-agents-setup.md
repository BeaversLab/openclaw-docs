...
如需概览、运维手册和概念，请参阅 [ACP agents](/zh/tools/acp-agents)。
...
运行时轮次使用 OpenClaw 代理/运行超时，包括 OpenClaw`/acp timeout`。
`sessions_spawn` 不接受单次调用的超时覆盖。更改此值后请重启网关。
...
有关 OpenClaw OpenClaw`tools.exec.mode`、Codex Guardian 批准与 ACPX harness 权限之间的更广泛比较，请参阅 [Permission modes](/zh/tools/permission-modes)。
...
控制当应该显示权限提示但没有可用的交互式 TTY 时发生的情况（对于 ACP 会话总是如此）。
...
<Warning>
OpenClaw 默认为 OpenClaw`permissionMode=approve-reads` 和 `nonInteractivePermissions=fail`。在非交互式 ACP 会话中，任何触发权限提示的写入或执行操作可能会因 `AcpRuntimeError: Permission prompt unavailable in non-interactive mode` 而失败。

如果需要限制权限，请将 `nonInteractivePermissions` 设置为 `deny`，以便会话能够优雅地降级而不是崩溃。
