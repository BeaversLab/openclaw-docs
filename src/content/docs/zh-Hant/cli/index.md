...
- [`infer`](/zh-Hant/cli/infer)
- [`memory`](/zh-Hant/cli/memory)
- [`wiki`](/zh-Hant/cli/wiki)
...
- [`tasks`](/zh-Hant/cli/tasks)
- [`flows`](/zh-Hant/cli/flows)
- [`dns`](/zh-Hant/cli/dns)
...
- [`qr`](/zh-Hant/cli/qr)
- [`plugins`](/zh-Hant/cli/plugins) (plugin commands)
- [`channels`](/zh-Hant/cli/channels)
- [`security`](/zh-Hant/cli/security)
- [`secrets`](/zh-Hant/cli/secrets)
- [`skills`](/zh-Hant/cli/skills)
- [`daemon`](/zh-Hant/cli/daemon) (legacy alias for gateway service commands)
- [`clawbot`](/zh-Hant/cli/clawbot) (legacy alias namespace)
- [`voicecall`](/zh-Hant/cli/voicecall) (plugin; if installed)
...
大部分的外掛程式變更需要重新啟動閘道。請參閱 [/plugin](/zh-Hant/tools/plugin)。
...
管理用於隔離代理程式執行的沙箱執行環境。請參閱 [/cli/sandbox](/zh-Hant/cli/sandbox)。
...
查詢公開目錄介面的頻道的自身 ID、對等 ID 和群組 ID。請參閱 [`openclaw directory`](/zh-Hant/cli/directory)。
...
從目前的 Gateway 設定產生行動裝置配對 QR 碼和設定代碼。請參閱 [`openclaw qr`](/zh-Hant/cli/qr)。
...
舊版別名命名空間。目前支援 `openclaw clawbot qr`，其對應至 [`openclaw qr`](/zh-Hant/cli/qr)。
...
Gmail Pub/Sub 掛勾設定 + 執行器。請參閱 [Gmail Pub/Sub](/zh-Hant/automation/cron-jobs#gmail-pubsub-integration)。
...
- `--thinking <level>` (針對所選模型的提供者設定檔進行驗證)
- `--verbose <on|off>`
...
如需完整行為、安全說明和範例，請參閱 [`acp`](/zh-Hant/cli/acp)。
...
- 詳情：請參閱 [Usage tracking](/zh-Hant/concepts/usage-tracking)。
...
在代理程式之間列出並管理 [背景任務](/zh-Hant/automation/tasks) 執行。
...
Gateway 服務管理指令的舊版別名。請參閱 [/cli/daemon](/zh-Hant/cli/daemon)。
...
如需備援行為和掃描策略，請參閱 [/concepts/models](/zh-Hant/concepts/models)。
...
`--all` 在設定驗證之前包含內建的提供者擁有的靜態目錄列。在符合的提供者憑證存在之前，列保持不可用狀態。
...
總是包含授權概述和授權儲存中配置檔案的 OAuth 過期狀態。
`--probe` 執行即時請求（可能會消耗 token 並觸發速率限制）。
探測列可以來自授權設定檔、環境認證，或 `models.json`。
預期的探測狀態包括 `ok`、`auth`、`rate_limit`、`billing`、`timeout`、
`format`、`unknown` 和 `no_model`。
當明確的 `auth.order.<provider>` 省略了儲存的設定檔時，探測會回報
`excluded_by_auth_order`，而不是無聲地嘗試該設定檔。
...
- `setup-token` 和 `paste-token` 是針對公開權杖認證方法的提供者之通用權杖指令。
- `setup-token` 需要互動式 TTY，並執行提供者的權杖認證方法。
- `paste-token` 會提示輸入權杖值，當省略 `--profile-id` 時，預設為認證設定檔 ID `<provider>:manual`。
- Anthropic `setup-token` / `paste-token` 仍可作為支援的 OpenClaw 權杖路徑使用，但若有可用，OpenClaw 現在傾向於重複使用 Claude CLI 和 `claude -p`。
...
管理排程工作 (Gateway RPC)。請參閱 [/automation/cron-jobs](/zh-Hant/automation/cron-jobs)。

子指令：

- `cron status [--json]`
- `cron list [--all] [--json]` (預設為表格輸出；使用 `--json` 取得原始資料)
- `cron add` (別名：`create`；需要 `--name` 以及 `--at` | `--every` | `--cron` 其中之一，以及 `--system-event` | `--message` 其中一個載荷)
- `cron edit <id>` (修補欄位)
...
`cron add|edit --model ...` 會針對該工作使用所選的允許模型。如果模型
不被允許，cron 會發出警告並改為回退至工作的代理程式/預設
模型選擇。設定的回退鏈仍然適用，但沒有明確個別工作
回退清單的單純模型覆寫，不再將代理程式主要模型附加為
隱藏的額外重試目標。
...
- `node` 從環境變數/設定解析閘道驗證（無 `--token`/`--password` 標誌）：`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`，然後是 `gateway.auth.*`。在本機模式下，節點主機會刻意忽略 `gateway.remote.*`；在 `gateway.mode=remote` 中，`gateway.remote.*` 會依照遠端優先順序規則參與。
- 節點主機驗證解析僅採用 `OPENCLAW_GATEWAY_*` 環境變數。

## 節點

`nodes` 與閘道通訊並以配對節點為目標。請參閱 [/nodes](/zh-Hant/nodes)。
...
瀏覽器控制 CLI (專用 Chrome/Brave/Edge/Chromium)。請參閱 [`openclaw browser`](/zh-Hant/cli/browser) 與 [瀏覽器工具](/zh-Hant/tools/browser)。
...
外掛提供的語音通話工具。僅在安裝並啟用了語音通話外掛時顯示。參見 [`openclaw voicecall`](/zh-Hant/cli/voicecall)。
