...
大多數外掛程式變更需要重新啟動閘道。請參閱 [/plugin](/zh-Hant/tools/plugin)。
...
管理用於隔離代理程式執行的沙盒執行環境。請參閱 [/cli/sandbox](/zh-Hant/cli/sandbox)。
...
查找公開目錄介面之通道的自己、同儕和群組 ID。請參閱 [`openclaw directory`](/zh-Hant/cli/directory)。
...
根據目前的 Gateway 配置生成行動裝置配對 QR 碼和設置代碼。請參閱 [`openclaw qr`](/zh-Hant/cli/qr)。
...
舊版別名命名空間。目前支援 `openclaw clawbot qr`，它會對應到 [`openclaw qr`](/zh-Hant/cli/qr)。
...
Gmail Pub/Sub 掛鉤設定 + 執行器。請參閱 [Gmail Pub/Sub](/zh-Hant/automation/cron-jobs#gmail-pubsub-integration)。
...
請參閱 [`acp`](/zh-Hant/cli/acp) 以了解完整行為、安全性備註和範例。
...
列出並管理各代理程式上的 [背景任務](/zh-Hant/automation/tasks) 執行。
...
閘道服務管理指令的舊版別名。請參閱 [/cli/daemon](/zh-Hant/cli/daemon)。
...
請參閱 [/concepts/models](/zh-Hant/concepts/models) 以了解回退行為和掃描策略。
...
`node` 會執行**無頭節點主機**或將其作為背景服務進行管理。請參閱 [`openclaw node`](/zh-Hant/cli/node)。
...
瀏覽器控制 CLI (專屬 Chrome/Brave/Edge/Chromium)。請參閱 [`openclaw browser`](/zh-Hant/cli/browser) 和 [瀏覽器工具](/zh-Hant/tools/browser)。
...
外掛程式提供的語音通話公用程式。僅在安裝並啟用語音通話外掛程式時顯示。請參閱 [`openclaw voicecall`](/zh-Hant/cli/voicecall)。
