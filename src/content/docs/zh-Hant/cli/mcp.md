...
當 OpenClaw 應自行主控程式設計演練會話並透過 ACP 路由該執行階段時，請使用 [`openclaw acp`](/zh-Hant/cli/acp)。
...
當 OpenClaw 應自行主控程式設計執行階段並將代理程式會話保留在 OpenClaw 內時，請改用 [`openclaw acp`](/zh-Hant/cli/acp)。
...
OpenClaw 會拒絕可能改變 stdio MCP 伺服器在首次 RPC 之前啟動方式的直譯器啟動環境變數鍵，即使它們出現在伺服器的 `env` 區塊中。被封鎖的鍵包括 `NODE_OPTIONS`、`PYTHONSTARTUP`、`PYTHONPATH`、`PERL5OPT`、`RUBYOPT`、`SHELLOPTS`、`PS4` 以及類似的執行時控制變數。啟動過程會因為配置錯誤而拒絕這些變數，以免它們注入隱含的序言、交換直譯器，或對 stdio 程序啟用偵錯工具。一般的憑證、代理程式和伺服器特定的環境變數（`GITHUB_TOKEN`、`HTTP_PROXY`、自訂 `*_API_KEY` 等）不受影響。
...
| --------------------- | ---------------------------------------------------------------------- |
| `url`                 | 遠端伺服器的 HTTP 或 HTTPS URL（必填）                                 |
| `transport`           | 設定為 `"streamable-http"` 以選擇此傳輸；若省略，OpenClaw 將使用 `sse` |
| `headers`             | HTTP 標頭的選用鍵值對映射（例如驗證權杖）                              |
| `connectionTimeoutMs` | 每個伺服器的連線逾時時間，以毫秒為單位（選用）                         |

OpenClaw 設定使用 `transport: "streamable-http"` 作為標準拼寫。透過 `openclaw mcp set` 儲存時，會接受 CLI 原生的 MCP `type: "http"` 值，並由 `openclaw doctor --fix` 在現有設定中修復，但內嵌 Pi 直接消耗的是 `transport`。
