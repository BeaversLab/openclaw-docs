...
若要調查緩慢的安裝、檢查、解除安裝或註冊表重新整理，請使用 `OPENCLAW_PLUGIN_LIFECYCLE_TRACE=1` 執行命令。追蹤會將階段計時寫入 stderr，並保持 JSON 輸出可解析。請參閱 [Debugging](/zh-Hant/help/debugging#plugin-lifecycle-trace)。

<Note>在 Nix 模式 (`OPENCLAW_NIX_MODE=1`) 中，外掛程式生命週期變異器已停用。請使用 Nix 來源進行此安裝，而不是 `plugins install`、`plugins update`、`plugins uninstall`、`plugins enable` 或 `plugins disable`；對於 nix-openclaw，請使用 agent-first 的 [Quick Start](https://github.com/openclaw/nix-openclaw#quick-start)。</Note>
...
`plugins init` 會建立一個使用 `defineToolPlugin` 的最小 TypeScript 工具外掛程式。`plugins build` 會匯入該項目，讀取其靜態工具中繼資料，寫入 `openclaw.plugin.json`，並保持 `package.json` `openclaw.extensions` 一致。`plugins validate` 會檢查生成的資訊清單、套件中繼資料和目前的項目匯出是否仍然一致。如需完整的編寫工作流程，請參閱 [Tool Plugins](/zh-Hant/plugins/tool-plugins)。
...
測試安裝時間安裝的維護人員可以使用受保護的環境變數覆寫自動外掛程式安裝來源。請參閱 [Plugin install overrides](/zh-Hant/plugins/install-overrides)。
...
<Note>ClawHub 是大多數外掛的主要發佈和探索平臺。Npm 仍是受支援的備選方案和直接安裝路徑。OpenClaw 擁有的 `@openclaw/*` 外掛套件再次發佈於 npm 上；請參閱 [npmjs.com/org/openclaw](https://www.npmjs.com/org/openclaw) 上的當前列表或 [外掛清單](/zh-Hant/plugins/plugin-inventory)。穩定版安裝使用 `latest`。 Beta 頻道安裝和更新優先使用 npm `beta` dist-tag（當該標籤 可用時），然後回退至 `latest`。</Note>

<AccordionGroup>
  <Accordion title="Config includes and invalid-config repair">
    如果您的 `plugins` 部分由單一檔案 `$include` 支援，`plugins install/update/enable/disable/uninstall` 將直接寫入該引入檔案並保持 `openclaw.json` 不變。根層級引入、引入陣列以及具有同層級覆寫的引入會以封閉式失敗（fail closed）處理，而不是扁平化。請參閱 [Config includes](/zh-Hant/gateway/configuration) 以了解支援的格式。

    如果安裝期間配置無效，`plugins install` 通常會以封閉式失敗處理並告知您先執行 `openclaw doctor --fix`。在 Gateway 啟動和熱重載期間，無效的外掛配置會像其他無效配置一樣以封閉式失敗處理；`openclaw doctor --fix` 可以隔離無效的外掛項目。唯一記載的安裝時例外情況，是針對明確選擇加入 `openclaw.install.allowInvalidConfigRecovery` 之外掛的狹隘捆綁外掛復原路徑。
  ...
  <Accordion title="--dangerously-force-unsafe-install">
    `--dangerously-force-unsafe-install` 是內建危險代碼掃描器誤報的緊急選項。即使內建掃描器報告 `critical` 發現，它也允許安裝繼續，但它**不會**繞過外掛程式 `before_install` hook 政策封鎖，也**不會**繞過掃描失敗。

    安裝掃描會忽略常見的測試檔案和目錄，例如 `tests/`、`__tests__/`、`*.test.*` 和 `*.spec.*`，以避免封鎖打包的測試模擬物件；宣告的外掛程式執行時進入點即使使用這些名稱之一，仍會被掃描。

    此 CLI 標誌適用於外掛程式安裝/更新流程。Gateway 支援的技能相依性安裝使用匹配的 `dangerouslyForceUnsafeInstall` 請求覆寫，而 `openclaw skills install` 仍然是一個單獨的 ClawHub 技能下載/安裝流程。

    如果您在 ClawHub 上發布的外掛程式被註冊表掃描隱藏或封鎖，請使用 [ClawHub publishing](/zh-Hant/clawhub/publishing) 中的發布者步驟。`--dangerously-force-unsafe-install` 只會影響您自己機器上的安裝；它不會要求 ClawHub 重新掃描外掛程式或將被封鎖的發布版本公開。
...
<Note>
`--force` 不支援搭配 `--link` 使用，因為連結安裝會重複使用來源路徑，而非複製到受管理的安裝目標。

在 npm 安裝上使用 `--pin`，可在將預設行為保持為未固定的同時，將解析後的確切規格 (`name@version`) 儲存於受管理的插件索引中。

</Note>

### 外掛程式索引

插件安裝中繼資料是由機器管理的狀態，而非使用者設定。安裝與更新會將其寫入至作用中 OpenClaw 狀態目錄下的 `plugins/installs.json`。其頂層 `installRecords` 映照是安裝中繼資料的持久來源，包含損壞或遺失插件清單的記錄。`plugins` 陣列則是衍生自清單的冷註冊快取。該檔案包含請勿編輯的警告，並由 `openclaw plugins update`、解除安裝、診斷以及冷插件註冊表使用。

當 OpenClaw 在配置中看到已發送的舊版 `plugins.installs` 記錄時，執行時讀取會將其視為兼容性輸入，而不會重寫 `openclaw.json`。顯式的插件寫入和 `openclaw doctor --fix` 會將這些記錄移至插件索引，並在允許配置寫入時刪除配置鍵；如果任一寫入失敗，則保留配置記錄以免安裝元數據丟失。
...
`uninstall` 會從 `plugins.entries`（持久化的插件索引）、插件允許/拒絕列表條目以及適用時的連結 `plugins.load.paths` 條目中移除插件記錄。除非設定了 `--keep-files`，否則當受控管理的安裝目錄位於 OpenClaw 的插件擴展根目錄下時，解除安裝也會移除該目錄。對於活躍的記憶體插件，記憶體插槽會重置為 `memory-core`。
...
更新適用於受控插件索引中已追蹤的插件安裝，以及 `hooks.internal.installs` 中已追蹤的 hook-pack 安裝。

<AccordionGroup>
  <Accordion title="解析插件 ID 與 npm 規範">
    當您傳遞插件 ID 時，OpenClaw 會重用該插件記錄的安裝規範。這意味著先前儲存的分發標籤（如 `@beta`）和確切的鎖定版本會在後續的 `update <id>` 執行中繼續被使用。

    對於 npm 安裝，您也可以傳遞帶有分發標籤或確切版本的明確 npm 套件規範。OpenClaw 會將該套件名稱解析回已追蹤的插件記錄，更新已安裝的插件，並記錄新的 npm 規範以供基於 ID 的未來更新使用。

    傳遞不帶版本或標籤的 npm 套件名稱也會解析回已追蹤的插件記錄。當插件被鎖定到確切版本，而您希望將其移回註冊表的默認發布線時，請使用此方法。

  </Accordion>
  <Accordion title="Beta channel updates">
    `openclaw plugins update` 會重用追蹤的插件規格，除非您傳遞了新的規格。`openclaw update` 此外也知道目前生效的 OpenClaw 更新頻道：在 beta 頻道上，預設系列的 npm 和 ClawHub 插件記錄會優先嘗試 `@beta`。如果不存在插件的 beta 版本，它們會回退到記錄的 default/latest 規格；如果 beta 套件存在但安裝驗證失敗，npm 插件也會回退。該回退會被回報為警告，且不會導致核心更新失敗。確切版本和明確標籤保持固定在該選擇器上。

  </Accordion>
  <Accordion title="Version checks and integrity drift">
    在進行實際 npm 更新之前，OpenClaw 會根據 npm 登錄表元數據檢查已安裝的套件版本。如果已安裝版本和記錄的構件身分已經符合解析目標，則會跳過更新，而不會下載、重新安裝或重寫 `openclaw.json`。

    當儲存的完整性雜湊存在且擷取的構件雜湊發生變更時，OpenClaw 會將其視為 npm 構件偏移。互動式 `openclaw plugins update` 指令會列印預期和實際的雜湊值，並在繼續之前要求確認。非互動式更新輔助程式會採取失敗關閉（fail closed）處理，除非呼叫者提供了明確的繼續策略。

  </Accordion>
  <Accordion title="--dangerously-force-unsafe-install on update">
    `--dangerously-force-unsafe-install` 也可用於 `plugins update`，作為一種應急覆寫（break-glass override），以解決插件更新期間內建危險代碼掃描的誤報。它仍然無法繞過插件 `before_install` 策略阻擋或掃描失敗阻擋，並且僅適用於插件更新，不適用於 hook-pack 更新。
  </Accordion>
</AccordionGroup>
...
Inspect 顯示身分、載入狀態、來源、清單功能、政策標誌、診斷、安裝中繼資料、套件功能，以及任何偵測到的 MCP 或 LSP 伺服器支援，預設不匯入外掛執行時期。新增 `--runtime` 以載入外掛模組並包含已註冊的 hooks、工具、命令、服務、Gateway 方法和 HTTP 路由。執行時期檢查會直接回報遺失的外掛相依性；安裝和修復作業保留在 `openclaw plugins install`、`openclaw plugins update` 和 `openclaw doctor --fix` 中。

外掛擁有的 CLI 命令通常安裝為根 `openclaw` 命令群組，但外掛也可能在核心父項（例如 `openclaw nodes`）下註冊巢狀命令。在 `inspect --runtime` 顯示 `cliCommands` 下的命令後，在列出的路徑執行它；例如，註冊了 `demo-git` 的外掛可以使用 `openclaw demo-git ping` 進行驗證。
...
請參閱 [Plugin shapes](/zh-Hant/plugins/architecture#plugin-shapes) 以深入了解功能模型。

<Note>`--json` 標誌會輸出適合於腳本和稽核的機器可讀報告。`inspect --all` 會呈現包含 shape、功能類型、相容性注意事項、套件功能和 hook 摘要欄位的整個範圍表格。`info` 是 `inspect` 的別名。</Note>
...
`doctor` 會回報外掛載入錯誤、清單/探索診斷、相容性注意事項，以及過時的外掛組態參考（例如遺失的外掛插槽）。當安裝樹狀結構和外掛組態乾淨時，它會列印 `No plugin issues detected.`。如果過時組態仍然存在但安裝樹狀結構在其他方面健全，摘要會說明這一點，而不是暗示外掛完全健全。

若設定的外掛程式存在於磁碟上但被載入器的路徑安全檢查封鎖，組態驗證會保留該外掛程式條目並將其回報為 `present but blocked`。請修正先前被封鎖之外掛程式的診斷問題（例如路徑擁有權或任何人皆可寫入的權限），而不要移除 `plugins.entries.<id>` 或 `plugins.allow` 組態。

對於模組形狀（module-shape）失敗（例如缺少 `register`/`activate` 匯出），請使用 `OPENCLAW_PLUGIN_LOAD_DEBUG=1` 重新執行，以在診斷輸出中包含簡潔的匯出形狀摘要。
...
使用 `plugins registry` 來檢查持續性登錄是否存在、為目前或過時。使用 `--refresh` 根據持續性外掛程式索引、組態原則以及 manifest/package 中繼資料來重建它。這是一條修復路徑，而非執行時期啟用路徑。

`openclaw doctor --fix` 也會修復與登錄相鄰的受管理 npm 偏差：如果受管理外掛程式 npm 專案下的孤兒或已復原 `@openclaw/*` 套件，或是舊版平面受管理 npm 根目錄遮蔽了配套外掛程式，doctor 會移除該過時套件並重建登錄，以便啟動程式針對配套 manifest 進行驗證。Doctor 也會將主機 `openclaw` 套件重新連結至宣告 `peerDependencies.openclaw` 的受管理 npm 外掛程式，因此類似 `openclaw/plugin-sdk/*` 的套件本機執行時期匯入能在更新或 npm 修復後正確解析。

<Warning>`OPENCLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY=1` 是用於登錄讀取失敗的已棄用緊急相容性開關。建議優先使用 `plugins registry --refresh` 或 `openclaw doctor --fix`；環境變數後備機制僅供遷移推出時的緊急啟動修復使用。</Warning>
...
Marketplace list 接受本機 marketplace 路徑、`marketplace.json` 路徑、類似 `owner/repo` 的 GitHub 簡寫、GitHub repo URL 或 git URL。`--json` 會列印已解析的來源標籤以及已解析的 marketplace manifest 與外掛程式項目。
