...
<Note>如果您有在目前的傳遞和儲存格式之前的 cron 工作，請執行 `openclaw doctor --fix`。Doctor 會將舊版的 cron 欄位正規化（`jobId`、`schedule.cron`、包含舊版 `threadId` 的頂層傳遞欄位、payload `provider` 傳遞別名），並在設定 `cron.webhook` 時，將簡單的 `notify: true` webhook 後備工作遷移至明確的 webhook 傳遞。</Note>
