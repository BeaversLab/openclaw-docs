...
- `--deep`：探測本地向量儲存就緒狀態、嵌入提供者就緒狀態，以及語意向量搜尋就緒狀態。單純的 `memory status` 保持快速，不執行即時嵌入或提供者探索工作；未知的向量儲存或語意向量狀態表示該指令未進行探測。QMD 詞彙 `searchMode: "search"` 會跳過語意向量探測和嵌入維護，即使使用了 `--deep`。
