...
- 使用**小型 Linux VPS**作為永遠在線的 Gateway 以降低成本。請參閱 [VPS 託管](/zh-Hant/vps)。
- 使用**專屬硬體**（Mac mini 或 Linux 主機），如果您需要完全的控制以及用於瀏覽器自動化的**住宅 IP**。許多網站會阻擋資料中心 IP，因此本機瀏覽通常效果更好。
- **混合式：**將 Gateway 保留在便宜的 VPS 上，並在需要瀏覽器/UI 自動化時將您的 Mac 作為**節點**連接。請參閱 [節點](/zh-Hant/nodes) 和 [Gateway 遠端](/zh-Hant/gateway/remote)。
...
若要真正實現隨時運作，請考慮使用專用的 Mac mini 或小型 VPS。請參閱 [VPS hosting](/zh-Hant/vps)。
