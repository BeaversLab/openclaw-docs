...
- 閘道：[手冊](/en/gateway) + [設定](/en/gateway/configuration)。
  - 協定：[閘道協定](/en/gateway/protocol) (節點 + 控制平面)。
...
Android 直接連線至閘道 WebSocket (預設 `ws://<host>:18789`) 並使用裝置配對 (`role: node`)。
...
Android Chat 分頁支援階段作業選擇 (預設 `main`，加上其他現有的階段作業)：
...
注意：節點從閘道 HTTP 伺服器載入畫布 (與 `gateway.port` 相同的連接埠，預設 `18789`)。
...
- `canvas.eval`、`canvas.snapshot`、`canvas.navigate`（使用 `{"url":""}` 或 `{"url":"/"}` 返回預設鷹架）。`canvas.snapshot` 返回 `{ format, base64 }`（預設 `format="jpeg"`）。
...
請參閱 [相機節點](/en/nodes/camera) 以了解參數和 CLI 輔助工具。
...
Android 可以將裝置通知作為事件轉發到閘道。您可以使用幾個控制項來設定轉發哪些通知以及何時轉發。

| 鍵                               | 類型           | 說明                                                               |
| -------------------------------- | -------------- | ------------------------------------------------------------------ |
| `notifications.allowPackages`    | string[]       | 僅轉發來自這些套件名稱的通知。如果設定此項，則會忽略所有其他套件。 |
| `notifications.denyPackages`     | string[]       | 永不轉發來自這些套件名稱的通知。在 `allowPackages` 之後套用。      |
| `notifications.quietHours.start` | string (HH:mm) | 免打擾時段的開始時間（本地裝置時間）。在此期間將會隱藏通知。       |
| `notifications.quietHours.end`   | 字串 (HH:mm)   | 免打擾時間視窗的結束時間。                                         |
| `notifications.rateLimit`        | 數字           | 每個應用程式每分鐘轉發通知的最大數量。超出的通知將被丟棄。         |

通知選擇器對轉發的通知事件也採用更安全的行為，以防止意外轉發敏感的系統通知。
...
<Note>通知轉發需要 Android 通知監聽權限。應用程式會在設定期間提示您授予此權限。</Note>
