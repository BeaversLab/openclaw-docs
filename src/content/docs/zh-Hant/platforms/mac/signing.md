...
- 呼叫 [`scripts/codesign-mac-app.sh`](https://github.com/openclaw/openclaw/blob/main/scripts/codesign-mac-app.sh) 來簽署主二進位檔案和 app bundle，以便 macOS 將每次重新構建視為同一個已簽署的 bundle，並保留 TCC 權限（通知、輔助使用、螢幕錄製、麥克風、語音）。若要獲得穩定的權限，請使用真實的簽署身分；臨時簽署是可選且不穩定的（請參閱 [macOS 權限](/zh-Hant/platforms/mac/permissions)）。
...
- **打包預設為 Node 24**：該腳本會執行 TS 構建和 Control UI 構建。為了相容性，目前 `22.19+` 的 Node 22 LTS 仍然受到支援。
...
關於」分頁會讀取這些鍵值以顯示版本、建置日期、git 提交，以及是否為偵錯版本（透過 `#if DEBUG`）。在程式碼變更後，請執行打包器以重新整理這些值。

## 為什麼

TCC 權限與套件識別碼 _及_ 程式碼簽章綁定。具有變動 UUID 的未簽署偵錯版本會導致 macOS 在每次重建後遺失授權。對二進位檔案進行簽署（預設為 ad-hoc）並保持固定的套件 ID/路徑（`dist/OpenClaw.app`）可在建置之間保留授權，這與 VibeTunnel 的方法一致。
