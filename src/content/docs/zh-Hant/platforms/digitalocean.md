...
如果您想要 $0/月 的選項並且不介意 ARM + 特定供應商的設定，請參閱 [Oracle Cloud 指南](/en/platforms/oracle)。
...
- Hetzner：價格/效能比高（請參閱 [Hetzner 指南](/en/install/hetzner)）
- Oracle Cloud：可以 $0/月 使用，但比較挑剔且僅限 ARM（請參閱 [Oracle 指南](/en/platforms/oracle)）
...
- Serve 將 Gateway 保持在僅限本機存取，並透過 Tailscale 身分標頭驗證 Control UI/WebSocket 流量（無 token 驗證假設受信任的 gateway 主機；HTTP API 不使用這些 Tailscale 標頭，而是遵循 gateway 的正常 HTTP 驗證模式）。
- 若改為要求明確的共用金鑰憑證，請設定 `gateway.auth.allowTailscale: false` 並使用 `gateway.auth.mode: "token"` 或 `"password"`。
...
- `~/.openclaw/` — `openclaw.json`、每個代理的 `auth-profiles.json`、頻道/供應商狀態以及會話資料
...
若要查看完整的設定指南，請參閱 [Oracle Cloud](/en/platforms/oracle)。關於註冊提示和註冊過程的故障排除，請參閱此 [社群指南](https://gist.github.com/rssnyder/51e3cfedd730e7dd5f4a816143b25dbd)。
...
- [Tailscale](/en/gateway/tailscale) — 安全遠端存取
