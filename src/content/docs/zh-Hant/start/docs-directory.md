---
summary: "最常用的 OpenClaw 文件精選連結。"
read_when:
  - You want quick access to key docs pages
title: "文件目錄"
---

# 文件目錄

<Note>此頁面是一個精選索引。如果您是新手，請從 [Getting Started](/en/start/getting-started) 開始。 如需文件的完整地圖，請參閱 [Docs hubs](/en/start/hubs)。</Note>

## 從這裡開始

- [文件中心 (連結至所有頁面)](/en/start/hubs)
- [說明](/en/help)
- [設定](/en/gateway/configuration)
- [設定範例](/en/gateway/configuration-examples)
- [斜線指令](/en/tools/slash-commands)
- [多代理程式路由](/en/concepts/multi-agent)
- [更新與還原](/en/install/updating)
- [配對 (DM 和節點)](/en/channels/pairing)
- [Nix 模式](/en/install/nix)
- [OpenClaw 助手設定](/en/start/openclaw)
- [技能](/en/tools/skills)
- [技能設定](/en/tools/skills-config)
- [工作區範本](/en/reference/templates/AGENTS)
- [RPC 配接器](/en/reference/rpc)
- [Gateway 操作手冊](/en/gateway)
- [節點 (iOS 和 Android)](/en/nodes)
- [Web 介面 (控制 UI)](/en/web)
- [探索與傳輸](/en/gateway/discovery)
- [遠端存取](/en/gateway/remote)

## 提供者與 UX

- [WebChat](/en/web/webchat)
- [控制 UI (瀏覽器)](/en/web/control-ui)
- [Telegram](/en/channels/telegram)
- [Discord](/en/channels/discord)
- [Mattermost (外掛程式)](/en/channels/mattermost)
- [BlueBubbles (iMessage)](/en/channels/bluebubbles)
- [iMessage (舊版)](/en/channels/imessage)
- [群組](/en/channels/groups)
- [WhatsApp 群組訊息](/en/channels/group-messages)
- [媒體圖片](/en/nodes/images)
- [媒體音訊](/en/nodes/audio)

## 隨行應用程式

- [macOS 應用程式](/en/platforms/macos)
- [iOS 應用程式](/en/platforms/ios)
- [Android 應用程式](/en/platforms/android)
- [Windows (WSL2)](/en/platforms/windows)
- [Linux 應用程式](/en/platforms/linux)

## 操作與安全

- [Sessions](/en/concepts/session)
- [Cron jobs](/en/automation/cron-jobs)
- [Webhooks](/en/automation/webhook)
- [Gmail hooks (Pub/Sub)](/en/automation/gmail-pubsub)
- [Security](/en/gateway/security)
- [Troubleshooting](/en/gateway/troubleshooting)
