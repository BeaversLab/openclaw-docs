...
<Steps>
  ...
  <Step title="頻道">
    ...
    - [iMessage](/zh-Hant/channels/imessage)：`imsg` CLI 路徑 + Messages DB 存取權；當 Gateway 在非 Mac 環境執行時，請使用 SSH 包裝器
...
<AccordionGroup>
  ...
  <Accordion title="xAI (Grok) OAuth">
    符合資格的 SuperGrok 或 X Premium 帳號的瀏覽器登入。這是大多數
    使用者推薦的 xAI 路徑。OpenClaw 會儲存產生的認證
    設定檔供 Grok 模型、Grok `web_search`、`x_search` 和 `code_execution` 使用。
  </Accordion>
  ...
  <Accordion title="xAI (Grok) API key">
    提示輸入 `XAI_API_KEY` 並將 xAI 設定為模型提供者。當您想要使用 xAI Console API 金鑰
    而非訂閱 OAuth 時使用此選項。
  </Accordion>
  ...
  <Accordion title="StepFun">
    系統會針對中國或全球端點上的 StepFun 標準版或 Step Plan 自動寫入配置。
  ...
  <Accordion title="自訂提供者">
    ...
    - **立即貼上 API 金鑰** (純文字)
    - **使用機密參照** (環境變數參照或已設定的提供者參照，並進行預檢驗證)

    非互動式旗標：
    - `--auth-choice custom-api-key`
    - `--custom-base-url`
    - `--custom-model-id`
    - `--custom-api-key` (選用；若未提供則回退為 `CUSTOM_API_KEY`)
...
- 當入站是從提供者驗證選項啟動時，模型挑選器會自動偏好該提供者。對於 Volcengine 和 BytePlus，相同的偏好設定也會符合其編碼方案變體 (`volcengine-plan/*`、
  `byteplus-plan/*`)。
...
<Note>
Headless 和伺服器提示：在具備瀏覽器的機器上完成 OAuth，然後將該代理程式的 `auth-profiles.json`（例如
...
- `tools.profile`（若未設定，本地上線預設為 `"coding"`；現有的明確值將被保留）
- `gateway.*`（mode、bind、auth、tailscale）
- `session.dmScope`（若未設定，本地上線預設此值為 `per-channel-peer`；現有的明確值將被保留）
...
- `skills.install.nodeManager`
  ...
  - 手動設定仍然可以稍後設定 `skills.install.nodeManager: "yarn"`。
- `wizard.lastRunAt`
