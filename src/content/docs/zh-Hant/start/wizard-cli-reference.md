...
若需簡短指南，請參閱 [Onboarding (CLI)](/zh-Hant/start/wizard)。
...
<Steps>
  ...
  <Step title="頻道">
    ...
    - [Signal](/zh-Hant/channels/signal)：選用性 `signal-cli` 安裝 + 帳號設定
    - [iMessage](/zh-Hant/channels/imessage)：`imsg` CLI 路徑 + 訊息資料庫存取；當 Gateway 在 Mac 以外執行時，請使用 SSH 包裝器
...
<AccordionGroup>
  ...
  <Accordion title="xAI (Grok) OAuth">
    符合資格的 SuperGrok 或 X Premium 帳戶的瀏覽器登入。這是
    大多數使用者推薦的 xAI 路徑。OpenClaw 將結果 auth
    profile 儲存給 Grok 模型、`x_search` 和 `code_execution`。
  </Accordion>
  ...
  <Accordion title="xAI (Grok) API 金鑰">
    提示輸入 `XAI_API_KEY` 並將 xAI 設定為模型提供者。當您想要
    使用 xAI Console API 金鑰而不是訂閱 OAuth 時使用此選項。
  </Accordion>
  ...
  <Accordion title="StepFun">
    針對中國或全球端點上的 StepFun 標準版或 Step Plan 自動寫入配置。
  ...
  <Accordion title="Ollama (Cloud and local open models)">
    ...
    主機支援模式會提示輸入基礎 URL (預設 `http://127.0.0.1:11434`)，探索可用的模型，並建議預設值。
...
- `--secret-input-mode ref` 會啟用參考模式，而非純文字金鑰儲存。在互動式設定中，您可以選擇：
  ...
  - 設定的提供商參考（`file` 或 `exec`），搭配提供商別名 + ID
...
<Note>
Headless 和伺服器提示：在具有瀏覽器的機器上完成 OAuth，然後將該代理的 `auth-profiles.json`（例如 `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`，或對應的 `$OPENCLAW_STATE_DIR/...` 路徑）複製到閘道主機。`credentials/oauth.json` 僅作為舊版導入來源。
</Note>
...
- 入中心：[入 (CLI)](/zh-Hant/start/wizard)
