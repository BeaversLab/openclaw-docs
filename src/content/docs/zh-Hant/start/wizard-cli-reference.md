...
如需簡易指南，請參閱 [Onboarding (CLI)](/en/start/wizard)。
...
<Steps>
  ...
  <Step title="Model 與認證">
    - 完整的選項矩陣請參閱 [Auth and model options](#auth-and-model-options)。
  </Step>
  <Step title="工作區">
    - 預設為 `~/.openclaw/workspace`（可設定）。
    - 播種首次執行啟動程序所需的工作區檔案。
    - 工作區佈局：[Agent workspace](/en/concepts/agent-workspace)。
  </Step>
  <Step title="Gateway">
  ...
  <Step title="Channels">
    ...
    - [Google Chat](/en/channels/googlechat)：服務帳戶 JSON + webhook audience
    - [Mattermost](/en/channels/mattermost)：bot token + base URL
    - [Signal](/en/channels/signal)：選用的 `signal-cli` 安裝 + 帳戶設定
...
<AccordionGroup>
  ...
  <Accordion title="MiniMax">
    配置會自動寫入。託管預設為 `MiniMax-M2.7`；API 金鑰設定使用
    `minimax/...`，而 OAuth 設定使用 `minimax-portal/...`。
    更多細節：[MiniMax](/en/providers/minimax)。
  </Accordion>
  <Accordion title="StepFun">
    會針對中國或全球端點上的 StepFun 標準版或 Step Plan 自動寫入設定。
  ...
  <Accordion title="Moonshot and Kimi Coding">
    Moonshot (Kimi K2) 和 Kimi Coding 的設定會自動寫入。
  ...
  <Accordion title="Custom provider">
    ...
    - **立即貼上 API 金鑰** (純文字)
    - **使用密鑰參照** (環境變數參照或已設定的供應商參照，並附帶預檢驗證)
...
- 當入職是從供應商驗證選擇開始時，模型選擇器會自動偏重該供應商。對於 Volcengine 和 BytePlus，同樣的偏好設定也會符合其 coding-plan 變體 (`volcengine-plan/*`, `byteplus-plan/*`)。
...
- 在非互動模式下，`--secret-input-mode ref` 僅支援環境變數。
  ...
  - 內聯金鑰旗標（例如 `--openai-api-key`）要求必須設定該環境變數；否則 onboarding 會快速失敗。
  - 對於自訂供應商，非互動式 `ref` 模式會將 `models.providers.<id>.apiKey` 儲存為 `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`。
  - 在該自訂供應商情況下，`--custom-api-key` 要求必須設定 `CUSTOM_API_KEY`；否則 onboarding 會快速失敗。
...
<Note>
Headless 和伺服器提示：在有瀏覽器的機器上完成 OAuth，然後將該 agent 的 `auth-profiles.json`（例如
...
- `tools.profile`（若未設定，local onboarding 預設為 `"coding"`；現有的明確值會予以保留）
- `gateway.*`（mode、bind、auth、tailscale）
- `session.dmScope`（若未設定，local onboarding 預設此值為 `per-channel-peer`；現有的明確值會予以保留）
...
- `skills.install.nodeManager`
  ...
  - 手動配置仍然可以稍後設定 `skills.install.nodeManager: "yarn"`。
- `wizard.lastRunAt`
...
WhatsApp 憑證存放在 `~/.openclaw/credentials/whatsapp/<accountId>/` 之下。
會話儲存在 `~/.openclaw/agents/<agentId>/sessions/` 之下。
...
- 在配置中寫入 `channels.signal.cliPath`
- JVM 版本需要 Java 21
- 優先使用原生版本（如果可用）
