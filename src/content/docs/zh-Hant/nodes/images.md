...
- 媒體理解（如果透過 `tools.media.*` 或共用的 `tools.media.models` 配置）會在模板化之前執行，並且可以將 `[Image]`、`[Audio]` 和 `[Video]` 區塊插入 `Body` 中。
  ...
  - 如果啟用的主要圖像模型本身已原生支援視覺功能，OpenClaw 會跳過 `[Image]` 摘要區塊，並將原始圖像傳遞給模型。
- 預設情況下，僅處理第一個相符的圖像/音訊/影片附件；設定 `tools.media.<cap>.attachments` 以處理多個附件。

## 限制與錯誤

**輸出傳送上限 (WhatsApp web 傳送)**

- 圖像：重新壓縮後上限為 `channels.whatsapp.mediaMaxMb` (預設值：50 MB)。
- 音訊/語音/影片：16 MB 上限；文件：100 MB 上限。
- 過大或無法讀取的媒體 → 記錄檔中會出現清楚的錯誤，並且跳過回覆。

**媒體理解上限 (轉錄/描述)**

- 圖像預設值：10 MB (`tools.media.image.maxBytes`)。
- 音訊預設值：20 MB (`tools.media.audio.maxBytes`)。
- 影片預設值：50 MB (`tools.media.video.maxBytes`)。
- 過大的媒體會跳過理解步驟，但仍會使用原始內容繼續回覆。
