...
<Steps>
  ...
  <Step title="Provider auth">
    - 已設定並支援此功能的 `models.providers.*` 項目會在內建的後援順序之前嘗試。
    - 即使不是內建的廠商外掛，具有支援圖像能力的模型之僅圖像設定提供者也會自動註冊媒體理解功能。
    - 若明確選取，例如透過 `agents.defaults.imageModel` 或 `openclaw infer image describe --model ollama/<vision-model>`，即可使用 Ollama 的圖像理解功能。
...
- `minimax`、`minimax-cn`、`minimax-portal` 和 `minimax-portal-cn` 圖像理解來自外掛擁有的 `MiniMax-VL-01` 媒體提供者。
- 即使舊版 MiniMax M2.x 聊天元數據聲稱有圖像輸入，自動圖像路由仍繼續使用 `MiniMax-VL-01`。
...
- 當提供者 API 不可用時，CLI 後援 (`whisper-cli`、`whisper`、`gemini`) 非常有用。
- `parakeet-mlx` 說明：使用 `--output-dir` 時，當輸出格式為 `txt` (或未指定)，OpenClaw 會讀取 `<output-dir>/<media-basename>.txt`；非 `txt` 格式則後援至 stdout。
...
<AccordionGroup>
  <Accordion title="檔案附件提取行為">
    - 提取的檔案文字會在附加至媒體提示之前被包裝為 **不受信的外部內容**。
    - 注入的區塊使用明確的邊界標記，如 `<<<EXTERNAL_UNTRUSTED_CONTENT id="...">>>` / `<<<END_EXTERNAL_UNTRUSTED_CONTENT id="...">>>`，並包含 `Source: External` 元數據行。
    - 此附件提取路徑刻意省略了冗長的 `SECURITY NOTICE:` 橫幅，以避免媒體提示過於冗長；邊界標記和元數據仍然保留。
    - 如果檔案沒有可提取的文字，OpenClaw 會注入 `[No extractable text]`。
    - 如果 PDF 在此路徑中後援至渲染的頁面影像，媒體提示會保留佔位符 `[PDF content rendered to images; images not forwarded to model]`，因為此附件提取步驟轉發的是文字區塊，而非渲染的 PDF 影像。
