...
3. **供應商驗證**
   - 會優先嘗試已設定且支援音訊的 `models.providers.*` 項目
   - 內建的後援順序：OpenAI → Groq → xAI → Deepgram → Google → SenseAudio → ElevenLabs → Mistral

自 2026-05-22 起，媒體理解不再支援 Gemini CLI 自動偵測。Google 正將 Gemini CLI 使用者轉移至 Antigravity CLI；音訊應使用本機或供應商轉錄，而圖片/影片 CLI 後援應移至 Antigravity CLI (`agy`)。
...
- 供應商驗證遵循標準模型驗證順序（auth profiles、env vars、`models.providers.*.apiKey`）。
...
- 當使用 `provider: "senseaudio"` 時，SenseAudio 會使用 `SENSEAUDIO_API_KEY`。
- SenseAudio 設定詳情：[SenseAudio](/zh-Hant/providers/senseaudio)。
- 音訊供應商可以透過 `tools.media.audio` 覆寫 `baseUrl`、`headers` 和 `providerOptions`。
- 預設大小上限為 20MB (`tools.media.audio.maxBytes`)。對於該模型，超過大小的音訊將被跳過，並嘗試下一個項目。
- 小於 1024 位元組的微小/空白音訊檔案會在供應商/CLI 轉錄前被跳過。
- 音訊的預設 `maxChars` 為 **未設定**（完整轉錄）。請設定 `tools.media.audio.maxChars` 或個別項目的 `maxChars` 以修剪輸出。
...
- 轉錄文字可作為 `{{Transcript}}` 提供給模板使用。
- `tools.media.audio.echoTranscript` 預設為關閉；啟用它可在代理程式處理之前將轉錄確認傳回來源聊天。
- `tools.media.audio.echoFormat` 可自訂回顯文字（佔位符：`{transcript}`）。
- CLI 標準輸出有上限（5MB）；請保持 CLI 輸出簡潔。
- CLI `args` 應使用 `{{MediaPath}}` 作為本機音訊檔案路徑。執行 `openclaw doctor --fix` 以從較舊的 `audio.transcription.command` 設定中遷移已棄用的 `{input}` 佔位符。
...
當為群組聊天設定 `requireMention: true` 時，OpenClaw 現在會在檢查提及**之前**轉錄音訊。這允許即使語音訊息包含提及也能被處理。

**運作方式：**

1. 如果語音訊息沒有文字內容，且群組要求提及，OpenClaw 會執行「預檢」轉錄。
2. 轉錄文字會檢查提及模式（例如 `@BotName`、emoji 觸發器）。
...
- 設定 `channels.telegram.groups.<chatId>.topics.<threadId>.disableAudioPreflight` 以覆蓋每個主題的設定（`true` 表示跳過，`false` 表示強制啟用）。
- 預設值為 `false`（當符合提及閘道條件時啟用預檢）。

**範例：** 使用者在設定有 `requireMention: true` 的 Telegram 群組中發送一段語音訊息，說道「嘿 @Claude，天氣怎麼樣？」。這段語音訊息會被轉錄，偵測到提及，然後代理程式會回覆。

## 注意事項

- 範圍規則使用首次匹配原則。`chatType` 會被正規化為 `direct`、`group` 或 `room`。
- 請確保您的 CLI 以 0 退出並列印純文字；JSON 需要透過 `jq -r .text` 進行調整。
- 對於 `parakeet-mlx`，如果您傳遞 `--output-dir`，OpenClaw 會在 `--output-format` 為 `txt`（或省略）時讀取 `<output-dir>/<media-basename>.txt`；非 `txt` 輸出格式則會回退到 stdout 解析。
- 請將逾時設定得合理（`timeoutSeconds`，預設為 60 秒），以避免阻塞回覆佇列。
