...
若要查看包含 `before_tool_call`、`before_agent_reply`、`before_install` 以及所有其他 plugin hooks 的完整 plugin hook 參考資料，請參閱 [Plugin Architecture](/zh-Hant/plugins/architecture#provider-runtime-hooks)。
...
- [插件架構](/zh-Hant/plugins/architecture#provider-runtime-hooks) — 完整的插件 hook 參考資料
