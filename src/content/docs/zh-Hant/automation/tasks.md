...
<AccordionGroup>
  ...
  <Accordion title="tasks maintenance">
    ...
    使用此項來預覽或套用任務、Task Flow 狀態以及過時 cron 執行階段會話登錄檔行的對帳、清理標記和修剪。

    對帳具備執行時感知能力：

    - ACP/子代理任務會檢查其支援的子會話。
    - 若子會話具備重新啟動復原標記，則子代理任務會被標記為遺失，而不會被視為可復原的支援會話。
    - Cron 任務會檢查 cron 執行時是否仍擁有該工作，然後在回退到 `lost` 之前，從持久的 cron 執行紀錄/工作狀態中復原終端狀態。僅 Gateway 程序對記憶體中的 cron 使用中工作集具有權威性；離線 CLI 稽核會使用持久的歷史紀錄，但不會僅因該本地 Set 為空就將 cron 任務標記為遺失。
    - 具備執行身分的 CLI 任務會檢查擁有的即時執行內容，而不僅是子會話或聊天會話列。

    完成清理也具備執行時感知能力：

    - 子代理完成會在繼續公告清理之前，盡力為子會話關閉追蹤的瀏覽器分頁/程序。
    - 隔離的 cron 完成會在執行完全拆除之前，盡力為 cron 會話關閉追蹤的瀏覽器分頁/程序。
    - 隔離的 cron 傳遞會在需要時等待後代子代理的後續追蹤，並抑制過時的父層確認文字，而不是將其公告。
    - 子代理完成傳遞僅使用子項最新的可見助理文字。Tool/toolResult 輸出不會被提升至子項結果文字。終端失敗的執行會公告失敗狀態，而不會重新播放擷取的回覆文字。
    - 清理失敗不會遮蔽真正的任務結果。

    套用維護時，OpenClaw 也會移除超過 7 天的過時 `cron:<jobId>:run:<uuid>` 會話登錄檔列，同時保留目前執行中 cron 工作的列，並保持非 cron 會話列不變。
