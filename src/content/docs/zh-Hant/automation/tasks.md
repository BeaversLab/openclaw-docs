...
<Note>尋找排程功能？請參閱 [Automation and tasks](/zh-Hant/automation) 以選擇合適的機制。此頁面是背景工作的活動記錄帳，而非排程器。</Note>
...
**直接傳遞** - 如果任務具有通道目標 (`requesterOrigin`)，完成訊息會直接發送到該通道 (Telegram、Discord、Slack 等)。對於子代理完成，OpenClaw 也會在可用時保留綁定的執行緒/主題路由，並且在放棄直接傳遞之前，可以從請求者會話的儲存路由 (`lastChannel` / `lastTo` / `lastAccountId`) 中填入遺失的 `to` / 帳號。
...
- [Automation & Tasks](/zh-Hant/automation) - 自動化機制總覽
