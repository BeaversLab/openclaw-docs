...
OpenClaw 應用層級分類器邏輯位於 `src/infra/net/ssrf.ts` 和 `src/shared/net/ip.ts` 中。相關的對等掛勾為 `BLOCKED_HOSTNAMES`、`BLOCKED_IPV4_SPECIAL_USE_RANGES`、`BLOCKED_IPV6_SPECIAL_USE_RANGES`、`RFC2544_BENCHMARK_PREFIX`，以及針對 NAT64、6to4、Teredo、ISATAP 和 IPv4 對映形式的內嵌 IPv4 哨兵處理。這些檔案在維護外部代理政策時是有用的參考，但 OpenClaw 不會在您的代理中自動匯出或強制執行這些規則。
