...
`defaultAccount` 控制當出站 API 未指定 `accountId` 時使用哪個帳戶。
`accounts.<id>.tts` 使用與 `messages.tts` 相同的形狀，並在全域 TTS 配置上進行深度合併，因此多機器人 Feishu 設置可以在全域保留共享的提供者憑證，同時僅針對每個帳戶覆寫語音、模型、人格或自動模式。

### 訊息限制

- `textChunkLimit` - 出站文本區塊大小（預設：`2000` 字元）
...
設定 `streaming: false` 以在一則訊息中傳送完整回覆。`blockStreaming` 預設為關閉；僅當您希望在最終回覆之前刷新已完成的助理區塊時才啟用它。

### 配額最佳化

使用兩個可選旗標來減少 Feishu/Lark API 呼叫次數：

- `typingIndicator`（預設 `true`）：設定 `false` 以跳過正在輸入反應呼叫
...
飛書/Lark 支援 DM 和群組執行緒訊息的 ACP。飛書/Lark ACP 是由文字指令驅動的——沒有原生的斜線指令選單，因此請直接在對話中使用 `/acp ...` 訊息。
...
`--thread here` 適用於 DM 和飛書/Lark 執行緒訊息。綁定對話中的後續訊息會直接路由到該 ACP 會話。

### 多代理路由

使用 `bindings` 將飛書/Lark DM 或群組路由到不同的代理程式。
...
參閱 [取得群組/使用者 ID](#get-groupuser-ids) 以取得查詢提示。
...
| `channels.feishu.groups.<chat_id>.requireMention` | 個別群組的 @提及覆寫；在允許清單模式下，明確的 ID 也會納入該群組 | 繼承             |
...
| `channels.feishu.blockStreaming`                  | 區塊完成回覆串流                                                 | `false`          |
...
傳入的飛書/Lark 音訊訊息會被正規化為媒體佔位符，而不是原始的 `file_key` JSON。當配置了 `tools.media.audio` 時，OpenClaw 會下載語音註記資源並在代理輪次之前執行共享的音訊轉錄，因此代理會接收口語轉錄內容。如果飛書直接在音訊負載中包含轉錄文本，則會直接使用該文本而不會再進行一次 ASR 呼叫。如果沒有音訊轉錄提供者，代理仍然會收到 `<media:audio>` 佔位符以及已儲存的附件，而不是原始的飛書資源負載。
...
原生的飛書/Lark 音訊氣泡使用飛書 `audio` 訊息類型，並且需要
Ogg/Opus 上傳媒體 (`file_type: "opus"`)。現有的 `.opus` 和 `.ogg` 媒體
會直接作為原生音訊發送。MP3/WAV/M4A 和其他可能的音訊格式
只有在回覆請求語音
傳遞 (`audioAsVoice` / 訊息工具 `asVoice`，包括 TTS 語音留言
回覆) 時，才會透過 `ffmpeg` 轉碼為 48kHz Ogg/Opus。一般的 MP3 附件保持為常規檔案。如果 `ffmpeg` 缺失或
轉換失敗，OpenClaw 將回退為檔案附件並記錄原因。
...
對於 `groupSessionScope: "group_topic"` 和 `"group_topic_sender"`，原生的
飛書/Lark 主題群組使用事件 `thread_id` (`omt_*`) 作為標準
主題會話金鑰。如果原生主題啟動事件省略了 `thread_id`，OpenClaw
會在路由輪次之前從飛書獲取它。OpenClaw 轉換為串列的正常群組回覆繼續使用回覆根訊息 ID (`om_*`)，以便
第一輪和後續輪次保持在同一個會話中。
...
- [群組](/zh-Hant/channels/groups) - 群聊行為和提及閘門
- [頻道路由](/zh-Hant/channels/channel-routing) - 訊息的會話路由
- [安全性](/zh-Hant/gateway/security) - 存取模型和強化
