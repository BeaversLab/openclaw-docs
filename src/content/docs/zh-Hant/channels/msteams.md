...
狀態：支援文字 + 私訊附件；頻道/群組檔案傳送需要 `sharePointSiteId` + Graph 權限（請參閱 [在群組聊天中傳送檔案](#sending-files-in-group-chats)）。投票會透過 Adaptive Cards 傳送。訊息動作會針對檔案優先的傳送顯示明確的 `upload-file`。
...
[`@microsoft/teams.cli`](https://www.npmjs.com/package/@microsoft/teams.cli) 會在單一指令中處理 Bot 註冊、資訊清單建立及認證產生。
...
對於正式環境部署，請考慮使用 [聯合驗證](/zh-Hant/channels/msteams#federated-authentication-certificate-plus-managed-identity) (憑證或受控識別)，而不是用戶端密碼。
...
Microsoft 會在 Teams 主動傳送訊息文件的 [建立交談](https://learn.microsoft.com/en-us/microsoftteams/platform/bots/how-to/conversations/send-proactive-messages?tabs=dotnet#create-the-conversation) 區段中發佈全域主動 Bot Connector 端點。請盡可能使用傳入活動的 `serviceUrl`；如果您需要全域主動端點，請使用 Microsoft 的表格。
...
- Session 遵循標準代理程式格式（請參閱 [/concepts/session](/zh-Hant/concepts/session)）：
...
- 投票會由閘道記錄在 OpenClaw 外掛程式狀態 SQLite 資料庫中的 `state/openclaw.sqlite` 下。
- 現有的 `msteams-polls.json` 檔案會在 MSTeams 外掛程式啟動時匯入一次。
- 閘道必須保持上線才能記錄投票。
- 投票尚不會自動發佈結果摘要，且目前尚無支援的投票結果 CLI。

## 簡報卡片

使用 `message` 工具、CLI 或普通回覆傳送，將語意展示載荷傳送給 Teams 使用者或對話。OpenClaw 會從通用展示合約將其轉譯為 Teams 自適應卡片。

`presentation` 參數接受語意區塊。當提供 `presentation` 時，訊息文字是可選的。按鈕會轉譯為自適應卡片的送出或 URL 動作。選單在 Teams 轉譯器中尚未原生支援，因此 OpenClaw 會在傳送前將其降級為可讀文字。
...
有關目標格式的詳細資訊，請參閱下方的 [Target formats](#target-formats)。
...
<Note>如果沒有 `user:` 前綴，名稱預設為群組或團隊解析。依顯示名稱指定人員目標時，請務必使用 `user:`。</Note>

## Proactive messaging

- 主動訊息僅在使用者互動**之後**才可能發生，因為我們會在該時間點儲存對話參照。
...
- Team key = `/team/` 之後的路徑區段 (URL 解碼，例如 `19:Bk4j...@thread.tacv2`；舊租用戶可能會顯示 `@thread.skype`，這也是有效的)
- Channel key = `/channel/` 之後的路徑區段 (URL 解碼)
- **忽略**用於 OpenClaw 路由的 `groupId` 查詢參數。它是 Microsoft Entra 群組 ID，而非傳入 Teams 活動中使用的 Bot Framework 對話 ID。
...
- **頻道中未顯示圖片：** 缺少 Graph 權限或管理員同意。請重新安裝 Teams 應用程式，並完全關閉/重新開啟 Teams。
- **頻道中沒有回應：** 預設需要提及；設定 `channels.msteams.requireMention=false` 或針對每個團隊/頻道進行設定。
- **版本不符 (Teams 仍顯示舊的資訊清單)：** 移除並重新新增應用程式，然後完全關閉 Teams 以重新整理。
- **從 webhook 收到 401 未授權：** 在沒有 Azure JWT 的情況下手動測試時屬預期情況 - 這表示端點可連線但驗證失敗。請使用 Azure Web Chat 進行正確測試。

### 資訊清單上傳錯誤

- **「圖示檔案不能為空白」：** 資訊清單參照的圖示檔案為 0 位元組。請建立有效的 PNG 圖示 (`outline.png` 為 32x32，`color.png` 為 192x192)。
- **「webApplicationInfo.Id 已在使用中」：** 應用程式仍安裝在另一個團隊/聊天中。請先找到並將其解除安裝，或等待 5-10 分鐘以進行傳播。
- **上傳時發生「發生問題」：** 請改透過 [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) 上傳，開啟瀏覽器開發者工具 (F12) → Network 索引標籤，並檢查回應主體以找出實際錯誤。
- **側載失敗：** 請嘗試「將應用程式上傳至您組織的應用程式目錄」，而不是「上傳自訂應用程式」 - 這通常可以繞過側載限制。
