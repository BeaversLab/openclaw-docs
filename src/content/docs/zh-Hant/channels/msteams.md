...
注意：群組聊天預設被封鎖 (`channels.msteams.groupPolicy: "allowlist"`)。若要允許群組回覆，請設定 `channels.msteams.groupAllowFrom` (或使用 `groupPolicy: "open"` 以允許任何成員，提及閘道控制)。
...
預設情況下，允許 Microsoft Teams 寫入由 `/config set|unset` 觸發的配置更新 (需要 `commands.config: true`)。
...
- 預設：`channels.msteams.dmPolicy = "pairing"`。未知的發送者將被忽略，直到被核准為止。
- `channels.msteams.allowFrom` 應使用穩定的 AAD 物件 ID。
- UPNs/顯示名稱是可變的；預設會停用直接比對，且僅在啟用 `channels.msteams.dangerouslyAllowNameMatching: true` 時才開啟。
- 當認證允許時，精靈可以透過 Microsoft Graph 將名稱解析為 ID。

**群組存取權**

- 預設值：`channels.msteams.groupPolicy = "allowlist"`（除非您新增 `groupAllowFrom`，否則為封鎖狀態）。當未設定時，請使用 `channels.defaults.groupPolicy` 來覆寫預設值。
- `channels.msteams.groupAllowFrom` 控制哪些寄件者可以在群組聊天/頻道中觸發（會回退至 `channels.msteams.allowFrom`）。
...
- 啟動時，OpenClaw 會將團隊/頻道和使用者允許清單名稱解析為 ID（當 Graph 權限允許時）
  並記錄對應關係；未解析的團隊/頻道名稱會保持輸入的樣子，但除非啟用 `channels.msteams.dangerouslyAllowNameMatching: true`，否則預設會在路由中被忽略。
...
2. 複製 **Microsoft App ID** (Microsoft 應用程式 ID) → 這就是您的 `appId`
3. 點擊 **Manage Password** (管理密碼) → 前往應用程式註冊
4. 在 **Certificates & secrets** (憑證與機密) 底下 → **New client secret** (新增用戶端密碼) → 複製 **Value** (值) → 這就是您的 `appPassword`
...
Teams 無法連接至 `localhost`。本機開發請使用通道：
...
您可以使用 [Teams 開發人員入口網站](https://dev.teams.microsoft.com/apps)，而不需手動建立 ZIP 資訊清單：
...
6. **執行 gateway**
   - 當外掛程式安裝完成且存在含有憑證的 `msteams` 設定時，Teams 頻道會自動啟動。

## 歷史記錄內容

- `channels.msteams.historyLimit` 控制有多少最近的頻道/群組訊息會被封裝到提示中。
- 會退回至 `messages.groupChat.historyLimit`。設定 `0` 以停用（預設為 50）。
- DM 歷史記錄可以透過 `channels.msteams.dmHistoryLimit`（使用者輪次）來限制。個別使用者覆寫：`channels.msteams.dms["<user_id>"].historyLimit`。
...
- `bots[].supportsFiles: true` 是個人範圍中處理檔案所必需的。
...
3. **重新壓縮** 包含圖示的宣告 (`manifest.json`, `outline.png`, `color.png`)
4. 上傳新的 zip 檔案：
...
**結論：** RSC 用於即時監聽；Graph API 用於歷史存取。若要補上離線時錯過的訊息，您需要搭配 `ChannelMessage.Read.All` 的 Graph API（需要管理員同意）。
...
**使用者提及的額外權限：** 對於對話中的使用者，使用者 @提及功能可立即使用。然而，如果您想要動態搜尋並提及 **不在目前對話中** 的使用者，請新增 `User.Read.All` （應用程式）權限並授與管理員同意。
...
關鍵設定（有關共用頻道模式請參閱 `/gateway/configuration`）：
...
- `channels.msteams.allowFrom`：DM 允許清單（建議使用 AAD 物件 ID）。精靈在設定期間會在可使用 Graph 時將名稱解析為 ID。
- `channels.msteams.dangerouslyAllowNameMatching`：緊急開關，用以重新啟用可變更的 UPN/顯示名稱比對及直接團隊/頻道名稱路由。
- `channels.msteams.textChunkLimit`：輸出文字區塊大小。
- `channels.msteams.chunkMode`：`length`（預設值）或 `newline`，以在長度區塊劃分前依空白行（段落邊界）進行分割。
...
- `channels.msteams.requireMention`：在頻道/群組中需要 @mention（預設為 true）。
...
- `channels.msteams.teams.<teamId>.toolsBySender`：預設的各團隊各傳送者工具原則覆寫（支援 `"*"` 萬用字元）。
...
- `toolsBySender` 索引鍵應使用明確的前綴：
  `id:`、`e164:`、`username:`、`name:` (舊版無前綴索引鍵仍僅對應至 `id:`)。
- `channels.msteams.sharePointSiteId`: 用於群組聊天/頻道中檔案上傳的 SharePoint 網站 ID (請參閱 [在群組聊天中傳送檔案](#sending-files-in-group-chats))。

## 路由與工作階段

- 工作階段索引鍵遵循標準代理程式格式 (請參閱 [/concepts/session](/en/concepts/session))：
  - 直接訊息 共用主要工作階段 (`agent:<agentId>:<mainKey>`)。
  - 頻道/群組訊息使用對話 ID：
...
**問題所在：** Teams API 並不公開頻道使用哪種 UI 樣式。如果您使用錯誤的 `replyStyle`：

- 在執行緒樣式頻道中使用 `thread` → 回覆會顯示得尷尬地巢狀化
- 在貼文樣式頻道中使用 `top-level` → 回覆會顯示為獨立的頂層貼文，而非在執行緒內
...
如果沒有 Graph 權限，帶有圖片的頻道訊息將只會以純文字形式接收 (機器人無法存取圖片內容)。
預設情況下，OpenClaw 只會從 Microsoft/Teams 主機名稱下載媒體。使用 `channels.msteams.mediaAllowHosts` 覆寫 (使用 `["*"]` 允許任何主機)。
授權標頭僅附加到 `channels.msteams.mediaAuthAllowHosts` 中的主機 (預設為 Graph + Bot Framework 主機)。請將此列表保持嚴格 (避免多租戶後綴)。

## 在群組聊天中傳送檔案

機器人可以使用 FileConsentCard 流程 (內建) 在 DM 中傳送檔案。但是，**在群組聊天/頻道中傳送檔案** 需要額外設定：
...
| 權限                                    | 分享行為                                    |
| --------------------------------------- | ------------------------------------------- |
| 僅限 `Sites.ReadWrite.All`              | 全組織分享連結 (組織中的任何人均可存取)     |
| `Sites.ReadWrite.All` + `Chat.Read.All` | 每個使用者的分享連結 (只有聊天成員可以存取) |

按使用者分享更安全，因為只有聊天參與者可以存取該檔案。如果缺少 `Chat.Read.All` 權限，機器人會退回到全組織分享。
...
上傳的檔案會儲存在已設定 SharePoint 網站預設文件庫中的 `/OpenClawShared/` 資料夾內。
...
請參閱 [Adaptive Cards 文件](https://adaptivecards.io/) 以了解卡片架構和範例。有關目標格式的詳細資訊，請參閱下方的 [Target formats](#target-formats)。
...
注意：如果不帶 `user:` 前綴，名稱預設為解析群組/團隊。當以顯示名稱為目標尋找人員時，請務必使用 `user:`。

## 主動訊息

- 只有在使用者互動**之後**才能發送主動訊息，因為我們會在該時點儲存對話參考。
...
- **頻道中未顯示圖片：** 遺失 Graph 權限或管理員同意。請重新安裝 Teams 應用程式，並完全結束/重新開啟 Teams。
- **頻道中無回應：** 預設需要提及；請設定 `channels.msteams.requireMention=false` 或針對每個團隊/頻道進行設定。
- **版本不符 (Teams 仍顯示舊的資訊清單)：** 移除並重新新增應用程式，然後完全結束 Teams 以重新整理。
- **從 webhook 收到 401 未經授權：** 在沒有 Azure JWT 的情況下手動測試時屬預期情況 - 表示端點可連線但驗證失敗。請使用 Azure Web Chat 進行正確測試。

### 資訊清單上傳錯誤

- **"Icon file cannot be empty":** 資訊清單引用的圖示檔案大小為 0 位元組。請建立有效的 PNG 圖示 (`outline.png` 為 32x32，`color.png` 為 192x192)。
- **"webApplicationInfo.Id already in use":** 應用程式仍安裝在另一個團隊/聊天中。請先找到並將其解除安裝，或等待 5-10 分鐘以供傳播。
- **上傳時出現 "Something went wrong":** 請改透過 [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) 上傳，開啟瀏覽器開發者工具 (F12) → Network 索引標籤，並檢查回應主體以取得實際錯誤。
- **側載失敗:** 請嘗試使用「Upload an app to your org's app catalog」而非「Upload a custom app」——這通常能避開側載限制。

### RSC 權限無法運作

1. 驗證 `webApplicationInfo.id` 與您機器人的 App ID 完全相符
