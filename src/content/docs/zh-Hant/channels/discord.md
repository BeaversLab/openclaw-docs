...
```
  ...
  <Step title="設定 OpenClaw 並配對">
    ...
    <Tabs>
      <Tab title="詢問您的代理程式">
        在任何現有頻道（例如 Telegram）上與您的 OpenClaw 代理程式聊天並告知它。如果 Discord 是您的第一個頻道，請改用 CLI / config 分頁。

        > "我已經在 config 中設定了我的 Discord 機器人 token。請使用 User ID `<user_id>` 和 Server ID `<server_id>` 完成 Discord 設定。"
      </Tab>
...
```
        ...
        支援純文字 `token` 值。對於跨 env/file/exec 提供者的 `channels.discord.token`，也支援 SecretRef 值。請參閱[機密管理](/en/gateway/secrets)。
...
<AccordionGroup>
  <Accordion title="回覆標籤與原生回覆">
    ...
    注意：`off` 會停用隱含回覆串接。明確的 `[[reply_to_*]]` 標籤仍會被遵從。
    `first` 總是將隱含的原生回覆參照附加至該輪次的第一個傳出 Discord 訊息。
    `batched` 僅在傳入輪次為多個訊息的防抖批次時，才會附加 Discord 的隱含原生回覆參照。當您主要希望對模稜兩可的爆發性聊天而非每個單一訊息輪次使用原生回覆時，這非常有用。

    訊息 ID 會呈現在內容/歷史中，以便 Agent 能鎖定特定訊息。

  </Accordion>

  <Accordion title="Live stream preview">
    OpenClaw 可以透過發送暫時訊息並在文字送達時編輯它來串流草稿回覆。

    - `channels.discord.streaming` 控制預覽串流 (`off` | `partial` | `block` | `progress`，預設：`off`)。
    - 預設保持 `off`，因為 Discord 預覽編輯可能會很快觸及速率限制，特別是當多個機器人或閘道共用同一個帳號或公會流量時。
...
```
  ...
  <Accordion title="歷史記錄、上下文和執行緒行為">
    ...
    - 父執行緒元資料可用於父會話連結
    - 除非存在執行緒特定的條目，否則執行緒組態繼承父頻道組態

    頻道主題會被作為**不信任**上下文（而非系統提示詞）注入。
    回覆和引用訊息的上下文目前保持接收時的狀態。
    Discord 允許清單主要限制誰可以觸發代理程式，而不是完整的補充上下文編輯邊界。

  </Accordion>

  <Accordion title="Thread-bound sessions for subagents">
    Discord 可以將串綁定到會話目標，以便該串中的後續訊息繼續路由至同一個會話（包括子代理會話）。

    指令：

    - `/focus <target>` 將目前/新的串綁定到子代理/會話目標
    - `/unfocus` 移除目前的串綁定
    - `/agents` 顯示正在執行和綁定狀態
    - `/session idle <duration|off>` 檢查/更新針對焦點綁定的閒置自動取消聚焦
    - `/session max-age <duration|off>` 檢查/更新針對焦點綁定的硬性最長存留時間
...
```
    ...
    - 如果針對帳戶停用了串綁定，則無法使用 `/focus` 和相關的串綁定操作。

    參閱[子代理](/en/tools/subagents)、[ACP 代理](/en/tools/acp-agents)和[設定參考](/en/gateway/configuration-reference)。

  </Accordion>

  <Accordion title="持續 ACP 頻道綁定">
    對於穩定的「始終在線」ACP 工作區，請設定針對 Discord 對話的頂層類型化 ACP 綁定。
...
```
    ...
    - `/acp spawn codex --bind here` 會將目前的 Discord 頻道或執行緒就地綁定，並將未來的訊息路由至同一個 ACP 會話。
    - 這仍然可能意味著「啟動新的 Codex ACP 會話」，但它不會自行建立新的 Discord 執行緒。現有的頻道保持為聊天介面。
    - Codex 可能仍會在其自己的 `cwd` 或磁碟上的後端工作區中執行。該工作區是運行時狀態，而非 Discord 執行緒。
    - 執行緒訊息可以繼承父頻道的 ACP 綁定。
    - 在已綁定的頻道或執行緒中，`/new` 和 `/reset` 會就地重設同一個 ACP 會話。
    - 臨時執行緒綁定仍然有效，並且在啟用時可以覆寫目標解析。
    - 僅當 OpenClaw 需要透過 `--thread auto|here` 建立/綁定子執行緒時，才需要 `spawnAcpSessions`。對於目前頻道中的 `/acp spawn ... --bind here` 則不需要。
  ...
  <Accordion title="反應通知">
    ...
    反應事件會轉換為系統事件，並附加至路由的 Discord 會話。
...
```
  ...
  <Accordion title="Gateway proxy">
    透過 `channels.discord.proxy` 的 HTTP(S) 代理伺服器來路由 Discord gateway WebSocket 流量以及啟動時的 REST 查詢 (應用程式 ID + 允許清單解析)。
...
```
    ...
    - 僅當 `channels.discord.dangerouslyAllowNameMatching: true` 時，才會依名稱/slug 匹配成員顯示名稱
  ...
  <Accordion title="Presence configuration">
    Presence 更新會在您設定狀態或活動欄位時，或當您啟用自動 Presence 時套用。
...
```
    ...
    - 0: 遊戲中 (Playing)
    - 1: 串流中 (Streaming) (需要 `activityUrl`)
    - 2: 聆聽中 (Listening)
    - 3: 觀看中 (Watching)
    - 4: 自訂 (Custom) (將活動文字用作狀態狀態；emoji 為選填)
    - 5: 競爭中 (Competing)

    自動 Presence 範例 (執行階段健康狀態訊號)：
...
```
    ...
    自動 Presence 會將執行階段可用性對應到 Discord 狀態：健康 => 上線, 降級或未知 => 閒置, 耗盡或無法使用 => 請勿打擾。選用文字覆寫：
  ...
  <Accordion title="Discord 中的審批">
    Discord 支援在私訊 (DM) 中透過按鈕處理審批，並可選擇在原始頻道中發布審批提示。
    ...
    當 `enabled` 未設定或為 `"auto"` 且至少可解析出一個審批者 (來自 `execApprovals.approvers` 或 `commands.ownerAllowFrom`) 時，Discord 會自動啟用原生執行審批。Discord 不會從頻道 `allowFrom`、舊版 `dm.allowFrom` 或私訊 `defaultTo` 推斷執行審批者。若要明確停用 Discord 作為原生審批客戶端，請設定 `enabled: false`。

    當 `target` 為 `channel` 或 `both` 時，審批提示會在頻道中顯示。僅已解析的審批者可以使用按鈕；其他使用者會收到暫時性拒絕回應。審批提示包含指令文字，因此僅在信任的頻道中啟用頻道傳遞。如果無法從 session key 推導出頻道 ID，OpenClaw 會回退至私訊傳遞。

    Discord 也會呈現其他聊天頻道使用的共用審批按鈕。原生 Discord 配接器主要增加了審批者私訊路由和頻道分發功能。
    當這些按鈕存在時，它們是主要的審批使用者體驗 (UX)；當工具結果顯示
    聊天審批無法使用或手動審批是唯一途徑時，OpenClaw
    應僅包含手動 `/approve` 指令。

    此處理器的 Gateway 驗證使用與其他 Gateway 客戶端相同的共用憑證解析合約：

    - 優先環境的本機驗證 (`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD` 然後是 `gateway.auth.*`)
    - 在本機模式下，僅當未設定 `gateway.auth.*` 時，`gateway.remote.*` 才能用作回退；已設定但未解析的本機 SecretRefs 會失敗關閉
    - 適用時透過 `gateway.remote.*` 支援遠端模式
    - URL 覆寫是覆寫安全的：CLI 覆寫不會重用隱式憑證，而環境變數覆寫僅使用環境變數憑證

    審批解析行為：

    - 前綴為 `plugin:` 的 ID 透過 `plugin.approval.resolve` 解析。
    - 其他 ID 透過 `exec.approval.resolve` 解析。
    - Discord 不會在此進行額外的 exec-to-plugin 回退跳躍；ID
      前綴決定呼叫哪種 gateway 方法。

    執行審批預設在 30 分鐘後過期。如果審批因未知的
    審批 ID 而失敗，請驗證審批者解析、功能啟用狀態，
    以及傳送的審批 ID 類型是否符合待處理請求。
...
OpenClaw 使用 Discord 組件 v2 進行執行核准和跨上下文標記。Discord 訊息動作也可以接受 `components` 以實作自訂 UI（進階；需要透過 discord 工具建構組件承載），而舊版的 `embeds` 仍然可用，但不建議使用。
...
- `voice.tts` 會覆蓋 `messages.tts`，僅用於語音播放。
- 語音文字記錄輪次是依據 Discord `allowFrom`（或 `dm.allowFrom`）推導出擁有者身分；非擁有者的發言者無法存取僅限擁有者的工具（例如 `gateway` 和 `cron`）。
- 預設啟用語音功能；設定 `channels.discord.voice.enabled=false` 以將其停用。
...
- 如果接收日誌反覆顯示 `DecryptionFailed(UnencryptedWhenPassthroughDisabled)`，這可能是 [discord.js #11419](https://github.com/discordjs/discord.js/issues/11419) 中追蹤到的上游 `@discordjs/voice` 接收錯誤。

## 語音訊息

Discord 語音訊息會顯示波形預覽，並需要 OGG/Opus 音訊加上元資料。OpenClaw 會自動產生波形，但它需要 Gateway 主機上有 `ffmpeg` 和 `ffprobe` 才能檢查和轉換音訊檔案。
...
```
  ...
  <Accordion title="設為不需提及但仍被封鎖">
    常見原因：

    - 沒有匹配的伺服器/頻道允許清單的 `groupPolicy="allowlist"`
    - `requireMention` 配置位置錯誤（必須在 `channels.discord.guilds` 或頻道項目下）
...
```
    ...
    針對緩慢的監聽器設定使用 `eventQueue.listenerTimeout`，並且僅當您想要為佇列中的代理輪次設定獨立的安全閥時才使用 `inboundWorker.runTimeoutMs`。
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - 如果在自動重新加入後仍持續失敗，請收集日誌並與 [discord.js #11419](https://github.com/discordjs/discord.js/issues/11419) 進行比對
...
- media/retry: `mediaMaxMb`, `retry`
  - `mediaMaxMb` 限制傳出 Discord 上傳（預設：`100MB`）
- actions: `actions.*`
...
- 將機器人權杖視為機密（在受監控環境中建議使用 `DISCORD_BOT_TOKEN`）。
- 授予最小權限的 Discord 權限。
