...
`/model` 和 `/models` 斜線指令會開啟一個互動式模型選擇器，其中包含提供者、模型和相容執行階段的下拉式選單，以及一個提交步驟。`/models add` 已被棄用，現在會傳回棄用訊息，而不是從聊天中註冊模型。選擇器的回應是暫時性的，只有觸發的使用者可以使用它。Discord 選單僅限 25 個選項，因此當您希望選擇器僅顯示針對特定提供者（例如 `openai` 或 `vllm`）動態探索到的模型時，請將 `provider/*` 項目新增至 `agents.defaults.models`。
...
- 在即時模式中，`voice.realtime.provider`、`voice.realtime.model` 和 `voice.realtime.speakerVoice` 會設定即時音訊工作階段。對於 OpenAI Realtime 2 搭配 Codex brain，請使用 `voice.realtime.model: "gpt-realtime-2"` 和 `voice.model: "openai/gpt-5.5"`。
