...
您需要建立一個包含機器人的新應用程式，將機器人新增到您的伺服器，並將其與 OpenClaw 配對。我們建議將您的機器人新增到您自己的私人伺服器。如果您還沒有私人伺服器，請[先建立一個](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server)（選擇 **Create My Own > For me and my friends**）。

<Steps>
  <Step title="Create a Discord application and bot">
    前往 [Discord Developer Portal](https://discord.com/developers/applications) 並點擊 **New Application**。將其命名為類似「OpenClaw」的名稱。

    點擊側邊欄的 **Bot**。將 **Username** 設定為您稱呼 OpenClaw 代理程式的任何名稱。
...
```
        ...
        支援純文字 `token` 值。在 env/file/exec 提供者之間也支援 `channels.discord.token` 的 SecretRef 值。請參閱 [Secrets Management](/en/gateway/secrets)。
...
```
  ...
  <Accordion title="Thread-bound sessions for subagents">
    ...
    - `/focus <target>` 將目前/新的執行緒綁定到子代理程式/會話目標
    - `/unfocus` 移除目前的執行緒綁定
    - `/agents` 顯示正在執行的工作和綁定狀態
...
```
    ...
    參閱 [Sub-agents](/en/tools/subagents)、[ACP Agents](/en/tools/acp-agents) 和 [Configuration Reference](/en/gateway/configuration-reference)。
...
```
    ...
    - `/acp spawn codex --bind here` 會將目前的 Discord 頻道或執行緒就地綁定，並確保後續訊息皆路由至同一個 ACP 工作階段。
    - 這可能仍代表「啟動新的 Codex ACP 工作階段」，但它本身並不會建立新的 Discord 執行緒。現有的頻道會維持為對話介面。
    - Codex 仍可能在其自身的 `cwd` 或磁碟上的後端工作區中執行。該工作區是執行時狀態，而非 Discord 執行緒。
    - 執行緒訊息可以繼承父頻道的 ACP 綁定。
    - 在已綁定的頻道或執行緒中，`/new` 和 `/reset` 會就地重設同一個 ACP 工作階段。
    - 臨時執行緒綁定仍然有效，並可在作用中時覆寫目標解析。
    - 僅當 OpenClaw 需透過 `--thread auto|here` 建立/綁定子執行緒時，才需要 `spawnAcpSessions`。對於目前頻道中的 `/acp spawn ... --bind here` 則不需要。
...
```
  ...
  <Accordion title="Discord 中的核准">
    Discord 支援在 DM 中透過按鈕進行核准處理，並可選擇在原始頻道中發佈核准提示。
    ...
    當 `enabled` 未設定或為 `"auto"`，且至少可以解析出一個核准者（來自 `execApprovals.approvers` 或 `commands.ownerAllowFrom`）時，Discord 會自動啟用原生執行核准。Discord 不會從頻道 `allowFrom`、舊版 `dm.allowFrom` 或直接訊息 `defaultTo` 推斷執行核准者。設定 `enabled: false` 可明確停用 Discord 作為原生核准用戶端。

    當 `target` 為 `channel` 或 `both` 時，核准提示會在頻道中顯示。只有已解析的核准者可以使用這些按鈕；其他使用者會收到暫時性的拒絕訊息。核准提示包含指令文字，因此僅在受信任的頻道中啟用頻道傳送。如果無法從 session key 推導出頻道 ID，OpenClaw 將回退至 DM 傳送。

    Discord 也會呈現其他聊天頻道使用的共用核准按鈕。原生 Discord 配接器主要增加了核准者 DM 路由和頻道分散功能。
    當這些按鈕存在時，它們是主要的核准體驗（UX）；只有在工具結果指出
    聊天核准不可用或手動核准是唯一途徑時，OpenClow
    才應包含手動 `/approve` 指令。

    此處理程式的 Gateway 驗證使用與其他 Gateway 用戶端相同的共用憑證解析契約：

    - env-first 本地驗證 (`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD` 然後 `gateway.auth.*`)
    - 在本地模式下，僅當 `gateway.auth.*` 未設定時，才能將 `gateway.remote.*` 作為回退；已設定但未解析的本地 SecretRefs 將會失敗封閉
    - 適用時透過 `gateway.remote.*` 進行遠端模式支援
    - URL 覆寫是覆寫安全的：CLI 覆寫不會重複使用隱式憑證，而 env 覆寫僅使用 env 憑證

    核准解析行為：

    - 前綴為 `plugin:` 的 ID 透過 `plugin.approval.resolve` 解析。
    - 其他 ID 透過 `exec.approval.resolve` 解析。
    - Discord 不會在此處執行額外的 exec-to-plugin 回退跳轉；
      ID 前綴決定了它呼叫哪個 gateway 方法。

    執行核准預設在 30 分鐘後過期。如果核准因未知
    的核准 ID 而失敗，請驗證核准者解析、功能啟用狀態，
    以及已傳送的核准 ID 種類是否符合待處理請求。

    相關文件：[執行核准](/en/tools/exec-approvals)
...
OpenClaw 使用 Discord 組件 v2 進行執行核准和跨上下文標記。Discord 訊息動作也可以接受 `components` 以實現自訂 UI（進階；需要透過 discord 工具建構組件載荷），而舊版 `embeds` 仍然可用但不建議使用。
...
- 語音逐字稿回合從 Discord `allowFrom`（或 `dm.allowFrom`）衍生擁有者狀態；非擁有者發言者無法存取僅限擁有者的工具（例如 `gateway` 和 `cron`）。
...
Discord 語音訊息會顯示波形預覽，並且需要 OGG/Opus 音訊加上中繼資料。OpenClaw 會自動生成波形，但它需要在閘道主機上安裝 `ffmpeg` 和 `ffprobe` 才能檢查和轉換音訊檔案。
...
<AccordionGroup>
  ...
  <Accordion title="Guild messages blocked unexpectedly">
    ...
    - 如果存在 guild `channels` 對映，則僅允許列出的頻道
...
```
  ...
  <Accordion title="Require mention false but still blocked">
    ...
    - `requireMention` 在錯誤的位置設定（必須位於 `channels.discord.guilds` 或頻道條目下）
...
```
    ...
    對於緩慢的監聽器設定，使用 `eventQueue.listenerTimeout`，並僅在您希望為佇列中的代理回合設定獨立的安全閥時，才使用 `inboundWorker.runTimeoutMs`。
  ...
  <Accordion title="Permissions audit mismatches">
    ...
    如果您使用 slug 鍵，執行時期比對可能仍然有效，但 probe 無法完全驗證權限。
...
- [組態參考 - Discord](/en/gateway/configuration-reference#discord)
...
- 傳入工作程式: `inboundWorker.runTimeoutMs`
...
- 媒體/重試: `mediaMaxMb`, `retry`
  - `mediaMaxMb` 限制傳出的 Discord 上傳 (預設: `100MB`)
- 動作: `actions.*`
