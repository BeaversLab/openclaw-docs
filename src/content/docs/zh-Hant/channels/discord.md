...
```
        ...
        支援純文字 `token` 值。在 env/file/exec 提供者之間也支援 `channels.discord.token` 的 SecretRef 值。請參閱 [Secrets Management](/zh-Hant/gateway/secrets)。
...
- [組態參考 - Discord](/zh-Hant/gateway/configuration-reference#discord)
...
- [Groups](/zh-Hant/channels/groups)
