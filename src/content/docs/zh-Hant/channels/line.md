...
狀態：隨附外掛程式。支援直接訊息、群組聊天、媒體、位置、Flex
訊息、範本訊息和快速回覆。不支援反應和串回。

## 隨附外掛程式

在目前的 OpenClaw 版本中，LINE 作為隨附外掛程式提供，因此正常的
封裝版本不需要單獨安裝。

如果您使用的是較舊的版本或排除了 LINE 的自訂安裝，請手動
安裝它：
...
2. 建立（或選擇）一個提供者並新增 **Messaging API** 頻道。
...
- LINE 簽章驗證取決於內文（原始內文的 HMAC），因此 OpenClaw 在驗證之前會套用嚴格的預先驗證內文限制和逾時。
- OpenClaw 從驗證過的原始請求位元組處理 webhook 事件。為了簽章完整性安全，會忽略上游中介軟體轉換後的 `req.body` 值。
...
- `channels.line.allowFrom`: 用於 DM 的許可清單 LINE 使用者 ID
- `channels.line.groupPolicy`: `allowlist | open | disabled`
- `channels.line.groupAllowFrom`: 群組的允許清單 LINE 使用者 ID
- 各群組覆寫：`channels.line.groups.<groupId>.allowFrom`
- 執行時備註：如果完全缺少 `channels.line`，執行時會回退到 `groupPolicy="allowlist"` 進行群組檢查（即使設定了 `channels.defaults.groupPolicy`）。

LINE ID 有區分大小寫。有效的 ID 如下所示：
...
- 串流感應會被緩衝；當代理程式運作時，LINE 會收到完整區塊並顯示載入動畫。
...
LINE 外掛也附帶一個用於 Flex 訊息預設集的 `/card` 指令：
...
- 已設定的 ACP 綁定和作用中的對話綁定 ACP 工作階段，會像在其他對話頻道上一樣在 LINE 上運作。

詳見 [ACP 代理程式](/zh-Hant/tools/acp-agents)。

## 輸出媒體

LINE 外掛支援透過代理程式訊息工具傳送圖片、影片和音訊檔案。媒體會透過 LINE 專屬傳遞路徑發送，並搭配適當的預覽和追蹤處理：

- **圖片**：作為 LINE 圖片訊息發送，並自動產生預覽。
- **影片**：搭配明確的預覽和內容類型處理發送。
- **音訊**：作為 LINE 音訊訊息發送。

輸出媒體 URL 必須是公開的 HTTPS URL。OpenClaw 會在將 URL 傳遞給 LINE 之前驗證目標主機名稱，並拒絕回送、連結本機和私人網路目標。
