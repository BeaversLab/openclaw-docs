...
Twitch 在目前的 OpenClaw 版本中作為內建外掛隨附，因此一般的打包版本不需要額外安裝。

如果您使用的是較舊的版本或排除 Twitch 的自訂安裝，請手動安裝：
...
2. 建立一個專屬的 Twitch 帳號給機器人使用 (或是使用現有的帳號)。
3. 產生憑證：[Twitch Token Generator](https://twitchtokengenerator.com/)
   - 選擇 **Bot Token**
   - 確認已選取範圍 `chat:read` 和 `chat:write`
   - 複製 **Client ID** 和 **Access Token**
4. 尋找您的 Twitch 使用者 ID：[https://www.streamweasels.com/tools/convert-twitch-username-to-user-id/](https://www.streamweasels.com/tools/convert-twitch-username-to-user-id/)
5. 設定 Token：
   - 環境變數：`OPENCLAW_TWITCH_ACCESS_TOKEN=...` (僅限預設帳號)
   - 或是設定檔：`channels.twitch.accessToken`
   - 如果兩者都有設定，設定檔優先 (環境變數回退僅適用於預設帳號)。
6. 啟動閘道。

**⚠️ 重要：** 新增存取控制 (`allowFrom` 或 `allowedRoles`) 以防止未授權的使用者觸發機器人。`requireMention` 預設為 `true`。
...
- 每個帳號對應到一個獨立的 session key `agent:<agentId>:twitch:<accountName>`。
...
如需嚴格的允許清單，請優先使用 `allowFrom`。如果您想要基於角色的存取權限，請改用 `allowedRoles`。

**可用角色：** `"moderator"`、`"owner"`、`"vip"`、`"subscriber"`、`"all"`。

**為什麼使用使用者 ID？** 使用者名稱可以更改，這允許冒充。使用者 ID 是永久的。
...
來自 [Twitch Token Generator](https://twitchtokengenerator.com/) 的 Token 無法自動更新 - 過期後請重新生成。

若要自動更新 Token，請在 [Twitch Developer Console](https://dev.twitch.tv/console) 建立您自己的 Twitch 應用程式並加入到設定中：
...
- 如果使用權杖重新整理，請驗證 `clientSecret` 和 `refreshToken` 是否已設定
...
- `clientSecret` - 選填：用於自動重新整理權杖
- `refreshToken` - 選填：用於自動重新整理權杖
- `expiresIn` - 權杖到期時間（秒）
