...
- [SMS](/zh-Hant/channels/sms) - 透過 Gateway webhook 由 Twilio 支援的 SMS（內建外掛程式）。
- [Synology Chat](/zh-Hant/channels/synology-chat) - 透過傳出與傳入 webhook 連線的 Synology NAS Chat（內建外掛程式）。
- [Telegram](/zh-Hant/channels/telegram) - 透過 grammY 使用的 Bot API；支援群組。
- [Tlon](/zh-Hant/channels/tlon) - 基於 Urbit 的通訊應用程式（內建外掛程式）。
...
- [WebChat](/zh-Hant/web/webchat) - 透過 WebSocket 連線的 Gateway WebChat 使用者介面。
- [WeChat](/zh-Hant/channels/wechat) - 透過 QR 登入使用的 Tencent iLink Bot 外掛程式；僅支援私人聊天（外部外掛程式）。
- [WhatsApp](/zh-Hant/channels/whatsapp) - 最受歡迎；使用 Baileys 且需要 QR 配對。
- [Yuanbao](/zh-Hant/channels/yuanbao) - Tencent Yuanbao 機器人（外部外掛程式）。
- [Zalo](/zh-Hant/channels/zalo) - Zalo Bot API；越南受歡迎的通訊應用程式（內建外掛程式）。
- [Zalo Personal](/zh-Hant/channels/zalouser) - 透過 QR 登入的 Zalo 個人帳戶（內建外掛程式）。

## 備註

- 通道可以同時執行；設定多個通道，OpenClaw 將會根據聊天進行路由。
- 最快的設定通常是 **Telegram**（簡單的機器人權杖）。WhatsApp 需要 QR 配對並在磁碟上儲存更多狀態。
- 群組行為因通道而異；請參閱 [群組](/zh-Hant/channels/groups)。
- 為了安全起見，會執行 DM 配對與允許清單；請參閱 [安全性](/zh-Hant/gateway/security)。
- 疑難排解：[通道疑難排解](/zh-Hant/channels/troubleshooting)。
