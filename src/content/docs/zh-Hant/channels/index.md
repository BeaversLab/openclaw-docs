...
- [Zalo](/zh-Hant/channels/zalo) — Zalo Bot API；越南受歡迎的訊息應用程式（內建外掛）。
- [Zalo Personal](/zh-Hant/channels/zalouser) — 透過 QR碼登入的 Zalo 個人帳號（內建外掛）。
...
- DM 配對和允許列表是為了安全而強制執行的；請參閱 [Security](/zh-Hant/gateway/security)。
- 疑難排解：[Channel troubleshooting](/zh-Hant/channels/troubleshooting)。
- 模型供應商有單獨的文件說明；請參閱 [Model Providers](/zh-Hant/providers/models)。
