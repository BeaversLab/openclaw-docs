...
<AccordionGroup>
  ...
  <Accordion title="串流行為備註">
    - 如果串流無法就地完成（例如貼文在串流過程中被刪除），OpenClaw 將會退回發送一個全新的最終貼文，以確保回覆不會遺失。
    - 僅包含推理的負載會從頻道貼文中隱藏，包括以 `> Reasoning:` 區塊引言形式到達的文字。設定 `/reasoning on` 以在其他介面查看思考過程；Mattermost 的最終貼文僅保留答案。
