...
請參閱 [Plugins](/zh-Hant/tools/plugin) 以了解插件行為和安裝規則。
...
請參閱 [Matrix migration](/zh-Hant/install/migrating-matrix) 以了解完整的升級流程、限制、恢復指令和常見的遷移訊息。
...
請參閱 [Groups](/zh-Hant/channels/groups) 以了解提及閘控 和允許清單 行為。
...
請參閱 [Pairing](/zh-Hant/channels/pairing) 以了解共享的 DM 配對流程和儲存佈局。
...
Matrix 斜線指令 (例如 `/new`、`/reset`、`/model`) 可在 DM 中直接運作。在聊天室中，OpenClaw 也能識別以機器人自己的 Matrix 提及為前綴的斜線指令，因此 `@bot:server /new` 會觸發指令路徑，而不需要自訂提及正規表示式。這能讓機器人對 Element 和類似客戶端在使用者輸入指令前以 Tab 鍵自動完成機器人時發出的聊天室風格 `@mention /command` 貼文保持回應。

授權規則仍然適用：指令傳送者必須滿足 DM 或聊天室允許清單/擁有者政策，就像普通訊息一樣。
...
頂層 `channels.matrix` 值會作為命名帳號的預設值，除非帳號覆蓋了它們。
您可以使用 `groups.<room>.account` 將繼承的房間項目限定於一個 Matrix 帳號。
沒有 `account` 的項目會在所有 Matrix 帳號之間保持共用，而具有 `account: "default"` 的項目當預設帳號直接在頂層 `channels.matrix.*` 上配置時仍然有效。
部分共用的驗證預設值本身不會建立一個單獨的隱式預設帳號。只有當該預設值具有新的驗證資訊（`homeserver` 加上 `accessToken`，或 `homeserver` 加上 `userId` 和 `password`）時，OpenClow 才會綜合頂層 `default` 帳號；當快取的憑證稍後滿足驗證時，命名帳號仍可透過 `homeserver` 加上 `userId` 保持可被發現。
如果 Matrix 已經只有一個命名帳號，或者 `defaultAccount` 指向現有的命名帳號金鑰，單帳號到多帳號的修復/設置升級會保留該帳號，而不是建立一個新的 `accounts.default` 項目。只有 Matrix 驗證/啟動金鑰會移入該升級的帳號；共用的傳遞策略金鑰保留在頂層。
當您希望 OpenClaw 在隱式路由、探測和 CLI 操作中偏好一個命名 Matrix 帳號時，請設定 `defaultAccount`。
如果配置了多個 Matrix 帳號並且其中一個帳號 ID 是 `default`，即使未設定 `defaultAccount`，OpenClaw 也會隱式使用該帳號。
如果您配置了多個命名帳號，請設定 `defaultAccount` 或為依賴隱式帳號選擇的 CLI 命令傳遞 `--account <id>`。
當您想要為單個命令覆蓋該隱式選擇時，請將 `--account <id>` 傳遞給 `openclaw matrix verify ...` 和 `openclaw matrix devices ...`。

有關共用的多帳號模式，請參閱 [Configuration reference](/zh-Hant/gateway/configuration-reference#multi-account-all-channels)。

## 私人/LAN 主伺服器

預設情況下，為了防止 SSRF 攻擊，OpenClaw 會阻擋私有的/內部的 Matrix 用戶端伺服器，除非您
針對每個帳戶明確選擇加入。

如果您的用戶端伺服器運行在 localhost、LAN/Tailscale IP 或內部主機名上，請為該 Matrix 帳戶啟用
...
此選擇加入僅允許受信任的私有/內部目標。公開的明文用戶端伺服器（例如
`http://matrix.example.org:8008`）仍然會被阻擋。請盡可能優先使用 `https://`。

## Proxying Matrix traffic

如果您的 Matrix 部署需要指定外傳 HTTP(S) 代理，請設定 `channels.matrix.proxy`：
...
OpenClaw 對執行時的 Matrix 流量和帳戶狀態偵測使用相同的代理設定。

## Target resolution

在 OpenClaw 要求您提供房間或使用者目標的任何地方，Matrix 都接受這些目標形式：
...
- 使用者查詢會向該用戶端伺服器上的 Matrix 使用者目錄發出請求。
- 房間查詢會直接接受明確的房間 ID 和別名，然後回退到搜尋該帳戶已加入的房間名稱。
- 已加入房間名稱查詢是盡力而為的。如果房間名稱無法解析為 ID 或別名，它將被執行時允許清單解析忽略。

## Configuration reference

- `enabled`：啟用或停用頻道。
- `name`：帳戶的可選標籤。
- `defaultAccount`：當設定多個 Matrix 帳戶時，偏好使用的帳戶 ID。
- `homeserver`：用戶端伺服器 URL，例如 `https://matrix.example.org`。
- `network.dangerouslyAllowPrivateNetwork`：允許此 Matrix 帳號連線到私人/內部 homeserver。當 homeserver 解析為 `localhost`、LAN/Tailscale IP 或內部主機（例如 `matrix-synapse`）時，請啟用此選項。
- `proxy`：Matrix 流量的選用 HTTP(S) 代理 URL。命名帳號可以使用自己的 `proxy` 覆寫頂層預設值。
- `userId`：完整的 Matrix 使用者 ID，例如 `@bot:example.org`。
- `accessToken`：基於 token 的認證存取權杖。對於 env/file/exec 提供者中的 `channels.matrix.accessToken` 和 `channels.matrix.accounts.<id>.accessToken`，支援純文字值和 SecretRef 值。請參閱[機密管理](/zh-Hant/gateway/secrets)。
...
- `encryption`：啟用 E2EE。
- `allowlistOnly`：當 `true` 時，將 `open` 房間原則升級為 `allowlist`，並強制所有啟用的 DM 原則（ `disabled` 除外，包括 `pairing` 和 `open`）變為 `allowlist`。不影響 `disabled` 原則。
- `allowBots`：允許來自其他已設定 OpenClaw Matrix 帳號（`true` 或 `"mentions"`）的訊息。
- `groupPolicy`：`open`、`allowlist` 或 `disabled`。
- `contextVisibility`：額外的房間上下文可見性模式 (`all`、`allowlist`、`allowlist_quote`)。
- `groupAllowFrom`：房間流量的使用者 ID 白名單。完整的 Matrix 使用者 ID 最安全；完全匹配目錄的項目會在啟動時以及在監控器運行期間白名單變更時解析。未解析的名稱將被忽略。
- `historyLimit`：作為群組歷史上下文包含的最大房間訊息數。如果未設定，則回退至 `messages.groupChat.historyLimit`；如果兩者均未設定，有效預設值為 `0`。設定 `0` 以停用。
- `replyToMode`：`off`、`first`、`all` 或 `batched`。
- `markdown`：用於傳出 Matrix 文字的選用 Markdown 渲染設定。
- `streaming`：`off`（預設值）、`"partial"`、`"quiet"`、`true` 或 `false`。`"partial"` 和 `true` 透過正常的 Matrix 文字訊息啟用「優先預覽」草稿更新。`"quiet"` 對於自託管推送規則設定使用非通知預覽通知。`false` 等同於 `"off"`。
- `blockStreaming`：`true` 在草稿預覽串流處於活動狀態時，為已完成的助手區塊啟用個別的進度訊息。
- `threadReplies`：`off`、`inbound` 或 `always`。
- `threadBindings`：執行緒繫結會話路由和生命週期的頻道層級覆寫。
- `startupVerification`：啟動時的自動自我驗證請求模式 (`if-unverified`、`off`)。
- `startupVerificationCooldownHours`：重試自動啟動驗證請求之前的冷卻時間。
- `textChunkLimit`：出站訊息區塊大小，以字元為單位（當 `chunkMode` 為 `length` 時適用）。
- `chunkMode`：`length` 依字元數分割訊息；`newline` 在行邊界分割。
- `responsePrefix`：可選字串，附加至此頻道所有出站回覆的前面。
- `ackReaction`：此頻道/帳號的可選 ack 反應覆寫。
- `ackReactionScope`：可選的 ack 反應範圍覆寫（`group-mentions`、`group-all`、`direct`、`all`、`none`、`off`）。
- `reactionNotifications`：入站反應通知模式（`own`、`off`）。
- `mediaMaxMb`：出站傳送和入站媒體處理的媒體大小上限，單位為 MB。
- `autoJoin`：邀請自動加入原則（`always`、`allowlist`、`off`）。預設值：`off`。適用於所有 Matrix 邀請，包括 DM 風格的邀請。
- `autoJoinAllowlist`：當 `autoJoin` 為 `allowlist` 時允許的房間/別名。別名項目會在處理邀請時解析為房間 ID；OpenClaw 不信任被邀請房間聲稱的別名狀態。
- `dm`：DM 原則區塊（`enabled`、`policy`、`allowFrom`、`sessionScope`、`threadReplies`）。
- `dm.policy`：在 OpenClaw 加入房間並將其分類為 DM 之後，控制 DM 存取權。它不會變更是否自動加入邀請。
- `dm.allowFrom`：DM 流量的使用者 ID 允許清單。完整的 Matrix 使用者 ID 最安全；精確的目錄符合項會在啟動時以及監視器執行時允許清單變更時進行解析。無法解析的名稱將被忽略。
- `dm.sessionScope`：`per-user`（預設）或 `per-room`。當您希望每個 Matrix 直訊房間即使對象相同也保持獨立的語境時，請使用 `per-room`。
- `dm.threadReplies`：僅限直訊的執行緒原則覆寫（`off`、`inbound`、`always`）。它會覆寫頂層 `threadReplies` 設定，用於直訊中的回覆放置和階段隔離。
- `execApprovals`：Matrix 原生的 exec 傳遞核准方式（`enabled`、`approvers`、`target`、`agentFilter`、`sessionFilter`）。
- `execApprovals.approvers`：獲准核准 exec 請求的 Matrix 使用者 ID。當 `dm.allowFrom` 已識別出核准者時，此為可選項。
- `execApprovals.target`：`dm | channel | both`（預設：`dm`）。
- `accounts`：命名的個別帳號覆寫。頂層 `channels.matrix` 值作為這些條目的預設值。
- `groups`：各房間的原則映射。優先使用房間 ID 或別名；未解析的房間名稱會在執行時被忽略。階段/群組身分使用解析後的穩定房間 ID。
- `groups.<room>.account`：在多帳號設定中，將繼承的房間條目限制為特定的 Matrix 帳號。
- `groups.<room>.allowBots`：針對已配置機器器傳送者的房間級別覆寫（`true` 或 `"mentions"`）。
- `groups.<room>.users`：各房間的傳送者允許清單。
- `groups.<room>.tools`：各房間的工具允許/拒絕覆寫。
- `groups.<room>.autoReply`：房間級別的提及閘門覆寫。`true` 會停用該房間的提及要求；`false` 則會強制重新啟用。
- `groups.<room>.skills`：可選的房間級別技能過濾器。
- `groups.<room>.systemPrompt`：可選的房間級別系統提示片段。
- `rooms`：`groups` 的舊版別名。
- `actions`：個別動作工具閘道 (`messages`, `reactions`, `pins`, `profile`, `memberInfo`, `channelInfo`, `verification`)。
