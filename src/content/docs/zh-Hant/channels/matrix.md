...
請參閱 [Plugins](/en/tools/plugin) 以了解插件行為與安裝規則。
...
請參閱 [Matrix migration](/en/install/migrating-matrix) 以了解完整的升級流程、限制、恢復指令與常見的遷移訊息。
...
請參閱 [Groups](/en/channels/groups) 以了解提及閘控與允許清單行為。
...
請參閱 [Pairing](/en/channels/pairing) 以了解共用 DM 配對流程與儲存佈局。
...
請參閱 [設定參考](/en/gateway/configuration-reference#multi-account-all-channels) 以了解共享多帳號模式。
...
- `accessToken`：基於權杖的身分驗證的存取權杖。在 env/file/exec 提供者中，`channels.matrix.accessToken` 和 `channels.matrix.accounts.<id>.accessToken` 支援純文字值和 SecretRef 值。請參閱[機密管理](/en/gateway/secrets)。
