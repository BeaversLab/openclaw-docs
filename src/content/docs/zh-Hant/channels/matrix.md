...
如果啟動發現引導狀態損壞且已設定 `channels.matrix.password`，OpenClaw 可以嘗試更嚴格的修復路徑。
如果目前的裝置已經由擁有者簽署，OpenClaw 將保留該身分，而不是自動重置它。

請參閱 [Matrix 遷移](/en/install/migrating-matrix) 以了解完整的升級流程、限制、恢復指令和常見的遷移訊息。
...
請參閱 [群組](/en/channels/groups) 以了解提及閘道和白名單行為。
...
請參閱 [配對](/en/channels/pairing) 以了解共享的 DM 配對流程和儲存配置。
...
您可以使用 `groups.<room>.account` 將繼承的房間條目限定於一個 Matrix 帳號。
沒有 `account` 的條目在所有 Matrix 帳號之間保持共享，而具有 `account: "default"` 的條目在頂層 `channels.matrix.*` 上直接配置預設帳號時仍然有效。
部分共享的 auth 預設值本身不會創建單獨的隱式預設帳號。僅當該預設值具有新的 auth（`homeserver` 加上 `accessToken`，或 `homeserver` 加上 `userId` 和 `password`）時，OpenClaw 才會綜合頂層 `default` 帳號；當快取的憑證稍後滿足 auth 時，命名帳號仍然可以透過 `homeserver` 加上 `userId` 保持可發現。
如果 Matrix 已經只有一個命名帳號，或者 `defaultAccount` 指向現有的命名帳號金鑰，則單帳號到多帳號的修復/設定升級會保留該帳號，而不是創建新的 `accounts.default` 條目。只有 Matrix auth/bootstrap 金鑰會移入該升級的帳號；共享的 delivery-policy 金鑰保留在頂層。
當您希望 OpenClaw 優先使用一個命名 Matrix 帳號進行隱式路由、探測和 CLI 操作時，請設定 `defaultAccount`。
如果您配置了多個命名帳號，請設定 `defaultAccount` 或為依賴隱式帳號選擇的 CLI 命令傳遞 `--account <id>`。
當您想要為一個命令覆寫該隱式選擇時，請將 `--account <id>` 傳遞給 `openclaw matrix verify ...` 和 `openclaw matrix devices ...`。

有關共享多帳號模式，請參閱 [Configuration reference](/en/gateway/configuration-reference#multi-account-all-channels)。
...
如果您的 Matrix 部署需要指定的 outbound HTTP(S) 代理，請設定 `channels.matrix.proxy`：
...
命名帳戶可以使用 `channels.matrix.accounts.<id>.proxy` 覆蓋頂層預設值。
OpenClaw 對運行時 Matrix 流量和帳戶狀態偵測使用相同的代理設定。
...
- `enabled`：啟用或停用頻道。
- `name`：帳戶的選用標籤。
- `defaultAccount`：設定多個 Matrix 帳戶時的首選帳戶 ID。
- `homeserver`：主伺服器 URL，例如 `https://matrix.example.org`。
- `network.dangerouslyAllowPrivateNetwork`：允許此 Matrix 帳戶連線到私人/內部主伺服器。當主伺服器解析為 `localhost`、LAN/Tailscale IP 或內部主機（例如 `matrix-synapse`）時，請啟用此選項。
- `proxy`：Matrix 流量的可選 HTTP(S) 代理 URL。指定的帳戶可以用自己的 `proxy` 覆蓋頂層預設值。
- `userId`：完整的 Matrix 使用者 ID，例如 `@bot:example.org`。
- `accessToken`：基於 Token 認證的存取權杖。對於跨 env/file/exec 提供者的 `channels.matrix.accessToken` 和 `channels.matrix.accounts.<id>.accessToken`，支援純文字值和 SecretRef 值。請參閱[祕密管理](/en/gateway/secrets)。
- `password`：基於密碼登入的密碼。支援純文字值和 SecretRef 值。
- `deviceId`：明確指定的 Matrix 裝置 ID。
- `deviceName`：密碼登入的裝置顯示名稱。
- `avatarUrl`：儲存的自我大頭照 URL，用於個人資料同步和 `profile set` 更新。
- `initialSyncLimit`：啟動同步期間擷取的最大事件數。
- `encryption`：啟用 E2EE。
- `allowlistOnly`：當設為 `true` 時，將 `open` 房間策略升級為 `allowlist`，並強制所有使用中的 DM 策略（`disabled` 除外，包括 `pairing` 和 `open`）變為 `allowlist`。不影響 `disabled` 策略。
- `allowBots`：允許來自已設定其他 OpenClaw Matrix 帳戶的訊息（`true` 或 `"mentions"`）。
...
- `historyLimit`：作為群組歷史紀錄上下文包含的最大房間訊息數量。預設回退至 `messages.groupChat.historyLimit`；若兩者皆未設定，則有效預設值為 `0`。設定 `0` 以停用。
- `replyToMode`：`off`、`first`、`all` 或 `batched`。
- `markdown`：用於輸出 Matrix 文字的選用 Markdown 渲染設定。
- `streaming`：`off`（預設）、`"partial"`、`"quiet"`、`true` 或 `false`。`"partial"` 和 `true` 會透過一般 Matrix 文字訊息啟用預覽優先的草稿更新。`"quiet"` 會對自託管推送規則設定使用不發送通知的預覽通知。`false` 等同於 `"off"`。
- `blockStreaming`：當草稿預覽串流處理作用中時，`true` 會針對已完成的助理區塊啟用獨立的進度訊息。
- `threadReplies`：`off`、`inbound` 或 `always`。
- `threadBindings`：對執行緒繫結的工作階段路由與生命週期的逐頻道覆寫。
- `startupVerification`：啟動時的自動自我驗證請求模式（`if-unverified`、`off`）。
- `startupVerificationCooldownHours`：重試自動啟動驗證請求前的冷卻時間。
- `textChunkLimit`：輸出訊息區塊大小，以字元計算（當 `chunkMode` 為 `length` 時適用）。
- `chunkMode`：`length` 會依字元數切割訊息；`newline` 會在行邊界切割。
- `responsePrefix`：預先附加至此頻道所有輸出回覆的選用字串。
...
- `mediaMaxMb`：用於出站傳送和入站媒體處理的媒體大小上限（MB）。
- `autoJoin`：邀請自動加入政策（`always`、`allowlist`、`off`）。預設值：`off`。適用於所有 Matrix 邀請，包括 DM 樣式的邀請。
- `autoJoinAllowlist`：當 `autoJoin` 為 `allowlist` 時允許的房間/別名。別名項目會在邀請處理期間解析為房間 ID；OpenClaw 不信任被邀請房間所聲稱的別名狀態。
- `dm`：DM 政策區塊（`enabled`、`policy`、`allowFrom`、`sessionScope`、`threadReplies`）。
- `dm.policy`：控制 OpenClaw 加入房間並將其分類為 DM 之後的 DM 存取。它不會改變是否自動加入邀請。
- `dm.allowFrom`：除非您已透過即時目錄查詢解析，否則條目應為完整的 Matrix 使用者 ID。
- `dm.sessionScope`：`per-user`（預設）或 `per-room`。當您希望每個 Matrix DM 房間即使對象相同也保持獨立語境時，請使用 `per-room`。
- `dm.threadReplies`：僅限 DM 的執行緒政策覆寫（`off`、`inbound`、`always`）。它會覆寫頂層 `threadReplies` 設定，用於 DM 中的回覆放置和工作階段隔離。
- `execApprovals`：Matrix 原生執行審核交付方式（`enabled`、`approvers`、`target`、`agentFilter`、`sessionFilter`）。
- `execApprovals.approvers`：獲准審核執行請求的 Matrix 使用者 ID。當 `dm.allowFrom` 已識別審核者時為選填。
- `execApprovals.target`：`dm | channel | both`（預設值：`dm`）。
- `accounts`：具名的每帳號覆寫。頂層 `channels.matrix` 值作為這些項目的預設值。
- `groups`：每房間原則映射。優先使用房間 ID 或別名；無法解析的房間名稱將在執行時被忽略。會話/群組身分在解析後使用穩定的房間 ID。
- `groups.<room>.account`：在多帳號設置中，將一個繼承的房間項目限制為特定的 Matrix 帳號。
- `groups.<room>.allowBots`：針對設定機器人發送者（`true` 或 `"mentions"`）的房間層級覆寫。
- `groups.<room>.users`：每房間發送者允許清單。
- `groups.<room>.tools`：每房間工具允許/拒絕覆寫。
- `groups.<room>.autoReply`：房間層級提及門控覆寫。`true` 會停用該房間的提及要求；`false` 則會將其重新啟用。
- `groups.<room>.skills`：選用的房間層級技能過濾器。
- `groups.<room>.systemPrompt`：選用的房間層級系統提示片段。
- `rooms`：`groups` 的舊版別名。
- `actions`：每動作工具門控（`messages`、`reactions`、`pins`、`profile`、`memberInfo`、`channelInfo`、`verification`）。
