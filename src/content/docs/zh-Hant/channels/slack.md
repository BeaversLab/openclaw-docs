...
`plugins install` 會註冊並啟用此外掛。在您於下方設定 Slack 應用程式和頻道設定之前，該外掛仍然不會執行任何操作。請參閱 [Plugins](/zh-Hant/tools/plugin) 以了解一般外掛行為和安裝規則。
  ...
  <Tab title="Socket Mode (預設)">
    <Steps>
      <Step title="建立新的 Slack 應用程式">
        開啟 [api.slack.com/apps](https://api.slack.com/apps/new) → **Create New App** → **From a manifest** → 選擇您的工作區 → 貼上下列其中一份資訊清單 → **Next** → **Create**。
...
```
        ...
        <Note>
          **Recommended** 符合 Slack 外掛的完整功能集：App Home、斜線指令、檔案、反應、釘選、群組 DM，以及 emoji/usergroup 讀取。當工作區政策限制範圍時，請選擇 **Minimal** — 它涵蓋 DM、頻道/群組紀錄、提及和斜線指令，但會捨棄檔案、反應、釘選、群組-DM (`mpim:*`)、`emoji:read` 和 `usergroups:read`。請參閱 [Manifest and scope checklist](#manifest-and-scope-checklist) 以了解每個範圍的理據，以及額外的斜線指令等附加選項。
        </Note>
  ...
  <Tab title="HTTP Request URLs">
    <Steps>
      <Step title="Create a new Slack app">
        開啟 [api.slack.com/apps](https://api.slack.com/apps/new) → **Create New App** → **From a manifest** → 選擇您的工作區 → 貼上下列其中一個資訊清單 → 將 `https://gateway-host.example.com/slack/events` 取代為您的公開 Gateway URL → **Next** → **Create**。
...
```
        ...
        <Note>
          **Recommended** (推薦) 符合 Slack 外掛的完整功能集；**Minimal** (最小化) 則捨棄了檔案、反應、釘選、群組 DM (`mpim:*`)、`emoji:read` 和 `usergroups:read`，以適用於受限的工作區。請參閱 [Manifest and scope checklist](#manifest-and-scope-checklist) 以了解各個範圍的原理。
        </Note>

        <Info>
          三個 URL 欄位 (`slash_commands[].url`、`event_subscriptions.request_url` 和 `interactivity.request_url` / `message_menu_options_url`) 都指向同一個 OpenClaw 端點。Slack 的資訊清單架構要求它們必須分別命名，但 OpenClaw 是依據 payload 類型進行路由，因此單一 `webhookPath` (預設為 `/slack/events`) 就足夠了。在 HTTP 模式下，沒有 `slash_commands[].url` 的斜線指令將會靜默無操作。
        </Info>
...
<AccordionGroup>
  <Accordion title="可選的原生斜線指令">
    ...
    您可以使用多個 [原生斜線指令](#commands-and-slash-behavior) 來代替單個具有細微差別的配置指令：
...
```
      ...
      <Tab title="HTTP Request URLs">
        使用與上述 Socket 模式相同的 `slash_commands` 列表，並為每個條目新增 `"url": "https://gateway-host.example.com/slack/events"`。範例：
...
<Tabs>
  ...
  <Tab title="提及與頻道使用者">
    ...
    `allowBots` 對於頻道和私人頻道採用保守策略：只有在傳送機器人明確列於該房間的 `users` 允許清單中，或當來自 `channels.slack.allowFrom` 的至少一個明確 Slack 擁有者 ID 目前是房間成員時，才會接受由機器人發送的房間訊息。萬用字元和顯示名稱擁有者項目不滿足擁有者存在條件。擁有者存在使用 Slack `conversations.members`；請確保應用程式對於該房間類型具有相符的讀取範圍 (公開頻道為 `channels:read`，私人頻道為 `groups:read`)。如果成員查詢失敗，OpenClaw 將捨棄由機器人發送的房間訊息。

    接受的由機器人發送的 Slack 訊息使用共享的 [機器人迴圈保護](/zh-Hant/channels/bot-loop-protection)。設定 `channels.defaults.botLoopProtection` 作為預算預設值，然後當工作區或頻道需要不同的限制時，使用 `channels.slack.botLoopProtection` 或 `channels.slack.channels.<id>.botLoopProtection` 覆寫。
...
原生指令需要在您的 Slack 應用程式中進行[額外的 manifest 設定](#additional-manifest-settings)，並且是在全域設定中透過 `channels.slack.commands.native: true` 或 `commands.native: true` 來啟用的。
...
同個對話中的 `/approve` 也適用於已支援指令的 Slack 頻道和私人訊息。請參閱 [Exec approvals](/zh-Hant/tools/exec-approvals) 以了解完整的核准轉送模型。
...
主要參考資料：[Configuration reference - Slack](/zh-Hant/gateway/config-channels#slack)。
...
<AccordionGroup>
  <Accordion title="頻道中無回覆">
    ...
    - `groupPolicy`
    - 頻道允許清單 (`channels.slack.channels`) — **金鑰必須是頻道 ID** (`C12345678`)，而不是名稱 (`#channel-name`)。由於頻道路由預設優先使用 ID，因此在 `groupPolicy: "allowlist"` 下，以名稱為基礎的金鑰會靜默失敗。若要尋找 ID：在 Slack 中對頻道按一下右鍵 → **Copy link** — 網址末尾的 `C...` 值即為頻道 ID。
    - `requireMention`
    - 各頻道的 `users` 允許清單
    - `messages.groupChat.visibleReplies`：正常的群組/頻道請求預設為 `"automatic"`。如果您選擇加入 `"message_tool"` 且日誌顯示助理文字但沒有 `message(action=send)` 呼叫，表示模型錯過了可見的訊息工具路徑。在此模式下，最終文字保持私密；請檢查 gateway 詳細日誌中的被抑制 payload 中繼資料，或者如果您希望每個正常的助理最終回覆都透過舊版路徑發布，請將其設為 `"automatic"`。
    - `messages.groupChat.unmentionedInbound`：如果它是 `"room_event"`，則未提及的允許頻道閒聊是環境上下文，並且保持靜默，除非代理呼叫 `message` 工具。請參閱 [Ambient room events](/zh-Hant/channels/ambient-room-events)。
...
```
  ...
  <Accordion title="DM 訊息被忽略">
    ...
    - 配對審核 / 允許清單項目 (`dmPolicy: "open"` 仍需要 `channels.slack.allowFrom: ["*"]`)
    - 群組 DM 使用 MPIM 處理；請啟用 `channels.slack.dm.groupEnabled`，並且若有設定，請將 MPIM 包含在 `channels.slack.dm.groupChannels` 中
...
```
  ...
  <Accordion title="Socket 模式無法連線">
    請在 Slack 應用程式設定中驗證機器人 + 應用程式權杖以及 Socket Mode 是否已啟用。
    `xapp-...` 應用程式層級權杖需要 `connections:write`，且 `xoxb-...`
    機器人權杖必須屬於與應用程式權杖相同的 Slack 應用程式/工作區。
  ...
  <Accordion title="HTTP mode not receiving events">
    ...
    - signing secret (簽署金鑰)
    - webhook path (Webhook 路徑)
    - Slack Request URLs (事件 + 互動 + 斜線指令)
    - 每個 HTTP 帳號唯一的 `webhookPath`
    - 公開 URL 終止 TLS 並將請求轉發到 Gateway 路徑
    - Slack 應用程式的 `request_url` 路徑完全符合 `channels.slack.webhookPath` (預設為 `/slack/events`)

    如果帳號快照中出現 `signingSecretStatus: "configured_unavailable"`，表示 HTTP 帳號已設定，但目前的執行階段無法解析 SecretRef 支援的簽署金鑰。

    重複出現的 `slack: webhook path ... already registered` 記錄表示兩個 HTTP 帳號正在使用相同的 `webhookPath`；請給每個帳號一個不同的路徑。
  ...
  <Accordion title="Native/slash commands not firing">
    ...
    Slack 不會自動建立或移除斜線指令。`commands.native: "auto"` 不會啟用 Slack 原生指令；請使用 `true` 並在 Slack 應用程式中建立相符的指令。在 HTTP 模式下，每個 Slack 斜線指令都必須包含 Gateway URL。在 Socket 模式下，指令酬載透過 websocket 傳送，而 Slack 會忽略 `slash_commands[].url`。

    另外請檢查 `commands.useAccessGroups`、DM 授權、頻道允許清單，以及每個頻道的 `users` 允許清單。對於遭阻擋的斜線指令傳送者，Slack 會傳回暫時性錯誤，包括：
...
| 媒體類型                     | 來源            | 目前行為                                                             | 備註                                                       |
| ---------------------------- | --------------- | -------------------------------------------------------------------- | ---------------------------------------------------------- |
| JPEG / PNG / GIF / WebP 圖片 | Slack 檔案 URL  | 下載並附加至回合以進行具備視覺能力的處理                             | 每個檔案的上限：`channels.slack.mediaMaxMb` (預設為 20 MB) |
| PDF 檔案                     | Slack 檔案 URL  | 已下載並作為檔案上下文公開給諸如 `download-file` 或 `pdf` 等工具使用 | Slack 傳入不會自動將 PDF 轉換為圖片視覺輸入                |
...
1. OpenClaw 使用 bot 權杖 (`xoxb-...`) 從 Slack 的私人 URL 下載檔案。
2. 成功時，檔案會被寫入媒體存放區。
...
- **視覺模型**：圖片分析會在支援視覺功能時使用作用中的回覆模型，或是使用在 `agents.defaults.imageModel` 中設定的圖片模型。
...
| 未設定視覺模型               | 圖像附件會儲存為媒體參照，但不會作為圖像進行分析          | 設定 `agents.defaults.imageModel` 或使用具備視覺能力的回覆模型        |
