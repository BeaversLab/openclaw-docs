...
<AccordionGroup>
  ...
  <Accordion title="匿名模型 (12) — 透過 Venice 代理">
...
`/models` 端點是公開的（列出清單時不需要認證），但推論需要有效的 API 金鑰。
...
Venice 使用基於點數的系統。請前往 [venice.ai/pricing](https://venice.ai/pricing) 查看當前費率：
...
<Accordion title="模型不可用">Venice 模型目錄會動態更新。執行 `openclaw models list` 以查看目前可用的模型。部分模型可能暫時離線。</Accordion>
