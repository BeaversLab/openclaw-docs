...
Model Studio 提供者可讓您存取 Alibaba Cloud Coding Plan 模型，包括 Qwen 和託管在平台上的第三方模型。
...
提供者會根據驗證選擇自動選取（全球使用 `modelstudio-api-key`，中國使用 `modelstudio-api-key-cn`）。您可以在設定中使用自訂的 `baseUrl` 進行覆寫。
...
大多數模型支援圖片輸入。上下文視窗範圍從 20萬 到 100萬 token。

## 環境注意事項

如果 Gateway 以守護程序 (launchd/systemd) 執行，請確保 `MODELSTUDIO_API_KEY` 對該程序可用（例如，在 `~/.openclaw/.env` 中或透過 `env.shellEnv`）。
