...
[Cerebras](https://www.cerebras.ai) 在自訂推論硬體上提供高速 OpenAI 相容推論。OpenClaw 包含一個內建的 Cerebras 提供者外掛，並配備靜態的四模型目錄。
...
<Note>如果 Gateway 作為守護程序（launchd、systemd、Docker）運行，請確保 `CEREBRAS_API_KEY` 可供該程序使用——例如在 `~/.openclaw/.env` 中或透過 `env.shellEnv`。僅存放在 `~/.profile` 中的金鑰對於受管服務將沒有幫助，除非該環境變數被單獨匯入。</Note>
