    ...
    **最適合：** 完全控制雲端或本地設定。
    ...
    <Steps>
      ...
      <Step title="Enable Ollama for OpenClaw">
        針對 `Cloud only`，請使用您真實的 `OLLAMA_API_KEY`。對於由主機支援的設定，任何預留位置值皆可運作：
...
<Note>如需完整的設定和行為細節，請參閱 [Ollama Web Search](/zh-Hant/tools/ollama-search)。</Note>
...
<Accordion title="模型成本">Ollama 是免費且在本地運行的，因此所有模型成本均設為 $0。這適用於自動發現和手動定義的模型。</Accordion>
  ...
  <Accordion title="記憶嵌入">
    內建的 Ollama 外掛為 [記憶搜尋](/zh-Hant/concepts/memory) 註冊了一個記憶嵌入提供者。它使用設定的 Ollama 基礎 URL 和 API 金鑰，呼叫 Ollama 目前的 `/api/embed` 端點，並在可能時將多個記憶區塊批次處理為一個 `input` 請求。
    ...
    查詢時的嵌入會針對需要或建議使用檢索前綴的模型使用它們，包括 `nomic-embed-text`、`qwen3-embedding` 和 `mxbai-embed-large`。記憶文件批次保持原始狀態，因此現有索引不需要進行格式遷移。
  ...
  <Accordion title="串流設定">
    ...
    對於原生 `/api/chat` 請求，OpenClaw 也會直接將思考控制轉發給 Ollama：`/think off` 和 `openclaw agent --thinking off` 會發送頂層 `think: false`，除非設定了明確的模型 `params.think`/`params.thinking` 值，而 `/think low|medium|high` 則會發送相符的頂層 `think` 努力字串。`/think max` 對應到 Ollama 最高的原生努力，`think: "high"`。

    <Tip>
    如果您需要使用 OpenAI 相容端點，請參閱上方的「舊版 OpenAI 相容模式」章節。在該模式下，串流和工具呼叫可能無法同時運作。
    </Tip>
...
<AccordionGroup>
  <Accordion title="WSL2 當機迴圈 (反覆重開機)">
    在使用 NVIDIA/CUDA 的 WSL2 環境中，官方 Ollama Linux 安裝程式會建立一個帶有 `Restart=always` 的 `ollama.service` systemd unit。如果該服務在 WSL2 開機期間自動啟動並載入 GPU 支援的模型，Ollama 可能會在載入模型時鎖定主機記憶體。Hyper-V 記憶體回收機制並非總是能回收那些被鎖定的頁面，導致 Windows 可能會終止 WSL2 VM，而 systemd 會再次啟動 Ollama，形成迴圈。

    常見跡象：

    - WSL2 反覆重開機或從 Windows 端終止
    - WSL2 啟動後不久，`app.slice` 或 `ollama.service` 佔用高 CPU
    - 收到來自 systemd 的 SIGTERM 訊號，而非 Linux OOM-killer 事件

    當 OpenClaw 偵測到 WSL2、啟用了 `Restart=always` 的 `ollama.service`，以及可見的 CUDA 標記時，會記錄啟動警告。
    ...
    在 Windows 端將此加入 `%USERPROFILE%\.wslconfig`，然後執行 `wsl --shutdown`：
    ...
    在 Ollama 服務環境中設定較短的 keep-alive，或者僅在需要時手動啟動 Ollama：
  ...
  <Accordion title="未偵測到 Ollama">
    請確保 Ollama 正在執行，且您已設定 `OLLAMA_API_KEY` (或驗證設定檔)，並且您**未**定義明確的 `models.providers.ollama` 項目：
  ...
  <Accordion title="沒有可用的模型">
    如果您的模型未列出，請在本地拉取模型或在 `models.providers.ollama` 中明確定義它。
  ...
  <Accordion title="Remote host works with curl but not OpenClaw">
    ...
    - `baseUrl` 指向 `localhost`，但 Gateway 執行在 Docker 或另一台主機上。
    - URL 使用了 `/v1`，這會選擇 OpenAI 相容模式而不是原生的 Ollama。
    - 遠端主機需要在 Ollama 端變更防火牆或區域網路 (LAN) 綁定設定。
    - 模型存在於您筆記型電腦的 daemon 上，但不存在於遠端 daemon 上。

  </Accordion>

  <Accordion title="Model outputs tool JSON as text">
    這通常表示供應商正在使用 OpenAI 相容模式，或者模型無法處理工具架構 (tool schemas)。
    ...
    如果小型本地模型仍然無法處理工具架構，請在該模型條目上設定 `compat.supportsTools: false` 並重新測試。

  </Accordion>

  <Accordion title="Kimi or GLM returns garbled symbols">
    託管的 Kimi/GLM 回應如果是長串的非語言符號，將被視為失敗的供應商輸出，而非成功的助手回答。這樣可以讓正常的重試、備援或錯誤處理機制接手，而不會將損壞的文字保存到會話中。

    如果這種情況頻繁發生，請記錄原始模型名稱、目前的會話檔案，以及該次執行是使用 `Cloud + Local` 還是 `Cloud only`，然後嘗試新的會話和備援模型：
  ...
  <Accordion title="Cold local model times out">
    大型本地模型在開始串流之前，可能需要很長的初次載入時間。請將逾時設定範圍限制在 Ollama 供應商，並可選擇性地要求 Ollama 在回合之間保持模型載入狀態：
  ...
  <Accordion title="Large-context model is too slow or runs out of memory">
    許多 Ollama 模型宣稱的上下文大小超過了您的硬體能順利運作的範圍。除非您設定了 `params.num_ctx`，否則原生的 Ollama 會使用 Ollama 自己的運行時上下文預設值。當您想要可預測的首個 token 延遲時，請同時限制 OpenClaw 的預算和 Ollama 的請求上下文：
    ...
    如果 OpenClaw 發送了過多的提示詞，請先降低 `contextWindow`。如果 Ollama 正在載入對該機器而言過大的運行時上下文，請降低 `params.num_ctx`。如果生成運行時間過長，請降低 `maxTokens`。

  </Accordion>
</AccordionGroup>

<Note>更多幫助：[故障排除](/zh-Hant/help/troubleshooting) 和 [常見問題](/zh-Hant/help/faq)。</Note>
