...
2. 將 Bedrock 提供者和模型新增到您的設定中 (不需要 `apiKey`)：
...
**變通方法：** 設定 `AWS_PROFILE=default` 以表示 AWS 憑證可用。實際的身份驗證仍透過 IMDS 使用執行個體角色。
...
- OpenClaw 依此順序公開憑證來源：`AWS_BEARER_TOKEN_BEDROCK`，
