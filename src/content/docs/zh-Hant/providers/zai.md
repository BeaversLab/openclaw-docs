...
Z.AI 是 **GLM** 模型的 API 平台。它为 GLM 提供 REST API，并使用 API 密鑰
進行身份驗證。請在 Z.AI 控制台中建立您的 API 金鑰。OpenClaw 使用 `zai` 提供者
搭配 Z.AI API 金鑰。
...
OpenClaw 在插件清單中附帶了捆綁的 `zai` 提供者目錄，因此唯讀
列表可以在不載入提供者執行時的情況下顯示已知的 GLM 列：
...
<Tip>
GLM 模型可作為 `zai/<model>` 使用（例如：`zai/glm-5`）。預設內建的模型參考是 `zai/glm-5.1`。
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    ...
    進階使用者仍然可以使用
    `params.extra_body.thinking` 覆寫確切的提供者負載。
