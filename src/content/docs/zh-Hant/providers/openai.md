...
工作階段覆蓋優先於設定。在 Sessions UI 中清除工作階段覆蓋
會將工作階段恢復為設定的預設值。

### OpenAI Responses 伺服器端壓縮

對於直接的 OpenAI Responses 模型（`openai/*` 使用 `api: "openai-responses"` 搭配 `baseUrl` 於 `api.openai.com`），OpenClaw 現在會自動啟用 OpenAI 伺服器端壓縮負載提示：
...
`responsesServerCompaction` 僅控制 `context_management` 注入。
