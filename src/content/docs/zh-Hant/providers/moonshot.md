...
<Tabs>
  ...
  <Tab title="Kimi Coding">
    **最適合用於：** 透過 Kimi Coding 端點處理程式碼相關任務。

    <Note>
    Kimi Coding 使用與 Moonshot (`moonshot/...`) 不同的 API 金鑰和提供者前綴 (`kimi/...`)。穩定的 API 模型參考是 `kimi/kimi-for-coding`；舊版參考 `kimi/kimi-code` 和 `kimi/k2p5` 仍被接受，並會正規化為該 API 模型 ID。
    </Note>
...
OpenClaw 也隨附 **Kimi** 作為 `web_search` 提供者，並由 Moonshot 網路
搜尋提供支援。
...
<AccordionGroup>
  <Accordion title="原生思考模式">
    ...
    請透過 `agents.defaults.models.<provider/model>.params` 針對每個模型進行設定：
    ...
    <Warning>
    當啟用 Moonshot 思考模式時，`tool_choice` 必須為 `auto` 或 `none`。為了相容性，OpenClaw 會將不相容的 `tool_choice` 值正規化為 `auto`。
    </Warning>

    Kimi K2.6 也接受一個選用的 `thinking.keep` 欄位，用於控制
  ...
  <Accordion title="Tool call id 淨化">
    Moonshot Kimi 提供的 tool_call id 格式類似於 `functions.<name>:<index>`。OpenClaw 會原封不動地保留它們，以便多輪工具呼叫能正常運作。
  ...
  <Accordion title="端點與模型參考參照">
    ...
    - Kimi 網路搜尋使用 `KIMI_API_KEY` 或 `MOONSHOT_API_KEY`，並預設為 `https://api.moonshot.ai/v1` 且搭配模型 `kimi-k2.6`。
    - 如有需要，請在 `models.providers` 中覆寫定價與內容中繼資料。
    - 如果 Moonshot 發布了模型的不同內容限制，請相應調整 `contextWindow`。
