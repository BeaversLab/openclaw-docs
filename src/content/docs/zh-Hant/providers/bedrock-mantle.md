...
<Tabs>
    ...
    **最適用於：** 您已經擁有 Mantle bearer token 的環境。
    ...
    <Steps>
      ...
      <Step title="驗證是否發現模型">
        ...
        發現的模型會顯示在 `amazon-bedrock-mantle` 提供者下。除非您想要覆寫預設值，否則不需要額外的設定。
      </Step>
    </Steps>
  ...
  <Tab title="IAM 憑證">
    **最適用於：** 使用與 AWS SDK 相容的憑證（共用設定、SSO、網頁身分、執行個體或工作角色）。
    ...
    <Steps>
      ...
      <Step title="驗證模型是否已探索">
        ```bash
        openclaw models list
        ```

        OpenClaw 會自動從憑證鏈產生 Mantle 持有人權杖 (bearer token)。
      </Step>
    </Steps>

    <Tip>
    當未設定 `AWS_BEARER_TOKEN_BEDROCK` 時，OpenClaw 會代表您從 AWS 預設憑證鏈產生持有人權杖，包括共用憑證/設定設定檔、SSO、網頁身分，以及執行個體或工作角色。
    </Tip>
...
OpenClaw 會嘗試從 AWS 預設憑證鏈產生 Mantle 持有人權杖。然後，它會透過查詢
...
<AccordionGroup>
  <Accordion title="推論支援">
    推論支援是根據包含 `thinking`、`reasoner` 或 `gpt-oss-120b` 等模式的模型 ID 推斷出來的。在探索過程中，OpenClaw 會自動為相符的模型設定 `reasoning: true`。
  </Accordion>

<Accordion title="端點無法使用">如果 Mantle 端點無法使用或未返回任何模型，則會靜默跳過該提供者。OpenClaw 不會報錯；其他已設定的提供者會繼續正常運作。</Accordion>

  <Accordion title="透過 Anthropic Messages 路由使用 Claude Opus 4.7">
    Mantle 也公開了一個 Anthropic Messages 路由，該路由透過相同的持有人驗證串流路徑傳輸 Claude 模型。Claude Opus 4.7 (`amazon-bedrock-mantle/claude-opus-4.7`) 可透過此路由呼叫，並使用提供者擁有的串流功能，因此 AWS 持有人權杖不會被視為 Anthropic API 金鑰。

    當您在 Mantle 提供者上鎖定 Anthropic Messages 模型時，OpenClaw 會針對該模型使用 `anthropic-messages` API 介面，而不是 `openai-completions`。驗證仍來自 `AWS_BEARER_TOKEN_BEDROCK` (或鑄造的 IAM 持有人權杖)。
  ...
  <Accordion title="與 Amazon Bedrock 提供者的關係">
    Bedrock Mantle 是與標準 [Amazon Bedrock](/zh-Hant/providers/bedrock) 提供者分開的提供者。Mantle 使用與 OpenAI 相容的 `/v1` 介面，而標準 Bedrock 提供者則使用原生的 Bedrock API。
...
<CardGroup cols={2}>
  <Card title="Amazon Bedrock" href="/zh-Hant/providers/bedrock" icon="cloud">
    專用於 Anthropic Claude、Titan 和其他模型的原生 Bedrock 提供者。
  </Card>
