...
- 當未設定 `AWS_BEARER_TOKEN_BEDROCK` 時，OpenClaw 可以使用相容 AWS SDK 的 IAM 憑證為您產生 Mantle 持有者權杖。
- 該持有者權杖與標準 [Amazon Bedrock](/en/providers/bedrock) 提供者所使用的 `AWS_BEARER_TOKEN_BEDROCK` 相同。
- 推理支援是根據包含 `thinking`、`reasoner` 或 `gpt-oss-120b` 等模式的模型 ID 來推斷的。
- 如果 Mantle 端點無法使用或未傳回任何模型，將會略過該提供者且不顯示任何訊息。
