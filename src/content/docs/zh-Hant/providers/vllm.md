...
OpenClaw 將 `vllm` 視為支援串流使用量計算的本機 OpenAI 相容供應商，因此狀態/內容 token 計數可以從 `stream_options.include_usage` 回應更新。
...
<Note>若您明確設定了 `models.providers.vllm`，將會跳過自動探索，且您必須手動定義模型。</Note>
...
<AccordionGroup>
  <Accordion title="代理樣式行為">
    vLLM 被視為代理樣式且相容 OpenAI 的 `/v1` 後端，而非原生
    OpenAI 端點。這意味著：
    ...
    | 提示快取提示 | 未發送 |
    | OpenAI 推理相容負載塑形 | 未套用 |
    | 隱藏的 OpenClaw 歸屬標頭 | 未於自訂基礎 URL 上注入 |

  </Accordion>

  <Accordion title="自訂基礎 URL">
    如果您的 vLLM 伺服器運行於非預設的主機或連接埠，請在明確的提供者設定中設定 `baseUrl`：
...
<AccordionGroup>
  ...
  <Accordion title="請求發生驗證錯誤">
    ...
    <Tip>
    如果您的 vLLM 伺服器未強制執行驗證，則任何非空的 `VLLM_API_KEY` 值均可作為 OpenClaw 的啟用訊號。
    </Tip>

  </Accordion>

  <Accordion title="未發現模型">
    自動探索需要設定 `VLLM_API_KEY` **並且**沒有明確的 `models.providers.vllm` 設定項目。如果您已手動定義提供者，OpenClaw 將會跳過探索並僅使用您宣告的模型。
  </Accordion>
</AccordionGroup>
