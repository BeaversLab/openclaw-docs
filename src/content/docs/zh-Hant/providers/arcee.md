...
[Arcee AI](https://arcee.ai) 透過與 OpenAI 相容的 API 提供對 Trinity 系列混合專家模型的存取。所有 Trinity 模型均採用 Apache 2.0 授權。
  ...
  <Tab title="透過 OpenRouter">
    <Steps>
      ...
      <Step title="設定預設模型">
        ...
        相同的模型參照適用於直接設定和 OpenRouter 設定（例如 `arcee/trinity-large-thinking`）。
      </Step>
    </Steps>
...
OpenClaw 目前隨附此內建的 Arcee 目錄：

| 模型參考                       | 名稱                   | 輸入 | 上下文 | 成本 (輸入/輸出每 1M) | 備註                       |
| ------------------------------ | ---------------------- | ---- | ------ | --------------------- | -------------------------- |
...
<Tip>入門預設設定將 `arcee/trinity-large-thinking` 設為預設模型。</Tip>
...
| 串流                                   | 是                          |
| 工具使用 / 函式呼叫                    | 是                          |
| 結構化輸出 (JSON 模式和 JSON 結構描述) | 是                          |
| 擴展思考                               | 是 (Trinity Large Thinking) |

<AccordionGroup>
  <Accordion title="環境備註">
    如果 Gateway 以守護程序 (launchd/systemd) 執行，請確保 `ARCEEAI_API_KEY`
  ...
  <Accordion title="OpenRouter 路由">
    ...
    OpenClaw 會根據您的驗證選擇以透明方式處理路由。請參閱
    [OpenRouter 供應商文件](/zh-Hant/providers/openrouter) 以取得 OpenRouter 特定
    的配置詳細資訊。
  </Accordion>
</AccordionGroup>
