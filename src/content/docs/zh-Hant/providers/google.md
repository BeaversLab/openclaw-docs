...
憑證優先順序是專用的 `webSearch.apiKey`，然後是 `GEMINI_API_KEY`，
接著是 `models.providers.google.apiKey`。`webSearch.baseUrl` 是選用的，
並存在於運營商代理或相容的 Gemini API 端點；當省略時，
Gemini 網路搜尋會重複使用 `models.providers.google.baseUrl`。請參閱
[Gemini search](/zh-Hant/tools/gemini-search) 以了解提供者特定的工具行為。
...
<Note>請參閱 [Image Generation](/zh-Hant/tools/image-generation) 以了解共用工具參數、提供者選擇和故障轉移行為。</Note>
...
內建的 `google` 外掛程式也透過共享的 `music_generate` 工具註冊音樂生成功能。
...
- 參考輸入：最多 10 張圖片
- 會話支援的執行會透過共享任務/狀態流程分離，包括 `action: "status"`
...
<Note>請參閱 [音樂生成](/zh-Hant/tools/music-generation) 以了解共享工具參數、提供者選擇和故障轉移行為。</Note>

## 文字轉語音

內建的 `google` 語音提供者使用具有 `gemini-3.1-flash-tts-preview` 的 Gemini API TTS 路徑。
...
- 語音備忘錄輸出：Google PCM 會被封裝為 WAV，並使用 `ffmpeg` 轉碼為 48 kHz Opus

Google 的批次 Gemini TTS 路徑會在完成的
`generateContent` 回應中傳回生成的音訊。若要獲得最低延遲的口語對話，請使用由
Gemini Live API 支援的 Google 即時語音提供者，而非批次 TTS。
...
Gemini API TTS 使用自然語言提示來控制樣式。設定
`audioProfile` 可在口語文字前附加可重複使用的樣式提示。當您的提示文字
提及具名發言者時，請設定 `speakerName`。

Gemini API TTS 也接受文字中的表達式方括號音訊標籤，
例如 `[whispers]` 或 `[laughs]`。若要在將標籤傳送至 TTS 的同時
將其排除在可見的聊天回覆之外，請將其放在 `[[tts:text]]...[[/tts:text]]`
區塊內：
...
適用於 Voice Call 和 Google Meet 等後端音訊橋接器。
...
<Note>
  Google Live API 透過 WebSocket 使用雙向音訊和函數呼叫。 OpenClaw 將電話/Meet 橋接音訊調整為 Gemini 的 PCM Live API 串流，並 將工具呼叫保留在共用的即時語音合約上。除非您需要採樣變更，否則請保留 `temperature` 未設定；因為對於 `temperature: 0`，Google Live 可以傳回不含音訊的逐字稿， 所以 OpenClaw 會省略非正值。 即使沒有 `languageCodes`，也已啟用 Gemini API 轉錄；目前的 Google SDK 會拒絕此 API
  路徑上的語言代碼提示。
</Note>
...
此測試也涵蓋 OpenAI 後端/WebRTC 路徑；Google 部分會產生與
控制 UI 對話所使用的相同受限 Live API 權杖形狀，開啟瀏覽器
WebSocket 端點，傳送初始設定負載，並等待
`setupComplete`。
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    ...
    - 使用
      `cachedContent` 或舊版 `cached_content` 來設定各模型或全域參數
    - 如果兩者都存在，`cachedContent` 優先
    - 範例值：`cachedContents/prebuilt-context`
    - Gemini 快取命中使用量會從
      上游 `cachedContentTokenCount` 正規化為 OpenClaw `cacheRead`
  ...
  <Accordion title="Gemini CLI JSON 使用說明">
    當使用 `google-gemini-cli` OAuth 提供者時，OpenClaw 會依照以下方式
    正規化 CLI JSON 輸出：

    - 回覆文字來自 CLI JSON 的 `response` 欄位。
    - 當 CLI 將 `usage` 留空時，使用量會退回到 `stats`。
  ...
  <Accordion title="環境與守護程序設定">
    如果 Gateway 以守護程序 方式執行，請確保 `GEMINI_API_KEY`
