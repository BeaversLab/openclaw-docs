...
`qwen` 擴充功能還在 **Standard**
DashScope 端點（而非 Coding Plan 端點）上公開了多模態功能：
...
<AccordionGroup>
  ...
  <Accordion title="功能計畫">
    `qwen` 擴充功能正被定位為整個 Qwen
    Cloud 表面的供應商主頁，而不僅僅是編碼/文字模型。

    - **文字/聊天模型：** 現已內建
    - **工具調用、結構化輸出、思考：** 繼承自 OpenAI 相容傳輸
    - **圖像生成：** 計劃在供應商外掛程式層級
    - **圖像/影片理解：** 現已在標準端點上內建
    - **語音/音訊：** 計劃在供應商外掛程式層級
    - **記憶嵌入/重排序：** 計劃通過嵌入介面卡表面
    - **影片生成：** 現已通過共享影片生成功能內建
