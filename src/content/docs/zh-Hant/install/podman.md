...
- `OPENCLAW_INSTALL_BROWSER` -- 預先安裝 Chromium 和 Xvfb 以進行瀏覽器自動化（設定為 `1` 以啟用）
...
該腳本會使用 `--userns=keep-id` 以您目前的 uid/gid 啟動容器，並將您的 OpenClaw 狀態綁定掛載（bind-mount）到容器中。
...
如果您執行了 `./scripts/podman/setup.sh --quadlet`，安裝程式會將 Quadlet 檔案安裝於：
...
工作階段和工作區在容器更換後都會保留。
Podman 設定還會為發布的閘道連接埠上的 `127.0.0.1` 和 `localhost` 預設 `gateway.controlUi.allowedOrigins`，以便本地儀表板與容器的非迴路綁定搭配運作。
...
- `OPENCLAW_PODMAN_BRIDGE_HOST_PORT` -- 主機連接埠對應到容器 `18790`
...
手動啟動器在最終確定容器/映像檔預設值之前會讀取 `~/.openclaw/.env`，因此您可以在那裡保存這些設定。

如果您使用非預設的 `OPENCLAW_CONFIG_DIR` 或 `OPENCLAW_WORKSPACE_DIR`，請為 `./scripts/podman/setup.sh` 以及後續的 `./scripts/run-openclaw-podman.sh launch` 指令設定相同的變數。存放庫本機啟動器不會在不同的 shell 之間保留自訂路徑覆寫。

Quadlet 說明：

- 產生的 Quadlet 服務刻意保持固定且經過強化的預設形態：`127.0.0.1` 發布連接埠、容器內的 `--bind lan`，以及 `keep-id` 使用者命名空間。
...
- 它會將 `~/.openclaw/.env` 作為諸如 `OPENCLAW_GATEWAY_TOKEN` 等數值的執行時期 `EnvironmentFile` 來讀取，但它不會使用手動啟動器針對 Podman 的特定覆寫允許清單。
...
- **透過主機 CLI 檢查健康狀態/狀態：** `openclaw gateway status --deep`（RPC 探測 + 額外的
  服務掃描）

## 疑難排解

- **設定或工作區權限被拒（EACCES）：** 容器預設以 `--userns=keep-id` 和 `--user <your uid>:<your gid>` 執行。請確保主機設定/工作區路徑由您目前的使用者擁有。
- **Gateway 啟動受阻（缺少 `gateway.mode=local`）：** 請確保 `~/.openclaw/openclaw.json` 存在並設定了 `gateway.mode="local"`。如果缺少，`scripts/podman/setup.sh` 會建立它。
- **容器 CLI 指令命中了錯誤的目標：** 請明確使用 `openclaw --container <name> ...`，或在您的 shell 中匯出 `OPENCLAW_CONTAINER=<name>`。
- **`openclaw update` 失敗並顯示 `--container`：** 這是預期的行為。請重新建置/拉取映像檔，然後重新啟動容器或 Quadlet 服務。
- **Quadlet 服務無法啟動：** 請執行 `systemctl --user daemon-reload`，然後執行 `systemctl --user start openclaw.service`。在無頭系統上，您可能還需要 `sudo loginctl enable-linger "$(whoami)"`。
- **SELinux 阻擋了掛載綁定：** 請保留預設的掛載行為；當 SELinux 處於 Enforcing 或 Permissive 模式時，啟動器會在 Linux 上自動新增 `:Z`。
