...
- `OPENCLAW_INSTALL_BROWSER` -- 預先安裝 Chromium 和 Xvfb 以用於瀏覽器自動化（設為 `1` 以啟用）
...
該腳本會以您目前的 uid/gid 使用 `--userns=keep-id` 啟動容器，並將您的 OpenClaw 狀態以 bind-mount 方式掛載到容器中。
...
在 macOS 上，Podman machine 可能會導致瀏覽器對閘道而言顯示為非本地端。如果在啟動後控制 UI 回報 device-auth 錯誤，請使用
[Podman and Tailscale](#podman--tailscale) 中的 Tailscale 指引。
...
如果您執行了 `./scripts/podman/setup.sh --quadlet`，設定程式會將 Quadlet 檔案安裝在：
...
工作階段和工作區都能在容器替換後繼續存在。
Podman 設定還會為 `127.0.0.1` 和已發布的閘道連接埠上的 `localhost` 準備 `gateway.controlUi.allowedOrigins`，以便本地儀表板與容器的非迴路綁定搭配使用。

手動啟動器的實用環境變數：

- `OPENCLAW_PODMAN_CONTAINER` -- 容器名稱（預設為 `openclaw`）
- `OPENCLAW_PODMAN_IMAGE` / `OPENCLAW_IMAGE` -- 要執行的映像檔
- `OPENCLAW_PODMAN_GATEWAY_HOST_PORT` -- 對映至容器 `18789` 的主機連接埠
- `OPENCLAW_PODMAN_BRIDGE_HOST_PORT` -- 對映至容器 `18790` 的主機連接埠
...
手動啟動器會在確認容器/映像檔預設值之前讀取 `~/.openclaw/.env`，因此您可以將這些設定保存在那裡。

如果您使用非預設的 `OPENCLAW_CONFIG_DIR` 或 `OPENCLAW_WORKSPACE_DIR`，請為 `./scripts/podman/setup.sh` 以及之後的 `./scripts/run-openclaw-podman.sh launch` 指令設定相同的變數。存放庫本機啟動器不會跨 shell 保留自訂路徑覆寫。

Quadlet 說明：

- 產生的 Quadlet 服務刻意保持固定且強化的預設形態：`127.0.0.1` 發布連接埠、容器內的 `--bind lan`，以及 `keep-id` 使用者命名空間。
- 它會固定使用 `OPENCLAW_NO_RESPAWN=1`、`Restart=on-failure` 和 `TimeoutStartSec=300`。
- 它會發布 `127.0.0.1:18789:18789`（閘道）和 `127.0.0.1:18790:18790`（橋接器）。
- 它會將 `~/.openclaw/.env` 作為執行時期的 `EnvironmentFile` 來讀取，例如用於 `OPENCLAW_GATEWAY_TOKEN` 等值，但它不會使用手動啟動器特定於 Podman 的覆寫允許清單。
...
- **透過主機 CLI 檢查健康狀態：** `openclaw gateway status --deep` (RPC 探測 + 額外
  服務掃描)

## 疑難排解

- **設定檔或工作區權限被拒 (EACCES)：** 容器預設以 `--userns=keep-id` 和 `--user <your uid>:<your gid>` 執行。請確保主機上的設定/工作區路徑由您目前的使用者所擁有。
- **Gateway 啟動受阻 (缺少 `gateway.mode=local`)：** 確認 `~/.openclaw/openclaw.json` 存在並設定 `gateway.mode="local"`。如果缺少該檔案，`scripts/podman/setup.sh` 將會建立它。
- **容器 CLI 命令作用於錯誤的目標：** 請明確使用 `openclaw --container <name> ...`，或在您的 shell 中匯出 `OPENCLAW_CONTAINER=<name>`。
...
- **SELinux 阻擋 bind 掛載：** 請勿變更預設的掛載行為；當 SELinux 處於 Enforcing 或 Permissive 模式時，啟動器會在 Linux 上自動新增 `:Z`。
