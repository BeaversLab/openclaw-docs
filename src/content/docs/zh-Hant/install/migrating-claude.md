...
<AccordionGroup>
  <Accordion title="指令與記憶">
    - 專案 `CLAUDE.md` 和 `.claude/CLAUDE.md` 內容會被複製或附加到 OpenClaw 代理工作區 `AGENTS.md` 中。
  ...
  <Accordion title="技能與指令">
    - 帶有 `SKILL.md` 檔案的 Claude 技能會被複製到 OpenClaw 工作區的技能目錄中。
    - 位於 `.claude/commands/` 或 `~/.claude/commands/` 下的 Claude 指令 Markdown 檔案會被轉換為帶有 `disable-model-invocation: true` 的 OpenClaw 技能。
