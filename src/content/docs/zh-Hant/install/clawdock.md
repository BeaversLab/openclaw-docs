...
如果您尚未設置 Docker，請從 [Docker](/zh-Hant/install/docker) 開始。
...
ClawDock 使用與 [Docker](/zh-Hant/install/docker) 中描述的相同的 Docker 配置分割：

- `<project>/.env` 用於 Docker 特定的值，如映像名稱、端口和網關令牌
- `~/.openclaw/.env` 用於基於環境變數的提供者金鑰和機器人權杖
- `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` 用於儲存的提供者 OAuth/API 金鑰驗證
...
- [Docker VM 執行時](/zh-Hant/install/docker-vm-runtime)
