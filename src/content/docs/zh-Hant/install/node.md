...
OpenClaw 需要 **Node 22.19 或更新版本**。**Node 24 是安裝、CI 和發行工作流程的預設與推薦執行環境**。Node 22 仍透過活躍的 LTS 線路獲得支援。[安裝腳本](/zh-Hant/install#alternative-install-methods) 會自動偵測並安裝 Node——本頁面適用於您想要自行設定 Node 並確保一切配置正確（版本、PATH、全域安裝）的情況。
