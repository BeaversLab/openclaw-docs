...
- **沙盒注意事項**：當啟用沙盒時，預設的沙盒後端會使用 Docker，但沙盒預設為關閉，且**不**需要完整的閘道在 Docker 中執行。此外也提供 SSH 和 OpenShell 沙盒後端。請參閱 [沙盒](/zh-Hant/gateway/sandboxing)。
...
- 足夠的磁碟空間以存放映像檔和日誌
- 如果在 VPS/公開主機上執行，請審閱
  [網路暴露的安全防護](/zh-Hant/gateway/security)，
...
此路由受到 Gateway 驗證保護。請勿暴露額外的
公開 `/metrics` 連接埠或未經驗證的反向代理路徑。請參閱
...
[Docker VM 執行時 - 什麼會保留在何處](/zh-Hant/install/docker-vm-runtime#what-persists-where)。

**磁碟增長熱點**：請留意 `media/`、session JSONL 檔案、共用的
SQLite 狀態資料庫、已安裝的插件套件根目錄，以及位於 `/tmp/openclaw/` 下的輪替檔案紀錄。
...
如果您是透過較舊的 `scripts/shell-helpers/clawdock-helpers.sh` 原始路徑安裝 ClawDock，請重新執行上述安裝指令，讓您的本機協助程式檔案能追蹤到新位置。

然後使用 `clawdock-start`、`clawdock-stop`、`clawdock-dashboard` 等。執行
`clawdock-help` 以取得所有指令的說明。
...
<Accordion title="Shared-network security note">`openclaw-cli` 使用 `network_mode: "service:openclaw-gateway"`，以便 CLI 指令可以透過 `127.0.0.1` 存取閘道。請將此視為共用的 信任邊界。Compose 設定會捨棄 `NET_RAW`/`NET_ADMIN` 並在 `openclaw-gateway` 和 `openclaw-cli` 上啟用 `no-new-privileges`。</Accordion>

  <Accordion title="Docker Desktop DNS failures in openclaw-cli">
    某些 Docker Desktop 設定在捨棄 `NET_RAW` 後，會導致來自共享網路
    `openclaw-cli` sidecar 的 DNS 查詢失敗，這在 `openclaw plugins install` 等 npm 支援的命令中會
    顯示為 `EAI_AGAIN`。
    請保留預設的強化 compose 檔案以進行正常閘道操作。下方的
    本機覆寫透過還原 Docker 的預設功能來放寬 CLI 容器的安全性姿態，因此僅將其用於
    需要套件登錄檔存取權的一次性 CLI 命令，而非作為您的預設 Compose
    叫用方式：
    ...
    如果您已經建立了長期執行的 `openclaw-cli` 容器，請使用相同的覆寫
  ...
  <Accordion title="Permissions and EACCES">
    ...
    相同的不符情況可能會顯示為外掛警告，例如
    `blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root)`
    緊接著 `plugin present but blocked`。這表示程序 uid 和
    已掛載的外掛目錄擁有者不一致。建議優先以預設 uid 1000 執行容器並修正
    掛載目錄的擁有權。只有在您有意長期以 root 身分執行
    OpenClaw 時，才將 `/path/to/openclaw-config/npm` 變更為 `root:root`。

  </Accordion>

  <Accordion title="Faster rebuilds">
    請排序您的 Dockerfile，以便快取相依性層。這樣可以避免除非 lockfile 變更，
    否則重新執行 `pnpm install`：
  ...
  <Accordion title="進階容器選項">
    預設映像檔以安全性為優先，並以非 root 身分 `node`。若要使用
    功能更完整的容器：

    1. **保存 `/home/node`**：`export OPENCLAW_HOME_VOLUME="openclaw_home"`
    2. **建置系統相依項**：`export OPENCLAW_IMAGE_APT_PACKAGES="git curl jq"`
    3. **建置 Python 相依項**：`export OPENCLAW_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **建置 Playwright Chromium**：`export OPENCLAW_INSTALL_BROWSER=1`
    5. **或是將 Playwright 瀏覽器安裝至已保存的磁碟區**：
       ```bash
       docker compose run --rm openclaw-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **保存瀏覽器下載內容**：使用 `OPENCLAW_HOME_VOLUME` 或
...
<Accordion title="OpenAI Codex OAuth (無頭模式 Docker)">如果您在精靈中選擇 OpenAI Codex OAuth，它會開啟瀏覽器 URL。在 Docker 或無頭模式環境中，請複製您重新導向後的完整 URL，並將其貼回精靈中以完成驗證。</Accordion>
  ...
  <Accordion title="基礎映像檔中繼資料">
    主要的 Docker 執行時映像檔使用 `node:24-bookworm-slim`，並包含 `tini` 作為進入點初始化程序 (PID 1)，以確保殭屍程序被回收，且在長時間執行的容器中能正確處理訊號。它會發布 OCI 基礎映像檔註解，包括 `org.opencontainers.image.base.name`、
    `org.opencontainers.image.source` 等。Node 基礎摘要會透過 Dependabot Docker 基礎映像檔 PR 重新整理；發行版本組建不會執行
    發行版升級層。請參閱
...
在獨立的 Docker 容器內執行代理程式工具執行 (Shell、檔案讀寫等)，而閘道本身則保留在主機上。這讓您能對不受信任或
多租戶的代理程式工作階段建立嚴格的隔離，而不需要將整個
閘道容器化。

沙箱範圍可以是每個代理程式 (預設)、每個工作階段，或是共用。每個範圍
都有自己的工作區，掛載於 `/workspace`。您也可以設定
允許/拒絕工具原則、網路隔離、資源限制以及瀏覽器
容器。

如需完整設定、映像檔、安全性說明和多代理程式設定檔，請參閱：

- [沙盒](/zh-Hant/gateway/sandboxing) -- 完整的沙盒參考資料
- [OpenShell](/zh-Hant/gateway/openshell) -- 存取沙盒容器的互動式 Shell
- [多重代理程式沙盒與工具](/zh-Hant/tools/multi-agent-sandbox-tools) -- 每個代理程式的覆寫設定
...
若未使用來源檢出進行 npm 安裝，請參閱 [沙盒 § 映像檔與設定](/zh-Hant/gateway/sandboxing#images-and-setup) 以取得內聯 `docker build` 指令。

## 疑難排解

<AccordionGroup>
  <Accordion title="缺少映像檔或沙盒容器無法啟動">
    使用
    [`scripts/sandbox-setup.sh`](https://github.com/openclaw/openclaw/blob/main/scripts/sandbox-setup.sh)
    建置沙盒映像檔（來源檢出），或使用 [沙盒 § 映像檔與設定](/zh-Hant/gateway/sandboxing#images-and-setup) 中的內聯 `docker build` 指令（npm 安裝），
    或將 `agents.defaults.sandbox.docker.image` 設定為您的自訂映像檔。
    容器會依需求為每個工作階段自動建立。
  </Accordion>

<Accordion title="沙盒中的權限錯誤">將 `docker.user` 設定為符合您掛載工作區所有權的 UID:GID， 或變更工作區資料夾的所有權 (chown)。</Accordion>

<Accordion title="在沙盒中找不到自訂工具">OpenClaw 使用 `sh -lc` (login shell) 執行指令，這會載入 `/etc/profile` 且可能重設 PATH。設定 `docker.env.PATH` 以預先加入您的 自訂工具路徑，或在 Dockerfile 中於 `/etc/profile.d/` 下新增腳本。</Accordion>
...
- [安裝總覽](/zh-Hant/install) — 所有安裝方法
- [Podman](/zh-Hant/install/podman) — Podman 做為 Docker 的替代方案
- [ClawDock](/zh-Hant/install/clawdock) — Docker Compose 社群設定
- [更新](/zh-Hant/install/updating) — 讓 OpenClaw 保持最新狀態
- [配置](/zh-Hant/gateway/configuration) — 安裝後的閘道配置
