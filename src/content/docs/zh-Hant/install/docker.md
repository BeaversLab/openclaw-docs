...
<Note>從儲存庫根目錄執行 `docker compose`。如果您啟用了 `OPENCLAW_EXTRA_MOUNTS` 或 `OPENCLAW_HOME_VOLUME`，設定腳本會寫入 `docker-compose.extra.yml`；請使用 `-f docker-compose.yml -f docker-compose.extra.yml` 將其包含在內。</Note>
...
<Note>在 `gateway.bind` 中使用綁定模式值 (`lan` / `loopback` / `custom` / `tailnet` / `auto`)，而不要 使用主機別名，例如 `0.0.0.0` 或 `127.0.0.1`。</Note>
...
<Accordion title="共享網路安全性說明">`openclaw-cli` 使用 `network_mode: "service:openclaw-gateway"`，讓 CLI 指令可以透過 `127.0.0.1` 存取 閘道。請將此視為共同的信任邊界。Compose 配置會捨棄 `NET_RAW`/`NET_ADMIN` 並在 `openclaw-cli` 上啟用 `no-new-privileges`。</Accordion>
...
<Accordion title="OpenAI Codex OAuth（無頭 Docker）">如果您在精靈中選擇 OpenAI Codex OAuth，它會開啟瀏覽器 URL。在 Docker 或無頭 設定中，複製您前往的完整重新導向 URL 並將其貼回精靈中以完成驗證。</Accordion>
...
<Accordion title="沙箱中的權限錯誤">將 `docker.user` 設定為符合您掛載工作區所有權的 UID:GID，或是對工作區資料夾執行 chown。</Accordion>

<Accordion title="在沙盒中找不到自訂工具">OpenClaw 使用 `sh -lc` (login shell) 執行命令，這會載入 `/etc/profile` 並可能重置 PATH。請設定 `docker.env.PATH` 以新增您的自訂工具路徑，或在您的 Dockerfile 中 `/etc/profile.d/` 下新增一個腳本。</Accordion>
