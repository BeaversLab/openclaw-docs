...
<Step title="Connect via SSH">```exec ssh user@gateway-host ```</Step>
...
<Step title="Install Node.js 24">```exec curl -fsSL https://deb.nodesource.com/setup_24.x | sudo -E bash - sudo apt install -y nodejs node --version ```</Step>
...
<Step title="安裝 OpenClaw">```exec curl -fsSL https://openclaw.ai/install.sh | bash ```</Step>
...
<Step title="驗證">```exec openclaw status sudo systemctl status openclaw journalctl -u openclaw -f ```</Step>
