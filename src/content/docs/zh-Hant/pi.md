...
系統提示詞是在 `buildAgentSystemPrompt()` (`system-prompt.ts`) 中建構的。它組裝了一個完整的提示詞，其中包含工具、工具呼叫風格、安全防護、OpenClaw 控制、技能、文件、工作區、沙箱、訊息傳遞、助理輸出指令、語音、靜默回覆、心跳、執行時期元資料等部分，以及在啟用時的記憶體和反應，還有選用的內容檔案和額外的系統提示詞內容。針對子代理程式使用的最小提示詞模式，這些部分會被修剪。
