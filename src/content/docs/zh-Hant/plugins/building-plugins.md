...
`operator.admin`，即使外掛請求了更狹窄的範圍也是如此。

需記住的 Hook guard 語意：

- `before_tool_call`：`{ block: true }` 是終止的，並會停止優先順序較低的處理程式。
- `before_tool_call`：`{ block: false }` 被視為不做決定。
- `before_tool_call`：`{ requireApproval: true }` 會暫停代理執行，並透過執行核准覆蓋層、Telegram 按鈕、Discord 互動，或任何頻道上的 `/approve` 指令提示使用者批准。
- `before_install`：`{ block: true }` 是終止的，並會停止優先順序較低的處理程式。
- `before_install`：`{ block: false }` 被視為不做決定。
- `message_sending`：`{ cancel: true }` 是終止的，並會停止優先順序較低的處理程式。
- `message_sending`：`{ cancel: false }` 被視為不做決定。

`/approve` 指令會透過有邊界的後援機制來處理 exec 和 plugin 核准：當找不到 exec核准 id 時，OpenClaw 會透過 plugin核准 重試相同的 id。Plugin 核准轉發可以透過 config 中的 `approvals.plugin` 獨立設定。
...
工具是 LLM 可以呼叫的型別化函式。它們可以是必備的（始終可用）或選用的（使用者選擇加入）：
...
對於提供者插件，除非介面確實是通用的，否則請將提供者專屬的輔助函式保留在這些套件根目錄的 barrel 中。目前內建的範例包括：

- Anthropic：Claude stream 包裝器與 `service_tier` / beta 輔助函式
- OpenAI：provider 建構器、預設模型輔助函式、即時提供者
- OpenRouter：provider 建構器加上上手/設定輔助函式

如果輔助函式僅在一個內建的提供者套件中有用，請將其保留在該套件根目錄的介面上，而不是將其提升至 `openclaw/plugin-sdk/*`。

部分生成的 `openclaw/plugin-sdk/<bundled-id>` 輔助介面為了內建插件的維護與相容性仍然存在，例如 `plugin-sdk/feishu-setup` 或 `plugin-sdk/zalo-setup`。請將這些視為保留的介面，而非新第三方插件的預設模式。
...
<Check>內部匯入使用本機模組，而非 SDK 自身匯入</Check>
<Check>測試通過 (`pnpm test -- <bundled-plugin-root>/my-plugin/`)</Check>
<Check>`pnpm check` 通過 (儲存庫內外掛程式)</Check>

## Beta 版發布測試

1. 留意 [openclaw/openclaw](https://github.com/openclaw/openclaw/releases) 上的 GitHub 發布標籤，並透過 `Watch` > `Releases` 訂閱。Beta 標籤看起來像 `v2026.3.N-beta.1`。您也可以開啟官方 OpenClaw X 帳號 [@openclaw](https://x.com/openclaw) 的通知以接收發布公告。
2. Beta 標籤出現後，請立即針對其測試您的外掛程式。穩定版發布前的時間通常只有幾個小時。
3. 測試後，在 `plugin-forum` Discord 頻道的您外掛程式主題串中貼文，說明 `all good` 或遇到的問題。如果您還沒有主題串，請建立一個。
4. 如果有任何問題，請開啟或更新一個標題為 `Beta blocker: <plugin-name> - <summary>` 的議題，並套用 `beta-blocker` 標籤。將議題連結放在您的主題串中。
5. 向 `main` 開啟一個標題為 `fix(<plugin-id>): beta blocker - <summary>` 的 PR，並在 PR 和您的 Discord 主題串中連結該議題。貢獻者無法標記 PR，因此標題是維護者和自動化的 PR 端訊號。有 PR 的阻礙性問題會被合併；沒有 PR 的可能仍會發布。維護者會在 Beta 測試期間監看這些主題串。
6. 沒有消息就是好消息。如果您錯過了時段，您的修正可能會在下一個週期發布。
