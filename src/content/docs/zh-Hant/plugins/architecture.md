...
<Info>這是**深度架構參考文件**。如需實用指南，請參閱： - [安裝並使用外掛](/en/tools/plugin) — 使用者指南 - [快速入門](/en/plugins/building-plugins) — 第一個外掛教學 - [頻道外掛](/en/plugins/sdk-channel-plugins) — 建構訊息頻道 - [提供者外掛](/en/plugins/sdk-provider-plugins) — 建構模型提供者 - [SDK 概覽](/en/plugins/sdk-overview) — import map 與註冊 API</Info>
...
使用 `openclaw plugins inspect <id>` 來查看外掛的形狀與能力細目。詳情請參閱 [CLI 參考資料](/en/cli/plugins#inspect)。
...
如需完整的啟動序列，請參閱 [載入流程](#load-pipeline)。
...
- manifest 元數據：`providerAuthEnvVars` 用於在執行時期載入前進行低成本的提供者環境授權查找，`providerAuthAliases` 用於共用授權的提供者變體，`channelEnvVars` 用於在執行時期載入前進行低成本的頻道環境/設定查找，以及 `providerAuthChoices` 用於在執行時期載入前進行低成本的入門/授權選擇標籤和 CLI 標誌元數據
- 設定時期鉤子：`catalog` / 舊版 `discovery` 加上 `applyConfigDefaults`
...
當供應商具有基於環境變數的憑證，且通用的 auth/status/model-selector 路徑應在不載入外掛程式執行時期的情況下看見這些憑證時，請使用 manifest `providerAuthEnvVars`。當一個供應商 ID 應重複使用另一個供應商 ID 的環境變數、auth 概要檔案、由設定支援的 auth 以及 API 金鑰 onboarding 選擇時，請使用 manifest `providerAuthAliases`。當 onboarding/auth-choice CLI 介面應在不載入供應商執行時期的情況下知道供應商的選擇 ID、群組標籤以及簡單的單旗標 auth 接線時，請使用 manifest `providerAuthChoices`。請將供應商執行時期 `envVars` 用於操作員面向的提示，例如 onboarding 標籤或 OAuth client-id/client-secret 設定變數。

當通道具有環境驅動的身份驗證或設定，且通用 shell-env 後備機制、設定/狀態檢查或設定提示應在不載入通道執行時的情況下看到這些資訊時，請使用 manifest `channelEnvVars`。
...
| #   | Hook                              | 功能                                                                                            | 使用時機                                                                                                    |
| --- | --------------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| 1   | `catalog`                         | 在 `models.json` 生成期間，將提供者設定發佈到 `models.providers`                                | 提供者擁有目錄或基本 URL 預設值                                                                             |
...
| 4   | `normalizeTransport`              | 在通用模型組裝之前，正規化提供者系列（provider-family）`api` / `baseUrl`                        | 提供者擁有同一傳輸家族中自訂提供者 ID 的傳輸清理權責                                                        |
| 5   | `normalizeConfig`                 | 在執行時/提供者解析之前正規化 `models.providers.<id>`                                           | 提供者需要隨附於插件的設定清理；內建的 Google 家族協助程式也會支援受支援的 Google 設定項目                  |
| 6   | `applyNativeStreamingUsageCompat` | 對設定提供者套用原生串流使用相容性重寫                                                          | 提供者需要端點驅動的原生串流使用中繼資料修復                                                                |
| 7   | `resolveConfigApiKey`             | 在執行階段授權載入之前，解析配置提供者的 env-marker 授權                                        | 提供者具有提供者擁有的 env-marker API 金鑰解析功能；`amazon-bedrock` 此處也具有內建的 AWS env-marker 解析器 |
| 8   | `resolveSyntheticAuth`            | 公開本地/自託管或配置支援的授權，而不持久化純文字                                               | 提供者可以使用合成/本機憑證標記操作                                                                         |
| 9   | `resolveExternalAuthProfiles`     | 疊加提供者擁有的外部身份驗證設定檔；預設 `persistence` 為 CLI/應用程式擁有的憑證 `runtime-only` | 提供者重複使用外部授權憑證，而不持久化複製的重新整理權杖                                                    |
...
| 24  | `formatApiKey`                    | 身份驗證設定檔格式化器：儲存的設定檔會變成執行時 `apiKey` 字串                                  | 供應商儲存額外驗證元資料，並需要自訂執行時權杖形狀                                                          |
| 25  | `refreshOAuth`                    | OAuth 重新整理覆寫，用於自訂重新整理端點或重新整理失敗策略                                      | 提供者不適合使用共享的 `pi-ai` 刷新器                                                                       |
...
| 32  | `augmentModelCatalog`             | 在發現後附加的合成/最終目錄行                                                                   | Provider 需要合成向後相容行於 `models list` 和選擇器中                                                      |
| 33  | `isBinaryThinking`                | 針對二元思維提供者的開/關推論切換                                                               | 提供者僅公開二元思維開/關                                                                                   |
| 34  | `supportsXHighThinking`           | `xhigh` 選定模型的推理支援                                                                      | Provider 僅在部分模型上需要 `xhigh`                                                                         |
| 35  | `resolveDefaultThinkingLevel`     | 特定模型系列的預設 `/think` 層級                                                                | Provider 擁有模型系列的預設 `/think` 政策                                                                   |
...
| 38  | `resolveUsageAuth`                | 解析 `/usage` 的使用/計費憑證及相關狀態介面                                                     | 提供者需要自訂使用量/配額 Token 解析或不同的使用量憑證                                                      |
...
匹配的 provider 外掛程式，然後回退到其他具有 hook 能力的 provider 外掛程式，
直到其中一個實際更改了模型 ID 或傳輸/配置為止。這確保了
alias/compat provider 填充層能正常運作，而無需呼叫者知道哪個
內綑外掛程式擁有該重寫權。如果沒有 provider hook 重寫受支援的
Google 系列配置項目，內綑的 Google 配置正規化工具仍會套用
該相容性清理。
...
- Anthropic 使用 `resolveDynamicModel`、`capabilities`、`buildAuthDoctorHint`、
  ...
  和 `wrapStreamFn`，因為它擁有 Claude 4.6 向後相容性、
  provider 系列提示、驗證修復指導、使用端點整合、
  prompt-cache 資格、感知驗證的配置預設值、Claude
  預設/自適應思考策略，以及針對
  beta 標頭、`/fast` / `serviceTier` 和 `context1m` 的 Anthropic 特定串流塑形。
- Anthropic 的 Claude 專用串流輔助函式目前仍保留在打包插件自身的 public `api.ts` / `contract-api.ts` 縫隙中。該套件表面匯出 `wrapAnthropicProviderStream`、`resolveAnthropicBetas`、
  `resolveAnthropicFastMode`、`resolveAnthropicServiceTier` 以及底層的
  Anthropic 包裝器建構器，而不是圍繞單一供應商的 beta-header 規則來擴展通用 SDK。
- OpenAI 使用 `resolveDynamicModel`、`normalizeResolvedModel` 和
  ...
  `openai-completions` -> `openai-responses` 正規化、Codex 感知授權
  提示、Spark 抑制、合成 OpenAI 列表行，以及 GPT-5 思維 /
  即時模型策略；`openai-responses-defaults` 串流系列擁有
  用於歸屬標頭、
  `/fast`/`serviceTier`、文字冗長度、原生 Codex 網頁搜尋、
  推理相容負載塑形以及 Responses 上下文管理的共享原生 OpenAI Responses 包裝器。
- OpenRouter 使用 `catalog` 加上 `resolveDynamicModel` 和
  `prepareDynamicModel`，因為該供應商是透傳式的，且可能在 OpenClaw 的靜態目錄更新之前公開新的
  模型 ID；它也使用
  `capabilities`、`wrapStreamFn` 和 `isCacheTtlEligible`，以便將
  供應商特定的請求標頭、路由中繼資料、推理修補和
  prompt-cache 策略保留在核心之外。其重播策略來自
  `passthrough-gemini` 系列，而 `openrouter-thinking` 串流系列
  則擁有代理推理注入和不支援模型 / `auto` 跳過。
...
- Google AI Studio 和 Gemini CLI OAuth 使用 `resolveDynamicModel`、
  ...
  推理輸出模式以及現代模型匹配，而
  `google-thinking` 串流系列則擁有 Gemini 思考酬載正規化；
...
- Anthropic Vertex 使用 `buildReplayPolicy` 透過
  `anthropic-by-model` 重播系列，讓 Claude 特定的重播清理
  範圍保持在 Claude ID，而不是每個 `anthropic-messages` 傳輸。
- Amazon Bedrock 使用 `buildReplayPolicy`、`matchesContextOverflowError`、
  ...
  Bedrock 專用的限流/未就緒/上下文溢出錯誤分類，適用於透過 Bedrock 連線的 Anthropic 流量；其重試策略仍共用相同的
  僅限 Claude 的 `anthropic-by-model` 防護。
- OpenRouter、Kilocode、Opencode 和 Opencode Go 使用 `buildReplayPolicy`
  透過 `passthrough-gemini` 重試系列，因為它們透過 OpenAI 相容的傳輸代理 Gemini
  模型，並且需要 Gemini 思維簽章清理，而不需要原生的 Gemini 重試驗證或
  bootstrap 重寫。
- MiniMax 使用 `buildReplayPolicy` 透過
  `hybrid-anthropic-openai` 重試系列，因為一個提供者同時擁有
  Anthropic 訊息和 OpenAI 相容語意；它在 Anthropic 端保留僅限 Claude 的
  思維區塊刪除，同時將推理輸出模式覆蓋回原生模式，而 `minimax-fast-mode` 串流系列擁有
  共用串流路徑上的快速模式模型重寫。
- Moonshot 使用 `catalog` 加上 `wrapStreamFn`，因為它仍使用共用的
  OpenAI 傳輸，但需要提供者擁有的思維載荷正規化；
  `moonshot-thinking` 串流系列將設定加上 `/think` 狀態對應至其
  原生二進位思維載荷。
- Kilocode 使用 `catalog`、`capabilities`、`wrapStreamFn` 和
  `isCacheTtlEligible`，因為它需要提供者擁有的請求標頭、
  推理載荷正規化、Gemini 逐字稿提示和 Anthropic
  快取 TTL 閘控；`kilocode-thinking` 串流系列在共用代理串流路徑上保留 Kilo 思維
  插入，同時跳過 `kilo/auto` 和
  其他不支援明確推理載荷的代理模型 ID。
- Z.AI 使用 `resolveDynamicModel`、`prepareExtraParams`、`wrapStreamFn`、
  ...
  使用量授權 + 配額擷取；`tool-stream-default-on` 資料流系列
  將預設啟用的 `tool_stream` 包裝器排除在各提供商手寫的膠合程式碼之外。
...
- `textToSpeech` 會傳回檔案/語音備忘錄介面的正常核心 TTS 輸出載荷。
- 使用核心 `messages.tts` 組態和提供者選擇。
- 傳回 PCM 音訊緩衝區 + 取樣率。外掛程式必須為提供者重新取樣/編碼。
- `listVoices` 對於每個提供者來說是可選的。將其用於廠商擁有的語音選擇器或設定流程。
...
- 影片生成已經遵循相同的模式：
  ...
  - 功能/頻道外掛程式使用 `api.runtime.videoGeneration.*`
...
- 當沒有產生轉錄輸出時（例如跳過/不支援的輸入），會傳回 `{ text: undefined }`。
...
- `provider` 和 `model` 是每次執行的選用覆寫，而非持續的階段作業變更。
- OpenClaw 僅對受信任的呼叫者遵循這些覆寫欄位。
- 對於外掛程式擁有的後援執行，操作員必須選擇加入 `plugins.entries.<id>.subagent.allowModelOverride: true`。
- 使用 `plugins.entries.<id>.subagent.allowedModels` 將受信任的外掛程式限制在特定的正規 `provider/model` 目標，或使用 `"*"` 明確允許任何目標。
...
- `api.runtime.webSearch.*` 是需要搜尋行為但不依賴代理程式工具包裝函式的功能/頻道外掛程式的偏好共享介面。
...
- `auth`: 必填。使用 `"gateway"` 要求一般 Gateway 驗證，或使用 `"plugin"` 進行外掛程式管理的驗證/ webhook 驗證。
...
- 外掛程式路由必須明確宣告 `auth`。
- 除非 `replaceExisting: true`，否則會拒絕完全相同的 `path + match` 衝突，且一個外掛程式無法取代另一個外掛程式的路由。
- 具有不同 `auth` 層級的重疊路由將被拒絕。請將 `exact`/`prefix` 透傳鏈保持在相同的驗證層級上。
- `auth: "plugin"` 路由**不會**自動接收操作員執行時範圍。它們是用於外掛程式管理的 webhooks/簽章驗證，而非有特權的 Gateway 輔助程式呼叫。
- `auth: "gateway"` 路由在 Gateway 請求執行時範圍內執行，但該範圍是刻意設為保守的：
  - 共享密碼 bearer 驗證 (`gateway.auth.mode = "token"` / `"password"`) 會將外掛程式路由執行時範圍鎖定為 `operator.write`，即使呼叫端發送了 `x-openclaw-scopes`
  - 可信的攜帶身分的 HTTP 模式（例如私有入口上的 `trusted-proxy` 或 `gateway.auth.mode = "none"`）僅在明確存在該標頭時才遵守 `x-openclaw-scopes`
  - 如果在那些攜帶身分的插件路由請求中缺少 `x-openclaw-scopes`，運行時範圍將回退到 `operator.write`
- 實用規則：不要假設網關認證插件路由是隱含的管理介面。如果您的路由需要僅限管理員的行為，請要求使用攜帶身分的認證模式，並記錄明確的 `x-openclaw-scopes` 標頭合約。

## 外掛程式 SDK 匯入路徑

在編寫插件時，使用 SDK 子路徑而不是單一的 `openclaw/plugin-sdk` 導入：

- `openclaw/plugin-sdk/plugin-entry` 用於插件註冊原語。
- `openclaw/plugin-sdk/core` 用於通用共享的插件面向合約。
...
- 穩定的通道原語，例如 `openclaw/plugin-sdk/channel-setup`、
  ...
  传入提及策略輔助函式、信封格式化以及传入信封
  上下文輔助函式的共享主體。
  `channel-setup` 是狹窄的可選安裝設定縫合點。
  `setup-runtime` 是由 `setupEntry` /
  延遲啟動使用的執行時安全設定介面，包括導入安全的設定修補適配器。
  `setup-adapter-runtime` 是具備環境感知能力的帳戶設定適配器縫合點。
  `setup-tools` 是小型 CLI/歸檔/文件輔助縫合點 (`formatCliCommand`,
...
- 領域子路徑，例如 `openclaw/plugin-sdk/channel-config-helpers`、
  ...
  `openclaw/plugin-sdk/directory-runtime` 用於共享執行時/配置輔助工具。
  `telegram-command-config` 是 Telegram 自訂指令
  正規化/驗證的狹窄公共接縫，即使捆綁的
  Telegram 合約介面暫時無法使用，它仍然保持可用。
  `text-runtime` 是共享的文字/Markdown/日誌接縫，包括
  助手可見文字剝離、Markdown 渲染/分塊輔助工具、編校
  輔助工具、指令標籤輔助工具和安全文字實用程式。
- 特定於核准的通道接縫應該偏好外掛程式上的一個 `approvalCapability`
  合約。然後，核心會透過該功能讀取核准授權、傳遞、渲染、
  原生路由和延遲原生處理程式行為，而不是將核准行為混合到
  不相關的外掛程式欄位中。
- `openclaw/plugin-sdk/channel-runtime` 已被棄用，僅作為較舊外掛程式的
  相容性填充層保留。新程式碼應該改為導入更狹窄的
  通用原語，並且 repo 程式碼不應新增
  該填充層的導入。
- 捆綁的擴充功能內部保持私有。外部外掛程式應僅使用
  `openclaw/plugin-sdk/*` 子路徑。OpenClaw 核心/測試程式碼可以使用
  外掛程式套件根目錄下的 repo 公共進入點，例如 `index.js`、`api.js`、
  `runtime-api.js`、`setup-entry.js` 以及範圍狹窄的檔案，例如
  `login-qr-api.js`。切勿從核心或
  另一個擴充功能導入外掛程式套件的 `src/*`。
- Repo entry point split:
  `<plugin-package-root>/api.js` 是輔助程式/型別桶檔,
  `<plugin-package-root>/runtime-api.js` 是僅限執行時期的桶檔,
  `<plugin-package-root>/index.js` 是打包後的外掛入口,
  而 `<plugin-package-root>/setup-entry.js` 是設定外掛入口。
- Current bundled provider examples:
  - Anthropic 使用 `api.js` / `contract-api.js` 作為 Claude 串流輔助程式，
    例如 `wrapAnthropicProviderStream`、beta 標頭輔助程式，以及
    `service_tier` 解析。
  - OpenAI 使用 `api.js` 作為提供者建構器、預設模型輔助程式，以及
    即時提供者建構器。
  - OpenRouter 使用 `api.js` 作為其提供者建構器以及上手/設定
    輔助程式，而 `register.runtime.js` 仍可重新匯出通用
    `plugin-sdk/provider-stream` 輔助程式供 Repo 本地使用。
...
- 通用共享原語仍然是首選的公開 SDK 合約。一小組保留用於相容性的打包
  頻道品牌輔助縫隙仍然存在。請將這些視為打包維護/相容性縫隙，而不是新的
  第三方匯入目標；新的跨頻道合約仍應放置於
  通用 `plugin-sdk/*` 子路徑或外掛本地的 `api.js` /
  `runtime-api.js` 桶檔中。

Compatibility note:

- 避免在新程式碼中使用根目錄的 `openclaw/plugin-sdk` 桶檔。
- 優先先使用狹窄穩定的原語。較新的 setup/pairing/reply/
...
- 打包擴充功能特定的輔助桶檔預設並不穩定。如果
  輔助程式僅由打包擴充功能需要，請將其保留在擴充功能的
  本地 `api.js` 或 `runtime-api.js` 縫隙之後，而不是將其升級至
  `openclaw/plugin-sdk/<extension>`。
- 新的共享輔助接縫應該是通用的，而非特定頻道的。共享目標解析屬於 `openclaw/plugin-sdk/channel-targets`；特定頻道的內部實作應保留在擁有該外掛程式的本地 `api.js` 或 `runtime-api.js` 接縫之後。
- 諸如 `image-generation`、`media-understanding` 和 `speech` 等特定功能的子路徑之所以存在，是因為目前的內建/原生外掛程式正在使用它們。它們的僅存在並不意味著每個匯出的輔助函數都是長期凍結的外部合約。

## 訊息工具架構

外掛程式應擁有特定頻道的 `describeMessageTool(...)` 結構描述貢獻。請將特定提供者的欄位保留在外掛程式中，而非共享核心裡。

對於共享的可攜式結構描述片段，請重用透過 `openclaw/plugin-sdk/channel-actions` 匯出的通用輔助函數：
...
- `messaging.inferTargetChatType({ to })` 決定在目錄查詢之前，是否應將正規化目標視為 `direct`、`group` 或 `channel`。
- `messaging.targetResolver.looksLikeId(raw, normalized)` 告訴核心輸入內容是否應跳過目錄搜尋，直接進行類似 ID 的解析。
- 當核心在正規化後或目錄查詢失敗後需要最終的提供者擁有之解析結果時，`messaging.targetResolver.resolveTarget(...)` 是外掛程式的後援機制。
- 一旦目標解析完成，`messaging.resolveOutboundSessionRoute(...)` 負責建構特定提供者的會話路由。

建議的拆分：

- 使用 `inferTargetChatType` 進行應在搜尋對等/群組之前發生的類別決策。
- 使用 `looksLikeId` 進行「將此視為明確/原生目標 ID」的檢查。
- 使用 `resolveTarget` 進行特定提供者的正規化後援，而非廣泛的目錄搜尋。
- 請將提供者原生的 ID（如聊天 ID、執行緒 ID、JID、帳號代碼和房間 ID）保留在 `target` 值或特定提供者的參數中，而非通用 SDK 欄位裡。

## 基於配置的目錄

從組態衍生目錄條目的外掛程式應將該邏輯保留在外掛程式內，並重複使用來自
...
提供者外掛程式可以使用
`registerProvider({ catalog: { run(...) { ... } } })` 定義用於推論的模型目錄。
...
當外掛程式擁有提供者專屬的模型 ID、基底 URL
預設值，或需要驗證的模型元資料時，請使用 `catalog`。

`catalog.order` 控制外掛程式目錄相對於 OpenClaw
內建隱含提供者的合併時機：
...
- `resolveAccount(...)` 是執行時期路徑。它被允許假設憑證
  已完全具體化，並且在缺少必要機密時能快速失敗。
- 唯讀指令路徑，例如 `openclaw status`、`openclaw status --all`、
  `openclaw channels status`、`openclaw channels resolve` 以及診斷/組態
  修復流程，應該不需要僅為了描述組態就具體化執行時期憑證。
...
- 若僅為報告唯讀可用性，您不需要回傳原始權杖值。回傳 `tokenStatus: "available"`（以及對應的來源欄位）對於狀態式指令來說已足夠。
...
外掛程式目錄可以包含帶有 `openclaw.extensions` 的 `package.json`：
...
每個條目都會成為一個外掛程式。如果該套件列出了多個擴充功能，外掛程式 ID 將會變成 `name/<fileBase>`。

如果您的外掛程式匯入 npm 相依項，請將其安裝在該目錄中，以便 `node_modules` 可用（`npm install` / `pnpm install`）。

安全性防護：解析符號連結後，每個 `openclaw.extensions` 條目必須保留在外掛程式目錄內。逃離套件目錄的條目將被拒絕。

安全性提示：`openclaw plugins install` 會使用 `npm install --omit=dev --ignore-scripts` 安裝外掛程式相依項（無生命週期腳本，執行行時無開發相依項）。請保持外掛程式相依樹為「純 JS/TS」，並避免需要 `postinstall` 建置的套件。

選用項：`openclaw.setupEntry` 可以指向一個輕量級的僅設定模組。當 OpenClaw 需要已停用通道外掛程式的設定介面，或是當通道外掛程式已啟用但尚未設定時，它會載入 `setupEntry` 而非完整的外掛程式進入點。這能在您的主要外掛程式進入點也連接工具、掛勾或其他僅執行行程式碼時，保持啟動和設定的輕量化。

選用項：`openclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen` 可以在閘道預聽啟動階段，讓通道外掛程式採用相同的 `setupEntry` 路徑，即使通道已經設定過也一樣。

僅當 `setupEntry` 完全覆蓋了閘道開始監聽之前必須存在的啟動表面時，才使用此選項。實際上，這意味著 setup entry 必須註冊啟動所依賴的每個通道所擁有的能力，例如：
...
Core 在需要將舊版單一帳戶通道設定升級為 `channels.<id>.accounts.*` 而不加載完整插件 entry 時會使用該表面。Matrix 是當前內建的範例：當已存在命名帳戶時，它只會將 auth/bootstrap 金鑰移動到命名的升級帳戶中，並且它可以保留已配置的非正預設帳戶金鑰，而不是總是建立 `accounts.default`。
...
當這些啟動表面包含閘道 RPC 方法時，請將它們保留在插件特定的前綴上。Core 管理命名空間 (`config.*`、
`exec.approvals.*`、`wizard.*`、`update.*`) 仍然被保留，並且總是解析為
`operator.admin`，即使插件請求了更小的範圍。
...
通道插件可以透過 `openclaw.channel` 宣傳設定/發現元數據，並透過 `openclaw.install` 提供安裝提示。這使核心目錄不包含數據。
...
- `detailLabel`：用於更豐富的目錄/狀態表面的次要標籤
- `docsLabel`：覆寫文件連結的連結文字
- `preferOver`：此目錄條目應凌駕的優先級較低的插件/通道 ID
- `selectionDocsPrefix`、`selectionDocsOmitLabel`、`selectionExtras`：選擇表面複製控制
- `markdownCapable`：將通道標記為支援 markdown 以進行輸出格式化決策
- `exposure.configured`：當設定為 `false` 時，從已配置通道列表表面隱藏通道
- `exposure.setup`：當設定為 `false` 時，從互動式設定/配置選擇器中隱藏通道
...
- `preferSessionLookupForAnnounceTarget`: 在解析公告目標時優先使用會話查找
...
上下文引擎外掛擁有用於攝取、組裝和壓縮的會話上下文協調。使用 `api.registerContextEngine(id, factory)` 從您的外掛註冊它們，然後使用 `plugins.slots.contextEngine` 選擇活動引擎。
...
2. 加入類型化的外掛註冊/執行時介面
   擴展 `OpenClawPluginApi` 和/或 `api.runtime`，加入最小有用的類型化功能介面。
3. 連接核心 + 頻道/功能消費者
   頻道和功能外掛應該透過核心來消費新功能，
   而非直接匯入廠商實作。
4. 註冊廠商實作
...
這就是 OpenClaw 如何在保持主見的同時，不至於硬編碼為某一個提供商的世界觀。請參閱 [Capability Cookbook](/en/tools/capability-cookbook) 以獲取具體的檔案檢查清單和實作範例。
...
- 當功能/頻道外掛需要使用時，在 `src/plugins/runtime/*` 中公開外掛執行時
- `src/test-utils/plugin-registration.ts` 中的捕獲/測試輔助工具
