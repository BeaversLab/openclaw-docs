...
**Agent harness** 是單一已準備 OpenClaw agent 週次的低層級執行器。它不是模型提供者，不是通道，也不是工具註冊表。如需使用者導向的心智模型，請參閱 [Agent runtimes](/zh-Hant/concepts/agent-runtimes)。
...
請**勿**僅為了新增新的 LLM API 而註冊 harness。對於一般的 HTTP 或 WebSocket 模型 API，請建置 [provider plugin](/zh-Hant/plugins/sdk-provider-plugins)。
...
3. `auto` 會詢問已註冊的 harness 是否支援解析後的
   提供者/模型。
4. 如果沒有符合的已註冊 harness，除非已停用 PI 備援，否則 OpenClaw 會使用 PI。

外掛程式載具失敗會顯示為執行失敗。在 `auto` 模式下，僅當沒有已註冊的外掛程式載具支援解析出的供應商/模型時，才會使用 PI 備援機制。一旦外掛程式載具認領了某次執行，OpenClaw 將不會透過 PI 重放該回合，因為這可能會改變授權/執行時語意或導致重複的副作用。
...
關於操作員設定、模型前綴範例以及 Codex 專用設定，請參閱 [Codex Harness](/zh-Hant/plugins/codex-harness)。
...
當套件外掛程式的清單在 `contracts.agentToolResultMiddleware` 中宣告目標 runtime id 時，可以透過 `api.registerAgentToolResultMiddleware(...)` 附加與 runtime 無關的 tool-result 中介軟體。這個受信任的縫隙是用於在 PI 或 Codex 將工具輸出餵回模型之前必須執行的非同步 tool-result 轉換。

舊版隨附外掛程式仍可針對僅限 Codex 應用程式伺服器的中介軟體使用 `api.registerCodexAppServerExtensionFactory(...)`，但新的結果轉換應使用執行階段中立 API。僅限 Pi 的 `api.registerEmbeddedExtensionFactory(...)` 掛鉤已被移除；Pi 工具結果轉換必須使用執行階段中立中介軟體。
...
當此模式運行時，Codex 擁有原生執行緒 ID、恢復行為、壓縮以及應用程式伺服器執行。OpenClaw 仍然擁有聊天通道、可見的逐字稿鏡像、工具政策、審核、媒體傳遞和工作階段選擇。當您需要證明只有 Codex 應用程式伺服器路徑可以聲稱該運行時，請使用 provider/model `agentRuntime.id: "codex"`。明確的外掛執行時會以封閉方式失敗；Codex 應用程式伺服器選擇失敗和執行時失敗不會透過 PI 進行重試。

## 運行時嚴格性

預設情況下，OpenClaw 使用 `auto` 提供者/模型執行時原則：註冊的插件掛接可以聲明提供者/模型配對，若無匹配項，則由 PI 處理該輪次。官方 OpenAI 提供者上的 OpenAI agent 引用預設為 Codex。當缺少掛接選擇應導致失敗而非透過 PI 路由時，請使用明確的提供者/模型插件執行時，例如 `agentRuntime.id: "codex"`。選定的插件掛接失敗總是會嚴重失敗。這不會阻擋明確的提供者/模型 `agentRuntime.id: "pi"`。
...
這可將文字、圖片、影片、音樂、TTS、核准和訊息工具輸出保留在與 PI 支援的執行相同的傳遞路徑上。

## 目前的限制

- 公開匯入路徑是通用的，但部分 attempt/result 型別別名仍帶有 `Pi` 名稱以保持相容性。
...
- [執行時期輔助程式](/zh-Hant/plugins/sdk-runtime)
- [供應商外掛程式](/zh-Hant/plugins/sdk-provider-plugins)
- [Codex 駝具](/zh-Hant/plugins/codex-harness)
