...
**Agent harness** 是一個已準備好的 OpenClaw agent 輪次的低層級執行器。它不是模型提供者，不是通道，也不是工具註冊表。
有關使用者導向的心智模型，請參閱 [Agent runtimes](/zh-Hant/concepts/agent-runtimes)。
...
請**不要**僅為了新增新的 LLM API 而註冊 harness。對於一般的 HTTP 或
WebSocket 模型 API，請建構 [provider plugin](/zh-Hant/plugins/sdk-provider-plugins)。
...
有關操作員設定、模型前綴範例以及僅限 Codex 的設定，請參閱
...
隨附的 `codex` harness 是內嵌 OpenClaw
agent 輪次的原生 Codex 模式。請先啟用隨附的 `codex` plugin，並且如果您的設定使用限制性允許清單，請在 `plugins.allow` 中包含 `codex`。
原生 app-server 設定應該使用 `openai/gpt-*`；OpenAI agent 輪次預設選擇 Codex harness。
舊版的 Codex model refs 路由應該使用 `openclaw doctor --fix` 修復，而舊版的 `codex/*` model refs 則保持為原生 harness 的相容性別名。

當此模式執行時，Codex 擁有原生執行緒 ID、恢復行為、
壓縮 以及 app-server 執行。OpenClaw 仍然擁有聊天通道、
可見的逐字稿鏡像、工具原則、核准、媒體傳遞和工作階段
選取。當您需要證明只有 Codex app-server 路徑能夠宣告該執行時，請使用 provider/model `agentRuntime.id: "codex"`。
明確的 plugin runtimes 失敗時會封閉；Codex app-server 選取失敗和 runtime 失敗不會透過其他 runtime 重試。

## 運行時嚴格性

預設情況下，OpenClaw 使用 `auto` provider/model runtime 原則：已註冊的
plugin harness 可以宣告 provider/model 配對，而當沒有匹配時，內嵌 runtime
會處理該輪次。官方 OpenAI provider 上的 OpenAI agent refs 預設為 Codex。
當缺少 harness 選取應該失敗而不是透過內嵌 runtime 路由時，請使用明確的 provider/model plugin runtime，例如
`agentRuntime.id: "codex"`。所選 plugin harness 失敗總是會
徹底失敗。這不會阻擋明確的 provider/model `agentRuntime.id: "openclaw"`。
...
如果您的駁程儲存了 sidecar 綁定，請實作 `reset(...)`，以便當擁有的 OpenClaw 會話重設時，OpenClaw 可以清除它。
...
- [Runtime 輔助程式](/zh-Hant/plugins/sdk-runtime)
- [提供者外掛程式](/zh-Hant/plugins/sdk-provider-plugins)
- [Codex 駁程](/zh-Hant/plugins/codex-harness)
