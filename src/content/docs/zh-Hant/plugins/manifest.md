...
若要了解相容的套件佈局，請參閱 [外掛程式套件](/en/plugins/bundles)。
...
有關完整的 `plugins.*` schema，請參閱 [設定參考](/en/gateway/configuration)。
...
- [Building Plugins](/en/plugins/building-plugins) — 插件入門
- [Plugin Architecture](/en/plugins/architecture) — 內部架構
- [SDK Overview](/en/plugins/sdk-overview) — 插件 SDK 參考
