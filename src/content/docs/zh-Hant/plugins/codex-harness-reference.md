...
| `codexPlugins`             | disabled                  | 針對已遷移之原始碼安裝策展外掛程式的原生 Codex 外掛程式/應用程式支援。請參閱 [Native Codex plugins](/zh-Hant/plugins/codex-native-plugins)。 |
...
- 針對媒體理解 `image` 工具，為 `tools.media.image.timeoutSeconds`
  轉換為毫秒，或 60 秒的媒體預設值。
- 30 秒的動態工具預設值。

動態工具預算上限為 600000 毫秒。逾時時，OpenClaw 會在支援的情況下中止工具訊號，並將失敗的動態工具回應傳回給 Codex，以便輪次能繼續進行，而不是讓會話處於 `processing`。

在 Codex 接受輪次後，以及在 OpenClaw 回應輪次範圍的應用程式伺服器請求後，harness 預期 Codex 會推進當前輪次並最終以 `turn/completed` 完成原生輪次。如果應用程式伺服器安靜 `appServer.turnCompletionIdleTimeoutMs`，OpenClaw 會盡力中斷 Codex 輪次，記錄診斷逾時，並釋放 OpenClaw 會話通道，以便後續的聊天訊息不會在過時的原生輪次後排隊。

對於同一輪次的大多數非終端通知會解除該短期監視，因為 Codex 已證明該輪次仍然存活。原始 `custom_tool_call_output` 完成內容會讓短期後工具監視保持啟用，因為它們是輪次範圍的工具結果交接。已完成的 `agentMessage` 項目和前工具原始助理 `rawResponseItem/completed` 項目會啟用助理輸出釋放：如果 Codex 隨後在沒有 `turn/completed` 的情況下保持安靜，OpenClaw 會盡力中斷原生輪次並釋放會話通道。後工具原始助理進度會繼續等待 `turn/completed` 或終端監視。超時診斷包含最後的應用程式伺服器通知方法，以及針對原始助理回應項目，包含項目類型、角色、ID 和有限的助理文字預覽。

## 模型探索

預設情況下，Codex 外掛程式會向應用程式伺服器要求可用的模型。模型可用性由 Codex 應用程式伺服器管理，因此當 OpenClaw 升級隨附的 `@openai/codex` 版本，或是當部署指向 `appServer.command` 不同的 Codex 二進位檔案時，清單可能會改變。可用性也可能以帳戶為範圍。在執行中的閘道上使用 `/codex models` 以查看該佈線程和帳戶的即時目錄。

如果探索失敗或超時，OpenClaw 會針對以下項目使用隨附的後援目錄：

- GPT-5.5
- GPT-5.4 mini
- GPT-5.2

目前的隨附佈線程為 `@openai/codex` `0.130.0`。針對該隨附應用程式伺服器進行的 `model/list` 探測傳回：
...
Codex 透過原生專案文檔探索自行處理 `AGENTS.md`。OpenClaw 不會撰寫合成的 Codex 專案文檔檔案，或依賴 Codex 後備檔名來取得 persona 檔案，因為 Codex 後備僅在 `AGENTS.md` 缺失時適用。

為了與 OpenClaw workspace 保持一致，Codex harness 會解析其他啟動檔案，包括 `SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、`HEARTBEAT.md`、`BOOTSTRAP.md` 和 `MEMORY.md`（如果存在），並透過 `thread/start` 和 `thread/resume` 上的 Codex 開發者指示進行轉送。這使 workspace persona 和 profile 設定檔上下文在原生 Codex 行為塑造通道上保持可見，而無需重複 `AGENTS.md`。
...
當 `appServer.command` 未設定時，`OPENCLAW_CODEX_APP_SERVER_BIN` 會繞過受管理的二進位檔案。

`OPENCLAW_CODEX_APP_SERVER_GUARDIAN=1` 已移除。請改用 `plugins.entries.codex.config.appServer.mode: "guardian"`，或使用 `OPENCLAW_CODEX_APP_SERVER_MODE=guardian` 進行一次性本機測試。對於可重複的部署，建議使用設定，因為它可以將外掛程式行為與 Codex harness 設定的其餘部分保留在同一個經過審查的檔案中。
