...
<Info>如果您之前尚未建置任何 OpenClaw 外掛程式，請先閱讀 [Getting Started](/zh-Hant/plugins/building-plugins) 以了解基本的套件 結構與清單設定。</Info>
...
僅在您需要更廣泛的傳入輔助程式集合時，才使用 `openclaw/plugin-sdk/channel-inbound`。
...
`api.runtime.channel.mentions` 為已依賴執行時期插入的打包頻道外掛程式，
公開了相同的共享提及輔助程式：
...
`openclaw/plugin-sdk/channel-mention-gating` 匯入，以避免載入不相關的傳入
執行時期輔助程式。

較舊的 `resolveMentionGating*` 輔助程式作為僅供相容性的匯出
保留在 `openclaw/plugin-sdk/channel-inbound` 上。新程式碼
...
<Steps>
  ...
  <Step title="套件與清單">
    建立標準的外掛程式檔案。`package.json` 中的 `channel` 欄位是讓這成為頻道外掛的關鍵。若要了解完整的套件中繼資料介面，請參閱 [外掛程式設定與組態](/zh-Hant/plugins/sdk-setup#openclaw-channel)：
  ...
  <Step title="建立頻道外掛物件">
    `ChannelPlugin` 介面有許多選用的配接器介面。從最精簡的部分開始 —— `id` 和 `setup` —— 並根據需求加入配接器。
  ...
  <Step title="連接進入點">
    ...
    可以在根目錄説明中顯示它們，而無需啟動完整的通道運行時，
    同時正常的完整載入仍會選取相同的描述符以進行實際的指令
    註冊。保留 `registerFull(...)` 僅用於運行時工作。
    如果 `registerFull(...)` 註冊了閘道 RPC 方法，請使用
    外掛程式專用的前綴。核心管理命名空間（`config.*`,
    `exec.approvals.*`, `wizard.*`, `update.*`）保留給系統使用，並且總是
    解析為 `operator.admin`。
  ...
  <Step title="新增設定進入點">
    建立 `setup-entry.ts` 以便在引導期間進行輕量級載入：
    ...
    當通道被停用或未設定時，OpenClaw 會載入此項而非完整進入點。這可避免在設定流程中載入繁重的運行時代碼。
    詳情請參閱[設定與配置](/zh-Hant/plugins/sdk-setup#setup-entry)。

    將設定安全的匯出分割到側車模組的組合工作區通道，如果它們
    也需要明確的設定時間運行時設定器，可以使用來自
    `openclaw/plugin-sdk/channel-entry-contract` 的 `defineBundledChannelSetupEntry(...)`。

  </Step>

  <Step title="處理傳入訊息">
    您的外掛程式需要接收來自平台的訊息並將其轉發給
    OpenClaw。典型的模式是一個驗證請求並透過您的通道傳入處理程式進行分派的 webhook：
    ...
    <Note>
      傳入訊息處理因通道而異。每個通道外掛程式擁有
      其自己的傳入管線。請查看組合的通道外掛程式
      （例如 Microsoft Teams 或 Google Chat 外掛程式套件）以了解實際的模式。
    </Note>
...
在 `src/channel.test.ts` 中編寫並置測試：
    ...
    如需共用測試輔助程式，請參閱 [測試](/zh-Hant/plugins/sdk-testing)。
...
<CardGroup cols={2}>
  ...
  <Card title="Target resolution" icon="crosshair" href="/zh-Hant/plugins/architecture#channel-target-resolution">
...
<Note>部分捆綁的輔助接縫仍然存在，用於捆綁外掛程式的維護與相容性。它們並非新頻道外掛程式的建議模式；除非您直接維護該捆綁外掛程式系列，否則建議優先使用來自通用 SDK 表面的通用 channel/setup/reply/runtime 子路徑。</Note>
