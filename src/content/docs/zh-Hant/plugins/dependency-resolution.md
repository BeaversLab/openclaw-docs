...
- npm 套件會安裝到位於
  `~/.openclaw/npm/projects/<encoded-package>` 下的各外掛專案中
...
`openclaw plugins install npm-pack:<path.tgz>` 針對本機 npm-pack tarball 使用相同的個別外掛 npm
...
如需目前隨附於核心套件、從外部安裝，或僅保留原始碼的外掛產生清單，請參閱 [Plugin inventory](/zh-Hant/plugins/plugin-inventory)。
