...
否則，部分供應商 SDK 可能會在將控制權交還給 OpenClaw 之前進入長達 `Retry-After` 的休眠。對於基於 Stainless 的 SDK（例如 Anthropic 和 OpenAI），OpenClaw 預設將 SDK 內部的 `retry-after-ms` / `retry-after` 等待上限設定為 60 秒，並立即回報較長的可重試回應，以便執行此故障轉移路徑。您可以使用 `OPENCLAW_SDK_RETRY_MAX_WAIT_SECONDS` 調整或停用此上限；請參閱 [/concepts/retry](/zh-Hant/concepts/retry)。

速率限制冷卻也可以限制在模型範圍內：

- 當已知失敗的模型 ID 時，OpenClaw 會為速率限制失敗記錄 `cooldownModel`。
- 當冷卻範圍僅限於特定模型時，仍可嘗試同一供應商下的其他同級模型。
- 計費/停用時間視窗仍會在跨模型層級封鎖整個設定檔。
...
計費/額度失敗（例如「額度不足」/「信用餘額過低」）被視為值得進行故障轉移，但它們通常不是暫時性的。OpenClaw 不會使用短暫冷卻，而是將設定檔標記為**已停用**（並使用較長的退避時間），並旋轉至下一個設定檔/供應商。

並非所有計費形式的回應都是 `402`，也並非所有 HTTP `402` 都會歸類至此。即使供應商改回 `401` 或 `403`，OpenClaw 仍會將明確的計費文字保留在計費通道中，但供應商特定的匹配器仍會限制在擁有它們的供應商範圍內（例如 OpenRouter 的 `403 Key limit
exceeded`). Meanwhile temporary `402` 使用視窗和
組織/工作區支出限制錯誤在訊息看起來可重試時會被分類為 `rate_limit`（例如 `weekly usage limit exhausted`，`daily
limit reached, resets tomorrow`, or `organization spending limit exceeded`）。
這些錯誤會停留在短暫冷卻/故障轉移路徑上，而不是長期的計費停用路徑。
...
- 計費退避（backoff）從 **5 小時** 開始，每次計費失敗加倍，上限為 **24 小時**。
- 如果設定檔在 **24 小時** 內未失敗（可設定），退避計數器會重置。
- 過載重試允許在模型回退之前進行 **1 次相同供應商的設定檔輪替**。
- 過載重試預設使用 **0 毫秒退避**。

## 模型回退

如果供應商的所有設定檔都失敗，OpenClaw 會移動到
`agents.defaults.model.fallbacks` 中的下一個模型。這適用於認證失敗、速率限制，以及
已耗盡設定檔輪替的逾時（其他錯誤不會推進回退）。

過載和速率限制錯誤的處理比計費冷卻更積極。預設情況下，OpenClaw 允許一次相同供應商的認證設定檔重試，
然後不等待就切換到下一個設定的模型回退。
諸如 `ModelNotReadyException` 等供應商忙碌訊號會落入該過載
組。您可以使用 `auth.cooldowns.overloadedProfileRotations`、
`auth.cooldowns.overloadedBackoffMs` 和
`auth.cooldowns.rateLimitedProfileRotations` 進行調整。

當執行以模型覆寫（hooks 或 CLI）開始時，在嘗試任何設定的回退後，回退仍然結束於
`agents.defaults.model.primary`。

### 候選鏈規則

OpenClaw 根據當前請求的 `provider/model`
加上設定的回退來建立候選清單。
...
- 明確設定的回退會被去重，但不會被模型
  白名單過濾。它們被視為明確的操作者意圖。
- 如果當前執行已經位於同一供應商
  系列中的設定回退上，OpenClaw 會繼續使用完整的設定鏈。
- 如果當前執行所在的供應商與設定不同，且該當前
  模型尚未成為設定回退鏈的一部分，OpenClaw 不會
  附加來自其他供應商的不相關設定回退。
- 當執行始於覆寫時，設定的主體會附加在
  末尾，以便一旦較早的候選者耗盡，鏈可以恢復到正常的預設值。
...
- 逾時型態的故障轉移錯誤
- 計費停用
- `LiveSessionModelSwitchError`，其被正規化為故障切換路徑，以便過期的持久化模型不會產生外部重試循環
- 當仍有剩餘候選時的其他無法識別的錯誤
...
當供應商的所有身分驗證配置文件都已處於冷卻狀態時，OpenClaw 不會
永久自動跳過該供應商。它會做出針對每個候選的決定：

- 持續的身分驗證失敗會立即跳過整個供應商。
- 帳單停用通常會跳過，但在節流時仍可探測主要候選，
  以便無需重新啟動即可恢復。
- 主要候選可能會在接近冷卻到期時被探測，並帶有每個供應商的節流。
- 當失敗看起來是暫時性時（`rate_limit`、`overloaded` 或未知），儘管處於冷卻狀態，仍可嘗試同一供應商的備用同級模型。這在速率限制限於模型範圍且同級模型可能仍立即恢復時特別相關。
- 暫時性冷卻探測限制為每次備用運行每個供應商一次，因此
  單一供應商不會拖延跨供應商備用。

## 會話覆蓋和即時模型切換

會話模型變更是共享狀態。活躍運行器、`/model` 指令、
壓縮/會話更新和即時會話協調都會讀取或寫入
同一會話條目的部分內容。

這意味著備用重試必須與即時模型切換協調：

- 只有明確的用戶驅動模型變更才會標記待處理的即時切換。這
  包括 `/model`、`session_status(model=...)` 和 `sessions.patch`。
- 系統驅動的模型變更，例如故障轉移輪替、心跳覆寫或壓縮，絕不會自行標記待處理的即時切換。
- 在故障轉移重試開始之前，回覆執行器會將選定的故障轉移覆寫欄位持久化到會話條目中。
- 即時會話協調優先使用持久化的會話覆寫，而非過時的執行時模型欄位。
- 如果故障轉移嘗試失敗，執行器僅會回滾其寫入的覆寫欄位，並且僅在它們仍匹配該失敗候選時才執行回滾。
...
3. 會話儲存仍然顯示舊的主要模型。
4. 即時會話協調讀取了過時的會話狀態。
5. 重試會在故障轉移嘗試開始之前被「彈回」到舊模型。

持久化的故障轉移覆寫關閉了該視窗，而精確的回滾則保持了較新的手動或執行時會話變更完整。

## 可觀測性和故障摘要

`runWithModelFallback(...)` 記錄每次嘗試的詳細資訊，這些資訊會匯入日誌和面向使用者的冷卻訊息：
...
當每個候選都失敗時，OpenClaw 會拋出 `FallbackSummaryError`。外部
回覆執行器可以使用它來建構更具體的訊息，例如「所有模型目前都受到速率限制」，並在已知時包含最近的冷卻到期時間。
...
請參閱 [Models](/zh-Hant/concepts/models) 以了解更廣泛的模型選擇與備援概覽。
