---
summary: "QA 堆疊概覽：qa-lab、qa-channel、儲存庫支援的場景、即時傳輸通道、傳輸配接器以及報告。"
read_when:
...
- `extensions/qa-channel`：綜合訊息通道，具備 DM、頻道、執行緒、回應、編輯和刪除介面。
- `extensions/qa-lab`：除錯器 UI 和 QA 總線，用於觀察對話紀錄、注入入站訊息以及匯出 Markdown 報告。
- `extensions/qa-matrix`，未來的執行器外掛：即時傳輸配接器，可在子 QA 閘道內驅動真實頻道。
- `qa/`：啟動任務和基準 QA 場景的儲存庫支援種子資產。
- [Mantis](/zh-Hant/concepts/mantis)：針對需要真實傳輸、瀏覽器螢幕截圖、VM 狀態和 PR 證據的錯誤，進行即時驗證前後比對。

## 指令介面

每個 QA 流程均在 `pnpm openclaw qa <subcommand>` 下執行。許多流程具有 `pnpm qa:*` 腳本別名；這兩種形式均受支援。
...
| `qa parity-report`                                  | 比較兩個 `qa-suite-summary.json` 檔案並寫入代理對等報告。                                                                                                                                                                                 |
| `qa character-eval`                                 | 在多個即時模型上執行角色 QA 場景，並生成評判報告。請參閱 [報告](#reporting)。                                                                                                                                                             |
...
| `qa mantis`                                         | 針對即時傳輸錯誤的前後驗證執行器，包含 Discord 狀態回應證據、Crabbox 桌面/瀏覽器冒煙測試以及 VNC 中的 Slack 冒煙測試。請參閱 [Mantis](/zh-Hant/concepts/mantis) 和 [Mantis Slack Desktop Runbook](/zh-Hant/concepts/mantis-slack-desktop-runbook)。 |
...
此通道的完整 CLI 參考、設定檔/情境目錄、環境變數和產出配置位於 [Matrix QA](/zh-Hant/concepts/qa-matrix)。簡而言之：它在 Docker 中佈建一個用完即丟的 Tuwunel homeserver，註冊臨時的 driver/SUT/observer 使用者，在範圍限定於該傳輸的子 QA gateway 中執行真正的 Matrix 外掛程式（不使用 `qa-channel`），然後在 `.artifacts/qa-e2e/matrix-<timestamp>/` 下寫入 Markdown 報告、JSON 摘要、observed-events 產出和組合輸出日誌。
...
它們以預先存在的真實頻道為目標，並包含兩個機器人（driver + SUT）。所需的環境變數、情境清單、輸出成品以及 Convex 憑證池記載於下方的 [Telegram, Discord, and Slack QA reference](#telegram-discord-and-slack-qa-reference) 中。
...
操作員檢查清單、GitHub workflow dispatch 指令、證據註解合約、hydrate-mode 決策表、時序解讀以及失敗處理步驟記載於 [Mantis Slack Desktop Runbook](/zh-Hant/concepts/mantis-slack-desktop-runbook) 中。
...
Matrix 因為其情境數量與支援 Docker 的 homeserver 佈建而有[專屬頁面](/zh-Hant/concepts/qa-matrix)。Telegram、Discord 和 Slack 則較小 — 各自只有少數情境、沒有設定檔系統、且針對既有的真實頻道 — 因此它們的參考資料位於此處。
...
下方的 SUT 清單有意將隨附的 Slack 外掛程式之生產環境安裝 (`extensions/slack/src/setup-shared.ts:10`) 縮減為即時 Slack QA 測試套件所涵蓋的權限和事件。如需使用者所見的生產頻道設定，請參閱 [Slack channel quick setup](/zh-Hant/channels/slack#quick-setup)；QA Driver/SUT 對組是有意分開的，因為此通道需要在一個工作區中有兩個不同的機器人使用者 ID。

**1. 建立 Driver 應用程式**

前往 [api.slack.com/apps](https://api.slack.com/apps) → _Create New App_ → _From a manifest_ → 選取 QA 工作區，貼上下列清單，然後按一下 _Install to Workspace_：
...
對於視覺化的真實用戶 Telegram 證明，建議使用保留的 Crabbox 工作階段：
...
`start` 為 TDLib CLI 驅動程式和 Telegram Desktop 見證程式持有一個獨佔的 Convex `telegram-user` 租約，啟動桌面錄製，並讓 Crabbox 保持運作以供代理程式執行任意的重現步驟。代理程式可以使用 `send`、`run`、`screenshot` 和 `status` 直到滿意為止，然後 `finish` 會在釋放憑證之前收集螢幕截圖、影片、動態修剪的影片/GIF、TDLib 探測輸出和日誌。`publish --session <file> --pr

<number>` comments only the motion GIF by default; `--full-artifacts` 是日誌和 JSON 輸出的明確選用選項。預設的 `probe` 指令仍然是快速 `/status` 煙霧測試的單一指令簡寫。

當 PR 需要確定性視覺差異時，請使用 `--mock-response-file <path>`：相同的模擬模型回覆可以在 `main` 和 PR 頂端上運行，同時 Telegram 格式器或傳遞層發生變更。捕捉預設值是針對 PR 註解調整的：標準 Crabbox 類別、24fps 桌面錄製、24fps 動態 GIF 以及 1920px 預覽寬度。變更前/後的註解應發布僅包含預期 GIF 的乾淨套件。

Slack 通道也可以使用此集區。Slack payload 形狀檢查目前位於 Slack QA 執行器而非 broker 中；請使用 `{ channelId: string, driverBotToken: string, sutBotToken: string, sutAppToken: string }`，搭配 Slack channel id，例如 `Cxxxxxxxxxx`。關於應用程式和範圍佈建，請參閱 [設定 Slack 工作區](#setting-up-the-slack-workspace)。

操作環境變數和 Convex broker 端點合約位於 [Testing → Shared Telegram credentials via Convex](/zh-Hant/help/testing#shared-telegram-credentials-via-convex-v1)（該區段名稱早於多通道集區；租用語意在所有類型中共享）。
...
這些刻意放在 git 中，以便人類和代理程式都能看到 QA 計劃。

`qa-lab` 應保持為通用的 markdown 執行器。每個情境 markdown 檔案是單次測試執行的來源事實，並應定義：
...
支援 `qa-flow` 的可重複執行時期介面允許保持通用性和橫向關聯性。例如，markdown 情境可以結合傳輸端輔助程式與瀏覽器端輔助程式，後者透過 Gateway `browser.request` 縫合來驅動嵌入式控制 UI，而無需新增特殊情況執行器。

情境檔案應依產品功能分組，而非來源樹資料夾。當檔案移動時，請保持情境 ID 穩定；請使用 `docsRefs` 和 `codeRefs` 進行實作追蹤。
...
- `mock-openai` 是感知情境的 OpenClaw mock。它仍是 repo 支援的 QA 和一致性檢測閘道的預設
  決定性 mock 通道。
- `aimock` 啟動一個由 AIMock 支援的 provider 伺服器，用於實驗性協定、
  fixture、錄製/重放和混亂覆蓋測試。它是附加性的，不會
  取代 `mock-openai` 情境分派器。

Provider 通道實作位於 `extensions/qa-lab/src/providers/` 之下。
每個 provider 擁有其預設值、本機伺服器啟動、閘道模型設定、
auth-profile 暫存需求，以及即時/mock 能力旗標。共享套件和
閘道程式碼應透過 provider 註冊表路由，而不是根據
provider 名稱進行分支判斷。

## 傳輸配接器

`qa-lab` 擁有用於 markdown QA 情境的通用傳輸接縫。`qa-channel` 是該接縫上的第一個配接器，但設計目標更廣泛：未來的真實或合成通道應插入相同的套件執行器，而不是新增傳輸專用的 QA 執行器。

在架構層級，劃分如下：

- `qa-lab` 負責通用情境執行、工作並行、產生品寫入和報告。
- 傳輸配接器負責閘道設定、就緒狀態、輸入和輸出觀察、傳輸動作，以及正規化的傳輸狀態。
- `qa/scenarios/` 下的 Markdown 情境檔案定義了測試執行；`qa-lab` 提供執行它們的可重複使用執行時介面。
...
當共享的 `qa-lab` 主機可以擁有此流程時，請勿新增新的頂層 QA 指令根。
...
- `openclaw qa <runner>` 如何掛載在共享 `qa` 根目錄下
- 如何為該傳輸設定閘道
- 如何檢查就緒狀態
- 如何注入傳入事件
- 如何觀察傳出訊息
...
2. 在共享 `qa-lab` 主機接縫上實作傳輸 runner。
3. 將特定傳輸的機制保留在 runner 外掛程式或頻道套件內。
4. 將 runner 掛載為 `openclaw qa <runner>`，而不是註冊競爭的根指令。Runner 外掛程式應在 `openclaw.plugin.json` 中宣告 `qaRunners`，並從 `runtime-api.ts` 匯出相符的 `qaRunnerCliRegistrations` 陣列。保持 `runtime-api.ts` 輕量化；延遲載入的 CLI 和 runner 執行應保留在個別的進入點後方。
5. 在主題式 `qa/scenarios/` 目錄下撰寫或改編 markdown 場景。
6. 為新場景使用通用場景輔助程式。
...
- 如果行為依賴於某個頻道傳輸，請將其保留在該 runner 外掛程式或外掛程式套件中。
- 如果場景需要多個頻道都能使用的新功能，請新增通用輔助程式，而不是在 `suite.ts` 中新增特定頻道的分支。
- 如果某個行為僅對一種傳輸有意義，請將場景保持為特定傳輸，並在場景合約中明確說明。
...
既有情境的相容性別名仍可使用 - `waitForQaChannelReady`、`waitForOutboundMessage`、`waitForNoOutbound`、`formatConversationTranscript`、`resetBus` - 但新情境的撰寫應使用通用名稱。這些別名的存在是為了避免必須一次性遷移（flag-day migration），而非作為未來的模型。
...
若要查看可用的情境清單 - 在評估後續工作範圍或接駁新傳輸時很有用 - 請執行 `pnpm openclaw qa coverage`（加入 `--json` 以取得機器可讀的輸出）。

若要進行字元和樣式檢查，請在多個即時模型參考上執行相同的情境，並撰寫一份評估後的 Markdown 報告：
...
此指令會在本機執行 QA gateway 子進程，而非使用 Docker。角色評估場景應透過 `SOUL.md` 設定角色，然後執行一般使用者輪次，例如聊天、工作區說明和小型檔案任務。不應告訴候選模型它正在接受評估。此指令會保留每份完整紀錄，記錄基本執行統計數據，然後要求評判模型在支援的情況下以快速模式和 `xhigh` 推理，依自然度、氛圍和幽默感對執行進行排名。
在比較提供者時使用 `--blind-judge-models`：評判提示仍然會取得每份紀錄和執行狀態，但候選參照會被中性標籤（例如 `candidate-01`）取代；報告會在解析後將排名對應回實際參照。
候選執行預設為 `high` 思考模式，GPT-5.5 使用 `medium`，支援此功能的較舊 OpenAI 評估參照則使用 `xhigh`。使用 `--model provider/model,thinking=<level>` 行內覆寫特定候選模型。`--thinking <level>` 仍會設定全域後備，而較舊的 `--model-thinking <provider/model=level>` 形式則為了相容性而保留。
OpenAI 候選參照預設為快速模式，因此在提供者支援的情況下會使用優先處理。當單一候選模型或評判需要覆寫時，請行內加入 `,fast`、`,no-fast` 或 `,fast=false`。僅當您想要對每個候選模型強制開啟快速模式時，才傳遞 `--fast`。候選模型和評判的持續時間會記錄在報告中以進行基準分析，但評判提示會明確表示不要按速度排名。
候選模型和評判模型執行的預設並發性均為 16。當提供者限制或本機 gateway 壓力導致執行過於干擾時，請降低 `--concurrency` 或 `--judge-concurrency`。
當未傳遞候選 `--model` 時，角色評估預設為
