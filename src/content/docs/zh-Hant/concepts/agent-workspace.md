...
**重要提示：** 工作區是**預設的目前工作目錄**，而不是嚴格的沙箱。工具會根據工作區解析相對路徑，但在未啟用沙箱的情況下，絕對路徑仍可存取主機上的其他位置。如果您需要隔離，請使用 [`agents.defaults.sandbox`](/zh-Hant/gateway/sandboxing) (和/或每個 Agent 的沙箱設定)。
...
- `SOUL.md`
  - 角色設定、語氣和界線。
  - 每個會話都會載入。
  - 指南：[SOUL.md 個性指南](/zh-Hant/concepts/soul)
...
- 多代理路由可以為每個代理使用不同的工作區。請參閱
  [通道路由](/zh-Hant/channels/channel-routing) 以了解路由配置。
- 如果啟用了 `agents.defaults.sandbox`，非主會話可以在 `agents.defaults.sandbox.workspaceRoot` 下使用每個會話獨立的沙箱
  工作區。

## 相關內容

- [常駐指令](/zh-Hant/automation/standing-orders) — 工作區檔案中的持久指令
- [心跳](/zh-Hant/gateway/heartbeat) — HEARTBEAT.md 工作區檔案
- [會話](/zh-Hant/concepts/session) — 會話存儲路徑
