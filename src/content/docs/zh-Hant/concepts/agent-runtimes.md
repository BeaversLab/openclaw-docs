...
4. 如果舊版配置仍包含 **`openai-codex/*` model refs**，請將其修復為
   `openai/<model>` 並搭配 `openclaw doctor --fix`；doctor 會透過新增 provider/model-scoped `agentRuntime.id: "codex"` 來保留 Codex auth
   route，即在舊的 model ref 暗示的地方。
5. 如果用戶明確提到 **ACP**、**acpx** 或 **Codex ACP adapter**，請使用
   ACP 並搭配 `runtime: "acp"` 和 `agentId: "codex"`。
...
關於 OpenAI 系列前綴的拆分，請參閱 [OpenAI](/zh-Hant/providers/openai) 和
...
1. Model-scoped runtime policy 優先採用。這可以存放在已設定的 provider
   model entry 或 `agents.defaults.models["provider/model"].agentRuntime` /
   `agents.list[].models["provider/model"].agentRuntime` 中。
2. Provider-scoped runtime policy 則在
   `models.providers.<provider>.agentRuntime` 採用。
3. 在 `auto` 模式下，已註冊的外掛程式 runtime 可以宣告支援的 provider/model
   配對。
4. 如果在 `auto` 模式下沒有 runtime 宣告承接某個輪次，OpenClaw 會將 PI 作為
   相容性 runtime。當執行必須嚴格時，請使用明確的 runtime id。

Whole-session 和 whole-agent runtime pins 會被忽略。這包括
...
`openclaw doctor --fix` 以移除過時的 whole-agent runtime config，並轉換
legacy runtime model refs，讓 OpenClaw 能夠保留意圖。

明確的 provider/model 外掛程式 runtime 會以失敗封閉 (fail closed) 方式處理。例如，
在 provider 或 model 上設定 `agentRuntime.id: "codex"` 表示使用 Codex 或明確的
selection/runtime 錯誤；它絕不會被靜默路由回 PI。
...
諸如 `claude-cli/claude-opus-4-7` 等舊版 refs 為了相容性仍受支援，但新配置應保持 provider/model 的規範性，並將
執行後端置於 provider/model runtime policy 中。

`auto` 模式對大多數提供者而言是刻意的保守策略。OpenAI agent
模型是例外：未設定的 runtime 和 `auto` 都會解析為 Codex
harness。明確的 PI runtime 設定仍是一種選擇加入的相容性途徑，用於
`openai/*` agent 週期；當與選定的 `openai-codex` auth profile 配對時，
OpenClaw 會在內部透過舊版 Codex-auth 傳輸路由 PI，同時
將公開的 model ref 保留為 `openai/*`。過時的 OpenAI PI session pins 會
被 runtime 選擇忽略，並可以使用 `openclaw doctor --fix` 清理。

如果 `openclaw doctor` 警告 `codex` 外掛程式已啟用，但
`openai-codex/*` 仍保留在設定中，請將其視為舊版路由狀態。執行
`openclaw doctor --fix` 將其重寫為 `openai/*` 並使用 Codex runtime。
...
| 動態工具勾點 是否運作？     | 外掛程式期望 `before_tool_call`、`after_tool_call` 以及 OpenClaw 擁有工具周圍的中介軟體。 |
...
- 諸如 `openai/gpt-5.5` 的 model ref 會告訴您選定的提供者/模型。
- 諸如 `codex` 的 runtime id 會告訴您哪個迴圈正在執行該週期。
