...
- 外部 CLI 重複使用因供應商而異：Codex CLI 可以引導一個空的 `openai:default` profile，但一旦 OpenClaw 擁有本機 OAuth profile，本機 refresh token 即為標準。如果該本機 refresh token 被拒絕，OpenClaw 可以使用可用的同帳戶 Codex CLI token 作為僅限執行時期的後備方案；其他整合可以保持外部管理，並重新讀取其 CLI auth store
...
精靈路徑為 `openclaw onboard` → auth choice `openai`。
...
- 例外：某些外部 CLI 憑證保持外部管理；OpenClaw 會重新讀取那些 CLI auth stores，而不是消耗複製的 refresh tokens。Codex CLI 引導被有意設計得較狹隘：它會播種一個空的 `openai:default` profile，然後由 OpenClaw 擁有的更新機制使本機 profile 保持標準。如果本機 Codex refresh 失敗，且 Codex CLI 擁有同一個帳戶的可用 token，OpenClaw 可能會使用該 token 處理目前的執行時期請求，而不會將其寫回 `auth-profiles.json`。
