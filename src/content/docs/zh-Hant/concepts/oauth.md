...
上述所有項目也遵守 `$OPENCLAW_STATE_DIR`（狀態目錄覆寫）。完整參考：[/gateway/configuration](/zh-Hant/gateway/configuration-reference#auth-storage)

關於靜態機密引用和執行時快照啟用行為，請參閱 [機密管理](/zh-Hant/gateway/secrets)。
...
<Warning>
Anthropic 的公開 Claude Code 文件指出，直接使用 Claude Code 仍會維持在 Claude 訂閱限制內，且 Anthropic 人員告訴我們，OpenClaw 風格的 Claude CLI 使用再次獲得允許。因此，除非 Anthropic 發布新政策，否則 OpenClaw 將 Claude CLI 重複使用和 `claude -p` 使用視為此整合的核准用法。

關於 Anthropic 目前的直接 Claude Code 方案文件，請參閱 [在 Pro 或 Max 方案中使用 Claude Code
](https://support.claude.com/en/articles/11145838-using-claude-code-with-your-pro-or-max-plan)
以及 [在 Team 或 Enterprise 方案中使用 Claude Code
...
OpenClaw 的互動式登入流程是在 `@mariozechner/pi-ai` 中實作並連接到精靈/指令的。
...
- 例外：某些外部 CLI 認證維持外部管理；OpenClaw 會重新讀取這些 CLI 認證儲存空間，而不是消耗已複製的重新整理權杖。
  Codex CLI 引導程序刻意設計得較狹隘：它會建立一個空的
  `openai-codex:default` 設定檔，然後由 OpenClaw 擁有的重新整理機制來維持本地設定檔的正確性。
...
- [Model failover](/zh-Hant/concepts/model-failover)（輪替 + 冷卻規則）
- [Slash commands](/zh-Hant/tools/slash-commands)（指令介面）

## 相關

- [Authentication](/zh-Hant/gateway/authentication) - 模型提供者認證概覽
- [Secrets](/zh-Hant/gateway/secrets) - 認證資訊儲存與 SecretRef
- [Configuration Reference](/zh-Hant/gateway/configuration-reference#auth-storage) - 認證設定金鑰
