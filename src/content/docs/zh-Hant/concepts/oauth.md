...
<Warning>
Anthropic 的公開 Claude Code 文件指出，直接使用 Claude Code 保持在
Claude 訂閱限制範圍內，且 Anthropic 人員告訴我們，OpenClaw 風格的 Claude
CLI 使用已被允許。因此，除非 Anthropic
發布新政策，否則 OpenClaw 將 Claude CLI 重用和
`claude -p` 使用視為此整合的許可行為。

關於 Anthropic 目前的直接 Claude Code 方案文件，請參閱 [將 Claude Code 與您的 Pro 或 Max
方案搭配使用](https://support.claude.com/en/articles/11145838-using-claude-code-with-your-pro-or-max-plan)
以及 [將 Claude Code 與您的 Team 或 Enterprise
方案搭配使用](https://support.anthropic.com/en/articles/11845131-using-claude-code-with-your-team-or-enterprise-plan/)。

如果您在 OpenClaw 中想要其他訂閱風格的選項，請參閱 [OpenAI
