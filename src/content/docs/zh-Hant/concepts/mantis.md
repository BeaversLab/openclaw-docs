...
對於人機迴圈的 Telegram 桌面設定，請使用情境建構器：
...
建構器會租用或重用 Crabbox 桌面，安裝原生 Linux Telegram Desktop 二進位檔，選擇性地還原使用者工作階段封存，使用租用的 Telegram SUT bot 權杖設定 OpenClaw，在連接埠 `38974` 上啟動 `openclaw gateway run`，將驅動程式 bot 就緒訊息張貼到租用的私密群組，然後從可見的 VNC 桌面擷取螢幕截圖和 MP4。Bot 權杖從不登入 Telegram Desktop；它僅用於設定 OpenClaw。桌面檢視器是從 `--telegram-profile-archive-env <name>` 還原的獨立 Telegram 使用者工作階段，或是透過 VNC 手動建立並使用 `--keep-lease` 保持運作。

實用的 Telegram 桌面建構器旗標：

- `--lease-id <cbx_...>` 針對操作員已登入 Telegram Desktop 的 VM 重新執行。
- `--telegram-profile-archive-env <name>` 從該環境變數讀取 base64 編碼的 `.tgz` Telegram Desktop 設定檔封存，並在啟動前還原它。
...
- `--credential-source convex --credential-role ci` 使用共用憑證代理程式，而不是直接的 Telegram 環境權杖。

每個發佈 PR 的場景都會在其報告旁寫入 `mantis-evidence.json`。
此 schema 是場景代碼與 GitHub 評論之間的交接：
...
評論觸發器經過刻意的狹隘設計。它僅針對具有寫入、維護者或管理員權限的使用者所發出的 PR 評論執行，且僅識別
Discord status-reaction 請求。預設情況下，它使用已知的錯誤基準 ref
...
預設情況下，它使用當前 PR head SHA 作為候選者並執行
...
第一個指令是明確且以場景為重點的。第二個指令之後可以根據標籤、變更的檔案和 ClawSweeper 審查發現，將 PR 或 issue 對應到推薦的 Mantis 場景。
...
4. 為基準 ref 準備一個乾淨的 checkout。
5. 安裝相依項並僅建置場景所需的內容。
6. 啟動一個具有獨立狀態目錄的子 OpenClaw Gateway。
7. 設定即時傳輸、提供者、模型和瀏覽器設定檔。
8. 執行場景並擷取基準證據。
9. 停止 gateway 並保留日誌。
10. 在同一個 VM 中準備候選 ref。
11. 執行相同的場景並擷取候選證據。
12. 比較 oracle 結果和視覺證據。
13. 寫入 Markdown、JSON、日誌、螢幕擷圖和選用的追蹤成品。
...
- **Bug 重現**：基準以預期的方式失敗。
- **Harness 失敗**：在 bug oracle 具有意義之前，環境設定、憑證、Discord API、瀏覽器或提供者即已失敗。

最終報告必須區分這些情況，以便維護者不會將不穩定的環境與產品行為混淆。

## Discord MVP

第一個場景應該針對伺服器頻道中的 Discord 狀態反應，其中來源回覆傳遞模式是 `message_tool_only`。

為什麼它是好的 Mantis 種子：

- 它在 Discord 中作為觸發訊息上的反應可見。
- 它透過 Discord 訊息反應狀態具有強大的 REST oracle。
- 它執行真實的 OpenClaw Gateway、Discord 機器人驗證、訊息分派、來源回覆傳遞模式、狀態反應狀態和模型回合生命週期。
- 它的範圍足夠窄，可以保持第一次實作的真誠。
...
基準證據應顯示排隊確認反應，但在僅工具模式下沒有生命週期轉換。候選證據應顯示當 `messages.statusReactions.enabled` 明確為 true 時正在執行的生命週期狀態反應。
...
"message_tool"`, `ackReaction: "👀"` 和明確的狀態反應來配置 SUT。預測器會
輪詢真實的 Discord 觸發訊息，並預期觀察到的序列
...
- `pnpm openclaw qa discord` 已經運行了一個具有驅動程式和
  SUT 機器人的即時 Discord 通道。
- 即時傳輸執行器已經在 `.artifacts/qa-e2e/` 下寫入報告和觀察到的訊息
  成品。
- Convex 憑證租約已經提供對共享即時傳輸憑證的獨佔存取權。
- 瀏覽器控制服務已經支援螢幕截圖、快照、無頭管理的設定檔和遠端 CDP 設定檔。
- QA Lab 已經具有一個用於傳輸形狀測試的偵錯器 UI 和匯流排。

第一個 Mantis 實作可以是這些組件之上的一個輕量級前後對比執行器，加上一個視覺證據層。
...
`mantis-summary.json` 應該是機器可讀的單一真實來源。
...
螢幕截圖是證據，不是機密。它們仍然需要編輯紀律：
私人頻道名稱、使用者名稱或訊息內容可能會出現。對於公開 PR，
在編輯機制更完善之前，最好優先使用 GitHub Actions 成品連結，而非內嵌圖片。

## 瀏覽器和 VNC

瀏覽器通道有兩種模式：

- **無頭自動化**：CI 的預設值。Chrome 啟用 CDP 運行，並由
  Playwright 或 OpenClaw 瀏覽器控制擷取螢幕截圖。
- **VNC 救援**：當登入、MFA、Discord 反自動化或
  視覺偵錯需要人員介入時，在同一台 VM 上啟用。

Discord 觀察者瀏覽器設定檔應該足夠持久，以避免每次執行時都必須登入，但也要與個人瀏覽器狀態隔離。設定檔屬於 Mantis 機器池，而不是開發者的筆記型電腦。

當 Mantis 卡住時，它會發佈一條包含以下內容的 Discord 狀態訊息：
...
首次私有部署可以將這些訊息發送到現有的操作員頻道，稍後再移至專用的 Mantis 頻道。

## 機器

對於第一次的遠端實作，Mantis 應優先透過 Crabbox 使用 AWS。Crabbox 為我們提供預熱機器、租用追蹤、水合、日誌、結果和清理。如果 AWS 容量太慢或不可用，請在相同的機器介面後面新增一個 Hetzner 供應商。

最低 VM 需求：

- 已安裝具備桌面功能的 Chrome 或 Chromium 的 Linux
- 用於瀏覽器自動化的 CDP 存取權
- 用於救援的 VNC 或 noVNC
- Node 22 和 pnpm
- OpenClaw 檢出和依賴快取
...
- 對 Discord、GitHub、模型供應商和憑證代理程式的出站存取權

VM 不應在預期的憑證或瀏覽器設定檔儲存之外，保留長期存在的原始密鑰。

## 密鑰

對於遠端執行，密鑰儲存在 GitHub 組織或儲存庫密鑰中；對於本機執行，則儲存在本機操作員控制的密鑰檔案中。
...
長期來看，Convex 憑證池應該仍是即時傳輸憑證的正常來源。GitHub secrets 用於引導 broker 和備用通道。Discord 狀態反應工作流會將 Mantis Crabbox secrets 對應回 Crabbox CLI 預期的 `CRABBOX_COORDINATOR` 和 `CRABBOX_COORDINATOR_TOKEN` 環境變數。簡單的 `CRABBOX_*` GitHub secret 名稱作為相容性備選方案仍會被接受。
...
公開的上傳工件也應該編輯 Discord 目標元數據，例如 bot、guild、channel 和 message ids。GitHub smoke workflow 這個原因啟用了 `OPENCLAW_QA_REDACT_PUBLIC_METADATA=1`。

如果 token 意外被貼上到 issue、PR、聊天或日誌中，請在儲存新 secret 後輪換它。

## GitHub artifacts 和 PR comments

Mantis workflows 應該將完整的證據捆綁包作為短期有效的 Actions artifact 上傳。當 workflow 為錯誤報告或修復 PR 執行時，它還應該將已編輯的 PNG 截圖發布到 `qa-artifacts` 分支，並在該錯誤或修復 PR 上 upsert 一條帶有內聯修復前/後截圖的評論。不要僅在通用的 QA 自動化 PR 上發布主要證據。原始日誌、觀察到的訊息和其他龐大的證據保留在 Actions artifact 中。

Production workflows 應該使用 Mantis GitHub App 發布這些評論，而不是使用 `github-actions[bot]`。將 app id 和 private key 儲存為 `MANTIS_GITHUB_APP_ID` 和 `MANTIS_GITHUB_APP_PRIVATE_KEY` GitHub Actions secrets。Workflow 使用隱藏標記作為 upsert 鍵，當 token 可以編輯該評論時更新它，並且當較舊的 bot 擁有的標記無法被編輯時建立一條新的 Mantis 擁有的評論。
...
當運行因為 harness 失敗而失敗時，評論必須說明這一點，而不是暗示 candidate 失敗了。

## Private deployment notes

Private deployment 可能已經有一個 Mantis Discord 應用程式。當它擁有正確的 bot 權限並且可以安全地輪換時，請重複使用該應用程式，而不是建立另一個 app。

透過密鑰或部署配置設定初始的操作員通知頻道。它可以先指向現有的維護者或操作頻道，然後在專用的 Mantis 頻道存在後移動至該處。

請勿將伺服器 ID、頻道 ID、機器人權杖、瀏覽器 Cookie 或 VNC 密碼放在此文件中。請將它們儲存在 GitHub 密鑰、憑證代理程式或操作員的本機密鑰存放區中。
...
- 用於執行緒錯誤的 Discord 訊息參考
- 用於 Slack 錯誤的 Slack 執行緒 ts 與反應 API 狀態
- 用於電子郵件錯誤的電子郵件訊息 ID 與標頭
- 當 UI 是唯一可靠的觀察物件時的瀏覽器截圖

視覺檢查應該是補充性的。如果平台 API 能證明該錯誤，請使用 API 作為通過/失敗預言機，並保留截圖以增加人員信心。

## 提供者擴展

在 Discord 之後，相同的執行器可以新增：

- Slack：反應、執行緒、應用程式提及、模態視窗、檔案上傳。
- 電子郵件：當連接器不足夠時，使用 `gog` 進行 Gmail 驗證和訊息執行緒處理。
- WhatsApp：QR 登入、重新識別、訊息傳遞、媒體、反應。
- Telegram：群組提及閘道、指令、可用的反應。
- Matrix：加密房間、執行緒或回覆關係、重啟恢復。

每種傳輸都應該有一個低成本的冒煙測試情境和一個或多個錯誤類別情境。昂貴的視覺情境應保持選用狀態。

## 未決問題

- 當重複使用現有的 Mantis 機器人時，哪個 Discord 機器人應該是驅動程式，哪個應該是被測系統 (SUT)？
- 觀察者瀏覽器登入應該使用人類 Discord 帳號、測試帳號，還是在第一階段僅使用機器人可讀取的 REST 證據？
- GitHub 應為 PR 保留 Mantis 成品多久？
- ClawSweeper 應該在何時自動建議使用 Mantis，而不是等待維護者指令？
- 在上傳至公開 PR 之前，是否應對截圖進行編修或裁切？
