...
`Mantis Telegram Desktop Proof` 是原生的 Telegram Desktop 代理前後對比包裝器。維護者可以透過 PR 評論中的 `@openclaw-mantis telegram desktop proof`、Actions UI 中的自由格式指令，或透過通用 `Mantis Scenario` 分派器來觸發它。工作流程會將 PR、基準參考、候選參考以及維護者的指令交給 Codex。代理會閱讀 PR，決定哪種 Telegram 可見行為可以證明變更，為基準和候選執行真實用戶 Crabbox Telegram Desktop 證明通道，迭代直到原生 GIF 有用為止，將配對的 `motionPreview` 構件寫入 `mantis-evidence.json`，上傳該組合件，並在 PR 號碼可用時發佈雙欄 PR 證據表格。
