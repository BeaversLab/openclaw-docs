...
`--pack` 與重複的 `--scenario` 標誌具有累加效果。明確指定的情境會先執行，然後套件情境會按 `QA_PERSONAL_AGENT_SCENARIO_IDS` 順序執行，並移除重複項。

該套件專為搭配 `qa-channel` 與 `mock-openai` 或其他本機 QA 提供者通道而設計。不應將其指向即時聊天服務或真實的個人帳戶。

## 隱私模型

這些情境僅使用假使用者、虛擬偏好設定、虛擬機密，以及由套件建立的暫時性 QA 閘道工作區。它們絕不能讀取或寫入真實的 OpenClaw 使用者記憶體、工作階段、憑證、啟動代理程式、全域設定或即時閘道狀態。

產出檔案會保留在現有的 QA 套件產出目錄下，並應被視為測試輸出。資料遮蔽檢查使用虛擬標記，因此檢查失敗時可以安全地檢視並提交問題。

## 擴充套件

在 `qa/scenarios/personal/` 下新增案例，然後將情境 ID 新增至
`QA_PERSONAL_AGENT_SCENARIO_IDS`。保持每個案例小而精悍、本地化、在 `mock-openai` 中具有決定性，並聚焦於單一個人助理行為。
