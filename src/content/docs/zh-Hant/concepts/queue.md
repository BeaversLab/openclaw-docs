...
- 啟用診斷時，在 `diagnostics.stuckSessionWarnMs` 之後仍停留在 `processing` 且未觀察到回覆、工具、狀態、區塊或 ACP 進度的會話會根據目前活動進行分類。作用中的工作記錄為 `session.long_running`；沒有最近進度的作用中工作記錄為 `session.stalled`；`session.stuck` 保留給沒有作用中工作的過時會話維護，並且只有該路徑可以釋放受影響的會話通道，以便佇列工作排空。重複的 `session.stuck` 診斷會在會話保持不變時退避。
