...
預覽串流也可以包含 **工具進度** 更新——例如「搜尋網路」、「讀取檔案」或「呼叫工具」等短狀態行——這些內容會在工具執行時出現在同一條預覽訊息中，位於最終回應之前。這讓多步驟工具輪次在視覺上保持活躍，而不是在第一次思考預覽和最終答案之間保持沈默。

支援的平台：

- 當啟用預覽串流時，**Discord**、**Slack**、**Telegram** 和 **Matrix** 預設會將工具進度串流到即時預覽編輯中。Microsoft Teams 在個人聊天中使用其原生進度串流。
