...
串流媒體必須使用結構化的 Payload 欄位，例如 `mediaUrl` 或
`mediaUrls`；串流文字不會被解析為附件指令。當區塊
串流提早發送媒體時，OpenClaw 會記得該輪次的發送紀錄。如果
最終的 Assistant Payload 重複相同的媒體 URL，最終的發送
會移除重複的媒體，而不是再次發送附件。

完全重複的最終 Payload 會被抑制。如果最終 Payload 在已
串流的媒體周圍新增了不同的文字，OpenClaw 仍會發送該新
文字，同時保持媒體僅發送一次。這可以防止在 Telegram 等
頻道上出現重複的語音訊息或檔案。
