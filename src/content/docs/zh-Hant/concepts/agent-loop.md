...
- Session 活性診斷：啟用診斷後，`diagnostics.stuckSessionWarnMs` 會將沒有觀察到回覆、工具、狀態、區塊或 ACP 進度的長時間 `processing` session 分類。活躍的嵌入式執行、模型呼叫和工具呼叫回報為 `session.long_running`；沒有近期進度回報的活躍工作回報為 `session.stalled`；`session.stuck` 保留給沒有活躍工作的過時 session 維護。過時的 session 維護會立即釋放受影響的 session 通道；停滯的嵌入式執行僅在 `diagnostics.stuckSessionAbortMs` 之後才會被中止排空（預設：至少 5 分鐘且為警告閾值的 3 倍），以便佇列的工作可以恢復，而不會僅僅因為執行緩慢就中斷。恢復會發出結構化的請求/完成結果，並且只有在相同的處理世代仍然是最新的時候，診斷狀態才會被標記為閒置。重複的 `session.stuck` 診斷會在 session 保持不變時退避。
