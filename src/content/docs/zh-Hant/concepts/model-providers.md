...
**LLM/模型提供者**的參考資料（不包括 WhatsApp/Telegram 等聊天頻道）。如需了解模型選擇規則，請參閱 [Models](/zh-Hant/concepts/models)。
...
<AccordionGroup>
  <Accordion title="模型參照與 CLI 輔助工具">
    ...
    - 設定後，`agents.defaults.models` 充當允許清單。
    - CLI 輔助工具：`openclaw onboard`、`openclaw models list`、`openclaw models set <provider/model>`。
    - `models.providers.*.contextWindow` / `contextTokens` / `maxTokens` 設定提供者層級的預設值；`models.providers.*.models[].contextWindow` / `contextTokens` / `maxTokens` 則針對各個模型進行覆寫。
    - 故障轉移規則、冷卻探測和會話覆寫持久性：[Model failover](/zh-Hant/concepts/model-failover)。
  ...
  <Accordion title="OpenAI 提供者/執行時期區分">
    ...
    - `openai/<model>` 預設針對代理程式回合使用原生 Codex 應用程式伺服器束縛。這是常見的 ChatGPT/Codex 訂閱設定。
    - `openai-codex/<model>` 是舊版設定，doctor 會將其重寫為 `openai/<model>`。
    - `openai/<model>` 加上提供者/模型 `agentRuntime.id: "pi"` 則是使用 PI 進行明確 API 金鑰或相容性路由。

    請參閱 [OpenAI](/zh-Hant/providers/openai) 和 [Codex harness](/zh-Hant/plugins/codex-harness)。如果提供者/執行時期的區分讓您感到困惑，請先閱讀 [Agent runtimes](/zh-Hant/concepts/agent-runtimes)。

    外掛程式自動啟用遵循相同的邊界：`openai/*` 代理程式參照會為預設路由啟用 Codex 外掛程式，而明確的提供者/模型 `agentRuntime.id: "codex"` 或舊版 `codex/<model>` 參照也需要它。

    GPT-5.5 在 `openai/gpt-5.5` 上預設透過原生 Codex 應用程式伺服器束縛提供，並且僅在提供者/模型執行時期政策明確選擇 `pi` 時才透過 PI 提供。
...
提供者 SDK 掛鉤和捆綁外掛範例的完整清單位於 [Provider plugins](/zh-Hant/plugins/sdk-provider-plugins)。需要完全自訂請求執行器的提供者，屬於另一個更深層的擴充介面。
...
- 原生 Codex 應用程式伺服器套件文件：[Codex harness](/zh-Hant/plugins/codex-harness)
- 舊版模型參考：`codex/gpt-*`
...
- 較舊的 `openai-codex/gpt-5.1*`、`openai-codex/gpt-5.2*` 和 `openai-codex/gpt-5.3*` 參考會被隱藏，因為 ChatGPT/Codex OAuth 帳戶會拒絕它們；請改用 `openai-codex/gpt-5.5` 或原生 Codex 運行時路由。
...
- 別名：`google/gemini-3.1-pro` 會被接受並正規化為 Google 的現行 Gemini API ID `google/gemini-3.1-pro-preview`
- CLI：`openclaw onboard --auth-choice gemini-api-key`
- 思考：`/think adaptive` 使用 Google 動態思考。Gemini 3/3.1 省略固定的 `thinkingLevel`；Gemini 2.5 會傳送 `thinkingBudget: -1`。
- 直接的 Gemini 執行也接受 `agents.defaults.models["google/<model>"].params.cachedContent` (或舊版 `cached_content`) 來轉發提供者原生的 `cachedContents/...` 控制碼；Gemini 快取命中會顯示為 OpenClaw `cacheRead`
...
Gemini CLI OAuth 隨附於打包的 `google` 外掛中。
...
<Steps>
  ...
  <Step title="登入">
    ...
    預設模型：`google-gemini-cli/gemini-3-flash-preview`。請**勿**將客戶端 ID 或金鑰貼到 `openclaw.json` 中。CLI 登入流程會將權杖儲存在閘道主機上的驗證設定檔中。
...
Gemini CLI JSON 回覆是從 `response` 解析而來；使用量會回退到 `stats`，其中 `stats.cached` 會正規化為 OpenClaw `cacheRead`。
...
- 提供商：`kilocode`
- 驗證：`KILOCODE_API_KEY`
- 模型範例：`kilocode/kilo/auto`
- CLI：`openclaw onboard --auth-choice kilocode-api-key`
- 基礎 URL：`https://api.kilo.ai/api/gateway/`
- 靜態回退目錄附帶 `kilocode/kilo/auto`；即時 `https://api.kilo.ai/api/gateway/models` 探索可以進一步擴充執行時期目錄。
- `kilocode/kilo/auto` 背後的精確上游路由是由 Kilo Gateway 管理，而非硬編碼在 OpenClaw 中。
...
<AccordionGroup>
  <Accordion title="OpenRouter">
    僅在已驗證的 `openrouter.ai` 路由上套用其應用程式歸因標頭和 Anthropic `cache_control` 標記。DeepSeek、Moonshot 和 ZAI 參照符合 OpenRouter 管理的提示詞快取 (prompt caching) 的 cache-TTL 資格，但不會收到 Anthropic 快取標記。作為代理樣式的 OpenAI 相容路徑，它會跳過僅限原生 OpenAI 的塑形 (`serviceTier`、Responses `store`、提示詞快取提示、OpenAI 推理相容)。由 Gemini 支援的參照僅保留代理 Gemini 的思維簽章清理。
  </Accordion>
  <Accordion title="Kilo Gateway">
    由 Gemini 支援的參照遵循相同的代理 Gemini 清理路徑；`kilocode/kilo/auto` 和其他不支援代理推理的參照會跳過代理推理注入。
  </Accordion>
  <Accordion title="MiniMax">
    API 金鑰導入會寫入明確的僅文字 M2.7 聊天模型定義；圖片理解保留在擁有者為外掛程式的 `MiniMax-VL-01` 媒體提供者上。
  </Accordion>
  <Accordion title="NVIDIA">
    模型 ID 使用 `nvidia/<vendor>/<model>` 命名空間 (例如 `nvidia/nvidia/nemotron-...` 以及 `nvidia/moonshotai/kimi-k2.5`)；選擇器會保留字面的 `<provider>/<model-id>` 組合，而傳送至 API 的標準金鑰則保持單一字首。
  </Accordion>
  <Accordion title="xAI">
    使用 xAI Responses 路徑。`grok-4.3` 是隨附的預設聊天模型。`/fast` 或 `params.fastMode: true` 會將 `grok-3`、`grok-3-mini`、`grok-4` 和 `grok-4-0709` 重寫為其 `*-fast` 變體。`tool_stream` 預設為開啟；透過 `agents.defaults.models["xai/<model>"].params.tool_stream=false` 停用。
  </Accordion>
  <Accordion title="Cerebras">
    作為內建的 `cerebras` 供應商插件隨附。GLM 使用 `zai-glm-4.7`；OpenAI 相容的基礎 URL 為 `https://api.cerebras.ai/v1`。
  </Accordion>
</AccordionGroup>
...
下列許多內建的供應商插件已發布預設目錄。僅在您想要覆寫預設基礎 URL、標頭或模型列表時，才使用明確的 `models.providers.<id>` 項目。

Gateway 模型功能檢查也會讀取明確的 `models.providers.<id>.models[]` 元資料。如果自訂或代理模型接受圖片，請在該模型上設定 `input: ["text", "image"]`，讓 WebChat 和節點來源的附件路徑將圖片作為原生模型輸入傳遞，而非僅文字的媒體參照。

`agents.defaults.models["provider/model"]` 僅控制代理程式的模型可見性、別名和個別模型元資料。它本身不會註冊新的執行階段模型。對於自訂供應商模型，還要新增 `models.providers.<provider>.models[]`，且至少包含匹配的 `id`。

### Moonshot AI (Kimi)

Moonshot 作為內建供應商插件隨附。預設使用內建供應商，並且僅在您需要覆寫基礎 URL 或模型元資料時才新增明確的 `models.providers.moonshot` 項目：
...
舊版的 `kimi/kimi-code` 和 `kimi/k2p5` 仍被接受為相容性模型 ID，並會正規化為 Kimi 的穩定 API 模型 ID。
...
MiniMax 是透過 `models.providers` 進行配置，因為它使用自訂端點：
...
請參閱 [/providers/minimax](/zh-Hant/providers/minimax) 以了解設定詳細資訊、模型選項和配置片段。

<Note>在 MiniMax 相容 Anthropic 的串流路徑上，OpenClaw 預設會停用思考，除非您明確設定，且 `/fast on` 會將 `MiniMax-M2.7` 重寫為 `MiniMax-M2.7-highspeed`。</Note>

外掛擁有的功能拆分：

- 文字/聊天預設值保持在 `minimax/MiniMax-M2.7`
- 圖片生成為 `minimax/image-01` 或 `minimax-portal/image-01`
- 圖片理解在兩個 MiniMax 驗證路徑上皆為外掛擁有的 `MiniMax-VL-01`
...
- 提供者：`lmstudio`
- 驗證：`LM_API_TOKEN`
- 預設推理基礎 URL：`http://localhost:1234/v1`
...
OpenClaw 使用 LM Studio 的原生 `/api/v1/models` 和 `/api/v1/models/load` 進行探索和自動載入，並預設使用 `/v1/chat/completions` 進行推理。如果您希望讓 LM Studio 的 JIT 載入、TTL 和自動驅逐來管理模型生命週期，請設定 `models.providers.lmstudio.params.preload: false`。請參閱 [/providers/lmstudio](/zh-Hant/providers/lmstudio) 以了解設定和疑難排解。
...
當您使用 `OLLAMA_API_KEY` 啟用時，OpenClaw 會在 `http://127.0.0.1:11434` 偵測到本機的 Ollama，且內建的供應商外掛會直接將 Ollama 新增至 `openclaw onboard` 和模型選擇器。請參閱 [/providers/ollama](/zh-Hant/providers/ollama) 以了解入門、雲端/本機模式和自訂設定。
...
<AccordionGroup>
  <Accordion title="Default optional fields">
    對於自訂供應商，`reasoning`、`input`、`cost`、`contextWindow` 和 `maxTokens` 為選填項目。若省略這些欄位，OpenClaw 預設為：
    ...
    建議：設定與您的 Proxy/模型 限制相符的明確值。

  </Accordion>
  <Accordion title="代理路由塑形規則">
    - 對於非原生端點上的 `api: "openai-completions"`（任何主機不是 `api.openai.com` 的非空 `baseUrl`），OpenClaw 會強制執行 `compat.supportsDeveloperRole: false`，以避免供應商因不支援的 `developer` 角色而傳回 400 錯誤。
    - 代理樣式的 OpenAI 相容路由也會跳過原生 OpenAI 專用的請求塑形：無 `service_tier`、無 Responses `store`、無 Completions `store`、無提示快取提示、無 OpenAI 推理相容酬載塑形，且無隱藏的 OpenClaw 歸因標頭。
    - 對於需要供應商特定欄位的 OpenAI 相容 Completions 代理，請設定 `agents.defaults.models["provider/model"].params.extra_body`（或 `extraBody`）以將額外的 JSON 合併到外傳請求主體中。
    - 對於 vLLM 聊天範本控制，請設定 `agents.defaults.models["provider/model"].params.chat_template_kwargs`。當會話思考層級關閉時，隨附的 vLLM 外掛程式會自動為 `vllm/nemotron-3-*` 發送 `enable_thinking: false` 和 `force_nonempty_content: true`。
    - 對於緩慢的本機模型或遠端 LAN/tailnet 主機，請設定 `models.providers.<id>.timeoutSeconds`。這會延長供應商模型 HTTP 請求處理，包括連線、標頭、主體串流和總保護擷取中止，而不會增加整個 Agent 執行時逾時。如果 `agents.defaults.timeoutSeconds` 或執行特定逾時較低，請一併提高該上限；供應商逾時無法延長整個執行。
    - 模型供應商 HTTP 呼叫僅針對已設定的供應商 `baseUrl` 主機名稱，允許 `198.18.0.0/15` 和 `fc00::/7` 中的 Surge、Clash 和 sing-box 假 IP DNS 回應。自訂/本機供應商端點也會信任該確切設定的 `scheme://host:port` 來源以進行受保護的模型請求，包括 loopback、LAN 和 tailnet 主機。這不是一個新的設定選項；您設定的 `baseUrl` 僅擴展該來源的請求原則。假 IP 主機名稱允許和確切來源信任是獨立的機制。其他私有、loopback、連結本機、中繼資料目的地和不同連接埠仍然需要明確的 `models.providers.<id>.request.allowPrivateNetwork: true` 加入設定。設定 `models.providers.<id>.request.allowPrivateNetwork: false` 以退出確切來源信任。
    - 如果 `baseUrl` 為空/省略，OpenClaw 會保留預設的 OpenAI 行為（其解析為 `api.openai.com`）。
    - 為了安全起見，明確的 `compat.supportsDeveloperRole: true` 在非原生 `openai-completions` 端點上仍然會被覆寫。
    - 對於非直接端點上的 `api: "anthropic-messages"`（任何非標準 `anthropic` 的供應商，或主機不是公用 `api.anthropic.com` 端點的自訂 `models.providers.anthropic.baseUrl`），OpenClaw 會隱含的 Anthropic beta 標頭（例如 `claude-code-20250219`、`interleaved-thinking-2025-05-14` 和 OAuth 標記），以便自訂 Anthropic 相容代理不會因不支援的 beta 旗標而拒絕請求。如果您的代理需要特定的 beta 功能，請明確設定 `models.providers.<id>.headers["anthropic-beta"]`。
...
- [設定參考](/zh-Hant/gateway/config-agents#agent-defaults) - 模型設定金鑰
- [模型容錯移轉](/zh-Hant/concepts/model-failover) - 故障轉移鏈與重試行為
