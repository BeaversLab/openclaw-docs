...
**LLM/模型供應商**參考（不包括 WhatsApp/Telegram 等聊天頻道）。關於模型選擇規則，請參閱 [Models](/zh-Hant/concepts/models)。
...
<AccordionGroup>
  <Accordion title="Model refs and CLI helpers">
    ...
    - `agents.defaults.models` 在設定時充當允許清單。
    - CLI 輔助工具：`openclaw onboard`、`openclaw models list`、`openclaw models set <provider/model>`。
    - `models.providers.*.contextWindow` / `contextTokens` / `maxTokens` 設定供應商層級的預設值；`models.providers.*.models[].contextWindow` / `contextTokens` / `maxTokens` 會針對各個模型覆寫這些預設值。
  ...
  <Accordion title="OpenAI provider/runtime split">
    OpenAI 系列路由是以前綴為特定依據的：

    - `openai/<model>` 預設針對 Agent 交談使用原生的 Codex 應用程式伺服器駕駛程式。這是常見的 ChatGPT/Codex 訂閱設定。
    - `openai-codex/<model>` 是舊版設定，doctor 會將其重寫為 `openai/<model>`。
    - `openai/<model>` 加上供應商/模型 `agentRuntime.id: "pi"` 會使用 PI 進行明確 API 金鑰或相容性路由。

    請參閱 [OpenAI](/zh-Hant/providers/openai) 和 [Codex harness](/zh-Hant/plugins/codex-harness)。如果供應商/執行階段區分讓您感到困惑，請先閱讀 [Agent runtimes](/zh-Hant/concepts/agent-runtimes)。

    外掛程式自動啟用遵循相同的邊界：`openai/*` Agent 參照會為預設路由啟用 Codex 外掛程式，而明確的供應商/模型 `agentRuntime.id: "codex"` 或舊版 `codex/<model>` 參照也需要它。

    GPT-5.5 在 `openai/gpt-5.5` 上預設可透過原生的 Codex 應用程式伺服器駕駛程式使用，且僅在供應商/模型執行階段原則明確選擇 `pi` 時透過 PI 使用。
...
提供者 SDK hooks 和捆綁外掛程式範例的完整清單位於 [Provider plugins](/zh-Hant/plugins/sdk-provider-plugins)。需要完全自訂請求執行器的提供者，是一個獨立、更深入的擴充介面。
...
- 原生 Codex 應用程式伺服器線上文件：[Codex harness](/zh-Hant/plugins/codex-harness)
- 舊版模型參考：`codex/gpt-*`
...
<CardGroup cols={3}>
  <Card title="Z.AI (GLM)" href="/zh-Hant/providers/zai">
...
有關設定詳情，請參閱 [/providers/kilocode](/zh-Hant/providers/kilocode)。
...
<AccordionGroup>
  ...
  <Accordion title="xAI">
    使用 xAI Responses 路徑。建議的路徑是 SuperGrok/X Premium OAuth；API 金鑰仍可透過 `XAI_API_KEY` 或外掛程式設定運作，且 Grok `web_search` 會在 API 金鑰回退之前重複使用相同的授權設定檔。`grok-4.3` 是內建的預設聊天模型，而 `grok-build-0.1` 可選用於建置/編碼專注的工作。`/fast` 或 `params.fastMode: true` 會將 `grok-3`、`grok-3-mini`、`grok-4` 和 `grok-4-0709` 重寫為其 `*-fast` 變體。`tool_stream` 預設為開啟；可透過 `agents.defaults.models["xai/<model>"].params.tool_stream=false` 停用。
  </Accordion>
  <Accordion title="Cerebras">
    以內建的 `cerebras` 提供者外掛程式形式提供。GLM 使用 `zai-glm-4.7`；OpenAI 相容的基底 URL 是 `https://api.cerebras.ai/v1`。
  </Accordion>
</AccordionGroup>

## 透過 `models.providers` (custom/base URL) 的提供者

使用 `models.providers` (或 `models.json`) 來新增 **自訂** 提供者或 OpenAI/Anthropic 相容的 Proxy。

以下許多內建的提供者外掛程式已經發佈了預設目錄。僅在您想要覆寫預設基底 URL、標頭或模型清單時，才使用明確的 `models.providers.<id>` 項目。

Gateway 模型能力檢查也會讀取明確的 `models.providers.<id>.models[]` 元資料。如果自訂或代理模型接受圖片，請在該模型上設定 `input: ["text", "image"]`，以便 WebChat 和節點來源的附加路徑將圖片作為原生模型輸入傳遞，而不是僅限文字的媒體參考。

`agents.defaults.models["provider/model"]` 僅控制模型的可見性、別名以及給代理程式的個別模型元資料。它本身不會註冊新的執行時期模型。對於自訂提供者模型，還需新增 `models.providers.<provider>.models[]` 並至少包含符合的 `id`。

### Moonshot AI (Kimi)

Moonshot 作為捆綁的提供者外掛程式隨附。預設情況下使用內建提供者，僅當您需要覆寫基礎 URL 或模型元資料時，才新增明確的 `models.providers.moonshot` 項目：
...
舊版 `kimi/kimi-code` 和 `kimi/k2p5` 仍作為相容模型 ID 被接受，並正規化為 Kimi 的穩定 API 模型 ID。
...
上手預設為編碼介面，但同時會註冊一般 `volcengine/*` 目錄。

在上手/設定模型選擇器中，Volcengine 驗證選項偏好同時顯示 `volcengine/*` 和 `volcengine-plan/*` 列。如果這些模型尚未載入，OpenClaw 會改為回退到未篩選的目錄，而不是顯示空的提供者範圍選擇器。
...
入門預設為編碼介面，但同時會註冊一般的 `byteplus/*` 目錄。

在入門/配置模型選擇器中，BytePlus 驗證選擇會優先顯示 `byteplus/*` 和 `byteplus-plan/*` 列。如果這些模型尚未載入，OpenClaw 將回退到未過濾的目錄，而不是顯示空的提供商範圍選擇器。
...
Synthetic 在 `synthetic` 提供商後面提供 Anthropic 相容的模型：
...
- MiniMax API 金鑰 (Global): `--auth-choice minimax-global-api`
- MiniMax API 金鑰 (CN): `--auth-choice minimax-cn-api`
- Auth: `MINIMAX_API_KEY` 用於 `minimax`；`MINIMAX_OAUTH_TOKEN` 或 `MINIMAX_API_KEY` 用於 `minimax-portal`

參閱 [/providers/minimax](/zh-Hant/providers/minimax) 以了解設定細節、模型選項和配置片段。

<Note>在 MiniMax 的 Anthropic 相容串流路徑上，除非您明確設定，否則 OpenClaw 預設會停用思考，並且 `/fast on` 會將 `MiniMax-M2.7` 重寫為 `MiniMax-M2.7-highspeed`。</Note>

外掛擁有的功能拆分：

- 文字/聊天預設值保持在 `minimax/MiniMax-M2.7`
- 圖像生成是 `minimax/image-01` 或 `minimax-portal/image-01`
- 圖像理解在兩個 MiniMax 認證路徑上皆為外掛擁有的 `MiniMax-VL-01`
...
然後設定一個模型 (替換為 `http://localhost:1234/api/v1/models` 回傳的其中一個 ID)：
...
OpenClaw 使用 LM Studio 的原生 `/api/v1/models` 和 `/api/v1/models/load` 進行探索 + 自動載入，並預設使用 `/v1/chat/completions` 進行推論。如果您希望 LM Studio 的 JIT 載入、TTL 和自動驅逐擁有模型生命週期，請設定 `models.providers.lmstudio.params.preload: false`。參閱 [/providers/lmstudio](/zh-Hant/providers/lmstudio) 以了解設定和疑難排解。
...
當您使用 `OLLAMA_API_KEY` 選擇加入時，Ollama 會在 `http://127.0.0.1:11434` 本機被偵測到，且隨附的提供者外掛會直接將 Ollama 加入 `openclaw onboard` 和模型選擇器。參閱 [/providers/ollama](/zh-Hant/providers/ollama) 以了解上手指引、雲端/本機模式和自訂配置。
...
<AccordionGroup>
  <Accordion title="Default optional fields">
    對於自訂供應商，`reasoning`、`input`、`cost`、`contextWindow` 和 `maxTokens` 是選用的。若省略，OpenClaw 預設為：
  ...
  <Accordion title="Proxy-route shaping rules">
    - 對於非原生端點上的 `api: "openai-completions"`（任何主機不是 `api.openai.com` 的非空 `baseUrl`），OpenClaw 會強制執行 `compat.supportsDeveloperRole: false`，以避免因提供者不支援的 `developer` 角色而導致的 400 錯誤。
    - 代理式 OpenAI 相容路由也會跳過原生 OpenAI 專用的請求塑形：無 `service_tier`、無 Responses `store`、無 Completions `store`、無提示快取提示、無 OpenAI 推理相容負載塑形，以及無隱藏的 OpenClaw 歸因標頭。
    - 對於需要供應商特定欄位的 OpenAI 相容 Completions 代理，請設定 `agents.defaults.models["provider/model"].params.extra_body`（或 `extraBody`）以將額外的 JSON 合併到出站請求主體中。
    - 對於 vLLM 聊天範本控制，請設定 `agents.defaults.models["provider/model"].params.chat_template_kwargs`。當會話思考層級關閉時，隨附的 vLLM 外掛會自動為 `vllm/nemotron-3-*` 發送 `enable_thinking: false` 和 `force_nonempty_content: true`。
    - 對於緩慢的本機模型或遠端 LAN/tailnet 主機，請設定 `models.providers.<id>.timeoutSeconds`。這會延伸提供者模型的 HTTP 請求處理，包括連線、標頭、主體串流和總受防護的取得中止，而不會增加整個代理程式執行時間逾時。如果 `agents.defaults.timeoutSeconds` 或特定執行的逾時較低，請一併提高該上限；提供者逾時無法延伸整個執行過程。
    - 模型提供者 HTTP 呼叫僅針對已設定的提供者 `baseUrl` 主機名稱，允許在 `198.18.0.0/15` 和 `fc00::/7` 中使用 Surge、Clash 和 sing-box 的偽 IP DNS 回應。自訂/本機提供者端點也會信任受防護模型請求的該確切已設定 `scheme://host:port` 來源，包括回環、LAN 和 tailnet 主機。這不是一個新的設定選項；您設定的 `baseUrl` 僅延伸該來源的請求原則。偽 IP 主機名稱許可和確切來源信任是獨立的機制。其他私有、回環、連結本機、中繼資料目的地和不同連接埠仍然需要明確的 `models.providers.<id>.request.allowPrivateNetwork: true` 選用加入。請設定 `models.providers.<id>.request.allowPrivateNetwork: false` 以選擇退出確切來源信任。
    - 如果 `baseUrl` 為空/省略，OpenClaw 將保留預設的 OpenAI 行為（即解析為 `api.openai.com`）。
    - 為了安全起見，明確的 `compat.supportsDeveloperRole: true` 仍會在非原生 `openai-completions` 端點上被覆蓋。
    - 對於非直接端點上的 `api: "anthropic-messages"`（任何非正規 `anthropic` 的提供者，或主機不是公用 `api.anthropic.com` 端點的自訂 `models.providers.anthropic.baseUrl`），OpenClaw 會抑制隱含的 Anthropic 測試版標頭（例如 `claude-code-20250219`、`interleaved-thinking-2025-05-14` 和 OAuth 標記），以便自訂 Anthropic 相容代理不會拒絕不支援的測試版旗標。如果您的代理需要特定的測試版功能，請明確設定 `models.providers.<id>.headers["anthropic-beta"]`。
