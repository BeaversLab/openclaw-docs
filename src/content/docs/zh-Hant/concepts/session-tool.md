...
- `action: "steer"` 以將後續指導傳送給執行中的子任務
- `action: "kill"` 以停止單一子任務或 `all`

## 產生子代理

`sessions_spawn` 預設會為背景任務建立一個獨立的 session。它總是非阻塞的——它會立即返回一個 `runId` 和
`childSessionKey`。原生子代理執行會在子 session 的第一個可見 `[Subagent Task]` 訊息中接收被委派的任务，而系統提示則僅攜帶子代理運行時規則和路由上下文。

主要選項：

- `runtime: "subagent"` (預設) 或 `"acp"` 用於外部駁動代理。
- `model` 和 `thinking` 覆寫用於子 session。
- `thread: true` 用於將產生的子代理綁定到聊天執行緒 (Discord, Slack 等)。
- `sandbox: "require"` 用於對子代理強制執行沙盒機制。
- 當子代理需要當前請求者的紀錄時，原生子代理使用 `context: "fork"`；若要獲得一個乾淨的子代理，則省略它或使用 `context: "isolated"`。
  綁定執行緒的原生子代理預設為 `context: "fork"`，除非
  `threadBindings.defaultSpawnContext` 另有指示。

預設的葉節點子代理不會獲得 session 工具。當
`maxSpawnDepth >= 2` 時，深度為 1 的編排器子代理還會額外接收
`sessions_spawn`、`subagents`、`sessions_list` 和 `sessions_history`，以便它們
能夠管理自己的子代理。葉節點執行仍然不會獲得遞迴的編排工具。

完成後，一個公告步驟會將結果發佈到請求者的頻道。
完成交付會在可用時保留綁定的執行緒/主題路由，而且如果
完成來源僅標識一個頻道，OpenClaw 仍然可以重複使用
請求者 session 中儲存的路由 (`lastChannel` / `lastTo`) 進行直接
交付。
...
預設值為 `tree`。無論配置如何，沙盒 session 都會被限制為 `tree`。

## 延伸閱讀

- [Session Management](/zh-Hant/concepts/session) -- 路由、生命週期、維護
- [ACP Agents](/zh-Hant/tools/acp-agents) -- 外部 harness 產生
- [Multi-agent](/zh-Hant/concepts/multi-agent) -- 多代理架構
- [Gateway Configuration](/zh-Hant/gateway/configuration) -- Session 工具設定選項
