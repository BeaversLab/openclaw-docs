...
`sessions_yield` 故意結束當前輪次，以便下一則訊息可以是您正在等待的後續事件。在產生子代理且您希望完成結果作為下一則訊息到達，而不是建立輪詢迴圈時，請使用它。

`subagents` 是已產生的 OpenClaw 子代理的可見性輔助工具。它支援 `action: "list"` 來檢查作用中/最近的執行。

## 產生子代理

`sessions_spawn` 預設會為背景工作建立一個獨立的作業階段。它始終是非阻塞的——它會立即傳回 `runId` 和 `childSessionKey`。原生子代理執行會在子作業階段的第一則可見 `[Subagent Task]` 訊息中接收被指派的工作，而系統提示詞僅攜帶子代理執行時期規則和路由上下文。

主要選項：

- `runtime: "subagent"` (預設) 或 `"acp"` 用於外部駕駛代理。
- `model` 和 `thinking` 覆寫用於子作業階段。
- `thread: true` 用於將產生的執行繫結到聊天執行緒 (Discord、Slack 等)。
- `sandbox: "require"` 用於對子項強制執行沙盒機制。
- 當子項需要目前的請求者記錄時，原生子代理使用 `context: "fork"`；若要建立乾淨的子項，請省略它或使用 `context: "isolated"`。除非 `threadBindings.defaultSpawnContext` 另有指示，否則繫結至執行緒的原生子代理預設為 `context: "fork"`。

預設的葉節點子代理不會獲得作業階段工具。當 `maxSpawnDepth >= 2` 時，深度為 1 的協調器子代理還會收到 `sessions_spawn`、`subagents`、`sessions_list` 和 `sessions_history`，以便它們能管理自己的子項。葉節點執行仍然不會獲得遞迴協調工具。

完成後，公告步驟會將結果發佈到請求者的頻道。完成交付會在可用時保留綁定的執行緒/主題路由，如果完成來源僅識別頻道，OpenClaw 仍可重用請求者會話的儲存路由 (`lastChannel` / `lastTo`) 進行直接交付。
...
預設值為 `tree`。無論配置為何，沙箱會話都會被限制為 `tree`。

## 延伸閱讀

- [Session Management](/zh-Hant/concepts/session) -- 路由、生命週期、維護
- [ACP Agents](/zh-Hant/tools/acp-agents) -- 外部機具生成
- [Multi-agent](/zh-Hant/concepts/multi-agent) -- 多代理架構
- [Gateway Configuration](/zh-Hant/gateway/configuration) -- 會話工具配置選項
