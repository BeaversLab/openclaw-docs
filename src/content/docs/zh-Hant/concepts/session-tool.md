...
`sessions_spawn` 預設會為背景任務建立一個隔離的會話。
它總是非阻塞的——它會立即回傳 `runId` 和
`childSessionKey`。

主要選項：

- `runtime: "subagent"` （預設）或 `"acp"`，用於外部 harness 代理。
- 子會話的 `model` 和 `thinking` 覆寫。
- `thread: true` 以將產生綁定到聊天執行緒（Discord、Slack 等）。
- `sandbox: "require"` 以對子任務強制執行沙盒機制。
- `context: "fork"` 用於原生子代理，當子代理需要當前的請求者記錄時；若要獲得乾淨的子代理，請省略它或使用 `context: "isolated"`。
  繫結至執行緒的原生子代理預設為 `context: "fork"`，除非 `threadBindings.defaultSpawnContext` 另有指示。

預設的葉子節點子代理不會獲得 session 工具。當 `maxSpawnDepth >= 2` 時，深度為 1 的協調器子代理還會額外接收 `sessions_spawn`、`subagents`、`sessions_list` 和 `sessions_history`，以便它們管理自己的子代。葉子節點執行仍然不會取得遞迴協調工具。

完成後，公告步驟會將結果發佈至請求者的頻道。完成交付會在可用時保留繫結的執行緒/主題路由，如果完成來源僅識別頻道，OpenClaw 仍然可以重複使用請求者 session 的儲存路由 (`lastChannel` / `lastTo`) 進行直接交付。
...
預設值為 `tree`。沙盒化 session 無論設定如何都會被限制為 `tree`。

## 延伸閱讀

- [Session Management](/zh-Hant/concepts/session) -- 路由、生命週期、維護
- [ACP Agents](/zh-Hant/tools/acp-agents) -- 外部框架生成
