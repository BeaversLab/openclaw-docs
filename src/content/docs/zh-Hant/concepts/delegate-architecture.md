...
這將 [多智能體路由](/zh-Hant/concepts/multi-agent) 從個人使用擴展到組織部署。
...
- 遵循 **[常駐指令](/zh-Hant/automation/standing-orders)** —— 在智能體 `AGENTS.md` 中定義的規則，指定它可以自主執行什麼操作，以及什麼操作需要人類批准（有關排程執行，請參閱 [Cron Jobs](/zh-Hant/automation/cron-jobs)）。
...
此層級結合了第 2 層級的權限與 [Cron Jobs](/zh-Hant/automation/cron-jobs) 和 [常駐指令](/zh-Hant/automation/standing-orders)。
...
請參閱 [沙盒隔離](/zh-Hant/gateway/sandboxing) 和 [多智能體沙盒與工具](/zh-Hant/tools/multi-agent-sandbox-tools)。
...
註冊一個具有 `Mail.Read` 和 `Calendars.Read` 應用程式權限的 Azure AD 應用程式。**在使用應用程式之前**，請使用 [應用程式存取原則](https://learn.microsoft.com/graph/auth-limit-mailbox-access) 限制存取範圍，僅限委派者和主體信箱：
...
使用 [多智能體路由](/zh-Hant/concepts/multi-agent) 綁定將傳入訊息路由到委派智能體：
...
切勿將主智能體的 `agentDir` 與委派者共享。有關驗證隔離的詳細資訊，請參閱 [多智能體路由](/zh-Hant/concepts/multi-agent)。
...
委派者的 `AGENTS.md` 定義了其自主權限 —— 它可以在無需請求的情況下執行什麼操作、什麼操作需要批准以及什麼操作是被禁止的。[Cron Jobs](/zh-Hant/automation/cron-jobs) 驅動其每日排程。

如果您授予 `sessions_history`，請記住這是一個受限的、經過安全過濾的
回想檢視。OpenClaw 會編輯類似憑證/權杖的文字、截斷長
內容、移除思考標籤 / `<relevant-memories>` 腳手架 / 純文字
工具呼叫 XML 載荷（包括 `<tool_call>...</tool_call>`、
`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、
`<function_calls>...</function_calls>` 和截斷的工具呼叫區塊） /
降級的工具呼叫腳手架 / 從助手回想中洩漏的 ASCII/全形模型控制
權杖 / 格式錯誤的 MiniMax 工具呼叫 XML，並且可以
用 `[sessions_history omitted: message too large]` 替換過大的列
而不是傳回原始轉錄傾印。
...
1. **為每個組織建立一個委派代理程式**。
2. **優先進行強化** — 工具限制、沙箱、硬性阻斷、稽核追蹤。
3. **透過身分識別提供者授予特定範圍的權限**（最小權限原則）。
4. **定義[常駐指令](/zh-Hant/automation/standing-orders)**以進行自主操作。
5. **排程 cron 工作**以處理週期性任務。
6. **審查並調整**能力層級，隨著信任的建立逐步調整。
