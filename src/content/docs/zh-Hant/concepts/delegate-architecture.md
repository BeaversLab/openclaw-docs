...
這將 [多智能體路由](/en/concepts/multi-agent) 從個人使用擴展到組織部署。
...
- 遵循 **[常駐指令](/en/automation/standing-orders)** —— 在智能體 `AGENTS.md` 中定義的規則，指定它可以自主執行什麼操作，以及什麼操作需要人類批准（有關排程執行，請參閱 [Cron Jobs](/en/automation/cron-jobs)）。
...
此層級結合了第 2 層級的權限與 [Cron Jobs](/en/automation/cron-jobs) 和 [常駐指令](/en/automation/standing-orders)。
...
請參閱 [沙盒隔離](/en/gateway/sandboxing) 和 [多智能體沙盒與工具](/en/tools/multi-agent-sandbox-tools)。
...
使用 [多智能體路由](/en/concepts/multi-agent) 綁定將傳入訊息路由到委派智能體：
...
切勿將主智能體的 `agentDir` 與委派者共享。有關驗證隔離的詳細資訊，請參閱 [多智能體路由](/en/concepts/multi-agent)。
...
委派者的 `AGENTS.md` 定義了其自主權限 —— 它可以在無需請求的情況下執行什麼操作、什麼操作需要批准以及什麼操作是被禁止的。[Cron Jobs](/en/automation/cron-jobs) 驅動其每日排程。
