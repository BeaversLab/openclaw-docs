...
- 遵循 **[常備指令](/zh-Hant/automation/standing-orders)** - 在代理程式的 `AGENTS.md` 中定義的規則，指定它可以自主執行什麼操作以及什麼操作需要人工批准（請參閱 [Cron Jobs](/zh-Hant/automation/cron-jobs) 以了解排程執行）。
...
委派的 `AGENTS.md` 定義了其自主權限 - 它可以在不詢問的情況下做什麼，什麼需要批准，以及什麼是被禁止的。[Cron Jobs](/zh-Hant/automation/cron-jobs) 驅動其每日排程。
