...
| 介面             | 金鑰                                                      | 使用時機                                                                            | 更多資訊                                                                         |
| ---------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| 本機模型執行階段 | `agents.defaults.experimental.localModelLean`             | 較小或較嚴格的本機後端無法應付 OpenClaw 完整的預設工具介面                          | [本機模型](/en/gateway/local-models)                                             |
| 記憶體搜尋       | `agents.defaults.memorySearch.experimental.sessionMemory` | 您希望 `memory_search` 為先前的會話逐字稿建立索引，並接受額外的儲存/索引成本        | [記憶體設定參考](/en/reference/memory-config#session-memory-search-experimental) |
| 結構化規劃工具   | `tools.experimental.planTool`                             | 您希望在相容的執行階段和 UI 中公開結構化的 `update_plan` 工具，以進行多步驟工作追蹤 | [Gateway 設定參考](/en/gateway/configuration-reference#toolsexperimental)        |
