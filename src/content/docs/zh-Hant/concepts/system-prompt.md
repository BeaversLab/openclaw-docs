...
對於頻道自動回覆運行，當直接/群組聊天上下文已包含解析後的特定於對話的 `NO_REPLY` 行為時，OpenClaw 可以省略通用的 **Silent Replies** 章節。這可以避免在全域系統提示和頻道上下文中重複 token 機制。
...
大文件會被截斷並加上標記。每個檔案的大小上限由 `agents.defaults.bootstrapMaxChars` 控制（預設值：12000）。所有檔案注入的引導內容總量上限由 `agents.defaults.bootstrapTotalMaxChars` 控制（預設值：60000）。缺失的檔案會注入一個簡短的「檔案缺失」標記。發生截斷時，OpenClaw 可以注入一個簡明的系統提示詞警告通知；透過 `agents.defaults.bootstrapPromptTruncationWarning`（`off`、`once`、`always`；預設值：`once`）進行控制。詳細的原始/注入計數會保留在 `/context`、`/status`、doctor 和日誌等診斷資訊中。
