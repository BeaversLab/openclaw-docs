...
在原生 Codex 韌體上，OpenClaw 避免在每次使用者輪次中重複穩定的工作區檔案。Codex 透過其自己的專案文件發現來載入 `AGENTS.md`。`SOUL.md`、`IDENTITY.md`、`TOOLS.md` 和 `USER.md` 會被轉發為 Codex 開發者指令。`HEARTBEAT.md` 內容不會被注入；心跳輪次會收到一個協作模式註記，當該檔案存在且非空時指向該檔案。來自已設定代理工作區的 `MEMORY.md` 內容不會貼上到每個原生 Codex 輪次中；當記憶工具可用於該工作區時，Codex 輪次會收到一個小型工作區記憶註記，並且在持續性記憶相關時應使用 `memory_search` 或 `memory_get`。如果工具被停用、記憶搜尋無法使用，或目前使用的工作區與代理記憶工作區不同，`MEMORY.md` 會回退到正常的受限輪次上下文路徑。目前使用中 `BOOTSTRAP.md` 的內容暫時保持正常的輪次上下文角色。
...
該位置可以指向巢狀技能，例如
`skills/personal/foo/SKILL.md`。巢狀結構僅用於組織用途；提示
仍然使用 `SKILL.md` frontmatter 中的扁平技能名稱。

合格條件包括技能 metadata 閘道、執行時期環境/設定檢查，
以及當配置了 `agents.defaults.skills` 或
`agents.list[].skills` 時的有效代理技能允許清單。

外掛程式捆綁的技能僅在其所屬的外掛程式啟用時才符合資格。
這讓工具外掛程式能夠提供更深層的操作指南，而無需將所有
指引直接嵌入到每個工具描述中。
...
- 各代理覆寫：`agents.list[].skillsLimits.maxSkillsPromptChars`
...
這種區分讓技能的大小與執行時期讀取/注入的大小分開，例如 `memory_get`、即時工具結果以及壓縮後的 AGENTS.md 更新。

## 文件

系統提示詞包含一個 **文件** 區段。當有本機文件可用時，它會指向本機 OpenClaw 文件目錄（Git 檢出中的 `docs/` 或隨附的 npm 套件文件）。如果本機文件無法使用，則會退而求其次使用 [https://docs.openclaw.ai](https://docs.openclaw.ai)。

同一區段也包含 OpenClaw 的來源位置。Git 檢出會公開本機來源根目錄，以便代理程式能直接檢查程式碼。套件安裝則包含 GitHub 來源 URL，並告訴代理程式在文件不完整或過時時到那裡檢視來源。提示詞也註明了公開文件鏡像、社群 Discord 和 ClawHub（[https://clawhub.ai](https://clawhub.ai））以供技能發現。它告訴模型優先查閱文件以了解 OpenClaw 的行為、指令、設定或架構，並在可能時自行執行 `openclaw status`（僅在無權限存取時詢問使用者）。針對設定，它會指引代理程式前往 `gateway` 工具動作 `config.schema.lookup` 以取得精確的欄位級文件與限制，然後前往 `docs/gateway/configuration.md` 和 `docs/gateway/configuration-reference.md` 以獲得更廣泛的指引。
