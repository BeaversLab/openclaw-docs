...
OpenClaw 會為每個代理執行建構一個自訂系統提示。該提示是 **OpenClaw 擁有** 的，且不使用運行時預設提示。
...
在原生 Codex 鞍座（harness）上，OpenClaw 避免在每次使用者輪次中重複穩定的工作區檔案。Codex 透過其自己的專案文件發現來載入 `AGENTS.md`。`SOUL.md`、`IDENTITY.md`、`TOOLS.md` 和 `USER.md` 會作為 Codex 開發者指示被轉發。精簡的 OpenClaw 技能清單也會作為輪次範圍的協作開發者指示被轉發。`HEARTBEAT.md` 內容不會被注入；心跳輪次會在該檔案存在且非空時收到指向該檔案的協作模式註記。來自已設定代理程式工作區的 `MEMORY.md` 內容不會貼上到每個原生 Codex 輪次中；當該工作區有可用的記憶工具時，Codex 輪次會在輪次範圍的協作開發者指示中收到一個小的工作區記憶註記，並且在持久化記憶相關時應該使用 `memory_search` 或 `memory_get`。如果工具被停用、記憶搜尋無法使用，或者目前使用的工作區與代理程式記憶工作區不同，`MEMORY.md` 會退回到正常的受限輪次上下文路徑。使用中的 `BOOTSTRAP.md` 內容目前維持正常的輪次上下文角色。

在非 Codex 韌體上，引導檔案會根據其現有的閘門繼續被組合到 OpenClaw 提示中。當預設代理的心跳被停用或 `agents.defaults.heartbeat.includeSystemPromptSection` 為 false 時，`HEARTBEAT.md` 會在正常執行中被省略。保持注入檔案簡潔，特別是非 Codex 的 `MEMORY.md`。`MEMORY.md` 意在作為精選的長期摘要；詳細的每日筆記應放在 `memory/*.md` 中，其中 `memory_search` 和 `memory_get` 可以按需擷取它們。過大的非 Codex `MEMORY.md` 檔案會增加提示用量，並且可能因為下方的引導檔案限制而被部分注入。

<Note>`memory/*.md` 每日檔案**不是**正常啟動專案內容的一部分。在一般回合中，它們是透過 `memory_search` 和 `memory_get` 工具按需存取的，因此除非模型明確讀取它們，否則不會計入上下文視窗。純 `/new` 和 `/reset` 回合是例外：執行時可以將最近的每日記憶作為一次性啟動內容區塊前置於該第一個回合。</Note>

大檔案會被截斷並加上標記。每個檔案的最大大小由
`agents.defaults.bootstrapMaxChars` 控制（預設值：12000）。跨檔案注入的啟動內容總計上限由 `agents.defaults.bootstrapTotalMaxChars` 控制
（預設值：60000）。缺失的檔案會注入一個簡短的缺失檔案標記。發生截斷時，
OpenClaw 可以注入一個簡明的系統提示警告通知；使用 `agents.defaults.bootstrapPromptTruncationWarning` 控制此行為
（`off`、`once`、`always`；
預設值：`always`）。詳細的原始/注入計數保留在診斷資訊中，例如
`/context`、`/status`、doctor 和日誌。

對於記憶檔案，截斷並不會造成資料遺失：檔案在磁碟上保持完整。
在原生 Codex 上，當可用時，`MEMORY.md` 會透過記憶工具按需讀取，
當工具無法執行時則退回到有邊界的提示。在其他
harness 上，模型只能看到縮短的注入副本，直到它直接讀取或
搜尋記憶為止。如果 `MEMORY.md` 在那裡被反覆截斷，請將其
濃縮為較短的持久摘要，並將詳細歷史記錄移至 `memory/*.md`，
或者有意地提高啟動限制。
...
如果您希望代理聽起來不那麼通用的話，請從
[SOUL.md Personality Guide](/zh-Hant/concepts/soul) 開始。

若要檢視每個注入檔案的貢獻程度（原始內容與注入內容的對比、截斷情況，以及工具 schema 開銷），請使用 `/context list` 或 `/context detail`。請參閱 [Context](/zh-Hant/concepts/context)。

## 時間處理

當已知使用者時區時，系統提示詞會包含專門的「目前日期與時間」章節。為了保持提示詞快取穩定，它現在僅包含 **時區**（不包含動態時鐘或時間格式）。

當代理需要當前時間時，請使用 `session_status`；狀態卡片中
會包含時間戳記行。同一個工具可以選擇性地設定針對該工作階段的模型
覆寫（`model=default` 可將其清除）。
...
提示會指示模型使用 `read` 來載入所列位置（工作區、受管理或捆綁）的 SKILL.md。如果沒有合格的技能，則會省略技能區塊。

原生 Codex 輪次會將此清單作為輪次範圍的協作開發者指示接收，而不是每輪次的使用者輸入，但保留確切排程提示的輕量級 cron 輪次除外。其他鞍座則保留正常的提示區段。

該位置可以指向巢狀技能，例如 `skills/personal/foo/SKILL.md`。巢狀結構僅用於組織；提示仍使用來自 `SKILL.md` frontmatter 的扁平技能名稱。

資格條件包括技能中繼資料閘道、執行時環境/組態檢查，以及當設定 `agents.defaults.skills` 或 `agents.list[].skills` 時的有效代理程式技能允許清單。

外掛程式捆綁的技能僅在其擁有的外掛程式啟用時才符合資格。這讓工具外掛程式能夠公開更深入的操作指南，而無需將所有指南直接嵌入每個工具描述中。
...
這種區分將技能大小與執行時讀取/注入大小（例如 `memory_get`、即時工具結果和壓縮後的 AGENTS.md 重新整理）分開。

## 文件

系統提示詞包含一個 **文件** 區段。當有本機文件可用時，它會指向本機 OpenClaw 文件目錄（Git 檢出中的 `docs/` 或隨附的 npm 套件文件）。如果本機文件不可用，它會退而求其次使用 [https://docs.openclaw.ai](https://docs.openclaw.ai)。

同一個區段也包含 OpenClaw 的來源位置。Git 檢出會公開本機來源根目錄，以便代理可以直接檢查程式碼。套件安裝則包含 GitHub 來源 URL，並告訴代理在文件不完整或過期時到那裡檢視來源。提示詞還會註明公開文件鏡像、社群 Discord 和 ClawHub（[https://clawhub.ai](https://clawhub.ai）），以便探索技能。它告訴模型優先查閱文件以了解 OpenClaw 行為、指令、組態或架構，並在可能時自行執行 `openclaw status`（僅在沒有存取權時才詢問使用者）。對於組態，它會特別指引代理使用 `gateway` 工具動作 `config.schema.lookup` 以取得確切的欄位層級文件和限制，然後參考 `docs/gateway/configuration.md` 和 `docs/gateway/configuration-reference.md` 以獲得更廣泛的指引。
