...
- 嵌入式執行器注意事項：
  - 當您變更 message-tool 探索輸入或壓縮執行時間上下文時，
    請保持兩個層級的覆蓋率。
  ...
  - 這些套件會驗證範圍 ID 和壓縮行為仍然流經
    真實的 `run.ts` / `compact.ts` 路徑；僅輔助測試並非
    這些整合路徑的充分替代方案。
- 集區注意事項：
...
- 完整的 Android 設定詳情：[Android App](/en/platforms/android)
...
- Gemini API vs Gemini CLI：
  ...
  - CLI：OpenClaw 呼叫本機 `gemini` 二進位檔；它有自己的驗證方式，且可能表現不同 (串流/工具支援/版本差異)。
...
Skills 目前仍缺少的部分 (請參閱 [Skills](/en/tools/skills))：
