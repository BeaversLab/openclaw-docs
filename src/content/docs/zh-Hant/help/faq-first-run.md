...
<Accordion title="How do I open the dashboard after onboarding?">精靈會在 onboarding 完成後立即使用乾淨（非 token 化）的儀表板 URL 開啟您的瀏覽器，並且也會在摘要中列印連結。請保持該分頁開啟；如果它沒有啟動，請在同一台機器上複製貼上列印出的 URL。</Accordion>
  ...
  <Accordion title="它可以在 Raspberry Pi 上執行嗎？">
    可以。Gateway 是輕量級的 —— 文件列出的 **512MB-1GB RAM**、**1 核心** 和約 **500MB** 磁碟空間已足夠個人使用，請注意 **Raspberry Pi 4 可以執行它**。

    如果您想要額外的空間（日誌、媒體、其他服務），建議使用 **2GB**，但這並非硬性最低要求。

    提示：一台小型的 Raspberry Pi/VPS 可以託管 Gateway，您可以將您的筆記型電腦/手機上的 **節點** 連接起來，用於本機螢幕/相機/畫布或指令執行。請參閱 [節點](/zh-Hant/nodes)。
...
<Accordion title="我可以同時執行一個「快速聊天」代理程式和一個「Opus 程式碼編寫」代理程式嗎？">可以。使用多代理程式路由：為每個代理程式指定其預設模型，然後將傳入路由（供應商帳號或特定 peer）綁定到各個代理程式。範例設定檔位於 [Multi-Agent Routing](/zh-Hant/concepts/multi-agent)。另請參閱 [Models](/zh-Hant/concepts/models) 與 [Configuration](/zh-Hant/gateway/configuration)。</Accordion>
  ...
  <Accordion title="在專用機器上執行 OpenClaw 有多重要？">
    並非必需，但為了**可靠性和隔離性**建議如此。

    - **專用主機 (VPS/Mac mini/Raspberry Pi)：** 永遠線上，較少睡眠/重新啟動中斷，權限更乾淨，更容易保持執行。
    - **共用的筆記型電腦/桌面：** 對於測試和主動使用完全沒問題，但請預期在機器睡眠或更新時會暫停。

    如果您想要兩全其美，請將 Gateway 保留在專用主機上，並將您的筆記型電腦作為 **節點** 連接，以用於本機螢幕/相機/執行工具。請參閱 [節點](/zh-Hant/nodes)。
