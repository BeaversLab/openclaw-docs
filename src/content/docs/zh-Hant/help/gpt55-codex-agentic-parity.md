...
- 針對 Claude Opus 4.6 的同等性聲明是基於軼事，而非可重現的場景
...
此版本增加了第一波 QA 實驗室對等套件，以便 GPT-5.5 和 Opus 4.6 可以通過相同的場景進行測試，並使用共享的證據進行比較。
...
- PR D 透過 QA-lab 證明相同場景下 GPT-5.5 與 Opus 4.6 的行為對比
...
| 在約定的指標上，GPT-5.5 匹配或擊敗 Opus 4.6  | PR D        | `qa-agentic-parity-report.md` 和 `qa-agentic-parity-summary.json` | 具備相同的場景覆蓋率，且在完成度、停止行為或有效工具使用方面無回退 |

## 如何解讀對等性判定

將 `qa-agentic-parity-summary.json` 中的判定作為第一波對等性套件的最终機器可讀決策。

- `pass` 表示 GPT-5.5 涵蓋了與 Opus 4.6 相同的場景，且在約定的總體指標上未出現回退。
