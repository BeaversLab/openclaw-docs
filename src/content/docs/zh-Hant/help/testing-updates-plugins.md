...
- 外掛程式的 npm 相依性會安裝在每個外掛程式專屬的受管理 npm 專案中，在信任之前進行掃描，並在解除安裝期間透過 npm 移除，以免提升的相依性殘留。
...
- Registry/套件來源行為：`test:docker:plugins` fixture 或 ClawHub
  fixture 伺服器。
- 相依性配置或清理行為：同時斷言執行時期執行與檔案系統邊界。npm 相依性可能會在該外掛程式的受管理 npm 專案內被提升，因此測試應證明該專案已被掃描/清理，而非僅假設外掛程式套件本地的 `node_modules` 樹狀結構。
