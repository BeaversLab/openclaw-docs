...
有關代理、工具、Gateway 執行時和其他頂層配置鍵，請參閱
...
- 帶有 `type: "acp"` 的頂層 `bindings[]` 條目可為論壇主題設定持續性的 ACP 綁定（請在 `match.peer.id` 中使用標準 `chatId:topic:topicId`）。欄位語義請參閱 [ACP Agents](/zh-Hant/tools/acp-agents#persistent-channel-bindings)。
- Telegram 串流預覽使用 `sendMessage` + `editMessageText`（適用於直接與群組聊天）。
- 重試策略：請參閱 [Retry policy](/zh-Hant/concepts/retry)。
...
- 帶有 `type: "acp"` 的頂層 `bindings[]` 條目可為頻道和執行緒設定持續性的 ACP 綁定（請在 `match.peer.id` 中使用頻道/執行緒 ID）。欄位語義請參閱 [ACP Agents](/zh-Hant/tools/acp-agents#persistent-channel-bindings)。
...
- `unfurlLinks` 和 `unfurlMedia` 會將 Slack 的 `chat.postMessage` 連結和媒體展開 (unfurl) 布林值傳遞給機器人回覆。省略它們以保持 Slack 的預設行為；在 `channels.slack.accounts.<accountId>` 設定它們，以覆寫單一帳戶的頂層預設值。
- 使用 `user:<id>` (DM) 或 `channel:<id>` 作為傳遞目標。

**反應通知模式：** `off`、`own` (預設)、`all`、`allowlist` (來自 `reactionAllowlist`)。

**執行緒會話隔離：** `thread.historyScope` 是每個執行緒獨立 (預設) 或在整個頻道中共享。`thread.inheritParent` 會將父頻道的逐字稿複製到新執行緒。

- Slack 原生串流以及 Slack 助理風格的「正在輸入...」執行緒狀態需要一個回覆執行緒目標。頂層 DM 預設保持在執行緒之外，以便它們仍然可以透過 Slack 草稿的發布與編輯預覽進行串流，而不是顯示執行緒風格的原生串流/狀態預覽。
- `typingReaction` 會在回覆執行時，對傳入的 Slack 訊息新增一個暫時性的反應，然後在完成時將其移除。請使用 Slack emoji 代碼，例如 `"hourglass_flowing_sand"`。
- `channels.slack.execApprovals`: Slack 原生執行核准傳遞與核准者授權。架構與 Discord 相同：`enabled` (`true`/`false`/`"auto"`)，`approvers` (Slack 使用者 ID)，`agentFilter`，`sessionFilter`，以及 `target` (`"dm"`、`"channel"` 或 `"both"`)。
...
Mattermost 在目前的 OpenClaw 版本中作為內建外掛隨附。較舊或自訂建構版本可以使用 `openclaw plugins install @openclaw/mattermost` 安裝目前的 npm 套件。在鎖定版本之前，請查看 [npmjs.com/package/@openclaw/mattermost](https://www.npmjs.com/package/@openclaw/mattermost) 以取得目前的 dist-tags。
...
聊天模式：`oncall` (回覆 @-提及，預設)、`onmessage` (每則訊息)、`onchar` (以觸發前綴開頭的訊息)。
...
- 原生斜線回呼會使用 Mattermost 在斜線命令註冊期間傳回的各命令權杖進行驗證。如果註冊失敗或未啟用任何命令，OpenClaw 會以 `Unauthorized: invalid command token.` 拒絕回呼。
- 對於私人/tailnet/內部回呼主機，Mattermost 可能需要 `ServiceSettings.AllowedUntrustedInternalConnections` 包含回呼主機/網域。請使用主機/網域值，而非完整的 URL。
- `channels.mattermost.configWrites`: 允許或拒絕 Mattermost 發起的設定寫入。
- `channels.mattermost.requireMention`: 在頻道中回覆前需要 `@mention`。
- `channels.mattermost.groups.<channelId>.requireMention`：逐通道提及閘門覆寫（預設為 `"*"`）。
- 當符合已設定的帳戶 ID 時，可選的 `channels.mattermost.defaultAccount` 會覆寫預設帳戶選擇。
...
- `channels.signal.account`：將通道啟動釘選至特定的 Signal 帳戶身分。
- `channels.signal.configWrites`：允許或拒絕 Signal 發起的設定寫入。
- 當符合已設定的帳戶 ID 時，可選的 `channels.signal.defaultAccount` 會覆寫預設帳戶選擇。

### iMessage

OpenClaw 會產生 `imsg rpc`（透過 stdio 的 JSON-RPC）。不需要常駐程式或連接埠。這是主機可以授予訊息資料庫和自動化權限時，新 OpenClaw iMessage 設定的首選路徑。

已移除 BlueBubbles 支援。`channels.bluebubbles` 在目前的 OpenClaw 上不是支援的執行時段設定介面。將舊設定遷移至 `channels.imessage`；使用 [BlueBubbles 移除與 imsg iMessage 路徑](/zh-Hant/announcements/bluebubbles-imessage) 查看簡短版本，並使用 [來自 BlueBubbles](/zh-Hant/channels/imessage-from-bluebubbles) 查看完整對照表。

如果 Gateway 並非在已登入訊息的 Mac 上執行，請保留 `channels.imessage.enabled=true` 並將 `channels.imessage.cliPath` 設定為 SSH 包裝器，該包裝器會在該 Mac 上執行 `imsg "$@"`。預設的本地 `imsg` 路徑僅限 macOS。
...
- 當符合已設定的帳戶 ID 時，可選的 `channels.imessage.defaultAccount` 會覆寫預設帳戶選擇。
...
- `channels.imessage.actions.*`：啟用同樣受 `imsg status` / `openclaw channels status --probe` 限制的私人 API 操作。
- `channels.imessage.includeAttachments` 預設為關閉；在預期 Agent 輪次中有傳入媒體之前，請將其設為 `true`。
- `channels.imessage.catchup.enabled`：選擇重播當 Gateway 關閉時抵達的傳入訊息。
- `channels.imessage.groups`：群組註冊表及各群組設定。使用 `groupPolicy: "allowlist"` 時，請設定明確的 `chat_id` 金鑰或 `"*"` 萬用字元條目，以便群組訊息能通過註冊表閘門。
- 頂層具有 `type: "acp"` 的 `bindings[]` 條目可將 iMessage 對話綁定至持久的 ACP 會話。請在 `match.peer.id` 中使用標準化的帳號識別碼或明確的聊天目標（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共用欄位語意：[ACP Agents](/zh-Hant/tools/acp-agents#persistent-channel-bindings)。
...
- `channels.matrix.proxy` 將 Matrix HTTP 流量路由透過明確的 HTTP(S) 代理伺服器。具名帳號可以使用 `channels.matrix.accounts.<id>.proxy` 覆寫此設定。
- `channels.matrix.network.dangerouslyAllowPrivateNetwork` 允許私人/內部家庭伺服器。`proxy` 和此網路選用設定是獨立的控制項。
- `channels.matrix.defaultAccount` 在多重帳號設定中選擇偏好的帳號。
- `channels.matrix.autoJoin` 預設為 `off`，因此被邀請的房間和新的直訊邀請會被忽略，直到您使用 `autoJoinAllowlist` 或 `autoJoin: "always"` 設定 `autoJoin: "allowlist"`。
- `channels.matrix.execApprovals`：Matrix 原生的 exec 執行核准傳遞與核准者授權。
  ...
  - `approvers`：獲准核准 exec 請求的 Matrix 使用者 ID（例如 `@owner:example.org`）。
...
- `channels.matrix.dm.sessionScope` 控制直訊如何歸組為工作階段：`per-user`（預設）依路由對等方共用，而 `per-room` 則隔離每個直訊房間。
...
- 完整的 Teams 設定（憑證、Webhook、直訊/群組原則、每團隊/每頻道覆寫）記載於 [Microsoft Teams](/zh-Hant/channels/msteams)。
...
- 可選的 `channels.irc.defaultAccount` 在與設定的帳戶 ID 匹配時，會覆蓋預設的帳戶選擇。
- 完整的 IRC 頻道配置（host/port/TLS/channels/allowlists/mention gating）記錄於 [IRC](/zh-Hant/channels/irc)。
...
- Env 權杖僅適用於 **預設** 帳戶。
- 基礎頻道設定適用於所有帳戶，除非每個帳戶另有覆寫。
- 使用 `bindings[].match.accountId` 將每個帳戶路由到不同的 agent。
- 如果您在仍使用單一帳戶頂層頻道配置時，透過 `openclaw channels add`（或頻道 onboarding）新增非預設帳戶，OpenClaw 會先將帳戶範圍的頂層單一帳戶值提升至頻道帳戶對應，以便原始帳戶繼續運作。大多數頻道會將其移至 `channels.<channel>.accounts.default`；Matrix 則可以保留現有的相符命名/預設目標。
- 現有的僅頻道綁定（無 `accountId`）會繼續符合預設帳戶；帳戶範圍的綁定保持可選。
- `openclaw doctor --fix` 也會透過將帳戶範圍的頂層單一帳戶值移至為該頻道選擇的提升帳戶來修復混合形狀。大多數頻道使用 `accounts.default`；Matrix 則可以保留現有的相符命名/預設目標。

### 其他外掛頻道

許多外掛頻道被配置為 `channels.<id>` 並記錄在其專用頻道頁面上（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
請參閱完整的頻道索引：[頻道](/zh-Hant/channels)。

### 群組聊天提及門控

群組訊息預設為 **需要提及**（metadata mention 或安全的 regex 模式）。適用於 WhatsApp、Telegram、Discord、Google Chat 和 iMessage 群組聊天。

可見的回覆是單獨控制的。群組/頻道聊天室預設為 `messages.groupChat.visibleReplies: "message_tool"`：OpenClaw 仍然會處理該輪次，但正常的最終回覆保持私密，若要讓可見的聊天室輸出則需要 `message(action=send)`。僅當您想要正常回覆被發送回聊天室的舊版行為時，才設定 `"automatic"`。若要將相同的「僅工具」可見回覆行為也套用至直接聊天，請設定 `messages.visibleReplies: "message_tool"`；Codex harness 也將該僅工具行為作為其未設定的直接聊天預設值。

僅工具的可見回覆需要一個能可靠呼叫工具的模型/執行時。如果會話日誌顯示助手的文字附帶 `didSendViaMessagingTool: false`，這表示模型產生了私有的最終答案，而不是呼叫訊息工具。請切換到該頻道使用更強大的工具呼叫模型，或者設定 `messages.groupChat.visibleReplies: "automatic"` 以還原舊版可見的最終回覆。

如果在現行工具原則下無法使用訊息工具，OpenClaw 將退回到自動可見回覆，而不是靜默抑制回應。`openclaw doctor` 會針對此不匹配發出警告。

檔案儲存後，閘道會熱重新載入 `messages` 設定。僅當在部署中停用了檔案監看或設定重新載入時，才需要重新啟動。

**提及類型：**

- **中繼資料提及**：原生平台的 @ 提及。在 WhatsApp 自我聊天模式下會被忽略。
- **文字模式**：`agents.list[].groupChat.mentionPatterns` 中的安全正則表達式模式。無效的模式和不安全的巢狀重複會被忽略。
- 僅在可以進行偵測時（原生提及或至少一種模式）才會執行提及閘門。
...
`messages.groupChat.historyLimit` 設定了全域預設值。頻道可以使用 `channels.<channel>.historyLimit`（或每個帳戶）進行覆寫。設定 `0` 以停用。

`messages.visibleReplies` 是全域來源輪次的預設值；`messages.groupChat.visibleReplies` 會針對群組/頻道來源輪次覆寫它。當未設定 `messages.visibleReplies` 時，harness 可以提供其自己的直接/來源預設值；Codex harness 預設為 `message_tool`。頻道允許清單和提及閘門仍然決定是否處理輪次。
...
解析：個別私訊覆寫 → 提供者預設 → 無限制（全部保留）。

支援：`telegram`、`whatsapp`、`discord`、`slack`、`signal`、`imessage`、`msteams`。

#### 自訊模式

將您自己的號碼包含在 `allowFrom` 中以啟用自訊模式（忽略原生的 @-提及，僅回應文字模式）：
...
- 此區塊用於配置指令介面。若要查看目前內建 + 捆綁的指令目錄，請參閱 [斜線指令](/zh-Hant/tools/slash-commands)。
- 此頁面為 **配置鍵值參考**，而非完整的指令目錄。頻道/外掛擁有的指令，例如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、裝置配對 `/pair`、記憶體 `/dreaming`、手機控制 `/phone` 以及 Talk `/voice`，均記載於其各自的頻道/外掛頁面以及 [斜線指令](/zh-Hant/tools/slash-commands) 中。
- 文字指令必須是以 `/` 開頭的 **獨立** 訊息。
- `native: "auto"` 會為 Discord/Telegram 開啟原生指令，而 Slack 則保持關閉。
- `nativeSkills: "auto"` 會為 Discord/Telegram 開啟原生技能指令，而 Slack 則保持關閉。
- 依頻道覆寫：`channels.discord.commands.native` (布林值或 `"auto"`)。對於 Discord，`false` 會在啟動期間跳過原生指令註冊與清理。
...
- `config: true` 會啟用 `/config` (讀取/寫入 `openclaw.json`)。對於 Gateway `chat.send` 客戶端，持續性 `/config set|unset` 寫入也需要 `operator.admin`；唯讀 `/config show` 對一般具寫入權限的操作員客戶端仍然可用。
- `mcp: true` 會為 `mcp.servers` 下的 OpenClaw 管理 MCP 伺服器配置啟用 `/mcp`。
- `plugins: true` 會啟用 `/plugins` 以用於外掛探索、安裝及啟用/停用控制。
- `channels.<provider>.configWrites` 會依頻道限制配置變更 (預設：true)。
- 對於多重帳號頻道，`channels.<provider>.accounts.<id>.configWrites` 也會限制以該帳號為目標的寫入 (例如 `/allowlist --config --account <id>` 或 `/config set channels.<provider>.accounts.<id>...`)。
...
- `allowFrom` 是依提供者而定。設定後，它將是 **唯一** 的授權來源 (頻道允許清單/配對與 `useAccessGroups` 將被忽略)。
- 當未設定 `allowFrom` 時，`useAccessGroups: false` 允許指令繞過存取群組原則。
- 指令文件地圖：
  - 內建 + 捆綁目錄：[斜線指令](/zh-Hant/tools/slash-commands)
  - 特定頻道指令介面：[頻道](/zh-Hant/channels)
