...
有關代理程式、工具、Gateway 執行階段和其他頂層金鑰，請參閱
...
- 具有 `type: "acp"` 的頂層 `bindings[]` 項目可設定論壇主題的持久 ACP 繫結（在 `match.peer.id` 中使用正準 `chatId:topic:topicId`）。欄位語義共用於 [ACP Agents](/zh-Hant/tools/acp-agents#persistent-channel-bindings)。
- Telegram 串流預覽使用 `sendMessage` + `editMessageText`（適用於直接與群組聊天）。
- 重試原則：請參閱 [Retry policy](/zh-Hant/concepts/retry)。
...
- 支援機器人撰寫入站訊息的頻道可以使用共用的 [bot loop protection](/zh-Hant/channels/bot-loop-protection)。設定 `channels.defaults.botLoopProtection` 作為基線配對預算，然後僅在單一介面需要不同限制時覆寫頻道或帳戶。
...
Mattermost 在目前的 OpenClaw 版本中作為內建外掛程式隨附。較舊或自訂的組建可以使用 `openclaw plugins install @openclaw/mattermost` 安裝目前的 npm 套件。在鎖定版本之前，請檢查 [npmjs.com/package/@openclaw/mattermost](https://www.npmjs.com/package/@openclaw/mattermost) 以取得目前的 dist-tags。
...
BlueBubbles 支援已被移除。`channels.bluebubbles` 在目前的 OpenClaw 上不是受支援的執行時配置介面。將舊的配置遷移到 `channels.imessage`；使用 [BlueBubbles 移除和 imsg iMessage 路徑](/zh-Hant/announcements/bluebubbles-imessage) 查看簡短版本，並使用 [來自 BlueBubbles](/zh-Hant/channels/imessage-from-bluebubbles) 查看完整的對照表。
...
- 頂層 `bindings[]` 項目搭配 `type: "acp"``chat_id:*` 可將 iMessage 對話綁定至持續的 ACP 會話。在 `match.peer.id` 中使用標準化的 handle 或明確的聊天目標 (%%PH:INLINE_CODE:298:7f2194b%%、`chat_guid:*`、`chat_identifier:*`)。共享欄位語義：[ACP Agents](/zh-Hant/tools/acp-agents#persistent-channel-bindings)。
...
- 完整的 Matrix 配置、目標規則和設定範例記載於 [Matrix](/zh-Hant/channels/matrix)。
...
- 完整的 Teams 配置（憑證、Webhook、DM/群組政策、每團隊/每頻道覆寫）記載於 [Microsoft Teams](/zh-Hant/channels/msteams)。
...
許多外掛頻道設定為 `channels.<id>` 並在其專屬的頻道頁面中記錄（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
請查看完整的頻道索引：[Channels](/zh-Hant/channels)。

### 群組提及閘控

群組訊息預設為 **要求提及**（metadata mention 或安全的 regex 模式）。適用於 WhatsApp、Telegram、Discord、Google Chat 和 iMessage 群組聊天。

可見回覆受到單獨控制。一般的群組和頻道請求預設為自動最終傳遞：最終的助理文字會透過舊版可見回覆路徑發布。某些 harness，包括 Codex，將直接/來源聊天預設為 message-tool 傳遞，因此只有在代理程式呼叫 `message(action=send)` 後才會發布可見輸出。如果模型返回最終文字但未呼叫訊息工具，該最終文字將保持私密，並且 gateway 詳細日誌會記錄被隱藏的 payload 元資料。

僅限工具的可見回覆需要能可靠呼叫工具的模型/執行時環境，建議用於最新代模型（如 GPT 5.5）的共享常駐房間。如果
工作階段日誌顯示助理文字帶有 `didSendViaMessagingTool: false`，
表示模型產生了私密的最終文字，而非呼叫訊息工具。請切換
至該頻道更強大的工具呼叫模型，檢查 gateway 詳細
日誌中的被隱藏 payload 摘要，或設定
`messages.groupChat.visibleReplies: "automatic"` 以對每個
群組/頻道請求使用可見的最終回覆。

如果在現行的工具原則下無法使用訊息工具，OpenClaw 將改回退到自動可見回覆，而不是靜靜地隱藏回應。`openclaw doctor` 會針對此不符之處發出警告。

**疑難排解：群組 @mention 觸發輸入後隨即沈默（無錯誤）**

症狀：群組/頻道 @提及顯示輸入指示器，且 gateway 日誌回報 `dispatch complete (queuedFinal=false, replies=0)`，但房間內沒有訊息出現。傳送給同一個代理的 DM 則正常回覆。

原因：群組/頻道的可見回覆模式解析為 `"message_tool"`，因此 OpenClaw 會執行該輪次但隱藏最終的助手文字，除非代理呼叫 `message(action=send)`。這不會產生錯誤，因為隱藏是設定好的行為。一般的群組和頻道輪次預設為 `"automatic"`，因此只有在 `messages.groupChat.visibleReplies`（或全域 `messages.visibleReplies`）被明確設定為 `"message_tool"` 時，才會出現此症狀。Harness `defaultVisibleReplies` 在此不適用 —— 群組/頻道解析器會忽略它；它只會影響直接/來源聊天（Codex harness 會以該方式隱藏直接聊天的最終訊息）。

修正方法：選擇一個更強的 tool-calling 模型、移除明確的 `"message_tool"` 覆蓋以還原為 `"automatic"` 預設值，或是設定 `messages.groupChat.visibleReplies: "automatic"` 以強制每個群組/頻道請求都顯示回覆。在檔案儲存後，gateway 會熱重新載入 `messages` 設定；僅當部署中停用檔案監看或設定重新載入時，才需要重新啟動 gateway。
...
`messages.groupChat.historyLimit` 設定全域預設值。頻道可以使用 `channels.<channel>.historyLimit`（或每個帳戶）進行覆蓋。設定 `0` 以停用。

`messages.groupChat.unmentionedInbound: "room_event"` 將未被提及的 always-on 群組/頻道訊息作為安靜的房間上下文提交至支援的頻道。被提及的訊息、指令和直接訊息則維持為使用者請求。如需完整的 Discord、Slack 和 Telegram 範例，請參閱 [Ambient room events](/zh-Hant/channels/ambient-room-events)。

`messages.visibleReplies` 是全域來源事件預設值；`messages.groupChat.visibleReplies` 會針對群組/頻道來源事件覆寫它。當未設定 `messages.visibleReplies` 時，直訊/來源聊天會使用選定的 runtime 或 harness 預設值。Codex harness 預設將直訊/來源聊天設為透過 message-tool 傳遞；請設定 `messages.visibleReplies: "automatic"` 以使用自動最終傳遞。頻道允許清單和提及閘門仍然決定事件是否被處理。
...
- 此區塊配置指令介面。若要查看目前的內建 + 捆綁指令目錄，請參閱 [Slash Commands](/zh-Hant/tools/slash-commands)。
- 此頁面是 **config-key 參考資料**，而非完整的指令目錄。頻道/外掛擁有的指令，例如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、device-pair `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice`，皆記載於其各自的頻道/外掛頁面以及 [Slash Commands](/zh-Hant/tools/slash-commands) 中。
- 文字指令必須是**獨立**訊息，且以 `/` 開頭。
- `native: "auto"` 會為 Discord/Telegram 開啟原生指令，保持 Slack 為關閉狀態。
- `nativeSkills: "auto"` 會為 Discord/Telegram 開啟原生技能指令，保持 Slack 為關閉狀態。
- 依頻道覆寫：`channels.discord.commands.native` (布林值或 `"auto"`)。對於 Discord，`false` 會跳過啟動期間的原生指令註冊和清理作業。
- 使用 `channels.<provider>.commands.nativeSkills` 依頻道覆寫原生技能註冊。
- `channels.telegram.customCommands` 會新增額外的 Telegram 機器人選單項目。
- `bash: true` 會為主機 Shell 啟用 `! <cmd>`。需要 `tools.elevated.enabled` 以及位於 `tools.elevated.allowFrom.<channel>` 中的發送者。
- `config: true` 會啟用 `/config` (讀取/寫入 `openclaw.json`)。對於 gateway `chat.send` 用戶端，持續性 `/config set|unset` 寫入也需要 `operator.admin`；唯讀 `/config show` 則維持供一般寫入範圍的操作員用戶端使用。
- `mcp: true` 會為 `mcp.servers` 下的 OpenClaw 管理 MCP 伺服器設定啟用 `/mcp`。
- `plugins: true` 會為外掛探索、安裝和啟用/停用控制啟用 `/plugins`。
- `channels.<provider>.configWrites` 會依頻道限制設定變更 (預設值：true)。
