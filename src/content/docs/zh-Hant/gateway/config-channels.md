...
有關代理、工具、Gateway 執行時和其他頂層設定鍵，請參閱
...
- 具有 `type: "acp"` 的頂層 `bindings[]` 項目會為論壇主題設定持續的 ACP 繫結（在 `match.peer.id` 中使用標準 `chatId:topic:topicId`）。欄位語義共用於 [ACP Agents](/zh-Hant/tools/acp-agents#persistent-channel-bindings)。
...
- 支援機器人發出傳入訊息的頻道可以使用共用的 [bot loop protection](/zh-Hant/channels/bot-loop-protection)。設定 `channels.defaults.botLoopProtection` 作為基準配對預算，然後僅在某一介面需要不同限制時覆寫該頻道或帳戶。
...
- 具有 `type: "acp"` 的頂層 `bindings[]` 項目會為頻道和執行緒設定持續的 ACP 繫結（在 `match.peer.id` 中使用頻道/執行緒 ID）。欄位語義共用於 [ACP Agents](/zh-Hant/tools/acp-agents#persistent-channel-bindings)。
...
Mattermost 在目前的 OpenClaw 版本中作為內附插件發佈。較舊或自訂的建構版本可以使用
`openclaw plugins install @openclaw/mattermost` 安裝目前的 npm 套件。在鎖定版本之前，請檢查
...
BlueBubbles 支援已被移除。`channels.bluebubbles` 在目前的 OpenClaw 上不是支援的執行時設定介面。請將舊的設定遷移至 `channels.imessage`；簡短版本請使用 [BlueBubbles removal and the imsg iMessage path](/zh-Hant/announcements/bluebubbles-imessage)，完整對照表請使用 [Coming from BlueBubbles](/zh-Hant/channels/imessage-from-bluebubbles)。
...
- 具有 `type: "acp"``chat_id:*` 的頂層 `bindings[]` 項目可以將 iMessage 對話繫結至持續的 ACP 工作階段。在 `match.peer.id` 中使用正規化的 handle 或明確的聊天目標（%%PH:INLINE_CODE:292:7f2194b%%、`chat_guid:*`、`chat_identifier:*`）。共用的欄位語義：[ACP Agents](/zh-Hant/tools/acp-agents#persistent-channel-bindings)。
...
- 完整的 Matrix 設定、目標規則與設定範例已記錄於 [Matrix](/zh-Hant/channels/matrix)。
...
- 完整的 Teams 設定（憑證、webhook、DM/群組原則、各團隊/各頻道覆寫）已記錄於 [Microsoft Teams](/zh-Hant/channels/msteams)。
...
- 完整的 IRC 頻道設定（主機/連接埠/TLS/頻道/允許清單/提及閘門）已記錄於 [IRC](/zh-Hant/channels/irc)。
...
許多外掛頻道設定為 `channels.<id>` 並記錄於其專屬頻道頁面（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 與 Twitch）。
...
可見回覆是分開控制的。一般群組與頻道請求預設為自動最終傳送：最終助理文字會透過舊版可見回覆路徑發布。部分工具架（包含 Codex）將直接/來源聊天預設為訊息工具傳送，因此只有在代理程式呼叫 `message(action=send)` 後才會發布可見輸出。若模型在未呼叫訊息工具的情況下回傳最終文字，該最終文字將保持私密，且 gateway 詳細記錄會記錄已隱藏的 Payload 中繼資料。

僅工具可見回覆需要能可靠呼叫工具的模型/執行時，建議用於最新世代模型（如 GPT 5.5）的共享常駐空間。如果
工作階段記錄顯示帶有 `didSendViaMessagingTool: false` 的助理文字，表示
模型產生了私密的最終文字而非呼叫訊息工具。請切換
至該頻道更強的工具呼叫模型、檢查 gateway 詳細
記錄中的已隱藏 Payload 摘要，或設定
`messages.groupChat.visibleReplies: "automatic"` 以對每個
群組/頻道請求使用可見的最終回覆。

如果在啟用的工具原則下無法使用訊息工具，OpenClaw 會改回自動可見回覆，而非靜靜隱藏回應。`openclaw doctor` 會針對此不符處發出警告。

**疑難排解：群組 @提及 觸發輸入中然後靜默無聲（無錯誤）**

症狀：群組/頻道 @提及 顯示輸入中指示器，且 gateway 記錄回報 `dispatch complete (queuedFinal=false, replies=0)`，但房間內沒有出現任何訊息。傳送給同一代理程式的 DM 則正常回覆。

原因：群組/頻道的可見回覆模式解析為 `"message_tool"`，因此 OpenClaw 會執行該輪次，但會隱藏最終的助手指令文字，除非代理程式呼叫 `message(action=send)`。這不是錯誤，因為隱藏是設定好的行為。一般的群組和頻道輪次預設為 `"automatic"`，因此只有在 `messages.groupChat.visibleReplies`（或全域 `messages.visibleReplies`）被明確設定為 `"message_tool"` 時，才會出現此症狀。`defaultVisibleReplies` 限制機制在此不適用 — 群組/頻道解析器會忽略它；它只影響直接/來源聊天（Codex 限制機制會以這種方式隱藏直接聊天的最終回應）。

修復方法：選擇更強大的工具呼叫模型、移除明確的 `"message_tool"` 覆蓋以回復為 `"automatic"` 預設值，或設定 `messages.groupChat.visibleReplies: "automatic"` 以強制每個群組/頻道請求都顯示回覆。檔案儲存後，閘道會熱重新載入 `messages` 設定；僅當在部署中停用檔案監看或設定重新載入時，才需要重新啟動閘道。

**提及類型：**

- **中繼資料提及**：原生平台的 @-提及。在 WhatsApp 自聊模式下會被忽略。
- **文字模式**：`agents.list[].groupChat.mentionPatterns` 中的安全正則表示式模式。無效的模式和不安全的巢狀重複會被忽略。
- 僅在能夠進行偵測時（原生提及或至少一種模式），才會強制執行提及閘門。
...
`messages.groupChat.unmentionedInbound: "room_event"` 會將未提及的常開群組/頻道訊息作為安靜的房間上下文提交到支援的頻道。被提及的訊息、指令和直接訊息仍保留為使用者請求。完整的 Discord、Slack 和 Telegram 範例，請參閱 [環境房間事件](/zh-Hant/channels/ambient-room-events)。

`messages.visibleReplies` 是全域來源事件預設值；`messages.groupChat.visibleReplies` 會覆寫群組/頻道來源事件的設定。當未設定 `messages.visibleReplies` 時，直接/來源聊天會使用選定的 runtime 或 harness 預設值。Codex harness 預設將直接/來源聊天設為 message-tool 傳送；設定 `messages.visibleReplies: "automatic"` 以使用自動最終傳送。頻道允許清單和提及閘門仍會決定是否處理事件。
...
- 此區塊配置指令介面。若要查看目前內建 + 捆綁的指令目錄，請參閱 [Slash Commands](/zh-Hant/tools/slash-commands)。
- 此頁面是一個 **配置鍵參考**，而非完整的指令目錄。頻道/外掛擁有的指令，例如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、device-pair `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice`，皆記錄在其各自的頻道/外掛頁面以及 [Slash Commands](/zh-Hant/tools/slash-commands) 中。
- 文字指令必須是 **獨立** 訊息，並以 `/` 開頭。
- `native: "auto"` 會為 Discord/Telegram 開啟原生指令，保持 Slack 關閉。
- `nativeSkills: "auto"` 會為 Discord/Telegram 開啟原生技能指令，保持 Slack 關閉。
- 依頻道覆寫：`channels.discord.commands.native` (布林值或 `"auto"`)。對於 Discord，`false` 會在啟動期間跳過原生指令註冊和清理。
- 使用 `channels.<provider>.commands.nativeSkills` 依頻道覆寫原生技能註冊。
- `channels.telegram.customCommands` 新增額外的 Telegram 機器人選單項目。
- `bash: true` 為主機 shell 啟用 `! <cmd>`。需要 `tools.elevated.enabled` 以及發送者在 `tools.elevated.allowFrom.<channel>` 中。
- `config: true` 啟用 `/config` (讀取/寫入 `openclaw.json`)。對於 gateway `chat.send` 客戶端，持續性 `/config set|unset` 寫入也需要 `operator.admin`；唯讀 `/config show` 對一般寫入範圍的操作員客戶端保持可用。
- `mcp: true` 為 `mcp.servers` 下由 OpenClaw 管理的 MCP 伺服器配置啟用 `/mcp`。
...
- `restart: false` 停用 `/restart` 和 gateway 重新啟動工具動作。預設：`true`。
- `ownerAllowFrom` 是僅限擁有者指令/工具的明確擁有者允許清單。它與 `allowFrom` 分開。
- `ownerDisplay: "hash"` 在系統提示中雜湊擁有者 ID。設定 `ownerDisplaySecret` 以控制雜湊。
- `allowFrom` 是依提供者而定。設定後，它是 **唯一** 的授權來源 (頻道允許清單/配對和 `useAccessGroups` 會被忽略)。
...
- [配置參考](/zh-Hant/gateway/configuration-reference) — 頂層鍵
- [配置 — agents](/zh-Hant/gateway/config-agents)
