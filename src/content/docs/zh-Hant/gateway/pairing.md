...
當已配對的裝置重新連線時，如果僅包含非敏感的元資料變更（例如，顯示名稱或客戶端平台提示），OpenClaw 會將其視為 `metadata-upgrade` 並自動核准重新連線，而不會提示。範圍升級（讀取到寫入/管理員）和公開金鑰變更**不**符合元資料升級自動核准的資格 — 它們仍保持為明確的重新核准請求。

## QR 配對輔助程式

`/pair qr` 將配對承載呈現為結構化媒體，以便行動裝置和瀏覽器客戶端可以直接掃描。現在刪除裝置也會清除同一裝置 ID 的過期待處理配對請求，因此 `nodes pending` 在撤銷後不再顯示孤立的資料列。

## 位置與轉發標頭

只有當原始 socket 和任何上游代理證據都一致時，閘道配對才會將連線視為回送。如果請求在回送上到達，但攜帶指向非本機來源的 `X-Forwarded-For` / `X-Forwarded-Host` / `X-Forwarded-Proto` 標頭，該轉發標頭證據將使回送位置主張無效。配對路徑隨後需要明確核准，而不是將請求靜默視為同主機連線。請參閱 [Trusted Proxy Auth](/zh-Hant/gateway/trusted-proxy-auth) 以了解操作員認證的等效規則。
