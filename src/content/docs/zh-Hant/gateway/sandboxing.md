...
- 任何被明確允許在沙箱外執行的工具（例如 `tools.elevated`）。
  ...
  - 如果沙箱關閉，`tools.elevated` 不會改變執行方式（已在主機上）。請參閱 [提升模式](/zh-Hant/tools/elevated)。
...
- 請參閱 [Sandbox vs Tool Policy vs Elevated](/zh-Hant/gateway/sandbox-vs-tool-policy-vs-elevated) 以了解綁定如何與工具政策和提權執行 互動。
...
參考：[Docker](/zh-Hant/install/docker#agent-sandbox)。
...
- [OpenShell](/zh-Hant/gateway/openshell) -- 受管沙箱後端設定、工作區模式和設定參考
- [Sandbox Configuration](/zh-Hant/gateway/configuration-reference#agentsdefaultssandbox)
- [Sandbox vs Tool Policy vs Elevated](/zh-Hant/gateway/sandbox-vs-tool-policy-vs-elevated) -- 除錯「為什麼被阻擋？」
- [Multi-Agent Sandbox & Tools](/zh-Hant/tools/multi-agent-sandbox-tools) -- 各代理覆寫與優先順序
