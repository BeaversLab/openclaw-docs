...
- 任何被明確允許在沙箱外執行的工具（例如 `tools.elevated`）。
  ...
  - 如果沙箱功能關閉，`tools.elevated` 不會改變執行方式（因為已在主機上）。參閱 [提權模式](/en/tools/elevated)。
...
當您希望 OpenClaw 在任意可透過 SSH 存取的機器上，將 `exec`、檔案工具和媒體讀取放入沙箱時，請使用 `backend: "ssh"`。
...
- 之後，`exec`、`read`、`write`、`edit`、`apply_patch`、prompt 媒體讀取以及傳入媒體暫存會透過 SSH 直接針對遠端工作區執行。
- OpenClaw 不會自動將遠端變更同步回本地工作區。

驗證素材：

- `identityFile`、`certificateFile`、`knownHostsFile`：使用既有的本機檔案並透過 OpenSSH 設定傳遞。
- `identityData`、`certificateData`、`knownHostsData`：使用內聯字串或 SecretRefs。OpenClaw 會透過一般的 secrets 執行時期快照解析它們，使用 `0600` 將其寫入暫存檔，並在 SSH 會話結束時刪除它們。
- 如果同時為同一個項目設定了 `*File` 和 `*Data`，則 `*Data` 在該 SSH 會話中優先生效。
...
- `openclaw sandbox recreate` 會刪除各個範圍 (per-scope) 的遠端根目錄，並在下次使用時從本機重新植入。
- SSH 後端不支援瀏覽器沙箱隔離。
...
OpenShell 重複使用與一般 SSH 後端相同的核心 SSH 傳輸和遠端檔案系統橋接器，並加入 OpenShell 專有的生命週期
...
- `mirror` (預設值)：本機工作區保持為主要權威。OpenClaw 會在執行前將本機檔案同步至 OpenShell，並在執行後將遠端工作區同步回來。
- `remote`：建立沙箱後，OpenShell 工作區即為主要權威。OpenClaw 會從本機工作區植入遠端工作區一次，之後檔案工具和執行操作會直接針對遠端沙箱執行，而不會將變更同步回來。

遠端傳輸細節：

- OpenClaw 會透過 `openshell sandbox ssh-config <name>` 向 OpenShell 要求沙箱專屬的 SSH 設定。
- Core 將該 SSH 設定寫入暫存檔案，開啟 SSH 連線，並重複使用 `backend: "ssh"` 所使用的相同遠端檔案系統橋接器。
- 在 `mirror` 模式下，僅生命週期有所不同：在執行前將本地同步到遠端，然後在執行後同步回來。

目前 OpenShell 的限制：

- 尚不支援沙箱瀏覽器
- OpenShell 後端不支援 `sandbox.docker.binds`
- `sandbox.docker.*` 下的 Docker 專用執行時期旋鈕仍然僅適用於 Docker 後端
...
當您希望 **本地工作區保持為標準來源** 時，請使用 `plugins.entries.openshell.config.mode: "mirror"`。
...
當您希望 **OpenShell 工作區成為標準來源** 時，請使用 `plugins.entries.openshell.config.mode: "remote"`。
...
- `openclaw sandbox list` 會顯示 OpenShell 執行時期以及 Docker 執行時期
- `openclaw sandbox recreate` 會刪除目前的執行時期，並讓 OpenClaw 在下次使用時重新建立
- prune 邏輯也是後端感知的
...
- `"none"` (預設值)：工具會在 `~/.openclaw/sandboxes` 下看到一個沙箱工作區。
- `"ro"`：將代理工作區以唯讀方式掛載至 `/agent`（停用 `write`/`edit`/`apply_patch`）。
- `"rw"`：將代理工作區以讀寫方式掛載至 `/workspace`。

使用 OpenShell 後端時：

- `mirror` 模式仍然在執行週期之間使用本地工作區作為標準來源
- `remote` 模式在初始種子之後使用遠端 OpenShell 工作區作為標準來源
- `workspaceAccess: "ro"` 和 `"none"` 仍然以相同方式限制寫入行為

傳入媒體會被複製到活躍的沙箱工作區（`media/inbound/*`）。
技能注意事項：`read` 工具是以沙箱為根目錄的。使用 `workspaceAccess: "none"` 時，
OpenClaw 會將符合條件的技能鏡像到沙箱工作區（`.../skills`），以便
讀取它們。使用 `"rw"` 時，可以從 `/workspace/skills` 讀取工作區技能。
...
全域和每個代理的綁定會被**合併**（而非替換）。在 `scope: "shared"` 下，每個代理的綁定會被忽略。

`agents.defaults.sandbox.browser.binds` 將額外的主機目錄僅掛載到 **沙箱瀏覽器** 容器中。

- 設定後（包括 `[]`），它會替換瀏覽器容器的 `agents.defaults.sandbox.docker.binds`。
...
- 綁定會繞過沙箱檔案系統：它們會以您設定的任何模式（`:ro` 或 `:rw`）公開主機路徑。
- OpenClaw 會阻擋危險的綁定來源（例如：`docker.sock`、`/etc`、`/proc`、`/sys`、`/dev`，以及會公開它們的父掛載點）。
- OpenClaw 也會封鎖常見的家目錄憑證根目錄，例如 `~/.aws`、`~/.cargo`、`~/.config`、`~/.docker`、`~/.gnupg`、`~/.netrc`、`~/.npm` 和 `~/.ssh`。
- 綁定驗證不僅僅是字串匹配。OpenClaw 會對源路徑進行標準化，然後透過最深的現有祖先再次解析，再重新檢查封鎖的路徑和允許的根目錄。
- 這意味著即使最終葉節點尚不存在，透過符號連結父目錄的逃逸仍會被阻斷。例如：如果 `run-link` 指向該處，`/workspace/run-link/new-file` 仍會被解析為 `/var/run/...`。
- 允許的源根目錄也會以相同方式進行標準化，因此在解析符號連結之前僅看起來位於允許清單內的路徑，仍會被拒絕並標記為 `outside allowed roots`。
- 敏感的掛載（機密、SSH 金鑰、服務憑證）應 `:ro`，除非絕對必要。
- 如果您只需要對工作區進行讀取存取，請結合 `workspaceAccess: "ro"`；綁定模式保持獨立。
- 請參閱 [Sandbox vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated) 以了解綁定如何與工具策略和提權執行互動。
...
注意：預設映像檔**不**包含 Node。如果技能需要 Node（或其他執行環境），請製作自訂映像檔或透過 `sandbox.docker.setupCommand` 安裝（需要網路出口 + 可寫入根目錄 + root 使用者）。

如果您想要一個包含常用工具且功能更齊全的沙箱映像檔（例如 `curl`、`jq`、`nodejs`、`python3`、`git`），請建構：
...
- 當啟用 `noSandbox` 時，會使用 `--no-sandbox` 和 `--disable-setuid-sandbox`。
- 三個圖形強化旗標 (`--disable-3d-apis`,
  `--disable-software-rasterizer`, `--disable-gpu`) 是選用的，當容器缺少 GPU 支援時很有用。
...
如果您需要不同的執行時期設定檔，請使用自訂瀏覽器映像檔並提供您自己的進入點。針對本機 (非容器) Chromium 設定檔，請使用
...
對於 Docker 閘道部署，`scripts/docker/setup.sh` 可以啟動沙箱配置。
...
`setupCommand` 會在沙箱容器建立後執行**一次**（而非每次執行時）。
它透過 `sh -lc` 在容器內執行。
...
- 預設的 `docker.network` 是 `"none"`（無出口流量），因此安裝套件將會失敗。
...
- 沙箱執行**不會**繼承主機 `process.env`。請使用
  `agents.defaults.sandbox.docker.env`（或自訂映像檔）來設定技能 API 金鑰。

## 工具政策 + 逃生機制

工具允許/拒絕政策在沙箱規則之前仍然適用。如果工具在全域或針對特定代理程式被拒絕，
啟用沙箱也不會讓它恢復使用。

`tools.elevated` 是一個明確的逃生機制，會在沙箱外執行 `exec`（預設為 `gateway`，或當執行目標是 `node` 時為 `node`）。
`/exec` 指令僅適用於已授權的發送者並且每個 session 會持續有效；若要徹底停用
`exec`，請使用工具政策拒絕（請參閱 [Sandbox vs Tool Policy vs Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated)）。

除錯：

- 使用 `openclaw sandbox explain` 來檢查有效的沙箱模式、工具政策和修復配置金鑰。
...
每個代理都可以覆寫沙箱與工具：
`agents.list[].sandbox` 和 `agents.list[].tools`（加上沙箱工具策略的 `agents.list[].tools.sandbox.tools`）。
...
- [OpenShell](/en/gateway/openshell) -- 受控沙箱後端設定、工作區模式和設定參考
- [沙箱設定](/en/gateway/configuration-reference#agentsdefaultssandbox)
