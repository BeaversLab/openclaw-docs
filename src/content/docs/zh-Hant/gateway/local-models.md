...
在本地運行是可行的，但 OpenClaw 需要較大的上下文 + 對提示注入的強力防禦。較小的顯卡會截斷上下文並導致安全性洩漏。目標要高：**≥2 台滿配的 Mac Studio 或同等 GPU 設備（約 3 萬美元以上）**。單個 **24 GB** GPU 僅適合延遲較高的輕量級提示。使用你能運行的**最大/完整版模型變體**；過度量化或「小型」檢查點會增加提示注入風險（請參閱 [安全性](/zh-Hant/gateway/security)）。

如果您想要最低摩擦力的本地設置，請從 [LM Studio](/zh-Hant/providers/lmstudio) 或 [Ollama](/zh-Hant/providers/ollama) 和 `openclaw onboard` 開始。本頁面是關於高端本地堆疊和自定義 OpenAI 相容本地伺服器的觀點性指南。
...
- 一些較小或較嚴格的本地後端在處理 OpenClaw 的完整
  agent-runtime 提示形狀時不穩定，尤其是在包含工具架構時。如果
  後端適用於微小的直接 `/v1/chat/completions` 調用，但在正常的
  OpenClaw agent 輪次中失敗，請首先嘗試
  `agents.defaults.experimental.localModelLean: true` 來捨棄重型
  默認工具，例如 `browser`、`cron` 和 `message`；這是一個實驗性
  標誌，不是穩定的默認模式設置。請參閱
  [實驗性功能](/zh-Hant/concepts/experimental-features)。如果這仍然失敗，請嘗試
...
- Gateway 能夠連接到代理？`curl http://127.0.0.1:1234/v1/models`。
- LM Studio 模型是否已卸載？請重新載入；冷啟動是常見的「卡住」原因。
- 當檢測到的上下文視窗低於 **32k** 時，OpenClaw 會發出警告，低於 **16k** 時則會阻止。如果遇到此預檢，請提高伺服器/模型上下文限制或選擇更大的模型。
- 出現上下文錯誤？降低 `contextWindow` 或提高您的伺服器限制。
- OpenAI 相容伺服器返回 `messages[].content ... expected a string`？
  在該模型條目上添加 `compat.requiresStringContent: true`。
- 微小的直接 `/v1/chat/completions` 調用有效，但 `openclaw infer model run`
  在 Gemma 或其他本地模型上失敗？首先使用
  `compat.supportsTools: false` 停用工具架構，然後重新測試。如果伺服器仍然僅在較大的 OpenClaw 提示上崩潰，請將其視為上游伺服器/模型的限制。
- 安全性：本地模型會跳過供應商端的過濾器；請將代理的範圍縮小並啟用壓縮，以限制提示注入的影響範圍。
