...
在本地運行是可行的，但 OpenClaw 需要較大的上下文 + 對提示注入的強力防禦。較小的顯卡會截斷上下文並導致安全性洩漏。目標要高：**≥2 台滿配的 Mac Studio 或同等 GPU 設備（約 3 萬美元以上）**。單個 **24 GB** GPU 僅適合延遲較高的輕量級提示。使用你能運行的**最大/完整版模型變體**；過度量化或「小型」檢查點會增加提示注入風險（請參閱 [安全性](/en/gateway/security)）。

如果您想要最低摩擦力的本地設置，請從 [LM Studio](/en/providers/lmstudio) 或 [Ollama](/en/providers/ollama) 和 `openclaw onboard` 開始。本頁面是關於高端本地堆疊和自定義 OpenAI 相容本地伺服器的觀點性指南。
...
- 一些較小或較嚴格的本地後端在處理 OpenClaw 的完整
  ...
  標誌，不是穩定的默認模式設置。請參閱
  [實驗性功能](/en/concepts/experimental-features)。如果這仍然失敗，請嘗試
