...
如需完整的操作員範圍模型、審批時檢查以及共享密碼語義，請參閱[操作員範圍](/zh-Hant/gateway/operator-scopes)。
...
<AccordionGroup>
  ...
  <Accordion title="對話與 TTS">
    - `talk.catalog` 傳回用於語音、串流轉錄與即時語音的唯讀 Talk 提供者目錄。其中包含提供者 ID、標籤、已設定狀態、公開的模型/語音 ID、標準模式、傳輸方式、Brain 策略與即時音訊/能力旗標，但不傳回提供者機密或修改全域設定。
    - `talk.config` 傳回有效的 Talk 設定承載；`includeSecrets` 需要 `operator.talk.secrets`（或 `operator.admin`）。
    - `talk.session.create` 為 `realtime/gateway-relay`、`transcription/gateway-relay` 或 `stt-tts/managed-room` 建立 Gateway 擁有的 Talk 工作階段。對於 `stt-tts/managed-room`，傳遞 `sessionKey` 的 `operator.write` 呼叫端也必須傳遞 `spawnedBy` 以取得範圍工作階段金鑰的可見性；無範圍的 `sessionKey` 建立與 `brain: "direct-tools"` 需要 `operator.admin`。
    - `talk.session.join` 驗證受管理房間的工作階段 Token，視需要發出 `session.ready` 或 `session.replaced` 事件，並傳回房間/工作階段中繼資料與最近的 Talk 事件，不含明文 Token 或已儲存 Token 的雜湊。
    - `talk.session.appendAudio` 將 base64 PCM 輸入音訊附加至 Gateway 擁有的即時中繼與轉錄工作階段。
    - `talk.session.startTurn`、`talk.session.endTurn` 與 `talk.session.cancelTurn` 驅動受管理房間的輪次生命週期，並在狀態清除前拒絕過期的輪次。
    - `talk.session.cancelOutput` 停止助理音訊輸出，主要用於 Gateway 中繼工作階段中的 VAD 閘控插話。
    - `talk.session.submitToolResult` 完成 Gateway 擁有的即時中繼工作階段所發出的提供者工具呼叫。若隨後會有最終結果，則傳遞 `options: { willContinue: true }` 以取得中間工具輸出；若工具結果應滿足提供者呼叫而不啟動另一個即時助理回應，則傳遞 `options: { suppressResponse: true }`。
    - `talk.session.steer` 將執行中的語音控制傳送至 Gateway 擁有的 Agent 支援 Talk 工作階段。它接受 `{ sessionId, text, mode? }`，其中 `mode` 為 `status`、`steer`、`cancel` 或 `followup`；若省略模式，則從口述文字中分類。
    - `talk.session.close` 關閉 Gateway 擁有的中繼、轉錄或受管理房間工作階段，並發出終端 Talk 事件。
    - `talk.mode` 設定/廣播 WebChat/Control UI 用戶端的目前 Talk 模式狀態。
    - `talk.client.create` 使用 `webrtc` 或 `provider-websocket` 建立用戶端擁有的即時提供者工作階段，同時由 Gateway 擁有設定、憑證、指示與工具原則。
    - `talk.client.toolCall` 讓用戶端擁有的即時傳輸可將提供者工具呼叫轉送至 Gateway 原則。第一個支援的工具是 `openclaw_agent_consult`；用戶端會收到執行 ID，並在提交提供者特定的工具結果前等待一般聊天生命週期事件。
    - `talk.client.steer` 傳送用戶端擁有即時傳輸的執行中語音控制。Gateway 會從 `sessionKey` 解析主動內嵌執行，並傳回結構化的接受/拒絕結果，而不是靜默捨棄導引。
    - `talk.event` 是即時、轉錄、STT/TTS、受管理房間、電話與會議配接器的單一 Talk 事件通道。
    - `talk.speak` 透過主動 Talk 語音提供者合成語音。
    - `tts.status` 傳回 TTS 已啟用狀態、主動提供者、備援提供者與提供者設定狀態。
    - `tts.providers` 傳回可見的 TTS 提供者清單。
    - `tts.enable` 與 `tts.disable` 切換 TTS 偏好狀態。
    - `tts.setProvider` 更新慣用的 TTS 提供者。
    - `tts.convert` 執行一次性文字轉語音轉換。

  </Accordion>

  <Accordion title="Secrets, config, update, and wizard">
    - `secrets.reload` 重新解析有效的 SecretRefs 並僅在完全成功時交換運行時密鑰狀態。
    - `secrets.resolve` 解析特定命令/目標集的命令目標密鑰分配。
    - `config.get` 返回當前配置快照和哈希。
    - `config.set` 寫入經過驗證的配置有效載荷。
    - `config.patch` 合併部分配置更新。
    - `config.apply` 驗證並替換完整的配置有效載荷。
    - `config.schema` 返回 Control UI 和 CLI 工具使用的實時配置架構有效載荷：架構、`uiHints`、版本和生成元數據，包括運行時可以加載時的外掛 + 通道架構元數據。該架構包括從 UI 使用的相同標籤和幫助文本派生的字段 `title` / `description` 元數據，包括嵌套對象、通配符、數組項，以及在存在匹配字段文檔時的 `anyOf` / `oneOf` / `allOf` 組合分支。
    - `config.schema.lookup` 返回一個配置路徑的路徑範圍查找有效載荷：規範化路徑、淺層架構節點、匹配的提示 + `hintPath`、可選的 `reloadKind`，以及用於 UI/CLI 下鑽的直接子摘要。`reloadKind` 是 `restart`、`hot` 或 `none` 之一，並反映了所請求路徑的 Gateway 配置重新載入計劃器。查找架構節點保留面向用戶的文檔和通用驗證字段（`title`、`description`、`type`、`enum`、`const`、`format`、`pattern`、數值/字串/陣列/物件界限，以及諸如 `additionalProperties`、`deprecated`、`readOnly`、`writeOnly` 之類的標誌）。子摘要公開 `key`、規範化的 `path`、`type`、`required`、`hasChildren`、可選的 `reloadKind`，加上匹配的 `hint` / `hintPath`。
    - `update.run` 執行網關更新流程，並僅在更新本身成功時安排重新啟動；具有會話的調用者可以包含 `continuationMessage`，以便啟動通過重新啟動延續佇列恢復一個後續代理週期。來自控制平面的套件管理器更新使用分離的託管服務交接，而不是替換實時 Gateway 內的套件樹。已啟動的交接返回帶有 `result.reason: "managed-service-handoff-started"` 和 `handoff.status: "started"` 的 `ok: true`；不可用或失敗的交接返回帶有 `managed-service-handoff-unavailable` 或 `managed-service-handoff-failed` 的 `ok: false`，以及需要手動 shell 更新時的 `handoff.command`。在已啟動的交接期間，重新啟動哨兵可能會簡要報告 `stats.reason: "restart-health-pending"`；延續將延遲，直到 CLI 驗證重新啟動的 Gateway 並寫入最終的 `ok` 哨兵。
    - `update.status` 返回最新的快取更新重新啟動哨兵，包括可用時重新啟動後的運行版本。
  ...
  <Accordion title="Agent and workspace helpers">
    ...
    - `tasks.list`、`tasks.get` 和 `tasks.cancel` 將 Gateway 任務帳本公開給 SDK 和操作員客戶端。
    - `artifacts.list`、`artifacts.get` 和 `artifacts.download` 公開針對特定 `sessionKey`、`runId` 或 `taskId` 範圍的轉錄衍生摘要和下載。運行和任務查詢會在服務端解析所屬會話，並僅返回具有匹配來源的轉錄媒體；不安全或本機 URL 來源會返回不支援的下載，而不是在服務端獲取。
    - `environments.list` 和 `environments.status` 將唯讀的 Gateway 本地和節點環境發現公開給 SDK 客戶端。
    - `agent.identity.get` 返回代理或會話的有效助手身分。
    - `agent.wait` 等待運行完成並在可用時返回終端快照。

  </Accordion>

  <Accordion title="Session control">
    - `sessions.list` 返回目前的工作階段索引，當設定了代理程式執行時間後端時，包含各個資料列的 `agentRuntime` 元資料。
    ...
    - `sessions.describe` 返回特定工作階段鍵值的單一 Gateway 工作階段資料列。
    - `sessions.resolve` 解析或正規化工作階段目標。
    - `sessions.create` 建立新的工作階段項目。
    - `sessions.send` 將訊息傳送至現有的工作階段。
    - `sessions.steer` 是作用中工作階段的中斷並引導變體。
    - `sessions.abort` 中止工作階段的作用中工作。呼叫者可以傳遞 `key` 加上選用的 `runId`，或是僅傳遞 `runId` 給 Gateway 能解析為工作階段的作用中執行。
    - `sessions.patch` 更新工作階段元資料/覆寫值，並回報解析的規範模型加上有效的 `agentRuntime`。
    - `sessions.reset`、`sessions.delete` 和 `sessions.compact` 執行工作階段維護。
    - `sessions.get` 返回完整的已儲存工作階段資料列。
    - 聊天執行仍使用 `chat.history`、`chat.send`、`chat.abort` 和 `chat.inject`。`chat.history` 已針對 UI 用戶端進行顯示正規化：行內指令標籤會從可見文字中移除，純文字工具呼叫 XML 載荷（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 以及截斷的工具呼叫區塊）與外洩的 ASCII/全形模型控制權杖會被移除，純靜音權杖助理資料列（例如精確的 `NO_REPLY` / `no_reply`）會被省略，而過大的資料列可以替換為佔位符。

  </Accordion>

  <Accordion title="Device pairing and device tokens">
    - `device.pair.list` 會傳回待處理及已核可的配對裝置。
    - `device.pair.approve`、`device.pair.reject` 和 `device.pair.remove` 管理裝置配對紀錄。
    - `device.token.rotate` 會在已核可的角色與呼叫者範圍界限內輪替配對裝置權杖。
    - `device.token.revoke` 會在已核可的角色與呼叫者範圍界限內撤銷配對裝置權杖。

  </Accordion>

  <Accordion title="Node pairing, invoke, and pending work">
    - `node.pair.request`、`node.pair.list`、`node.pair.approve`、`node.pair.reject`、`node.pair.remove` 和 `node.pair.verify` 涵蓋節點配對與引導驗證。
    - `node.list` 和 `node.describe` 會傳回已知/已連線的節點狀態。
    - `node.rename` 更新配對節點的標籤。
    - `node.invoke` 將指令轉發至已連線的節點。
    - `node.invoke.result` 會傳回呼叫請求的結果。
    - `node.event` 將節點發起的事件帶回到閘道中。
    - `node.pending.pull` 和 `node.pending.ack` 是已連線節點的佇列 API。
    - `node.pending.enqueue` 和 `node.pending.drain` 管理離線/未連線節點的持久待處理工作。

  </Accordion>

  <Accordion title="核准家族">
    - `exec.approval.request`、`exec.approval.get`、`exec.approval.list` 和 `exec.approval.resolve` 涵蓋一次性執行核准請求以及待處理核准的查詢/重放。
    - `exec.approval.waitDecision` 等待一個待處理的執行核准並傳回最終決策（或在逾時時傳回 `null`）。
    - `exec.approvals.get` 和 `exec.approvals.set` 管理閘道執行核准策略快照。
    - `exec.approvals.node.get` 和 `exec.approvals.node.set` 透過節點中繼指令管理節點本地的執行核准策略。
  ...
  <Accordion title="自動化、技能與工具">
    - 自動化：`wake` 排程立即或下一次心跳的喚醒文字注入；`cron.get`、`cron.list`、`cron.status`、`cron.add`、`cron.update`、`cron.remove`、`cron.run` 和 `cron.runs` 管理排程工作。
    - `cron.run` 仍是手動執行的佇列式 RPC。需要完成語意的客戶端應讀取傳回的 `runId` 並輪詢 `cron.runs`。
    - `cron.runs` 接受選用的非空 `runId` 篩選器，以便客戶端能追蹤一個已排入佇列的手動執行，而不會與同一工作的其他歷史項目產生競爭。
...
- `chat`：UI 聊天更新，例如 `chat.inject` 和其他僅限文字記錄的聊天事件。在協議 v4 中，Delta 載荷攜帶 `deltaText`；`message` 保持為累積的助理快照。非前綴替換會設定 `replace=true` 並使用 `deltaText` 作為替換文字。
...
- `tasks.list` 需要 `operator.read`。
  - Params：選填的 `status` (`"queued"`、`"running"`、`"completed"`、
    `"failed"`、`"cancelled"` 或 `"timed_out"`) 或這些狀態的陣列，
    選填的 `agentId`、選填的 `sessionKey`、從 `1` 到
    `500` 的選填 `limit`，以及選填的字串 `cursor`。
...
- `tasks.cancel` 需要 `operator.write`。
  ...
  - `found` 回報分類帳是否具有相符的任務。`cancelled`
    回報執行環境是否接受或記錄取消動作。

`TaskSummary` 包含 `id`、`status` 和選填的中繼資料，例如 `kind`、
...
- 操作員可以呼叫 `commands.list` (`operator.read`) 以取得代理程式的執行環境
  指令清單。
  - `agentId` 為選填；若要讀取預設的代理程式工作區，請將其省略。
  - `scope` 控制主要的 `name` 以哪個介面為目標：
    - `text` 會傳回主要文字指令權杖，但不包含開頭的 `/`
    - 當可用時，`native` 和預設的 `both` 路徑會傳回具備提供者感知的原生名稱
  - `textAliases` 攜帶精確的斜線別名，例如 `/model` 和 `/m`。
  - 當存在時，`nativeName` 攜帶具備提供者感知的原生命令名稱。
  - `provider` 是可選的，且僅影響原生命名以及原生外掛程式命令的可用性。
  - `includeArgs=false` 會從回應中省略序列化的引數中繼資料。
- 操作員可以呼叫 `tools.catalog` (`operator.read`) 來擷取代理程式的執行階段工具目錄。回應包含分組的工具和來源中繼資料：
  - `source`：`core` 或 `plugin`
  - `pluginId`：當為 `source="plugin"` 時的外掛程式擁有者
...
- 操作員可以呼叫 `tools.invoke` (`operator.write`) 透過與 `/tools/invoke` 相同的閘道原則路徑來叫用其中一個可用工具。
  - `name` 是必要的。`args`、`sessionKey`、`agentId`、`confirm` 和 `idempotencyKey` 是可選的。
  - 如果同時存在 `sessionKey` 和 `agentId`，解析出的工作階段代理程式必須符合 `agentId`。
  - 回應是面向 SDK 的封套，包含 `ok`、`toolName`、可選的 `output` 和具類型的 `error` 欄位。核准或原則拒絕會在載荷中傳回 `ok:false`，而不是繞過閘道工具原則管線。
...
- 操作員可以呼叫 `skills.upload.begin`、`skills.upload.chunk` 和
  `skills.upload.commit` (`operator.admin`) 以在安裝私有技能封存之前進行預備。這是供受信任用戶端使用的獨立管理員上傳路徑，而非一般的 ClawHub 技能安裝流程，且預設為停用，除非啟用 `skills.install.allowUploadedArchives`。
  - `skills.upload.begin({ kind: "skill-archive", slug, sizeBytes, sha256?, force?, idempotencyKey? })`
    建立綁定至該 slug 和 force 值的上傳。
  - `skills.upload.chunk({ uploadId, offset, dataBase64 })` 在精確的解碼偏移處附加位元組。
  - `skills.upload.commit({ uploadId, sha256? })` 會驗證最終大小和
    SHA-256。提交僅會完成上傳；它不會安裝技能。
  - 上傳的技能封存是包含 `SKILL.md` 根目錄的 zip 封存。封存的內部目錄名稱從不會選擇安裝目標。
- 操作員可以透過三種模式呼叫 `skills.install` (`operator.admin`)：
  ...
  - 上傳模式：`{ source: "upload", uploadId, slug, force?, sha256?, timeoutMs? }`
    會將已提交的上傳安裝至預設代理程式工作區的 `skills/<slug>`
    目錄。Slug 和 force 值必須與原始的
...
- 操作員可以透過兩種模式呼叫 `skills.update` (`operator.admin`)：
  ...
  - Config 模式修補 `skills.entries.<skillKey>` 數值，例如 `enabled`、
    `apiKey` 和 `env`。
...
- 省略或 `"default"`：目前的執行時行為。如果設定了 `agents.defaults.models`，回應為允許的目錄，包括針對 `provider/*` 項目動態探索到的模型。否則回應為完整的 Gateway 目錄。
- `"configured"`：選擇器大小的行為。如果設定了 `agents.defaults.models`，它仍然優先，包括針對 `provider/*` 項目的提供者範圍探索。如果沒有允許清單，回應會使用明確的 `models.providers.*.models` 項目，僅在沒有已設定模型資料列時才退回到完整目錄。
- `"all"`：完整的 Gateway 目錄，繞過 `agents.defaults.models`。將其用於診斷和探索 UI，而非一般的模型選擇器。
...
- 批准後，轉發的 `node.invoke system.run` 呼叫會重用該標準的
  `systemRunPlan` 作為授權的 command/cwd/session 上下文。
- 如果呼叫者在準備與最終批准的 `system.run` 轉發之間變更了 `command`、`rawCommand`、`cwd`、`agentId` 或
  `sessionKey`，
  gateway 會拒絕執行，而不是信任變更後的負載。

## 代理傳遞後備

- `agent` 請求可以包含 `deliver=true` 以請求出站傳遞。
- `bestEffortDeliver=false` 保持嚴格行為：未解析或僅內部的傳遞目標將返回 `INVALID_REQUEST`。
- `bestEffortDeliver=true` 允許在無法解析外部可傳遞路由時（例如內部/webchat 會話或不明確的多通道配置）回退到僅限會話執行。
- 最終 `agent` 結果在請求傳遞時可能包含 `result.deliveryStatus`，使用與 [`openclaw agent --json --deliver`](/zh-Hant/cli/agent#json-delivery-status) 中記錄相同的 `sent`、`suppressed`、`partial_failed` 和 `failed` 狀態。
...
`src/gateway/client.ts` 中的參考客戶端使用這些預設值。這些值在協議 v4 中保持穩定，並且是第三方客戶端的預期基線。
...
| 預設 tick 間隔（於 `hello-ok` 之前） | `30_000` 毫秒                                        | `src/gateway/client.ts`                                                         |
| Tick 逾時關閉                        | 當靜默超過 `tickIntervalMs * 2` 時傳送 `4000` 程式碼 | `src/gateway/client.ts`                                                         |
...
- 共享金鑰 Gateway 驗證依據設定的驗證模式，使用 `connect.params.auth.token` 或
  `connect.params.auth.password`。
- 承載身分的模式，例如 Tailscale Serve
  (`gateway.auth.allowTailscale: true`) 或非本機迴路
  `gateway.auth.mode: "trusted-proxy"`，會根據
  請求標頭滿足連線驗證檢查，而非 `connect.params.auth.*`。
- 私有入口 `gateway.auth.mode: "none"` 完全跳過共享金鑰連線驗證；
  請勿在公網或不受信任的入口上暴露該模式。
- 配對後，Gateway 會發布一個範圍限定於連線
  角色 + 範圍的 **裝置權杖** (device token)。它會在 `hello-ok.auth.deviceToken` 中
  回傳，客戶端應將其保存以供日後連線使用。
- 客戶端應在每次成功連線後保存主要 `hello-ok.auth.deviceToken`。
- 使用該 **已儲存** 的裝置權杖重新連線時，也應重複使用為該權杖儲存的已批准範圍集合。
  這樣可保留已授予的讀取/探測/狀態存取權，並避免無聲地將重新連線收窄為較狹隘的隱含僅限管理員範圍。
- 客戶端連線驗證組裝（`src/gateway/client.ts` 中的
  `selectConnectAuth`）：
  - `auth.password` 是正交的，且一旦設定就會被轉發。
  - `auth.token` 會依優先順序填入：首先是明確的共享權杖，
    接著是明確的 `deviceToken`，然後是儲存的每裝置權杖（以
    `deviceId` + `role` 為鍵）。
  - `auth.bootstrapToken` 僅在上述方法均未解析出
    `auth.token` 時才會發送。共享 Token 或任何已解析的裝置 Token 會抑制此行為。
  - 在一次性 `AUTH_TOKEN_MISMATCH` 重試時，對已儲存裝置 Token 的自動提升僅限於**受信任的端點**——
    迴路，或具有釘選 `tlsFingerprint` 的 `wss://`。未進行釘選的公用 `wss://`
    不符合資格。
- 內建的設定代碼啟動程序會傳回主要節點
  `hello-ok.auth.deviceToken`，以及用於受信任行動裝置移交的 `hello-ok.auth.deviceTokens` 中的受限操作員 Token。該操作員 Token
  不包含 `operator.admin`、`operator.pairing` 和 `operator.talk.secrets`。
- 當非基準設定代碼啟動程序正在等待核准時，`PAIRING_REQUIRED`
  詳細資訊會包含 `recommendedNextStep: "wait_then_retry"`、`retryable: true`
  和 `pauseReconnect: false`。用戶端應持續使用相同的
  啟動 Token 重新連線，直到請求被核准或 Token 變為無效。
- 僅當連線使用了諸如 `wss://` 或迴路/本機配對等受信任傳輸上的啟動驗證時，才應持久化 `hello-ok.auth.deviceTokens`。
- 如果用戶端提供了**明確的** `deviceToken` 或明確的 `scopes`，該
  呼叫端請求的範圍集將保持權威性；僅當用戶端重用已儲存的每裝置 Token 時，才會重用快取的範圍。
- 裝置 Token 可以透過 `device.token.rotate` 和
  `device.token.revoke` 進行輪替/撤銷（需要 `operator.pairing` 範圍）。
- `device.token.rotate` 會傳回輪替中繼資料。它僅對已使用該裝置 Token 進行驗證的相同裝置呼叫回傳替代 Bearer Token，以便僅使用 Token 的用戶端可以在重新連線前保存其替代 Token。共享/管理員輪替不會回傳 Bearer Token。
- 令牌的簽發、輪換和撤銷仍受限於記錄在該裝置配對條目中的批准角色集；令牌變更無法擴展或針對配對批准從未授予的裝置角色。
- 對於配對裝置 Token 工作階段，除非呼叫者也擁有 `operator.admin`，否則裝置管理僅限於自身範圍：非管理員呼叫者只能移除/撤銷/輪替
  屬於其**自己**的裝置項目。
- `device.token.rotate` 和 `device.token.revoke` 還會根據呼叫者的目前 session scopes 檢查目標 operator token 的 scope set。非 admin 呼叫者無法輪換或撤銷比其目前持有的範圍更廣的 operator token。
- Auth 失敗包含 `error.details.code` 以及恢復提示：
...
- `AUTH_SCOPE_MISMATCH` 表示 device token 已被識別，但未涵蓋請求的 role/scopes。客戶端不應將此顯示為無效 token；應提示 operator 重新配對或批准較窄/較廣的 scope 合約。
...
- WS 用戶端通常會在 `connect` 期間 (operator + node) 包含 `device` 身分。唯一無裝置的 operator 例外是明確信任路徑：
  - `gateway.controlUi.allowInsecureAuth=true` 僅限於不安全 HTTP 相容性。
  - 成功的 `gateway.auth.mode: "trusted-proxy"` operator Control UI auth。
  - `gateway.controlUi.dangerouslyDisableDeviceAuth=true` (緊急存取，嚴重的安全性降級)。
  - 使用共用 gateway token/password 進行驗證的直接回送 `gateway-client` 後端 RPC。
- 省略裝置身分會產生 scope 後果。當 Control UI 連線缺少裝置身分時，`shouldClearUnboundScopesForMissingDeviceIdentity` 會將 token、password 和 trusted-proxy auth 的自宣告 scopes 清除為空集合。連線在明確信任路徑上被允許，但 scope-gated 方法會失敗。例外是具有 `allowInsecureAuth` 的本機 Control UI token/password sessions，它們會保留 scopes。對於其他情況，僅將 `gateway.controlUi.dangerouslyDisableDeviceAuth=true` 設定為緊急 scope 保留路徑。
- 所有連線必須對伺服器提供的 `connect.challenge` nonce 進行簽署。

### 裝置 auth 遷移診斷

對於仍使用預挑戰簽名行為的舊版客戶端，`connect` 現在會在 `error.details.code` 下返回具有穩定 `error.details.reason` 的 `DEVICE_AUTH_*` 詳細代碼。
...
| `device signature invalid`  | `DEVICE_AUTH_SIGNATURE_INVALID`  | `device-signature`       | 簽名載荷與 v2 載荷不匹配。                      |
| `device signature expired`  | `DEVICE_AUTH_SIGNATURE_EXPIRED`  | `device-signature-stale` | 簽名的時間戳超出允許的偏移範圍。                |
...
- 對包含服務器 nonce 的 v2 載荷進行簽名。
- 在 `connect.params.device.nonce` 中發送相同的 nonce。
- 首選的簽名載荷是 `v3`，它除了設備/客戶端/角色/作用域/令牌/nonce 字段外，還綁定了 `platform` 和 `deviceFamily`。
- 舊版 `v2` 簽名為了兼容性仍被接受，但在重新連接時，配對設備元數據固定仍然控制指令策略。

## TLS + 固定

- WS 連接支援 TLS。
- 客戶端可以可選地固定網關證書指紋（請參閱 `gateway.tls` 配置以及 `gateway.remote.tlsFingerprint` 或 CLI `--tls-fingerprint`）。

## 作用域

此協定公開了**完整的閘道 API**（狀態、頻道、模型、聊天、代理、會話、節點、核准等）。確切的介面是由 `src/gateway/protocol/schema.ts` 中的 TypeBox schema 定義的。
