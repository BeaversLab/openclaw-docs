...
- 對於直接連線到遠端閘道的原生行動用戶端，建議優先使用 [WebChat](/zh-Hant/web/webchat) 或 [閘道協定](/zh-Hant/gateway/protocol)，並實作配對裝置引導程序/裝置權杖流程，如此裝置便不需要共用的 HTTP 權杖/密碼。
- 當您正在整合具有自己的使用者、房間、Webhook 遞送或輸出傳輸的外部訊息網路時，請改為建構頻道外掛程式。請參閱 [建構外掛程式](/zh-Hant/plugins/building-plugins)。
...
當設定任一 token 上限欄位時，該值會透過代理程式串流參數通道轉發到上游提供者。傳送到上游提供者的實際線路欄位名稱由提供者傳輸選擇：對於 OpenAI 系列端點為 `max_completion_tokens`，而對於僅接受舊有名稱的提供者（例如 Mistral 和 Chutes）則為 `max_tokens`。取樣欄位 (`temperature`、`top_p`、`frequency_penalty`、`presence_penalty`、`seed`) 遵循相同的串流參數通道；由於 ChatGPT 型態的 Codex Responses 後端使用固定取樣，因此會在伺服器端將其移除。
...
- `tool_choice: "required"`（目前尚未於執行時強制執行；將在實施硬性強制執行後支援）
...
當 `stream: true` 時，工具呼叫會以增量 SSE 區塊的形式發出：
...
收到 `tool_calls` 後，用戶端應執行請求的函式，並發送包含以下內容的後續請求：
...
- 如果您想要該代理使用特定的後端供應商/模型，請設定該代理的一般預設模型或發送 `x-openclaw-model`
...
如果傳回 `openclaw/default`，大多數 Open WebUI 設定都可以使用相同的基礎 URL 和權杖進行連線。
...
- `/v1/models` 會傳回 OpenClaw 代理目標，而非原始供應商目錄。
- `openclaw/default` 永遠存在，因此一個穩定的 ID 可在不同環境中運作。
