...
- `max_completion_tokens`：數字；每次呼叫的總完成 Token 上限（包含推理 Token）。目前的 OpenAI Chat Completions 欄位名稱；當同時傳送 `max_completion_tokens` 和 `max_tokens` 時為優先選項。
- `max_tokens`：數字；為了向後相容而接受的舊版別名。當 `max_completion_tokens` 也存在時會被忽略。

當設定任一欄位時，該值會透過代理串流參數通道轉發至上游提供者。實際發送給上游提供者的實體欄位名稱是由提供者傳輸層選擇的：針對 OpenAI 系列端點為 `max_completion_tokens`，而對於僅接受舊版名稱的提供者（如 Mistral 和 Chutes）則為 `max_tokens`。
...
- `tool_choice: "required"`（尚未在執行時期強制執行；將在實施強制執行後提供支援）
...
工具呼叫前的助理註解會在 `choices[0].message.content` 中回傳（可能為空）。

### 串流工具回傳形狀

當 `stream: true` 時，工具呼叫會以遞增的 SSE 區塊發出：

- 初始的助理角色增量
- 選用的助理註解增量
- 一或多個 `delta.tool_calls` 區塊，承載工具身分與參數片段
...
收到 `tool_calls` 後，客戶端應執行請求的函數，並發送包含以下內容的後續請求：

- 先前的助理工具呼叫訊息
- 一或多個 `role: "tool"` 訊息，且帶有相符的 `tool_call_id`

這允許閘道代理程式執行繼續相同的推理循環，並產生最終的助理回答。
...
- API 金鑰：您的 Gateway 持有者權杖 (bearer token)
...
- 如果您想要為該代理程式指定特定的後端供應商/模型，請設定代理程式的正常預設模型，或發送 `x-openclaw-model`
...
如果該請求傳回 `openclaw/default`，大多數 Open WebUI 設定都可以使用相同的 Base URL 和權杖連線。
...
- `/v1/models` 會傳回 OpenClaw 代理程式目標，而非原始供應商目錄。
- `openclaw/default` 永遠存在，因此一個穩定的 ID 可在各環境中運作。
- 後端提供者/模型覆寫屬於 `x-openclaw-model`，而非 OpenAI 的 `model` 欄位。
