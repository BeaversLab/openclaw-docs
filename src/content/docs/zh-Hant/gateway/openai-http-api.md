...
- 對於直接連線到遠端閘道的原生行動客戶端，建議優先使用 [WebChat](/zh-Hant/web/webchat) 或 [Gateway Protocol](/zh-Hant/gateway/protocol)，並實作配對裝置引導/裝置權杖流程，以便裝置不需要共享 HTTP 權杖/密碼。
- 當您整合具備自己的使用者、房間、Webhook 傳遞或出站傳輸的外部訊息網路時，請改為建置頻道外掛程式。請參閱 [建置外掛程式](/zh-Hant/plugins/building-plugins)。
...
- `max_completion_tokens`：number；每次呼叫的總補全 token 上限（包含推理 token）。目前的 OpenAI Chat Completions 欄位名稱；當同時傳送 `max_completion_tokens` 與 `max_tokens` 時，優先採用此欄位。
...
- 先前的助理工具呼叫訊息
- 一條或多條帶有匹配 `tool_call_id` 的 `role: "tool"` 訊息
