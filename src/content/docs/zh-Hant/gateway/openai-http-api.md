...
- 對於直接連線到遠端閘道的原生行動客戶端，請優先選擇 [WebChat](/zh-Hant/web/webchat) 或 [Gateway Protocol](/zh-Hant/gateway/protocol)，並實作配對裝置啟動/裝置權杖流程，以便裝置不需要共享的 HTTP 權杖/密碼。
- 當您整合具有自己的使用者、聊天室、Webhook 傳遞或輸出傳輸的外部訊息網路時，請改為建構頻道外掛。請參閱 [建構外掛](/zh-Hant/plugins/building-plugins)。
...
當設定了任一 token 上限欄位時，該值會透過代理串流參數通道轉發給上游供應商。傳送給上游供應商的實際傳輸欄位名稱取決於供應商傳輸方式：對於 OpenAI 系列端點為 `max_completion_tokens`，而對於僅接受舊版名稱的供應商（例如 Mistral 和 Chutes）則為 `max_tokens`。採樣欄位（`temperature`、`top_p`）遵循相同的串流參數通道；由於 ChatGPT 型的 Codex Responses 後端使用固定採樣，因此會在伺服器端將其移除。

### 不支援的變體

對於不支援的工具變體，該端點會傳回 `400 invalid_request_error`，包括：

- 非陣列的 `tools`
- 非函式工具項目
- 缺少 `tool.function.name`
- `tool_choice` 變體，例如 `allowed_tools` 和 `custom`
- `tool_choice: "required"`（尚未在執行時強制執行；將在實施嚴格強制後支援）
- `tool_choice: { "type": "function", "function": { "name": "..." } }`（理由與 `required` 相同）
- 與提供的 `tools` 不符的 `tool_choice.function.name` 值
...
工具呼叫之前的助手評註會在 `choices[0].message.content` 中傳回（可能為空）。

### 串流工具回應形狀

當 `stream: true` 時，工具呼叫會以漸進式 SSE 區塊發出：

- 初始助手角色增量
- 選用助手評註增量
- 一或多個攜帶工具身分與參數片段的 `delta.tool_calls` 區塊
- 具有 `finish_reason: "tool_calls"` 的最終區塊
- `data: [DONE]`
...
收到 `tool_calls` 後，客戶端應執行所請求的函式，並傳送包含以下內容的後續請求：

- 先前的助手工具呼叫訊息
- 一或多個具有相符 `tool_call_id` 的 `role: "tool"` 訊息

這允許閘道代理程式執行繼續相同的推理迴圈，並產生最終的助手回答。
...
- 如果您想要為該代理程式指定特定的後端提供者/模型，請設定代理程式的正常預設模型，或傳送 `x-openclaw-model`
...
如果傳回 `openclaw/default`，則大多數 Open WebUI 設定都可以使用相同的 Base URL 和 Token 連線。
...
- `/v1/models` 會傳回 OpenClaw 代理程式目標，而非原始提供者目錄。
- `openclaw/default` 永遠存在，因此一個穩定的 ID 可跨環境運作。
- 後端供應商/模型覆寫屬於 `x-openclaw-model`，而不是 OpenAI `model` 欄位。
