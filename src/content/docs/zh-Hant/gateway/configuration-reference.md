...
`~/.openclaw/openclaw.json` 的核心設定參考。如需工作導向的概覽，請參閱 [設定](/en/gateway/configuration)。
...
- [記憶體設定參考](/en/reference/memory-config)，針對 `agents.defaults.memorySearch.*`、`memory.qmd.*`、`memory.citations`，以及 `plugins.entries.memory-core.config.dreaming` 下的 dreaming 設定
- 目前內建 + 套件指令目錄的 [斜線指令](/en/tools/slash-commands)
...
- 頂層 `bindings[]` 項目若包含 `type: "acp"`，則為論壇主題設定持續的 ACP 綁定（請在 `match.peer.id` 中使用正式的 `chatId:topic:topicId`）。欄位語義詳見 [ACP Agents](/en/tools/acp-agents#channel-specific-settings)。
...
- 頂層 `bindings[]` 項目搭配 `type: "acp"` 可設定頻道和執緒的持久化 ACP 綁定（在 `match.peer.id` 中使用頻道/執緒 ID）。欄位語義在 [ACP Agents](/en/tools/acp-agents#channel-specific-settings) 中共享。
...
- 頂層 `bindings[]` 項目若帶有 `type: "acp"`，可將 BlueBubbles 對話綁定至持久的 ACP 工作階段。請在 `match.peer.id` 中使用 BlueBubbles handle 或目標字串（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共用欄位語意：[ACP Agents](/en/tools/acp-agents#channel-specific-settings)。
...
- 頂層 `bindings[]` 項目若帶有 `type: "acp"`，可將 iMessage 對話綁定到持久的 ACP 會話。在 `match.peer.id` 中使用標準化的 handle 或明確的聊天目標（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共用欄位語義：[ACP Agents](/en/tools/acp-agents#channel-specific-settings)。
...
- 完整的 Matrix 設定、目標規則和設定範例記載於 [Matrix](/en/channels/matrix)。
...
- 完整的 Teams 設定（憑證、Webhook、私訊/群組原則、各團隊/各頻道的覆寫）記載於 [Microsoft Teams](/en/channels/msteams)。
...
- 完整的 IRC 頻道設定（主機/連接埠/TLS/頻道/允許清單/提及閘道）記載於 [IRC](/en/channels/irc)。
...
許多擴充通道被配置為 `channels.<id>` 並在其專屬通道頁面中記錄（例如飛書、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
查看完整的通道索引：[Channels](/en/channels)。
...
- 此區塊設定指令介面。若要查看目前的內建 + 捆綁指令目錄，請參閱 [Slash Commands](/en/tools/slash-commands)。
- 本頁面是 **設定鍵值參考**，並非完整的指令目錄。通道/外掛擁有的指令，例如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、device-pair `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice`，均記錄在其各自的通道/外掛頁面以及 [Slash Commands](/en/tools/slash-commands) 中。
- 文字指令必須是 **standalone** 訊息，並以 `/` 開頭。
- `native: "auto"` 會為 Discord/Telegram 開啟原生指令，並讓 Slack 保持關閉。
- `nativeSkills: "auto"` 會為 Discord/Telegram 開啟原生技能指令，並讓 Slack 保持關閉。
- 依通道覆寫：`channels.discord.commands.native` (bool 或 `"auto"`)。`false` 會清除先前註冊的指令。
...
- `config: true` 啟用 `/config` (讀取/寫入 `openclaw.json`)。對於 gateway `chat.send` 客戶端，持續性 `/config set|unset` 寫入也需要 `operator.admin`；唯讀 `/config show` 仍可讓一般寫入範圍的操作員客戶端使用。
- `mcp: true` 為 `mcp.servers` 下的 OpenClaw 管理之 MCP 伺服器設定啟用 `/mcp`。
- `plugins: true` 為外掛探索、安裝以及啟用/停用控制啟用 `/plugins`。
- `channels.<provider>.configWrites` 會依通道限制設定變更 (預設：true)。
- 對於多帳戶通道，`channels.<provider>.accounts.<id>.configWrites` 也會限制以該帳戶為目標的寫入 (例如 `/allowlist --config --account <id>` 或 `/config set channels.<provider>.accounts.<id>...`)。
- `restart: false` 會停用 `/restart` 和 gateway 重啟工具動作。預設值：`true`。
- `ownerAllowFrom` 是僅限擁有者指令/工具的明確擁有者允許清單。它與 `allowFrom` 分開。
- `ownerDisplay: "hash"` 會對系統提示中的擁有者 ID 進行雜湊。設定 `ownerDisplaySecret` 以控制雜湊。
- `allowFrom` 是針對各提供者的。設定後，它將是 **唯一** 的授權來源 (通道允許清單/配對和 `useAccessGroups` 將被忽略)。
- 當未設定 `allowFrom` 時，`useAccessGroups: false` 允許指令繞過存取群組原則。
- 指令文件地圖：
  - 內建 + 捆綁目錄：[Slash Commands](/en/tools/slash-commands)
  - 特定通道指令介面：[Channels](/en/channels)
...
- `params` 合併優先順序（配置）：`agents.defaults.params`（全域基礎）會被 `agents.defaults.models["provider/model"].params`（每個模型）覆寫，然後 `agents.list[].params`（匹配的代理 ID）會按鍵覆寫。詳情請參閱 [提示詞快取](/en/reference/prompt-caching)。
...
- `mode`：`default` 或 `safeguard`（針對長歷史的分塊摘要）。參見 [壓縮](/en/concepts/compaction)。
- `provider`：已註冊的壓縮提供者外掛程式的 ID。設定後，會呼叫提供者的 `summarize()`，而非內建的 LLM 摘要。失敗時回退至內建方法。設定提供者會強制啟用 `mode: "safeguard"`。參見 [壓縮](/en/concepts/compaction)。
...
- `notifyUser`：當 `true` 時，在壓縮開始時向使用者發送簡短通知（例如，「正在壓縮上下文...」）。預設停用以保持壓縮靜默。
...
有關行為的詳細資訊，請參閱[工作階段修剪](/en/concepts/session-pruning)。
...
有關行為 + 分塊的詳細資訊，請參閱 [串流](/en/concepts/streaming)。
...
嵌入式代理的可選沙箱。完整指南請參閱 [沙箱](/en/gateway/sandboxing)。
...
- `thinkingDefault`：選用的每個代理程式預設思考層級 (`off | minimal | low | medium | high | xhigh | adaptive`)。當未設定每個訊息或工作階段的覆寫時，會覆寫此代理程式的 `agents.defaults.thinkingDefault`。
...
原生 Moonshot 端點在共享的 `openai-completions` 傳輸上宣佈支援串流使用相容性，且 OpenClaw 金鑰依賴端點功能而不僅僅是內建的提供者 ID。
...
請參閱 [本機模型](/en/gateway/local-models)。TL;DR：在強大的硬體上透過 LM Studio Responses API 執行大型本機模型；保留託管模型的合併以作為備援。
...
- `plugins.entries.memory-core.config.dreaming`：記憶體夢境設定。請參閱 [Dreaming](/en/concepts/dreaming) 以了解階段和閾值。
  - `enabled`：主夢境開關（預設值 `false`）。
  - `frequency`：每次完整夢境掃描的 cron 頻率（預設為 `"0 3 * * *"`）。
  - 階段策略和閾值屬於實作細節（非使用者面向的配置金鑰）。
- 完整的記憶體配置位於 [記憶體配置參考](/en/reference/memory-config)：
...
- `existing-session` 設定檔僅適用於主機 (host-only)，並使用 Chrome MCP 而非 CDP。
...
- **Docker 說明**：預設的 `loopback` 綁定會在容器內監聽 `127.0.0.1`。使用 Docker 橋接網路（`-p 18789:18789`）時，流量會到達 `eth0`，因此無法存取閘道。請使用 `--network host`，或設定 `bind: "lan"`（或搭配 `customBindHost: "0.0.0.0"` 使用 `bind: "custom"`）以監聽所有介面。
- **驗證 (Auth)**：預設為必填。非迴路 綁定需要閘道驗證。實務上這意味著共用權杖/密碼或具備 `gateway.auth.mode: "trusted-proxy"` 的身分感知反向 Proxy。入門精靈預設會產生一個權杖。
- 如果同時設定了 `gateway.auth.token` 和 `gateway.auth.password`（包括 SecretRefs），請將 `gateway.auth.mode` 明確設定為 `token` 或 `password`。當同時設定兩者且模式未設定時，啟動和服務安裝/修復流程將會失敗。
- `gateway.auth.mode: "none"`：明確的無驗證模式。僅用於受信任的本機迴路設定；入門提示刻意不提供此選項。
- `gateway.auth.mode: "trusted-proxy"`：將驗證委派給身分感知反向 Proxy，並信任來自 `gateway.trustedProxies` 的身分標頭（請參閱[受信任 Proxy 驗證](/en/gateway/trusted-proxy-auth)）。此模式預期來自**非迴路**的 Proxy 來源；同主機迴路反向 Proxy 不符合受信任 Proxy 的驗證條件。
- `gateway.auth.allowTailscale`：當 `true` 時，Tailscale Serve 身分標頭可以滿足控制 UI/WebSocket 驗證（透過 `tailscale whois` 驗證）。HTTP API 端點**不**會使用該 Tailscale 標頭驗證；它們改為遵循閘道的正常 HTTP 驗證模式。此無權杖流程假設閘道主機是受信任的。當 `tailscale.mode = "serve"` 時，預設為 `true`。
- `gateway.auth.rateLimit`：可選的驗證失敗限制器。根據用戶端 IP 和驗證範圍（共用金鑰和裝置權杖會獨立追蹤）套用。被封鎖的嘗試會回傳 `429` + `Retry-After`。
  - 在非同步 Tailscale Serve 控制UI 路徑上，來自相同 `{scope, clientIp}` 的失敗嘗試會在寫入失敗前序列化。因此，來自同一用戶端的並發錯誤嘗試可能會在第二次請求時觸發限制器，而不是兩者都當作一般不符而競爭通過。
  - `gateway.auth.rateLimit.exemptLoopback` 預設為 `true`；當您刻意也要對 localhost 流量進行速率限制時（例如測試環境或嚴格的 Proxy 部署），請設定 `false`。
- 瀏覽器來源的 WS 驗證嘗試一律會受到節流，並停用迴路豁免（防止瀏覽器對 localhost 暴力破解的縱深防禦）。
- 在迴路上，這些瀏覽器來源鎖定會根據標準化的 `Origin` 值隔離，因此來自一個 localhost 來源的失敗不會自動鎖定不同的來源。
- `tailscale.mode`：`serve`（僅限 tailnet，迴路綁定）或 `funnel`（公開，需要驗證）。
- `controlUi.allowedOrigins`：閘道 WebSocket 連線的明確瀏覽器來源白名單。當預期來自非迴路來源的瀏覽器用戶端時，此為必填。
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`：危險模式，會為刻意依賴 Host 標頭來源原則的部署啟用 Host 標頭來源後援。
- `remote.transport`：`ssh`（預設）或 `direct`（ws/wss）。對於 `direct`，`remote.url` 必須是 `ws://` 或 `wss://`。
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`：用戶端緊急覆蓋設定，允許對受信任的私人網路 IP 使用純文字 `ws://`；對於純文字，預設仍僅限迴路。
- `gateway.remote.token` / `.password` 是遠端用戶端憑證欄位。它們本身不會設定閘道驗證。
- `gateway.push.apns.relay.baseUrl`：官方 / TestFlight iOS 版本在將轉發支援的註冊發布到閘道後，所使用的外部 APNs 轉發器的 HTTPS 基礎 URL。此 URL 必須符合編譯至 iOS 版本中的轉發器 URL。
- `gateway.push.apns.relay.timeoutMs`：閘道到轉發器的傳送逾時（以毫秒為單位）。預設為 `10000`。
- 轉發器支援的註冊會委派給特定的閘道身分。配對的 iOS 應用程式會取得 `gateway.identity.get`，將該身分包含在轉發器註冊中，並將註冊範圍的傳送授權轉發給閘道。另一個閘道無法重複使用該已儲存的註冊。
- `OPENCLAW_APNS_RELAY_BASE_URL` / `OPENCLAW_APNS_RELAY_TIMEOUT_MS`：上述轉發器設定的暫時環境覆寫。
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`：僅限開發用的迴路 HTTP 轉發器 URL 緊急通道。正式運作的轉發器 URL 應維持使用 HTTPS。
- `gateway.channelHealthCheckMinutes`：通道健康監控間隔（以分鐘為單位）。設定 `0` 以全域停用健康監控重啟。預設：`5`。
- `gateway.channelStaleEventThresholdMinutes`：過時 Socket 臨界值（以分鐘為單位）。請將此值保持大於或等於 `gateway.channelHealthCheckMinutes`。預設：`30`。
- `gateway.channelMaxRestartsPerHour`：每個通道/帳戶在每小時滾動視窗內的最大健康監控重啟次數。預設：`10`。
- `channels.<provider>.healthMonitor.enabled`：各通道選擇不進行健康監控重啟，同時保持全域監控啟用。
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`：多重帳戶通道的個別帳戶覆寫。設定後，其優先順序高於通道層級的覆寫。
- 本機閘道呼叫路徑只有在未設定 `gateway.auth.*` 時，才能使用 `gateway.remote.*` 作為後援。
- 如果 `gateway.auth.token` / `gateway.auth.password` 透過 SecretRef 明確設定但未解析，解析將會失敗關閉（不會遮蔽遠端後援）。
- `trustedProxies`：終止 TLS 或插入轉發用戶端標頭的反向 Proxy IP。僅列出您控制的 Proxy。對於同主機 Proxy/本機偵測設定（例如 Tailscale Serve 或本機反向 Proxy），迴路項目仍然有效，但它們**不**會讓迴路請求有資格使用 `gateway.auth.mode: "trusted-proxy"`。
- `allowRealIpFallback`：當 `true` 時，如果缺少 `X-Forwarded-For`，閘道會接受 `X-Real-IP`。預設為 `false` 以執行失敗關閉行為。
...
- 像是 `{{messages[0].subject}}` 這類的模板會從 payload 讀取。
- `transform` 可以指向一個傳回 hook 動作的 JS/TS 模組。
  - `transform.module` 必須是相對路徑，且必須停留在 `hooks.transformsDir` 之內（絕對路徑和路徑遍歷會被拒絕）。
- `agentId` 路由到特定的 agent；未知的 ID 會回退到預設值。
- `allowedAgentIds`：限制明確路由（`*` 或省略 = 允許全部，`[]` = 拒絕全部）。
- `defaultSessionKey`：針對沒有明確 `sessionKey` 的 agent 執行，選用的固定 session 金鑰。
...
- Gateway 在啟動時若已設定，會自動啟動 `gog gmail watch serve`。設定 `OPENCLAW_SKIP_GMAIL_WATCHER=1` 以停用。
- 不要與 Gateway 同時執行另一個獨立的 `gog gmail watch serve`。
...
- 在 Gateway 埠下透過 HTTP 提供 agent 可編輯的 HTML/CSS/JS 和 A2UI：
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- 僅限本機：保持 `gateway.bind: "loopback"`（預設）。
- 非迴路綁定：canvas 路由需要 Gateway 身份驗證（token/password/trusted-proxy），與其他 Gateway HTTP 介面相同。
- Node WebViews 通常不發送身份驗證標頭；在節點配對並連接後，Gateway 會發布用於 canvas/A2UI 存取的節點範圍功能 URL。
- 功能 URL 綁定到作用中的節點 WS 會話並會快速過期。不使用基於 IP 的後備機制。
...
- 主機名稱預設為 `openclaw`。可使用 `OPENCLAW_MDNS_HOSTNAME` 覆蓋。
...
- `auth-profiles.json` refs 包含在執行階段解析和稽核覆蓋範圍中。
...
- 預設情況下，符號連結命令路徑會被拒絕。設定 `allowSymlinkCommand: true` 以在驗證解析的目標路徑時允許符號連結路徑。
- 如果配置了 `trustedDirs`，信任目錄檢查會套用於解析的目標路徑。
- `exec` 子環境預設為最小化；使用 `passEnv` 明確傳遞所需的變數。
- Secret refs 在啟用時解析為記憶體內快照，然後請求路徑僅讀取快照。
- 啟用期間套用啟用介面過濾：啟用介面上未解析的 refs 會導致啟動/重新載入失敗，而非啟用介面則會跳過並輸出診斷資訊。
...
- `auth-profiles.json` 支援靜態憑證模式的值級別參照 (`api_key` 的 `keyRef`，`token` 的 `tokenRef`)。
- OAuth 模式的設定檔 (`auth.profiles.<id>.mode = "oauth"`) 不支援由 SecretRef 支援的 auth-profile 憑證。
- 靜態執行時憑證來自記憶體中解析的快照；傳統的靜態 `auth.json` 條目會在發現時被清除。
- 從 `~/.openclaw/credentials/oauth.json` 匯入的傳統 OAuth。
...
- `billingBackoffHours`：當設定檔因真正的計費/餘額不足錯誤而失敗時，以小時為單位的基本退避時間 (預設：`5`)。即使在 `401`/`403` 回應上，明確的計費文字仍可能落入此處，但特定供應商的文字匹配器僅限於擁有它們的供應商範圍內 (例如 OpenRouter `Key limit exceeded`)。可重試的 HTTP `402` 使用量視窗或組織/工作區支出限制訊息則會保留在 `rate_limit` 路徑中。
- `billingBackoffHoursByProvider`：可選的各供應商覆寫，用於計費退避小時數。
- `billingMaxHours`：計費退避指數增長的小時數上限 (預設：`24`)。
- `authPermanentBackoffMinutes`：高置信度 `auth_permanent` 失敗的基本退避時間 (以分鐘為單位) (預設：`10`)。
- `authPermanentMaxMinutes`：`auth_permanent` 退避增長的分鐘數上限 (預設：`60`)。
- `failureWindowHours`：用於退避計數器的滾動視窗 (以小時為單位) (預設：`24`)。
- `overloadedProfileRotations`：在切換到模型備援之前，針對過載錯誤進行同供應商身分驗證設定檔輪換的最大次數（預設：`1`）。供應商忙碌類型的錯誤（例如 `ModelNotReadyException`）皆屬此類。
- `overloadedBackoffMs`：重試過載的供應商/設定檔輪換前的固定延遲（預設：`0`）。
- `rateLimitedProfileRotations`：在切換到模型備援之前，針對速率限制錯誤進行同供應商身分驗證設定檔輪換的最大次數（預設：`1`）。該速率限制區塊包含供應商格式的文字，例如 `Too many concurrent requests`、`ThrottlingException`、`concurrency limit reached`、`workers_ai ... quota limit exceeded` 和 `resource exhausted`。
...
- `maxFileBytes`：在停止寫入前的日誌檔案最大大小（位元組）（正整數；預設：`524288000` = 500 MB）。生產環境部署請使用外部日誌輪替。
...
- `otel.flushIntervalMs`: 定期遙測沖洗間隔（毫秒）。
...
- `auto.stableDelayHours`: 穩定通道自動套用前的最小延遲時間（小時）（預設值：`6`；最大值：`168`）。
- `auto.stableJitterHours`: 額外的穩定通道推出分散視窗（小時）（預設值：`12`；最大值：`168`）。
- `auto.betaCheckIntervalHours`: Beta 通道檢查的頻率（小時）（預設值：`1`；最大值：`24`）。
...
- `enabled`: 全域 ACP 功能開關（預設值：`false`）。
- `dispatch.enabled`: ACP 會話回合調度的獨立開關（預設值：`true`)。設定 `false` 以在阻擋執行的同時保持 ACP 指令可用。
- `backend`：預設 ACP 執行時後端 ID（必須符合已註冊的 ACP 執行時外掛）。
- `defaultAgent`：當產生的程序未指定明確目標時，使用的後備 ACP 目標代理程式 ID。
- `allowedAgents`：允許用於 ACP 執行時階段的代理程式 ID 允許清單；空白表示無額外限制。
- `maxConcurrentSessions`：最大並行作用中 ACP 階段數。
...
- `stream.hiddenBoundarySeparator`：在隱藏工具事件後可見文字之前的分隔符（預設值：`"paragraph"`）。
...
- `runtime.ttlMinutes`：ACP 階段工作程式符合清理資格前的閒置 TTL（分鐘）。
...
- `cli.banner.taglineMode` 控制橫幅標語樣式：
  ...
  - `"default"`：固定中立標語（`All your chats, one OpenClaw.`）。
...
目前的版本不再包含 TCP 橋接器。節點透過 Gateway WebSocket 連線。`bridge.*` 金鑰不再是配置架構的一部分（必須移除否則驗證會失敗；`openclaw doctor --fix` 可移除未知金鑰）。
...
- `sessionRetention`：在從 `sessions.json` 中修剪之前，保留已完成的獨立 cron 執行階段的時間長度。同時也控制已刪除 cron 腳本的已封存清理作業。預設值：`24h`；設定 `false` 以停用。
- `runLog.maxBytes`：每次執行日誌檔案 (`cron/runs/<jobId>.jsonl`) 在修剪前的最大大小。預設值：`2_000_000` 位元組。
- `runLog.keepLines`：觸發執行日誌修剪時保留的最新行數。預設值：`2000`。
- `webhookToken`：用於 cron webhook POST 遞送 (`delivery.mode = "webhook"`) 的 bearer token，如果省略則不傳送 auth 標頭。
...
- `maxAttempts`：暫時性錯誤時單次工作的最大重試次數（預設值：`3`；範圍：`0`–`10`）。
...
- `cooldownMs`：針對同一任務重複發送警報之間的最小毫秒數（非負整數）。
...
- `channel`：公告傳遞的頻道覆蓋。`"last"` 會重複使用最後已知的傳遞頻道。
- `to`：明確的公告目標或 webhook URL。在 webhook 模式下為必填。
- `accountId`：傳遞的可選帳號覆蓋。
- 針對每個任務的 `delivery.failureDestination` 會覆蓋此全域預設值。
- 當未設定全域或針對任務的失敗目的地時，已透過 `announce` 傳遞的任務在失敗時會退回至該主要公告目標。
