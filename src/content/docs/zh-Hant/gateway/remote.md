...
此軟體庫透過在專用主機（桌面/伺服器）上執行單一 Gateway（主控端）並將客戶端連線至它，來支援「透過 SSH 進行遠端」功能。
...
- macOS 應用程式的「節點模式」只是透過 Gateway WebSocket 的節點客戶端。
...
當 gateway 僅限回環時，請將 URL 保持在 `ws://127.0.0.1:18789` 並先開啟 SSH 隧道。
在 macOS 應用程式的 SSH 隧道傳輸中，探索到的 gateway 主機名稱應置於
...
簡而言之：除非您確定需要綁定，否則**請將 Gateway 保持僅限回環**。
...
- **Trusted-proxy** 驗證預設期望非回環的具身分感知代理設定。
  同主機回環反向代理需要明確的 `gateway.auth.trustedProxy.allowLoopback = true`。
