...
- `sshPort=<port>`（僅完整模式；在最小模式和關閉模式下省略）
- `cliPath=<path>`（僅完整模式；在最小模式和關閉模式下省略）
...
啟用後，Bonjour 使用 `discovery.mdns.mode` 來決定發布多少 TXT 元數據。相同的模式控制廣域 DNS-SD 記錄中的可選 TXT 提示。預設模式是 `minimal`；僅當客戶端需要 `cliPath` 或 `sshPort` 提示時才使用 `full`。使用 `off` 可在不更改外掛程式啟用狀態的情況下抑制 LAN 多播；當 `discovery.wideArea.enabled` 為 true 時，廣域 DNS-SD 仍可發布最小的 Gateway 信標。
...
當 `OPENCLAW_DISABLE_BONJOUR` 未設定時，內建的 Bonjour 外掛程式會自動停用偵測到的容器中的 LAN 多播廣告。Docker 橋接網路通常不會在容器和 LAN 之間轉發 mDNS 多播（`224.0.0.251:5353`），因此來自容器的廣告很少能讓探索正常運作。

重要注意事項：

- Bonjour 在 macOS 主機上會自動啟動，在其他地方則為選用。將其
  保持停用並不會停止 Gateway；它只會跳過 LAN 多點傳播廣告。
- 停用 Bonjour 不會改變 `gateway.bind`；Docker 預設仍為 `OPENCLAW_GATEWAY_BIND=lan`，因此發布的主機連接埠可以正常運作。
- 停用 Bonjour 不會停用廣域 DNS-SD。當 Gateway 和節點不在同一個 LAN 上時，請使用廣域探索
  或 Tailnet。
- 在 Docker 外重複使用相同的 `OPENCLAW_CONFIG_DIR` 不會保留容器自動停用策略。
- 僅對主機網路、macvlan 或其他已知可傳遞 mDNS 多播的網路設定 `OPENCLAW_DISABLE_BONJOUR=0`；將其設定為 `1` 以強制停用。
...
3. 當 Bonjour 停用時使用直接目標：
   ...
   - LAN 客戶端：`http://<gateway-host>:18789`
...
4. 如果您在 Docker 中故意啟用了 Bonjour 外掛程式並使用 `OPENCLAW_DISABLE_BONJOUR=0` 強制廣告，請從主機測試多播：
   ...
   如果瀏覽結果為空或 Gateway 日誌顯示重複的 ciao 看門狗取消，請還原 `OPENCLAW_DISABLE_BONJOUR=1` 並使用直接連線或 Tailnet 路由。
...
- **Docker 橋接網路**：Bonjour 會在偵測到的容器中自動停用。僅對主機、macvlan 或其他支援 mDNS 的網路設定 `OPENCLAW_DISABLE_BONJOUR=0`。
- **休眠 / 介面變動**：macOS 可能會暫時遺漏 mDNS 結果；請重試。
- **瀏覽正常但解析失敗**：請保持機器名稱簡單（避免表情符號或標點符號），然後重新啟動 Gateway。服務實例名稱源自主機名稱，因此過於複雜的名稱可能會讓某些解析器感到困惑。

## 跳脫的實例名稱（`\032`）

Bonjour/DNS-SD 通常會在服務實例名稱中將位元組轉義為十進位 `\DDD` 序列（例如，空格會變成 `\032`）。
...
- `openclaw plugins enable bonjour` 會在未預設啟用內建 LAN 探索外掛程式的主機上啟用它。
- `openclaw plugins disable bonjour` 會透過停用內建外掛程式來停用 LAN 多點傳播廣告。
- `OPENCLAW_DISABLE_BONJOUR=1` 會在不變更外掛程式設定的情況下停用 LAN 多點傳播廣告；接受的真值為 `1`、`true`、`yes` 和 `on`（舊版：`OPENCLAW_DISABLE_BONJOUR`）。
- `OPENCLAW_DISABLE_BONJOUR=0` 會強制開啟 LAN 多點傳播廣告，包括在偵測到的容器內；接受的假值為 `0`、`false`、`no` 和 `off`。
- 當 Bonjour 外掛程式已啟用且未設定 `OPENCLAW_DISABLE_BONJOUR` 時，Bonjour 會在一般主機上進行廣告，並在偵測到的容器內自動停用。
...
- 探索原則與傳輸選取：[Discovery](/zh-Hant/gateway/discovery)
