...
**本頁內容：** [信任模型](#scope-first-personal-assistant-security-model) | [快速稽核](#quick-check-openclaw-security-audit) | [強化基準](#hardened-baseline-in-60-seconds) | [DM 存取模型](#dm-access-model-pairing--allowlist--open--disabled) | [設定強化](#configuration-hardening-examples) | [事件回應](#incident-response)
...
另請參閱：[形式化驗證（安全模型）](/en/security/formal-verification)
...
`security audit --fix` 保持刻意的狹窄：它將常見的開放群組原則翻轉為允許清單，還原 `logging.redactSensitive: "tools"`，收緊狀態/設定/包含檔案的權限，並且在 Windows 上執行時使用 Windows ACL 重設來代替 POSIX `chmod`。
...
- 如果某人可以修改 Gateway 主機狀態/設定（`~/.openclaw`，包括 `openclaw.json`），請將其視為可信賴的操作者。
...
- 會話識別碼（`sessionKey`、會話 ID、標籤）是路由選擇器，而不是授權權杖。
...
- **閘道**是控制平面與原則表面（`gateway.auth`、工具原則、路由）。
- **節點**是與該閘道配對的遠端執行表面（指令、裝置操作、主機本機功能）。
- 經過閘道驗證的呼叫者在閘道範圍內受信任。配對後，節點操作即為該節點上受信任的操作員操作。
- `sessionKey` 是路由/內文選擇，而非每個使用者驗證。
- 執行核准（允許清單 + 詢問）是操作員意圖的防護措施，而非惡意多租戶隔離。
- 對於受信任的單一操作員設置，OpenClaw 的產品預設是在 `gateway`/`node` 上允許執行主機指令而無需批准提示（`security="full"`，`ask="off"`，除非您收緊設置）。該預設是有意設計的使用者體驗，本身並非漏洞。
...
| 邊界或控制                                      | 含義                         | 常見誤讀                                         |
| ----------------------------------------------- | ---------------------------- | ------------------------------------------------ |
| `gateway.auth`（令牌/密碼/受信任代理/裝置驗證） | 向閘道 API 驗證呼叫者        | 「需要在每個幀上進行每條訊息簽名才能保證安全」   |
| `sessionKey`                                    | 用於上下文/會話選擇的路由鍵  | 「會話鍵是用戶驗證邊界」                         |
| 提示/內容防護機制                               | 降低模型濫用風險             | 「僅提示注入證明驗證繞過」                       |
| `canvas.eval` / 瀏覽器評估                      | 啟用時的有意操作員功能       | 「在此信任模型中，任何 JS 評估原語自動構成漏洞」 |
| 本地 TUI `!` shell                              | 操作員顯式觸發的本地執行     | 「本地 shell 便捷命令是遠端注入」                |
...
- 將正常操作員讀取路徑存取（例如 `sessions.list`/`sessions.preview`/`chat.history`）歸類為共享閘道設置中的 IDOR 的主張。
- 僅限本地主機的部署發現（例如僅環回閘道上的 HSTS）。
- 針對此存儲庫中不存在的入站路徑，有關 Discord 入站 Webhook 簽章的研究發現。
- 當真正的執行邊界仍然是 Gateway 的全域節點指令政策加上節點自身的執行核准時，將節點配對元數據視為 `system.run` 的隱藏第二層每個指令核准層的報告。
- 將 `sessionKey` 視為認證令牌的「缺少每個使用者授權」研究發現。

## 研究人員預檢查清單

在開啟 GHSA 之前，請驗證所有以下事項：

1. 複製步驟在最新的 `main` 或最新版本上仍然有效。
2. 報告包含確切的路徑（`file`、函數、行範圍）和測試過的版本/提交。
3. 影響範圍跨越了記錄在案的信任邊界（不僅僅是提示詞注入）。
4. 該聲明未列在 [範圍之外](https://github.com/openclaw/openclaw/blob/main/SECURITY.md#out-of-scope) 中。
...
- 設定 `session.dmScope: "per-channel-peer"`（或是多帳號頻道的 `"per-account-channel-peer"`）。
...
允許清單控制觸發和指令授權。`contextVisibility` 設定控制如何過濾補充情境（引用回覆、執行緒根目錄、擷取的歷史記錄）：

- `contextVisibility: "all"` (預設) 保留接收到的補充上下文。
- `contextVisibility: "allowlist"` 根據有效的允許清單檢查，過濾補充上下文至被允許的傳送者。
- `contextVisibility: "allowlist_quote"` 的行為類似於 `allowlist`，但仍保留一個明確的引用回覆。

針對每個頻道或每個房間/對話設定 `contextVisibility`。有關設定詳情，請參閱 [群組聊天](/en/channels/groups#context-visibility)。

諮詢分類指導：

- 僅顯示「模型可以查看來自非允許清單傳送者的引用或歷史文字」的主張，是可透過 `contextVisibility` 解決的加固發現，而非單獨構成身份驗證或沙箱邊界繞過。
...
- **執行審批漂移**（`security=full`、`autoAllowSkills`、沒有 `strictInlineEval` 的直譯器允許清單）：主機執行防護機制是否仍在按預期運作？
  - `security="full"` 是一個廣泛的姿態警告，而非錯誤的證明。它是受信任的個人助理設定所選的預設值；僅當您的威脅模型需要審批或允許清單防護機制時才應收緊。
- **網路暴露**（Gateway 繫結/身份驗證、Tailscale Serve/Funnel、弱/短身份驗證權杖）。
- **瀏覽器控制暴露**（遠端節點、中繼連接埠、遠端 CDP 端點）。
- **本機磁碟衛生**（權限、符號連結、設定包含、「已同步資料夾」路徑）。
- **外掛程式**（擴充功能存在但無明確允許清單）。
- **原則漂移/錯誤配置**（已配置沙箱 docker 設定但沙箱模式關閉；無效的 `gateway.nodes.denyCommands` 模式，因為比對僅限於精確的指令名稱（例如 `system.run`）且不檢查 shell 文字；危險的 `gateway.nodes.allowCommands` 項目；全域 `tools.profile="minimal"` 被每個代理的設定檔覆寫；在寬鬆的工具原則下可存取的外掛程式工具）。
- **執行時期預期漂移**（例如假設隱含執行仍代表 `sandbox`，但當 `tools.exec.host` 現在預設為 `auto` 時，或在沙箱模式關閉時明確設定 `tools.exec.host="sandbox"`）。
...
您在實際部署中極有可能看到的高信號 `checkId` 值（非詳盡列表）：
...
| `fs.config_include.perms_writable`                            | 關鍵          | 配置包含檔案可被其他人修改                                                | 從 `openclaw.json` 參照的包含檔案權限                                                                | 是       |
| `fs.config_include.perms_group_readable`                      | 警告          | 群組使用者可以讀取包含的機密/設定                                         | 從 `openclaw.json` 參照的包含檔案權限                                                                | 是       |
| `fs.config_include.perms_world_readable`                      | 關鍵          | 包含的機密/設定可被任何人讀取                                             | 從 `openclaw.json` 參照的包含檔案權限                                                                | 是       |
...
| `gateway.http.no_auth`                                        | 警告/嚴重     | 可透過 `auth.mode="none"` 存取的閘道 HTTP API                             | `gateway.auth.mode`, `gateway.http.endpoints.*`                                                      | 否       |
...
| `gateway.real_ip_fallback_enabled`                            | warn/critical | 信任 `X-Real-IP` 後援可能會透過 Proxy 設定錯誤啟用來源 IP 偽造            | `gateway.allowRealIpFallback`, `gateway.trustedProxies`                                              | 否       |
...
| `discovery.mdns_full_mode`                                    | warn/critical | mDNS 完整模式會在本機網路上廣播 `cliPath`/`sshPort` 中繼資料              | `discovery.mdns.mode`, `gateway.bind`                                                                | no       |
...
| `hooks.path_root`                                             | 嚴重          | Hook 路徑是 `/`，使得進入更容易發生衝突或路由錯誤                         | `hooks.path`                                                                                         | 否       |
...
| `browser.remote_cdp_http`                                     | 警告          | 透過純 HTTP 的遠端 CDP 缺乏傳輸加密                                       | browser profile `cdpUrl`                                                                             | 否       |
| `browser.remote_cdp_private_host`                             | 警告          | 遠端 CDP 以私人/內部主機為目標                                            | browser profile `cdpUrl`, `browser.ssrfPolicy.*`                                                     | 否       |
...
| `tools.exec.host_sandbox_no_sandbox_defaults`                 | warn          | 當 sandbox 關閉時，`exec host=sandbox` 失敗時會封鎖                       | `tools.exec.host`, `agents.defaults.sandbox.mode`                                                    | no       |
| `tools.exec.host_sandbox_no_sandbox_agents`                   | warn          | 當 sandbox 關閉時，每個 agent 的 `exec host=sandbox` 失敗時會封鎖         | `agents.list[].tools.exec.host`, `agents.list[].sandbox.mode`                                        | no       |
...
| `tools.exec.safe_bins_interpreter_unprofiled`                 | 警告          | `safeBins` 中沒有明確設定檔的解譯器/執行時 bins 會擴大執行風險            | `tools.exec.safeBins`，`tools.exec.safeBinProfiles`，`agents.list[].tools.exec.*`                    | 否       |
...
| `skills.workspace.symlink_escape`                             | 警告          | 工作區 `skills/**/SKILL.md` 解析至工作區根目錄之外（符號連結鏈漂移）      | 工作區 `skills/**` 檔案系統狀態                                                                      | 否       |
...
| `plugins.code_safety.entry_path`                              | 警告          | 外掛進入路徑指向隱藏或 `node_modules` 位置                                | 外掛清單 `entry`                                                                                     | 否       |
| `plugins.code_safety.entry_escape`                            | 嚴重          | 外掛進入點逃脫外掛目錄                                                    | 外掛清單 `entry`                                                                                     | 否       |
...
會完全停用裝置身分識別檢查。這是嚴重的安全性降級；
除非您正在主動除錯且能夠快速還原，否則請保持關閉。

與那些危險的旗標分開來看，成功的 `gateway.auth.mode: "trusted-proxy"`
可以在沒有裝置身分識別的情況下接受 **操作員** 控制階層 UI 工作階段。這是一種
故意的驗證模式行為，而不是 `allowInsecureAuth` 快捷方式，且它
仍然不適用於節點角色控制階層 UI 工作階段。

當啟用此設定時，`openclaw security audit` 會發出警告。

## 不安全或危險旗標摘要

當啟用已知的不安全/危險除錯開關時，`openclaw security audit` 包含 `config.insecure_or_dangerous_flags`。該檢查目前匯總：
...
- `channels.synology-chat.dangerouslyAllowInheritedWebhookPath` (擴充功能頻道)
- `channels.zalouser.dangerouslyAllowNameMatching` (擴充功能頻道)
- `channels.zalouser.accounts.<accountId>.dangerouslyAllowNameMatching` (擴充功能頻道)
...
如果您在反向代理（如 nginx、Caddy、Traefik 等）後執行 Gateway，請設定
`gateway.trustedProxies` 以正確處理轉發的用戶端 IP。

當 Gateway 從**不**在 `trustedProxies` 中的位址偵測到代理標頭時，它將**不**會將連線視為本地用戶端。如果停用了 Gateway 驗證，這些連線將被拒絕。這可防止驗證繞過，否則經過代理的連線看起來像是來自 localhost 並獲得自動信任。

`gateway.trustedProxies` 也會提供給 `gateway.auth.mode: "trusted-proxy"`，但該驗證模式更嚴格：

- trusted-proxy 驗證會**在回環來源代理上失敗關閉**
- 相同主機的回環反向代理仍可使用 `gateway.trustedProxies` 進行本地用戶端偵測和轉發 IP 處理
- 對於相同主機的回環反向代理，請使用 token/password 驗證，而不是 `gateway.auth.mode: "trusted-proxy"`
...
當設定 `trustedProxies` 時，Gateway 使用 `X-Forwarded-For` 來判斷用戶端 IP。除非明確設定 `gateway.allowRealIpFallback: true`，否則 `X-Real-IP` 預設會被忽略。
...
- 詳細的部署指引請參閱 [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth#tls-termination-and-hsts)。
- 對於非回環 Control UI 部署，預設需要 `gateway.controlUi.allowedOrigins`。
- `gateway.controlUi.allowedOrigins: ["*"]` 是一種明確的允許所有瀏覽器來源策略，而非經過強化的預設設定。在嚴密控制的本地測試之外，請避免使用它。
- 即使在啟用了一般 loopback 豁免的情況下，loopback 上的瀏覽器來源驗證失敗仍會受到速率限制，但鎖定金鑰的範圍是依據標準化 `Origin` 值，而不是一個共用的 localhost bucket。
- `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` 啟用 Host 標頭來源回退模式；請將其視為一種危險的操作員選擇策略。
- 請將 DNS 重新綁定 和 proxy-host 標頭行為視為部署強化問題；保持 `trustedProxies` 嚴密，並避免將 gateway 直接暴露於公用網際網路。

## 本地工作階段日誌儲存在磁碟上

OpenClaw 會將工作階段逐字稿儲存在磁碟上的 `~/.openclaw/agents/<agentId>/sessions/*.jsonl` 下。
這是工作階段連續性所必需的（並可選地用於工作階段記憶體索引），但這也意味著
**任何具有檔案系統存取權的程序/使用者都可以讀取這些日誌**。請將磁碟存取視為信任
邊界，並鎖定 `~/.openclaw` 的權限（請參見下方的稽核章節）。如果您需要
代理之間更強的隔離，請在不同的 OS 使用者或不同的主機上執行它們。
...
- Gateway 透過 `gateway.nodes.allowCommands` / `denyCommands` 套用粗糙的全域節點指令策略。
- 在 Mac 上透過 **Settings → Exec approvals** (security + ask + allowlist) 進行控制。
- 每個節點的 `system.run` 策略是節點自己的執行核准檔案 (`exec.approvals.node.*`)，其嚴格程度可以高於或低於 gateway 的全域指令 ID 策略。
...
- 對於 `host=node`，基於批准的執行也會儲存一個規範的準備 `systemRunPlan`；稍後批准的轉發會重用該儲存的計畫，且閘道驗證會在建立批准請求後拒絕呼叫者對 command/cwd/session 上下文的編輯。
...
- **Skills watcher**：對 `SKILL.md` 的變更可以在下一個代理回合更新技能快照。
...
斜線指令和指令僅對 **已授權的發送者** 有效。授權源自
頻道允許清單/配對以及 `commands.useAccessGroups`（請參閱[組態](/en/gateway/configuration)
和[斜線指令](/en/tools/slash-commands)）。如果頻道允許清單為空白或包含 `"*"`，
則該頻道的指令實際上處於開放狀態。

`/exec` 是供已授權操作員使用的僅限會話便利功能。它**不**會寫入組態或
變更其他工作階段。
...
`commands.restart=false` 僅封鎖重新啟動動作。它不會停用 `gateway` 組態/更新動作。
...
- 如果您安裝或更新外掛程式（`openclaw plugins install <package>`、`openclaw plugins update <id>`），請將其視為執行不受信任的程式碼：
  ...
  - OpenClaw 會在安裝/更新前執行內建的危險程式碼掃描。`critical` 發現結果預設會遭到封鎖。
  - OpenClaw 使用 `npm pack`，然後在該目錄中執行 `npm install --omit=dev`（npm 生命週期腳本可以在安裝期間執行程式碼）。
  - 建議優先使用鎖定的確切版本（`@scope/pkg@1.2.3`），並在啟用前檢查磁碟上解壓縮後的程式碼。
  - `--dangerously-force-unsafe-install` 僅用於外掛程式安裝/更新流程中內建掃描誤報的緊急情況。它不會繞過外掛程式 `before_install` 掛鉤策略阻斷，也不會繞過掃描失敗。
  - Gateway 支援的技能依賴安裝遵循相同的危險/可疑區分：內建 `critical` 發現會進行阻斷，除非呼叫者明確設定 `dangerouslyForceUnsafeInstall`，而可疑發現仍僅發出警告。`openclaw skills install` 保持為單獨的 ClawHub 技能下載/安裝流程。

詳細資訊：[外掛程式](/en/tools/plugin)

## DM 存取模型（配對 / 允許清單 / 開放 / 停用）

所有目前支援 DM 的頻道都支援 DM 策略（`dmPolicy` 或 `*.dm.policy`），該策略會在訊息處理**之前**對傳入的 DM 進行把關：

- `pairing`（預設值）：未知發送者會收到一個簡短的配對碼，且機器人會忽略其訊息直到獲得批准。配對碼會在 1 小時後過期；重複的 DM 在建立新請求之前不會重新發送配對碼。預設情況下，待處理請求每個頻道上限為 **3 個**。
- `allowlist`：未知發送者會被封鎖（無配對握手）。
- `open`：允許任何人傳送 DM（公開）。**必須**要求頻道允許清單包含 `"*"`（明確選擇加入）。
- `disabled`：完全忽略傳入的 DM。
...
根據預設，OpenClaw 會將**所有 DM 路由到主工作階段**，以便您的助理在裝置和頻道之間保持連續性。如果**多個人**可以向機器人發送 DM（開放 DM 或多人允許清單），請考慮隔離 DM 工作階段：
...
這可以防止跨用戶上下文洩漏，同時保持群組聊天隔離。

這是一個訊息上下文邊界，而非主機管理員邊界。如果使用者彼此間存在惡意關係且共用同一個 Gateway 主機/設定，請改為針對每個信任邊界執行獨立的 gateway。

### 安全 DM 模式（建議）

將上述程式碼片段視為**安全 DM 模式**：

- 預設值：`session.dmScope: "main"`（所有 DM 共用同一個工作階段以保持連續性）。
- 本機 CLI 設定預設值：若未設定則寫入 `session.dmScope: "per-channel-peer"`（保留現有的明確值）。
- 安全 DM 模式：`session.dmScope: "per-channel-peer"`（每個頻道+傳送者配對都會獲得獨立的 DM 上下文）。
- 跨頻道對等隔離：`session.dmScope: "per-peer"`（每個傳送者在所有相同類型的頻道中共用一個工作階段）。

如果您在同一個頻道上執行多個帳戶，請改用 `per-account-channel-peer`。如果同一個人透過多個頻道聯絡您，請使用 `session.identityLinks` 將這些 DM 工作階段合併為一個標準身分。請參閱[工作階段管理](/en/concepts/session)與[設定](/en/gateway/configuration)。
...
- **DM 允許清單**（`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`；舊版：`channels.discord.dm.allowFrom`、`channels.slack.dm.allowFrom`）：允許誰在直接訊息中與機器人交談。
  - 當 `dmPolicy="pairing"` 時，核准會被寫入 `~/.openclaw/credentials/` 下的帳戶範圍配對允許清單儲存庫（預設帳戶為 `<channel>-allowFrom.json`，非預設帳戶為 `<channel>-<accountId>-allowFrom.json`），並與設定允許清單合併。
- **群組允許清單**（特定頻道）：機器人完全接受訊息的來源群組/頻道/伺服器。
  - 常見模式：
    - `channels.whatsapp.groups`、`channels.telegram.groups`、`channels.imessage.groups`：每個群組的預設值，例如 `requireMention`；設定後，它也充當群組允許清單（包含 `"*"` 以保持允許所有行為）。
    - `groupPolicy="allowlist"` + `groupAllowFrom`：限制誰可以在群組會議（WhatsApp/Telegram/Signal/iMessage/Microsoft Teams）中觸發機器人。
    - `channels.discord.guilds` / `channels.slack.channels`：針對每個介面的允許清單 + 提及預設值。
  - 群組檢查按此順序執行：首先是 `groupPolicy`/群組允許清單，其次是提及/回覆啟動。
  - 回覆機器人訊息（隱含提及）並**不會**繞過發送者允許清單，例如 `groupAllowFrom`。
  - **安全提示：**將 `dmPolicy="open"` 和 `groupPolicy="open"` 視為最後手段的設定。應盡量少用它們；除非您完全信任房間內的每個成員，否則優先使用配對 + 允許清單。

詳細資訊：[設定](/en/gateway/configuration) 和 [群組](/en/channels/groups)

## 提示注入（它是什麼，為什麼它很重要）

提示注入是指攻擊者精心製作一條訊息，操縱模型執行不安全的操作（「忽略你的指令」、「傾印你的檔案系統」、「跟隨此連結並執行命令」等）。

即使有強大的系統提示，**提示注入尚未解決**。系統提示防護僅是軟性指導；硬性強制來自工具原則、執行核准、沙盒和通道允許清單（且操作員可透過設計停用這些功能）。實務上有幫助的做法：

- 保持傳入的 DM 鎖定（配對/允許清單）。
- 在群組中優先使用提及閘門；避免在公開房間中使用「常開」機器人。
- 預設將連結、附件和貼上的指令視為惡意。
- 在沙盒中執行敏感的工具執行；將機密保持在代理可存取的檔案系統之外。
- 注意：沙盒是選用的。如果沙盒模式關閉，隱含的 `host=auto` 會解析為閘道主機。明確的 `host=sandbox` 仍會以失敗關閉，因為沒有可用的沙盒執行環境。如果您希望該行為在設定中明確顯示，請設定 `host=gateway`。
- 將高風險工具（`exec`、`browser`、`web_fetch`、`web_search`）限制在受信任的代理或明確的允許清單中。
- 如果您允許清單中的直譯器 (`python`、`node`、`ruby`、`perl`、`php`、`lua`、`osascript`)，請啟用 `tools.exec.strictInlineEval`，這樣內聯 eval 表單仍需要明確批准。
- **模型選擇很重要：** 舊型/較小/舊版模型對於提示詞注入和工具誤用的防禦能力顯著較弱。對於啟用工具的代理，請使用可用的最強最新世代、經指令強化的模型。
...
- Hook 負載是不可信的內容，即使傳送來自於您控制的系統 (郵件/文件/網頁內容可能攜帶提示詞注入)。
- 較弱的模型層級會增加此風險。對於 Hook 驅動的自動化，建議使用強大的現代模型層級，並保持工具政策嚴格 (`tools.profile: "messaging"` 或更嚴格)，並在可能的情況下進行沙盒化。

### 提示詞注入不需要公開 DM

即使只有**您**能傳送訊息給機器人，透過機器人讀取的任何**不可信內容** (網頁搜尋/擷取結果、瀏覽器頁面、
電子郵件、文件、附件、貼上的日誌/程式碼)，提示詞注入仍可能發生。換句話說：傳送者並不是
唯一的威脅面；**內容本身**可能攜帶惡意指令。

當啟用工具時，典型的風險是洩漏上下文或觸發
工具呼叫。您可以透過以下方式減少爆炸半徑：

- 使用唯讀或停用工具的 **reader agent** 來總結不可信內容，
  然後將摘要傳遞給您的主要代理。
- 對於啟用工具的代理，除非必要，否則請保持 `web_search` / `web_fetch` / `browser` 關閉。
...
- 對於 OpenResponses 檔案輸入，解碼後的 `input_file` 文字仍會被當作
  **不受信任的外部內容** 注入。不要僅因為 Gateway 在本機解碼了該檔案，就依賴該檔案文字是受信任的。注入的區塊仍帶有明確的
  `<<<EXTERNAL_UNTRUSTED_CONTENT ...>>>` 邊界標記以及 `Source: External`
  中繼資料，儘管此路徑省略了較長的 `SECURITY NOTICE:` 橫幅。
- 當媒體理解 從附加的文件中提取文字並將其附加至媒體提示時，會套用相同的基於標記的包裝方式。
- 為任何接觸不受信任輸入的代理啟用沙盒機制和嚴格的工具允許清單。
- 不要在提示中包含機密資訊；請改透過 Gateway 主機上的 env/config 傳遞它們。

### 模型強度 (安全性備註)

對提示注入 的抵禦能力在各模型等級中並**不**統一。較小/較便宜的模型通常更容易受到工具濫用和指令劫持，特別是在對抗性提示之下。

<Warning>對於啟用工具的代理或讀取不受信任內容的代理，使用較舊/較小模型時的提示注入風險通常過高。請勿在弱勢模型等級上執行這些工作負載。</Warning>

建議：

- 對於任何可以執行工具或接觸檔案/網路的機器人，**請使用最新世代、最高等級的模型**。
- **請勿對啟用工具的代理或不受信任的收件匣使用較舊/較弱/較小的等級**；提示注入風險過高。
- 如果您必須使用較小的模型，**請縮小爆炸半徑** (唯讀工具、強式沙盒、最小檔案系統存取權、嚴格的允許清單)。
- 執行小型模型時，**請為所有工作階段啟用沙盒**，並**停用 web_search/web_fetch/browser**，除非輸入受到嚴格控制。
- 對於僅用於聊天、具有可信輸入且不使用工具的個人助理，通常使用較小的模型即可。
...
在群組設定中，請將其視為僅供 **除錯** 使用，除非明確需要，否則請將其關閉。

指引：

- 在公開群組中保持 `/reasoning` 和 `/verbose` 為停用狀態。
- 如果您啟用它們，請僅在受信任的私人訊息 (DM) 或嚴格控制的房間中進行。
- 請注意：詳細輸出可能包含工具參數、URL 以及模型看到的資料。
...
- 除非您完全了解其影響，否則不要讓 canvas 內容與具有特權的網頁介面共用相同的來源 (origin)。

綁定模式 控制 Gateway 的監聽位置：

- `gateway.bind: "loopback"` (預設值)：僅限本機用戶端可以連接。
- 非 loopback 綁定（`"lan"`、`"tailnet"`、`"custom"`）會擴大攻擊面。僅在具備 Gateway 驗證（共用 token/密碼或正確設定的非 loopback 受信任 proxy）以及實體防火牆的情況下使用它們。

經驗法則：

- 優先使用 Tailscale Serve 而非 LAN 綁定（Serve 將 Gateway 保持在 loopback，並由 Tailscale 處理存取）。
- 如果必須綁定到 LAN，請將該埠的防火牆設定為僅允許來源 IP 的嚴格白名單；切勿廣泛進行埠轉送。
...
避免在文件片段中將介面名稱（如 `eth0`）硬編碼。介面名稱
因 VPS 映像檔而異（`ens3`、`enp*` 等），且不相符可能會意外
略過您的拒絕規則。
...
- `cliPath`：CLI 二進位檔案的完整檔案系統路徑（揭露使用者名稱和安裝位置）
- `sshPort`：公告主機上的 SSH 可用性
- `displayName`、`lanHost`：主機名資訊

**營運安全考量：** 廣播基礎設施細節會讓區域網路上的任何人更容易進行偵察。即使是像檔案系統路徑和 SSH 可用性這種「無害」資訊，也能幫助攻擊者繪製您的環境地圖。

**建議：**

1. **最簡模式**（預設，建議用於暴露的閘道）：從 mDNS 廣播中省略敏感欄位：
...
在最簡模式下，閘道仍然會廣播足以進行裝置探索的資訊（`role`、`gatewayPort`、`transport`），但會省略 `cliPath` 和 `sshPort`。需要 CLI 路徑資訊的應用程式可以透過已驗證的 WebSocket 連線來取得。

### 0.5) 鎖定閘道 WebSocket（本機驗證）

閘道驗證**預設為必填**。如果未設定有效的閘道驗證路徑，
閘道將拒絕 WebSocket 連線（失效安全/fail‑closed）。

入職流程預設會產生一個權杖（即使是回環），因此
本機用戶端必須進行驗證。

設定一個權杖，以便**所有** WS 用戶端都必須進行驗證：
...
注意：`gateway.remote.token` / `.password` 是用戶端憑證來源。它們本身**並不**保護本機 WS 存取。
本機呼叫路徑僅在未設定 `gateway.auth.*` 時，才能將 `gateway.remote.*` 作為後備。
如果透過 SecretRef 明確配置了 `gateway.auth.token` / `gateway.auth.password` 但未解析，解析將會失敗關閉（不會有遠端後備遮罩）。
選項：使用 `wss://` 時，使用 `gateway.remote.tlsFingerprint` 釘選遠端 TLS。
純文字 `ws://` 預設僅限回送。對於受信任的私人網路路徑，請在用戶端程序上設定 `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` 作為緊急存取。

本機裝置配對：

- 為了保持同主機用戶端的順暢，直接本機回送連線的裝置配對會自動核准。
- OpenClaw 也有一個狹窄的後端/容器本機自我連線路徑，用於受信任的共用金鑰協助流程。
- Tailnet 和 LAN 連線（包括同主機 tailnet 繫結）在配對時被視為遠端，仍需核准。

驗證模式：

- `gateway.auth.mode: "token"`：共用 bearer token（建議大多數設定使用）。
- `gateway.auth.mode: "password"`：密碼驗證（建議透過 env 設定：`OPENCLAW_GATEWAY_PASSWORD`）。
- `gateway.auth.mode: "trusted-proxy"`：信任具備身分感知能力的反向 Proxy 來驗證使用者並透過標頭傳遞身分（請參閱[受信任的 Proxy 驗證](/en/gateway/trusted-proxy-auth)）。

輪換檢查清單（token/密碼）：

1. 產生/設定新的密鑰（`gateway.auth.token` 或 `OPENCLAW_GATEWAY_PASSWORD`）。
2. 重新啟動 Gateway（如果 Gateway 由 macOS app 監督，則重新啟動該 app）。
3. 更新任何遠端用戶端（呼叫 Gateway 的機器上的 `gateway.remote.token` / `.password`）。
4. 驗證您無法再使用舊憑證連線。

### 0.6) Tailscale Serve 身分標頭

當 `gateway.auth.allowTailscale` 為 `true`（Serve 的預設值）時，OpenClaw 會接受 Tailscale Serve 身分標頭（`tailscale-user-login`）作為 Control UI/WebSocket 驗證。OpenClaw 透過本機 Tailscale 守護程式（`tailscale whois`）解析 `x-forwarded-for` 位址，並將其與標頭進行比對以驗證身分。這僅針對命中 loopback 且包含由 Tailscale 注入的 `x-forwarded-for`、`x-forwarded-proto` 和 `x-forwarded-host` 的請求觸發。
對於此非同步身分檢查路徑，限制器記錄失敗之前，相同 `{scope, ip}` 的失敗嘗試會被序列化。因此，來自一個 Serve 用戶端的併發錯誤重試可以立即鎖定第二次嘗試，而不是作為兩次單純的不匹配而競爭通過。
HTTP API 端點（例如 `/v1/*`、`/tools/invoke` 和 `/api/channels/*`）**不**使用 Tailscale 身分標頭驗證。它們仍遵循閘道設定的 HTTP 驗證模式。

重要的邊界注意事項：

- 閘道 HTTP bearer 驗證實際上就是全有或全無的操作員存取權。
- 請將能夠呼叫 `/v1/chat/completions`、`/v1/responses` 或 `/api/channels/*` 的憑證視為該閘道的完整存取操作員機密。
- 在 OpenAI 相容的 HTTP 層面上，共用密鑰 bearer 驗證會恢復完整的預設操作員範圍（`operator.admin`、`operator.approvals`、`operator.pairing`、`operator.read`、`operator.talk.secrets`、`operator.write`）以及代理程式輪次的擁有者語意；較窄的 `x-openclaw-scopes` 值不會減少該共用密鑰路徑。
- HTTP 上的逐請求範圍語意僅在請求來自具有身分的模式（例如受信任的代理驗證或私人入口上的 `gateway.auth.mode="none"`）時適用。
- 在那些具有身分的模式中，省略 `x-openclaw-scopes` 會回退至正常的操作員預設範圍集；當您想要較窄的範圍集時，請明確傳送標頭。
- `/tools/invoke` 遵循相同的共享密鑰規則：該處的權杖/密碼持有者驗證同樣被視為完整的操作員存取權限，而承載身分的模式仍會遵守宣告的範圍。
- 請勿與不受信任的呼叫者共享這些憑證；建議每個信任邊界使用個別的閘道。

**信任假設：** 無權杖的 Serve 驗證假設閘道主機是受信任的。
請勿將此視為防範同主機惡意程序的保護措施。如果在閘道主機上可能執行
不受信任的本機程式碼，請停用 `gateway.auth.allowTailscale`
並要求使用 `gateway.auth.mode: "token"` 或
`"password"` 進行明確的共享密鑰驗證。

**安全規則：** 請勿從您自己的反向代理伺服器轉送這些標頭。如果您在閘道之前
終止 TLS 或進行代理，請停用
`gateway.auth.allowTailscale` 並改用共享密鑰驗證（`gateway.auth.mode:
"token"` or `"password"`）或 [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth)。

受信任的代理伺服器：

- 如果您在 Gateway 之前終止 TLS，請將 `gateway.trustedProxies` 設為您的代理伺服器 IP。
- OpenClaw 將信任來自這些 IP 的 `x-forwarded-for`（或 `x-real-ip`），以決定用於本機配對檢查和 HTTP 驗證/本機檢查的用戶端 IP。
- 請確保您的代理伺服器**覆寫** `x-forwarded-for` 並阻擋對 Gateway 連接埠的直接存取。

請參閱 [Tailscale](/en/gateway/tailscale) 和 [Web 概觀](/en/web)。

### 0.6.1) 透過節點主機控制瀏覽器（建議）

如果您的 Gateway 是遠端的，但瀏覽器在另一台機器上執行，請在瀏覽器機器上執行 **node host**
並讓 Gateway 代理瀏覽器動作（請參閱 [Browser 工具](/en/tools/browser)）。
將節點配對視為管理員存取權。
...
- `openclaw.json`：設定可能包含令牌（gateway、remote gateway）、提供者設定和允許清單。
- `credentials/**`：通道憑證（例如：WhatsApp 憑證）、配對允許清單、舊版 OAuth 匯入。
- `agents/<agentId>/agent/auth-profiles.json`：API 金鑰、令牌設定檔、OAuth 令牌，以及可選的 `keyRef`/`tokenRef`。
- `secrets.json`（可選）：由 `file` SecretRef 提供者（`secrets.providers`）使用的檔案支援機密負載。
- `agents/<agentId>/agent/auth.json`：舊版相容性檔案。靜態 `api_key` 條目在發現時會被清除。
- `agents/<agentId>/sessions/**`：會話記錄（`*.jsonl`）+ 路由元數據（`sessions.json`），可能包含私人訊息和工具輸出。
- 捆绑的插件套件：已安裝的插件（及其 `node_modules/`）。
...
- 在 Gateway 主機上使用全碟加密。
- 如果主機是共享的，請優先使用專用的 OS 使用者帳戶來執行 Gateway。

### 0.8) 日誌 + 會話記錄（編修 + 保留）

即使存取控制正確，日誌和會話記錄也可能洩露敏感資訊：

- Gateway 日誌可能包含工具摘要、錯誤和 URL。
- 會話記錄可能包含貼上的機密、檔案內容、命令輸出和連結。

建議：

- 保持工具摘要編修功能開啟（`logging.redactSensitive: "tools"`；預設值）。
- 透過 `logging.redactPatterns` 為您的環境新增自訂模式（令牌、主機名稱、內部 URL）。
- 分享診斷資訊時，優先使用 `openclaw status --all`（可貼上、已編修機密），而非原始日誌。
- 如果您不需要長期保留，請修剪舊的會話記錄和日誌檔。
...
對於基於電話號碼的頻道，請考慮使用與您的個人號碼不同的電話號碼來運行 AI：
...
- `agents.defaults.sandbox.workspaceAccess: "ro"` (或若無工作區存取權限則使用 `"none"`)
- 阻擋 `write`、`edit`、`apply_patch`、`exec`、`process` 等的工具允許/封鎖清單。

額外的加固選項：

- `tools.exec.applyPatch.workspaceOnly: true` (預設值)：確保 `apply_patch` 即使在關閉沙箱的情況下也無法在工作區目錄之外寫入/刪除。僅在您有意讓 `apply_patch` 存取工作區外的檔案時，才設定為 `false`。
- `tools.fs.workspaceOnly: true` (選用)：將 `read`/`write`/`edit`/`apply_patch` 路徑和原生提示圖片自動載入路徑限制在工作區目錄內 (如果您目前允許絕對路徑且想要單一防護措施，這會很有用)。
- 保持檔案系統根目錄狹窄：避免為代理程式工作區/沙箱工作區設定廣泛的根目錄，例如您的家目錄。廣泛的根目錄可能會將敏感的本機檔案 (例如 `~/.openclaw` 下的狀態/設定檔) 暴露給檔案系統工具。

### 5) 安全基準 (複製/貼上)

一個「安全預設」設定，可將 Gateway 保持私密、要求私訊配對，並避免常駐的群組機器人：
...
如果您也希望工具執行「預設更安全」，請為任何非擁有者的代理程式新增沙箱並封鎖危險工具 (範例請參閱下方的「每個代理程式的存取設定檔」)。

聊天驅動代理程式輪次的內建基準：非擁有者傳送者無法使用 `cron` 或 `gateway` 工具。
...
- **工具沙盒**（`agents.defaults.sandbox`，主機 Gateway + Docker 隔離的工具）：[沙盒機制](/en/gateway/sandboxing)

注意：為了防止跨 Agent 存取，請將 `agents.defaults.sandbox.scope` 維持在 `"agent"`（預設值）
或 `"session"` 以進行更嚴格的每個會話隔離。`scope: "shared"` 使用單一容器/工作區。

同時也請考慮沙盒內的 Agent 工作區存取權：

- `agents.defaults.sandbox.workspaceAccess: "none"`（預設值）會將 Agent 工作區設為禁止存取；工具會在 `~/.openclaw/sandboxes` 下的沙盒工作區執行
- `agents.defaults.sandbox.workspaceAccess: "ro"` 會將 Agent 工作區以唯讀方式掛載至 `/agent`（停用 `write`/`edit`/`apply_patch`）
- `agents.defaults.sandbox.workspaceAccess: "rw"` 會將 Agent 工作區以讀寫方式掛載至 `/workspace`
- 額外的 `sandbox.docker.binds` 會針對正規化和標準化的來源路徑進行驗證。如果解析結果導向被封鎖的根目錄，例如 `/etc`、`/var/run` 或 OS 主目錄下的憑證目錄，父目錄符號連結技巧和標準主目錄別名仍會失敗封閉。

重要提示：`tools.elevated` 是在沙盒外執行 exec 的全域基準逃逸通道。預設的有效主機是 `gateway`，當 exec 目標設定為 `node` 時則是 `node`。請嚴格管制 `tools.elevated.allowFrom`，切勿對陌生人啟用它。您可以透過 `agents.list[].tools.elevated` 針對各個 Agent 進一步限制提升權限。請參閱[提升權限模式](/en/tools/elevated)。

### 子 Agent 委派防護機制

如果您允許會話工具，請將委派的子 Agent 執行視為另一個邊界決策：

- 除非 Agent 真的需要委派，否則請拒絕 `sessions_spawn`。
- 請將 `agents.defaults.subagents.allowAgents` 以及所有針對每個代理的 `agents.list[].subagents.allowAgents` 覆寫限制在已知安全的目標代理上。
- 對於任何必須保持在沙盒內的工作流程，請使用 `sandbox: "require"` 呼叫 `sessions_spawn`（預設為 `inherit`）。
- 當目標子執行環境未處於沙盒中時，`sandbox: "require"` 會快速失敗。

## 瀏覽器控制風險

啟用瀏覽器控制會賦予模型驅動真實瀏覽器的能力。
如果該瀏覽器設定檔已包含已登入的工作階段，模型便能
存取這些帳號和資料。請將瀏覽器設定檔視為 **敏感狀態**：

- 最好為代理使用專用的設定檔（預設的 `openclaw` 設定檔）。
- 避免將代理指向您個人的日常使用設定檔。
- 除非您信任沙盒代理，否則請保持主機瀏覽器控制處於停用狀態。
- 獨立的回送瀏覽器控制 API 僅接受共享金鑰驗證
  (閘道 token bearer auth 或閘道密碼)。它不使用
  trusted-proxy 或 Tailscale Serve 身分標頭。
- 將瀏覽器下載視為不受信任的輸入；優先使用獨立的下載目錄。
- 如果可能的話，請在代理設定檔中停用瀏覽器同步/密碼管理器（以減少爆炸半徑）。
- 對於遠端閘道，請假設「瀏覽器控制」等同於對該設定檔所能存取之任何內容的「操作員存取權」。
- 請將閘道和節點主機限制為僅透過 tailnet 存取；避免將瀏覽器控制連接埠暴露給區域網路或公用網際網路。
- 當您不需要時，請停用瀏覽器代理路由（`gateway.nodes.browser.mode="off"`）。
- Chrome MCP 現有工作階段模式 **並非**「更安全」；只要該主機 Chrome 設定檔能存取的地方，它就能以您的身份行事。

### 瀏覽器 SSRF 政策（預設為信任網路）

OpenClaw 的瀏覽器網路政策預設為信任操作員模式：允許存取私人/內部目的地，除非您明確停用它們。

- 預設值：`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`（未設定時隱含此值）。
- 舊版別名：`browser.ssrfPolicy.allowPrivateNetwork` 為了相容性仍被接受。
- 嚴格模式：設定 `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: false` 以預設封鎖私人/內部/特殊用途目的地。
- 在嚴格模式下，使用 `hostnameAllowlist`（諸如 `*.example.com` 的模式）和 `allowedHostnames`（精確主機例外，包括像 `localhost` 這樣的被封鎖名稱）作為明確的例外。
- 導航會在請求前進行檢查，並在導航後對最終的 `http(s)` URL 進行盡力的再次檢查，以減少基於重新導線的轉向攻擊。
...
使用多重代理路由時，每個代理都可以有自己的沙箱 + 工具策略：
利用此功能為每個代理提供 **完全存取**、**唯讀** 或 **無存取權**。
有關詳細資訊和優先順序規則，請參閱 [多重代理沙箱與工具](/en/tools/multi-agent-sandbox-tools)。
...
3. **凍結存取：** 將有風險的私訊/群組切換到 `dmPolicy: "disabled"` / 要求提及，並移除 `"*"` 允許所有條目（如果您有的話）。

### 輪替（如果祕密洩漏則假設已遭入侵）

1. 輪替 Gateway 驗證（`gateway.auth.token` / `OPENCLAW_GATEWAY_PASSWORD`）並重新啟動。
2. 在可以呼叫 Gateway 的任何機器上輪替遠端用戶端祕密（`gateway.remote.token` / `.password`）。
3. 輪替提供者/API 憑證（WhatsApp 憑證、Slack/Discord 權杖、`auth-profiles.json` 中的模型/API 金鑰，以及使用時的加密祕密載荷值）。

### 稽核

1. 檢查 Gateway 日誌：`/tmp/openclaw/openclaw-YYYY-MM-DD.log`（或 `logging.file`）。
2. 檢閱相關的紀錄：`~/.openclaw/agents/<agentId>/sessions/*.jsonl`。
3. 檢閱最近的設定變更（任何可能擴大存取權限的項目：`gateway.bind`、`gateway.auth`、dm/group policies、`tools.elevated`、外掛程式變更）。
...
- 會話紀錄 + 簡短的日誌尾部（經過編輯後）
- 攻擊者傳送的內容 + 代理程式的動作
- Gateway 是否暴露於回環介面之外（LAN/Tailscale Funnel/Serve）

## 金鑰掃描

CI 在 `secrets` 任務中執行 `detect-secrets` pre-commit hook。
推送到 `main` 時總是會執行全檔案掃描。當有可用的基礎提交時，Pull requests 會使用變更檔案的
快速路徑，否則會回退到全檔案掃描。
如果失敗，表示有新的候選項尚未在基線中。
...
2. 了解工具：
   ...
   - `detect-secrets audit` 會開啟互動式審查，將每個基線
     項目標記為真實或誤報。
3. 對於真實的金鑰：輪替/移除它們，然後重新執行掃描以更新基線。
...
5. 如果您需要新的排除項目，請將其加入 `.detect-secrets.cfg` 並使用相符的 `--exclude-files` / `--exclude-lines` 參數重新產生
   基線（設定檔僅供參考；detect-secrets 不會自動讀取它）。

一旦 `.secrets.baseline` 反映了預期狀態，請提交更新。

## 回報安全性問題

在 OpenClaw 中發現了漏洞嗎？請負責任地回報：

1. Email: [security@openclaw.ai](mailto:security@openclaw.ai)
2. 在修復之前請勿公開發布
3. 我們會標註您的貢獻（除非您希望保持匿名）
