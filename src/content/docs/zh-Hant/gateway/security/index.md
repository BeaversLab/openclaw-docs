...
**本頁內容：** [信任模型](#scope-first-personal-assistant-security-model) | [快速審查](#quick-check-openclaw-security-audit) | [強化基準](#hardened-baseline-in-60-seconds) | [DM 存取模型](#dm-access-model-pairing-allowlist-open-disabled) | [組態強化](#configuration-hardening-examples) | [事件回應](#incident-response)
...
另請參閱：[正式驗證 (安全性模型)](/en/security/formal-verification)
...
4. 該聲明未列於 [範圍之外](https://github.com/openclaw/openclaw/blob/main/SECURITY.md#out-of-scope)。
...
針對每個頻道或每個房間/對話設定 `contextVisibility`。有關設定詳情，請參閱 [群組聊天](/en/channels/groups#context-visibility-and-allowlists)。
...
- 詳細的部署指引請參閱 [受信任的 Proxy 驗證](/en/gateway/trusted-proxy-auth#tls-termination-and-hsts)。
...
詳情 + 磁碟上的檔案：[配對](/en/channels/pairing)
...
如果您在同一頻道上執行多個帳戶，請改用 `per-account-channel-peer`。如果同一個人透過多個頻道與您聯絡，請使用 `session.identityLinks` 將這些 DM 工作階段合併為一個標準身分。請參閱 [工作階段管理](/en/concepts/session) 和 [組態](/en/gateway/configuration)。
...
詳情：[組態](/en/gateway/configuration) 和 [群組](/en/channels/groups)
...
`/reasoning`、`/verbose` 和 `/trace` 可能會公開
並不適用於公開頻道的內部推理、工具
輸出或外掛程式診斷資訊。在群組設定中，請將其視為 **僅供偵錯
使用**，除非您明確需要它們，否則請將其關閉。
...
- 預設值：`18789`
- 組態/旗標/環境變數: `gateway.port`, `--port`, `OPENCLAW_GATEWAY_PORT`
...
會因 VPS 映像檔而異（`ens3`、`enp*` 等），不匹配可能會意外
跳過您的拒絕規則。
...
閘道透過 mDNS（在 5353 連接埠上的 `_openclaw-gw._tcp`）廣播其存在，以便進行本地裝置探索。在完整模式中，這包括可能暴露操作細節的 TXT 記錄：
...
在最小化模式中，閘道仍會廣播足夠的資訊以供裝置探索（`role`、`gatewayPort`、`transport`），但會省略 `cliPath` 和 `sshPort`。需要 CLI 路徑資訊的應用程式可以改透過經過驗證的 WebSocket 連線來取得。
...
本地呼叫路徑僅當 `gateway.auth.*` 未設定時，才能將 `gateway.remote.*` 作為後備。
如果 `gateway.auth.token` / `gateway.auth.password` 是透過 SecretRef 明確設定且未解析，解析將失敗關閉（沒有遠端後備遮罩）。
選用：使用 `wss://` 時，使用 `gateway.remote.tlsFingerprint` 釘選遠端 TLS。
純文字 `ws://` 預設僅限回環。對於受信任的私人網路路徑，在客戶端程序上設定 `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` 作為緊急開關。
...
- `gateway.auth.mode: "trusted-proxy"`：信任一個具備身分感知的反向代理伺服器以驗證使用者並透過標頭傳遞身分（請參閱 [Trusted Proxy Auth](/en/gateway/trusted-proxy-auth)）。

輪換檢查清單（令牌/密碼）：

1. 產生/設定一個新的秘密金鑰（`gateway.auth.token` 或 `OPENCLAW_GATEWAY_PASSWORD`）。
...
當 `gateway.auth.allowTailscale` 為 `true`（Serve 的預設值）時，OpenClaw 接受 Tailscale Serve 身分標頭（`tailscale-user-login`）用於 Control UI/WebSocket 驗證。OpenClaw 透過本機 Tailscale 守護程式（`tailscale whois`）解析 `x-forwarded-for` 位址並與標頭比對來驗證身分。這僅對命中 loopback 且包含 Tailscale 注入的 `x-forwarded-for`、`x-forwarded-proto` 和 `x-forwarded-host` 的請求觸發。
對於此非同步身分檢查路徑，相同 `{scope, ip}` 的失敗嘗試會在限制器記錄失敗前進行序列化。因此，來自一個 Serve 用戶端的並發錯誤重試可以立即鎖定第二次嘗試，而不是像兩個單純的不匹配一樣競爭。
...
- 將可呼叫 `/v1/chat/completions`、`/v1/responses` 或 `/api/channels/*` 的憑證視為該 Gateway 的完整存權操作員秘密。
- 在 OpenAI 相容的 HTTP 表面上，共享密碼 bearer 驗證會恢復完整的預設操作員範圍（`operator.admin`、`operator.approvals`、`operator.pairing`、`operator.read`、`operator.talk.secrets`、`operator.write`）以及代理程式回合的所有者語意；較狹隘的 `x-openclaw-scopes` 值不會減少該共享密碼路徑。
- HTTP 上的每個請求範圍語義僅在請求來自具備身分的模式（例如私人入口上的受信任代理驗證或 `gateway.auth.mode="none"`）時才會生效。
- 在這些具備身分的模式下，若省略 `x-openclaw-scopes`，則會回退至正常的操作員預設範圍集；當您需要更狹窄的範圍集時，請明確發送該標頭。
- `/tools/invoke` 遵循相同的共享金鑰規則：該處的 Token/密碼持有者驗證也會被視為完整的操作員存取權限，而具備身分的模式則仍會遵守已宣告的範圍。
- 請勿與不受信任的呼叫者共用這些憑證；建議每個信任邊界使用獨立的閘道。

**信任假設：** 無 Token 的 Serve 驗證假設閘道主機是受信任的。
請勿將此視為針對惡意同主機程序的防護。若閘道主機上可能執行
不受信任的本機程式碼，請停用 `gateway.auth.allowTailscale`
並要求使用 `gateway.auth.mode: "token"` 或
`"password"` 進行明確的共享金鑰驗證。

**安全規則：** 請勿從您自己的反向代理轉發這些標頭。如果您
在閘道之前終止 TLS 或進行代理，請停用
`gateway.auth.allowTailscale` 並改用共享金鑰驗證 (`gateway.auth.mode:
"token"` or `"password"`) 或 [受信任的代理驗證](/en/gateway/trusted-proxy-auth)。

受信任的代理：

- 如果您在閘道之前終止 TLS，請將 `gateway.trustedProxies` 設定為您的代理 IP。
- OpenClaw 將信任來自這些 IP 的 `x-forwarded-for`（或 `x-real-ip`），以判斷用於本機配對檢查以及 HTTP 驗證/本機檢查的客戶端 IP。
- 請確保您的代理**覆寫** `x-forwarded-for` 並封鎖對閘道連接埠的直接存取。

請參閱 [Tailscale](/en/gateway/tailscale) 和 [Web 概覽](/en/web)。

### 0.6.1) 透過節點主機控制瀏覽器 (推薦)

如果您的閘道位於遠端，但瀏覽器在另一台機器上執行，請在瀏覽器機器上執行一個 **節點主機**
並讓閘道代理瀏覽器動作（請參閱 [瀏覽器工具](/en/tools/browser)）。
請將節點配對視為管理員存取權。
...
請假設 `~/.openclaw/`（或 `$OPENCLAW_STATE_DIR/`）下的任何項目可能包含機密或私人資料：

- `openclaw.json`：配置可能包含令牌（gateway、遠端 gateway）、提供者設定和允許清單。
- `credentials/**`：通道憑證（例如：WhatsApp 憑證）、配對允許清單、舊版 OAuth 匯入。
- `agents/<agentId>/agent/auth-profiles.json`：API 金鑰、令牌設定檔、OAuth 令牌，以及選用的 `keyRef`/`tokenRef`。
...
- `agents/<agentId>/sessions/**`：會話紀錄（`*.jsonl`）+ 路由元數據（`sessions.json`），其中可能包含私人訊息和工具輸出。
- 套件外掛程式套件：已安裝的外掛程式（及其 `node_modules/`）。
...
- 分享診斷資訊時，優先使用 `openclaw status --all`（可貼上、已編輯機密），而非原始日誌。
...
- `agents.defaults.sandbox.workspaceAccess: "ro"`（或 `"none"` 表示無工作區存取權）
- 封鎖 `write`、`edit`、`apply_patch`、`exec`、`process` 等的工具允許/拒絕清單。

額外的強化選項：

- `tools.exec.applyPatch.workspaceOnly: true` (預設值)：確保即使關閉沙箱，`apply_patch` 也無法在工作區目錄之外寫入/刪除。僅當您有意讓 `apply_patch` 接觸工作區之外的檔案時，才將其設定為 `false`。
- `tools.fs.workspaceOnly: true` (可選)：將 `read`/`write`/`edit`/`apply_patch` 路徑和原生提示詞映像自動載入路徑限制在工作區目錄內（如果您目前允許絕對路徑並希望設置單一防線，這會很有用）。
- 保持檔案系統根目錄狹窄：避免為代理工作區/沙箱工作區使用像您的主目錄這樣寬泛的根目錄。寬泛的根目錄可能會將敏感的本地檔案（例如 `~/.openclaw` 下的狀態/配置）暴露給檔案系統工具。
...
聊天驅動代理輪次的內建基線：非所有者發送者無法使用 `cron` 或 `gateway` 工具。
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (預設值) 使代理工作區保持禁止進入；工具針對 `~/.openclaw/sandboxes` 下的沙箱工作區執行
- `agents.defaults.sandbox.workspaceAccess: "ro"` 以唯讀方式在 `/agent` 掛載代理工作區（停用 `write`/`edit`/`apply_patch`）
- `agents.defaults.sandbox.workspaceAccess: "rw"` 以讀寫方式在 `/workspace` 掛載代理工作區
- 額外的 `sandbox.docker.binds` 會針對正規化和規範化的來源路徑進行驗證。父目錄符號連結技巧和規範化主目錄別名如果解析到被阻擋的根目錄，例如 `/etc`、`/var/run` 或 OS 主目錄下的憑證目錄，仍然會以封閉方式失敗。

重要提示：`tools.elevated` 是全域基準逃生艙，可在沙箱外執行 exec。有效的主機預設為 `gateway`，當 exec 目標設定為 `node` 時則為 `node`。請嚴格控管 `tools.elevated.allowFrom`，不要為陌生人啟用它。您可以透過 `agents.list[].tools.elevated` 進一步限制每個代理程式的提權。請參閱[提權模式](/en/tools/elevated)。

### 子 Agent 委派防護機制

如果您允許 session 工具，請將委派的子 Agent 執行視為另一個邊界決策：

- 除非代理程式真的需要委派，否則請拒絕 `sessions_spawn`。
- 請將 `agents.defaults.subagents.allowAgents` 和任何針對特定代理程式的 `agents.list[].subagents.allowAgents` 覆寫限制在已知安全的目標代理程式。
...
- 預設值：`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` 未設定，因此瀏覽器導航會阻擋私人/內部/特殊用途的目的地。
- 舊版別名：為了相容性，仍接受 `browser.ssrfPolicy.allowPrivateNetwork`。
- 選用模式：設定 `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true` 以允許私人/內部/特殊用途的目的地。
- 在嚴格模式下，使用 `hostnameAllowlist`（類似 `*.example.com` 的模式）和 `allowedHostnames`（確切的主機例外，包括被阻擋的名稱，如 `localhost`）來設定明確的例外。
- 導航會在請求前進行檢查，並在導航後的最终 `http(s)` URL 上進行盡力而為的再次檢查，以減少基於重定向的跳轉。
...
使用多代理路由時，每個代理可以擁有自己的沙箱 + 工具策略：
利用此功能為每個代理提供 **完全訪問**、**唯讀** 或 **無訪問** 權限。
詳情及優先級規則請參閱 [Multi-Agent Sandbox & Tools](/en/tools/multi-agent-sandbox-tools)。
...
3. **凍結訪問：** 將有風險的 DM/群組切換到 `dmPolicy: "disabled"` / 要求提及，並移除 `"*"` 允許所有 的條目（如果您之前設置了它們）。

### 輪換 (如果祕密外洩，假設已遭到入侵)

1. 輪換 Gateway 認證 (`gateway.auth.token` / `OPENCLAW_GATEWAY_PASSWORD`) 並重新啟動。
2. 在所有可以調用 Gateway 的機器上輪換遠端客戶端密鑰 (`gateway.remote.token` / `.password`)。
3. 輪換提供商/API 憑證（WhatsApp 憑證、Slack/Discord 令牌、`auth-profiles.json` 中的模型/API 金鑰，以及使用的加密密鑰負載值）。

### 稽核

1. 檢查 Gateway 日誌：`/tmp/openclaw/openclaw-YYYY-MM-DD.log` (或 `logging.file`)。
2. 檢視相關的對話記錄：`~/.openclaw/agents/<agentId>/sessions/*.jsonl`。
3. 檢視最近的配置變更（任何可能擴大訪問權限的變更：`gateway.bind`、`gateway.auth`、dm/群組策略、`tools.elevated`、外掛程式變更）。
...
推送到 `main` 總是會執行全文件掃描。Pull requests 在有基礎提交時會使用變更文件
的快速路徑，否則會回退到全文件掃描。
如果失敗，則表示有新的候選項尚未在基線中。
...
5. 如果您需要新增排除項目，請將其新增至 `.detect-secrets.cfg` 並使用匹配的
   `--exclude-files` / `--exclude-lines` 旗標重新產生
   基線（該設定檔僅供參考；detect-secrets 不會自動讀取它）。
