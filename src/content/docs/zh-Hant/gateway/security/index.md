...
頻道允許清單/配對以及 `commands.useAccessGroups` (請參閱 [組態](/zh-Hant/gateway/configuration)
...
如果您在同一頻道上執行多個帳戶，請改用 `per-account-channel-peer`。如果同一個人透過多個頻道與您聯絡，請使用 `session.identityLinks` 將這些 DM 工作階段合併為一個標準身分。請參閱 [工作階段管理](/zh-Hant/concepts/session) 和 [組態](/zh-Hant/gateway/configuration)。
...
- `gateway.auth.mode: "trusted-proxy"`：信任一個具備身分感知的反向代理伺服器以驗證使用者並透過標頭傳遞身分（請參閱 [Trusted Proxy Auth](/zh-Hant/gateway/trusted-proxy-auth)）。
...
"token"` or `"password"`) 或 [受信任的代理驗證](/zh-Hant/gateway/trusted-proxy-auth)。
...
並讓閘道代理瀏覽器動作（請參閱 [瀏覽器工具](/zh-Hant/tools/browser)）。
請將節點配對視為管理員存取權。
...
專用文件：[沙箱機制](/zh-Hant/gateway/sandboxing)
...
重要提示：`tools.elevated` 是全域基準逃生艙，可在沙箱外執行 exec。有效的主機預設為 `gateway`，當 exec 目標設定為 `node` 時則為 `node`。請嚴格控管 `tools.elevated.allowFrom`，不要為陌生人啟用它。您可以透過 `agents.list[].tools.elevated` 進一步限制每個代理程式的提權。請參閱[提權模式](/zh-Hant/tools/elevated)。
...
詳情及優先級規則請參閱 [Multi-Agent Sandbox & Tools](/zh-Hant/tools/multi-agent-sandbox-tools)。
