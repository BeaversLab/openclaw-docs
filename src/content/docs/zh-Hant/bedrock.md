...
如果偵測到 AWS 憑證，OpenClaw 可以自動探索支援 **串流** 和 **文字輸出** 的 Bedrock 模型。探索使用 `bedrock:ListFoundationModels` 且會被快取（預設：1 小時）。
...
**變通方法：** 設定 `AWS_PROFILE=default` 以表示 AWS 憑證可用。實際的驗證仍然透過 IMDS 使用執行個體角色。
...
或附加受控原則 `AmazonBedrockFullAccess`。
