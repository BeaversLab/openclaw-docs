...
讓隨附的 Codex app-server harness 遵循內嵌 OpenClaw turns 已遵循的相同 OpenClaw context-engine 生命週期合約。

使用 provider/model `agentRuntime.id: "codex"` 或 `codex/*` 模型的 session，應仍允許選定的 context-engine 外掛程式（例如 `lossless-claw`）控制 context 組裝、post-turn ingest、維護，以及 OpenClaw 層級的壓縮策略，只要在 Codex app-server 邊界允許的範圍內。
...
Codex harness 實作會接收與內建 OpenClaw attempts 相同的 `EmbeddedRunAttemptParams`：
...
內建 OpenClaw attempts 會直接呼叫 context-engine 生命週期：
...
- `runtime: "openclaw"` 選取內建 OpenClaw harness
- `runtime: "codex"` 選擇已註冊的 Codex 線束
- `runtime: "auto"` 允許插件線束聲明支援的提供者
- 未符合的 `auto` runs 使用內建 OpenClaw harness
...
目前可重複使用的生命週期輔助函式位於內嵌 agent runner 下：

- `src/agents/embedded-agent-runner/run/attempt.context-engine-helpers.ts`
- `src/agents/embedded-agent-runner/run/attempt.prompt-helpers.ts`
- `src/agents/embedded-agent-runner/context-engine-maintenance.ts`

Codex 應匯入 harness 中立的輔助函式，而非深入 runner 實作細節。
...
在同一個 PR 中更新內建 harness 的呼叫位置。

中立輔助函式的名稱不應提及內建 harness。
...
此順序讓通用 prompt hooks 能看到與 Codex 將接收到的相同 prompt。如果我們需要嚴格的 OpenClaw 對等性，請在 hook 組合之前執行 context-engine 組裝，因為內建 harness 會在其 prompt pipeline 之後將 context-engine `systemPromptAddition` 套用至最終系統 prompt。重要的不變性是 context engine 與 hooks 皆有確定性且已記載的順序。
...
- 內建 harness context-engine 測試應繼續保持不變並通過。
...
3. `before_prompt_build` 應在 context-engine 組裝之前或之後執行？
   ...
   建議：在針對 Codex 進行 context-engine 投射之後，以便通用 harness hooks 能看到 Codex 將實際接收到的 prompt/開發者指示。如果內建 harness 對等性要求相反的順序，請將選定的順序編碼在測試中並在此處記載。
...
- 現有的內置 harness context-engine 行為保持不變。
