...
| Vitest 和一般 CI | **Job:** `Run normal full CI`<br />**Child workflow:** `CI`<br />**Proves:** 對目標 ref 進行手動完整 CI 圖形，包括 Linux Node lanes、bundled plugin shards、channel contracts、Node 22 相容性、`check`、`check-additional`、build smoke、docs checks、Python skills、Windows、macOS、Control UI i18n，以及透過 umbrella 進行的 Android。<br />**Rerun:** `rerun_group=ci`。                              |
...
QA release-check 通道僅供參考。僅 QA 的失敗會回報為警告，並不會阻擋 release-check 驗證器；當您需要新的 QA 憑證時，請重新執行 `rerun_group=qa`、`qa-parity` 或 `qa-live`。
