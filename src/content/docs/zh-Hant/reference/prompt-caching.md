...
完整行為請參閱 [Session Pruning](/zh-Hant/concepts/session-pruning)。
...
- 相容 OpenAI 的 Completions 提供者僅在其模型配置明確設定 `compat.supportsPromptCacheKey: true` 時才會收到 `prompt_cache_key`。長期保留轉發是一項獨立功能：明確的 `cacheRetention: "long"` 僅在該相容條目也支援長期快取保留時才會發送 `prompt_cache_retention: "24h"`。諸如 Mistral 等提供者可以在設定 `compat.supportsLongCacheRetention: false` 以抑制長期保留欄位的同時選擇啟用快取金鑰。`cacheRetention: "none"` 會抑制這兩個欄位。
- OpenAI 回應透過 `usage.prompt_tokens_details.cached_tokens`（或在 Responses API 事件上的 `input_tokens_details.cached_tokens`）公開快取的提示詞 token。OpenClaw 會將其對應到 `cacheRead`。
- OpenAI 不會公開個別的快取寫入 token 計數器，因此即使在提供者正在暖快取時，`cacheWrite` 在 OpenAI 路徑上也會保持 `0`。
- OpenAI 會傳回有用的追蹤和速率限制標頭，例如 `x-request-id`、`openai-processing-ms` 和 `x-ratelimit-*`，但快取命中帳戶處理應來自 usage payload，而不是標頭。
- 實際上，OpenAI 的行為通常類似於初始前綴快取，而不是 Anthropic 風格的移動式完整歷史記錄重用。穩定的長前綴文本輪次在目前的即時探測中可能接近 `4864` 個快取 token 的平台期，而重度工具或 MCP 風格的逐字稿即使在完全重複時也通常平台期接近 `4608` 個快取 token。

### Anthropic Vertex

- Vertex AI 上的 Anthropic 模型 (`anthropic-vertex/*`) 支援 `cacheRetention`，其方式與直接 Anthropic 相同。
- `cacheRetention: "long"` 對應到 Vertex AI 端點上實際的 1 小時提示詞快取 TTL。
...
- 非 Anthropic 的 Bedrock 模型會在執行時強制設為 `cacheRetention: "none"`。

### OpenRouter 模型

對於 `openrouter/anthropic/*` 模型引用，僅當請求仍針對已驗證的 OpenRouter 路由時，OpenClaw 才會在系統/開發者提示區塊中注入 Anthropic `cache_control`，以改善提示快取的重用（在其預設端點上為 `openrouter`，或任何解析為 `openrouter.ai` 的提供者/基本 URL）。

對於 `openrouter/deepseek/*`、`openrouter/moonshot*/*` 和 `openrouter/zai/*` 模型引用，允許使用 `contextPruning.mode: "cache-ttl"`，因為 OpenRouter 會自動處理提供者端的提示快取。OpenClaw 不會在這些請求中注入 Anthropic `cache_control` 標記。

DeepSeek 快取建構是盡力而為的，可能需要幾秒鐘。緊接著的後續請求可能仍會顯示 `cached_tokens: 0`；請在短暫延遲後使用重複的相同前綴請求進行驗證，並將 `usage.prompt_tokens_details.cached_tokens` 作為快取命中訊號。
...
- 直接 Gemini 傳輸（`api: "google-generative-ai"`）透過上游 `cachedContentTokenCount` 回報快取命中；OpenClaw 將其對應到 `cacheRead`。
- 當在直接 Gemini 模型上設定 `cacheRetention` 時，OpenClaw 會自動建立、重用和重新整理 Google AI Studio 執行時系統提示的 `cachedContents` 資源。這意味著您不再需要手動預先建立快取內容控制代碼。
- 您仍然可以將現有的 Gemini 快取內容控制代碼作為 `params.cachedContent`（或舊版 `params.cached_content`）傳遞給已配置的模型。
- 這與 Anthropic/OpenAI 提示前綴快取是分開的。對於 Gemini，OpenClaw 管理提供者原生的 `cachedContents` 資源，而不是將快取標記注入請求中。

### Gemini CLI JSON 用量

- Gemini CLI JSON 輸出也可以透過 `stats.cached` 呈現快取命中；
  OpenClaw 會將其對應到 `cacheRead`。
...
OpenClaw 將系統提示詞拆分為由內部快取前綴邊界分隔的 **穩定前綴 (stable prefix)** 和 **易變後綴 (volatile suffix)**。邊界上方的內容（工具定義、技能元資料、工作區檔案和其他相對靜態的上下文）經過排序，以確保在多輪對話中保持位元組一致。邊界下方的內容（例如 `HEARTBEAT.md`、執行時間戳記和其他每輪元資料）允許變更而不會使快取的前綴失效。

關鍵設計選擇：

- 穩定的工作區專案上下文檔案會排在 `HEARTBEAT.md` 之前，以便心跳頻繁變動不會破壞穩定的前綴。
...
如果您在設定或工作區變更後看到 `cacheWrite` 出現意外激增，請檢查變更是位於快取邊界的上方還是下方。將易變內容移至邊界下方（或將其穩定化）通常能解決此問題。

## OpenClaw 快取穩定性防護機制

在請求到達提供者之前，OpenClaw 也會確保幾個對快取敏感的 Payload 形狀保持確定性：

- 在工具註冊之前，Bundle MCP 工具目錄會經過確定性排序，因此 `listTools()` 順序的變更不會導致工具區塊變動並破壞提示詞快取前綴。
...
對於一般面向使用者的診斷，當即時會話項目缺少這些計數器時，`/status` 和其他使用量摘要可以使用最新的文字記錄使用量項目作為 `cacheRead` / `cacheWrite` 的備用來源。
...
- 目前的即時斷言使用源自 `gpt-5.4-mini` 上觀察到的即時行為的保守底線檢查：
...
- 大多數回合的 `cacheWrite` 很高：請檢查不穩定的 system-prompt 輸入，並確認模型/提供者支援您的快取設定。
- Anthropic 的 `cacheWrite` 很高：通常表示快取斷點落在每次請求都會變更的內容上。
- OpenAI 的 `cacheRead` 很低：請確認穩定字串位於開頭，重複字串至少為 1024 個 token，且共享快取的回合重複使用了相同的 `prompt_cache_key`。
- `cacheRetention` 無效：請確認模型金鑰符合 `agents.defaults.models["provider/model"]`。
- Bedrock Nova/Mistral 帶有快取設定的請求：預期執行時會強制設定為 `none`。
