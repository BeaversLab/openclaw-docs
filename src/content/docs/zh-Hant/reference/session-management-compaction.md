...
- [逐字稿清理](/zh-Hant/reference/transcript-hygiene)
...
逐字稿由 `@earendil-works/pi-coding-agent` 的 `SessionManager` 管理。
...
- 助手會以確切的靜音權杖 `NO_REPLY` /
  `no_reply` 開始其輸出，以表示「不要傳送回覆給使用者」。
- OpenClaw 會在傳遞層中移除/隱藏此內容。
- 確切的靜音權杖隱藏區分大小寫，因此 `NO_REPLY` 和
  `no_reply` 當整個 Payload 只是靜音權杖時皆算數。
- 這僅適用於真正的背景/無傳遞輪次；它不是一般可執行使用者請求的捷徑。

自 `2026.1.10` 起，當部分區塊以 `NO_REPLY` 開頭時，OpenClaw 也會隱藏 **草稿/輸入中串流**，因此靜音操作不會在輪次中途洩漏部分輸出。

---

## 壓縮前「記憶體清除」（已實作）

目標：在自動壓縮發生之前，執行一個靜音的代理輪次，將持久狀態寫入磁碟（例如代理工作區中的 `memory/YYYY-MM-DD.md`），以便壓縮無法刪除關鍵上下文。
...
2. 當它超過「軟性閾值」（低於 Pi 的壓縮閾值）時，向代理執行一個靜默的
   「立即寫入記憶體」指令。
3. 使用確切的靜默權杖 `NO_REPLY` / `no_reply`，以便用戶
   看不到任何內容。

配置 (`agents.defaults.compaction.memoryFlush`)：

- `enabled` (預設值：`true`)
- `model` (可選的精確供應商/模型覆蓋，用於刷新輪次，例如 `ollama/qwen3:8b`)
- `softThresholdTokens` (預設值：`4000`)
- `prompt` (刷新輪次的用戶訊息)
- `systemPrompt` (為刷新輪次附加的額外系統提示)

註記：

- 預設的提示/系統提示包含一個 `NO_REPLY` 提示以
  抑制傳遞。
- 當設定 `model` 時，刷新輪次使用該模型而不繼承
  作用中工作階段的後備鏈，因此僅限本地的維護不會
  靜默地後退到付費的對話模型。
- 刷新每個壓縮週期執行一次 (追蹤於 `sessions.json`)。
- 刷新僅針對嵌入式 Pi 工作階段執行 (CLI 後端會跳過它)。
- 當工作階段工作區為唯讀時會跳過刷新 (`workspaceAccess: "ro"` 或 `"none"`)。
- 請參閱 [記憶體](/zh-Hant/concepts/memory) 以了解工作區檔案佈局和寫入模式。

Pi 也在擴充功能 API 中公開了一個 `session_before_compact` 掛鉤，但 OpenClaw 的
刷新邏輯目前位於 Gateway 端。
...
- 工作階段金鑰錯誤？從 [/concepts/session](/zh-Hant/concepts/session) 開始，並確認 `/status` 中的 `sessionKey`。
- Store 與逐字稿不匹配？從 `openclaw status` 確認 Gateway 主機和 store 路徑。
- 壓縮垃圾訊息？檢查：
  - 模型上下文視窗 (太小)
  - 壓縮設定 (`reserveTokens` 對於模型視窗來說太高可能會導致較早的壓縮)
  - 工具結果膨脹：啟用/調整工作階段修剪
- 靜默回合洩漏？請確認回覆以 `NO_REPLY` 開頭（不區分大小寫的精確標記），並且您使用的是包含串流抑制修復的版本。
