...
詳情請參閱 [/token-use](/en/reference/token-use)。
...
壓縮是**持久性**的（與 session pruning 不同）。請參閱 [/concepts/session-pruning](/en/concepts/session-pruning)。
...
OpenClaw 支援針對背景任務的「靜默」輪次，使用者不應看到中間輸出。

慣例：

- 助理會以精確的靜默 token `NO_REPLY` /
  `no_reply` 開始其輸出，以表示「不向使用者傳送回覆」。
- OpenClaw 會在傳遞層中移除/隱藏此內容。
- 精確的靜默 token 隱藏不區分大小寫，因此當整個負載僅為靜默 token 時，
  `NO_REPLY` 和 `no_reply` 都會被計入。
- 這僅適用於真正的背景/無傳遞輪次；它不是普通可執行使用者請求的捷徑。

自 `2026.1.10` 起，當部分區塊以 `NO_REPLY` 開頭時，OpenClaw 也會隱藏 **草稿/輸入串流**，因此靜默操作不會在輪次中途洩漏部分輸出。

---

## 預壓縮「記憶體清除」(已實作)

目標：在自動壓縮發生之前，執行一個靜默的代理輪次，將持久狀態寫入磁碟（例如代理工作區中的 `memory/YYYY-MM-DD.md`），以便壓縮無法刪除關鍵上下文。

OpenClaw 使用 **臨界前清除** 方法：

1. 監控會話上下文使用量。
2. 當它跨越「閾值」（低於 Pi 的壓縮閾值）時，向代理執行一個靜默的「立即寫入記憶體」指令。
3. 使用精確的靜默權杖 `NO_REPLY` / `no_reply`，以便使用者什麼也看不到。
...
- `prompt` (清空回合的使用者訊息)
- `systemPrompt` (為清空回合附加的額外系統提示)

備註：

- 預設的提示/系統提示包含一個 `NO_REPLY` 提示以抑制傳遞。
- 清空操作每個壓縮週期執行一次 (在 `sessions.json` 中追蹤)。
- 清空操作僅針對嵌入式 Pi 會話執行。
- 當會話工作區為唯讀時 (`workspaceAccess: "ro"` 或 `"none"`)，將跳過清空操作。
- 請參閱 [記憶體](/en/concepts/memory) 以了解工作區檔案佈局和寫入模式。

Pi 也在擴充 API 中公開了 `session_before_compact` 掛鉤，但 OpenClaw 的清空邏輯目前位於 Gateway 端。

---

## 故障排除檢查清單

- 會話金鑰錯誤？請從 [/concepts/session](/en/concepts/session) 開始，並確認 `sessionKey` 中的 `/status`。
- 儲存與逐字稿不相符？請從 `openclaw status` 確認 Gateway 主機和儲存路徑。
- 壓縮垃圾訊息？請檢查：
  - 模型上下文視窗 (太小)
  - 壓縮設定 (`reserveTokens` 對於模型視窗來說太高，可能會導致提早壓縮)
  - 工具結果膨脹：啟用/調整會話修剪
- 靜默回合洩漏？請確認回覆以 `NO_REPLY` 開頭 (不區分大小寫的精確權杖)，並且您使用的版本包含串流抑制修復。
