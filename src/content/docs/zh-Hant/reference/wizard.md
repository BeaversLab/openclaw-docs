...
<Steps>
  ...
  <Step title="Channels">
    ...
    - [Telegram](/en/channels/telegram)：Bot 權杖。
    - [Discord](/en/channels/discord)：Bot 權杖。
    - [Google Chat](/en/channels/googlechat)：服務帳戶 JSON + Webhook 受眾。
    - [Mattermost](/en/channels/mattermost) (外掛程式)：Bot 權杖 + 基礎 URL。
    - [Signal](/en/channels/signal)：可選的 `signal-cli` 安裝 + 帳戶配置。
...
<Note>如果未偵測到 GUI，上線流程會印出 Control UI 的 SSH 連接埠轉發指令，而不會開啟瀏覽器。 如果缺少 Control UI 資源，上線流程會嘗試建置它們；後備方案是 `pnpm ui:build` (自動安裝 UI 依賴項)。</Note>
...
<Note>`--json` **並不** 表示非互動模式。請使用 `--non-interactive` (以及 `--workspace`) 進行指令碼操作。</Note>
