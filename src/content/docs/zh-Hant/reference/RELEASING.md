...
- 當您僅需釋候選版本完整的常規 CI 涵蓋範圍時，直接執行手動 `CI` 工作流程。手動 CI 分派會略過變更範圍限定，並強制執行 Linux Node 分片、bundled-plugin 分片、通道合約、Node 22 相容性、`check`、`check-additional`、建構冒煙測試、文件檢查、Python 技能、Windows、macOS、Android 和 Control UI i18n 通道。
...
若要進行有界恢復，請將 `rerun_group` 傳遞給覆蓋範圍。`all` 是真正的發行候選版本執行，`ci` 僅執行一般 CI 子項，`plugin-prerelease` 僅執行僅限發行的外掛程式子項，`release-checks` 執行每個發行測試區塊，而較狹窄的發行群組則是 `install-smoke`、`cross-os`、`live-e2e`、`package`、`qa`、`qa-parity`、`qa-live` 和 `npm-telegram`。針對性 `npm-telegram` 重新執行需要 `release_package_spec` 或 `npm_telegram_package_spec`；使用 `release_profile=full` 的完整/全部執行會使用 release-checks 套件構件。針對性跨作業系統重新執行可以新增 `cross_os_suite_filter=windows/packaged-upgrade` 或其他作業系統/套件篩選器。QA release-check 失敗僅供參考；僅限 QA 的失敗不會阻擋發行驗證。

### Vitest

Vitest 測試區塊是手動 `CI` 子工作流程。手動 CI 會故意略過變更範圍限定，並針對發行候選版本強制執行一般測試圖形：Linux Node 分片、捆綁外掛程式分片、通道合約、Node 22 相容性、`check`、`check-additional`、建構煙霧測試、文件檢查、Python 技能、Windows、macOS、Android 和 Control UI i18n。
