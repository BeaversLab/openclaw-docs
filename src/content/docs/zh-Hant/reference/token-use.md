...
請參閱 [系統提示詞](/en/concepts/system-prompt) 中的完整細項。
...
如需實用的細項（依據每個注入的檔案、工具、技能和系統提示詞大小），請使用 `/context list` 或 `/context detail`。請參閱 [Context](/en/concepts/context)。
...
在 [Gateway configuration](/en/gateway/configuration) 中進行設定，並在 [Session pruning](/en/concepts/session-pruning) 中查看行為細節。
...
如需完整的逐步設定指南，請參閱 [Prompt Caching](/en/reference/prompt-caching)。

對於 Anthropic API 定價，讀取快取的費用遠低於輸入 token，而寫入快取則以較高的倍率計費。請參閱 Anthropic 的 prompt caching pricing 以了解最新費率和 TTL 倍率：
...
需求：憑證必須符合使用長上下文資格。若不符合，Anthropic 將針對該請求回應供應商端的速率限制錯誤。

如果您使用 OAuth/訂閱 token (`sk-ant-oat-*`) 來驗證 Anthropic，
OpenClaw 將跳過 `context-1m-*` beta 標頭，因為 Anthropic 目前會因該組合而回傳 HTTP 401。

## 降低 Token 壓力的技巧

- 使用 `/compact` 來總結長對話。
- 在您的工作流程中裁剪大型工具輸出。
- 針對包含大量截圖的對話，請降低 `agents.defaults.imageMaxDimensionPx`。
- 保持技能描述簡短（技能清單會被注入提示詞中）。
- 對於冗長、探索性的工作，請偏好使用較小的模型。

請參閱 [Skills](/en/tools/skills) 以了解確切的技能清單負載公式。
