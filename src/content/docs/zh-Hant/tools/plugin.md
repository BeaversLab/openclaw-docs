...
兩者都會出現在 `openclaw plugins list` 下。有關套件的詳細資訊，請參閱 [外掛程式套件](/zh-Hant/plugins/bundles)。

如果您正在編寫原生外掛程式，請從 [建構外掛程式](/zh-Hant/plugins/building-plugins)
和 [外掛程式 SDK 概覽](/zh-Hant/plugins/sdk-overview) 開始。
...
尋找第三方外掛程式？請參閱 [社群外掛程式](/zh-Hant/plugins/community)。
...
有關完整細節，請參閱 [`openclaw plugins` CLI 參考](/zh-Hant/cli/plugins)。
...
如需完整的類型化 Hook 行為，請參閱 [SDK Overview](/zh-Hant/plugins/sdk-overview#hook-decision-semantics)。
...
- [Registering Tools](/zh-Hant/plugins/building-plugins#registering-agent-tools) — 在外掛程式中新增代理工具
- [Plugin Internals](/zh-Hant/plugins/architecture) — 能力模型與載入管道
