...
對於基於 session 的 agent 執行，OpenClaw 會將音樂生成啟動為
背景任務，在任務分類帳中追蹤它，然後在音軌準備好時再次喚醒 agent，
以便 agent 可以告知用戶並附加完成的音頻。生成的媒體完成項由 agent 通過
訊息工具傳遞。如果請求者的 session 不活動，並且某些生成的
音頻在訊息工具傳遞中仍然缺失，OpenClaw 將發送一個
僅包含缺失音頻的冪等直接回退。完成喚醒
明確警告 agent，對於此路由，常規的最終回覆是私有的。
