...
<AccordionGroup>
  ...
  <Accordion title="Timeouts">針對緩慢的圖像後端設定 `agents.defaults.imageGenerationModel.timeoutMs`。每次呼叫的 `timeoutMs` 工具參數會覆寫已設定的預設值。Google、OpenRouter 和 xAI 託管的圖像提供者使用 180 秒的預設值；Azure OpenAI 圖像生成使用 600 秒。Codex 動態工具呼叫遵循相同的逾時預算，並以 OpenClaw 的 600000 毫秒動態工具橋接器上限為界。</Accordion>
  <Accordion title="Inspect at runtime">使用 `action: "list"` 來檢查目前已註冊的提供者、其預設模型以及 auth env-var 提示。</Accordion>
</AccordionGroup>
...
OpenAI、OpenRouter、Google 和 xAI 透過 `images` 參數支援最多 5 張參考圖像。fal 對於 Flux image-to-image 支援 1 張參考圖像，對於 GPT Image 2 編輯最多支援 10 張，對於 Nano Banana 2 編輯最多支援 14 張。MiniMax 和 ComfyUI 支援 1 張。
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (和 gpt-image-1.5)">
    ...
    **不**會對該請求無訊息地回退至 `OPENAI_API_KEY` ——
    ...
    與參考影像轉發至 OpenAI。OpenAI **不**會
    直接接收 `aspectRatio` 或 `resolution`；若有可能，OpenClaw 會將這些參數
    對應至受支援的 `size`，否則工具會將其回報為
    已忽略的覆寫。
    ...
    頂層 `background` 提示是供應商中立的，當選取
    OpenAI 供應商時，目前會對應至相同的 OpenAI `background` 請求欄位。
    未宣告背景支援的供應商會改為在 `ignoredOverrides` 中傳回該提示，
    而非接收不支援的參數。

    若要將 OpenAI 影像生成透過 Azure OpenAI 部署進行路由
    而非透過 `api.openai.com`，請參閱
  ...
  <Accordion title="OpenRouter 影像模型">
    ...
    透過 OpenRouter 的聊天完成影像 API 進行路由。請選取
    帶有 `openrouter/` 前綴的 OpenRouter 影像模型：
    ...
    `action: "list"` 來查看您設定的外掛程式公開了哪些選項。
  ...
  <Accordion title="xAI grok-imagine-image">
    內建的 xAI 提供者對於僅提示詞的請求使用 `/v1/images/generations`，當存在 `image` 或 `images` 時則使用 `/v1/images/edits`。

    - 模型：`xai/grok-imagine-image`、`xai/grok-imagine-image-quality`
    - 數量：最多 4 張
    - 參考圖：一張 `image` 或最多五張 `images`
    - 寬高比：`1:1`、`16:9`、`9:16`、`4:3`、`3:4`、`2:3`、`3:2`
    - 解析度：`1K`、`2K`
    - 輸出：作為 OpenClaw 管理的圖片附件返回

    OpenClaw 故意不公開 xAI 原生的 `quality`、`mask`、
    `user` 或額外的原生專用寬高比，直到這些控制項存在於共享的跨提供者 `image_generate` 契約中。
...
同樣的 `--output-format` 和 `--background` 標誌可用於
`openclaw infer image edit`；`--openai-background` 仍保留為
OpenAI 專用別名。目前，除 OpenAI 外的打包供應商未宣告明確的背景控制，因此對它們而言，`background: "transparent"` 會被回報為已忽略。

## 相關

- [工具總覽](/zh-Hant/tools) - 所有可用的代理工具
- [ComfyUI](/zh-Hant/providers/comfy) - 本地 ComfyUI 和 Comfy Cloud 工作流程設定
- [fal](/zh-Hant/providers/fal) - fal 圖像與視訊供應商設定
- [Google (Gemini)](/zh-Hant/providers/google) - Gemini 圖像供應商設定
- [MiniMax](/zh-Hant/providers/minimax) - MiniMax 圖像供應商設定
- [OpenAI](/zh-Hant/providers/openai) - OpenAI Images 供應商設定
- [Vydra](/zh-Hant/providers/vydra) - Vydra 圖像、視訊與語音設定
- [xAI](/zh-Hant/providers/xai) - Grok 圖像、視訊、搜尋、程式碼執行與 TTS 設定
