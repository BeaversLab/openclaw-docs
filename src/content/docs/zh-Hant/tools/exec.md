...
- 對於 gateway + node，無需審核的主機執行是預設值。如果您想要審核/允許清單行為，請同時收緊 `tools.exec.*` 與主機 `~/.openclaw/exec-approvals.json`；請參閱 [Exec approvals](/zh-Hant/tools/exec-approvals#yolo-mode-no-approval)。
...
- `tools.exec.strictInlineEval`（預設值：false）：當為 true 時，內聯直譯器 eval 形式（例如 `python -c`、`node -e`、`ruby -e`、`perl -e`、`php -r`、`lua -e` 和 `osascript -e`）需要審查者或明確批准。在 `mode=auto` 中，原生自動審查者可能允許明顯低風險的一次性指令；如果審查者提出請求，該請求將轉給人員處理。`allow-always` 仍可持續記錄良性的直譯器/腳本調用，但內聯 eval 形式不會變成持久的允許規則。
- `tools.exec.commandHighlighting`（預設值：false）：當為 true 時，批准提示可以高亮顯示指令文字中由解析器衍生的指令範圍。將其設定為 `true`（全域或針對每個代理程式）以啟用指令文字高亮顯示，而不變更 exec 批准政策。
- `tools.exec.pathPrepend`：要前置於 exec 執行的 `PATH` 的目錄列表（僅限 gateway + sandbox）。
- `tools.exec.safeBins`：僅 stdin 的安全二進位檔，無需明確的允許清單條目即可執行。有關行為的詳細資訊，請參閱 [安全二進位檔](/zh-Hant/tools/exec-approvals-advanced#safe-bins-stdin-only)。
- `tools.exec.safeBinTrustedDirs`：用於 `safeBins` 路徑檢查的其他受信任明確目錄。`PATH` 條目絕不會自動受信任。內建預設值為 `/bin` 和 `/usr/bin`。
- `tools.exec.safeBinProfiles`：每個安全 bin 的可選自訂 argv 政策（`minPositional`、`maxPositional`、`allowedValueFlags`、`deniedFlags`）。
...
- `host=gateway`：將您的登入 shell `PATH` 合併到 exec 環境中。`env.PATH` 覆寫對於主機執行會被拒絕。守護程式本身仍以最少的 `PATH` 執行：
  - macOS：`/opt/homebrew/bin`、`/usr/local/bin`、`/usr/bin`、`/bin`
  - Linux：`/usr/local/bin`、`/usr/bin`、`/bin`
    - 為防止使用者 shell 配置（如 `~/.zshenv` 或 `/etc/zshenv`）在啟動時覆寫優先路徑，`tools.exec.pathPrepend` 條目會在執行前安全地前置至 shell 指令中最終的 `PATH`。
- `host=sandbox`：在容器內執行 `sh -lc`（登入 shell），因此 `/etc/profile` 可能會重設 `PATH`。
  OpenClaw 會在載入 profile 後透過內部環境變數（無 shell 插值）前置 `env.PATH`；
  `tools.exec.pathPrepend` 也適用於此。
- `host=node`：僅您傳遞的非受阻環境變數覆寫會傳送至節點。`env.PATH` 覆寫會
  在主機執行時被拒絕，並被節點主機忽略。如果您需要在節點上使用額外的 PATH 條目，
...
使用 `/exec` 為 `host`、`security`、`ask` 和 `node` 設定 **每個 Session** 的預設值。
...
`/exec` 僅對**已授權的發送者**（頻道允許清單/配對加上 `commands.useAccessGroups`）有效。
它僅更新**工作階段狀態**，不會寫入設定。已授權的外部頻道發送者可以
設定這些工作階段預設值。內部閘道/網路聊天用戶端需要 `operator.admin` 才能將其永久儲存。
若要完全停用 exec，請透過工具原則（`tools.deny: ["exec"]` 或針對每個代理程式）拒絕它。除非您明確設定
`security=full` 和 `ask=off`，否則主機核准仍然適用。

## Exec 核准（伴隨應用程式 / 節點主機）

沙箱代理程式可以要求在 `exec` 於閘道或節點主機上執行之前進行每次請求的核准。
...
（`Exec running` / `Exec finished`）。被拒絕或逾時的核准屬於終結狀態，且不會
使用拒絕系統事件喚醒代理程式工作階段。
在具有原生核准卡片/按鈕的頻道上，代理程式應優先依賴該
原生 UI，並且僅當工具結果明確表示聊天核准無法使用或手動核准是
唯一途徑時，才包含手動 `/approve` 命令。

## 允許清單 + 安全 bins

手動允許清單執行會比對解析後的二進位路徑 glob 和純指令名稱 glob。純名稱僅比對透過 PATH 叫用的指令，因此當指令是 `rg` 時，`rg` 可以比對 `/opt/homebrew/bin/rg`，但無法比對 `./rg` 或 `/tmp/rg`。
當 `security=allowlist` 時，僅當管線中的每個片段都被列入允許清單或是安全 bin 時，shell 指令才會被自動允許。在允許清單模式下，除非每個頂層片段都滿足允許清單（包括安全 bin），否則會拒絕鏈結（`;`、`&&`、`||`）和重導向。重導向仍然不受支援。持久的 `allow-always` 信任權限無法繞過該規則：鏈結指令仍要求每個頂層片段都必須符合。

`autoAllowSkills` 是 exec 批准中一條獨立的便利途徑。它與手動路徑允許清單條目不同。若要嚴格執行明確信任，請保持 `autoAllowSkills` 為停用狀態。
...
請勿將 `safeBins` 視為通用允許清單，且請勿新增解譯器/執行環境二進位檔（例如 `python3`、`node`、`ruby`、`bash`）。如果您需要這些項目，請使用明確的允許清單項目並保持批准提示為啟用狀態。
當解譯器/執行環境 `safeBins` 項目缺少明確的設定檔時，`openclaw security audit` 會發出警告，而 `openclaw doctor --fix` 可以建立缺少的自訂 `safeBinProfiles` 項目。
當您明確將行為廣泛的二進位檔（例如 `jq`）新增回 `safeBins` 時，`openclaw security audit` 和 `openclaw doctor` 也會發出警告。
如果您明確允許解譯器，請啟用 `tools.exec.strictInlineEval`，這樣內嵌程式碼評估表單仍然需要審閱者或明確的批准。
...
對於 OpenAI 和 OpenAI Codex 模型，它預設為啟用。僅在您想要停用它或將其限制為特定模型時才使用設定：
...
- `deny: ["write"]` 不會拒絕 `apply_patch`；請明確拒絕 `apply_patch`，或者在也應封鎖修補寫入時使用 `deny: ["group:fs"]`。
- 設定位於 `tools.exec.applyPatch` 之下。
- `tools.exec.applyPatch.enabled` 預設為 `true`；將其設定為 `false` 即可針對 OpenAI 模型停用此工具。
- `tools.exec.applyPatch.workspaceOnly` 預設為 `true`（限制於工作區內）。僅當您有意讓 `apply_patch` 在工作區目錄之外寫入/刪除時，才將其設定為 `false`。

## 相關

- [Exec Approvals](/zh-Hant/tools/exec-approvals) — 殼層指令的核准閘道
