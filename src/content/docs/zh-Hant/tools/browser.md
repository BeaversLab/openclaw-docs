...
如果完全找不到 `openclaw browser`，或者代理程式表示瀏覽器工具無法使用，請跳至 [缺少瀏覽器指令或工具](/en/tools/browser#missing-browser-command-or-tool)。

## 外掛程式控制

預設的 `browser` 工具現在是一個隨附的外掛程式，預設為啟用狀態。這意味著您可以停用或取代它，而不需要移除 OpenClaw 外掛程式系統的其餘部分：
...
在安裝另一個提供相同 `browser` 工具名稱的外掛程式之前，請先停用隨附的外掛程式。預設的瀏覽器體驗需要兩者：

- `plugins.entries.browser.enabled` 未停用
- `browser.enabled=true`

如果您只關閉外掛程式，隨附的瀏覽器 CLI (`openclaw browser`)、
閘道方法 (`browser.request`)、代理程式工具和預設的瀏覽器控制服務
都會一起消失。您的 `browser.*` 設定將保持完整，供
取代的外掛程式重複使用。
...
如果在升級後 `openclaw browser` 突然變成未知指令，或者
代理程式回報瀏覽器工具遺失，最常見的原因是
限制性的 `plugins.allow` 列表中未包含 `browser`。
...
- 當設定 `plugins.allow` 時，僅靠 `browser.enabled=true` 是不夠的。
- 當設定 `plugins.allow` 時，僅靠 `plugins.entries.browser.enabled=true` 也是不夠的。
- `tools.alsoAllow: ["browser"]` **不會**載入隨附的瀏覽器外掛程式。它只會在外掛程式載入後調整工具政策。
- 如果您不需要限制性的外掛程式允許清單，移除 `plugins.allow` 也會恢復預設的內建瀏覽器行為。

典型症狀：

- `openclaw browser` 是一個未知的指令。
- `browser.request` 遺失。
- 代理程式回報瀏覽器工具無法使用或遺失。

## 設定檔：`openclaw` vs `user`

- `openclaw`：受管理的、獨立的瀏覽器（無需擴充功能）。
...
- 當現有的登入工作階段很重要，且使用者
  在電腦旁點擊/核准任何附加提示時，請優先使用 `profile="user"`。
- `profile` 是當您想要特定瀏覽器模式時的明確覆寫設定。
...
- 瀏覽器控制服務會繫結到從 `gateway.port` 推導出的連接埠的回介面
  （預設值：`18791`，即 gateway + 2）。
- 如果您覆寫 Gateway 連接埠（`gateway.port` 或 `OPENCLAW_GATEWAY_PORT`），
  推導出的瀏覽器連接埠會移動以保持在同一個「系列」中。
- 當未設定時，`cdpUrl` 預設為受管理的本機 CDP 連接埠。
- `remoteCdpTimeoutMs` 適用於遠端（非回介面）CDP 可連線性檢查。
- `remoteCdpHandshakeTimeoutMs` 適用於遠端 CDP WebSocket 可連線性檢查。
- 瀏覽器導覽/開啟分頁會在導覽前受到 SSRF 防護，並在導覽後對最終的 `http(s)` URL 進行盡力檢查。
- 在嚴格 SSRF 模式下，遠端 CDP 端點探索/探測（`cdpUrl`，包括 `/json/version` 查詢）也會受到檢查。
- `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` 預設為停用。僅在您有意信任私人網路瀏覽器存取時，才將其設定為 `true`。
...
[Browserless](https://browserless.io) 是一個託管的 Chromium 服務，透過 HTTPS 和 WebSocket 公開 CDP 連接 URL。OpenClaw 可以使用任一種形式，但對於遠端瀏覽器設定檔，最簡單的選項是來自 Browserless 連線文件中的直接 WebSocket URL。
...
- **WebSocket 端點** (`ws://` / `wss://`) — OpenClaw 直接連接，
  略過 `/json/version`。將此用於如
  [Browserless](https://browserless.io)、
  [Browserbase](https://www.browserbase.com) 等服務，或任何提供 WebSocket URL 的供應商。
...
- 免費層級允許每月一個並行會話和一個瀏覽器小時。
  請參閱 [定價](https://www.browserbase.com/pricing) 以了解付費方案的限製。
...
- `ACT_SELECTOR_UNSUPPORTED` (HTTP 400)：`selector` 與不支援的動作類型一起使用。
- `ACT_EVALUATE_DISABLED` (HTTP 403)：`evaluate` (或 `wait --fn`) 已由配置停用。
- `ACT_TARGET_ID_MISMATCH` (HTTP 403)：頂層或批次 `targetId` 與請求目標衝突。
- `ACT_EXISTING_SESSION_UNSUPPORTED` (HTTP 501)：此動作不支援既有工作階段設定檔。

其他執行時期的失敗仍可能傳回 `{ "error": "<message>" }` 而不含
`code` 欄位。
...
- ARIA 快照
- 當每個分頁的 CDP WebSocket 可用時，受管理的 `openclaw` 瀏覽器的頁面截圖
- `existing-session` / Chrome MCP 設定檔的頁面截圖
- 來自快照輸出的 `existing-session` 參照型截圖 (`--ref`)
...
`OPENCLAW_HOME_VOLUME` 或繫結掛載來保存。請參閱 [Docker](/en/install/docker)。
...
所有命令都接受 `--browser-profile <name>` 來指定特定的設定檔。
所有命令也都接受 `--json` 以獲得機器可讀的輸出（穩定的負載）。
...
- `upload` 和 `dialog` 是**準備**（arming）呼叫；請在觸發選擇器/對話框的點擊/按壓之前執行它們。
...
- `snapshot`：
  - `--format ai`（安裝 Playwright 時的預設值）：返回帶有數字引用的 AI 快照（`aria-ref="<n>"`）。
  - `--format aria`：返回無障礙樹（無引用；僅供檢查）。
  - `--efficient`（或 `--mode efficient`）：精簡角色快照預設（互動 + 精簡 + 深度 + 較低 maxChars）。
  - 設定預設值（僅限工具/CLI）：設定 `browser.snapshotDefaults.mode: "efficient"` 以在呼叫者未傳遞模式時使用高效的快照（請參閱 [Gateway configuration](/en/gateway/configuration-reference#browser)）。
  - 角色快照選項（`--interactive`、`--compact`、`--depth`、`--selector`）會強制使用基於角色的快照，並包含如 `ref=e12` 的引用。
  - `--frame "<iframe selector>"` 將角色快照的作用範圍限定在 iframe（與角色參照如 `e12` 配合使用）。
  - `--interactive` 輸出一個扁平、易於選擇的互動元素列表（最適合驅動操作）。
  - `--labels` 新增一個僅限視口的螢幕截圖，並覆蓋參照標籤（列印 `MEDIA:<path>`）。
...
- **AI 快照（數字參照）**：`openclaw browser snapshot`（預設值；`--format ai`）
  ...
  - 在內部，該參照透過 Playwright 的 `aria-ref` 解析。

- **角色快照（如 `e12` 的角色參照）**：`openclaw browser snapshot --interactive`（或 `--compact`、`--depth`、`--selector`、`--frame`）
  - 輸出：帶有 `[ref=e12]`（及可選 `[nth=1]`）的基於角色的清單/樹狀結構。
  - 操作：`openclaw browser click e12`、`openclaw browser highlight e12`。
  - 在內部，該參照透過 `getByRole(...)` 解析（對於重複項則加上 `nth()`）。
  - 新增 `--labels` 以包含帶有覆蓋 `e12` 標籤的視口螢幕截圖。

參照行為：

- 參照在導覽之間**並不穩定**；如果操作失敗，請重新執行 `snapshot` 並使用新的參照。
- 如果角色快照是使用 `--frame` 擷取的，則角色參照的作用範圍將限定在該 iframe，直到下一次角色快照為止。
...
JSON 中的角色快照包含 `refs` 以及一個小型的 `stats` 區塊 (lines/chars/refs/interactive)，以便工具推斷負載大小和密度。
...
- `browser act kind=evaluate` / `openclaw browser evaluate` 和 `wait --fn`
  會在頁面上下文中執行任意的 JavaScript。提示詞注入可以引導這一行為。如果您不需要，請使用 `browser.evaluateEnabled=false` 將其停用。
- 關於登入和防機器人提示（X/Twitter 等），請參閱 [Browser login + X/Twitter posting](/en/tools/browser-login)。
...
- `browser` 接受：
  ...
  - 如果連接了具備瀏覽器功能的節點，工具可能會自動路由到該節點，除非您釘選 `target="host"` 或 `target="node"`。
...
- [沙盒機制](/en/gateway/sandboxing) — 沙盒環境中的瀏覽器控制
