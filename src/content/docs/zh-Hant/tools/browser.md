...
如果完全找不到 `openclaw browser`，或者代理程式表示瀏覽器工具
無法使用，請跳至[缺少瀏覽器指令或工具](/zh-Hant/tools/browser#missing-browser-command-or-tool)。
...
[Browserless](https://browserless.io) 是一項託管的 Chromium 服務，透過 HTTPS 和 WebSocket
公開 CDP 連線 URL。OpenClaw 可以使用任一種形式，但
...
- **HTTP(S) 探索** — `http://host[:port]` 或 `https://host[:port]`。
  OpenClaw 呼叫 `/json/version` 來探索 WebSocket 偵錯工具 URL，然後
  進行連線。沒有 WebSocket 後援。
- **直接 WebSocket 端點** — `ws://host[:port]/devtools/<kind>/<id>` 或
  `wss://...` 搭配 `/devtools/browser|page|worker|shared_worker|service_worker/<id>`
  路徑。OpenClaw 透過 WebSocket 握手直接連線，並完全跳過
  `/json/version`。
- **純 WebSocket 根路徑** — 不帶 `/devtools/...` 路徑的 `ws://host[:port]` 或 `wss://host[:port]`（例如 [Browserless](https://browserless.io)、
  ...
  會退回到純根路徑的直接 WebSocket 握手。如果宣傳的
  WebSocket 端點拒絕 CDP 握手，但配置的純根路徑
  接受它，OpenClaw 也會退回到該根路徑。這使得指向
  本地 Chrome 的純 `ws://` 仍然可以連接，因為 Chrome 僅在來自 `/json/version` 的特定每目標路徑上接受 WebSocket
  升級，而託管
  提供商在其探索端點宣傳不適合 Playwright CDP 的短期 URL 時，仍可使用其根 WebSocket 端點。

### Browserbase

[Browserbase](https://www.browserbase.com) 是一個運行
...
- Browserbase 會在 WebSocket 連接時自動建立瀏覽器會話，因此不需要
  手動建立會話的步驟。
- 免費層級允許一個並行會話和每月一個瀏覽器小時。
  請參閱 [pricing](https://www.browserbase.com/pricing) 了解付費方案的限製。
- 請參閱 [Browserbase docs](https://docs.browserbase.com) 以獲取完整的 API
  參考、SDK 指南和整合範例。
...
- 瀏覽器控制僅限迴路；存取透過 Gateway 的認證或節點配對流動。
...
- 如果啟用了瀏覽器控制且未設定共享密鑰驗證，OpenClaw 會在啟動時自動生成 `gateway.auth.token` 並將其保存到配置中。
- 當 `gateway.auth.mode` 已經是 `password`、`none` 或 `trusted-proxy` 時，OpenClaw 將**不**會自動生成該令牌。
...
- 內建的 `user` 設定檔使用 Chrome MCP 自動連線，該功能以
  預設的本機 Google Chrome 設定檔為目標。

若是 Brave、Edge、Chromium 或非預設的 Chrome 設定檔，請使用 `userDataDir`。
`~` 會展開為您的 OS 家目錄：
...
- `tabs` 列出您已開啟的瀏覽器分頁
- `snapshot` 從選取的即時分頁傳回參照

如果連線失敗，請檢查下列事項：

- 目標的 Chromium 系列瀏覽器版本為 `144+`
- 已在該瀏覽器的檢查頁面中啟用遠端偵錯
- 瀏覽器已顯示，且您已接受連線同意提示
- `openclaw doctor` 會遷移舊的擴充功能型瀏覽器設定，並檢查是否已在本機安裝
  Chrome 以供預設自動連線設定檔使用，但無法
  代表您啟用瀏覽器端的遠端偵錯
...
- 此途徑的風險高於隔離的 `openclaw` 設定檔，因為它
  可以在您已登入的瀏覽器工作階段中執行操作。
- OpenClaw 不會為此驅動程式啟動瀏覽器；它僅會進行連線。
- OpenClaw 在此處使用官方的 Chrome DevTools MCP `--autoConnect` 流程。如果設定了
  `userDataDir`，它會被傳遞至該使用者資料目錄。
- Existing-session 可以附加到選定的主機或透過已連線的瀏覽器節點。如果 Chrome 位於其他地方且未連線瀏覽器節點，請改用
  遠端 CDP 或節點主機。

### 自訂 Chrome MCP 啟動

當預設的 `npx chrome-devtools-mcp@latest` 流程不符合需求時（例如離線主機、
固定版本、自供二進位檔案），請覆寫每個設定檔產生的 Chrome DevTools MCP 伺服器：

| 欄位         | 說明                                                                                               |
| ------------ | -------------------------------------------------------------------------------------------------- |
| `mcpCommand` | 要產生的執行檔，用來取代 `npx`。會按原樣解析；絕對路徑會受到支援。                                 |
| `mcpArgs`    | 原樣傳遞給 `mcpCommand` 的引數陣列。會取代預設的 `chrome-devtools-mcp@latest --autoConnect` 引數。 |

當在 existing-session 設定檔上設定 `cdpUrl` 時，OpenClaw 會跳過
`--autoConnect` 並自動將端點轉發至 Chrome MCP：

- `http(s)://...` → `--browserUrl <url>` (DevTools HTTP 探索端點)。
- `ws(s)://...` → `--wsEndpoint <url>` (直接 CDP WebSocket)。

端點旗標與 `userDataDir` 無法合併使用：當設定
`cdpUrl` 時，Chrome MCP 啟動會忽略 `userDataDir`，因為 Chrome MCP 是附加到端點後方的執行中瀏覽器，而非開啟設定檔
目錄。

<Accordion title="現有工作階段功能限制">

與受控的 `openclaw` 設定檔相比，現有工作階段的驅動程式有更多限制：

- **螢幕截圖** — 頁面擷取和 `--ref` 元素擷取有效；CSS `--element` 選擇器無效。`--full-page` 無法與 `--ref` 或 `--element` 結合。頁面或基於參照的元素螢幕截圖不需要 Playwright。
- **動作** — `click`、`type`、`hover`、`scrollIntoView`、`drag` 和 `select` 需要快照參照（無 CSS 選擇器）。`click-coords` 點擊可見視口座標，不需要快照參照。`click` 僅限左鍵。`type` 不支援 `slowly=true`；請使用 `fill` 或 `press`。`press` 不支援 `delayMs`。`type`、`hover`、`scrollIntoView`、`drag`、`select`、`fill` 和 `evaluate` 不支援每次呼叫的逾時。`select` 接受單一值。
- **等待 / 上傳 / 對話框** — `wait --url` 支援精確、子字串和 Glob 模式；`wait --load networkidle` 不受支援。上傳 Hook 需要 `ref` 或 `inputRef`，一次一個檔案，無 CSS `element`。對話框 Hook 不支援逾時覆寫。
...
- **確定性分頁控制**：`tabs` 首先返回 `suggestedTargetId`，然後是穩定的 `tabId` 代碼，例如 `t1`、可選標籤和原始 `targetId`。
...
為了腳本編寫和偵錯，Gateway 公開了一個小型**僅限回傳 HTTP 控制 API** 以及匹配的 `openclaw browser` CLI (快照、引用、等待增強功能、JSON 輸出、偵錯工作流程)。請參閱
[瀏覽器控制 API](/zh-Hant/tools/browser-control) 以取得完整參考資料。

## 疑難排解

對於 Linux 特有的問題 (特別是 snap Chromium)，請參閱
[瀏覽器疑難排解](/zh-Hant/tools/browser-linux-troubleshooting)。

對於 WSL2 Gateway + Windows Chrome 分割主機設定，請參閱
...
- CDP 啟動或就緒失敗：
  ...
  - `Port <port> is in use for profile "<name>" but not by openclaw` 當
    配置了回送外部 CDP 服務但未設定 `attachOnly: true` 時
- 導覽 SSRF 封鎖：
  - 當 `start` 和 `tabs` 仍然正常運作時，`open`、`navigate`、快照或分頁開啟流程因瀏覽器/網路原則錯誤而失敗
...
- 如果 `start` 失敗並顯示 `not reachable after start`，請先對 CDP 準備狀態進行疑難排解。
- 如果 `start` 成功但 `tabs` 失敗，表示控制平面仍然不健全。請將此視為 CDP 連線性問題，而非頁面瀏覽問題。
- 如果 `start` 和 `tabs` 成功，但 `open` 或 `navigate` 失敗，表示瀏覽器控制平面已啟動，失敗原因在於瀏覽原則或目標頁面。
- 如果 `start`、`tabs` 和 `open` 全部成功，表示基本的受控瀏覽器控制路徑健全。

重要的行為細節：

- 即使您未設定 `browser.ssrfPolicy`，瀏覽器設定也會預設為「失敗即封閉」的 SSRF 原則物件。
- 對於本機回送 `openclaw` 受控設定檔，CDP 健康檢查會刻意針對 OpenClaw 自身的本機控制平面略過瀏覽器 SSRF 連線性強制執行。
- 瀏覽保護是分開的。成功的 `start` 或 `tabs` 結果並不代表之後的 `open` 或 `navigate` 目標是被允許的。

安全性指引：

- 預設情況下**切勿**放寬瀏覽器 SSRF 原則。
- 偏好使用狹隘的主機例外，例如 `hostnameAllowlist` 或 `allowedHostnames`，而非廣泛的私人網路存取。
- 僅在經過審查且需要私人網路瀏覽器存取的故意信任環境中，才使用 `dangerouslyAllowPrivateNetwork: true`。
...
- `browser` 接受：
  ...
  - 如果連接了具備瀏覽器能力的節點，工具可能會自動路由至該節點，除非您釘選 `target="host"` 或 `target="node"`。
...
- [安全性](/zh-Hant/gateway/security) — 瀏覽器控制的風險與防護
