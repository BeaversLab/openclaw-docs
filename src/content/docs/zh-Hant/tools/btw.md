...
這種區分是有意為之的。如果 BTW 重複使用一般的 `chat` 事件路徑，客戶端會將其視為一般的對話記錄。

因為 BTW 使用單獨的即時事件，且不會從 `chat.history` 重播，所以它在重新載入後會消失。
...
Gateway 正確地將 BTW 作為 `chat.side_result` 發送，且 BTW 未包含在 `chat.history` 中，因此對於 Web 而言，持久性合約已經是正確的。

目前的 Control UI 仍然需要一個專用的 `chat.side_result` 消費者，以便在瀏覽器中即時呈現 BTW。在該客戶端支援落實之前，BTW 是一個具有完整 TUI 和外部通道行為的 Gateway 層級功能，但尚未成為完整的瀏覽器 UX。
...
當您希望答案成為工作階段未來工作內容的一部分時，請不要使用 `/btw`。
