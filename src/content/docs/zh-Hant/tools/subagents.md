...
- 對於 ACP 駕駛程式會話（Codex、Claude Code、Gemini CLI），請使用帶有 `runtime: "acp"` 的 `sessions_spawn`，並參閱 [ACP Agents](/zh-Hant/tools/acp-agents)。
...
請參閱 [組態參考](/zh-Hant/gateway/configuration-reference) 和 [斜線指令](/zh-Hant/tools/slash-commands) 以了解目前的配接器詳細資訊。
