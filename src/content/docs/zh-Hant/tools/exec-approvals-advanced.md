...
當需要批准時，exec 工具會立即返回並帶有批准 ID。使用該 ID 來關聯後續經批准執行的系統事件（`Exec finished`，以及設定時的 `Exec running`）。
如果在逾時前未收到決定，該請求將被視為批准逾時，並顯示為終端主機命令拒絕。對於具有原始會話的主代理非同步批准，OpenClaw 也會透過內部後續追蹤恢復該會話，讓代理察覺到該命令未執行，而非稍後修復遺失的結果。

### 後續傳遞行為

經批准的非同步 exec 執行完成後，OpenClaw 會向同一會話發送後續追蹤 `agent` 輪次。
被拒絕的非同步批准使用相同的主會話後續追蹤路徑來傳達拒絕狀態，但它們不會註冊提升的運行時移交，也不會執行命令。無法恢復主會話的拒絕，若存在安全直接路由，則會被抑制或透過該路由回報。
