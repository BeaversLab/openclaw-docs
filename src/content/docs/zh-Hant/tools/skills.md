...
OpenClaw 使用與 **[AgentSkills](https://agentskills.io)** 相容的技能資料夾來教導代理如何使用工具。每個技能都是一個包含 `SKILL.md` 的目錄，該檔案帶有 YAML frontmatter 和指令。OpenClaw 載入 **套件技能** 以及可選的本機覆寫，並在載入時根據環境、配置和二進位檔案的存在進行篩選。
...
外掛程式可以透過在 `openclaw.plugin.json` 中列出 `skills` 目錄（相對於外掛程式根目錄的路徑）來隨附其自身的技能。當啟用外掛程式時，外掛程式技能會載入。目前這些目錄會被合併到與 `skills.load.extraDirs` 相同的低優先級路徑中，因此同名的內建、受管理、代理程式或工作區技能會覆寫它們。
您可以透過外掛程式設定項目上的 `metadata.openclaw.requires.config` 來限制它們。請參閱 [Plugins](/en/tools/plugin) 以了解探索/設定，並參閱 [Tools](/en/tools) 以了解這些技能所教的工具表面。

## ClawHub (安裝 + 同步)

ClawHub 是 OpenClaw 的公開技能註冊表。請前往 [https://clawhub.ai](https://clawhub.ai) 瀏覽。使用原生 `openclaw skills` 指令來探索/安裝/更新技能，或者在您需要發佈/同步工作流程時使用獨立的 `clawhub` CLI。
...
- 若要了解更廣泛的威脅模型和檢查清單，請參閱 [安全性](/en/gateway/security)。
...
OpenClaw 會在 **工作階段開始時** 對符合資格的技能進行快照，並在同一工作階段的後續輪次中重複使用該清單。對技能或設定的變更將在下一個新工作階段生效。

當啟用技能監視器或是出現新的合格遠端節點時，技能也可以在會話中途重新整理（見下文）。將其視為一種**熱重新載入**：更新的列表會在下一個代理輪次中載入。

如果該會話的有效代理技能允許清單發生變化，OpenClaw 會重新整理快照，使可見技能與目前代理保持一致。

## 遠端 macOS 節點 (Linux 閘道)

如果閘道執行於 Linux，但連接了一個**macOS 節點**並且**允許 `system.run`** (Exec approvals 安全性未設定為 `deny`)，則當該節點上存在所需的二進位檔案時，OpenClaw 可以將僅限 macOS 的技能視為合格。代理應透過帶有 `host=node` 的 `exec` 工具來執行這些技能。

這取決於節點報告其指令支援情況以及透過 `system.run` 進行的 bin 探測。如果 macOS 節點稍後離線，技能仍保持可見；在節點重新連線之前，叫用可能會失敗。

## 技能監視器 (自動重新整理)

預設情況下，OpenClaw 會監視技能資料夾，並在 `SKILL.md` 檔案變更時更新技能快照。在 `skills.load` 下進行設定：
...
當技能合格時，OpenClaw 會將可用技能的精簡 XML 列表注入系統提示詞（透過 `pi-coding-agent` 中的 `formatSkillsForPrompt`）。成本是確定的：
...
- Token 計數因模型分詞器而異。粗略的 OpenAI 風格估計約為每 token 4 個字元，因此每個技能 **97 個字元 ≈ 24 個 token**，再加上您的實際欄位長度。

## 受控技能生命週期

OpenClaw 提供了一組基準技能作為安裝（npm 套件或 OpenClaw.app）的一部分的**附帶技能 (bundled skills)**。`~/.openclaw/skills` 用於本機覆寫（例如，固定/修補技能而不變更附帶副本）。工作區技能為使用者所有，並會在名稱衝突時覆寫上述兩者。

## 設定參考

請參閱 [Skills config](/en/tools/skills-config) 以取得完整的設定架構。
