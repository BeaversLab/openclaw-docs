...
您可以透過外掛程式設定項目上的 `metadata.openclaw.requires.config` 來對其進行閘道控制。請參閱 [Plugins](/zh-Hant/tools/plugin) 以了解探索/設定，並參閱 [Tools](/zh-Hant/tools) 以了解這些技能所教導的工具介面。
...
- 對於不受信任的輸入和風險工具，請優先使用沙箱執行。請參閱 [Sandboxing](/zh-Hant/gateway/sandboxing)。
...
- 如需更廣泛的威脅模型與檢查清單，請參閱 [Security](/zh-Hant/gateway/security)。
...
請參閱 [技能設定](/zh-Hant/tools/skills-config) 以了解完整的設定架構。
