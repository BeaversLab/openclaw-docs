...
<Steps>
  <Step title="選擇提供者">
    ...
    Local CLI 不需要 API 金鑰即可使用。請參閱 [提供者矩陣](#supported-providers)
    以取得完整清單。
  </Step>
...
5. 此主機的本地 `/tts` 偏好設定
6. 啟用 [模型覆寫](#model-driven-directives) 時的內聯 `[[tts:...]]` 指令
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    <ParamField path="auto" type='"off" | "always" | "inbound" | "tagged"'>
      Auto-TTS 模式。`inbound` 僅在收到語音訊息後發送音訊；`tagged` 僅在回覆包含 `[[tts:...]]` 指令或 `[[tts:text]]` 區塊時發送音訊。
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      依語音供應商 ID 索引的供應商擁有設定。舊版直接區塊 (`messages.tts.openai`、`.elevenlabs`、`.microsoft`、`.edge`) 會由 `openclaw doctor --fix` 重寫；僅提交 `messages.tts.providers.<id>`。
    </ParamField>
...
該工具接受可選的 `channel` 和 `timeoutMs` 欄位；`timeoutMs` 是以毫秒為單位的單次請求供應商逾時時間。單次請求的值會覆蓋 `messages.tts.timeoutMs`；設定的 TTS 逾時時間會覆蓋任何插件編寫的供應商預設值。
...
| `tts.enable`      | 將本地自動偏好設為 `always`。       |
| `tts.disable`     | 將本地自動偏好設為 `off`。          |
...
- [語音通話插件](/zh-Hant/plugins/voice-call)
