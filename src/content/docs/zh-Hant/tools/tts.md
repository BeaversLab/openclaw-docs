...
否。自動 TTS 預設為**關閉**。您可以在設定中透過 `messages.tts.auto` 啟用，或針對每個階段使用 `/tts always`（別名：`/tts on`）啟用。

當未設定 `messages.tts.provider` 時，OpenClaw 會依照註冊表自動選擇順序，挑選第一個設定的語音提供商。
...
- `limit` 和 `summary` 儲存在本地偏好設定中，而非主要設定檔。
...
`tts` 工具將文字轉換為語音，並返回用於
回覆傳送的音訊附件。當頻道是飛書、Matrix、Telegram 或 WhatsApp 時，
音訊將以語音訊息而非檔案附件的形式傳送。
