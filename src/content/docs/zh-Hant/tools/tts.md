...
OpenClaw 可以使用 ElevenLabs、Google Gemini、Microsoft、MiniMax 或 OpenAI 將輸出回覆轉換為音訊。
...
- **Microsoft**（主要或備援供應商；目前的內建實作使用 `node-edge-tts`）
- **MiniMax**（主要或備援供應商；使用 T2A v2 API）
- **OpenAI**（主要或備援供應商；亦用於摘要）

### Microsoft 語音備註

內建的 Microsoft 語音供應商目前透過 `node-edge-tts` 函式庫使用 Microsoft Edge 的
線上神經 TTS 服務。這是一個託管服務（非本機），
使用 Microsoft 端點，並且不需要 API 金鑰。
`node-edge-tts` 公開了語音配置選項和輸出格式，
但並非所有選項都受到該服務支援。使用 `edge` 的舊版配置和指令輸入
仍然有效，並會正規化為 `microsoft`。

由於此路徑是沒有發布 SLA 或配額的公用網路服務，
請將其視為盡力而為。如果您需要保證的限制和支援，請使用 OpenAI
或 ElevenLabs。

## 選用金鑰

如果您想要使用 OpenAI、ElevenLabs、Google Gemini 或 MiniMax：
...
否。自動 TTS 預設為**關閉**。請在設定中透過 `messages.tts.auto` 啟用，或在本機使用 `/tts on`。

當未設定 `messages.tts.provider` 時，OpenClaw 會依照註冊表自動選取順序選擇第一個已設定的語音供應商。
...
Google Gemini TTS 使用 Gemini API 金鑰路徑。在此處，受限於 Gemini API 的 Google Cloud Console API 金鑰是有效的，這與內建的 Google 圖片生成供應商使用的金鑰類型相同。解析順序為 `messages.tts.providers.google.apiKey` -> `models.providers.google.apiKey` ->
...
- `maxTextLength`：TTS 輸入的硬性上限（字元）。如果超出，`/tts audio` 將會失敗。
- `timeoutMs`：請求逾時（毫秒）。
- `prefsPath`：覆寫本地偏好設定 JSON 路徑（provider/limit/summary）。
- `apiKey` 值會回退至環境變數（`ELEVENLABS_API_KEY`/`XI_API_KEY`、`GEMINI_API_KEY`/`GOOGLE_API_KEY`、`MINIMAX_API_KEY`、`OPENAI_API_KEY`）。
...
- `providers.openai.baseUrl`：覆寫 OpenAI TTS 端點。
  ...
  - 非預設值會被視為與 OpenAI 相容的 TTS 端點，因此接受自訂模型和語音名稱。
- `providers.elevenlabs.voiceSettings`：
  - `stability`, `similarityBoost`, `style`: `0..1`
  - `useSpeakerBoost`: `true|false`
  - `speed`: `0.5..2.0` (1.0 = 正常)
- `providers.elevenlabs.applyTextNormalization`: `auto|on|off`
- `providers.elevenlabs.languageCode`: 雙字母 ISO 639-1 代碼（例如 `en`、`de`）
- `providers.elevenlabs.seed`: 整數 `0..4294967295`（盡力確保確定性）
- `providers.minimax.baseUrl`: 覆蓋 MiniMax API 基礎 URL（預設 `https://api.minimax.io`，環境變數：`MINIMAX_API_HOST`）。
...
- `providers.google.model`: Gemini TTS 模型（預設 `gemini-3.1-flash-tts-preview`）。
- `providers.google.voiceName`: Gemini 預建語音名稱（預設 `Kore`；`voice` 也可接受）。
- `providers.google.baseUrl`: 覆蓋 Gemini API 基礎 URL。僅接受 `https://generativelanguage.googleapis.com`。
  - 如果省略 `messages.tts.providers.google.apiKey`，TTS 可以在回退到環境變數之前重用 `models.providers.google.apiKey`。
...
啟用後，模型可以發出 `[[tts:...]]` 指令來覆寫單一回覆的語音，以及一個可選的 `[[tts:text]]...[[/tts:text]]` 區塊，用來提供僅應出現在音訊中的表達式標籤（笑聲、歌唱提示等）。
...
- `provider`（註冊的語音提供者 ID，例如 `openai`、`elevenlabs`、`google`、`minimax` 或 `microsoft`；需要 `allowProvider: true`）
- `voice`（OpenAI 語音）、`voiceName` / `voice_name` / `google_voice`（Google 語音）或 `voiceId`（ElevenLabs / MiniMax）
- `model`（OpenAI TTS 模型、ElevenLabs 模型 ID 或 MiniMax 模型）或 `google_model`（Google TTS 模型）
- `stability`、`similarityBoost`、`style`、`speed`、`useSpeakerBoost`
- `vol` / `volume` (MiniMax 音量，0-10)
- `pitch` (MiniMax 音調，-12 至 12)
...
- **MiniMax**: MP3 (`speech-2.8-hd` 模型，32kHz 取樣率)。不原生支援語音備忘錄格式；請使用 OpenAI 或 ElevenLabs 以保證 Opus 語音訊息。
- **Google Gemini**: Gemini API TTS 傳回原始 24kHz PCM。OpenClaw 會將其包裝為 WAV 以用於音訊附件，並針對 Talk/電話直接傳回 PCM。此路徑不支援原生 Opus 語音備忘錄格式。
- **Microsoft**: 使用 `microsoft.outputFormat` (預設 `audio-24khz-48kbitrate-mono-mp3`)。
  ...
  - 如果設定的 Microsoft 輸出格式失敗，OpenClaw 將重試使用 MP3。

OpenAI/ElevenLabs 輸出格式是依管道固定的（見上文）。
...
如果回覆超過 `maxLength` 且摘要功能關閉（或沒有用於摘要模型的 API 金鑰），則
跳過音訊並發送一般文字回覆。
...
Discord 注意事項：`/tts` 是 Discord 內建指令，因此 OpenClaw 在此
將 `/voice` 註冊為原生指令。文字 `/tts ...` 仍然有效。
...
- OpenAI 和 ElevenLabs API 失敗現在包含已解析的提供者錯誤詳情和請求 ID（當由提供者返回時），這些資訊會顯示在 TTS 錯誤/日誌中。

## Agent 工具

`tts` 工具將文字轉換為語音並傳回音訊附件以供回覆傳送。當頻道是 Feishu、Matrix、Telegram 或 WhatsApp 時，音訊會以語音訊息而非檔案附件的形式傳送。
