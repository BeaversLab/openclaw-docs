...
5. 此主機的本地 `/tts` 偏好設定
6. 當啟用 [模型覆寫](#model-driven-directives) 時，使用內聯 `[[tts:...]]` 指令
...
- 跳過非常短的回覆（少於 10 個字元）。
- 當啟用摘要時，使用
  `summaryModel` (或 `agents.defaults.model.primary`) 總結長回覆。
- 將產生的音訊附加到回覆中。
- 在 `mode: "final"` 中，在文字串流完成後仍針對串流的最終回覆發送純音訊 TTS；
  生成的媒體會經過與正常回覆附件相同的
  頻道媒體標準化程序。

如果回覆超過 `maxLength` 且摘要功能關閉 (或沒有摘要模型的 API 金鑰)，
則跳過音訊並發送正常文字回覆。
...
| 其他頻道                            | **MP3** (來自 ElevenLabs 的 `mp3_44100_128`，來自 OpenAI 的 `mp3`)。44.1 kHz / 128 kbps 是語音的預設設定。                                 |
| Talk / 電話                         | 提供商原生的 **PCM** (Inworld 22050 Hz，Google 24 kHz)，或是來自 Gradium 用於電話的 `ulaw_8000`。                                          |

各提供商備註：

- **飛書 / WhatsApp 轉碼：** 當語音訊息回覆為 MP3/WebM/WAV/M4A 格式時，頻道外掛會使用 `ffmpeg` 將其轉碼為 48 kHz Ogg/Opus。WhatsApp 透過 Baileys 使用 `ptt: true` 和 `audio/ogg; codecs=opus` 發送。如果轉換失敗：飛書會降級為附加原始檔案；WhatsApp 發送將失敗，而不是發布不相容的 PTT 資料。
- **MiniMax / 小米 MiMo：** 預設 MP3 (MiniMax `speech-2.8-hd` 為 32 kHz)；透過 `ffmpeg` 針對語音訊息目標轉碼為 48 kHz Opus。
- **Local CLI：** 使用設定的 `outputFormat`。語音訊息目標會轉換為 Ogg/Opus，電話輸出會轉換為原始 16 kHz 單聲道 PCM。
- **Google Gemini：** 傳回原始 24 kHz PCM。OpenClaw 將附件封裝為 WAV，將語音訊息目標轉碼為 48 kHz Opus，並直接為 Talk/電話傳回 PCM。
- **Inworld：** MP3 附件，原生 `OGG_OPUS` 語音訊息，用於 Talk/語音的原始 `PCM` 22050 Hz。
- **xAI：** 預設為 MP3；`responseFormat` 可以 `mp3|wav|pcm|mulaw|alaw`。使用 xAI 的批量 REST 端點 —— **不**使用串流 WebSocket TTS。原生 Opus 語音訊息格式**不**受支援。
- **Microsoft：** 使用 `microsoft.outputFormat` (預設 `audio-24khz-48kbitrate-mono-mp3`)。 Telegram `sendVoice` 接受 OGG/MP3/M4A；如果您需要保證的 Opus 語音訊息，請使用 OpenAI/ElevenLabs。如果設定的 Microsoft 格式失敗，OpenClaw 將使用 MP3 重試。
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="provider" type="string">
      語音提供商 ID。若未設定，OpenClaw 將依據註冊表自動選擇順序使用第一個已設定的提供商。舊版 `provider: "edge"` 會由 `openclaw doctor --fix` 重寫為 `"microsoft"`。
    </ParamField>
    <ParamField path="persona" type="string">
      來自 `personas` 的啟用 persona ID。會被正規化為小寫。
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      依語音提供商 ID 索引的提供商專屬設定。舊版的直接區塊（`messages.tts.openai`、`.elevenlabs`、`.microsoft`、`.edge`）會由 `openclaw doctor --fix` 重寫；僅提交 `messages.tts.providers.<id>`。
    </ParamField>
    <ParamField path="maxTextLength" type="number">
      TTS 輸入字元的硬性上限。若超過則 `/tts audio` 會失敗。
    </ParamField>
    <ParamField path="timeoutMs" type="number">
      請求逾時時間（毫秒）。
    </ParamField>
    <ParamField path="prefsPath" type="string">
      覆蓋本機偏好設定 JSON 路徑（provider/limit/summary）。預設為 `~/.openclaw/settings/tts.json`。
    </ParamField>
  </Accordion>
...
<Accordion title="Azure 語音">
  ...
  <ParamField path="outputFormat" type="string">
    標準音訊的 Azure `X-Microsoft-OutputFormat`。預設值 `audio-24khz-48kbitrate-mono-mp3`。
  </ParamField>
  <ParamField path="voiceNoteOutputFormat" type="string">
    語音訊息輸出的 Azure `X-Microsoft-OutputFormat`。預設值 `ogg-24khz-16bit-mono-opus`。
  </ParamField>
</Accordion>

<Accordion title="ElevenLabs">
  <ParamField path="apiKey" type="string">
    回退至 `ELEVENLABS_API_KEY` 或 `XI_API_KEY`。
  </ParamField>
  ...
  <ParamField path="voiceSettings" type="object">
    `stability`、`similarityBoost`、`style`（各 `0..1`）、`useSpeakerBoost`（`true|false`）、`speed`（`0.5..2.0`、`1.0` = 一般）。
  </ParamField>
...
<Accordion title="Google Gemini">
  <ParamField path="apiKey" type="string">
    會回退至 `GEMINI_API_KEY` / `GOOGLE_API_KEY`。若省略，TTS 可在環境變數回退前重用 `models.providers.google.apiKey`。
  </ParamField>
  <ParamField path="model" type="string">
  ...
  <ParamField path="speakerName" type="string">
    當您的提示詞使用具名說話者時，附加在口語文本前的可選說話者標籤。
  </ParamField>
  <ParamField path="promptTemplate" type='"audio-profile-v1"'>
    設定為 `audio-profile-v1` 可將啟用的 persona 提示詞欄位包裝在確定性的 Gemini TTS 提示詞結構中。
  </ParamField>
  <ParamField path="personaPrompt" type="string">
    附加至範本「導演備註」的 Google 特定額外 persona 提示詞文字。
  </ParamField>
  <ParamField path="baseUrl" type="string">
...
<Accordion title="Gradium">
  ...
  <Accordion title="Inworld">
    ...
    <ParamField path="modelId" type="string">預設值 `inworld-tts-1.5-max`。其他選項：`inworld-tts-1.5-mini`、`inworld-tts-1-max`、`inworld-tts-1`。</ParamField>
    <ParamField path="speakerVoiceId" type="string">預設值 `Sarah`。舊版別名：`voiceId`。</ParamField>
...
<Accordion title="Microsoft (no API key)">
  ...
  <ParamField path="outputFormat" type="string">
    Microsoft 輸出格式。預設 `audio-24khz-48kbitrate-mono-mp3`。隨附的 Edge 支援傳輸並不支援所有格式。
  </ParamField>
...
<Accordion title="MiniMax">
  <ParamField path="apiKey" type="string">
    回退至 `MINIMAX_API_KEY`。通过 `MINIMAX_OAUTH_TOKEN`、`MINIMAX_CODE_PLAN_KEY` 或 `MINIMAX_CODING_API_KEY` 進行 Token Plan 認證。
  </ParamField>
  <ParamField path="baseUrl" type="string">
...
<Accordion title="OpenAI">
  ...
  <ParamField path="instructions" type="string">
    明確的 OpenAI `instructions` 欄位。設定後，個人提示欄位將**不會**自動對應。
  </ParamField>
  <ParamField path="extraBody / extra_body" type="Record<string, unknown>">
    合併到 `/audio/speech` 請求主體中的額外 JSON 欄位，位於生成的 OpenAI TTS 欄位之後。將此用於 OpenAI 相容的端點（例如 Kokoro），這些端點需要供應商專屬金鑰（例如 `lang`）；不安全的原型金鑰會被忽略。
  </ParamField>
  <ParamField path="baseUrl" type="string">
...
<Accordion title="Volcengine (BytePlus Seed Speech)">
  ...
  <ParamField path="resourceId" type="string">
    預設值 `seed-tts-1.0`。環境變數：`VOLCENGINE_TTS_RESOURCE_ID`。當您的專案具有 TTS 2.0 權限時，請使用 `seed-tts-2.0`。
  </ParamField>
  ...
  <ParamField path="speakerVoice" type="string">
    聲音類型。預設值 `en_female_anna_mars_bigtts`。環境變數：`VOLCENGINE_TTS_VOICE`。舊版別名：`voice`。
  </ParamField>
...
<Accordion title="xAI">
  ...
  <ParamField path="speakerVoiceId" type="string">
    預設值 `eve`。可用聲音：`ara`、`eve`、`leo`、`rex`、`sal`、`una`。舊版別名：`voiceId`。
  </ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="baseUrl" type="string">預設值 `https://api.xiaomimimo.com/v1`。Env: `XIAOMI_BASE_URL`。</ParamField>
    <ParamField path="model" type="string">預設值 `mimo-v2.5-tts`。Env: `XIAOMI_TTS_MODEL`。也支援 `mimo-v2-tts`。</ParamField>
    <ParamField path="speakerVoice" type="string">預設值 `mimo_default`。Env: `XIAOMI_TTS_VOICE`。舊版別名：`voice`。</ParamField>
    <ParamField path="format" type='"mp3" | "wav"'>預設值 `mp3`。Env: `XIAOMI_TTS_FORMAT`。</ParamField>
    <ParamField path="style" type="string">選用的自然語言風格指令，作為使用者訊息發送；不會被朗讀。</ParamField>
  </Accordion>
</AccordionGroup>

## Agent 工具

`tts` 工具將文字轉換為語音並傳回音訊附件用於回覆傳遞。在飛書、Matrix、Telegram 和 WhatsApp 上，音訊會以語音訊息的形式傳遞，而不是檔案附件。當 `ffmpeg` 可用時，飛書和 WhatsApp 可以在此路徑上轉碼非 Opus 格式的 TTS 輸出。

WhatsApp 透過 Baileys 將音訊作為 PTT 語音訊息（帶有 `ptt: true` 的 `audio`）發送，並會將可見文字與 PTT 音訊**分開**發送，因為用戶端並不會在語音訊息上持續渲染字幕。

此工具接受選用的 `channel` 和 `timeoutMs` 欄位；`timeoutMs` 是以毫秒為單位的單次呼叫提供者請求逾時時間。單次呼叫的值會覆蓋 `messages.tts.timeoutMs`；已配置的 TTS 逾時設定會覆蓋任何外掛程式編寫的提供者預設值。
