...
讓 OpenClaw 透過 ACP 後端外掛程式執行外部編碼程式 (例如 Pi、Claude Code、
Cursor、Copilot、Droid、OpenClaw ACP、OpenCode、Gemini CLI 及其他
支援的 ACPX 程式)。

每次 ACP 會話的產生都會作為 [背景任務](/zh-Hant/automation/tasks) 進行追蹤。

<Note>
**ACP 是外部程式路徑，而非預設的 Codex 路徑。** 原生
Codex 應用程式伺服器外掛程式擁有 `/codex ...` 控制項以及用於代理程式回合的預設
`openai/gpt-*` 嵌入式執行環境；ACP 則擁有
`/acp ...` 控制項和 `sessions_spawn({ runtime: "acp" })` 會話。

如果您希望 Codex 或 Claude Code 以外部 MCP 用戶端身分
直接連線至現有的 OpenClaw 頻道交談，請使用
...
OpenClaw 外掛工具和內建 OpenClaw 工具預設**不**會對外開放給
ACP harness。僅當 harness
應該直接呼叫這些工具時，才在
...
<AccordionGroup>
  ...
  <Accordion title="Native Codex 路由規則">
    當啟用時應路由到 **原生 Codex 外掛程式** 的自然語言觸發條件：
    ...
    對於原生 Codex 工具事件，OpenClaw 會注入一個每輪次的原生
    hook 中繼，因此外掛程式 hook 可以阻擋 `before_tool_call`、觀察
    `after_tool_call`，並透過 OpenClaw 核准來路由 Codex `PermissionRequest` 事件。
    Codex `Stop` hooks 會被中繼到
    OpenClaw `before_agent_finalize`，外掛程式可以在那裡請求再一次
    模型傳遞，然後 Codex 完成其答案。該中繼保持
    刻意保守：它不會變更原生 Codex 工具
    引數或重寫 Codex 執行緒記錄。僅當
    您想要 ACP 執行時/會話模型時才使用明確的 ACP。嵌入式 Codex
    支援邊界記載於
    [Codex harness v1 支援合約](/zh-Hant/plugins/codex-harness-runtime#v1-support-contract) 中。
...
CLI 後端是獨立的僅文字本機備援執行階段 - 請參閱 [CLI Backends](/zh-Hant/gateway/cli-backends)。
...
- **iMessage 私訊/群組：** `match.channel="imessage"` + `match.peer.id="<handle|chat_id:*|chat_guid:*|chat_identifier:*>"`。為了穩定的群組綁定，建議使用 `chat_id:*`。
...
- 臨時執行時期綁定 (例如由執行緒聚焦流程建立的) 在存在的情況下仍然適用。
- 對於沒有明確 `cwd` 的跨代理程式 ACP 生成，OpenClaw 會從代理程式設定繼承目標代理程式工作區。
- 缺少繼承的工作區路徑會回退到後端預設 cwd；非缺少的存取失敗會顯示為生成錯誤。
...
<Tabs>
  <Tab title="From sessions_spawn">
    使用 `runtime: "acp"` 從代理轉輪或工具呼叫啟動 ACP 工作階段。
    ...
    <Note>
    `runtime` 預設為 `subagent`，因此請針對 ACP 工作階段明確設定 `runtime: "acp"`。如果省略 `agentId`，OpenClaw 在設定後會使用 `acp.defaultAgent`。`mode: "session"` 需要 `thread: true` 來保持持續的綁定對話。
    </Note>
  ...
  <Tab title="From /acp command">
    ...
    請參閱 [Slash commands](/zh-Hant/tools/slash-commands)。
...
<ParamField path="agentId" type="string">
  ACP 目標 harness ID。若設定，則回退至 `acp.defaultAgent`。
</ParamField>
<ParamField path="thread" type="boolean" default="false">
  請求執行緒綁定流程（在支援的情況下）。
</ParamField>
<ParamField path="mode" type='"run" | "session"' default="run">
  `"run"` 為單次；`"session"` 為持久。若 `thread: true` 且
  省略 `mode`，OpenClaw 可能根據執行時路徑
  預設為持久行為。`mode: "session"` 需要 `thread: true`。
</ParamField>
<ParamField path="cwd" type="string">
  請求的執行時工作目錄（由後端/執行時政策驗證）。
  若省略，ACP 啟動繼承目標代理工作區（在設定時）；
  遺失的繼承路徑回退至後端預設值，而實際存取錯誤會被回報。
</ParamField>
...
<ParamField path="resumeSessionId" type="string">
  恢復現有 ACP 會話，而不是建立新的。
  代理透過 `session/load` 重播其對話記錄。
  需要 `runtime: "acp"`。
</ParamField>
<ParamField path="streamTo" type='"parent"'>
  `"parent"` 將初始 ACP 執行進度摘要
  以系統事件串流回請求者會話。接受的回應包括
  指向會話範圍 JSONL 記錄
  (`<sessionId>.acp-stream.jsonl`) 的 `streamLogPath`，
  您可以追蹤該記錄以取得完整中繼歷史。
</ParamField>
<ParamField path="runTimeoutSeconds" type="number">
  在 N 秒後中止 ACP 子回合。`0` 將回合
  保留在閘道的無逾時路徑上。相同的值會套用至
  閘道執行和 ACP 執行時，因此停滯/配額用盡的 harness
  不會無限期地佔用父代理通道。
</ParamField>
<ParamField path="model" type="string">
  ACP 子會話的明確模型覆寫。Codex ACP 啟動
  會在 `session/new` 之前將 OpenClaw Codex 參照（如 `openai-codex/gpt-5.4`）
  正規化為 Codex ACP 啟動設定；斜線形式（如
  `openai-codex/gpt-5.4/high`）也會設定 Codex ACP 推理強度。
  其他 harness 必須通告 ACP `models` 並支援
  `session/set_model`；否則 OpenClaw/acpx 會明確失敗，
  而不是無聲地回退至目標代理預設值。
</ParamField>
<ParamField path="thinking" type="string">
  明確的思考/推理強度。對於 Codex ACP，`minimal` 對應至
  低強度，`low`/`medium`/`high`/`xhigh`
  直接對應，而 `off`
  則省略推理強度啟動覆寫。
</ParamField>
...
<Tabs>
  ...
  <Tab title="--thread auto|here|off">
    | 模式   | 行為                                                                                            |
    | ------ | --------------------------------------------------------------------------------------------------- |
    | `auto` | 在啟用的執行緒中：綁定該執行緒。在執行緒外：如果支援，則建立/綁定子執行緒。 |
    | `here` | 要求目前啟用的執行緒；如果不在執行緒中則失敗。                                                  |
...
<AccordionGroup>
  <Accordion title="互動式 ACP 會話">
    ...
    - `/acp spawn ... --bind here` 將目前對話綁定至 ACP 會話。
    - `/acp spawn ... --thread ...` 將頻道執行緒/主題綁定至 ACP 會話。
    - 透過持續設定的 `bindings[].type="acp"`，將符合條件的對話路由至同一個 ACP 會話。

    已綁定對話中的後續訊息會直接路由至
    ACP 會話，而 ACP 的輸出則會傳回至同一個
    頻道/執行緒/主題。

    OpenClaw 傳送至 harness 的內容：

    - 一般的已綁定後續訊息會以提示文字形式傳送，並僅在 harness/backend 支援時一併傳送附件。
    - `/acp` 管理指令與本機 Gateway 指令會在 ACP 分發前進行攔截。
    - 執行時期產生的完成事件會依照目標進行具體化。OpenClaw 代理程式會取得 OpenClaw 的內部執行時期環境封包；外部 ACP harness 則會取得包含子結果與指示的純文字提示。原始的 `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` 環境封包絕不應傳送至外部 harness 或儲存為 ACP 使用者對話紀錄文字。
    - ACP 對話紀錄項目使用使用者可見的觸發文字或純文字完成提示。內部事件中繼資料會盡可能在 OpenClaw 中保持結構化，且不會被視為使用者撰寫的聊天內容。
  ...
  <Accordion title="父級擁有的單次 ACP 工作階段">
    ...
    - 子項輪次在原生 sub-agent 產生所使用的相同背景通道上執行，因此緩慢的 ACP harness 不會阻擋無關的主工作階段工作。
    - 完成結果會透過任務完成公告路徑回報。OpenClaw 會在將內部完成中繼資料傳送給外部 harness 之前，將其轉換為純 ACP 提示，因此 harness 不會看到僅限 OpenClaw 的執行時期內容標記。
    - 當使用者面向的回覆有用時，父級會以正常的助理語氣重寫子項結果。

    請**勿**將此路徑視為父級與子項之間的點對點聊天。子項已經有一個完成通道可回傳給父級。

  </Accordion>
  <Accordion title="sessions_send 與 A2A 傳遞">
    `sessions_send` 可以在產生後以另一個工作階段為目標。對於一般的
    同層級工作階段，OpenClaw 會在注入訊息後使用 agent 對 agent (A2A) 後續路徑：
    ...
    該 A2A 路徑是發送者需要可見後續回應時的同層級傳送的備援方案。當不相關的工作階段可以
    看見並傳送訊息給 ACP 目標時，它會保持啟用，例如在寬鬆的
    `tools.sessions.visibility` 設定下。

    只有當請求者是其自身父級擁有的單次 ACP 子項的父級時，OpenClaw 才會略過 A2A 後續程序。在這種情況下，
    在任務完成之上執行 A2A 可能會以子項結果喚醒父級，
    將父級的回覆轉發回子項，並
    建立父級/子項回饋迴路。`sessions_send` 結果會針對該擁有子項案例回報
    `delivery.status="skipped"`，因為
    完成路徑已負責處理結果。

  </Accordion>
  <Accordion title="Resume an existing session">
    使用 `resumeSessionId` 來繼續之前的 ACP 工作階段，而不是
    重新開始。代理程式會透過 `session/load` 重播其對話歷史記錄，
    因此它能掌握先前發生事情的全部脈絡。
    ...
    - 將 Codex 工作階段從您的筆記型電腦交接到您的手機 - 告訴您的代理程式接續您之前停止的地方。
    - 繼續您最初在 CLI 中以互動方式開始的程式設計工作階段，現在透過您的代理程式以無頭模式繼續。
    - 接續因閘道重新啟動或閒置逾時而中斷的工作。

    注意事項：

    - `resumeSessionId` 僅適用於 `runtime: "acp"`；預設的子代理程式執行環境會忽略此 ACP 專用欄位。
    - `streamTo` 僅適用於 `runtime: "acp"`；預設的子代理程式執行環境會忽略此 ACP 專用欄位。
    ...
    - `resumeSessionId` 會還原上游 ACP 對話歷史記錄；`thread` 和 `mode` 仍正常套用於您正在建立的新 OpenClaw 工作階段，因此 `mode: "session"` 仍需要 `thread: true`。
    - 目標代理程式必須支援 `session/load`（Codex 和 Claude Code 支援）。
    - 如果找不到工作階段 ID，產生會失敗並顯示清楚的錯誤訊息 - 不會自動無聲地回退到新的工作階段。

  </Accordion>
  <Accordion title="部署後的冒煙測試">
    在部署閘道後，請執行實際的端對端檢查，而
    不要只信任單元測試：

    1. 在目標主機上驗證已部署的閘道版本和提交記錄。
    2. 開啟一個至即時代理程式的暫時性 ACPX 橋接器工作階段。
...
ACP 工作階段目前是在主機執行環境上運行，**而非** 在
OpenClaw sandbox 內部。

<Warning>
**安全性邊界：**

- 外部駕駛器具備根據其自身 CLI 權限和所選 `cwd` 進行讀取/寫入的能力。
- OpenClaw 的 sandbox 原則**並不**封裝 ACP 駕駛程式的執行。
- OpenClaw 仍會強制執行 ACP 功能閘道、允許的代理程式、工作階段擁有權、通道綁定和閘道傳遞原則。
- 請使用 `runtime: "subagent"` 進行由 sandbox 強制執行的 OpenClaw 原生工作。

</Warning>

目前的限制：

- 如果請求者工作階段是在 sandbox 內，則對於 `sessions_spawn({ runtime: "acp" })` 和 `/acp spawn`，ACP 的產生都會被封鎖。
...
Current-conversation 綁定和 thread 綁定都會參與步驟 2。
...
| 指令                 | 功能說明                                               | 範例                                                          |
| -------------------- | ------------------------------------------------------ | ------------------------------------------------------------- |
| `/acp spawn`         | 建立 ACP session；可選的 current bind 或 thread bind。 | `/acp spawn codex --bind here --cwd /repo`                    |
| `/acp cancel`        | 取消目標 session 中正在進行的 turn。                   | `/acp cancel agent:codex:acp:<uuid>`                          |
| `/acp steer`         | 傳送 steer 指令給正在運行的 session。                  | `/acp steer --session support inbox prioritize failing tests` |
| `/acp close`         | 關閉 session 並解除 thread 目標的綁定。                | `/acp close`                                                  |
| `/acp status`        | 顯示後端、模式、狀態、執行時選項、功能。               | `/acp status`                                                 |
| `/acp set-mode`      | 設定目標 session 的執行時模式。                        | `/acp set-mode plan`                                          |
...
| `/acp reset-options` | 移除 session 執行時選項覆寫。                          | `/acp reset-options`                                          |
| `/acp sessions`      | 從儲存庫列出最近的 ACP sessions。                      | `/acp sessions`                                               |
| `/acp doctor`        | 後端健康狀況、功能、可執行的修復措施。                 | `/acp doctor`                                                 |
| `/acp install`       | 輸出確定性安裝和啟用步驟。                             | `/acp install`                                                |

`/acp status` 顯示有效的執行時期選項以及執行時期層級和後端層級的會話識別碼。當後端缺少某項功能時，不支援的控制錯誤會清楚地顯示出來。`/acp sessions` 會讀取目前綁定或請求者會話的儲存空間；目標權杖（`session-key`、`session-id` 或 `session-label`）會透過閘道會話探索來解析，包括每個代理程式的自訂 `session.store` 根目錄。

### 執行時期選項對應

`/acp` 具有便利指令和一個通用設定器。等效的操作：

| 指令                         | 對應到                       | 備註                                                                                                                                                                              |
| ---------------------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | 執行時期設定鍵 `model`       | 對於 Codex ACP，OpenClaw 會將 `openai-codex/<model>` 標準化為配接器模型 ID，並將斜線推理後綴（例如 `openai-codex/gpt-5.4/high`）對應至 `reasoning_effort`。                       |
| `/acp set thinking <level>`  | 標準選項 `thinking`          | 當存在時，OpenClaw 會發送後端通告的等效項，優先選擇 `thinking`，然後是 `effort`、`reasoning_effort` 或 `thought_level`。對於 Codex ACP，配接器會將數值對應至 `reasoning_effort`。 |
| `/acp permissions <profile>` | 標準選項 `permissionProfile` | 當存在時，OpenClaw 會發送後端通告的等效項，例如 `approval_policy`、`permission_profile`、`permissions` 或 `permission_mode`。                                                     |
| `/acp timeout <seconds>`     | 標準選項 `timeoutSeconds`    | 當存在時，OpenClaw 會發送後端通告的等效項，例如 `timeout` 或 `timeout_seconds`。                                                                                                  |
...
有關 acpx harness 設定（Claude Code / Codex / Gemini CLI 別名）、plugin-tools 和 OpenClaw-tools MCP 橋接器，以及 ACP 權限模式，請參閱 [ACP agents - setup](/zh-Hant/tools/acp-agents-setup)。
...
| `/acp doctor` 回報啟動後後端尚未就緒                                        | 後端外掛程式遺失、已停用、被允許/拒絕原則封鎖，或其設定的可執行檔無法使用。         | 安裝/啟用後端外掛程式，重新執行 `/acp doctor`，並且如果它持續不健康，請檢查後端安裝或原則錯誤。                                                               |
| 找不到 Harness 指令                                                         | 轉接器 CLI 未安裝、外部外掛程式遺失，或是非 Codex 轉接器的首次執行 `npx` 擷取失敗。 | 執行 `/acp doctor`，在 Gateway 主機上安裝/預熱轉接器，或明確設定 acpx 代理程式指令。                                                                          |
| 從 harness 收到「找不到模型」                                               | 模型 ID 對另一個供應商/harness 有效，但對此 ACP 目標無效。                          | 使用該 harness 列出的模型、在 harness 中設定模型，或省略覆寫。                                                                                                |
| 從 harness 收到供應商驗證錯誤                                               | OpenClaw 狀況良好，但目標 CLI/供應商未登入。                                        | 登入或在 Gateway 主機環境中提供所需的供應商金鑰。                                                                                                             |
| `Unable to resolve session target: ...`                                     | 金鑰/ID/標籤權杖錯誤。                                                              | 執行 `/acp sessions`，複製確切的金鑰/標籤，然後重試。                                                                                                         |
| `--bind here requires running /acp spawn inside an active ... conversation` | `--bind here` 在沒有可綁定的對話時使用。                                            | 移至目標聊天/頻道並重試，或使用未綁定的生成 (spawn)。                                                                                                         |
| `Conversation bindings are unavailable for <channel>.`                      | 適配器缺乏當前對話 ACP 綁定功能。                                                   | 在支援的地方使用 `/acp spawn ... --thread ...`，設定頂層 `bindings[]`，或移至支援的頻道。                                                                     |
| `--thread here requires running /acp spawn inside an active ... thread`     | `--thread here` 在執行緒上下文之外使用。                                            | 移至目標執行緒或使用 `--thread auto`/`off`。                                                                                                                  |
| `Only <user-id> can rebind this channel/conversation/thread.`               | 另一位使用者擁有作用中的綁定目標。                                                  | 以擁有者身份重新綁定，或使用不同的對話或執行緒。                                                                                                              |
| `Thread bindings are unavailable for <channel>.`                            | 適配器缺乏執行緒綁定功能。                                                          | 使用 `--thread off` 或移至支援的適配器/頻道。                                                                                                                 |
| `Sandboxed sessions cannot spawn ACP sessions ...`                          | ACP 運行時位於主機端；請求者階段作業處於沙箱中。                                    | 從沙箱階段作業使用 `runtime="subagent"`，或從非沙箱階段作業執行 ACP 生成 (spawn)。                                                                            |
| `sessions_spawn sandbox="require" is unsupported for runtime="acp" ...`     | 為 ACP 運行時請求 `sandbox="require"`。                                             | 使用 `runtime="subagent"` 進行所需的沙箱隔離，或從非沙箱階段作業將 ACP 與 `sandbox="inherit"` 搭配使用。                                                      |
| `Cannot apply --model ... did not advertise model support`                  | 目標工具程式 未公開一般 ACP 模型切換功能。                                          | 使用宣傳支援 ACP `models`/`session/set_model` 的工具程式，使用 Codex ACP 模型參照，或如果工具程式有自己的啟動標誌，則直接在其中設定模型。                     |
| 綁定階段作業缺少 ACP 中繼資料                                               | 過期/已刪除的 ACP 階段作業中繼資料。                                                | 使用 `/acp spawn` 重建，然後重新綁定/聚焦執行緒。                                                                                                             |
| `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`    | `permissionMode` 在非互動式 ACP 階段作業中封鎖寫入/執行。                           | 將 `plugins.entries.acpx.config.permissionMode` 設定為 `approve-all` 並重新啟動閘道。請參閱 [權限設定](/zh-Hant/tools/acp-agents-setup#permission-configuration)。 |
| ACP 階段作業提早失敗且輸出很少                                              | 權限提示被 `permissionMode`/`nonInteractivePermissions` 封鎖。                      | 檢查 gateway 日誌中的 `AcpRuntimeError`。如需完整權限，請設定 `permissionMode=approve-all`；如需優雅降級，請設定 `nonInteractivePermissions=deny`。           |
| ACP 會話在完成工作後無限期停滯                                              | Harness 程序已結束，但 ACP 會話未報告完成。                                         | 更新 OpenClaw；目前的 acpx 清理機制會在關閉和 Gateway 啟動時收回 OpenClaw 擁有的過時包裝程式和配接器程序。                                                    |
| Harness 看到 `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>`                        | 內部事件信封從 ACP 邊界洩漏。                                                       | 更新 OpenClaw 並重新執行完成流程；外部 harness 應僅接收純文字完成提示。                                                                                       |
