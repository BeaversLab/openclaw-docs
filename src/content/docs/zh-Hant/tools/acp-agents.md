...
如果您用自然語言要求 OpenClaw「在 Codex 中運行此程式」或「在線程中啟動 Claude Code」，OpenClaw 應將該請求路由到 ACP 運行時（而非原生子代理運行時）。每次 ACP 會話生成都會被追蹤為一個[背景任務](/en/automation/tasks)。

如果您希望 Codex 或 Claude Code 作為外部 MCP 客戶端直接
連接到現有的 OpenClaw 頻道對話，請使用 [`openclaw mcp serve`](/en/cli/mcp)
而非 ACP。
...
| 重複使用本地 AI CLI 作為純文字後備模型                           | [CLI 後端](/en/gateway/cli-backends) | 非 ACP。無 OpenClaw 工具、無 ACP 控制、無工具運行時                                    |
...
- CLI 後端是獨立的純文字本機備援執行環境。請參閱 [CLI Backends](/en/gateway/cli-backends)。
...
參閱 [Slash Commands](/en/tools/slash-commands)。
...
參閱 [配置參考](/en/gateway/configuration-reference)。
