...
關於安全二進制檔（僅 stdin 的快速路徑）、直譯器綁定細節，以及如何將審核提示轉發到 Slack/Discord/Telegram（或作為原生審核用戶端執行），請參閱
...
在節點回報事件後，這些訊息會發布至 agent 的 session。被拒絕的 exec 核對是終結性的：OpenClaw 可以將拒絕通知回報給操作員或直接聊天路由，但它不會將 `Exec denied` 發布回 agent session 或喚醒 agent 工作。閘道託管的 exec 核對在指令完成時（以及可選地，在執行時間超過閾值時）會發出相同的生命週期事件。受核准閘管的 exec 會在這些訊息中重複使用核准 id 作為 `runId`，以便於關聯。

## 拒絕核准的行為

當非同步 exec 核對被拒絕時，OpenClaw 會將該請求視為終結性請求。它可以向操作員或直接聊天路由顯示簡潔的拒絕訊息，但不會透過 agent session 傳回拒絕指引。這能防止被拒絕的指令變成另一個模型輪次，並避免 agent 重複使用相同指令先前執行的輸出。
...
- 核准僅適用於來自**已授權發送者**的主機 exec 請求。未授權的發送者無法發出 `/exec`。
- `/exec security=full` 是授權操作員在 session 層級的便利功能，設計上會略過核准。若要徹底封鎖主機 exec，請將核准安全性設定為 `deny` 或透過工具原則拒絕 `exec` 工具。
