...
對於非同步工具，OpenClaw 會向提供者提交請求，立即返回任務 ID，並在任務分帳中追蹤該工作。在工作執行期間，Agent 會繼續回應其他訊息。當提供者完成處理後，OpenClaw 會使用生成的媒體路徑喚醒 Agent，以便它能告知使用者並透過訊息工具轉發結果。如果請求者的工作階段處於非活躍狀態，或其主動喚醒失敗，且仍有部分生成的媒體未透過訊息工具傳送，OpenClaw 將發送一個僅包含缺失媒體的等幂直接備援方案。已透過訊息工具傳送的媒體將不會再次發布。
