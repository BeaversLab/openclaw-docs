...
Deepgram、DeepInfra、ElevenLabs、Mistral、OpenAI、SenseAudio 和 xAI 在配置後，均可透過批次 `tools.media.audio` 路徑轉錄輸入音訊。針對提及閘門或指令解析進行預檢的頻道外掛程式會在輸入上下文中標記已轉錄的附件，因此共享的媒體理解流程會重複使用該轉錄文本，而不會對相同的音訊進行第二次 STT 呼叫。
