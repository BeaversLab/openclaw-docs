...
對於非同步工具，OpenClaw 會向提供者提交請求，立即傳回任務 ID，並在任務分類帳中追蹤該工作。在工作執行時，Agent 會繼續回應其他訊息。當提供者完成時，OpenClaw 會使用生成的媒體路徑喚醒 Agent，以便它能告知使用者並透過訊息工具轉發結果。如果請求者會話處於非活動狀態，且訊息工具傳遞中仍缺少部分生成的媒體，OpenClaw 將傳送僅包含缺失媒體的冪等直接回退。已透過訊息工具傳遞的媒體不會再次張貼。
