...
如果您使用 `OPENROUTER_API_KEY`、在 `plugins.entries.perplexity.config.webSearch.apiKey` 中設定了 `sk-or-...` 金鑰，或設定了 `plugins.entries.perplexity.config.webSearch.baseUrl` / `model`，提供者將會切換至 chat-completions 路徑，並傳回附帶引用的 AI 綜合答案，而非結構化的 Search API 結果。
...
如果您已經在針對 Perplexity Sonar 使用 OpenRouter，請保留 `provider: "perplexity"` 並在 Gateway 環境中設定 `OPENROUTER_API_KEY`，或者在 `plugins.entries.perplexity.config.webSearch.apiKey` 中儲存 `sk-or-...` 金鑰。
...
對於 gateway 安裝，請將其放在
`~/.openclaw/.env` 中（或您的服務環境）。請參閱 [環境變數](/zh-Hant/help/faq#env-vars-and-env-loading)。
...
對於舊版 Sonar/OpenRouter 相容路徑：

- `query`、`count` 和 `freshness` 均可接受
- `count` 在此僅用於相容性；回應仍為一個帶有引用的綜合
  答案，而非 N 個結果的清單
...
- Sonar/OpenRouter 相容模式會回傳一個附帶引用的綜合答案，而不是結構化的結果列
...
- [Brave Search](/zh-Hant/tools/brave-search) -- 具備國家/語言篩選器的結構化結果
- [Exa Search](/zh-Hant/tools/exa-search) -- 具備內容擷取功能的神經搜尋
