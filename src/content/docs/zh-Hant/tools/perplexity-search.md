...
- OpenRouter 或明確的 `plugins.entries.perplexity.config.webSearch.baseUrl` / `model` 會將 Perplexity 切換回 Sonar 聊天完成以維持相容性
- Sonar/OpenRouter 相容性會傳回一個帶有引用的合成答案，而不是結構化結果行
