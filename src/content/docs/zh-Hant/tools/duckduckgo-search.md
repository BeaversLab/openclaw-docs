...
<Warning>DuckDuckGo 是一個 **實驗性、非官方** 的整合功能，它從 DuckDuckGo 的非 JavaScript 搜尋頁面擷取結果 — 而非透過官方 API。預期可能會因為機器人驗證頁面或 HTML 變更而偶爾失效。</Warning>
...
- **實驗性** — 從 DuckDuckGo 的非 JavaScript HTML
  搜尋頁面蒐集結果，而非透過官方 API 或 SDK
- **機器人驗證風險** — 在大量或自動化使用時，DuckDuckGo 可能會顯示
  CAPTCHA 或封鎖請求
- **HTML 解析** — 結果取決於頁面結構，該結構可能會
  在未預告的情況下變更
- **自動偵測順序** — DuckDuckGo 是自動偵測中的第一個免金鑰備選
  (順序 100)。具有已設定金鑰的 API 提供者會優先執行，然後是 Ollama Web Search (順序 110)，再來是 SearXNG (順序 200)
- **未設定時，安全搜尋預設為中等**
