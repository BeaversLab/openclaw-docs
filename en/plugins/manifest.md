...
[Capability model](/en/tools/plugin#public-capability-model).
...
See [Configuration reference](/en/configuration) for the full `plugins.*` schema.
...
- `providerAuthChoices` is the cheap metadata path for auth-choice pickers,
  ...
  [Provider runtime hooks](/en/tools/plugin#provider-runtime-hooks).
