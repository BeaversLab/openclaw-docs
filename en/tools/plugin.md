...
Npm specs are registry-only. See [install rules](/en/cli/plugins#install) for
details on pinning, prerelease gating, and supported spec formats.

3. Restart the Gateway, then configure under `plugins.entries.<id>.config`.

See [Voice Call](/en/plugins/voice-call) for a concrete example plugin.
Looking for third-party listings? See [Community plugins](/en/plugins/community).
Need the bundle compatibility details? See [Plugin bundles](/en/plugins/bundles).
...
See [Plugin bundles](/en/plugins/bundles) for the exact detection rules, mapping
...
and JSON Schema instead. See [Plugin manifest](/en/plugins/manifest).
...
Tool authoring guide: [Plugin agent tools](/en/plugins/agent-tools).
...
[Configuration reference](/en/configuration) for the full config schema.
...
Declare `kind` in your [plugin manifest](/en/plugins/manifest).
...
See [`openclaw plugins` CLI reference](/en/cli/plugins) for full details on each
...
See the dedicated guide: [Plugin agent tools](/en/plugins/agent-tools).
...
See [Voice Call](/en/plugins/voice-call) and `extensions/voice-call/README.md` for setup and usage.
