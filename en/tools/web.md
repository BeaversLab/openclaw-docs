...
See [Brave Search setup](/en/tools/brave-search), [Perplexity Search setup](/en/tools/perplexity-search), and [Tavily Search setup](/en/tools/tavily) for provider-specific details.
...
Tavily `web_search` supports `query` and `count` (up to 20 results). For Tavily-specific controls like `search_depth`, `topic`, `include_answer`, or domain filters, use `tavily_search` from the bundled Tavily plugin. For URL content extraction, use `tavily_extract`. See [Tavily](/en/tools/tavily) for details.
