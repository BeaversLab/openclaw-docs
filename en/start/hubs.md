---
summary: "Hubs that link to every OpenClaw doc"
read_when:
  - You want a complete map of the documentation
title: "Docs Hubs"
---

# Docs hubs

Use these hubs to discover every page, including deep dives and reference docs that don’t appear in the left nav.

## Start here

- [Index](/en/)
- [Getting Started](/en/start/getting-started)
- [Onboarding](/en/start/onboarding)
- [Wizard](/en/start/wizard)
- [Setup](/en/start/setup)
- [Dashboard (local Gateway)](http://127.0.0.1:18789/)
- [Help](/en/help)
- [Configuration](/en/gateway/configuration)
- [Configuration examples](/en/gateway/configuration-examples)
- [OpenClaw assistant](/en/start/openclaw)
- [Showcase](/en/start/showcase)
- [Lore](/en/start/lore)

## Installation + updates

- [Docker](/en/install/docker)
- [Nix](/en/install/nix)
- [Updating / rollback](/en/install/updating)
- [Bun workflow (experimental)](/en/install/bun)

## Core concepts

- [Architecture](/en/concepts/architecture)
- [Network hub](/en/network)
- [Agent runtime](/en/concepts/agent)
- [Agent workspace](/en/concepts/agent-workspace)
- [Memory](/en/concepts/memory)
- [Agent loop](/en/concepts/agent-loop)
- [Streaming + chunking](/en/concepts/streaming)
- [Multi-agent routing](/en/concepts/multi-agent)
- [Compaction](/en/concepts/compaction)
- [Sessions](/en/concepts/session)
- [Sessions (alias)](/en/concepts/sessions)
- [Session pruning](/en/concepts/session-pruning)
- [Session tools](/en/concepts/session-tool)
- [Queue](/en/concepts/queue)
- [Slash commands](/en/tools/slash-commands)
- [RPC adapters](/en/reference/rpc)
- [TypeBox schemas](/en/concepts/typebox)
- [Timezone handling](/en/concepts/timezone)
- [Presence](/en/concepts/presence)
- [Discovery + transports](/en/gateway/discovery)
- [Bonjour](/en/gateway/bonjour)
- [Channel routing](/en/concepts/channel-routing)
- [Groups](/en/concepts/groups)
- [Group messages](/en/concepts/group-messages)
- [Model failover](/en/concepts/model-failover)
- [OAuth](/en/concepts/oauth)

## Providers + ingress

- [Chat channels hub](/en/channels)
- [Model providers hub](/en/providers/models)
- [WhatsApp](/en/channels/whatsapp)
- [Telegram](/en/channels/telegram)
- [Telegram (grammY notes)](/en/channels/grammy)
- [Slack](/en/channels/slack)
- [Discord](/en/channels/discord)
- [Mattermost](/en/channels/mattermost) (plugin)
- [Signal](/en/channels/signal)
- [iMessage](/en/channels/imessage)
- [Location parsing](/en/channels/location)
- [WebChat](/en/web/webchat)
- [Webhooks](/en/automation/webhook)
- [Gmail Pub/Sub](/en/automation/gmail-pubsub)

## Gateway + operations

- [Gateway runbook](/en/gateway)
- [Gateway pairing](/en/gateway/pairing)
- [Gateway lock](/en/gateway/gateway-lock)
- [Background process](/en/gateway/background-process)
- [Health](/en/gateway/health)
- [Heartbeat](/en/gateway/heartbeat)
- [Doctor](/en/gateway/doctor)
- [Logging](/en/gateway/logging)
- [Sandboxing](/en/gateway/sandboxing)
- [Dashboard](/en/web/dashboard)
- [Control UI](/en/web/control-ui)
- [Remote access](/en/gateway/remote)
- [Remote gateway README](/en/gateway/remote-gateway-readme)
- [Tailscale](/en/gateway/tailscale)
- [Security](/en/gateway/security)
- [Troubleshooting](/en/gateway/troubleshooting)

## Tools + automation

- [Tools surface](/en/tools)
- [OpenProse](/en/prose)
- [CLI reference](/en/cli)
- [Exec tool](/en/tools/exec)
- [Elevated mode](/en/tools/elevated)
- [Cron jobs](/en/automation/cron-jobs)
- [Cron vs Heartbeat](/en/automation/cron-vs-heartbeat)
- [Thinking + verbose](/en/tools/thinking)
- [Models](/en/concepts/models)
- [Sub-agents](/en/tools/subagents)
- [Agent send CLI](/en/tools/agent-send)
- [Terminal UI](/en/tui)
- [Browser control](/en/tools/browser)
- [Browser (Linux troubleshooting)](/en/tools/browser-linux-troubleshooting)
- [Polls](/en/automation/poll)

## Nodes, media, voice

- [Nodes overview](/en/nodes)
- [Camera](/en/nodes/camera)
- [Images](/en/nodes/images)
- [Audio](/en/nodes/audio)
- [Location command](/en/nodes/location-command)
- [Voice wake](/en/nodes/voicewake)
- [Talk mode](/en/nodes/talk)

## Platforms

- [Platforms overview](/en/platforms)
- [macOS](/en/platforms/macos)
- [iOS](/en/platforms/ios)
- [Android](/en/platforms/android)
- [Windows (WSL2)](/en/platforms/windows)
- [Linux](/en/platforms/linux)
- [Web surfaces](/en/web)

## macOS companion app (advanced)

- [macOS dev setup](/en/platforms/mac/dev-setup)
- [macOS menu bar](/en/platforms/mac/menu-bar)
- [macOS voice wake](/en/platforms/mac/voicewake)
- [macOS voice overlay](/en/platforms/mac/voice-overlay)
- [macOS WebChat](/en/platforms/mac/webchat)
- [macOS Canvas](/en/platforms/mac/canvas)
- [macOS child process](/en/platforms/mac/child-process)
- [macOS health](/en/platforms/mac/health)
- [macOS icon](/en/platforms/mac/icon)
- [macOS logging](/en/platforms/mac/logging)
- [macOS permissions](/en/platforms/mac/permissions)
- [macOS remote](/en/platforms/mac/remote)
- [macOS signing](/en/platforms/mac/signing)
- [macOS release](/en/platforms/mac/release)
- [macOS gateway (launchd)](/en/platforms/mac/bundled-gateway)
- [macOS XPC](/en/platforms/mac/xpc)
- [macOS skills](/en/platforms/mac/skills)
- [macOS Peekaboo](/en/platforms/mac/peekaboo)

## Workspace + templates

- [Skills](/en/tools/skills)
- [ClawHub](/en/tools/clawhub)
- [Skills config](/en/tools/skills-config)
- [Default AGENTS](/en/reference/AGENTS.default)
- [Templates: AGENTS](/en/reference/templates/AGENTS)
- [Templates: BOOTSTRAP](/en/reference/templates/BOOTSTRAP)
- [Templates: HEARTBEAT](/en/reference/templates/HEARTBEAT)
- [Templates: IDENTITY](/en/reference/templates/IDENTITY)
- [Templates: SOUL](/en/reference/templates/SOUL)
- [Templates: TOOLS](/en/reference/templates/TOOLS)
- [Templates: USER](/en/reference/templates/USER)

## Experiments (exploratory)

- [Onboarding config protocol](/en/experiments/onboarding-config-protocol)
- [Cron hardening notes](/en/experiments/plans/cron-add-hardening)
- [Group policy hardening notes](/en/experiments/plans/group-policy-hardening)
- [Research: memory](/en/experiments/research/memory)
- [Model config exploration](/en/experiments/proposals/model-config)

## Testing + release

- [Testing](/en/reference/test)
- [Release checklist](/en/reference/RELEASING)
- [Device models](/en/reference/device-models)
