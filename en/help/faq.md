...
4. **Deep probes**
   ...
   Runs gateway health checks + provider probes (requires a reachable gateway). See [Health](/en/gateway/health).
...
6. **Run the doctor (repairs)**
   ...
   Repairs/migrates config/state + runs health checks. See [Doctor](/en/gateway/doctor).
...
7. **Gateway snapshot**
   ...
   Asks the running gateway for a full snapshot (WS-only). See [Health](/en/gateway/health).
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    Install docs: [Install](/en/install), [Installer flags](/en/install/installer), [Updating](/en/install/updating).
...
<Accordion title="How do I open the dashboard after onboarding?">
  The wizard opens your browser with a clean (non-tokenized) dashboard URL right after onboarding
  and also prints the link in the summary. Keep that tab open; if it didn't launch, copy/paste the
  ...
  <Accordion title="Does it run on Raspberry Pi?">
    ...
    local screen/camera/canvas or command execution. See [Nodes](/en/nodes).
  ...
  <Accordion title="It is stuck on wake up my friend / onboarding will not hatch. What now?">
    ...
    is pointed at the right Gateway. See [Remote access](/en/gateway/remote).
  ...
  <Accordion title="Cannot access docs.openclaw.ai (SSL error)">
    ...
    detail: [Troubleshooting](/en/help/faq#cannot-access-docsopenclaw-ai-ssl-error).
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    More detail: [Development channels](/en/install/development-channels) and [Installer flags](/en/install/installer).
  ...
  <Accordion title="How do I try the latest bits?">
    ...
    Docs: [Update](/en/cli/update), [Development channels](/en/install/development-channels),
    [Install](/en/install).
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    More options: [Installer flags](/en/install/installer).
  ...
  <Accordion title="How do I install OpenClaw on Linux?">
    ...
    - Linux quick path + service install: [Linux](/en/platforms/linux).
    - Full walkthrough: [Getting Started](/en/start/getting-started).
    - Installer + updates: [Install & updates](/en/install/updating).
  ...
  <Accordion title="Where are the cloud/VPS install guides?">
    ...
    - [VPS hosting](/en/vps) (all providers in one place)
    ...
    Hub: [Platforms](/en/platforms). Remote access: [Gateway remote](/en/gateway/remote).
...
<Accordion title="How does Anthropic setup-token auth work?">
  `claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in
  the web console). You can run it on **any machine**. Choose **Anthropic token (paste
  setup-token)** in onboarding or paste it with `openclaw models auth paste-token --provider
  anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an
  API key (no auto-refresh). More detail: [OAuth](/en/concepts/oauth).
</Accordion>
  ...
  <Accordion title="Where do I find an Anthropic setup-token?">
    ...
    Copy the token it prints, then choose **Anthropic token (paste setup-token)** in onboarding. If you want to run it on the gateway host, use `openclaw models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `openclaw models auth paste-token --provider anthropic`. See [Anthropic](/en/providers/anthropic).

  </Accordion>

  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    Yes - via **setup-token**. OpenClaw no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/en/providers/anthropic) and [OAuth](/en/concepts/oauth).
...
<Accordion title="Is AWS Bedrock supported?">
  Yes - via pi-ai's **Amazon Bedrock (Converse)** provider with **manual config**. You must supply
  AWS credentials/region on the gateway host and add a Bedrock provider entry in your models config.
  See [Amazon Bedrock](/en/providers/bedrock) and [Model providers](/en/providers/models). If you
  prefer a managed key flow, an OpenAI-compatible proxy in front of Bedrock is still a valid option.
</Accordion>

<Accordion title="How does Codex auth work?">
  OpenClaw supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the
  OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model
  providers](/en/concepts/model-providers) and [Onboarding (CLI)](/en/start/wizard).
</Accordion>
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    ...
    See [OAuth](/en/concepts/oauth), [Model providers](/en/concepts/model-providers), and [Onboarding (CLI)](/en/start/wizard).
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    ...
    This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/en/concepts/model-providers).

  </Accordion>

<Accordion title="Is a local model OK for casual chats?">
  Usually no. OpenClaw needs large context + strong safety; small cards truncate and leak. If you
  must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see
  [/gateway/local-models](/en/gateway/local-models). Smaller/quantized models increase
  prompt-injection risk - see [Security](/en/gateway/security).
</Accordion>

<Accordion title="How do I keep hosted model traffic in a specific region?">
  Pick region-pinned endpoints. OpenRouter exposes US-hosted options for MiniMax, Kimi, and GLM;
  choose the US-hosted variant to keep data in-region. You can still list Anthropic/OpenAI alongside
  these by using `models.mode: "merge"` so fallbacks stay available while respecting the regioned
  provider you select.
</Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    ...
    You only need a Mac **for macOS-only tools**. For iMessage, use [BlueBubbles](/en/channels/bluebubbles) (recommended) - the BlueBubbles server runs on any Mac, and the Gateway can run on Linux or elsewhere. If you want other macOS-only tools, run the Gateway on a Mac or pair a macOS node.
  ...
  <Accordion title="Do I need a Mac mini for iMessage support?">
    ...
    any Mac works. **Use [BlueBubbles](/en/channels/bluebubbles)** (recommended) for iMessage - the BlueBubbles server runs on macOS, while the Gateway can run on Linux or elsewhere.
    ...
    Docs: [BlueBubbles](/en/channels/bluebubbles), [Nodes](/en/nodes),
...
<Accordion title="Can multiple people use one WhatsApp number with different OpenClaw instances?">
  Yes, via **multi-agent routing**. Bind each sender's WhatsApp **DM** (peer `kind: "direct"`,
  sender E.164 like `+15551234567`) to a different `agentId`, so each person gets their own
  workspace and session store. Replies still come from the **same WhatsApp account**, and DM access
  control (`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`) is global per WhatsApp
  account. See [Multi-Agent Routing](/en/concepts/multi-agent) and
  [WhatsApp](/en/channels/whatsapp).
</Accordion>

<Accordion title='Can I run a "fast chat" agent and an "Opus for coding" agent?'>
  Yes. Use multi-agent routing: give each agent its own default model, then bind inbound routes
  (provider account or specific peers) to each agent. Example config lives in [Multi-Agent
  Routing](/en/concepts/multi-agent). See also [Models](/en/concepts/models) and
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **OpenClaw-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/en/tools/browser).
  ...
  <Accordion title="How important is it to run OpenClaw on a dedicated machine?">
    ...
    If you want the best of both worlds, keep the Gateway on a dedicated host and pair your laptop as a **node** for local screen/camera/exec tools. See [Nodes](/en/nodes).
    For security guidance, read [Security](/en/gateway/security).
  ...
  <Accordion title="Can I run OpenClaw in a VM and what are the requirements?">
    ...
    compatibility. See [Windows](/en/platforms/windows), [VPS hosting](/en/vps).
    If you are running macOS in a VM, see [macOS VM](/en/install/macos-vm).
...
<AccordionGroup>
  ...
  <Accordion title="Can OpenClaw help with lead gen, outreach, ads, and blogs for a SaaS?">
    ...
    Docs: [Security](/en/gateway/security).
...
<AccordionGroup>
  ...
  <Accordion title="How can I use different models for different tasks?">
    ...
    See [Cron jobs](/en/automation/cron-jobs), [Multi-Agent Routing](/en/concepts/multi-agent), and [Slash commands](/en/tools/slash-commands).
  ...
  <Accordion title="How do thread-bound subagent sessions work on Discord?">
    ...
    Docs: [Sub-agents](/en/tools/subagents), [Discord](/en/channels/discord), [Configuration Reference](/en/gateway/configuration-reference), [Slash commands](/en/tools/slash-commands).
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.openclaw/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/en/tools/skills) and [ClawHub](/en/tools/clawhub).
...
<AccordionGroup>
  ...
  <Accordion title="Can I keep DMs personal but make groups public/sandboxed with one agent?">
    ...
    Setup walkthrough + example config: [Groups: personal DMs + public groups](/en/channels/groups#pattern-personal-dms-public-groups-single-agent)

    Key config reference: [Gateway configuration](/en/gateway/configuration-reference#agentsdefaultssandbox)
...
<Accordion title="How do I bind a host folder into the sandbox?">
  ...
  `"/home/user/src:/src:ro"`). Global + per-agent binds merge; per-agent binds are ignored when
  `scope: "shared"`. Use `:ro` for anything sensitive and remember binds bypass the sandbox
  filesystem walls. See [Sandboxing](/en/gateway/sandboxing#custom-bind-mounts) and [Sandbox vs Tool
  Policy vs
  Elevated](/en/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check) for
...
<AccordionGroup>
  <Accordion title="Is all data used with OpenClaw saved locally?">
    ...
    Related: [Agent workspace](/en/concepts/agent-workspace), [Memory](/en/concepts/memory).
  ...
  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    ...
    See [Agent workspace](/en/concepts/agent-workspace) and [Memory](/en/concepts/memory).
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Docs: [Agent workspace](/en/concepts/agent-workspace).

  </Accordion>

<Accordion title="How do I completely uninstall OpenClaw?">
  See the dedicated guide: [Uninstall](/en/install/uninstall).
</Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="How do I use Brave for browser control?">
    ...
    See the full config examples in [Browser](/en/tools/browser#use-brave-or-another-chromium-based-browser).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Can two OpenClaw instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    ...
    via SSH/Tailscale (see [Remote access](/en/gateway/remote)).
...
<Accordion title="Is there an API / RPC way to apply config?">
  Yes. `config.apply` validates + writes the full config and restarts the Gateway as part of the
  operation.
</Accordion>
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    This keeps the gateway bound to loopback and exposes HTTPS via Tailscale. See [Tailscale](/en/gateway/tailscale).
  ...
  <Accordion title="Should I install on a second laptop or just add a node?">
    ...
    **node**. That keeps a single Gateway and avoids duplicated config. Local node tools are
    ...
    Docs: [Nodes](/en/nodes), [Nodes CLI](/en/cli/nodes), [Multiple gateways](/en/gateway/multiple-gateways).
...
<AccordionGroup>
  ...
  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    ...
    See [/concepts/model-providers](/en/concepts/model-providers) and [/environment](/en/help/environment).
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of OpenClaw instances (one CEO and many agents)?">
    ...
    Docs: [Multi-agent routing](/en/concepts/multi-agent), [Sub-agents](/en/tools/subagents), [Agents CLI](/en/cli/agents).
  ...
  <Accordion title="How do I completely reset OpenClaw but keep it installed?">
    ...
    - Onboarding also offers **Reset** if it sees an existing config. See [Onboarding (CLI)](/en/start/wizard).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    Per-agent overrides use `agents.list[].heartbeat`. Docs: [Heartbeat](/en/gateway/heartbeat).
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    Docs: [WhatsApp](/en/channels/whatsapp), [Directory](/en/cli/directory), [Logs](/en/cli/logs).
  ...
  <Accordion title="Why does OpenClaw not reply in a group?">
    ...
    See [Groups](/en/channels/groups) and [Group messages](/en/channels/group-messages).

  </Accordion>

<Accordion title="Do groups/threads share context with DMs?">
  Direct chats collapse to the main session by default. Groups/channels have their own session keys,
  and Telegram topics / Discord threads are separate sessions. See [Groups](/en/channels/groups) and
  [Group messages](/en/channels/group-messages).
</Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="What model do you recommend?">
    ...
    **For routine/low-stakes chat:** use cheaper fallback models and route by agent role.
    ...
    MiniMax has its own docs: [MiniMax](/en/providers/minimax) and
    ...
    injection and unsafe behavior. See [Security](/en/gateway/security).

    More context: [Models](/en/concepts/models).
...
<Accordion title="What do OpenClaw, Flawd, and Krill use for models?">
  - These deployments can differ and may change over time; there is no fixed provider
  recommendation. - Check the current runtime setting on each gateway with `openclaw models status`.
  ...
  <Accordion title="Can I use GPT 5.2 for daily tasks and Codex 5.3 for coding?">
    ...
    See [Models](/en/concepts/models) and [Slash commands](/en/tools/slash-commands).
...
Related: [/concepts/oauth](/en/concepts/oauth) (OAuth flows, token storage, multi-account patterns)
...
<AccordionGroup>
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Still stuck? Run `openclaw status --all` and follow [Troubleshooting](/en/gateway/troubleshooting). See [Dashboard](/en/web/dashboard) for auth details.
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    See [Troubleshooting](/en/gateway/troubleshooting) for more.
  ...
  <Accordion title="The Gateway is up but replies never arrive. What should I check?">
    ...
    Docs: [Channels](/en/channels), [Troubleshooting](/en/gateway/troubleshooting), [Remote access](/en/gateway/remote).
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    Docs: [Dashboard](/en/web/dashboard), [Remote access](/en/gateway/remote), [Troubleshooting](/en/gateway/troubleshooting).
...
<AccordionGroup>
  ...
  <Accordion title="Is prompt injection only a concern for public bots?">
    ...
    Details: [Security](/en/gateway/security).
  ...
  <Accordion title="Should my bot have its own email, GitHub account, or phone number?">
    ...
    Docs: [Security](/en/gateway/security), [Pairing](/en/channels/pairing).
...
<Accordion title="Can I use cheaper models for personal assistant tasks?">
  Yes, **if** the agent is chat-only and the input is trusted. Smaller tiers are more susceptible to
  instruction hijacking, so avoid them for tool-enabled agents or when reading untrusted content. If
  you must use a smaller model, lock down tools and run inside a sandbox. See
  [Security](/en/gateway/security).
</Accordion>
...
<AccordionGroup>
  <Accordion title="What is the default model for Anthropic with an API key?">
    In OpenClaw, credentials and model selection are separate. Setting `ANTHROPIC_API_KEY` (or
    storing an Anthropic API key in auth profiles) enables authentication, but the actual default
    model is whatever you configure in `agents.defaults.model.primary` (for example,
    `anthropic/claude-sonnet-4-6` or `anthropic/claude-opus-4-6`). If you see `No credentials found
    for profile "anthropic:default"`, it means the Gateway couldn't find Anthropic credentials in
    the expected `auth-profiles.json` for the agent that's running.
  </Accordion>
</AccordionGroup>
