...
- API 金鑰：`CLOUDFLARE_AI_GATEWAY_API_KEY`（您的供應商 API 金鑰，用於透過 Gateway 發出的請求）
...
如果您在 Cloudflare 中啟用了 Gateway 驗證，請新增 `cf-aig-authorization` 標頭（這除了您的提供者 API 金鑰之外）。
...
如果 Gateway 作為守護程序（launchd/systemd）運行，請確保 `CLOUDFLARE_AI_GATEWAY_API_KEY` 對該程序可用（例如，在 `~/.openclaw/.env` 中或透過 `env.shellEnv`）。
