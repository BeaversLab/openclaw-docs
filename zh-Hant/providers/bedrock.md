...
read_when:
  - 您想將 Amazon Bedrock 模型與 OpenClaw 搭配使用
  - 您需要設定 AWS 憑證/區域 以進行模型呼叫
...
如果偵測到 AWS 憑證，OpenClaw 可以自動探索支援 **串流** 和 **文字輸出** 的 Bedrock 模型。探索使用 `bedrock:ListFoundationModels` 並會進行快取 (預設：1 小時)。
...
當在附加了 IAM 角色的 EC2 執行個體上執行 OpenClaw 時，AWS SDK 將自動使用執行個體元資料服務 (IMDS) 進行驗證。然而，OpenClaw 的憑證偵測目前僅檢查環境變數，而不檢查 IMDS 憑證。
