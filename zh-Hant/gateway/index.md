...
<CardGroup cols={2}>
  ...
  <Card title="Secrets plan contract" icon="shield-check" href="/zh-Hant/gateway/secrets-plan-contract">
...
<Note>
Gateway 組態重新載入會監看有效的組態檔案路徑（解析自 profile/state 預設值，或設定時的 `OPENCLAW_CONFIG_PATH`）。
