...
OpenClaw 可以為 Gateway 儀表板和 WebSocket 連接埠自動設定 Tailscale **Serve** (tailnet) 或 **Funnel** (公開)。這讓 Gateway 綁定在 loopback 上，同時由 Tailscale 提供 HTTPS、路由以及 (針對 Serve) 身份標頭。
...
- `password` (透過 `OPENCLAW_GATEWAY_PASSWORD` 或設定共用密鑰)
...
daemon (`tailscale whois`) 解析 `x-forwarded-for` 位址並在接受前與標頭比對來驗證身分。
僅當請求來自 loopback 並帶有 Tailscale 的 `x-forwarded-for`、`x-forwarded-proto` 和 `x-forwarded-host`
標頭時，OpenClaw 才會將請求視為 Serve。
...
token/密碼驗證。
若需要明確的認證，請設定 `gateway.auth.allowTailscale: false` 或
強制使用 `gateway.auth.mode: "password"`。
...
- `tailscale.mode: "funnel"` 會拒絕啟動，除非 auth mode 為 `password`，以避免公開暴露。
...
- `gateway.bind: "auto"` 優先使用 loopback；如果您希望僅使用 Tailnet，請使用 `tailnet`。
- Serve/Funnel 僅公開 **Gateway control UI + WS**。節點透過
  相同的 Gateway WS 端點連線，因此 Serve 可用於節點存取。

## 瀏覽器控制（遠端 Gateway + 本機瀏覽器）

如果您在一台機器上執行 Gateway，但希望驅動另一台機器上的瀏覽器，
請在瀏覽器機器上執行 **node host**，並確保兩者位於同一個 tailnet 中。
Gateway 會將瀏覽器操作代理至節點；不需要額外的控制伺服器或 Serve URL。

避免使用 Funnel 進行瀏覽器控制；請將節點配對視為操作員存取。

## Tailscale 先決條件 + 限制

- Serve 需要為您的 tailnet 啟用 HTTPS；若缺少則 CLI 會提示。
- Serve 會注入 Tailscale 身份標頭；Funnel 則不會。
- Funnel 需要 Tailscale v1.38.3+、MagicDNS、啟用 HTTPS 以及 funnel node 屬性。
