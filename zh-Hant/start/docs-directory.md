---
summary: "最常用的 OpenClaw 文件的精選連結。"
read_when:
  - You want quick access to key docs pages
title: "文件目錄"
---

<Note>
  此頁面是精選索引。如果您是新手，請從[開始使用](/zh-Hant/start/getting-started)開始。若要查看完整的文件地圖，請參閱[文件中心](/zh-Hant/start/hubs)。
</Note>

## 從這裡開始

- [文件中心（所有頁面連結）](/zh-Hant/start/hubs)
- [說明](/zh-Hant/help)
- [設定](/zh-Hant/gateway/configuration)
- [設定範例](/zh-Hant/gateway/configuration-examples)
- [斜線指令](/zh-Hant/tools/slash-commands)
- [多代理程式路由](/zh-Hant/concepts/multi-agent)
- [更新與還原](/zh-Hant/install/updating)
- [配對（DM 與節點）](/zh-Hant/channels/pairing)
- [Nix 模式](/zh-Hant/install/nix)
- [OpenClaw 助理設定](/zh-Hant/start/openclaw)
- [技能](/zh-Hant/tools/skills)
- [技能設定](/zh-Hant/tools/skills-config)
- [工作區範本](/zh-Hant/reference/templates/AGENTS)
- [RPC 配接器](/zh-Hant/reference/rpc)
- [Gateway 手冊](/zh-Hant/gateway)
- [節點（iOS 和 Android）](/zh-Hant/nodes)
- [Web 介面（控制 UI）](/zh-Hant/web)
- [探索與傳輸](/zh-Hant/gateway/discovery)
- [遠端存取](/zh-Hant/gateway/remote)

## 提供者與使用者體驗

- [WebChat](/zh-Hant/web/webchat)
- [控制 UI（瀏覽器）](/zh-Hant/web/control-ui)
- [Telegram](/zh-Hant/channels/telegram)
- [Discord](/zh-Hant/channels/discord)
- [Mattermost（外掛程式）](/zh-Hant/channels/mattermost)
- [BlueBubbles（iMessage）](/zh-Hant/channels/bluebubbles)
- [iMessage（舊版）](/zh-Hant/channels/imessage)
- [群組](/zh-Hant/channels/groups)
- [WhatsApp 群組訊息](/zh-Hant/channels/group-messages)
- [媒體圖片](/zh-Hant/nodes/images)
- [媒體音訊](/zh-Hant/nodes/audio)

## 配套應用程式

- [macOS 應用程式](/zh-Hant/platforms/macos)
- [iOS 應用程式](/zh-Hant/platforms/ios)
- [Android 應用程式](/zh-Hant/platforms/android)
- [Windows (WSL2)](/zh-Hant/platforms/windows)
- [Linux 應用程式](/zh-Hant/platforms/linux)

## 操作與安全

- [工作階段](/zh-Hant/concepts/session)
- [Cron 排程工作](/zh-Hant/automation/cron-jobs)
- [Webhooks](/zh-Hant/automation/webhook)
- [Gmail hooks (Pub/Sub)](/zh-Hant/automation/gmail-pubsub)
- [安全性](/zh-Hant/gateway/security)
- [疑難排解](/zh-Hant/gateway/troubleshooting)

import footerZhHant from "/components/footer/zh-Hant.mdx";

<footerZhHant />
