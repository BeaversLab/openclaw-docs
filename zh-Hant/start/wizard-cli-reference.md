...
<Note>
  如果未偵測到 GUI，精靈會列印控制 UI 的 SSH 連接埠轉發指令，而不是開啟瀏覽器。如果控制 UI
  資產遺失，精靈會嘗試建置它們；備用方案是 `pnpm ui:build`（自動安裝 UI 依賴項）。
</Note>
...
<Note>
  - 如果 gateway 僅限回環，請使用 SSH 通道或 tailnet。 - 探索提示： - macOS：Bonjour (`dns-sd`) -
...
<Note>
  部分通道是以外掛程式形式提供。在設定期間選取時，精靈會提示在通道設定前 安裝外掛程式（npm
  或本機路徑）。
</Note>
