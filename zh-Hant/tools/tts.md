...
OpenClaw 可以使用 ElevenLabs、Microsoft 或 OpenAI 將外部回覆轉換為音訊。
只要 OpenClaw 能發送音訊的地方都能運作；Telegram 會收到圓形的語音訊息氣泡。
...
並非所有選項都受到該服務支援。使用 `edge` 的舊版設定和指令輸入
仍然有效，並會正規化為 `microsoft`。

由於此途徑是一個沒有發布 SLA 或配額的公共網路服務，
...
如果設定了多個提供者，會先使用選取的提供者，其他的則作為備援選項。
...
否。自動 TTS 預設為 **關閉**。請在設定中透過
`messages.tts.auto` 啟用，或每次對話使用 `/tts always`（別名：`/tts on`）。

一旦開啟 TTS，Microsoft 語音**預設**為啟用，並且當沒有 OpenAI 或 ElevenLabs API 金鑰可用時會自動使用。

## 設定

TTS 設定位於 `messages.tts` 下的 `openclaw.json`。
...
- 如果 `provider` **未設定**，OpenClaw 偏好 `openai`（如果有金鑰），然後是 `elevenlabs`（如果有金鑰），
  否則為 `microsoft`。
...
- `maxTextLength`：TTS 輸入的硬上限（字元）。若超出則 `/tts audio` 會失敗。
- `timeoutMs`：請求逾時（毫秒）。
...
- `openai.baseUrl`：覆寫 OpenAI TTS 端點。
  ...
  - 非預設值將被視為相容 OpenAI 的 TTS 端點，因此接受自訂模型與聲音名稱。
- `elevenlabs.voiceSettings`：
  - `stability`、`similarityBoost`、`style`：`0..1`
  - `useSpeakerBoost`：`true|false`
  - `speed`：`0.5..2.0`（1.0 = 正常）
- `elevenlabs.applyTextNormalization`：`auto|on|off`
- `elevenlabs.languageCode`：兩字母 ISO 639-1（例如 `en`、`de`）
...
- `microsoft.outputFormat`：Microsoft 輸出格式（例如 `audio-24khz-48kbitrate-mono-mp3`）。
  - 請參閱 Microsoft Speech 輸出格式以取得有效值；並非所有格式都受內建的 Edge 傳輸支援。
- `microsoft.rate` / `microsoft.pitch` / `microsoft.volume`：百分比字串（例如 `+10%`、`-5%`）。
- `microsoft.saveSubtitles`：在音訊檔案旁寫入 JSON 字幕。
- `microsoft.proxy`：Microsoft 語音要求的代理伺服器 URL。
- `microsoft.timeoutMs`：要求逾時覆寫（毫秒）。
...
啟用後，模型可以發出 `[[tts:...]]` 指令以覆寫單一回覆的語音，
以及選用的 `[[tts:text]]...[[/tts:text]]` 區塊，用於
提供僅應出現在音訊中的表情標籤（笑聲、歌唱提示等）。
...
- **Telegram**：Opus 語音訊息（來自 ElevenLabs 的 `opus_48000_64`，來自 OpenAI 的 `opus`）。
  - 48kHz / 64kbps 是語音訊息的良好取捨，並且是圓形氣泡所需的格式。
...
- **Microsoft**：使用 `microsoft.outputFormat`（預設 `audio-24khz-48kbitrate-mono-mp3`）。
  ...
  - Telegram `sendVoice` 接受 OGG/MP3/M4A；如果您需要保證的
    Opus 語音訊息，請使用 OpenAI/ElevenLabs。 citeturn1search1
  - 如果設定的 Microsoft 輸出格式失敗，OpenClaw 會以 MP3 重試。

OpenAI/ElevenLabs 格式是固定的；Telegram 預期語音訊息使用者體驗使用 Opus。
...
- 跳過非常短的回覆（< 10 個字元）。
- 當使用 `agents.defaults.model.primary`（或 `summaryModel`）啟用時，摘要長回覆。
- 將產生的音訊附加到回覆。

如果回覆超過 `maxLength` 且摘要功能已關閉（或沒有摘要模型的
API 金鑰），將跳過
音訊並發送一般文字回覆。
...
只有一個指令：`/tts`。
請參閱 [斜線指令](/zh-Hant/tools/slash-commands) 以了解啟用細節。

Discord 註記：`/tts` 是 Discord 的內建指令，因此 OpenClaw 在此將
...
與 Telegram 相容時，該工具會包含 `[[audio_as_voice]]`，讓
Telegram 傳送語音訊息氣泡。
