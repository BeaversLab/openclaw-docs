...
OpenClaw 可以使用 ElevenLabs、Microsoft 或 OpenAI 將外傳回覆轉換為音訊。
只要 OpenClaw 能發送音訊的地方，它都能運作；在 Telegram 上會收到圓形的語音訊息氣泡。

## 支援的服務

- **ElevenLabs**（主要或備援提供者）
- **Microsoft**（主要或備援提供者；目前的內建實作使用 `node-edge-tts`，未提供 API 金鑰時的預設選項）
- **OpenAI**（主要或備援提供者；亦用於摘要）

### Microsoft 語音說明

內建的 Microsoft 語音提供者目前透過 `node-edge-tts` 函式庫使用 Microsoft Edge 的線上神經 TTS 服務。這是一個託管服務（非本機），使用 Microsoft 端點，且不需要 API 金鑰。
`node-edge-tts` 公開了語音設定選項和輸出格式，但並非所有選項都受到服務支援。使用 `edge` 的舊版設定和指令輸入仍然有效，並會正規化為 `microsoft`。

由於此途徑是沒有公開 SLA 或配額的公共網路服務，請將其視為盡力而為。如果您需要保證的限制與支援，請使用 OpenAI 或 ElevenLabs。
...
Microsoft 語音**不**需要 API 金鑰。如果找不到 API 金鑰，OpenClaw 預設會使用 Microsoft（除非透過 `messages.tts.microsoft.enabled=false` 或 `messages.tts.edge.enabled=false` 停用）。

如果設定了多個提供者，會優先使用選取的提供者，其他的則作為備援選項。
自動摘要使用設定的 `summaryModel`（或 `agents.defaults.model.primary`），因此如果您啟用摘要，該提供者也必須通過驗證。
...
`messages.tts.auto` 啟用它，或每個會話使用 `/tts always`（別名：`/tts on`）。

一旦 TTS 開啟，Microsoft 語音**會**預設啟用，並且當沒有 OpenAI 或 ElevenLabs API 金鑰可用時會自動使用。

## 設定

TTS 設定位於 `openclaw.json` 中的 `messages.tts` 下方。
...
- `auto`：自動 TTS 模式（`off`、`always`、`inbound`、`tagged`）。
  - `inbound` 僅在收到傳入語音訊息後發送音頻。
  - `tagged` 僅當回覆包含 `[[tts]]` 標籤時才發送音頻。
- `enabled`：舊版切換開關（doctor 會將其遷移至 `auto`）。
- `mode`：`"final"`（預設）或 `"all"`（包含工具/區塊回覆）。
- `provider`：語音提供者 ID，例如 `"elevenlabs"`、`"microsoft"` 或 `"openai"`（後備為自動）。
- 如果 **未設定** `provider`，OpenClaw 會偏好 `openai`（如果有金鑰），然後是 `elevenlabs`（如果有金鑰），
  否則為 `microsoft`。
- 舊版 `provider: "edge"` 仍然有效，並且會被標準化為 `microsoft`。
...
- `apiKey` 的值會退回到環境變數（`ELEVENLABS_API_KEY`/`XI_API_KEY`、`OPENAI_API_KEY`）。
...
- `elevenlabs.languageCode`：2 字母 ISO 639-1 代碼（例如 `en`、`de`）
...
- `microsoft.outputFormat`：Microsoft 輸出格式（例如 `audio-24khz-48kbitrate-mono-mp3`）。
  - 請參閱 Microsoft Speech 輸出格式以取得有效值；並非所有格式都受內建的 Edge 支援傳輸所支援。
...
啟用後，模型可以發出 `[[tts:...]]` 指令來覆寫單一回覆的語音，以及可選的 `[[tts:text]]...[[/tts:text]]` 區塊，用於提供僅應出現在音訊中的表達式標籤（笑聲、歌唱提示等）。
...
斜線指令會將本地覆蓋項寫入 `prefsPath`（預設值為：
`~/.openclaw/settings/tts.json`，可使用 `OPENCLAW_TTS_PREFS` 或
`messages.tts.prefsPath` 覆蓋）。
...
- **Microsoft**：使用 `microsoft.outputFormat`（預設 `audio-24khz-48kbitrate-mono-mp3`）。
  - 隨附的傳輸層接受 `outputFormat`，但並非所有服務都提供所有格式。
  - 輸出格式值遵循 Microsoft Speech 輸出格式（包括 Ogg/WebM Opus）。
  - Telegram `sendVoice` 接受 OGG/MP3/M4A；如果您需要
    有保證的 Opus 語音訊息，請使用 OpenAI/ElevenLabs。 citeturn1search1
  - 如果設定的 Microsoft 輸出格式失敗，OpenClaw 會以 MP3 重試。

OpenAI/ElevenLabs 格式是固定的；Telegram 預期語音訊息 UX 使用 Opus。
...
- 跳過非常短的回覆（< 10 個字元）。
- 當使用 `agents.defaults.model.primary`（或 `summaryModel`）啟用時，將摘要長回覆。
- 將產生的音訊附加到回覆。

如果回覆超過 `maxLength` 且摘要關閉（或摘要模型沒有 API 金鑰），將跳過音訊
並發送正常文字回覆。
...
只有一個指令：`/tts`。
請參閱 [斜線指令](/zh-Hant/tools/slash-commands) 以了解啟用詳情。

Discord 註記：`/tts` 是 Discord 的內建指令，因此 OpenClaw 在那裡將
...
`tts` 工具會將文字轉換為語音並傳回 `MEDIA:` 路徑。當結果
與 Telegram 相容時，該工具會包含 `[[audio_as_voice]]`，以便
Telegram 發送語音訊息氣泡。
