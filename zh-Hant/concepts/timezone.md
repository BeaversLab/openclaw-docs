...
設定 `agents.defaults.userTimezone` 以告知模型使用者的本地時區。若未設定，OpenClaw 會在執行階段解析**主機時區**（無需寫入設定）。
...
- `Current Date & Time` 區段，包含本地時間和時區
- `Time format: 12-hour` 或 `24-hour`

您可以使用 `agents.defaults.timeFormat` (`auto` | `12` | `24`) 來控制提示詞格式。
