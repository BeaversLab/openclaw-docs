...
- **專用硬體**（Mac mini 或 Linux 主機）如果您想要完全控制和用於瀏覽器自動化的**住宅 IP**。許多網站會阻擋資料中心 IP，因此本機瀏覽通常效果更好。
...
1. 選取語言和地區
2. 跳過 Apple ID（如果您之後想要使用 iMessage，請登入）
...
依照引導提示設定您的模型供應商（Anthropic、OpenAI 等）。
...
若要真正全天候運作，請考慮專屬的 Mac mini 或小型 VPS。請參閱 [VPS hosting](/zh-Hant/vps)。
...
| 問題                 | 解決方案                                                         |
| -------------------- | ---------------------------------------------------------------- |
| 無法 SSH 連入 VM     | 檢查 VM 的「系統設定」中是否已啟用「遠端登入」                   |
...
- [Lume CLI 參考資料](https://cua.ai/docs/lume/reference/cli-reference)
- [無人值守 VM 設定](https://cua.ai/docs/lume/guide/fundamentals/unattended-setup) (進階)
- [Docker 沙盒](/zh-Hant/install/docker) (替代隔離方法)
