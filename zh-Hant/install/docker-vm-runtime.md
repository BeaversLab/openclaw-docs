---
summary: "長期執行 OpenClaw Gateway 主機的共享 Docker VM 執行時步驟"
read_when:
...
如果在 `pnpm install --frozen-lockfile` 期間建構失敗並出現 `Killed` 或 `exit code 137`，表示 VM 記憶體不足。
重試前請使用更大的機器類別。
...
所有長期存留的狀態都必須在重新啟動、重新建構和重新開機後保留。
...
| 代理工作區        | `/home/node/.openclaw/workspace/` | 主機磁碟區掛載    | 程式碼與代理產出物          |
