---
summary: "在 GCP Compute Engine VM (Docker) 上 24/7 運行 OpenClaw Gateway，並具備持久狀態"
read_when:
...
使用 Docker 在 GCP Compute Engine VM 上運行持續性的 OpenClaw Gateway，具有持久狀態、內建二進位檔案和安全的重啟行為。

如果您想要「以約每月 5-12 美元的代價實現 OpenClaw 24/7 運行」，這是在 Google Cloud 上可靠的設定方案。
價格會因機器類型和區域而異；請選擇符合您工作負載的最小 VM，如果遇到 OOM (記憶體不足) 則進行擴充。
...
<Steps>
  <Step title="安裝 gcloud CLI (或使用 Console)">
    ...
    **選項 B：Cloud Console**
    ...
    所有步驟皆可透過位於 [https://console.cloud.google.com](https://console.cloud.google.com) 的網頁 UI 完成
  ...
  <Step title="建立 VM">
    **機器類型：**
    ...
    | --------- | ------------------------ | ------------------ | -------------------------------------------- |
    | e2-medium | 2 vCPU, 4GB RAM          | ~$25/mo            | 對於本機 Docker 建置最可靠                    |
  ...
  <Step title="SSH 進入 VM">
    ...
    **Console：**
    ...
    在 Compute Engine 控制台中，點擊您 VM 旁邊的「SSH」按鈕。
  ...
  <Step title="Docker Compose 設定">
    ...
    `--allow-unconfigured` 僅為了便於引導（bootstrap），它不能取代適當的 gateway 設定。仍須設定驗證（`gateway.auth.token` 或密碼）並對您的部署使用安全的綁定設定。
  ...
  <Step title="共用的 Docker VM 執行時步驟">
    ...
    - [將必要的二進位檔打包至映像檔中](/zh-Hant/install/docker-vm-runtime#bake-required-binaries-into-the-image)
  ...
  <Step title="GCP 特定的啟動說明">
    ...
    當綁定到區域網路 (`OPENCLAW_GATEWAY_BIND=lan`) 時，請在繼續之前設定信任的瀏覽器來源：

    ```bash
    docker compose run --rm openclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```

    如果您變更了閘道埠，請將 `18789` 取代為您設定的埠。
...
在建立 VM 後，SSH 金鑰傳播可能需要 1-2 分鐘的時間。請稍後再試。
...
確保您的帳戶具有所需的 IAM 權限 (Compute OS Login 或 Compute OS Admin Login)。
...
2. 授與 Compute Instance Admin 角色 (或更狹隘的自訂角色)：
...
參閱 [https://cloud.google.com/iam/docs/understanding-roles](https://cloud.google.com/iam/docs/understanding-roles) 以了解 IAM 角色詳細資訊。
