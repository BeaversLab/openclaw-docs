...
<Note>
  如果安裝成功但在新終端機中找不到 `openclaw`，請參閱 [Node.js
...
<Steps>
  ...
  <Step title="確保預設使用 Node.js 24">
    ...
    設定腳本)。為了相容性，OpenClaw 目前仍支援 Node 22 LTS，即 `22.16+`。
  </Step>
  <Step title="確保 Git">如果缺少 Git 則進行安裝。</Step>
  <Step title="安裝 OpenClaw">
    - `npm` 方法 (預設)：全域 npm 安裝 - `git` 方法：複製/更新儲存庫，使用 pnpm
    安裝依賴，建構，然後在 `~/.local/bin/openclaw` 安裝包裝程式
  </Step>
  <Step title="安裝後任務">
    - 在升級和 git 安裝時運行 `openclaw doctor --non-interactive`（盡最大努力） -
    在適當時嘗試加入（TTY 可用、未停用加入程序、且 bootstrap/config 檢查通過） - 預設
...
<Steps>
  ...
  <Step title="預設確保 Node.js 24">
    如果缺失，嘗試透過 winget、接著 Chocolatey，然後 Scoop 進行安裝。為了相容性，Node 22 LTS，目前
    `22.16+`，仍然受支援。
  </Step>
  <Step title="安裝 OpenClaw">
    - `npm` 方法 (預設)：使用選定的 `-Tag` 進行全域 npm 安裝 - `git` 方法：複製/更新 repo，使用 pnpm
    安裝/建置，並在 `%USERPROFILE%\.local\bin\openclaw.cmd` 安裝 wrapper
  </Step>
  <Step title="安裝後任務">
    盡可能將所需的 bin 目錄加入使用者的 PATH，然後在升級和 git 安裝時執行 `openclaw doctor
...
<Note>若使用 `-InstallMethod git` 但缺少 Git，腳本將會結束並列印 Git for Windows 連結。</Note>
...
<Accordion title="為什麼在 Linux 上 npm 會遇到 EACCES？">
  某些 Linux 設定將 npm 全域前綴指向 root 擁有的路徑。`install.sh` 可以將前綴切換到 `~/.npm-global`
...
<Accordion title="Windows：「openclaw is not recognized」">
  執行 `npm config get prefix` 並將該目錄新增到您的使用者 PATH（在 Windows 上不需要 `\bin`
