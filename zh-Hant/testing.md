...
- 完整閘道檢查（推送前預期執行）：`pnpm build && pnpm check && pnpm test`
...
當除錯真實的供應商/模型時（需要真實憑證）：

- Live 測試組（模型 + 閘道工具/映像探測）：`pnpm test:live`

提示：當您只需要一個失敗案例時，建議透過下方描述的允許清單環境變數來縮小 Live 測試範圍。

## 測試組（什麼在哪裡執行）

可以將這些測試組視為「現實感逐層遞增」（且不穩定性/成本也隨之遞增）：
...
- 範圍：
  - 多實例閘道端對端行為
  - WebSocket/HTTP 介面、節點配對和更繁重的網路操作
- 預期：
  - 在 CI 中執行（於管道中啟用時）
  - 不需要真實金鑰
  - 比單元測試有更多移動部件（可能較慢）
...
- 範圍：
  - 「這個提供商/模型在真實憑證下*今天*真的能運作嗎？」
  - 捕捉提供商格式變更、工具呼叫怪癖、驗證問題以及速率限制行為
- 預期：
  - 設計上不保證 CI 穩定（涉及真實網路、真實的提供商政策、配額與中斷）
  - 需要花費金錢 / 會消耗速率限制
  - 建議執行縮小範圍的子集，而不是「全部」
  - Live 執行會 source `~/.profile` 以取得缺少的 API 金鑰
...
- 編輯邏輯/測試時：執行 `pnpm test`（如果您有大幅變更，則加上 `pnpm test:coverage`）
- 涉及閘道網路功能 / WS 協定 / 配對時：加上 `pnpm test:e2e`
- 除錯「我的機器人掛了」/ 特定提供商失敗 / 工具呼叫時：執行縮小範圍的 `pnpm test:live`

## Live：模型冒煙測試（設定檔金鑰）

Live 測試分為兩層，以便我們能隔離失敗原因：

- 「直接模型」告訴我們該提供商/模型是否能使用給定的金鑰進行回答。
- 「閘道冒煙測試」告訴我們完整的閘道+代理程式管線是否對該模型正常運作（sessions、history、tools、sandbox policy 等）。
...
- 目標：
  - 列舉已探索的模型
  - 使用 `getApiKeyForModel` 來選取您擁有憑證的模型
  - 對每個模型執行一個小型完成請求（並在需要時執行特定的迴歸測試）
- 如何啟用：
  - `pnpm test:live`（若直接呼叫 Vitest 則使用 `OPENCLAW_LIVE_TEST=1`）
- 設定 `OPENCLAW_LIVE_MODELS=modern`（或 `all`，即 modern 的別名）以實際執行此套件；否則它會跳過，讓 `pnpm test:live` 專注於閘道冒煙測試
- 如何選取模型：
  ...
  - 或 `OPENCLAW_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-5,..."`（逗號分隔的允許清單）
- 如何選取提供商：
  - `OPENCLAW_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"`（逗號分隔的允許清單）
- 金鑰來源：
  - 預設值：profile store 和環境變數後備
  - 設定 `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` 以強制僅使用 **profile store**
- 存在原因：
  - 將「提供者 API 故障 / 金鑰無效」與「Gateway agent pipeline 故障」區分開來
  - 包含小型、獨立的回歸測試（範例：OpenAI Responses/Codex Responses reasoning replay + tool-call flows）

### 第 2 層：Gateway + dev agent smoke（這是「@openclaw」實際上的操作）

- 測試：`src/gateway/gateway-models.profiles.live.test.ts`
- 目標：
  - 啟動一個程序內的 gateway
  - 建立/修補 `agent:dev:*` session（每次執行可覆寫模型）
  - 迭代帶有金鑰的模型並斷言：
    - 「有意義」的回應（無工具）
    - 真實的工具呼叫正常運作（讀取探測）
    - 選用的額外工具探測（exec+read probe）
    - OpenAI 回歸路徑（tool-call-only → follow-up）保持運作正常
- 探測詳情（讓您可以快速解釋失敗原因）：
  - `read` probe：測試會在工作區寫入一個 nonce 檔案，並要求 agent `read` 該檔案並將 nonce 回傳。
  - `exec+read` probe：測試會要求 agent 將 nonce `exec`-寫入暫存檔，然後將其 `read` 回來。
  - image probe：測試會附加一個生成的 PNG（貓 + 隨機代碼），並預期模型會回傳 `cat <CODE>`。
...
- 如何選取模型：
  - 預設值：現代白名單（Opus/Sonnet/Haiku 4.5、GPT-5.x + Codex、Gemini 3、GLM 4.7、MiniMax M2.1、Grok 4）
  - `OPENCLAW_LIVE_GATEWAY_MODELS=all` 是現代白名單的別名
  - 或是設定 `OPENCLAW_LIVE_GATEWAY_MODELS="provider/model"`（或逗號分隔清單）來縮小範圍
- 如何選取提供者（避免「全部使用 OpenRouter」）：
...
- 目標：使用本機 CLI 後端驗證 Gateway + agent 管線，而不需變動您的預設配置。
- 啟用：
  - `pnpm test:live` （如果直接呼叫 Vitest 則使用 `OPENCLAW_LIVE_TEST=1` ）
...
- 覆寫（選用）：
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` 以傳送真實的圖片附件（路徑會被注入到 prompt 中）。
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` 以將圖片檔案路徑作為 CLI 引數傳遞，而非透過 prompt 注入。
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` （或 `"list"` ）用於在設定 `IMAGE_ARG` 時控制圖片引數的傳遞方式。
  - `OPENCLAW_LIVE_CLI_BACKEND_RESUME_PROBE=1` 以傳送第二輪對話並驗證恢復流程。
- `OPENCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` 以保持 Claude Code CLI MCP 設定啟用（預設會使用暫時的空檔案停用 MCP 設定）。
...
沒有固定的「CI 模型清單」（live 是選用的），但這些是建議在擁有金鑰的開發機上定期涵蓋的**推薦**模型。
...
- Mistral: `mistral/`…（選擇一個你已啟用的支援“工具”的模型）
- Cerebras: `cerebras/`…（如果你有存取權限）
- LM Studio: `lmstudio/`…（本機；工具調用取決於 API 模式）
...
- OpenRouter: `openrouter/...` (數百種模型；使用 `openclaw models scan` 尋找支援工具+圖片的候選)
- OpenCode Zen: `opencode/...` (透過 `OPENCODE_API_KEY` / `OPENCODE_ZEN_API_KEY` 進行驗證)

更多你可以包含在 live matrix 中的提供商（如果你有憑證/設定）：
...
- 透過 `models.providers` (自訂端點)：`minimax` (雲端/API)，加上任何 OpenAI/Anthropic 相容的代理伺服器（LM Studio, vLLM, LiteLLM 等）

提示：不要嘗試在文件中硬碼「所有模型」。權威清單是 `discoverModels(...)` 在您的機器上傳回的內容，加上任何可用的金鑰。

## 憑證（切勿提交）

Live 測試以與 CLI 相同的方式探索憑證。實際影響：

- 如果 CLI 運作正常，Live 測試應該能找到相同的金鑰。
- 如果 Live 測試顯示「no creds」，請按照除錯 `openclaw models list` / 模型選擇的方式進行除錯。

- Profile store：`~/.openclaw/credentials/`（首選；這是測試中「profile keys」的含義）
...
這些會在 repo Docker 映像檔內執行 `pnpm test:live`，掛載您的本地設定目錄和工作區（並在掛載時載入 `~/.profile`）：
...
- 外掛（自訂擴充功能載入 + registry smoke）：`pnpm test:docker:plugins`（腳本：`scripts/e2e/plugins-docker.sh`）
...
- 驗證連線配線和配置效果的端對端精靈流程（`src/gateway/gateway.wizard.e2e.test.ts`）。

技能方面目前仍缺少的部分（請參閱 [技能](/zh-Hant/tools/skills)）：

- **決策：**當提示詞中列出技能時，Agent 是否會選擇正確的技能（或避免不相關的技能）？
- **合規性：**Agent 在使用前是否會閱讀 `SKILL.md` 並遵循必要的步驟/參數？
- **工作流程合約：**斷言工具順序、對話歷史傳遞和沙箱邊界的多輪場景。

未來的評估應首先保持確定性：

- 使用模擬供應商的情境執行器，以斷言工具呼叫 + 順序、技能檔案讀取和連線配線。
- 一套專注於技能的小型情境測試（使用 vs 避免使用、閘控、提示詞注入）。
- 僅在建立 CI 安全測試套件之後，才進行選用的即時評估（自願加入、環境變數控制）。

## 新增回歸測試（指導原則）

當您修復在即時環境中發現的供應商/模型問題時：

- 如果可能，請新增 CI 安全的回歸測試（模擬/存根供應商，或擷取確切的請求形狀轉換）
- 如果本質上僅限於即時環境（速率限制、驗證政策），請保持即時測試範圍縮小，並透過環境變數設為自願加入
- 優先針對捕捉該錯誤的最小層級：
  - 供應商請求轉換/重放錯誤 → 直接模型測試
  - gateway 對話/歷史/工具管道錯誤 → gateway 即時冒煙測試或 CI 安全 gateway 模擬測試
