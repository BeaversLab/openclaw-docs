...
- **專用硬體**（Mac mini 或 Linux 主機）如果您想要完全控制以及用於瀏覽器自動化的 **住宅 IP**。許多網站會封鎖資料中心 IP，因此本機瀏覽通常效果更好。
- **混合模式：** 將 Gateway 保留在便宜的 VPS 上，並在需要瀏覽器/UI 自動化時將您的 Mac 作為 **節點** 連線。請參閱 [Nodes](/zh-Hant/nodes) 和 [Gateway remote](/zh-Hant/gateway/remote)。

當您特別需要僅限 macOS 的功能（iMessage/BlueBubbles）或希望與日常使用的 Mac 嚴格隔離時，請使用 macOS VM。

## macOS VM 選項

### 在您 Apple Silicon Mac 上的本機 VM (Lume)

使用 [Lume](https://cua.ai/docs/lume) 在您現有的 Apple Silicon Mac 上於沙盒化的 macOS VM 中執行 OpenClaw。

這為您提供了：

- 完全隔離的 macOS 環境（您的主機系統保持乾淨）
...
如果您希望在雲端中使用 macOS，託管的 Mac 提供商也是可行的選擇：

- [MacStadium](https://www.macstadium.com/) (託管 Mac)
- 其他託管的 Mac 供應商也適用；請遵循其 VM + SSH 文件
...
將 `youruser` 替換為您建立的帳號，並將 IP 替換為您的 VM IP。
...
依照入體提示設定您的模型供應商（Anthropic、OpenAI 等）。
...
VM 將在背景執行。OpenClaw 的守護程式會保持閘道運作。
...
若要真正的全天候運作，請考慮專用的 Mac mini 或小型 VPS。請參閱 [VPS 託管](/zh-Hant/vps)。
...
- [Docker 沙箱隔離](/zh-Hant/install/docker) (另一種隔離方法)
