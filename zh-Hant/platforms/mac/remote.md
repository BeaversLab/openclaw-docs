...
此流程允許 macOS 應用程式充當在另一台主機（桌面/伺服器）上運行的 OpenClaw gateway 的完整遙控器。這是應用程式的 **Remote over SSH**（遠端運行）功能。所有功能——健康檢查、語音喚醒轉送和 Web Chat——都重複使用來自 _Settings → General_ 的相同遠端 SSH 設定。

## 模式

- **本地端（此 Mac）**：所有東西都在筆記型電腦上執行。不涉及 SSH。
- **Remote over SSH (預設值)**：OpenClaw 指令在遠端主機上執行。mac 應用程式會開啟一個 SSH 連線，其中包含 `-o BatchMode`，加上您選擇的身分/金鑰以及本地連接埠轉發。
...
- **SSH tunnel**（預設值）：使用 `ssh -N -L ...` 將 gateway 連接埠轉發到 localhost。Gateway 會將節點的 IP 視為 `127.0.0.1`，因為 tunnel 是 loopback。
- **直接（ws/wss）**：直接連線到閘道 URL。閘道會看到真實的用戶端 IP。

## 遠端主機的先決條件

1. 安裝 Node + pnpm 並建置/安裝 OpenClaw CLI (`pnpm install && pnpm build && pnpm link --global`)。
2. 確保 `openclaw` 位於非互動式 shell 的 PATH 中（如有需要，請建立符號連結到 `/usr/local/bin` 或 `/opt/homebrew/bin`）。
...
2. 在 **OpenClaw runs** 下，選擇 **Remote over SSH** 並設定：
   ...
   - **SSH target**：`user@host`（可選的 `:port`）。
     - 如果閘道位於相同的區域網路並廣播 Bonjour，請從探索清單中選擇它以自動填入此欄位。
   - **Gateway URL**（僅限 Direct）：`wss://gateway.example.ts.net`（或是 `ws://...` 用於本地/LAN）。
   - **身分檔案**（進階）：您的金鑰路徑。
   - **Project root**（進階）：用於指令的遠端簽出路徑。
   - **CLI path**（進階）：可執行的 `openclaw` 進入點/二進位檔的可選路徑（當廣播時會自動填入）。
3. 點擊 **Test remote**。成功表示遠端 `openclaw status --json` 運作正常。失敗通常意味著 PATH/CLI 問題；exit 127 表示在遠端找不到 CLI。
...
- 節點透過 `node.list` / `node.describe` 廣播其權限狀態，以便代理人知道有哪些可用功能。

## Security notes

- 偏好在遠端主機上使用 loopback 綁定並透過 SSH 或 Tailscale 連線。
- SSH tunneling 使用嚴格的主機金鑰檢查；請先信任主機金鑰，使其存在於 `~/.ssh/known_hosts` 中。
...
使用 `openclaw` 和 `node.invoke` 從腳本中為每個通知挑選聲音，例如：
