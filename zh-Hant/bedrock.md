...
read_when:
  - 您想要使用 Amazon Bedrock 模型與 OpenClaw
  - 您需要設定 AWS 憑證/區域以進行模型呼叫
title: "Amazon Bedrock"
---

# Amazon Bedrock

OpenClaw 可以透過 pi‑ai 的 **Bedrock Converse** 串流提供者來使用 **Amazon Bedrock** 模型。Bedrock 驗證使用 **AWS SDK 預設憑證鏈**，而非 API 金鑰。
...
如果偵測到 AWS 憑證，OpenClaw 可以自動探索支援 **串流** 和 **文字輸出** 的 Bedrock 模型。探索使用 `bedrock:ListFoundationModels` 並且會被快取 (預設：1 小時)。
...
在附加了 IAM 角色的 EC2 執行個體上執行 OpenClaw 時，AWS SDK 將會自動使用執行個體中繼資料服務 (IMDS) 進行驗證。然而，OpenClaw 的憑證偵測目前僅檢查環境變數，不檢查 IMDS 憑證。
...
- OpenClaw 會依下列順序使用憑證來源：`AWS_BEARER_TOKEN_BEDROCK`，然後是 `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`，接著是 `AWS_PROFILE`，最後是預設的 AWS SDK 鏈結。
- 推論支援取決於模型；請查看 Bedrock 模型卡以瞭解目前的功能。
- 如果您偏好受管理的金鑰流程，您也可以在 Bedrock 前方放置 OpenAI 相容的 Proxy，並將其設定為 OpenAI 提供者。
