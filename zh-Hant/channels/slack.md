...
<Tip>
對於操作/目錄讀取，使用者權杖可在設定時優先使用。對於寫入操作，Bot 權杖仍然優先；僅在 `userTokenReadOnly: false` 且無 Bot 權杖可用時，才允許使用者權杖寫入。
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Outbound text and files">
    ...
    - `channels.slack.chunkMode="newline"` 啟用以段落為主的分割
    - 檔案傳送使用 Slack 上傳 API，且可包含執行緒回覆 (`thread_ts`)
    - 已設定時，傳出媒體上限遵循 `channels.slack.mediaMaxMb`；否則通道傳送會使用媒體管線的 MIME 類型預設值
  </Accordion>
