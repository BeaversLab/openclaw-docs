...
<Note>
OpenClaw 建議盡可能在獨立號碼上執行 WhatsApp。(頻道詮釋資料與設定流程已針對此設定進行最佳化，但也支援個人號碼設定。)
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="外傳媒體行為">
    ...
    - 為了相容語音訊息，`audio/ogg` 會被重寫為 `audio/ogg; codecs=opus`
    - 可透過影片傳送上的 `gifPlayback: true` 支援動畫 GIF 播放
    - 傳送多媒體回覆載荷時，說明文字會套用至第一個媒體項目
