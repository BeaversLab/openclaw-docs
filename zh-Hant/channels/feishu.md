...
Feishu (Lark) 是企業用於訊息傳遞與協作的團隊聊天平台。此外掛程式透過平台的 WebSocket 事件訂閱將 OpenClaw 連接至 Feishu/Lark 機器人，因此無需公開 webhook URL 即可接收訊息。

---

## 隨附外掛程式

目前的 OpenClaw 版本隨附 Feishu，因此無需另行安裝外掛程式。

如果您使用的是未包含隨附 Feishu 的舊版本或自訂安裝版本，請手動安裝：
...
❗ **重要事項：** 請妥善保管 App Secret，不要公開。

![取得憑證](../images/feishu-step3-credentials.png)

### 4. 設定權限

在 **權限管理** 頁面上，點擊 **批量匯入權限** 並貼上：
...
如果您的租戶位於 Lark（國際版），請將域名設定為 `lark`（或完整的域名字串）。您可以在 `channels.feishu.domain` 或個別帳戶 (`channels.feishu.accounts.<id>.domain`) 中進行設定。
...
您可以透過兩個可選標誌來減少 Feishu API 的使用量：
...
- **飛書機器人頻道**：由閘道管理的飛書機器人
- **確定性路由**：回覆總是返回飛書
- **會話隔離**：私人訊息共享一個主會話；群組是隔離的
- **WebSocket 連線**：透過飛書 SDK 的長連線，無需公開 URL
...
除了允許群組本身外，該群組中的 **所有訊息** 都會根據發送者的 open_id 進行過濾：只有列在 `groups.<chat_id>.allowFrom` 中的用戶其訊息會被處理；來自成員的訊息將被忽略 (這是完整的發送者級別過濾，不僅適用於 /reset 或 /new 等控制命令)。
...
> 注意：Feishu 尚未支援原生指令選單，因此必須以文字形式傳送指令。
...
當傳出 API 未明確指定 `accountId` 時，`defaultAccount` 控制使用哪個 Feishu 帳號。

### 訊息限制

- `textChunkLimit`：傳出文字區塊大小 (預設：2000 字元)
- `mediaMaxMb`：媒體上傳/下載限制（預設：30MB）

### 串流回應

飛書支援透過互動卡片進行串流回應。啟用後，機器人會在生成文字時更新卡片。
...
設定 `streaming: false` 以在發送前等待完整回應。
...
飛書 ACP 是由文字指令驅動的。沒有原生的斜線指令選單，因此請在對話中直接使用 `/acp ...` 訊息。

#### 永久性 ACP 繫結

使用頂層類型化 ACP 繫結，將飛書私訊或主題討論串釘選到永久性 ACP 會話。
...
在飛書私訊或主題討論串中，您可以就地產生並繫結 ACP 會話：
...
- 繫結的私訊/主題中的後續訊息會直接路由到該 ACP 會話。
- v1 不支援一般的非主題群組聊天。

### 多代理路由

使用 `bindings` 將飛書私訊或群組路由到不同的代理。
...
| `channels.feishu.accounts.<id>.domain`            | 單一帳號 API 網域覆寫         | `feishu`         |
...
| `"allowlist"` | 僅限 `allowFrom` 中的使用者可以聊天               |
| `"open"`      | 允許所有使用者（需要在 allowFrom 中包含 `"*"`）   |
| `"disabled"`  | 停用私訊                                          |
...
- ✅ 影片/媒體
- ✅ 互動式卡片
- ⚠️ 豐富文字 (貼文樣式格式與卡片，而非任意的飛書編輯功能)

### 串列與回覆

- ✅ 內聯回覆
- ✅ 當飛書提供 `reply_in_thread` 時的主題串列回覆
- ✅ 回覆串列/主題訊息時，媒體回覆會保持對串列/主題的感知
