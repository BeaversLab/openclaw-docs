...
狀態：支援文字 + 私訊附件；頻道/群組檔案傳送需要 `sharePointSiteId` + Graph 權限（請參閱[在群組聊天中傳送檔案](#sending-files-in-group-chats)）。投票是透過 Adaptive Cards 發送的。
...
- UPN/顯示名稱是可變的；預設停用直接比對，僅在 `channels.msteams.dangerouslyAllowNameMatching: true` 時啟用。
- 當憑證允許時，精靈可以透過 Microsoft Graph 將名稱解析為 ID。

**群組存取**

- 預設值：`channels.msteams.groupPolicy = "allowlist"`（除非您新增 `groupAllowFrom`，否則封鎖）。使用 `channels.defaults.groupPolicy` 覆寫未設定時的預設值。
...
- 透過在 `channels.msteams.teams` 下列出團隊和頻道來界定群組/頻道回覆的範圍。
...
- 啟動時，OpenClaw 會將團隊/頻道和使用者允許清單名稱解析為 ID（當 Graph 權限允許時）
  並記錄對應關係；未解析的團隊/頻道名稱會保持輸入原樣，但除非啟用 `channels.msteams.dangerouslyAllowNameMatching: true`，否則預設會在路由時被忽略。
...
3. 按一下 **管理密碼** → 前往應用程式註冊
4. 在 **憑證與祕密** 下 → **新增用戶端祕密** → 複製 **值** → 這是您的 `appPassword`
5. 前往 **概觀** → 複製 **目錄 (租用戶) ID** → 這是您的 `tenantId`
...
除了手動建立資訊清單 ZIP 檔外，您也可以使用 [Teams 開發人員入口網站](https://dev.teams.microsoft.com/apps)：
...
1. **安裝 Microsoft Teams 外掛程式**
   ...
   - 從本機結帳安裝：`openclaw plugins install ./extensions/msteams`
...
- 會回退至 `messages.groupChat.historyLimit`。設定 `0` 以停用（預設值 50）。
- DM 歷程記錄可以用 `channels.msteams.dmHistoryLimit`（使用者輪次）來限制。每位使用者的覆寫設定：`channels.msteams.dms["<user_id>"].historyLimit`。
...
- `ChannelMessage.Read.Group` (應用程式) - 接收所有頻道訊息，無需 @提及
...
3. **將宣告與圖示重新壓縮** (`manifest.json`、`outline.png`、`color.png`)
4. 上傳新的 zip 檔案：
...
**重點：** RSC 用於即時監聽；Graph API 用於歷史存取。若要在離線時追上錯過的訊息，您需要搭配 `ChannelMessage.Read.All` 的 Graph API（需要管理員同意）。
...
**用於提及使用者的額外權限：** 對於對話中的使用者，User @mentions 可直接使用。然而，如果您想要動態搜尋並提及 **不在目前對話中** 的使用者，請新增 `User.Read.All`（應用程式）權限並授與管理員同意。
...
- `channels.msteams.dangerouslyAllowNameMatching`：緊急開關，用以重新啟用可變 UPN/顯示名稱比對及直接團隊/頻道名稱路由。
- `channels.msteams.textChunkLimit`：輸出文字區塊大小。
- `channels.msteams.chunkMode`：`length`（預設）或 `newline`，以便在長度區塊分割之前依空白行（段落邊界）分割。
...
- `channels.msteams.teams.<teamId>.channels.<conversationId>.toolsBySender`：每頻道每傳送者工具原則覆寫 (支援 `"*"` 萬用字元)。
...
**問題：** Teams API 並不公開頻道使用的 UI 樣式。如果您使用錯誤的 `replyStyle`：

- 在串列樣式頻道中使用 `thread` → 回覆會顯得尷尬地巢狀顯示
- 在貼文樣式頻道中使用 `top-level` → 回覆會顯示為獨立的頂層貼文，而非在串列中
...
如果沒有 Graph 權限，包含圖片的頻道訊息將只會以純文字形式接收 (Bot 無法存取圖片內容)。
預設情況下，OpenClaw 僅從 Microsoft/Teams 主機名下載媒體。使用 `channels.msteams.mediaAllowHosts` 覆蓋 (使用 `["*"]` 以允許任何主機)。
授權標頭僅附加到 `channels.msteams.mediaAuthAllowHosts` 中的主機 (預設為 Graph + Bot Framework 主機)。請保持此列表嚴格 (避免多租用戶尾碼)。
...
Bot 沒有個人的 OneDrive 磁碟機 (`/me/drive` Graph API 端點不適用於應用程式身分識別)。若要在群組聊天/頻道中傳送檔案，Bot 會上傳至 **SharePoint 網站** 並建立分享連結。
...
逐用戶分享更為安全，因為只有聊天參與者才能存取該檔案。如果缺少 `Chat.Read.All` 權限，機器人將退回到全組織分享。
...
- 投票結果會由閘道記錄在 `~/.openclaw/msteams-polls.json` 中。
- 閘道必須保持上線才能記錄投票。
...
請參閱 [Adaptive Cards 文件](https://adaptivecards.io/) 以了解卡片架構和範例。如需目標格式的詳細資訊，請參閱下方的 [Target formats](#target-formats)。
...
注意：如果沒有 `user:` 前綴，名稱預設為群組/團隊解析。當以顯示名稱為目標時，請務必使用 `user:`。
...
- 團隊 ID = `/team/` 之後的路徑片段 (URL 解碼後，例如 `19:Bk4j...@thread.tacv2`)
- 頻道 ID = `/channel/` 之後的路徑片段 (URL 解碼後)
- **請忽略** `groupId` 查詢參數
...
1. 驗證 `webApplicationInfo.id` 是否完全符合您的 Bot 應用程式 ID
