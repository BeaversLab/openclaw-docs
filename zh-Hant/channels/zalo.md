...
狀態：實驗性。支援訊息 (DM)。下方的 [功能](#capabilities) 區塊反映了目前 Marketplace-bot 的行為。
...
1. 安裝 Zalo 外掛：
   - 從原始碼檢出：`openclaw plugins install ./extensions/zalo`
...
Zalo 是一款聚焦於越南的訊息應用程式；其 Bot API 允許閘道執行機器人以進行 1對1 對話。
它非常適合用於您希望將路由確定性地導回 Zalo 的支援或通知情境。

本頁面反映了 OpenClaw 目前對於 **Zalo Bot Creator / Marketplace bots** 的行為。
**Zalo Official Account (OA) bots** 是不同的 Zalo 產品介面，其行為可能有所不同。

- 一個由閘道擁有的 Zalo Bot API 頻道。
- 確定性路由：回覆會傳回 Zalo；模型從不自選頻道。
- 訊息 (DM) 共用代理的主要工作階段。
- 下方的 [功能](#capabilities) 區塊顯示了目前 Marketplace-bot 的支援情況。
...
2. 建立一個新的機器人並設定其設定。
3. 複製完整的機器人權杖 (通常為 `numeric_id:secret`)。對於 Marketplace bots，可用的執行時期權杖可能會在建立後顯示於機器人的歡迎訊息中。
...
如果您稍後轉移到支援群組的 Zalo 機器人介面，您可以明確新增特定群組的配置，例如 `groupPolicy` 和 `groupAllowFrom`。如需目前的 Marketplace 機器人行為，請參閱 [功能](#capabilities)。

環境變數選項：`ZALO_BOT_TOKEN=...` (僅適用於預設帳戶)。

多帳戶支援：使用 `channels.zalo.accounts` 搭配各個帳戶的權杖以及可選的 `name`。
...
- 回覆一律會路由回同一個 Zalo 聊天。
- 預設使用長輪詢；透過 `channels.zalo.webhookUrl` 可使用 Webhook 模式。

## 限制

- 傳出文字會被分割為 2000 個字元的區塊 (Zalo API 限制)。
- 媒體下載/上傳受 `channels.zalo.mediaMaxMb` 限制 (預設值為 5)。
...
- 預設值：`channels.zalo.dmPolicy = "pairing"`。未知發送者會收到配對代碼；在核准之前訊息將被忽略 (代碼在 1 小時後過期)。
- 透過以下方式核准：
  - `openclaw pairing list zalo`
  - `openclaw pairing approve zalo <CODE>`
- 配對是預設的權杖交換方式。詳細資訊：[配對](/zh-Hant/channels/pairing)
- `channels.zalo.allowFrom` 接受數字使用者 ID (無法提供使用者名稱查詢)。
...
- 如果未設定 `groupAllowFrom`，Zalo 將回退到使用 `allowFrom` 進行發送者檢查。
- 執行時註記：如果完全缺少 `channels.zalo`，為了安全起見，執行時仍會回退到 `groupPolicy="allowlist"`。

群組策略值（當您的機器人介面提供群組存取權時）為：

- `groupPolicy: "disabled"` — 封鎖所有群組訊息。
- `groupPolicy: "open"` — 允許任何群組成員（提及限制）。
- `groupPolicy: "allowlist"` — 預設為失敗關閉 (fail-closed)；僅接受允許的發送者。
...
如需快速支援概覽，請參閱 [Capabilities](#capabilities)。以下註記在行為需要額外背景資訊時補充細節。

- **文字訊息**：完全支援，包含 2000 個字元的分段。
- **純文字 URL**：行為如同一般文字輸入。
- **連結預覽 / 豐富連結卡片**：請參閱 [Capabilities](#capabilities) 中的 Marketplace-bot 狀態；它們無法可靠地觸發回覆。
- **圖片訊息**：請參閱 [Capabilities](#capabilities) 中的 Marketplace-bot 狀態；接收圖片的處理並不可靠（僅有輸入指示器而沒有最終回覆）。
...
扁平的頂層金鑰（`channels.zalo.botToken`、`channels.zalo.dmPolicy` 和類似項目）是舊版單一帳號簡寫。對於新組態，建議使用 `channels.zalo.accounts.<id>.*`。這兩種格式在此都有記載，因為它們存在於架構中。
...
- `channels.zalo.allowFrom`: DM 允許清單 (使用者 IDs)。`open` 需要 `"*"`。精靈會詢問數字 IDs。
...
- `channels.zalo.groupAllowFrom`: 群組發送者允許清單 (使用者 IDs)。未設定時會退回至 `allowFrom`。
- `channels.zalo.mediaMaxMb`: 連入/連出媒體上限 (MB，預設 5)。
- `channels.zalo.webhookUrl`: 啟用 webhook 模式 (需要 HTTPS)。
- `channels.zalo.webhookSecret`: webhook 金鑰 (8-256 個字元)。
...
- `channels.zalo.accounts.<id>.tokenFile`: 各帳號一般權杖檔案。符號連結會被拒絕。
- `channels.zalo.accounts.<id>.name`: 顯示名稱。
- `channels.zalo.accounts.<id>.enabled`: 啟用/停用帳號。
- `channels.zalo.accounts.<id>.dmPolicy`: 各帳號 DM 政策。
- `channels.zalo.accounts.<id>.allowFrom`: 各帳號允許清單。
- `channels.zalo.accounts.<id>.groupPolicy`: 各帳號群組政策。存在於設定中；請參閱 [Capabilities](#capabilities) 和 [Access control (Groups)](#access-control-groups) 以了解目前的 Marketplace-bot 行為。
- `channels.zalo.accounts.<id>.groupAllowFrom`: 各帳號群組發送者允許清單。
- `channels.zalo.accounts.<id>.webhookUrl`: 各帳號 webhook URL。
- `channels.zalo.accounts.<id>.webhookSecret`: 各帳號 webhook 金鑰。
- `channels.zalo.accounts.<id>.webhookPath`: 各帳號 webhook 路徑。
- `channels.zalo.accounts.<id>.proxy`: 各帳號 Proxy URL。
