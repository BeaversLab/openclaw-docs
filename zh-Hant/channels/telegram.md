...
<Note>
  權杖解析順序會辨識帳號。實際上，配置值優先於環境變數後備，且 `TELEGRAM_BOT_TOKEN`
  僅適用於預設帳號。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="群組權限">
    ...
    管理員機器人會收到所有群組訊息，這對於始終啟用的群組行為很有用。
...
<Tabs>
  <Tab title="DM policy">
    ...
    `channels.telegram.allowFrom` 接受數值形式的 Telegram 用戶 ID。接受並標準化 `telegram:` / `tg:` 前綴。
    若 `dmPolicy: "allowlist"` 的 `allowFrom` 為空，則會封鎖所有直接訊息，且會被配置驗證拒絕。
    入職程序接受 `@username` 輸入並將其解析為數值 ID。
    如果您已升級且您的配置包含 `@username` 允許清單條目，請執行 `openclaw doctor --fix` 來解析它們 (盡力而為；需要 Telegram bot token)。
    如果您先前依賴 pairing-store 允許清單檔案，`openclaw doctor --fix` 可以在允許清單流程中將條目還原到 `channels.telegram.allowFrom` (例如當 `dmPolicy: "allowlist"` 尚無明確 ID 時)。

    對於單一擁有者的機器人，建議使用帶有明確數值 `allowFrom` ID 的 `dmPolicy: "allowlist"`，以讓存取政策在配置中保持持久 (而非依賴先前的配對核准)。
...
```
  ...
  <Tab title="群組原則與允許列表">
    ...
    `groupAllowFrom` 用於群組傳送者過濾。如果未設定，Telegram 會回退到 `allowFrom`。
    `groupAllowFrom` 項目應為數值的 Telegram 使用者 ID (`telegram:` / `tg:` 前綴會被正規化)。
    請勿將 Telegram 群組或超級群組聊天 ID 放在 `groupAllowFrom` 中。負數聊天 ID 應歸於 `channels.telegram.groups` 之下。
    非數值項目在傳送者授權時會被忽略。
    安全邊界 (`2026.2.25+`)：群組傳送者授權**不會**繼承 DM 配對儲存庫的核准。
    配對僅限 DM。對於群組，請設定 `groupAllowFrom` 或各群組/各主題的 `allowFrom`。
    執行時期備註：如果完全缺少 `channels.telegram`，執行時期會預設為 fail-closed `groupPolicy="allowlist"`，除非明確設定 `channels.defaults.groupPolicy`。
...
```
    ...
    <Warning>
      常見錯誤：`groupAllowFrom` 不是 Telegram 群組的允許列表。
...
- 私訊 (DM) 可以攜帶 `message_thread_id`；OpenClaw 使用具有執行緒感知能力的會話金鑰進行路由，並在回覆時保留執行緒 ID。
- 輪詢使用 grammY 執行器進行每個聊天/每個執行緒的排序。整體執行器接收端並行性使用 `agents.defaults.maxConcurrent`。
...
<AccordionGroup>
  <Accordion title="即時串流預覽 (訊息編輯)">
    ...
    - 私訊：OpenClaw 保留相同的預覽訊息並就地進行最終編輯 (無第二則訊息)
    - 群組/話題：OpenClaw 保留相同的預覽訊息並就地進行最終編輯 (無第二則訊息)

    對於複雜回覆 (例如媒體內容)，OpenClaw 會回退至正常的最終傳遞，然後清理預覽訊息。
  ...
  <Accordion title="格式與 HTML 後援">
    外寄文字使用 Telegram `parse_mode: "HTML"`。

    - 類 Markdown 的文字會轉譯為 Telegram 相容的 HTML。
    - 原始模型 HTML 會被轉義以減少 Telegram 解析錯誤。
...
```
    ...
    - 即使在 Telegram 選單中未顯示，外掛/技能指令在輸入時仍可運作

    如果停用原生指令，內建指令將會移除。若已設定，自訂/外掛指令仍可能註冊。

    常見設定失敗：

    - `setMyCommands failed` 搭配 `BOT_COMMANDS_TOO_MUCH` 表示 Telegram 選單在裁減後仍然溢位；請減少外掛/技能/自訂指令或停用 `channels.telegram.commands.native`。
...
```
  ...
  <Accordion title="適用於代理程式和自動化的 Telegram 訊息動作">
    ...
    執行時傳送使用使用中的組態/秘密快照 (啟動/重新載入)，因此動作路徑不會在每次傳送時執行臨時的 SecretRef 重新解析。
  ...
  <Accordion title="論壇主題與討論串行為">
    ...
    **每個主題的代理程式路由**：每個主題可以透過在主題設定中設定 `agentId` 來路由到不同的代理程式。這會賦予每個主題自己獨立的工作區、記憶體和階段。範例：
    ...
    每個主題隨後會有自己的階段金鑰：`agent:zu:telegram:group:-1001234567890:topic:3`

    **永續 ACP 主題綁定**：論壇主題可以透過頂層類型化的 ACP 綁定來釘選 ACP 駕駛艙階段：
    ...
    **從聊天產生綁定討論串的 ACP**：
    ...
    - `/acp spawn <agent> --thread here|auto` 可以將目前的 Telegram 主題綁定到新的 ACP 階段。
    - 後續的主題訊息會直接路由到綁定的 ACP 階段（無需 `/acp steer`）。
    ...
    - 具有 `message_thread_id` 的私人聊天會保留 DM 路由，但會使用具討論串感知的階段金鑰/回覆目標。
...
```
    ...
    貼圖（在可能的情況下）僅描述一次並進行快取，以減少重複的視覺呼叫。
...
```
  ...
  <Accordion title="Reaction notifications">
    Telegram 反應會以 `message_reaction` 更新的形式到達（與訊息 payload 分開）。
    ...
    - 反應事件仍遵守 Telegram 存取控制 (`dmPolicy`, `allowFrom`, `groupPolicy`, `groupAllowFrom`)；未授權的發送者會被捨棄。
    - Telegram 不會在反應更新中提供 thread ID。
      - 非論壇群組會路由到群組聊天會話
      - 論壇群組會路由到群組一般主題會話 (`:topic:1`)，而非確切的原始主題

    用於 polling/webhook 的 `allowed_updates` 會自動包含 `message_reaction`。
  ...
  <Accordion title="Ack reactions">
    ...
    - 代理身份表情符號備用方案 (`agents.list[].identity.emoji`，否則為 "👀")
...
```
  ...
  <Accordion title="限制、重試與 CLI 目標">
    ...
    - `channels.telegram.chunkMode="newline"` 優先在段落邊界（空白行）進行長度分割。
    - `channels.telegram.mediaMaxMb` (預設 100) 限制傳入與傳出的 Telegram 媒體大小。
    - `channels.telegram.timeoutSeconds` 覆寫 Telegram API 客戶端逾時設定（若未設定，則使用 grammY 預設值）。
    - 群組情境歷史記錄使用 `channels.telegram.historyLimit` 或 `messages.groupChat.historyLimit` (預設 50)；`0` 則停用。
    - DM 歷史記錄控制：
      - `channels.telegram.dmHistoryLimit`
      - `channels.telegram.dms["<user_id>"].historyLimit`
    - `channels.telegram.retry` 設定套用於 Telegram 發送輔助程式 (CLI/tools/actions)，以處理可恢復的傳出 API 錯誤。
...
```
    ...
    - `--buttons` 用於內聯鍵盤 (inline keyboards)，當 `channels.telegram.capabilities.inlineButtons` 允許時
    - `--force-document` 將傳出圖片與 GIF 以文件形式發送，而非壓縮照片或動態媒體上傳

    動作閘控：

    - `channels.telegram.actions.sendMessage=false` 停用傳出 Telegram 訊息，包括投票
    - `channels.telegram.actions.poll=false` 停用 Telegram 投票建立，但保留一般發送功能

  </Accordion>

  <Accordion title="Telegram 中的執行審批">
    Telegram 支援在審批者的私訊（DM）中進行執行審批，並可選擇在原始聊天或主題中張貼審批提示。
    ...
    審批者必須是 Telegram 的數位使用者 ID。當 `enabled` 為 false 或 `approvers` 為空時，Telegram 不會作為執行審批客戶端。審批請求將會退回至其他已配置的審批路由或執行審批備援政策。

    傳遞規則：

    - `target: "dm"` 僅將審批提示傳送至已配置的審批者私訊
    - `target: "channel"` 將提示傳送回原始 Telegram 聊天/主題
    - `target: "both"` 傳送至審批者私訊及原始聊天/主題

    只有已配置的審批者可以核准或拒絕。非審批者無法使用 `/approve` 且無法使用 Telegram 審批按鈕。

    頻道傳遞會在聊天中顯示指令文字，因此僅在受信任的群組/主題中啟用 `channel` 或 `both`。當提示落入論壇主題時，OpenClaw 會為審批提示和審核後的後續追蹤保留該主題。
...
<AccordionGroup>
  <Accordion title="機器人不回應非提及的群組訊息">
    ...
    - 如果設定為 `requireMention=false`，Telegram 隱私模式必須允許完全可見。
      - BotFather：`/setprivacy` -> Disable
      - 然後將機器人從群組移除並重新加入
    - 當配置期望非提及的群組訊息時，`openclaw channels status` 會發出警告。
  ...
  <Accordion title="指令部分運作或完全無法運作">
    ...
    - `setMyCommands failed` 並伴有 `BOT_COMMANDS_TOO_MUCH` 表示原生選單項目過多；請減少外掛/技能/自訂指令或停用原生選單
    - `setMyCommands failed` 並伴有網路/擷取錯誤，通常表示無法透過 DNS/HTTPS 連線至 `api.telegram.org`

  </Accordion>

  <Accordion title="輪詢或網路不穩定">

    - Node 22+ + 自訂 fetch/proxy 如果 AbortSignal 類型不相符，可能會觸發立即中止行為。
    - 部份主機會先將 `api.telegram.org` 解析為 IPv6；損壞的 IPv6 出網連線可能導致 Telegram API 間歇性故障。
    - 如果日誌包含 `TypeError: fetch failed` 或 `Network request for 'getUpdates' failed!`，OpenClaw 現在會將這些錯誤視為可復原的網路錯誤進行重試。
...
- `channels.telegram.tokenFile`：從常規檔案路徑讀取權杖。不支援符號連結 (symlinks)。
- `channels.telegram.dmPolicy`：`pairing | allowlist | open | disabled` (預設：配對)。
- `channels.telegram.allowFrom`：DM 白名單 (Telegram 使用者數字 ID)。`allowlist` 至少需要一個發送者 ID。`open` 需要 `"*"`。`openclaw doctor --fix` 可以將舊版 `@username` 項目解析為 ID，並可在白名單移轉流程中從配對儲存檔案中復原白名單項目。
- `channels.telegram.actions.poll`：啟用或停用建立 Telegram 投票 (預設：啟用；仍需 `sendMessage`)。
- `channels.telegram.defaultTo`：CLI `--deliver` 在未提供明確的 `--reply-to` 時使用的預設 Telegram 目標。
- `channels.telegram.groupPolicy`：`open | allowlist | disabled` (預設：allowlist)。
- `channels.telegram.groupAllowFrom`：群組傳送者允許名單 (數字 Telegram 使用者 ID)。`openclaw doctor --fix` 可將舊版 `@username` 項目解析為 ID。非數字項目在授權時會被忽略。群組授權不使用 DM 配對儲存後備 (`2026.2.25+`)。
- 多帳號優先順序：
  - 當配置了兩個或多個帳號 ID 時，請設定 `channels.telegram.defaultAccount` (或包含 `channels.telegram.accounts.default`) 以明確預設路由。
  - 如果兩者皆未設定，OpenClaw 將回退至第一個標準化的帳號 ID，且 `openclaw doctor` 會發出警告。
  ...
  - 命名帳號不繼承 `channels.telegram.accounts.default.allowFrom` / `groupAllowFrom`。
- `channels.telegram.groups`：各群組的預設值 + 允許名單 (使用 `"*"` 設定全域預設值)。
  ...
  - `channels.telegram.groups.<id>.allowFrom`：各群組傳送者允許名單覆寫。
  - `channels.telegram.groups.<id>.systemPrompt`：群組的額外系統提示詞。
  - `channels.telegram.groups.<id>.enabled`：當 `false` 時停用群組。
  - `channels.telegram.groups.<id>.topics.<threadId>.*`：各主題覆寫 (群組欄位 + 僅限主題的 `agentId`)。
  - `channels.telegram.groups.<id>.topics.<threadId>.agentId`：將此主題路由至特定代理 (覆寫群組層級和綁定路由)。
- `channels.telegram.groups.<id>.topics.<threadId>.groupPolicy`：groupPolicy (`open | allowlist | disabled`) 的各主題覆寫。
- `channels.telegram.groups.<id>.topics.<threadId>.requireMention`：針對每個主題的提及閘門覆寫。
- 頂層 `bindings[]`，包含 `type: "acp"` 與正則主題 ID `chatId:topic:topicId` 於 `match.peer.id` 中：持續性 ACP 主題綁定欄位（請參閱 [ACP Agents](/zh-Hant/tools/acp-agents#channel-specific-settings)）。
- `channels.telegram.direct.<id>.topics.<threadId>.agentId`：將 DM 主題路由到特定代理程式（行為與論壇主題相同）。
- `channels.telegram.execApprovals.enabled`：啟用 Telegram 作為此帳戶的基於聊天室的執行核准用戶端。
- `channels.telegram.execApprovals.approvers`：被允許核准或拒絕執行請求的 Telegram 使用者 ID。啟用執行核准時為必填。
- `channels.telegram.execApprovals.target`：`dm | channel | both`（預設值：`dm`）。`channel` 和 `both` 會在存在時保留來源 Telegram 主題。
- `channels.telegram.execApprovals.agentFilter`：轉發核准提示的選用代理程式 ID 篩選器。
- `channels.telegram.execApprovals.sessionFilter`：轉發核准提示的選用工作階段金鑰篩選器（子字串或正則表達式）。
- `channels.telegram.accounts.<account>.execApprovals`：針對帳戶的 Telegram 執行核准路由與核准者授權覆寫。
- `channels.telegram.capabilities.inlineButtons`：`off | dm | group | all | allowlist`（預設值：允許清單）。
- `channels.telegram.accounts.<account>.capabilities.inlineButtons`：針對帳戶的覆寫。
- `channels.telegram.commands.nativeSkills`：啟用/停用 Telegram 原生技能指令。
- `channels.telegram.replyToMode`：`off | first | all`（預設值：`off`）。
- `channels.telegram.textChunkLimit`：輸出區塊大小（字元）。
- `channels.telegram.chunkMode`：`length`（預設值）或 `newline` 以在長度分割前依空行（段落邊界）分割。
- `channels.telegram.linkPreview`：切換輸出訊息的連結預覽（預設值：true）。
- `channels.telegram.streaming`：`off | partial | block | progress`（即時串流預覽；預設值：`partial`；`progress` 對應至 `partial`；`block` 為舊版預覽模式相容性）。Telegram 預覽串流使用單一預覽訊息，並直接編輯該訊息。
- `channels.telegram.mediaMaxMb`：Telegram 入站/出站媒體大小上限（MB，預設值：100）。
- `channels.telegram.retry`：當 Telegram 發送輔助程式（CLI/tools/actions）遇到可復原的出站 API 錯誤時的重試原則（attempts、minDelayMs、maxDelayMs、jitter）。
...
- `channels.telegram.actions.reactions`：管控 Telegram 工具回應。
- `channels.telegram.actions.sendMessage`：管控 Telegram 工具訊息發送。
- `channels.telegram.actions.deleteMessage`：管控 Telegram 工具訊息刪除。
- `channels.telegram.actions.sticker`：管控 Telegram 貼圖動作 — 發送與搜尋（預設值：false）。
- `channels.telegram.reactionNotifications`：`off | own | all` — 控制哪些回應會觸發系統事件（若未設定則預設為 `own`）。
- `channels.telegram.reactionLevel`：`off | ack | minimal | extensive` — 控制代理的回應能力（若未設定則預設為 `minimal`）。

- [Configuration reference - Telegram](/zh-Hant/gateway/configuration-reference#telegram)

Telegram 特有的重要欄位：

- 啟動/驗證 (startup/auth)：`enabled`、`botToken`、`tokenFile`、`accounts.*`（`tokenFile` 必須指向一般檔案；不支援符號連結）
...
- 主串/回覆：`replyToMode`
- 串流：`streaming` (preview)、`blockStreaming`
...
- [配對 (Pairing)](/zh-Hant/channels/pairing)
- [頻道路由 (Channel routing)](/zh-Hant/channels/channel-routing)
- [多代理路由 (Multi-agent routing)](/zh-Hant/concepts/multi-agent)
- [疑難排解 (Troubleshooting)](/zh-Hant/channels/troubleshooting)
