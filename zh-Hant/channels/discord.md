...
<Note>
  Token 解析具有帳戶感知能力。Config token 值優先於 env 後備值。`DISCORD_BOT_TOKEN`
  僅用於預設帳戶。對於進階的傳出呼叫（訊息工具/頻道操作）， 該呼叫會使用明確的單次呼叫
  `token`。這適用於傳送和讀取/探測風格的 操作（例如
  read/search/fetch/thread/pins/permissions）。帳戶策略/重試設定
  仍然來自作用中執行時快照中選取的帳戶。
</Note>
