...
function translateFile(filePath) {
  ...
  // 提取所有 i18n:todo 块
  const todoRegex = /<!-- i18n:todo -->([\s\S]*?)<!-- \/i18n:todo -->/g;
  let match;
  const replacements = [];
  ...
  while ((match = todoRegex.exec(content)) !== null) {
    ...
    let translatedContent = innerContent;
    let contentModified = false;

    // 尝试翻译每个完整的段落
    for (const [english, chinese] of Object.entries(translations)) {
      // 精确匹配整个段落或句子
      if (innerContent === english || innerContent.startsWith(english + '\n') || innerContent.endsWith('\n' + english)) {
        translatedContent = innerContent.replace(english, chinese);
        contentModified = true;
        console.log(`  ✓ Translated: ${english.substring(0, 40)}...`);
        break;
      }
    }
    ...
    if (!contentModified) {
      for (const [english, chinese] of Object.entries(translations)) {
        if (innerContent.includes(english) && english.length > 20) { // 只匹配较长的短语
          translatedContent = innerContent.replace(english, chinese);
          contentModified = true;
          console.log(`  ✓ Partial match: ${english.substring(0, 40)}...`);
          break;
        }
      }
    }
  ...
  for (const replacement of replacements) {
    content = content.replace(replacement.original, replacement.translated);
  }

  if (modifiedCount > 0) {
    fs.writeFileSync(filePath, content, 'utf8');
    console.log(`✓ Translated ${modifiedCount} sections in ${path.basename(filePath)}`);
    return true;
  } else {
    console.log(`  No translations applied to ${path.basename(filePath)}`);
    return false;
  }
}
...
console.log('Starting smart batch translation...\n');
